...
local hasGraphicsSlider = true --(game:GetService("CoreGui").Version >= 5)
...
local function createGameSettingsMenu(baseZIndex, shield)
    ...
    virtualVersionLabel.Font = Enum.Font.Arial
    virtualVersionLabel.FontSize = Enum.FontSize.Size18
    virtualVersionLabel.Position = UDim2.new(0,32,0,100)
    ...
    for i = 1, #virtualVersionEnumItems do
        ...
        -- Check if this version is within the allowed range
        if enumItem.Value >= minVersion.Value and enumItem.Value <= maxVersion.Value then
            local displayName = enumItem.Name
            
            -- Add (Default) label to minimum version
            if enumItem == minVersion then
                displayName = displayName .. " (Default)"
            end
            
            videoNames[filteredIndex] = displayName
            videoNameToItem[displayName] = enumItem
    ...
    virtualVersionDropDown, updateVirtualVersionDropDownSelection, toggleVirtualVersionVisibility = RbxGui.CreateDropDownMenu(videoNames, 
        function(text) 
            Spawn(function()
                ...
                wait(0.2)
                UserSettings().GameSettings.VirtualVersion = videoNameToItem[text]
            end)
        end)
...
if UserSettings then
	...
	local createSettingsDialog = function()
		...
		if settingsButton == nil then
			...
			settingsButton.Image = "ayaasset://textures/ui/SettingsButton.png"
			...
			if currentMenuSelection == nil then
				game.GuiService:AddCenterDialog(shield, Enum.CenterDialogType.ModalDialog,
					...
					function()
						...
						if syncVirtualVersionSetting then
							syncVirtualVersionSetting()
						end
			...
			function()
				game.GuiService:AddCenterDialog(shield, Enum.CenterDialogType.ModalDialog,
					...
					function()
						...
						goToMenu(settingsFrame,"GameMainMenu","none",UDim2.new(0,525,0,430))
						shield.Visible = true
...
function()
	...
	goToMenu(settingsFrame,"GameSettingsMenu","right",UDim2.new(0,525,0,430))
	shield.Visible = true
	shield.Active = true
	settingsFrame.Parent:TweenPosition(UDim2.new(0.5, -262,0.5, -200),Enum.EasingDirection.InOut,Enum.EasingStyle.Sine,tweenTime,true)
	settingsFrame.Parent:TweenSize(UDim2.new(0,525,0,430),Enum.EasingDirection.InOut,Enum.EasingStyle.Sine,tweenTime,true)
