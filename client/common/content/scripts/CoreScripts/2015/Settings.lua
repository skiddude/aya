...
local updateVirtualVersionDropDownSelection = nil
local updateSmartCameraDropDownSelection = nil
local updateMovementDropDownSelection = nil
local tweenTime = 0.2

local mouseLockLookScreenUrl = "ayaasset://textures/2015/MouseLockLook.png"
local classicLookScreenUrl = "ayaasset://textures/2015/ClassicLook.png"
...
function forceGoToMenu(container,menuName, moveDirection,size,position)
    if type(menuName) ~= "string" then return end
    ...
    for i = 1, #containerChildren do
        if containerChildren[i].Name == menuName then
            ...
            selectedMenu = true
            if size and position then
                containerChildren[i].Size = size
                containerChildren[i].Position = position
            elseif size then
                containerChildren[i].Size = size
                containerChildren[i].Position = UDim2.new(0.5,-size.X.Offset/2,0.5,-size.Y.Offset/2)
            else
                containerChildren[i].Position = UDim2.new(0,0,0,0)
            end
        else
            if moveDirection == "left" then
                containerChildren[i].Position = UDim2.new(-1,-525,0,0)
            elseif moveDirection == "right" then
                containerChildren[i].Position = UDim2.new(1,525,0,0)
            elseif moveDirection == "up" then
                containerChildren[i].Position = UDim2.new(0,0,-1,-400)
            elseif moveDirection == "down" then
                containerChildren[i].Position = UDim2.new(0,0,1,400)
            end
            delay(tweenTime,function()
...
local function createHelpDialog(baseZIndex)
	...
	buttons[2].Function = function() 
		image.Image = "ayaasset://textures/2015/Settings/MoveButton.png"
	end 
	buttons[3] = {}
	buttons[3].Text = "Gear"
	buttons[3].Function = function() 
		image.Image = "ayaasset://textures/2015/Settings/GearButton.png"
	end
	buttons[4] = {}
	buttons[4].Text = "Zoom"
	buttons[4].Function = function() 	
		image.Image = "ayaasset://textures/2015/Settings/ZoomButton.png"
	end 
...
local function createGameMainMenu(baseZIndex, shield)
	...
	stopRecordButton.Image = "ayaasset://textures/ui/RecordStop.png"
...
local function createGameSettingsMenu(baseZIndex, shield)
	...
	----------------------------------------------------------------------------------------------------
	--  V I R T U A L   V E R S I O N   S E T T I N G S
	----------------------------------------------------------------------------------------------------
    ...
    local version = UserSettings().GameSettings.VirtualVersion
    ...
    local virtualVersionNames = {}
    local virtualVersionNameToItem = {}
    ...
    for i = 1, #virtualVersionEnumItems do
        ...
        if enumItem.Value >= minVersion.Value and enumItem.Value <= maxVersion.Value then
            local displayName = enumItem.Name
            
            virtualVersionNames[filteredIndex] = displayName
            virtualVersionNameToItem[displayName] = enumItem
            
            if version == enumItem then
                currentSelectionIndex = filteredIndex
            end
    ...
    local virtualVersionDropDown = nil
	virtualVersionDropDown, updateVirtualVersionDropDownSelection, toggleVirtualVersionVisibility = RbxGui.CreateDropDownMenu(virtualVersionNames, 
		function(text) 
			Spawn(function()
				...
				wait(0.2)
			UserSettings().GameSettings.VirtualVersion = virtualVersionNameToItem[text]

			end)
		end, false, true, baseZIndex + 1)
	virtualVersionDropDown.Name = "VirtualVersionField"
	virtualVersionDropDown.Position = UDim2.new(0, 270, 0, itemTop)
	virtualVersionDropDown.Size = UDim2.new(0,200,0,32)
	virtualVersionDropDown.Parent = gameSettingsMenuFrame
	virtualVersionDropDown.DropDownMenuButton.Text = version == Enum.VirtualVersion["2015"] and "2015" or "2014"
    if virtualVersionNames[currentSelectionIndex] then
        virtualVersionDropDown.DropDownMenuButton.Text = virtualVersionNames[currentSelectionIndex]
    end
	toggleVirtualVersionVisibility()
...
if UserSettings then
	...
	local createSettingsDialog = function()
		...
		if settingsButton == nil then
			...
			settingsButton.Image = "ayaasset://textures/ui/homeButton.png"
		...
		forceShowMainMenu = function(overrideMenu, overrideDir, overrideSize)
			if shield.Visible and overrideMenu then
				goToMenu(settingsFrame,overrideMenu,overrideDir,overrideSize)
				return
			end
			...
			game:GetService("GuiService"):AddCenterDialog(shield, Enum.CenterDialogType.ModalDialog,
				...
				function()
					...
					forceGoToMenu(settingsFrame,menuToGoTo,direction,menuSize)
					shield.Visible = true
					shield.Active = true
					settingsFrame.Parent.Position = UDim2.new(0.5, -262,0.5, -200)
					settingsFrame.Parent.Size = UDim2.new(0,525,0,430)
					Spawn(function()
						local function easeInOutQuad(t, b, c, d)
							t = t / (d / 2)
							if t < 1 then
...
local createSaveDialogs = function()
	...
	while spinnerNum <= 8 do
	    ...
	    spinnerImage.Image = "ayaasset://textures/2015/Settings/Spinner.png"
			...
			while spin do
				...
				while pos < 8 do
					if pos == spinPos or pos == ((spinPos+1)%8) then
						spinnerIcons[pos+1].Image = "ayaasset://textures/2015/Settings/SpinnerPos1.png"
					else
						spinnerIcons[pos+1].Image = "ayaasset://textures/2015/Settings/Spinner.png"
					end
...
delay(0, function()
	waitForChild(gui.UserSettingsShield.Settings.SettingsStyle, "GameSettingsMenu")
	forceShowMainMenu("GameSettingsMenu","down",UDim2.new(0,525,0,300))
end)