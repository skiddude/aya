...
local IsMaxFriendsCount = false
local IsFollowersEnabled = false
local IsNewNotifications = true
...
--[[ Images ]]--
local EXPAND_ICON = 'ayaasset://textures/ui/expandPlayerList.png'
...
local function getInstanceIcon(player)
	local userId = player and player.userId
	if userId then
		-- Check cache first
		if colorCache[userId] then
			return colorCache[userId]
		end

		-- Not in cache, fetch from API
		local wasSuccess, result = pcall(function()
			return HttpRbxApiService:GetAsync('playerlist-icon?user_id=' .. userId, true)
		end)
		
		if not wasSuccess then
			print("getInstanceIcon() failed because", result)
			return nil
		end
...
local function getMembershipIcon(player)
	...
	if AyaService:IsUsingInstance() then
		local icon = getInstanceIcon(player)
		if icon ~= nil then
			return icon
		end
	end
...
local function canSendFriendRequest(otherPlayer)
	...
	if not UsingInstance then return false end 
...
local inputBegan = ExpandFrame.InputBegan:connect(function(inputObject)
...
local inputChanged = UserInputService.InputChanged:connect(function(inputObject)
...
local inputEnded = UserInputService.InputEnded:connect(function(inputObject)
