...
do
    ...
    local colorCache = {}
    function Util.GetChatColor(player)
        ...
        if userId then
            ...
            local wasSuccess, result = pcall(function()
                return HttpRbxApiService:GetAsync('chat-color?user_id=' .. userId, true)
            end)
            
            if not wasSuccess then
                print("Util.GetChatColor() failed because", result)
                return nil
            end
...
local function CreatePlayerChatMessage(settings, playerChatType, sendingPlayer, chattedMessage, receivingPlayer)
    ...
    local function CreateMessageGuiElement()
        ...
        if chatTypeDisplayText then
            ...
            if this.SendingPlayer and AyaService:IsUsingInstance() then
                local color = Util.GetChatColor(this.SendingPlayer)
                if color ~= nil then
                    chatMessage.TextColor3 = color
                end
            end
...
local function CreateChat()
    ...
    function this:OnPlayerAdded(newPlayer)
        if newPlayer then
            spawn(function() Util.GetChatColor(newPlayer) end)
        end
        if NON_CORESCRIPT_MODE then
    ...
    function this:GetBlockedPlayersAsync()
        ...
        local url = secureBaseUrl .. "blocked-users" .. "?" .. "user_id=" .. tostring(userId)
        if userId > 0 then
            local blockList = nil
            ...
            if blockList and blockList['success'] == true and blockList['result'] then
    ...
    function this:CreateTouchDeviceChatButton()
        return Util.Create'ImageButton'
        {
            ...
            Image = 'ayaasset://textures/2015/ChatScript/TouchDeviceChatButton.png';
        };
    end
