...
static const QString ShortcutExceptions = "toggleBuildModeAction filePublishedProjectsAction";
...
void KeySequenceEdit::handleKeyEvent(QKeyEvent* e)
{
    ...
    int key = e->key();
    if (key == Qt::Key_Control || key == Qt::Key_Shift || key == Qt::Key_Meta || key == Qt::Key_Alt || key == Qt::Key_Super_L || key == Qt::Key_AltGr)
        return;
...
void ShortcutsModel::resetData()
{
    ...
    for (; iter != objects.end(); ++iter)
    {
        ...
        if (action && !action->text().isEmpty() && !action->text().startsWith("&") && !ShortcutExceptions.contains(action->objectName()))
        {
...
void ShortcutsModel::sort(int column, Qt::SortOrder order)
{
    ...
    std::sort(m_sortOrder.begin(), m_sortOrder.end(), comparator);
...
{
    for (QList<ShortcutData>::iterator iter = m_shortcutData.begin(); iter != m_shortcutData.end(); iter++)
    {
        if (iter->hasOverride())
        {
...
RobloxKeyboardConfigWidget::RobloxKeyboardConfigWidget(RobloxMainWindow& MainWindow, QWidget* parent)
    : QWidget(parent)
    , m_mainWindow(MainWindow)
    , m_filterEdit(NULL)
{
...
void RobloxKeyboardConfigWidget::restoreAllDefaults()
{
    ...
    for (; iter != objects.end(); ++iter)
    {
        ...
        if (action && !action->text().isEmpty() && !action->text().startsWith("&") && !ShortcutExceptions.contains(action->objectName()))
        {
            QMap<QString, QKeySequence>::const_iterator iter = RobloxKeyboardConfig::singleton().defaultShortcuts().find(action->objectName());
...
void RobloxKeyboardConfigWidget::dataChanged(const QModelIndex&, const QModelIndex&)
{
    Q_EMIT(dataChanged());
}
