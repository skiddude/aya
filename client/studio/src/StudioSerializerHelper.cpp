...
bool StudioSerializerHelper::saveAs(const QString& fileName, const QString& label, bool asynchronous, bool useBinaryFormat,
    Aya::DataModel* pDataModel, QString& outErrorMessage, bool publishAssets, char* thumbnailBuffer, size_t thumbnailSize)
{
    bool result = false;
    ...
    if (fileName.isEmpty())
    {
        ...
        Aya::StandardOut::singleton()->print(Aya::MESSAGE_ERROR, qPrintable(tr("Error saving file '%1' - %2").arg(fileName).arg(outErrorMessage)));
        return false;
    }
    ...
    {
        outErrorMessage = tr("Failed to write to file '%1'.\n\n"
    ...
    if (FFlag::StudioCSGAssets && publishAssets && pDataModel->getPlaceID() != 0)
    {
        try
        {
            ...
            Aya::PartOperationAsset::publishAll(pDataModel, publishTimeoutMills);
        }
        catch (Aya::DataModel::SerializationException&)
        {
    ...
    if (useBinaryFormat)
    {
        UpdateUIManager::Instance().waitForLongProcess(
            label, boost::bind(&StudioSerializerHelper::serializeBinary, fileName, pDataModel, &result, &outErrorMessage));
    }
    else
    {
        ...
        UpdateUIManager::Instance().waitForLongProcess(label, boost::bind(&StudioSerializerHelper::generateXML, pDataModel, xml));

        if (asynchronous)
        {
            new boost::thread(boost::bind(&StudioSerializerHelper::serializeXMLAsynchronous, outputFilename, xml));
            result = true;
        }
        else
        {
            ...
            UpdateUIManager::Instance().waitForLongProcess(
                label, boost::bind(&StudioSerializerHelper::serializeXML, outputFilename, xml, &result, &outErrorMessage));
            UpdateUIManager::Instance().setBusy(false);
        }
    }

    std::ofstream out(fileName.toStdString().c_str(), std::ios::app | std::ios::binary);
    out.write(thumbnailBuffer, thumbnailSize);
 ...
 *
 */
void StudioSerializerHelper::generateXML(Aya::DataModel* pDataModel, XmlElement* xml)
{
    Aya::DataModel::LegacyLock lock(pDataModel, Aya::DataModelJob::Write);
    pDataModel->writeChildren(xml, Aya::SerializationCreator);
}
 ...
 */
void StudioSerializerHelper::serializeXML(const QString& fileName, XmlElement* xml, bool* outResult, QString* outErrorMessage)
{
    QMutexLocker locker(&m_Mutex);

    AYAASSERT(xml);
    AYAASSERT(outResult);
    AYAASSERT(outErrorMessage);

    *outResult = false;
    *outErrorMessage = "";

    // create a temp file on disk, open to force creation
    QTemporaryFile tempFile(QDir::tempPath() + "/Aya-XXXXXX.rbxl");
    if (!tempFile.open())
    {
        *outErrorMessage = tr("Failed to open temporary file while saving.\n\n"
    ...
    try
    {
        ...
        {
            *outErrorMessage = tr("Failed to delete original file '%1'.\n\n"
        ...
        {
            *outErrorMessage = tr("Failed to save data to '%1'.\n\n"
    ...
    {
        *outResult = false;
        *outErrorMessage = tr("Error in saving data to '%1'.\n\n"
                              "Please try again or save with a different file name.")
                               .arg(fileName);
    }
}

void StudioSerializerHelper::serializeXMLAsynchronous(const QString& fileName, XmlElement* xml)
{
    QMutexLocker locker(&m_Mutex);
    AYAASSERT(xml);
    ...
    if (!tempFile.open())
    {
        QString message = tr("Failed to open temporary file while saving.\n\n"
                             "Make sure there is enough hard drive space for the directory '%1' "
                             "and please try again.")
                              .arg(QDir::tempPath());
        Aya::StandardOut::singleton()->print(Aya::MESSAGE_ERROR, qPrintable(message));
        return;
    }
    ...
    try
    {
        ...
        if (outputFileInfo.exists() && !QFile::remove(fileName))
        {
            QString message = tr("Failed to delete original file '%1'.\n\n"
                                 "Original data has not been overwritten.  "
                                 "Please try again or save with a different file name.")
                                  .arg(fileName);
            Aya::StandardOut::singleton()->print(Aya::MESSAGE_ERROR, qPrintable(message));
        ...
        if (!QFile::copy(tempFile.fileName(), fileName))
        {
            QString message = tr("Failed to save data to '%1'.\n\n"
                                 "Data is temporarily saved in '%2'.")
                                  .arg(fileName)
                                  .arg(tempFile.fileName());
            tempFile.setAutoRemove(false); // don't remove it, this is a valid copy
    ...
    {
        QString message = tr("Error in saving data to '%1'.\n\n"
                             "Please try again or save with a different file name.")
                              .arg(fileName);
        Aya::StandardOut::singleton()->print(Aya::MESSAGE_ERROR, qPrintable(message));
    }
...
void StudioSerializerHelper::serializeBinary(const QString& fileName, Aya::DataModel* pDataModel, bool* outResult, QString* outErrorMessage)
{
    QMutexLocker locker(&m_Mutex);
    AYAASSERT(pDataModel);

    *outResult = false;
    *outErrorMessage = "";

    // create a temp file on disk, open to force creation
    QTemporaryFile tempFile(QDir::tempPath() + "/Aya-XXXXXX.bin.rbxl");
    if (!tempFile.open())
    {
        *outErrorMessage = tr("Failed to open temporary file while saving.\n\n"
                              "Make sure there is enough hard drive space for the directory '%1' "
    ...
    try
    {
        ...
        // serialize the data model
        Aya::SerializerBinary::serialize(stream, pDataModel);
...
QByteArray StudioSerializerHelper::getDataModelHash(Aya::DataModel* pDataModel)
{
    std::stringstream strStream(std::ios_base::out | std::ios::binary);
    {
        Aya::DataModel::LegacyLock lock(pDataModel, Aya::DataModelJob::Read);
        Aya::SerializerBinary::serialize(strStream, pDataModel);
    }
...
bool StudioSerializerHelper::saveAsIfModified(const QByteArray& hashToCompareWith, Aya::DataModel* pDataModel, const QString& fileName)
{
    bool result = false;
...
bool StudioSerializerHelper::saveDebuggerData(const QString& debugInfoFile)
{
    ...
    try
    {
        ...
        const Aya::Scripting::DebuggerManager::Debuggers debuggers = debuggerManager.getDebuggers();
        if (debuggers.size() > 0)
        {
            ...
            Aya::SerializerBinary::serialize(stream, &debuggerManager);
