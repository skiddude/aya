...
static const char* layoutXmlConfigFile = "ribbon.xml";
...
#if defined(Q_WS_MAC) || defined(__linux)
...
void RobloxRibbonMainWindow::setupRibbonBar()
{
    ...
    if (m_bInitialized)
        throw Aya::runtime_error("Ribbon bar already initialized.");


    QFile file(QString::fromStdString(std::string(":/") + layoutXmlConfigFile));
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text))
    {
        std::cerr << "Failed to open ribbon XML file: " << layoutXmlConfigFile << std::endl;
        throw std::runtime_error("Failed to open ribbon XML file.");
    }
    ...
    {
        std::cerr << "Error parsing XML: " << errorMsg.toStdString() << " at line " << errorLine << ", column " << errorColumn << std::endl;
        throw std::runtime_error("Failed to parse ribbon XML file.");
    }

    QDomElement docElem = doc.documentElement();
    if (docElem.tagName() != elemMainWindow)
        throw Aya::runtime_error("Failed to parse ribbon XML file.  File must start with MainWindow element!");
...
void RobloxRibbonMainWindow::updateRecentFile(const QString& fileName)
{
    ...
    if (!fileName.isEmpty() && (fileName.endsWith(".rbxl", Qt::CaseInsensitive) || fileName.endsWith(".rbxlx", Qt::CaseInsensitive)) &&
        !(fileName.endsWith("visit.rbxl", Qt::CaseSensitive)) && !(fileName.endsWith("server.rbxl", Qt::CaseSensitive)))
    {
        RobloxSettings settings;
...
void RobloxRibbonMainWindow::parseAndCreateRibbonBar(const QDomElement& docElem)
{
    ...
    if (!ribbonBarElement.isNull())
    {
        ...
        {
            m_pRibbonMinimizeAction =
                ribbonBar()->addAction(QIcon(":/images/icons/ribbonMinimize.png"), "Toggle ribbon visiblity", Qt::ToolButtonIconOnly);
...
{
    for (QDomElement domElementIterator = tabPageElement.firstChildElement(); !domElementIterator.isNull();
        domElementIterator = domElementIterator.nextSiblingElement())
    {
#if defined(Q_WS_MAC) || defined(__linux)
        ...
        if (!flagName.isEmpty())
        {
            try
            {
                ...
                bool keepItem = Aya::GlobalAdvancedSettings::singleton()->getFFlag(flagName.toStdString());
                if (negate)
                    keepItem = !keepItem;
        ...
        else if (domElementTagName == elemAddAction)
        {
            ...
            {
                throw Aya::runtime_error("Invalid action type: %s", actionType.toStdString().c_str());
            }
        }
...
Qtitan::RibbonGroup* RobloxRibbonMainWindow::parseAndCreateRibbonGroup(const QDomElement& domElement, QObject* pParent)
{
    ...
    if (!pParentPage)
        throw Aya::runtime_error("Group parent must be a RibbonPage!");
...
QWidget* RobloxRibbonMainWindow::parseAndCreateColorPicker(const QDomElement& domElement, QObject* pParent)
{
    ...
    if (!pRibbonGroup)
        throw Aya::runtime_error("Menu buttons must have a ribbon group parent element!");

    QString text = getText(domElement);
    QIcon icon = getIcon(domElement);
    if (icon.isNull() && text.isEmpty())
        throw Aya::runtime_error("Colorpicker must have an icon or text!");
    ...
    if (!pAction)
        throw Aya::runtime_error("Failed to create action: %s", getName(domElement).toStdString().c_str());
    ...
    if (pRibbonGroup && !pRibbonGroup->layout())
    {
        pRibbonGroup->setLayout(new QVBoxLayout());
    }
...
QMenu* RobloxRibbonMainWindow::parseAndCreateMenuPopupButton(const QDomElement& domElement, QObject* pParent)
{
    ...
    QIcon icon = getIcon(domElement);
    if (icon.isNull() && text.isEmpty())
        throw Aya::runtime_error("Drop downs must have an icon or text!");
    ...
    if (menu->selectChangesText && !domElement.attribute(attribSelectChangesText).isEmpty() &&
    ...
    if (pRibbonGroup && !pRibbonGroup->layout())
    {
        pRibbonGroup->setLayout(new QVBoxLayout()); // or any other layout class
    }
...
Qtitan::RibbonGallery* RobloxRibbonMainWindow::parseAndCreateRibbonGallery(const QDomElement& domElement, QObject* pParent)
{
    Qtitan::RibbonGroup* pRibbonGroup = qobject_cast<Qtitan::RibbonGroup*>(pParent);
    if (!pRibbonGroup)
        throw Aya::runtime_error("Gallery parent must be a ribbon group!");
    ...
    AYAASSERT(pGalleryGroup->itemCount());
    ...
    int galleryHeight = (qCeil(pGalleryGroup->itemCount() / (float)numColumns) * itemSize.height());
    ...
    else
    {
        ...
        QAction* pAction =
            pRibbonGroup->addAction(getIcon(domElement), getText(domElement), getStyle(domElement), pPopupMenu, QToolButton::InstantPopup);
...
Qtitan::RibbonGalleryItem* RobloxRibbonMainWindow::parseAndCreateGalleryItem(const QDomElement& domElement, QObject* pParent)
{
    ...
    if (!pGalleryGroup)
        throw Aya::runtime_error("galleryitem must be beneath a gallery group");
    ...
    pGalleryItem->setSizeHint(pGalleryGroup->size());
...
QActionGroup* RobloxRibbonMainWindow::parseAndCreateActionGroup(const QDomElement& domElement, QObject* pParent)
{
    ...
    if (!pGroup && !pMenu)
        throw Aya::runtime_error("actiongroup elements must be beneath a group or menu");
    ...
    if (!pActionGroup)
    {
        ...
        connect(pActionGroup, SIGNAL(triggered(QAction*)), this, SLOT(handleActionGroupSelected(QAction*)));
    }
...
QButtonGroup* RobloxRibbonMainWindow::parseAnCreateButtonGroup(const QDomElement& domElement, QObject* pParent)
{
    Qtitan::RibbonGroup* pRibbonGroup = qobject_cast<Qtitan::RibbonGroup*>(pParent);
    if (!pRibbonGroup)
        throw Aya::runtime_error("buttongroup must have a ribbon group parent element!");
...
QAction* RobloxRibbonMainWindow::parseAndCreateSplitButton(const QDomElement& domElement, QObject* pParent)
{
    Qtitan::RibbonGroup* pRibbonGroup = qobject_cast<Qtitan::RibbonGroup*>(pParent);
    if (!pRibbonGroup)
        throw Aya::runtime_error("splitbutton must have a ribbon group parent element!");
...
QAction* RobloxRibbonMainWindow::parseAndCreateSeparator(const QDomElement& domElement, QObject* pParent)
{
    ...
    throw Aya::runtime_error("addseparator elements must be beneath a menu or group!");
}
...
QRadioButton* RobloxRibbonMainWindow::parseAndCreateRadioButton(const QDomElement& domElement, QObject* pParent)
{
    ...
    if (!pRibbonGroup && !pButtonGroup)
        throw Aya::runtime_error("radio buttons must be beneath a button/ribbon group");

    QString objectName = getName(domElement);
    if (objectName.isEmpty())
        throw Aya::runtime_error("radio button name can not be empty!");
...
QCheckBox* RobloxRibbonMainWindow::parseAndCreateCheckBox(const QDomElement& domElement, QObject* pParent, bool actionMode)
{
    Qtitan::RibbonGroup* pRibbonGroup = qobject_cast<Qtitan::RibbonGroup*>(pParent);
    if (!pRibbonGroup)
        throw Aya::runtime_error("checkbox must be beneath a ribbon group");

    QString objectName = getName(domElement);
    if (objectName.isEmpty())
        throw Aya::runtime_error("checkbox name element can not be empty!");
...
QComboBox* RobloxRibbonMainWindow::parseAndCreateComboBox(const QDomElement& domElement, QObject* pParent)
{
    ...
    // add combobox items
    for (QDomElement domElementIterator = domElement.firstChildElement(); !domElementIterator.isNull();
        domElementIterator = domElementIterator.nextSiblingElement())
    {
...
{
    if (!pParent)
        throw Aya::runtime_error("Failed to create action: %s.  No parent element provided!", getName(domElement).toStdString().c_str());
...
QMenu* RobloxRibbonMainWindow::parseAndCreateGroupOptionsMenu(const QDomElement& domElement, QObject* pParent)
{
    Qtitan::RibbonGroup* pRibbonGroup = qobject_cast<Qtitan::RibbonGroup*>(pParent);
    if (!pRibbonGroup)
        throw Aya::runtime_error("optionsMenu must be beneath a ribbon group");

    // QAction* optionButtonAction = pRibbonGroup->getOptionButtonAction();
    QAction* optionButtonAction = pRibbonGroup->optionButtonAction();
    if (!optionButtonAction)
        throw Aya::runtime_error("optionsMenu must has an action to associate");
...
QMenu* RobloxRibbonMainWindow::parseAndCreateSubMenu(const QDomElement& domElement, QObject* parent)
{
    ...
    if (!menuParent)
        throw Aya::runtime_error("submenu must be beneath a QMenu");
...
void RobloxRibbonMainWindow::addActionToParent(QAction* pAction, QObject* pParent, const QDomElement& domElement)
{
    if (Qtitan::RibbonGallery* pRibbonGallery = qobject_cast<Qtitan::RibbonGallery*>(pParent))
    {
        ...
        if (!pRibbonGalleryGroup)
            throw Aya::runtime_error("Failed to create action: %s. RibbonGallery must have RibbonGalleryGroup!", qPrintable(pAction->objectName()));
...
void RobloxRibbonMainWindow::handleRibbonMinimizationChanged(bool minimized)
{
    ...
    m_pRibbonMinimizeAction->setIcon(minimized ? QIcon(":/images/icons/ribbonMaximize.png") : QIcon(":/images/icons/ribbonMinimize.png"));
...
void RobloxRibbonMainWindow::handleStyleChange(QAction* pAction)
{
    ...
    Qtitan::RibbonStyle::Theme themeId = Qtitan::RibbonStyle::Office2013White;
...
void RobloxRibbonMainWindow::initialize()
{
    ...
    QFile file(assetFolder + "/fonts/SourceSansPro-Regular.ttf");
    if (!file.open(QIODevice::ReadOnly))
    {
        qWarning("Could not open the font file");
        return;
    }
    ...
    AYAASSERT(!loadedFontFamilies.empty());

    m_ribbonFont = QFont(loadedFontFamilies.at(0), 9);
    m_ribbonFont.setHintingPreference(QFont::HintingPreference::PreferNoHinting);
#ifdef Q_WS_MAC
    // On Mac we need to have bigger fonts
...
void RobloxRibbonMainWindow::setTheme(Qtitan::RibbonStyle::Theme themeId)
{
    ...
    {
        if (themeId == Qtitan::OfficeStyle::Windows7Scenic || themeId == Qtitan::OfficeStyle::Office2010Blue ||
            themeId == Qtitan::OfficeStyle::Office2010Silver || themeId == Qtitan::OfficeStyle::Office2010Black ||
            themeId == Qtitan::OfficeStyle::Office2013White || themeId == Qtitan::OfficeStyle::Office2013Gray)
        {
