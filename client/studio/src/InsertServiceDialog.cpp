...
void InsertServiceDialog::setDataModel(boost::shared_ptr<Aya::DataModel> pDataModel)
{
...
void InsertServiceDialog::recreateWidget()
{
    ...
    InsertObjectWidget::createWidgets(true, m_pInsertObjectListWidget, &Aya::Reflection::ClassDescriptor::rootDescriptor(), m_pDataModel);
...
void InsertServiceDialog::onItemInsertRequested(QListWidgetItem* pListWidgetItem)
{
    ...
    try
    {
        ...
        boost::shared_ptr<Aya::Instance> pService =
            Aya::ServiceProvider::create(m_pDataModel.get(), pInsertListWidgetItem->getInstance()->getClassName());
        if (!pService)
            throw std::runtime_error(QString("Failed to create Service object of type %s")
                    .arg(pInsertListWidgetItem->getInstance()->getClassNameStr().c_str())
                    .toStdString());

        // TODO -- currently this undo/redo doesn't work
        // for undo redo --
        std::string action = Aya::format("Insert %s", pService->getName().c_str());
        Aya::ChangeHistoryService::requestWaypoint(action.c_str(), m_pDataModel.get());
    }
    catch (std::bad_alloc&)
    {
        throw;
...
void InsertServiceDialog::redrawDialog()
{
    if (m_pDataModel)
    {
        ...
        while (currentChild < numChild)
        {
            ...
            if (pCurrentItem)
            {
                AYAASSERT(dynamic_cast<Aya::Service*>(pCurrentItem->getInstance().get()));
                // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_WARNING, "InsertService: current object = %s and visible = %d",
                // pCurrentItem->getInstance()->getClassNameStr().c_str(), visible);
