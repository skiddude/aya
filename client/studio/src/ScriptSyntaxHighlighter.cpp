...
static bool IsANumberChar(int ch)
{
    ...
    return (ch < 0x80) &&
           (isdigit(ch) || toupper(ch) == 'E' || ch == '.' || ch == '-' || ch == '+' || (ch >= 'a' && ch <= 'f') || (ch >= 'A' && ch <= 'F'));
}
...
{
    m_keywordPatterns << "and" << "break" << "do" << "else" << "elseif" << "end" << "false" << "for" << "function" << "if" << "in" << "local" << "nil"
                      << "not" << "or" << "repeat" << "return" << "then" << "true" << "until" << "while" << "_ALERT" << "_ERRORMESSAGE" << "_INPUT"
                      << "_PROMPT" << "_OUTPUT _STDERR" << "_STDIN" << "_STDOUT" << "call" << "dostring" << "foreach" << "foreachi" << "getn"
                      << "globals" << "newtype" << "rawget" << "rawset" << "require" << "sort" << "tinsert" << "tremove" << "_G" << "getfenv"
                      << "getmetatable" << "ipairs" << "loadlib" << "next" << "pairs" << "pcall" << "rawegal" << "rawget" << "rawset" << "require"
                      << "setfenv" << "setmetatable" << "xpcall" << "string" << "table" << "math" << "coroutine" << "io" << "os" << "debug" << "abs"
                      << "acos" << "asin" << "atan" << "atan2" << "ceil" << "cos" << "deg" << "exp" << "floor" << "format" << "frexp" << "gsub"
                      << "ldexp" << "log" << "log10" << "max" << "min" << "mod" << "rad" << "random" << "randomseed" << "sin" << "sqrt" << "strbyte"
                      << "strchar" << "strfind" << "strlen" << "strlower" << "strrep" << "strsub" << "strupper" << "tan" << "openfile" << "closefile"
                      << "readfrom" << "writeto" << "appendto" << "remove" << "rename" << "flush" << "seek" << "tmpfile" << "tmpname" << "read"
                      << "write" << "clock" << "date" << "difftime" << "execute" << "exit" << "getenv" << "setlocale" << "time";
...
void ScriptSyntaxHighlighter::setLexState(RBXLuaLexState lexState, int currentPos)
{
    ...
    switch (m_lexState)
    {
    case AYA_LUA_COMMENT:
    case AYA_LUA_COMMENTLINE:
        format = m_commentFormat;
        break;
    case AYA_LUA_NUMBER:
        format = m_numberFormat;
        break;
    case AYA_LUA_PREPROCESSOR:
        format = m_preprocessorFormat;
        break;
    case AYA_LUA_OPERATOR:
        format = m_operatorFormat;
        break;
    case AYA_LUA_STRING:
    case AYA_LUA_STRINGEOL:
    case AYA_LUA_CHARACTER:
    case AYA_LUA_LITERALSTRING:
...
void ScriptSyntaxHighlighter::highlightBlock(const QString& text)
{
    ...
    if (m_lexState == AYA_LUA_LITERALSTRING || m_lexState == AYA_LUA_COMMENT)
    {
    ...
    for (int ii = 0; ii < text.size(); ii++)
    {
        ...
        if (m_lexState == AYA_LUA_OPERATOR)
        {
            if (ii > 0)
            {
                if (text[ii - 1] == QChar('{'))
            ...
            setLexState(AYA_LUA_DEFAULT, ii);
        }
        else if (m_lexState == AYA_LUA_NUMBER)
        {
            // Stop the number definition on non-numerical non-dot non-eE non-sign non-hexdigit char
            if (!IsANumberChar(text[ii].unicode()))
            {
                setLexState(AYA_LUA_DEFAULT, ii);
            }
        ...
        else if (m_lexState == AYA_LUA_IDENTIFIER)
        {
            if (!IsAWordChar(text[ii].unicode()) || (text[ii] == QChar('.')))
            {
                ...
                {
                    m_lexState = AYA_LUA_WORD;
                    hasFold |= checkApplyFoldState(keyword);
                }
                setLexState(AYA_LUA_DEFAULT, ii);
            }
        }
        else if (m_lexState == AYA_LUA_COMMENTLINE || m_lexState == AYA_LUA_PREPROCESSOR)
        {
            if (ii == text.size() - 1)
                setLexState(AYA_LUA_DEFAULT, ++ii);
        }
        else if (m_lexState == AYA_LUA_STRING)
        {
            if (text[ii] == QChar('\\'))
            {
                if (ii + 1 < text.size())
            ...
            {
                m_lexState = AYA_LUA_STRINGEOL;
                setLexState(AYA_LUA_DEFAULT, ++ii);
            }
        }
        else if (m_lexState == AYA_LUA_CHARACTER)
        {
            if (text[ii] == QChar('\\'))
            {
                if (ii + 1 < text.size())
        ...
        else if (m_lexState == AYA_LUA_LITERALSTRING || m_lexState == AYA_LUA_COMMENT)
        {
            if (text[ii] == QChar('['))
            {
                int sep = LongDelimCheck(text, ii);
                if (sep == 1 && m_sepCount == 1)
                { // [[-only allowed to nest
                    m_nestLevel++;
                    ii++;
                    hasFold |= checkApplyFoldState("[[");
                }
        ...
        if (m_lexState == AYA_LUA_DEFAULT)
        {
            if (ii >= text.size())
                break;

            if (IsADigit(text[ii].unicode()) || (text[ii] == QChar('.') && (ii + 1 < text.size()) && IsADigit(text[ii + 1].unicode())))
            {
                m_lexState = AYA_LUA_NUMBER;
                if (text[ii] == QChar('0') && (ii + 1 < text.size()) && text[ii + 1].toUpper() == QChar('X'))
                {
                    ii++;
                }
            }
            else if (IsAWordStart(text[ii].unicode()))
            {
                m_lexState = AYA_LUA_IDENTIFIER;
            }
            else if (text[ii] == QChar('\"'))
            {
                m_lexState = AYA_LUA_STRING;
            }
            else if (text[ii] == QChar('\''))
            {
                m_lexState = AYA_LUA_CHARACTER;
            }
            else if (text[ii] == QChar('['))
            {
                ...
                if (m_sepCount == 0)
                {
                    m_lexState = AYA_LUA_OPERATOR;
                }
                else
                {
                    m_nestLevel = 1;
                    hasFold |= checkApplyFoldState("[==[");
                    setLexState(AYA_LUA_LITERALSTRING, ii);
                    ii += m_sepCount;
                }
            }
            else if (text[ii] == QChar('-') && (ii + 1 < text.size()) && text[ii + 1] == QChar('-'))
            {
                setLexState(AYA_LUA_COMMENTLINE, ii);
            ...
            {
                m_lexState = AYA_LUA_PREPROCESSOR; // Obsolete since Lua 4.0, but still in old code
            }
            else if (IsLuaOperator(text[ii].unicode()))
            {
                m_lexState = AYA_LUA_OPERATOR;
            }
        }
    ...
    if (m_lexState == AYA_LUA_OPERATOR)
    {
        ...
        setLexState(AYA_LUA_DEFAULT, finalIndex);
    }
    else if (m_lexState == AYA_LUA_NUMBER || m_lexState == AYA_LUA_COMMENTLINE || m_lexState == AYA_LUA_PREPROCESSOR)
    {
        setLexState(AYA_LUA_DEFAULT, finalIndex);
    }
    else if (m_lexState == AYA_LUA_STRING || m_lexState == AYA_LUA_CHARACTER)
    {
        if (finalIndex > 0)
        {
            if (text[finalIndex - 1] == QChar('\\'))
...
void ScriptSyntaxHighlighter::onPropertyChanged(const Aya::Reflection::PropertyDescriptor* pDescriptor)
{
