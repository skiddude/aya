...
public:
    ...
    FindData(LuaSourceBuffer instance, int lineNumber, QString text, QString hierarchy = QString())
        : m_instance(instance)
        , m_line(lineNumber)
        , m_lineText(text)
    ...
    void setPlaceData(int position, int length)
    {
        m_position = position;
        m_length = length;
    }
...
public:
    ...
    void addItems(QList<FindData> items)
    {
        m_FindList.append(items);
    }
...
public:
    ...
    inline friend eFindFlags operator|(eFindFlags a, eFindFlags b)
    {
        return static_cast<eFindFlags>(static_cast<int>(a) | static_cast<int>(b));
    }
...
private:
    ...
    void populateWidget(shared_ptr<Aya::DataModel> dm, const QString& findStr, eFindFlags flags, bool allScripts);

    void populateScriptList(bool allScripts);

    QTextCursor findNextInScript(
        const QTextDocument& doc, const QString& findStr, const QTextCursor& cursor, QTextDocument::FindFlags flags, bool regExp);

    void getNextScript(int index, shared_ptr<Aya::DataModel> dm, const QString& findStr, eFindFlags flags);
    void searchScriptChain(
    ...
    QString getInstanceHierarchy(shared_ptr<Aya::Instance> script);
