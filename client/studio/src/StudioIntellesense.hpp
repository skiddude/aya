...
public:
    ...
    bool activate(QString& text, int row, int& cursorIndex, QKeyEvent* keyEvent, QRect positionRange, bool searchDatamodel, QWidget* focusWidget,
        const boost::optional<std::vector<Aya::ScriptAnalyzer::IntellesenseResult>>& result);
    void deactivate();
...
private:
    ...
    int repopulateUserVariables(const std::vector<Aya::ScriptAnalyzer::IntellesenseResult>& result, unsigned int row, unsigned int column);
    ...
    void populateProperty(const Aya::Reflection::PropertyDescriptor* d);
    void populateFunction(const Aya::Reflection::FunctionDescriptor* d);
    void populateYieldFunction(const Aya::Reflection::YieldFunctionDescriptor* d);
    void populateEvent(const Aya::Reflection::EventDescriptor* d);
    void populateCallBack(const Aya::Reflection::CallbackDescriptor* d);
    void populateEnumerator(const Aya::Reflection::EnumDescriptor::Item* d);

    int populateItemLists(
    ...
    bool populateItemListsFromVariant(const Aya::Reflection::Variant varValue, bool showAllChildren = false);

    void repopulateListWidget(int itemFlags, QString currentWord, int lineIndex = 0);
    void repopulateListWidgetHelper(
    ...
    shared_ptr<Aya::LuaSourceContainer> getCurrentScript();
