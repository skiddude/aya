...
public:
     ...
     */
    virtual bool find(bool loop, const QRegularExpression& regExp, QTextDocument::FindFlags flags) = 0;

    virtual void moveNext(bool forward) = 0;
    /**
     * Replaces the text for the current item.
     */
    virtual void replace(const QString& text, const QRegularExpression& regExp, QTextDocument::FindFlags flags) = 0;

    virtual void replaceNext(const QRegularExpression& rx, const QString& text) = 0;
    virtual void replaceAll(const QRegularExpression& rx, const QString& text) = 0;
...
public:
    ...
    bool getSearchParameters(bool forward, QRegularExpression& outRegExp, QTextDocument::FindFlags& outflags);
...
public:
    ...
    QRegularExpression getSearchString();
...
public:
    ...
    {
        AYAASSERT(m_pFindDialog);
        return m_pFindDialog;
    }
    ReplaceDialog* getReplaceDialog()
    {
        AYAASSERT(m_pReplaceDialog);
        return m_pReplaceDialog;
    }
    FindAllDialog* getFindAllDialog()
    {
        AYAASSERT(m_pFindAllDialog);
        return m_pFindAllDialog;
    }
    GoToLineDialog* getGotoLineDialog()
    {
        AYAASSERT(m_pGotoLineDialog);
        return m_pGotoLineDialog;
    }
    EmbeddedFindReplaceWidget* getEmbeddedFindDialog()
    {
        AYAASSERT(m_pEmbeddedFindReplaceWidget);
        return m_pEmbeddedFindReplaceWidget;
    }
