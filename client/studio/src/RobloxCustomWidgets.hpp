...
public:
    ...
    virtual QString getSelectedItemQString()
    {
        return QString("");
    }
...
public:
    ...
    virtual PickerFrame* getPickerFrame()
    {
        return NULL;
    }

    void setCustomToolButtonStyle(Qt::ToolButtonStyle style)
    {
        m_toolButtonStyle = style;
    }
    Qt::ToolButtonStyle getCustomToolButtonStyle() const
    {
...
public:
    PopupLaunchEditor(
        PropertyItem* pParentItem, QWidget* pParent, const QIcon& itemIcon, const QString& labelText, int buttonWidth, QWidget* proxyWidget = NULL);
...
public:
    ...
    QString getImageUrl(const QString& lastValue, const QPoint& location, boost::shared_ptr<Aya::DataModel> dm);
...
class PartSelectionTool : public Aya::Named<Aya::MouseCommand, sPartSelectionTool>
{
public:
    PartSelectionTool(Aya::Workspace* workspace);
    void setHighlightPart(boost::shared_ptr<Aya::PartInstance> pPart)
    {
        m_pHighlightPart = pPart;
    }

    Aya::signal<void(Aya::PartInstance*, bool&)> partSelectedSignal;
    Aya::signal<void(Aya::PartInstance*, bool&)> partHoveredOverSignal;
...
private:
    ...
    /*override*/ void onMouseHover(const shared_ptr<Aya::InputObject>& inputObject);
    /*override*/ shared_ptr<Aya::MouseCommand> onMouseDown(const shared_ptr<Aya::InputObject>& inputObject);
    /*override*/ shared_ptr<Aya::MouseCommand> onMouseUp(const shared_ptr<Aya::InputObject>& inputObject);
    /*override*/ void render3dAdorn(Aya::Adorn* adorn);

    boost::shared_ptr<Aya::PartInstance> m_pHighlightPart;
    std::string m_CursorName;
    bool m_bIsValidPartSelected;
};

class PrimaryPartSelectionEditor
    : public QWidget
    , public InstanceSelectionHandler
{
    Q_OBJECT
public:
    PrimaryPartSelectionEditor(QWidget* parent, PropertyItem* pItem, const QString& labelText, boost::shared_ptr<Aya::ModelInstance> instance);
...
private:
    ...
    /*override*/ void onInstanceSelected(boost::shared_ptr<Aya::Instance> instance);
    /*override*/ void onInstanceHovered(boost::shared_ptr<Aya::Instance> instance);

    void onPartSelected(Aya::PartInstance* pPart);
    void onPartHoveredOver(Aya::PartInstance* pPart, bool& isValid);

    bool setPrimaryPart(Aya::PartInstance* pPart);
    ...
    boost::weak_ptr<Aya::PartInstance> m_pTempPrimaryPart;

    PropertyItem* m_pPrimaryPartPropertyItem;

    Aya::signals::scoped_connection m_cPartSelectedConnection;
    Aya::signals::scoped_connection m_cPartHoveredOverConnection;
};
