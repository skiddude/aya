...
void RobloxBrowser::drawLoadingWatermark()
{
    if (m_loadingTimer)
    {
        ...
        QRect roundRect(centerX - rectSize * 0.5f, centerY - rectSize * 0.5f, rectSize, rectSize);
        painter.drawRoundedRect(roundRect, 18, 18);
        ...
        for (int i = 0; i < numFins; i++)
        {
            ...
            pen.setColor(QColor(loadingFinColor.red(), loadingFinColor.green(), loadingFinColor.blue(),
            ...
            painter.drawLine(floor(centerX + sinpos * innerRadius + 0.5f), floor(centerY + cospos * innerRadius + 0.5f),
                floor(centerX + sinpos * outerRadius + 0.5f), floor(centerY + cospos * outerRadius + 0.5f));
        }
        m_refreshIncr += 0.08f;
    }
}
