...
bool SelectionHighlightAdornable::getSelectionDimensions(
    Workspace* workspace, shared_ptr<Instance> instance, Part* out, float* lineThickness, bool* checkChildren)
{
    *checkChildren = false;

    if (!workspace->contains(instance.get()) || Instance::fastSharedDynamicCast<MegaClusterInstance>(instance))
    {
        return false;
    }
    ...
    else if (shared_ptr<FaceInstance> faceInstance = Instance::fastSharedDynamicCast<FaceInstance>(instance))
    {
        if (PartInstance* partParent = Instance::fastDynamicCast<PartInstance>(faceInstance->getParent()))
        {
            ...
            float points[6] = {halfSize.x, halfSize.y, halfSize.z, -halfSize.x, -halfSize.y, -halfSize.z};
    ...
    else if (Instance::fastSharedDynamicCast<Tool>(instance) || Instance::fastSharedDynamicCast<Accoutrement>(instance))
    {
        *checkChildren = true;
    }
...
void SelectionHighlightAdornable::render(shared_ptr<Instance> instance, Part part, float lineThickness, Adorn* adorn)
{
    bool isFaceInstance = Instance::fastSharedDynamicCast<FaceInstance>(instance) != NULL;
    Draw::selectionBox(
        part, adorn, isFaceInstance ? Color3::orange() : G3D::Color4(133.0f / 255.0f, 64.0f / 255.0f, 245.0f / 255.0f, 1.0f), lineThickness);
    if (shared_ptr<ModelInstance> m = Instance::fastSharedDynamicCast<ModelInstance>(instance))
    {
        ...
        {
            Draw::selectionBox(primary->getPart(), adorn, ModelInstance::primaryPartSelectColor(), ModelInstance::primaryPartLineThickness());
        }
    }
}
