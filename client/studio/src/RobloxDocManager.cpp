...
bool RobloxDocManager::requestCloseAllDocs()
{
    ...
    {
        Aya::BaldPtr<IRobloxDoc> doc = m_KeyDocMap.begin().value();
...
{
    AYAASSERT(!m_OpenScriptDocs.contains(&doc));
...
{
    Aya::BaldPtr<IRobloxDoc> pDoc = NULL;
...
RobloxScriptDoc* RobloxDocManager::openDoc(LuaSourceBuffer script)
{
    ...
    boost::scoped_ptr<Aya::DataModel::LegacyLock> lock;
    ...
    lock.reset(new Aya::DataModel::LegacyLock(m_pPlayDoc->getDataModel(), Aya::DataModelJob::Write));

    // check if it is a linked script, and if so substitute opening linked source
    if (shared_ptr<Aya::LuaSourceContainer> lsc = script.toInstance())
    {
    ...
    {
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Error in opening script");
        pDoc->close();
        delete pDoc;
        return NULL;
    }
...
void RobloxDocManager::openDoc(shared_ptr<Aya::Instance> script, int lineNumber)
{
...
{
    const Aya::BaldPtr<QWidget> widget = doc.getViewer();
    AYAASSERT(widget);

    const QString keyName = doc.keyName();
    AYAASSERT(!keyName.isEmpty());
...
void RobloxDocManager::removeDoc(IRobloxDoc& doc)
{
    ...
    if (&doc == m_pPlayDoc)
    {
        ...
        for (tScriptDocs::iterator itr = copyOfScriptDocs.begin(); itr != copyOfScriptDocs.end(); ++itr)
        {
            m_pMainWindow->requestDocClose(*itr);
        }
        AYAASSERT(m_OpenScriptDocs.empty());
...
void RobloxDocManager::setCurrentDoc(IRobloxDoc* doc)
{
    ...
    {
        Aya::PluginManager::singleton()->setCurrentDataModel(NULL);
    }
...
void RobloxDocManager::updateWindowMenu()
{
    ...
    {
        Aya::BaldPtr<IRobloxDoc> doc = iter.value();
...
{
    const Aya::BaldPtr<QAction> action = qobject_cast<QAction*>(sender());
