...
static bool populateValues(QTreeWidgetItem* pParentItem, const QString& key, const Aya::Reflection::Variant varValue);
    ...
    : m_bBreakpointEncountered(false)
{
    ...
    icons[eDV_BREAKPOINTS] = QString::fromUtf8(":/images/icons/16x16/breakpoints.png");
    icons[eDV_CALLSTACK] = QString::fromUtf8(":/images/icons/16x16/callStack.png");
    icons[eDV_WATCH] = QString::fromUtf8(":/images/icons/16x16/watch.png");
    ...
    //
    m_cConnections.push_back(
        DebuggerClientManager::Instance().debuggingStarted.connect(boost::bind(&DebuggerViewsManager::onDebuggingStarted, this)));
    m_cConnections.push_back(
        DebuggerClientManager::Instance().debuggingStopped.connect(boost::bind(&DebuggerViewsManager::onDebuggingStopped, this)));

    // if debugger client get's changed (update dock widgets)
    m_cConnections.push_back(
        DebuggerClientManager::Instance().clientActivated.connect(boost::bind(&DebuggerViewsManager::onDebuggerClientActivated, this, _1, _2)));
    m_cConnections.push_back(
        DebuggerClientManager::Instance().clientDeactivated.connect(boost::bind(&DebuggerViewsManager::onDebuggerClientDeactivated, this)));

    // update dock widgets whenever exeuction gets paused or started
    m_cConnections.push_back(
        DebuggerClientManager::Instance().executionDataCleared.connect(boost::bind(&DebuggerViewsManager::onExecutionDataCleared, this)));
    m_cConnections.push_back(
        DebuggerClientManager::Instance().breakpointEncountered.connect(boost::bind(&DebuggerViewsManager::onBreakpointEncountered, this, _1, _2)));
    ...
    // breakpoint
    m_cConnections.push_back(
        DebuggerClientManager::Instance().breakpointAdded.connect(boost::bind(&DebuggerViewsManager::onBreakpointAdded, this, _1)));
    m_cConnections.push_back(
        DebuggerClientManager::Instance().breakpointRemoved.connect(boost::bind(&DebuggerViewsManager::onBreakpointRemoved, this, _1)));
    ...
    m_cConnections.push_back(
        DebuggerClientManager::Instance().debuggersListUpdated.connect(boost::bind(&DebuggerViewsManager::onDebuggersListUpdated, this, _1)));
}
...
void DebuggerViewsManager::onDebuggerClientActivated(int currentBreakpoint, Aya::Scripting::ScriptDebugger::Stack currentStack)
{
...
void DebuggerViewsManager::onBreakpointAdded(shared_ptr<Aya::Scripting::DebuggerBreakpoint> spBreakpoint)
{
    getViewWidget<BreakpointsWidget>(eDV_BREAKPOINTS).addBreakpoint(spBreakpoint);
}

void DebuggerViewsManager::onBreakpointRemoved(shared_ptr<Aya::Scripting::DebuggerBreakpoint> spBreakpoint)
{
    getViewWidget<BreakpointsWidget>(eDV_BREAKPOINTS).removeBreakpoint(spBreakpoint);
}

void DebuggerViewsManager::onWatchAdded(shared_ptr<Aya::Scripting::DebuggerWatch> spWatch)
{
    getViewWidget<WatchWidget>(eDV_WATCH).addWatch(spWatch);
}

void DebuggerViewsManager::onWatchRemoved(shared_ptr<Aya::Scripting::DebuggerWatch> spWatch)
{
...
// StackDetailsItem
//--------------------------------------------------------------------------------------------
StackDetailsItem::StackDetailsItem(const Aya::Scripting::ScriptDebugger::FunctionInfo& functionInfo, bool isFirstItem)
    : m_bIsFirstItem(isFirstItem)
{
    ...
    if (m_bIsFirstItem)
        setIcon(0, QIcon(":/images/icons/16x16/debugger_arrow.png"));
...
void StackDetailsItem::updateIcon(bool isCurrent)
{
    ...
    if (m_bIsFirstItem)
    {
        pDebuggerClient->setMarker(getLine() - 1, ":/images/icons/16x16/debugger_arrow.png", true);
        pDebuggerClient->setCurrentLine(getLine() - 1);
        return;
    }

    pDebuggerClient->setMarker(getLine() - 1, isCurrent ? ":/images/icons/16x16/debugger_arrow_curve_left.png" : "", true);
    setIcon(0, QPixmap(isCurrent ? ":/images/icons/16x16/debugger_arrow_curve_left.png" : ""));
}
...
void CallStackTreeWidget::setCallStack(const Aya::Scripting::ScriptDebugger::Stack& stack)
{
    m_pPreviousItem = NULL;
    clear();

    for (Aya::Scripting::ScriptDebugger::Stack::const_iterator iter = stack.begin(); iter != stack.end(); ++iter)
...
void CallStackWidget::populateDebuggers(DebuggerClient* pDebuggerClient)
{
    ...
    if (DebuggerClientManager::Instance().getCurrentDebuggerClient() && (m_pCurrentDebuggerClient == pDebuggerClient) &&
    ...
    boost::shared_ptr<Aya::Instance> script;
    bool indexSet = false;
    ...
    for (unsigned int ii = 0; ii < debuggerClients.size(); ++ii)
    {
        ...
        if (!currentDebuggerClient || !Aya::Scripting::DebuggerManager::singleton().findDebugger(currentDebuggerClient->getScript().get()))
            continue;
        ...
        const Aya::Scripting::ScriptDebugger::PausedThreads& pausedThreads = currentDebuggerClient->getPausedThreads();
        for (Aya::Scripting::ScriptDebugger::PausedThreads::const_iterator it = pausedThreads.begin(); it != pausedThreads.end(); ++it)
        {
...
void CallStackWidget::setCallStack(const Aya::Scripting::ScriptDebugger::Stack& stack)
{
...
void CallStackWidget::onCurrentIndexChanged(int index)
{
    ...
    boost::shared_ptr<Aya::Script> spScript = shared_from(static_cast<Aya::Script*>(var.value<void*>()));
    if (spScript)
    {
        ...
        if (pDebuggerClient)
        {
            ...
            if (match.hasMatch())
            {
                int threadID = match.captured(1).toInt(0, 16);
...
// BreakpointItem
//--------------------------------------------------------------------------------------------
BreakpointItem::BreakpointItem(boost::shared_ptr<Aya::Scripting::DebuggerBreakpoint> spBreakpoint)
    ...
    , m_bIgnoreWaypointRequest(false)
{
    ...
    Aya::Scripting::ScriptDebugger* spDebugger = Aya::Instance::fastDynamicCast<Aya::Scripting::ScriptDebugger>(spBreakpoint->getParent());
    if (spDebugger)
    {
...
bool BreakpointItem::hasBreakpoint(boost::shared_ptr<Aya::Scripting::DebuggerBreakpoint> spBreakpoint)
{
...
{
    Aya::ScopedAssign<bool> ignoreBreakpointModificationEvent(m_bIgnoreBreakpointModification, true);

    if (column == 0)
    {
        Aya::DataModel::LegacyLock lock(DebuggerClientManager::Instance().getDataModel(), Aya::DataModelJob::Write);
        Aya::Scripting::DebuggerBreakpoint::prop_Enabled.setValue(m_spBreakpoint.get(), checkState(column) == Qt::Checked);
    ...
    else if (column == 4)
    {
        ...
        {
            Aya::DataModel::LegacyLock lock(DebuggerClientManager::Instance().getDataModel(), Aya::DataModelJob::Write);
            Aya::Scripting::DebuggerBreakpoint::prop_Condition.setValue(m_spBreakpoint.get(), modifiedCondition.toStdString());
        }
    }
}
...
void BreakpointItem::activate()
{
    ...
    Aya::Scripting::ScriptDebugger* pDebugger = Aya::Instance::fastDynamicCast<Aya::Scripting::ScriptDebugger>(m_spBreakpoint->getParent());
    if (pDebugger)
    {
...
{
    Aya::ScopedAssign<bool> ignoreBreakpointModificationEvent(m_bIgnoreWaypointRequest, true);
...
void BreakpointItem::onBreakpointPropertyChanged(const Aya::Reflection::PropertyDescriptor* pPropertyDescriptor)
{
    ...
    if (*pPropertyDescriptor == Aya::Scripting::DebuggerBreakpoint::prop_Enabled)
        setBreakpointEnabled(m_spBreakpoint->isEnabled());
}

void BreakpointItem::onScriptAncestryChanged(shared_ptr<Aya::Instance> newParent)
{
    // update breakpoint name if the script has change in hierarchy
    if (newParent)
    {
        Aya::Scripting::ScriptDebugger* spDebugger = Aya::Instance::fastDynamicCast<Aya::Scripting::ScriptDebugger>(m_spBreakpoint->getParent());
        if (spDebugger)
            setText(2, spDebugger->getScript()->getFullName().c_str());
    }
}
...
void BreakpointsTreeWidget::addBreakpoint(boost::shared_ptr<Aya::Scripting::DebuggerBreakpoint> spBreakpoint)
{
    if (spBreakpoint && !findBreakpointItem(spBreakpoint))
        addTopLevelItem(new BreakpointItem(spBreakpoint));
}

void BreakpointsTreeWidget::removeBreakpoint(boost::shared_ptr<Aya::Scripting::DebuggerBreakpoint> spBreakpoint)
{
    if (spBreakpoint)
    {
...
BreakpointItem* BreakpointsTreeWidget::findBreakpointItem(boost::shared_ptr<Aya::Scripting::DebuggerBreakpoint> spBreakpoint)
{
    ...
    , m_bIgnoreBreakpointModification(false)
{
    ...
    QIcon icon;
    icon.addFile(QString::fromUtf8(":/images/icons/16x16/breakpointDelete.png"), QSize(), QIcon::Normal, QIcon::Off);
    ...
    QIcon icon1;
    icon1.addFile(QString::fromUtf8(":/images/icons/16x16/deleteAllBreakpoints.png"), QSize(), QIcon::Normal, QIcon::Off);
    ...
    QIcon icon2;
    icon2.addFile(QString::fromUtf8(":/images/icons/16x16/enableDisableBreakpoints.png"), QSize(), QIcon::Normal, QIcon::Off);
    ...
    QIcon icon3;
    icon3.addFile(QString::fromUtf8(":/images/icons/16x16/gotoSource.png"), QSize(), QIcon::Normal, QIcon::Off);
    ...
    connect(UpdateUIManager::Instance().getMainWindow().toggleAllBreakpointsStateAction, SIGNAL(triggered()), m_pDisableEnableAllBreakpointsTB,
        SIGNAL(clicked()));
...
void BreakpointsWidget::addBreakpoint(boost::shared_ptr<Aya::Scripting::DebuggerBreakpoint> breakpoint)
{
    if (!m_bIgnoreBreakpointModification)
...
void BreakpointsWidget::removeBreakpoint(boost::shared_ptr<Aya::Scripting::DebuggerBreakpoint> breakpoint)
{
    if (!m_bIgnoreBreakpointModification)
...
void BreakpointsWidget::onPushButtonClicked()
{
    ...
    Aya::ScopedAssign<bool> ignorePropertyModifyEvent(m_bIgnoreBreakpointModification, true);
...
// WatchItem
//--------------------------------------------------------------------------------------------
WatchItem::WatchItem(boost::shared_ptr<Aya::Scripting::DebuggerWatch> spWatch)
    : m_spWatch(spWatch)
    , m_bIsInValid(true)
{
...
void WatchItem::updateWatchValue()
{
    ...
    try
    {
        ...
        Aya::Reflection::Variant Value = DebuggerClientManager::Instance().getWatchValue(m_spWatch);
        if (!Value.isVoid())
...
bool WatchItem::hasWatch(boost::shared_ptr<Aya::Scripting::DebuggerWatch> spWatch)
{
...
void WatchItem::columnModified(int column)
{
    ...
    Aya::ScopedAssign<bool> ignoreWatchModificationEvent(m_bIgnoreWatchModification, true);
    if (column == 0)
    {
        ...
        if (watchExpression != m_spWatch->getExpression())
        {
            ...
            Aya::DataModel::LegacyLock lock(DebuggerClientManager::Instance().getDataModel(), Aya::DataModelJob::Write);
            Aya::Scripting::DebuggerWatch::prop_Expression.setValue(m_spWatch.get(), watchExpression);
...
void WatchItem::onWatchPropertyChanged(const Aya::Reflection::PropertyDescriptor* pPropertyDescriptor)
{
    if (m_bIgnoreWatchModification || !m_spWatch)
        return;

    if (*pPropertyDescriptor == Aya::Scripting::DebuggerWatch::prop_Expression)
        setText(0, QString::fromStdString(Aya::Scripting::DebuggerWatch::prop_Expression.getValue(m_spWatch.get())));
}
...
void WatchItem::onScriptAncestryChanged(shared_ptr<Aya::Instance> newParent)
{
    // update script name if the script has change in hierarchy
    if (newParent)
    {
        QString scriptName;
        Aya::Scripting::ScriptDebugger* spDebugger = Aya::Instance::fastDynamicCast<Aya::Scripting::ScriptDebugger>(m_spWatch->getParent());
...
public:
    DummyWatchItem()
        : WatchItem(boost::shared_ptr<Aya::Scripting::DebuggerWatch>())
...
public:
    ...
    QWidget* createEditor(QWidget* parent, const QStyleOptionViewItem& option, const QModelIndex& index) const
    {
        ...
        if (dummyWatchItem)
        {
            ...
            {
                Aya::StandardOut::singleton()->print(Aya::MESSAGE_INFO, "Please open a script document to add a new watch expression");
                return NULL;
            }
        }
...
void WatchTreeWidget::addWatch(boost::shared_ptr<Aya::Scripting::DebuggerWatch> spWatch)
{
...
void WatchTreeWidget::removeWatch(boost::shared_ptr<Aya::Scripting::DebuggerWatch> spWatch)
{
...
{
    Aya::ScopedAssign<bool> ignoreWatchDeleteEvent(m_bIgnoreWatchModification, true);
    QList<QTreeWidgetItem*> selItems = selectedItems();
    if (selItems.length() > 0)
    {
...
void WatchTreeWidget::addWatch()
{
    ...
    else
    {
        ...
        else
        {
            ...
            watchExpression = QInputDialog::getText(
                this, tr("Watch"), tr("Watch Expression:"), QLineEdit::Normal, watchExpression, NULL, Qt::WindowTitleHint | Qt::WindowSystemMenuHint);
            if (!watchExpression.isEmpty())
                DebuggerClientManager::Instance().addWatch(watchExpression);
        }
    }
...
WatchItem* WatchTreeWidget::findWatchItem(boost::shared_ptr<Aya::Scripting::DebuggerWatch> spWatch)
{
    int numChild = topLevelItemCount(), currentChild = 0;
    WatchItem* pWatchItem = NULL;
...
void WatchWidget::addWatch(shared_ptr<Aya::Scripting::DebuggerWatch> spWatch)
{
    m_pWatchTreeWidget->addWatch(spWatch);
}

void WatchWidget::removeWatch(shared_ptr<Aya::Scripting::DebuggerWatch> spWatch)
{
...
void DebuggerToolTipWidget::showToolTip(const QPoint& pos, const QString& key, const Aya::Reflection::Variant& value)
{
...
void DebuggerToolTipWidget::computeToolTipSize()
{
    ...
    if (screen) {
        ...
        // Fallback to primary screen if no screen is found at the position
...
bool populateValues(QTreeWidgetItem* pParentItem, const QString& key, const Aya::Reflection::Variant varValue)
{
    bool result = true;
    QString textToSet;

    if (varValue.isType<boost::shared_ptr<Aya::Instance>>())
    {
        boost::shared_ptr<Aya::Instance> pInstance = varValue.get<boost::shared_ptr<Aya::Instance>>();
        textToSet = pInstance->getName().c_str();

        for (Aya::Reflection::PropertyIterator iter = pInstance->properties_begin(); iter != pInstance->properties_end(); ++iter)
        {
            if (Aya::Reflection::RefPropertyDescriptor::isRefPropertyDescriptor((*iter).getDescriptor()))
            {
                const Aya::Reflection::RefPropertyDescriptor* desc =
                    boost::polymorphic_downcast<const Aya::Reflection::RefPropertyDescriptor*>(&(*iter).getDescriptor());
                Aya::Reflection::DescribedBase* pBase = desc->getRefValue(pInstance.get());
                if (pBase)
                {
                    Aya::Instance* instance = boost::polymorphic_downcast<Aya::Instance*>(pBase);
                    if (instance)
                    {
                        if (!pInstance->contains(instance))
                            populateValues(addChildren(pParentItem), desc->name.c_str(), Aya::shared_from(instance));
                    }
                    else
                    {
    ...
    else if (varValue.isType<Aya::Vector2>())
    {
        // get x, y
        textToSet = Aya::StringConverter<Aya::Vector2>::convertToString(varValue.get<Aya::Vector2>()).c_str();

        Aya::Vector2 vec2 = varValue.get<Aya::Vector2>();

        populateValues(addChildren(pParentItem), "X", vec2.x);
        populateValues(addChildren(pParentItem), "Y", vec2.y);
    }
    else if (varValue.isType<Aya::Vector2int16>())
    {
        // get x, y
        textToSet = Aya::StringConverter<Aya::Vector2int16>::convertToString(varValue.get<Aya::Vector2int16>()).c_str();

        Aya::Vector2int16 vec2 = varValue.get<Aya::Vector2int16>();
        populateValues(addChildren(pParentItem), "X", int(vec2.x));
        populateValues(addChildren(pParentItem), "Y", int(vec2.y));
    }
    else if (varValue.isType<Aya::Vector3>())
    {
        textToSet = Aya::StringConverter<Aya::Vector3>::convertToString(varValue.get<Aya::Vector3>()).c_str();

        Aya::Vector3 vec3 = varValue.get<Aya::Vector3>();
    ...
    {
        textToSet = Aya::StringConverter<G3D::Vector3int16>::convertToString(varValue.get<G3D::Vector3int16>()).c_str();
    ...
    {
        textToSet = Aya::StringConverter<G3D::CoordinateFrame>::convertToString(varValue.get<G3D::CoordinateFrame>()).c_str();

        G3D::CoordinateFrame coordinateFrame = varValue.get<G3D::CoordinateFrame>();
        populateValues(addChildren(pParentItem), "p", Aya::Reflection::Variant(coordinateFrame.translation));
        populateValues(addChildren(pParentItem), "lookVector", Aya::Reflection::Variant(coordinateFrame.lookVector()));
    }
    else if (varValue.isType<Aya::Region3>())
    {
        textToSet = Aya::StringConverter<Aya::Region3>::convertToString(varValue.get<Aya::Region3>()).c_str();

        Aya::Region3 region = varValue.get<Aya::Region3>();
        populateValues(addChildren(pParentItem), "CFrame", Aya::Reflection::Variant(region.getCFrame()));
        populateValues(addChildren(pParentItem), "Size", Aya::Reflection::Variant(region.getSize()));
    }
    else if (varValue.isType<Aya::Rect2D>())
    {
        textToSet = Aya::StringConverter<Aya::Rect2D>::convertToString(varValue.get<Aya::Rect2D>()).c_str();

        Aya::Rect2D rect2D = varValue.get<Aya::Rect2D>();
        populateValues(addChildren(pParentItem), "Min", Aya::Reflection::Variant(rect2D.x0y0()));
        populateValues(addChildren(pParentItem), "Max", Aya::Reflection::Variant(rect2D.x1y1()));
        populateValues(addChildren(pParentItem), "Width", Aya::Reflection::Variant(rect2D.width()));
        populateValues(addChildren(pParentItem), "Height", Aya::Reflection::Variant(rect2D.height()));
    }
    else if (varValue.isType<Aya::Region3int16>())
    {
        textToSet = Aya::StringConverter<Aya::Region3int16>::convertToString(varValue.get<Aya::Region3int16>()).c_str();

        Aya::Region3int16 region = varValue.get<Aya::Region3int16>();
        populateValues(addChildren(pParentItem), "Min", Aya::Reflection::Variant(region.getMinPos()));
        populateValues(addChildren(pParentItem), "Max", Aya::Reflection::Variant(region.getMaxPos()));
    }
    else if (varValue.isType<Aya::BrickColor>())
    {
        textToSet = Aya::StringConverter<Aya::BrickColor>::convertToString(varValue.get<Aya::BrickColor>()).c_str();

        Aya::BrickColor brickColor = varValue.get<Aya::BrickColor>();
        populateValues(addChildren(pParentItem), "Color", Aya::Reflection::Variant(brickColor.color3()));
        populateValues(addChildren(pParentItem), "Name", Aya::Reflection::Variant(std::string(brickColor.name())));
        populateValues(addChildren(pParentItem), "Number", Aya::Reflection::Variant((int)brickColor.number));
    }
    else if (varValue.isType<G3D::Color3>())
    {
        textToSet = Aya::StringConverter<G3D::Color3>::convertToString(varValue.get<G3D::Color3>()).c_str();

        G3D::Color3 color = varValue.get<G3D::Color3>();
        populateValues(addChildren(pParentItem), "r", Aya::Reflection::Variant(color.r));
        populateValues(addChildren(pParentItem), "g", Aya::Reflection::Variant(color.g));
        populateValues(addChildren(pParentItem), "b", Aya::Reflection::Variant(color.b));
    }
    else if (varValue.isType<Aya::RbxRay>())
    {
        textToSet = Aya::StringConverter<Aya::RbxRay>::convertToString(varValue.get<Aya::RbxRay>()).c_str();

        Aya::RbxRay ray = varValue.get<Aya::RbxRay>();
        populateValues(addChildren(pParentItem), "Origin", Aya::Reflection::Variant(ray.origin()));
        populateValues(addChildren(pParentItem), "Direction", Aya::Reflection::Variant(ray.direction()));
    }
    else if (varValue.isType<Aya::UDim>())
    {
        textToSet = Aya::StringConverter<Aya::UDim>::convertToString(varValue.get<Aya::UDim>()).c_str();

        Aya::UDim dim = varValue.get<Aya::UDim>();
        populateValues(addChildren(pParentItem), "Scale", Aya::Reflection::Variant(dim.scale));
        populateValues(addChildren(pParentItem), "Offset", Aya::Reflection::Variant(int(dim.offset)));
    }
    else if (varValue.isType<Aya::UDim2>())
    {
        textToSet = Aya::StringConverter<Aya::UDim2>::convertToString(varValue.get<Aya::UDim2>()).c_str();

        Aya::UDim2 dim2 = varValue.get<Aya::UDim2>();
        populateValues(addChildren(pParentItem), "X", Aya::Reflection::Variant(dim2.x));
        populateValues(addChildren(pParentItem), "Y", Aya::Reflection::Variant(dim2.y));
    }
    else if (varValue.isType<shared_ptr<const Aya::Reflection::ValueMap>>())
    {
        textToSet = "Map";
        shared_ptr<const Aya::Reflection::ValueMap> valMap = varValue.get<shared_ptr<const Aya::Reflection::ValueMap>>();

        for (Aya::Reflection::ValueMap::const_iterator iter = valMap->begin(); iter != valMap->end(); ++iter)
        {
            populateValues(addChildren(pParentItem), QString("[%1]").arg(iter->first.c_str()), iter->second);
        }
    }
    else if (varValue.isType<shared_ptr<const Aya::Reflection::ValueTable>>())
    {
        textToSet = "Table";
        shared_ptr<const Aya::Reflection::ValueTable> valMap = varValue.get<shared_ptr<const Aya::Reflection::ValueTable>>();
        for (Aya::Reflection::ValueTable::const_iterator iter = valMap->begin(); iter != valMap->end(); ++iter)
        {
            populateValues(addChildren(pParentItem), QString("[%1]").arg(iter->first.c_str()), iter->second);
        }
    }
    else if (varValue.isType<shared_ptr<const Aya::Reflection::ValueArray>>())
    {
        textToSet = "Array";
        shared_ptr<const Aya::Reflection::ValueArray> valMap = varValue.get<shared_ptr<const Aya::Reflection::ValueArray>>();
        Aya::Reflection::ValueArray::const_iterator iter = valMap->begin();
    ...
    else if (varValue.isType<shared_ptr<const Aya::Instances>>())
    {
        textToSet = "Instances";
        shared_ptr<const Aya::Instances> valMap = varValue.get<shared_ptr<const Aya::Instances>>();
        Aya::Instances::const_iterator iter = valMap->begin();
    ...
    else if (varValue.isType<shared_ptr<const Aya::Reflection::Tuple>>())
    {
        textToSet = "Tuple";
        shared_ptr<const Aya::Reflection::Tuple> valMap = varValue.get<shared_ptr<const Aya::Reflection::Tuple>>();
        Aya::Reflection::ValueArray::const_iterator iter = valMap->values.begin();
    ...
    else if (varValue.isType<Aya::Lua::WeakFunctionRef>())
    {
        textToSet = "function";
