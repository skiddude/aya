...
{
    AYAASSERT(m_pReply->isFinished()); // Catch weird cases where request was aborted
    delete m_pReply;
}
...
void RobloxNetworkReply::handleRedirection()
{
    ...
    if (!redirectUrl.isEmpty() && m_followRedirections && !m_redirectUrls.contains(redirectUrl)) // Prevent infinite loops
                                                                                                 // TODO: Detect cycles
    {
...
QNetworkReply* RobloxNetworkReply::getLastNetworkReply() const
{
    if (m_childReply)
    {
        AYAASSERT(m_followRedirections);            // will only have a m_childReply if followRedirections is true
