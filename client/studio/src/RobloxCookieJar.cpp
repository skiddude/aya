...
{
    Aya::Http::setCookiesForDomain(domain.toStdString(), QString("%1=%2").arg(QString(cookie.name())).arg(QString(cookie.value())).toStdString());
}
...
{
    for (CookieReadStatusMap::iterator cookiesReadMapIter = m_cookieReadMap.begin(); cookiesReadMapIter != m_cookieReadMap.end();
         ++cookiesReadMapIter)
    {
