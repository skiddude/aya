...
struct Killpr
{
    ...
    {
        return int(a.w + 0.5f) == id;
    }
};



SplineEditor::SplineEditor(QWidget* parent, SplineDef* def, const char* settingsName)
    : Base(parent, Qt::WindowTitleHint | Qt::WindowSystemMenuHint | Qt::FramelessWindowHint)
{
    AYAASSERT(def);
...
void SplineEditor::onCanvasMouse(QMouseEvent* e)
{
    ...
    case QEvent::MouseButtonPress:
    {
        ...
        if (buttons == Qt::LeftButton && m_mode == Free)
        {
            ...
            {
                if (dbgmm)
                    qDebug("MousePress left click - on a point handle");
                ...
                if (getsel() == leftmostPoint())
                    m_Xconstraint = 0;
                else if (getsel() == rightmostPoint())
                    m_Xconstraint = 1;
                else
                    m_Xconstraint = -1;
            ...
            {
                if (dbgmm)
                    qDebug("MousePress left click - on an envelope handle");
            ...
            else if (m_spline->pt.size() < Aya::NumberSequence::kMaxSize) // we're not on a point, create a new point and drag it
            {
                if (dbgmm)
                    qDebug("MousePress left click - not on a point handle");
                float ptx, pty;
                ...
                {
                    if (getsel() == leftmostPoint())
                        pt->x = m_Xconstraint = 0;
                    else if (getsel() == rightmostPoint())
                        pt->x = m_Xconstraint = 1;
                    else
                        m_Xconstraint = -1;
                }
        ...
        {
            if (dbgmm)
                qDebug("MousePress left+right - while dragging");
        ...
        {
            if (dbgmm)
                qDebug("MousePress right click - while newDragging");
    ...
    case QEvent::MouseButtonRelease:
    {
        ...
        {
            if (dbgmm)
                qDebug("MouseRelease - while dragging");
    ...
    case QEvent::MouseMove:
    {
        ...
        case Free:
            if (dbgmm)
                qDebug("MouseMove - free movement");
        ...
        {
            if (dbgmm)
                qDebug("MouseMove - while dragging");
        ...
        {
            if (dbgmm)
                qDebug("MouseMove - while right-dragging");
        ...
        {
            if (dbgmm)
                qDebug("MouseMove - while dragging envelope handle");
            ...
            {
            case Sel_EnvUpper:
                pt->z = (fy - by + m_savePt.z);
                break;
            case Sel_EnvLower:
                pt->z = (by - fy + m_savePt.z);
                break;
            default:
                AYAASSERT(0);
                break;
...
{
    show = show && m_spline->pt.size() < Aya::NumberSequence::kMaxSize;
    if (show)
    {
        float x, y;
...
SplineEditorCanvas::SplineEditorCanvas(QWidget* parent, SplineEditor* owner, SplineDef* spline)
    : QWidget(parent)
    , m_pOwner(owner)
    , m_spline(spline)
{
...
static Aya::NumberSequence::Key convert(const G3D::Vector4& a)
{
    return Aya::NumberSequence::Key(a.x, a.y, a.z);
}

static G3D::Vector4 convert(const Aya::NumberSequence::Key& k, int j)
{
    return G3D::Vector4(k.time, k.value, k.envelope, 0.1f + j);
}

static void convert(SplineDef* dest, const Aya::NumberSequence* src, float ymax)
{
    const std::vector<Aya::NumberSequence::Key>& keys = src->getPoints();
    dest->ymax = ymax;
...
static void convert(Aya::NumberSequence* dest, const SplineDef* src)
{
    std::vector<Aya::NumberSequence::Key> keys;
    keys.reserve(src->pt.size());
    for (int j = 0, e = src->pt.size(); j < e; ++j)
    {
        Aya::NumberSequence::Key key = convert(src->pt[j]);
        if (key.time >= 0.0f && key.time <= 1.0f)
            keys.push_back(key);
    }
    if (!Aya::NumberSequence::validate(keys, false))
        AYAASSERT(Aya::NumberSequence::validate(keys,
            false)); // if this happens, then the editor is supplying a borked curve and this conversion function is not smart enough to filter it
    *dest = keys;
}

SplineEditorAdapter::SplineEditorAdapter(
    const Aya::Reflection::PropertyDescriptor* pPropertyDescriptor, boost::shared_ptr<Aya::DataModel> dataModel, Aya::Instance* inst)
    ...
    , m_inhibitPropChanged(false)
{
    ...
    if (pPropertyDescriptor == &Aya::CustomParticleEmitter::prop_transp)
    {
        ymax = 1;
        settingsName = "Transparency";
    }
    else if (pPropertyDescriptor == &Aya::CustomParticleEmitter::prop_size)
    {
        ymax = 10;
        settingsName = "Size";
    }

    Aya::Reflection::Variant variant;
    m_pPropertyDescriptor->getVariant(m_pInst, variant);
    Aya::NumberSequence ns = variant.get<Aya::NumberSequence>();
...
void SplineEditorAdapter::onEdited(bool undo)
{
    ...
    if (undo)
    {
        Aya::ChangeHistoryService::requestWaypoint(m_pPropertyDescriptor->name.c_str(), m_dataModel.get());
    }

    m_inhibitPropChanged = false;
}

void SplineEditorAdapter::onPropChanged(const Aya::Reflection::PropertyDescriptor* pd)
{
    if (!m_inhibitPropChanged && pd == m_pPropertyDescriptor)
    {
        ...
        QMetaObject::invokeMethod(
            m_pSplineEditor, "refreshData", Qt::QueuedConnection, Q_ARG(void*, sd)); // this sucks, Qt can't into arbitrary types
    }
}

void SplineEditorAdapter::onAncestryChanged(shared_ptr<Aya::Instance>, shared_ptr<Aya::Instance>)
{
    Aya::DataModel* dm = Aya::DataModel::get(m_pInst);
