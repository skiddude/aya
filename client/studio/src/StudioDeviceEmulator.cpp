...
void StudioDeviceEmulator::setMenuIcon(const EmulationDevice& device)
{
    ...
    m_deviceMenu->setIcon(QIcon(QString(":/images/icons/Test/%1").arg(iconLocation)));
}
...
void StudioDeviceEmulator::activateCurrentDevice()
{
    ...
    if (Aya::UserInputService* userInputService = Aya::ServiceProvider::find<Aya::UserInputService>(ide->getDataModel()))
...
QAction* StudioDeviceEmulator::createAction(EmulationDevice device)
{
    ...
    pAction->setIcon(QIcon(QString(":/images/icons/16x16/%1").arg(iconLocation)));
