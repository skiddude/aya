...
class RobloxScriptReview::ScriptPerfUpdateJob : public Aya::DataModelJob
{
public:
    ...
    ScriptPerfUpdateJob(RobloxScriptReview* scriptReview, boost::shared_ptr<Aya::DataModel> dataModel)
        : Aya::DataModelJob("RobloxScriptReview", Aya::DataModelJob::Read, false, dataModel, Aya::Time::Interval(0))
    ...
    : m_pTimer(new QTimer(this))
{
    ...
    // setclickabble? i dont know if this is the equivalent method in qt5
...
void RobloxScriptReview::setDataModel(boost::shared_ptr<Aya::DataModel> pDataModel)
{
...
void RobloxScriptReview::AddValue(const Aya::Reflection::Variant& value)
{
    // DE2502 - Do not show objects with name "Unknown" and count -1
    shared_ptr<const Aya::Reflection::Tuple> tuple = value.cast<shared_ptr<const Aya::Reflection::Tuple>>();
    ...
    if (iter != m_records.end())
    {
        ...
        pRecord->setText(1, Aya::format("%d", tuple->values[2].cast<int>()).c_str());
        pRecord->setText(2, Aya::format("%.3f%%", tuple->values[3].cast<double>()).c_str());
        pRecord->setText(3, Aya::format("%.1f/s", tuple->values[4].cast<double>()).c_str());
    }
    else
    {
...
void RobloxScriptReview::update()
{
    ...
    boost::shared_ptr<Aya::ScriptContext> pScriptContext = Aya::shared_from(Aya::ServiceProvider::create<Aya::ScriptContext>(m_pDataModel.get()));
    if (pScriptContext)
    {
        pScriptContext->setCollectScriptStats(true);
        if (shared_ptr<const Aya::Reflection::Tuple> stats = pScriptContext->getScriptStats())
        {
            Aya::Reflection::ValueArray::const_iterator curIter = stats->values.begin();
...
void RobloxScriptReview::createUpdateItemsJob(boost::shared_ptr<Aya::DataModel> dataModel)
{
    AYAASSERT(!m_pUpdateItemsJob);
