...
QPixmap getBranchIndicator(QStyle::State iconState)
{
    ...
    if (!QPixmapCache::find(QString::number(iconState), &pix))
    {
        ...
        branchOption.rect = QRect(BranchIndicatorBorder, BranchIndicatorBorder, BranchIndicatorSize / 2 + BranchIndicatorBorder,
...
QByteArray getResourceFileBytes(const QString& resourceName)
{
    ...
    AYAASSERT(resource.data());
    AYAASSERT(resource.size() > 0);
...
class RBXMessageBoxStyle : public QProxyStyle
{
};
...
{
    updateWindowTitle(this);
    this->setStyle(new RBXMessageBoxStyle());
}

RBXMessageBox::RBXMessageBox(
    QMessageBox::Icon icon, const QString& title, const QString& text, QMessageBox::StandardButtons buttons, QWidget* parent, Qt::WindowFlags f)
...
void StackedWidgetAnimator::moveIndex(int fromIndex, int toIndex)
{
    ...
    animationGroup->addAnimation(
        getAnimation(fromWidget, QPoint(fromWidgetPos.x(), fromWidgetPos.y()), QPoint(offsetx + fromWidgetPos.x(), offsety + fromWidgetPos.y())));
    animationGroup->addAnimation(
        getAnimation(toWidget, QPoint(-offsetx + toWidgetPos.x(), offsety + toWidgetPos.y()), QPoint(toWidgetPos.x(), toWidgetPos.y())));
