    ...
    : QDialog(parent, Qt::WindowTitleHint | Qt::WindowSystemMenuHint)
{
    ...
    connect(m_SettingsTypesTable, SIGNAL(currentItemChanged(QTableWidgetItem*, QTableWidgetItem*)), this,
        SLOT(onCategorySelected(QTableWidgetItem*, QTableWidgetItem*)));
...
void RobloxSettingsDialog::onReset()
{
    ...
    if (!hasSeenWarning)
    {
        QMessageBox::information(this, "Reset Settings", "Resetting settings will not take effect until you restart Aya");
        hasSeenWarning = true;
    }

    Aya::GlobalAdvancedSettings::singleton()->eraseSettingsStore();
}
...
bool RobloxSettingsDialog::close()
{
    ...
    if (!hasSeenWarning)
    {
        QMessageBox::information(this, "Save Changes", "Some setting changes will not take effect until you restart Aya");
        hasSeenWarning = true;
    }
...
{
    boost::shared_ptr<Aya::Instance> container = Aya::shared_from(Aya::GlobalAdvancedSettings::singleton().get());
    if (!container)
        return;
    ...
    m_descendantAddedConnection =
        container->getOrCreateDescendantAddedSignal()->connect(boost::bind(&RobloxSettingsDialog::onDescendantAdded, this, _1));
    m_descendantRemovingConnection =
        container->getOrCreateDescendantRemovingSignal()->connect(boost::bind(&RobloxSettingsDialog::onDescendantRemoving, this, _1));
    ...
    Aya::Selection* sel = Aya::ServiceProvider::create<Aya::Selection>(Aya::GlobalAdvancedSettings::singleton().get());
...
void RobloxSettingsDialog::onDescendantAdded(boost::shared_ptr<Aya::Instance> child)
{
    if (!child)
        return;

    const Aya::Reflection::ClassDescriptor* pClassDescriptor = &child->getDescriptor();
    if (pClassDescriptor)
    {
...
bool RobloxSettingsDialog::hasProperties(const Aya::Reflection::ClassDescriptor* pClassDescriptor)
{
    if (!pClassDescriptor)
        return false;

    bool retVal = false;
    Aya::Reflection::ClassDescriptor::ClassDescriptors::const_iterator iter = pClassDescriptor->derivedClasses_begin();
    for (; iter != pClassDescriptor->derivedClasses_end(); iter++)
    {
        Aya::Reflection::ClassDescriptor* pDerived = *iter;
        if (pDerived)
    ...
    if (!retVal)
    {
        Aya::Reflection::MemberDescriptorContainer<Aya::Reflection::PropertyDescriptor>::Collection::const_iterator iter =
            pClassDescriptor->begin<Aya::Reflection::PropertyDescriptor>();
        for (; iter != pClassDescriptor->end<Aya::Reflection::PropertyDescriptor>(); iter++)
        {
            const Aya::Reflection::PropertyDescriptor* pPropertyDescriptor = *iter;
            if (pPropertyDescriptor)
            {
                if (pPropertyDescriptor->owner != *pClassDescriptor)
                    continue;

                if (!pPropertyDescriptor->isPublic() || pPropertyDescriptor->security != Aya::Security::None)
                {
                    continue;
                }

                Aya::Reflection::Metadata::Member* pMemeber = Aya::Reflection::Metadata::Reflection::singleton()->get(*pPropertyDescriptor);
                if (Aya::Reflection::Metadata::Item::isDeprecated(pMemeber, *pPropertyDescriptor))
                    continue;
...
void RobloxSettingsDialog::onDescendantRemoving(boost::shared_ptr<Aya::Instance> child)
{
    if (!child)
        return;
...
void RobloxSettingsDialog::onCategorySelected(QTableWidgetItem* current, QTableWidgetItem* previous)
{
    ...
    Aya::Selection* sel = m_SettingsTree->getSelection();
    if (!sel)
        return;
...
void SelectionPropertyTree::addSelectionInstance(boost::shared_ptr<Aya::Instance> pInstance)
{
    if (!pInstance)
        return;
...
void SelectionPropertyTree::removeSelectionInstance(boost::shared_ptr<Aya::Instance> pInstance)
{
    if (!pInstance)
        return;
...
void SelectionPropertyTree::setSelection(Aya::Selection* value)
{
...
void SelectionPropertyTree::onSelectionChanged(const Aya::SelectionChanged& evt)
{
    ...
    if (evt.addedItem)
    {
        ...
        if (evt.addedItem->getName() != StudioSettings && !hasShownWarning)
        {
            QMessageBox::warning(this, "Warning",
                "<p>Changing settings may make Aya unstable.</p>"
                "<p>We recommend that you don't change any non-Studio settings "
                "unless instructed to do so by a Aya engineer.</p>");
...
void SelectionPropertyTree::initProperty(
    const Aya::Reflection::ClassDescriptor* pClassDescriptor, const Aya::Reflection::PropertyDescriptor* pPropertyDescriptor)
{
    ...
    if (pPropertyDescriptor->owner != Aya::Instance::classDescriptor())
    {
        PropertyTreeWidget::initProperty(pClassDescriptor, pPropertyDescriptor);
    }
}

SettingsItem::SettingsItem(boost::shared_ptr<Aya::Instance> inst, QString name)
