    ...
    , m_closeOnMouseRelease(false)
{
    ...
    m_colorsInfo.reserve(Aya::BrickColor::colorPalette().size());
    ...
    for (int row = 0; row < numRows; row++)
        for (int col = 0; col < numCols; col++)
        {
            if ((qAbs(numRows / 2 - row) + qAbs(numCols / 2 - col) < maxPolygonsInARow) && (row % 2 == col % 2))
            {
                ...
                polygon.translate(
                    qSqrt(3) / 2 * (COLOR_POLY_SIDE + COLOR_POLY_DIFF) * col + 18, (COLOR_POLY_SIDE + COLOR_POLY_DIFF) * 1.5 * row + 24);

                Aya::BrickColor bColor = Aya::BrickColor::colorPalette().operator[](numColorsUsed++);
                m_colorsInfo.push_back(ColorKeyInfo(
                    QColor(bColor.color3uint8().r, bColor.color3uint8().g, bColor.color3uint8().b), polygon, bColor.name().c_str(), bColor.asInt()));
            }
        }

    static Aya::BrickColor::Number lowerLineColorIDs[12] = {Aya::BrickColor::brick_199, Aya::BrickColor::brick_194, Aya::BrickColor::roblox_1002,
        Aya::BrickColor::brick_325, Aya::BrickColor::brick_348, Aya::BrickColor::brick_26, Aya::BrickColor::brick_302, Aya::BrickColor::brick_311,
        Aya::BrickColor::brick_320, Aya::BrickColor::brick_335, Aya::BrickColor::roblox_1001, Aya::BrickColor::brick_1};

    int colorCount = Aya::BrickColor::colorPalette().size(), lowerLineColorCount = 0;
    ...
    for (int ii = 0; ii < numCols - 1; ++ii)
    {
        ...
        Aya::BrickColor bColor;
        if (numColorsUsed < colorCount)
            bColor = Aya::BrickColor::colorPalette().operator[](numColorsUsed++);
        else
            bColor = Aya::BrickColor(lowerLineColorIDs[lowerLineColorCount++]);
...
bool ColorPickerFrame::event(QEvent* evt)
{
    if (evt->type() == QEvent::ToolTip)
    {
        ...
        if (colorIndex >= 0)
        {
            Aya::BrickColor color(m_colorsInfo[colorIndex].colorValue);
...
{
    int colorCount = Aya::BrickColor::colorPalette().size();
    int temp = 3, result = 0;
    while (true)
    {
...
static QPolygonF getPolygonFromSide(int side)
{
    ...
    polygon << QPointF(dx, -side / 2) << QPointF(0, -side) << QPointF(-dx, -side / 2) << QPointF(-dx, side / 2) << QPointF(0, side)
            << QPointF(dx, side / 2) << QPointF(dx, -side / 2);
    return polygon;
}
...
QPolygonF ColorPickerFrame::getPolygon(PolygonSize polySize)
{
    ...
    AYAASSERT(polySize == PolygonSize_Normal);
...
{
    m_materialValues.append(Aya::PLASTIC_MATERIAL);
    m_materialValues.append(Aya::WOOD_MATERIAL);
    m_materialValues.append(Aya::WOODPLANKS_MATERIAL);
    m_materialValues.append(Aya::SLATE_MATERIAL);
    m_materialValues.append(Aya::CONCRETE_MATERIAL);
    m_materialValues.append(Aya::METAL_MATERIAL);
    m_materialValues.append(Aya::RUST_MATERIAL);
    m_materialValues.append(Aya::DIAMONDPLATE_MATERIAL);
    m_materialValues.append(Aya::ALUMINUM_MATERIAL);
    m_materialValues.append(Aya::GRASS_MATERIAL);
    m_materialValues.append(Aya::ICE_MATERIAL);
    m_materialValues.append(Aya::BRICK_MATERIAL);
    m_materialValues.append(Aya::SAND_MATERIAL);
    m_materialValues.append(Aya::FABRIC_MATERIAL);
    m_materialValues.append(Aya::GRANITE_MATERIAL);
    m_materialValues.append(Aya::MARBLE_MATERIAL);
    m_materialValues.append(Aya::PEBBLE_MATERIAL);
    m_materialValues.append(Aya::COBBLESTONE_MATERIAL);
    m_materialValues.append(Aya::SMOOTH_PLASTIC_MATERIAL);
    ...
    if (evt->type() == QEvent::ToolTip)
    {
        ...
        if (materialIndex >= 0)
        {
            std::string materialName =
                Aya::Reflection::EnumDesc<Aya::PartMaterial>::singleton().convertToString((Aya::PartMaterial)m_materialValues[materialIndex]);
...
int MaterialPickerFrame::getMaterialIndex(const QPoint& pos)
{
    ...
    {
        int materialIndex =
            ((pos.x() - MARGIN_X) / (MAT_IMAGE_SIZE + MATERIAL_SPACE)) + MATERIAL_COLS * ((pos.y() - MARGIN_Y) / (MAT_IMAGE_SIZE + MATERIAL_SPACE));
...
{
    QPoint location;
    QScreen* screen = QGuiApplication::screenAt(mapToGlobal(QPoint(0, 0))); // Corrected line
    ...
    if (mapToGlobal(QPoint(0, rect.bottom())).y() + pickerFrameSize.height() <= screen->geometry().height())
        location = mapToGlobal(rect.bottomLeft());
    else
        location = mapToGlobal(rect.topLeft() - QPoint(0, pickerFrameSize.height()));

    location.rx() = qMax(screen->geometry().left(), qMin(location.x(), screen->geometry().right() - pickerFrameSize.width()));
    location.ry() += 1;
...
PickerFrame* ColorPickerToolButton::getPickerFrame()
{
    ...
    m_pPickerFrame->setSelectedItem(Aya::ColorVerb::getCurrentColor().asInt());
...
PickerFrame* FillColorPickerToolButton::getPickerFrame()
{
    ...
    {
        m_pPickerFrame->setSelectedItem(Aya::FillTool::color.get().asInt());
    }
...
PopupLaunchEditor::PopupLaunchEditor(
    PropertyItem* pParentItem, QWidget* pParent, const QIcon& itemIcon, const QString& labelText, int buttonWidth, QWidget* proxyWidget)
    : QWidget(pParent)
...
void PopupLaunchEditor::onButtonClicked()
{
    if (m_pParentItem)
    {
        ...
        auto treeWidget = m_pParentItem->getTreeWidget();
        if (treeWidget)
        {
            treeWidget->closeEditor(this, QAbstractItemDelegate::NoHint);
        }
    }
}
...
QString ImagePickerWidget::getImageUrl(const QString& lastValue, const QPoint& location, boost::shared_ptr<Aya::DataModel> dm)
{
    Aya::ContentProvider* cp = dm->find<Aya::ContentProvider>();

    resultString = lastValue;

    std::vector<QString> imageNames;
    UpdateUIManager::Instance().getViewWidget<RobloxGameExplorer>(eDW_GAME_EXPLORER).getListOfImages(&imageNames);
    ...
    QObject::connect(lineEdit, SIGNAL(returnPressed()), this, SLOT(acceptLineEditValue()));
    ...
    for (size_t i = 0; i < imageNames.size(); ++i)
    {
        ...
        if (shared_ptr<const std::string> data = cp->requestContentString(Aya::ContentId(url.toStdString()), Aya::ContentProvider::PRIORITY_MFC))
        {
            QPixmap p;
    ...
    QObject::connect(scrollList, SIGNAL(itemClicked(QListWidgetItem*)), this, SLOT(handleListItemClicked(QListWidgetItem*)));
...
{
    QScreen* screenAtLocation = QGuiApplication::screenAt(location);
    QRect monitorGeometry;

    if (screenAtLocation)
    {
        monitorGeometry = screenAtLocation->geometry();
    }
    else
    {
        // Fallback to primary screen if no screen is found at the location
        QScreen* primaryScreen = QGuiApplication::primaryScreen();
        if (primaryScreen)
        {
            monitorGeometry = primaryScreen->geometry();
        }
    }
    ...
    return QPoint(
        std::max(minLocation.x(), std::min(location.x(), maxLocation.x())), std::max(minLocation.y(), std::min(location.y(), maxLocation.y())));
}
...
NumberSequencePropertyWidget::NumberSequencePropertyWidget(QWidget* parent, Callbacks* c)
    : QWidget(parent)
    , cb(c)
{
...
PartSelectionTool::PartSelectionTool(Aya::Workspace* workspace)
    : Aya::Named<Aya::MouseCommand, sPartSelectionTool>(workspace)
    , m_CursorName("ArrowFarCursor")
    , m_bIsValidPartSelected(false)
{
}

void PartSelectionTool::onMouseHover(const shared_ptr<Aya::InputObject>& inputObject)
{
    bool isValid = false;
    Aya::PartInstance* pPartHoveredOver = getPart(workspace, inputObject);
    partHoveredOverSignal(pPartHoveredOver, isValid);

    setHighlightPart(isValid ? shared_from(pPartHoveredOver) : boost::shared_ptr<Aya::PartInstance>());
}

shared_ptr<Aya::MouseCommand> PartSelectionTool::onMouseDown(const shared_ptr<Aya::InputObject>& inputObject)
{
    // first emit signal
    ...
    return shared_ptr<Aya::MouseCommand>();
}

shared_ptr<Aya::MouseCommand> PartSelectionTool::onMouseUp(const shared_ptr<Aya::InputObject>& inputObject)
{
    if (!m_bIsValidPartSelected)
        return shared_from(this);
    return shared_ptr<Aya::MouseCommand>();
...
void PartSelectionTool::render3dAdorn(Aya::Adorn* adorn)
{
    Aya::MouseCommand::render3dAdorn(adorn);
    if (m_pHighlightPart)
    {
        Aya::Draw::selectionBox(
            m_pHighlightPart->getPart(), adorn, Aya::ModelInstance::primaryPartHoverOverColor(), Aya::ModelInstance::primaryPartLineThickness());
    }
}
...
PrimaryPartSelectionEditor::PrimaryPartSelectionEditor(
    QWidget* parent, PropertyItem* pPropertyItem, const QString& labelText, boost::shared_ptr<Aya::ModelInstance> instance)
    : QWidget(parent)
    ...
    , m_pModelInstance(instance)
{
    ...
    pRemoveButton->setIcon(QIcon(":/images/icons/16x16/delete_x_16_h.png"));
    ...
    Aya::Workspace* workspace = RobloxDocManager::Instance().getPlayDoc()->getDataModel()->getWorkspace();
    m_pSelectionTool = Aya::Creatable<Aya::MouseCommand>::create<PartSelectionTool>(workspace);
    ...
    m_cPartHoveredOverConnection =
        m_pSelectionTool->partHoveredOverSignal.connect(boost::bind(&PrimaryPartSelectionEditor::onPartHoveredOver, this, _1, _2));
...
PrimaryPartSelectionEditor::~PrimaryPartSelectionEditor()
{
    ...
    if (RobloxDocManager::Instance().getPlayDoc())
    {
        ...
        {
            workspace->setMouseCommand(boost::shared_ptr<Aya::MouseCommand>());
...
bool PrimaryPartSelectionEditor::eventFilter(QObject* watched, QEvent* evt)
{
    if ((evt->type() == QEvent::ActionChanged) && (RobloxDocManager::Instance().getPlayDoc()))
    {
        ...
        static QString mouseCommandActions(
            "advArrowToolAction actionFillColor actionMaterial anchorAction glueSurfaceAction "
...
void PrimaryPartSelectionEditor::onPartHoveredOver(Aya::PartInstance* pPart, bool& isValid)
{
...
void PrimaryPartSelectionEditor::onInstanceSelected(boost::shared_ptr<Aya::Instance> instance)
{
    setPrimaryPart(Aya::Instance::fastDynamicCast<Aya::PartInstance>(instance.get()));
}

void PrimaryPartSelectionEditor::onInstanceHovered(boost::shared_ptr<Aya::Instance> instance)
{
    bool isValid = false;
    onPartHoveredOver(Aya::Instance::fastDynamicCast<Aya::PartInstance>(instance.get()), isValid);
}

void PrimaryPartSelectionEditor::onPartSelected(Aya::PartInstance* pPart)
{
...
bool PrimaryPartSelectionEditor::setPrimaryPart(Aya::PartInstance* pPart)
{
    if (pPart && pPart->isDescendantOf(m_pModelInstance.get()))
    {
        if (pPart == m_pModelInstance->getPrimaryPartSetByUser())
        {
            Aya::StandardOut::singleton()->print(Aya::MESSAGE_ERROR, "Selected part is already set as primary part.");
            return false;
        }

        AYAASSERT(RobloxDocManager::Instance().getPlayDoc() && RobloxDocManager::Instance().getPlayDoc()->getDataModel());
        Aya::DataModel::LegacyLock lock(RobloxDocManager::Instance().getPlayDoc()->getDataModel(), Aya::DataModelJob::Write);
...
{
    QApplication::restoreOverrideCursor();
    QPixmap pixmap(Aya::ContentProvider::getAssetFile(cursorImage).c_str());
    if (!pixmap.isNull())
    {
