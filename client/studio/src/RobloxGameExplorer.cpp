...
Q_DECLARE_METATYPE(Aya::HttpFuture);
...
const CategoryWebSettings kWebSettings[ENTITY_CATEGORY_MAX] = {
    // Places
    {"Places", "PlaceId", "ID", "%API_PROXY%/universes/get-universe-places?universeId=%UNIVERSE_ID%&page=%ITEM_ID%",
        "%WEB_BASE%/ide/places/create?universeId=%UNIVERSE_ID%", "%WEB_BASE%/ide/places/%ITEM_ID%/updatesettings",
        "%WEB_BASE%/universes/removeplace?universeId=%UNIVERSE_ID%&placeId=%ITEM_ID%", "%WEB_BASE%/asset/?id=%ITEM_ID%",
        "%DATA_BASE%/Data/Upload.ashx?assetid=%ITEM_ID%", &placesPropertiesFixer},
    // Developer products
    {
        "DeveloperProducts", "ProductId", "ProductId", "%API_PROXY%/developerproducts/list?universeId=%UNIVERSE_ID%&page=%ITEM_ID%",
        "%API_PROXY%/developerproducts/add?universeId=%UNIVERSE_ID%", "%API_PROXY%/developerproducts/update?universeId=%UNIVERSE_ID%",
        "",  // remove not supported
        "",  // dev products don't have content to get/set
        "",  // no content, so no content write url
        NULL // no properties fixer
    },
    {"Places", "PlaceId", "BadgeId", "%API_PROXY%/universes/get-universe-places?universeId=%UNIVERSE_ID%&page=%ITEM_ID%", "", "", "", "", "",
        &badgesPropertiesFixer},
    // Named assets
    {"Aliases", "Name", "Name", "%API_PROXY%/universes/get-aliases?universeId=%UNIVERSE_ID%&page=%ITEM_ID%",
...
const CategoryWebSettings badgesWebSettings = {
    "GameBadges", "PlaceId", "BadgeId", "%WEB_BASE%/badges/list-badges-for-place/json?placeId=%ITEM_ID%", "", "", "", "", "", NULL};
...
QVariant getEntityId(EntityProperties* settings, EntityCategory category)
{
    ...
    if (settings)
    {
        ...
        AYAASSERT(intId.is_initialized() != stringId.is_initialized());
        if (intId)
        {
            id = intId.get();
        }
...
{
    QVariant result;
    result.setValue<boost::function<void()>>(std::move(val));
    return result;
}
...
{
    QVariant result;
    result.setValue<EntityProperties*>(std::move(val));
    return result;
}
...
{
    QVariant result;
    result.setValue<boost::function<void(const QPoint&)>>(std::move(val));
    return result;
}
...
void blockingMakeRoot(int gameId, int placeId, bool* success)
{
    try
    {
        ...
        HttpAsync::post(formatUrl("%WEB_BASE%/universes/setrootplace?universeid=%UNIVERSE_ID%&placeid=%ITEM_ID%", gameId, placeId), d).get();
    }
...
void blockingUnMakeRoot(int gameId, int placeId, bool* success)
{
    ...
    {
        HttpPostData d("", Http::kContentTypeDefaultUnspecified, false);
        HttpAsync::post(formatUrl("%WEB_BASE%/universes/unrootplace?universeid=%UNIVERSE_ID%&placeid=%ITEM_ID%", gameId, placeId), d).get();
    }
    catch (const Aya::base_exception& e)
    {
...
void placesPropertiesFixer(int universeId, EntityProperties* properties)
{
    ...
    properties->setFromJsonFuture(HttpAsync::get(formatUrl("%API_PROXY%/Marketplace/ProductInfo?assetId=%ITEM_ID%", universeId, id), options));
    ...
    try
    {
        ...
        boost::function<shared_ptr<const ValueTable>()> f = extractProperty<shared_ptr<const ValueTable>>(
            HttpAsync::get(formatUrl("%WEB_BASE%/places/%ITEM_ID%/settings", universeId, id), options), "Creator");
...
void badgesPropertiesFixer(int universeId, EntityProperties* properties)
{
    ...
    HttpOptions options;
    options.setDoNotUseCachedResponse();
    boost::function<shared_ptr<const ValueArray>()> f = extractProperty<shared_ptr<const ValueArray>>(
        HttpAsync::get(formatUrl(badgesWebSettings.fetchPageUrlFormatString, universeId, id), options), badgesWebSettings.jsonItemGroupFieldName);
...
void namedAssetsPropertiesFixer(int universeId, EntityProperties* properties)
{
    if (boost::optional<shared_ptr<const ValueTable>> optAssetInfo = properties->get<shared_ptr<const ValueTable>>("Asset"))
    {
...
QString namedImagesDisplayNameGetter(EntityProperties* settings)
{
    ...
    AYAASSERT(name.startsWith(kImageAssetNamePrefix));
...
void findAvailableImageNameFromFilename(const QString& fileName, const std::vector<QString>* usedNames, QString* availableImageName)
{
    ...
    *availableImageName = completedBase;
    ...
    for (size_t attempt = 0; attempt <= usedNames->size() && !validateImageName(*availableImageName, usedNames, &unusedErrorMessage); ++attempt)
    {
        *availableImageName = QString("%1 (%2)").arg(completedBase).arg(attempt + 1);
    }
}

void createImageAssetAndNameThread(const QString& name, const QString& imageFileName, int currentGameId, boost::optional<int> currentGameGroupId,
    bool* created, QString* errorMessage)
{
    EntityProperties createImageAssetResponse;
    HttpPostData postData(shared_ptr<std::istream>(new std::ifstream(imageFileName.toStdString().c_str(), std::ios::binary)),
    ...
    Http http(formatUrl(kWebSettings[ENTITY_CATEGORY_NamedAssets].createFormatString, currentGameId, name));
...
void bulkAddNewImageNamesThread(
    std::vector<std::pair<QString, QString>>* fileAndImageNames, int currentGameId, boost::optional<int> currentGameGroupId)
{
    ...
    {
        threadGroup.create_thread(boost::bind(&createImageAssetAndNameThread, kImageAssetNamePrefix + (*fileAndImageNames)[i].second,
            (*fileAndImageNames)[i].first, currentGameId, currentGameGroupId, (bool*)(&results[i]), &errorMessages[i]));
    }
...
void updateNameThread(const std::string& properties, const std::string& updateUrl, bool* success, std::string* responseValue)
{
...
{
    if (name == properties->get<std::string>("Name").get() && assetType == properties->get<int>("AssetTypeId").get())
    {
        return properties;
    }
    else
...
{
    return QDir(QCoreApplication::applicationDirPath() + QString("/data/studio/unpublished-changes/%1/").arg(universeId)).absolutePath() + "/";
}
...
void removeFile(QString path, HttpFuture future)
{
    ...
    {
        StandardOut::singleton()->printf(MESSAGE_ERROR, "Error while publishing script: %s", e.what());
    }
}
...
boost::optional<int> getGroupIdFromPlaceEntityProperties(EntityPropertiesForCategory::ref categoryProperties)
{
    ...
    categoryProperties->transform<EntityProperties*>(boost::bind(&getGroupIdFromPlaceEntityPropertyHelper, &groupId, _1), &unused);
    return groupId;
}
...
void EntityProperties::setFromEntitySettings(EntityProperties& other)
{
    ...
    AYAASSERT(!other.hasPendingFuture);
    AYAASSERT(other.ableToGetFuture);
    AYAASSERT(other.ableToParseJson);
...
boost::optional<Variant> EntityProperties::getVariant(const std::string& fieldName)
{
    ...
    Aya::Reflection::ValueTable::iterator itr = data.find(fieldName);
    if (itr != data.end())
    {
        result = itr->second;
    }
...
template<class IdType>
void EntityPropertiesForCategory::publishTo(
    ref target, const std::vector<std::string>& ignoreFieldsWhenCopying, boost::unordered_map<IdType, boost::function<IdType()>>* outputRemaps)
{
    waitUntilLoaded();
    target->waitUntilLoaded();
    AYAASSERT(&webSettings == &target->webSettings);
    ...
    for (EntityCollection::iterator itr = data.begin(); itr != data.end(); ++itr)
    {
        ...
        bool found = false;
        for (EntityCollection::iterator targetItr = target->data.begin(); !found && targetItr != target->data.end(); ++targetItr)
        {
            const boost::optional<IdType> targetId = targetItr->get<IdType>(webSettings.perItemIdFieldName);
            AYAASSERT(targetId);
            if (usedTargetIds.count(targetId.get()) == 0 && targetItr->get<std::string>("Name").get() == name.get())
            {
                found = true;
                ...
                for (size_t i = 0; i < ignoreFieldsWhenCopying.size(); ++i)
                {
                    ...
                    AYAASSERT(valueToKeep.is_initialized());
                ...
                danglingFutures.push_back(HttpAsync::post(formatUrl(webSettings.updateFormatString, target->universeId, targetId.get()), postData));
            }
        }
        if (!found)
        {
            ...
            idRemaps[id.get()] = extractProperty<IdType>(
                HttpAsync::post(formatUrl(webSettings.createFormatString, target->universeId), postData), webSettings.jsonItemIdFieldName);
        }
    }
    ...
    if (!webSettings.removeFormatString.empty())
    {
        for (EntityCollection::iterator itr = target->data.begin(); itr != target->data.end(); ++itr)
        {
            ...
            if (usedTargetIds.count(targetId.get()) == 0)
            {
                ...
                danglingFutures.push_back(HttpAsync::post(formatUrl(webSettings.removeFormatString, target->universeId, targetId.get()), postData));
            }
        }
    }
...
void EntityPropertiesForCategory::handlePage(int loadedPage, std::string* json, std::exception* error)
{
    ...
    {
        StandardOut::singleton()->printf(MESSAGE_ERROR, "Error while loading entities for universe %d: %s", universeId, errorStatus.c_str());
        return;
    }
    ...
    try
    {
        shared_ptr<const ValueArray> jsonList = topLevel->at(webSettings.jsonItemGroupFieldName).cast<shared_ptr<const ValueArray>>();
...
void AddImageDialog::createImageAndNameThread()
{
    ...
    createImageAssetAndNameThread(name, fileNameLabel->text(), currentGameId, currentGameGroupId, created, &errorMessage);
    if (*created)
    {
        *newName = name;
    }
...
void AddImageDialog::openFileSelector()
{
    ...
    QString fileName = QDir::toNativeSeparators(
        QFileDialog::getOpenFileName(dialog, "Choose image to upload", imageLastDir, "Images (*.jpg *.jpeg *.png *.gif *.bmp);;All Files (*.*)"));
...
void AddImageDialog::createButtonClicked()
{
    ...
    {
        UpdateUIManager::Instance().waitForLongProcess("Creating asset name...", boost::bind(&AddImageDialog::createImageAndNameThread, this));

        if (*created)
        {
            UpdateUIManager::Instance().getViewWidget<RobloxGameExplorer>(eDW_GAME_EXPLORER).reloadDataFromWeb();
    ...
    , startPlaceOpenRequested(false)
{
    if (!FFlag::GameExplorerUseV2AliasEndpoint)
    {
        ...
        namedAssetSettings->createFormatString = "%API_PROXY%/universes/create-alias?universeId=%UNIVERSE_ID%";
        namedAssetSettings->updateFormatString = "%API_PROXY%/universes/update-alias?universeId=%UNIVERSE_ID%&oldName=%ITEM_ID%";
    }

    CategoryUiSettings placeSettings = {"Places", QtUtilities::getPixmap(QString::fromStdString(GetAssetFolder()) + "/textures/ClassImages.PNG", 19), &generalDisplayNameGetter,
        NULL, // shouldBuildExplorerItemCallback
        boost::bind(&RobloxGameExplorer::placeDoubleClickCallback, this, _1), boost::bind(&RobloxGameExplorer::placeContextMenuHandler, this, _1, _2),
        boost::bind(&RobloxGameExplorer::placeGroupContextMenuHandler, this, _1),
        boost::bind(&RobloxGameExplorer::checkRowForNameUpdate, this, ENTITY_CATEGORY_Places, _1), CategoryUiSettings::FlatNames};
    uiSettings[ENTITY_CATEGORY_Places] = placeSettings;

    CategoryUiSettings devProductSettings = {"Developer Products", QPixmap(":/images/DeveloperProduct.png"), &generalDisplayNameGetter,
        NULL, // shouldBuildExplorerItemCallback
        ...
        boost::bind(&RobloxGameExplorer::checkRowForNameUpdate, this, ENTITY_CATEGORY_DeveloperProducts, _1), CategoryUiSettings::FlatNames};
    uiSettings[ENTITY_CATEGORY_DeveloperProducts] = devProductSettings;

    CategoryUiSettings badgeSettings = {"Badges", QPixmap(":/16x16/images/Studio 2.0 icons/16x16/badges.png"), &generalDisplayNameGetter,
        NULL, // shouldBuildExplorerItemCallback
        NULL, // double click callback
        boost::bind(&RobloxGameExplorer::badgesContextMenuHandler, this, _1, _2), NULL, NULL, CategoryUiSettings::FlatNames};
    uiSettings[ENTITY_CATEGORY_Badges] = badgeSettings;

    CategoryUiSettings namedAssetSettings = {"Assets", QtUtilities::getPixmap(QString::fromStdString(GetAssetFolder()) + "/textures/ClassImages.PNG", 52), &generalDisplayNameGetter,
        &namedAssetsShouldBuildExplorerItemCallback, boost::bind(&RobloxGameExplorer::namedAssetsDoubleClickCallback, this, _1),
        ...
        boost::bind(&RobloxGameExplorer::checkRowForNameUpdate, this, ENTITY_CATEGORY_NamedAssets, _1), CategoryUiSettings::NameAsFolderPath};
    ...
    connect(gameNameEdit, SIGNAL(editingFinished()), this, SLOT(doneEditingUniverseName()));
    connect(reloadButton, SIGNAL(clicked()), this, SLOT(reloadDataFromWeb()));
    connect(this, SIGNAL(doubleClicked(QModelIndex)), this, SLOT(doubleClickHandler(QModelIndex)));
    connect(this, SIGNAL(customContextMenuRequested(const QPoint&)), this, SLOT(contextMenuHandler(const QPoint&)));
    connect(getModel(), SIGNAL(itemChanged(QStandardItem*)), this, SLOT(itemChangedHandler(QStandardItem*)));
}

boost::optional<std::string> RobloxGameExplorer::findCachedAssetOrEmpty(const Aya::ContentId& contentId, int universeId)
{
...
void RobloxGameExplorer::updateAsset(const Aya::ContentId& contentId, const std::string& newContents)
{
    AYAASSERT(contentId.isNamedAsset());
    AYAASSERT(currentGameId != kNoGameLoaded);
...
void RobloxGameExplorer::publishNamedAssetsToCurrentSlot()
{
    ...
    QMetaObject::invokeMethod(this, "refreshNamedScriptIcons", Qt::QueuedConnection, Q_ARG(int, currentSessionId));
}

HttpFuture RobloxGameExplorer::publishScriptAsset(
    const std::string& scriptText, boost::optional<int> optionalAssetId, boost::optional<int> optionalGroupId)
{
    ...
    QString uploadUrl;
    uploadUrl = QString::fromStdString(
        formatUrl("%DATA_BASE%/Data/Upload.ashx?type=Lua&name=Script&description=Script&ispublic=false&json=1&assetId=%ITEM_ID%", -1, assetId));
...
void RobloxGameExplorer::getListOfImages(std::vector<QString>* out)
{
    ...
    entitySettings[ENTITY_CATEGORY_NamedAssets]->transform<QString>(boost::bind(&getNameOrEmptyIfNotAssetType, ASSET_TYPE_ID_Image, _1), out);
    out->erase(std::remove_if(out->begin(), out->end(), boost::bind(&QString::isEmpty, _1)), out->end());
}
...
void RobloxGameExplorer::getListOfScripts(std::vector<QString>* out)
{
    ...
    entitySettings[ENTITY_CATEGORY_NamedAssets]->transform<QString>(boost::bind(&getNameOrEmptyIfNotAssetType, ASSET_TYPE_ID_Script, _1), out);
    out->erase(std::remove_if(out->begin(), out->end(), boost::bind(&QString::isEmpty, _1)), out->end());
}
...
void RobloxGameExplorer::getListOfAnimations(std::vector<QString>* out)
{
    ...
    entitySettings[ENTITY_CATEGORY_NamedAssets]->transform<QString>(boost::bind(&getNameOrEmptyIfNotAssetType, ASSET_TYPE_ID_Animation, _1), out);
    out->erase(std::remove_if(out->begin(), out->end(), boost::bind(&QString::isEmpty, _1)), out->end());
}
...
void RobloxGameExplorer::openGameFromGameId(int gameId)
{
    ...
    if (gameId != currentGameId && ideDoc)
    {
        ...
        AYAASSERT((currentPlaceId == kNoGameLoaded) == (currentGameId == kNoGameLoaded));
        ...
        AYAASSERT(state == QMessageBox::Yes);
...
void RobloxGameExplorer::openGameFromPlaceId(int placeId)
{
    ...
    Http getUniverse(formatUrl("%API_PROXY%/universes/get-universe-containing-place?placeid=%ITEM_ID%", -1, placeId));
    getUniverse.doNotUseCachedResponse = true;
    getUniverse.get(
        boost::bind(&RobloxGameExplorer::handleUniverseForPlaceResponse, QPointer<RobloxGameExplorer>(this), currentSessionId, placeId, _1, _2));
}
...
std::pair<int, boost::function<std::string()>> RobloxGameExplorer::buildPlaceContentPair(EntityProperties* properties)
{
            ...
            if (shared_ptr<DataModel> dm = ideDoc->getEditDataModel())
            {
                ...
                catch (const std::exception& e)
                {
                    StandardOut::singleton()->printf(
                        MESSAGE_ERROR, "Error when saving current place: %s. Continuing to publish with last known copy of this place.", e.what());
                }
            }
        }
    }
    if (!useLocalContent)
    {
        ...
        options.setDoNotUseCachedResponse();
        result.second =
            boost::bind(&HttpFuture::get, HttpAsync::get(formatUrl(placeWebSettings.getContentFormatString, currentGameId, result.first)));
    }

    return result;
}

RobloxGameExplorer::NamedAssetCopyInfo RobloxGameExplorer::buildNamedContentInfo(EntityPropertiesForCategory::ref targetProperties,
    int sourceUniverseId, boost::optional<int> targetUniverseGroupId, boost::unordered_map<int, boost::function<int()>>* devProductIdRemap,
    EntityProperties* properties)
{
    ...
    AYAASSERT(sourceAssetTypeId == (int)ASSET_TYPE_ID_Script);
    ...
    targetProperties->transform<EntityProperties*>(
        boost::bind(&getPropertiesIfNameAndAssetTypeMatch, boost::ref(result.assetName), sourceAssetTypeId, _1), &foundProperty);
    ...
    {
        StandardOut::singleton()->printf(MESSAGE_ERROR, "Failed to update asset %s", result.assetName.c_str());
    ...
    try
    {
        *targetUniverseId = newUniverseFuture();
        ...
        UniverseSettings targetUniverseSettings;
        {
            ...
            targetUniverseSettings.setFromJsonFuture(
                HttpAsync::get(formatUrl("%API_PROXY%/universes/get-info?universeId=%UNIVERSE_ID%", *targetUniverseId), getOptions));
        }
        ...
        EntityPropertiesForCategory::ref targetPlaces = EntityPropertiesForCategory::loadFromWeb(
            kWebSettings[ENTITY_CATEGORY_Places], *targetUniverseId, boost::bind(&CEvent::Set, &targetPlacesLoadedEvent));
        ...
        EntityPropertiesForCategory::ref targetDevProducts = EntityPropertiesForCategory::loadFromWeb(
            kWebSettings[ENTITY_CATEGORY_DeveloperProducts], *targetUniverseId, boost::bind(&CEvent::Set, &devProductsLoadedEvent));
        ...
        if (!FFlag::GameExplorerUseV2AliasEndpoint)
        {
            targetNamedAssets = EntityPropertiesForCategory::loadFromWeb(
                kWebSettings[ENTITY_CATEGORY_NamedAssets], *targetUniverseId, boost::bind(&CEvent::Set, &namedAssetsLoadedEvent));
        }
        ...
        entitySettings[ENTITY_CATEGORY_NamedAssets]->publishTo<std::string>(targetNamedAssets, std::vector<std::string>(), NULL);
        ...
        for (size_t i = 0; i < placeContentFutures.size(); ++i)
        {
            ...
            danglingFutures.push_back(HttpAsync::post(formatUrl(kWebSettings[ENTITY_CATEGORY_Places].writeContentFormatString, *targetUniverseId,
        ...
        entitySettings[ENTITY_CATEGORY_NamedAssets]->transform<NamedAssetCopyInfo>(
            boost::bind(&RobloxGameExplorer::buildNamedContentInfo, this, targetNamedAssets, currentGameId, targetGroupId, &devProductIdRemap, _1),
            &namedAssetCopyInfos);
        ...
        for (size_t i = 0; i < namedAssetCopyInfos.size(); ++i)
        {
            ...
            if (info.needsTargetIdUpdate)
            {
                if (!FFlag::GameExplorerUseV2AliasEndpoint)
                {
                    ...
                    {
                        throw std::runtime_error(format("Unable to update asset \"%s\", aborting publish", info.assetName.c_str()));
                    }
                }
                ...
                danglingFutures.push_back(HttpAsync::post(
                    formatUrl(kWebSettings[ENTITY_CATEGORY_NamedAssets].updateFormatString, *targetUniverseId, info.assetName), postData));
            }
        }
...
void RobloxGameExplorer::handleUniverseForPlaceResponse(
    QPointer<RobloxGameExplorer> explorer, int originatingSessionId, int placeId, std::string* result, std::exception* exception)
{
    if (explorer.isNull())
        return;
    if (explorer->currentSessionId != originatingSessionId)
        return;

    Variant var;
    if (result && WebParser::parseJSONObject(*result, var) && var.isType<shared_ptr<const ValueTable>>())
    {
        ...
        if (itr != table->end() && !itr->second.isVoid() && itr->second.isNumber())
        {
            ...
            QMetaObject::invokeMethod(explorer, "doOpenGameFromGameId", Qt::QueuedConnection, Q_ARG(int, gameId));
        }
    }
}
...
void RobloxGameExplorer::doneLoadingDataForCategory(QPointer<RobloxGameExplorer> explorer, int originatingSessionId, EntityCategory category)
{
    ...
    if (!explorer->entitySettings[category]->isLoadedAndParsed() || !explorer->gameSettings.isLoadedAndParsed())
    {
        QMetaObject::invokeMethod(explorer, "failedToLoadSettings", Qt::QueuedConnection, Q_ARG(int, originatingSessionId));
        return;
    }

    QMetaObject::invokeMethod(explorer, "populateWithLoadedData", Qt::QueuedConnection, Q_ARG(int, originatingSessionId), Q_ARG(int, category));

    if (category == ENTITY_CATEGORY_Places)
    {
        QMetaObject::invokeMethod(explorer, "afterPlacesLoadedFinished", Qt::QueuedConnection, Q_ARG(int, originatingSessionId));
    }

    if (category == ENTITY_CATEGORY_NamedAssets)
    {
        QMetaObject::invokeMethod(explorer, "afterNamedAssetsFinished", Qt::QueuedConnection, Q_ARG(int, originatingSessionId));
    }

    if (category == ENTITY_CATEGORY_Badges)
    {
        QMetaObject::invokeMethod(explorer, "afterBadgesFinished", Qt::QueuedConnection, Q_ARG(int, originatingSessionId));
    }
}
...
{
    QModelIndexList resultList = getModel()->match(getModel()->index(0, 0), ROLE_CategoryIfGroupRoot, category, 1, Qt::MatchExactly);
    AYAASSERT(resultList.size() <= 1);
...
void RobloxGameExplorer::namedAssetsDoubleClickCallback(EntityProperties* properties)
{
    ...
    {
        RobloxDocManager::Instance().openDoc(
            LuaSourceBuffer::fromContentId(ContentId::fromGameAssetName(properties->get<std::string>("Name").get())));
    }
}
...
void RobloxGameExplorer::openPlace(int placeId)
{
    if (placeId != currentPlaceId)
    {
        UpdateUIManager::Instance().getMainWindow().handleFileOpen("", IRobloxDoc::IDE,
            QString::fromStdString(formatUrl(
                "%WEB_BASE%/Game/edit.ashx?PlaceID=%ITEM_ID%&upload=%ITEM_ID%&testmode=false&universeId=%UNIVERSE_ID%", currentGameId, placeId)));
    }
    else
    {
...
QStandardItem* RobloxGameExplorer::buildItem(
    ...
    if (uiSettings.nameMode == CategoryUiSettings::NameAsFolderPath)
    {
        ...
        for (int pathPieceIndex = 0; pathPieceIndex < path.size() - 1; ++pathPieceIndex)
        {
            ...
            if (!found)
            {
                ...
                newParent->setData(QtUtilities::getPixmap(QString::fromStdString(GetAssetFolder()) + "/textures/ClassImages.PNG", 77), Qt::DecorationRole);
...
void RobloxGameExplorer::publishInternal(boost::function<int()> newUniverseFuture)
{
    ...
    UpdateUIManager::Instance().waitForLongProcess(
        "Publishing Game...", boost::bind(&RobloxGameExplorer::publishGameThread, this, newUniverseFuture, &publishSucceeded, &targetGameId));
...
void RobloxGameExplorer::populateWithLoadedData(int originatingSessionId, int cat)
{
    ...
    QStandardItem* group = findGroup(category);
    if (currentSessionId != originatingSessionId || group == NULL)
        return;

    const CategoryUiSettings& settings = uiSettings[category];

    std::vector<QStandardItem*> items;
    entitySettings[category]->transform(boost::function<QStandardItem*(EntityProperties*)>(boost::bind(
                                            &RobloxGameExplorer::buildItem, boost::ref(settings), boost::ref(kWebSettings[cat]), group, _1)),
        &items);
    ...
    if (gameSettings.hasRootPlace())
    {
        if (QStandardItem* placeGroup = findGroup(ENTITY_CATEGORY_Places))
        {
            for (int i = 0; i < placeGroup->rowCount(); ++i)
            {
                ...
                {
                    placeItem->setData(QtUtilities::getPixmap(QString::fromStdString(GetAssetFolder()) + "/textures/ClassImages.PNG", 25), Qt::DecorationRole);
                }
            }
        }
    }
...
{
    QVariant var;
    var.setValue<HttpFuture>(std::move(future));
    QMetaObject::invokeMethod(
        explorer, "thumbnailLoadedForImage", Qt::QueuedConnection, Q_ARG(int, originatingSessionId), Q_ARG(QModelIndex, item), Q_ARG(QVariant, var));
}
    ...
    if (QStandardItem* badgesGroup = findGroup(ENTITY_CATEGORY_Badges))
    {
        for (int i = 0; i < badgesGroup->rowCount(); ++i)
        {
            ...
            if (EntityProperties* properties = placesItem->data(ROLE_EntityProperties).value<EntityProperties*>())
            {
                ...
                placesItem->setData(
                    qvar2(boost::bind(&RobloxGameExplorer::badgesPlaceContextMenuHandler, this, _1, properties)), ROLE_ContextMenuCallback);
                ...
                for (size_t i = 0; i < children.size(); ++i)
                {
                    ...
                    boost::optional<shared_ptr<const Reflection::ValueTable>> thumbnailDetails =
                        childProp.get<shared_ptr<const Reflection::ValueTable>>("Thumbnail");
                    if (thumbnailDetails.is_initialized() && thumbnailDetails.get())
                    {
                        ...
                        if (iter != thumbnailDetails.get()->end())
                        {
                            ...
                            if (!thumbnailUrl.empty())
                            {
                                ...
                                HttpAsync::get(thumbnailUrl, options)
                                    .then(boost::bind(&invokeThumbnailLoadedForImage, this, currentSessionId, newItem->index(), _1));
                            }
                        }
                    }
                }
        ...
        if (EntityProperties* properties = namedImageItem->data(ROLE_EntityProperties).value<EntityProperties*>())
        {
            ...
            if (assetId.is_initialized())
            {
                HttpAsync::get(formatUrl("%WEB_BASE%/asset/?id=%ITEM_ID%", -1, assetId.get()))
                    .then(boost::bind(&invokeThumbnailLoadedForImage, this, currentSessionId, namedImageItem->index(), _1));
            }
...
static void addNewPlaceHelperThread(int universeId, bool* succeeded)
{
    ...
    std::string defaultSettingsUrl =
        formatUrl("%WEB_BASE%/ide/places/defaultsettings?universeId=%UNIVERSE_ID%&templateId=%ITEM_ID%", universeId, FInt::NewGameTemplateId);
...
void RobloxGameExplorer::addNewPlace()
{
    ...
    UpdateUIManager::Instance().waitForLongProcess(QString("Adding Place..."), boost::bind(&addNewPlaceHelperThread, currentGameId, &succeeded));
...
static void blockingRemovePlace(int gameId, int placeId, bool isRoot, bool* reload)
{
    ...
    {
        HttpPostData d("", Http::kContentTypeDefaultUnspecified, false);
        HttpAsync::post(formatUrl(kWebSettings[ENTITY_CATEGORY_Places].removeFormatString, gameId, placeId), d).get();
    }
    catch (const Aya::base_exception& e)
    {
...
void RobloxGameExplorer::placeContextMenuHandler(const QPoint& point, EntityProperties* properties)
{
    ...
    makeRoot->setIcon(QtUtilities::getPixmap(QString::fromStdString(GetAssetFolder()) + "/textures/ClassImages.PNG", 25));
    ...
    {
        UpdateUIManager::Instance().waitForLongProcess(
            QString("Changing start place for game"), boost::bind(&blockingMakeRoot, currentGameId, placeId, &reload));
    }
...
void RobloxGameExplorer::badgesContextMenuHandler(const QPoint& point, EntityProperties* properties)
{
    ...
    {
        openAndFocusConfigureDoc(formatUrl("%WEB_BASE%/My/Item.aspx?ID=%ITEM_ID%", currentGameId, badgeId));
    }
...
void RobloxGameExplorer::badgesPlaceContextMenuHandler(const QPoint& point, EntityProperties* properties)
{
    ...
    if (result == createBadgeAction)
    {
        ...
        {
            openAndFocusConfigureDoc(formatUrl("%WEB_BASE%/My/NewBadge.aspx?targetID=%ITEM_ID%", currentGameId, placeId));
        }
    }
}
...
static void blockingRemoveNamedAsset(int universeId, const std::string& name, bool* success)
{
    ...
    {
        HttpPostData d("", Http::kContentTypeDefaultUnspecified, false);
        HttpAsync::post(formatUrl(kWebSettings[ENTITY_CATEGORY_NamedAssets].removeFormatString, universeId, name), d).get();
    }
    catch (const Aya::base_exception& e)
    {
        StandardOut::singleton()->printf(MESSAGE_ERROR, "Error while removing name: %s", e.what());
        *success = false;
        return;
    }
...
void RobloxGameExplorer::namedAssetsContextMenuHandler(const QPoint& point, EntityProperties* properties)
{
    ...
    if (result == remove)
    {
        ...
        std::string name = properties->get<std::string>("Name").get();
        UpdateUIManager::Instance().waitForLongProcess(
            QString("Removing name"), boost::bind(&blockingRemoveNamedAsset, currentGameId, name, &needReload));

        if (properties->get<int>("AssetTypeId") == (int)ASSET_TYPE_ID_Script)
        {
            RobloxDocManager::Instance().closeDoc(LuaSourceBuffer::fromContentId(ContentId::fromGameAssetName(name)));
        }
    }
    else if (result == rename)
    {
    ...
    else if (revertToLastPublished && result == revertToLastPublished)
    {
        ...
        if (mb.exec() == QMessageBox::Yes)
        {
            ...
            refreshNamedScriptIcons(currentSessionId);
            if (RobloxScriptDoc* scriptDoc =
                    RobloxDocManager::Instance().findOpenScriptDoc(LuaSourceBuffer::fromContentId(ContentId::fromGameAssetName(name))))
            {
    ...
    else if (publishScript && result == publishScript)
    {
        ...
        if (mb.exec() == QMessageBox::Yes)
        {
            ...
            UpdateUIManager::Instance().waitForLongProcess(
                "Publishing LinkedSource...", boost::bind(&boost::shared_future<void>::get, &publishFuture));
            refreshNamedScriptIcons(currentSessionId);
        }
    }
...
void RobloxGameExplorer::placeGroupContextMenuHandler(const QPoint& point)
{
    ...
    {
        openAndFocusConfigureDoc(formatUrl("%WEB_BASE%/universes/configure?id=%UNIVERSE_ID%#/#places", currentGameId));
    }
}
...
void RobloxGameExplorer::developerProductGroupContextMenuHandler(const QPoint& point)
{
    ...
    if (result == configure)
    {
        openAndFocusConfigureDoc(formatUrl("%WEB_BASE%/universes/configure?id=%UNIVERSE_ID%#/#developerProducts", currentGameId));
    }
}
...
void RobloxGameExplorer::bulkAddNewImageNames()
{
    ...
    QStringList files =
        QFileDialog::getOpenFileNames(this, "Choose images to upload", imageLastDir, "Images (*.jpg *.jpeg *.png *.gif *.bmp);;All Files (*.*)");
    ...
    if (!files.empty())
    {
        ...
        AYAASSERT(((int)index) == copy.size());

        UpdateUIManager::Instance().waitForLongProcess(
            "Uploading Images", boost::bind(&bulkAddNewImageNamesThread, &fileAndImageNames, currentGameId, currentGameGroupId));
        reloadDataFromWeb();
    }
}
...
void RobloxGameExplorer::doneEditingUniverseName()
{
    ...
    UpdateUIManager::Instance().waitForLongProcess("Updating Game Name...",
        boost::bind(&updateNameThread, gameSettings.asJson(),
            formatUrl("%API_PROXY%/universes/update-universe?universeid=%UNIVERSE_ID%", currentGameId), &success, &newSettings));
...
void RobloxGameExplorer::reloadDataFromWeb()
{
    ...
    HttpOptions getOptions;
    getOptions.setDoNotUseCachedResponse();
    gameSettings.setFromJsonFuture(HttpAsync::get(formatUrl("%API_PROXY%/universes/get-info?universeId=%UNIVERSE_ID%", currentGameId), getOptions));

    if (!gameSettings.currentUserHasAccess())
    {
        nonGameLoaded();
        return;
    }
    ...
    for (unsigned int i = 0; i < ENTITY_CATEGORY_MAX; ++i)
    {
        ...
        entitySettings[(EntityCategory)i] = EntityPropertiesForCategory::loadFromWeb(kWebSettings[i], currentGameId,
            boost::bind(&doneLoadingDataForCategory, QPointer<RobloxGameExplorer>(this), currentSessionId, (EntityCategory)i));
    }
}
...
void RobloxGameExplorer::checkRowForNameUpdate(EntityCategory category, QStandardItem* entityRow)
{
    ...
    if (category == ENTITY_CATEGORY_NamedAssets)
    {
        ...
        for (QStandardItem* stepParent = entityRow->parent(); stepParent != NULL && stepParent != findGroup(category);
    ...
    if (oldName.is_initialized() && oldName.get() != newName.toStdString())
    {
        ...
        if (validated)
        {
            ...
            UpdateUIManager::Instance().waitForLongProcess(
                ...
                boost::bind(&updateNameThread, props->asJson(), formatUrl(kWebSettings[category].updateFormatString, currentGameId, rowId), &success,
                    (std::string*)NULL));
        }
        ...
        else
        {
            ...
            if (category == ENTITY_CATEGORY_NamedAssets)
            {
                ...
                // update temp files
                if (boost::optional<std::string> oldContentOverride = findCachedAssetOrEmpty(oldContentId, currentGameId))
                {
...
boost::shared_future<void> RobloxGameExplorer::publishIfThereAreLocalModifications(EntityProperties* settings)
{
    ...
    {
        return boost::shared_future<void>(publishScriptAsset(localVersion.get(), settings->get<int>("AssetId").get(), currentGameGroupId)
