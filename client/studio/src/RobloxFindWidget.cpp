...
void RobloxFindWidget::initWidget()
{
    ...
    m_pFindInAllScriptsButton->setIcon(QIcon(":/images/icons/16x16/find.png"));
    ...
    m_pGotoLocationButton->setIcon(QIcon(":/images/icons/16x16/undo_square_16.png"));
    ...
    m_pPreviousButton->setIcon(QIcon(":/images/icons/16x16/arrowleft_green_16.png"));
    ...
    m_pNextButton->setIcon(QIcon(":/images/icons/16x16/arrowright_green_16.png"));
    ...
    m_pClearButton->setIcon(QIcon(":/images/icons/16x16/delete_x_16_h.png"));
    ...
    m_pStopButton->setIcon(QIcon(":/images/icons/16x16/stop_16.png"));
...
void RobloxFindWidget::findAll(const QString& findStr, bool allScripts, eFindFlags flags)
{
    ...
    addItem(FindData(QString("Find all \"%1\", %2").arg(findStr, allScripts ? "All Scripts " : "Current Script")));

    if (RobloxIDEDoc* ideDoc = RobloxDocManager::Instance().getPlayDoc())
        if (Aya::DataModel* dm = ideDoc->getDataModel())
        {
            dm->submitTask(
                boost::bind(&RobloxFindWidget::populateWidget, this, shared_from(dm), findStr, flags, allScripts), Aya::DataModelJob::Read);
            return;
        }
...
static void appendIfScript(shared_ptr<Aya::Instance> instance, QList<LuaSourceBuffer>& scriptList, Aya::CoreGuiService* coreGuiService)
{
    shared_ptr<Aya::Script> script = Aya::Instance::fastSharedDynamicCast<Aya::Script>(instance);
    shared_ptr<Aya::ModuleScript> moduleScript = Aya::Instance::fastSharedDynamicCast<Aya::ModuleScript>(instance);
    shared_ptr<Aya::LuaSourceContainer> lsc = Aya::Instance::fastSharedDynamicCast<Aya::LuaSourceContainer>(instance);

    if ((script || moduleScript) && (!coreGuiService || !instance->isDescendantOf(coreGuiService)) && lsc->getScriptId().isNull())
...
void RobloxFindWidget::populateScriptList(bool allScripts)
{
    ...
    if (allScripts)
    {
        ...
        {
            m_scriptList.append(LuaSourceBuffer::fromContentId(Aya::ContentId::fromGameAssetName(itr->toStdString())));
        }

        if (RobloxIDEDoc* ideDoc = RobloxDocManager::Instance().getPlayDoc())
            if (Aya::DataModel* dm = ideDoc->getDataModel())
                dm->visitDescendants(boost::bind<void>(&appendIfScript, _1, boost::ref(m_scriptList), dm->find<Aya::CoreGuiService>()));
    }
    else
    {
        ...
        if (!scriptDoc)
        {
            Aya::StandardOut::singleton()->print(Aya::MESSAGE_WARNING, "No script currently available.");
            return;
        }
...
void RobloxFindWidget::populateWidget(shared_ptr<Aya::DataModel> dm, const QString& findStr, eFindFlags flags, bool allScripts)
{
...
void RobloxFindWidget::getNextScript(int index, shared_ptr<Aya::DataModel> dm, const QString& findStr, eFindFlags flags)
{
    if (m_stopPopulating)
    {
        stopPopulatingItems();
    ...
    try
    {
        ...
        dm->submitTask(
            boost::bind(&RobloxFindWidget::searchScriptChain, this, index, sourceText, dm, findStr, flags, hierarchy), Aya::DataModelJob::None);
    }
    catch (const Aya::base_exception&)
    {
        if (index + 1 < m_scriptList.count())
            dm->submitTask(boost::bind(&RobloxFindWidget::getNextScript, this, index + 1, dm, findStr, flags), Aya::DataModelJob::Read);
        else
            stopPopulatingItems();
    }
}

void RobloxFindWidget::searchScriptChain(
    int index, const QString& sourceText, shared_ptr<Aya::DataModel> dm, const QString& findStr, eFindFlags flags, const QString& heirarchy)
{
    if (m_stopPopulating)
    {
        stopPopulatingItems();
    ...
    QTextDocument::FindFlags findFlags = QFlags<QTextDocument::FindFlag>(0);
    if (flags & MATCH_CASE)
    ...
    if (index + 1 < m_scriptList.count() && !m_stopPopulating)
        dm->submitTask(boost::bind(&RobloxFindWidget::getNextScript, this, index + 1, dm, findStr, flags), Aya::DataModelJob::Read); // Need read lock
    else
        stopPopulatingItems();
}

QTextCursor RobloxFindWidget::findNextInScript(
    const QTextDocument& doc, const QString& findStr, const QTextCursor& cursor, QTextDocument::FindFlags flags, bool regExp)
{
    if (regExp)
        return doc.find(QRegularExpression(findStr), cursor, flags);
    else
...
void RobloxFindWidget::stopPopulatingItems()
{
    if (!m_stopPopulating)
    {
        ...
        addItem(FindData(QString("Matching lines: %1    Matching scripts: %2    Total scripts searched: %3")
                             .arg(QString::number(m_matchesFound), QString::number(m_matchingScripts), QString::number(m_searchedScripts))));
...
QString RobloxFindWidget::getInstanceHierarchy(shared_ptr<Aya::Instance> instance)
{
    if (!instance)
        return "";
    ...
    const Aya::Instance* pParent = instance->getParent();
    const Aya::Instance* pChild = instance.get();
...
QVariant FindListModel::data(const QModelIndex& index, int role) const
{
    ...
    if (row >= 0 && row < m_previousCount && role == Qt::DisplayRole)
    {
        QMutexLocker locker(&m_listMutex);
...
int FindListModel::openScript(int row)
{
    ...
    if (shared_ptr<Aya::LuaSourceContainer> lsc = buffer.toInstance())
    {
    ...
    Aya::BaldPtr<ScriptTextEditor> editor = scriptDoc->getTextEditor();
    if (!editor)
        return -1;
