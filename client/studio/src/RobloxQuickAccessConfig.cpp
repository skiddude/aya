...
void QuickAccessActionsListWidget::accept()
{
    ...
    while (currentChild < numChild)
    {
        ...
        {
            quickAccessCollection.append(RobloxQuickAccessConfig::ActionData(
                pCurrentItem->getAction(), pCurrentItem->data(QAC_VISIBILITY, Qt::CheckStateRole).toBool(), true));
        }
    }
    ...
    , m_dataChanged(false)
{
    ...
    m_pMoveUpButton->setIcon(QIcon(":/images/icons/ribbonMinimize.png"));

    m_pMoveDownButton = new QPushButton(this);
    m_pMoveDownButton->setIcon(QIcon(":/images/icons/ribbonMaximize.png"));
