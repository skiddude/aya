    ...
    , m_MouseDown(false)
{
    ...
    Aya::BaldPtr<QWidget> cornerWidget = new QWidget(m_TabWidget);
    Aya::BaldPtr<QLayout> layout = new QHBoxLayout(cornerWidget);
...
{
    AYAASSERT(m_Docs.indexOf(&doc) == -1);
    AYAASSERT(doc.getViewer());

    Aya::BaldPtr<QWidget> widget = doc.getViewer();
    AYAASSERT(widget);
    ...
    AYAASSERT((index != -1) && (index < m_Docs.size()));
...
void DocTabManager::onCurrentChanged(int index)
{
    ...
    {
        Aya::BaldPtr<IRobloxDoc> doc = m_Docs[index];
...
{
    Aya::BaldPtr<IRobloxDoc> doc = getCurrentDoc();
    if (doc)
...
void DocTabManager::setDockHoverOverPos(const QPoint& globalPos)
{
    ...
    else
    {
        ...
        if (m_FakeTab->isVisible())
        {
            AYAASSERT(m_TabWidget->indexOf(m_FakeTab) != -1);
...
{
    if (m_TabWidget->isAncestorOf(newWidget) && m_TabWidget->currentIndex() != -1 && m_TabWidget->currentIndex() < m_Docs.size())
    {
        Aya::BaldPtr<IRobloxDoc> doc = m_Docs[m_TabWidget->currentIndex()];
        RobloxDocManager::Instance().setCurrentDoc(doc);
    }
}
...
void DocTabManager::setupTabPageSwitchingShortcuts()
{
    ...
    shortcuts.append(QKeySequence(QKeySequence::NextChild));
    shortcuts.append(QKeySequence(QKeyCombination(Qt::CTRL | Qt::SHIFT, Qt::Key_Right)));
    nextTabAction->setShortcuts(shortcuts);
    connect(nextTabAction, &QAction::triggered, this, &DocTabManager::showNextTabPage);

    shortcuts.clear();
    QAction* prevTabAction = new QAction(&m_MainWindow);
    shortcuts.append(QKeySequence(QKeySequence::PreviousChild));
    shortcuts.append(QKeySequence(QKeyCombination(Qt::CTRL | Qt::SHIFT, Qt::Key_Left)));
    prevTabAction->setShortcuts(shortcuts);
    connect(prevTabAction, &QAction::triggered, this, &DocTabManager::showPreviousTabPage);
