...
ScriptComboBox::ScriptComboBox(QWidget* parent)
{
    ...
    setStyleSheet("QComboBox { padding: 4px; }");
    m_OldStyle = "QComboBox { padding: 4px; }";
    ...
    int fontId = QFontDatabase::addApplicationFont(assetFolder + "/fonts/cascadiacode.ttf");
...
void ScriptComboBox::setCommandHistory(const QStringList& commandHistory)
{
    ...
    if (!m_CommandHistory.isEmpty())
    {
        for (int i = 0; i < m_CommandHistory.size(); ++i)
        {
            ...
            if (Aya::ScriptContext::checkSyntax(command.toStdString(), errorLine, errorMessage))
                addItem(command);
        }
        setCurrentIndex(-1);
    }
...
bool ScriptComboBox::event(QEvent* e)
{
    ...
    if (e->type() == QEvent::KeyPress)
    {
        ...
        if (AuthoringSettings::singleton().intellisenseEnabled && modifiers != Qt::ControlModifier)
        {
            ...
            if (lineEdit()->hasSelectedText() &&
                (key < Qt::Key_nobreakspace || ((key == Qt::Key_Tab || key == Qt::Key_Return) && Studio::Intellesense::singleton().isActive())))
                lineEdit()->del();
...
void ScriptComboBox::onScriptInputReturnPressed()
{
    if (!RobloxDocManager::Instance().getPlayDoc())
    {
        Aya::StandardOut::singleton()->printf(
            Aya::MESSAGE_ERROR, "Command cannot be executed without an open place. Please open a place and try again.");
        return;
    }
    ...
    if (!toExecute.isEmpty())
    {
        int errorLine = 0;
        std::string errorMessage;
        if (Aya::ScriptContext::checkSyntax(toExecute.toStdString(), errorLine, errorMessage))
        {
        ...
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_OUTPUT, STRING_BY_ID(CommandOutStringId), toExecute.toStdString().c_str());
...
QRect ScriptComboBox::getCursorPosition()
{
    ...
    parentGeometry.setX(parentGeometry.x() + 21 + fontMetrics.horizontalAdvance(lineEdit()->text().left(lineEdit()->cursorPosition())));
