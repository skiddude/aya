...
DocDockWidget::DocDockWidget(RobloxMainWindow& mainWindow, DocDockManager& docManager, IRobloxDoc& doc)
...
void DocDockWidget::simulateDrag()
{
    if (m_IsSimulatingDrag)
    {
        ...
        const QPoint offset(width() / 2, style()->pixelMetric(QStyle::PM_TitleBarHeight) / 2);
...
bool DocDockWidget::event(QEvent* event)
{
    ...
    if (event->type() == QEvent::NonClientAreaMouseButtonRelease || event->type() == QEvent::MouseButtonRelease)
    {
        ...
        Aya::BaldPtr<QMouseEvent> mouseEvent = static_cast<QMouseEvent*>(event);
    ...
    else if (event->type() == QEvent::NonClientAreaMouseButtonPress || event->type() == QEvent::MouseButtonPress)
    {
