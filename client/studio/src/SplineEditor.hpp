...
public:
    ...
    void select(int ptid)
    {
        m_selPt = ptid;
    } // marks the given point as selected
    void setHighlight(int what)
    {
        m_highlight_ = what;
    } // 0 = none, 1 - point, 2 - upper size handle, 3 - lower size handle
    int getPointAt(int x, int y); // returns the point id at the given coordinates, (-1) if not found
    std::pair<int, int> getEnvHandleAt(int x,
        int y); // returns the point id of the size handle and the size handle type (0 = upper, 1 = lower) at the given coordinates, (-1) if not found


    void ghost(float x, float y, float e)
    {
        m_ghostX = x;
        m_ghostY = y;
        m_ghostE = e;
    } // updates the ghost, x=-1 to disable
...
private:
    ...
    shared_ptr<Aya::DataModel> m_dataModel;
    Aya::Instance* m_pInst;
    SplineEditor* m_pSplineEditor;
    SplineDef m_spline;
    Aya::signals::scoped_connection m_propChangedConn, m_ancestryChangedConn;
...
private Q_SLOTS:
    ...
    void onPropChanged(const Aya::Reflection::PropertyDescriptor* pd); // called when the property changes externally to refresh the stuff
    void onAncestryChanged(shared_ptr<Aya::Instance>, shared_ptr<Aya::Instance>);
};
