...
QSize RobloxCategoryItemDelegate::sizeHint(const QStyleOptionViewItem& option, const QModelIndex& index) const
{
...
void RobloxReportView::drawBranches(QPainter* painter, const QRect& rect, const QModelIndex& index) const
{
    ...
    if (pCategoryItem)
    {
        ...
        opt.rect = primitive;
        opt.state =
            QStyle::State_Item | extraFlags | QStyle::State_Children | (pCategoryItem->isExpanded() ? QStyle::State_Open : QStyle::State_None);
