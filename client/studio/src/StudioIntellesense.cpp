    ...
    , m_cursorHeight(0)
{
    ...
    connect(m_AutoCompleteMenu, SIGNAL(itemClicked(QListWidgetItem*)), this, SLOT(singleClick(QListWidgetItem*)));

    connect(m_AutoCompleteMenu, SIGNAL(itemDoubleClicked(QListWidgetItem*)), this, SLOT(doubleClick(QListWidgetItem*)));
...
bool Intellesense::activate(QString& text, int row, int& cursorIndex, QKeyEvent* keyEvent, QRect positionRange, bool searchDatamodel,
    QWidget* focusWidget, const boost::optional<std::vector<Aya::ScriptAnalyzer::IntellesenseResult>>& result)
{
    ...
    else if ((qkey).isLetter() || (true /* StudioVariableIntellesense */ && qkey.isNumber()) ||
             (FFlag::StudioIntellisenseAllowUnderscore && key == Qt::Key_Minus && modifiers == Qt::ShiftModifier))
    {
        QChar keyEventCharacter = qkey; // Remove this with removal of FFlag::StudioIntellisenseAllowUnderscore
...
void escapeSequence(QString& str)
{
    ...
    while (stringSize--)
    {
        switch (str.at(stringSize).unicode())
        {
        case '"':
        case '\\':
        case '\n':
...
bool Intellesense::replaceTextWithCurrentItem(QString& text, int& cursorIndex)
{
    ...
    if (replacementText.contains(QRegularExpression("[^a-zA-Z0-9_]")) || replacementText.contains(QRegularExpression("^[0-9]+$")))
        invalidFormat = true;
    ...
    QString previousText = text.left(text.lastIndexOf(QRegularExpression(true /* StudioVariableIntellesense */ ? "[^a-zA-Z0-9_]" : "[\\.\\:]"), offset) + 1);
...
{
    int pos = index + 1;
    QStringList stringList = text.split(QRegularExpression(FFlag::StudioIntellisenseAllowUnderscore ? "[^a-zA-Z\\d\\.\\:\\_]" : "[^a-zA-Z\\d\\.\\:]"));
    ...
    for (QStringList::iterator i = stringList.begin(); i != stringList.end(); ++i)
    {
        ...
        if (pos <= 0)
            return i->left(i->lastIndexOf(QRegularExpression("[\\.\\:]")));
    }
    return QString("");
}
...
{
    QStringList stringList = text.split(QRegularExpression("\\s"));
...
bool Intellesense::isEnumerator(QString previousWord)
{
    ...
    {
        for (std::vector<const Aya::Reflection::EnumDescriptor*>::const_iterator i = Aya::Reflection::EnumDescriptor::enumsBegin();
             i != Aya::Reflection::EnumDescriptor::enumsEnd(); ++i)
        {
            Aya::Reflection::Metadata::Item* metadata = Aya::Reflection::Metadata::Reflection::singleton()->get(**i);
            if (Aya::Reflection::Metadata::Item::isDeprecated(metadata, **i))
            {
                continue;
            }
    ...
    else if (previousWord.startsWith("Enum."))
    {
             ...
             i != Aya::Reflection::EnumDescriptor::enumsEnd(); ++i)
        {
            ...
            {
                std::for_each((*i)->begin(), (*i)->end(), boost::bind(&Intellesense::populateEnumerator, this, _1));
                return true;
            }
        }
    }
...
int Intellesense::populateItemLists(
    QString text, int row, int index, bool searchDatamodel, const boost::optional<std::vector<Aya::ScriptAnalyzer::IntellesenseResult>>& result)
{
    if (true /* StudioVariableIntellesense */)
        clearAllItemLists();

    QString previousWord = getPreviousWord(text, index);

    if (true /* StudioVariableIntellesense */ && isAfterDeclarationKeyword(text, index))
        return 0;

    bool gettingService = false;
    QRegularExpression service(FFlag::StudioIntellisenseCodeExecutionFixEnabled ? "([gG]ame:(?:Find|Get)Service\\([^)]+\\)[.a-zA-Z\\d]*)([.:][a-zA-Z\\d]*)$"
                                                                     : "([gG]ame:(?:Find|Get)Service\\(.+\\))[.:][a-zA-Z\\d]*$");
    QRegularExpressionMatch match = service.match(text);
    gettingService = match.hasMatch();
    if (gettingService)
    {
        previousWord = match.captured(1);
    ...
    QChar previousDivider = text.at(qMax(text.lastIndexOf(QRegularExpression("[^a-zA-Z\\d\\_]"), index - text.size() - 1), 0));
    if (previousDivider != '.' && previousDivider != ':')
    {
        if (true /* StudioVariableIntellesense */ && result)
    ...
    shared_ptr<Aya::LuaSourceContainer> script = shared_ptr<Aya::LuaSourceContainer>();
...
QString Intellesense::getCurrentScriptPath()
{
    ...
    {
        shared_ptr<Aya::Instance> currentInstance = scriptDoc->getCurrentScript().toInstance();
...
shared_ptr<Aya::LuaSourceContainer> Intellesense::getCurrentScript()
{
    if (RobloxScriptDoc* scriptDoc = dynamic_cast<RobloxScriptDoc*>(RobloxDocManager::Instance().getCurrentDoc()))
    {
        return scriptDoc->getCurrentScript().toInstance();
    }
    return shared_ptr<Aya::LuaSourceContainer>();
}

template<bool includeType>
static void catenateArg(int& count, QString& text, const Aya::Reflection::SignatureDescriptor::Item& item)
{
    if (count > 0)
        text += ", ";
    if (includeType)
...
void Intellesense::populateProperty(const Aya::Reflection::PropertyDescriptor* d)
{
    if (!d || !d->isPublic() || !d->isScriptable())
        return;

    Aya::Reflection::Metadata::Item* metadata = Aya::Reflection::Metadata::Reflection::singleton()->get(*d);
...
void Intellesense::populateFunction(const Aya::Reflection::FunctionDescriptor* d)
{
    if (!d)
        return;
    ...
    int count = 0;
    std::for_each(
        signature.arguments.begin(), signature.arguments.end(), boost::bind(&catenateArg<true>, boost::ref(count), boost::ref(toolTip), _1));
    toolTip += ")";
...
void Intellesense::populateYieldFunction(const Aya::Reflection::YieldFunctionDescriptor* d)
{
    if (!d)
        return;
...
void Intellesense::populateEvent(const Aya::Reflection::EventDescriptor* d)
{
    if (!d || !d->isPublic() || !d->isScriptable())
        return;
...
void Intellesense::populateCallBack(const Aya::Reflection::CallbackDescriptor* d)
{
    if (!d)
        return;
...
void Intellesense::populateEnumerator(const Aya::Reflection::EnumDescriptor::Item* d)
{
    Aya::Reflection::Metadata::Item* metadata = Aya::Reflection::Metadata::Reflection::singleton()->get(*d);
    if (Aya::Reflection::Metadata::Item::isDeprecated(metadata, *d))
    {
        return;
    }
...
void Intellesense::showToolTip(QListWidgetItem* listItem)
{
    ...
    if (!tooltip.isEmpty() && isActive())
    {
        ...
        IntellesenseTooltip::singleton().showText(
            m_AutoCompleteFrame->pos() + QPoint(m_AutoCompleteFrame->width(), 0), tooltip, m_focusProxy, bottomLeftAlt, topLeftAlt);
    }
    else
    {
        IntellesenseTooltip::singleton().hideText();
...
int Intellesense::repopulateUserVariables(const std::vector<Aya::ScriptAnalyzer::IntellesenseResult>& result, unsigned int row, unsigned int column)
{
    ...
    for (auto it : result)
    {
        ...
        {
            if (((it.location.begin.line < row) || (it.location.begin.line == row && it.location.begin.column <= column)) &&
                (it.location.end.line > row || (it.location.end.line == row && it.location.end.column >= column)))
            {
...
bool Intellesense::populateItemListsFromVariant(const Aya::Reflection::Variant varValue, bool showAllChildren)
{
    if (varValue.isType<boost::shared_ptr<Aya::Instance>>())
    {
        ...
        std::for_each(pInstance->getDescriptor().begin<Aya::Reflection::PropertyDescriptor>(),
            pInstance->getDescriptor().end<Aya::Reflection::PropertyDescriptor>(), boost::bind(&Intellesense::populateProperty, this, _1));

        std::for_each(pInstance->getDescriptor().begin<Aya::Reflection::FunctionDescriptor>(),
            pInstance->getDescriptor().end<Aya::Reflection::FunctionDescriptor>(), boost::bind(&Intellesense::populateFunction, this, _1));

        std::for_each(pInstance->getDescriptor().begin<Aya::Reflection::YieldFunctionDescriptor>(),
            pInstance->getDescriptor().end<Aya::Reflection::YieldFunctionDescriptor>(), boost::bind(&Intellesense::populateYieldFunction, this, _1));

        std::for_each(pInstance->getDescriptor().begin<Aya::Reflection::EventDescriptor>(),
            pInstance->getDescriptor().end<Aya::Reflection::EventDescriptor>(), boost::bind(&Intellesense::populateEvent, this, _1));

        std::for_each(pInstance->getDescriptor().begin<Aya::Reflection::CallbackDescriptor>(),
            pInstance->getDescriptor().end<Aya::Reflection::CallbackDescriptor>(), boost::bind(&Intellesense::populateCallBack, this, _1));

        if (pInstance->getChildren())
        {
            Aya::Instances::const_iterator iter = pInstance->getChildren()->begin();
    ...
    else if (varValue.isType<Aya::Vector3int16>())
    ...
    else if (varValue.isType<Aya::Color3>())
    ...
    else if (varValue.isType<Aya::PhysicalProperties>())
...
void Intellesense::repopulateListWidgetHelper(
    QList<QListWidgetItem>& populateVector, QList<QListWidgetItem>* startsWithVector, int itemType, QString matchingWord)
{
    int insertPos = 0;
...
bool Intellesense::handleAutoCompleteMenu(const QRect& positionRange, int itemFlags, QString currentWord, int lineIndex)
{
    ...
    if (!positionRange.isNull())
    {
        ...
        QScreen* screenAtWidget = QGuiApplication::screenAt(m_focusProxy->geometry().center());
        QRect screenBounds;

        if (screenAtWidget) {
            screenBounds = screenAtWidget->geometry();
        } else {
            // Fallback to primary screen if no screen is found at the widget's position
            QScreen* primaryScreen = QGuiApplication::primaryScreen();
            if (primaryScreen) {
                screenBounds = primaryScreen->geometry();
            }
        }
...
bool Intellesense::eventFilter(QObject* watched, QEvent* evt)
{
    if (isActive())
    {
        ...
        else if (evt->type() == QEvent::KeyPress || evt->type() == QEvent::MouseMove || evt->type() == QEvent::Wheel)
...
void IntellesenseTooltip::showText(const QPoint& pos, const QString& text, QWidget* focusWidget, const QPoint& bottomLeftAlt,
    const QPoint& topLeftAlt) //, int altX, const QPoint& altPos)
{
    ...
    body = body.remove(QRegularExpression("<[^>]*>"));

    QFont boldFont = toolTipLabel->font();
    boldFont.setBold(true);
    QFontMetrics fontMetric = QFontMetrics(boldFont);

    int toolTipWidth = fontMetric.horizontalAdvance(header);
    int toolTipHeight = fontMetric.height() * (qCeil((float)fontMetric.horizontalAdvance(body) / (float)toolTipWidth) + AUTOCOMPLETE_FRAME_MARGIN);
    ...
    QRect screenBounds;
    if (focusWidget) {
        QScreen* screenAtWidget = QGuiApplication::screenAt(focusWidget->geometry().center());
        if (screenAtWidget) {
            screenBounds = screenAtWidget->geometry();
        } else {
            // Fallback to primary screen if no screen is found at the widget's position
        ...
        // Get the primary screen's geometry if focusWidget is nullptr
        QScreen* primaryScreen = QGuiApplication::primaryScreen();
        if (primaryScreen) {
            screenBounds = primaryScreen->geometry();
        }
