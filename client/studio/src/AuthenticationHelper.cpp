...
QString AuthenticationHelper::getLoggedInUserUrl()
{
    ...
    {
        return QString::fromStdString(ReplaceTopSubdomain(result.toStdString(), "assetgame"));
    }
    return result;
}
...
bool AuthenticationHelper::verifyUserAndAuthenticate(int timeOutTime)
{
    ...
    try
    {
        ...
        networkRequest.setRawHeader("User-Agent", userAgentStr.toUtf8());
        ...
        if (!networkReply->waitForFinished(timeOutTime, 100))
        {
            ...
            Aya::StandardOut::singleton()->print(Aya::MESSAGE_ERROR, "Unable to get current user. The request timed out.");
            networkReply->deleteLater();
            return false;
        }
...
QString AuthenticationHelper::doHttpGetRequest(const QString& str, const Aya::HttpAux::AdditionalHeaders& externalHeaders)
{
    std::string result = "";
    try
    {
    ...
    , m_bIgnoreFinishedSignal(false)
{
    ...
    connect(&RobloxNetworkAccessManager::Instance(), SIGNAL(sslErrors(QNetworkReply*, const QList<QSslError>&)), this,
...
{
    return Aya::MachineIdUploader::uploadMachineId(qPrintable(RobloxSettings::getBaseURL())) != Aya::MachineIdUploader::RESULT_MachineBanned;
}
...
bool AuthenticationHelper::eventFilter(QObject* watched, QEvent* evt)
{
    ...
    else if (eventType == QEvent::ApplicationActivate)
    {
        ...
        if (QString::compare(savedCookieKey, currentCookieKey, Qt::CaseSensitive) != 0)
        {
            ...
            AYAASSERT(getHttpUserId() == getWebKitUserId());
        }
    }
...
int AuthenticationHelper::authenticateWebKitFromHttp()
{
    ...
    Aya::HttpAux::AdditionalHeaders externalHeaders;
...
void AuthenticationHelper::authenticateUserAsync(const QString& url, const QString& ticket)
{
    ...
    if (!url.isEmpty() && !ticket.isEmpty())
    {
        ...
        m_HttpAuthenticationFuture = QtConcurrent::run(
            [this, authenticationUrl]()
            {
                return this->authenticateHttpSession(authenticationUrl);
            });
        m_HttpAuthenticationFutureWatcher.setFuture(m_HttpAuthenticationFuture);

        m_QtWebkitAuthenticationFuture = QtConcurrent::run(
            [this, authenticationUrl]()
            {
                return this->authenticateQtWebkitSession(authenticationUrl);
            });
    ...
    else
    {
        ...
        if (secCookie.isEmpty())
        {
            m_QtWebkitAuthenticationFuture = QtConcurrent::run(
                [this]()
                {
                    return authenticateQtWebkitSession();
                });
            m_QtWebkitAuthenticationFutureWatcher.setFuture(m_QtWebkitAuthenticationFuture);
        }
        else
        {
            ...
            m_HttpAuthenticationFuture = QtConcurrent::run(
                [this]()
                {
                    return authenticateHttpSession();
                });
            m_HttpAuthenticationFutureWatcher.setFuture(m_HttpAuthenticationFuture);
        }
    }
...
void AuthenticationHelper::authenticateHttpSessionAsync(bool state)
{
    ...
    else
    {
        m_HttpAuthenticationFuture = QtConcurrent::run(
            [this]()
            {
                return deAuthenticateHttpSession();
            });
    }
...
bool AuthenticationHelper::authenticateHttpSession()
{
    ...
    {
        Aya::StandardOut::singleton()->print(Aya::MESSAGE_ERROR, "Unable to authenticate user. Request timed out.");
    }
...
bool AuthenticationHelper::authenticateQtWebkitSession(const QString& authenticationUrl)
{
    ...
    networkRequest.setRawHeader("RBXAuthenticationNegotiation:", baseURL.toUtf8());
    networkRequest.setRawHeader("User-Agent", userAgentStr.toUtf8());
...
QString AuthenticationHelper::getHttpRequest(const QString& urlString, const Aya::HttpAux::AdditionalHeaders& externalHeaders)
{
    static QMutex httpRequestMutex;
    QMutexLocker lock(&httpRequestMutex);

    Aya::HttpOptions options;
    options.addHeader("RBXAuthenticationNegotiation:", ::GetBaseURL());

    for (Aya::HttpAux::AdditionalHeaders::const_iterator iter = externalHeaders.begin(); iter != externalHeaders.end(); ++iter)
        options.addHeader(iter->first, iter->second);

    Aya::HttpFuture request = Aya::HttpAsync::get(urlString.toStdString(), options);
    std::string result;
    ...
    {
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "Error while authenticating user: %s", e.what());
    }
...
void AuthenticationHelper::httpSessionAuthenticationDone()
{
    ...
    AYAASSERT(m_CurrentHttpRequestState != -1);
