...
void ScriptSideWidget::updateArea()
{
    ...
    int newWidth = fm.horizontalAdvance(QLatin1Char('9')) * digits + 10 + (FFlag::LuaDebugger ? (m_debugArea + m_foldArea) : m_foldArea);
    ;
    if (width() != newWidth)
    {
...
void ScriptSideWidget::paintEvent(QPaintEvent* evt)
{
    ...
    while (block.isValid())
    {
        ...
        if (block.isVisible())
        {
            ...
            const int x = width() - (FFlag::LuaDebugger ? (m_debugArea + m_foldArea) : m_foldArea) - fm.horizontalAdvance(strNumber) - 3;
            ...
            if (FFlag::LuaDebugger)
            {
                ...
                if (pUserData)
                {
                    if (pUserData->getBreakpointState())
                        painter.drawPixmap(
                            xofsB + 1, qRound(position.y()) - 1, QtUtilities::getBreakpointPixmap(pUserData->getBreakpointState(), m_debugArea - 2));

                    if (!pUserData->getMarker().isEmpty())
                    {
                        QPixmap marker(pUserData->getMarker());
                        painter.drawPixmap(xofsB + 1, qRound(position.y()) - 2,
                            marker.scaled(m_debugArea - 2, m_debugArea - 2, Qt::KeepAspectRatio, Qt::SmoothTransformation));
                    }
                }
            }
...
void ScriptSideWidget::onPropertyChanged(const Aya::Reflection::PropertyDescriptor* pDescriptor)
{
