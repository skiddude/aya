...
//--------------------------------------------------------------------------------------------
class PropertyItemUpdateJob : public Aya::DataModelJob
{
public:
    typedef enum
    {
...
public:
    Aya::CEvent update;  // Set when the UI thread may update
    Aya::CEvent updated; // Set when the UI thread has updated


    PropertyItemUpdateJob(PropertyTreeWidget* pPropertyWidget, shared_ptr<Aya::DataModel> dataModel)
        : Aya::DataModelJob("PropGrid", Aya::DataModelJob::Read, false, dataModel, Aya::Time::Interval(0))
    ...
    virtual Aya::TaskScheduler::StepResult stepDataModelJob(const Stats& stats)
    {
...
void PropertyTreeWidget::setDataModel(boost::shared_ptr<Aya::DataModel> pDataModel)
{
...
void PropertyTreeWidget::createUpdateJob()
{
    ...
    Aya::TaskScheduler::singleton().add(m_pUpdateItemsJob);
}
...
void PropertyTreeWidget::removeUpdateJob()
{
    if (m_pUpdateItemsJob)
    {
        ...
        Aya::TaskScheduler::singleton().removeBlocking(m_pUpdateItemsJob);
...
void PropertyTreeWidget::restoreOriginalValues(PropertyItem* item)
{
    ...
    Aya::DataModel::LegacyLock lock(getDataModel(), Aya::DataModelJob::Write);

    for (boost::unordered_map<shared_ptr<Aya::Instance>, Aya::Reflection::Variant>::const_iterator iter = m_originalValues.begin();
        iter != m_originalValues.end(); ++iter)
...
void PropertyTreeWidget::storeOriginalValues(PropertyItem* item)
{
    ...
    if (Aya::Selection* selection = Aya::ServiceProvider::create<Aya::Selection>(getDataModel().get()))
    {
        Aya::Instance* prevInstance = NULL;
...
void PropertyTreeWidget::addInstance(Aya::Instance* pInstance)
{
...
void PropertyTreeWidget::removeInstance(Aya::Instance* pInstance)
{
    // remove connection
...
void PropertyTreeWidget::onPropertyChanged(const Aya::Reflection::PropertyDescriptor* pPropertyDescriptor)
{
...
void PropertyTreeWidget::update()
{
    if (m_pUpdateItemsJob)
    {
        ...
        Aya::TaskScheduler::singleton().reschedule(m_pUpdateItemsJob);
    ...
    if (m_itemMap.empty())
    {
        visitClassProperties(&Aya::Reflection::ClassDescriptor::rootDescriptor());
    ...
    if (m_bIsPropertyChanged && !m_bIgnorePropertyChanged)
    {
        ...
        m_invalidItems.clear();
        std::for_each(tempInvalidItems.begin(), tempInvalidItems.end(), boost::bind(&PropertyItem::update, _1));
        m_bIsPropertyChanged = false;
    }
    else
    {
...
void PropertyTreeWidget::visitClassProperties(const Aya::Reflection::ClassDescriptor* pClassDescriptor)
{
    // visit classes
    std::for_each(pClassDescriptor->derivedClasses_begin(), pClassDescriptor->derivedClasses_end(),
        boost::bind(&PropertyTreeWidget::visitClassProperties, this, _1));

    // initialize properties
    std::for_each(pClassDescriptor->begin<Aya::Reflection::PropertyDescriptor>(), pClassDescriptor->end<Aya::Reflection::PropertyDescriptor>(),
        boost::bind(&PropertyTreeWidget::initProperty, this, pClassDescriptor, _1));
}

void PropertyTreeWidget::initProperty(
    const Aya::Reflection::ClassDescriptor* pClassDescriptor, const Aya::Reflection::PropertyDescriptor* pPropertyDescriptor)
{
    ...
    Aya::Reflection::Metadata::Member* pMember = Aya::Reflection::Metadata::Reflection::singleton()->get(*pPropertyDescriptor);

    // only show public, editable properties
    if (!pPropertyDescriptor->isPublic() || !pPropertyDescriptor->isEditable() ||
        (pPropertyDescriptor->security != Aya::Security::None && !(Aya::Reflection::Metadata::Item::isBrowsable(pMember, *pPropertyDescriptor))))
    {
        return;
    }

    if (Aya::Reflection::Metadata::Item::isDeprecated(pMember, *pPropertyDescriptor))
        return;
...
CategoryItem* PropertyTreeWidget::getCategoryItem(const Aya::Reflection::PropertyDescriptor* pPropertyDescriptor)
{
    ...
    pItem->setExpanded(m_bStoreCollapasedState
                           ? (!collapsedCategoryItems.contains(categoryName) && RobloxSettings().value(getRegistryKey(pItem), true).toBool())
                           : true);
    ...
    , m_bUpdateRequested(false)
{
    ...
    pMainGridLayout->setColumnStretch(0, 1);

    QLineEdit* lineEdit = new QLineEdit();
    lineEdit->setPlaceholderText(
        QString("Filter Properties (%1)").arg(UpdateUIManager::Instance().getMainWindow().propertiesFilterAction->shortcut().toString()));

    m_pFilterEdit = new QComboBox();
    m_pFilterEdit->setEditable(true);
    m_pFilterEdit->setLineEdit(lineEdit);
    m_pFilterEdit->lineEdit()->setClearButtonEnabled(true);
    m_pFilterEdit->setInsertPolicy(QComboBox::InsertAtTop);
    m_pFilterEdit->setMaxCount(10);
    m_pFilterEdit->setAutoFillBackground(true);

    /*
    QPalette palette = m_pFilterEdit->palette();
    palette.setColor(QPalette::PlaceholderText, Qt::black);
    m_pFilterEdit->setPalette(palette);
    */
    ...
    connect(m_pFilterEdit, SIGNAL(editTextChanged(QString)), m_pTreeWidget, SLOT(setFilter(QString)));
}
...
void RobloxPropertyWidget::setDataModel(boost::shared_ptr<Aya::DataModel> pDataModel)
{
    if (m_pDataModel == pDataModel)
        return;
    ...
    if (m_pDataModel)
    {
        ...
        if (pSelection)
        {
            ...
            m_cInstanceSelectionChanged =
                pSelection->selectionChanged.connect(boost::bind(&RobloxPropertyWidget::onInstanceSelectionChanged, this, _1));
        }
    }
...
void RobloxPropertyWidget::addInstance(boost::shared_ptr<Aya::Instance> pInstance)
{
    if (!pInstance)
        return;

    // Aya::readwrite_concurrency_catcher::scoped_write_request lock(m_selectedInstancesGuard);
    m_selectedInstaces.push_back(pInstance.get());
    m_pTreeWidget->addInstance(pInstance.get());
}

void RobloxPropertyWidget::removeInstance(boost::shared_ptr<Aya::Instance> pInstance)
{
    if (!pInstance)
        return;
...
void RobloxPropertyWidget::onInstanceSelectionChanged(const Aya::SelectionChanged& evt)
{
    ...
    if (!m_pFilterEdit->currentText().isEmpty())
        m_pFilterEdit->clear();
...
void RobloxPropertyWidget::updateSelectionInfoLabel()
{
    ...
    const Aya::Name* className = &(*curIter)->getClassName();
