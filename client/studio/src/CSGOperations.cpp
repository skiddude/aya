...
Aya::CSGOperations::CSGOperations(DataModel* pDatamodel, MessageCallback failedFun)
    : mDatamodel(pDatamodel)
    , operationFailed(failedFun)
{
}

bool Aya::CSGOperations::doUnion(std::vector<boost::shared_ptr<Aya::Instance>>::const_iterator start,
    std::vector<boost::shared_ptr<Aya::Instance>>::const_iterator end, boost::shared_ptr<PartOperation>& partOperation)
{
    bool allNeg = true;
    for (Instances::const_iterator iter = start; iter != end; ++iter)
    {
        if (!Aya::Instance::fastSharedDynamicCast<NegateOperation>(*iter))
        {
            allNeg = false;
            break;
        }
    ...
    if (!setUnionMesh(partOperation, start, end, false))
    {
        ...
        partOperation = boost::shared_ptr<Aya::PartOperation>();
        return false;
    }
    ...
    if (FFlag::CSGDelayParentingOperationToEnd)
    {
        ...
        for (auto iter : selectionCopy)
        {
            ...
            if (shared_ptr<PartOperation> partOp = Aya::Instance::fastSharedDynamicCast<PartOperation>(iter))
            {
                dictionaryService->retrieveData(*partOp);
                nrDictionaryService->retrieveData(*partOp);

                partOp->setMeshData(Aya::BinaryString(""));
                partOp->setPhysicsData(Aya::BinaryString(""));
            }
        }
    }
    else
    {
        for (Aya::Instances::const_iterator iter = start; iter != end; ++iter)
        {
            (*iter)->setParent(partOperation.get());
    ...
    {
        char buf[128];
        sprintf(buf, "This union is too complex.  There is a %d triangle limit for unions.  Please undo the last change.",
            (int)Aya::PartOperation::getMaximumTriangleCount());
        partOperation->setParent(NULL);
        partOperation = boost::shared_ptr<Aya::PartOperation>();
    ...
    if (FFlag::CSGDelayParentingOperationToEnd)
    {
        AYAASSERT(partOperation->getParent() == NULL);
        AYAASSERT(newOperationParent != NULL);
...
bool Aya::CSGOperations::doNegate(std::vector<boost::shared_ptr<Aya::Instance>>::const_iterator start,
    std::vector<boost::shared_ptr<Aya::Instance>>::const_iterator end, std::vector<shared_ptr<Aya::Instance>>& toSelect)
{
    std::vector<shared_ptr<Aya::Instance>> toRemove;

    bool success = negateSelection(toRemove, toSelect, start, end);

    if (!success)
    {
        for (std::vector<shared_ptr<Aya::Instance>>::iterator iter = toSelect.begin(); iter != toSelect.end(); ++iter)
            (*iter)->setParent(NULL);
        return true;
    }

    for (std::vector<shared_ptr<Aya::Instance>>::iterator iter = toRemove.begin(); iter != toRemove.end(); ++iter)
        (*iter)->setParent(NULL);

    return true;
}

bool Aya::CSGOperations::doSeparate(std::vector<boost::shared_ptr<Aya::Instance>>::const_iterator start,
    std::vector<boost::shared_ptr<Aya::Instance>>::const_iterator end, std::vector<shared_ptr<Aya::Instance>>& ungroupedItems)
{
    // First make a copy of the selection list
    Instances itemsToUngroup;
    for (std::vector<boost::shared_ptr<Aya::Instance>>::const_iterator iter = start; iter != end; ++iter)
    {
...
bool Aya::CSGOperations::setOperationMesh(shared_ptr<PartOperation> partOperation, Instances instances)
{
...
bool Aya::CSGOperations::recalculateMesh(shared_ptr<PartOperation> partOperation)
{
...
bool Aya::CSGOperations::createPartMesh(PartInstance* part, CSGMesh* modelData)
{
...
void Aya::CSGOperations::resetInstanceCFrame(std::vector<boost::shared_ptr<Instance>>::const_iterator start,
    std::vector<boost::shared_ptr<Instance>>::const_iterator end, boost::unordered_map<boost::shared_ptr<Instance>, CoordinateFrame> originalCFrames)
{
...
static void warnIfChildInstance(shared_ptr<Aya::Instance> instance, shared_ptr<Aya::PartInstance> partInstance, const std::string& operationType)
{
    if (instance->fastDynamicCast<Aya::DataModelMesh>())
        return;

    Aya::StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "Warning:  Child object \"%s\" of parent \"%s\" is not supported by %s.",
        instance->getFullName().c_str(), partInstance->getFullName().c_str(), operationType.c_str());
}

void Aya::CSGOperations::warnIfChildInstances(shared_ptr<PartInstance> partInstance, const std::string& operationType)
{
    partInstance->visitDescendants(boost::bind(&warnIfChildInstance, _1, partInstance, operationType));
}

const char* Aya::CSGOperations::unionFailedMsg =
    "This union is not currently solvable.  Try offseting the parts from each other or try unioning each part individually.";

bool Aya::CSGOperations::setUnionMesh(shared_ptr<PartOperation> finalPartOperation, std::vector<boost::shared_ptr<Instance>>::const_iterator start,
    std::vector<boost::shared_ptr<Instance>>::const_iterator end, bool isChildRecalculation)
{
        ...
        if (shared_ptr<PartInstance> partInstance = Instance::fastSharedDynamicCast<PartInstance>(*iter))
        {
            ...
            if (!isChildRecalculation)
            {
                if (!primaryDeltaSet)
                {
                    ...
                    primaryDelta =
                        CoordinateFrame(partInstance->getCoordinateFrame().rotation.inverse(), -partInstance->getCoordinateFrame().translation);
                }
                originalCFrames.emplace(*iter, partInstance->getCoordinateFrame());
                partInstance->setCoordinateFrame(
                    CoordinateFrame(primaryDelta.rotation) * (partInstance->getCoordinateFrame() + primaryDelta.translation));
            }
        }
    ...
    {
        char buf[128];
        sprintf(buf, "This union would be %d triangles.  There is a %d triangle limit for unions.", (int)numTriangles,
            (int)PartOperation::getMaximumTriangleCount());
        if (!isChildRecalculation)
            resetInstanceCFrame(start, end, originalCFrames);
        finalPartOperation->setParent(NULL);
        operationFailed("Union Rejected", buf);
...
bool Aya::CSGOperations::setNegateMesh(shared_ptr<NegateOperation> negateOperation, shared_ptr<Instance> instance)
{
    ...
    NonReplicatedCSGDictionaryService* nrDictionaryService =
        FFlag::CSGDelayParentingOperationToEnd ? mDatamodel->create<NonReplicatedCSGDictionaryService>()
                                               : ServiceProvider::create<NonReplicatedCSGDictionaryService>(DataModel::get(negateOperation.get()));
    ...
    CSGMeshCGAL meshData = *static_cast<CSGMeshCGAL*>(negateOperation->getMesh().get());
...
bool Aya::CSGOperations::negateSelection(Instances& toRemove, Instances& toSelect, std::vector<boost::shared_ptr<Instance>>::const_iterator start,
    std::vector<boost::shared_ptr<Instance>>::const_iterator end)
{
    ...
    for (std::vector<boost::shared_ptr<Instance>>::const_iterator iter = start; iter != end; ++iter)
    {
        if (!(Instance::fastSharedDynamicCast<PartOperation>(*iter) || Instance::fastSharedDynamicCast<NegateOperation>(*iter) ||
...
bool Aya::CSGOperations::separate(const shared_ptr<Instance>& wInstance, Instances& separatedItems, Instances& itemsToDelete)
{
    CSGDictionaryService* dictionaryService = ServiceProvider::create<CSGDictionaryService>(DataModel::get(wInstance.get()));
    NonReplicatedCSGDictionaryService* nrDictionaryService =
        ServiceProvider::create<NonReplicatedCSGDictionaryService>(DataModel::get(wInstance.get()));
    ...
    if (shared_ptr<PartOperation> partOperation = Instance::fastSharedDynamicCast<PartOperation>(wInstance))
    {
        ...
        for (std::vector<boost::shared_ptr<Instance>>::const_iterator iter = instances.begin(); iter != instances.end(); ++iter)
        {
            ...
            if (shared_ptr<PartInstance> part = Instance::fastSharedDynamicCast<PartInstance>(*iter))
            {
                ...
                newFrame.translation =
                    (partOperation->getCoordinateFrame().rotation * newFrame.translation) + partOperation->getCoordinateFrame().translation;
...
Aya::Instance* Aya::CSGOperations::getCommonParentOrWorkspace(Instances::const_iterator start, Instances::const_iterator end)
{
