...
ScriptTextEditor::ScriptTextEditor(RobloxScriptDoc* pScriptDoc, RobloxMainWindow* pMainWindow)
    ...
    , m_selectionMutex()
{
    ...
    m_PropertyChangedConnection =
        AuthoringSettings::singleton().propertyChangedSignal.connect(boost::bind(&ScriptTextEditor::onPropertyChanged, this, _1));
...
void ScriptTextEditor::focusInEvent(QFocusEvent* evt)
{
    ...
    if (AuthoringSettings::singleton().intellisenseEnabled)
        connect(
            &Studio::Intellesense::singleton(), SIGNAL(doubleClickSignal(QListWidgetItem*)), this, SLOT(intellesenseDoubleClick(QListWidgetItem*)));
}
...
void ScriptTextEditor::setViewportMargins(int left, int top, int right, int bottom)
{
...
void ScriptTextEditor::homeEvent(QKeyEvent* e)
{
    ...
    int moveOffset = left.indexOf(QRegularExpression("\\S"));

    if (moveOffset < 0)
        currentCursor.movePosition(
            QTextCursor::StartOfBlock, (e->modifiers() == Qt::ShiftModifier) ? QTextCursor::KeepAnchor : QTextCursor::MoveAnchor);
    else
        currentCursor.setPosition(
            currentCursor.block().position() + moveOffset, (e->modifiers() == Qt::ShiftModifier) ? QTextCursor::KeepAnchor : QTextCursor::MoveAnchor);
...
void ScriptTextEditor::endEvent(QKeyEvent* e)
{
    ...
    int moveOffset = right.lastIndexOf(QRegularExpression("\\S"));

    if (moveOffset < 0)
        currentCursor.movePosition(
            QTextCursor::EndOfBlock, (e->modifiers() == Qt::ShiftModifier) ? QTextCursor::KeepAnchor : QTextCursor::MoveAnchor);
    else
        currentCursor.setPosition(
            currentCursor.position() + moveOffset + 1, (e->modifiers() == Qt::ShiftModifier) ? QTextCursor::KeepAnchor : QTextCursor::MoveAnchor);
...
void ScriptTextEditor::keyPressEvent(QKeyEvent* e)
{
    ...
    else if (key == Qt::Key_Return || key == Qt::Key_Backspace)
    {
        ...
        AYAASSERT(!currentTextCursor.isNull());

        QTextBlock startBlock = currentTextCursor.block();
        AYAASSERT(startBlock.isValid());
        AYAASSERT(startBlock.userData());
        ...
        if ((userData.getFoldState() > 0) && endBlock.isValid() && (startBlock != endBlock) && !endBlock.isVisible())
        {
            // if cursor is at the end then just set it at the end of end block
            if (key == Qt::Key_Return && !currentTextCursor.hasSelection() && currentTextCursor.atBlockEnd())
            {
...
void ScriptTextEditor::replaceSelection(int startPos, int endPos, const QString& text)
{
    if (text.isEmpty())
        return;
    ...
    if (previousBlock.isValid())
    {
        ...
        if (pUserData)
        {
            ...
            if ((foldState < 0) || isElse)
            {
                ...
                {
                    requiredTabs = ScriptEditorUtils::noOfStartingTabs(ScriptEditorUtils::findFoldBoundary(previousBlock, true));
                }
            ...
            {
                const int noOfOpeningTabs = ScriptEditorUtils::noOfStartingTabs(ScriptEditorUtils::findFoldBoundary(previousBlock, true));
...
void ScriptTextEditor::autoCompleteBlock(const QString& text, QString& indent)
{
    ...
    QRegularExpression commentMatchingExp = QRegularExpression("(--)?\\[=*\\[");
    if (text.contains(QRegularExpression("\\brepeat\\b")) && !text.contains(QRegularExpression("\\buntil\\b")))
        foldEnd = QString("until");
    else
    {
        QRegularExpressionMatch match = commentMatchingExp.match(text);
        if (match.hasMatch() && !text.contains(match.captured(0).replace('[', ']')))
        {
            foldEnd = QString(match.captured(0).replace('[', ']'));
        }
    }
 ...
 */
void ScriptTextEditor::setErrorLine(int errorLine, QString errorMessage)
{
    ...
    if (visibleLine >= 0 && (m_visibleErrorLine != visibleLine || m_errorMessage != message))
    {
...
void ScriptTextEditor::setAnalysisResult(const Aya::ScriptAnalyzer::Result& analysisResults)
{
    ...
    QList<QTextEdit::ExtraSelection> extraSelections;
    Aya::ScriptAnalyzer::Location loc;
    int startPos = -1, endPos = -1;
    bool atEof = false;

    if (analysisResults.error)
    {
        Aya::ScriptAnalyzer::Error error = analysisResults.error.get();
        loc = error.location;
    ...
    {
        Aya::ScriptAnalyzer::Warning warning = analysisResults.warnings[i];
        ...
        {
            QString message;
            message.asprintf("W%03d: %s", warning.code, warning.text.c_str());
...
bool ScriptTextEditor::appendExtraSelection(
    const QTextCharFormat& format, int startPos, int endPos, bool atEof, QList<QTextEdit::ExtraSelection>& appendList)
{
...
void ScriptTextEditor::processTooltip(QHelpEvent* evt)
{
    ...
    std::pair<MessageCollection::const_iterator, MessageCollection::const_iterator> ret =
        m_scriptMessageForRange.equal_range(Range(cursorPos, cursorPos));
...
void ScriptTextEditor::commentSelection()
{
    ...
    while (tmpBlock.isValid() && tmpBlock.blockNumber() <= endBlock.blockNumber())
    {
        ...
        bool isLastBlock = (tmpBlock == document()->lastBlock());
        if (isLastBlock || (!isDownwards && isEndInMid && (tmpBlock.blockNumber() == endBlock.blockNumber())))
        {
            modifiedText.remove(modifiedText.length() - 1, 1); // Remove the extra "\n"
        }
...
void ScriptTextEditor::moveNext(bool forward)
{
    ...
    {
        std::map<int, int>::iterator locationIter =
            forward ? m_foundItems.lower_bound(cursor.selectionEnd()) : m_foundItems.lower_bound(cursor.selectionStart());
        if (forward)
        {
...
void ScriptTextEditor::replaceNext(const QRegularExpression& rx, const QString& text)
{
    QMutexLocker lock(&m_selectionMutex);
    if (m_foundItems.size() == 0)
        return;
...
void ScriptTextEditor::replaceAll(const QRegularExpression& rx, const QString& text)
{
    QMutexLocker lock(&m_selectionMutex);
    if (m_foundItems.size() == 0)
        return;
...
void ScriptTextEditor::findNext(bool moveCursorToBeginningOfSelection)
{
    ...
    else
    {
        ...
        if (!FindReplaceProvider::instance().getReplaceDialog()->isVisible() && !FindReplaceProvider::instance().getFindDialog()->isVisible() &&
            !isActiveWindow())
        {
            activateWindow();
            setFocus();
...
bool ScriptTextEditor::actionState(const QString& actionID, bool& enableState, bool& checkedState)
{
    ...
    {
        static QString requireSelectionActions = "cutAction copyAction commentSelectionAction uncommentSelectionAction deleteSelectedAction";
...
void ScriptTextEditor::onPropertyChanged(const Aya::Reflection::PropertyDescriptor* pDescriptor)
{
    ...
    if (!pDescriptor || pDescriptor->category.str == sScriptCategoryName)
    {
        ...
        const int spaceWidth = metrics.horizontalAdvance(' ');
        setTabStopDistance(AuthoringSettings::singleton().editorTabWidth * spaceWidth);
...
{
    const QRegularExpression expr(expression);
    ...
    while (offset >= 0 && offset <= text.length())
    {
        ...
        {
            match = expr.match(text, offset, QRegularExpression::NormalMatch);
        }
        else
        {
            match = expr.match(text, offset, QRegularExpression::NoMatch);
        }
        ...
        if (options & QTextDocument::FindWholeWords)
        {
            ...
            if ((start != 0 && isWordCharacter(text.at(start - 1))) || (end != text.length() && isWordCharacter(text.at(end))))
            {
...
QTextCursor ScriptTextEditor::findInDoc(
    const QTextDocument& doc, const QRegularExpression& expr, const QTextCursor& from, QTextDocument::FindFlags options)
{
...
bool ScriptTextEditor::find(bool loop, const QRegularExpression& regExp, QTextDocument::FindFlags flags)
{
    if (FFlag::StudioEmbeddedFindDialogEnabled)
    {
        clearFoundItems();
...
void ScriptTextEditor::replace(const QString& text, const QRegularExpression& regExp, QTextDocument::FindFlags flags)
{
...
QString ScriptTextEditor::constructStyleSheet()
{
    ...
    styleSheet = QString(STRINGIFY(QPlainTextEdit {
        color:
            % 1;
        background:
            % 2;
...
void ScriptTextEditor::selectText(const Aya::ScriptAnalyzer::Location& location, bool trySettingBlocksVisible)
{
...
void ScriptTextEditor::populateValuesFromLocation(const Aya::ScriptAnalyzer::Location& location, int& startPos, int& endPos, bool& atEof)
{
...
int ScriptTextEditor::getStartPosition(const Aya::ScriptAnalyzer::Location& location)
{
...
int ScriptTextEditor::getEndPosition(const Aya::ScriptAnalyzer::Location& location)
{
    QTextBlock endBlock = document()->findBlockByNumber(location.end.line);
    if (endBlock.isValid() && endBlock.isVisible())
...
void ScriptTextEditor::updateFindSelection()
{
    ...
    Aya::Time timeCheck = Aya::Time::now<Aya::Time::Fast>();
    ...
    while ((Aya::Time::now<Aya::Time::Fast>() - timeCheck).msec() < 10)
    {
    ...
    timeCheck = Aya::Time::now<Aya::Time::Fast>();
    ...
    while ((Aya::Time::now<Aya::Time::Fast>() - timeCheck).msec() < 10)
    {
        for (int i = 0; i < 100; ++i)
        {
            ...
            std::multimap<int, QTextEdit::ExtraSelection>::iterator selectionToRemove =
                m_extraSelectionsToRemove.lower_bound(firstVisibleBlockPosition);
...
{
    shared_ptr<Aya::DataModel> dm = m_pDocument->getDataModel();

    void* resultPtr = NULL;

    {
        Aya::DataModel::LegacyLock l(dm.get(), Aya::DataModelJob::Write);

        std::string code = m_pScriptEditor->toPlainText().toStdString();
        shared_ptr<Aya::Instance> scriptInstance = m_pDocument->getCurrentScript().toInstance();
        // Linked script source won't have an instance, so use a fake one to appease the analyzer
        if (!scriptInstance)
        {
            scriptInstance = Aya::Creatable<Aya::Instance>::create<Aya::Script>();
        }

        Aya::ScriptAnalyzer::Result result = Aya::ScriptAnalyzer::analyze(dm.get(), scriptInstance, code);
        QMetaObject::invokeMethod(m_pScriptEditor, "setAnalysisResult", Qt::QueuedConnection, Q_ARG(Aya::ScriptAnalyzer::Result, result));
    }
}
...
void FindThread::setData(const QRegularExpression& regExp, QTextDocument::FindFlags flags)
{
    restart = true;
...
bool FindThread::findInBlockNoCursor(
    const QString& text, int startOfBlock, const QRegularExpression& expression, int offset, QTextDocument::FindFlags options, int& start, int& end)
{
    QRegularExpressionMatch match;
    int idx = -1;
    ...
    while (offset >= 0 && offset <= text.length())
    {
        ...
        match = (options & QTextDocument::FindBackward)
                    ? expression.match(text, offset, QRegularExpression::NormalMatch, QRegularExpression::AnchoredMatchOption)
                    : expression.match(text, offset);
        idx = match.capturedStart();
        if (idx == -1)
            return false;
...
void FindThread::doWork()
{
    ...
    Aya::DataModel::LegacyLock lock(dm.get(), Aya::DataModelJob::Write);
    ...
    while (m_searchStartBlockLine != m_searchLastBlockLine)
    {
        ...
        if (hasFoundItem && (Aya::Time::now<Aya::Time::Fast>() - timeCheck).msec() > 20)
        {
            timeCheck = Aya::Time::now<Aya::Time::Fast>();
            hasFoundItem = false;
    ...
    QMetaObject::invokeMethod(
        FindReplaceProvider::instance().getEmbeddedFindDialog(), "setNumberOfFoundResults", Q_ARG(int, m_pScriptEditor->numberOfFoundItems()));
...
QTextBlock ScriptEditorUtils::findFoldBoundary(QTextBlock endBlock, bool bUp)
{
