...
public:
    ...
    void update(const Aya::ScriptAnalyzer::Warning& warning)
    {
        m_location = warning.location;
        m_warningCode = warning.code;
        m_bIsError = false;

        QString text = QString::fromStdString(warning.text).toHtmlEscaped();

        setText(0, QString("<a href=\"http://\">%1</a>: (%2,%3) %4")
                       .arg(getWarningCodeString(m_warningCode))
                       .arg(m_location.begin.line + 1)
                       .arg(m_location.begin.column + 1)
                       .arg(text));
    }

    void update(const Aya::ScriptAnalyzer::Error& error)
    {
        m_location = error.location;
        m_warningCode = 0;
        m_bIsError = true;

        QString text = QString::fromStdString(error.text).toHtmlEscaped();
    ...
    bool hasHyperLinkText(const QPoint& pos)
    {
        if (!isError())
        {
            ...
            if (pTreeWidget)
            {
                ...
                rect.setRight(rect.left() + fm.horizontalAdvance(getWarningCodeString(0)) + kSADocLeftMargin);
    ...
    QVariant data(int column, int role) const
    {
        if (role == Qt::ToolTipRole)
        {
            ...
            if (!colText.isEmpty())
                return colText.remove(QRegularExpression("<[^>]*>"));
        }
        return QTreeWidgetItem::data(column, role);
    }

    const Aya::ScriptAnalyzer::Location& getLocation()
    {
        return m_location;
    }
    bool isError() const
    ...
    static QString getWarningCodeString(int code)
    {
        return QString().asprintf("W%03d", code);
    }

private:
    Aya::ScriptAnalyzer::Location m_location;
    int m_warningCode;
    bool m_bIsError;
};
    ...
    ScriptCategoryItem(const LuaSourceBuffer& script)
    {
        ...
        if (script.toInstance())
            m_cPropertyChangedConnection =
                script.toInstance()->propertyChangedSignal.connect(boost::bind(&ScriptCategoryItem::onPropertyChanged, this, _1));
    }
            ...
            if (pTreeWidget)
            {
                if (FFlag::StudioUpdateSAResultsInUIThread)
                    QMetaObject::invokeMethod(
                        pTreeWidget, "updateItemName", Qt::QueuedConnection, Q_ARG(QModelIndex, pTreeWidget->indexFromItem(this)));
                else
    ...
    void paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
    {
        ...
        if (optionV4.state & QStyle::State_Selected)
        {
            ...
            if (pItem && pItem->isError())
            {
                ...
                htmlText = QString("<font color=\"yellow\">%1</font>").arg(optionV4.text.remove(QRegularExpression("<[^>]*>")));
            }
            else
            {
...
void ScriptAnalysisTreeWidget::setDataModel(boost::shared_ptr<Aya::DataModel> dataModel)
{
    AYAASSERT(m_dataModel == NULL);
    m_dataModel = dataModel;
}

void ScriptAnalysisTreeWidget::updateResults(
    const LuaSourceBuffer& script, const Aya::ScriptAnalyzer::Result& result, bool showErrors, bool showWarnings)
{
    if (script.empty())
        return;
...
{
    m_dataModel = boost::shared_ptr<Aya::DataModel>();
    m_InstanceMap.clear();
...
void ScriptAnalysisTreeWidget::updateItemName(const QModelIndex& index)
{
    ...
    if (pItem)
    {
        ...
        if (!script.empty())
        {
            Aya::DataModel::LegacyLock lock(m_dataModel, Aya::DataModelJob::Read);
            pItem->setText(0, QString("<b>%1</b>").arg(script.getFullName().c_str()));
...
void ScriptAnalysisTreeWidget::instanceRemoving(boost::shared_ptr<Aya::Instance> script)
{
...
void ScriptAnalysisTreeWidget::categoryItemBecomingLinked(QTreeWidgetItem* item)
{
    ...
    if (FFlag::StudioUpdateSAResultsInUIThread)
    {
        ...
        Aya::ScriptAnalyzer::Result result;
        QMetaObject::invokeMethod(qobject_cast<ScriptAnalysisWidget*>(parent()), "updateResultsInternal", Qt::AutoConnection,
            Q_ARG(shared_ptr<Aya::Instance>, script.toInstance()), Q_ARG(Aya::ScriptAnalyzer::Result, result));
    }
    else
    {
...
//--------------------------------------------------------------------------------------------
LinkedSourceInstance::LinkedSourceInstance(boost::shared_ptr<Aya::LuaSourceContainer> instance)
    : m_LuaSourceContainer(instance)
{
    m_cPropertyChangedConnection =
        m_LuaSourceContainer->propertyChangedSignal.connect(boost::bind(&LinkedSourceInstance::onPropertyChanged, this, _1));
}

void LinkedSourceInstance::onPropertyChanged(const Aya::Reflection::PropertyDescriptor* pDescriptor)
{
    ...
    , m_bDisplayCurrentScriptState(true)
{
    ...
    qRegisterMetaType<Aya::HttpFuture>("Aya::HttpFuture");
    ...
    m_pErrorsToolButton->setIcon(QIcon(":/images/icons/16x16/stop_16.png"));
    ...
    m_pWarningsToolButton->setIcon(QIcon(":/images/icons/16x16/bug_error.png"));
    ...
    connect(&RobloxDocManager::Instance(), SIGNAL(currentDocChanged()), this, SLOT(onCurrentDocChanged()));

    updateButtonsTextAndState();
}

void ScriptAnalysisWidget::updateResults(const LuaSourceBuffer& script, const Aya::ScriptAnalyzer::Result& result)
{
    AYAASSERT(!FFlag::StudioUpdateSAResultsInUIThread || (QThread::currentThread() == qApp->thread()));
...
void ScriptAnalysisWidget::setDataModel(boost::shared_ptr<Aya::DataModel> dataModel)
{
    m_dataModel = boost::shared_ptr<Aya::DataModel>();
    ...
    if (dataModel && dataModel->isStudio() && (UpdateUIManager::Instance().getMainWindow().getBuildMode() != BM_BASIC))
    {
        ...
        m_dataModelDescendantAdded =
            dataModel->getOrCreateDescendantAddedSignal()->connect(boost::bind(&ScriptAnalysisWidget::updateResultsInstance, this, _1));

        m_dataModelDescendantRemoving =
            dataModel->getOrCreateDescendantRemovingSignal()->connect(boost::bind(&ScriptAnalysisWidget::onInstanceRemoving, this, _1));

        m_treeWidgetEmbeddedSourceRemoved =
            m_pTreeWidget->embeddedSourceRemoved.connect(boost::bind(&ScriptAnalysisWidget::updateResultsInstance, this, _1));
...
void ScriptAnalysisWidget::onInstanceRemoving(boost::shared_ptr<Aya::Instance> instance)
{
    ...
    if (!script.empty())
    {
        ...
        if (FFlag::StudioUpdateSAResultsInUIThread)
        {
            // this will make sure we are removing the category item as the result is empty
            Aya::ScriptAnalyzer::Result result;
            QMetaObject::invokeMethod(this, "updateResultsInternal", Qt::AutoConnection, Q_ARG(shared_ptr<Aya::Instance>, instance),
                Q_ARG(Aya::ScriptAnalyzer::Result, result));
        }
        else
        {
...
template<typename T>
static const T* findFirstAncestorByType(const Aya::Instance* instance)
{
    while (instance)
    {
...
void ScriptAnalysisWidget::updateResultsInstance(const shared_ptr<Aya::Instance>& instance)
{
    if (!instance)
        return;

    if (!instance->isA<Aya::Script>() && !instance->isA<Aya::ModuleScript>())
        return;

    if (findFirstAncestorByType<Aya::CoreGuiService>(instance.get()))
        return;
    ...
    if (instance && (instance->isA<Aya::Script>() || instance->isA<Aya::ModuleScript>()))
    {
        ...
        {
            Aya::ScriptAnalyzer::Result result = Aya::ScriptAnalyzer::analyze(m_dataModel.get(), instance, script.getScriptText());
...
void ScriptAnalysisWidget::onLinkedSourceRemoved(boost::shared_ptr<LinkedSourceInstance> linkedSourceInstance)
{
    ...
    if (iter != m_LinkedSources.end())
    {
        ...
        {
            QMetaObject::invokeMethod(
                this, "analyzeAndUpdateResults", Qt::QueuedConnection, Q_ARG(shared_ptr<Aya::Instance>, linkedSourceInstance->getInstance()));
        }
        else
        {
...
static void invokeAnalyzeScriptAssetSource(ScriptAnalysisWidget* analysisWidget, const QString& scriptAssetName, Aya::HttpFuture future)
{
    QMetaObject::invokeMethod(
        analysisWidget, "analyzeScriptAssetSource", Qt::QueuedConnection, Q_ARG(QString, scriptAssetName), Q_ARG(Aya::HttpFuture, future));
}

void ScriptAnalysisWidget::requestAnalyzeScriptAssetSource(const QString& scriptAssetName)
{
    /*
    d9mz - https://tenor.com/view/haojks-boyplnt-olaf-i-dont-give-a-fuck-olaf-idgaf-idgaf-olaf-gif-1007619786786395106
    RobloxGameExplorer& gameExplorer = UpdateUIManager::Instance().getViewWidget<RobloxGameExplorer>(eDW_GAME_EXPLORER);
    Aya::ContentId scriptContentId = Aya::ContentId::fromGameAssetName(scriptAssetName.toStdString());

    Aya::HttpFuture future = LuaSourceBuffer::getScriptAssetSource(scriptContentId, gameExplorer.getCurrentGameId());
    if (future.is_ready())
    {
...
void ScriptAnalysisWidget::analyzeScriptAssetSource(const QString& scriptAssetName, Aya::HttpFuture future)
{
    RobloxGameExplorer& gameExplorer = UpdateUIManager::Instance().getViewWidget<RobloxGameExplorer>(eDW_GAME_EXPLORER);
    ...
    shared_ptr<Aya::Instance> scriptInstance = Aya::Creatable<Aya::Instance>::create<Aya::Script>();

    Aya::DataModel::LegacyLock lock(m_dataModel.get(), Aya::DataModelJob::Write);
    Aya::ScriptAnalyzer::Result result = Aya::ScriptAnalyzer::analyze(m_dataModel.get(), scriptInstance, scriptCode);

    // update results in main thread
    updateResultsScriptAsset(scriptAssetName, result);
}

void ScriptAnalysisWidget::updateResultsScriptAsset(const QString& scriptAssetName, const Aya::ScriptAnalyzer::Result& scriptResults)
{
    Aya::ContentId contentId = Aya::ContentId::fromGameAssetName(scriptAssetName.toStdString());
    if (!contentId.isNull())
        updateResults(LuaSourceBuffer::fromContentId(contentId), scriptResults);
}

void ScriptAnalysisWidget::updateResultsInternal(shared_ptr<Aya::Instance> script, const Aya::ScriptAnalyzer::Result& result)
{
    Aya::DataModel::LegacyLock lock(m_dataModel.get(), Aya::DataModelJob::Write);
    updateResults(LuaSourceBuffer::fromInstance(script), result);
}

void ScriptAnalysisWidget::analyzeAndUpdateResults(shared_ptr<Aya::Instance> script)
{
    Aya::DataModel::LegacyLock lock(m_dataModel.get(), Aya::DataModelJob::Write);
