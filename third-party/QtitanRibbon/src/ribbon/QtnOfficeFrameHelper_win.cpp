...
typedef HRESULT(WINAPI* PtrDrawThemeTextEx)(
    HANDLE hTheme, HDC hdc, int iPartId, int iStateId, LPCWSTR pszText, int cchText, DWORD dwTextFlags, LPRECT pRect, const HLP_DTTOPTS* pOptions);
...
protected:
    ...
    typedef HWINEVENTHOOK(WINAPI* LPFNSETWINEVENTHOOK)(
        UINT eventMin, UINT eventMax, HMODULE hmodWinEventProc, WINEVENTPROC lpfnWinEventProc, DWORD idProcess, DWORD idThread, UINT dwflags);
...
void CALLBACK OfficeFrameHelperWin::FrameHelperEventHook::WinEventProc(
    HWINEVENTHOOK /*hWinEventHook*/, DWORD event, HWND hWnd, LONG idObject, LONG /*idChild*/, DWORD /*dwEventThread*/, DWORD /*dwmsEventTime*/)
{
...
void OfficeFrameHelperWin::saveMargins()
{
    if (QWindow* window = m_frame->windowHandle())
    {
        ...
        {
            QMargins oldcustomMargins = qvariant_cast<QMargins>(
                QGuiApplication::platformNativeInterface()->windowProperty(platformWindow, QStringLiteral("WindowsCustomMargins")));
...
bool OfficeFrameHelperWin::clientMetrics(int& iCaptionWidth, int& iCaptionHeight) const
{
    ...
    const UINT cbProperSize = (comCtlVersion() < MAKELONG(1, 6)) ? sizeof(QTN_OLDNONCLIENTMETRICS) : sizeof(NONCLIENTMETRICS);
...
bool OfficeFrameHelperWin::isTheme2016() const
{
    ...
    {
        OfficeStyle::Theme theme = officeStyle->getTheme();
        if (theme == OfficeStyle::Office2016Colorful || theme == OfficeStyle::Office2016White || theme == OfficeStyle::Office2016DarkGray ||
...
void OfficeFrameHelperWin::initStyleOption(StyleOptionFrame* option)
{
    ...
    option->isBackstageVisible =
        m_ribbonBar->isBackstageVisible() && (bool)m_ribbonBar->style()->styleHint((QStyle::StyleHint)RibbonStyle::SH_RibbonBackstageHideTabs);
...
void OfficeFrameHelperWin::drawTitleBar(QPainter* painter, const StyleOptionTitleBar& opt)
{
    ...
    ::GetWindowTextW(m_hwndFrame, text, len + 1);
    drawDwmCaptionText(
        painter, opt.rcTitleText, text[0] != Q_NULL ? QString::fromWCharArray(text) : opt.text, opt.clrText, opt.state & QStyle::State_Active, false);
    delete[] text;
    ...
    if (opt.drawIcon && !opt.icon.isNull())
    {
        ...
        if (HICON hIcon = windowIcon(opt.icon, opt.rect))
        {
            ...
            if (primaryScreen)
            {
                // Create a temporary widget to get the window ID
                QWidget tempWidget;
                tempWidget.setScreen(primaryScreen);
                hTheme = pOpenThemeData((HWND)tempWidget.winId(), L"WINDOW");
            }
...
void OfficeFrameHelperWin::drawDwmCaptionText(
    QPainter* painter, const QRect& rc, const QString& strWindowText, const QColor& colText, bool active, bool contextTab)
{
...
HRGN OfficeFrameHelperWin::calcFrameRegion(QSize sz)
{
    ...
    RECT rgnTopFrame[] = {{4, 0, cx - 4, 1}, {2, 1, cx - 2, 2}, {1, 2, cx - 1, 3}, {1, 3, cx - 1, 4}, {0, 4, cx, cy - 4}};

    RECT rgnRibbonBottomFrame[] = {{1, cy - 4, cx - 1, cy - 2}, {2, cy - 2, cx - 2, cy - 1}, {4, cy - 1, cx - 4, cy - 0}};
...
{
    HDC hdc = static_cast<HDC>(
        QGuiApplication::platformNativeInterface()->nativeResourceForBackingStore(QByteArrayLiteral("getDC"), farame->backingStore()));
    *offset = QPoint(0, 0);
...
bool OfficeFrameHelperWin::winEvent(MSG* msg, long* result)
{
    ...
    else if (msg->message == WM_WINDOWPOSCHANGING)
    {
        ...
        if (((lpwndpos->flags & SWP_NOSIZE) == 0) && (m_szFrameRegion != szFrameRegion) && ((getStyle() & WS_CHILD) == 0))
        {
            m_skipNCPaint = true;
        }
        return false;
    ...
    else if (msg->message == WM_NCCALCSIZE && !m_lockNCPaint)
    {
        ...
        //=================Start add by sgy 2019-01-08 �����2013��������ʱtab�ؼ���ˢ������==============
        if (!isTheme2013())
            postRecalcFrameLayout();
        //=================End add by sgy 2019-01-08 �����2013��������ʱtab�ؼ���ˢ������==============
        return true;
    }
    ...
    else if (m_hwndFrame != Q_NULL && msg->message == WM_SHOWWINDOW)
    {
        ...
        if (!bShow)
        {
            ...
            ::GetWindowRect(m_hwndFrame, &rc);
            ::SetWindowPos(
                m_hwndFrame, Q_NULL, rc.left - val, rc.top - valTitle, rc.right - rc.left, rc.bottom - rc.top, SWP_NOZORDER | SWP_NOACTIVATE);
        }
    }
    ...
    else if (msg->message == WM_SHOWWINDOW)
    {
        ...
        if (!bShow)
        {
            ...
            RECT rc;
            ::GetWindowRect(m_hwndFrame, &rc);
            ::SetWindowPos(m_hwndFrame, Q_NULL, rc.left - valLeft, rc.top - val, rc.right - rc.left, rc.bottom - rc.top - valTitle,
    ...
    if (obj == m_frame)
    {
        ...
        if (event->type() == QEvent::Resize)
        {
            ...
            else if (event->type() == QEvent::StyleChange)
            {
                ...
                if (hasMaximize)
                {
                    m_frame->setWindowState(m_frame->windowState() & ~(Qt::WindowMinimized | Qt::WindowMaximized | Qt::WindowFullScreen));
                }
#endif // QT_VERSION >= QT_VERSION_CHECK(5, 0, 0)
                ...
                if (hasMaximize)
                {
                    m_frame->setWindowState((m_frame->windowState() & ~(Qt::WindowMinimized | Qt::WindowFullScreen)) | Qt::WindowMaximized);
                }
#endif // QT_VERSION >= QT_VERSION_CHECK(5, 0, 0)
            }
