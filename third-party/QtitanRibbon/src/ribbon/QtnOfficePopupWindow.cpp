    ...
    do
    {
        ...
        while (index < m_listPopup.count())
        {
            ...
            if (curPopup)
            {
                ...
                QRect rcPopup(
                    QPoint(curPopup->getPosition().x() - szCurPopup.width(), curPopup->getPosition().y() - szCurPopup.height()), szCurPopup);
...
void OfficePopupWindowPrivate::handleMouseMoveEvent(QMouseEvent* event)
{
    ...
    if (m_dragging && m_form)
    {
        ...
        QRect rcScreenGeometry;

        if (primaryScreen)
        {
            rcScreenGeometry = primaryScreen->geometry();
        }
...
/*!
    Static method to quick show the office popup box with given \a parent, \a titleIcon - title icon, \a titleText - title text and bodyText - body
   text. You may use HTML to fill \a titleText and \a bodyText. Optional parameters \a closeButtonVisible and \a decoration are used for the
   customization purposes. After it will be closed the instance of the window will be deleted automaticaly.

    \image ribbonui_officepopupwindow5.png
    To show such message on the desktop you need invoke code bellow:

    \code
    QString titleHtml = "<span style=\"font-family:'MS Shell Dlg 2'; font-size:8pt; font-weight:600; font-style:normal;\">John Smith</span><br>Re:
   Your request";
    ...
    \endcode
*/
void OfficePopupWindow::showPopup(QWidget* parent, const QIcon& titleIcon, const QString& titleText, const QString& bodyText, bool closeButtonVisible,
    OfficeStyle::OfficePopupDecoration decoration)
{
...
/*!
\brief Sets the drag-and-drop \a enabled flag for the popup box. If \a enabled is true then the user may use grip to drag-and-drop window over
desktop.
*/
