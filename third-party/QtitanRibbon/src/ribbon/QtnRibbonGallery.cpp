...
bool RibbonGalleryPrivate::eventFilter(QObject* object, QEvent* event)
{
    ...
    if (event->type() == QEvent::Show)
    {
        if (OfficePopupMenu* popup = qobject_cast<OfficePopupMenu*>(object))
        {
            QTN_P(RibbonGallery);
            QPoint pos;
            QScreen* screenAtPopup = QGuiApplication::screenAt(m_buttonPopup->geometry().center());
            QRect screen;

            if (screenAtPopup) {
                screen = screenAtPopup->availableGeometry();
            } else {
...
QSize RibbonGalleryItem::sizeHint() const
{
    ...
    return d.m_items->qtn_d().m_sizeItem;
}
...
QRect RibbonGallery::borders() const
{
    ...
    rcBorders.adjust(margins.left(), margins.top(), margins.right(), margins.bottom());
