...
void RibbonBarPrivate::init()
{
    ...
    m_logotypeLabel = std::make_unique<LogotypeLabel>(&p);
    ...
    if (validIndex(index))
    {
        ...
        else
        {
            if (RibbonPage* page = p.getPage(index))
            {
                ...
                QPoint pt = p.mapToGlobal(QPoint(0, 0));
                ...
                if (screen) {
                    ...
                    // Fallback to primary screen if no screen is found at the point 'p'
                ...
                int h = height;
                if (p.mapToGlobal(QPoint(0, rect.bottom())).y() + h <= availableGeometry.height()) 
...
void RibbonBar::setMinimizedFlag(bool flag)
{
    ...
    if (d.m_minimized != flag)
    {
        ...
        if (d.m_minimized)
        {
		   //�˴��ᵼ��ribbon��С��ʱ��ǰ�����page�ٴ�ʱ����ʾ��sgy 2019-01-08 ע�͵�
...
#ifdef Q_OS_WIN
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
bool RibbonBar::nativeEvent(const QByteArray& eventType, void* message, qintptr* result)
#else
...
bool RibbonBar::loadTranslation(const QString& country)
{
    ...
    if (country.isEmpty())
    {
        foreach(const QString &language, QLocale().uiLanguages())
        {
