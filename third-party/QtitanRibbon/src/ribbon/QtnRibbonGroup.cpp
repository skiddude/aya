...
void RibbonGroupPrivate::showGroupScroll()
{
    ...
    {
        QScreen* screenAtWidget = QGuiApplication::screenAt(p.geometry().center());
        QRect screen;
...
int RibbonGroupPrivate::calcMinimumWidth(RibbonControlSizeDefinition::GroupSize size) const
{
    ...
    opt.initFrom(&p); //opt.text = m_title;
...
void RibbonGroup::mousePressEvent(QMouseEvent* event)
{
    ...
    if ( isReduced() )
    {
        ...
        if (event->button() == Qt::LeftButton) 
        {
            ...
            int h = rc.height();
            if (this->mapToGlobal(QPoint(0, rc.bottom())).y() + h <= screen()->geometry().height())
                pnt = this->mapToGlobal(rc.bottomLeft());
            else 
                pnt = this->mapToGlobal(rc.topLeft() - QPoint(0, h));

            const int desktopFrame = style()->pixelMetric(QStyle::PM_MenuDesktopFrameWidth, 0, this);
            if (pnt.x() + rc.width() - 1 > screen()->geometry().right() - desktopFrame)
                pnt.setX(screen()->geometry().right() - desktopFrame - rc.width() + 1);
            if (pnt.x() < screen()->geometry().left() + desktopFrame)
                pnt.setX(screen()->geometry().left() + desktopFrame);
