...
QSize RibbonSystemButton::sizeHint() const
{
    ...
    int heightTabs = ribbonBar->qtn_d().heightTabs() - 2;
    return QSize(sz.width() + indent, heightTabs).expandedTo(QSize(0, 0));
}
                    ...
                    if (rectMenu.isValid())
                    {
                        ...
                        if (posButton.y() < posMenu.y())
                        {
                            ...
                            if (ribbonBar->tabBarPosition() != RibbonBar::TabBarBottomPosition)
                            {
                                ...
                                move(QPoint(posMenu.x(), posParent.y() + ribbonBar->titleBarHeight() + ribbonBar->qtn_d().topBorder()));
                            }
                            else
                            {
                                QPoint posParent = ribbonBar->mapToGlobal(ribbonBar->pos());
                                RibbonSystemButton* systemButton = ribbonBar->getSystemButton();
                                move(QPoint(posMenu.x(), posParent.y() + systemButton->geometry().top() + systemButton->geometry().height() / 2 + ribbonBar->qtn_d().topBorder()));
                            }
                        }
                    }
                }
...
{
    RibbonPageSystemPopupListCaption::enterEvent(static_cast<QEnterEvent*>(event));
