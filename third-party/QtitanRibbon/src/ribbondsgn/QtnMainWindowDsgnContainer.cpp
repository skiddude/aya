...
bool MainWindowContainer::canRemove(int index) const
{
    QWidget* widget = m_widgets.at(index);
    if (QToolBar* toolBar = qobject_cast<QToolBar*>(widget)) 
    {
        return m_mainWindow->findChild<QToolBar*>(toolBar->objectName()) != 0;
    } 
    else if (RibbonBar* ribbonBar = qobject_cast<RibbonBar*>(widget)) 
    {
        return m_mainWindow->ribbonBar() == ribbonBar;
    } 
    else if (QMenuBar* menuBar = qobject_cast<QMenuBar*>(widget)) 
    {
        return m_mainWindow->menuBar() == menuBar;
    } 
    else if (QStatusBar* statusBar = qobject_cast<QStatusBar*>(widget)) 
    {
        return m_mainWindow->statusBar() == statusBar;
    } 
    else if (QDockWidget* dockWidget = qobject_cast<QDockWidget*>(widget)) 
    {
        return m_mainWindow->findChild<QDockWidget*>(dockWidget->objectName()) != 0;
    }
...
void MainWindowContainer::remove(int index)
{
    ...
    //else if (widget->metaObject()->className() == create_qtitan_className("RibbonStyle"))
    //{
