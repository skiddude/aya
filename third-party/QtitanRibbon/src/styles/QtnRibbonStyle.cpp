...
static QFont qtn_LOGFONTtoQFont(LOGFONTW& lf, bool dpiaware)
{
...
QFont RibbonStyle::font(const QWidget* widget) const
{
    if (qobject_cast<const RibbonBar*>(widget) || widget == Q_NULL)
    {
      ...
      LOGFONTW convertedLfMenuFont;
      // Directly assign values as ncm.lfMenuFont.lfFaceName is already WCHAR
      wcscpy_s(convertedLfMenuFont.lfFaceName, ncm.lfMenuFont.lfFaceName);
      convertedLfMenuFont.lfHeight = ncm.lfMenuFont.lfHeight;
      convertedLfMenuFont.lfWidth = ncm.lfMenuFont.lfWidth;
      convertedLfMenuFont.lfEscapement = ncm.lfMenuFont.lfEscapement;
      convertedLfMenuFont.lfOrientation = ncm.lfMenuFont.lfOrientation;
      convertedLfMenuFont.lfWeight = ncm.lfMenuFont.lfWeight;
      convertedLfMenuFont.lfItalic = ncm.lfMenuFont.lfItalic;
      convertedLfMenuFont.lfUnderline = ncm.lfMenuFont.lfUnderline;
      convertedLfMenuFont.lfStrikeOut = ncm.lfMenuFont.lfStrikeOut;
      convertedLfMenuFont.lfCharSet = ncm.lfMenuFont.lfCharSet;
      convertedLfMenuFont.lfOutPrecision = ncm.lfMenuFont.lfOutPrecision;
      convertedLfMenuFont.lfClipPrecision = ncm.lfMenuFont.lfClipPrecision;
      convertedLfMenuFont.lfQuality = ncm.lfMenuFont.lfQuality;
      convertedLfMenuFont.lfPitchAndFamily = ncm.lfMenuFont.lfPitchAndFamily;

      return qtn_LOGFONTtoQFont(convertedLfMenuFont, isDPIAware());
#else 
      QMenuBar menu;
...
void RibbonStyle::polish(QWidget* widget)
{
    ...
    else if (qobject_cast<QTabBar*>(widget) && qobject_cast<QMdiArea*>(widget->parentWidget()))
    {
        ...
        QPalette palette = widget->palette();
        palette.setColor(QPalette::Window, helper().getColor("TabManager", "AccessHeader"));
        widget->setPalette(palette);
        ...
        case PM_MenuBarVMargin:
            {
                ...
                if (!option)
                {
                    if (const RibbonBar* ribbonBar = qobject_cast<const RibbonBar*>(widget))
                        ret += ribbonBar->qtn_d().topBorder() + ribbonBar->titleBarHeight() + 2;
                }
            }
            break;
...
bool RibbonStyle::drawGroupControlEntry(const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    ...
    if (widget->property(_qtn_Group).toBool())
    {
        ...
        //bool beginGroupNext = nextWidget->property(_qtn_Begin_Group).toBool();
