...
QColor OfficePaintManager::textColor(
    bool selected, bool pressed, bool enabled, bool checked, bool popuped, BarType barType, BarPosition barPosition) const
{
...
bool OfficePaintManager::drawToolBar(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const QStyleOptionToolBar* toolbar = qstyleoption_cast<const QStyleOptionToolBar*>(opt))
    {
        if (toolbar->state & QStyle::State_Horizontal)
        {
            ...
            p->fillRect(QRect(QPoint(rcFill.left(), rcFill.bottom()), QSize(rcFill.width(), rcFill.height() - rcFill.height() - 2)),
                d.m_clrCommandBar.color(QPalette::Dark));

            p->fillRect(QRect(QPoint(toolbar->rect.left() + 5, toolbar->rect.bottom() - 1), QSize(toolbar->rect.width() /* - 7*/, 1)),
                d.m_clrCommandBar.color(QPalette::Shadow));

            p->fillRect(QRect(QPoint(toolbar->rect.right(), toolbar->rect.top() - 1), QSize(1, toolbar->rect.height())),
                d.m_clrCommandBar.color(QPalette::Shadow));
        }
        else
        {
            ...
            p->fillRect(QRect(QPoint(toolbar->rect.right() - 1, toolbar->rect.top() + 1), QSize(1, toolbar->rect.height() /* - 7*/)),
                d.m_clrCommandBar.color(QPalette::Shadow));

            p->fillRect(QRect(QPoint(toolbar->rect.left(), toolbar->rect.bottom()), QSize(toolbar->rect.width(), 1)),
                d.m_clrCommandBar.color(QPalette::Shadow));
        }
        return true;
    }
...
void OfficePaintManager::drawSysButton(QPainter* p, const QRect& rect, bool down, bool highlighted, bool closeButton) const
{
    if (down || highlighted)
    {
        ...
        drawImage(soImageButton, *p, rect, sourceRectImage(soImageButton.rect(), down || highlighted ? 1 : 0, 2), QRect(QPoint(3, 3), QPoint(3, 3)),
            QColor(0xFF, 0, 0xFF));
    }
}
...
bool OfficePaintManager::drawIndicatorDockWidgetResizeHandle(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    QRgb clr1 = DrawHelpers::blendColors(d.m_palSplitter.color(QPalette::Light).rgb(), d.m_palSplitter.color(QPalette::Dark).rgb(),
        float(opt->rect.bottom() - rcFrame.top()) / fHeight);

    QRgb clr2 = DrawHelpers::blendColors(
        d.m_palSplitter.color(QPalette::Light).rgb(), d.m_palSplitter.color(QPalette::Dark).rgb(), float(opt->rect.top() - rcFrame.top()) / fHeight);
...
bool OfficePaintManager::drawIndicatorMenuCheckMark(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    QRect rc(QPoint(opt->rect.left() + opt->rect.width() * 0.5 - iconSize * 0.5, opt->rect.top() + opt->rect.height() * 0.5 - iconSize * 0.5),
...
// for Buttons
/*! \internal */
bool OfficePaintManager::drawIndicatorCheckRadioButton(
...
/*! \internal */
bool OfficePaintManager::drawIndicatorViewItemCheck(
    QStyle::PrimitiveElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
...
bool OfficePaintManager::drawPanelButtonCommand(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    {
        uint resolve_mask = btn->palette.resolveMask();
        ...
        if (!enabled)
            ...
            state = 1;
        else if ((selected && pressed) /*|| qobject_cast<const QPushButton*>(w)*/)
            state = 2;
        else if (selected || pressed)
            state = 1;

        drawImage(soImage, *p, opt->rect, sourceRectImage(soImage.rect(), state, 5), QRect(QPoint(4, 4), QPoint(4, 4)), QColor(0xFF, 0, 0xFF));
        return true;
    }
    return false;
}
...
bool OfficePaintManager::drawControlEdit(const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    ...
    if (const QStyleOptionFrame* panel = qstyleoption_cast<const QStyleOptionFrame*>(opt))
    {
        ...
        uint resolve_mask = panel->palette.resolveMask();
        ...
        if (widget)
        {
            ...
            if (QAbstractSpinBox* spinbox = qobject_cast<QAbstractSpinBox*>(widget->parentWidget()))
                resolve_mask |= spinbox->palette().resolveMask();
        }
...
bool OfficePaintManager::drawComboBox(const QStyleOptionComplex* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const QStyleOptionComboBox* cmb = qstyleoption_cast<const QStyleOptionComboBox*>(opt))
    {
        ...
        if (cmb->frame)
        {
            ...
            bool usePalette = false;
            uint resolve_mask = cmb->palette.resolveMask();
            if (resolve_mask & (1 << QPalette::Base))
                usePalette = true;
            ...
            //                enabled ? cmb->palette.brush(QPalette::Normal, QPalette::AlternateBase) : cmb->palette.brush(QPalette::Disabled,
            //                QPalette::Base));
...
bool OfficePaintManager::drawSpinBox(const QStyleOptionComplex* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const QStyleOptionSpinBox* sb = qstyleoption_cast<const QStyleOptionSpinBox*>(opt))
    {
        ...
        QPixmap pImage =
            rcTop.width() > 12 && rcTop.height() > 12 ? cached("ControlGalleryScrollArrowGlyphs.png") : cached("ToolbarButtonSpinArrowGlyphs.png");
        ...
        if (sb->subControls & QStyle::SC_SpinBoxUp)
        {
            ...
            if (sb->state & QStyle::State_MouseOver)
                drawImage(soBackground, *p, rcTop, sourceRectImage(soBackground.rect(), state, 10), QRect(QPoint(2, 2), QPoint(2, 2)),
                    QColor(0xFF, 0, 0xFF));
            ...
            QSize sz(rcSrc.size());
            QRect rcDraw(
                QPoint(rcTop.left() + (rcTop.width() / 2 - sz.width() / 2), rcTop.top() + (rcTop.height() / 2 - sz.height() / 2)), rcSrc.size());
        ...
        if (sb->subControls & QStyle::SC_SpinBoxDown)
        {
            ...
            if (sb->state & QStyle::State_MouseOver)
                drawImage(soBackground, *p, rcBottom, sourceRectImage(soBackground.rect(), state, 10), QRect(QPoint(2, 2), QPoint(2, 2)),
                    QColor(0xFF, 0, 0xFF));
            ...
            QSize sz(rcSrc.size());
            QRect rcDraw(
                QPoint(rcBottom.left() + (rcBottom.width() / 2 - sz.width() / 2), rcBottom.top() + (rcBottom.height() / 2 - sz.height() / 2)),
                rcSrc.size());
            drawImage(pImage, *p, rcDraw, rcSrc, QRect(QPoint(0, 0), QPoint(0, 0)), QColor(0xFF, 0, 0xFF));
        }
        return true;
...
bool OfficePaintManager::drawTabBarTabShape(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    {
        QPixmap soImage = tab->position == QStyleOptionTab::Beginning || tab->position == QStyleOptionTab::OnlyOneTab ? cached("TabItemTopLeft.png")
                                                                                                                      : cached("TabItemTop.png");
...
bool OfficePaintManager::drawScrollBar(const QStyleOptionComplex* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const QStyleOptionSlider* scrollbar = qstyleoption_cast<const QStyleOptionSlider*>(opt))
    {
        ...
        bool light =
            w && (qobject_cast<const RibbonGallery*>(w->parentWidget()) || qobject_cast<const QDialog*>(w->topLevelWidget()) || ::isAncestor(w));

        QPixmap soImage;
        if (opt->state & QStyle::State_Horizontal)
        {
            soImage = light ? cached("ControlGalleryScrollHorizontalLight.png") : cached("ControlGalleryScrollHorizontalDark.png");
            drawImage(soImage, *p, newScrollbar.rect, sourceRectImage(soImage.rect(), 0, 2), QRect(QPoint(0, 1), QPoint(0, 1)));
        }
        else
        {
            soImage = light ? cached("ControlGalleryScrollVerticalLight.png") : cached("ControlGalleryScrollVerticalDark.png");
            drawImage(soImage, *p, newScrollbar.rect, sourceRectImage(soImage.rect(), 0, 2), QRect(QPoint(1, 0), QPoint(1, 0)));
        }
...
bool OfficePaintManager::drawScrollBarLine(QStyle::ControlElement element, const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const QStyleOptionSlider* scrollbar = qstyleoption_cast<const QStyleOptionSlider*>(opt))
    {
        ...
        QPixmap soImage;
        if (opt->state & QStyle::State_Horizontal)
            soImage = light ? cached("ControlGalleryScrollArrowsHorizontalLight.png") : cached("ControlGalleryScrollArrowsHorizontalDark.png");
        else
            soImage = light ? cached("ControlGalleryScrollArrowsVerticalLight.png") : cached("ControlGalleryScrollArrowsVerticalDark.png");
        ...
        if (!opt->rect.isEmpty())
        {
            ...
            if (opt->state & QStyle::State_Horizontal)
            {
                if (opt->rect.width() > 10)
                {
                    ...
                    drawImage(
                        soImageScrollThumbGripperHorizontal, *p, rcGripper, sourceRectImage(soImageScrollThumbGripperHorizontal.rect(), state, 3));
                }
            }
            else
            {
...
bool OfficePaintManager::drawScrollBarPage(QStyle::ControlElement element, const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    bool light = w && (qobject_cast<const RibbonGallery*>(w->parentWidget()) || qobject_cast<const QDialog*>(w->topLevelWidget()) || ::isAncestor(w));
...
bool OfficePaintManager::drawHeaderSection(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    DrawHelpers::drawGradientFill(*p, opt->rect,
        columnPressed ? d.m_palGradientColumnPushed.color(QPalette::Light) : d.m_palGradientColumn.color(QPalette::Light),
        columnPressed ? d.m_palGradientColumnPushed.color(QPalette::Dark) : d.m_palGradientColumn.color(QPalette::Dark), true);

    p->fillRect(QRect(QPoint(opt->rect.left(), opt->rect.bottom()), QSize(opt->rect.width(), 1)), d.m_palGradientColumn.color(QPalette::Shadow));
...
bool OfficePaintManager::drawHeaderEmptyArea(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    DrawHelpers::drawGradientFill(*p, opt->rect, d.m_palGradientColumn.color(QPalette::Light), d.m_palGradientColumn.color(QPalette::Dark), true);
...
bool OfficePaintManager::drawIndicatorHeaderArrow(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const QStyleOptionHeader* header = qstyleoption_cast<const QStyleOptionHeader*>(opt))
    {
        ...
        {
            DrawHelpers::drawTriangle(
                *p, QPoint(pt.x() - 4, pt.y() - 2), QPoint(pt.x(), pt.y() + 2), QPoint(pt.x() + 4, pt.y() - 2), d.m_clrColumnSeparator);
        }
    ...
    case QStyle::PE_IndicatorArrowLeft:
    {
        ...
        else if (pe == QStyle::PE_IndicatorArrowDown)
        {
            if (qobject_cast<const QToolButton*>(w) && !qobject_cast<const QTabBar*>(w->parentWidget()))
            {
                ...
                rcEntry.translate(opt->rect.width() / 2, (opt->rect.height() - rcSrc.height()) / 2 + ((opt->rect.height() - rcSrc.height()) % 2));
                drawImage(soIndicator, *p, rcEntry, rcSrc, QRect(QPoint(0, 0), QPoint(0, 0)), QColor(0xFF, 0, 0xFF));
                return true;
            }
        }
...
#define CALCSTATE (!enabled ? 4 : pressed ? 3 : selected ? 2 : selected || pressed ? 1 : 0)

                drawImage(soImage, *p, toolbutton->rect, sourceRectImage(soImage.rect(), CALCSTATE, 5), QRect(QPoint(3, 3), QPoint(3, 3)),
                    QColor(0xFF, 0, 0xFF));
                return true;
            }
        }
...
/*! \internal */
void OfficePaintManager::drawRectangle(
    QPainter* p, const QRect& rect, bool selected, bool pressed, bool enabled, bool checked, bool popuped, BarType barType, BarPosition barPos) const
{
    QTN_D_STYLE(const OfficeStyle);
    if (popuped)
    {
    ...
    else if (barType != TypePopup)
    {
        if (!enabled)
        {
            if (/*isKeyboardSelected(selected)*/ false)
            {
                ...
                DrawHelpers::drawGradientFill(*p, rect,
                    checked ? d.m_palLunaPushed.color(QPalette::Light) : d.m_palLunaSelected.color(QPalette::Light),
                    checked ? d.m_palLunaPushed.color(QPalette::Dark) : d.m_palLunaSelected.color(QPalette::Dark), true);
                DrawHelpers::draw3DRect(
                    *p, d.m_clrHighlightPushedBorder, d.m_clrHighlightPushedBorder, rect.x(), rect.y(), rect.width() - 1, rect.height() - 1, true);
            }
            else if (checked)
            {
                // [1]
                DrawHelpers::drawGradientFill(*p, rect, d.m_palLunaChecked.color(QPalette::Light), d.m_palLunaChecked.color(QPalette::Dark), true);
                DrawHelpers::draw3DRect(*p, d.m_clrHighlightDisabledBorder, d.m_clrHighlightDisabledBorder, rect.x(), rect.y(), rect.width() - 1,
                    rect.height() - 1, true);
            }
            return;
        }
        ...
        else if (checked && !selected && !pressed)
        {
            ...
            DrawHelpers::drawGradientFill(*p, rect, d.m_palLunaChecked.color(QPalette::Light), d.m_palLunaChecked.color(QPalette::Dark), true);
            DrawHelpers::draw3DRect(
                *p, d.m_clrHighlightCheckedBorder, d.m_clrHighlightCheckedBorder, rect.x(), rect.y(), rect.width() - 1, rect.height() - 1, true);
        }
        else if (checked && selected && !pressed)
        {
            // [1]
        ...
        else if (selected || pressed)
        {
            ...
            DrawHelpers::draw3DRect(
                *p, d.m_clrHighlightBorder, d.m_clrHighlightBorder, rect.x(), rect.y(), rect.width() - 1, rect.height() - 1, true);
        }
    }
    else // if (barPos == BarPopup && selected && barType == TypePopup)
    {
        ...
        if (!enabled)
        {
            if (/*isKeyboardSelected(bSelected)*/ false)
                DrawHelpers::drawRectangle(
                    *p, rc, d.m_clrHighlightBorder, barPos != BarPopup ? (checked ? d.m_clrHighlightPushed : d.m_clrHighlight) : d.m_clrMenubarFace);
            else if (checked)
...
bool OfficePaintManager::styleHint(
    QStyle::StyleHint /*hint*/, const QStyleOption* /*opt*/, const QWidget* /*widget*/, QStyleHintReturn* /*returnData*/, int& /*ret*/) const
{
    return false;
}
...
QColor OfficePaintManager2013::textColor(
    bool selected, bool pressed, bool enabled, bool checked, bool popuped, BarType barType, BarPosition barPosition) const
{
    return ((OfficeStyle*)baseStyle())->getTextColor(selected, pressed, enabled, checked, popuped, barType, barPosition);
}
...
bool OfficePaintManager2013::drawIndicatorDockWidgetResizeHandle(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (!(opt->state & QStyle::State_Horizontal) || rect.height() > rect.width() * 4)
    {
        ...
        if (const QMainWindow* mainWindow = qobject_cast<const QMainWindow*>(w))
        {
            ...
            QScreen* screenAtMainWindow = QGuiApplication::screenAt(mainWindow->geometry().center());
            QRect screen;

            if (screenAtMainWindow)
            {
                screen = screenAtMainWindow->availableGeometry();
            }
            else
            {
                // Fallback to primary screen if no screen is found at the mainWindow's position
...
// for Buttons
/*! \internal */
bool OfficePaintManager2013::drawIndicatorCheckRadioButton(
    QStyle::PrimitiveElement element, const QStyleOption* option, QPainter* p, const QWidget* widget) const
{
    Q_UNUSED(widget);
    QTN_D_STYLE(OfficeStyle)
    ...
    else
    {
        ...
        if (stateChecked == 1)
        {
            ...
            //            QRect rcIndicator(QPoint(rc.x() + (rc.width() - iImageWidth)/2, rc.y() + (rc.height() - iImageHeight)/2), QSize(iImageWidth,
            //            iImageHeight)); drawIcon(p, rcIndicator.topLeft(), Icon_Check, highlighted ? Black2 : Gray, QSize(9, 9));
        }
...
/*! \internal */
bool OfficePaintManager2013::drawIndicatorViewItemCheck(
    QStyle::PrimitiveElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    return drawIndicatorCheckRadioButton(element, option, painter, widget);
}
...
bool OfficePaintManager2013::drawSpinBox(const QStyleOptionComplex* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const QStyleOptionSpinBox* sb = qstyleoption_cast<const QStyleOptionSpinBox*>(opt))
    {
        ...
        if (sb->subControls & QStyle::SC_SpinBoxDown)
        {
            ...
            drawIcon(
                p, rcBottom, index[false ? 1 : 0][1], enabledIndicator ? OfficePaintManager2013::Black : OfficePaintManager2013::Gray, QSize(9, 9));
        }
...
/*! \internal */
void OfficePaintManager2013::drawRectangle(
    QPainter* p, const QRect& rect, bool selected, bool pressed, bool enabled, bool checked, bool popuped, BarType barType, BarPosition barPos) const
{
    Q_UNUSED(popuped);
    Q_UNUSED(enabled);
...
/*! \internal */
bool OfficePaintManager2013::styleHint(
    QStyle::StyleHint hint, const QStyleOption* opt, const QWidget* w, QStyleHintReturn* returnData, int& ret) const
{
    Q_UNUSED(opt)
    Q_UNUSED(returnData)
    switch (hint)
...
{
    QTN_D_STYLE(OfficeStyle)
    m_clrAccentLight = DARKMODE ? QColor(69, 55, 91) : QColor(231, 217, 253);
    m_clrAccentHilight = DrawHelpers::pixelAlpha(d.m_clrAccent.rgb(), 115);
    m_clrAccentText = d.m_clrAccent;
    ...
    m_clrHighlightDn = DARKMODE ? QColor(28, 28, 28) : QColor(219, 219, 219);
    ...
    d.m_clrEditCtrlBorder = DARKMODE ? QColor(34, 34, 34) : QColor(171, 171, 171);

    d.m_clrMenubarBorder = QColor(182, 182, 182);   // scorll bar otuline + outline of the menu
    ...
    m_clrMenuItemBorder = QColor(212, 212, 212);

    m_clrTextDisabled = m_clrBarDkShadow;
    m_clrControlEditDisabled = m_clrBarFace;
    m_clrControl = m_clrBarHilite;
    m_clrEditBoxBorder = DARKMODE ? QColor(34, 34, 34) : QColor(171, 171, 171);

    m_ImagesBlack = mapTo3dColors(m_ImagesSrc, QColor(0, 0, 0), QColor(119, 119, 119));
    m_ImagesBlack2 = mapTo3dColors(m_ImagesSrc, QColor(0, 0, 0), QColor(136, 136, 136));
}
...
{
    m_ImagesSrc = QPixmap(":/images/icons/menuimg-pro24.png");
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
...
void OfficePaintManager2016::setupPalette(QWidget* widget) const
{
    ...
    else if (theme() == OfficeStyle::Office2016DarkGray)
    {
        ...
        {
            QPalette palette = widget->palette();
            palette.setColor(QPalette::Window, QColor(171, 171, 171));
            widget->setPalette(palette);
        }
