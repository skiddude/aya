        ...
        if (d.m_flatFrame && optRibbonBar->frameHelper)
        {
            ...
            else
            {
                ...
                if (!soImage.isNull())
                {
                    ...
                    QRect rcArea(
                        QPoint(rcRibbonClient.left(), rcRibbonTabs.bottom() - height), QPoint(rcRibbonClient.right(), rcRibbonTabs.bottom()));
...
bool RibbonPaintManager::drawToolBar(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    {
        QPixmap soImage = toolbar->state & QStyle::State_Horizontal ? cached("ToolbarFaceHorizontal.png") : cached("ToolbarFaceVertical.png");
    ...
    if (qobject_cast<const RibbonGroup*>(w))
    {
        if (const QStyleOptionGroupBox* optGroup = qstyleoption_cast<const QStyleOptionGroupBox*>(opt))
        {
            if (optGroup->text.isEmpty())
            {
                p->save();
                int margin = 3;
                p->setPen(opt->palette.color(QPalette::Window).darker(114));
            ...
            else
            {
                ...
                if (optGroup->textAlignment == Qt::AlignBottom)
                {
                    ...
                    int indent = 0;
                    QPalette pal;
                    pal.setColor(QPalette::WindowText, opt->palette.color(QPalette::Window).darker(114));
                    baseStyle()->proxy()->drawItemText(p, rect.adjusted(indent + 1, 1, -indent - 1, -1),
                        Qt::AlignLeft | Qt::AlignVCenter | Qt::TextShowMnemonic, pal, optGroup->state & QStyle::State_Enabled, titleText,
                        QPalette::WindowText);
                    p->restore();
                }
                else if (optGroup->textAlignment == Qt::AlignTop)
                {
                    ...
                    QPalette pal;
                    pal.setColor(QPalette::WindowText, opt->palette.color(QPalette::Window).darker(114));
                    baseStyle()->proxy()->drawItemText(p, rect.adjusted(indent + rect.width() - rectText.width() - 2, 1, -indent - 1, -1),
                        Qt::AlignLeft | Qt::AlignVCenter | Qt::TextShowMnemonic, pal, optGroup->state & QStyle::State_Enabled, titleText,
                        QPalette::WindowText);
                    p->restore();
                }
                else if (optGroup->textAlignment == Qt::AlignVCenter)
                {
                    ...
                    p->setPen(opt->palette.color(QPalette::Window).darker(114));
                    p->drawLine(
                        rect.center().x(), rect.top() + rectText.width() + rectText.width() + margin + 2, rect.center().x(), rect.bottom() - margin);
                    p->drawLine(
                        rect.center().x(), rect.top() + margin, rect.center().x(), rect.bottom() - (rectText.width() + rectText.width() + margin));
                    ...
                    QPalette pal;
                    pal.setColor(QPalette::WindowText, opt->palette.color(QPalette::Window).darker(114));
                    baseStyle()->proxy()->drawItemText(p, rect.adjusted(indent + (rect.width() - rectText.width()) / 2 - 2, 1, -indent - 1, -1),
                        Qt::AlignLeft | Qt::AlignVCenter | Qt::TextShowMnemonic, pal, optGroup->state & QStyle::State_Enabled, titleText,
                        QPalette::WindowText);
                    p->restore();
                }
...
bool RibbonPaintManager::drawPanelButtonTool(const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    ...
    if (widget && !qobject_cast<const RibbonTitleButton*>(widget))
    {
        if (const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
        {
            ...
            if (toolBarControl && widget->property(_qtn_Group).toBool())
            {
                ...
                soImageSplit = left ? cached("ToolbarButtonsSpecialSplitLeft.png") : cached("ToolbarButtonsSpecialSplitCenter.png");

                soImageSplitDropDown =
                    right ? cached("ToolbarButtonsSpecialSplitDropDownRight.png") : cached("ToolbarButtonsSpecialSplitDropDownCenter.png");
                specialOffset = 1;
            }
            else
            {
                soImageSplit = smallSize ? cached("ToolbarButtonsSplit22.png") : cached("ToolbarButtonsSplit50.png");
                soImageSplitDropDown = smallSize ? cached("ToolbarButtonsSplitDropDown22.png") : cached("ToolbarButtonsSplitDropDown50.png");
            }
            ...
            QRect rcSplit = smallSize
                                ? QRect(QPoint(rcButton.left(), rcButton.top()), QPoint(rcButton.right() - popupr.width() - 2, rcButton.bottom()))
            ...
            QRect rcSplitDropDown =
                smallSize ? QRect(QPoint(rcButton.right() - popupr.width() - 1, rcButton.top()), QPoint(rcButton.right(), rcButton.bottom()))
            ...
            {
                if (popuped)
                    stateSplitDropDown = 2;
                else if (!selected && !pressed)
                    stateSplitDropDown = 2;
                else if ((selected && pressed))
                    stateSplitDropDown = 0;
                else if (selected || pressed)
                    stateSplitDropDown = !mouseInSplitDropDown ? 4 : 0;
                else
                    stateSplitDropDown = 4;
            }
            else
            {
                if (popuped)
            ...
            if (stateSplitDropDown != -1)
            {
                if (smallSize)
                {
                    drawImage(soImageSplitDropDown, *p, rcSplitDropDown,
                        sourceRectImage(soImageSplitDropDown.rect(), stateSplitDropDown, 5 + specialOffset), QRect(QPoint(2, 2), QPoint(2, 2)),
                        QColor(0xFF, 0, 0xFF));
                }
                else
                {
    ...
    else if (const StyleCaptionButton* toolbutton = qstyleoption_cast<const StyleCaptionButton*>(opt))
    {
        ...
        if (toolbutton->rect.width() < 27)
            strGlyphSize = "17.png";
        if (toolbutton->rect.width() < 15)
            strGlyphSize = "13.png";
        ...
        if (!soImageIndicator.isNull())
        {
            ...
            QRect rcGlyph(QPoint((toolbutton->rect.right() + toolbutton->rect.left() - rcSrc.width() + 1) / 2,
                              (toolbutton->rect.top() + toolbutton->rect.bottom() - rcSrc.height() + 2) / 2),
                rcSrc.size());
...
void RibbonPaintManager::fillRibbonTabControl(const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    if (const RibbonTab* tab = qobject_cast<const RibbonTab*>(widget))
    {
        if (ContextHeader* contextHeader = tab->getContextHeader())
        {
            ...
            if (!soImage.isNull())
            {
                drawImage(soImage, *p, QRect(rcFirst.left(), opt->rect.top(), rcLast.right() + 1, rcLast.bottom()), soImage.rect(),
                    QRect(QPoint(2, 2), QPoint(2, 2)));
            }
        }
    }
    ...
    if (const StyleOptionBackstageButton* btnStyle = qstyleoption_cast<const StyleOptionBackstageButton*>(opt))
    {
        ...
        if (checked && btnStyle->tabStyle)
        {
            ...
            if (!soImage.isNull())
            {
                ...
                int y = rc.center().y();
                QRect rectImage(
                    QPoint(rc.right() - rcSrc.width(), y - rcSrc.height() / 2), QPoint(rc.right(), y - rcSrc.height() / 2 + rcSrc.height()));
...
void RibbonPaintManager::drawReducedGroup(const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    ...
    if (const QStyleOptionToolButton* optGroup = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
    {
        ...
        if (!strFirstRow.isEmpty())
        {
            ...
            baseStyle()->proxy()->drawItemText(p, rectFirstRow, flags | Qt::AlignHCenter, optGroup->palette, optGroup->state & QStyle::State_Enabled,
                strFirstRow, QPalette::WindowText);
        }
        ...
        if (!strSecondRow.isEmpty())
        {
            ...
            baseStyle()->proxy()->drawItemText(p, rectSecondRow, flags | Qt::AlignLeft, optGroup->palette, optGroup->state & QStyle::State_Enabled,
                strSecondRow, QPalette::WindowText);
        }
        //===========================================================================================
        ...
        QPoint pt = QPoint(strSecondRow.isEmpty() ? rcText.center().x() - 1 : rectSecondRow.left() + width - 4,
            strSecondRow.isEmpty() ? rectFirstRow.bottom() : rectSecondRow.center().y());
            ...
            else
            {
                ...
                if (state == 4)
                {
                    ...
                    if (!soImage.isNull())
                    {
                        ...
                        {
                            drawImage(soImage, *p, opt->rect, sourceRectImage(soImage.rect(), 5, 6), QRect(QPoint(5, 5), QPoint(5, 5)),
                                QColor(0xFF, 0, 0xFF));
                        }
                    }
                }
        ...
        if (selected || highlighted)
        {
            ...
            if (!soImage.isNull())
            {
                if (ribbonOption->tabBarPosition == RibbonBar::TabBarBottomPosition)
                {
                    ...
                    drawImage(
                        soImage, *p, opt->rect, sourceRectImage(soImage.rect(), state, 6), QRect(QPoint(5, 5), QPoint(5, 5)), QColor(0xFF, 0, 0xFF));
            }
        }
    }
...
bool RibbonPaintManager::drawSlider(const QStyleOptionComplex* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const QStyleOptionSlider* slider = qstyleoption_cast<const QStyleOptionSlider*>(opt))
    {
        if (slider->orientation == Qt::Horizontal)
        {
            ...
            QRect rcTrackDest(
                QPoint(groove.left(), (groove.top() + groove.bottom() - soTrack.height()) / 2), QSize(groove.width(), soTrack.height()));
            p->drawPixmap(rcTrackDest, soTrack, soTrack.rect());

            if (slider->maximum >= slider->minimum)
            {
                QPixmap soTick = cached("SliderTick.png");
                QRect rcTrackTickDest(
                    QPoint((groove.left() + groove.right() - soTick.width()) / 2, (groove.top() + groove.bottom() - soTick.height()) / 2),
                    soTick.size());
...
void RibbonPaintManager::drawGroupScrollButton(const QStyleOption* option, QPainter* p, const QWidget* widget) const
{
    ...
    if (const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>(option))
    {
        QPixmap soRibbonScrollButton =
            cached(toolbutton->arrowType == Qt::LeftArrow ? "RibbonGroupScrollButtonLeft.png" : "RibbonGroupScrollButtonRight.png");
        ...
        QPixmap soRibbonScrollGlyph =
            cached(toolbutton->arrowType == Qt::LeftArrow ? "RibbonGroupScrollLeftGlyph.png" : "RibbonGroupScrollRightGlyph.png");
...
bool RibbonPaintManager::drawTabBarTabShape(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const QStyleOptionTab* tab = qstyleoption_cast<const QStyleOptionTab*>(opt))
    {
        ...
        drawImage(
            soImage, painter, QRect(QPoint(0, 0), sz), sourceRectImage(soImage.rect(), state, 4), QRect(QPoint(5, heightMap), QPoint(22, heightMap)));
...
bool RibbonPaintManager::drawFrame(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (qobject_cast<const RibbonMainWindow*>(w))
    {
        if (const StyleOptionFrame* optFrame = qstyleoption_cast<const StyleOptionFrame*>(opt))
        {
            ...
            {
                if (nLeftBorder > 2)
                    p->fillRect(rc.left() + 2, rcFrame.top(), nLeftBorder - 3, bordersHeight,
                        active ? d.m_clrFrameBorderActive2 : d.m_clrFrameBorderInactive2);
                if (nLeftBorder > 2)
                    p->fillRect(rc.left() + nLeftBorder - 1, rcFrame.top(), 1, bordersHeight,
                        active ? d.m_clrFrameBorderActive3 : d.m_clrFrameBorderInactive3);

                if (nRightBorder > 2)
                    p->fillRect(rc.right() - nRightBorder + 1, rcFrame.top(), nRightBorder - 3, bordersHeight,
                        active ? d.m_clrFrameBorderActive2 : d.m_clrFrameBorderInactive2);
                if (nRightBorder > 2)
                    p->fillRect(
                        rc.right() - nRightBorder, rcFrame.top(), 1, bordersHeight, active ? d.m_clrFrameBorderActive3 : d.m_clrFrameBorderInactive3);
            }
            else
            {
                if (nLeftBorder > 2)
                ...
                if (nLeftBorder > 3)
                    p->fillRect(rc.left() + 3, rcFrame.top(), nLeftBorder - 3, bordersHeight,
                        active ? d.m_clrFrameBorderActive3 : d.m_clrFrameBorderInactive3);

                if (nRightBorder > 2)
                    p->fillRect(rc.right() - 3, rcFrame.top(), 1, bordersHeight, active ? d.m_clrFrameBorderActive2 : d.m_clrFrameBorderInactive2);
                if (nRightBorder > 3)
                    p->fillRect(rc.right() - nRightBorder, rcFrame.top(), nRightBorder - 3, bordersHeight,
                        active ? d.m_clrFrameBorderActive3 : d.m_clrFrameBorderInactive3);
            }
            ...
            if (nBottomBorder > 1)
            {
                if (d.m_flatFrame)
                {
                    p->fillRect(rc.left() + nLeftBorder, rc.bottom() - nBottomBorder, rc.width() - nLeftBorder - nRightBorder, 1,
                        active ? d.m_clrFrameBorderActive3 : d.m_clrFrameBorderInactive3);
                    p->fillRect(rc.left() + 1, rc.bottom() - nBottomBorder + 1, rc.width() - 2, nBottomBorder - 2,
                        active ? d.m_clrFrameBorderActive2 : d.m_clrFrameBorderInactive2);
                }
                else
                {
                    p->fillRect(rc.left() + 1, rc.bottom() - nBottomBorder, rc.width() - 2, nBottomBorder - 1,
                        active ? d.m_clrFrameBorderActive3 : d.m_clrFrameBorderInactive3);
                }
            }
            ...
            if (optFrame->titleVisible)
            {
                ...
                {
                    rcTopCenter =
                        QRect(QPoint(rc.left() + rcTopLeft.width(), rc.top()), QPoint(rc.right() - rcSrcTopRight.width(), rcCaption.bottom()));
            ...
            if (optFrame->hasStatusBar)
            {
...
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
                        QRect rcLight(QPoint(rc.left() + 1, rc.bottom() - nStatusHeight - nBottomBorder + 1),
                            QPoint(rc.left() + nLeftBorder, rc.bottom() - nBottomBorder));
#else
                        QRect rcLight(QPoint(rc.left() + 1, rc.bottom() - nStatusHeight - nBottomBorder),
                            QPoint(rc.left() + nLeftBorder, rc.bottom() - nBottomBorder));
#endif
...
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
                        QRect rcDark(QPoint(rc.right() - nRightBorder, rc.bottom() - nStatusHeight - nBottomBorder + 1),
                            QPoint(rc.right() - 2, rc.bottom() - nBottomBorder));
#else
                        QRect rcDark(QPoint(rc.right() - nRightBorder, rc.bottom() - nStatusHeight - nBottomBorder),
                            QPoint(rc.right() - 2, rc.bottom() - nBottomBorder));
#endif
                ...
                {
                    QRect rcLight(QPoint(rc.left() + 1, rc.bottom() - nStatusHeight - nBottomBorder),
                        QPoint(rc.left() + nLeftBorder, rc.bottom() - nBottomBorder + 1));
                    p->fillRect(rcLight, active ? d.m_clrFrameBorderActive2 : d.m_clrFrameBorderInactive2);

                    QRect rcDark(QPoint(rc.right() - nRightBorder, rc.bottom() - nStatusHeight - nBottomBorder),
                        QPoint(rc.right() - 2, rc.bottom() - nBottomBorder + 1));
            ...
            if (optFrame->hasStatusBar || roundedCornersAlways)
            {
                ...
                if (nRightBorder > 3)
                {
                    ...
                    if (!soImage.isNull())
                    {
                        rcSrc = sourceRectImage(soImage.rect(), active ? 0 : 1, 2);
                        QRect rcBottomRight(
                            QPoint(rc.right() - rcSrc.width() - 1, rc.bottom() - rcSrc.height()), QPoint(rc.right() - 1, rc.bottom()));
    ...
    if (const StyleSystemToolButton* toolbutton = qstyleoption_cast<const StyleSystemToolButton*>(option))
    {
        if (qobject_cast<const RibbonBar*>(widget->parentWidget()))
        {
            if (toolbutton->toolButtonStyle != Qt::ToolButtonFollowStyle)
            {
                ...
                uint resolve_mask = toolbutton->palette.resolveMask();
                ...
                if (usePalette)
                {
                    ...
                    QBrush brHighlightButton =
                        (resolve_mask & (1 << QPalette::Highlight)) ? toolbutton->palette.brush(QPalette::Active, QPalette::Highlight) : brButton;
                    QBrush brPopupButton =
                        (resolve_mask & (1 << QPalette::Dark)) ? toolbutton->palette.brush(QPalette::Active, QPalette::Dark) : brButton;
                    ...
                    /*
                                        ...
                                        QColor color = !enabled ? toolbutton->colorBackground.darker(20) : popuped ? (selected ?
                       toolbutton->colorBackground.darker(105) : toolbutton->colorBackground.darker(115)) : false ? toolbutton->colorBackground :
                       selected ? toolbutton->colorBackground.darker(105) : toolbutton->colorBackground;
                ...
                if (toolbutton->toolButtonStyle == Qt::ToolButtonIconOnly && !toolbutton->icon.isNull())
                {
                    ...
                    QRect rc = toolbutton->rect;
                    QPixmap pxIcon = toolbutton->icon.pixmap(
                        toolbutton->iconSize, enabled ? QIcon::Normal : QIcon::Disabled, isHighlighted ? QIcon::On : QIcon::Off);
        ...
        if (!toolbutton->icon.isNull())
        {
            ...
            bool selected = toolbutton->state & QStyle::State_MouseOver;
            QPixmap pxIcon =
                toolbutton->icon.pixmap(toolbutton->iconSize, enabled ? QIcon::Normal : QIcon::Disabled, selected ? QIcon::On : QIcon::Off);
...
void RibbonPaintManager::drawQuickAccessButton(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
    {
        ...
        drawImage(soQuickButton, *p, toolbutton->rect,
            sourceRectImage(soQuickButton.rect(),
                !w->isEnabled() ? 4
                : popuped       ? 2
                : selected      ? 1
                                : 0,
    ...
    Qtitan::BarType barType, Qtitan::BarPosition barPos) const
{
    ...
    else if (barPos == BarPopup && selected && barType == TypePopup)
    {
        ...
        QPixmap soButton = !enabled    ? smallSize ? cached("MenuSelectedDisabledItem22.png") : cached("MenuSelectedDisabledItem54.png")
                              : smallSize ? cached("MenuSelectedItem22.png")
                                       : cached("MenuSelectedItem54.png");
            ...
            if (act || focus)
            {
                ...
                if (focus)
                {
                    ...
                    if (!soImage.isNull())
                    {
                        QRect rcSrc = soImage.rect();
                        QRect rcGlyph(
                            QPoint(opt->rect.right() - rcSrc.width() + 1, (opt->rect.top() + opt->rect.bottom() - rcSrc.height()) / 2), rcSrc.size());
...
/*! \internal */
void RibbonPaintManager2013::drawPixTitleBackground(
    QPainter* p, const QRect& rectDst, const QPixmap& pixTitleBackground, const QRect& rectSrc, const QWidget* widget) const
{
    Q_UNUSED(widget);
...
void RibbonPaintManager2013::drawGroup(const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    ...
    QRect rect = opt->rect;
    QPen savePen = p->pen();
    p->setPen(DARKMODE ? QColor(34, 34, 34) : QColor(204, 204, 204));
...
void RibbonPaintManager2013::drawReducedGroup(const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    ...
    if (const QStyleOptionToolButton* optGroup = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
    {
        ...
        QPoint pt = QPoint(strSecondRow.isEmpty() ? rcText.center().x() - 4 : rectSecondRow.left() + width + 1,
            strSecondRow.isEmpty() ? rectFirstRow.bottom() : (rectSecondRow.top() + (rectSecondRow.height() - 9) / 2) + 1);
        ...
        if (const QStyleOptionGroupBox* optGroup = qstyleoption_cast<const QStyleOptionGroupBox*>(opt))
        {
            ...
            else
            {
                ...
                if (optGroup->textAlignment == Qt::AlignBottom)
                {
                    ...
                    int indent = 0;
                    QPalette pal;
                    pal.setColor(QPalette::WindowText, QColor(204, 204, 204).darker());
                    baseStyle()->proxy()->drawItemText(p, rect.adjusted(indent + 1, 1, -indent - 1, -1),
                        Qt::AlignLeft | Qt::AlignVCenter | Qt::TextShowMnemonic, pal, optGroup->state & QStyle::State_Enabled, titleText,
                        QPalette::WindowText);
                    p->restore();
    ...
    if (w && !qobject_cast<const RibbonTitleButton*>(w))
    {
        if (const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
        {
            ...
            if (mouseInSplit || checked)
            {
                ...
                {
                    QRect rcSplitUp =
                        smallSize ? QRect(QPoint(rcButton.left(), rcButton.top()), QPoint(rcButton.right() - popupr.width() - 2, rcButton.bottom()))
                                  : QRect(QPoint(rcButton.left(), rcButton.top()), QPoint(rcButton.right(), rcButton.bottom() - popupr.height() - 2));
            ...
            if (mouseInSplitDropDown || popuped || checked)
            {
                if (!popuped)
                {
                    QRect rcSplitDown =
                        smallSize
                            ? QRect(QPoint(rcButton.right() - popupr.width() - 1, rcButton.top()), QPoint(rcButton.right(), rcButton.bottom()))
                            : QRect(QPoint(rcButton.left(), rcButton.bottom() - popupr.height() - 1), QPoint(rcButton.right(), rcButton.bottom()));
    ...
    if (const QStyleOptionSlider* slider = qstyleoption_cast<const QStyleOptionSlider*>(opt))
    {
        if (slider->orientation == Qt::Horizontal)
        {
            ...
            if ((opt->subControls & QStyle::SC_SliderHandle))
            {
                ...
                {
                    // rj: for the selected ribbon bar items
                    clrFill = DARKMODE ? QColor(28, 28, 28) : QColor(219, 219, 219);
                    clrLine = DARKMODE ? QColor(219, 219, 219) : QColor(219, 219, 219);
                }
...
void RibbonPaintManager2013::drawTabShape(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    if (const StyleRibbonOptionHeader* optTab = qstyleoption_cast<const StyleRibbonOptionHeader*>(opt))
    {
        if (optTab->state & QStyle::State_Selected)
        {
            ...
            p->fillRect(rectTab,
                d.m_isActiveTabAccented ? (optTab->state & QStyle::State_MouseOver ? m_clrAccentHilight : d.m_clrAccent) : m_clrRibbonGroupsFill);
            ...
            // rj: top line above selecteed ribbon item
            QRect topRect(opt->rect.left(), opt->rect.top(), opt->rect.width(), 3);
            p->setBrush(QColor(133, 64, 245));
            p->setPen(Qt::NoPen);
            p->drawRect(topRect);
        }
        else
    ...
    if (const StyleSystemToolButton* toolbutton = qstyleoption_cast<const StyleSystemToolButton*>(option))
    {
        if (qobject_cast<const RibbonBar*>(widget->parentWidget()))
        {
            if (toolbutton->toolButtonStyle != Qt::ToolButtonFollowStyle)
            {
                ...
                QPainterPath path;
                int radius = 15; // Adjust this value to change the roundness
                path.moveTo(toolbutton->rect.left(), toolbutton->rect.top());
                path.lineTo(toolbutton->rect.right() - radius, toolbutton->rect.top());
                path.arcTo(toolbutton->rect.right() - radius, toolbutton->rect.top(), radius, radius, 90, -90);
                path.lineTo(toolbutton->rect.right(), toolbutton->rect.bottom() - radius);
                path.arcTo(toolbutton->rect.right() - radius, toolbutton->rect.bottom() - radius, radius, radius, 0, -90);
                path.lineTo(toolbutton->rect.left(), toolbutton->rect.bottom());
                path.closeSubpath();
                ...
                {
                    if (isHighlighted)
                        p->fillPath(path, m_clrAccentHilight);
                }
                else
                {
                    bool fill = true;
                    ...
                    if (fill)
                        p->fillPath(path, isHighlighted ? m_clrAccentHilight : d.m_clrAccent);
                }
...
// for RibbonBar
/*! \internal */
void RibbonPaintManager2016::drawPixTitleBackground(
    QPainter* p, const QRect& rectDst, const QPixmap& pixTitleBackground, const QRect& rectSrc, const QWidget* widget) const
{
    QTN_D_STYLE(RibbonStyle);
...
#endif
    {
        QColor separatorColor = d.m_clrRibbonSeparator;
        if (f->frameShape == QFrame::HLine)
        {
...
void RibbonPaintManager2016::setupPalette(QWidget* widget) const
{
    ...
    if (qobject_cast<QToolButton*>(widget) &&
        (qobject_cast<RibbonTabBar*>(widget->parentWidget()) || qobject_cast<RibbonQuickAccessBar*>(widget->parentWidget())))
    {
        QPalette palette = widget->palette();
    ...
    if (themeId == OfficeStyle::Office2016Black)
    {
        ...
        {
            QPalette palette = widget->palette();
            palette.setBrush(QPalette::Window, helper().getColor("Window", "BarLight").lighter(114));
            widget->setPalette(palette);
        }
        else if (qobject_cast<QGroupBox*>(widget))
        {
            ...
            palette.setColor(QPalette::WindowText, helper().getColor("Window", "BarText"));
            widget->setPalette(palette);
        }
