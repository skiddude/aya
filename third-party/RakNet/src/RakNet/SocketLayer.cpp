...
extern void ProcessNetworkPacket(const SystemAddress systemAddress, const char* data, const int length, RakPeer* rakPeer,
    RakNetSmartPtr<RakNetSocket> rakNetSocket, RakNet::TimeUS timeRead);
} // namespace RakNet
...
bool SocketLayer::IsSocketFamilySupported(const char* hostAddress, unsigned short socketFamily)
{
    ...
    if (hostAddress && (_stricmp(hostAddress, "UNASSIGNED_SYSTEM_ADDRESS") == 0 || hostAddress[0] == 0))
    {
...
void SocketLayer::SetDoNotFragment(SOCKET listenSocket, int opt, int IPPROTO)
{
    ...
    if (setsockopt__(listenSocket, IPPROTO, IP_DONTFRAGMENT, (char*)&opt, sizeof(opt)) == -1)
    {
        ...
        FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, dwIOError,
...
SOCKET SocketLayer::CreateBoundSocket_Old(
    unsigned short port, bool blockingSocket, const char* forceHostAddress, unsigned int sleepOn10048, unsigned int extraSocketOptions)
{
    (void)blockingSocket;
    ...
    if (ret <= -1)
    {
        ...
        if (dwIOError == 10048)
        {
            if (sleepOn10048 == 0)
                return (SOCKET)-1;
            // Vista has a bug where it returns WSAEADDRINUSE (10048) if you create, shutdown, then rebind the socket port unless you wait a while
            // first. Wait, then rebind
            RakSleep(100);
        ...
        LPVOID messageBuffer;
#if defined(AYA_PLATFORM_DURANGO) // Roblox Change
        // something has gone wrong here...
        ...
        // Free the buffer.
        LocalFree(messageBuffer);
#endif // defined(AYA_PLATFORM_DURANGO)
...
SOCKET SocketLayer::CreateBoundSocket(unsigned short port, bool blockingSocket, const char* forceHostAddress, unsigned int sleepOn10048,
    unsigned int extraSocketOptions, unsigned short socketFamily)
{
    (void)blockingSocket;
    ...
    // On Ubuntu, "" returns "No address associated with hostname" while 0 works.
    if (forceHostAddress && (_stricmp(forceHostAddress, "UNASSIGNED_SYSTEM_ADDRESS") == 0 || forceHostAddress[0] == 0))
    {
        getaddrinfo(0, portStr, &hints, &servinfo);
    }
    else
...
void SocketLayer::RecvFromBlocking_Old(const SOCKET s, RakPeer* rakPeer, unsigned short remotePortRakNetWasStartedOn_PS3,
    unsigned int extraSocketOptions, char* dataOut, int* bytesReadOut, SystemAddress* systemAddressOut, RakNet::TimeUS* timeRead)
{
    (void)rakPeer;
...
void SocketLayer::RecvFromBlocking(const SOCKET s, RakPeer* rakPeer, unsigned short remotePortRakNetWasStartedOn_PS3, unsigned int extraSocketOptions,
    char* dataOut, int* bytesReadOut, SystemAddress* systemAddressOut, RakNet::TimeUS* timeRead)
{
#if RAKNET_SUPPORT_IPV6 != 1
...
int SocketLayer::SendTo_PS3Lobby(
    SOCKET s, const char* data, int length, const SystemAddress& systemAddress, unsigned short remotePortRakNetWasStartedOn_PS3)
{
    (void)s;
    (void)data;
    (void)length;
...
int SocketLayer::SendTo_360(SOCKET s, const char* data, int length, const char* voiceData, int voiceLength, const SystemAddress& systemAddress,
...
int SocketLayer::SendTo_PC(SOCKET s, const char* data, int length, const SystemAddress& systemAddress, const char* file, const long line)
{
    ...
    */
    ...
    do
    {
        ...
        RAKNET_DEBUG_PRINTF("%i. SendTo_PC, time=%" PRINTF_64_BIT_MODIFIER "u, elapsed=%" PRINTF_64_BIT_MODIFIER
                            "u, length=%i, returned=%i, binaryAddress=%i, port=%i, file=%s, line=%i\n",
...
#endif
int SocketLayer::SendTo(SOCKET s, const char* data, int length, SystemAddress& systemAddress, unsigned short remotePortRakNetWasStartedOn_PS3,
...
// int SocketLayer::SendTo( SOCKET s, const char *data, int length, const char ip[ 16 ], unsigned short port, unsigned short
// remotePortRakNetWasStartedOn_PS3, unsigned int extraSocketOptions, const char *file, const long line )
// {
...
RakNet::RakString SocketLayer::GetSubNetForSocketAndIp(SOCKET inSock, RakNet::RakString inIpString)
{
    ...
    if (WSAIoctl(inSock, SIO_GET_INTERFACE_LIST, 0, 0, &InterfaceList, sizeof(InterfaceList), &nBytesReturned, 0, 0) == SOCKET_ERROR)
    {
        return "";
    }
...
void GetMyIP_Win32(SystemAddress addresses[MAXIMUM_NUMBER_OF_INTERNAL_IDS])
{
    ...
    if (gethostname(ac, sizeof(ac)) == -1)
    {
        ...
        // Free the buffer.
        LocalFree(messageBuffer);
#endif // defined(AYA_PLATFORM_WIN_MOBILE)
