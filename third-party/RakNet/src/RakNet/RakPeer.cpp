...
#include "RakNetVersion.hpp"
#include "NetworkIDManager.hpp"
#include "gettimeofday.hpp"
#include "SignaledEvent.hpp"
#include "SuperFastHash.hpp"
#include "RakAlloca.hpp"
#include "WSAStartupSingleton.hpp"

#ifdef USE_THREADED_SEND
#include "SendToThread.hpp"
#endif
...
static const char OFFLINE_MESSAGE_DATA_ID[16] = {(const char)0x00, (const char)0xFF, (const char)0xFF, (const char)0x00, (const char)0xFE,
    (const char)0xFE, (const char)0xFE, (const char)0xFE, (const char)0xFD, (const char)0xFD, (const char)0xFD, (const char)0xFD, (const char)0x12,
    (const char)0x34, (const char)0x56, (const char)0x78};
...
// \param[in] maxConnections The maximum number of connections between this instance of RakPeer and another instance of RakPeer. Required so the
// network can preallocate and for thread safety. A pure client would set this to 1.  A pure server would set it to the number of allowed clients.- A
// hybrid would set it to the sum of both types of connections
// \param[in] localPort The port to listen for connections on.
// \param[in] _threadSleepTimer How many ms to Sleep each internal update cycle. With new congestion control, the best results will be obtained by
// passing 10.
// \param[in] socketDescriptors An array of SocketDescriptor structures to force RakNet to listen on a particular IP address or port (or both).  Each
// SocketDescriptor will represent one unique socket.  Do not pass redundant structures.  To listen on a specific port, you can pass
// &socketDescriptor, 1SocketDescriptor(myPort,0); such as for a server.  For a client, it is usually OK to just pass SocketDescriptor();
...
StartupResult RakPeer::Startup(unsigned short maxConnections, SocketDescriptor* socketDescriptors, unsigned socketDescriptorCount, int threadPriority)
{
    ...
    for (i = 0; i < socketDescriptorCount; i++)
    {
        ...
        if (socketDescriptors[i].port != 0 &&
            SocketLayer::IsPortInUse(socketDescriptors[i].port, addrToBind, socketDescriptors[i].socketFamily) == true)
        {
            DerefAllSockets();
        ...
        {
            rns->s = (unsigned int)SocketLayer::CreateBoundSocket(
                socketDescriptors[i].port, true, addrToBind, 100, socketDescriptors[i].extraSocketOptions, socketDescriptors[i].socketFamily);
        }
        else
        {
        }
        ...
        int zero = 0;
        if (SocketLayer::SendTo((SOCKET)rns->s, (const char*)&zero, 4, rns->boundAddress, rns->remotePortRakNetWasStartedOn_PS3_PSP2,
                rns->extraSocketOptions, _FILE_AND_LINE_) != 0)
        {
            DerefAllSockets();
    ...
    if (maximumNumberOfPeers == 0)
    {
        ...
        remoteSystemLookup =
            RakNet::OP_NEW_ARRAY<RemoteSystemIndex*>((unsigned int)maximumNumberOfPeers * REMOTE_SYSTEM_LOOKUP_HASH_MULTIPLE, _FILE_AND_LINE_);
...
// bRequireClientKey: Should be set to false for most servers.  Allows the server to accept a public key from connecting clients as a proof of
// identity but eats twice as much CPU time as a normal connection
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
ConnectionAttemptResult RakPeer::Connect(const char* host, unsigned short remotePort, const char* passwordData, int passwordDataLength,
    PublicKey* publicKey, unsigned connectionSocketIndex, unsigned sendConnectionAttemptCount, unsigned timeBetweenSendConnectionAttemptsMS,
    RakNet::TimeMS timeoutTime)
{
    ...
    return SendConnectionRequest(host, remotePort, passwordData, passwordDataLength, publicKey, connectionSocketIndex, 0, sendConnectionAttemptCount,
        timeBetweenSendConnectionAttemptsMS, timeoutTime);
}

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

ConnectionAttemptResult RakPeer::ConnectWithSocket(const char* host, unsigned short remotePort, const char* passwordData, int passwordDataLength,
    RakNetSmartPtr<RakNetSocket> socket, PublicKey* publicKey, unsigned sendConnectionAttemptCount, unsigned timeBetweenSendConnectionAttemptsMS,
    RakNet::TimeMS timeoutTime)
{
    ...
    return SendConnectionRequest(host, remotePort, passwordData, passwordDataLength, publicKey, 0, 0, sendConnectionAttemptCount,
...
void RakPeer::Shutdown(unsigned int blockDuration, unsigned char orderingChannel, PacketPriority disconnectionNotificationPriority)
{
    ...
    for (i = 0; i < socketList.Size(); i++)
    {
        ...
        if (SocketLayer::SendTo(socketList[i]->s, (const char*)&i, 1, sa2, socketList[i]->remotePortRakNetWasStartedOn_PS3_PSP2,
                socketList[i]->extraSocketOptions, _FILE_AND_LINE_) != 0)
            break;
    }
    ...
    while (isRecvFromLoopThreadActive.GetValue() > 0 && RakNet::GetTimeMS() < timeout)
    {
        ...
        {
            SystemAddress sa2 = socketList[i]->boundAddress;
            SocketLayer::SendTo(socketList[i]->s, (const char*)&i, 1, sa2, socketList[i]->remotePortRakNetWasStartedOn_PS3_PSP2,
                socketList[i]->extraSocketOptions, _FILE_AND_LINE_);
        }
...
// Returns:
// \return 0 on bad input. Otherwise a number that identifies this message. If \a reliability is a type that returns a receipt, on a later call to
// Receive() you will get ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS with bytes 1-4 inclusive containing this number
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
uint32_t RakPeer::Send(const char* data, const int length, PacketPriority priority, PacketReliability reliability, char orderingChannel,
    const AddressOrGUID systemIdentifier, bool broadcast, uint32_t forceReceiptNumber)
{
#ifdef _DEBUG
    ...
    SendBuffered(
        data, length * 8, priority, reliability, orderingChannel, systemIdentifier, broadcast, RemoteSystemStruct::NO_ACTION, usedSendReceipt);
...
uint32_t RakPeer::Send(const RakNet::BitStream* bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel,
    const AddressOrGUID systemIdentifier, bool broadcast, uint32_t forceReceiptNumber)
{
#ifdef _DEBUG
    ...
    SendBuffered((const char*)bitStream->GetData(), bitStream->GetNumberOfBitsUsed(), priority, reliability, orderingChannel, systemIdentifier,
...
// \param[in] orderingChannel When using ordered or sequenced messages, what channel to order these on. Messages are only ordered relative to other
// messages on the same stream
// \param[in] systemIdentifier Who to send this packet to, or in the case of broadcasting who not to send it to. Pass either a SystemAddress structure
// or a RakNetGUID structure. Use UNASSIGNED_SYSTEM_ADDRESS or to specify none
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
uint32_t RakPeer::SendList(const char** data, const int* lengths, const int numParameters, PacketPriority priority, PacketReliability reliability,
    char orderingChannel, const AddressOrGUID systemIdentifier, bool broadcast, uint32_t forceReceiptNumber)
{
#ifdef _DEBUG
    RakAssert(data);
#endif
    ...
    SendBufferedList(data, lengths, numParameters, priority, reliability, orderingChannel, systemIdentifier, broadcast, RemoteSystemStruct::NO_ACTION,
        usedSendReceipt);
...
Packet* RakPeer::Receive(void)
{
    ...
    do
    {
        ...
        //		unsigned char msgId;
        if ((packet->length >= sizeof(unsigned char) + sizeof(RakNet::Time)) && ((unsigned char)packet->data[0] == ID_TIMESTAMP))
        {
        ...
        if (packet) // Aya, enclose this loop in if statement, previous loop might already processed this packet
        {
            for (i = 0; i < pluginListNTS.Size(); i++)
            {
...
void RakPeer::CloseConnection(
    const AddressOrGUID target, bool sendDisconnectionNotification, unsigned char orderingChannel, PacketPriority disconnectionNotificationPriority)
{
    /*
...
// \param[out] addresses All system addresses. Size of the list is the number of connections. Size of the list will match the size of the \a guids
// list.
...
bool RakPeer::Ping(const char* host, unsigned short remotePort, bool onlyReplyOnAcceptingConnections, unsigned connectionSocketIndex)
{
    ...
    SocketLayer::SendTo(socketList[realIndex]->s, (const char*)bitStream.GetData(), (int)bitStream.GetNumberOfBytesUsed(), systemAddress,
        socketList[realIndex]->remotePortRakNetWasStartedOn_PS3_PSP2, socketList[realIndex]->extraSocketOptions, _FILE_AND_LINE_);
...
// pushAtHead True to push the packet so that the next receive call returns it.  False to push it at the end of the queue (obviously pushing it at the
// end makes the packets out of order)
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
bool RakPeer::SendOutOfBand(const char* host, unsigned short remotePort, const char* data, BitSize_t dataLength, unsigned connectionSocketIndex)
{
    ...
    RakAssert(dataLength <=
              (MAX_OFFLINE_DATA_LENGTH + sizeof(unsigned char) + sizeof(RakNet::Time) + RakNetGUID::size() + sizeof(OFFLINE_MESSAGE_DATA_ID)));
...
int RakPeer::GetIndexFromSystemAddress(const SystemAddress systemAddress, bool calledFromNetworkThread) const
{
    ...
    if (systemAddress.systemIndex != (SystemIndex)-1 && systemAddress.systemIndex < maximumNumberOfPeers &&
        remoteSystemList[systemAddress.systemIndex].systemAddress == systemAddress && remoteSystemList[systemAddress.systemIndex].isActive)
...
int RakPeer::GetIndexFromGuid(const RakNetGUID guid)
{
    ...
    if (guid.systemIndex != (SystemIndex)-1 && guid.systemIndex < maximumNumberOfPeers && remoteSystemList[guid.systemIndex].guid == guid &&
...
#endif
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
ConnectionAttemptResult RakPeer::SendConnectionRequest(const char* host, unsigned short remotePort, const char* passwordData, int passwordDataLength,
    PublicKey* publicKey, unsigned connectionSocketIndex, unsigned int extraData, unsigned sendConnectionAttemptCount,
...
ConnectionAttemptResult RakPeer::SendConnectionRequest(const char* host, unsigned short remotePort, const char* passwordData, int passwordDataLength,
    PublicKey* publicKey, unsigned connectionSocketIndex, unsigned int extraData, unsigned sendConnectionAttemptCount,
    unsigned timeBetweenSendConnectionAttemptsMS, RakNet::TimeMS timeoutTime, RakNetSmartPtr<RakNetSocket> socket)
{
    RakAssert(passwordDataLength <= 256);
    SystemAddress systemAddress;
    systemAddress.FromStringExplicitPort(host, remotePort);
...
RakPeer::RemoteSystemStruct* RakPeer::GetRemoteSystemFromSystemAddress(
    const SystemAddress systemAddress, bool calledFromNetworkThread, bool onlyActive) const
{
    unsigned i;
...
void RakPeer::ParseConnectionRequestPacket(
    RakPeer::RemoteSystemStruct* remoteSystem, const SystemAddress& systemAddress, const char* data, int byteSize)
{
    ...
    if (_using_security)
    {
        ...
        if (doClientKey == 1)
        {
            ...
            if (_require_client_public_key)
            {
                ...
                {
                    RakNet::BitStream bitStream;
                    bitStream.Write((MessageID)ID_REMOTE_SYSTEM_REQUIRES_PUBLIC_KEY); // Report an error since the client is not providing an identity
                                                                                      // when it is necessary to connect
                    bitStream.Write((unsigned char)2);                                // Indicate client identity is invalid
                    SendImmediate((char*)bitStream.GetData(), bitStream.GetNumberOfBytesUsed(), IMMEDIATE_PRIORITY, RELIABLE, 0, systemAddress, false,
                        false, RakNet::GetTimeUS(), 0);
                    remoteSystem->connectMode = RemoteSystemStruct::DISCONNECT_ASAP_SILENTLY;
                    return;
                }
            }
    ...
    if (incomingPasswordLength != passwordLength || memcmp(password, incomingPassword, incomingPasswordLength) != 0)
    {
        ...
        SendImmediate((char*)bitStream.GetData(), bitStream.GetNumberOfBytesUsed(), IMMEDIATE_PRIORITY, RELIABLE, 0, systemAddress, false, false,
            RakNet::GetTimeUS(), 0);
        remoteSystem->connectMode = RemoteSystemStruct::DISCONNECT_ASAP_SILENTLY;
        return;
    }
...
void RakPeer::OnConnectionRequest(RakPeer::RemoteSystemStruct* remoteSystem, RakNet::Time incomingTimestamp)
{
    ...
    SendImmediate((char*)bitStream.GetData(), bitStream.GetNumberOfBitsUsed(), IMMEDIATE_PRIORITY, RELIABLE_ORDERED, 0, remoteSystem->systemAddress,
        false, false, RakNet::GetTimeUS(), 0);
}

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::NotifyAndFlagForShutdown(
    const SystemAddress systemAddress, bool performImmediate, unsigned char orderingChannel, PacketPriority disconnectionNotificationPriority)
{
    RakNet::BitStream temp(sizeof(unsigned char));
    temp.Write((MessageID)ID_DISCONNECTION_NOTIFICATION);
    if (performImmediate)
    {
        SendImmediate((char*)temp.GetData(), temp.GetNumberOfBitsUsed(), disconnectionNotificationPriority, RELIABLE_ORDERED, orderingChannel,
    ...
    {
        SendBuffered((const char*)temp.GetData(), temp.GetNumberOfBitsUsed(), disconnectionNotificationPriority, RELIABLE_ORDERED, orderingChannel,
            systemAddress, false, RemoteSystemStruct::DISCONNECT_ASAP, 0);
    }
}
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
RakPeer::RemoteSystemStruct* RakPeer::AssignSystemAddressToRemoteSystemList(const SystemAddress systemAddress,
    RemoteSystemStruct::ConnectMode connectionMode, RakNetSmartPtr<RakNetSocket> incomingRakNetSocket, bool* thisIPConnectedRecently,
    SystemAddress bindingAddress, int incomingMTU, RakNetGUID guid, bool useSecurity)
{
    ...
    if (limitConnectionFrequencyFromTheSameIP)
    {
        if (IsLoopbackAddress(systemAddress, false) == false)
        {
            for (i = 0; i < maximumNumberOfPeers; i++)
            {
                if (remoteSystemList[i].isActive == true && remoteSystemList[i].systemAddress.EqualsExcludingPort(systemAddress) &&
                ...
                else
                {
                    ...
                    if (socketListIndex == socketList.Size())
                    {
                        ...
                        if (incomingRakNetSocket->remotePortRakNetWasStartedOn_PS3_PSP2 == 0)
                            rns->s = (unsigned int)SocketLayer::CreateBoundSocket(bindingAddress.GetPort(), true, ipListFoundIndexStr, 0,
                                incomingRakNetSocket->extraSocketOptions, incomingRakNetSocket->socketFamily);
                        else
                            rns->s = (unsigned int)SocketLayer::CreateBoundSocket_PS3Lobby(
                                bindingAddress.GetPort(), true, ipListFoundIndexStr, incomingRakNetSocket->socketFamily);
                        if ((SOCKET)rns->s == (SOCKET)-1)
                        {
...
void RakPeer::RemoveFromActiveSystemList(const SystemAddress& sa)
{
    ...
    RakAssert("activeSystemList invalid, entry not found in RemoveFromActiveSystemList. Ensure that AddToActiveSystemList and "
              "RemoveFromActiveSystemList are called by the same thread." &&
              0);
}
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/*
...
void RakPeer::PingInternal(const SystemAddress target, bool performImmediate, PacketReliability reliability)
{
    ...
    bitStream.Write(RakNet::GetTime());
    if (performImmediate)
        SendImmediate((char*)bitStream.GetData(), bitStream.GetNumberOfBitsUsed(), IMMEDIATE_PRIORITY, reliability, 0, target, false, false,
            RakNet::GetTimeUS(), 0);
    else
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::SendBuffered(const char* data, BitSize_t numberOfBitsToSend, PacketPriority priority, PacketReliability reliability,
    char orderingChannel, const AddressOrGUID systemIdentifier, bool broadcast, RemoteSystemStruct::ConnectMode connectionMode, uint32_t receipt)
{
    BufferedCommandStruct* bcs;
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::SendBufferedList(const char** data, const int* lengths, const int numParameters, PacketPriority priority, PacketReliability reliability,
    char orderingChannel, const AddressOrGUID systemIdentifier, bool broadcast, RemoteSystemStruct::ConnectMode connectionMode, uint32_t receipt)
{
    BufferedCommandStruct* bcs;
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::SendImmediate(char* data, BitSize_t numberOfBitsToSend, PacketPriority priority, PacketReliability reliability, char orderingChannel,
    const AddressOrGUID systemIdentifier, bool broadcast, bool useCallerDataAllocation, RakNet::TimeUS currentTime, uint32_t receipt)
{
    unsigned* sendList;
...
//			int debugIndex = GetRemoteSystemIndex(systemIdentifier.systemAddress);
        ...
        if (remoteSystemList[remoteSystemIndex].isActive && remoteSystemList[remoteSystemIndex].connectMode != RemoteSystemStruct::DISCONNECT_ASAP &&
    ...
    for (sendListIndex = 0; sendListIndex < sendListSize; sendListIndex++)
    {
        ...
        remoteSystemList[sendList[sendListIndex]].reliabilityLayer.Send(data, numberOfBitsToSend, priority, reliability, orderingChannel,
            useData == false, remoteSystemList[sendList[sendListIndex]].MTUSize, currentTime, receipt);
        if (useData)
    ...
    // Return value only meaningful if true was passed for useCallerDataAllocation.  Means the reliability layer used that data copy, so the caller
...
bool ProcessOfflineNetworkPacket(SystemAddress systemAddress, const char* data, const int length, RakPeer* rakPeer,
    RakNetSmartPtr<RakNetSocket> rakNetSocket, bool* isOfflineMessage, RakNet::TimeUS timeRead)
{
    (void)timeRead;
    ...
    if (rakPeer->IsBanned(str1))
    {
        ...
        SocketLayer::SendTo(rakNetSocket->s, (char*)bs.GetData(), bs.GetNumberOfBytesUsed(), systemAddress,
            rakNetSocket->remotePortRakNetWasStartedOn_PS3_PSP2, rakNetSocket->extraSocketOptions, _FILE_AND_LINE_);
    ...
    // The reason for all this is that the reliability layer has no way to tell between offline messages that arrived late for a player that is now
    // connected, and a regular encoding. So I insert OFFLINE_MESSAGE_DATA_ID into the stream, the encoding of which is essentially impossible to hit
    // by chance
    if (length <= 2)
    {
    ...
    {
        *isOfflineMessage =
            memcmp(data + sizeof(unsigned char) + sizeof(RakNet::Time), OFFLINE_MESSAGE_DATA_ID, sizeof(OFFLINE_MESSAGE_DATA_ID)) == 0;
    }
    else if ((unsigned char)data[0] == ID_UNCONNECTED_PONG &&
             (size_t)length >= sizeof(unsigned char) + sizeof(RakNet::TimeMS) + RakNetGUID::size() + sizeof(OFFLINE_MESSAGE_DATA_ID))
    {
        *isOfflineMessage = memcmp(data + sizeof(unsigned char) + sizeof(RakNet::Time) + RakNetGUID::size(), OFFLINE_MESSAGE_DATA_ID,
    ...
    else if (((unsigned char)data[0] == ID_OPEN_CONNECTION_REPLY_1 || (unsigned char)data[0] == ID_OPEN_CONNECTION_REPLY_2 ||
                 (unsigned char)data[0] == ID_OPEN_CONNECTION_REQUEST_1 || (unsigned char)data[0] == ID_OPEN_CONNECTION_REQUEST_2 ||
                 (unsigned char)data[0] == ID_CONNECTION_ATTEMPT_FAILED || (unsigned char)data[0] == ID_NO_FREE_INCOMING_CONNECTIONS ||
    ...
    if (*isOfflineMessage)
    {
        ...
        if (((unsigned char)data[0] == ID_UNCONNECTED_PING_OPEN_CONNECTIONS || (unsigned char)(data)[0] == ID_UNCONNECTED_PING) &&
            length == sizeof(unsigned char) + sizeof(RakNet::Time) + sizeof(OFFLINE_MESSAGE_DATA_ID))
        {
            if ((unsigned char)(data)[0] == ID_UNCONNECTED_PING || rakPeer->AllowIncomingConnections()) // Open connections with players
            {
                // #if !defined(_XBOX) && !defined(X360)
                ...
                for (i = 0; i < rakPeer->pluginListNTS.Size(); i++)
                    rakPeer->pluginListNTS[i]->OnDirectSocketSend(
                        (const char*)outBitStream.GetData(), outBitStream.GetNumberOfBytesUsed(), systemAddress);

                SocketLayer::SendTo(rakNetSocket->s, (const char*)outBitStream.GetData(), (unsigned int)outBitStream.GetNumberOfBytesUsed(),
                    systemAddress, rakNetSocket->remotePortRakNetWasStartedOn_PS3_PSP2, rakNetSocket->extraSocketOptions, _FILE_AND_LINE_);
        ...
        // UNCONNECTED MESSAGE Pong with no data.
                 ...
                 (size_t)length <
                     sizeof(unsigned char) + sizeof(RakNet::Time) + RakNetGUID::size() + sizeof(OFFLINE_MESSAGE_DATA_ID) + MAX_OFFLINE_DATA_LENGTH)
        {
            packet = rakPeer->AllocPacket(
                (unsigned int)(length - sizeof(OFFLINE_MESSAGE_DATA_ID) - RakNetGUID::size() - sizeof(RakNet::Time) + sizeof(RakNet::TimeMS)),
                _FILE_AND_LINE_);
                ...
                if (rcs->systemAddress == systemAddress)
                {
                    if (serverHasSecurity)
                    {
                        ...
                        if (rcs->publicKeyMode == PKM_ACCEPT_ANY_PUBLIC_KEY)
                        {
                            ...
                            if (!rcs->client_handshake->Initialize(public_key) || !rcs->client_handshake->GenerateChallenge(rcs->handshakeChallenge))
                            {
                        ...
                        if (cat::SecureEqual(public_key, rcs->remote_public_key, cat::EasyHandshake::PUBLIC_KEY_BYTES) == false)
                        {
                            rakPeer->requestedConnectionQueueMutex.Unlock();
                            CAT_AUDIT_PRINTF("AUDIT: Expected public key does not match what was sent by server -- Reporting back "
                                             "ID_PUBLIC_KEY_MISMATCH to user\n");
       ...
       // Message does not contain a challenge
                    ...
                    SocketLayer::SendTo(rakPeer->socketList[rcs->socketIndex]->s, (const char*)bsOut.GetData(), bsOut.GetNumberOfBytesUsed(),
                        rcs->systemAddress, rakPeer->socketList[rcs->socketIndex]->remotePortRakNetWasStartedOn_PS3_PSP2,
                        rakPeer->socketList[rcs->socketIndex]->extraSocketOptions, _FILE_AND_LINE_);
        ...
        else if ((unsigned char)(data)[0] == (MessageID)ID_OPEN_CONNECTION_REPLY_2)
        {
            ...
            for (i = 0; i < rakPeer->requestedConnectionQueue.Size(); i++)
            {
                ...
                if (rcs->systemAddress == systemAddress)
                {
                    ...
                    if (doSecurity)
                    {
                        if (rcs->client_handshake == 0)
                        {
                            CAT_AUDIT_PRINTF("AUDIT: Server wants security but we didn't set a public key -- Reporting back "
                                             "ID_REMOTE_SYSTEM_REQUIRES_PUBLIC_KEY to user\n");
                            rakPeer->requestedConnectionQueueMutex.Unlock();
                    ...
                    if (remoteSystem == 0)
                    {
                        ...
                        {
                            remoteSystem =
                                rakPeer->AssignSystemAddressToRemoteSystemList(systemAddress, RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER,
                                    rakNetSocket, &thisIPConnectedRecently, bindingAddress, mtu, guid, doSecurity);
                        }
                        else
                        {
                            remoteSystem =
                                rakPeer->AssignSystemAddressToRemoteSystemList(systemAddress, RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER,
                                    rcs->socket, &thisIPConnectedRecently, bindingAddress, mtu, guid, doSecurity);
                        }
                    }
                    ...
                    if (thisIPConnectedRecently == false)
                    {
                        ...
                        if (remoteSystem)
                        {
                            ...
                            rakPeer->SendImmediate((char*)temp.GetData(), temp.GetNumberOfBitsUsed(), IMMEDIATE_PRIORITY, RELIABLE, 0, systemAddress,
                                false, false, timeRead, 0);
                        }
                        else
                        {
            ...
            return true;
                 ...
                 (unsigned char)(data)[0] == (MessageID)ID_INVALID_PASSWORD || (unsigned char)(data)[0] == (MessageID)ID_IP_RECENTLY_CONNECTED ||
        ...
        { /*
            ...
            SocketLayer::SendTo(rakNetSocket->s, (const char*)bsOut.GetData(), bsOut.GetNumberOfBytesUsed(), systemAddress,
                rakNetSocket->remotePortRakNetWasStartedOn_PS3_PSP2, rakNetSocket->extraSocketOptions, _FILE_AND_LINE_);
        }
        else if ((unsigned char)(data)[0] == ID_OPEN_CONNECTION_REQUEST_2)
        {
            ...
            if (outcome == 1)
            {
                ...
                SocketLayer::SendTo(rakNetSocket->s, (const char*)bsAnswer.GetData(), bsAnswer.GetNumberOfBytesUsed(), systemAddress,
                    rakNetSocket->remotePortRakNetWasStartedOn_PS3_PSP2, rakNetSocket->extraSocketOptions, _FILE_AND_LINE_);
            ...
            bool thisIPConnectedRecently = false;
            rssFromSA = rakPeer->AssignSystemAddressToRemoteSystemList(systemAddress, RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER, rakNetSocket,
                &thisIPConnectedRecently, bindingAddress, mtu, guid, requiresSecurityOfThisClient);
            ...
            if (requiresSecurityOfThisClient)
            {
                ...
                if (rakPeer->_server_handshake->ProcessChallenge(
                        remoteHandshakeChallenge, rssFromSA->answer, rssFromSA->reliabilityLayer.GetAuthenticatedEncryption()))
                {
...
void ProcessNetworkPacket(
    SystemAddress systemAddress, const char* data, const int length, RakPeer* rakPeer, RakNet::TimeUS timeRead, BitStream& updateBitStream)
{
...
void ProcessNetworkPacket(SystemAddress systemAddress, const char* data, const int length, RakPeer* rakPeer,
    RakNetSmartPtr<RakNetSocket> rakNetSocket, RakNet::TimeUS timeRead, BitStream& updateBitStream)
{
#if LIBCAT_SECURITY == 1
#ifdef CAT_AUDIT
    printf("AUDIT: RECV ");
    ...
    if (remoteSystem)
    {
        ...
        {
            remoteSystem->reliabilityLayer.HandleSocketReceiveFromConnectedPlayer(data, length, systemAddress, rakPeer->pluginListNTS,
                remoteSystem->MTUSize, rakNetSocket->s, &rnr, rakNetSocket->remotePortRakNetWasStartedOn_PS3_PSP2, rakNetSocket->extraSocketOptions,
                timeRead, updateBitStream);
        }
    }
}
...
bool RakPeer::RunUpdateCycle(RakNet::TimeUS timeNS, RakNet::Time timeMS, BitStream& updateBitStream)
{
    ...
    while ((recvFromStruct = bufferedPackets.PopInaccurate()) != 0)
    {
        ...
        if (socketListIndex != socketList.Size())
            ProcessNetworkPacket(recvFromStruct->systemAddress, recvFromStruct->data, recvFromStruct->bytesRead, this, socketList[socketListIndex],
                recvFromStruct->timeRead, updateBitStream);
    ...
    while ((bcs = bufferedCommands.PopInaccurate()) != 0)
    {
        if (bcs->command == BufferedCommandStruct::BCS_SEND)
        {
            ...
            callerDataAllocationUsed = SendImmediate((char*)bcs->data, bcs->numberOfBitsToSend, bcs->priority, bcs->reliability, bcs->orderingChannel,
                bcs->systemIdentifier, bcs->broadcast, true, timeNS, bcs->receipt);
            if (callerDataAllocationUsed == false)
                rakFree_Ex(bcs->data, _FILE_AND_LINE_);

            // Set the new connection state AFTER we call sendImmediate in case we are setting it to a disconnection state, which does not allow
            // further sends
            ...
            if (rcs->nextRequestTime < timeMS)
            {
                ...
                else
                {
                    ...
                    if (rcs->socket.IsNull())
                    {
                        ...
                        if (SocketLayer::SendTo(socketList[rcs->socketIndex]->s, (const char*)bitStream.GetData(), bitStream.GetNumberOfBytesUsed(),
                                rcs->systemAddress, socketList[rcs->socketIndex]->remotePortRakNetWasStartedOn_PS3_PSP2,
                                socketList[rcs->socketIndex]->extraSocketOptions, _FILE_AND_LINE_) == -10040)
                        {
                    ...
                    else
                    {
                        ...
                        RakNet::Time sendToStart = RakNet::GetTime();
                        if (SocketLayer::SendTo(rcs->socket->s, (const char*)bitStream.GetData(), bitStream.GetNumberOfBytesUsed(),
                                rcs->systemAddress, socketList[rcs->socketIndex]->remotePortRakNetWasStartedOn_PS3_PSP2,
                                socketList[rcs->socketIndex]->extraSocketOptions, _FILE_AND_LINE_) == -10040)
                        {
                            // Don't use this MTU size again
    ...
    while (count < activeSystemListSize)
    {
        ...
        bool bufferEmpty = remoteSystem->reliabilityLayer.Update(remoteSystem->rakNetSocket->s, systemAddress, remoteSystem->MTUSize, timeNS,
            maxOutgoingBPS, pluginListNTS, &rnr, remoteSystem->rakNetSocket->remotePortRakNetWasStartedOn_PS3_PSP2,
            remoteSystem->rakNetSocket->extraSocketOptions, updateBitStream); // systemAddress only used for the internet simulator test
        ...
        if (remoteSystem->reliabilityLayer.IsDeadConnection() ||
            ...
            (remoteSystem->connectMode == RemoteSystemStruct::DISCONNECT_ON_NO_ACK &&
                (remoteSystem->reliabilityLayer.AreAcksWaiting() == false || remoteSystem->reliabilityLayer.AckTimeout(timeMS) == true)) ||
                ...
                timeMS > remoteSystem->connectionTime && timeMS - remoteSystem->connectionTime > 10000)))
        {
            ...
            // TODO - RakNet 4.0 - Return a different message identifier for DISCONNECT_ASAP_SILENTLY and DISCONNECT_ASAP than for
            // DISCONNECT_ON_NO_ACK The first two mean we called CloseConnection(), the last means the other system sent us
                ...
                remoteSystem->connectMode == RemoteSystemStruct::DISCONNECT_ON_NO_ACK)
            {
                ...
                FASTLOG3(FLog::RakNetDisconnect, "Raknet: connection lost: connectMode: %d, timeMS:%u, connectionTime %u", remoteSystem->connectMode,
                    timeMS, remoteSystem->connectionTime);
                FASTLOG2(FLog::RakNetDisconnect, "Raknet: connection lost: AckTimeout %d, IsOutgoingDataWaiting %d",
                    remoteSystem->reliabilityLayer.AckTimeout(timeMS) ? 1 : 0, remoteSystem->reliabilityLayer.IsOutgoingDataWaiting() ? 1 : 0);
        ...
        while (bitSize > 0)
        {
            ...
            else
            {
                ...
                if ((unsigned char)(data)[0] == ID_CONNECTION_REQUEST)
                {
                    ...
                    // 04/28/06 Downgrading connections from connected will close the connection due to security at
                    // ((remoteSystem->connectMode!=RemoteSystemStruct::CONNECTED && time > remoteSystem->connectionTime && time -
                    // remoteSystem->connectionTime > 10000))
                    if (remoteSystem->connectMode == RemoteSystemStruct::REQUESTED_CONNECTION)
                    {
                        ParseConnectionRequestPacket(remoteSystem, systemAddress, (const char*)data, byteSize);
                    }
                    ...
                    rakFree_Ex(data, _FILE_AND_LINE_);
                         ...
                         byteSize > sizeof(unsigned char) + sizeof(unsigned int) + sizeof(unsigned short) + sizeof(RakNet::Time) * 2)
                {
                ...
                else if ((unsigned char)data[0] == ID_CONNECTED_PING && byteSize == sizeof(unsigned char) + sizeof(RakNet::Time))
                {
                    ...
                    SendImmediate((char*)outBitStream.GetData(), outBitStream.GetNumberOfBitsUsed(), IMMEDIATE_PRIORITY, UNRELIABLE, 0, systemAddress,
                        false, false, RakNet::GetTimeUS(), 0);
                ...
                else if ((unsigned char)(data)[0] == ID_CONNECTION_REQUEST_ACCEPTED)
                {
                    if (byteSize > sizeof(MessageID) + sizeof(unsigned int) + sizeof(unsigned short) + sizeof(SystemIndex) + sizeof(RakNet::Time) * 2)
                    {
                        ...
                        if (remoteSystem->connectMode == RemoteSystemStruct::HANDLING_CONNECTION_REQUEST ||
                            remoteSystem->connectMode == RemoteSystemStruct::REQUESTED_CONNECTION || allowConnectionResponseIPMigration)
                        ...
                        if (allowConnection)
                        {
                            ...
                            // As soon as a multiple of 16 byte packet arrives on the remote system, we will turn on AES.  This works because all
                            // encrypted packets are multiples of 16 and the packets I happen to be sending before this are not a multiple of 16
                            // bytes.  Otherwise there is no way to know if a packet that arrived is encrypted or not so the other side won't know to
                            // turn on encryption or not.
                            RakAssert((outBitStream.GetNumberOfBytesUsed() & 15) != 0);
                            SendImmediate((char*)outBitStream.GetData(), outBitStream.GetNumberOfBitsUsed(), IMMEDIATE_PRIORITY, RELIABLE_ORDERED, 0,
                                systemAddress, false, false, RakNet::GetTimeUS(), 0);
...
RAK_THREAD_DECLARATION(RakNet::RecvFromLoop)
{
    ...
    while (rakPeer->endThreads == false)
    {
        ...
        if (recvFromStruct != NULL)
        {
            ...
            SocketLayer::RecvFromBlocking(s, rakPeer, remotePortRakNetWasStartedOn_PS3, extraSocketOptions, recvFromStruct->data,
...
// #if ((_WIN32_WINNT >= 0x0400) || (_WIN32_WINDOWS > 0x0400)) &&
        ...
        #pragma message("-- RakNet: Using Sleep(). Uncomment USE_WAIT_FOR_MULTIPLE_EVENTS in RakNetDefines.h if you want to use WaitForSingleObject
instead. --")
