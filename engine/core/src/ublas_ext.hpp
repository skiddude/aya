...
typename M::size_type lu_factorize_singular(M& m, PM& pm, const IsZeroFunctor& iszero)
{
    ...
    for (; rowi < size && columni < size2; ++rowi, ++columni)
    {
        ...
        project(m, range(rowi + 1, size1), range(columni + 1, size2))
            .minus_assign(outer_prod(project(mci, range(rowi + 1, size1)), project(mri, range(columni + 1, size2))));
    }
    return rowi;
}

template<class PM, class MV>
BOOST_UBLAS_INLINE void unswap_rows(const PM& pm, MV& mv, vector_tag)
{
...
template<class PM, class MV>
BOOST_UBLAS_INLINE void unswap_rows(const PM& pm, MV& mv, matrix_tag)
{
    typedef typename PM::size_type size_type;
    typedef typename MV::value_type value_type;
...
// Dispatcher
template<class PM, class MV>
BOOST_UBLAS_INLINE void unswap_rows(const PM& pm, MV& mv)
{
