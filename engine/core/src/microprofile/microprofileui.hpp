...
#define MicroProfileMouseButton(foo, bar)                                      \
  do {                                                                         \
  } while (0)
#define MicroProfileMousePosition(foo, bar, z)                                 \
  do {                                                                         \
  } while (0)
#define MicroProfileModKey(key)                                                \
  do {                                                                         \
  } while (0)
#define MicroProfileDraw(foo, bar)                                             \
  do {                                                                         \
  } while (0)
#define MicroProfileIsDrawing() 0
#define MicroProfileToggleDisplayMode()                                        \
  do {                                                                         \
  } while (0)
#define MicroProfileSetDisplayMode(f)                                          \
  do {                                                                         \
  } while (0)
#else
...
#define MICROPROFILE_NUM_FRAMES                                                \
  (MICROPROFILE_MAX_FRAME_HISTORY - (MICROPROFILE_GPU_FRAME_DELAY + 1))
...
MICROPROFILEUI_API void
MicroProfileDraw(uint32_t nWidth,
                 uint32_t nHeight); //! call if drawing microprofilers
MICROPROFILEUI_API bool MicroProfileIsDrawing();
MICROPROFILEUI_API void MicroProfileToggleGraph(MicroProfileToken nToken);
MICROPROFILEUI_API bool MicroProfileDrawGraph(uint32_t nScreenWidth,
                                              uint32_t nScreenHeight);
MICROPROFILEUI_API void
MicroProfileToggleDisplayMode(); // switch between off, bars, detailed
MICROPROFILEUI_API void
MicroProfileSetDisplayMode(int); // switch between off, bars, detailed
MICROPROFILEUI_API void MicroProfileClearGraph();
MICROPROFILEUI_API void MicroProfileMousePosition(uint32_t nX, uint32_t nY,
                                                  int nWheelDelta);
MICROPROFILEUI_API void MicroProfileModKey(uint32_t nKeyState);
MICROPROFILEUI_API void MicroProfileMouseButton(uint32_t nLeft,
                                                uint32_t nRight);
MICROPROFILEUI_API void
MicroProfileDrawLineVertical(int nX, int nTop, int nBottom, uint32_t nColor);
MICROPROFILEUI_API void MicroProfileDrawLineHorizontal(int nLeft, int nRight,
                                                       int nY, uint32_t nColor);
...
MICROPROFILEUI_API void MicroProfileDrawText(int nX, int nY, uint32_t nColor,
                                             const char *pText,
                                             uint32_t nNumCharacters);
MICROPROFILEUI_API void
MicroProfileDrawBox(int nX, int nY, int nX1, int nY1, uint32_t nColor,
                    MicroProfileBoxType = MicroProfileBoxTypeFlat);
MICROPROFILEUI_API void
MicroProfileDrawLine2D(uint32_t nVertices, float *pVertices, uint32_t nColor);
...
MICROPROFILEUI_API void MicroProfileCustomGroup(const char *pCustomName,
                                                uint32_t nMaxTimers,
                                                uint32_t nAggregateFlip,
                                                float fReferenceTime,
                                                uint32_t nFlags);
MICROPROFILEUI_API void MicroProfileCustomGroupAddTimer(const char *pCustomName,
                                                        const char *pGroup,
                                                        const char *pTimer);
...
MICROPROFILE_DEFINE(g_MicroProfileDetailed, "MicroProfile", "Detailed View",
                    0x8888000);
MICROPROFILE_DEFINE(g_MicroProfileDrawGraph, "MicroProfile", "Draw Graph",
                    0xff44ee00);
MICROPROFILE_DEFINE(g_MicroProfileDrawBarView, "MicroProfile", "DrawBarView",
                    0x00dd77);
...
struct SOptionDesc {
  SOptionDesc() {}
  SOptionDesc(uint8_t nSubType, uint8_t nIndex, const char *fmt, ...)
      : nSubType(nSubType), nIndex(nIndex) {
    va_list args;
    va_start(args, fmt);
...
static float g_MicroProfileReferenceTimePresets[] = {
    5.f, 10.f, 15.f, 20.f, 33.33f, 66.66f, 100.f, 250.f, 500.f, 1000.f};
...
enum {
  MICROPROFILE_NUM_REFERENCE_PRESETS =
      sizeof(g_MicroProfileReferenceTimePresets) /
      sizeof(g_MicroProfileReferenceTimePresets[0]),
  MICROPROFILE_NUM_OPACITY_PRESETS = sizeof(g_MicroProfileOpacityPresets) /
                                     sizeof(g_MicroProfileOpacityPresets[0]),
#if MICROPROFILE_CONTEXT_SWITCH_TRACE
  MICROPROFILE_OPTION_SIZE = MICROPROFILE_NUM_REFERENCE_PRESETS +
                             MICROPROFILE_NUM_OPACITY_PRESETS * 2 + 2 + 7,
#else
  MICROPROFILE_OPTION_SIZE = MICROPROFILE_NUM_REFERENCE_PRESETS +
                             MICROPROFILE_NUM_OPACITY_PRESETS * 2 + 2 + 3,
#endif
};
...
struct MicroProfileUI {
  ...
  float fDetailedOffset; // display offset relative to start of latest
                         // displayable frame.
  ...
  MicroProfileGroupMenuItem
      GroupMenu[MICROPROFILE_MAX_GROUPS + MICROPROFILE_MAX_CATEGORIES];
...
static uint32_t g_nMicroProfileContextSwitchThreadColors
    [MICROPROFILE_NUM_CONTEXT_SWITCH_COLORS] = // palette generated by
                                               // http://tools.medialab.sciences-po.fr/iwanthue/index.php
    {
        0x63607B, 0x755E2B, 0x326A55, 0x523135, 0x904F42, 0x87536B,
        0x346875, 0x5E6046, 0x35404C, 0x224038, 0x413D1E, 0x5E3A26,
        0x5D6161, 0x4C6234, 0x7D564F, 0x5C4352,
};
...
void MicroProfileInitUI() {
  ...
  if (!bInitialized) {
    ...
    for (int i = 0; i < MICROPROFILE_NUM_REFERENCE_PRESETS; ++i) {
      UI.Options[nIndex++] =
          SOptionDesc(0, i, "  %6.2fms", g_MicroProfileReferenceTimePresets[i]);
    }
...
void MicroProfileStringArrayAddLiteral(MicroProfileStringArray *pArray,
                                       const char *pLiteral) {
  MP_ASSERT(pArray->nNumStrings < MICROPROFILE_TOOLTIP_MAX_STRINGS);
  pArray->ppStrings[pArray->nNumStrings++] = pLiteral;
}

MICROPROFILE_FORMAT(2, 3)
void MicroProfileStringArrayFormat(MicroProfileStringArray *pArray,
                                   const char *fmt, ...) {
  MP_ASSERT(pArray->nNumStrings < MICROPROFILE_TOOLTIP_MAX_STRINGS);
  ...
  va_end(args);
  MP_ASSERT(pArray->pBufferPos <
            pArray->Buffer + MICROPROFILE_TOOLTIP_STRING_BUFFER_SIZE);
}
void MicroProfileStringArrayCopy(MicroProfileStringArray *pDest,
                                 MicroProfileStringArray *pSrc) {
  ...
  for (uint32_t i = 0; i < MICROPROFILE_TOOLTIP_MAX_STRINGS; ++i) {
    if (i < pSrc->nNumStrings) {
      if (pSrc->ppStrings[i] >= &pSrc->Buffer[0] &&
          pSrc->ppStrings[i] <
              &pSrc->Buffer[0] + MICROPROFILE_TOOLTIP_STRING_BUFFER_SIZE) {
...
void MicroProfileFloatWindowSize(const char **ppStrings, uint32_t nNumStrings,
                                 uint32_t *pColors, uint32_t &nWidth,
                                 uint32_t &nHeight,
                                 uint32_t *pStringLengths = 0) {
  uint32_t *nStringLengths =
      pStringLengths ? pStringLengths
                     : (uint32_t *)alloca(nNumStrings * sizeof(uint32_t));
  ...
  nWidth = (MICROPROFILE_TEXT_WIDTH + 1) * (2 + nWidth) +
           2 * MICROPROFILE_BORDER_SIZE;
  if (pColors)
    nWidth += MICROPROFILE_TEXT_WIDTH + 1;
  nHeight = (MICROPROFILE_TEXT_HEIGHT + 1) * nTextCount +
            2 * MICROPROFILE_BORDER_SIZE;
}

void MicroProfileDrawFloatWindow(uint32_t nX, uint32_t nY,
                                 const char **ppStrings, uint32_t nNumStrings,
                                 uint32_t nColor, uint32_t *pColors = 0) {
  uint32_t nWidth = 0, nHeight = 0;
  uint32_t *nStringLengths = (uint32_t *)alloca(nNumStrings * sizeof(uint32_t));
  MicroProfileFloatWindowSize(ppStrings, nNumStrings, pColors, nWidth, nHeight,
                              nStringLengths);
  uint32_t nTextCount = nNumStrings / 2;
  ...
  MicroProfileDrawBox(nX - 1, nY - 1, nX + nWidth + 1, nY + nHeight + 1,
                      0xff000000 | nColor);
  ...
  for (uint32_t i = 0; i < nTextCount; ++i) {
    int i0 = i * 2;
    if (pColors) {
      MicroProfileDrawBox(nX - MICROPROFILE_TEXT_WIDTH, nY, nX,
                          nY + MICROPROFILE_TEXT_WIDTH,
                          pColors[i] | 0xff000000);
    }
    MicroProfileDrawText(nX + 1, nY + 1, (uint32_t)-1, ppStrings[i0],
                         (uint32_t)strlen(ppStrings[i0]));
    MicroProfileDrawText(
        nX + nWidth - nStringLengths[i0 + 1] * (MICROPROFILE_TEXT_WIDTH + 1),
        nY + 1, (uint32_t)-1, ppStrings[i0 + 1],
        (uint32_t)strlen(ppStrings[i0 + 1]));
    nY += (MICROPROFILE_TEXT_HEIGHT + 1);
  }
}
void MicroProfileDrawTextBox(uint32_t nX, uint32_t nY, const char **ppStrings,
                             uint32_t nNumStrings, uint32_t nColor,
                             uint32_t *pColors = 0) {
  uint32_t nWidth = 0, nHeight = 0;
  uint32_t *nStringLengths = (uint32_t *)alloca(nNumStrings * sizeof(uint32_t));
  ...
  nWidth = (MICROPROFILE_TEXT_WIDTH + 1) * (2 + nWidth) +
           2 * MICROPROFILE_BORDER_SIZE;
  nHeight =
      (MICROPROFILE_TEXT_HEIGHT + 1) * nHeight + 2 * MICROPROFILE_BORDER_SIZE;
  if (nX + nWidth > UI.nWidth)
    nX = UI.nWidth - nWidth;
  if (nY + nHeight > UI.nHeight)
    nY = UI.nHeight - nHeight;
  ...
  if (UI.nRangeBeginIndex != UI.nRangeEndIndex && UI.pRangeLog) {
    ...
    for (int i = 0; i < MICROPROFILE_META_MAX; ++i) {
      if (S.MetaCounters[i].pName && (nMetaSum[i] || nMetaSumInclusive[i])) {
        ...
        MicroProfileStringArrayFormat(pToolTip, "%s excl",
                                      S.MetaCounters[i].pName);
        MicroProfileStringArrayFormat(pToolTip, "%5lld",
                                      (long long)nMetaSum[i]);
        MicroProfileStringArrayFormat(pToolTip, "%s incl",
                                      S.MetaCounters[i].pName);
        MicroProfileStringArrayFormat(
            pToolTip, "%5lld", (long long)(nMetaSum[i] + nMetaSumInclusive[i]));
      }
    }
  }
}
...
void MicroProfileDrawFloatTooltip(uint32_t nX, uint32_t nY, uint32_t nToken,
                                  uint64_t nTime) {
  MicroProfile &S = *MicroProfileGet();

  uint32_t nIndex = MicroProfileGetTimerIndex(nToken);
  uint32_t nAggregateFrames = S.nAggregateFrames ? S.nAggregateFrames : 1;
  uint32_t nAggregateCount =
      S.Aggregate[nIndex].nCount ? S.Aggregate[nIndex].nCount : 1;
  ...
  float fToMs = MicroProfileTickToMsMultiplier(
      bGpu ? MicroProfileTicksPerSecondGpu() : MicroProfileTicksPerSecondCpu());
  ...
  float fAverageExclusive =
      fToMs * (S.AggregateExclusive[nIndex] / nAggregateFrames);
  ...
  MicroProfileStringArrayFormat(&ToolTip, "%6.2f",
                                double(nAggregateCount) / nAggregateFrames);
  ...
  MicroProfileDrawFloatWindow(nX, nY + 20, &ToolTip.ppStrings[0],
                              ToolTip.nNumStrings,
                              S.TimerInfo[nTimerId].nColor);

  if (UI.nMouseLeftMod) {
    int nIndex = (g_MicroProfileUI.LockedToolTipFront +
                  MICROPROFILE_TOOLTIP_MAX_LOCKED - 1) %
                 MICROPROFILE_TOOLTIP_MAX_LOCKED;
    g_MicroProfileUI.nLockedToolTipColor[nIndex] = S.TimerInfo[nTimerId].nColor;
    MicroProfileStringArrayCopy(&g_MicroProfileUI.LockedToolTips[nIndex],
                                &ToolTip);
...
void MicroProfileZoomTo(int64_t nTickStart, int64_t nTickEnd,
                        MicroProfileTokenType eToken) {
  MicroProfile &S = *MicroProfileGet();

  bool bGpu = eToken == MicroProfileTokenTypeGpu;
  int64_t nStart = bGpu ? S.Frames[S.nFrameCurrent].nFrameStartGpu
                        : S.Frames[S.nFrameCurrent].nFrameStartCpu;
  uint64_t nFrequency =
      bGpu ? MicroProfileTicksPerSecondGpu() : MicroProfileTicksPerSecondCpu();

  float fToMs = MicroProfileTickToMsMultiplier(nFrequency);
  UI.fDetailedOffsetTarget =
      MicroProfileLogTickDifference(nStart, nTickStart) * fToMs;
  UI.fDetailedRangeTarget =
      MicroProfileLogTickDifference(nTickStart, nTickEnd) * fToMs;
}
...
void MicroProfileCenter(int64_t nTickCenter) {
  ...
  UI.fDetailedOffsetTarget = UI.fDetailedOffset =
      fCenter - 0.5f * UI.fDetailedRange;
}
...
void MicroProfileDrawDetailedContextSwitchBars(uint32_t nY, uint32_t nThreadId,
                                               uint32_t nContextSwitchStart,
                                               uint32_t nContextSwitchEnd,
                                               int64_t nBaseTicks,
                                               uint32_t nBaseY) {
  MicroProfile &S = *MicroProfileGet();
  int64_t nTickIn = -1;
  ...
  for (uint32_t j = nContextSwitchStart; j != nContextSwitchEnd;
    ...
    if (nTickIn == -1) {
      ...
      if (CS.nThreadOut == nThreadId) {
        ...
        float fMsStart =
            fToMs * MicroProfileLogTickDifference(nBaseTicks, nTickIn);
        float fMsEnd =
            fToMs * MicroProfileLogTickDifference(nBaseTicks, nTickOut);
        if (fMsStart <= fMsEnd) {
          ...
          uint32_t nColor = g_nMicroProfileContextSwitchThreadColors
              [CS.nCpu % MICROPROFILE_NUM_CONTEXT_SWITCH_COLORS];
          float fXDist = MicroProfileMax(fXStart - fMouseX, fMouseX - fXEnd);
          bool bHover = fXDist < MICROPROFILE_HOVER_DIST &&
                        fYStart <= fMouseY && fMouseY <= fYEnd &&
                        nBaseY < fMouseY;
          if (bHover) {
          ...
          if (nIntegerWidth) {
            MicroProfileDrawBox((int)fXStart, (int)fYStart, (int)fXEnd,
                                (int)fYEnd, nColor | UI.nOpacityForeground,
            ...
            if (nLineDrawn != nLineX) {
              nLineDrawn = nLineX;
              MicroProfileDrawLineVertical(nLineX, fYStart + 0.5f, fYEnd + 0.5f,
                                           nColor | UI.nOpacityForeground);
            }
          }
        }
        nTickIn = -1;
...
void MicroProfileDrawDetailedBars(uint32_t nWidth, uint32_t nHeight, int nBaseY,
  ...
  int64_t nDetailedOffsetTicksCpu =
      MicroProfileMsToTick(fDetailedOffset, MicroProfileTicksPerSecondCpu());
  int64_t nDetailedOffsetTicksGpu =
      MicroProfileMsToTick(fDetailedOffset, MicroProfileTicksPerSecondGpu());
  int64_t nBaseTicksCpu = nDetailedOffsetTicksCpu + nFrameStartCpu;
  int64_t nBaseTicksGpu = nDetailedOffsetTicksGpu + nFrameStartGpu;
  int64_t nBaseTicksEndCpu =
      nBaseTicksCpu +
      MicroProfileMsToTick(fDetailedRange, MicroProfileTicksPerSecondCpu());
  int64_t nBaseTicksEndGpu =
      nBaseTicksGpu +
      MicroProfileMsToTick(fDetailedRange, MicroProfileTicksPerSecondGpu());
  ...
  if (UI.nTickReferenceCpu && UI.nTickReferenceGpu) {
    nBaseTicksGpu = (nBaseTicksCpu - UI.nTickReferenceCpu) *
                        nTicksPerSecondGpu / nTicksPerSecondCpu +
                    UI.nTickReferenceGpu;
  }

  MicroProfileFrameState *pFrameFirst = pFrameCurrent;
  int64_t nGapTime =
      MicroProfileTicksPerSecondCpu() * MICROPROFILE_GAP_TIME / 1000;
  for (uint32_t i = 0;
       i < MICROPROFILE_MAX_FRAME_HISTORY - MICROPROFILE_GPU_FRAME_DELAY; ++i) {
    uint32_t nPrevIndex =
        (S.nFrameCurrent + MICROPROFILE_MAX_FRAME_HISTORY - i) %
  ...
  for (uint32_t i = pFrameFirst - &S.Frames[0]; i != nFrameNext;
       i = (i + 1) % MICROPROFILE_MAX_FRAME_HISTORY) {
    uint64_t nTickStart = S.Frames[i].nFrameStartCpu;
    float fMsStart =
        fToMsCpu * MicroProfileLogTickDifference(nBaseTicksCpu, nTickStart);
    float fXStart = fMsStart * fMsToScreen;

    MicroProfileDrawLineVertical(fXStart, nBaseY, nBaseY + nHeight,
                                 UI.nOpacityForeground | 0xbbbbbb);
  }

  {
    ...
    float fStepLabelOffset =
        (fStep * fMsToScreen -
         nStepLabelLength * (MICROPROFILE_TEXT_WIDTH + 1)) /
        2;
    ...
    for (float f = fStart; f < fMsEnd;) {
      ...
      float fNext = f + fStep;
      MicroProfileDrawBox(
          (int)((fStart - fMsBase) * fMsToScreen), nBaseY,
          (int)((fNext - fMsBase) * fMsToScreen + 1), nBaseY + nHeight,
          UI.nOpacityBackground | g_nMicroProfileBackColors[nColorIndex++ & 1]);

      if (nStepLabelLength)
        MicroProfileDrawText(
            (fStart - fMsBase) * fMsToScreen + fStepLabelOffset, nBaseY,
            UI.nOpacityForeground | 0x808080, StepLabel, nStepLabelLength);
  ...
  uint64_t nMouseOverToken = pMouseOver ? MicroProfileLogTimerIndex(*pMouseOver)
  ...
  if (nHoverCounter >= 245)
    ...
    nHoverCounterDelta = 10;
  UI.nHoverColor = (nHoverCounter << 24) | (nHoverCounter << 16) |
                   (nHoverCounter << 8) | nHoverCounter;
  uint32_t nHoverCounterShared = nHoverCounter >> 2;
  UI.nHoverColorShared = (nHoverCounterShared << 24) |
                         (nHoverCounterShared << 16) |
                         (nHoverCounterShared << 8) | nHoverCounterShared;
  ...
  S.nContextSwitchHoverThread = S.nContextSwitchHoverThreadAfter =
  ...
  if (S.bContextSwitchRunning) {
    MicroProfileContextSwitchSearch(&nContextSwitchStart, &nContextSwitchEnd,
                                    nBaseTicksCpu, nBaseTicksEndCpu);
  }

  uint64_t nActiveGroup =
      S.nAllGroupsWanted ? S.nGroupMask : S.nActiveGroupWanted;
  ...
  if (!bSkipBarView) {
    for (uint32_t i = 0; i < MICROPROFILE_MAX_THREADS; ++i) {
      ...
      /// note: this may display new samples as old data, but this will only
      /// happen when
      ...
      do {
        MP_ASSERT(pFrameLogFirst >= &S.Frames[0] &&
                  pFrameLogFirst < &S.Frames[MICROPROFILE_MAX_FRAME_HISTORY]);
        uint32_t nNewGet = pFrameLogFirst->nLogStart[i];

        // If [nNewGet, nGet) intersects with [nPut, nFront) then frame is not
        // valid
        bool bIsValid =
            (nPut <= nFront)
                // ... [ nPut ... nFront ) ...
                ? ((nNewGet <= nGet) ? (nGet <= nPut || nNewGet >= nFront)
                                     : (nNewGet >= nFront && nGet <= nPut))
                // ... nFront ) ... [ nPut ...
                : ((nNewGet <= nGet) ? (nFront <= nNewGet && nGet <= nPut)
                                     : false);
      ...
      snprintf(ThreadName, sizeof(ThreadName) - 1, "%04llx: %s",
               (long long)nThreadId, &pLog->ThreadName[0]);
      nY += 3;
      uint32_t nThreadColor = -1;
      if (pLog->nThreadId == nContextSwitchHoverThreadAfter ||
          pLog->nThreadId == nContextSwitchHoverThreadBefore)
        nThreadColor = UI.nHoverColorShared | 0x906060;
      MicroProfileDrawText(0, nY, nThreadColor, &ThreadName[0],
                           (uint32_t)strlen(&ThreadName[0]));
      nY += 3;
      nY += MICROPROFILE_TEXT_HEIGHT + 1;

      if (S.bContextSwitchRunning) {
        MicroProfileDrawDetailedContextSwitchBars(
            nY, pLog->nThreadId, nContextSwitchStart, nContextSwitchEnd,
            nBaseTicks, nBaseY);
      ...
      for (uint32_t j = 0; j < 2; ++j) {
        ...
        for (uint32_t k = nStart; k < nEnd; ++k) {
          ...
          if (MP_LOG_ENTER == nType) {
            ...
            MicroProfileLogEntry *pEntryEnter =
                pLog->Log + nStack[nStackPos - 1];
            if (MicroProfileLogTimerIndex(*pEntryEnter) !=
                MicroProfileLogTimerIndex(*pEntry)) {
            ...
            if (!(nActiveGroup &
                  (1ull << S.TimerInfo[nTimerIndex].nGroupIndex))) {
              nStackPos--;
              continue;
            }
            if (nMouseOverToken == nTimerIndex) {
              if (pEntry == pMouseOver) {
                ...
                if (bGpu) {
                  ...
                  uint32_t nCpuBegin =
                      (nStack[nStackPos - 1] + 1) % MICROPROFILE_BUFFER_SIZE;
                  ...
                  if (MicroProfileLogType(LogCpuBegin) == MP_LOG_GPU_EXTRA &&
                      MicroProfileLogType(LogCpuEnd) == MP_LOG_GPU_EXTRA) {
            ...
            if (pName[0] == '$' && pEntryEnter < pEntry &&
                MicroProfileLogType(pEntryEnter[1 + bGpu]) == MP_LOG_LABEL) {
              const char *pLabel = MicroProfileGetLabel(
                  MicroProfileLogGetTick(pEntryEnter[1 + bGpu]));
            ...
            float fMsStart =
                fToMs * MicroProfileLogTickDifference(nBaseTicks, nTickStart);
            float fMsEnd =
                fToMs * MicroProfileLogTickDifference(nBaseTicks, nTickEnd);
            float fXStart = fMsStart * fMsToScreen;
            float fXEnd = fMsEnd * fMsToScreen;
            ...
            if (nIntegerWidth) {
...
#endif
                nHoverTime =
                    MicroProfileLogTickDifference(nTickStart, nTickEnd);
              ...
              if (nIntegerWidth > 3 * MICROPROFILE_TEXT_WIDTH) {
                ...
                int nCharacters = (nTextWidth - MICROPROFILE_TEXT_WIDTH) /
                                  (MICROPROFILE_TEXT_WIDTH + 1);
                if (nCharacters > 0) {
                  MicroProfileDrawText(
                      (int)(fXStartText + 1), (int)(fYStart + 1), -1, pName,
                      MicroProfileMin<uint32_t>(nNameLen, nCharacters));
                }
              }
#endif
              ++nNumBoxes;
              ...
              if (nLineX != (int)nLinesDrawn[nStackPos]) {
                ...
                MicroProfileDrawLineVertical(nLineX, (int)(fYStart + 0.5f),
                                             (int)(fYEnd + 0.5f),
                                             nColor | UI.nOpacityForeground);
                ++nNumLines;
              }
            ...
            nStackPos--;
            if (0 == nStackPos) {
              if (bGpu ? (nTickStart > nBaseTicksEndGpu)
                       : (nTickStart > nBaseTicksEndCpu)) {
                break;
              }
            }
          }
  ...
  if (S.bContextSwitchRunning &&
      (S.bContextSwitchAllThreads || S.bContextSwitchNoBars)) {
    ...
    if (S.bContextSwitchAllThreads) {
      for (uint32_t i = nContextSwitchStart; i != nContextSwitchEnd;
           i = (i + 1) % MICROPROFILE_CONTEXT_SWITCH_BUFFER_SIZE) {
    ...
    for (uint32_t i = nStart; i < nNumThreads; ++i) {
      ...
      if (nThreadId) {
        ...
        int nStrLen = snprintf(
            ThreadName, sizeof(ThreadName) - 1, "%04x: %s%s",
            (uint32_t)nThreadId, cLocal,
            i < nNumThreadsBase ? &S.Pool[i]->ThreadName[0]
                                : MICROPROFILE_THREAD_NAME_FROM_ID(nThreadId));
        uint32_t nThreadColor = -1;
        if (nThreadId == nContextSwitchHoverThreadAfter ||
            nThreadId == nContextSwitchHoverThreadBefore)
          nThreadColor = UI.nHoverColorShared | 0x906060;
        MicroProfileDrawDetailedContextSwitchBars(
            nY + 2, nThreadId, nContextSwitchStart, nContextSwitchEnd,
            nBaseTicksCpu, nBaseY);
  ...
  if (!S.nRunning) {
    ...
    if (nSelectedFrame != -1) {
      ...
      UI.nRangeEnd =
          S.Frames[(nSelectedFrame + 1) % MICROPROFILE_MAX_FRAME_HISTORY]
              .nFrameStartCpu;
    ...
    if (UI.nRangeBegin != UI.nRangeEnd) {
      float fMsStart = fToMsCpu * MicroProfileLogTickDifference(nBaseTicksCpu,
                                                                UI.nRangeBegin);
      float fMsEnd =
          fToMsCpu * MicroProfileLogTickDifference(nBaseTicksCpu, UI.nRangeEnd);
      float fXStart = fMsStart * fMsToScreen;
      float fXEnd = fMsEnd * fMsToScreen;
      MicroProfileDrawBox((int)fXStart, nBaseY, (int)fXEnd, nHeight,
                          MICROPROFILE_FRAME_COLOR_HIGHTLIGHT,
                          MicroProfileBoxTypeFlat);
      MicroProfileDrawLineVertical((int)fXStart, nBaseY, nHeight,
                                   MICROPROFILE_FRAME_COLOR_HIGHTLIGHT |
                                       0x44000000);
      MicroProfileDrawLineVertical((int)fXEnd, nBaseY, nHeight,
                                   MICROPROFILE_FRAME_COLOR_HIGHTLIGHT |
                                       0x44000000);
      ...
      float fStartTextWidth =
          (float)((1 + MICROPROFILE_TEXT_WIDTH) * nLenStart);
      float fStartTextX = fXStart - fStartTextWidth - 2;
      MicroProfileDrawBox((int)fStartTextX, nBaseY,
                          (int)(fStartTextX + fStartTextWidth + 2),
                          MICROPROFILE_TEXT_HEIGHT + 2 + nBaseY, 0x33000000,
                          MicroProfileBoxTypeFlat);
      MicroProfileDrawText((int)fStartTextX + 1, nBaseY, (uint32_t)-1, sBuffer,
                           nLenStart);
      uint32_t nLenEnd =
          snprintf(sBuffer, sizeof(sBuffer) - 1, "%.2fms", fMsEnd);
      MicroProfileDrawBox(
          (int)(fXEnd + 1), nBaseY,
          (int)(fXEnd + 1 + (1 + MICROPROFILE_TEXT_WIDTH) * nLenEnd + 3),
          MICROPROFILE_TEXT_HEIGHT + 2 + nBaseY, 0x33000000,
          MicroProfileBoxTypeFlat);
      MicroProfileDrawText((int)(fXEnd + 2), nBaseY + 1, (uint32_t)-1, sBuffer,
                           nLenEnd);
    ...
    if (UI.nRangeBeginGpu != UI.nRangeEndGpu) {
      float fMsStart = fToMsGpu * MicroProfileLogTickDifference(
                                      nBaseTicksGpu, UI.nRangeBeginGpu);
      float fMsEnd = fToMsGpu * MicroProfileLogTickDifference(nBaseTicksGpu,
                                                              UI.nRangeEndGpu);
      float fXStart = fMsStart * fMsToScreen;
      float fXEnd = fMsEnd * fMsToScreen;
      MicroProfileDrawBox((int)fXStart, nBaseY, (int)fXEnd, nHeight,
                          MICROPROFILE_FRAME_COLOR_HIGHTLIGHT_GPU,
                          MicroProfileBoxTypeFlat);
      MicroProfileDrawLineVertical((int)fXStart, nBaseY, nHeight,
                                   MICROPROFILE_FRAME_COLOR_HIGHTLIGHT_GPU |
                                       0x44000000);
      ...
      MicroProfileDrawBox((int)fStartTextX, nBaseY,
                          ...
                          MICROPROFILE_TEXT_HEIGHT + 2 + nBaseY, 0x33000000,
                          MicroProfileBoxTypeFlat);
      MicroProfileDrawText((int)(fStartTextX + 1), nBaseY, (uint32_t)-1,
                           sBuffer, nLenStart);
      uint32_t nLenEnd =
          snprintf(sBuffer, sizeof(sBuffer) - 1, "%.2fms", fMsEnd);
      MicroProfileDrawBox(
      ...
      if (UI.nMouseRight) {
        MicroProfileZoomTo(UI.nRangeBeginGpu, UI.nRangeEndGpu,
...
void MicroProfileDrawDetailedFrameHistory(uint32_t nWidth, uint32_t nHeight,
                                          uint32_t nBaseY,
                                          uint32_t nSelectedFrame) {
  MicroProfile &S = *MicroProfileGet();
  ...
  MicroProfileDrawBox(
      0, nBaseY, nWidth, nBaseY + MICROPROFILE_FRAME_HISTORY_HEIGHT,
      0xff000000 | g_nMicroProfileBackColors[0], MicroProfileBoxTypeFlat);
  float fToMs =
      MicroProfileTickToMsMultiplier(MicroProfileTicksPerSecondCpu()) *
      S.fRcpReferenceTime;
  float fToMsGpu =
      MicroProfileTickToMsMultiplier(MicroProfileTicksPerSecondGpu()) *
      S.fRcpReferenceTime;

  MicroProfileFrameState *pFrameCurrent = &S.Frames[S.nFrameCurrent];
  uint64_t nFrameStartCpu = pFrameCurrent->nFrameStartCpu;
  int64_t nDetailedOffsetTicksCpu =
      MicroProfileMsToTick(UI.fDetailedOffset, MicroProfileTicksPerSecondCpu());
  int64_t nCpuStart = nDetailedOffsetTicksCpu + nFrameStartCpu;
  int64_t nCpuEnd =
      nCpuStart +
      MicroProfileMsToTick(UI.fDetailedRange, MicroProfileTicksPerSecondCpu());
  ;

  float fSelectionStart = (float)nWidth;
  float fSelectionEnd = 0.f;
  for (uint32_t i = 0; i < MICROPROFILE_NUM_FRAMES; ++i) {
    uint32_t nIndex = (S.nFrameCurrent + MICROPROFILE_MAX_FRAME_HISTORY - i) %
                      MICROPROFILE_MAX_FRAME_HISTORY;
    ...
    if (nIndex == nSelectedFrame)
      nColor = (uint32_t)-1;
    MicroProfileDrawBox((int)fXStart, (int)(nBaseY + fScale * nBarHeight),
                        (int)fXEnd, nBaseY + MICROPROFILE_FRAME_HISTORY_HEIGHT,
  ...
  MicroProfileDrawBox((int)fSelectionStart, nBaseY, (int)fSelectionEnd,
                      nBaseY + MICROPROFILE_FRAME_HISTORY_HEIGHT,
                      MICROPROFILE_FRAME_HISTORY_COLOR_HIGHTLIGHT,
                      MicroProfileBoxTypeFlat);
}
void MicroProfileDrawDetailedView(uint32_t nWidth, uint32_t nHeight,
  ...
  if (UI.nMouseY > nBaseY &&
      UI.nMouseY <= nBaseY + MICROPROFILE_FRAME_HISTORY_HEIGHT &&
      UI.nActiveMenu == (uint32_t)-1) {

    nSelectedFrame =
        ((MICROPROFILE_NUM_FRAMES) * (UI.nWidth - UI.nMouseX) / UI.nWidth);
    nSelectedFrame =
        (S.nFrameCurrent + MICROPROFILE_MAX_FRAME_HISTORY - nSelectedFrame) %
        MICROPROFILE_MAX_FRAME_HISTORY;
    ...
    if (UI.nMouseDownLeft) {
      uint64_t nFrac =
          (1024 * (MICROPROFILE_NUM_FRAMES) * (UI.nMouseX) / UI.nWidth) % 1024;
      int64_t nRangeBegin = S.Frames[nSelectedFrame].nFrameStartCpu;
      int64_t nRangeEnd =
          S.Frames[(nSelectedFrame + 1) % MICROPROFILE_MAX_FRAME_HISTORY]
              .nFrameStartCpu;
      MicroProfileCenter(nRangeBegin +
                         (nRangeEnd - nRangeBegin) * nFrac / 1024);
    }
  } else {
    UI.nHoverFrame = -1;
  }
  ...
  if (bDrawBars) {
    MicroProfileDrawDetailedBars(nWidth, nHeight,
                                 nBaseY + MICROPROFILE_FRAME_HISTORY_HEIGHT,
                                 nSelectedFrame);
  }
...
void MicroProfileDrawTextRight(uint32_t nX, uint32_t nY, uint32_t nColor,
                               const char *pStr, uint32_t nStrLen) {
  MicroProfileDrawText(nX - nStrLen * (MICROPROFILE_TEXT_WIDTH + 1), nY, nColor,
                       pStr, nStrLen);
}
void MicroProfileDrawHeader(int32_t nX, uint32_t nWidth, const char *pName) {
  if (pName) {
    MicroProfileDrawBox(nX - 8, MICROPROFILE_TEXT_HEIGHT + 2, nX + nWidth + 5,
                        MICROPROFILE_TEXT_HEIGHT + 2 +
                            (MICROPROFILE_TEXT_HEIGHT + 1),
                        0xff000000 | g_nMicroProfileBackColors[1]);
    MicroProfileDrawText(nX, MICROPROFILE_TEXT_HEIGHT + 2, (uint32_t)-1, pName,
                         (uint32_t)strlen(pName));
  }
}

typedef void (*MicroProfileLoopGroupCallback)(uint32_t nTimer, uint32_t nIdx,
                                              uint64_t nGroupMask, uint32_t nX,
                                              uint32_t nY, void *pData);

void MicroProfileLoopActiveGroupsDraw(int32_t nX, int32_t nY, const char *pName,
                                      MicroProfileLoopGroupCallback CB,
                                      void *pData) {
  MicroProfile &S = *MicroProfileGet();
...
void MicroProfileCalcTimers(float *pTimers, float *pAverage, float *pMax,
                            float *pMin, float *pCallAverage, float *pExclusive,
                            float *pAverageExclusive, float *pMaxExclusive,
                            uint64_t nGroup, uint32_t nSize) {
  MicroProfile &S = *MicroProfileGet();
  ...
  for (uint32_t j = 0; j < MICROPROFILE_MAX_GROUPS; ++j) {
    if (nMask & nGroup) {
      const float fToMs = MicroProfileTickToMsMultiplier(
          S.GroupInfo[j].Type == MicroProfileTokenTypeGpu
        ...
        if (nTokenMask & nMask) {
          {
            ...
            uint32_t nAggregateFrames =
                S.nAggregateFrames ? S.nAggregateFrames : 1;
            uint32_t nAggregateCount =
                S.Aggregate[nTimer].nCount ? S.Aggregate[nTimer].nCount : 1;
            float fToPrc = S.fRcpReferenceTime;
            float fMs = fToMs * (S.Frame[nTimer].nTicks);
            float fPrc = MicroProfileMin(fMs * fToPrc, 1.f);
            float fAverageMs =
                fToMs * (S.Aggregate[nTimer].nTicks / nAggregateFrames);
            ...
            float fMinMs = fToMs * (S.AggregateMin[nTimer] != uint64_t(-1)
                                        ? S.AggregateMin[nTimer]
                                        : 0);
            float fMinPrc = MicroProfileMin(fMinMs * fToPrc, 1.f);
            float fCallAverageMs =
                fToMs * (S.Aggregate[nTimer].nTicks / nAggregateCount);
            float fCallAveragePrc =
                MicroProfileMin(fCallAverageMs * fToPrc, 1.f);
            float fMsExclusive = fToMs * (S.FrameExclusive[nTimer]);
            float fPrcExclusive = MicroProfileMin(fMsExclusive * fToPrc, 1.f);
            float fAverageMsExclusive =
                fToMs * (S.AggregateExclusive[nTimer] / nAggregateFrames);
            float fAveragePrcExclusive =
                MicroProfileMin(fAverageMsExclusive * fToPrc, 1.f);
            float fMaxMsExclusive = fToMs * (S.AggregateMaxExclusive[nTimer]);
            float fMaxPrcExclusive =
                MicroProfileMin(fMaxMsExclusive * fToPrc, 1.f);
            pTimers[nIdx] = fMs;
            pTimers[nIdx + 1] = fPrc;
...
void MicroProfileDrawBarArrayCallback(uint32_t nTimer, uint32_t nIdx,
                                      uint64_t nGroupMask, uint32_t nX,
                                      uint32_t nY, void *pExtra) {
  ...
  if (pTimers2 && pTimers2[nIdx] > 0.1f)
    snprintf(sBuffer, SBUF_MAX - 1, "%5.2f %3.1fx", pTimers[nIdx],
             pTimers[nIdx] / pTimers2[nIdx]);
  else
    snprintf(sBuffer, SBUF_MAX - 1, "%5.2f", pTimers[nIdx]);
  if (!pTimers2)
    MicroProfileDrawBox(nX + nTextWidth, nY,
                        (int)(nX + nTextWidth + fWidth * pTimers[nIdx + 1]),
                        nY + nHeight,
                        UI.nOpacityForeground | S.TimerInfo[nTimer].nColor,
                        MicroProfileBoxTypeBar);
  MicroProfileDrawText(nX, nY, (uint32_t)-1, sBuffer,
                       (uint32_t)strlen(sBuffer));
}

uint32_t MicroProfileDrawBarArray(int32_t nX, int32_t nY, float *pTimers,
                                  const char *pName, uint32_t nTotalHeight,
                                  float *pTimers2 = NULL) {
  const uint32_t nTextWidth = 6 * (1 + MICROPROFILE_TEXT_WIDTH);
  const uint32_t nWidth = MICROPROFILE_BAR_WIDTH;

  MicroProfileDrawLineVertical(nX - 5, 0, nTotalHeight + nY,
                               UI.nOpacityBackground |
                                   g_nMicroProfileBackColors[0] |
                                   g_nMicroProfileBackColors[1]);
  float *pTimersArray[2] = {pTimers, pTimers2};
  MicroProfileLoopActiveGroupsDraw(
      nX, nY, pName, MicroProfileDrawBarArrayCallback, pTimersArray);
  MicroProfileDrawHeader(nX, nTextWidth + nWidth, pName);
  return nWidth + 5 + nTextWidth;
}
void MicroProfileDrawBarCallCountCallback(uint32_t nTimer, uint32_t nIdx,
                                          uint64_t nGroupMask, uint32_t nX,
                                          uint32_t nY, void *pExtra) {
  MicroProfile &S = *MicroProfileGet();
  char sBuffer[SBUF_MAX];
...
uint32_t MicroProfileDrawBarCallCount(int32_t nX, int32_t nY,
                                      const char *pName) {
  MicroProfileLoopActiveGroupsDraw(nX, nY, pName,
                                   MicroProfileDrawBarCallCountCallback, 0);
...
void MicroProfileDrawBarMetaAverageCallback(uint32_t nTimer, uint32_t nIdx,
                                            uint64_t nGroupMask, uint32_t nX,
                                            uint32_t nY, void *pExtra) {
  ...
  MicroProfileDrawText(nX - nLen * (MICROPROFILE_TEXT_WIDTH + 1), nY,
                       (uint32_t)-1, sBuffer, nLen);
}

uint32_t MicroProfileDrawBarMetaAverage(int32_t nX, int32_t nY,
                                        uint64_t *pCounters, const char *pName,
                                        uint32_t nTotalHeight) {
  if (!pName)
    return 0;
  MicroProfileDrawLineVertical(nX - 5, 0, nTotalHeight + nY,
                               UI.nOpacityBackground |
                                   g_nMicroProfileBackColors[0] |
                                   g_nMicroProfileBackColors[1]);
  uint32_t nTextWidth = (1 + MICROPROFILE_TEXT_WIDTH) *
                        MicroProfileMax<uint32_t>(6, (uint32_t)strlen(pName));
  ...
  MicroProfileLoopActiveGroupsDraw(nX + nTextWidth, nY, pName,
                                   MicroProfileDrawBarMetaAverageCallback,
                                   &Args);
  MicroProfileDrawHeader(nX, 5 + nTextWidth, pName);
  return 5 + nTextWidth;
}

void MicroProfileDrawBarMetaCountCallback(uint32_t nTimer, uint32_t nIdx,
                                          uint64_t nGroupMask, uint32_t nX,
                                          uint32_t nY, void *pExtra) {
  uint64_t *pCounters = (uint64_t *)pExtra;
  char sBuffer[SBUF_MAX];
  int nLen =
      snprintf(sBuffer, SBUF_MAX - 1, "%5lld", (long long)pCounters[nTimer]);
  MicroProfileDrawText(nX - nLen * (MICROPROFILE_TEXT_WIDTH + 1), nY,
                       (uint32_t)-1, sBuffer, nLen);
}

uint32_t MicroProfileDrawBarMetaCount(int32_t nX, int32_t nY,
                                      uint64_t *pCounters, const char *pName,
                                      uint32_t nTotalHeight) {
  if (!pName)
    return 0;
  ...
  uint32_t nTextWidth = (1 + MICROPROFILE_TEXT_WIDTH) *
                        MicroProfileMax<uint32_t>(6, (uint32_t)strlen(pName));
  MicroProfileLoopActiveGroupsDraw(nX + nTextWidth, nY, pName,
                                   MicroProfileDrawBarMetaCountCallback,
                                   pCounters);
  MicroProfileDrawHeader(nX, 5 + nTextWidth, pName);
  return 5 + nTextWidth;
}

void MicroProfileDrawBarLegendCallback(uint32_t nTimer, uint32_t nIdx,
                                       uint64_t nGroupMask, uint32_t nX,
                                       uint32_t nY, void *pExtra) {
  MicroProfile &S = *MicroProfileGet();
  ...
  MicroProfileDrawTextRight(nX, nY, S.TimerInfo[nTimer].nColor,
                            S.TimerInfo[nTimer].pName,
                            (uint32_t)strlen(S.TimerInfo[nTimer].pName));
...
uint32_t MicroProfileDrawBarLegend(int32_t nX, int32_t nY,
                                   uint32_t nTotalHeight, uint32_t nMaxWidth) {
  MicroProfileDrawLineVertical(nX - 5, nY, nTotalHeight,
                               UI.nOpacityBackground |
                                   g_nMicroProfileBackColors[0] |
                                   g_nMicroProfileBackColors[1]);
  MicroProfileLoopActiveGroupsDraw(nMaxWidth, nY, 0,
                                   MicroProfileDrawBarLegendCallback, 0);
  return nX;
}
...
bool MicroProfileDrawGraph(uint32_t nScreenWidth, uint32_t nScreenHeight) {
  ...
  MicroProfileDrawBox(nX, nY, nX + MICROPROFILE_GRAPH_WIDTH,
                      nY + MICROPROFILE_GRAPH_HEIGHT,
                      0x88000000 | g_nMicroProfileBackColors[0]);
  ...
  if (bMouseOver) {
    ...
    MicroProfileDrawLineVertical((int)fXAvg, nY, nY + MICROPROFILE_GRAPH_HEIGHT,
                                 (uint32_t)-1);
  }
  ...
  float *pGraphData =
      (float *)alloca(sizeof(float) * MICROPROFILE_GRAPH_HISTORY * 2);
  for (uint32_t i = 0; i < MICROPROFILE_MAX_GRAPHS; ++i) {
    if (S.Graph[i].nToken != MICROPROFILE_INVALID_TOKEN) {
      uint32_t nGroupId = MicroProfileGetGroupIndex(S.Graph[i].nToken);
      bool bGpu = S.GroupInfo[nGroupId].Type == MicroProfileTokenTypeGpu;
      float fToMs = MicroProfileTickToMsMultiplier(
          bGpu ? MicroProfileTicksPerSecondGpu()
               : MicroProfileTicksPerSecondCpu());
      float fToPrc = fToMs * S.fRcpReferenceTime * 3 / 4;

      float fX = (float)nX;
      for (uint32_t j = 0; j < MICROPROFILE_GRAPH_HISTORY; ++j) {
        float fWeigth = MicroProfileMin(
            fToPrc *
                (S.Graph[i].nHistory[(j + nPut) % MICROPROFILE_GRAPH_HISTORY]),
            1.f);
        pGraphData[(j * 2)] = fX;
        pGraphData[(j * 2) + 1] = fY - fDY * fWeigth;
        fX += fDX;
      }
      MicroProfileDrawLine2D(
          MICROPROFILE_GRAPH_HISTORY, pGraphData,
          UI.nOpacityForeground |
              S.TimerInfo[MicroProfileGetTimerIndex(S.Graph[i].nToken)].nColor);
    }
  }
  {
    ...
    MicroProfileDrawLineHorizontal(nX, nX + MICROPROFILE_GRAPH_WIDTH, (int)fY1,
                                   0xffdd4444);
    MicroProfileDrawLineHorizontal(nX, nX + MICROPROFILE_GRAPH_WIDTH, (int)fY2,
                                   0xff000000 | g_nMicroProfileBackColors[0]);
    MicroProfileDrawLineHorizontal(nX, nX + MICROPROFILE_GRAPH_WIDTH, (int)fY3,
                                   0xff000000 | g_nMicroProfileBackColors[0]);

    char buf[32];
    int nLen = snprintf(buf, sizeof(buf) - 1, "%5.2fms", S.fReferenceTime);
    MicroProfileDrawText(nX + 1, (int)(fY1 - (2 + MICROPROFILE_TEXT_HEIGHT)),
                         (uint32_t)-1, buf, nLen);
  }
  ...
  if (bMouseOver) {
    ...
    uint32_t nGraphIndex =
        (S.nGraphPut + MICROPROFILE_GRAPH_HISTORY -
         int(MICROPROFILE_GRAPH_HISTORY * (1.f - fMouseXPrc))) %
        MICROPROFILE_GRAPH_HISTORY;
    ...
    for (uint32_t i = 0; i < MICROPROFILE_MAX_GRAPHS; ++i) {
      if (S.Graph[i].nToken != MICROPROFILE_INVALID_TOKEN) {
        ...
        MicroProfileStringArrayFormat(
            &Strings, "%5.2fms", fToMs * (S.Graph[i].nHistory[nGraphIndex]));
      }
    }
    if (nTextCount) {
      MicroProfileDrawFloatWindow(nX, nY, Strings.ppStrings,
                                  Strings.nNumStrings, 0, pColors);
    }
...
void MicroProfileDumpTimers() {
  ...
  MicroProfileCalcTimers(pTimers, pAverage, pMax, pMin, pCallAverage,
                         pTimersExclusive, pAverageExclusive, pMaxExclusive,
                         nActiveGroup, nNumTimers);
...
uint32_t MicroProfileDrawCounterRecursive(uint32_t nIndex, uint32_t nY,
                                          uint32_t nOffset,
                                          uint32_t nTimerWidth) {
  MicroProfile &S = *MicroProfileGet();
  const uint32_t nHeight = MICROPROFILE_TEXT_HEIGHT;
  ...
  bool bInside = (UI.nActiveMenu == (uint32_t)-1) &&
                 ((UI.nMouseY >= nY0) && (UI.nMouseY < (nY0 + nHeight + 1)));
  if (bInside && (UI.nMouseLeft || UI.nMouseRight)) {
    CI.nClosed = !CI.nClosed;
  }
  uint32_t nTotalWidth = nTimerWidth + nCounterWidth +
                         MICROPROFILE_COUNTER_WIDTH + nLimitWidth +
                         3 * (MICROPROFILE_TEXT_WIDTH + 1);
  uint32_t nBackColor = 0xff000000 | (g_nMicroProfileBackColors[nOffset & 1] +
                                      ((bInside) ? 0x002c2c2c : 0));
  MicroProfileDrawBox(0, nY0, nTotalWidth, nY0 + (nHeight + 1) + 1, nBackColor);
  uint32_t nIndent =
      MICROPROFILE_COUNTER_INDENT * CI.nLevel * (MICROPROFILE_TEXT_WIDTH + 1);
  if (CI.nFirstChild != -1 && CI.nClosed) {
    MicroProfileDrawText(nIndent, nY0, 0xffffffff, "*", 1);
  }

  MicroProfileDrawText(nIndent + MICROPROFILE_TEXT_WIDTH + 1, nY0, 0xffffffff,
                       CI.pName, CI.nNameLen);
  char buffer[64];
  int64_t nCounterValue = S.Counters[nIndex].load();
  uint32_t nX = nTimerWidth + nCounterWidth;
  int nLen = MicroProfileFormatCounter(S.CounterInfo[nIndex].eFormat,
                                       nCounterValue, buffer, sizeof(buffer));
  ...
  if (nLimit) {
    ...
    int nLen = MicroProfileFormatCounter(S.CounterInfo[nIndex].eFormat, nLimit,
    ...
    MicroProfileDrawBox(nX, nY0,
                        nX + (int)(fBoxPrc * MICROPROFILE_COUNTER_WIDTH),
                        nY0 + nHeight, 0xffffffff, MicroProfileBoxTypeFlat);
    MicroProfileDrawBox(nX + 1, nY0 + 1, nX + MICROPROFILE_COUNTER_WIDTH - 1,
                        nY0 + nHeight - 1, nBackColor, MicroProfileBoxTypeFlat);
    MicroProfileDrawBox(
        nX + 1, nY0 + 1,
        nX + (int)(fCounterPrc * (MICROPROFILE_COUNTER_WIDTH - 1)),
        nY0 + nHeight - 1, 0xff0088ff, MicroProfileBoxTypeFlat);
  }
  ...
  if (!CI.nClosed) {
    ...
    while (nChild != -1) {
      nOffset =
          MicroProfileDrawCounterRecursive(nChild, nY, nOffset, nTimerWidth);
...
void MicroProfileDrawCounterView(uint32_t nScreenWidth,
                                 uint32_t nScreenHeight) {
  MicroProfile &S = *MicroProfileGet();
  ...
  for (uint32_t i = 0; i < S.nNumCounters; ++i) {
    uint32_t nWidth = (2 + S.CounterInfo[i].nNameLen +
                       MICROPROFILE_COUNTER_INDENT * S.CounterInfo[i].nLevel) *
                      (MICROPROFILE_TEXT_WIDTH + 1);
  ...
  MicroProfileDrawHeader(
      nX, UI.nCounterWidth + 1 * (MICROPROFILE_TEXT_WIDTH * 3), "Value");
  nX += UI.nCounterWidth;
  nX += 1 * (MICROPROFILE_TEXT_WIDTH * 3);
  MicroProfileDrawHeader(nX, UI.nLimitWidth + MICROPROFILE_COUNTER_WIDTH,
                         "Limit");
  uint32_t nTotalWidth = nTimerWidth + UI.nCounterWidth +
                         MICROPROFILE_COUNTER_WIDTH + UI.nLimitWidth +
                         3 * (MICROPROFILE_TEXT_WIDTH + 1);

  MicroProfileDrawLineVertical(nTimerWidth - 2, 0, nOffset * (nHeight + 1) + nY,
                               UI.nOpacityBackground |
                                   g_nMicroProfileBackColors[0] |
                                   g_nMicroProfileBackColors[1]);
  MicroProfileDrawLineHorizontal(
      0, nTotalWidth, 2 * MICROPROFILE_TEXT_HEIGHT + 3,
      UI.nOpacityBackground | g_nMicroProfileBackColors[0] |
          g_nMicroProfileBackColors[1]);
...
void MicroProfileDrawBarView(uint32_t nScreenWidth, uint32_t nScreenHeight) {
  ...
  for (uint32_t j = 0; j < MICROPROFILE_MAX_GROUPS; ++j) {
    if (nActiveGroup & (1ll << j)) {
      ...
      nNumGroups += 1;
      nMaxTimerNameLen =
          MicroProfileMax(nMaxTimerNameLen, S.GroupInfo[j].nMaxTimerNameLen);
    }
  }
  uint32_t nTimerWidth =
      2 + (4 + nMaxTimerNameLen) * (MICROPROFILE_TEXT_WIDTH + 1);
  ...
  uint32_t nWidth = 0;
  {
      ...
      if (S.nBars & i) {
        if (i >= MP_DRAW_META_FIRST) {
          if (nMetaIndex < MICROPROFILE_META_MAX &&
              S.MetaCounters[nMetaIndex].pName) {
            uint32_t nStrWidth =
                (uint32_t)strlen(S.MetaCounters[nMetaIndex].pName);
          ...
          nWidth +=
              MICROPROFILE_BAR_WIDTH + 6 + 6 * (1 + MICROPROFILE_TEXT_WIDTH);
    ...
    for (uint32_t i = 0; i < nNumTimers + nNumGroups + 1; ++i) {
      ...
      bool bInside =
          (UI.nActiveMenu == (uint32_t)-1) &&
          ((UI.nMouseY >= nY0) && (UI.nMouseY < (nY0 + nHeight + 1)));
      MicroProfileDrawBox(nX, nY0, nWidth + nX, nY0 + (nHeight + 1) + 1,
                          UI.nOpacityBackground |
                              (g_nMicroProfileBackColors[nColorIndex++ & 1] +
                               ((bInside) ? 0x002c2c2c : 0)));
    }
    nX += 10;
  }
  ...
  if (S.nBars & MP_DRAW_AVERAGE)
    nX +=
        MicroProfileDrawBarArray(nX, nY, pAverage, "Average", nTotalHeight) + 1;
  if (S.nBars & MP_DRAW_MAX)
    nX += MicroProfileDrawBarArray(
              nX, nY, pMax, (!UI.bShowSpikes) ? "Max Time" : "Max Time, Spike",
              nTotalHeight, UI.bShowSpikes ? pAverage : NULL) +
          1;
  if (S.nBars & MP_DRAW_MIN)
    nX += MicroProfileDrawBarArray(
              nX, nY, pMin, (!UI.bShowSpikes) ? "Min Time" : "Min Time, Spike",
              nTotalHeight, UI.bShowSpikes ? pAverage : NULL) +
          1;
  ...
  if (S.nBars & MP_DRAW_TIMERS_EXCLUSIVE)
    nX += MicroProfileDrawBarArray(nX, nY, pTimersExclusive, "Exclusive Time",
                                   nTotalHeight) +
          1;
  if (S.nBars & MP_DRAW_AVERAGE_EXCLUSIVE)
    nX += MicroProfileDrawBarArray(nX, nY, pAverageExclusive,
                                   "Exclusive Average", nTotalHeight) +
          1;
  if (S.nBars & MP_DRAW_MAX_EXCLUSIVE)
    nX += MicroProfileDrawBarArray(
              nX, nY, pMaxExclusive,
              (!UI.bShowSpikes) ? "Exclusive Max Time" : "Excl Max Time, Spike",
              nTotalHeight, UI.bShowSpikes ? pAverageExclusive : NULL) +
          1;
  ...
  for (int i = 0; i < MICROPROFILE_META_MAX; ++i) {
    if (0 != (S.nBars & (MP_DRAW_META_FIRST << i)) && S.MetaCounters[i].pName) {
      ...
      if (S.nBars & MP_DRAW_TIMERS)
        nX += MicroProfileDrawBarMetaCount(
                  nX, nY, &S.MetaCounters[i].nCounters[0],
                  S.MetaCounters[i].pName, nTotalHeight) +
              1;
      if (S.nBars & MP_DRAW_AVERAGE) {
        snprintf(buffer, nBufferSize - 1, "%s Avg", S.MetaCounters[i].pName);
        nX += MicroProfileDrawBarMetaAverage(nX, nY,
                                             &S.MetaCounters[i].nAggregate[0],
                                             buffer, nTotalHeight) +
              1;
      }
      if (S.nBars & MP_DRAW_MAX) {
        snprintf(buffer, nBufferSize - 1, "%s Max", S.MetaCounters[i].pName);
        nX += MicroProfileDrawBarMetaCount(nX, nY,
                                           &S.MetaCounters[i].nAggregateMax[0],
                                           buffer, nTotalHeight) +
              1;
      }
    }
  ...
  for (uint32_t i = 0; i < nNumTimers + nNumGroups + 1; ++i) {
    ...
    bool bInside = (UI.nActiveMenu == (uint32_t)-1) &&
                   ((UI.nMouseY >= nY0) && (UI.nMouseY < (nY0 + nHeight + 1)));
    MicroProfileDrawBox(nX, nY0, nTimerWidth, nY0 + (nHeight + 1) + 1,
                        0xff000000 |
                            (g_nMicroProfileBackColors[nColorIndex++ & 1] +
                             ((bInside) ? 0x002c2c2c : 0)));
  }
  ...
  for (uint32_t j = 0; j < MICROPROFILE_MAX_GROUPS; ++j) {
    if (nActiveGroup & (1ll << j)) {
      MicroProfileDrawText(nX, nY + (1 + nHeight) * nLegendOffset, (uint32_t)-1,
                           S.GroupInfo[j].pName, S.GroupInfo[j].nNameLen);
      nLegendOffset += S.GroupInfo[j].nNumTimers + 1;
    }
  }
  MicroProfileDrawHeader(nX, nTimerWidth - 5, "Group");
  MicroProfileDrawTextRight(nTimerWidth - 3, MICROPROFILE_TEXT_HEIGHT + 2,
                            (uint32_t)-1, "Timer", 5);
  MicroProfileDrawLineVertical(nTimerWidth, 0, nTotalHeight + nY,
                               UI.nOpacityBackground |
                                   g_nMicroProfileBackColors[0] |
                                   g_nMicroProfileBackColors[1]);
  MicroProfileDrawLineHorizontal(0, nWidth, 2 * MICROPROFILE_TEXT_HEIGHT + 3,
                                 UI.nOpacityBackground |
                                     g_nMicroProfileBackColors[0] |
                                     g_nMicroProfileBackColors[1]);
}
...
const char *MicroProfileUIMenuAggregate(int nIndex, bool *bSelected) {
  MicroProfile &S = *MicroProfileGet();
  int nNumPresets = (int)sizeof(g_MicroProfileAggregatePresets) /
                    (int)sizeof(g_MicroProfileAggregatePresets[0]);
...
const char *MicroProfileUIMenuTimers(int nIndex, bool *bSelected) {
  ...
  *bSelected = 0 != (S.nBars & (1 << nIndex));
  ...
  case 4:
    return "Call Count";
  case 5:
    return "Exclusive Timers";
  case 6:
    return "Exclusive Average";
  case 7:
...
const char *MicroProfileUIMenuOptions(int nIndex, bool *bSelected) {
  MicroProfile &S = *MicroProfileGet();
  if (nIndex >= MICROPROFILE_OPTION_SIZE)
    return 0;
  switch (UI.Options[nIndex].nSubType) {
  case 0:
    *bSelected = S.fReferenceTime ==
                 g_MicroProfileReferenceTimePresets[UI.Options[nIndex].nIndex];
    break;
  case 1:
    *bSelected = UI.nOpacityBackground >> 24 ==
                 g_MicroProfileOpacityPresets[UI.Options[nIndex].nIndex];
    break;
  case 2:
    *bSelected = UI.nOpacityForeground >> 24 ==
                 g_MicroProfileOpacityPresets[UI.Options[nIndex].nIndex];
    break;
  case 3:
...
const char *MicroProfileUIMenuPreset(int nIndex, bool *bSelected) {
  ...
  *bSelected = false;
  int nNumPresets =
      sizeof(g_MicroProfilePresetNames) / sizeof(g_MicroProfilePresetNames[0]);
  int nIndexSave = nIndex - nNumPresets - 1;
  if (nIndex == nNumPresets)
    return "--";
  else if (nIndexSave >= 0 && nIndexSave < nNumPresets) {
    snprintf(buf, sizeof(buf) - 1, "Save '%s'",
             g_MicroProfilePresetNames[nIndexSave]);
    return buf;
  } else if (nIndex < nNumPresets) {
    snprintf(buf, sizeof(buf) - 1, "Load '%s'",
             g_MicroProfilePresetNames[nIndex]);
    return buf;
  } else {
    return 0;
  }
...
void MicroProfileUIClickOptions(int nIndex) {
  ...
  case 1:
    UI.nOpacityBackground =
        g_MicroProfileOpacityPresets[UI.Options[nIndex].nIndex] << 24;
    break;
  case 2:
    UI.nOpacityForeground =
        g_MicroProfileOpacityPresets[UI.Options[nIndex].nIndex] << 24;
...
void MicroProfileUIClickDump(int nIndex) {
  ...
  char Name[128] = {};
  strftime(Name, sizeof(Name), "microprofile-%Y%m%d-%H%M%S.html",
           localtime(&t));
...
#define SBUF_SIZE 256
  char buffer[256];
  MicroProfileDrawBox(nX, nY, nX + nWidth,
                      nY + (MICROPROFILE_TEXT_HEIGHT + 1) + 1,
                      0xff000000 | g_nMicroProfileBackColors[1]);
...
#define MICROPROFILE_MENU_MAX 16
  ...
  snprintf(AggregateText, sizeof(AggregateText) - 1, "Aggregate[%d]",
           S.nAggregateFlip ? S.nAggregateFlip : S.nAggregateFlipCount);
  ...
  if (UI.GroupMenuCount != S.nGroupCount + S.nCategoryCount) {
    ...
    std::sort(&UI.GroupMenu[0], &UI.GroupMenu[UI.GroupMenuCount],
              [](const MicroProfileGroupMenuItem &l,
                 const MicroProfileGroupMenuItem &r) -> bool {
  ...
  MicroProfileSubmenuCallback GroupCallback[MICROPROFILE_MENU_MAX] = {
      MicroProfileUIMenuMode,      MicroProfileUIMenuGroups,
      MicroProfileUIMenuAggregate, MicroProfileUIMenuTimers,
      MicroProfileUIMenuOptions,   MicroProfileUIMenuPreset,
      MicroProfileUIMenuCustom,    MicroProfileUIMenuDump,
  };

  MicroProfileClickCallback CBClick[MICROPROFILE_MENU_MAX] = {
      MicroProfileUIClickMode,      MicroProfileUIClickGroups,
      MicroProfileUIClickAggregate, MicroProfileUIClickTimers,
      MicroProfileUIClickOptions,   MicroProfileUIClickPreset,
      MicroProfileUIClickCustom,    MicroProfileUIClickDump,
  };
  ...
  for (uint32_t i = 0; i < nNumMenuItems; ++i) {
    ...
    if (UI.nMouseY <= MICROPROFILE_TEXT_HEIGHT && UI.nMouseX <= nEnd &&
        UI.nMouseX >= nX) {
      MicroProfileDrawBox(nX - 1, nY, nX + nLen * (MICROPROFILE_TEXT_WIDTH + 1),
                          nY + (MICROPROFILE_TEXT_HEIGHT + 1) + 1, 0xff888888);
      nSelectMenu = i;
  ...
  if ((uint32_t)-1 != nMenu && GroupCallback[nMenu]) {
    ...
    if (UI.nMouseY <= nY + nHeight + 0 && UI.nMouseY >= nY - 0 &&
        UI.nMouseX <= nX + nWidth + 0 && UI.nMouseX >= nX - 0) {
      UI.nActiveMenu = nMenu;
    } else if (nSelectMenu == (uint32_t)-1) {
      UI.nActiveMenu = (uint32_t)-1;
    }
    MicroProfileDrawBox(nX, nY, nX + nWidth, nY + nHeight,
                        0xff000000 | g_nMicroProfileBackColors[1]);
    for (int i = 0; i < nNumLines; ++i) {
      ...
      if (UI.nMouseY >= nY && UI.nMouseY < nY + MICROPROFILE_TEXT_HEIGHT + 1) {
        ...
        MicroProfileDrawBox(nX, nY, nX + nWidth,
                            nY + MICROPROFILE_TEXT_HEIGHT + 1, 0xff888888);
      }
      int nLen = snprintf(buffer, SBUF_SIZE - 1, "%c %s", bSelected ? '*' : ' ',
                          pString);
      MicroProfileDrawText(nX, nY, (uint32_t)-1, buffer, nLen);
      nY += MICROPROFILE_TEXT_HEIGHT + 1;
    }
...

  {
    ...
    int nLen =
        snprintf(FrameTimeMessage, sizeof(FrameTimeMessage) - 1,
                 "Time[%6.2f] Avg[%6.2f] Max[%6.2f]", fMs, fAverageMs, fMaxMs);
    pMenuText[nNumMenuItems++] = &FrameTimeMessage[0];
    MicroProfileDrawText(nWidth - nLen * (MICROPROFILE_TEXT_WIDTH + 1), 0, -1,
                         FrameTimeMessage, nLen);
  }
}
...
void MicroProfileMoveGraph() {
  ...
  if (nZoom) {
    ...
    if (nZoom > 0) {
      UI.fDetailedRangeTarget = UI.fDetailedRange *=
          UI.nModDown ? 1.40f : 1.05f;
    } else {
      float fNewDetailedRange =
          UI.fDetailedRange / (UI.nModDown ? 1.40f : 1.05f);
  ...
  if (nPanX) {
    UI.fDetailedOffsetTarget = UI.fDetailedOffset +=
        -nPanX * UI.fDetailedRange / UI.nWidth;
  }
...
void MicroProfileDrawCustom(uint32_t nWidth, uint32_t nHeight) {
  if ((uint32_t)-1 != UI.nCustomActive) {
    ...
    uint32_t nAggregateFrames = S.nAggregateFrames ? S.nAggregateFrames : 1;
    uint32_t nExtraOffset =
        1 + ((pCustom->nFlags & MICROPROFILE_CUSTOM_STACK) != 0 ? 3 : 0);
    uint32_t nOffsetYBase =
        nHeight - (nExtraOffset + nCount) * (1 + MICROPROFILE_TEXT_HEIGHT) -
        MICROPROFILE_CUSTOM_PADDING;
    uint32_t nOffsetY = nOffsetYBase;
    float fReference = pCustom->fReference;
    float fRcpReference = 1.f / fReference;
    uint32_t nReducedWidth =
        UI.nWidth - 2 * MICROPROFILE_CUSTOM_PADDING - MICROPROFILE_GRAPH_WIDTH;
    ...
    MicroProfileDrawBox(MICROPROFILE_CUSTOM_PADDING - 1, nOffsetY - 1,
                        MICROPROFILE_CUSTOM_PADDING + nReducedWidth + 1,
                        UI.nHeight - MICROPROFILE_CUSTOM_PADDING + 1,
                        0x88000000 | g_nMicroProfileBackColors[0]);
    ...
    for (uint32_t i = 0; i < nCount; ++i) {
      ...
      float fToMs = MicroProfileTickToMsMultiplier(
          S.GroupInfo[nGroupIndex].Type == MicroProfileTokenTypeGpu
              ? MicroProfileTicksPerSecondGpu()
              : MicroProfileTicksPerSecondCpu());
      pTime[i] = S.Frame[nTimerIndex].nTicks * fToMs;
      pTimeAvg[i] =
          fToMs * (S.Aggregate[nTimerIndex].nTicks / nAggregateFrames);
      pTimeMax[i] = fToMs * (S.AggregateMax[nTimerIndex]);
      pColors[i] = S.TimerInfo[nTimerIndex].nColor;
    }

    MicroProfileDrawText(MICROPROFILE_CUSTOM_PADDING +
                             3 * MICROPROFILE_TEXT_WIDTH,
                         nOffsetY, (uint32_t)-1, "Avg", sizeof("Avg") - 1);
    MicroProfileDrawText(MICROPROFILE_CUSTOM_PADDING +
                             13 * MICROPROFILE_TEXT_WIDTH,
                         nOffsetY, (uint32_t)-1, "Max", sizeof("Max") - 1);
    for (uint32_t i = 0; i < nCount; ++i) {
      ...
      MicroProfileDrawText(nOffsetX, nOffsetY, (uint32_t)-1, Buffer, nSize);
      nOffsetX += (nSize + 2) * (MICROPROFILE_TEXT_WIDTH + 1);
      nSize = snprintf(Buffer, sizeof(Buffer) - 1, "%s:%s",
                       S.GroupInfo[nGroupIndex].pName, pTimerInfo->pName);
      MicroProfileDrawText(nOffsetX, nOffsetY, pTimerInfo->nColor, Buffer,
                           nSize);
      nOffsetX += (nSize + 2) * (MICROPROFILE_TEXT_WIDTH + 1);
    ...
    if (pCustom->nFlags & MICROPROFILE_CUSTOM_BARS) {
      ...
      float *pMs = pCustom->nFlags & MICROPROFILE_CUSTOM_BAR_SOURCE_MAX
                       ? pTimeMax
                       : pTimeAvg;
      const char *pString =
          pCustom->nFlags & MICROPROFILE_CUSTOM_BAR_SOURCE_MAX ? "Max" : "Avg";
      MicroProfileDrawText(nMaxOffsetX, nOffsetY, (uint32_t)-1, pString,
                           (uint32_t)strlen(pString));
      int nSize = snprintf(Buffer, sizeof(Buffer) - 1, "%6.2fms", fReference);
      MicroProfileDrawText(nReducedWidth -
                               (1 + nSize) * (MICROPROFILE_TEXT_WIDTH + 1),
                           nOffsetY, (uint32_t)-1, Buffer, nSize);
      for (uint32_t i = 0; i < nCount; ++i) {
        nOffsetY += (1 + MICROPROFILE_TEXT_HEIGHT);
        uint32_t nWidth = MicroProfileMin(
            nMaxWidth, (uint32_t)(nMaxWidth * pMs[i] * fRcpReference));
        MicroProfileDrawBox(nMaxOffsetX, nOffsetY, nMaxOffsetX + nWidth,
                            nOffsetY + MICROPROFILE_TEXT_HEIGHT,
                            pColors[i] | 0xff000000);
      }
    }
    if (pCustom->nFlags & MICROPROFILE_CUSTOM_STACK) {
      ...
      const char *pString =
          pCustom->nFlags & MICROPROFILE_CUSTOM_STACK_SOURCE_MAX ? "Max"
                                                                 : "Avg";
      MicroProfileDrawText(MICROPROFILE_CUSTOM_PADDING, nOffsetY, (uint32_t)-1,
                           pString, (uint32_t)strlen(pString));
      int nSize = snprintf(Buffer, sizeof(Buffer) - 1, "%6.2fms", fReference);
      MicroProfileDrawText(nReducedWidth -
                               (1 + nSize) * (MICROPROFILE_TEXT_WIDTH + 1),
                           nOffsetY, (uint32_t)-1, Buffer, nSize);
      ...
      float *pMs = pCustom->nFlags & MICROPROFILE_CUSTOM_STACK_SOURCE_MAX
                       ? pTimeMax
                       : pTimeAvg;
      for (uint32_t i = 0; i < nCount; ++i) {
        ...
        if (nX < nXEnd) {
          MicroProfileDrawBox(nX, nOffsetY, nXEnd,
                              nOffsetY + MICROPROFILE_TEXT_HEIGHT,
                              pColors[i] | 0xff000000);
        }
      }
...
void MicroProfileDraw(uint32_t nWidth, uint32_t nHeight) {
  ...
  if (S.nDisplay) {
    ...
    if (S.nDisplay == MP_DRAW_DETAILED || S.nDisplay == MP_DRAW_FRAME) {
      MicroProfileDrawDetailedView(
          nWidth, nHeight, /* bDrawBars= */ S.nDisplay == MP_DRAW_DETAILED);
    ...
    if (!bHidden) {
      ...
      for (int i = 0; i < MICROPROFILE_TOOLTIP_MAX_LOCKED; ++i) {
        int nIndex = (g_MicroProfileUI.LockedToolTipFront + i) %
                     MICROPROFILE_TOOLTIP_MAX_LOCKED;
        if (g_MicroProfileUI.LockedToolTips[nIndex].ppStrings[0]) {
          ...
          MicroProfileFloatWindowSize(
              g_MicroProfileUI.LockedToolTips[nIndex].ppStrings,
              g_MicroProfileUI.LockedToolTips[nIndex].nNumStrings, 0,
              nToolTipWidth, nToolTipHeight, 0);
          uint32_t nStartY = nHeight - nToolTipHeight - 2;
          if (!bDeleted && UI.nMouseY > nStartY &&
              UI.nMouseX > nLockedToolTipX &&
              UI.nMouseX <= nLockedToolTipX + nToolTipWidth &&
              (UI.nMouseLeft || UI.nMouseRight)) {
            bDeleted = true;
            int j = i;
            for (; j < MICROPROFILE_TOOLTIP_MAX_LOCKED - 1; ++j) {
              int nIndex0 = (g_MicroProfileUI.LockedToolTipFront + j) %
                            MICROPROFILE_TOOLTIP_MAX_LOCKED;
              int nIndex1 = (g_MicroProfileUI.LockedToolTipFront + j + 1) %
                            MICROPROFILE_TOOLTIP_MAX_LOCKED;
              MicroProfileStringArrayCopy(
                  &g_MicroProfileUI.LockedToolTips[nIndex0],
                  &g_MicroProfileUI.LockedToolTips[nIndex1]);
            }
            MicroProfileStringArrayClear(
                &g_MicroProfileUI
                     .LockedToolTips[(g_MicroProfileUI.LockedToolTipFront + j) %
                                     MICROPROFILE_TOOLTIP_MAX_LOCKED]);
          } else {
            MicroProfileDrawFloatWindow(
                nLockedToolTipX, nHeight - nToolTipHeight - 2,
                &g_MicroProfileUI.LockedToolTips[nIndex].ppStrings[0],
                g_MicroProfileUI.LockedToolTips[nIndex].nNumStrings,
                g_MicroProfileUI.nLockedToolTipColor[nIndex]);
      ...
      if (UI.nActiveMenu == 9) {
        if (S.nDisplay & MP_DRAW_DETAILED) {
          ...
          MicroProfileStringArrayFormat(&DetailedHelp, "%s",
                                        MICROPROFILE_HELP_LEFT);
          MicroProfileStringArrayAddLiteral(&DetailedHelp, "Toggle Graph");
          MicroProfileStringArrayFormat(&DetailedHelp, "%s",
                                        MICROPROFILE_HELP_RIGHT);
          MicroProfileStringArrayAddLiteral(&DetailedHelp, "Zoom");
          MicroProfileStringArrayFormat(&DetailedHelp, "%s + %s",
                                        MICROPROFILE_HELP_MOD,
                                        MICROPROFILE_HELP_LEFT);
          ...
          MicroProfileStringArrayAddLiteral(&DetailedHelp, "Zoom");
          MicroProfileDrawFloatWindow(
              nWidth, MICROPROFILE_FRAME_HISTORY_HEIGHT + 20,
              DetailedHelp.ppStrings, DetailedHelp.nNumStrings, 0xff777777);

          MicroProfileStringArray DetailedHistoryHelp;
          MicroProfileStringArrayClear(&DetailedHistoryHelp);
          MicroProfileStringArrayFormat(&DetailedHistoryHelp, "%s",
                                        MICROPROFILE_HELP_LEFT);
          ...
          MicroProfileStringArrayAddLiteral(&DetailedHistoryHelp,
                                            "Zoom to frame");
          MicroProfileDrawFloatWindow(nWidth, 20, DetailedHistoryHelp.ppStrings,
                                      DetailedHistoryHelp.nNumStrings,
                                      0xff777777);
          ...
          MicroProfileStringArrayFormat(&BarHelp, "%s + %s",
                                        MICROPROFILE_HELP_MOD,
                                        MICROPROFILE_HELP_LEFT);
          ...
          MicroProfileDrawFloatWindow(
              nWidth, MICROPROFILE_FRAME_HISTORY_HEIGHT + 20, BarHelp.ppStrings,
              BarHelp.nNumStrings, 0xff777777);
        }
        MicroProfileStringArray Debug;
        MicroProfileStringArrayClear(&Debug);
        MicroProfileStringArrayAddLiteral(&Debug, "Memory Usage");
        MicroProfileStringArrayFormat(&Debug, "%4.2fmb",
                                      S.nMemUsage / (1024.f * 1024.f));
#if MICROPROFILE_WEBSERVER
        MicroProfileStringArrayAddLiteral(&Debug, "Web Server Port");
        MicroProfileStringArrayFormat(&Debug, "%d",
                                      MicroProfileWebServerPort());
#endif
        uint32_t nFrameNext =
            (S.nFrameCurrent + 1) % MICROPROFILE_MAX_FRAME_HISTORY;
        MicroProfileFrameState *pFrameCurrent = &S.Frames[S.nFrameCurrent];
        MicroProfileFrameState *pFrameNext = &S.Frames[nFrameNext];
        ...
        MicroProfileStringArrayFormat(
            &Debug, "%9d [%7d]", S.nContextSwitchUsage,
            MICROPROFILE_CONTEXT_SWITCH_BUFFER_SIZE / S.nContextSwitchUsage);
#endif
        ...
        for (int i = 0; i < MICROPROFILE_MAX_THREADS; ++i) {
          if (pFrameCurrent->nLogStart[i] && S.Pool[i]) {
            ...
            uint32_t nUsage = nStart <= nEnd
                                  ? (nEnd - nStart)
                                  : (nEnd + MICROPROFILE_BUFFER_SIZE - nStart);
            uint32_t nFrameSupport = (nUsage == 0)
                                         ? MICROPROFILE_BUFFER_SIZE
                                         : MICROPROFILE_BUFFER_SIZE / nUsage;
            MicroProfileStringArrayFormat(&Debug, "%s",
                                          &S.Pool[i]->ThreadName[0]);
            MicroProfileStringArrayFormat(&Debug, "%9d [%7d]", nUsage,
                                          nFrameSupport);
          }
        }

        MicroProfileDrawFloatWindow(0, nHeight - 10, Debug.ppStrings,
                                    Debug.nNumStrings, 0xff777777);
      }

      if (UI.nActiveMenu == (uint32_t)-1 && !bMouseOverGraph) {
        if (UI.nHoverToken != MICROPROFILE_INVALID_TOKEN) {
          MicroProfileDrawFloatTooltip(UI.nMouseX, UI.nMouseY,
                                       (uint32_t)UI.nHoverToken, UI.nHoverTime);
        } else if (S.nContextSwitchHoverThreadAfter != (uint32_t)-1 &&
                   S.nContextSwitchHoverThreadBefore != (uint32_t)-1) {
          float fToMs =
              MicroProfileTickToMsMultiplier(MicroProfileTicksPerSecondCpu());
          MicroProfileStringArray ToolTip;
          MicroProfileStringArrayClear(&ToolTip);
          MicroProfileStringArrayAddLiteral(&ToolTip, "Context Switch");
          MicroProfileStringArrayFormat(&ToolTip, "%04x",
                                        S.nContextSwitchHoverThread);
          MicroProfileStringArrayAddLiteral(&ToolTip, "Before");
          MicroProfileStringArrayFormat(&ToolTip, "%04x",
                                        S.nContextSwitchHoverThreadBefore);
          MicroProfileStringArrayAddLiteral(&ToolTip, "After");
          MicroProfileStringArrayFormat(&ToolTip, "%04x",
                                        S.nContextSwitchHoverThreadAfter);
          MicroProfileStringArrayAddLiteral(&ToolTip, "Duration");
          int64_t nDifference = MicroProfileLogTickDifference(
              S.nContextSwitchHoverTickIn, S.nContextSwitchHoverTickOut);
          MicroProfileStringArrayFormat(&ToolTip, "%6.2fms",
                                        fToMs * nDifference);
          MicroProfileStringArrayAddLiteral(&ToolTip, "CPU");
          MicroProfileStringArrayFormat(&ToolTip, "%d",
                                        S.nContextSwitchHoverCpu);
          MicroProfileDrawFloatWindow(UI.nMouseX, UI.nMouseY + 20,
                                      &ToolTip.ppStrings[0],
                                      ToolTip.nNumStrings, -1);
        } else if (UI.nHoverFrame != -1) {
          uint32_t nNextFrame =
              (UI.nHoverFrame + 1) % MICROPROFILE_MAX_FRAME_HISTORY;
...
#if MICROPROFILE_DEBUG
          MicroProfileStringArrayFormat(&ToolTip, "%p",
                                        &S.Frames[UI.nHoverFrame]);
#else
          MicroProfileStringArrayAddLiteral(&ToolTip, "");
#endif
    ...
    {
      float fCursor[8] = {
          float(MicroProfileMax(0, (int)UI.nMouseX - 3)),
          float(UI.nMouseY),
          float(MicroProfileMin(nWidth, UI.nMouseX + 3)),
          float(UI.nMouseY),
          float(UI.nMouseX),
          float(MicroProfileMax((int)UI.nMouseY - 3, 0)),
          float(UI.nMouseX),
          float(MicroProfileMin(nHeight, UI.nMouseY + 3)),
      };
  ...
  UI.fDetailedOffset =
      UI.fDetailedOffset + (UI.fDetailedOffsetTarget - UI.fDetailedOffset) *
                               MICROPROFILE_ANIM_DELAY_PRC;
  UI.fDetailedRange =
      UI.fDetailedRange + (UI.fDetailedRangeTarget - UI.fDetailedRange) *
                              MICROPROFILE_ANIM_DELAY_PRC;
}
...
void MicroProfileModKey(uint32_t nKeyState) { UI.nModDown = nKeyState ? 1 : 0; }
...
void MicroProfileMouseButton(uint32_t nLeft, uint32_t nRight) {
  bool bCanRelease = abs((int)(UI.nMouseDownX - UI.nMouseX)) +
                         abs((int)(UI.nMouseDownY - UI.nMouseY)) <
                     3;
...
void MicroProfileDrawLineVertical(int nX, int nTop, int nBottom,
                                  uint32_t nColor) {
  MicroProfileDrawBox(nX, nTop, nX + 1, nBottom, nColor);
}

void MicroProfileDrawLineHorizontal(int nLeft, int nRight, int nY,
                                    uint32_t nColor) {
  ...
  for (uint32_t i = 0; i < MICROPROFILE_MAX_GRAPHS; ++i) {
    ...
    if (Header.nGraphName[i] && Header.nGraphGroupName[i]) {
      ...
      for (uint32_t j = 0; j < S.nTotalTimers; ++j) {
        ...
        if (0 == MP_STRCASECMP(pGraphName, S.TimerInfo[j].pName) &&
            0 == MP_STRCASECMP(pGraphGroupName,
                               S.GroupInfo[nGroupIndex].pName)) {
          MicroProfileToken nToken =
              MicroProfileMakeToken(1ll << nGroupIndex, (uint16_t)j);
          S.Graph[i].nToken =
              nToken; // note: group index is stored here but is checked without
                      // in MicroProfileToggleGraph()!
...
void MicroProfileCustomGroup(const char *pCustomName, uint32_t nMaxTimers,
                             uint32_t nAggregateFlip, float fReferenceTime,
                             uint32_t nFlags) {
  ...
  MP_ASSERT(
      UI.nCustomTimerCount <=
      MICROPROFILE_CUSTOM_MAX_TIMERS); // bump MICROPROFILE_CUSTOM_MAX_TIMERS
...
void MicroProfileCustomGroupAddTimer(const char *pCustomName,
                                     const char *pGroup, const char *pTimer) {
  uint32_t nIndex = MicroProfileCustomGroupFind(pCustomName);
  ...
  MP_ASSERT(nToken !=
            MICROPROFILE_INVALID_TOKEN); // Timer must be registered first.
