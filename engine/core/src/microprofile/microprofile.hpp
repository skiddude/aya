...
#define MICROPROFILE_LABEL(group, name) \
    do \
    { \
    } while (0)
#define MICROPROFILE_LABELF(group, name, ...) \
    do \
    { \
    } while (0)
...
#define MicroProfileDumpFile(path, type, frames) \
    do \
    { \
    } while (0)
...
#define MICROPROFILE_SCOPEI(group, name, color) \
    static MicroProfileToken MICROPROFILE_TOKEN_PASTE(g_mp, __LINE__) = MicroProfileGetToken(group, name, color, MicroProfileTokenTypeCpu); \
...
#define MICROPROFILE_SCOPEGPUI(name, color) \
    static MicroProfileToken MICROPROFILE_TOKEN_PASTE(g_mp, __LINE__) = MicroProfileGetToken("GPU", name, color, MicroProfileTokenTypeGpu); \
    MicroProfileScopeHandler MICROPROFILE_TOKEN_PASTE(foo, __LINE__)(MICROPROFILE_TOKEN_PASTE(g_mp, __LINE__))
#define MICROPROFILE_META_CPU(name, count) \
    static MicroProfileToken MICROPROFILE_TOKEN_PASTE(g_mp_meta, __LINE__) = MicroProfileGetMetaToken(name); \
    MicroProfileMetaUpdate(MICROPROFILE_TOKEN_PASTE(g_mp_meta, __LINE__), count, MicroProfileTokenTypeCpu)
#define MICROPROFILE_META_GPU(name, count) \
    static MicroProfileToken MICROPROFILE_TOKEN_PASTE(g_mp_meta, __LINE__) = MicroProfileGetMetaToken(name); \
    MicroProfileMetaUpdate(MICROPROFILE_TOKEN_PASTE(g_mp_meta, __LINE__), count, MicroProfileTokenTypeGpu)
#define MICROPROFILE_LABEL(group, name) \
    static MicroProfileToken MICROPROFILE_TOKEN_PASTE(g_mp, __LINE__) = MicroProfileGetLabelToken(group); \
    MicroProfileLabel(MICROPROFILE_TOKEN_PASTE(g_mp, __LINE__), name)
#define MICROPROFILE_LABELF(group, name, ...) \
    static MicroProfileToken MICROPROFILE_TOKEN_PASTE(g_mp, __LINE__) = MicroProfileGetLabelToken(group); \
    MicroProfileLabelFormat(MICROPROFILE_TOKEN_PASTE(g_mp, __LINE__), name, ##__VA_ARGS__)
#define MICROPROFILE_COUNTER_ADD(name, count) \
    static MicroProfileToken MICROPROFILE_TOKEN_PASTE(g_mp_counter, __LINE__) = MicroProfileGetCounterToken(name); \
    MicroProfileCounterAdd(MICROPROFILE_TOKEN_PASTE(g_mp_counter, __LINE__), count)
#define MICROPROFILE_COUNTER_SET(name, count) \
    static MicroProfileToken MICROPROFILE_TOKEN_PASTE(g_mp_counter, __LINE__) = MicroProfileGetCounterToken(name); \
    MicroProfileCounterSet(MICROPROFILE_TOKEN_PASTE(g_mp_counter, __LINE__), count)
#define MICROPROFILE_COUNTER_SET_LIMIT(name, count) \
    static MicroProfileToken MICROPROFILE_TOKEN_PASTE(g_mp_counter, __LINE__) = MicroProfileGetCounterToken(name); \
...
MICROPROFILE_API MicroProfileToken MicroProfileGetToken(
    const char* sGroup, const char* sName, uint32_t nColor, MicroProfileTokenType Token = MicroProfileTokenTypeCpu);
...
MICROPROFILE_API void MicroProfileContextSwitchSearch(
...
void MicroProfileCalcAllTimers(float* pTimers, float* pAverage, float* pMax, float* pMin, float* pCallAverage, float* pExclusive,
    float* pAverageExclusive, float* pMaxExclusive, float* pTotal, uint32_t nSize)
{
    for (uint32_t i = 0; i < S.nTotalTimers && i < nSize; ++i)
    {
        ...
        const float fToMs = MicroProfileTickToMsMultiplier(
            S.GroupInfo[nGroupId].Type == MicroProfileTokenTypeGpu ? MicroProfileTicksPerSecondGpu() : MicroProfileTicksPerSecondCpu());
        uint32_t nTimer = i;
        uint32_t nIdx = i * 2;
...
float MicroProfileGetTime(const char* pGroup, const char* pName)
{
    ...
    float fToMs = MicroProfileTickToMsMultiplier(
        S.GroupInfo[nGroupIndex].Type == MicroProfileTokenTypeGpu ? MicroProfileTicksPerSecondGpu() : MicroProfileTicksPerSecondCpu());
...
void MicroProfileContextSwitchSearch(uint32_t* pContextSwitchStart, uint32_t* pContextSwitchEnd, uint64_t nBaseTicksCpu, uint64_t nBaseTicksEndCpu)
{
    ...
    nContextSwitchStart = nContextSwitchEnd =
        (nContextSwitchPut + MICROPROFILE_CONTEXT_SWITCH_BUFFER_SIZE - 1) % MICROPROFILE_CONTEXT_SWITCH_BUFFER_SIZE;
...
void MicroProfileDumpHtml(MicroProfileWriteCallback CB, void* Handle, int nMaxFrames, const char* pHost)
{
    ...
    for (uint32_t i = 0; i < S.nGroupCount; ++i)
    {
        ...
        MicroProfilePrintf(CB, Handle, "GroupInfo[%d] = MakeGroup(%d, \"%s\", %d, %d, %d, %f, %f, %f, '#%06x');\n", S.GroupInfo[i].nGroupIndex,
            ...
            S.GroupInfo[i].Type == MicroProfileTokenTypeGpu ? 1 : 0, fToMs * S.AggregateGroup[i], fToMs * S.AggregateGroup[i] / nAggregateFrames,
            fToMs * S.AggregateGroupMax[i],
            ((MICROPROFILE_UNPACK_RED(nColor) & 0xff) << 16) | ((MICROPROFILE_UNPACK_GREEN(nColor) & 0xff) << 8) |
    ...
    for (uint32_t i = 0; i < S.nTotalTimers; ++i)
    {
        ...
        MicroProfilePrintf(CB, Handle, "TimerInfo[%d] = MakeTimer(%d, \"%s\", %d, '#%06x','#%06x', %f, %f, %f, %f, %f, %d, %f,\n",
            ...
            ((MICROPROFILE_UNPACK_RED(nColor) & 0xff) << 16) | ((MICROPROFILE_UNPACK_GREEN(nColor) & 0xff) << 8) |
                (MICROPROFILE_UNPACK_BLUE(nColor) & 0xff),
            ((MICROPROFILE_UNPACK_RED(nColorDark) & 0xff) << 16) | ((MICROPROFILE_UNPACK_GREEN(nColorDark) & 0xff) << 8) |
                (MICROPROFILE_UNPACK_BLUE(nColorDark) & 0xff),
            pAverage[nIdx], pMax[nIdx], pAverageExclusive[nIdx], pMaxExclusive[nIdx], pCallAverage[nIdx], S.Aggregate[i].nCount, pTotal[nIdx]);
    ...
    for (uint32_t i = 0; i < S.nNumCounters; ++i)
    {
        ...
        MicroProfilePrintf(CB, Handle, "MakeCounter(%d, %d, %d, %d, %d, '%s', %lld, '%s', %lld, '%s', %f, %f),\n", i, S.CounterInfo[i].nParent,
            S.CounterInfo[i].nSibling, S.CounterInfo[i].nFirstChild, S.CounterInfo[i].nLevel, S.CounterInfo[i].pName, (long long)nCounter, Formatted,
            (long long)nLimit, FormattedLimit, fCounterPrc, fBoxPrc);
    }
    MicroProfilePrintString(CB, Handle, "];\n\n");
    ...
    for (uint32_t i = 0; i < nNumFrames; ++i)
    {
        ...
        for (uint32_t j = 0; j < S.nNumLogs; ++j)
        {
            ...
            {
                uint32_t nLogType = MicroProfileLogType(pLog->Log[k]);
                uint64_t nTick = (nLogType == MP_LOG_ENTER || nLogType == MP_LOG_LEAVE) ? MicroProfileLogTickDifference(nStartTick, pLog->Log[k])
                                 : (nLogType == MP_LOG_GPU_EXTRA)                       ? MicroProfileLogTickDifference(nTickStart, pLog->Log[k])
                                                                                        : 0;
...
void MicroProfileGpuFlip()
{
    ...
    for (uint32_t nIndex = S.GPU.nQueryGet[nNextFrame]; nIndex != S.GPU.nQueryGet[nNextNextFrame];
         nIndex = (nIndex + 1) % MICROPROFILE_GPU_MAX_QUERIES)
    {
        uint64_t nResult = 0;

        HRESULT hr;
        do
            hr = S.GPU.pDeviceContext->GetData(S.GPU.pQueries[nIndex], &nResult, sizeof(nResult), 0);
        while (hr == S_FALSE);
...
uint32_t MicroProfileGpuInsertTimer()
{
    ...
    if (nNextIndex == S.GPU.nQueryGet[S.GPU.nNextFrame])
        return (uint32_t)-1;
...
bool MicroProfileGetGpuTickReference(int64_t* pOutCPU, int64_t* pOutGpu)
{
    ...
    HRESULT hr;
    do
        hr = S.GPU.pDeviceContext->GetData(S.GPU.pSyncQuery, &nResult, sizeof(nResult), 0);
    while (hr == S_FALSE);
