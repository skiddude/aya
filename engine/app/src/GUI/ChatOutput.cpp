...
using Aya::Network::Players;

const char* PlayerChatLine::ROBLOXNAME = "(Aya)";
...
PlayerChatLine::PlayerChatLine(
    ChatType chatType, boost::shared_ptr<Network::Player> player, const std::string& message, float startTime, bool isLocalPlayer)
...
GameChatLine::GameChatLine(
    boost::shared_ptr<Instance> origin, const std::string& message, float startTime, bool isLocalPlayer, BubbleColor bubbleColor)
    : ChatLine(Instance::fastDynamicCast<ModelInstance>(origin.get()) ? ChatLine::PLAYER_GAME_CHAT : ChatLine::BOT_CHAT, message, startTime,
          bubbleColor, isLocalPlayer)
{
    this->origin = origin;
}
...
static shared_ptr<Scale9Frame> createChatBubbleMain(const std::string& filePrefix)
{
    ...
    chatBubbleMain->setSlicePrefix("ayaasset://textures/" + filePrefix + ".png");
...
static shared_ptr<Scale9Frame> createChatBubbleWithTail(const std::string& filePrefix, const UDim2& position, const UDim2& size)
{
    ...
    chatBubbleTail->setImage("ayaasset://textures/ui/dialog_tail.png");
...
static shared_ptr<GuiObject> createChatImposter(const std::string& filePrefix, const std::string& dotDotDot, float yOffset)
{
    ...
    result->setImage("ayaasset://textures/" + filePrefix + ".png");
    ...
    image->setImage("ayaasset://textures/" + dotDotDot + ".png");
...
bool ChatOutput::removeExpired()
{
    ...
    int maxBubblesPerPlayer = std::min(Aya::GameSettings::singleton().bubbleChatMaxBubbles, MaxChatBubblesPerPlayer);
...
void ChatOutput::onGameChatMessage(boost::shared_ptr<Instance> origin, const std::string& message, ChatService::ChatColor color)
{
    ...
    switch (color)
    {
    case ChatService::CHAT_BLUE:
        bubbleColor = ChatLine::BLUE;
        break;
    case ChatService::CHAT_GREEN:
        bubbleColor = ChatLine::GREEN;
        break;
    case ChatService::CHAT_RED:
        bubbleColor = ChatLine::RED;
        break;
    }
...
void ChatOutput::onPlayerChatMessage(const Network::ChatMessage& event)
{
    ...
    while (fifo.size() > (size_t)Aya::GameSettings::singleton().chatScrollLength)
    {
        removeOldest();
    }
...
void ChatOutput::renderBubbleImposters(Adorn* adorn, weak_ptr<const Instance> weakOwner, weak_ptr<PartInstance> weakHead)
{
    ...
    ViewportBillboarder viewportBillboarder(
        Vector3(0, 0, 0), Vector3(0, 0, 0), Vector2(0, 0), UDim2(3, 0, 3.6, 0), NULL /*work in actual pixel space*/);
    viewportBillboarder.update(
        adorn->getViewport(), *workspace->getConstCamera(), head.get()->getPartSizeXml(), head.get()->calcRenderingCoordinateFrame());
    ...
    Vector3 extentsOffset, Vector3 studsOffset)
{
    ...
    const UDim2 size(UDim(22, 25), UDim(3, 0)); /// we need some size, but we are not constrained by this size.
                                                ...
                                                /// note also: bubble is drawn in actual pixel space. but size of bubble is set using some stud size
                                                /// contribution (will get smaller as we get farther). We will maintain character width so that
                                                /// maximum width always corresponds to same number of chars. this is to ensure that there is no
                                                /// re-layouting of text as we move away we will align bottom of text to top of viewport. this means
                                                /// that the vertical dimention is only usefull for adjusting the size of the bubble arrow.
    ...
    for (size_t i = playerfifo.size() - 1; i != ~0; --i)
    {
        ...
        // textrect[chatCount] = Rect2D::xywh((float)(viewsize.center().x - textBounds.x /2), (float)(lineCursor - textBounds.y), textBounds.x,
        // textBounds.y).border((float)-textMargin);
    ...
    for (size_t i = 0; i < playerfifo.size(); ++i)
    {
        ...
        if (chatCount == 1)
        {
            ...
            {
                if (GuiObject* tail =
                        Instance::fastDynamicCast<GuiObject>(scalingChatBubbleWithTail[bubbleColor]->findFirstChildByName("ChatBubbleTailFrame")))
                {
        ...
        adornView.drawFont2D(playerfifo[i]->message, center,
            (float)textSize,                           // size
            false, tColor, Color4(Color3::black(), 0), // no outline
            font, xAlin, Text::YALIGN_CENTER, availableTextSpace, Rect2D::xyxy(-1, -1, -1, -1));
...
void ChatOutput::render2d_bubbleStyle(Adorn* adorn, bool playerBubbleChat)
{
    ...
    for (CharacterChatMap::iterator it = characterSortedMsg.begin(); it != characterSortedMsg.end(); ++it)
    {
        ...
        {
            Aya::Frustum frustum(workspace->getCamera()->frustum());
            it->second.isVisible = frustum.containsPoint(headCFrame.translation);
            it->second.isMoving = !head->getVelocity().linear.isZero();
            if (shared_ptr<BillboardGui> billboardGui = it->second.billboardGui.lock())
            {
                billboardGui->setRenderFunction(boost::bind(
                    &ChatOutput::renderBubbles, this, _2, weak_from(it->first), weak_from(head), playerBubbleChat, extentsOffset, studsOffset));
            }
        }
    }
}
