...
const Color4& GuiItem::disabledFill()
{
    static Color4 c(.7f, .7f, .7f, .5f);
    return c;
}
const Color4& GuiItem::translucentBackdrop()
{
    static Color4 c(.6f, .6f, .6f, .6f);
    return c;
}
const Color4& GuiItem::translucentDebugBackdrop()
{
    static Color4 c(.0f, .0f, .0f, .35f);
    return c;
}
const Color4& GuiItem::menuSelect()
{
    static Color4 c(.7f, .7f, .7f, 1.0f);
    return c;
}
...
GuiResponse GuiItem::process(const shared_ptr<InputObject>& event)
{
    ...
    AYAASSERT(focus == NULL);
    return processNonFocus(event);
}


void GuiItem::label2d(Adorn* adorn, const std::string& label, const Color4& fill, const Color4& border, Text::XAlign align) const
{
    if (label.length())
    {
        ...
        adorn->drawFont2D(label, pos,
            (float)getFontSize(), // size
            false, fill, border, Text::FONT_LEGACY, align, Text::YALIGN_CENTER);
    }
}
...
TopMenuBar::TopMenuBar(const std::string& _title, Layout::Style _layoutStyle, bool translucentBackdrop)
{
    init();
    setName(_title);
...
TopMenuBar::TopMenuBar(const std::string& _title, Layout::Style _layoutStyle, Color4 _backdropColor)
{
    init();
    setName(_title);
    layoutStyle = _layoutStyle;
...
GuiResponse TopMenuBar::process(const shared_ptr<InputObject>& event)
{
    if (isVisible())
    {
        ...
        else if ((backdropColor.a > 0.0) && event->isMouseEvent() && getMyRect(Canvas(event->getWindowSize())).pointInRect(event->get2DPosition()))
        {
...
void UnifiedWidget::render2dMe(Adorn* adorn)
{
    ...
    label2d(adorn, getName(), Color3::black(), Color4::clear(), Text::XALIGN_CENTER);
}
...
GuiResponse UnifiedWidget::processShown_InTitle(const shared_ptr<InputObject>& event)
{
    ...
    AYAASSERT(menuState == SHOWN_APPEARING);
...
GuiResponse UnifiedWidget::process(const shared_ptr<InputObject>& event)
{
    if (isVisible())
    {
        if (event->isMouseEvent())
        {
            switch (menuState)
            {
            case UnifiedWidget::NOTHING:
                return processNothing(event);
            case UnifiedWidget::HOVER:
                return processHover(event);
            case UnifiedWidget::SHOWN_APPEARING:
            case UnifiedWidget::SHOWN:
                return processShown(event); // with or without focus....
            }
        }
        else
        {
...
void TextDisplay::render2d(Adorn* adorn)
{
    if (isVisible())
    {
        label2d(adorn, getLabel(), fontColor, borderColor, align);
    }
}
