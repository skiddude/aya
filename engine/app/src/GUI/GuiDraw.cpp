...
void GuiDrawImage::tryCreateTextureProxy(
    Adorn* adorn, const std::string& contentString, const std::string& context, Aya::TextureProxyBaseRef& textureRef, bool& isWaiting)
{
    ...
    if (((content.isAsset() || content.isItem()) && !Aya::ContentProvider::findAsset(content).empty()) || !(content.isAsset() || content.isItem()))
    {
        textureRef = adorn->createTextureProxy(content, isWaiting, false, context);
    }
}

void GuiDrawImage::setImageFromRef(Aya::TextureProxyBaseRef ref)
{
    normal = ref;
}

bool GuiDrawImage::setImage(
    Adorn* adorn, const TextureId& textureId, unsigned imageState, Vector2* outSize, Instance* contextInstance, const char* context)
{
    if (textureId != currentTexture)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (normal && (hover || !waitingHover) && (down || !waitingDown) && (disable || !waitingDisable) && (selected || !waitingSelected) &&
                    (selectedHover || !waitingSelectedHover) && (selectedDown || !waitingSelectedDown))
                {
                    currentTexture = textureId;
                }
            }
...
Vector2 GuiDrawImage::getImageSize() const
{
    ...
    {
        Aya::Vector2 sz = normal->getOriginalSize();
        size.x = sz.x;
        size.y = sz.y;
    }
    return size;
...
void GuiDrawImage::draw(Adorn* adorn, const Aya::TextureProxyBaseRef& texture, const Rect& rect, const Vector2& texul, const Vector2& texbr,
    const Color4& color, const Rect& clipRect, const Color4& behind, const Color4& inFront)
{
    ...
    if (clipRect2D.intersects(rect2D))
    {
        ...
        { // backup - use the normal texture
            Aya::Rect2D intersectRect = clipRect2D.intersect(rect2D);
...
void GuiDrawImage::render2dImpl(Adorn* adorn, bool enabled, const Rect& rect, const Vector2& texul, const Vector2& texbr, const Color4& color,
    const Modifier& modifier, Gui::WidgetState state, bool isSelected)
{
    ...
    else
    {
        ...
        else if (state == Gui::DOWN_AWAY)
        { // hover/depressed+away: draw yellow
...
void GuiDrawImage::render2d(Adorn* adorn, bool enabled, const Rect& rect, Gui::WidgetState state, bool isSelected)
{
    render2dImpl(adorn, enabled, rect, Vector2(0, 0), Vector2(1, 1), Color3::white(), Rotation2D(), state, isSelected);
}

void GuiDrawImage::render2d(Adorn* adorn, bool enabled, const Rect& rect, const Vector2& texul, const Vector2& texbr, const Color4& color,
...
void GuiDrawImage::render2d(Adorn* adorn, bool enabled, const Rect& rect, const Vector2& texul, const Vector2& texbr, const Color4& color,
    const Rect& clipRect, Gui::WidgetState state, bool isSelected)
{
