...
public:
    ...
    const Instance* getOrigin() const
    {
        return origin.lock().get();
    }
...
private:
    ...
    void renderBubbles(Adorn* adorn, weak_ptr<const Instance> owner, weak_ptr<PartInstance> head, bool playerAndGameChat, Vector3 extentsOffset,
        Vector3 studsOffset);
    typedef GuiItem Super;
    static const int MaxChatBubblesPerPlayer;
    static const int MaxChatLinesPerBubble;

    Aya::Network::Players* players;
    ...
    struct ScalingInfo
    {
        ...
        UDim2 getPosition(Vector2 size)
        {
            ...
            return UDim2(
                size.x < scalingCutoff.x ? scalingPosition.x : fixedPosition.x, size.y < scalingCutoff.y ? scalingPosition.y : fixedPosition.y);
        }
        UDim2 getSize(Vector2 size)
        {
            return UDim2(size.x < scalingCutoff.x ? scalingSize.x : fixedSize.x, size.y < scalingCutoff.y ? scalingSize.y : fixedSize.y);
        }
    ...
    Aya::signals::scoped_connection playerChatMessageConnection;
    Aya::signals::scoped_connection gameChatMessageConnection;
