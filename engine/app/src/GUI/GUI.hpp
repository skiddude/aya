...
private:
    ...
    /*override*/ const Aya::Name& getClassName() const
    {
...
protected:
    ...
    void label2d(Adorn* adorn, const std::string& label, const Color4& fill, const Color4& border, Text::XAlign align = Text::XALIGN_LEFT) const;
    ...
    {
        return false;
    } // in general, Gui Items don't lose focus while processing

    // standard virtual overrides....
    virtual Vector2 getPosition(Canvas canvas) const
    {
        AYAASSERT(getGuiParent());
        return getGuiParent()->getChildPosition(this, canvas);
    }

    virtual Vector2 getChildPosition(const GuiItem* child, Canvas canvas) const
    { // This should always be override if ever used
        AYAASSERT(0);
        return Vector2::zero();
    }
...
public:
    virtual Vector2 getSize(Canvas canvas) const
    {
        return getGuiSize();
    }
    ...
    void addGuiItem(shared_ptr<GuiItem> guiItem)
    {
        guiItem->setParent(this);
    }

    void setGuiSize(const Vector2& size)
    {
        guiSize = size;
    }
    const Vector2& getGuiSize() const
    {
        return guiSize;
    }
...
class GuiRoot : public Reflection::Described<GuiRoot, sGuiRoot, GuiItem, Reflection::ClassDescriptor::INTERNAL_LOCAL, Aya::Security::LocalUser>
{
public:
    GuiRoot();
...
public:
    ...
    TopMenuBar(const std::string& _title, Layout::Style layoutStyle, bool translucentBackdrop = false);
    TopMenuBar(const std::string& _title, Layout::Style _layoutStyle, Color4 _backdropColor);
...
public:
    ...
    RelativePanel(const Layout& layout)
    {
        init(layout);
    }
...
private:
    ...
    /*override*/ Vector2 getChildPosition(const GuiItem* child, Canvas canvas) const;
    /*override*/ bool canLoseFocus()
    {
        return true;
    }
    /*override*/ void onLoseFocus();
    /*override*/ int getFontSize() const
    {
        return 8;
    }
...
protected:
    bool showChildren()
    {
        return (menuState >= SHOWN_APPEARING);
    }
...
protected:
    ...
    /*override*/ float getTextSize() const
    {
        return textSize;
    }
    /*override*/ bool isVisible() const
...
public:
    ...
    void setTextSize(float _fontSize)
    {
        fontSize = _fontSize;
        setFontSize(_fontSize);
        setGuiSize(Vector2(fontSize * 10.0f, fontSize * 2.0f));
    }
    void setFontColor(const Color4& _color)
    {
        fontColor = _color;
    }
    void setBorderColor(const Color4& _color)
    {
        borderColor = _color;
    }
    void setAlign(Text::XAlign _align)
    {
        align = _align;
    }
