    ...
    , kMaxScheduleSize(maxScheduleSize)
{
    ...
    {
        poolLocks[i].reset(new Aya::spin_mutex());
...
BaseThreadPool::~BaseThreadPool()
{
    ...
    case LockAndKill:
    {
        AYAASSERT(pool.size() == poolLocks.size());
        boost::posix_time::milliseconds timeout(50);
        for (unsigned i = 0; i < pool.size(); ++i)
        {
            Aya::spin_mutex::scoped_lock lock(*poolLocks[i]);
...
void BaseThreadPool::loop(boost::shared_ptr<PoolData> poolData, boost::shared_ptr<Aya::spin_mutex> lock, ShutdownPolicy shutdownPolicy)
{
    Aya::set_thread_name("aya_BaseThreadPool");
    while (true)
    {
        {
        ...
        {
            boost::function<void(boost::shared_ptr<Aya::spin_mutex>)> task;
...
bool ThreadPool::schedule(boost::function<void(boost::shared_ptr<Aya::spin_mutex>)> task)
{
...
bool PriorityThreadPool::schedule(boost::function<void(boost::shared_ptr<Aya::spin_mutex>)> task, float priority)
{
    if (!shouldSchedule(poolData))
    {
        return false;
...
bool PriorityThreadPool::PriorityThreadPoolData::getNextTask(boost::function<void(boost::shared_ptr<Aya::spin_mutex>)>& task)
{
