...
class Pages : public DescribedNonCreatable<Pages, Instance, sPages, Reflection::ClassDescriptor::RUNTIME_LOCAL>
{
protected:
    bool finished;
...
class StandardPages : public DescribedNonCreatable<StandardPages, Pages, sStandardPages, Reflection::ClassDescriptor::RUNTIME_LOCAL>
{
...
class FriendPages : public DescribedNonCreatable<FriendPages, Pages, sFriendPages, Reflection::ClassDescriptor::RUNTIME_LOCAL>
{
    weak_ptr<DataModel> weakDM;
    std::string fieldName;
    std::string requestUrl;
...
public:
    LuaWebService();
    void asyncRequest(const std::string& url, float priority, boost::function<void(shared_ptr<const Reflection::ValueArray>)> resumeFunction,
        boost::function<void(std::string)> errorFunction);
    void asyncRequest(const std::string& url, float priority, boost::function<void(shared_ptr<const Reflection::ValueMap>)> resumeFunction,
        boost::function<void(std::string)> errorFunction);
    void asyncRequest(
    ...
    // Skips the caches
    void asyncRequestNoCache(const std::string& url, float priority, boost::function<void(shared_ptr<const Reflection::ValueMap>)> callback,
        AsyncHttpQueue::ResultJob resultJob);

    // will block until api access request has returned
    bool isApiAccessEnabled();
    void setCheckApiAccessBecauseInStudio();

    static bool parseWebJSONResponseHelper(
