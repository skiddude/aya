...
template<>
bool XmlNameValuePair::getValue(Aya::ProtectedString& value) const
{
    std::string tmp;
    if (getValue(tmp))
    {
        value = Aya::ProtectedString::fromTrustedSource(tmp);
        return true;
    }
    return false;
}
...
{
    static TType<Aya::ProtectedString> type("ProtectedString");
    return type;
}
