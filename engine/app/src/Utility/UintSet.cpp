...
UintSet::UpdateBitInfo::UpdateBitInfo(unsigned int intVal)
    : bitGroup(intVal >> kShiftForGroup)
    , bitPosition(intVal & kBitInGroupMask)
    , mask(1 << bitPosition)
{
}
...
{
    UpdateBitInfo info(intVal);
    if (isEmpty || info.bitGroup < minBitGroup || info.bitGroup > maxBitGroup)
    {
...
void UintSet::pop_smallest(unsigned int* out)
{
    ...
    AYAASSERT(flag != 0);
    AYAASSERT((flag & (flag - 1)) == 0); // assert exactly 1 bit is set
