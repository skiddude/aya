...
Rect::Location Rect::pointInBorder(const G3D::Vector2& point, float borderRatio)
{
    G3D::Vector2 delta = high * borderRatio;

    if (point.x < delta.x)
        return LEFT;
    if (point.x > (high.x - delta.x))
        return RIGHT;
    if (point.y < delta.y)
        return TOP;
    if (point.y > (high.y - delta.y))
        return BOTTOM;
...
Vector2 Rect::positionPoint(const Vector2& point, Location xLocation, Location yLocation) const
{
    ...
    {
    case LEFT:
        answer.x = low.x + point.x;
        break;
    case RIGHT:
        answer.x = high.x - point.x;
        break;
    default:
        AYAASSERT(0);
        break;
    ...
    {
    case TOP:
        answer.y = low.y + point.y;
        break;
    case BOTTOM:
        answer.y = high.y - point.y;
        break;
    default:
        AYAASSERT(0);
        break;
...
Rect Rect::positionChild(const Rect& child, Location xLocation, Location yLocation) const
{
    ...
    case LEFT:
        ...
        break;
    case CENTER:
        answer.x = 0.5f * (high.x - childSize.x);
        break;
    case RIGHT:
        answer.x = high.x - childSize.x;
        break;
    default:
        AYAASSERT(0);
        break;
    ...
    case TOP:
        ...
        break;
    case CENTER:
        answer.y = 0.5f * (high.y - childSize.y);
        break;
    case BOTTOM:
        answer.y = high.y - childSize.y;
        break;
    default:
        AYAASSERT(0);
        break;
