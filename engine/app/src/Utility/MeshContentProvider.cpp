...
MeshContentProvider::MeshContentProvider()
    : DescribedNonCreatable<MeshContentProvider, CacheableContentProvider, sMeshContentProvider, Aya::Reflection::ClassDescriptor::RUNTIME_LOCAL>(
          CACHE_ENFORCE_MEMORY_SIZE, 1024 * 1024 * 32)
{
...
TaskScheduler::StepResult MeshContentProvider::ProcessTask(const std::string& id, shared_ptr<const std::string> data)
{
    ...
    {
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "MeshContentProvider failed to process %s because 'could not fetch'", id.c_str());
    }
