...
{
    AYAASSERT(!insertEdge->inSpanningTree());
...
{
    AYAASSERT(removeEdge->inSpanningTree());
...
void SpanningTree::swapTree(SpanningEdge* deactivate, SpanningEdge* activate, SpanningNode* newParent)
{
    if (!activate)
    {
        AYAASSERT(deactivate);
    }

    if (deactivate)
    {
        AYAASSERT_IF_VALIDATING(validateTree(deactivate->getChildSpanningNode()->getRoot<SpanningNode>()));
    }
    ...
    if (activate)
    {
        AYAASSERT_IF_VALIDATING(validateTree(activate->getChildSpanningNode()->getRoot<SpanningNode>()));
    }
}
...
SpanningNode* SpanningTree::testEdgeToParent(int testSide, SpanningNode* child, SpanningEdge*& answer, int& lightSide)
{
    ...
    {
        if (!answer || edge->isLighterThan(answer))
        { // i.e. edge is lighter than current answer
            answer = edge;
            lightSide = testSide;
        }
    }
...
public:
    ...
    FindHeaviest(std::set<SpanningNode*>& _tree, SpanningEdge*& _heaviest, SpanningNode*& _newParent)
        : tree(_tree)
        , heaviest(_heaviest)
    ...
    void operator()(SpanningNode* node, SpanningEdge* edge)
    {
        if (!edge->inSpanningTree())
        {
            ...
            if (!other || (tree.find(other) == tree.end()))
            { // not in the tree
