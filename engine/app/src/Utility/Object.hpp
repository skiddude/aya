...
boost::shared_ptr<T> shared_polymorphic_downcast(const boost::shared_ptr<U>& r)
{
...
template<class T, class U>
boost::shared_ptr<T> shared_dynamic_cast(const boost::shared_ptr<U>& r)
{
    return boost::dynamic_pointer_cast<T>(r);
}

template<class T, class U>
boost::shared_ptr<T> shared_static_cast(const boost::shared_ptr<U>& r)
{
    return boost::static_pointer_cast<T>(r);
}
...
template<class T, class U>
shared_ptr<T> shared_from_polymorphic_downcast(enable_shared_from_this<U>* r)
{
    return r ? shared_polymorphic_downcast<T>(r->shared_from_this()) : shared_ptr<T>();
}

template<class T, class U>
shared_ptr<T> shared_from_static_cast(enable_shared_from_this<U>* r)
{
    return r ? shared_static_cast<T>(r->shared_from_this()) : shared_ptr<T>();
}

template<class T, class U>
shared_ptr<T> shared_from_dynamic_cast(enable_shared_from_this<U>* r)
{
    return r ? shared_dynamic_cast<T>(r->shared_from_this()) : shared_ptr<T>();
}

template<class T>
inline bool weak_equal(const weak_ptr<T>& lhs, const weak_ptr<T>& rhs)
{
    return !(lhs < rhs) && !(rhs < lhs);
}

class AyaInterface ICreator
{
public:
...
class FactoryProduct : public BaseClass
{
    ...
    private:
        ...
        const Aya::Name& getClassNameUnconstructed() const
        {
            return Aya::Name::declare<sClassName>();
        }

    public:
        static bool wasConstructed()
        {
            return isConstructed == isConstructedTrue();
        }

        /* override */ shared_ptr<Reflection::DescribedBase> create() const
        {
            AYAASSERT(wasConstructed());
            return Creatable<FactoryClass>::template create<Class>();
        }

        const Aya::Name& getClassName() const
        {
            AYAASSERT(wasConstructed());
            return Aya::Name::declare<sClassName>();
        }
        ...
        Creator()
        {
            ...
            const Aya::Name& name = getClassNameUnconstructed();

            auto& creators = Creatable<FactoryClass>::getCreators();

            AYAASSERT(creators.find(&name) == creators.end());
            AYAASSERT(!wasConstructed());
...
protected:
    FactoryProduct() {}
    template<class Arg0>
    FactoryProduct(Arg0 arg0)
        : BaseClass(arg0)
    {
    }
    template<class Arg0, class Arg1>
    FactoryProduct(Arg0 arg0, Arg1 arg1)
        : BaseClass(arg0, arg1)
    {
    }
    ...
    {
        AYAASSERT(Creator::wasConstructed());
...
public:
    ...
    static const Aya::Name& className()
    {
        return static_getCreator().getClassName();
    };
    static bool isNullClassName()
    {
        AYAASSERT(!className().empty());
        return false;
    };
    const Aya::Name& getClassName() const
    {
...
template<class Class, class BaseClass, const char* const& sClassName, class FactoryClass>
#if defined(_WIN32) && !defined(__clang__)
typename const FactoryProduct<Class, BaseClass, sClassName, FactoryClass>::Creator
    FactoryProduct<Class, BaseClass, sClassName, FactoryClass>::creatorPrivate;
#else
typename FactoryProduct<Class, BaseClass, sClassName, FactoryClass>::Creator
    FactoryProduct<Class, BaseClass, sClassName, FactoryClass>::creatorPrivate;
#endif
...
public:
    ...
    template<class Arg0>
    NonFactoryProduct(Arg0 arg0)
        : BaseClass(arg0)
    {
    }
    template<class Arg0, class Arg1>
    NonFactoryProduct(Arg0 arg0, Arg1 arg1)
        : BaseClass(arg0, arg1)
    {
    }
    template<class Arg0, class Arg1, class Arg2>
    NonFactoryProduct(Arg0 arg0, Arg1 arg1, Arg2 arg2)
        : BaseClass(arg0, arg1, arg2)
    {
    }
    template<class Arg0, class Arg1, class Arg2, class Arg3>
    NonFactoryProduct(Arg0 arg0, Arg1 arg1, Arg2 arg2, Arg3 arg3)
    ...
    static bool isNullClassName()
    {
        AYAASSERT(className().empty() == (sClassName == NULL));
