...
void DataModel::internalSave(ContentId contentId)
{
    ...
    catch (std::exception&)
    {
        ...
        std::string logFile = "";
        {
            ...
            for (std::map<std::string, int>::iterator iter = dataModelReportingData.begin(); iter != dataModelReportingData.end(); ++iter)
            {
...
{
    shared_ptr<std::stringstream> stream = serializeDataModel(SAVE_ALL);
    ...
    try
    {
        ...
        if (!Aya::Http::isInstanceSite(uploadUrl.c_str()))
