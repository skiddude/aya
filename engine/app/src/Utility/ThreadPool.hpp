...
public:
    ...
    struct PoolData
    {
        ...
        virtual bool getNextTask(boost::function<void(boost::shared_ptr<Aya::spin_mutex>)>& task) = 0;
    };

private:
    int count;
    const size_t kMaxScheduleSize;
    std::vector<boost::shared_ptr<Aya::spin_mutex>> poolLocks;
    std::vector<boost::shared_ptr<boost::thread>> pool;
    ShutdownPolicy shutdownPolicy;

    static void loop(boost::shared_ptr<PoolData> poolData, boost::shared_ptr<Aya::spin_mutex> lock, ShutdownPolicy shutdownPolicy);
...
{
private:
    struct ThreadPoolData : public PoolData
    {
        typedef Aya::safe_queue<boost::function<void(boost::shared_ptr<Aya::spin_mutex>)>> Queue;
        Queue queue;

        /*override*/ bool getNextTask(boost::function<void(boost::shared_ptr<Aya::spin_mutex>)>& task)
        {
...
public:
    ...
    bool schedule(boost::function<void(boost::shared_ptr<Aya::spin_mutex>)> task);
};
...
private:
    struct PriorityTask
    {
        boost::function<void(boost::shared_ptr<Aya::spin_mutex>)> func;
        float priority;
        PriorityTask(boost::function<void(boost::shared_ptr<Aya::spin_mutex>)> func, float priority)
            : func(func)
            , priority(priority)
        {
        }
    ...
    struct PriorityThreadPoolData : public PoolData
    {
        typedef Aya::safe_heap<PriorityTask> Heap;
        Heap heap;
...
public:
    ...
    bool schedule(boost::function<void(boost::shared_ptr<Aya::spin_mutex>)> task, float priority);
};
} // namespace Aya
