...
public:
    ...
    inline void printContentNames()
    {
        ...
        {
            StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "%s", iter->first.c_str());
        }
    }
...
public:
    MemEnforcedLRUCache(const unsigned long maxSize)
        : maxMemSize(maxSize)
    {
    }
