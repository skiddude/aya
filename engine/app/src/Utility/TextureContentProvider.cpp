...
TextureContentProvider::TextureContentProvider()
    : DescribedNonCreatable<TextureContentProvider, CacheableContentProvider, sTextureContentProvider,
          Aya::Reflection::ClassDescriptor::RUNTIME_LOCAL>(CACHE_ENFORCE_MEMORY_SIZE, 1024 * 1024 * 32)
{
    setName(sTextureContentProvider);
}

void TextureContentProvider::setTextureAllocator(boost::function<Aya::Image*(std::istream&, const std::string&)> textureAllocator)
{
    ...
    else
    {
        if (ContentId(id).isHttp())
            Aya::StandardOut::singleton()->printf(
                Aya::MESSAGE_ERROR, "TextureContentProvider failed to process %s because 'could not fetch'", id.c_str());
    }
