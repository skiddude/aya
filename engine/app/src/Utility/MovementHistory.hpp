...
public:
    ...
    void getMovementNodeList(const Time& lastCutOffTime, const Time& currentCutOffTime, std::deque<MovementNode>& result, bool crossPacketCompression,
        const CoordinateFrame& lastSentCFrame, CoordinateFrame& outCalculatedBaselineCFrame, Vector3& outCalculatedLinearVelocity) const;
    const CoordinateFrame& getBaselineCFrame() const
    {
        return baselineCFrame;
    }
