...
#define OPUS_VOICE_SAMPLERATE 48000
#define OPUS_VOICE_BUFSIZE 960
...
private:
    ...
    typedef boost::unordered_set<ChromiumFrame*> ChromiumFrames;

    typedef boost::unordered_set<VoiceChatOutput*> VoiceChatOutputs;

    friend class SoundChannel;
    shared_ptr<FMOD::System> system;
    FMOD::Sound* micInput; // BACK FROM THE GRAVE
    ...
    ChromiumFrames chromiumFrames;
    VoiceChatOutputs voiceChatOutputs;
    FMOD::ChannelGroup* channelMaster;

    OpusDecoder* opusDecoder;
    FMOD::Channel* opusChannel;

    shared_ptr<SoundJob> soundJob;

    Aya::signals::scoped_connection gameSettingsChangedConnection;
    ...
    void updateChromiumFrames();
    void updateVoiceChatOutputs();
...
public:
    ...
    void playOpusData(const unsigned char* data, size_t dataSize, bool is3d, CoordinateFrame cframe = CoordinateFrame());
#endif
    ...
    void setListener(ListenerType listenerType, shared_ptr<const Aya::Reflection::Tuple> value);
    shared_ptr<const Aya::Reflection::Tuple> getListener();
    ...
    void registerChromiumFrame(ChromiumFrame* chromiumFrame);
    void unregisterChromiumFrame(ChromiumFrame* chromiumFrame);

    void registerVoiceChatOutput(VoiceChatOutput* voiceChatOutput);
    void unregisterVoiceChatOutput(VoiceChatOutput* voiceChatOutput);
...
public:
    SoundJob(SoundService* soundService)
        : DataModelJob("Sound", DataModelJob::Write, false, shared_from_dynamic_cast<DataModel>(DataModel::get(soundService)), Time::Interval(0.003))
        , fps(30)
