...
NormalIdMask normalIdToMask(NormalId normal)
{
    ...
    case NORM_Z:
        return NORM_Z_MASK;
    case NORM_X_NEG:
        return NORM_X_NEG_MASK;
    case NORM_Y_NEG:
        return NORM_Y_NEG_MASK;
    case NORM_Z_NEG:
        return NORM_Z_NEG_MASK;
    default:
        break;
    }
...
{
    AYAASSERT(validNormalId(normalId));
    Vector3 answer;
    int xyz = normalId % 3;
...
Matrix3 normalIdToMatrix3Internal(NormalId normalId)
{
    ...
    AYAASSERT(uInObject == uvwToObject(objectToUvw(uInObject, normalId), normalId));
    AYAASSERT(vInObject == uvwToObject(objectToUvw(vInObject, normalId), normalId));
    AYAASSERT(wInObject == uvwToObject(objectToUvw(wInObject, normalId), normalId));

    return Matrix3(uInObject.x, vInObject.x, wInObject.x, uInObject.y, vInObject.y, wInObject.y, uInObject.z, vInObject.z, wInObject.z);
}
...
const Matrix3& normalIdToMatrix3(NormalId normalId)
{
    ...
    case NORM_X:
    {
        static Matrix3 x = normalIdToMatrix3Internal(NORM_X);
        return x;
    }
    case NORM_Y:
    {
        static Matrix3 y = normalIdToMatrix3Internal(NORM_Y);
        return y;
    }
    case NORM_Z:
    {
        static Matrix3 z = normalIdToMatrix3Internal(NORM_Z);
        return z;
    }
    case NORM_X_NEG:
    {
        static Matrix3 xn = normalIdToMatrix3Internal(NORM_X_NEG);
        return xn;
    }
    case NORM_Y_NEG:
    {
        static Matrix3 yn = normalIdToMatrix3Internal(NORM_Y_NEG);
        return yn;
    }
    case NORM_Z_NEG:
    {
        static Matrix3 zn = normalIdToMatrix3Internal(NORM_Z_NEG);
        return zn;
    }
    default:
    {
...
{
    AYAASSERT((v == Vector3::unitX()) || (v == Vector3::unitY()) || (v == Vector3::unitZ()) || (v == -Vector3::unitX()) || (v == -Vector3::unitY()) ||
              (v == -Vector3::unitZ()));
    ...
    {
        AYAASSERT(0); // This function assumes a unit, orthogally aligned Vector3
