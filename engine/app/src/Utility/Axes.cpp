...
std::string StringConverter<Axes>::convertToString(const Axes& value)
{
    ...
    {
        if (value.getAxis((Aya::Vector3::Axis)axis))
        {
            result += seperator;
            switch ((Aya::Vector3::Axis)axis)
            {
            case Aya::Vector3::X_AXIS:
                result += "X";
                break;
            case Aya::Vector3::Y_AXIS:
                result += "Y";
                break;
            case Aya::Vector3::Z_AXIS:
                result += "Z";
                break;
            default:
                break;
...
{
template<>
EnumDesc<Aya::Vector3::Axis>::EnumDesc()
    : EnumDescriptor("Axis")
{
    addPair(Aya::Vector3::X_AXIS, "X");
    addPair(Aya::Vector3::Y_AXIS, "Y");
    addPair(Aya::Vector3::Z_AXIS, "Z");
}

template<>
Aya::Vector3::Axis& Variant::convert<Aya::Vector3::Axis>(void)
{
    return genericConvert<Aya::Vector3::Axis>();
}
} // namespace Reflection

template<>
bool Aya::StringConverter<G3D::Vector3::Axis>::convertToValue(const std::string& text, Aya::Vector3::Axis& value)
{
    if (text.find("Y") != std::string::npos || text.find("Top") != std::string::npos || text.find("Bottom") != std::string::npos)
    {
        value = Aya::Vector3::Y_AXIS;
        return true;
    }
    if (text.find("Z") != std::string::npos || text.find("Back") != std::string::npos || text.find("Front") != std::string::npos)
    {
        value = Aya::Vector3::Z_AXIS;
        return true;
    }
    if (text.find("X") != std::string::npos || text.find("Right") != std::string::npos || text.find("Left") != std::string::npos)
    {
        value = Aya::Vector3::X_AXIS;
        return true;
    }
    return false;
}
template<>
bool StringConverter<Aya::Axes>::convertToValue(const std::string& text, Axes& value)
{
    value.clear();
