...
private:
    ...
    inline const CoordinateFrame& recordAndReturnHermite(
        const CoordinateFrame& value, const FrameInfo& startFrame, const Time& local, const RemoteTime& remote)
    {
        beingMoved = true;
    ...
    const CoordinateFrame& interpolateHermiteSpline(
        const Time& now, const Time& targetTime, const unsigned int& upper, const PartInstance* part = NULL);
...
public:
    ...
    void setValue(PartInstance* part, const CoordinateFrame& value, const Velocity& vel, const RemoteTime& timeStamp, Time now, float localTimeOffest,
        int numNodesAhead);

    void setTargetDelay(float value);

    void setUiStepId(int id)
    {
        uiStepId = id;
    }
    int getUiStepId() const
    {
        return uiStepId;
    }

    double getLocalToRemoteTimeOffset()
    {
        return localToRemoteTimeOffset;
    }
