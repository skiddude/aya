...
IndexedMesh::~IndexedMesh()
{
    AYAASSERT(!lower);
    AYAASSERT(!upper);
    AYAASSERT(!computedUpper);
    AYAASSERT(!getTypedParent<IndexedMesh>());
    ...
    if (newLower != lower)
    {
        if (lower) // pull out of the chain - I have no lower, so am out of commission!
        {
            ...
            while (numChildren() > 0)
            {
                ...
                AYAASSERT(backChild->getTypedParent<IndexedMesh>() == this);
...
void IndexedMesh::onParentChanged(IndexedTree* oldParent)
{
    if (oldParent)
    {
        IndexedMesh* oldMeshParent = aya_static_cast<IndexedMesh*>(oldParent);
...
const IndexedMesh* IndexedMesh::getConstIndexedMeshParent() const
{
    ...
    AYAASSERT(answer == computeParentFromLower());
    return answer;
}
...
{
    AYAASSERT_SLOW(computedUpper == IndexedMesh::computeConstUpper(this));
...
{
    bool answer = (lower && lower->getConstUpper());

    AYAASSERT(answer == (lower && (computeConstUpper(lower)->lower == lower)));
