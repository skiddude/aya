...
class IndexArray : public boost::noncopyable // You can't copy elements from one array to another
...
public:
    ...
    {
        AYAASSERT(item);
        AYAASSERT(indexOf(item) == -1);
        AYAASSERT_IF_VALIDATING(array.find(item) == array.end());
    ...
    inline void fastRemove(Item* item)
    {
        ...
        AYAASSERT(array[removeIndex] == item);
    ...
    inline bool fastContains(Item* item) const
    {
        ...
        AYAASSERT_IF_VALIDATING(answer == underlyingArray().contains(item));
        return answer;
    }
    ...
    inline Item* const operator[](int n) const
    { // the pointer is const?
        AYAASSERT(indexOf(array[n]) == n);
        return array[n];
    }

    inline Item* const operator[](unsigned int n) const
    { // the pointer is const?...
        AYAASSERT(indexOf(array[n]) == n);
        return array[n];
    }
