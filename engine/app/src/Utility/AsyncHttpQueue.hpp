...
public:
    ...
    typedef boost::function<void(RequestResult, std::istream*, shared_ptr<const std::string> response, shared_ptr<std::exception> exception)>
        RequestCallback;
    ...
    void asyncRequest(const std::string& id, float priority, RequestCallback* callback, ResultJob jobType, bool ignoreBadRequests = false,
    ...
    static void dispatchCallback(RequestCallback theCallback, Instance* instance, RequestResult result, boost::shared_ptr<const std::string> data,
        ResultJob jobType, shared_ptr<std::exception> exception);
    ...
    double getAvgTimeInQueue()
    {
        return avgTimeInQueue.value();
    }
    double getAvgRequestCompleteTime()
    {
        return avgRequestCompleteTime.value();
    }
    int getNumSlowRequests()
    {
        return numSlowRequests;
    }
...
protected:
    ...
    struct Request
    {
        ...
        Aya::Time startTime;          // the time when this request was issued
    ...
    static void processRequests(boost::weak_ptr<AsyncHttpQueue> httpQueue, RequestHandle request, boost::shared_ptr<Aya::spin_mutex> lock);
