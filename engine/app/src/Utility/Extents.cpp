...
Extents Extents::clampInsideOf(const Extents& other) const
...
Extents Extents::toWorldSpace(const CoordinateFrame& localCoord) const
{
    ...
    Vector3 newHalfSize = Vector3(
        ...
        fabs(localCoord.rotation[2][0]) * halfSize[0] + fabs(localCoord.rotation[2][1]) * halfSize[1] +
            fabs(localCoord.rotation[2][2]) * halfSize[2]);
...
Vector3 Extents::clamp(const Extents& innerExtents) const
{
    ...
    for (int i = 0; i < 3; i++)
    {
        AYAASSERT(innerExtents.size()[i] <= this->size()[i]);
