...
#else
struct StrHash : std::__unary_function<const char*, std::size_t>
{
    size_t operator()(const char* s) const
    {
        size_t h = boost::hash_range(s, s + strlen(s));
...
struct StrEqualTo : public std::__binary_function<const char*, const char*, bool>
{
    bool operator()(const char* a, const char* b) const
    {
        return strcmp(a, b) == 0;
...
static Aya::mutex& moo2()
{
    static Aya::mutex mutex2;
    return mutex2;
}
static void initMoo()
{
...
void Name::setOrderIndex()
{
    ...
    for (; i > 0; --i)
    {
        AYAASSERT(ordered[i - 1]->str != str);
        if (str > ordered[i - 1]->str)
            break;

        ordered[i - 1]->sortIndex++;
        AYAASSERT(ordered[i - 1]->sortIndex == i);
    }
    this->sortIndex = i;
    ...
    for (size_t j = 0; j < ordered.size(); ++j)
        AYAASSERT(ordered[j]->sortIndex == j);
#endif
}
...
const Name& Name::declare(const char* const& sName)
{
    ...
    AYAASSERT(sName[0] == '\0' || (!result->str.empty() && lookup(sName) != Name::getNullName()));
    return *result;
}
...
std::ostream& Aya::operator<<(std::ostream& os, const Name& name)
{
