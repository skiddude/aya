...
static std::string BuildGenericApiUrl(
    const std::string& baseUrl, const std::string& serviceNameIn, const std::string& path, const std::string& key, const char* scheme = "http")
{
    std::string serviceName(serviceNameIn);
    if ("" != serviceName)
        serviceName += ".";

    std::string rbxUrl = GetBaseURL();

    // Remove "http://" or "https://" from baseUrl if present
    std::string cleanBaseUrl = baseUrl;
    if (cleanBaseUrl.find("http://") == 0)
    {
        cleanBaseUrl = cleanBaseUrl.substr(7);
    }
    else if (cleanBaseUrl.find("https://") == 0)
    {
        cleanBaseUrl = cleanBaseUrl.substr(8);
    }

    size_t pos = cleanBaseUrl.find(rbxUrl);

    std::string subUrl = cleanBaseUrl.substr(0, pos);
    ...
    if (subUrl.empty())
    {
        url = format_string("%s://%s%s/%s/?apiKey=%s", scheme, serviceName.c_str(), path.c_str(), GetBaseURL().c_str(), key.c_str());
    }
    else
    {
        if (subUrl.find("www") != std::string::npos)
        {
            subUrl = subUrl.replace(0, 4, "");
            url = format_string("%s://%s%s%s/%s/?apiKey=%s", scheme, serviceName.c_str(), subUrl.c_str(), rbxUrl.c_str(), path.c_str(), key.c_str());
        }
        ...
        else if (subUrl.find(".sitetest3") != std::string::npos) // Special case for URLs like alberto.sitetest3, navin.sitetest3, etc..
        {
