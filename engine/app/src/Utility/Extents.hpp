...
public:
    ...
    Extents(const Vector3& _min, const Vector3& _max)
        ...
        , high(_max)
    {
        AYAASSERT_SLOW(low == low.min(high));
        AYAASSERT_SLOW(high == high.max(low));
    }
    ...
    */
    ...
    { // 	Extents& operator+= (const Vector3& shiftVector) {
        low += shiftVector;
        high += shiftVector;
    }

    void scale(float x)
    { //	Extents& operator*= (float x) {
        low *= x;
        high *= x;
    }
    ...
    {
        return ((point.x >= low.x) && (point.y >= low.y) && (point.z >= low.z) && (point.x <= high.x) && (point.y <= high.y) && (point.z <= high.z));
    }

    bool fuzzyContains(const Vector3& point, float slop) const
    {
        return ((point.x >= (low.x - slop)) && (point.y >= (low.y - slop)) && (point.z >= (low.z - slop)) && (point.x <= (high.x + slop)) &&
                (point.y <= (high.y + slop)) && (point.z <= (high.z + slop)));
    }


    bool overlapsOrTouches(const Extents& other) const
    { // true if sides are exactly equal (touching)
        return ((this->low.x > other.high.x) || (this->low.y > other.high.y) || (this->low.z > other.high.z) || (this->high.y < other.low.y) ||
                   (this->high.x < other.low.x) || (this->high.z < other.low.z))
                   ? false
                   : true;
    }
    ...
    {
        if ((this->low.x >= other.high.x) || (this->low.y >= other.high.y) || (this->low.z >= other.high.z) || (this->high.y <= other.low.y) ||
            (this->high.x <= other.low.x) || (this->high.z <= other.low.z))
