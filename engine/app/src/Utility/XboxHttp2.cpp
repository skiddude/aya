...
#include "Utility/HttpPlatformImpl.hpp" // TODO this is here for cache. Maybe move cache to separate file?
...
namespace boost
{
void intrusive_ptr_add_ref(const IUnknown* p)
{
    const_cast<IUnknown*>(p)->AddRef();
}
void intrusive_ptr_release(const IUnknown* p)
{
    const_cast<IUnknown*>(p)->Release();
}
} // namespace boost
...
public:
    ...
    Callback(const std::string& url_)
        : url(url_)
    {
        refcnt = 1;
        status = -1;
        error = S_OK;
        evt = CreateEvent(0, TRUE, 0, 0);
        cacheValid = false;

        timeStart = Aya::Time::nowFastSec();
        timeRedirect = 0;
        timeHeaders = -1;
        timeData = -1;
        timeDone = -1;
    ...
    virtual HRESULT __stdcall OnRedirect(IXMLHTTPRequest2* pXHR, const WCHAR* pwszRedirectUrl) override
    {
        ...
        timeRedirect = Aya::Time::nowFastSec() - timeStart;
        if (kForceHttpAssets)
            return S_OK; // don't spam
        ...
        Aya::HttpPlatformImpl::Cache::CacheResult cacheResult = Aya::HttpPlatformImpl::Cache::CacheResult::open(url.c_str(), redirect.c_str());

        if (cacheResult.isValid())
        {
            timeData = Aya::Time::nowFastSec() - timeStart;
            response.clear();
            response = cacheResult.getResponseBody().toString();
            timeDone = timeData = Aya::Time::nowFastSec() - timeStart;
            cacheValid = true;
    ...
    {
        status = dwStatus;
        timeHeaders = Aya::Time::nowFastSec() - timeStart;
        return S_OK;
    }
...
static void addHeader(IXMLHTTPRequest2* request, const std::wstring& header, const std::wstring value, const std::string& theUrl)
{
    ...
    if (FAILED(hr))
        throw Aya::runtime_error("http failed to add header '%S' : '%S' (0x%x) [%s]", header.c_str(), value.c_str(), hr, theUrl.c_str());
}

namespace Aya
{
void Http::httpGetPostXbox(bool isPost, std::istream& data, const std::string& contentType, bool compressData,
    const HttpAux::AdditionalHeaders& additionalHeaders, bool externalRequest, HttpCache::Policy cachePolicy, std::string& response)
{
    ...
    AYAPROFILER_SCOPE("Http", __FUNCTION__);
    AYAPROFILER_LABEL("Http", theUrl.c_str());

    Aya::Timer<Aya::Time::Precise> timer;

    static const std::wstring userAgentHeader = L"User-Agent";
    static const std::wstring userAgentValue = s2ws(Http::rbxUserAgent) + L" Aya Xbox App 1.0.0";
    ...
    if (theUrl.size() == 0)
        throw Aya::runtime_error("http empty url");

    if (!Http::trustCheck(theUrl.c_str(), externalRequest))
        throw Aya::runtime_error("http trust check failed for %s", theUrl.c_str());
    ...
    if (FAILED(hr) || !request)
        throw Aya::runtime_error("http is not available at the moment (0x%x) [%s]", hr, theUrl.c_str());
    ...
    if (FAILED(hr))
        throw Aya::runtime_error("http could not open connection (0x%x) [%s]", hr, theUrl.c_str());
    ...
    if (!isPost)
    {
        ...
        hr = request->Send(0, 0);
        if (FAILED(hr))
            throw Aya::runtime_error("http failed to send (0x%x) [%s]", hr, theUrl.c_str());
    }
    else
    {
        // POST
        ...
        if (FAILED(hr))
            Aya::runtime_error("http send failed (0x%x) [%s]", hr, theUrl.c_str());
    }
    ...
    if (!callback->wait())
    {
        // timeout
        request->Abort();
        throw Aya::runtime_error("http TIMEOUT [%s]", theUrl.c_str());
    }

    hr = callback->getHresult();
    if (FAILED(hr))
        throw Aya::runtime_error("http error hr=0x%x [%s]", hr, theUrl.c_str());
    ...
    if (statusCode == 200 || statusCode == 201 || callback->getCacheValid())
    {
        ...
        if (kReportHttpTimings)
        {
            ...
            dprintf("HTTP %s %d %.0f ms (%.0f ms / %.0f ms / %.0f ms) : %s%s%S\n", (isPost ? "POST" : "GET"), statusCode, 1000 * callback->timeDone,
                1000 * callback->timeRedirect, 1000 * callback->timeHeaders, 1000 * callback->timeData, theUrl.c_str(), (*redir ? " ---->  " : ""),
                redir);
        }
        return; // success!
    }
