...
public:
    HttpQueueStatsItem(AsyncHttpQueue* _httpQueue, Instance* httpQueueOwner)
        : httpQueue(_httpQueue)
    {
...
static void InvokeAsyncCallback(AsyncHttpQueue::RequestCallback func, AsyncHttpQueue::RequestResult result, shared_ptr<const std::string> response,
...
void AsyncHttpQueue::dispatchCallback(AsyncHttpQueue::RequestCallback theCallback, Instance* instance, RequestResult result,
    boost::shared_ptr<const std::string> data, ResultJob jobType, shared_ptr<std::exception> exception)
{
    dispatchGenericCallback(boost::bind(&InvokeAsyncCallback, theCallback, result, data, exception), instance, jobType);
}
static void callback(AsyncHttpQueue::CallbackWrapper f, Instance* owner, AsyncHttpQueue::RequestResult result, shared_ptr<std::string> response,
    shared_ptr<std::exception> exception)
{
    try
    {
    ...
    {
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "ContentProvider callback exception: %s", e.what());
    }
}
...
void AsyncHttpQueue::processRequests(
    boost::weak_ptr<AsyncHttpQueue> weakHttpQueue, RequestHandle request, boost::shared_ptr<Aya::spin_mutex> criticalSection)
{
    ...
    try
    {
        try
        {
            try
            {
                ...
                {
                    Aya::spin_mutex::scoped_lock lock(*criticalSection);
                ...
                else
                {
                    ...
                    httpRequest.reset(new Aya::Http(url));
            ...
            catch (Aya::http_status_error& e)
            {
                // This means the server has accepted  the request, but is taking longer to process, call again after some time and hopefully we will
                // get a 200 OK once server is done processing
                if (e.statusCode == 202)
                {
                    if (boost::shared_ptr<AsyncHttpQueue> httpQueue = weakHttpQueue.lock())
                    {
    ...
    catch (std::logic_error& e)
    {
        ...
        {
            Aya::StandardOut::singleton()->printf(Aya::MESSAGE_SENSITIVE, "Content failed for %s because %s", url.c_str(), e.what());
            Aya::StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "Content failed because %s", e.what());
        }
...
AsyncHttpQueue::FailedUrl::FailedUrl(const char* url)
    : url(url)
#ifdef _DEBUG
    , expiration(Aya::Time::now<Time::Fast>() + Time::Interval(10 * 60))
#else
    , expiration(Aya::Time::now<Time::Fast>() + Time::Interval(5 * 60))
#endif
{
}
...
void AsyncHttpQueue::asyncRequest(
    const std::string& id, float priority, RequestCallback* callback, ResultJob jobType, bool ignoreBadRequests, const std::string& expectedType)
{
    ...
    else
    {
        if (callback)
            dispatchCallback(
                *callback, owner, Failed, shared_ptr<std::string>(), jobType, shared_ptr<std::exception>(new std::runtime_error("Bad request")));
    }
}
