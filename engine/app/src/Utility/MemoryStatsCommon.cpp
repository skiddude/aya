...
{
    AYAASSERT(MemoryStats::totalMemoryBytes() > 0);
...
size_t slowGetMemoryPoolAllocation()
{
    ...
    for (iter = Aya::poolAllocationList.begin(); iter != Aya::poolAllocationList.end(); iter++)
    {
...
size_t slowGetMemoryPoolAvailability()
{
    ...
    std::vector<size_t*>::iterator iter;
    for (iter = Aya::poolAvailabilityList.begin(); iter != Aya::poolAvailabilityList.end(); iter++)
    {
...
void releaseAllPoolMemory()
{
    ...
    for (iter = Aya::poolReleaseMemoryFuncList.begin(); iter != Aya::poolReleaseMemoryFuncList.end(); iter++)
    {
        (*iter)();
    }
}
...
MemoryLevel slowCheckMemoryLevel(memsize_t extraMemoryUsed)
{
    ...
    else if ((totalPoolAvailability + physicalMemoryAvailability >= kLowMemWatermarkBytes + extraMemoryUsed) ||
             (totalPoolAvailability + bytesToMinMemory >= kLowMemWatermarkBytes))
    {
