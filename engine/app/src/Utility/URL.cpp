...
public:
    enum CharClassBit
    {
        ...
        CharClassBit_Pchar = 0x10 | CharClassBit_Unreserved | CharClassBit_PercentEncoded | CharClassBit_SubDelim,
        CharClassBit_RegName = CharClassBit_Unreserved | CharClassBit_PercentEncoded | CharClassBit_SubDelim,
...
const uint8_t RFC3986::charClassTable_[256] = {,
...
Url Url::fromComponents(
    const std::string& scheme, const std::string& host, const std::string& path, const std::string& query, const std::string& fragment)
{
    Url url;
    url.scheme_ = scheme;
    url.host_ = host;
...
{
    std::string result;
    result.reserve(scheme_.length() + 3 + host_.length() + path_.length() + 1 + query_.length() + 1 + fragment_.length() + 1);
...
{
    // split the host into the reg-name part & the port part (if any)
    std::string::size_type colonPos = host_.find(':');

    if (colonPos != std::string::npos)
    {
        std::string regName = host_.substr(0, colonPos);
        std::string port = host_.substr(colonPos + 1);

        // validate
        if (!RFC3986::checkStringIsOfClass(regName, RFC3986::CharClassBit_RegName))
        {
            return false;
        }
    ...
    {
        // no port #
        if (!RFC3986::checkStringIsOfClass(host_, RFC3986::CharClassBit_RegName))
        {
            return false;
        }
    }
