...
using namespace Aya;
using namespace Aya::Soundscape;
...
template<>
Aya::Soundscape::ListenerType& Variant::convert<Aya::Soundscape::ListenerType>(void)
{
    return genericConvert<Aya::Soundscape::ListenerType>();
}
} // namespace Reflection
template<>
bool StringConverter<Aya::Soundscape::ListenerType>::convertToValue(const std::string& text, Aya::Soundscape::ListenerType& value)
{
    return Reflection::EnumDesc<Aya::Soundscape::ListenerType>::singleton().convertToValue(text.c_str(), value);
}
} // namespace Aya
...
Reflection::BoundProp<float> SoundService::prop_dopplerscale(
    "DopplerScale", category_Data, &SoundService::dopplerscale, &SoundService::on3DSettingChanged);
Reflection::BoundProp<float> SoundService::prop_distancefactor(
    "DistanceFactor", category_Data, &SoundService::distancefactor, &SoundService::on3DSettingChanged);
Reflection::BoundProp<float> SoundService::prop_rolloffscale(
    "RolloffScale", category_Data, &SoundService::rolloffscale, &SoundService::on3DSettingChanged);
Reflection::EnumPropDescriptor<SoundService, ReverbType> SoundService::prop_AmbientReverb(
    "AmbientReverb", category_Data, &SoundService::getAmbientReverb, &SoundService::setAmbientReverb);
...
Reflection::BoundFuncDesc<SoundService, void(ListenerType, shared_ptr<const Aya::Reflection::Tuple>)> func_setListener(
    &SoundService::setListener, "SetListener", "listenerType", "listener", Security::None);
Reflection::BoundFuncDesc<SoundService, shared_ptr<const Aya::Reflection::Tuple>()> func_getListener(
    &SoundService::getListener, "GetListener", Security::None);
...
void SoundService::openFmod()
{
    ...
    if (!soundDisabled && Aya::GameSettings::singleton().soundEnabled)
    {
        try
        {
            ...
            if (num_drivers)
            {
                ...
                {
                    checkResultNoThrow(
                        system->getDriverInfo(driver, Aya::AyaDbgInfo::s_instance.AudioDeviceName, DBG_STRING_MAX, NULL, NULL, NULL, NULL),
                        "getDriverInfo", this, system.get());
                }
            }
            else
            {
            ...
            channelMaster->setVolume(Aya::GameBasicSettings::singleton().getMasterVolume());

            // listen for master volume changes
            gameSettingsChangedConnection =
                Aya::GameBasicSettings::singleton().propertyChangedSignal.connect(boost::bind(&SoundService::gameSettingsChanged, this, _1));
...
#ifdef ENABLE_VOICE_CHAT
            this->opusDecoder = opus_decoder_create(OPUS_VOICE_SAMPLERATE, OPUS_VOICE_CHANNELS, NULL);
#endif
...
FMOD::DSP* SoundService::createDSP(FMOD_DSP_DESCRIPTION& dspdesc)
{
    ...
    try
    {
        StandardOut::singleton()->printf(
            MESSAGE_INFO, "SoundService::createDSP: system value = %p, enabled value = %d", system.get(), (int)enabled());
    ...
    catch (const Aya::base_exception& ex)
    {
...
int SoundService::getSampleRate()
{
    ...
    {
        StandardOut::singleton()->printf(
            MESSAGE_INFO, "SoundService::getSampleRate: system value = %p, enabled value = %d", system.get(), (int)enabled());
...
SoundService::~SoundService()
{
    AYAASSERT(stockSounds.size() == 0);
    AYAASSERT(loadedSounds.size() == 0);
    AYAASSERT(loaded3DSounds.size() == 0);
    AYAASSERT(!system);
    AYAASSERT(!soundJob);
}
...
#ifdef ENABLE_VOICE_CHAT
    opus_decoder_destroy(this->opusDecoder);
#endif
...
{
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_OFF;
        reverbs[NoReverb] = p;
    }
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_GENERIC;
        reverbs[GenericReverb] = p;
    }
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_PADDEDCELL;
        reverbs[PaddedCell] = p;
    }
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_PADDEDCELL;
        reverbs[Room] = p;
    ...
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_BATHROOM;
        reverbs[Bathroom] = p;
    }
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_LIVINGROOM;
        reverbs[LivingRoom] = p;
    }
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_STONEROOM;
        reverbs[StoneRoom] = p;
    }
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_AUDITORIUM;
        reverbs[Auditorium] = p;
    }
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_CONCERTHALL;
        reverbs[ConcertHall] = p;
    }
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_CAVE;
        reverbs[Cave] = p;
    }
    {
    ...
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_HANGAR;
        reverbs[Hangar] = p;
    }
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_CARPETTEDHALLWAY;
        reverbs[CarpettedHallway] = p;
    }
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_HALLWAY;
        reverbs[Hallway] = p;
    }
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_STONECORRIDOR;
        reverbs[StoneCorridor] = p;
    }
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_ALLEY;
        reverbs[Alley] = p;
    }
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_FOREST;
        reverbs[Forest] = p;
    }
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_CITY;
        reverbs[City] = p;
    }
    {
    ...
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_QUARRY;
        reverbs[Quarry] = p;
    }
    {
    ...
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_PARKINGLOT;
        reverbs[ParkingLot] = p;
    }
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_SEWERPIPE;
        reverbs[SewerPipe] = p;
    }
    {
        FMOD_REVERB_PROPERTIES p = FMOD_PRESET_UNDERWATER;
        reverbs[UnderWater] = p;
    }
}
...
void SoundService::setListener(ListenerType listenerType, shared_ptr<const Aya::Reflection::Tuple> value)
{
    if (!((value && value->values.size() > 0) || listenerType == CameraListener))
    {
        throw Aya::runtime_error("SoundService:SetListener called with an incorrect argument.");
        return;
    }
    ...
    if (listenerType == CFrame)
    {
        ...
        {
            throw Aya::runtime_error("SoundService:SetListener value given is not a valid CFrame when given Enum.ListenerType.CFrame");
            return;
        }
    }
    else if (listenerType == ObjectPosition || listenerType == ObjectCFrame)
    {
        ...
        if (tupleIter->isType<shared_ptr<Instance>>())
        {
            ...
            {
                throw Aya::runtime_error("SoundService:SetListener value given does not have a location");
                return;
            }
        }
        else
        {
            throw Aya::runtime_error("SoundService:SetListener value given is not a valid instance");
            return;
        }
    }
...
shared_ptr<const Aya::Reflection::Tuple> SoundService::getListener()
{
    shared_ptr<Aya::Reflection::Tuple> result(new Aya::Reflection::Tuple(2));
...
void SoundService::playSound(SoundType soundType)
{
    ...
    if (iter != stockSounds.end())
    {
        ...
        {
            std::stringstream ss;
            ss << reinterpret_cast<void*>(iter->second.get()) << ") with soundId = " << iter->second->getSoundId().toString()
               << ", isLoaded = " << iter->second->isSoundLoaded();
...
#ifdef ENABLE_VOICE_CHAT
void SoundService::playOpusData(const unsigned char* data, size_t dataSize, bool is3d, CoordinateFrame cframe)
{
    if (enabled())
    {
        // Aya::StandardOut::singleton()->printf(MESSAGE_SENSITIVE, "Playing %d bytes of opus data", sizeof(data));

        opus_int16* pcm = new opus_int16[OPUS_VOICE_BUFSIZE];

        int result = opus_decode(opusDecoder, data, dataSize, pcm, OPUS_VOICE_BUFSIZE, 0);
        if (result < 0)
        {
            Aya::StandardOut::singleton()->printf(MESSAGE_WARNING, "Opus decode failed (%d): %s", result, opus_strerror(result));
            return;
        }
        ...
        exinfo.defaultfrequency = OPUS_VOICE_SAMPLERATE; // sample rate
        exinfo.length = OPUS_VOICE_BUFSIZE * sizeof(opus_int16);

        system->createSound((const char*)pcm, FMOD_CREATESAMPLE | FMOD_OPENMEMORY | FMOD_OPENRAW | (is3d ? FMOD_3D : 0), &exinfo, &sound);
        system->playSound(sound, 0, false, &opusChannel);

        if (is3d)
        {
            FMOD_VECTOR pos, vel;
            pos.x = cframe.translation.x;
            pos.y = cframe.translation.y;
            pos.z = cframe.translation.z;
            vel.x = 0.0f;
            vel.y = 0.0f;
            vel.z = 0.0f;
            printf("3d %f %f %f\n", pos.x, pos.y, pos.z);
            opusChannel->set3DAttributes(&pos, &vel);
        }
    }
}
#endif
...
void SoundService::step(const Time::Interval& timeSinceLastStep)
{
                ...
                if (convert(cf.translation, listener_pos))
                {
                    ...
                    if (convert(-cf.lookVector(), listener_forward))
                    {
                        ...
                        if (convert(cf.upVector(), listener_up))
                        {
                            ...
                            Aya::PVInstance* pv = dynamic_cast<PVInstance*>(camera->getCameraSubject());
                            Aya::PartInstance* part = pv ? pv->getPrimaryPart() : NULL;
                            if (part)
    ...
    updateChromiumFrames();
    updateVoiceChatOutputs();
...
void SoundService::updateMasterChannelGroup(const Time::Interval& timeSinceLastStep)
{
    ...
    const float storedMasterVolume = muting ? getMasterVolume() : Aya::GameBasicSettings::singleton().getMasterVolume();
    ...
    {
        volumeStep =
            ((masterChannelFadeTimeMsec / timeSinceLastStep.msec() * storedMasterVolume) / masterChannelFadeTimeMsec) * (muting ? -1.0f : 1.0f);
    }
...
void SoundService::updateChromiumFrames()
{
    for (ChromiumFrames::iterator it = chromiumFrames.begin(); chromiumFrames.end() != it; ++it)
    {
        ChromiumFrame* chromiumFrame = *it;
        chromiumFrame->updateWorldAudio();
    }
}

void SoundService::updateVoiceChatOutputs()
{
#ifdef ENABLE_VOICE_CHAT
    for (VoiceChatOutputs::iterator it = voiceChatOutputs.begin(); voiceChatOutputs.end() != it; ++it)
    {
        VoiceChatOutput* voiceChatOutput = *it;
        voiceChatOutput->update3D();
    }
#endif
}
...
{
    if (propertyDescriptor == &Aya::GameBasicSettings::singleton().prop_masterVolume)
    {
        setMasterVolume(Aya::GameBasicSettings::singleton().getMasterVolume());
    }
}
...
void SoundService::registerChromiumFrame(ChromiumFrame* chromiumFrame)
{
    chromiumFrames.insert(chromiumFrame);
}

void SoundService::unregisterChromiumFrame(ChromiumFrame* chromiumFrame)
{
    ChromiumFrames::iterator iter = chromiumFrames.find(chromiumFrame);
    if (chromiumFrames.end() != iter)
    {
        chromiumFrames.erase(iter);
    }
}

void SoundService::registerVoiceChatOutput(VoiceChatOutput* voiceChatOutput)
{
    voiceChatOutputs.insert(voiceChatOutput);
}

void SoundService::unregisterVoiceChatOutput(VoiceChatOutput* voiceChatOutput)
{
    VoiceChatOutputs::iterator iter = voiceChatOutputs.find(voiceChatOutput);
    if (voiceChatOutputs.end() != iter)
    {
        voiceChatOutputs.erase(iter);
    }
}
...
void SoundService::checkResultNoThrow(FMOD_RESULT result, const char* fmodOperation, const void* rbxFmodParent, const void* fmodObject)
{
    if (FMOD_OK != result)
    {
        if (DebugSettings::singleton().soundWarnings || FLog::FMOD)
        {
            ...
            AYAASSERT(!FFlag::DebugBreakOnFMODErrors || FMOD_OK == result);
        }
    }
}
...
void SoundService::checkResult(FMOD_RESULT result, const char* fmodOperation, const void* rbxFmodParent, const void* fmodObject)
{
    if (FMOD_OK != result)
    {
        std::string message = Aya::format("FMOD %d: %s during %s", result, FMOD_ErrorString(result), fmodOperation);
