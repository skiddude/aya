...
public:
    ...
    inline void pop(Vector3int16* out)
    {
        ...
        Vector3int16 local =
            Vector3int16((indexOfNextCellToIssue & 0x1f), ((indexOfNextCellToIssue >> 10) & 0xf), ((indexOfNextCellToIssue >> 5) & 0x1f));
...
private:
    ...
    enum
    {
        kChunkSize = Voxel::kXZ_CHUNK_SIZE * Voxel::kXZ_CHUNK_SIZE * Voxel::kY_CHUNK_SIZE
    };
};
...
struct OneQuarterClusterChunkCellIterator
{
    ...
    {
        (*out) = cellOffset + Vector3int16((index & 0xf), ((index >> 8) & 0xf), ((index >> 4) & 0xf));
    }
    static inline void nextCellInIterationOrder(const Vector3int16& cellpos, Vector3int16* out)
    {
        ...
        // AYAASSERT(index+1 < (int)StreamRegion::getTotalVoxelVolumeOfARegion());
        cellFromIndex(offset, index + 1, out);
    }

    inline void pop(Vector3int16* out)
    {
        AYAASSERT(internalSize);
