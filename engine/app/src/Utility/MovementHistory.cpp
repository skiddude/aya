...
void MovementHistory::getMovementNodeList(const Time& lastCutOffTime, const Time& currentCutOffTime,
    std::deque<MovementHistory::MovementNode>& result, bool crossPacketCompression, const CoordinateFrame& lastSendCFrame,
    CoordinateFrame& outCalculatedBaselineCFrame, Vector3& outCalculatedLinearVelocity) const
{
    AYAASSERT(startIndex < MH_NUM_MAX_NODES);
    AYAASSERT(size <= MH_NUM_MAX_NODES);
    AYAASSERT(result.size() == 0);
    int numNodeToSkip = 0;
    ...
    if (size > 0)
    {
        ...
        if (estimatedNodeInterval > 0.f)
        {
            ...
            AYAASSERT(numNodeToSkip * estimatedNodeInterval <= nodeIntervalCap);
        }
    }
    ...
    if (crossPacketCompression)
    {
        ...
        AYAASSERT(baselineCFrameDeltaNode.translation.precisionLevel == 255 ||
                  (outCalculatedBaselineCFrame.translation - baselineCFrame.translation).magnitude() <
                      (baselineCFrameDeltaNode.translation.precisionLevel + 1) * MH_MIN_PRECISION * 2);
    }
    ...
    if (size > 0)
    {
        ...
        AYAASSERT(!Math::isNanInfVector3(outCalculatedLinearVelocity));
...
void MovementHistory::pushBack(MovementNode node)
{
    ...
    if (size == MH_NUM_MAX_NODES)
    {
        ...
        popFront();
        AYAASSERT(size == MH_NUM_MAX_NODES - 1);
    }
...
{
    AYAASSERT(numNodesToConcat <= size);
    AYAASSERT(lastIndex < MH_NUM_MAX_NODES);
    ...
    resultNode.delta2Ms = (Aya::uint8_t)totalTime;
    return resultNode;
}
