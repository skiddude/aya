...
namespace Aya
{
enum CacheSizeEnforceMethod
{
    CACHE_ENFORCE_MEMORY_SIZE,
    CACHE_ENFORCE_OBJECT_COUNT
};
...
public:
public:
    ControlledLRUCache(const unsigned long maxSize, CacheSizeEnforceMethod method = CACHE_ENFORCE_OBJECT_COUNT)
        : maxSize(maxSize)
        , enforceMethod(method)
    {
    ...
    inline void resize(unsigned long newSize)
    {
        ...
        AYAASSERT((enforceMethod == CACHE_ENFORCE_MEMORY_SIZE ? memSize() : size()) <= newSize);
    }
...
{
private:
    Aya::ControlledLRUCache<Key, Data> cache;
    boost::mutex mutex;
