...
boost::shared_ptr<void> CacheableContentProvider::requestContent(
    const ContentId& id, float priority, bool markUsed, AsyncHttpQueue::RequestResult& result)
{
    if (id.isNull())
    {
    ...
    if (ContentProvider* contentProvider = ServiceProvider::create<ContentProvider>(this))
    {
        ...
        lruCache->insert(id.toString(), cachedItem);
        {
            ...
            contentProvider->getContent(id, priority,
                boost::bind(&CacheableContentProvider::LoadContentCallbackHelper, weak_from(this), _1, _2, _3, id.toString()),
...
void CacheableContentProvider::LoadContentCallbackHelper(boost::weak_ptr<CacheableContentProvider> cacheableContentProvider,
    AsyncHttpQueue::RequestResult result, std::istream* filestream, shared_ptr<const std::string> data, std::string id)
{
...
void CacheableContentProvider::LoadContentCallback(
    AsyncHttpQueue::RequestResult result, std::istream* filestream, shared_ptr<const std::string> data, std::string id)
{
    if (contentJob)
...
TaskScheduler::StepResult CacheableContentProvider::ProcessTaskHelper(
