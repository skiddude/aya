...
public:
    ...
    Id(int x, int y, int z)
        : internalValue(Vector3int16(x, y, z))
    {
    }
...
inline Vector3int16 getRegionDimensionsInVoxels()
{
    ...
    return Vector3int16(kRegionXDimensionInVoxels, kRegionYDimensionInVoxels, kRegionZDimensionInVoxels);
}

inline Vector3int16 getRegionDimensionInVoxelsAsBitShifts()
{
    using namespace Constants;
    return Vector3int16(kRegionXDimensionInVoxelsAsBitShift, kRegionYDimensionInVoxelsAsBitShift, kRegionZDimensionInVoxelsAsBitShift);
}

inline Vector3int16 getMaxVoxelOffsetInsideRegion()
{
    using namespace Constants;
    return Vector3int16(kMaxXVoxelOffsetInsideRegion, kMaxYVoxelOffsetInsideRegion, kMaxZVoxelOffsetInsideRegion);
}
...
inline Vector3int16 voxelCoordinateRelativeToEnclosingRegion(const Vector3int16& globalVoxelCoordinate)
{
    return globalVoxelCoordinate & getMaxVoxelOffsetInsideRegion();
}

inline Vector3int16 globalVoxelCoordinateFromRegionAndRelativeCoordinate(const Id& id, const Vector3int16& relativeCoordinate)
{
...
{
    static const Vector3int32 kHalfDimensionInStuds(
        Vector3int32::one() << (_PrivateConstants::kRegionDimensionInStudsAsBitShifts - Vector3int16::one()));
    return smallestCornerOfRegionInGlobalCoordStuds(id) + kHalfDimensionInStuds;
}
