...
const G3D::Color3 Color::colorFromTemperature(float value) // 0.0 == cold, 1.0 == hot;
{
    ...
    if (clamped < 1.0)
    {
        return Color3::black().lerp(Color3::blue(), clamped);
    }
    if (clamped < 2.0)
    {
        return Color3::blue().lerp(Color3::purple(), clamped - 1);
    }
    if (clamped < 3.0)
    {
        return Color3::purple().lerp(Color3::red(), clamped - 2);
    }
    if (clamped < 4.0)
    {
        return Color3::red().lerp(Color3::orange(), clamped - 3);
    }
    if (clamped < 5.0)
    {
        return Color3::orange().lerp(Color3::yellow(), clamped - 4);
    }
    else
    {
        return Color3::yellow().lerp(Color3::white(), clamped - 5);
    }
}
