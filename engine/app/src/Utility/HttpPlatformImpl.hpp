...
struct Header
{
#define AYA_CACHE_FILE_MAGIC 0x52425848 // RBXH
#define AYA_CACHE_FILE_VERSION 0x1
#define AYA_CACHE_URL_MAX_LENGTH 1024U

    const uint32_t magic;
    const uint32_t version;
    const uint32_t urlBytes;
    const uint8_t url[AYA_CACHE_URL_MAX_LENGTH]; // not null-terminated
...
public:
    ...
    Data(const char* data, const size_t bytes)
        : underlying(reinterpret_cast<const uint8_t*>(data))
        , bytes(bytes)
    {
        AYAASSERT(sizeof(char) == sizeof(uint8_t));
    }
...
public:
    explicit CacheResult(const std::string& invalidReason)
