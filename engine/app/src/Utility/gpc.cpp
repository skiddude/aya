...
#define OPTIMAL(v, i, n) ((v[PREV_INDEX(i, n)].y != v[i].y) || (v[NEXT_INDEX(i, n)].y != v[i].y))
...
static void add_st_edge(st_node** st, it_node** it, edge_node* edge, double dy)
{
    ...
    else
    {
        ...
        if ((edge->xt >= (*st)->xt) || (edge->dx == (*st)->dx) || (fabs(den) <= DBL_EPSILON))
        {
...
static void minimax_test(gpc_polygon* subj, gpc_polygon* clip, gpc_op op)
{
    ...
    MALLOC(o_table, subj->num_contours * clip->num_contours * sizeof(int), "overlap table creation", int);

    /* Check all subject contour bounding boxes against clip boxes */
    for (s = 0; s < subj->num_contours; s++)
        for (c = 0; c < clip->num_contours; c++)
            o_table[c * subj->num_contours + s] = (!((s_bbox[s].xmax < c_bbox[c].xmin) || (s_bbox[s].xmin > c_bbox[c].xmax))) &&
                                                  (!((s_bbox[s].ymax < c_bbox[c].ymin) || (s_bbox[s].ymin > c_bbox[c].ymax)));
...
void gpc_add_contour(gpc_polygon* p, gpc_vertex_list* new_contour, int hole)
{
    ...
    MALLOC(extended_hole, (p->num_contours + 1) * sizeof(int), "contour hole addition", int);

    /* Create an extended contour array */
    MALLOC(extended_contour, (p->num_contours + 1) * sizeof(gpc_vertex_list), "contour addition", gpc_vertex_list);
    ...
    MALLOC(extended_contour[c].vertex, new_contour->num_vertices * sizeof(gpc_vertex), "contour addition", gpc_vertex);
...
void gpc_polygon_clip(gpc_op op, gpc_polygon* subj, gpc_polygon* clip, gpc_polygon* result)
{
    ...
    if (((subj->num_contours == 0) && (clip->num_contours == 0)) || ((subj->num_contours == 0) && ((op == GPC_INT) || (op == GPC_DIFF))) ||
    ...
    while (scanbeam < sbt_entries)
    {
        ...
        for (edge = aet; edge; edge = edge->next)
        {
            ...
            if (exists[CLIP] || exists[SUBJ])
            {
                ...
                case GPC_DIFF:
                case GPC_INT:
                    contributing = (exists[CLIP] && (parity[SUBJ] || horiz[SUBJ])) || (exists[SUBJ] && (parity[CLIP] || horiz[CLIP])) ||
        ...
        if (scanbeam < sbt_entries)
        {
            ...
            for (intersect = it; intersect; intersect = intersect->next)
            {
                ...
                if ((e0->bundle[ABOVE][CLIP] || e0->bundle[ABOVE][SUBJ]) && (e1->bundle[ABOVE][CLIP] || e1->bundle[ABOVE][SUBJ]))
                {
                    ...
                    in[CLIP] = (e0->bundle[ABOVE][CLIP] && !e0->bside[CLIP]) || (e1->bundle[ABOVE][CLIP] && e1->bside[CLIP]) ||
                               (!e0->bundle[ABOVE][CLIP] && !e1->bundle[ABOVE][CLIP] && e0->bside[CLIP] && e1->bside[CLIP]);
                    in[SUBJ] = (e0->bundle[ABOVE][SUBJ] && !e0->bside[SUBJ]) || (e1->bundle[ABOVE][SUBJ] && e1->bside[SUBJ]) ||
    ...
    {
        MALLOC(result->hole, result->num_contours * sizeof(int), "hole flag table creation", int);
        MALLOC(result->contour, result->num_contours * sizeof(gpc_vertex_list), "contour creation", gpc_vertex_list);
        ...
        for (poly = out_poly; poly; poly = npoly)
        {
            ...
            if (poly->active)
            {
                ...
                MALLOC(result->contour[c].vertex, result->contour[c].num_vertices * sizeof(gpc_vertex), "vertex creation", gpc_vertex);
...
void gpc_tristrip_clip(gpc_op op, gpc_polygon* subj, gpc_polygon* clip, gpc_tristrip* result)
{
    ...
    {
        MALLOC(result->strip, result->num_strips * sizeof(gpc_vertex_list), "tristrip list creation", gpc_vertex_list);
        ...
        for (tn = tlist; tn; tn = tnn)
        {
            ...
            if (tn->active > 2)
            {
                ...
                MALLOC(result->strip[s].vertex, tn->active * sizeof(gpc_vertex), "tristrip creation", gpc_vertex);
                v = 0;
                if (INVERT_TRISTRIPS)
                {
                    lt = tn->v[RIGHT];
