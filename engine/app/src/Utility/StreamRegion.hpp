...
const int kRegionSizeInStudsAsBitShift = kRegionSizeInVoxelsAsBitShift + Voxel::kCELL_SIZE_AS_BIT_SHIFT;

const Vector3int32 kRegionDimensionInVoxelsAsBitShifts(kRegionSizeInVoxelsAsBitShift, kRegionSizeInVoxelsAsBitShift, kRegionSizeInVoxelsAsBitShift);

const Vector3int32 kRegionDimensionInStudsAsBitShifts(kRegionSizeInStudsAsBitShift, kRegionSizeInStudsAsBitShift, kRegionSizeInStudsAsBitShift);
...
public:
    Id()
        : internalValue(Vector3int32(0, 0, 0))
    {
    }
    ...
    Id(int x, int y, int z)
        : internalValue(Vector3int32(x, y, z))
    {
    }
    ...
    Id operator+(const Vector3int32& other) const
    {
        return Id(internalValue + other);
    }
    Id operator+(const Id& other) const
    {
        return Id(internalValue + other.internalValue);
    }
...
{
    static Vector3int32 v = Vector3int32(
        StreamRegion::Id::streamGridCellSizeInStuds(), StreamRegion::Id::streamGridCellSizeInStuds(), StreamRegion::Id::streamGridCellSizeInStuds());
    return v;
}

inline const Vector3int32& gridCellHalfDimension()
{
    static Vector3int32 v = Vector3int32(StreamRegion::Id::streamGridCellSizeInStuds() / 2, StreamRegion::Id::streamGridCellSizeInStuds() / 2,
...
{
    return Id(Vector3int32(voxelCoordinate) >> _PrivateConstants::kRegionDimensionInVoxelsAsBitShifts);
}
...
inline Vector3int16 getMinVoxelCoordinateInsideRegion(const Id& id)
{
    ...
    return (id.value() << _PrivateConstants::kRegionDimensionInVoxelsAsBitShifts).toVector3int16();
}

inline Vector3int16 getMaxVoxelOffsetInsideRegion()
{
    using namespace _PrivateConstants;
    return Vector3int16(kMaxVoxelOffsetInsideRegion, kMaxVoxelOffsetInsideRegion, kMaxVoxelOffsetInsideRegion);
}
...
inline IdExtents regionExtentsFromContactManagerLevelAndExtents(int level, ExtentsInt32 contactManagerExtents)
{
    ...
    ExtentsInt32 scaledExtents = SpatialHashStatic::scaleExtents(level, kSpatialHashMaxLevel, contactManagerExtents);
    IdExtents result;
...
inline bool coarseMovementCausesStreamRegionChange(
    const ContactManagerSpatialHash::CoarseMovementCallback::UpdateInfo& info, IdExtents* oldExtents, IdExtents* newExtents)
{

    typedef ContactManagerSpatialHash::CoarseMovementCallback::UpdateInfo UpdateInfo;

    AYAASSERT_SLOW(info.updateType == UpdateInfo::UPDATE_TYPE_Change || info.updateType == UpdateInfo::UPDATE_TYPE_Insert);

    (*newExtents) = regionExtentsFromContactManagerLevelAndExtents(info.newLevel, info.newSpatialExtents);

    if (info.updateType == UpdateInfo::UPDATE_TYPE_Change)
    {
        (*oldExtents) = regionExtentsFromContactManagerLevelAndExtents(info.oldLevel, info.oldSpatialExtents);
