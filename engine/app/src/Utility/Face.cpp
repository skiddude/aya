...
Face Face::projectOverlapOnMe(const Face& other) const
{
    ...
    // AYAASSERT(Face::hasOverlap(f0, f1, 1e-6f));
    Vector3 normal[2];
    float minL[2];
    float maxL[2];
    ...
    return Face(f0.c0 + (normal[0] * minL[0] + normal[1] * minL[1]), f0.c0 + (normal[0] * minL[0] + normal[1] * maxL[1]),
        f0.c0 + (normal[0] * maxL[0] + normal[1] * maxL[1]), f0.c0 + (normal[0] * maxL[0] + normal[1] * minL[1]));
}
