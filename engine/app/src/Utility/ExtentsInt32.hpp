...
public:
    ...
    const Vector3int32& min() const
    {
        return low;
    }
    const Vector3int32& max() const
    {
        return high;
    }
    ...
    {
        return ((x >= low.x) && (y >= low.y) && (z >= low.z) && (x <= high.x) && (y <= high.y) && (z <= high.z));
    }
    ...
    bool overlapsOrTouches(const ExtentsInt32& other) const
    ...
    static bool overlapsOrTouches(const ExtentsInt32& e0, const ExtentsInt32& e1)
    {
