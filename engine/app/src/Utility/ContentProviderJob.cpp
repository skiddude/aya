...
ContentProviderJob::ContentProviderJob(shared_ptr<DataModel> dataModel, const char* name,
    boost::function<TaskScheduler::StepResult(std::string, shared_ptr<const std::string>)> processFunc, boost::function<void(std::string)> errorFunc)
...
void ContentProviderJob::addTask(
    const std::string& id, AsyncHttpQueue::RequestResult result, std::istream* filestream, shared_ptr<const std::string> data)
{
    try
    {
    ...
    {
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "%s failed to load %s because %s", name.c_str(), id.c_str(), e.what());
        errorFunc(id);
    }
}
...
TaskScheduler::StepResult ContentProviderJob::processTask(const ContentProviderTask& task)
{
    ...
    {
        errorFunc(task.id);
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "%s failed to process %s because %s", name.c_str(), task.id.c_str(), e.what());
    }
...
TaskScheduler::StepResult ContentProviderJob::stepDataModelJob(const Stats& stats)
{
    ...
    {
        float tasksToRun = updateStepsRequiredForCyclicExecutive(stats.timespanSinceLastStep.seconds(), tasks.size(),
            (float)DFInt::MaxContentProviderRunsPerStep, (float)DFInt::MaxContentProviderRunsAccumulated);
        ...
        for (int i = 0; i < (int)tasksToRun; i++)
        {
            ...
            {
                AYAPROFILER_SCOPE("Content", "processTask");
