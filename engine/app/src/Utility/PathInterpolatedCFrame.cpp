...
void PathInterpolatedCFrame::setValue(PartInstance* part, const CoordinateFrame& value, const Velocity& vel, const RemoteTime& remoteSendTime,
    Time now, float localTimeOffset, int numNodesAhead)
{
    int numOutstandingNodes =
        DFFlag::CleanUpInterpolationTimestamps ? (numNodesAhead + DFInt::InterpolationBufferMinSize) : (numNodesAhead + NUM_BUFFER_NODES);
...
RemoteTime PathInterpolatedCFrame::computeSampleTargetTime(const Time& now)
{
    if (DFFlag::CleanUpInterpolationTimestamps)
    {
        ...
        return (now - Time::Interval(localToRemoteTimeOffset)) -
               Time::Interval(((double)DFInt::InterpolationDelayFactorTenths / 10.0f) * (averageFrameDelay + targetDelayInSeconds));
    }
    else
    {
        ...
        if (targetDelayInSeconds > 0)
        {
            ...
            double acutalDelay =
                ((now - Time::Interval(localToRemoteTimeOffset)) - (prevFrame.remoteTime + Time::Interval(localDeltaTime))).seconds();
...
CoordinateFrame PathInterpolatedCFrame::computeValue(PartInstance* part, const Time& t)
{
    ...
    FASTLOG4F(FLog::ttMetricP1, "frameinfos %f  %f %f %f", (float)(frameInfos.size()) / 10.0, (targetTime - frameInfos.front().remoteTime).seconds(),
        (frameInfos.back().remoteTime - targetTime).seconds(), (frameInfos.back().remoteTime - frameInfos.front().remoteTime).seconds());
...
const CoordinateFrame& PathInterpolatedCFrame::interpolateHermiteSpline(
    const Time& now, const Time& targetTime, const unsigned int& upper, const PartInstance* part)
{
...
const CoordinateFrame& PathInterpolatedCFrame::interpolate(
    const Time& now, const Time& targetTime, const unsigned int& upper, const PartInstance* part)
{
    targetFrame = upper;
    ...
    if (part)
    {
        ...
        {
            // StandardOut::singleton()->printf(MESSAGE_INFO, "deltaVelocity too large: %f, teleporting instead of interpolation",
    ...
    if (!DFFlag::RemoveInterpolationSmoothing)
    {
        if (0 < upper && upper < frameInfos.size() - 1)
        {
            ...
            alpha = (targetTime - frameInfos[upper - 1].remoteTime).seconds() /
                    (frameInfos[upper].remoteTime - frameInfos[upper - 1].remoteTime).seconds();
