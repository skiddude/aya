...
void Aya::Profiling::init(bool enabled)
{
...
void Aya::Profiling::setEnabled(bool enabled)
{
    profilingEnabled = enabled;
#ifdef _WIN32
    ...
    // of a Mark.
    AYAASSERT(markTlsIndex);
#endif
}

bool Aya::Profiling::isEnabled()
{
...
void CodeProfiler::log(bool frameTick, double wallTimeElapsed)
{
    ...
    // AYAASSERT(wallTimeElapsed>=-0.001);
...
Mark::Mark(CodeProfiler& section, bool frameTick, bool logInclusive)
    : section(section)
    , frameTick(frameTick)
    , enabled(profilingEnabled || DebugSettings::singleton().enableProfiling)
    , logInclusive(logInclusive)
{
#if 1
    if (enabled)
    {
...
BucketProfile::BucketProfile(const int* bucketLimits, int size)
...
BucketProfile::BucketProfile(const BucketProfile& orig)
    : data(orig.data)
    , bucketLimits(orig.bucketLimits)
    , total(0)
{
}
