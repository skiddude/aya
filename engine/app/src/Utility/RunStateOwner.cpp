...
static Reflection::EventDesc<RunService, void(double, double)> event_Stepped(
    &RunService::scriptSteppedSignal, "Stepped", "time", "step", Security::None);
...
static Reflection::EventDesc<RunService, void(double), Aya::signal<void(double)>, Aya::signal<void(double)>* (RunService::*)(bool)>
    event_RenderStepped(&RunService::getOrCreateScriptRenderSteppedSignal, "RenderStepped", "step", Security::None);
...
static Reflection::BoundFuncDesc<RunService, void()> resetFunction(
    &RunService::stop, "Reset", Security::Plugin, Reflection::Descriptor::Attributes::deprecated());
...
static Reflection::BoundFuncDesc<RunService, void(std::string)> func_unbindRenderStepEarly(
    &RunService::unbindFunctionFromRenderStepEarly, "UnbindFromRenderStep", "name", Security::None);
static Reflection::BoundFuncDesc<RunService, void(std::string, int, Lua::WeakFunctionRef)> func_bindRenderStepEarly(
    &RunService::bindFunctionToRenderStepEarly, "BindToRenderStep", "name", "priority", "function", Security::None);
...
public:
    struct StepRecord
    {
        StepRecord(Time beginT, float ti, Time endT = Time())
            : beginTime(beginT)
            , timeInterval(ti)
            , endTime(endT)
        {
    ...
    virtual double averageStepsPerSecond() const
    {
        ...
        else
        {
            ...
            Aya::Time lastCountedTimestamp = stepRecords.begin() != stepRecords.end() ? (stepRecords.end() - 1)->endTime : Aya::Time();
    ...
    virtual TaskScheduler::StepResult stepDataModelJob(const Stats& stats)
    {
        ...
        if ((lerpCheck > 0.051 || lerpCheck < 0.049) || (noclipHighCheck.x < 0.95e6f || noclipHighCheck.y < 0.96e6f || noclipHighCheck.z < 0.97e6f) ||
...
public:
    ...
    HeartbeatTask(shared_ptr<RunService> runService)
        ...
        , lastHeartbeatStamp(Aya::Time::now<Aya::Time::Fast>())
    {
        cyclicExecutive = true;
    ...
    TaskScheduler::StepResult stepDataModelJob(const Stats& stats)
    {
        ...
        if (!DFFlag::VariableHeartbeat && (Aya::TaskScheduler::singleton().isCyclicExecutive() && cyclicExecutive))
        {
            ...
            float testSteps = updateStepsRequiredForCyclicExecutive(stats.timespanSinceLastStep.seconds(), fps, maxStepsPerCycle, maxStepsPerCycle);
            ...
            {
                Aya::Time currentTime = Aya::Time::now<Aya::Time::Fast>();
...
TaskScheduler::Job* RunService::getPhysicsJob()
{
    return physicsJob.get();
}
TaskScheduler::Job* RunService::getHeartbeat()
{
    return heartbeatTask.get();
};
...
{
    AYAPROFILER_SCOPE("Physics", "gameStepped");
...
void RunService::unbindFunctionFromRenderStepEarly(std::string name)
{
    ...
    if (erasedFuncCount > 1)
    {
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_WARNING,
            "RunService:UnbindFromRenderStep removed different functions with same reference name %s %i times.", name.c_str(), erasedFuncCount);
    }
}
            ...
            if (ScriptContext* sc = ServiceProvider::create<ScriptContext>(strongThis.get()))
            {
                ...
                catch (Aya::base_exception& e)
                {
                    StandardOut::singleton()->printf(
                        MESSAGE_ERROR, "RunService:fireRenderStepEarlyFunctions unexpected error while invoking callback: %s", e.what());
                }
            }
        }
    }
    ...
    for (EventCallbackMap::iterator iter = renderSteppedEarlyCallbackMap.begin(); iter != renderSteppedEarlyCallbackMap.end(); ++iter)
    {
        ...
        for (std::vector<FunctionNameRefPair>::iterator funcIter = functionVector.begin(); funcIter != functionVector.end(); ++funcIter)
        {
            try
            {
                ...
                {
                    InvokeCallback(weak_from(this), funcIter->second, Aya::make_shared<Reflection::Tuple>());
                }
            }
            catch (const std::runtime_error& e)
            {
                throw Aya::runtime_error("RunService::InvokeCallback failed because %s", e.what());
            }
        }
    }
}
...
double RunService::heartbeatFps() const
{
    return heartbeatTask->averageStepsPerSecond();
}

double RunService::physicsAverageStep() const
{
    return physicsJob->averageStepTime();
}
double RunService::heartbeatAverageStep() const
{
    return heartbeatTask->averageStepTime();
}

double RunService::physicsCpuFraction() const
{
    return physicsJob->averageDutyCycle();
}
double RunService::heartbeatCpuFraction() const
{
    return heartbeatTask->averageDutyCycle();
}

Aya::signal<void(double)>* RunService::getOrCreateScriptRenderSteppedSignal(bool create)
{
