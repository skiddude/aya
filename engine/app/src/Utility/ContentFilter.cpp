...
static Reflection::BoundFuncDesc<ContentFilter, void(std::string)> func_setFilterUrl(
    &ContentFilter::setFilterUrl, "SetFilterUrl", "url", Security::LocalUser);
static Reflection::BoundFuncDesc<ContentFilter, void(int, int)> func_setFilterLimits(
    &ContentFilter::setFilterLimits, "SetFilterLimits", "outstandingRequests", "cacheSize", Security::LocalUser);
REFLECTION_END();
...
void ContentFilter::doFilterRequest(std::string value)
{
    if (requestSet.find(value) == requestSet.end() && requestSet.size() < maxOutstandingRequests)
    {
        ...
        Http(url, Http::WinHttp)
            .post(value, Http::kContentTypeDefaultUnspecified, false, boost::bind(&staticDoFilterResult, _1, _2, weak_from(this), value));
    }
}
