...
private:
    ...
    static Aya::mutex* robloxResponceLock;
    static Aya::mutex* cdnResponceLock;
...
public:
    ...
    static Aya::atomic<int> cdnSuccessCount;
    static Aya::atomic<int> cdnFailureCount;
    static Aya::atomic<int> alternateCdnSuccessCount;
    static Aya::atomic<int> alternateCdnFailureCount;
    static double lastCdnFailureTimeSpan;
    static Aya::atomic<int> robloxSuccessCount;
    static Aya::atomic<int> robloxFailureCount;

    static WindowAverage<double, double> robloxResponce;
    static WindowAverage<double, double> cdnResponce;

    static Aya::mutex* getRobloxResponceLock();
    static Aya::mutex* getCdnResponceLock();

    std::string url;
    Http()
        : instanceApi(defaultApi)
        , url("")
    {
        init();
    }
    ...
    Http(const std::string& url, API api)
        : instanceApi(api)
        , url(url)
    {
        init();
    ...
    {
        sendTimeoutMillis = timeout;
    }
    void setDataSendTimeout(int timeout)
    {
        dataSendTimeoutMillis = timeout;
    }
    ...
    // Async
    void post(const std::string& input, const std::string& contentType, bool compress, boost::function<void(std::string*, std::exception*)> handler,
    ...
    static bool isInstanceSite(const char* url);
    ...
    void applyAdditionalHeaders(Aya::HttpAux::AdditionalHeaders& outHeaders);

private:
    void httpGetPost(bool isPost, std::istream& dataStream, const std::string& contentType, bool compressData,
        const HttpAux::AdditionalHeaders& additionalHeaders, bool allowExternal, std::string& response, bool forceNativeHttp = false);
#if defined(AYA_PLATFORM_DURANGO)
    void httpGetPostXbox(bool isPost, std::istream& dataStream, const std::string& contentType, bool compressData,
        const HttpAux::AdditionalHeaders& additionalHeaders, bool allowExternal, HttpCache::Policy cachePolicy, std::string& response);
#elif defined(_WIN32)
    void httpGetPostWinInet(bool isPost, std::istream& dataStream, const std::string& contentType, bool compressData,
        const HttpAux::AdditionalHeaders& additionalHeaders, bool allowExternal, std::string& response);
    void httpGetPostWinHttp(bool isPost, std::istream& dataStream, const std::string& contentType, bool compressData,
        const HttpAux::AdditionalHeaders& additionalHeaders, bool allowExternal, std::string& response);
#endif
    bool doHttpGetPostWithNativeFallbackForReporting(bool isPost, std::istream& dataStream, const std::string& contentType, bool compressData,
        const HttpAux::AdditionalHeaders& additionalHeaders, bool allowExternal, std::string& response);
