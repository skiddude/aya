...
float Math::sumDeltaAxis(const Matrix3& r0, const Matrix3& r1)
{
    float answer = 0.0f;
    for (int i = 0; i < 3; ++i)
    { // using row instead of columns (true vectors) - should give similar results
...
void Math::mulMatrixMatrixTranspose(const Matrix3& _m0, const Matrix3& _m1, Matrix3& _answer)
{
    ...
    for (int iRow = 0; iRow < 3; iRow++)
    {
        ...
        {
            answer[iRow * 3 + iCol] = m0[iRow * 3] * m1[iCol * 3] + m0[iRow * 3 + 1] * m1[iCol * 3 + 1] + m0[iRow * 3 + 2] * m1[iCol * 3 + 2];
        }
    }
}
...
void Math::mulMatrixTransposeMatrix(const Matrix3& _m0, const Matrix3& _m1, Matrix3& _answer)
{
    ...
    for (int iRow = 0; iRow < 3; iRow++)
    {
        for (int iCol = 0; iCol < 3; iCol++)
        {
            answer[iRow * 3 + iCol] = m0[iRow] * m1[iCol] + m0[iRow + 3] * m1[iCol + 3] + m0[iRow + 6] * m1[iCol + 6];
        }
    }
}
...
{
    AYAASSERT((angle >= -Math::pi()) && (angle <= Math::pi()));
...
{
    AYAASSERT(G3D::fuzzyGe(limitLo, -Math::pif()) && G3D::fuzzyLe(limitLo, Math::pif()));
    AYAASSERT(G3D::fuzzyGe(limitHi, -Math::pif()) && G3D::fuzzyLe(limitHi, Math::pif()));
    rot = radWrap(rot);
    if (limitLo <= limitHi)
    {
        if (rot < limitLo)
...
{
    AYAASSERT((newRotationNotWound >= -pi()) && (newRotationNotWound <= pi()));
    ...
    if ((currentQuadrant == 2) && (newQuadrant == 3))
    { // increasing angle over the threshold
        windings++;
    }
    ...
    AYAASSERT(fabs(answer - currentRotationWound) < 0.5);
...
void Math::lerpArray(const G3D::Array<float>& before, const G3D::Array<float>& after, G3D::Array<float>& answer, float alpha)
{
    AYAASSERT(before.size() == after.size());
    AYAASSERT(before.size() == answer.size());
...
{
    AYAASSERT(angle <= (Math::pif() + 0.0001f));
    AYAASSERT(angle >= -(Math::pif() + 0.0001f));
...
unsigned char Math::rotationToByte(float angle)
{
    ...
    AYAASSERT(delta <= segSizeRadians());
#endif
...
Matrix3 Math::getIWorldAtPoint(const Vector3& cofmPos, const Vector3& worldPos, const Matrix3& iWorldAtCofm, float mass)
{
    ...
    float xz = d.x * d.z;
    float yz = d.y * d.z;
    Matrix3 answer = iWorldAtCofm + mass * Matrix3(y2 + z2, -xy, -xz, -xy, x2 + z2, -yz, -xz, -yz, x2 + y2);
    return answer;
}

// see  http://kwon3d.com/theory/moi/triten.html
Matrix3 Math::getIBodyAtPoint(const Vector3& p, const Matrix3& iBody, float mass)
{
    float x2 = p.x * p.x;
    float y2 = p.y * p.y;
    float z2 = p.z * p.z;
    ...
    float xz = p.x * p.z;
    float yz = p.y * p.z;
    Matrix3 answer = iBody + mass * Matrix3(y2 + z2, -xy, -xz, -xy, x2 + z2, -yz, -xz, -yz, x2 + y2);
...
Matrix3 Math::fromDirectionCosines(
    const Vector3& fromX, const Vector3& fromY, const Vector3& fromZ, const Vector3& toX, const Vector3& toY, const Vector3& toZ)
{
...
bool Math::isAxisAligned(const Matrix3& matrix)
{
    ...
    return ((x[0] == 1) && (x[1] == 1) && (x[2] == 1) && (y[0] == 1) && (y[1] == 1) && (y[2] == 1));
}
...
#ifdef _DEBUG
    AYAASSERT(isOrthonormal(answer));
...
Vector3 Math::safeDirection(const Vector3& v)
{
    ...
    if (fMagnitude > 1e-12f)
    {
        ...
        AYAASSERT(answer.magnitude() < 1.01f);
        return answer;
    }
    else
    {
...
{
    return Vector3((float)iRound(point.x), (float)iRound(point.y), (float)iRound(point.z));
}

// return 0..pi
float Math::angle(const Vector3& v0, const Vector3& v1)
{
    AYAASSERT_FISHING(v0.isUnit());
    AYAASSERT_FISHING(v1.isUnit());

    const float dot = v0.dot(v1);

    const float answer = (dot >= 1.0f) ? 0.0f : ((dot <= -1.0f) ? pif() : ::acosf(dot));
    return answer;
}

float Math::elevationAngle(const Vector3& look)
{
    AYAASSERT_VERY_FAST(look.y >= -1.0f);
    AYAASSERT_VERY_FAST(look.y <= 1.0f);

    const float answer = (look.y >= 1.0) ? piHalff() : ((look.y <= -1.0) ? -piHalff() : ::asin(look.y));
    return answer;
}

float Math::smallAngle(const Vector3& v0, const Vector3& v1)
{
    AYAASSERT_VERY_FAST(angle(v0, v1) < (Math::pif() * 0.10));
...
{
    AYAASSERT_VERY_FAST(Math::fuzzyEq(v0.magnitude(), 1.0f, 1e-4f)); // lower tolerance
    AYAASSERT_VERY_FAST(Math::fuzzyEq(v1.magnitude(), 1.0f, 1e-4f)); // lower tolerance
...
bool Math::orthonormalizeIfNecessary(Matrix3& m)
{
    ...
    {
        m.orthonormalize();
        AYAASSERT_IF_VALIDATING(isOrthonormal(m));
        return true;
    }
    else
    {
...
Matrix3 Math::alignAxesClosest(const Matrix3& align, const Matrix3& target)
{
    ...
    AYAASSERT_VERY_FAST(isOrthonormal(answer));
...
Vector3 Math::toSmallAngles(const Matrix3& matrix)
{
    ...
    return Vector3(0.5f * (matrix[2][1] - matrix[1][2]), 0.5f * (matrix[0][2] - matrix[2][0]), 0.5f * (matrix[1][0] - matrix[0][1]));
}
...
Vector3 Math::sortVector3(const Vector3& v)
{
    ...
    AYAASSERT_VERY_FAST(ans.z >= ans.y);
    AYAASSERT_VERY_FAST(ans.y >= ans.x);
    return ans;
}
...
Vector3 Math::closestPointOnRay(const Aya::RbxRay& pointOnRay, const Aya::RbxRay& otherRay)
{
...
const Matrix3 Math::matrixTiltQuadrant(int quadrant)
{
    ...
    case (0):
        return Matrix3(1, 0, 0, 0, 0, -1, 0, 1, 0);
...
bool Math::intersectRayConvexPolygon(const Aya::RbxRay& ray, const std::vector<Vector3>& poly, Vector3& hit, bool oneSided)
{
    if (poly.size() < 3)
    {
        AYAASSERT(0);
...
bool intersectLinePlaneOld(const Aya::RbxRay& ray, const G3D::Plane& plane, G3D::Vector3& hit)
{
    ...
    Aya::RbxRay oppositeRay = Aya::RbxRay::fromOriginAndDirection(ray.origin(), -ray.direction());
...
#ifdef _DEBUG
    RbxRay tempRay = Aya::RbxRay::fromOriginAndDirection(line.point(), line.direction());
    Vector3 tempHit;
    intersectLinePlaneOld(tempRay, plane, tempHit);
    AYAASSERT(tempHit.fuzzyEq(hit));
#endif
...
bool Math::lineSegmentDistanceIfCrossing(
    const Vector3& line1Pt1, const Vector3& line1Pt2, const Vector3& line2Pt1, const Vector3& line2Pt2, float& distance, float adjustEdgeTol)
{
