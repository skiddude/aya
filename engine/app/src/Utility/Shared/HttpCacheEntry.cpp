...
using namespace Aya;
using namespace Aya::HttpPlatformImpl::Cache;
using namespace Aya::HttpCache;
...
{
    static const uint32_t m = htonl(AYA_CACHE_FILE_MAGIC);
    return m;
}
...
void cleanCache(const CacheCleanOptions& options)
{
        ...
        (options.flagCleanUpBasedOnMemory && availableSpaceInGB <= options.numGigaBytesAvailableTrigger))
    {
        ...
        for (DirectoryEntries::iterator it = dirents.begin(); dirents.end() != it && std::distance(it, dirents.end()) > numFilesToKeep; ++it)
        {
...
static void* readWholeFile(const boost::filesystem::path& path, size_t* size)
{
    ...
    do
    {
        ...
        if (0 != fseek(vf, 0, SEEK_END))
            break;
        sz = (size_t)ftell(vf);
        if (0 != fseek(vf, 0, SEEK_SET))
            break;

        data = new char[sz];
        if (!data)
            break;
        if (sz != fread(data, 1, sz, vf))
            break;
...
static bool writeWholeFile(const boost::filesystem::path& path, const void* buffer, size_t size)
{
    ...
    do
    {
        ...
        if (!vf)
            break;
        if (size != fwrite(buffer, 1, size, vf))
            break;
        fclose(vf);
        return true;
    } while (0);
...
static void ceDeleter(Aya::HttpPlatformImpl::Cache::CacheEntry* e)
{
    delete[] ((char*)e);
}
...
CacheResult CacheResult::open(const char* assetUrl, const char* cdnUrl)
{
    try
    {
        ...
        if (AYA_CACHE_FILE_VERSION != hdr.version)
        {
        ...
        // This case will cover a scenario where we have a Non 200 cdn response cached (Example 404) & we will continue to fail as the 404 were cached
        // earlier Now when we see a Non 200 we do not cache it in update routine, but if it was cached earlier, we will not load it & it will become
        // stale & cleaned up with decay for the old 404 cached data It will fail at HTTP layer and we will again retry the CDN, if it is again 404
        // that is fine and will not be cached, but if it is a 200 then it will correctly get cached
...
CacheResult CacheResult::update(const char* assetUrl, const char* cdnUrl, const uint32_t responseCode, const Data& headers, const Data& body)
{
    ...
    // Aya: explicitly cast to uint32_t
    Header cacheHeader = {
        gMagic(),                                                                          // magic
        AYA_CACHE_FILE_VERSION,                                                            // version
        (uint32_t)std::min(strlen(cdnUrl), static_cast<size_t>(AYA_CACHE_URL_MAX_LENGTH)), // urlBytes
        {'\0'},                                                                            // url
        responseCode,                                                                      // responseCode
        (uint32_t)headers.size(),                                                          // responseHeadersSize
        calculateHash(headers),                                                            // responseHeadersHash
        (uint32_t)body.size(),                                                             // responseBodySize
