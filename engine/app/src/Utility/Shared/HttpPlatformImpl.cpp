 ...
 **/
#ifdef _WIN32
// Ensure Winsock2 is available before any Windows or network headers
...
DYNAMIC_FASTFLAGVARIABLE(HttpZeroLatencyCaching,
    false) // Never kill this flag, ZeroLatencyCahing will be ON only on Clients. We will still use CDN Caching on RCC & Studio
DYNAMIC_FASTFLAGVARIABLE(CleanMutexHttp, true)
DYNAMIC_FASTFLAGVARIABLE(SSLErrorLogAll, false)

using namespace Aya;
using namespace Aya::HttpPlatformImpl;
...
static void print_cookies(const char* tag, CURL* curl)
{
    CURLcode res;
    ...
    StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "print_cookies() from:%s Curl cookies:\n", tag);
    res = curl_easy_getinfo(curl, CURLINFO_COOKIELIST, &cookies);
    if (res != CURLE_OK)
    {
        StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "print_cookies() Curl curl_easy_getinfo failed\n");
        return;
    }
    nc = cookies, i = 1;
    while (nc)
    {
        StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "print_cookies() [%d]: %s\n", i, nc->data);
        nc = nc->next;
        i++;
    }
    if (i == 1)
    {
        StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "print_cookies() (none)\n");
    }
...
public:
    static void initializeCachingThreadHandler()
    {
        ...
        boost::thread(thread_wrapper(g, "aya_http_cache_clean"));
...
void addHeader(curl_slist*& headers, const char* header)
{
    ...
    if (NULL == headers)
    {
        throw Aya::runtime_error("Error adding header %s", header);
    }
}
...
class CurlHandle
{
    ...
    void logCacheResultStatus(const char* assetUrl, const char* cdnUrl, const char* openFailureReason, const std::string& kHeaders,
        size_t bytesDecompressed, std::string& binaryData)
    {
        ...
        // Write some http download statistics to a file that can be used for detailed analyses.
        {
            ...
            {
                firstLogRun = false;
                fout << "AssetURL,CDN_URL,FilePath,ContentEncoding,ContentType,MagicNum,ContentLength,BytesDecompressed,HitOrMiss,BytesMissedSoFar,"
                        "BytesHitSoFar"
                     << std::endl;
            }
            ...
            fout << assetUrl << ',' << cdnUrl << ',' << Cache::cacheFilePath(cdnUrl).string() << ',' << contentEncoding << ',' << contentType << ','
                 << magic << ',' << contentLength << ',' << bytesDecompressed << ',' << kSucceeded << ',' << totalBytesMissed << ',' << totalBytesHit
                 << std::endl;
        }
#endif
    }
...
public:
    CurlHandle(const std::string& url, bool externalRequest, HttpCache::Policy cachePolicy, long connectTimeoutMillis, long performTimeoutMillis)
        : url(url)
        , isPost(false)
        ...
        , curlsh(gCurlsh)
    {
        ...
        else
        {
            ...
            if (proxyPort)
            {
                logCurlError("CURLOPT_PROXYPORT", curl_easy_setopt(curl, CURLOPT_PROXYPORT, proxyPort));
            }
        }

        // fiddler
        // logCurlError("CURLOPT_PROXY", curl_easy_setopt(curl, CURLOPT_PROXY, "http://127.0.0.1:8000"));
        ...
        logCurlError("CURLOPT_ACCEPT_ENCODING", curl_easy_setopt(curl, CURLOPT_ACCEPT_ENCODING, "gzip, deflate"));
        ...
        AYAASSERT(initialized ? Http::CookieSharingUndefined != cookieSharingPolicy : Http::CookieSharingUndefined == cookieSharingPolicy);
    ...
    void setupPostData(std::istream& dataStream, bool compressData)
    {
        ...
        if (compressData)
        {
            ...
            catch (io::gzip_error& e)
            {
                ...
                throw Aya::runtime_error("Upload GZip error %d, ZLib error %d, \"%s\"", e1, e2, url.c_str());
            }
        ...
        logCurlError("CURLOPT_POSTFIELDSIZE_LARGE", curl_easy_setopt(curl, CURLOPT_POSTFIELDSIZE_LARGE, (curl_off_t)postData.size()));
    }

    void setupHeaders(const std::string& contentType, const HttpAux::AdditionalHeaders& headers)
    {
        FASTLOG(DFLog::HttpTrace, "Setting up CURL headers.");
        AYAASSERT(!curlHeaders);
        ...
        for (HttpAux::AdditionalHeaders::const_iterator it = headers.begin(); headers.end() != it; ++it)
        {
    ...
    void updateCsrfToken(const char* token)
    {
        ...
        if (isPost && !externalRequest)
        {
            ...
            {
                foundCsrfHeader = 0 == strncmp(list->data, kXCSRFTokenHeaderKey.c_str(), std::min(strlen(list->data), kXCSRFTokenHeaderKey.size()));
            ...
            if (foundCsrfHeader)
            {
                ...
                if (!dupdata)
                {
                    ...
                    throw Aya::runtime_error("CurlHandle(%p), strdup failure: %s", this, errmsg);
                }
        ...
        if (!isPost && HttpCache::PolicyFinalRedirect == cachePolicy)
        {
            ...
            if (cacheResult.isValid())
            {
                responseHeaders.write(
                    reinterpret_cast<const char*>(cacheResult.getResponseHeader().data()), cacheResult.getCacheHeader().responseHeadersSize);
    ...
    long perform(std::string& response, bool zeroLatencyCaching)
    {
        ...
        {
            for (int redirects = 0;
                 (302 == responseCode || 301 == responseCode) && (redirects < DFInt::HttpMaxRedirects || -1 == DFInt::HttpMaxRedirects); ++redirects)
            {
                cacheFailed = false;
                responseHeaders.str("");
                responseHeaders.clear();
...
void init(Http::CookieSharingPolicy cookieSharingPolicy)
{
    ...
    if (!initialized)
    {
...

        {
            ...
            boost::function0<void> g = boost::bind(&StandardOut::print_exception, f, MESSAGE_ERROR, false);
            boost::thread(thread_wrapper(g, "aya_http_cache_stats_report"));
        }
...
void perform(HttpOptions& options, std::string& response)
{
    ...
    shared_ptr<CurlHandle> curlHandle(
        new CurlHandle(url, options.externalRequest, options.cachePolicy, options.connectTimeoutMillis, options.performTimeoutMillis));
    ...
    else
    {
        if (DFFlag::HttpZeroLatencyCaching && HttpCache::PolicyFinalRedirect == options.cachePolicy && curlHandle->getOldCachedData(response))
        {
            ...
            threadPool->schedule(boost::bind(&Aya::HttpPlatformImpl::updateCachedData, curlHandle));
            return;
        }
    }
#endif
    ...
    // 202 is not treated as success, with 202 we call again(Look in in AsynHttpQueue.cpp) after some time and hopefully we will get a 200 OK once
    ...
    if (statusCode < 200 || statusCode > 299 || statusCode == 202)
    {
        ...
        throw Aya::http_status_error(statusCode, curlHandle->getResponseCodeReason());
#endif
    }
}
} // namespace HttpPlatformImpl
