...
inline float piHalff()
{
    return static_cast<float>(piHalf());
}
inline float twoPif()
{
    return static_cast<float>(twoPi());
}
...
inline Vector2 expandVector2(const Vector2& v, int expand)
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        answer[i] += expand * Aya::Math::sign(v[i]);
    }
    return answer;
}
...
{
    return (v.x < v.y) ? ((v.x < v.z) ? v.y * v.z : v.y * v.x) : ((v.y < v.z) ? v.x * v.z : v.x * v.y);
}
...
inline const Plane& yPlane()
{
    static Plane p(Vector3(0.0, 1.0, 0.0), Vector3::zero());
    return p;
}
Vector3 closestPointOnRay(const Aya::RbxRay& pointOnRay, const Aya::RbxRay& otherRay);
...
{
    return Matrix3(0.0f, -v.z, v.y, v.z, 0.0f, -v.x, -v.y, v.x, 0.0f);
}
...
{
    AYAASSERT_VERY_FAST((0 <= iCol) && (iCol < 3));
...
Matrix3 getIWorldAtPoint(const Vector3& cofmPos, const Vector3& worldPos, const Matrix3& iWorldAtCofm, float mass);
Matrix3 getIBodyAtPoint(const Vector3& pos, const Matrix3& iBody, float mass);
...
// std::vector
void lerpArray(const G3D::Array<float>& before, const G3D::Array<float>& after, G3D::Array<float>& answer, float alpha);
...
{
    return radians * (180.0f / Math::pif());
}
inline float degreesToRadians(float degrees)
{
    return degrees * (Math::pif() / 180.0f);
}
 ...
 */
inline float getHeading(const Vector3& look)
{
    return atan2(-look.x, -look.z);
}
inline float getElevation(const Vector3& look)
{
    return asin(look.y);
}
...
inline float radWrap(double rad)
{ // extra part
    if ((rad >= -pi()) && (rad < pi()))
    {
...
bool intersectRayConvexPolygon(const Aya::RbxRay& ray, const std::vector<Vector3>& poly, Vector3& hit, bool oneSided);
bool lineSegmentDistanceIfCrossing(
    const Vector3& line1Pt1, const Vector3& line1Pt2, const Vector3& line2Pt1, const Vector3& line2Pt2, float& distance, float adjustEdgeTol = 0.0f);
