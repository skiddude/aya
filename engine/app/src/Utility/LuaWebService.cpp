...
AYA_REGISTER_CLASS(Pages);
AYA_REGISTER_CLASS(StandardPages);
AYA_REGISTER_CLASS(FriendPages);
...
static Reflection::BoundYieldFuncDesc<Pages, void(void)> func_advanceToNextPageAsync(
    &Pages::advanceToNextPageAsync, "AdvanceToNextPageAsync", Security::None);
REFLECTION_END();
...
StandardPages::StandardPages(weak_ptr<DataModel> weakDM, const std::string& requestUrl, const std::string& fieldName)
    : weakDM(weakDM)
...
void StandardPages::fetchNextChunk(boost::function<void()> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    ...
    {
        if (Aya::HttpRbxApiService* apiService = Aya::ServiceProvider::find<Aya::HttpRbxApiService>(dm.get()))
        {
            apiService->getAsync(HttpRbxApiService::getApiUrlPath(url), true, Aya::PRIORITY_DEFAULT,
    ...
    {
        Aya::Http(url).get(boost::bind(&StandardPages::processFetch, shared_from(this), _1, _2, resumeFunction, errorFunction));
    }
}
...
void StandardPages::processFetchSuccess(
    std::string response, boost::function<void()> resumeFunction, boost::function<void(std::string)> errorFunction)
{
...
{
    errorFunction(Aya::format("StandardPages: Request Failed because %s", error.c_str()));
}

void StandardPages::processFetch(
...
void FriendPages::fetchNextChunk(boost::function<void()> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    ...
    if (!firstTime)
    {
        ...
        {
            Aya::Http(url).get(boost::bind(&FriendPages::processFetch, shared_from(this), _1, _2, resumeFunction, errorFunction));
        }
    }
    else
    {
...
{
    errorFunction(Aya::format("FriendPages: Request Failed because %s", error.c_str()));
}

void FriendPages::processFetch(
    std::string* response, std::exception* exception, boost::function<void()> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    DataModel::LegacyLock lock(weakDM.lock(), DataModelJob::Write);
    shared_ptr<const Reflection::ValueArray> result;
...
bool LuaWebService::parseWebJSONResponseHelper(
    std::string* response, std::exception* exception, shared_ptr<const Reflection::ValueTable>& result, std::string& status)
{
    if (response == NULL)
    {
...
bool LuaWebService::TryDispatchRequest(AsyncHttpCache<LuaWebService::CachedLuaWebServiceInfo, true>* webCache, const std::string& url,
...
template<typename ResultType>
bool LuaWebService::TryRawDispatchRequest(AsyncHttpCache<LuaWebService::CachedRawLuaWebServiceInfo, true>* webCache, const std::string& url,
    boost::function<void(ResultType)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    ...
    boost::function<void(shared_ptr<const Reflection::ValueArray>)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    if (!checkCache(url, resumeFunction, errorFunction))
    {
        AsyncHttpQueue::RequestCallback callback =
            boost::bind(&LuaWebService::Callback<shared_ptr<const Reflection::ValueArray>>, weak_from(this), _1, url, resumeFunction, errorFunction);
        webCache->asyncRequest(url, priority, &callback, AsyncHttpQueue::AsyncNone);
    }
}

void LuaWebService::asyncRequestNoCache(const std::string& url, float priority,
    boost::function<void(shared_ptr<const Reflection::ValueMap>)> resultFunction, AsyncHttpQueue::ResultJob resultJob)
{
    boost::function<void(std::string)> errorFunction = boost::bind(resultFunction, shared_ptr<const Reflection::ValueMap>());
    AsyncHttpQueue::RequestCallback callback =
        boost::bind(&LuaWebService::Callback<shared_ptr<const Reflection::ValueMap>>, weak_from(this), _1, url, resultFunction, errorFunction);
    webCache->asyncRequest(url, priority, &callback, resultJob, true);
}


void LuaWebService::asyncRequest(const std::string& url, float priority, boost::function<void(shared_ptr<const Reflection::ValueMap>)> resumeFunction,
    boost::function<void(std::string)> errorFunction)
{
    if (!checkCache(url, resumeFunction, errorFunction))
    {
        AsyncHttpQueue::RequestCallback callback =
            boost::bind(&LuaWebService::Callback<shared_ptr<const Reflection::ValueMap>>, weak_from(this), _1, url, resumeFunction, errorFunction);
        webCache->asyncRequest(url, priority, &callback, AsyncHttpQueue::AsyncNone);
    }
}
...
void LuaWebService::asyncRequest(
    const std::string& url, float priority, boost::function<void(bool)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    if (!checkCache(url, resumeFunction, errorFunction))
    {
        AsyncHttpQueue::RequestCallback callback =
            boost::bind(&LuaWebService::Callback<bool>, weak_from(this), _1, url, resumeFunction, errorFunction);
        webCache->asyncRequest(url, priority, &callback, AsyncHttpQueue::AsyncNone);
    }
}

void LuaWebService::asyncRequest(
    const std::string& url, float priority, boost::function<void(int)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    if (!checkCache(url, resumeFunction, errorFunction))
    {
        AsyncHttpQueue::RequestCallback callback =
            boost::bind(&LuaWebService::Callback<int>, weak_from(this), _1, url, resumeFunction, errorFunction);
        webCache->asyncRequest(url, priority, &callback, AsyncHttpQueue::AsyncNone);
    }
}

void LuaWebService::asyncRequest(
    const std::string& url, float priority, boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    if (!checkCache(url, resumeFunction, errorFunction))
    {
...
bool LuaWebService::isApiAccessEnabled()
{
    if (checkApiAccess)
    {
        if (!apiAccess.is_initialized() || (Time::now<Time::Fast>() > timeToRecheckApiAccess))
        {
            ...
            Http http(format("%s/universes/get-info?placeId=%d", ::trim_trailing_slashes(cp->getUnsecureApiBaseUrl()).c_str(), dm->getPlaceID()));
            http.doNotUseCachedResponse = true;
            try
            {
                if (Aya::HttpRbxApiService* apiService = Aya::ServiceProvider::find<Aya::HttpRbxApiService>(dm))
                {
                    apiService->get(format("universes/get-info?placeId=%d", dm->getPlaceID()), false, Aya::PRIORITY_EXTREME, response);
                }
            }
            ...
            if (!response.empty())
            {
                ...
                {
                    Aya::Reflection::ValueTable::const_iterator itr = v->find("StudioAccessToApisAllowed");
            ...
            timeToRecheckApiAccess = Time::now<Time::Fast>() + Time::Interval::from_milliseconds(DFInt::TimeBetweenCheckingApiAccessMillis);
        }
