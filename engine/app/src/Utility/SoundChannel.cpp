...
{
template<>
EnumDesc<Aya::Soundscape::RollOffMode>::EnumDesc()
    : EnumDescriptor("RollOffMode")
{
    addPair(Aya::Soundscape::Inverse, "Inverse");
    addPair(Aya::Soundscape::Linear, "Linear");
    addPair(Aya::Soundscape::LinearSquare, "LinearSquare");
    addPair(Aya::Soundscape::InverseTapered, "InverseTapered");
}
template<>
Aya::Soundscape::RollOffMode& Variant::convert<Aya::Soundscape::RollOffMode>(void)
{
    return genericConvert<Aya::Soundscape::RollOffMode>();
}
} // namespace Reflection
template<>
bool StringConverter<Aya::Soundscape::RollOffMode>::convertToValue(const std::string& text, Aya::Soundscape::RollOffMode& value)
{
    return Reflection::EnumDesc<Aya::Soundscape::RollOffMode>::singleton().convertToValue(text.c_str(), value);
}
} // namespace Aya
...
static Reflection::PropDescriptor<SoundChannel, SoundId> sound_desc_SoundId(
    "SoundId", category_Data, &SoundChannel::getSoundId, &SoundChannel::setSoundId);
...
static Reflection::PropDescriptor<SoundChannel, float> sound_desc_MinDistance(
    "MinDistance", category_Data, &SoundChannel::getMinDistance, &SoundChannel::setMinDistance);
static Reflection::PropDescriptor<SoundChannel, float> sound_desc_MaxDistance(
    "MaxDistance", category_Data, &SoundChannel::getMaxDistance, &SoundChannel::setMaxDistance);
static Reflection::EnumPropDescriptor<SoundChannel, RollOffMode> sound_desc_RollOffMode(
    "RollOffMode", category_Data, &SoundChannel::getRollOffMode, &SoundChannel::setRollOffMode);

static Reflection::PropDescriptor<SoundChannel, double> sound_desc_SoundLength(
    "TimeLength", category_Data, &SoundChannel::getSoundLength, NULL, Reflection::PropertyDescriptor::UI);
static Reflection::PropDescriptor<SoundChannel, double> sound_desc_SoundPosition(
    "TimePosition", category_Data, &SoundChannel::getSoundPosition, &SoundChannel::setSoundPositionLua, Reflection::PropertyDescriptor::UI);

static Reflection::PropDescriptor<SoundChannel, bool> sound_desc_Looped(
    "Looped", category_Behavior, &SoundChannel::getLooped, &SoundChannel::setLooped);
...
static Reflection::PropDescriptor<SoundChannel, bool> sound_desc_IsPlaying(
    "IsPlaying", category_Data, &SoundChannel::isPlaying, NULL, Reflection::PropertyDescriptor::UI);
static Reflection::PropDescriptor<SoundChannel, bool> sound_desc_isPaused(
    "IsPaused", category_Data, &SoundChannel::isPaused, NULL, Reflection::PropertyDescriptor::UI);

static Reflection::EventDesc<SoundChannel, void(std::string, int)> sound_loopedSignal(
    &SoundChannel::soundLoopedSignal, "DidLoop", "soundId", "numOfTimesLooped");
...
// @mdolli
// PlaybackSpeed
static Reflection::PropDescriptor<SoundChannel, float> sound_desc_PlaybackSpeed(
    "PlaybackSpeed", category_Data, &SoundChannel::getPlaybackSpeed, &SoundChannel::setPlaybackSpeed, Reflection::PropertyDescriptor::UI);

//////////////////////////////////////////////////////
// Backend Events/Properties
/////////////////////////////////////////////////
static Reflection::PropDescriptor<SoundChannel, int> sound_desc_PlayCount(
    "PlayCount", category_Data, &SoundChannel::getPlayCount, &SoundChannel::setPlayCount, Reflection::PropertyDescriptor::REPLICATE_ONLY);
static Reflection::RemoteEventDesc<SoundChannel, void(int)> event_timePositionUpdatedFromServer(&SoundChannel::timePositionUpdatedFromServerSignal,
    "TimePositionUpdated", "newPositionSeconds", Security::Roblox, Reflection::RemoteEventCommon::REPLICATE_ONLY,
    Reflection::RemoteEventCommon::BROADCAST);
static Reflection::RemoteEventDesc<SoundChannel, void(int)> event_timePositionUpdatedFromServerScript(
    &SoundChannel::timePositionUpdatedFromServerScriptSignal, "TimePositionUpdatedFromScript", "newPositionSeconds", Security::Roblox,
    Reflection::RemoteEventCommon::REPLICATE_ONLY, Reflection::RemoteEventCommon::BROADCAST);
static Reflection::RemoteEventDesc<SoundChannel, void(int)> event_soundResumedFromServer(&SoundChannel::soundResumedFromServerSignal,
    "SoundResumedFromServer", "currentTimePosition", Security::Roblox, Reflection::RemoteEventCommon::REPLICATE_ONLY,
    Reflection::RemoteEventCommon::BROADCAST);
...
/////////////////////////////////////////////////
static Reflection::BoundFuncDesc<SoundChannel, void()> sound_stopFunctionDep(
    &SoundChannel::stop, "stop", Security::None, Reflection::Descriptor::Attributes::deprecated(sound_stopFunction));
static Reflection::BoundFuncDesc<SoundChannel, void()> sound_playFunctionDeprecated(
    &SoundChannel::play, "play", Security::None, Reflection::Descriptor::Attributes::deprecated(sound_playFunction));
static Reflection::BoundFuncDesc<SoundChannel, void()> sound_dep_pauseFunction(
    &SoundChannel::pause, "pause", Security::None, Reflection::Descriptor::Attributes::deprecated(sound_pauseFunction));
static Reflection::PropDescriptor<SoundChannel, bool> sound_desc_dep_IsPlaying(
    "isPlaying", category_Data, &SoundChannel::isPlaying, NULL, Reflection::PropertyDescriptor::Attributes::deprecated(sound_desc_IsPlaying));
REFLECTION_END();
...
void SoundChannel::releaseChannel()
{
    ...
    if (fmod_channel)
    {
        ...
        AYAASSERT(oldSound == 0 || oldSound == this);
#endif
...
void SoundChannel::updateListenState(const Time::Interval& timeSinceLastStep)
{
    ...
    if (controlledByAndIsServer() && (timeSinceLastStep.msec() > 0))
    {
        ...
        if (isPlaying() && !isPaused())
        {
            ...
            {
                position = 0;
                DataModel::scoped_write_request request(Aya::DataModel::get(this));
                stop();
            }
            else
            {
...
void SoundChannel::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
    ...
    if (newProvider)
    {
        ...
        if (Aya::Network::Players::clientIsPresent(newProvider))
        {
            // only get initial position update from server if sound is replicated to each client
            if (isHeardGlobally())
            {
                serverUpdatedTimeConnection =
                    timePositionUpdatedFromServerSignal.connect(boost::bind(&SoundChannel::serverUpdatedTimePosition, this, _1));
            }

            serverScriptUpdatedTimeConnection =
                timePositionUpdatedFromServerScriptSignal.connect(boost::bind(&SoundChannel::serverUpdatedTimePositionFromScript, this, _1));
...
namespace
{
void onSoundLoadedForSoundChannel(shared_ptr<SoundChannel> soundChannel, const SoundId& soundId, const Instance* context, bool shouldPlayOnLoad,
    AsyncHttpQueue::RequestResult result, shared_ptr<const std::string> item)
{
    switch (result)
    {
    case AsyncHttpQueue::Failed:
        ...
        {
            Aya::StandardOut::singleton()->printf(MESSAGE_ERROR, "Sound failed to load %s : %s", context->getFullName().c_str(), soundId.c_str());
        }
        break;
...
bool SoundChannel::isPlaying() const
{
    ...
    if (!fmod_channel)
    {
        if (Aya::Network::Players::clientIsPresent(this, false))
        {
            return false;
        }
...
void SoundChannel::loadSound(const Instance* context, bool shouldPlayOnLoad)
{
    ...
    if (!SoundService::soundDisabled && !soundDisabled && Aya::GameSettings::singleton().soundEnabled)
    {
            ...
            if (!isSoundLoaded())
            {
                ...
                else
                {
                    ...
                    contentProvider->preloadContentWithCallback(soundId, ContentProvider::PRIORITY_SOUND,
                        boost::bind(&onSoundLoadedForSoundChannel, shared_from(this), soundId, context, shouldPlayOnLoad, _1,
                            shared_ptr<const std::string>()),
                        AsyncHttpQueue::AsyncWrite, FString::AssetTypeHeaderForSounds);
                }
            }
            else
            {
...
float SoundChannel::getPlaybackSpeed() const
{
    return playbackSpeed;
}

void SoundChannel::setPlaybackSpeed(float value)
{
    if (playbackSpeed != value)
    {
        float frequency = 0;
        FMOD_RESULT fmodResult = fmod_channel->getFrequency(&frequency);

        if (fmodResult == FMOD_OK)
        {
            playbackSpeed = value;
            SoundService::checkResultNoThrow(fmod_channel->setFrequency(frequency * value), "setFrequency", this, fmod_channel);
        }
    }
}
...
{
    if (isHeardGlobally() && !Aya::Network::Players::serverIsPresent(this))
    {
        if (Aya::Network::Players::clientIsPresent(this))
        {
            throw std::runtime_error("Sound.TimePosition was set from local script while either in Workspace or SoundService. Only use a server "
                                     "script to set TimePosition when a sound is in these locations.");
            return;
        }
    }
...
void SoundChannel::setPlayCount(int value)
{
    ...
    {
    case -1:
        if (isHeardGlobally() && Aya::Network::Players::clientIsPresent(this))
        {
            soundPositionSeconds = 0;
        }
        stop();
...
void SoundChannel::update3D(FMOD::Channel* channel)
{
    if (is3D && part)
    {
        ...
        if (rollOff == Aya::Soundscape::RollOffMode::Inverse)
            fmod_channel->setMode(FMOD_3D_INVERSEROLLOFF);
        else if (rollOff == Aya::Soundscape::RollOffMode::Linear)
            fmod_channel->setMode(FMOD_3D_LINEARROLLOFF);
        else if (rollOff == Aya::Soundscape::RollOffMode::LinearSquare)
            fmod_channel->setMode(FMOD_3D_LINEARSQUAREROLLOFF);
        else if (rollOff == Aya::Soundscape::RollOffMode::InverseTapered)
...
void SoundChannel::onChannelEnd(const FMOD_CHANNEL* channel)
{
    ...
    if (fmod_channel && reinterpret_cast<const void*>(channel) == reinterpret_cast<const void*>(fmod_channel))
    {
        ...
        {
            sound->unacquire();
            if (DataModel* dm = Aya::DataModel::get(this))
            {
                dm->submitTask(boost::bind(&SoundChannel::soundEnded, shared_from(this), getSoundId().toString()), Aya::DataModelJob::Write);
            }
        }
    }
}

FMOD_RESULT F_CALLBACK callbackChannelEnd(FMOD_CHANNELCONTROL* channelControl, FMOD_CHANNELCONTROL_TYPE type,
    FMOD_CHANNELCONTROL_CALLBACK_TYPE callbacktype, void* commanddata1, void* commanddata2)
{
    ...
    if (type == FMOD_CHANNELCONTROL_CHANNEL)
    {
        ...
        {
            // TODO: For some reason not all channels call this function... I have no idea why.  The result is that we have some sound "leaks"
            // reported by getSoundStats()
            Aya::Soundscape::SoundChannel* sound;
...
{
    return SoundService::soundDisabled && Aya::Network::Players::serverIsPresent(this) && isHeardGlobally();
}
