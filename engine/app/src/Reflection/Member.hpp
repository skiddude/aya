...
protected:
    MemberDescriptor(const ClassDescriptor& owner, const char* name, const char* category, Attributes attributes, Security::Permissions security)
        ...
        , owner(owner)
        , category(Aya::Name::declare(category))
        , security(security)
    {
    }
...
public:
    ...
    public:
        ...
        const MemberDescriptorType& getDescriptor() const
        {
            return **iter;
        }
    };
...
protected:
    ...
    {
        AYAASSERT(replaceable != descriptor);
        {
    ...
    void declare(MemberDescriptorType* descriptor)
    {
        ...
        // Add this to allDescriptors (in a determanistic order)
        {
            ...
            while (iter != allDescriptors().end())
            {
                ...
                if (desc == descriptor)
                    goto SKIP;
                int compare = Aya::Name::compare(descriptor->name, desc->name);
                if (compare < 0)
                    break;
                if (compare == 0)
                {
                    // This descriptor name already exists in a different class
                    compare = Aya::Name::compare(descriptor->owner.name, desc->owner.name);
