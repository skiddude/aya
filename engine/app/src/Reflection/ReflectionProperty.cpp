...
{
    {
        ...
        AYAASSERT(iter == allEnums().end() || *iter != this);
...
{
    AYAASSERT(std::find(allEnums().begin(), allEnums().end(), this) != allEnums().end());
...
PropertyDescriptor::PropertyDescriptor(ClassDescriptor& classDescriptor, const Type& type, const char* name, const char* category,
    Attributes attributes, Security::Permissions security, bool isEnum)
    ...
    , bIsEnum(isEnum)
{
    AYAASSERT(!type.tag.empty());
    AYAASSERT(!type.name.empty());
