...
#if 0
			typedef boost::unordered_map<const Aya::Name*, const EnumDescriptor*> EnumNameTable;
#else
    typedef std::map<const Aya::Name*, const EnumDescriptor*> EnumNameTable;
#endif
...
public:
    ...
    static const EnumDescriptor* lookupDescriptor(const Aya::Name& name)
    {
...
private:
    ...
    std::map<const Aya::Name*, Enum> nameToEnum;
    std::map<const Aya::Name*, Enum> nameToEnumLegacy;
    std::vector<const Aya::Name*> enumToName; // maps enum to Name (there may be gaps)
    ...
    {
        AYAASSERT_VERY_FAST(value >= 0);
        // No spaces in enums:
        AYAASSERT_VERY_FAST(std::string(name).find(' ') == std::string::npos);
        // No no CamelCase in enums:
        AYAASSERT_VERY_FAST(!isCamel(name));
        ...
        if (enumToName.size() <= (size_t)value)
            enumToName.resize(value + 1, &Aya::Name::getNullName());
    ...
    void addLegacy(int oldValue, const char* name, Enum value)
    {
        ...
        nameToEnumLegacy[&Aya::Name::declare(name)] = value;
    }
...
public:
    const Aya::Name& convertToName(const Enum& value) const
    {
        AYAASSERT(value >= 0);
        AYAASSERT(value < enumToItem.size());
        if (value < 0)
            return Aya::Name::getNullName();
    ...
    {
        AYAASSERT(value >= 0);
        AYAASSERT((size_t)value < enumToItem.size());
        if (value < 0)
            return "";
    ...
    bool convertToValue(const Aya::Name& name, Enum& value) const
    {
        typename std::map<const Aya::Name*, Enum>::const_iterator iter = nameToEnum.find(&name);
    ...
    {
        Enum e;
        if (convertToValue(Aya::Name::lookup(text), e))
    ...
    {
        return convertToValue(Aya::Name::lookup(text), value);
    }
...
// Helper macro
// GCC does not generate the registrar variable defination & fails at Link Time. Force Construct by passing in an dummy arg to ctor. That works. WEIRD
// huh?
#define AYA_REGISTER_ENUM(Enum) \
    namespace Aya \
    { \
    namespace Reflection \
    { \
    ...
    template<> \
    EnumRegistrar<Enum> EnumRegistrar<Enum>::registrar(0); \
    template<> \
    TypeRegistrar<Enum> TypeRegistrar<Enum>::registrar(0); \
    } \
    }
...
class EnumRegistrar : boost::noncopyable
{
    ...
    //// GCC does not generate the registrar variable defination & fails at Link Time. Force Construct by passing in an dummy arg to ctor. That works.
    /// WEIRD huh?
    EnumRegistrar(int i)
        : x(i)
    {
