...
#include "Xml/Reference.hpp" // TODO: Reflection namespace should not know about Tree
...
class AyaBaseClass PropertyDescriptor : public MemberDescriptor
{
private:
...
public:
    ...
    enum Functionality
    {
        ...
        UI = 1 + 0 + (4) + 0 +
             16,                               // isPublic,			     (canXmlRead), 	   		     isScriptable	//Remove canXmlRead from UI
...
protected:
    PropertyDescriptor(ClassDescriptor& classDescriptor, const Type& type, const char* name, const char* category, Attributes attributes,
        Security::Permissions security, bool isEnum = false);
...
public:
    ...
    {
        return bIsScriptable != 0;
    }

    void setEditable(bool editable)
    {
        bIsEditable = editable ? 1 : 0;
    }
    inline bool isEditable() const
    {
        return bIsEditable != 0;
    }
    ...
    {
        AYAASSERT(bCanXmlRead == 0 || !isReadOnly());
        return bCanXmlRead != 0;
    }
    inline bool canXmlWrite() const
    {
        AYAASSERT(bCanXmlWrite == 0 || !isWriteOnly());
        return bCanXmlWrite != 0;
    }
    inline bool canReplicate() const
    {
        AYAASSERT(bCanReplicate == 0 || (!isReadOnly() && !isWriteOnly()));
...
public:
    ...
    virtual void read(DescribedBase* instance, const XmlElement* element, Aya::IReferenceBinder& binder) const;

private:
    virtual void writeValue(const DescribedBase* instance, XmlElement* element) const = 0;
    virtual void readValue(DescribedBase* instance, const XmlElement* element, Aya::IReferenceBinder& binder) const = 0;
    //\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\/
};
...
protected:
    ...
    TypedPropertyDescriptor(ClassDescriptor& classDescriptor, const Type& type, const char* name, const char* category, std::auto_ptr<GetSet> getset,
        Attributes flags, Security::Permissions security)
    ...
    TypedPropertyDescriptor(ClassDescriptor& classDescriptor, const char* name, const char* category, std::auto_ptr<GetSet> getset, Attributes flags,
        Security::Permissions security)
        : PropertyDescriptor(classDescriptor, Type::singleton<V>(), name, category, flags, security)
        , getset(getset)
    {
        if (this->getset.get())
            this->checkFlags();
...
public:
    ...
    inline ConstProperty(const PropertyDescriptor& descriptor, const DescribedBase* instance)
        ...
        , instance(instance)
    {
        AYAASSERT(!instance || descriptor.isMemberOf(instance));
    }
    ...
    inline const DescribedBase* getInstance() const
    {
        return instance;
    }
    inline const PropertyDescriptor& getDescriptor() const
    {
        return *descriptor;
    }
...
public:
    ...
    inline void read(const XmlElement* element, Aya::IReferenceBinder& binder)
    {
...
class AyaInterface EnumPropertyDescriptor : public PropertyDescriptor
{
public:
...
protected:
    EnumPropertyDescriptor(ClassDescriptor& classDescriptor, const EnumDescriptor& enumDescriptor, const char* name, const char* category,
        Attributes flags = STANDARD, Security::Permissions security = Security::None)
...
class AyaBaseClass RefPropertyDescriptor : public PropertyDescriptor
{
...
public:
    ...
    RefPropertyDescriptor(ClassDescriptor& classDescriptor, const Type& type, const char* name, const char* category, Attributes flags = STANDARD,
    ...
    {
        static const Aya::Name& name = Aya::Name::lookup("Object");
    ...
    static bool isRefPropertyDescriptor(const PropertyDescriptor& descriptor)
    {
        ...
        AYAASSERT(result == (0 != dynamic_cast<const Reflection::RefPropertyDescriptor*>(&descriptor)));
        return result;
    }
};
...
class BoundProp : public Reflection::TypedPropertyDescriptor<V>
{
    ...
    public:
        BoundPropGetSet(BoundProp& desc, V Class::* member, ChangedMember changed)
            : desc(desc)
            , member(member)
            , changed(changed)
        {
...
public:
    template<class Class>
    BoundProp(const char* name, const char* category, V Class::* member, void (Class::*changed)(const Reflection::PropertyDescriptor&),
        typename PropertyDescriptor::Attributes flags = PropertyDescriptor::STANDARD, Security::Permissions security = Security::None)
        : Reflection::TypedPropertyDescriptor<V>(
              Class::classDescriptor(), name, category, std::auto_ptr<typename TypedPropertyDescriptor<V>::GetSet>(), flags, security)
    {
        this->getset.reset(new BoundPropGetSet<Class>(*this, member, changed));
        this->checkFlags();
    }
    template<class Class>
    BoundProp(const char* name, const char* category, V Class::* member, typename PropertyDescriptor::Attributes flags = PropertyDescriptor::STANDARD,
        Security::Permissions security = Security::None)
        : Reflection::TypedPropertyDescriptor<V>(
              Class::classDescriptor(), name, category, std::auto_ptr<typename TypedPropertyDescriptor<V>::GetSet>(), flags, security)
    {
