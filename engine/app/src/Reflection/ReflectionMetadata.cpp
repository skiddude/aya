...
AYA_REGISTER_CLASS(Metadata::Reflection);
AYA_REGISTER_CLASS(Metadata::Class);
AYA_REGISTER_CLASS(Metadata::Item);
AYA_REGISTER_CLASS(Metadata::Classes);
AYA_REGISTER_CLASS(Metadata::Functions);
AYA_REGISTER_CLASS(Metadata::YieldFunctions);
AYA_REGISTER_CLASS(Metadata::Member);
AYA_REGISTER_CLASS(Metadata::Properties);
AYA_REGISTER_CLASS(Metadata::Events);
AYA_REGISTER_CLASS(Metadata::Callbacks);
AYA_REGISTER_CLASS(Metadata::Enums);
AYA_REGISTER_CLASS(Metadata::Enum);
AYA_REGISTER_CLASS(Metadata::EnumItem);
...
shared_ptr<Metadata::Reflection> Metadata::Reflection::safe_static_do_get_singleton()
{
    ...
    if (!sing)
    {
        sing = Aya::Creatable<Aya::Instance>::create<Reflection>();
        sing->load(GetAssetFolder() + "/ReflectionMetadata.xml");
    }
    return sing;
}
...
public:
    ...
    void writeClass(const Reflection::ClassDescriptor* d)
    {
        if (*d != Reflection::ClassDescriptor::rootDescriptor())
        {
            ...
            std::for_each(d->begin<PropertyDescriptor>(), d->end<PropertyDescriptor>(), boost::bind(&Writer::writeProperty, this, _1, d));

            // Enumerate Functions
            std::for_each(d->begin<FunctionDescriptor>(), d->end<FunctionDescriptor>(), boost::bind(&Writer::writeFunction, this, _1, d));

            // Enumerate Yield-Functions
            std::for_each(
                d->begin<YieldFunctionDescriptor>(), d->end<YieldFunctionDescriptor>(), boost::bind(&Writer::writeYieldFunction, this, _1, d));

            // Enumerate Events
            std::for_each(d->begin<EventDescriptor>(), d->end<EventDescriptor>(), boost::bind(&Writer::writeEvent, this, _1, d));

            // Enumerate Callbacks
            std::for_each(d->begin<CallbackDescriptor>(), d->end<CallbackDescriptor>(), boost::bind(&Writer::writeCallback, this, _1, d));
        }

        // Recurse
        std::for_each(d->derivedClasses_begin(), d->derivedClasses_end(), boost::bind(&Writer::writeClass, this, _1));
    }

    void writeEnum(const EnumDescriptor* d)
    {
        stream << "Enum " << d->name.c_str() << '\n';
        std::for_each(d->begin(), d->end(), boost::bind(&Writer::writeEnumItem, this, _1));
    }
    ...
    void writeAPI()
    {
        ...
        std::for_each(EnumDescriptor::enumsBegin(), EnumDescriptor::enumsEnd(), boost::bind(&Writer::writeEnum, this, _1));
    }

    template<typename T>
    const T* checkConflict(const T* desc, std::map<std::string, const T*>& map)
    {
    ...
    template<typename T>
    void writeConflictingMembers(const ClassDescriptor* desc, std::map<std::string, const MemberDescriptor*>& members)
    {
        const MemberDescriptorContainer<T>* container = desc;

        for (typename MemberDescriptorContainer<T>::Collection::const_iterator it = container->descriptors_begin();
            it != container->descriptors_end(); ++it)
        {
