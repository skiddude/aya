...
#include "Reflection/Descriptor.hpp"
...
public:
    ...
    template<>
    const Type& getSingleton<void>();

    template<>
    const Type& getSingleton<std::string>();
...
protected:
    ...
    Type(const char* name, T* dummy)
        ...
        , tag(Name::lookup(name))
        , isNumber(boost::is_arithmetic<T>::value)
        , isFloat(boost::is_floating_point<T>::value) // Corrected from is_float to is_floating_point
        , isEnum(false)
    {
        *isOutdated = false;
        *isReplicable = true;
...
std::ostream& operator<<(std::ostream& os, const Aya::Reflection::Type& type);

// Handy macro for registering a type
#define AYA_REGISTER_TYPE(mType) \
    template<> \
    Aya::Reflection::TypeRegistrar<mType> Aya::Reflection::TypeRegistrar<mType>::registrar(0)
...
class Variant
{
    ...
    const Type* _type;
    Aya::placement_any<Storage> value;
...
struct Tuple
{
    ...
    const Variant& at(size_t i) const
    {
        return values[i];
    }
};
...
public:
    ...
    public:
        Item(const Aya::Name* name, const Type* type, const Variant& defaultValue);
        Item(const Aya::Name* name, const Type* type);
        const Aya::Name* name;
        const Type* type;
    ...
    void addArgument(const Aya::Name& name, const Type& type);
    void addArgument(const Aya::Name& name, const Type& type, const Variant& defaultValue);

    SignatureDescriptor();
};

template<class ValueType>
ValueType& Aya::Reflection::Variant::genericConvert()
{
    ...
    throw Aya::runtime_error("Unable to cast %s to %s", _type->tag.c_str(), Type::singleton<ValueType>().tag.c_str());
}
} // namespace Reflection
} // namespace Aya
