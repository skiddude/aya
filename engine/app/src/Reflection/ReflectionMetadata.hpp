...
class Reflection : public DescribedCreatable<Reflection, Instance, sReflection>
{
    ...
    static void safe_static_init_singleton()
    {
        safe_static_do_get_singleton();
    }
...
public:
    ...
    {
        return dynamic_cast<const Class*>(getParent());
    };

    int getExplorerOrder() const
    {
        return explorerOrder == 0 ? (getBase() && getBase()->getExplorerOrder()) : explorerOrder;
    }
    ...
    {
        return explorerImageIndex == -1 ? (getBase() && getBase()->getExplorerImageIndex()) : std::max(explorerImageIndex, 0); // don't ever return -1
    }
