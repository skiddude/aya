...
SignatureDescriptor::Item::Item(const Aya::Name* name, const Type* type, const Variant& defaultValue)
    : name(name)
    , type(type)
    , defaultValue(defaultValue)
{
    // Arguments must be camelCase
    AYAASSERT(name->c_str()[0] == tolower(name->c_str()[0]));
}

SignatureDescriptor::Item::Item(const Aya::Name* name, const Type* type)
    : name(name)
    , type(type)
    , defaultValue()
{
    // Arguments must be camelCase
    AYAASSERT(name->c_str()[0] == tolower(name->c_str()[0]));
    AYAASSERT(defaultValue.isVoid());
}

void SignatureDescriptor::addArgument(const Aya::Name& name, const Type& type)
{
    Item i(&name, &type);
    arguments.push_back(i);
}

void SignatureDescriptor::addArgument(const Aya::Name& name, const Type& type, const Variant& defaultValue)
{
    AYAASSERT(defaultValue.isVoid() || defaultValue.type() == type);
