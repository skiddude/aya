...
Reflection::PropDescriptor<Instance, bool> Instance::propArchivable(
    "Archivable", category_Behavior, &Instance::getIsArchivable, &Instance::setIsArchivable, Reflection::PropertyDescriptor::SCRIPTING);
Reflection::PropDescriptor<Instance, bool> propArchivableDeprecated(
    "archivable", category_Behavior, &Instance::getIsArchivable, &Instance::setIsArchivable, Reflection::PropertyDescriptor::LEGACY_SCRIPTING);
...
static Reflection::BoundFuncDesc<Instance, shared_ptr<Instance>(std::string, bool)> findFirstChild(
    &Instance::findFirstChildByName2, "FindFirstChild", "name", "recursive", false, Security::None);
static Reflection::BoundFuncDesc<Instance, shared_ptr<Instance>(std::string, bool)> findFirstChildDeprecated(
    &Instance::findFirstChildByName2, "findFirstChild", "name", "recursive", false, Security::None);
static Reflection::BoundFuncDesc<Instance, shared_ptr<Instance>(std::string)> findFirstChildOfClass(
    &Instance::findFirstChildOfClass, "FindFirstChildOfClass", "name", Security::None);
...
static Reflection::BoundFuncDesc<Instance, void()> dep_Destroy(
    &Instance::destroy, "destroy", Security::None, Reflection::PropertyDescriptor::Attributes::deprecated(func_Destroy));
...
static Reflection::BoundFuncDesc<Instance, shared_ptr<const Instances>()> func_descendants(
    &Instance::getDescendants, "GetDescendants", Security::None);
...
static Reflection::BoundFuncDesc<Instance, bool(shared_ptr<Instance>)> func_isDescendantOf(
    &Instance::isDescendantOf2, "IsDescendantOf", "ancestor", Security::None);
static Reflection::BoundFuncDesc<Instance, bool(shared_ptr<Instance>)> dep_isDescendantOf(&Instance::isDescendantOf2, "isDescendantOf", "ancestor",
    Security::None, Reflection::PropertyDescriptor::Attributes::deprecated(func_isDescendantOf));
static Reflection::BoundFuncDesc<Instance, bool(shared_ptr<Instance>)> func_isAncestorOf(
    &Instance::isAncestorOf2, "IsAncestorOf", "descendant", Security::None);
static Reflection::BoundFuncDesc<Instance, std::string(int)> func_GetReadableId(
    &Instance::getReadableDebugId, "GetDebugId", "scopeLength", 4, Security::Plugin);
static Reflection::PropDescriptor<Instance, std::string> prop_className(
    "ClassName", category_Data, &Instance::getClassNameStr, NULL, Reflection::PropertyDescriptor::UI);
static Reflection::PropDescriptor<Instance, std::string> prop_classNameDeprecated(
    "className", category_Data, &Instance::getClassNameStr, NULL, Reflection::PropertyDescriptor::Attributes::deprecated(prop_className));
static Reflection::PropDescriptor<Instance, int> prop_dataCost(
    "DataCost", category_Data, &Instance::getPersistentDataCost, NULL, Reflection::PropertyDescriptor::UI, Security::RobloxPlace);

static Reflection::BoundYieldFuncDesc<Instance, shared_ptr<Instance>(std::string)> func_WaitForChild(
    &Instance::waitForChild, "WaitForChild", "childName", Security::None);
...
const Reflection::RefPropDescriptor<Instance, Instance> Instance::propParent(
    "Parent", category_Data, &Instance::getParentDangerous, &Instance::setParent, Reflection::PropertyDescriptor::UI);
const Reflection::PropDescriptor<Instance, bool> Instance::propRobloxLocked("RobloxLocked", category_Data, &Instance::getRobloxLocked,
    &Instance::setRobloxLocked, Reflection::PropertyDescriptor::SCRIPTING, Security::Plugin);

Reflection::EventDesc<Instance, void(shared_ptr<Instance>), Aya::signal<void(shared_ptr<Instance>)>,
    Aya::signal<void(shared_ptr<Instance>)>* (Instance::*)(bool)>
...
Reflection::EventDesc<Instance, void(shared_ptr<Instance>, shared_ptr<Instance>)> event_ancestryChanged(
    &Instance::ancestryChangedSignal, "AncestryChanged", "child", "parent");
Reflection::EventDesc<Instance, void(const Reflection::PropertyDescriptor*)> event_propertyChanged(
    &Instance::propertyChangedSignal, "Changed", "property");
REFLECTION_END();

shared_ptr<Instance> Instance::createChild(const Aya::Name& className, Aya::CreatorRole creatorRole)
{
...
void Instance::readChild(const XmlElement* childElement, IReferenceBinder& binder, CreatorRole creatorRole)
{
    ...
    else
    {
        ...
        message += "\"";
        Aya::StandardOut::singleton()->print(Aya::MESSAGE_WARNING, message.c_str());
#endif
...
void Instance::readChildren(const XmlElement* element, IReferenceBinder& binder, Aya::CreatorRole creatorRole)
{
    if (!element)
        return;
...
void Instance::read(const XmlElement* element, IReferenceBinder& binder, Aya::CreatorRole creatorRole)
{
    // Read the referent attribute, if it exists
...
void Instance::writeChildren(XmlElement* container, Aya::CreatorRole creatorRole, const SaveFilter saveFilter)
{
    writeChildren(container, itIsInScope, creatorRole, saveFilter);
}

void Instance::writeChildren(
    XmlElement* container, const boost::function<bool(Instance*)>& isInScope, Aya::CreatorRole creatorRole, const SaveFilter saveFilter)
{
    if (children)
    {
...
void Instance::writeProperties(XmlElement* container) const
{
    ...
    Aya::Reflection::ConstPropertyIterator iter = properties_begin();
...
XmlElement* Instance::writeXml(const boost::function<bool(Instance*)>& isInScope, Aya::CreatorRole creatorRole)
{
    // TODO: archivable==false messes up functions like clone()
    //       find a better way to do this
    if (!getIsArchivable() || (getClassName() == Aya::Name::getNullName()))
        return NULL;
...
ChildAdded::ChildAdded(Instance* child)
    : child(shared_from(child))
{
}
ChildAdded::ChildAdded(const ChildAdded& event)
    : child(event.child)
{
}

ChildRemoved::ChildRemoved(Instance* child)
    : child(shared_from(child))
{
}
ChildRemoved::ChildRemoved(const ChildRemoved& event)
    : child(event.child)
{
}
...
{
    securityCheck(Aya::Security::Context::current());
}

void Instance::securityCheck(Aya::Security::Context& context) const
{
...
public:
    SetParentSentry(Instance* instance)
        : mInstance(instance)
    {
        mInstance->isSettingParent = true;
    }
    ~SetParentSentry()
    {
        mInstance->isSettingParent = false;
    }
...
bool Instance::setParentInternal(Instance* newParent, bool ignoreLock)
{
    ...
    {
        std::string message = Aya::format("The Parent property of %s is locked, current parent: %s, new parent %s", getFullName().c_str(),
            parent ? parent->getName().c_str() : "NULL", newParent ? newParent->getName().c_str() : "NULL");
        throw std::runtime_error(message);
    }

    if (this == newParent)
    {
        std::string message = Aya::format("Attempt to set %s as its own parent", getFullName().c_str());
        throw std::runtime_error(message);
    }
    ...
    {
        AYAASSERT(newParent);
        std::string message = Aya::format(
            "Attempt to set parent of %s to %s would result in circular reference", getFullName().c_str(), newParent->getFullName().c_str());
        throw std::runtime_error(message);
    }
    ...
    if (isSettingParent)
    {
        ...
        std::string message =
            Aya::format("Something unexpectedly tried to set the parent of %s to %s while trying to set the parent of %s. Current parent is %s.",
...
shared_ptr<const Instances> Instance::getDescendants()
{
    if (children)
    {
        copy_on_write_ptr<Instances> descendants;
        return getDescendantsRecursive(descendants.write());
    }
    return NULL;
}

shared_ptr<const Instances> Instance::getDescendantsRecursive(shared_ptr<Instances> descendants)
{
    if (children)
    {
        ...
        {
            Instance* i = iter->get();
            descendants->push_back(shared_from(i));
            i->getDescendantsRecursive(descendants);
        }
        return descendants;
    }
    return NULL;
...
XmlElement* Instance::toNewXmlRoot(Instance* instance, Aya::CreatorRole creatorRole)
{
...
public:
    ...
    Cloner(const Instance* instance, CreatorRole creatorRole)
...
private:
    ...
    shared_ptr<Instance> createClones(shared_ptr<const Instance> instance)
    {
        ...
        if (!copy)
            throw std::runtime_error(Aya::format("%s cannot be cloned", instance->getFullName().c_str()));

        AYAASSERT(map.find(instance.get()) == map.end());
    ...
    void copyProperties(const Map::value_type& value)
    {
        ...
        std::for_each(source->properties_begin(), source->properties_end(), boost::bind(&Cloner::copyProperty, this, _1, source, value.second.get()));
    }

    // Copy non-Parent serializable property
    void copyProperty(const Aya::Reflection::ConstProperty& prop, const Instance* source, Instance* destination)
    {
        ...
        AYAASSERT(source == prop.getInstance());
    ...
    void setParent(shared_ptr<const Instance> source)
    {
        ...
        AYAASSERT(source->getParent());
        AYAASSERT(map.find(source->getParent()) != map.end());
    ...
    void notifyCloning(const Map::value_type& value)
    {
        ...
        if (source->onDemandRead())
            const_cast<Aya::Instance*>(source)->onDemandWrite()->instanceClonedSignal(value.second);
    }
};
...
{
    return clone(Aya::ScriptingCreator);
}


// Declare some dictionary entries
static const Aya::Name& nameName = Aya::Name::declare("Name");
static const Aya::Name& nameParent = Aya::Name::declare("Parent");
...
void Instance::destroy()
{
    if (getIsParentLocked() && getParent())
        throw Aya::runtime_error("The Parent property of %s is locked", getFullName().c_str());
    ...
    //    i:Remove()
    // end
    std::for_each(getDescriptor().begin<Reflection::EventDescriptor>(), getDescriptor().end<Reflection::EventDescriptor>(),
...
shared_ptr<Aya::signal<void()>> Instance::getPropertyChangedSignal(std::string propertyName)
{
    OnDemandInstance* onDemand = onDemandWrite();

    // Check if signal already exists for this property
    OnDemandInstance::PropertyChangedSignalMap::iterator it = onDemand->propertyChangedSignals.find(propertyName);
    if (it != onDemand->propertyChangedSignals.end())
    {
        return it->second;
    }

    // Create a new signal for this property
    shared_ptr<Aya::signal<void()>> signal = Aya::make_shared<Aya::signal<void()>>();
    onDemand->propertyChangedSignals[propertyName] = signal;
    return signal;
}
...
void Instance::raisePropertyChanged(const Aya::Reflection::PropertyDescriptor& descriptor)
{
    PropertyChanged event(Aya::Reflection::Property(descriptor, this));
    ...
    // Fire property-specific signal if it exists
    if (onDemandRead())
    {
        OnDemandInstance* onDemand = onDemandWrite();
        OnDemandInstance::PropertyChangedSignalMap::iterator it = onDemand->propertyChangedSignals.find(descriptor.name.toString());
        if (it != onDemand->propertyChangedSignals.end())
        {
            (*it->second)();
        }
...
void Instance::raiseEventInvocation(
...
const void* Instance::getSetParentAddr()
{
    ...
    bool (Aya::Instance::*thisPmfn)(Aya::Instance*, bool) = &setParentInternal;
...
#else
volatile const uintptr_t rbxTextEndNeg = 0x140000000;
#endif
