...
private:
    ...
    mutable std::map<const Aya::Name*, shared_ptr<Instance>> serviceMap;

public:
    Aya::signal<void()> closingSignal;
    Aya::signal<void()> closingLateSignal;
    Aya::signal<void(shared_ptr<Instance> service)> serviceAddedSignal;
    Aya::signal<void(shared_ptr<Instance> service)> serviceRemovingSignal;
    ...
    // mutable Aya::reentrant_concurrency_catcher threadGuard;
    template<class ServiceClass>
    ServiceClass* create() const
    {
        // Aya::reentrant_concurrency_catcher::scoped_lock lock(threadGuard);
        ...
        if (service == NULL)
        {
            ...
            AYAASSERT((ServiceClass::className() == Aya::Name::getNullName()) || serviceMap.find(&ServiceClass::className()) != serviceMap.end());
        }
        return service;
    }
    ...
    static shared_ptr<Instance> create(Instance* context, const Aya::Name& name);
    shared_ptr<Instance> getPublicServiceByClassNameString(std::string name);

protected:
    /*override*/ shared_ptr<Instance> createChild(const Aya::Name& className, Aya::CreatorRole creatorRole);
...
private:
    ...
    shared_ptr<Instance> findServiceByClassName(const Aya::Name& className) const;
