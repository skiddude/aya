...
Reflection::BoundFuncDesc<ServiceProvider, shared_ptr<Instance>(std::string)> ServiceProvider::func_FindService(
    &ServiceProvider::findPublicServiceByClassNameString, "FindService", "className", Security::None);
Reflection::BoundFuncDesc<ServiceProvider, shared_ptr<Instance>(std::string)> ServiceProvider::func_GetService(
    &ServiceProvider::getPublicServiceByClassNameString, "GetService", "className", Security::None);
Reflection::BoundFuncDesc<ServiceProvider, shared_ptr<Instance>(std::string)> ServiceProvider::dep_service(
    &ServiceProvider::getPublicServiceByClassNameString, "service", "className", Security::None,
    Reflection::Descriptor::Attributes::deprecated(ServiceProvider::func_GetService));
Reflection::BoundFuncDesc<ServiceProvider, shared_ptr<Instance>(std::string)> ServiceProvider::dep_GetService(
    &ServiceProvider::getPublicServiceByClassNameString, "getService", "className", Security::None,
    Reflection::Descriptor::Attributes::deprecated(ServiceProvider::func_GetService));
...
static Reflection::EventDesc<ServiceProvider, void(shared_ptr<Instance>)> event_ServiceAdded(
    &ServiceProvider::serviceAddedSignal, "ServiceAdded", "service");
static Reflection::EventDesc<ServiceProvider, void(shared_ptr<Instance>)> event_ServiceRemoving(
    &ServiceProvider::serviceRemovingSignal, "ServiceRemoving", "service");
REFLECTION_END();

ServiceProvider::ServiceProvider() {}

ServiceProvider::ServiceProvider(const char* name)
    : Super(name)
{
}
...
{
    static Aya::atomic<int> index(-1);
    return ++index;
}

shared_ptr<Instance> ServiceProvider::getPublicServiceByClassNameString(std::string sName)
{
    const Name& serviceName = Aya::Name::lookup(sName);
    if (serviceName.empty())
        throw Aya::runtime_error("'%s' is not a valid Service name", sName.c_str());
...
shared_ptr<Instance> ServiceProvider::findServiceByClassName(const Aya::Name& className) const
{
    std::map<const Aya::Name*, shared_ptr<Instance>>::iterator iter = serviceMap.find(&className);
...
{
    std::map<const Aya::Name*, shared_ptr<Instance>>::iterator iter = serviceMap.find(&instance->getClassName());
    if (iter != serviceMap.end())
...
void ServiceProvider::onChildAdded(Instance* instance)
{
    ...
    AYAASSERT(Instance::fastDynamicCast<ServiceProvider>(instance) == NULL);
    ...
    if (service != NULL)
    {
        ...
        if (instance->getClassName() != Aya::Name::getNullName())
        {
            AYAASSERT_SLOW(findServiceByClassName(instance->getClassName()) == NULL);
            instance->lockParent();
...
shared_ptr<Instance> ServiceProvider::createChild(const Aya::Name& className, Aya::CreatorRole creatorRole)
{
...
shared_ptr<Instance> ServiceProvider::create(Instance* context, const Aya::Name& name)
{
    while (context != NULL)
    {
