...
bool HandlesBase::findTargetHandle(const shared_ptr<InputObject>& inputObject, Vector3& hitPointWorld, NormalId& hitNormalId)
{
    if (PartInstance* part = adornee.lock().get())
    {
        ...
        if (HandleHitTest::hitTestHandleLocal(part->getPartPrimitive()->getExtentsLocal(), part->getLocation(), getHandleType(), gridRay,
                hitPointWorld, hitNormalId, getHandlesNormalIdMask()))
        {
            return true;
        }
    }
...
bool HandlesBase::getDistanceFromHandle(
    const shared_ptr<InputObject>& inputObject, NormalId localNormalId, const Vector3& hitPointWorld, float& distance)
{
    if (PartInstance* part = adornee.lock().get())
    {
        Workspace* workspace = Workspace::findWorkspace(this);
        ...
        RbxRay axisRay = Aya::RbxRay::fromOriginAndDirection(hitPointWorld, worldNormal);
        RbxRay gridRay = MouseCommand::getUnitMouseRay(inputObject, workspace);
...
bool HandlesBase::getFacePosFromHandle(
    const shared_ptr<InputObject>& inputObject, NormalId faceId, const Vector3& hitPointWorld, Vector2& relativePos, Vector2& absolutePos)
{
    if (PartInstance* part = adornee.lock().get())
    {
        Workspace* workspace = Workspace::findWorkspace(this);
...
bool HandlesBase::getAngleRadiusFromHandle(const shared_ptr<InputObject>& inputObject, NormalId faceId, const Vector3& hitPointWorld, float& angle,
    float& radius, float& absangle, float& absradius)
{
    Vector2 relpos;
    Vector2 abspos;
...
void HandlesBase::render2d(Adorn* adorn)
{
    ...
    if (part)
    {
        DrawAdorn::handles2d(part->getPartSizeXml(), part->getCoordinateFrame(), *(workspace->getConstCamera()), adorn, getHandleType(), color, false,
...
void HandlesBase::render3dAdorn(Adorn* adorn)
{
    ...
    if (mouseDownCaptureInfo && mouseDownCaptureInfo->hitNormalId != NORM_UNDEFINED)
    {
    ...
    if (part)
    {
        Aya::DrawAdorn::handles3d(part->getPartSizeXml(), part->getCoordinateFrame(), adorn, getHandleType(),
            workspace->getConstCamera()->coordinateFrame().translation, color, getHandleType() == HANDLE_ROTATE, getHandlesNormalIdMask(),
            highlightedNormalId);
    }
}
