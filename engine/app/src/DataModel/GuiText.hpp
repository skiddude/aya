...
public:
    ...
    TextService::FontSize getFontSize() const
    {
        return fontSize;
    }
    ...
    TextService::Font getFont() const
    {
        return font;
    }

    BrickColor getTextColor() const
    {
        return BrickColor::closest(textColor);
    }

    Color3 getTextColor3() const
    {
        return textColor;
    }

    float getTextTransparency() const
    {
        return textTransparency;
    }

    float getTextStrokeTransparency() const
    {
        return textStrokeTransparency;
    }

    Color3 getTextStrokeColor3() const
    {
    ...
    bool getTextScale() const
    {
        return textScale;
    }

    TextService::XAlignment getXAlignment() const
    {
        return xAlignment;
    }

    TextService::YAlignment getYAlignment() const
    {
        return yAlignment;
    }
...
#define DECLARE_GUI_TEXT_MIXIN() \
    ...
    int getPosInString(Aya::Vector2 cursorPos) const; \
...
\
    ...
    static const Reflection::PropDescriptor<Class, float> prop_TextSize("TextSize", category_Text, &Class::getTextSize, &Class::setTextSize, Reflection::PropertyDescriptor::STANDARD); \
...
\
    void Class::setTextSize(float value) \
    { \
        if (GuiTextMixin::textSize != value) \
        { \
            GuiTextMixin::textSize = value; \
        ...
        if (Network::Players::frontendProcessing(this, false)) \
            ...
            { \
                if (Typesetter* typesetter = textService->getTypesetter(GuiTextMixin::font)) \
                    return typesetter->measure(GuiTextMixin::text, GuiObject::convertFontSize(GuiTextMixin::fontSize), GuiTextMixin::textWrap ? getRect2D().wh() : Vector2::zero()); \
            } \
\
        return Vector2::zero(); \
    } \
\
    int Class::getPosInString(Aya::Vector2 cursorPos) const \
    { \
        if (Network::Players::frontendProcessing(this, false)) \
            if (TextService* textService = ServiceProvider::create<TextService>(this)) \
            { \
...
\
                    ...
                    return typesetter->getCursorPositionInText( \
                        getText(), pos, GuiObject::convertFontSize(GuiTextMixin::fontSize), TextService::ToTextXAlign(GuiTextMixin::getXAlignment()), TextService::ToTextYAlign(GuiTextMixin::getYAlignment()), GuiTextMixin::textWrap ? getRect2D().wh() : Vector2::zero(), absoluteRotation, cursorPos); \
                } \
            } \
\
        return -1; \
    ...
    { \
        if (Network::Players::frontendProcessing(this, false)) \
            ...
            { \
                ...
                { \
                    bool result = false; \
                    Vector2 bounds = typesetter->measure(GuiTextMixin::text, GuiObject::convertFontSize(GuiTextMixin::fontSize), GuiTextMixin::textWrap ? getRect2D().wh() : Vector2::zero(), &result); \
