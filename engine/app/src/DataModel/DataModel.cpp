...
#include "DataModel/DataModel.hpp" // TODO - minimize these includes, and in the .h file
...
#include "World/ContactManager.hpp" // for fastclear
...
#include "Utility/SimSendFilter.hpp"
...
FASTFLAGVARIABLE(LegacyMessageBox, false)
...
static void dummyLoader(Aya::DataModel*) {}
...
boost::function<void(Aya::DataModel*)> DataModel::loaderFunc = boost::function<void(Aya::DataModel*)>(dummyLoader);


static Reflection::BoundFuncDesc<DataModel, void()> func_loadPlugins(&DataModel::loadPlugins, "LoadPlugins", Security::Roblox);

Reflection::EventDesc<DataModel, void(shared_ptr<Instance>, const Reflection::PropertyDescriptor*)> event_ItemChanged(
    &DataModel::itemChangedSignal, "ItemChanged", "object", "descriptor");

static Reflection::BoundFuncDesc<DataModel, shared_ptr<const Instances>(ContentId)> getContentFunctionOld(
    &DataModel::fetchAsset, "get", "url", Security::LocalUser);
static Reflection::BoundFuncDesc<DataModel, shared_ptr<const Instances>(ContentId)> getContentFunction(
    &DataModel::fetchAsset, "GetObjects", "url", Security::Plugin);

static const Reflection::BoundYieldFuncDesc<DataModel, bool(Instance::SaveFilter)> savePlaceAsyncFunction(
    &DataModel::savePlaceAsync, "SavePlace", "saveFilter", DataModel::SAVE_ALL, Security::None);
...
static Reflection::BoundFuncDesc<DataModel, void(bool)> setRemoteBuildMode(
    &DataModel::setRemoteBuildMode, "SetRemoteBuildMode", "buildModeEnabled", Security::LocalUser);
...
static Reflection::BoundFuncDesc<DataModel, void(std::string)> setServerSaveUrlFunction(
    &DataModel::setServerSaveUrl, "SetServerSaveUrl", "url", Security::LocalUser);
...
static Reflection::BoundYieldFuncDesc<DataModel, std::string(std::string)> httpGetAsyncFunction(
    &DataModel::httpGetAsync, "HttpGetAsync", "url", kHttpPermission);
static Reflection::BoundYieldFuncDesc<DataModel, std::string(std::string, std::string, std::string)> httpPostAsyncFunction(
    &DataModel::httpPostAsync, "HttpPostAsync", "url", "data", "contentType", "*/*", kHttpPermission);
static Reflection::BoundFuncDesc<DataModel, std::string(std::string, bool)> httpGetFunction(
    &DataModel::httpGet, "HttpGet", "url", "synchronous", false, kHttpPermission);
static Reflection::BoundFuncDesc<DataModel, std::string(std::string, std::string, bool, std::string)> httpPostFunction(
    &DataModel::httpPost, "HttpPost", "url", "data", "synchronous", false, "contentType", "*/*", kHttpPermission);

static Reflection::BoundFuncDesc<DataModel, shared_ptr<const Reflection::ValueArray>()> getJobsInfo(
    &DataModel::getJobsInfo, "GetJobsInfo", Security::Plugin);
static Reflection::BoundFuncDesc<DataModel, void(std::string, std::string, std::string, std::string, std::string)> func_reportMeasurement(
    &DataModel::reportMeasurement, "ReportMeasurement", "id", "key1", "value1", "key2", "value2", Security::RobloxScript);

#if defined(AYA_STUDIO) || defined(AYA_SERVER) || defined(AYA_TEST_BUILD)
static Reflection::BoundFuncDesc<DataModel, void(bool)> sanitizeFunction(
    &DataModel::clearContents, "ClearContent", "resettingSimulation", Security::LocalUser);
#endif
...
static Reflection::PropDescriptor<DataModel, bool> prop_isPersonalServer("IsPersonalServer", category_State, &DataModel::getIsPersonalServer,
    &DataModel::setIsPersonalServer, Reflection::PropertyDescriptor::SCRIPTING, Security::RobloxScript);
static Reflection::PropDescriptor<DataModel, bool> prop_canSaveLocal(
    "LocalSaveEnabled", category_State, &DataModel::canSaveLocal, NULL, Reflection::PropertyDescriptor::UI, Security::RobloxScript);
...
static Reflection::BoundFuncDesc<DataModel, void(bool)> finishShutdownFunction(
    &DataModel::completeShutdown, "FinishShutdown", "localSave", Security::RobloxScript);

static Reflection::BoundFuncDesc<DataModel, void(std::string)> func_SetUiMessage(
    &DataModel::setUiMessage, "SetMessage", "message", Security::LocalUser);
...
static Reflection::BoundFuncDesc<DataModel, void()> func_SetUIMessageBrickCount(
    &DataModel::setUiMessageBrickCount, "SetMessageBrickCount", Security::LocalUser);
...
static Reflection::BoundFuncDesc<DataModel, shared_ptr<const Reflection::ValueArray>()> getJobsExtendedStats(
    &DataModel::getJobsExtendedStats, "GetJobsExtendedStats", Security::Plugin);
static Reflection::BoundFuncDesc<DataModel, double(std::string, double)> getJobTimePeakFraction(
    &DataModel::getJobTimePeakFraction, "GetJobTimePeakFraction", "jobname", "greaterThan", Security::Plugin);
static Reflection::BoundFuncDesc<DataModel, double(std::string, double)> getJobIntervalPeakFraction(
    &DataModel::getJobIntervalPeakFraction, "GetJobIntervalPeakFraction", "jobname", "greaterThan", Security::Plugin);
// TODO: Make this part of settings, not per-document. After all, it is a global setting
static Reflection::BoundFuncDesc<DataModel, void(double)> setJobsExtendedStatsWindow(
    &DataModel::setJobsExtendedStatsWindow, "SetJobsExtendedStatsWindow", "seconds", Security::LocalUser);
...
static Reflection::BoundFuncDesc<DataModel, void(int, bool)> setPlaceId(
    &DataModel::setPlaceID, "SetPlaceId", "placeId", "robloxPlace", false, Security::Plugin);
static Reflection::BoundFuncDesc<DataModel, void(int, bool)> setPlaceIdDeprecated(
    &DataModel::setPlaceID, "SetPlaceID", "placeID", "robloxPlace", false, Security::Plugin);
static Reflection::BoundFuncDesc<DataModel, void(std::string)> setGameInstanceId(
    &DataModel::setGameInstanceID, "SetGameInstanceId", "instanceID", Security::Plugin);
...
static Reflection::BoundFuncDesc<DataModel, void(int, DataModel::CreatorType)> setCreatorId(
    &DataModel::setCreatorID, "SetCreatorId", "creatorId", "creatorType", Security::Plugin);
static Reflection::BoundFuncDesc<DataModel, void(int, DataModel::CreatorType)> setCreatorIdDeprecated(
    &DataModel::setCreatorID, "SetCreatorID", "creatorID", "creatorType", Security::Plugin);
...
static Reflection::BoundFuncDesc<DataModel, void(DataModel::GearGenreSetting, int)> func_setGear(
    &DataModel::setGear, "SetGearSettings", "genreRestriction", "allowedGenres", Security::Plugin);

static Reflection::RefPropDescriptor<DataModel, Workspace> prop_Workspace(
    "Workspace", category_Data, &DataModel::getWorkspace, NULL, Reflection::PropertyDescriptor::UI);
static Reflection::RefPropDescriptor<DataModel, Workspace> prop_workspace(
    "workspace", category_Data, &DataModel::getWorkspace, NULL, Reflection::PropertyDescriptor::Attributes::deprecated(prop_Workspace));
Reflection::RefPropDescriptor<DataModel, Instance> DataModel::prop_lighting(
    "lighting", category_Data, &DataModel::getLightingDeprecated, NULL, Reflection::PropertyDescriptor::Attributes::deprecated());

static Reflection::PropDescriptor<DataModel, int> prop_placeId(
    "PlaceId", category_State, &DataModel::getPlaceID, NULL, Reflection::PropertyDescriptor::UI);
static Reflection::PropDescriptor<DataModel, int> prop_placeVersion(
    "PlaceVersion", category_State, &DataModel::getPlaceVersion, NULL, Reflection::PropertyDescriptor::UI);
static Reflection::PropDescriptor<DataModel, int> prop_creatorId(
    "CreatorId", category_State, &DataModel::getCreatorID, NULL, Reflection::PropertyDescriptor::UI);
static Reflection::PropDescriptor<DataModel, bool> prop_forceR15(
    "ForceR15", category_Behavior, &DataModel::getForceR15, &DataModel::setForceR15, Reflection::PropertyDescriptor::UI);
static Reflection::EnumPropDescriptor<DataModel, DataModel::CreatorType> prop_creatorType(
    "CreatorType", category_State, &DataModel::getCreatorType, NULL, Reflection::PropertyDescriptor::UI);
static Reflection::EnumPropDescriptor<DataModel, DataModel::Genre> prop_genre(
    "Genre", category_State, &DataModel::getGenre, NULL, Reflection::PropertyDescriptor::UI);
static Reflection::EnumPropDescriptor<DataModel, DataModel::GearGenreSetting> prop_gearGenreSetting(
    "GearGenreSetting", category_State, &DataModel::getGearGenreSetting, NULL, Reflection::PropertyDescriptor::UI);
static Reflection::BoundFuncDesc<DataModel, bool(DataModel::GearType)> func_isGearTypeAllowed(
    &DataModel::isGearTypeAllowed, "IsGearTypeAllowed", "gearType", Security::None);
...
static Reflection::PropDescriptor<DataModel, std::string> prop_getJobId(
    "JobId", "JobInfo", &DataModel::getJobId, NULL, Reflection::PropertyDescriptor::UI);


static Reflection::EventDesc<DataModel, void(bool)> event_graphicsQualityChangeRequest(
    &DataModel::graphicsQualityShortcutSignal, "GraphicsQualityChangeRequest", "betterQuality");
...
static Reflection::BoundFuncDesc<DataModel, void(std::string, std::string)> addStatFunction(
    &DataModel::addCustomStat, "AddStat", "displayName", "stat", Security::LocalUser);
static Reflection::BoundFuncDesc<DataModel, void(std::string)> removeStatFunction(
    &DataModel::removeCustomStat, "RemoveStat", "stat", Security::LocalUser);
...
static Reflection::BoundAsyncCallbackDesc<DataModel, shared_ptr<const Reflection::Tuple>(void)> callback_onClose(
    "OnClose", &DataModel::onCloseCallback, &DataModel::onCloseCallbackChanged);
...
static Reflection::BoundFuncDesc<DataModel, void(std::string)> func_setVIPServerId(
    &DataModel::setVIPServerId, "SetVIPServerId", "newId", Security::LocalUser);
...
static Reflection::BoundFuncDesc<DataModel, void(int)> func_setVIPServerOwnerId(
    &DataModel::setVIPServerOwnerId, "SetVIPServerOwnerId", "newId", Security::LocalUser);
REFLECTION_END();
...
template<>
GameBasicSettings::VirtualVersion& Variant::convert<GameBasicSettings::VirtualVersion>(void)
{
    return genericConvert<GameBasicSettings::VirtualVersion>();
}
} // namespace Reflection

template<>
bool Aya::StringConverter<DataModel::CreatorType>::convertToValue(const std::string& text, DataModel::CreatorType& value)
{
    return Reflection::EnumDesc<DataModel::CreatorType>::singleton().convertToValue(text.c_str(), value);
}

template<>
bool Aya::StringConverter<GameBasicSettings::VirtualVersion>::convertToValue(const std::string& text, GameBasicSettings::VirtualVersion& value)
{
    return Reflection::EnumDesc<GameBasicSettings::VirtualVersion>::singleton().convertToValue(text.c_str(), value);
}

template<>
bool Aya::StringConverter<DataModel::Genre>::convertToValue(const std::string& text, DataModel::Genre& value)
{
    return Reflection::EnumDesc<DataModel::Genre>::singleton().convertToValue(text.c_str(), value);
}

template<>
bool Aya::StringConverter<DataModel::GearGenreSetting>::convertToValue(const std::string& text, DataModel::GearGenreSetting& value)
{
    return Reflection::EnumDesc<DataModel::GearGenreSetting>::singleton().convertToValue(text.c_str(), value);
}

template<>
bool Aya::StringConverter<DataModel::GearType>::convertToValue(const std::string& text, DataModel::GearType& value)
{
    return Reflection::EnumDesc<DataModel::GearType>::singleton().convertToValue(text.c_str(), value);
}

template<>
bool Aya::StringConverter<DataModel::SaveFilter>::convertToValue(const std::string& text, DataModel::SaveFilter& value)
{
...
public:
    ...
    Aya::timestamped_safe_queue<Task> tasks;
...
private:
    ...
    void processTasks()
    {
        ...
        while ((count < size) && (tasks.pop_if_present(task))) // need to keep count because task can add more tasks
        {
            ...
            Aya::Time timeNow = Aya::Time::now<Aya::Time::Fast>();
...
{
    static Aya::atomic<int> count = 0;
    return Aya::format("DataModel-%d", (int)++count);
}

bool DataModel::canSave(const Aya::Instance* instance)
{
...
bool DataModel::serverSavePlace(
...
DataModel::RequestShutdownResult DataModel::requestShutdown(bool useLuaShutdownForSave)
{
    ...
    Visit* visit = Aya::ServiceProvider::find<Visit>(this);
...
void DataModel::doDataModelSetup(shared_ptr<DataModel> dataModel, bool startHeartbeat, bool loadingScreen)
{
    ...
    Aya::DataModel::LegacyLock::mainThreadId = GetCurrentThreadId();
    ...
    if (loadingScreen && !(TeleportService::didTeleport() && TeleportService::getCustomTeleportLoadingGui()))
    {
        try
        {
            if (boost::optional<ProtectedString> source = CoreScript::fetchSource("LoadingScript"))
            {
                ...
                {
                    sc->executeInNewThread(Aya::Security::RobloxGameScript_, *source, "LoadingScript");
                }
            }
        }
...
shared_ptr<DataModel> DataModel::createDataModel(
    bool startHeartbeat, Aya::Verb* lockVerb, bool loading, GameBasicSettings::VirtualVersion virtualVersion)
{
    FASTLOG1(FLog::CloseDataModel, "Create DataModel - heartbeat(%d)", startHeartbeat);

    shared_ptr<DataModel> dataModel = Creatable<Instance>::create<Aya::DataModel>(lockVerb);
    dataModel->setInitialVersion(virtualVersion);
    dataModel->setShouldShowLoadingScreen(loading);

    doDataModelSetup(dataModel, startHeartbeat, loading);
    return dataModel;
}
...
bool DataModel::onlyJobsLeftForThisArbiterAreGenericJobs()
{
    ...
    std::vector<boost::shared_ptr<const TaskScheduler::Job>>::iterator itr = jobs.begin();
...
void DataModel::doCloseDataModel(shared_ptr<DataModel> dataModel)
{
    ...
    AYAASSERT(dataModel->isInitialized);
    ...
    AYAASSERT(dataModel->onlyJobsLeftForThisArbiterAreGenericJobs());

    FASTLOG(FLog::CloseDataModel, "Removing GenericJobs...");
    {
        bool blockingRemove = DataModel::BlockingDataModelShutdown && Aya::DebugSettings::singleton().blockingRemove;
...
void DataModel::closeDataModel(shared_ptr<DataModel> dataModel)
{
    ...
    if (dataModel->onCloseCallback)
    {
        ...
        AYAASSERT(!dataModel->currentThreadHasWriteLock());
...
void DataModel::onCloseCallbackChanged(const CloseCallback& oldValue)
{
    if (onCloseCallback && oldValue)
    {
        ...
        throw Aya::runtime_error("OnClose is already set");
    }
}
...
DataModel::DataModel(Aya::Verb* lockVerb)
    : Super("Game")
    , VerbContainer(NULL)
...
void DataModel::initializeContents(bool startHeartbeat)
{
    ...
    ServiceProvider::create<AyaService>();
    ...
    if (TeleportService* ts = ServiceProvider::create<TeleportService>(this))
    {
        ...
        {
            if (Aya::ScreenGui* loadingGui = Instance::fastDynamicCast<Aya::ScreenGui>(TeleportService::getCustomTeleportLoadingGui().get()))
            {
...
Aya::atomic<int> DataModel::count;

DataModel::~DataModel()
{
    count--;
    if (placeID != 0)
        AyaDbgInfo::RemovePlace(placeID);
}
...
struct DataModel::LegacyLock::Implementation : boost::noncopyable
{
    ...
    SAFE_STATIC(Aya::safe_queue<shared_ptr<Events>>, eventsPool)
    ...
    SAFE_STATIC(Aya::thread_specific_reference<GenericJob>, currentJob)
    ...
    {
        AYAPROFILER_SCOPE("Jobs", "LegacyLock");
...
{
    Aya::mutex::scoped_lock lock(genericJobsLock);
...
void DataModel::saveToRoblox(boost::function<void(bool)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    if (canSave(this))
    {
        if (Visit* visit = Aya::ServiceProvider::find<Visit>(this))
        {
...
void DataModel::HttpHelper(std::string* response, std::exception* exception, boost::function<void(std::string)> resumeFunction,
...
{
    Aya::Http(url).get(boost::bind(&HttpHelper, _1, _2, resumeFunction, errorFunction));
}
...
void DataModel::doHttpPost(const std::string& url, const std::string& data, const std::string& contentType,
    boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    Http(url).post(data, contentType, data.size() > 256, boost::bind(&HttpHelper, _1, _2, resumeFunction, errorFunction));
}
...
{
    AYAASSERT(isInitialized); //  Show to David or Erik - threading issue
    robloxScriptModifiedCheck(Aya::httpGetFunction.security);
    if (url.size() > 0)
    {
        if (synchronous)
        {
...
void DataModel::httpGetAsync(std::string url, boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    ...
    robloxScriptModifiedCheck(Aya::httpGetAsyncFunction.security);
...
std::string DataModel::httpPost(std::string url, std::string data, bool synchronous, std::string optionalContentType)
{
    ...
    robloxScriptModifiedCheck(Aya::httpPostFunction.security);
    if (synchronous)
    {
...
void DataModel::httpPostAsync(std::string url, std::string data, std::string optionalContentType, boost::function<void(std::string)> resumeFunction,
    boost::function<void(std::string)> errorFunction)
{
    AYAASSERT(isInitialized); //  Show to David or Erik - threading issue
    ...
    robloxScriptModifiedCheck(Aya::httpPostAsyncFunction.security);
...
std::auto_ptr<std::istream> DataModel::loadAssetIdIntoStream(int assetID)
{
    ...
    StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "DataModel loading from: %s", url.c_str());
...
void DataModel::loadContent(ContentId contentId)
{
    ...
    StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "DataModel Loading %s", contentId.c_str());
...
void DataModel::processAfterLoad()
{
    ...
    if (DFFlag::MaterialPropertiesEnabled)
    {
        ...
        {
            Aya::PartInstance::convertToNewPhysicalPropRecursive(this);
        }
    }
}
...
{
    AYAASSERT(isInitialized); // If hit show to David or Erik - threading issue

#ifdef AYA_PLAYER
    if (Security::Context::current().identity != Security::RobloxGameScript_)
    {
        throw std::runtime_error("Can't insert at this time");
    }
#endif

    Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "contentId: %s", contentId.c_str());
...
{
    Aya::Network::Player* player = Aya::Network::Players::findLocalPlayer(this);
    if (player)
    {
...
void DataModel::renderDebugMenu()
{
    if (debugMenu)
        debugMenu->render();
}
...
std::string DataModel::getUpdatedMessageBoxText()
{
    ...
    {
        printMessage = "Bricks: " + getMetric("numPrimitives") + "   Connectors: " + getMetric("numJoints");

        // d9mz - weird char padding effect going on here
        // too lazy to fix it
        MegaClusterInstance* terrain = Instance::fastDynamicCast<MegaClusterInstance>(workspace->getTerrain());
        ...
        if (cellCount > 0)
        {
            ...
            oss << std::fixed << std::setprecision(3) << cellCount / 1000000.0f << " mil";
...
bool DataModel::canRenderMouse()
{
    ...
    bool playerExists = (Aya::Network::Players::findLocalPlayer(this) != NULL);
    ...
    return (!Aya::MouseCommand::isAdvArrowToolEnabled() || playerExists) && userInputService->getMouseEnabled();
}

void DataModel::renderMessageBox(Adorn* adorn)
{
    std::string messageBoxText = getUpdatedMessageBoxText();
    ...
    // box size
    const float maxWidth = 540.0f;
    const float maxHeight = 165.0f;
    float rectWidth = std::min(width * 0.5f, maxWidth);
    float rectHeight = std::min(height * 0.25f, maxHeight);

    // draw box
    float left = centerX - rectWidth / 2;
    float top = (centerY - rectHeight / 2) - 16;
    float right = centerX + rectWidth / 2;
    float bottom = centerY + rectHeight / 2;

    // draw box
    Rect2D rect = Rect2D::xyxy(left, top, right, bottom);
    G3D::Color4 grayColor(0.1f, 0.1f, 0.1f, 0.6f);
    adorn->rect2d(rect, grayColor);

    // draw text
    G3D::Color4 whiteColor(1.0f, 1.0f, 1.0f, 1.0f);
    G3D::Color4 transparent(1.0f, 1.0f, 1.0f, 0.0f);
    Vector2 textPosition(centerX, centerY - adorn->get2DStringBounds(messageBoxText, 12).y / 2);
    adorn->drawFont2D(
        messageBoxText, textPosition, 20.5f, true, whiteColor, transparent, Text::FONT_ARIALBOLD, Text::XALIGN_CENTER, Text::YALIGN_CENTER);
}

void DataModel::renderPass2d(Adorn* adorn, IMetric* graphicsMetric)
{
    AYAPROFILER_SCOPE("Render", "Pass2d");
    ...
    if (!DFFlag::AllowHideHudShortcut || renderGuisActive)
    {
        ...
        // 3. Render the Aya gui elements
        coreGuiService->render2d(adorn);

        // 4. Render debug menu
        renderDebugMenu();

        // 5. Render old school MessageBox
        // ignore for 2014+
        // exception: if you're on 2016 and the initialvirtualversion is 2013 or something
        if (showLoadingScreen())
        {
            GameBasicSettings::VirtualVersion vv = GameBasicSettings::singleton().getVirtualVersion();
            if (initialVirtualVersion >= GameBasicSettings::VERSION_2014 && vv >= GameBasicSettings::VERSION_2014 && showLoadingScreen())
                renderMessageBox(adorn);
        }

        // 7. Finally, render the mouse
        renderMouse(adorn);
    }
...
ContentId DataModel::getRenderMouseCursor()
{
    if (forceArrowCursor)
    {
        if (!MouseCommand::isAdvArrowToolEnabled() || (Aya::Network::Players::findLocalPlayer(this) || Network::Players::serverIsPresent(this)))
        {
...
{
    AYAPROFILER_SCOPE("Render", "Pass3dAdorn");
    ...
    {
        ServiceProvider::create<Aya::Network::Players>(this)->renderDPhysicsRegions(adorn);
    }
...
void DataModel::renderStep(float timeIntervalSeconds)
{
    ...
    if (FFlag::RelativisticCameraFixEnable && workspace)
    {
        if (Camera* camera = workspace->getCamera())
        {
            ...
            //
            // Camera was being updated after the UI processing occurred, however the SurfaceGUI processing (which is part of the UI pipeline)
            // required updated camera to perform raycasts. This resulted in a bug where the raycast would use the camera from previous frame - not
            // good if the camera was attached to a moving object with SurfaceGUIs on it.
            //
            // The correct way would be to split out the UI processing that modifies the camera (e.g. movement, panning) into a separate call and do
            // that before the camera update, and perform the rest afterwards.
            //
...
// Instead use this on XCode http://developer.apple.com/library/ios/#documentation/Performance/Conceptual/ManagingMemory/Articles/MallocDebug.html
#ifdef _WIN32
    AYAASSERT_IF_VALIDATING(_CrtCheckMemory());
#endif
    bool isCyclicExecutive = Aya::TaskScheduler::singleton().isCyclicExecutive();
    ...
    // Physics Step
    Aya::Network::GameMode gameMode = Network::Players::getGameMode(this);
    if (updatePhysicsInstructions(gameMode))
    {
        ...
        {
            Aya::Profiling::Mark mark2(*workspace->profileWorkspaceAssemble, true);
            workspace
                ->assemble(); // must do assembly here, because "handle fallen parts" may disassemble.  Also, runService->gameStepped may disassemble
        }
        {
            ...
            FASTLOG3F(
                FLog::CyclicExecutiveThrottling, "DataModel::PhysicsStep dt: %f, dtCyclicExecutive: %f, dutyDt: %f", dt, dtCyclicExecutive, dutyDt);
            if (isCyclicExecutive)
            {
    ...
    if (gameMode == Aya::Network::DPHYS_GAME_SERVER && (physicsStepID % 4) == 0)
    {
...
bool DataModel::updatePhysicsInstructions(Network::GameMode gameMode)
{
    ...
    simSendFilter.networkAddress = Aya::Network::Players::findLocalSimulatorAddress(this);
    ...
    case Network::EDIT:
    {
        AYAASSERT(simSendFilter.networkAddress == Network::NetworkOwner::Unassigned());
    ...
    {
        AYAASSERT(simSendFilter.networkAddress == Network::NetworkOwner::Server());
    ...
    {
        AYAASSERT(Network::Players::clientIsPresent(this, true));
        simSendFilter.mode = SimSendFilter::dPhysClient;
        ServiceProvider::create<Aya::Network::Players>(this)->buildClientRegion(simSendFilter.region);
...
void DataModel::checkFetchExperimentalFeatures()
{
    ...
    {
        apiService->get(Aya::format("game/GetAllowedExperimentalFeatures?placeId=%i", getPlaceID()), true, PRIORITY_EXTREME, response);
    }
...
GuiResponse DataModel::processAccelerators(const shared_ptr<InputObject>& event) // true if used the event
{
    ...
    case InputObject::TYPE_KEYBOARD:
    {
        ...
        if (event->isKeyUpEvent())
        {
            ...
            case AYA_SDLK_LALT:
            case AYA_SDLK_RALT:
                Aya::GameBasicSettings::singleton().setFreeLook(false);
                break;
            default:
                Aya::GameBasicSettings::singleton().setFreeLook(false);
                break;
...

        {
            ...
            else
            {
                ...
                case AYA_SDLK_o:
                    ...
                    break;
                case AYA_SDLK_COMMA:
                    // Camera panning is hard-coded to be always enabled, so we just do a quick check before we get a camera panning verb, so this
                    // doesn't break the Scriptable Camera
                ...
                case AYA_SDLK_F1:
                    if (!Aya::GameBasicSettings::singleton().inStudioMode())
                    {
                        if (event->mod == 0)
                ...
                case AYA_SDLK_F2:
                    if (!Aya::GameBasicSettings::singleton().inStudioMode() && event->isKeyPressedWithShiftEvent(event->getKeyCode()))
                ...
                case AYA_SDLK_F5: // TODO: use for run command in studio
                    if (!Aya::GameBasicSettings::singleton().inStudioMode() && event->isKeyPressedWithShiftEvent(event->getKeyCode()))
                ...
                case AYA_SDLK_F6:
                    ...
                    break;
                case AYA_SDLK_F7:
                    if (DFFlag::AllowHideHudShortcut && !Aya::GameBasicSettings::singleton().getUsedHideHudShortcut())
                    {
                        if (Aya::GuiService* guiService = Aya::ServiceProvider::find<Aya::GuiService>(this))
                        {
                            if (!guiService->notificationCallback.empty())
                            {
                                Aya::GameBasicSettings::singleton().setUsedHideHudShortcut(true);
                                guiService->notificationCallback("HUD Hidden", "Press F7 again when you want to unhide all GUIs",
                                    boost::bind(notificationCallbackSuccess, this), boost::bind(notificationCallbackError, _1));
                            }
                        }
                    }
                    else
                ...
                case AYA_SDLK_SYSREQ: // This is technically print ...... directx doesn't have a print key code :(
                case AYA_SDLK_PRINT:
                    verb = getWhitelistVerb("", "Screen", "shot");
                    break;
                case AYA_SDLK_LALT:
                case AYA_SDLK_RALT:
                    Aya::GameBasicSettings::singleton().setFreeLook(
                        true); // if we are in camlock, holding alt (option on mac) allows us to pan camera freely
                    break;
...
void DataModel::setInitialScreenSize(Aya::Vector2 newScreenSize)
{
    if (shared_ptr<Aya::ScreenGui> coreScreenGui = coreGuiService->getRobloxScreenGui())
    {
...
{
    if (GamepadService* gamepadService = ServiceProvider::find<Aya::GamepadService>(this))
    {
...
GuiResponse DataModel::processProfilerEvent(const shared_ptr<InputObject>& event)
{
    if (event->isKeyDownEvent())
    {
        unsigned int ctrlOrCmd = AYA_KMOD_LCTRL | AYA_KMOD_RCTRL | AYA_KMOD_LMETA | AYA_KMOD_RMETA;

        if (event->isKeyDownEvent(AYA_SDLK_F6) && (event->mod & ctrlOrCmd))
            return Profiler::toggleVisible() ? GuiResponse::sunk() : GuiResponse::notSunk();

        if (event->isKeyDownEvent(AYA_SDLK_p) && (event->mod & ctrlOrCmd))
    ...
    if (event->isMouseEvent())
    {
        ...
        // imgui takes precedence...
        return ImGui::handleMouse(flags, mouseX, mouseY, mouseWheel, mouseButton)      ? GuiResponse::sunk()
               : Profiler::handleMouse(flags, mouseX, mouseY, mouseWheel, mouseButton) ? GuiResponse::sunk()
                                                                                       : GuiResponse::notSunk();
    }
...
{
    AYAASSERT(event->getUserInputType() != InputObject::TYPE_NONE);
    if (!isInitialized)
        return false;
    ...
    if (!response.wasSunk())
    {
        FASTLOG(FLog::UserInputProfile, "Handing to Profiler/ImGui");
    ...
    {
        if (GamepadService* gamepadService = Aya::ServiceProvider::create<GamepadService>(this))
        {
...
{
    debugMenu->addCustomStat(name, value);
}

void DataModel::removeCustomStat(std::string str)
{
    debugMenu->removeCustomStat(str);
}

void DataModel::writeStatsSettings()
{
    debugMenu->saveCustomStats();
}
...
void DataModel::clearContents(bool resettingSimulation)
{
    ...
    if (workspace)
    {
        ...
        if (workspace->getTerrain())
            workspace->getTerrain()->unlockParent();
        ...
        {
            FASTLOG3(FLog::CloseDataModel, "Clearing World -- %d Bodies, %d Points, %d Constraints", world->getNumBodies(), world->getNumPoints(),
                world->getNumConstraints());
            FASTLOG3(FLog::CloseDataModel, "Clearing World -- %d HashNodes, %d MaxBucketSize, %d NumLinkCalls", world->getNumHashNodes(),
                world->getMaxBucketSize(), world->getNumLinkCalls());
            FASTLOG3(FLog::CloseDataModel, "Clearing World -- %d Contacts, %d Joints, %d Primitives", world->getNumContacts(), world->getNumJoints(),
                world->getNumPrimitives());
        }

        FASTLOG3(FLog::CloseDataModel, "Clearing Workspace -- %d Instances, %d Parts, %d Scripts", workspace->countDescendantsOfType<Instance>(),
            workspace->countDescendantsOfType<PartInstance>(), workspace->countDescendantsOfType<BaseScript>());
...
double DataModel::getMetricValue(const std::string& metric) const
{
    ...
    throw Aya::runtime_error("%s is not a valid metric.", metric.c_str());
}

void DataModel::buildSimpleStatsOutput(shared_ptr<Instance> instance, std::string* output)
{
...
void DataModel::buildNetworkStatsOutput(shared_ptr<Instance> instance, std::string* output)
{
    if (Stats::Item* item = instance->fastDynamicCast<Stats::Item>())
    {
        std::string itemName = item->getName();
...
std::string DataModel::getMetric(const std::string& valueName) const
{
    ...
    if (valueName.compare(0, 11, "RenderStats") == 0)
        return tempMetric->getMetric(valueName);
    if (valueName.compare(0, 3, "FRM") == 0)
        return Aya::format("%.3g", tempMetric->getMetricValue(valueName));
    ...
    if (stats)
    {
        ...
        if (network && network->numChildren() > 1)
        {
            ...
            {
                // raknet
                if (Stats::Item* raknetItem =
                        Instance::fastDynamicCast<Stats::Item>(item->findFirstChildByName("Stats")->findFirstChildByName(valueName)))
                    return Aya::format("%.0f", raknetItem->getValue());
                ...
                {
                    return Aya::format("%.2f", item->findFirstChildByName("Send kBps")->fastDynamicCast<Stats::Item>()->getValue());
                }
                else if (valueName == "IncomingStats")
                {
                    return Aya::format("%.2f, %.2f, %.0f", item->findFirstChildByName("Receive kBps")->fastDynamicCast<Stats::Item>()->getValue(),
                ...
                else if (valueName == "InData")
                {
                    ...
                    float kBps = numPacket * avgSize / 1000.f;
                    return Aya::format("%.2f, %.2f, %.2fB", avgSize, kBps, numPacket);
                }
                else if (valueName == "InPhysics")
                {
                    ...
                    float kBps = numPacket * avgSize / 1000.f;
                    return Aya::format("%.2f, %.2f, %.2fB, %.2f", avgSize, kBps, numPacket,
                ...
                else if (valueName == "InTouches")
                {
                    ...
                    float kBps = numPacket * avgSize / 1000.f;
                    return Aya::format("%.2f, %.2f, %.2fB", kBps, numPacket, avgSize);
                }
                ...
                else if (valueName == "OutData")
                {
                    ...
                    float kBps = numPacket * avgSize / 1000.f;
                    return Aya::format("%.2f, %.2f, %.2fB, %.2f%%", avgSize, kBps, numPacket,
                        outProps->findFirstChildByName("Throttle")->fastDynamicCast<Stats::Item>()->getValue());
                }
                else if (valueName == "OutTouches")
                {
                    ...
                    float kBps = numPacket * avgSize / 1000.f;
                    return Aya::format("%.2f, %.2f, %.2fB, %.0f", kBps, numPacket, avgSize,
                        outTouches->findFirstChildByName("WaitingTouches")->fastDynamicCast<Stats::Item>()->getValue());
                }
                ...
                else if (valueName == "InPhysicsDetails")
                {
                    ...
                    {
                        inPhysicsDetails->visitChildren(boost::bind(&DataModel::buildNetworkStatsOutput, _1, &result));
                    }
                    return result;
                }
                else if (valueName == "OutPhysicsDetails")
                {
                    ...
                    {
                        outPhysicsDetails->visitChildren(boost::bind(&DataModel::buildNetworkStatsOutput, _1, &result));
                    }
                    return result;
                }
                else if (valueName == "InDataDetails")
                {
                    ...
                    {
                        inDataDetails->visitChildren(boost::bind(&DataModel::buildNetworkStatsOutput, _1, &result));
                    }
                    return result;
                }
                ...
                else if (valueName == "ReceivedStreamData")
                {
                    ...
                    {
                        streamStats->visitChildren(boost::bind(&DataModel::buildSimpleStatsOutput, _1, &result));
                    }
                    return result;
                }
                else if (valueName == "FreeMemory")
                {
                    float result = Aya::NetworkSettings::singleton().getFreeMemoryMBytes();
                    return Aya::format("%.2fMB", result);
                }
                else if (valueName == "MemoryLevel")
                {
                    int result = Aya::MemoryStats::slowCheckMemoryLevel(
                        ((Aya::MemoryStats::memsize_t)Aya::NetworkSettings::singleton().getExtraMemoryUsedInMB()) * 1024 * 1024);
                    return Aya::format("%d", result);
                }
            }
        }
        ...
        if (httpQueue)
        {
            ...
            {
                return Aya::format("%.2f msec", httpQueue->findFirstChildByName("Average time in queue")->fastDynamicCast<Stats::Item>()->getValue());
            }
            else if (valueName == "HttpProcessTime")
            {
                return Aya::format("%.2f msec", httpQueue->findFirstChildByName("Average process time")->fastDynamicCast<Stats::Item>()->getValue());
            }
            else if (valueName == "HttpSlowReq")
            {
                return Aya::format("%.0f", httpQueue->findFirstChildByName("Num slow requests")->fastDynamicCast<Stats::Item>()->getValue());
            }
        }
    }
    ...
    {
        boost::format fmt("%.1f/s %.1f msec %d%%");
        fmt % tempMetric->getMetricValue("Render FPS") % (1000.0 * tempMetric->getMetricValue("Render Job Time")) %
            (int)(100.0 * tempMetric->getMetricValue("Render Duty"));
        return fmt.str();
    }
    ...
    else if (valueName == "Effective FPS")
        return Aya::format("%.1f/s", 1000.0 / tempMetric->getMetricValue("Delta Between Renders"));
    else if (valueName == "Render Nominal FPS")
        return Aya::format("%.1f/s", tempMetric->getMetricValue(valueName));
    else if (valueName == "Delta Between Renders")
        return Aya::format("%.3g", tempMetric->getMetricValue(valueName));
    else if (valueName == "Total Render")
        return Aya::format("%.3g", tempMetric->getMetricValue(valueName));
    else if (valueName == "Render Prepare")
        return Aya::format("%.3g", tempMetric->getMetricValue(valueName));
    else if (valueName == "Present Time")
        return Aya::format("%.3g", tempMetric->getMetricValue(valueName));
    else if (valueName == "GPU Delay")
        return Aya::format("%.3g", tempMetric->getMetricValue(valueName));

    else if (valueName == "RequestQueueSize")
        return StringConverter<int>::convertToString(ServiceProvider::create<ContentProvider>()->getRequestQueueSize());

    else if (valueName == "drawId")
        return StringConverter<int>::convertToString(drawId);
    else if (valueName == "worldId")
        return StringConverter<int>::convertToString(world->getWorldStepId());
    else if (valueName == "Graphics Mode")
        return tempMetric->getMetric(valueName);
    else if (valueName == "VirtualVersion")
        return tempMetric->getMetric(valueName);
    else if (valueName == "Particles")
        return StringConverter<int>::convertToString(static_cast<int>(tempMetric->getMetricValue(valueName)));
    else if (valueName == "Shadow Casters")
        return StringConverter<int>::convertToString(static_cast<int>(tempMetric->getMetricValue(valueName)));
    else if (valueName == "Anti-Aliasing")
        return tempMetric->getMetric(valueName);
    else if (valueName == "Bevels")
        return tempMetric->getMetric(valueName);
    else if (valueName == "Video Memory")
        return Log::formatMem(SystemUtil::getVideoMemory());
    ...
    else if (valueName == "Player Radius")
    {
        ...
        for (; iter != end; ++iter)
        {
            Aya::Network::Player* player = boost::polymorphic_downcast<Aya::Network::Player*>(iter->get());
        ...
        return StringConverter<int>::convertToString(numPartInstances);
    else if (valueName == "numMovingPrimitives")
        return StringConverter<int>::convertToString(workspace->getNumberMoving());
    ...
    else if (valueName == "numContactsAll")
    {
        ...
        fmt % world->getNumContacts() % world->getMetric(IWorldStage::NUM_CONTACTSTAGE_CONTACTS) %
            world->getMetric(IWorldStage::NUM_STEPPING_CONTACTS) % world->getMetric(IWorldStage::NUM_TOUCHING_CONTACTS);
        return fmt.str();
    }
        ...
        return StringConverter<float>::convertToString(BlockBlockContact::pairHitRatio());
    else if (valueName == "contactFeatureHitRatio")
        return StringConverter<float>::convertToString(BlockBlockContact::featureHitRatio());
    ...
    else if (valueName == "environmentSpeed")
        return StringConverter<float>::convertToString(world->getEnvironmentSpeed());


    else if (valueName == "Break Time" || valueName == "Assembler Time" || valueName == "Filter Time" || valueName == "UI Step" ||
             valueName == "Broadphase" || valueName == "Collision" || valueName == "Wake" || valueName == "Sleep" || valueName == "Joint Update" ||
             valueName == "Joint Sleep" || valueName == "Kernel Bodies" || valueName == "Kernel Connectors")
    {
        if (stats)
        {
            ...
            {
                shared_ptr<Stats::Item> dataModelStep =
                    shared_from_polymorphic_downcast<Stats::Item>(workspace->findFirstChildByName("DataModel Step"));
                if (dataModelStep && dataModelStep->numChildren() > 0)
                {
                    shared_ptr<Stats::Item> workspaceStep =
                        shared_from_polymorphic_downcast<Stats::Item>(dataModelStep->findFirstChildByName("Workspace Step"));
                    if (workspaceStep && workspaceStep->numChildren() > 0)
                    {
                        shared_ptr<Stats::Item> worldStep =
                            shared_from_polymorphic_downcast<Stats::Item>(workspaceStep->findFirstChildByName("World Step"));
                        if (worldStep && worldStep->numChildren() > 0)
                        {
                            Aya::Instance* profilerItem = worldStep->findFirstChildByName(valueName);
                            Aya::Stats::Item* profilerStatItem = Instance::fastDynamicCast<Aya::Stats::Item>(profilerItem);
                            if (profilerStatItem)
    ...
    else if (valueName == "solverIterations")
        return StringConverter<int>::convertToString(world->getKernel()->fakeDeceptiveSolverIterations());
    else if (valueName == "matrixSize")
        return StringConverter<int>::convertToString(world->getKernel()->fakeDeceptiveMatrixSize());
    ...
    else if (valueName == "numBodiesAll")
    {
        ...
        fmt % world->getKernel()->numBodiesMax() % world->getKernel()->numFreeFallBodies() % world->getKernel()->numContactBodies() %
            world->getKernel()->numJointBodies() % world->getKernel()->numRealTimeBodies() % world->getKernel()->numLeafBodies();
        return fmt.str();
    }
    ...
    {
        boost::format fmt("%d %d %d %d %d %d");
        fmt % world->getKernel()->numConnectors() % world->getKernel()->numContactConnectors() % world->getKernel()->numJointConnectors() %
            world->getKernel()->numRealTimeConnectors() % world->getKernel()->numSecondPassConnectors() % world->getKernel()->numHumanoidConnectors();
        return fmt.str();
    }

    else if (valueName == "numBodies")
        return StringConverter<int>::convertToString(world->getKernel()->numBodies());
    else if (valueName == "maxBodies")
        return StringConverter<int>::convertToString(world->getKernel()->numBodiesMax());
    else if (valueName == "numLeafBodies")
        return StringConverter<int>::convertToString(world->getKernel()->numLeafBodies());
    ...
    else if (valueName == "numIterations")
    {
        ...
        fmt % world->getKernel()->numMaxIterations() % world->getKernel()->numIterations() % world->getKernel()->getMaxSolverError() %
            world->getKernel()->getSolverError();
        return fmt.str();
    }

    else if (valueName == "numConstraints")
        return StringConverter<int>::convertToString(world->getKernel()->numConnectors());
    else if (valueName == "numPoints")
        return StringConverter<int>::convertToString(world->getKernel()->numPoints());
    else if (valueName == "percentConnectorsActive")
        return StringConverter<float>::convertToString(ContactConnector::percentActive());
...
static void appendJobInfo(DataModel* dataModel, shared_ptr<const TaskScheduler::Job> job, Reflection::ValueArray* result)
{
    ...
    shared_ptr<Reflection::ValueArray> info(Aya::make_shared<Reflection::ValueArray>());
    ...
    result->push_back(shared_ptr<const Aya::Reflection::ValueArray>(info));
}
...
void DataModel::setPlaceID(int placeID, bool robloxPlace)
{
    ...
    Http::placeID = Aya::format("%d", placeID);
}
...
static void gameStartInfoLoadedHelperSuccess(weak_ptr<DataModel> dm, std::string json)
{
    ...
    if (Aya::WebParser::parseJSONObject(json, v))
    {
...
void DataModel::requestGameStartInfo()
{
    ...
    if (Aya::HttpRbxApiService* apiService = Aya::ServiceProvider::create<Aya::HttpRbxApiService>(this))
    {
        apiService->getAsync(Aya::format("universes/%d/game-start-info", universeId), false, Aya::PRIORITY_DEFAULT,
            boost::bind(&gameStartInfoLoadedHelperSuccess, weak_from(this), _1), boost::bind(&gameStartInfoLoadedHelperError, weak_from(this), _1));
    }
}
...
{
    throw Aya::runtime_error("load plugins not supported");
}
...
std::string DataModel::getVIPServerId() const
{
    ...
    {
        Aya::StandardOut::singleton()->printf(MESSAGE_WARNING, "VIPServerID checked on client, but only set on server.");
    }
...
int DataModel::getVIPServerOwnerId() const
{
    if (Aya::Network::Players::clientIsPresent(this))
    {
        Aya::StandardOut::singleton()->printf(MESSAGE_WARNING, "VIPServerOwnerID checked on client, but only set on server.");
    }
...
static void getJobTimePeakFractionFunc(
    DataModel* dataModel, shared_ptr<const TaskScheduler::Job> job, std::string& jobname, double greaterThan, double* result)
{
    if (job->getArbiter().get() != dataModel)
        return;
...
static void getJobIntervalPeakFractionFunc(
    DataModel* dataModel, shared_ptr<const TaskScheduler::Job> job, std::string& jobname, double greaterThan, double* result)
{
    if (job->getArbiter().get() != dataModel)
        return;
...
int DataModel::getMaxPlayers() const
{
    const Network::Players* players = find<Network::Players>();
    if (!players)
        return 0;
...
void DataModel::ShowMessage(weak_ptr<Aya::DataModel> weakDataModel, int slot, const std::string& message, double duration)
{
    if (shared_ptr<Aya::DataModel> dataModel = weakDataModel.lock())
    {
        if (shared_ptr<Aya::CoreGuiService> coreGuiService = shared_from(dataModel->find<Aya::CoreGuiService>()))
        {
    ...
    : dataModel(DataModel::get(context))
{
    if (dataModel)
    {
        Aya::mutex::scoped_lock lock(dataModel->debugLock);

        AYAASSERT(dataModel->write_requested == 0);
        AYAASSERT(dataModel->read_requested == 0);
        AYAASSERT(dataModel->writeRequestingThread == 0);
...
DataModel::scoped_write_request::~scoped_write_request()
{
    if (dataModel)
    {
        ...
        AYAASSERT(dataModel->write_requested == 1);
        AYAASSERT(dataModel->read_requested == 0);
        AYAASSERT(dataModel->writeRequestingThread == GetCurrentThreadId());
...
unsigned int DataModel::allHackFlagsOredTogether()
{
    ...
    for (boost::unordered_set<unsigned int>::iterator itr = hackFlagSet.begin(); itr != hackFlagSet.end(); ++itr)
    {
        result |= *itr;
    }
#endif
...
void DataModel::processHttpRequestResponseOnLock(DataModel* dataModel, std::string* response, std::exception* exception,
    boost::function<void(shared_ptr<std::string>, shared_ptr<std::exception> exception)> onLockAcquired)
{
    ...
    {
        dataModel->submitTask(boost::bind(onLockAcquired, responseCopy, exceptionCopy), Aya::DataModelJob::TaskType::Write);
    }
    else
    {
