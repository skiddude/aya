...
static Reflection::BoundFuncDesc<GamePassService, void(std::string)> func_SetPlayerHasPassUrl(
    &GamePassService::setPlayerHasPassUrl, "SetPlayerHasPassUrl", "playerHasPassUrl", Security::LocalUser);

static Reflection::BoundYieldFuncDesc<GamePassService, bool(shared_ptr<Instance>, int)> func_PlayerHasPass(
    &GamePassService::playerHasPass, "PlayerHasPass", "player", "gamePassId", Security::None);
REFLECTION_END();
...
void GamePassService::dispatchRequest(
...
void GamePassService::playerHasPass(
    shared_ptr<Instance> playerInstance, int gamePassId, boost::function<void(bool)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    ...
    else if (shared_ptr<Network::Player> player = Instance::fastSharedDynamicCast<Network::Player>(playerInstance))
    {
        ...
        std::string url(Aya::format(playerHasPassUrl.c_str(), playerId, gamePassId));
