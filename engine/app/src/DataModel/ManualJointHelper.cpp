...
void ManualJointHelper::findPermissibleJointSurfacePairs(void)
{
    ...
    for (unsigned int i = 0; i < selectedPrimitives.size(); i++)
    {
        ...
        if (workspace)
        {
            workspace->getWorld()->getContactManager()->getPrimitivesTouchingExtents(
                selectedPrimitives[i]->getFastFuzzyExtents(), selectedPrimitives[i], 0, found);
    ...
    if (mCPrim && mCPrim->getGeometryType() == Geometry::GEOMETRY_MEGACLUSTER)
    {
        for (unsigned int i = 0; i < selectedPrimitives.size(); i++)
        {
            if (selectedPrimitives[i]->getGeometryType() != Aya::Geometry::GEOMETRY_MEGACLUSTER)
            {
                std::vector<Vector3int16> cells;
                static_cast<const MegaClusterPoly*>(mCPrim->getConstGeometry())
                    ->findPlanarTouchesWithGeom(mCPrim->getCoordinateFrame(), *(selectedPrimitives[i]->getConstGeometry()),
                        selectedPrimitives[i]->getCoordinateFrame(), &cells);
...
void ManualJointHelper::createTerrainJointSurfacePair(Primitive& p0, Primitive& p1, Vector3int16& cellIndex)
{
    ...
    AYAASSERT(part0->getParent());
    PartInstance* part1 = PartInstance::fromPrimitive(&p1);
    AYAASSERT(part1->getParent());

    ConstraintSurfacePair* aPair = NULL;

    AYAASSERT(p1.getConstGeometry()->getGeometryType() != Aya::Geometry::GEOMETRY_MEGACLUSTER);
    size_t fix = -1;
    ...
    size_t otherFaceId;
    std::vector<Vector3> intersectPolyInP0 =
        terrainPoly->findCellIntersectionWithGeom(cellIndex, p0.getCoordinateFrame(), *(p1.getConstGeometry()), p1.getCoordinateFrame(), otherFaceId);
...
void ManualJointHelper::createJointSurfacePair(Primitive& p0, size_t& face0Id, Primitive& p1, size_t& face1Id)
{
    ...
    {
        AYAASSERT(p1.getConstGeometry()->getGeometryType() != Aya::Geometry::GEOMETRY_MEGACLUSTER);
        if (p0.getConstGeometry()->getGeometryType() == Aya::Geometry::GEOMETRY_MEGACLUSTER)
...
void TerrainManualJointSurfacePair::dynamicDraw(Adorn* adorn)
{
    if (p1 && p0 && p1->getConstGeometry() && p0->getConstGeometry())
    {
        ...
        const MegaClusterPoly* terrainPoly = static_cast<const MegaClusterPoly*>(p0->getConstGeometry());
        size_t tempFaceId;
        std::vector<Vector3> intersectPolyInP0 = terrainPoly->findCellIntersectionWithGeom(
            cellIndex, p0->getCoordinateFrame(), *(p1->getConstGeometry()), p1->getCoordinateFrame(), tempFaceId);
...
void TerrainManualJointSurfacePair::createJoint(void)
{
    ...
    if (PartInst0 && PartInst1)
    {
        if (AdvArrowTool::advManualJointType == DRAG::STRONG_MANUAL_JOINT)
        {
            ...
            AYAASSERT(mJoint->getJointType() == Joint::MANUAL_WELD_JOINT);
