...
private:
    ...
    Aya::signals::scoped_connection heartbeatSignalConnection;
    ...
    static void startAwardPointsBatching(weak_ptr<Aya::DataModel> weakDm);
    ...
    bool canUseService();
    void internalGetPointBalance(int userId, int placeId, shared_ptr<std::string> methodName, shared_ptr<std::string> keyToReturn,
        boost::function<void(int)> resumeFunction, boost::function<void(std::string)> errorFunction);

public:
    PointsService();

    Aya::remote_signal<void(int, int, int, int)> pointsAwardedSignal;
    ...
    void awardPoints(int userId, int amount, boost::function<void(shared_ptr<const Reflection::Tuple>)> resumeFunction,
