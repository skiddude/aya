...
const char* const Aya::sPluginManager = "PluginManager";
const char* const Aya::sPlugin = "Plugin";
const char* const Aya::sToolbar = "Toolbar";
const char* const Aya::sButton = "Button";

using namespace Aya;


// Reflections
static Reflection::CustomBoundFuncDesc<PluginManager, shared_ptr<Instance>()> func_createPlugin(
    &PluginManager::createPlugin, "CreatePlugin", Security::Plugin);
...
static Reflection::BoundFuncDesc<Plugin, shared_ptr<Instance>(std::string)> func_createToolbar(
    &Plugin::createToolbar, "CreateToolbar", "name", Security::Plugin);
static Reflection::BoundFuncDesc<Plugin, void(std::string, Reflection::Variant)> func_setSetting(
    &Plugin::setSetting, "SetSetting", "key", "value", Security::Plugin);
static Reflection::BoundFuncDesc<Plugin, Reflection::Variant(std::string)> func_getSetting(
    &Plugin::getSetting, "GetSetting", "key", Security::Plugin);
...
static Reflection::BoundFuncDesc<Plugin, void(void)> func_saveSelectedToRoblox(
    &Plugin::saveSelectedToRoblox, "SaveSelectedToRoblox", Security::Plugin);

// CSG
static Reflection::BoundFuncDesc<Plugin, shared_ptr<Instance>(shared_ptr<const Instances>)> plugin_Union(
    &Plugin::csgUnion, "Union", "objects", Security::Plugin);
static Reflection::BoundFuncDesc<Plugin, shared_ptr<const Instances>(shared_ptr<const Instances>)> plugin_Negate(
    &Plugin::csgNegate, "Negate", "objects", Security::Plugin);
static Reflection::BoundFuncDesc<Plugin, shared_ptr<const Instances>(shared_ptr<const Instances>)> plugin_Separate(
    &Plugin::csgSeparate, "Separate", "objects", Security::Plugin);

static Reflection::BoundYieldFuncDesc<Plugin, int(std::string)> func_promptForExistingAssetId(
    &Plugin::promptForExistingAssetId, "PromptForExistingAssetId", "assetType", Security::Plugin);

static Reflection::BoundFuncDesc<Toolbar, shared_ptr<Instance>(std::string, std::string, std::string)> func_createButton(
    &Toolbar::createButton, "CreateButton", "text", "tooltip", "iconname", Security::Plugin);
...
static Reflection::BoundFuncDesc<Plugin, void(shared_ptr<Aya::Instance>, int)> func_openScript(
    &Plugin::openScriptDoc, "OpenScript", "script", "lineNumber", 0, Security::Plugin);

static Reflection::BoundFuncDesc<PluginManager, void(std::string)> func_exportPlace(
    &PluginManager::exportPlace, "ExportPlace", "filePath", "", Security::Plugin);
static Reflection::BoundFuncDesc<PluginManager, void(std::string)> func_exportSelection(
    &PluginManager::exportSelection, "ExportSelection", "filePath", "", Security::Plugin);

static Reflection::PropDescriptor<Plugin, bool> prop_CollisionActive(
    "CollisionEnabled", category_Data, &Plugin::isCollisionOn, NULL, Reflection::PropertyDescriptor::UI);
static Reflection::PropDescriptor<Plugin, float> prop_GridSize(
    "GridSize", category_Data, &Plugin::getGridSize, NULL, Reflection::PropertyDescriptor::UI);
static Reflection::BoundFuncDesc<Plugin, AdvArrowTool::JointCreationMode(void)> func_getJoinMode(
    &Plugin::getJoinMode, "GetJoinMode", Security::Plugin);
REFLECTION_END();

namespace
{
void runAllPluginScriptsHelper(ScriptContext* scriptContext, shared_ptr<Plugin> plugin, shared_ptr<Instance> descendant)
{
    ...
    if (script != NULL && !script->isDisabled())
    {
        ...
        try
        {
            scriptContext->executeInNewThreadWithExtraGlobals(
                Security::StudioPlugin, script->getEmbeddedCodeSafe(), script->getFullName().c_str(), extraGlobals);
        }
...
void Button::processIconLoaded(ContentId contentId, AsyncHttpQueue::RequestResult result, std::istream* stream, shared_ptr<const std::string> unused)
{
    if (isDeleted)
    {
        return;
...
shared_ptr<Instance> Toolbar::createButton(std::string text, std::string tooltip, std::string iconName)
{
    ...
    if (fetchIconFromAssetUrl)
    {
        ...
        try
        {
            ...
            contentProvider->getContent(contentId, ContentProvider::PRIORITY_GUI, boost::bind(&Button::processIconLoaded, b, contentId, _1, _2, _3),
        ...
        catch (const std::runtime_error&)
        {
            ...
            StandardOut::singleton()->printf(MESSAGE_ERROR, "Encountered error while loading plugin icon %s", iconName.c_str());
        }
    }
...
{
    return Aya::AdvArrowTool::getJointCreationMode();
}
...
void Plugin::openScriptDoc(shared_ptr<Aya::Instance> script, int lineNumber)
{
...
void PluginManager::startModelPluginScripts(DataModel* dataModel)
{
    ...
    if (iter != state.end())
    {
        ...
        {
            for (pluginsIter pluginIter = entry.plugins.begin(); pluginIter != entry.plugins.end(); ++pluginIter)
            {
                (*pluginIter)->visitDescendants(boost::bind(&runAllPluginScriptsHelper, scriptContext, *pluginIter, _1));
            }
        }
    }
}
...
{
    Aya::Plugin* activePlugin = getActivePlugin(dataModel);
    if (activePlugin)
    {
...
void PluginManager::buttonClick(DataModel* dataModel, void* id)
{
    ...
    if (iter != state.end())
    {
        Aya::DataModel::LegacyLock lock(dataModel, DataModelJob::Write);
...
{
    getStudioHost()->exportPlace(filePath, Aya::ExporterSaveType_Everything);
}

void PluginManager::exportSelection(std::string filePath)
{
    getStudioHost()->exportPlace(filePath, Aya::ExporterSaveType_Selection);
}
...
void Plugin::promptForExistingAssetId(
    std::string assetType, boost::function<void(int)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    pluginManager->getStudioHost()->promptForExistingAssetId(assetType, resumeFunction, errorFunction);
}

void PluginManager::fireDragEnterEvent(DataModel* dataModel, shared_ptr<const Aya::Instances> instances, Vector2 location)
{
    if (!dataModel)
        return;

    if (Aya::Plugin* activePlugin = getActivePlugin(dataModel))
    {
        Aya::DataModel::LegacyLock lock(dataModel, DataModelJob::Write);
        shared_ptr<InputObject> input = Creatable<Instance>::create<InputObject>(
            InputObject::TYPE_MOUSEMOVEMENT, InputObject::INPUT_STATE_CHANGE, Vector3(location.x, location.y, 0), Vector3::zero(), dataModel);
