...
Vector3 RootInstance::computeCharacterInsertPoint(const Vector3& sizeOfInsertedModel)
{
    if (ModelInstance* model = Network::Players::findLocalCharacter(this))
    {
        ...
        Vector3 insertPoint = modelExtents.bottomCenter() + (7.0f * cameraLook) // default distance to move from character
                              + (halfSize * cameraLook);
        insertPoint = DragUtilities::toGrid(insertPoint);
        return insertPoint;
    }
...
void RootInstance::movePartsToCameraFocus(PartArray& partArray)
{
    ...
    Aya::Plugin* activePlugin = PluginManager::singleton()->getActivePlugin(DataModel::get(this));
...
{
    AYAASSERT(requestedParent && this->contains(requestedParent));
...
void RootInstance::publicInsertRaw(
    const Instances& instances, Instance* requestedParent, PartArray& partArray, bool joinPartsInInstancesOnly, bool suppressPartMove)
{
    ...
    if (Aya::GameBasicSettings::singleton().inStudioMode()) // only works in studio (InsertService::Insert calls this function)
...
void RootInstance::insertRemoteCharacterView(
    const Instances& instances, PartArray& partArray, const Vector3* positionHint, PromptMode promptMode, bool suppressMove)
{
    AYAASSERT(instances.size() > 0);
...
void RootInstance::insertIdeView(const Instances& instances, PartArray& partArray, PromptMode promptMode, bool suppressMove)
{
    ...
    if ((instances.size() == 1))
    {
        ...
        if (Instance::fastDynamicCast<Tool>(single) && (promptMode == PUT_TOOL_IN_STARTERPACK))
        {
            single->setParent(ServiceProvider::create<StarterPackService>(this));
            return;
        }
...
void RootInstance::insert3dView(
    const Instances& instances, PromptMode promptMode, bool suppressMove, const Vector3* positionHint, bool lerpCameraInStudio)
{
    AYAASSERT(instances.size() > 0);
...
void RootInstance::insertDecal(Decal* d, Aya::InsertMode insertMode)
{
    Workspace* ws = ServiceProvider::find<Workspace>(this);
    if (ws)
        ws->startDecalDrag(d, insertMode);
}
...
void RootInstance::insertPasteInstances(const Instances& instances, Instance* requestedParent,
    InsertMode insertMode, // RAW: don't move, TREE: only move up, 3D_View: insert point
    PromptMode promptMode, const Vector3* positionHint, Instances* remaining, bool lerpCameraInStudio)
{
    doInsertInstances(instances, requestedParent, insertMode, promptMode, positionHint, remaining, true, lerpCameraInStudio);
}

void RootInstance::doInsertInstances(const Instances& instances, Instance* requestedParent,
    InsertMode insertMode, // RAW: don't move, TREE: only move up, 3D_View: insert point
    PromptMode promptMode, const Vector3* positionHint, Instances* remaining, bool forceSuppressMove, bool lerpCameraInStudio)
{
    AYAASSERT(instances.size() > 0);
    ...
    if (requestedParent && !this->contains(requestedParent))
    {
        ...
        for (size_t i = 0; i < instances.size(); ++i)
        {
            ...
            if (dynamic_cast<Aya::Service*>(pInstance))
                Aya::StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "Do Menu Insert->Service, to insert a Service");
            else
    ...
    else
    {
        ...
        for (size_t i = 0; i < instances.size(); ++i)
        {
            ...
            else if (dynamic_cast<Aya::Service*>(instance)) // Do not allow insertion of services from Toolbox or any other place. Insert Service
                                                            // Dialog does not call this fn so it is fine.
            {
                Aya::StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "Do Menu Insert->Service, to insert a Service");
            }
            else
            {
                ...
                if (decal)
                {
                    ...
                    if (insertMode != INSERT_TO_3D_VIEW)
                    {
                        ...
                        if (Instance::fastDynamicCast<Aya::PartInstance>(requestedParent))
...
void RootInstance::insertInstances(const Instances& instances, Instance* requestedParent,
    InsertMode insertMode, // RAW: don't move, TREE: only move up, 3D_View: insert point
    PromptMode promptMode, const Vector3* positionHint, Instances* remaining, bool lerpCameraInStudio)
{
    doInsertInstances(instances, requestedParent, insertMode, promptMode, positionHint, remaining, false, lerpCameraInStudio);
}
