...
public:
    ...
    Item(const char* name)
        : DescribedNonCreatable<Item, Instance, sStatsItem>(name)
        , val(0)
    {
    }
...
public:
    JsonWriter(std::stringstream& s, bool& needComma)
        : s(s)
        , needComma(needComma)
        , hasNonJsonType(false) {};
    ...
    bool seenNonJsonType()
    {
        return hasNonJsonType;
    }
};

// A generic mechanism for displaying stats (like 3D FPS, network traffic, etc.)
class StatsService
    : public DescribedNonCreatable<StatsService, Instance, sStats, Reflection::ClassDescriptor::INTERNAL_LOCAL, Aya::Security::LocalUser>
    , public Service
{
    std::string customReportUrl;
    typedef boost::unordered_map<std::string, Aya::Time> LastReportTimes;
    LastReportTimes lastReportTimes;
    Aya::signals::scoped_connection baseUrlConnection;

public:
    StatsService()
        : DescribedNonCreatable<StatsService, Instance, sStats, Reflection::ClassDescriptor::INTERNAL_LOCAL, Aya::Security::LocalUser>("Stats")
...
private:
    ...
    static void addCategoryAndTable(const std::string& category, const Reflection::ValueTable& data, shared_ptr<std::stringstream> stream);
