...
template<template<typename> class BaseSignal, typename Signature>
class LatchedSignal : public BaseSignal<Signature>
{
public:
    ...
    template<typename F>
    Aya::signals::connection connect(const F& function)
    {
        Aya::signals::connection result = BaseSignal<Signature>::connect(function);
        queue.process();
        return result;
    }
    ...
    void push(const boost::function<void()>& item)
    {
        if (!queue.push(item))
        {
            ...
            Aya::StandardOut::singleton()->printf(
                Aya::MESSAGE_ERROR, "Remote event invocation queue exhausted for %s; did you forget to implement %s?", name.c_str(), eventName);
        }
    }
...
public:
    ...
    typedef boost::function<void(shared_ptr<Instance>, shared_ptr<const Reflection::Tuple>,
        boost::function<void(shared_ptr<const Reflection::Tuple>)>, boost::function<void(std::string)>)>
        ServerInvokeCallback;
    ...
    void invokeServer(shared_ptr<const Reflection::Tuple> arguments, boost::function<void(shared_ptr<const Reflection::Tuple>)> resumeFunction,
        boost::function<void(std::string)> errorFunction);
    void invokeClient(shared_ptr<Instance> player, shared_ptr<const Reflection::Tuple> arguments,
        boost::function<void(shared_ptr<const Reflection::Tuple>)> resumeFunction, boost::function<void(std::string)> errorFunction);

    Aya::remote_signal<void(int, shared_ptr<Instance>, shared_ptr<const Reflection::Tuple>)> remoteOnInvokeServer;
    Aya::remote_signal<void(int, shared_ptr<const Reflection::Tuple>)> remoteOnInvokeClient;

    Aya::remote_signal<void(int, shared_ptr<const Reflection::Tuple>)> remoteOnInvokeSuccess;
    Aya::remote_signal<void(int, std::string)> remoteOnInvokeError;
...
private:
    ...
    int createRemoteInvocation(const weak_ptr<Instance>& player, const boost::function<void(shared_ptr<const Reflection::Tuple>)>& resumeFunction,
        const boost::function<void(std::string)>& errorFunction);

    virtual void processRemoteEvent(
        const Reflection::EventDescriptor& descriptor, const Reflection::EventArguments& args, const SystemAddress& source);

    void localInvokeServer(shared_ptr<Instance> player, shared_ptr<const Reflection::Tuple> arguments,
        boost::function<void(shared_ptr<const Reflection::Tuple>)> resumeFunction, boost::function<void(std::string)> errorFunction);
    void localInvokeClient(shared_ptr<const Reflection::Tuple> arguments, boost::function<void(shared_ptr<const Reflection::Tuple>)> resumeFunction,
        boost::function<void(std::string)> errorFunction);
...
public:
    ...
    LatchedSignal<Aya::remote_signal, void(shared_ptr<Instance>, shared_ptr<const Reflection::Tuple>)>* getOrCreateOnServerEvent(bool create = true);
    LatchedSignal<Aya::remote_signal, void(shared_ptr<const Reflection::Tuple>)>* getOrCreateOnClientEvent(bool create = true);
...
private:
    ...
    LatchedSignal<Aya::remote_signal, void(shared_ptr<Instance>, shared_ptr<const Reflection::Tuple>)> onServerEvent;
    LatchedSignal<Aya::remote_signal, void(shared_ptr<const Reflection::Tuple>)> onClientEvent;
};
