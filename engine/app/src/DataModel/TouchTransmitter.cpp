...
public:
    ...
    bool checkTouch(const shared_ptr<PartInstance>& other, TouchPair::Type type)
    {
        ...
        static const Aya::Time::Interval expiration = Aya::Time::Interval(5);

        const Aya::Time now = Aya::Time::nowFast();
        ...
        for (size_t i = 0; i < items.size();)
        {
            ...
            if (item.other.lock() == other)
            {
                ...
                {
                    item.lastType = type;
                    // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Touch accepted");
                    return true;
                }
                else
                {
                    // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Touch rejected");
                    //  Rejected!
                    return false;
                }
            }
            ...
            {
                // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Touch expired");
