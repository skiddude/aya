...
private:
    ...
    Aya::signals::scoped_connection screenGuiConnection;
...
public:
    ...
    {
        // deprecated, used only in older corescripts, so we can
        // just set this as true for linux
        return true;
    };
    ...
    Aya::signal<void(std::string, std::string)> keyPressed;
    Aya::signal<void()> escapeKeyPressed;
    Aya::signal<void(GuiService::SpecialKey, std::string)> specialKeyPressed;

    Aya::signal<void(std::string)> newErrorSignal;
    Aya::signal<void(UiMessageType, std::string)> newUiMessageSignal;

    Aya::signal<void()> showLeaveConfirmationSignal;

    Aya::signal<void()> menuOpenedSignal;
    Aya::signal<void()> menuClosedSignal;

    void fireMenuOpenedSignal()
    {
        menuOpenedSignal();
    }
    void fireMenuClosedSignal()
    {
        menuClosedSignal();
    }
    ...
    bool processKeyDown(const shared_ptr<Aya::InputObject>& event);

    const Vector4& getGlobalGuiInset() const
    {
        return guiInset;
    }
    shared_ptr<const Reflection::Tuple> getGlobalGuiInset2();
    ...
    bool getGamepadNavEnabled() const
    {
        return gamepadNavEnabled;
    }
    void setGamepadNavEnabled(bool value);

    bool getCoreGamepadNavEnabled() const
    {
        return coreGamepadNavEnabled;
    }
    ...
    typedef boost::function<void(
        std::string, std::string, Reflection::AsyncCallbackDescriptor::ResumeFunction, Reflection::AsyncCallbackDescriptor::ErrorFunction)>
