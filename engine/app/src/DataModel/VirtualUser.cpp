...
static Reflection::BoundFuncDesc<VirtualUser, void(Aya::Vector2, Aya::CoordinateFrame)> func_ClickButton1(
    &VirtualUser::clickButton1, "ClickButton1", "position", "camera", Aya::CoordinateFrame(), Security::TestLocalUser);
static Reflection::BoundFuncDesc<VirtualUser, void(Aya::Vector2, Aya::CoordinateFrame)> func_Button1Down(
    &VirtualUser::button1Down, "Button1Down", "position", "camera", Aya::CoordinateFrame(), Security::TestLocalUser);
static Reflection::BoundFuncDesc<VirtualUser, void(Aya::Vector2, Aya::CoordinateFrame)> func_Button1Up(
    &VirtualUser::button1Up, "Button1Up", "position", "camera", Aya::CoordinateFrame(), Security::TestLocalUser);
static Reflection::BoundFuncDesc<VirtualUser, void(Aya::Vector2, Aya::CoordinateFrame)> func_ClickButton2(
    &VirtualUser::clickButton2, "ClickButton2", "position", "camera", Aya::CoordinateFrame(), Security::TestLocalUser);
static Reflection::BoundFuncDesc<VirtualUser, void(Aya::Vector2, Aya::CoordinateFrame)> func_Button2Down(
    &VirtualUser::button2Down, "Button2Down", "position", "camera", Aya::CoordinateFrame(), Security::TestLocalUser);
static Reflection::BoundFuncDesc<VirtualUser, void(Aya::Vector2, Aya::CoordinateFrame)> func_Button2Up(
    &VirtualUser::button2Up, "Button2Up", "position", "camera", Aya::CoordinateFrame(), Security::TestLocalUser);
static Reflection::BoundFuncDesc<VirtualUser, void(Aya::Vector2, Aya::CoordinateFrame)> func_MoveMouse(
    &VirtualUser::moveMouse, "MoveMouse", "position", "camera", Aya::CoordinateFrame(), Security::TestLocalUser);
...
static Reflection::BoundFuncDesc<VirtualUser, std::string()> func_StopRecording(
    &VirtualUser::stopRecording, "StopRecording", Security::TestLocalUser);
static Reflection::BoundFuncDesc<VirtualUser, void()> func_CaptureController(
    &VirtualUser::captureInputDevice, "CaptureController", Security::TestLocalUser);
...
static Reflection::BoundFuncDesc<VirtualUser, void(std::string)> func_SetKeyDown(
    &VirtualUser::setKeyDown, "SetKeyDown", "key", Security::TestLocalUser);
...
public:
    ...
    virtual void setKeyState(Aya::KeyCode code, Aya::ModCode modCode, char modifiedKey, bool isDown)
    {
        if (code < 0)
            return;
...
void VirtualUser::sendMouseEvent(
    InputObject::UserInputType eventType, InputObject::UserInputState eventState, Aya::Vector2 position, Aya::CoordinateFrame camera)
{
    ...
    const shared_ptr<InputObject>& downEvent =
        Creatable<Instance>::create<InputObject>(eventType, eventState, Vector3(mousePosition, 0), Vector3::zero(), Aya::DataModel::get(this));

    Camera* c = getDataModel()->getWorkspace()->getCamera();
    if (c && camera != CoordinateFrame())
    {
        // Aya::CoordinateFrame oldCamera = c->getCameraCoordinateFrame();
...
void VirtualUser::moveMouse(Aya::Vector2 position, Aya::CoordinateFrame camera)
{
    sendMouseEvent(InputObject::TYPE_MOUSEMOVEMENT, InputObject::INPUT_STATE_CHANGE, position, camera);
}

void VirtualUser::clickButton1(Aya::Vector2 position, Aya::CoordinateFrame camera)
{
    button1Down(position, camera);
    button1Up(position, camera);
}
void VirtualUser::button1Down(Aya::Vector2 position, Aya::CoordinateFrame camera)
{
    sendMouseEvent(InputObject::TYPE_MOUSEBUTTON1, InputObject::INPUT_STATE_BEGIN, position, camera);
}
void VirtualUser::button1Up(Aya::Vector2 position, Aya::CoordinateFrame camera)
{
    sendMouseEvent(InputObject::TYPE_MOUSEBUTTON1, InputObject::INPUT_STATE_END, position, camera);
}

void VirtualUser::clickButton2(Aya::Vector2 position, Aya::CoordinateFrame camera)
{
    button2Down(position, camera);
    button2Up(position, camera);
}
void VirtualUser::button2Down(Aya::Vector2 position, Aya::CoordinateFrame camera)
{
    sendMouseEvent(InputObject::TYPE_MOUSEBUTTON2, InputObject::INPUT_STATE_BEGIN, position, camera);
}
void VirtualUser::button2Up(Aya::Vector2 position, Aya::CoordinateFrame camera)
{
...
KeyCode VirtualUser::convert(const std::string& key)
{
    ...
    if (key.length() != 1)
        throw Aya::runtime_error("Unsupported key %s", key.c_str());
...
void VirtualUser::setKeyDown(std::string key)
{
    ...
    virtualHardwareDevice->setKeyState(keyCode, Aya::AYA_KMOD_NONE, 0, true);

    const shared_ptr<InputObject>& downEvent = Creatable<Instance>::create<InputObject>(
        InputObject::TYPE_KEYBOARD, InputObject::INPUT_STATE_BEGIN, keyCode, (ModCode)0, (char)0, Aya::DataModel::get(this));
    getDataModel()->processInputObject(downEvent);
}
...
void VirtualUser::setKeyUp(std::string key)
{
    ...
    virtualHardwareDevice->setKeyState(keyCode, Aya::AYA_KMOD_NONE, 0, false);

    const shared_ptr<InputObject>& upEvent = Creatable<Instance>::create<InputObject>(
        InputObject::TYPE_KEYBOARD, InputObject::INPUT_STATE_END, keyCode, (ModCode)0, (char)0, Aya::DataModel::get(this));
...
static Aya::Vector2 toNormalized(Vector2int16 windowSize, const Vector2int16 mousePosition)
{
    Vector2 position;
...
{
    writeWait();
    recording << "virtualUser:" << func << "('" << Aya::format("0x%02x", event->getKeyCode()) << "')";
...
void VirtualUser::writeMouse(const char* func, const shared_ptr<InputObject>& event)
{
    ...
    recording << "CFrame.new(" << camera.translation.x << ", " << camera.translation.y << ", " << camera.translation.z << ", " << q.x << ", " << q.y
              << ", " << q.z << ", " << q.w << ")";
    recording << ")\n";
}
