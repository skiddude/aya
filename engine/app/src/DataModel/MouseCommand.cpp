...
Instance* MouseCommand::getTopSelectable3d(PartInstance* part) const
{
    ...
    while (temp && temp != workspace)
    {
        ...
        answer = selectable && selectable->isSelectable3d() ? temp : answer;
        temp = temp->getParent();
    }
    return answer;
}
...
{
    std::string asset = getCursorName();

    // hack
    if (asset.find('/') == std::string::npos)
    {
        asset = "textures/" + getCursorName() + ".png";

        if (getCursorName() == "StudsCursor" || getCursorName() == "InletCursor" || getCursorName() == "UniversalCursor")
        {
    ...
    {
        return TextureId(asset.c_str());
    }
}

void MouseCommand::capture()
{
    AYAASSERT(!workspace->getCurrentMouseCommand()->captured());
    capturedMouse = true;
}
...
Surface MouseCommand::getSurface(
    Workspace* workspace, const shared_ptr<InputObject>& inputObject, const HitTestFilter* filter, PartInstance*& part, int& surfaceId)
{
    ...
    {
        Aya::RbxRay gridRay = getUnitMouseRay(inputObject, workspace);
...
PartInstance* MouseCommand::getPartByLocalCharacter(
    Workspace* workspace, const shared_ptr<InputObject>& inputObject, const HitTestFilter* filter, Vector3& hitWorld)
{
...
PartInstance* MouseCommand::getPart(Workspace* workspace, const shared_ptr<InputObject>& inputObject, const HitTestFilter* filter, Vector3& hitWorld)
{
    ...
    PartInstance* bestPart = getMousePart(unitRay, *contactManager, ignorePrims, filter, hitWorld);
...
PartInstance* MouseCommand::getMousePart(const RbxRay& unitRay, const ContactManager& contactManager, const std::vector<const Primitive*>& ignore,
    const HitTestFilter* filter, Vector3& hitPoint, float maxSearchGrid)
{
    if (!unitRay.direction().isUnit())
        return NULL;
    RbxRay searchRay = RbxRay::fromOriginAndDirection(unitRay.origin(), unitRay.direction() * maxSearch());

    Primitive* bestPrimitive = contactManager.getHit(searchRay, &ignore, filter, hitPoint);

    return bestPrimitive ? PartInstance::fromPrimitive(bestPrimitive) : NULL;
}


PartInstance* MouseCommand::getMousePart(const RbxRay& unitRay, const ContactManager& contactManager, const Primitive* ignore,
    const HitTestFilter* filter, Vector3& hitPoint, float maxSearchGrid)
{
    std::vector<const Primitive*> ignorePrims;
