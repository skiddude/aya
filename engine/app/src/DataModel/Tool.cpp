...
static const Reflection::PropDescriptor<Tool, CoordinateFrame> prop_Grip(
    "Grip", category_Appearance, &Tool::getGrip, &Tool::setGrip, Reflection::PropertyDescriptor::STANDARD);
static const Reflection::PropDescriptor<Tool, Vector3> prop_GripPos(
    "GripPos", category_Appearance, &Tool::getGripPos, &Tool::setGripPos, Reflection::PropertyDescriptor::UI);
static const Reflection::PropDescriptor<Tool, Vector3> prop_GripForward(
    "GripForward", category_Appearance, &Tool::getGripForward, &Tool::setGripForward, Reflection::PropertyDescriptor::UI);
static const Reflection::PropDescriptor<Tool, Vector3> prop_GripUp(
    "GripUp", category_Appearance, &Tool::getGripUp, &Tool::setGripUp, Reflection::PropertyDescriptor::UI);
static const Reflection::PropDescriptor<Tool, Vector3> prop_GripRight(
    "GripRight", category_Appearance, &Tool::getGripRight, &Tool::setGripRight, Reflection::PropertyDescriptor::UI);
...
static const Reflection::PropDescriptor<Tool, bool> prop_manualActivation(
    "ManualActivationOnly", category_Behavior, &Tool::getManualActivationOnly, &Tool::setManualActivationOnly);
...
static const Reflection::PropDescriptor<Tool, bool> prop_requiresHandle(
    "RequiresHandle", category_Behavior, &Tool::getRequiresHandle, &Tool::setRequiresHandle);
...
static Reflection::RemoteEventDesc<Tool, void()> event_Activated(
    &Tool::activatedSignal, "Activated", Security::None, Reflection::RemoteEventCommon::SCRIPTING, Reflection::RemoteEventCommon::CLIENT_SERVER);
static Reflection::RemoteEventDesc<Tool, void()> event_Deactivated(
    &Tool::deactivatedSignal, "Deactivated", Security::None, Reflection::RemoteEventCommon::SCRIPTING, Reflection::RemoteEventCommon::CLIENT_SERVER);
...
Tool::~Tool()
{
    ...
    AYAASSERT(!torsoChildAdded.connected());
    AYAASSERT(!torsoChildRemoved.connected());
}
        ...
        if (Workspace* workspaceLocal = Workspace::findWorkspace(character))
        {
            while (Tool* tool = character->findFirstChildOfType<Tool>())
            {
                if (!tool->isDroppable())
                    ...
                    tool->setParent(workspaceLocal); // ok - should drop!  check it out
            }
        }
    }
}
...
void Tool::setBackendToolState(int value)
{
    if (value != backendToolState)
    {
        ...
        if ((backendToolState >= EQUIPPED) && (value < EQUIPPED))
        {
            ...
            if (currentMouse)
            {
                currentMouse->setIcon(Aya::TextureId());
            }
            onUnequipped();
        }
...
void Tool::connectTouchEvent()
{
    ...
    if (handle)
    {
        ...
        FASTLOG3(FLog::TouchedSignal, "Connecting Tool to touched signal, instance: %p, part: %p, part signal: %p", this, handle,
...
void Tool::setDesiredState(ToolState desiredState, const ServiceProvider* serviceProvider)
{
...
#endif
    AYAASSERT((numToolsInCharacter = getNumToolsInCharacter()) || 1);
    ...
    else
    {
        ...
        else if (desiredState < currentState)
        {
            do
            {
                ...
                {
                case EQUIPPED:
                    downFrom_Equipped(Network::Players::backendProcessing(serviceProvider));
                    break;
                case HAS_TORSO:
                    downFrom_HasTorso();
                    break;
    ...
    AYAASSERT(numToolsInCharacter == getNumToolsInCharacter());
}
...
void Tool::upTo_InCharacter()
{
    ...
    if (getParent())
    {
        ...
        characterChildRemoved =
            getParent()->onDemandWrite()->childRemovedSignal.connect(boost::bind(&Tool::onEvent_RemovedBackend, shared_from(this), _1));
    }
    else
    {
...
void Tool::upTo_Equipped()
{
    ...
    else if (Network::Players::backendProcessing(this))
    {
        if (handle)
        {
            ...
            if (!weld)
                armChildAdded =
                    arm->onDemandWrite()->childAddedSignal.connect(boost::bind(&Tool::onEvent_AddedToArmBackend, shared_from(this), _1, handle));
...
void Tool::onEvent_RemovedBackend(shared_ptr<Instance> child)
{
    ...
    if (ServiceProvider::findServiceProvider(this))
    {
        if (child.get() != this) // tool parent changed is handled in ancestor stuff, because this might not be hooked up
        {
            ...
            {
                AYAASSERT(backendToolState >= EQUIPPED);
...
void Tool::setTimerCallback(weak_ptr<Network::Player> player)
{
    ...
    if (callback)
    {
        boost::function<void()> func = boost::bind(&Tool::moveOtherToolsToBackpack, this, player);
...
static void moveToBackpack(shared_ptr<Instance> child, Tool* toolToKeep, Backpack* backpack)
{
...
void Tool::onAncestorChanged(const AncestorChanged& event)
{
    ...
    if (newProvider)
    {
        ...
        AYAASSERT(ServiceProvider::findServiceProvider(this) == newProvider);
...
void Tool::setMousePositionForInputType()
{
    if (Aya::UserInputService* inputService = Aya::ServiceProvider::find<Aya::UserInputService>(this))
    {
        ...
        else if (inputService->getTouchEnabled())
        {
            ...
            Aya::StandardOut::singleton()->printf(MESSAGE_INFO, "current num of touches is %lu", currentTouches.size());
            if (currentTouches.size() > 0)
            {
                ...
                shared_ptr<InputObject> fakeMouseActivatePosition =
                    Aya::Creatable<Aya::Instance>::create<Aya::InputObject>(InputObject::TYPE_MOUSEMOVEMENT, InputObject::INPUT_STATE_CHANGE,
                        Vector3(avgTouchPosition.x, avgTouchPosition.y, 0), Vector3::zero(), Aya::DataModel::get(this));
...
void Tool::luaActivate()
{
    ...
    else
    {
        Aya::StandardOut::singleton()->printf(
            MESSAGE_WARNING, "Tool:Activate() called from script when tool is not equipped. Tool will not be activated.");
    }
}
...
void Tool::activate(bool manuallyActivated)
{
    ...
    AYAASSERT(Network::Players::frontendProcessing(this));
...
void Tool::onLocalClicked()
{
    ...
    AYAASSERT(this->getNumToolsInCharacter() < 2);
}
