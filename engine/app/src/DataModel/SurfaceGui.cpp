...
static const Reflection::RefPropDescriptor<SurfaceGui, Instance> prop_adornee(
    "Adornee", category_Data, &SurfaceGui::getAdornee, &SurfaceGui::setAdornee);
...
static const Reflection::PropDescriptor<SurfaceGui, Vector2> prop_CanvasSize(
    "CanvasSize", category_Data, &SurfaceGui::getCanvasSize, &SurfaceGui::setCanvasSize);
...
static const Reflection::PropDescriptor<SurfaceGui, float> prop_maxDistance(
    "ToolPunchThroughDistance", category_Data, &SurfaceGui::getToolPunchThroughDistance, &SurfaceGui::setToolPunchThroughDistance);
REFLECTION_END();
...
{
    return !!Network::Players::findLocalPlayer(this) && this->isDescendantOf(Aya::ServiceProvider::find<Aya::StarterGuiService>(this));
}

void SurfaceGui::unProcess()
{
    shared_ptr<InputObject> mouseMoveEvent =
        Creatable<Instance>::create<InputObject>(InputObject::TYPE_MOUSEMOVEMENT, InputObject::INPUT_STATE_CANCEL, Aya::DataModel::get(this));
    mouseMoveEvent->setPosition(Vector3(-1e4, -1e4, -1e4));
    Super::process(mouseMoveEvent);

    shared_ptr<InputObject> focusEndEvent =
        Creatable<Instance>::create<InputObject>(InputObject::TYPE_FOCUS, InputObject::INPUT_STATE_CANCEL, Aya::DataModel::get(this));
...
GuiResponse SurfaceGui::process3d(const shared_ptr<InputObject>& event, Vector3 point3d, bool ignoreMaxDistance)
{
    ...
    if (Aya::ModelInstance* character = Network::Players::findLocalCharacter(this))
    {
