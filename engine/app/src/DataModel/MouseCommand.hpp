...
public:
    ...
    static PartInstance* getPart(
        Workspace* workspace, const shared_ptr<InputObject>& inputObject, const HitTestFilter* filter = NULL, Vector3& hitWorld = ignoreVector3);
    PartInstance* getUnlockedPart(const shared_ptr<InputObject>& inputObject, Vector3& hitWorld = ignoreVector3);
    static PartInstance* getPartByLocalCharacter(
        Workspace* workspace, const shared_ptr<InputObject>& inputObject, const HitTestFilter* filter = NULL, Vector3& hitWorld = ignoreVector3);
    ...
    static Surface getSurface(
        Workspace* workspace, const shared_ptr<InputObject>& inputObject, const HitTestFilter* filter, PartInstance*& part, int& surfaceId);
    ...
    virtual shared_ptr<MouseCommand> onRightMouseDown(const shared_ptr<InputObject>& inputObject)
    {
        return shared_from(this);
    }
...
protected:
    ...
    virtual shared_ptr<MouseCommand> onKeyDown(const shared_ptr<InputObject>& inputObject)
    {
        return shared_from(this);
    }
    virtual shared_ptr<MouseCommand> onKeyUp(const shared_ptr<InputObject>& inputObject)
    {
        return shared_from(this);
    }
    ...
    virtual shared_ptr<MouseCommand> onPeekKeyUp(const shared_ptr<InputObject>& inputObject)
    {
        return shared_ptr<MouseCommand>();
    }
    ...
    virtual shared_ptr<MouseCommand> onRightMouseUp(const shared_ptr<InputObject>& inputObject)
    {
        return shared_from(this);
    }
    virtual shared_ptr<MouseCommand> onMouseWheelForward(const shared_ptr<InputObject>& inputObject)
    {
        return shared_ptr<MouseCommand>();
    }
    virtual shared_ptr<MouseCommand> onMouseWheelBackward(const shared_ptr<InputObject>& inputObject)
    {
        return shared_ptr<MouseCommand>();
    }
...
public:
    ...
    virtual shared_ptr<MouseCommand> isSticky() const
    {
        return shared_ptr<MouseCommand>();
    }
    virtual bool drawConnectors() const
    {
        return false;
    }                                      // default mouse command no draw connectors
    virtual TextureId getCursorId() const; // called on draw

    static PartInstance* getMousePart(const RbxRay& unitRay, const ContactManager& contactManager, const Primitive* ignore,
        const HitTestFilter* filter, Vector3& hitPoint = ignoreVector3, float maxSearchGrid = maxSearch());

    static PartInstance* getMousePart(const RbxRay& unitRay, const ContactManager& contactManager, const std::vector<const Primitive*>& ignore,
        const HitTestFilter* filter, Vector3& hitPoint = ignoreVector3, float maxSearchGrid = maxSearch());
    ...
    static void enableAdvArrowTool(bool state)
    {
        advArrowToolEnabled = state;
    }
    static bool isAdvArrowToolEnabled()
    {
        return advArrowToolEnabled;
    }
...
private:
    ...
    {
        return isAdvArrowToolEnabled() ? "advCursor-default" : "ArrowCursor";
    }
};
