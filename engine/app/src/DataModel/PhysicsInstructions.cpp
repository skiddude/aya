...
#include "DataModel/PhysicsInstructions.hpp" // TODO - minimize these includes, and in the .h file
...
void PhysicsInstructions::changeSimulationRadius(Aya::Network::Player* dPhysPlayer, float change)
{
    timeSinceLastRadiusChange = 0.0;

    AYAASSERT(dPhysPlayer);
...
void PhysicsInstructions::changeMaxSimulationRadius(Aya::Network::Player* dPhysPlayer, float change)
{
    AYAASSERT(dPhysPlayer);
...
double PhysicsInstructions::dPhysicsClientDutyPercent()
{
    return 0.40;
} // note this is higher than normal
double PhysicsInstructions::dPhysicsClientEThrottleDutyPercent()
{
    return 0.60;
} // note this is higher than normal
...
void PhysicsInstructions::setThrottlesBase(
...
void PhysicsInstructions::setCyclicThrottles(Aya::Network::Player* dPhysPlayer, Workspace* workspace, double cyclicDt, double dt, double dutyDt)
{
...
void PhysicsInstructions::setThrottles(Aya::Network::Player* dPhysPlayer, Workspace* workspace, double dt, double dutyDt)
{
    averageDt.sample(dt);
    averageDutyDt.sample(dutyDt);
