...
const char* const Aya::sTeleportService = "TeleportService";


static Reflection::BoundYieldFuncDesc<TeleportService, shared_ptr<const Reflection::Tuple>(int)> func_GetPlayerPlaceInstanceAsync(
    &TeleportService::GetPlayerPlaceInstanceAsync, "GetPlayerPlaceInstanceAsync", "userId", Security::None);
static Reflection::BoundFuncDesc<TeleportService,
    void(int, std::string, shared_ptr<Instance>, std::string, Reflection::Variant, shared_ptr<Instance>)>
    func_TeleportToPlaceInstance(&TeleportService::TeleportToPlaceInstance, "TeleportToPlaceInstance", "placeId", "instanceId", "player",
        shared_ptr<Instance>(), "spawnName", std::string(), "teleportData", Reflection::Variant(), "customLoadingScreen", shared_ptr<Instance>(),
        Security::None);

static Reflection::BoundFuncDesc<TeleportService, void(int, std::string, shared_ptr<Instance>, Reflection::Variant, shared_ptr<Instance>)>
    func_TeleportToSpawnByName(&TeleportService::TeleportToSpawnByName, "TeleportToSpawnByName", "placeId", "spawnName", "player",
        shared_ptr<Instance>(), "teleportData", Reflection::Variant(), "customLoadingScreen", shared_ptr<Instance>(), Security::None);
static Reflection::BoundFuncDesc<TeleportService, void(int, shared_ptr<Instance>, Reflection::Variant, shared_ptr<Instance>)> func_Teleport(
    &TeleportService::Teleport, "Teleport", "placeId", "player", shared_ptr<Instance>(), "teleportData", Reflection::Variant(), "customLoadingScreen",
    shared_ptr<Instance>(), Security::None);

static Reflection::BoundYieldFuncDesc<TeleportService, std::string(int)> func_ReserveServer(
    &TeleportService::ReserveServer, "ReserveServer", "placeId", Security::None);
static Reflection::BoundFuncDesc<TeleportService,
    void(int, std::string, shared_ptr<const Instances>, std::string, Reflection::Variant, shared_ptr<Instance>)>
    func_TeleportToPrivateServer(&TeleportService::TeleportToPrivateServer, "TeleportToPrivateServer", "placeId", "reservedServerAccessCode",
        "players", "spawnName", std::string(), "teleportData", Reflection::Variant(), "customLoadingScreen", shared_ptr<Instance>(), Security::None);

static Reflection::BoundFuncDesc<TeleportService, Reflection::Variant(std::string)> func_GetTeleportSetting(
    &TeleportService::GetTeleportSetting, "GetTeleportSetting", "setting", Security::None);
static Reflection::BoundFuncDesc<TeleportService, void(std::string, Reflection::Variant)> func_SetTeleportSetting(
    &TeleportService::SetTeleportSetting, "SetTeleportSetting", "setting", "value", Security::None);

static Reflection::BoundCallbackDesc<bool(std::string, int, std::string)> callback_ConfirmationCallback(
    "ConfirmationCallback", &TeleportService::confirmationCallback, "message", "placeId", "spawnName", Security::RobloxScript);
static Reflection::BoundCallbackDesc<void(std::string)> errorCallback(
    "ErrorCallback", &TeleportService::showErrorCallback, "message", Security::RobloxScript);
static Reflection::BoundFuncDesc<TeleportService, void()> func_TeleportCancel(
    &TeleportService::TeleportCancel, "TeleportCancel", Security::RobloxScript);

static Reflection::BoundFuncDesc<TeleportService, Reflection::Variant()> func_GetLocalTeleportData(
    &TeleportService::getLocalPlayerTeleportData, "GetLocalPlayerTeleportData", Security::None);

static Reflection::BoundProp<bool> prop_CustomizedTeleportUI(
    "CustomizedTeleportUI", category_Data, &TeleportService::customizedTeleportUI, Reflection::PropertyDescriptor::Attributes::deprecated());

Reflection::EventDesc<TeleportService, void(shared_ptr<Instance>, Reflection::Variant)> event_playerArrivedFromTeleport(
    &TeleportService::playerArrivedFromTeleportSignal, "LocalPlayerArrivedFromTeleport", "loadingGui", "dataTable");
REFLECTION_END();
...
{
    setName("TeleportService");
    _baseUrl = GetBaseURL(); // hack
}
...
{
    AYAASSERT(!Workspace::serverIsPresent(this));
    ...
    try
    {
        ...
        AYAASSERT(!_baseUrl.empty());

#if defined(AYA_PLATFORM_DURANGO)
        char extraStuff[1024];
        sprintf_s(extraStuff, 1024, "&gamerTag=%s",
            _callback->xBox_getGamerTag().c_str()); // We'll probably drop this crap in the future. Ask me in 5 years.       -- Max
#else
        ...
        if (FFlag::UseBuildGenericGameUrl)
        {
            if (teleportInfo->at("teleportType").get<TeleportType>() == TeleportType_ToInstance)
            {
                ...
                char urlBuf[2048] = {0};
                sprintf_s(urlBuf, 2048,
                    "Game/PlaceLauncher.ashx?request=RequestGameJob&placeId=%d&gameId=%s&isPartyLeader=false&gender=&isTeleport=true%s",
                    teleportInfo->at("placeId").get<int>(), teleportInfo->at("instanceId").get<std::string>().c_str(), extraStuff);
            ...
            else if (teleportInfo->at("teleportType").get<TeleportType>() == TeleportType_ToPlace)
            {
                ...
                _spawnName = teleportInfo->at("spawnName").get<std::string>();
                char urlBuf[2048] = {0};
                sprintf_s(urlBuf, 2048, "Game/PlaceLauncher.ashx?request=RequestGame&placeId=%d&isPartyLeader=false&gender=&isTeleport=true%s",
                    teleportInfo->at("placeId").get<int>(), extraStuff);
                url = BuildGenericGameUrl(_baseUrl, urlBuf);
            }
            else if (teleportInfo->at("teleportType").get<TeleportType>() == TeleportType_ToReservedServer)
            {
                ...
                char urlBuf[2048] = {0};
                sprintf_s(urlBuf, 2048,
                    "Game/PlaceLauncher.ashx?request=RequestPrivateGame&placeId=%d&accessCode=%s&linkCode=&privateGameMode=ReservedServer%s",
                    teleportInfo->at("placeId").get<int>(),
                    Aya::Http::urlEncode(teleportInfo->at("reservedServerAccessCode").get<std::string>()).c_str(), extraStuff);
                url = BuildGenericGameUrl(_baseUrl, urlBuf);
            }
        }
        else
        {
            if (teleportInfo->at("teleportType").get<TeleportType>() == TeleportType_ToInstance)
            {
                ...
                char urlBuf[2048] = {0};
                sprintf_s(urlBuf, 2048,
                    "%s/Game/PlaceLauncher.ashx?request=RequestGameJob&placeId=%d&gameId=%s&isPartyLeader=false&gender=&isTeleport=true%s",
                    _baseUrl.c_str(), teleportInfo->at("placeId").get<int>(), teleportInfo->at("instanceId").get<std::string>().c_str(), extraStuff);
                url = urlBuf;
                _spawnName = teleportInfo->at("spawnName").get<std::string>();
            }
            else if (teleportInfo->at("teleportType").get<TeleportType>() == TeleportType_ToPlace)
            {
                ...
                _spawnName = teleportInfo->at("spawnName").get<std::string>();
                char urlBuf[2048] = {0};
                sprintf_s(urlBuf, 2048, "%s/Game/PlaceLauncher.ashx?request=RequestGame&placeId=%d&isPartyLeader=false&gender=&isTeleport=true%s",
                    _baseUrl.c_str(), teleportInfo->at("placeId").get<int>(), extraStuff);
                url = urlBuf;
            }
            else if (teleportInfo->at("teleportType").get<TeleportType>() == TeleportType_ToReservedServer)
            {
                ...
                char urlBuf[2048] = {0};
                sprintf_s(urlBuf, 2048,
                    "%s/Game/PlaceLauncher.ashx?request=RequestPrivateGame&placeId=%d&accessCode=%s&linkCode=&privateGameMode=ReservedServer%s",
                    _baseUrl.c_str(), teleportInfo->at("placeId").get<int>(),
                    Aya::Http::urlEncode(teleportInfo->at("reservedServerAccessCode").get<std::string>()).c_str(), extraStuff);
                url = urlBuf;
                _spawnName = teleportInfo->at("spawnName").get<std::string>();
            }
        ...
        retryTimer.reset();
        teleportThread.reset(new boost::thread(
            Aya::thread_wrapper(boost::bind(&TeleportService::TeleportThreadImpl, shared_from(this), teleportInfo), "Teleport Thread")));
    }
...
void TeleportService::TeleportThreadImpl(shared_ptr<const Reflection::ValueTable> teleportInfo)
{
    ...
    while (true)
    {
        ...
        else
        {
            try
            {
                ...
                if (parseResult)
                {
                    ...
                    else
                    {
                        ...
                        {
                            cancelable = false; // at this point, the teleport action cannot be canceled anymore because the web service could
                                                // disconnect the player from current server at any time
                                                // fire teleport signal
                            try
                            {
                ...
                if (found)
                {
                    ...
                    if (Aya::GameBasicSettings::singleton().inStudioMode())
                    {
                        ...
                        int universeId = v.cast<shared_ptr<const Reflection::ValueTable>>()->at("UniverseId").cast<int>();
                        ...
                        {
                            sprintf_s(urlBuf, 2048, "%s/Game/Visit.ashx?placeID=%d&IsPlaySolo=1&FromTeleport=1&universeId=%d", _baseUrl.c_str(),
                                teleportInfo->at("placeId").get<int>(), universeId);
                            script = urlBuf;
                        }
                    }
                ...
                else if (retries < 0)
                {
                    ...
                    try
                    {
                        ...
                        if (Network::Player* player = Network::Players::findLocalPlayer(this))
                        {
                            ...
                            AYAASSERT(teleportInfo->at("placeId").get<int>() >= -1);
            ...
            double timeToSleepMs = 1000.f - retryTimer.delta().msec();
            if (timeToSleepMs > 0.f)
            {
                boost::this_thread::sleep(boost::posix_time::milliseconds(static_cast<long>(timeToSleepMs)));
            }
            retryTimer.reset();
        }
    }
...
void TeleportService::ServerTeleport(
    shared_ptr<Instance> characterOrPlayerInstance, shared_ptr<Reflection::ValueTable> teleportInfo, shared_ptr<Instance> customLoadingGUI)
{
    AYAASSERT(Workspace::serverIsPresent(this));
    ...
    if (!player)
    {
        ...
        if (player)
        {
            StandardOut::singleton()->printf(
                MESSAGE_WARNING, "Passing Character instance into teleport functions is deprecated. Please use Player instance instead.");
        }
    }
    if (player)
    {
        ...
        if (customLoadingGUI)
        {
            customLoadingGUI->setParent(ServiceProvider::create<InsertService>(
                this)); // customLoadingGui needs to be parented to a Replication Container so it can be replicated to the client
        }
...
void TeleportService::Teleport(
    int placeId, shared_ptr<Instance> characterOrPlayerInstance, Reflection::Variant teleportData, shared_ptr<Instance> customLoadingGUI)
{
    TeleportToSpawnByName(placeId, "", characterOrPlayerInstance, teleportData, customLoadingGUI);
}

void TeleportService::TeleportToSpawnByName(int placeId, std::string spawnName, shared_ptr<Instance> characterOrPlayerInstance,
    Reflection::Variant teleportData, shared_ptr<Instance> customLoadingGUI)
{
    if (placeId <= 0)
    {
...
void TeleportService::ProcessGetPlayerPlaceInstanceResultsSuccess(
    std::string response, boost::function<void(shared_ptr<const Reflection::Tuple>)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
...
void TeleportService::GetPlayerPlaceInstanceAsync(
    int playerId, boost::function<void(shared_ptr<const Reflection::Tuple>)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    if (Aya::HttpRbxApiService* apiService = Aya::ServiceProvider::find<Aya::HttpRbxApiService>(this))
    {
        ...
        apiService->getAsync(url, true, Aya::PRIORITY_SERVER_ELEVATED,
...
void TeleportService::TeleportToPlaceInstance(int placeId, std::string instanceId, shared_ptr<Instance> characterOrPlayerInstance,
    std::string spawnName, Reflection::Variant teleportData, shared_ptr<Instance> customLoadingGUI)
{
    shared_ptr<Reflection::ValueTable> teleportInfo(new Reflection::ValueTable);
    (*teleportInfo)["placeId"] = placeId;
    (*teleportInfo)["spawnName"] = spawnName;
...
void TeleportService::ProcessGrantAccessSuccess(
    shared_ptr<const Instances> players, shared_ptr<Reflection::ValueTable> teleportInfo, shared_ptr<Instance> customLoadingGUI, std::string response)
{
...
void TeleportService::TeleportToPrivateServer(int placeId, std::string reservedServerAccessCode, shared_ptr<const Instances> players,
    std::string spawnName, Reflection::Variant teleportData, shared_ptr<Instance> customLoadingGUI)
{
    if (Workspace::serverIsPresent(this))
    {
        ...
        char urlBuf[2048] = {0};
        sprintf_s(urlBuf, 2048, "reservedservers/grantaccess?reservedServerAccessCode=%s", Aya::Http::urlEncode(reservedServerAccessCode).c_str());
        url = urlBuf;
        ...
        if (playerFound)
        {
            if (Aya::HttpRbxApiService* apiService = Aya::ServiceProvider::find<Aya::HttpRbxApiService>(this))
            {
                ...
                apiService->postAsync(url, playerIdsStream.str(), true, Aya::PRIORITY_SERVER_ELEVATED, HttpService::APPLICATION_URLENCODED,
...
void TeleportService::ProcessReserveServerResultsSuccess(
    std::string response, boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
...
void TeleportService::ReserveServer(int placeId, boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    if (Workspace::serverIsPresent(this))
    {
        if (Aya::HttpRbxApiService* apiService = Aya::ServiceProvider::find<Aya::HttpRbxApiService>(this))
        {
            ...
            apiService->postAsync(url, "", true, Aya::PRIORITY_SERVER_ELEVATED, HttpService::APPLICATION_URLENCODED,
