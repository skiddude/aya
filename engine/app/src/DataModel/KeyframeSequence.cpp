...
DYNAMIC_FASTFLAGVARIABLE(AnimationEasingStylesEnabled, true)
...
static Reflection::BoundFuncDesc<KeyframeSequence, shared_ptr<const Instances>()> func_getKeyframes(
    &KeyframeSequence::getKeyframes, "GetKeyframes", Security::None);
static Reflection::BoundFuncDesc<KeyframeSequence, void(shared_ptr<Instance>)> func_addKeyframe(
    &KeyframeSequence::addKeyframe, "AddKeyframe", "keyframe", Security::None);
static Reflection::BoundFuncDesc<KeyframeSequence, void(shared_ptr<Instance>)> func_removeKeyframe(
    &KeyframeSequence::removeKeyframe, "RemoveKeyframe", "keyframe", Security::None);
...
const Reflection::EnumPropDescriptor<KeyframeSequence, KeyframeSequence::Priority> prop_Priority(
    "Priority", category_Data, &KeyframeSequence::getPriority, &KeyframeSequence::setPriority);
REFLECTION_END();
...
{
template<>
EnumDesc<Aya::KeyframeSequence::Priority>::EnumDesc()
    ...
    for (size_t i = 0; i < cache.animatedJoints.size(); ++i)
    {
        ...
        for (size_t j = 0; j < jointposes.size(); ++j)
        {
            if (jointposes[j].joint.first.lock() != NULL)
            {
                ...
                if (joint != NULL && joint->getParentName() == parentName && joint->getPartName() == childName)
                {
...
Vector3 lerpAxisAngle(const Vector3& r0, const Vector3& r1, float w0, float w1)
{
    ...
    if (dot < 0) // vectors oposite. shortest path _possibly_ not the linear lerp.
    {
        float l0 = r0.length();
        float l1 = r1.length();
        AYAASSERT(l0 <= Math::pif() + 0.01 && l1 <= Math::pif() + 0.01);
        if (!(l0 < l1))
        {
