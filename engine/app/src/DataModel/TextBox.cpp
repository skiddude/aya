...
static const Reflection::PropDescriptor<TextBox, bool> prop_ClearTextOnFocus(
    "ClearTextOnFocus", category_Data, &TextBox::getClearTextOnFocus, &TextBox::setClearTextOnFocus);
...
static const Reflection::EventDesc<TextBox, void(bool, const shared_ptr<Instance>)> event_FocusLost(
    &TextBox::focusLostSignal, "FocusLost", "enterPressed", "inputThatCausedFocusLoss");
...
void TextBox::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
    ...
    if (newProvider)
    {
        ...
        if (Aya::UserInputService* userInputService = Aya::ServiceProvider::create<Aya::UserInputService>(newProvider))
            textBoxFinishedEditingConnection =
                userInputService->textBoxFinishedEditing.connect(bind(&TextBox::externalReleaseFocus, this, _1, _2, _3));
    }
}
...
GuiResponse TextBox::process(const shared_ptr<InputObject>& event)
{
    ...
    if (event->getUserInputType() == InputObject::TYPE_FOCUS && event->getUserInputState() == InputObject::INPUT_STATE_END)
    {
...
GuiResponse TextBox::processGamepadEvent(const shared_ptr<InputObject>& event)
{
    ...
    if (event->getKeyCode() == AYA_SDLK_GAMEPAD_BUTTONA && event->getUserInputState() == InputObject::INPUT_STATE_BEGIN)
    {
        if (isSelectedObject())
        {
...
int TextBox::getCursorPos(Aya::Vector2 mousePos)
{
...
void TextBox::gainFocus(const shared_ptr<InputObject>& event)
{
    ...
    if (Aya::UserInputService* userInputService = Aya::ServiceProvider::create<Aya::UserInputService>(this))
...
void TextBox::captureFocus()
{
    ...
    gainFocus(shared_ptr<Aya::InputObject>());
}
...
void TextBox::keyUp(RepeatKeyState::KeyType keyType, Aya::KeyCode keyCode, char key)
{
    if (repeatKeyState.state == RepeatKeyState::STATE_DEPRESSED ||
        (repeatKeyState.state == RepeatKeyState::STATE_REPEATING && !UserInputService::IsUsingNewKeyboardEvents()))
...
void TextBox::keyDown(RepeatKeyState::KeyType keyType, Aya::KeyCode keyCode, char key)
{
...
{
    AYAASSERT(event->isTextInputEvent());
...
GuiResponse TextBox::processKeyEvent(const shared_ptr<InputObject>& event)
{
    ...
    if (iAmFocus)
    {
        if (event->isKeyDownEvent())
        {
            ...
            {
                Aya::UserInputService* inputService = Aya::ServiceProvider::find<Aya::UserInputService>(this);
                AYAASSERT(inputService);
        ...
        if ((event->getKeyCode() == AYA_SDLK_LSHIFT || event->getKeyCode() == AYA_SDLK_RSHIFT) &&
            (repeatKeyState.state == RepeatKeyState::STATE_DEPRESSED ||
                (repeatKeyState.state == RepeatKeyState::STATE_REPEATING && !UserInputService::IsUsingNewKeyboardEvents())))
            repeatKeyState.state = RepeatKeyState::STATE_NONE;
    }
    else if (!iAmFocus && event->isKeyDownEvent() && isSelectedObject() && event->getKeyCode() == AYA_SDLK_RETURN)
        captureFocus();
...
{
    if (event->getKeyCode() == Aya::AYA_SDLK_v)
    {
        if (Aya::UserInputService* inputService = Aya::ServiceProvider::find<Aya::UserInputService>(this))
        {
            std::vector<Aya::ModCode> modCodes = inputService->getCommandModCodes();
            for (std::vector<Aya::ModCode>::iterator it = modCodes.begin(); it != modCodes.end(); ++it)
            {
...
void TextBox::render2d(Adorn* adorn)
{
    if (iAmFocus)
    {
        ...
        render2dTextImpl(adorn, getRenderBackgroundColor4(), showingCursor ? getTextWithCursor() : getTextWithBlankCursor(), getFont(), getFontSize(),
            getRenderTextColor4(), getRenderTextStrokeColor4(), getTextWrap(), getTextScale(), getXAlignment(), getYAlignment());
    }
    else
        render2dTextImpl(adorn, getRenderBackgroundColor4(), getText(), getFont(), getFontSize(), getRenderTextColor4(), getRenderTextStrokeColor4(),
            getTextWrap(), getTextScale(), getXAlignment(), getYAlignment());
