...
AYA_REGISTER_ENUM(Path::PathStatus);
...
static Reflection::BoundYieldFuncDesc<PathfindingService, shared_ptr<Instance>(Vector3, Vector3, float)> func_computeRawPathAsync(
    &PathfindingService::computeRawPathAsync, "ComputeRawPathAsync", "start", "finish", "maxDistance", Security::None);

static Reflection::BoundYieldFuncDesc<PathfindingService, shared_ptr<Instance>(Vector3, Vector3, float)> func_computeSmoothPathAsync(
    &PathfindingService::computeSmoothPathAsync, "ComputeSmoothPathAsync", "start", "finish", "maxDistance", Security::None);

static Reflection::BoundYieldFuncDesc<Path, int(int)> func_checkOcclusionAsync(
    &Path::checkOcclusionAsync, "CheckOcclusionAsync", "start", Security::None);
...
{
template<>
Reflection::EnumDesc<Path::PathStatus>::EnumDesc()
    : Aya::Reflection::EnumDescriptor("PathStatus")
{
...
static Reflection::EnumPropDescriptor<Path, Path::PathStatus> prop_PathStatus("Status", category_Data, &Path::getStatus, NULL);
REFLECTION_END();
...
Vector3int16 getOffsetByIndex(int dir)
{
    ...
    {
    case 0:
        return Vector3int16(1, 0, 0);
    case 1:
        return Vector3int16(0, 0, -1);
    case 2:
        return Vector3int16(-1, 0, 0);
    case 3:
        return Vector3int16(0, 0, 1);
...
PathfindingService::PathfindingService()
    : lastChunkId(
          SpatialRegion::Id(std::numeric_limits<G3D::int16>::max(), std::numeric_limits<G3D::int16>::max(), std::numeric_limits<G3D::int16>::max()))
    , lastChunk(NULL)
    , currentFrameNum(0)
    , voxelizer(true /* collisionTransparency */)
{
    setName(sPathfindingService);
...
#else
    auto itWeight = openNodesByWeight.insert(std::make_pair(distance, it));
#endif
    it->second.itWeight = itWeight;

    AYAASSERT_SLOW(checkOpenNodesCounts());
...
{
    AYAASSERT(openNodesByWeight.size() != 0);

#if !defined(__linux) && !defined(__APPLE__)
    OpenNodesByWeight::iterator top = openNodesByWeight.begin();
#else
    auto top = openNodesByWeight.begin();
#endif
    Node* result = &(top->second->second);

    AYAASSERT(result->closed == false);
...
void PathfindingService::collectOldChunks()
{
    ...
    for (std::vector<SpatialRegion::Id>::iterator it = chunkIds.begin(); it != chunkIds.end(); ++it)
    {
        ...
        if ((chunk->age + DFInt::PathfindingAgeToCollectChunks) < currentFrameNum)
        {
            if (*it == lastChunkId)
            {
                lastChunkId = SpatialRegion::Id(
                    std::numeric_limits<G3D::int16>::max(), std::numeric_limits<G3D::int16>::max(), std::numeric_limits<G3D::int16>::max());
                lastChunk = NULL;
            }
...
{
    AYAPROFILER_SCOPE("Voxel", "countOrComputeDirtyChunks");
...
PathfindingService::PathfindingState::PathfindingState()
    : nodesByPos(DFInt::PathfindingDefaultBucketNum)
{
}

void PathfindingService::computePath(CreatePathRequest& request)
{
    AYAPROFILER_SCOPE("Voxel", "computePath");
    ...
    else
    {
        pathPoints.push_back(
            request.isSmooth ? Voxel::worldSpaceToCellSpace(request.finish) : Vector3(request.finishCell) + Vector3(0.5f, 0.5f, 0.5f));
    }
    ...
    while (parentCell != startCell)
    {
        ...
        AYAASSERT(itOpen != state.nodesByPos.end());
...
void PathfindingService::computeRawPathAsync(Vector3 start, Vector3 finish, float maxDistance,
...
void PathfindingService::computeSmoothPathAsync(Vector3 start, Vector3 finish, float maxDistance,
    boost::function<void(shared_ptr<Instance>)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    computePathAsync(start, finish, maxDistance, true, resumeFunction, errorFunction);
}

void PathfindingService::computePathAsync(Vector3 start, Vector3 finish, float maxDistance, bool isSmooth,
    boost::function<void(shared_ptr<Instance>)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
...
void PathfindingService::executeThrottledRequests()
{
    ...
    do
    {
        AYAPROFILER_SCOPE("Voxel", "executeRequest");
        ...
        FASTLOG3(
            FLog::PathfindingDetail, "Popped request, finish cell: %i %i %i", request->finishCell.x, request->finishCell.y, request->finishCell.z);

        AYAPROFILER_LABELF("Voxel", "(%d %d %d) -> (%d %d %d)", request->startCell.x, request->startCell.y, request->startCell.z,
            request->finishCell.x, request->finishCell.y, request->finishCell.z);
...
ExtentsInt32 hashToVoxelChunk(const ExtentsInt32& extents, int level)
{
    ...
    AYAASSERT((1 << voxelShift.x) == kVoxelChunkSizeXZ);
    AYAASSERT((1 << voxelShift.y) == kVoxelChunkSizeY);
    AYAASSERT((1 << voxelShift.z) == kVoxelChunkSizeXZ);
...
Path::Path(weak_ptr<PathfindingService> service, PathStatus status)
    : service(service)
    , status(status)
{
}
