...
static const Reflection::PropDescriptor<SkateboardPlatform, int> propThrottle(
    "Throttle", "Control", &SkateboardPlatform::getThrottle, &SkateboardPlatform::setThrottle);
static const Reflection::PropDescriptor<SkateboardPlatform, int> propSteer(
    "Steer", "Control", &SkateboardPlatform::getSteer, &SkateboardPlatform::setSteer);
static const Reflection::PropDescriptor<SkateboardPlatform, bool> propStickyWheels(
    "StickyWheels", "Control", &SkateboardPlatform::getStickyWheels, &SkateboardPlatform::setStickyWheels);

static Reflection::EnumPropDescriptor<SkateboardPlatform, SkateboardPlatform::MoveState> propMoveState(
    "MoveState", "Control", &SkateboardPlatform::getMoveState, &SkateboardPlatform::setMoveState, Reflection::PropertyDescriptor::REPLICATE_ONLY);
static Reflection::EventDesc<SkateboardPlatform, void(SkateboardPlatform::MoveState, SkateboardPlatform::MoveState)> event_MoveStateChanged(
    &SkateboardPlatform::moveStateChangedSignal, "MoveStateChanged", "newState", "oldState");

static Reflection::EventDesc<SkateboardPlatform, void(shared_ptr<Instance>, shared_ptr<Instance>)> event_Equipped(
    &SkateboardPlatform::equippedSignal, "Equipped", "humanoid", "skateboardController");
static Reflection::EventDesc<SkateboardPlatform, void(shared_ptr<Instance>, shared_ptr<Instance>)> dep_Equipped(&SkateboardPlatform::equippedSignal,
    "equipped", "humanoid", "skateboardController", Security::None, Reflection::Descriptor::Attributes::deprecated(event_Equipped));
static Reflection::EventDesc<SkateboardPlatform, void(shared_ptr<Instance>)> event_Unequipped(
    &SkateboardPlatform::unequippedSignal, "Unequipped", "humanoid");
static Reflection::EventDesc<SkateboardPlatform, void(shared_ptr<Instance>)> dep_Unequipped(&SkateboardPlatform::unequippedSignal, "unequipped",
    "humanoid", Security::None, Reflection::Descriptor::Attributes::deprecated(event_Unequipped));

static Reflection::RefPropDescriptor<SkateboardPlatform, SkateboardController> prop_Controller(
    "Controller", "Control", &SkateboardPlatform::getController, NULL, Reflection::PropertyDescriptor::UI);
static Reflection::RefPropDescriptor<SkateboardPlatform, Humanoid> prop_ControllingHumanoid(
    "ControllingHumanoid", "Control", &SkateboardPlatform::getControllingHumanoid, NULL, Reflection::PropertyDescriptor::UI);

static Reflection::BoundFuncDesc<SkateboardPlatform, void(Vector3)> desc_ApplySpecificImpulse(
    &SkateboardPlatform::applySpecificImpulse, "ApplySpecificImpulse", "impulseWorld", Security::None);

static Reflection::RemoteEventDesc<SkateboardPlatform, void(shared_ptr<Instance>)> event_createPlatformMotor6D(
    &SkateboardPlatform::createPlatformMotor6DSignal, "RemoteCreateMotor6D", "humanoid", Security::None,
    Reflection::RemoteEventCommon::REPLICATE_ONLY, Reflection::RemoteEventCommon::CLIENT_SERVER);
static Reflection::RemoteEventDesc<SkateboardPlatform, void()> event_destroyPlatformMotor6D(&SkateboardPlatform::destroyPlatformMotor6DSignal,
    "RemoteDestroyMotor6D", Security::None, Reflection::RemoteEventCommon::REPLICATE_ONLY, Reflection::RemoteEventCommon::CLIENT_SERVER);
REFLECTION_END();
...

{
    ...
    AYAASSERT(Edge::getPrimitive(0) == NULL);
    AYAASSERT(Edge::getPrimitive(1) == NULL);

    createPlatformMotor6DSignal.connect(
        boost::bind(static_cast<void (SkateboardPlatform::*)(shared_ptr<Instance>)>(&SkateboardPlatform::createPlatformMotor6DInternal), this, _1));
...
void SkateboardPlatform::onLocalPlatformStanding(Humanoid* humanoid)
{
    ...
    else
    {
        Aya::StandardOut::singleton()->printf(
            Aya::MESSAGE_ERROR, "Error mounting skateboard platform. Both Humanoid and SkateboardPlatform must be direct children of a Model");
    }
}
...
void SkateboardPlatform::countGroundedWheels()
{
    ...
    if (humanoid && geom)
    {
        ...
        {
            Aya::RbxRay caster(wheels[i].joint->getJointWorldCoord(1).translation, downforceDir * 2.0f); // search 2 stud
...
bool SkateboardPlatform::stepUi(double distributedGameTime)
{
    ...
    if (motor6D)
    {
        AYAASSERT(humanoidFromMotor6D(motor6D) == this->humanoid);
        AYAASSERT(humanoidFromMotor6D(motor6D) == this->humanoid);
    }
...
void SkateboardPlatform::doLoadWheels(Primitive* primitive)
{
    for (int i = 0; i < primitive->numChildren(); ++i)
    {
        ...
        {
            Wheel w;
            w.joint = aya_static_cast<RotateJoint*>(joint);
            wheels.append(w);
        }
    }
}
...
{
    Body* root = getEngineBody();
    const CoordinateFrame& boardCFrame = getCoordinateFrame(); // is it okay to be calling this in computeForce? if we can garantee that the root body
                                                               // is the board, we could use the root body cframe instead.
...
void SkateboardPlatform::renderGuange(
        ...
        adorn->rect2d(speedo.toRect2D(), Color3::blue());	// note Aya color
...
/*bool SkateboardPlatform::findLip(Adorn* adorn)
{
        ...
        for (int i = 0; i < ARRAYSIZE(samples); i++)
        {
                ...
                if (hitPrim)
                {
                        ...
                        if (p)
                        {
                                if (p->getPartType() == Aya::TRUSS_PART) return true;
                        }
                }
