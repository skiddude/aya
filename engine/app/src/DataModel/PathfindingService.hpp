...
public:
    ...
    void computePathAsync(Vector3 start, Vector3 finish, float maxDistance, bool isSmooth, boost::function<void(shared_ptr<Instance>)> resumeFunction,
        boost::function<void(std::string)> errorFunction);

    void computeRawPathAsync(Vector3 start, Vector3 finish, float maxDistance, boost::function<void(shared_ptr<Instance>)> resumeFunction,
        boost::function<void(std::string)> errorFunction);

    void computeSmoothPathAsync(Vector3 start, Vector3 finish, float maxDistance, boost::function<void(shared_ptr<Instance>)> resumeFunction,
        boost::function<void(std::string)> errorFunction);
...
private:
    ...
    struct Node
    {
        ...
        std::multimap<float, boost::unordered_map<Vector3int16, Node, boost::hash<Vector3int16>, std::equal_to<Vector3int16>,
                                 boost::fast_pool_allocator<Vector3int16, boost::default_user_allocator_new_delete,
                                     boost::details::pool::null_mutex>>::iterator>::iterator itWeight;
    ...
    typedef boost::unordered_map<Vector3int16, Node, boost::hash<Vector3int16>, std::equal_to<Vector3int16>,
        boost::fast_pool_allocator<Vector3int16, boost::default_user_allocator_new_delete, boost::details::pool::null_mutex>>
        NodesByPos;

#if !defined(__linux) && !defined(__APPLE__)
    typedef std::multimap<float, NodesByPos::iterator, std::less<float>,
        boost::fast_pool_allocator<float, boost::default_user_allocator_new_delete, boost::details::pool::null_mutex>>
        OpenNodesByWeight;
#else
    typedef std::multimap<float, NodesByPos::iterator, std::less<float>,
        boost::fast_pool_allocator<std::pair<const float, NodesByPos::iterator>, boost::default_user_allocator_new_delete,
