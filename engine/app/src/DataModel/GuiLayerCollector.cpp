...
void GuiLayerCollector::onDescendantAdded(Instance* instance)
{
    ...
    rebuildGuiVector = true;
    if (GuiBase* gb = Aya::Instance::fastDynamicCast<GuiBase>(instance))
    {
        shared_ptr<GuiBase> sharedGb = shared_from(gb);
        propertyConnections[instance] =
            instance->propertyChangedSignal.connect(boost::bind(&GuiLayerCollector::descendantPropertyChanged, this, sharedGb, _1));
    }
}
...
void GuiLayerCollector::LoadZ(const shared_ptr<Aya::Instance>& instance, GuiLayers guiVectors[])
{
    if (GuiBase* gb = instance->fastDynamicCast<GuiBase>())
    {
        if (gb->canProcessMeAndDescendants())
        {
            ...
            AYAASSERT(queue < GUIQUEUE_COUNT);
            AYAASSERT(z <= GuiBase::maxZIndex());
...
void GuiLayerCollector::loadZVectors()
{
    ...
    AYAASSERT(isAllGuiQueuesHaveCorrectNumberOfLayers(mGuiVectors));
...
GuiResponse GuiLayerCollector::doProcessGesture(const boost::shared_ptr<GuiBase>& guiBase, const UserInputService::Gesture& gesture,
...
GuiResponse GuiLayerCollector::processGesture(const UserInputService::Gesture& gesture,
    const shared_ptr<const Aya::Reflection::ValueArray>& touchPositions, const shared_ptr<const Reflection::Tuple>& args)
{
        ...
        for (int queue = GUIQUEUE_COUNT - 1; queue >= 0; --queue)
        {
            for (GuiVector::reverse_iterator itr = mGuiVectors[queue][z].rbegin(); itr != mGuiVectors[queue][z].rend(); ++itr)
            {
                ...
                if (DFFlag::FixClippedScrollingFrameNavigation)
                {
                    ...
                    {
                        if (guiObject->getSelectable() &&
                            (guiObject->isCurrentlyVisible() || (myAncestorScrollingFrame && guiObject->isDescendantOf(myAncestorScrollingFrame))))
                        {
                ...
                else
                {
                    ...
                    {
                        if (guiObject->getSelectable() &&
                            (guiObject->isCurrentlyVisible() || (ancestorScrollingFrame && ancestorScrollingFrame->isCurrentlyVisible())))
                        {
                            guiObjects.push_back(guiObject);
                        }
                    }
...
GuiResponse GuiLayerCollector::process(const shared_ptr<InputObject>& event, bool sinkIfMouseOver)
{
    ...
    if (event->isMouseEvent() && !guiResponse.wasSunk() && guiResponse.getMouseWasOver() && sinkIfMouseOver)
    {
