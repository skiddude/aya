...
#include "DataModel/ActionStation.hpp"
...
private:
    ...
    // "Backend" connections
    Aya::signals::scoped_connection platformTouched;
    Aya::signals::scoped_connection humanoidDonePlatformStanding;
    ...
    void onEvent_platformTouched(shared_ptr<Instance> other)
    {
        if (Humanoid* h = Humanoid::humanoidFromBodyPart(other.get()))
        {
            ...
            {
                if (this->sleepTimeUp() && !findPlatformMotor6D() && !h->getPlatformStanding() && !h->getDead() &&
                    !torso->computeNetworkOwnerIsSomeoneElse() && Workspace::contextInWorkspace(torso) && Workspace::contextInWorkspace(this))
                {
                    AYAASSERT(h->getTorsoSlow()->getPartPrimitive()->getAssembly() != this->getPartPrimitive()->getAssembly());
    ...
    void createPlatformMotor6DInternal(Humanoid* h)
    {
        ...
        AYAASSERT(pTorsoPart); // we just checked for this

        CoordinateFrame originalPlatformCoord = this->getCoordinateFrame();
        AYAASSERT(this->getCoordinateFrame() == this->getPartPrimitive()->getCoordinateFrame());
        ...
        AYAASSERT(torsoAssembly != this->getPartPrimitive()->getAssembly());
    ...
    void onChildAdded(Instance* child)
    {
        if (Motor6D* childMotor6D = isChildPlatformMotor6D(child))
        {
            ...
            if (Humanoid* h = humanoidFromMotor6D(childMotor6D))
            {
                ...
                humanoidDonePlatformStanding =
                    h->donePlatformStandingSignal.connect(boost::bind(&PlatformImpl::onEvent_humanoidDonePlatformStanding, this));
...
protected:
    ...
    void onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
    {
        ...
        if (newProvider)
        {
            platformTouched = ActionStation<Base>::onDemandWrite()->localSimulationTouchedSignal.connect(
                boost::bind(&PlatformImpl::onEvent_platformTouched, this, _1));
        }
    }
...
public:
    Aya::remote_signal<void(shared_ptr<Instance>)> createPlatformMotor6DSignal;
    Aya::remote_signal<void()> destroyPlatformMotor6DSignal;
    ...
    Platform()
    {
        ...
        createPlatformMotor6DSignal.connect(
            boost::bind(static_cast<void (Platform::*)(shared_ptr<Instance>)>(&Platform::createPlatformMotor6DInternal), this, _1));
