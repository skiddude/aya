...
class AyaInterface ISelectionBase
{
    friend class Selection;
...
public:
    Aya::signal<void(const SelectionChanged&)> selectionChanged;
    Aya::signal<void()> luaSelectionChanged;
    ...
    shared_ptr<Instance> front() const
    {
        return selection->size() > 0 ? selection->front() : shared_ptr<Instance>();
    }
    shared_ptr<Instance> back() const
    {
        return selection->size() > 0 ? selection->back() : shared_ptr<Instance>();
    }
    Instances::const_iterator begin() const
    {
        return selection->begin();
    }
    Instances::const_iterator end() const
    {
        return selection->end();
    }
    const copy_on_write_ptr<Instances>& getSelection() const
    {
        return selection;
    }
    ...
    shared_ptr<const Instances> getSelection2()
    {
    ...
    public:
        AddIterator(Selection* selection)
            : selection(selection)
        {
        }
    ...
    public:
        RemoveIterator(Selection* selection)
            : selection(selection)
        {
        }
    ...
    public:
        ToggleIterator(Selection* selection)
            : selection(selection)
        {
        }
...
private:
    ...
    void propagateChangeSignalToLua(const Aya::SelectionChanged& event);
    bool instanceCanLiveInSelection(Instance* instance);

    Aya::signals::connection selectionChangedConnection;
};
...
public:
    ...
    {
        AYAASSERT(rootSelection != NULL);
        return rootSelection.get();
    }

    const CollectionType& items() const
    {
    ...
    inline C* front()
    {
        return size() > 0 ? filteredSelection.front() : NULL;
    }
    inline const C* front() const
    {
        return size() > 0 ? filteredSelection.front() : NULL;
    }
    inline C* back()
    {
        return size() > 0 ? filteredSelection.back() : NULL;
    }
    inline const C* back() const
    {
        return size() > 0 ? filteredSelection.back() : NULL;
    }

    typename CollectionType::const_iterator begin() const
    {
        return filteredSelection.begin();
    }
    typename CollectionType::const_iterator end() const
    {
        return filteredSelection.end();
    }
