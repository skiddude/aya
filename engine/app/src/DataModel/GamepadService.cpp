...
int GamepadService::getGamepadIntForEnum(InputObject::UserInputType gamepadEnum)
{
    switch (gamepadEnum)
    {
    case InputObject::TYPE_GAMEPAD1:
        return 0;
    case InputObject::TYPE_GAMEPAD2:
        return 1;
    case InputObject::TYPE_GAMEPAD3:
        return 2;
    case InputObject::TYPE_GAMEPAD4:
        return 3;
    case InputObject::TYPE_GAMEPAD5:
        return 4;
    case InputObject::TYPE_GAMEPAD6:
        return 5;
    case InputObject::TYPE_GAMEPAD7:
        return 6;
    case InputObject::TYPE_GAMEPAD8:
        return 7;
    default:
        return -1;
    }
...
InputObject::UserInputType GamepadService::getGamepadEnumForInt(int controllerIndex)
{
    ...
    {
    case 0:
        return InputObject::TYPE_GAMEPAD1;
    case 1:
        return InputObject::TYPE_GAMEPAD2;
    case 2:
        return InputObject::TYPE_GAMEPAD3;
    case 3:
        return InputObject::TYPE_GAMEPAD4;
    case 4:
        return InputObject::TYPE_GAMEPAD5;
    case 5:
        return InputObject::TYPE_GAMEPAD6;
    case 6:
        return InputObject::TYPE_GAMEPAD7;
    case 7:
        return InputObject::TYPE_GAMEPAD8;
    default:
        return InputObject::TYPE_NONE;
    }
}

shared_ptr<Aya::InputObject> GamepadService::createInputObjectForGamepadKeyCode(Aya::KeyCode keyCode, Aya::InputObject::UserInputType gamepadType)
{
    return Creatable<Instance>::create<InputObject>(
        gamepadType, Aya::InputObject::INPUT_STATE_END, G3D::Vector3::zero(), keyCode, Aya::DataModel::get(this));
}

void GamepadService::createControllerKeyMapForController(int controllerIndex)
{
    Aya::InputObject::UserInputType gamepadType = getGamepadEnumForInt(controllerIndex);

    Gamepad controllerKeyMap;
    controllerKeyMap[Aya::AYA_SDLK_GAMEPAD_BUTTONA] = createInputObjectForGamepadKeyCode(Aya::AYA_SDLK_GAMEPAD_BUTTONA, gamepadType);
    controllerKeyMap[Aya::AYA_SDLK_GAMEPAD_BUTTONB] = createInputObjectForGamepadKeyCode(Aya::AYA_SDLK_GAMEPAD_BUTTONB, gamepadType);
    controllerKeyMap[Aya::AYA_SDLK_GAMEPAD_BUTTONX] = createInputObjectForGamepadKeyCode(Aya::AYA_SDLK_GAMEPAD_BUTTONX, gamepadType);
    controllerKeyMap[Aya::AYA_SDLK_GAMEPAD_BUTTONY] = createInputObjectForGamepadKeyCode(Aya::AYA_SDLK_GAMEPAD_BUTTONY, gamepadType);
    controllerKeyMap[Aya::AYA_SDLK_GAMEPAD_BUTTONSTART] = createInputObjectForGamepadKeyCode(Aya::AYA_SDLK_GAMEPAD_BUTTONSTART, gamepadType);
    controllerKeyMap[Aya::AYA_SDLK_GAMEPAD_BUTTONSELECT] = createInputObjectForGamepadKeyCode(Aya::AYA_SDLK_GAMEPAD_BUTTONSELECT, gamepadType);

    controllerKeyMap[Aya::AYA_SDLK_GAMEPAD_BUTTONL1] = createInputObjectForGamepadKeyCode(Aya::AYA_SDLK_GAMEPAD_BUTTONL1, gamepadType);
    controllerKeyMap[Aya::AYA_SDLK_GAMEPAD_BUTTONR1] = createInputObjectForGamepadKeyCode(Aya::AYA_SDLK_GAMEPAD_BUTTONR1, gamepadType);

    controllerKeyMap[Aya::AYA_SDLK_GAMEPAD_BUTTONL2] = createInputObjectForGamepadKeyCode(Aya::AYA_SDLK_GAMEPAD_BUTTONL2, gamepadType);
    controllerKeyMap[Aya::AYA_SDLK_GAMEPAD_BUTTONR2] = createInputObjectForGamepadKeyCode(Aya::AYA_SDLK_GAMEPAD_BUTTONR2, gamepadType);

    controllerKeyMap[Aya::AYA_SDLK_GAMEPAD_BUTTONL3] = createInputObjectForGamepadKeyCode(Aya::AYA_SDLK_GAMEPAD_BUTTONL3, gamepadType);
    controllerKeyMap[Aya::AYA_SDLK_GAMEPAD_BUTTONR3] = createInputObjectForGamepadKeyCode(Aya::AYA_SDLK_GAMEPAD_BUTTONR3, gamepadType);

    controllerKeyMap[Aya::AYA_SDLK_GAMEPAD_DPADUP] = createInputObjectForGamepadKeyCode(Aya::AYA_SDLK_GAMEPAD_DPADUP, gamepadType);
    controllerKeyMap[Aya::AYA_SDLK_GAMEPAD_DPADDOWN] = createInputObjectForGamepadKeyCode(Aya::AYA_SDLK_GAMEPAD_DPADDOWN, gamepadType);
    controllerKeyMap[Aya::AYA_SDLK_GAMEPAD_DPADLEFT] = createInputObjectForGamepadKeyCode(Aya::AYA_SDLK_GAMEPAD_DPADLEFT, gamepadType);
    controllerKeyMap[Aya::AYA_SDLK_GAMEPAD_DPADRIGHT] = createInputObjectForGamepadKeyCode(Aya::AYA_SDLK_GAMEPAD_DPADRIGHT, gamepadType);

    controllerKeyMap[Aya::AYA_SDLK_GAMEPAD_THUMBSTICK1] = createInputObjectForGamepadKeyCode(Aya::AYA_SDLK_GAMEPAD_THUMBSTICK1, gamepadType);
    controllerKeyMap[Aya::AYA_SDLK_GAMEPAD_THUMBSTICK2] = createInputObjectForGamepadKeyCode(Aya::AYA_SDLK_GAMEPAD_THUMBSTICK2, gamepadType);
...
void GamepadService::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
    ...
    if (newProvider)
    {
        for (int i = 0; i < AYA_MAX_GAMEPADS; ++i)
        {
        ...
        if (Aya::Workspace* workspace = Aya::ServiceProvider::find<Workspace>(this))
        {
            cameraCframeUpdateConnection =
                workspace->getCamera()->cframeChangedSignal.connect(boost::bind(&GamepadService::cameraCframeChanged, this, _1));
...
void GamepadService::onInputChanged(const shared_ptr<Instance>& event)
{
    if (InputObject* inputObject = Instance::fastDynamicCast<InputObject>(event.get()))
    {
        ...
        if (event->getKeyCode() == Aya::AYA_SDLK_GAMEPAD_THUMBSTICK1)
        {
...
{
    CoreGuiService* coreGuiService = Aya::ServiceProvider::find<CoreGuiService>(this);
    GuiResponse response = trySelectGuiObject(inputVector, shared_ptr<InputObject>(), coreGuiService);

    if (!response.wasSunk())
    {
        if (Aya::Network::Players* players = ServiceProvider::find<Aya::Network::Players>(this))
        {
...
GuiResponse GamepadService::trySelectGuiObject(const Vector2& inputVector, const shared_ptr<InputObject>& event, BasePlayerGui* guiToProcess)
{
    ...
    if (inputVector == Vector2::zero())
    {
        if (event && (isNavigationGamepad(event->getUserInputType()) || isCoreGuiProcessing) &&
            (event->getKeyCode() == Aya::AYA_SDLK_GAMEPAD_THUMBSTICK1 || event->isDPadEvent()))
        {
        ...
        if (event && (event->getKeyCode() == Aya::AYA_SDLK_GAMEPAD_THUMBSTICK1 || event->isDPadEvent()))
        {
...
{
    UserInputService* userInputService = Aya::ServiceProvider::find<UserInputService>(this);
    if (!userInputService)
    {
        return GuiResponse::notSunk();
    }

    return process(event, Aya::ServiceProvider::find<CoreGuiService>(this));
}

GuiResponse GamepadService::processDev(const shared_ptr<InputObject>& event)
{
    if (autoGuiSelectionAllowed && event->getKeyCode() == Aya::AYA_SDLK_GAMEPAD_BUTTONSELECT &&
        event->getUserInputState() == InputObject::INPUT_STATE_BEGIN)
    {
...
GuiResponse GamepadService::process(const shared_ptr<InputObject>& event, BasePlayerGui* guiToProcess)
{
    ...
    {
        if (Aya::GuiObject* guiObject = guiService->getSelectedGuiObject())
        {
