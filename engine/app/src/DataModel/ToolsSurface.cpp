...
void SurfaceTool::render3dAdorn(Adorn* adorn)
{
    ...
    if (partInstance)
    {
        AYAASSERT(surface.getPartInstance());

        DrawAdorn::partSurface(partInstance->getPart(), surfaceId, adorn);
    }
}

DecalTool::DecalTool(Workspace* workspace, Decal* decal, Aya::InsertMode insertMode)
    : Named<SurfaceTool, sDecalTool>(workspace)
{
    this->decal = shared_from(decal);
    this->insertMode = insertMode;
    parentIsPart = Instance::fastDynamicCast<Aya::PartInstance>(decal->getParent()) != NULL;
}
...
void DecalTool::onMouseHover(const shared_ptr<InputObject>& inputObject)
{
    ...
    if ((insertMode == Aya::INSERT_TO_3D_VIEW) && !parentIsPart)
    {
        if (partInstance)
        {
...
shared_ptr<MouseCommand> DecalTool::onKeyDown(const shared_ptr<InputObject>& inputObject)
{
    ...
    if (inputObject->getKeyCode() == AYA_SDLK_DELETE || inputObject->getKeyCode() == AYA_SDLK_ESCAPE)
    {
...
shared_ptr<MouseCommand> DecalTool::onMouseUp(const shared_ptr<InputObject>& inputObject)
{
    ...
    if (Instance::fastDynamicCast<Aya::PartInstance>(decal->getParent()) != NULL)
    {
        parentIsPart = true;
