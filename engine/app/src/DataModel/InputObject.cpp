...
static const Reflection::EnumPropDescriptor<InputObject, InputObject::UserInputType> prop_userInputType(
    "UserInputType", category_Data, &InputObject::getUserInputType, &InputObject::setInputType, Reflection::PropertyDescriptor::SCRIPTING);
static const Reflection::EnumPropDescriptor<InputObject, InputObject::UserInputState> prop_userInputState(
    "UserInputState", category_State, &InputObject::getUserInputState, &InputObject::setInputState, Reflection::PropertyDescriptor::SCRIPTING);
static const Reflection::PropDescriptor<InputObject, Vector3> prop_Position(
    "Position", category_Data, &InputObject::getPosition, &InputObject::setPosition, Reflection::PropertyDescriptor::SCRIPTING);
static const Reflection::PropDescriptor<InputObject, Vector3> prop_inputDelta(
    "Delta", category_Data, &InputObject::getDelta, &InputObject::setDelta, Reflection::PropertyDescriptor::SCRIPTING);
static const Reflection::EnumPropDescriptor<InputObject, KeyCode> prop_KeyCode(
    "KeyCode", category_Data, &InputObject::getKeyCode, &InputObject::setKeyCode, Reflection::PropertyDescriptor::SCRIPTING);
...
InputObject::InputObject()
    ...
    , position(0, 0, 0)
    , keyCode(AYA_SDLK_UNKNOWN)
    , mod(AYA_KMOD_NONE)
    , modifiedKey(AYA_SDLK_UNKNOWN)
    , modCodes(0)
    , inputText("")
...
InputObject::InputObject(UserInputType inputType, UserInputState inputState, G3D::Vector3 newPosition, G3D::Vector3 newDelta, DataModel* dm)
    ...
    , inputText("")
    , scanCode(SDL_SCANCODE_UNKNOWN)
    , weakWorkspace(weak_from(Aya::ServiceProvider::find<Workspace>(dm)))
...
InputObject::InputObject(UserInputType inputType, UserInputState inputState, KeyCode k, unsigned int newModCodes, SDL_Scancode newScanCode,
    std::string newInputText, DataModel* dm)
    : inputType(inputType)
    , inputState(inputState)
    , position(0, 0, 0)
    , keyCode(k)
...
Vector4 InputObject::getGuiInset() const
{
    ...
    if (shared_ptr<Aya::Workspace> workspace = weakWorkspace.lock())
    {
        if (Aya::GuiService* guiService = Aya::ServiceProvider::find<Aya::GuiService>(workspace.get()))
        {
...
bool InputObject::isPublicEvent()
{
    ...
    if (inputType == InputObject::TYPE_MOUSEIDLE || inputType == InputObject::TYPE_MOUSEDELTA || sourceInputType != inputType)
    {
        return false;
    }
...
bool InputObject::isAltEvent() const
{
    ...
    {
        return ((modCodes & AYA_KMOD_LALT) != 0) || ((modCodes & AYA_KMOD_RALT) != 0);
    }

    return ((mod & AYA_KMOD_LALT) != 0) || ((mod & AYA_KMOD_RALT) != 0);
}
bool InputObject::isCtrlEvent() const
{
    if (UserInputService::IsUsingNewKeyboardEvents())
    {
        return ((modCodes & AYA_KMOD_LCTRL) != 0) || ((modCodes & AYA_KMOD_RCTRL) != 0);
    }

    return ((mod & AYA_KMOD_LCTRL) != 0) || ((mod & AYA_KMOD_RCTRL) != 0);
}
bool InputObject::isCarriageReturnKey() const
{
    return ((keyCode == AYA_SDLK_RETURN) || (keyCode == AYA_SDLK_KP_ENTER));
}

bool InputObject::isDeleteKey() const
{
    return (keyCode == AYA_SDLK_DELETE);
}

bool InputObject::isBackspaceKey() const
{
    return (keyCode == AYA_SDLK_BACKSPACE);
}

bool InputObject::isClearKey() const
{
    return (keyCode == AYA_SDLK_CLEAR);
}

bool InputObject::isEscapeKey() const
{
    return (keyCode == AYA_SDLK_ESCAPE);
}
...
bool InputObject::isNavigationKey() const
{
    ...
    return keyCode == AYA_SDLK_DOWN || keyCode == AYA_SDLK_LEFT || keyCode == AYA_SDLK_RIGHT || keyCode == AYA_SDLK_UP || keyCode == AYA_SDLK_a || keyCode == AYA_SDLK_w ||
           keyCode == AYA_SDLK_s || keyCode == AYA_SDLK_d;
}
