...
const EnumPropDescriptor<ExtrudedPartInstance, ExtrudedPartInstance::VisualTrussStyle> ExtrudedPartInstance::prop_styleXml("style",
    category_ExtrudedPart, &ExtrudedPartInstance::getVisualTrussStyle, &ExtrudedPartInstance::setVisualTrussStyle, PropertyDescriptor::STREAMING);

static const EnumPropDescriptor<ExtrudedPartInstance, ExtrudedPartInstance::VisualTrussStyle> prop_styleUi(
    "Style", category_ExtrudedPart, &ExtrudedPartInstance::getVisualTrussStyle, &ExtrudedPartInstance::setVisualTrussStyle, PropertyDescriptor::UI);
...
void ExtrudedPartInstance::setPartSizeXml(const Vector3& rbxSize)
{
    ...
    else
    {
        ...
        if (countMinExtrudedDistance(newSize) < 2)
        {
            ...
            {
                AYAASSERT(newSize.z >= newSize.x && newSize.z >= newSize.y || !newSize.isFinite());
                newSize.x = MinExtrudedPartSize.x;
                newSize.y = MinExtrudedPartSize.y;
            }
        }
...
Faces ExtrudedPartInstance::getResizeHandleMask() const
{
    ...
    else
    {
        ...
        {
            AYAASSERT(size.y == MinExtrudedDimensionSize && size.z == MinExtrudedDimensionSize);
            return Faces(NORM_X_MASK | NORM_X_NEG_MASK);
        }
        else if (size.y != MinExtrudedDimensionSize)
        {
            AYAASSERT(size.x == MinExtrudedDimensionSize && size.z == MinExtrudedDimensionSize);
            return Faces(NORM_Y_MASK | NORM_Y_NEG_MASK);
        }
        else
        {
            AYAASSERT(size.z != MinExtrudedDimensionSize);
            AYAASSERT(size.x == MinExtrudedDimensionSize && size.y == MinExtrudedDimensionSize);
