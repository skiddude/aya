...
class BasePlayerGui
    : public DescribedNonCreatable<BasePlayerGui, Instance, sBasePlayerGui>
...
private:
    ...
    bool isCloserGuiObject(const Rect2D& currentRect, const Vector2& inputDirection, const GuiObject* guiObjectToTest, float& minIntersectDist,
        float& minProjection, const GuiObject* nextSelectionGuiObject);
...
public:
    ...
    bool getMouseWasOverGui() const
    {
        return mouseWasOverGui;
    }
    ...
    GuiResponse processGesture(const UserInputService::Gesture gesture, shared_ptr<const Aya::Reflection::ValueArray> touchPositions,
    ...
    shared_ptr<GuiObject> getSelectedObject() const
    {
        return selectedGuiObject.lock();
    }
...
protected:
    ...
    GuiResponse processGestureOnChild(GuiBase2d* guiBase, const UserInputService::Gesture gesture,
...
public:
    ...
    Aya::signal<void(float)> topbarTransparencyChangedSignal;
...
public:
    Aya::signal<void(StarterGuiService::CoreGuiType, bool)> coreGuiChangedSignal;
    static const Reflection::PropDescriptor<StarterGuiService, bool> prop_ResetPlayerGui;

    StarterGuiService();
    bool getShowGui() const
    {
        return showGui;
    }
    void setShowGui(bool value);

    bool getResetPlayerGui() const
    {
        return resetPlayerGui;
    }
...
{
private:
    typedef DescribedNonCreatable<CoreGuiService, BasePlayerGui, sCoreGuiService, Reflection::ClassDescriptor::INTERNAL_LOCAL, Security::Plugin>
        Super;
...
public:
    ...
    // used to add/remove standard lua gui elements in (for Aya in-game gui) -bt
    ...
    shared_ptr<Aya::ScreenGui> getRobloxScreenGui();
...
private:
    shared_ptr<Aya::Instance> screenGui;

    Aya::Instances onScreenMessages;
};
} // namespace Aya
