...
void TouchInputService::onServiceProvider(ServiceProvider* oldSp, ServiceProvider* newSp)
{
    ...
    if (newSp)
    {
        if (UserInputService* inputService = Aya::ServiceProvider::find<UserInputService>(newSp))
        {
...
void TouchInputService::addTouchToBuffer(void* touch, Vector3 rbxLocation, InputObject::UserInputState newState)
{
    ...
    if (newState == Aya::InputObject::INPUT_STATE_BEGIN)
    {
        touchCount++;
    ...
    else
    {
        ...
        if (newState == Aya::InputObject::INPUT_STATE_END)
        {
...
void TouchInputService::processTouchBuffer()
{
    ...
    for (TouchBufferMap::iterator iter = tmpTouchBuffer.begin(); iter != tmpTouchBuffer.end(); ++iter)
    {
        ...
        {
            currentInputObj = Aya::Creatable<Aya::Instance>::create<Aya::InputObject>(Aya::InputObject::TYPE_TOUCH,
                Aya::InputObject::INPUT_STATE_NONE, Aya::Vector3(0, 0, 0), Aya::Vector3(0, 0, 0), Aya::DataModel::get(this));
        ...
        {
            Aya::InputObject::UserInputState inputState = (*touchIter).second;
            ...
            if (inputState == Aya::InputObject::INPUT_STATE_CHANGE && newDelta == Vector3::zero())
            {
                // this is an unnecessary touch, do nothing with it
            }
            else
            {
                if (inputState != Aya::InputObject::INPUT_STATE_BEGIN)
                {
                ...
                if (currentInputObj->getUserInputState() == Aya::InputObject::INPUT_STATE_END ||
                    currentInputObj->getUserInputState() == Aya::InputObject::INPUT_STATE_CANCEL)
                {
