    ...
    , public Service
{
    ...
    Aya::signals::scoped_connection playerRemovingConnection;
    ...
    Aya::signals::scoped_connection playerAddedReceiptConnection;
    Aya::signals::scoped_connection playerPurchaseReceiptConnection;
    Aya::signals::scoped_connection verifyPurchaseConnection;
    ...
    void processPlayerOwnsAssetResponseSuccess(
    ...
    static void processReceiptSuccessContinuation(
...
public:
    ...
    Aya::remote_signal<void(shared_ptr<Instance> player, std::string productId, bool isPurchased)> nativePurchaseFinished;
    Aya::remote_signal<void(shared_ptr<Instance> player, std::string productId)> promptNativePurchaseRequested;

    Aya::remote_signal<void(shared_ptr<Instance> player, std::string productId, std::string receipt, bool isPurchased)> thirdPartyPurchaseFinished;
    Aya::remote_signal<void(shared_ptr<Instance> player, std::string productId)> promptThirdPartyPurchaseRequested;

    Aya::remote_signal<void(shared_ptr<Instance> player, int assetId, bool equipIfPurchased, CurrencyType currencyType)>
        promptProductPurchaseRequested;
    Aya::remote_signal<void(int userId, int assetId, bool isPurchased)> promptProductPurchaseFinished;

    Aya::remote_signal<void(shared_ptr<Instance> player, int assetId, bool isPurchased)> promptPurchaseFinished;
    Aya::remote_signal<void(shared_ptr<Instance> player, int assetId, bool equipIfPurchased, CurrencyType currencyType)> promptPurchaseRequested;

    Aya::remote_signal<void(std::string response, int userId, int productId)> clientPurchaseSuccess;
    Aya::remote_signal<void(shared_ptr<const Reflection::ValueTable> responseTable)> serverPurchaseVerification;

    Aya::remote_signal<void(shared_ptr<const Reflection::Tuple>)> clientLuaDialogRequested;
    Aya::remote_signal<void(bool, shared_ptr<Instance>)> luaDialogCallbackSignal;

    typedef boost::function<void(shared_ptr<const Reflection::ValueTable>, Reflection::AsyncCallbackDescriptor::ResumeFunction,
    ...
    void getProductInfo(const int assetId, const MarketplaceService::InfoType infoType,
        boost::function<void(shared_ptr<const Reflection::ValueTable>)> resumeFunction, boost::function<void(std::string)> errorFunction);

    void onReceivedRawProductInfoSuccess(std::string urlPath, std::string response, boost::weak_ptr<Aya::DataModel> weakDM,
        boost::function<void(shared_ptr<const Reflection::ValueTable>)> resumeFunction, boost::function<void(std::string)> errorFunction);
    void onReceivedRawProductInfoError(std::string error, boost::weak_ptr<Aya::DataModel> weakDM, boost::function<void(std::string)> errorFunction);

    void playerOwnsAsset(const shared_ptr<Instance> player, const int assetId, boost::function<void(bool)> resumeFunction,
        boost::function<void(std::string)> errorFunction);

    bool launchClientLuaDialog(std::string message, std::string accept, std::string decline, shared_ptr<Instance> player,
    ...
    void getDeveloperProductsAsync(boost::function<void(shared_ptr<Instance>)> resumeFunction, boost::function<void(std::string)> errorFunction);
    ...
    void processRemoteEvent(
        const Reflection::EventDescriptor& descriptor, const Reflection::EventArguments& args, const SystemAddress& source) override;
