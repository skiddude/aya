...
static Reflection::RemoteEventDesc<LogService, void(std::string, MessageType, int)> event_ServerOutputMessage(&LogService::serverOutputMessageSignal,
    "ServerMessageOut", "message", "messageType", "timestamp", Security::RobloxScript, Reflection::RemoteEventCommon::SCRIPTING,
    Reflection::RemoteEventCommon::CLIENT_SERVER);
static Reflection::RemoteEventDesc<LogService, void(shared_ptr<Instance>)> event_RequestServerOutputSignal(&LogService::requestServerOutputSignal,
    "RequestServerOutputSignal", "requestingPlayer", Security::Roblox, Reflection::RemoteEventCommon::REPLICATE_ONLY,
    Reflection::RemoteEventCommon::CLIENT_SERVER);
static Reflection::RemoteEventDesc<LogService, void(shared_ptr<Instance>, std::string)> event_requestScriptExecutionSignal(
    &LogService::requestScriptExecutionSignal, "RequestScriptExecutionSignal", "requestingPlayer", "source", Security::Roblox,
    Reflection::RemoteEventCommon::REPLICATE_ONLY, Reflection::RemoteEventCommon::CLIENT_SERVER);
REFLECTION_END();

static void raiseServerEventFromMessage(shared_ptr<LogService> logService, const StandardOutMessage& message, const SystemAddress& systemAddress)
{
    ...
    , logHistory(DFInt::MaxLogHistory)
{
    ...
    messageOutConnection = StandardOut::singleton()->messageOut.connect(boost::bind(&LogService::onMessageOut, this, _1));
}
...
shared_ptr<const Reflection::ValueArray> LogService::getLogHistory()
{
    ...
    {
        shared_ptr<Reflection::ValueTable> logTable = Aya::make_shared<Reflection::ValueTable>();
...
void LogService::onServiceProvider(ServiceProvider* oldSp, ServiceProvider* newSp)
{
    if (newSp)
    {
        listenForServerLogRequestConnection =
            requestServerOutputSignal.connect(boost::bind(&LogService::maybeConnectPlayerToServerLogs, weak_from(this), _1));
...
void LogService::processRemoteEvent(
...
void LogService::doFireEvent(StandardOutMessage message, weak_ptr<LogService> logService)
{
    if (shared_ptr<LogService> sharedLogService = logService.lock())
    {
        ...
        playerList.erase(std::remove_if(playerList.begin(), playerList.end(), &playerNoLongerActive), playerList.end());

        for (std::list<weak_ptr<Network::Player>>::iterator itr = playerList.begin(); itr != playerList.end(); ++itr)
        {
            if (shared_ptr<Network::Player> player = itr->lock())
            {
                raiseServerEventFromMessage(sharedLogService, message, player->getRemoteAddressAsRbxAddress());
            }
        }
    }
}

void LogService::runCallbackIfPlayerHasConsoleAccess(shared_ptr<Network::Player> p, boost::function<void()> callback)
{
    if (DataModel* dm = DataModel::get(this))
    {
        if (ContentProvider* cp = dm->find<ContentProvider>())
        {
            Http http(format("%s/users/%d/canmanage/%d", ::trim_trailing_slashes(cp->getApiBaseUrl()).c_str(), p->getUserID(), dm->getPlaceID()));
            http.get(boost::bind(&LogService::handleCanManageResponse, callback, weak_from(this), _1, _2));
        }
    }
}
...
void LogService::handleCanManageResponse(
    boost::function<void()> callback, weak_ptr<LogService> weakLogService, std::string* responseString, std::exception* exception)
{
    if (shared_ptr<LogService> logService = weakLogService.lock())
    {
        ...
        if (LuaWebService::parseWebJSONResponseHelper(responseString, exception, valueTable, status))
        {
            ...
            if (canManageItr != valueTable->end() && canManageItr->second.isType<bool>() && canManageItr->second.cast<bool>())
            {
...
void LogService::maybeConnectPlayerToServerLogs(weak_ptr<LogService> weakLogService, shared_ptr<Instance> instance)
{
    if (shared_ptr<LogService> logService = weakLogService.lock())
    {
        if (Network::Players::serverIsPresent(logService.get()))
        {
            ...
            {
                logService->runCallbackIfPlayerHasConsoleAccess(
                    p, boost::bind(&LogService::connectPlayerToServerLogs, weakLogService, weak_ptr<Network::Player>(p)));
            }
        }
    }
}

void LogService::connectPlayerToServerLogs(weak_ptr<LogService> weakLogService, weak_ptr<Network::Player> weakPlayer)
{
    if (shared_ptr<LogService> logService = weakLogService.lock())
    {
...
{
    AYAASSERT(Network::Players::serverIsPresent(this));
    if (Network::Players::serverIsPresent(this))
    {
        ...
        {
            runCallbackIfPlayerHasConsoleAccess(p, boost::bind(&LogService::executeServerScript, weak_from(this), source));
        }
    }
}
...
void LogService::executeServerScript(weak_ptr<LogService> weakLogService, std::string source)
{
    ...
    if (shared_ptr<LogService> logService = weakLogService.lock())
    {
        if (ScriptContext* scriptContext = ServiceProvider::create<ScriptContext>(logService.get()))
        {
            ...
            try
            {
                scriptContext->executeInNewThread(Security::GameScript_, ProtectedString::fromTrustedSource(source), "console");
            }
