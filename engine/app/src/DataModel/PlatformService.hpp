...
struct IPlatformAPI
{
    ...
    //			}
    virtual int fetchInventoryInfo(shared_ptr<Aya::Reflection::ValueArray> values) = 0;
...
public:
    ...
    Aya::signal<void(int)> viewChanged;
    Aya::signal<void(int)> gameJoinedSignal;
    Aya::signal<void()> userAuthCompleteSignal;
    Aya::signal<void(int)> userAccountChangeSignal;
    Aya::signal<void(int)> robuxAmountChangedSignal;
    Aya::signal<void(std::string)> networkStatusChangedSignal;
    Aya::signal<void(std::string)> keyboardClosedSignal;

    Aya::signal<void(std::string)> lostActiveUser;
    Aya::signal<void(std::string)> gainedActiveUser;

    Aya::signal<void(std::string)> lostUserGamepad;
    Aya::signal<void(std::string)> gainedUserGamepad;

    Aya::signal<void(int)> voiceChatUserTalkingStartSignal;
    Aya::signal<void(int)> voiceChatUserTalkingEndSignal;

    Aya::signal<void()> suspendSignal;
    ...
    void popupProfileUI(InputObject::UserInputType gamepadId, std::string uid, boost::function<void(void)> resumeFunction,
    ...
    void beginSetRobloxCredentials(
        std::string accountName, std::string password, boost::function<void(int)> resumeFunction, boost::function<void(std::string)> errorFunction);
    ...
    void beginGetInventoryInfo(
        boost::function<void(shared_ptr<const Reflection::ValueArray>)> resumeFunction, boost::function<void(std::string)> errorFunction);
    void beginGetCatalogInfo(
        boost::function<void(shared_ptr<const Reflection::ValueArray>)> resumeFunction, boost::function<void(std::string)> errorFunction);
    void beginPlatformStorePurchase(
...
private:
    ...
    volatile long baFlag, bnccFlag, bsgFlag, bffFlag, ppuiFlag, ppruiFlag, bgiiFlag, bgciFlag, bpspFlag, alFlag, bgpmFlag,
        bgigpFlag; // flags for reenterancy prevention
};
} // namespace Aya