...
public:
    ...
    InputObject(UserInputType inputType, UserInputState inputState, KeyCode k, unsigned int newModCodes, SDL_Scancode newScancode,
        std::string newInputText, DataModel* dm);

    G3D::Vector2int16 getWindowSize() const;

    void setInputType(UserInputType newInputType);
    UserInputType getUserInputType() const
    {
        return inputType;
    }

    UserInputType getSourceUserInputType() const
    {
        return sourceInputType;
    }
    void setSourceUserInputType(UserInputType newInputType)
    {
        sourceInputType = newInputType;
    }

    void setInputState(UserInputState newInputState);
    UserInputState getUserInputState() const
    {
        return inputState;
    }
    ...
    unsigned int getModCodes() const
    {
        return modCodes;
    }
    ...
    {
        return (
            inputState == INPUT_STATE_BEGIN && (inputType == TYPE_MOUSEBUTTON1 || inputType == TYPE_MOUSEBUTTON2 || inputType == TYPE_MOUSEBUTTON3));
    }

    bool isMouseUpEvent() const
    {
        return (
            inputState == INPUT_STATE_END && (inputType == TYPE_MOUSEBUTTON1 || inputType == TYPE_MOUSEBUTTON2 || inputType == TYPE_MOUSEBUTTON3));
    }
    ...
    {
        return (inputType == TYPE_KEYBOARD && (inputState != INPUT_STATE_END && inputState != INPUT_STATE_NONE));
    }
    bool isKeyUpEvent() const
    {
        return (inputType == TYPE_KEYBOARD && (inputState == INPUT_STATE_END));
    }
    bool isKeyDownEvent(KeyCode code) const
    {
        return (inputType == TYPE_KEYBOARD && (inputState != INPUT_STATE_END && inputState != INPUT_STATE_NONE) && (keyCode == code));
    }
    bool isKeyUpEvent(KeyCode code) const
    {
        return (inputType == TYPE_KEYBOARD && (inputState == INPUT_STATE_END) && (keyCode == code));
    }
    bool isKeyPressedWithShiftEvent() const
    {
        return (inputType == TYPE_KEYBOARD && (inputState != INPUT_STATE_END && inputState != INPUT_STATE_NONE) &&
                ((mod & AYA_KMOD_LSHIFT) || (mod & AYA_KMOD_RSHIFT)));
    }
    ...
    {
        return (inputType == TYPE_KEYBOARD && (inputState != INPUT_STATE_END && inputState != INPUT_STATE_NONE) &&
                ((mod & AYA_KMOD_LCTRL) || (mod & AYA_KMOD_RCTRL)));
    }
    ...
    {
        return isGamepadEvent() && (keyCode == AYA_SDLK_GAMEPAD_DPADDOWN || keyCode == AYA_SDLK_GAMEPAD_DPADUP || keyCode == AYA_SDLK_GAMEPAD_DPADLEFT ||
                                       keyCode == AYA_SDLK_GAMEPAD_DPADRIGHT);
    }
