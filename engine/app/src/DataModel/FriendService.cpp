...
static Reflection::BoundFuncDesc<FriendService, void(std::string)> func_SetCreateFriendRequestUrl(
    &FriendService::setCreateFriendRequestUrl, "SetCreateFriendRequestUrl", "url", Security::LocalUser);
static Reflection::BoundFuncDesc<FriendService, void(std::string)> func_SetDeleteFriendRequestUrl(
    &FriendService::setDeleteFriendRequestUrl, "SetDeleteFriendRequestUrl", "url", Security::LocalUser);
static Reflection::BoundFuncDesc<FriendService, void(std::string)> func_SetMakeFriendUrl(
    &FriendService::setMakeFriendUrl, "SetMakeFriendUrl", "url", Security::LocalUser);
static Reflection::BoundFuncDesc<FriendService, void(std::string)> func_SetBreakFriendUrl(
    &FriendService::setBreakFriendUrl, "SetBreakFriendUrl", "url", Security::LocalUser);
static Reflection::BoundFuncDesc<FriendService, void(std::string)> func_SetGetFriendsUrl(
    &FriendService::setGetFriendsUrl, "SetGetFriendsUrl", "url", Security::LocalUser);
...
static Reflection::RemoteEventDesc<FriendService, void(int, int, FriendService::FriendEventType)> desc_friendEventReplicating(
    &FriendService::friendEventReplicatingSignal, "RemoteFriendEventSignal", "userId", "userId", "event", Security::Roblox,
    Reflection::RemoteEventCommon::REPLICATE_ONLY, Reflection::RemoteEventCommon::BROADCAST);
static Reflection::RemoteEventDesc<FriendService, void(int, int, FriendService::FriendStatus)> desc_friendStatusReplicating(
    &FriendService::friendStatusReplicatingSignal, "RemoteFriendStatusSignal", "userId", "userId", "status", Security::Roblox,
    Reflection::RemoteEventCommon::REPLICATE_ONLY, Reflection::RemoteEventCommon::CLIENT_SERVER);

static Reflection::BoundFuncDesc<FriendService, void(std::string)> func_SetFriendsOnlineUrl(
    &FriendService::setFriendsOnlineUrl, "SetFriendsOnlineUrl", "url", Security::LocalUser);
REFLECTION_END();
...
void FriendService::processServiceSuccess(int maxFriends, std::string result,
    boost::function<void(shared_ptr<const Reflection::ValueArray>)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    ...
    shared_ptr<Reflection::ValueArray> lTable(Aya::make_shared<Reflection::ValueArray>());
...
{
    errorFunction(Aya::format("FriendService: Request Failed because %s", error.c_str()));
}

void FriendService::getFriendsOnline(
    int maxFriends, boost::function<void(shared_ptr<const Reflection::ValueArray>)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    ...
    {
        apiService->getAsync(FString::FriendsOnlineUrl, true, Aya::PRIORITY_DEFAULT,
...
void FriendService::issueFriendRequestOrMakeFriendship(int userId, int otherUserId)
{
    ...
    if (!createFriendRequestUrl.empty() && !isGuest)
    {
        ...
        Http(Aya::format(createFriendRequestUrl.c_str(), userId, otherUserId))
    ...
    if (friendRequestSet.find(std::pair<int, int>(otherUserId, userId)) != friendRequestSet.end())
    {
        ...
        {
            Http(Aya::format(makeFriendUrl.c_str(), userId, otherUserId)).post("", Http::kContentTypeDefaultUnspecified, false, &DontCareResponse);
        }
    }
    else
    {
...
void FriendService::rejectFriendRequestOrBreakFriendship(int userId, int otherUserId)
{
    ...
    {
        // Fire and forget on the website
        Http(Aya::format(breakFriendUrl.c_str(), userId, otherUserId)).post("", Http::kContentTypeDefaultUnspecified, false, &DontCareResponse);
    }

    if (!deleteFriendRequestUrl.empty() && !isGuest)
    {
        // Fire and forget on the website
        Http(Aya::format(deleteFriendRequestUrl.c_str(), userId, otherUserId))
            .post("", Http::kContentTypeDefaultUnspecified, false, &DontCareResponse);
    }
...
void FriendService::ProcessBulkFriendResponse(
    weak_ptr<FriendService> weakFriendService, int userId, std::set<int> userIdsRequested, std::string* response, std::exception* error)
{
    if (shared_ptr<FriendService> friendService = weakFriendService.lock())
    {
        ...
        if (response && response->size() > 1)
        {
            ...
            {
                Aya::Log::current()->writeEntry(Log::Error, e.what());
            }
        }
        else
        {
        ...
        DataModel::get(friendService.get())
            ->submitTask(boost::bind(&FriendService::StoreFriendsHelper, weakFriendService, userId, map), DataModelJob::Write);
    }
}
    ...
    if (Network::Players::backendProcessing(this, false))
    {
        if (players.size() > 0)
        {
            if (!getBulkFriendsUrl.empty() && userId >= 0)
            {
                ...
                ostr << Aya::format(getBulkFriendsUrl.c_str(), userId);
