...
const Reflection::PropDescriptor<PartOperation, BinaryString> PartOperation::desc_ChildData("ChildData", category_Data, &PartOperation::getChildData,
    &PartOperation::setChildData, Reflection::PropertyDescriptor::CLUSTER, Security::Roblox);
const Reflection::PropDescriptor<PartOperation, BinaryString> PartOperation::desc_MeshData(
    "MeshData", category_Data, &PartOperation::getMeshData, &PartOperation::setMeshData, Reflection::PropertyDescriptor::STREAMING, Security::Roblox);
const Reflection::PropDescriptor<PartOperation, BinaryString> PartOperation::desc_PhysicsData("PhysicsData", category_Data,
    &PartOperation::getPhysicsData, &PartOperation::setPhysicsData, Reflection::PropertyDescriptor::STREAMING, Security::Roblox);
const Reflection::PropDescriptor<PartOperation, Vector3> PartOperation::desc_InitialSize("InitialSize", category_Data, &PartOperation::getInitialSize,
    &PartOperation::setInitialSize, Reflection::PropertyDescriptor::STREAMING, Security::Roblox);
const Reflection::PropDescriptor<PartOperation, bool> PartOperation::desc_UsePartColor(
    "UsePartColor", category_Data, &PartOperation::getUsePartColor, &PartOperation::setUsePartColor);
const Reflection::EnumPropDescriptor<PartOperation, PartOperation::FormFactor> PartOperation::desc_FormFactor("FormFactor", category_Data,
    &PartOperation::getFormFactor, &PartOperation::setFormFactor, Reflection::PropertyDescriptor::STREAMING, Security::Roblox);
const Reflection::PropDescriptor<PartOperation, ContentId> PartOperation::desc_AssetId(
    "AssetId", category_Data, &PartOperation::getAssetId, &PartOperation::setAssetId, Reflection::PropertyDescriptor::STREAMING, Security::Roblox);
const Reflection::EnumPropDescriptor<PartOperation, CollisionFidelity> PartOperation::prop_CollisionFidelity("CollisionFidelity", category_Behavior,
    &PartOperation::getCollisionFidelity, &PartOperation::setCollisionFidelity, Reflection::PropertyDescriptor::PUBLIC_SERIALIZED);
...
const BinaryString PartOperation::peekChildData(Aya::Instance* context)
{
    ...
    Aya::CSGDictionaryService* dictionaryService = ServiceProvider::find<CSGDictionaryService>(context ? context : this);
    Aya::NonReplicatedCSGDictionaryService* nrDictionaryService =
        ServiceProvider::create<NonReplicatedCSGDictionaryService>(context ? context : this);
...
const BinaryString& PartOperation::getChildDataBlocking(Aya::Instance* context) const
{
    if (FFlag::StudioCSGAssets)
    {
        if (hasAssetId)
        {
            ...
            boost::shared_ptr<PartOperationAsset> partOperationAsset =
                boost::static_pointer_cast<PartOperationAsset>(mcp->blockingRequestContent(getAssetId(), true));
...
shared_ptr<CSGMesh> PartOperation::getMesh()
{
    if (!cachedMesh)
    {
        ...
        CSGDictionaryService* dictionaryService = ServiceProvider::create<CSGDictionaryService>(Aya::DataModel::get(this));
...
void PartOperation::setCollisionFidelity(CollisionFidelity value)
{
    if (value != getCollisionFidelity())
    {
        ...
        {
            Aya::StandardOut::singleton()->printf(MESSAGE_WARNING, "Cannot change SolidModel CollisionFidelity during Run-Time");
        }
    }
}
...
Vector3 PartOperation::uiToXmlSize(const Vector3& uiSize) const
{
    ...
    {
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_WARNING, "Cannot set SolidModel size to Vector3(0, 0, 0)");
...
bool PartOperation::createPhysicsData(const CSGMesh* mData)
{
    if (FFlag::CSGPhysicsLevelOfDetailEnabled)
    {
        ...
        try
        {
            ...
            if (getCollisionFidelity() == CollisionFidelity_Default)
            {
                ...
                if (vertices.size() > 0 && indices.size() > 0)
                {
                    decompBinString = BinaryString(
                        TriangleMesh::generateDecompositionData(indices.size() / 3, &indices[0], vertices.size(), (btScalar*)&vertices[0].x()));
            ...
            else if (getCollisionFidelity() == CollisionFidelity_Hull)
            {
                ...
                {
                    decompBinString =
                        BinaryString(TriangleMesh::generateConvexHullData(indices.size() / 3, &indices[0], vertices.size(), &vertices[0]));
                    getPrimitive(this)->resetGeometryType(Geometry::GEOMETRY_TRI_MESH);
                }
                else
                {
    ...
    else
    {
        ...
        if (primitiveMesh)
        {
            ...
            {
                BinaryString decompBinString =
                    BinaryString(primitiveMesh->generateDecompositionGeometry(getCSGVertexPositions(mData->getVertices()), mData->getIndices()));
...
{
    CSGDictionaryService* dictionaryService = ServiceProvider::create<CSGDictionaryService>(Aya::DataModel::get(sourceOperation));
    if (PartOperation* iteratedPartOp = Aya::Instance::fastDynamicCast<PartOperation>(descendant.get()))
    {
        bool haveSameGeometry;
        /// StudioCSGAssets flag introduces some new conditions that we cannot meet with the old check. If these are enabled we must have a new
        /// condition that checks.
        if (FFlag::StudioCSGAssets)
        {
            haveSameGeometry = (sourceOperation->getMesh().get() != NULL && iteratedPartOp->getMesh().get() != NULL &&
                                sourceOperation->getMesh().get() == iteratedPartOp->getMesh().get());
        }
        else
        {
            haveSameGeometry = dictionaryService->getHashKey(sourceOperation->getMeshData().value()) ==
                               dictionaryService->getHashKey(iteratedPartOp->getMeshData().value());
        }
...
{
    if (Aya::DataModel::get(this)->isStudio())
    {
        ...
        {
            Aya::DataModel::get(this)->visitDescendants(boost::bind(&visitDescendentsSetPhysics, _1, this));
        }
        else
        {
...
void PartOperation::setBulletObjectsScale(const Vector3& newBoundingBoxSize)
{
    ...
    {
        TriangleMesh* primitiveMesh = aya_static_cast<TriangleMesh*>(getPrimitive(this)->getGeometry());
        primitiveMesh->updateObjectScale(generateHashKey(getNonKeyPhysicsData()), getNonKeyPhysicsData(),
