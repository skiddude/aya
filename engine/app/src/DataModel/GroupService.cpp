...
static Reflection::BoundYieldFuncDesc<GroupService, Reflection::Variant(int)> func_getGroupInfo(
    &GroupService::getGroupInfoAsync, "GetGroupInfoAsync", "groupId", Security::None);
static Reflection::BoundYieldFuncDesc<GroupService, shared_ptr<Instance>(int)> func_getAlliesAsync(
    &GroupService::getAlliesAsync, "GetAlliesAsync", "groupId", Security::None);
static Reflection::BoundYieldFuncDesc<GroupService, shared_ptr<Instance>(int)> func_getEnemiesAsync(
    &GroupService::getEnemiesAsync, "GetEnemiesAsync", "groupId", Security::None);
static Reflection::BoundYieldFuncDesc<GroupService, shared_ptr<const Reflection::ValueArray>(int)> func_getGroupsAsync(
    &GroupService::getGroupsAsync, "GetGroupsAsync", "userId", Security::None);
REFLECTION_END();
...
void GroupService::getGroupInfoAsync(
    const int groupId, boost::function<void(Reflection::Variant)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    if (groupId <= 0)
    {
    ...
    {
        apiService->getAsync(format(FString::GroupInfoUrl.c_str(), "", groupId), true, Aya::PRIORITY_DEFAULT,
...
void GroupService::onReceivedRawGroupInfoSuccess(weak_ptr<DataModel> weakDataModel, std::string response,
...
void GroupService::getGroupsAsync(
    int userId, boost::function<void(shared_ptr<const Reflection::ValueArray>)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    ...
    if (Aya::HttpRbxApiService* apiService = Aya::ServiceProvider::find<Aya::HttpRbxApiService>(this))
    {
        apiService->getAsync(format(FString::GetGroupsUrl.c_str(), "", userId), true, Aya::PRIORITY_DEFAULT,
...
void GroupService::onReceivedRawGetGroupsSuccess(weak_ptr<DataModel> weakDataModel, std::string result,
...
void GroupService::getAlliesAsync(
    int groupId, boost::function<void(shared_ptr<Instance>)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    std::string url;
...
void GroupService::getEnemiesAsync(
    int groupId, boost::function<void(shared_ptr<Instance>)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    std::string url;
    if (ContentProvider* cp = DataModel::get(this)->create<ContentProvider>())
