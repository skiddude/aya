...
static const PropDescriptor<VelocityMotor, float> prop_MaxVelocity(
    "MaxVelocity", category_Data, &VelocityMotor::getMaxVelocity, &VelocityMotor::setMaxVelocity);
static const PropDescriptor<VelocityMotor, float> prop_DesiredAngle(
    "DesiredAngle", category_Data, &VelocityMotor::getDesiredAngle, &VelocityMotor::setDesiredAngle);
static const PropDescriptor<VelocityMotor, float> prop_CurrentAngle(
    "CurrentAngle", category_Data, &VelocityMotor::getCurrentAngle, &VelocityMotor::setCurrentAngle);
...
static const EnumPropDescriptor<Feature, Feature::TopBottom> prop_TopBottom(
    "TopBottom", category_Data, &Feature::getTopBottom, &Feature::setTopBottom);
static const EnumPropDescriptor<Feature, Feature::LeftRight> prop_LeftRight(
    "LeftRight", category_Data, &Feature::getLeftRight, &Feature::setLeftRight);
...
CoordinateFrame Feature::computeLocalCoordinateFrame() const
{
    ...
    else
    {
        ...
        NormalId rotateId = (getCoordOrientation() == Z_OUT) ? faceId : intToNormalId((faceId + 3) % 6);
...
void MotorFeature::join(Instance* i0, Instance* i1)
{
    if (canJoin(i0, i1))
    {
        ...
        Hole* hole = aya_static_cast<Hole*>(holeInstance);
        MotorFeature* axle = aya_static_cast<MotorFeature*>(axleInstance);
...
void VelocityMotor::onAncestorChanged(const AncestorChanged& event)
{
    ...
    if (oldWorld != newWorld)
    {
        if (oldWorld)
        {
            ...
            setPart(0, NULL);
            setPart(1, NULL);
            AYAASSERT(!motorJoint()->findWorld());
        }
        if (newWorld)
        {
