...
namespace Aya
{
AYA_REGISTER_CLASS(DataStore);
AYA_REGISTER_CLASS(OrderedDataStore);
AYA_REGISTER_CLASS(DataStorePages);

const char* const sGlobalDataStore = "GlobalDataStore";


static Reflection::BoundYieldFuncDesc<DataStore, Reflection::Variant(std::string)> func_getAsync(
    &DataStore::getAsync, "GetAsync", "key", Security::None);
static Reflection::BoundYieldFuncDesc<DataStore, void(std::string, Reflection::Variant)> func_setAsync(
    &DataStore::setAsync, "SetAsync", "key", "value", Security::None);
static Reflection::BoundYieldFuncDesc<DataStore, shared_ptr<const Reflection::Tuple>(std::string, Lua::WeakFunctionRef)> func_updateAsync(
    &DataStore::updateAsync, "UpdateAsync", "key", "transformFunction", Security::None);
static Reflection::BoundYieldFuncDesc<DataStore, Reflection::Variant(std::string, int)> func_incrementAsync(
    &DataStore::incrementAsync, "IncrementAsync", "key", "delta", 1, Security::None);
static Reflection::BoundFuncDesc<DataStore, Aya::signals::connection(std::string, Lua::WeakFunctionRef)> func_onUpdate(
    &DataStore::onUpdate, "OnUpdate", "key", "callback", Security::None);

static Reflection::BoundYieldFuncDesc<OrderedDataStore, shared_ptr<Instance>(bool, int, Reflection::Variant, Reflection::Variant)>
    func_getSortedAsync(&OrderedDataStore::getSortedAsync, "GetSortedAsync", "ascending", "pagesize", "minValue", Reflection::Variant(), "maxValue",
        Reflection::Variant(), Security::None);
REFLECTION_END();

namespace Lua
{
shared_ptr<Reflection::Tuple> callCallback(
    Lua::WeakFunctionRef function, shared_ptr<const Reflection::Tuple> args, boost::intrusive_ptr<WeakThreadRef> cachedCallbackThread);
    ...
    std::string key, Reflection::Variant value, boost::function<void()> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    ...
    request.handler =
        boost::bind(&DataStore::processFetchSingleKey, shared_from(this), _1, _2, key, /*expectSubKey*/ false, resumeFunction, errorFunction);
...
void DataStore::updateAsync(std::string key, Lua::WeakFunctionRef transformFunc,
    boost::function<void(shared_ptr<const Reflection::Tuple>)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    ...
    shared_ptr<Lua::WeakFunctionRef> transform = Aya::make_shared<Lua::WeakFunctionRef>(transformFunc);

    CachedKeys::iterator it = cachedKeys.find(key);
    if (it == cachedKeys.end())
    {
        DataStoreService::HttpRequest request;
        createFetchNewKeyRequest(key, boost::bind(&DataStore::runTransformFunction, shared_from(this), key, transform, resumeFunction, errorFunction),
...
void DataStore::runTransformFunction(std::string key, shared_ptr<Lua::WeakFunctionRef> transform,
    boost::function<void(shared_ptr<const Reflection::Tuple>)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    CachedKeys::iterator it = cachedKeys.find(key);
    AYAASSERT(it != cachedKeys.end());
    ...
    std::stringstream postData;
    postData << "value=";
    postData << Aya::Http::urlEncode(newValue);
    postData << "&expectedValue=";
    postData << Aya::Http::urlEncode(expectedValue);
...
void DataStore::processSetIf(std::string key, shared_ptr<Lua::WeakFunctionRef> transform, std::string* response, std::exception* exception,
    boost::function<void(shared_ptr<const Reflection::Tuple>)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    if (response)
        FASTLOGS(FLog::DataStore, "SetIf returned %s", *response);

    DataModel::processHttpRequestResponseOnLock(DataModel::get(this), response, exception,
        boost::bind(&DataStore::lockAcquiredProcessSetIf, shared_from(this), key, transform, _1, _2, resumeFunction, errorFunction));
}

void DataStore::lockAcquiredProcessSetIf(std::string key, shared_ptr<Lua::WeakFunctionRef> transform, shared_ptr<std::string> response,
...
void DataStore::lockAcquiredProcessFetchSingleKey(shared_ptr<std::string> response, shared_ptr<std::exception> exception, std::string key,
    bool expectSubKey, boost::function<void()> callback, boost::function<void(std::string)> errorFunction)
{
    shared_ptr<const Reflection::ValueTable> result;
    std::string status;
    if (!LuaWebService::parseWebJSONResponseHelper(response.get(), exception.get(), result, status))
    ...
    if (expectSubKey)
    {
        ...
        {
            shared_ptr<const Reflection::ValueArray> subArray = itData->second.cast<shared_ptr<const Reflection::ValueArray>>();
            ...
            if (subArray->size() != 0)
            {
                ...
                shared_ptr<const Reflection::ValueTable> keyValueTable = itEntry->cast<shared_ptr<const Reflection::ValueTable>>();
...
bool DataStore::updateCachedKey(const std::string& key, const Reflection::Variant& rawValue)
{
    ...
    else
    {
        ...
        AYAASSERT(hasNonJsonType == false);
    }
...
std::string DataStore::urlEncodeIfNeeded(const std::string& input)
{
    ...
    {
        return Aya::Http::urlEncode(input);
    }
    return input;
}


void DataStore::createFetchNewKeyRequest(const std::string& key, boost::function<void()> callback, boost::function<void(std::string)> errorFunction,
    DataStoreService::HttpRequest& request)
{
...
{
    return isLegacy ? format("&qkeys[%u].scope=%s&qkeys[%u].target=&qkeys[%u].key=%s", index, scopeUrlEncodedIfNeeded.c_str(), index, index,
                          urlEncodeIfNeeded(key).c_str())
                    : format("&qkeys[%u].scope=%s&qkeys[%u].target=%s&qkeys[%u].key=%s", index, scopeUrlEncodedIfNeeded.c_str(), index,
                          urlEncodeIfNeeded(key).c_str(), index, nameUrlEncodedIfNeeded.c_str());
}
...
{
    int placeId = DataModel::get(this)->getPlaceID();
    return isLegacy ? format("%sset?placeId=%i&key=%s&&type=%s&scope=%s&target=&valueLength=%u", serviceUrl.c_str(), placeId,
                          urlEncodeIfNeeded(key).c_str(), getDataStoreTypeString(), scopeUrlEncodedIfNeeded.c_str(), valueLength)
                    : format("%sset?placeId=%i&key=%s&&type=%s&scope=%s&target=%s&valueLength=%u", serviceUrl.c_str(), placeId,
                          nameUrlEncodedIfNeeded.c_str(), getDataStoreTypeString(), scopeUrlEncodedIfNeeded.c_str(), urlEncodeIfNeeded(key).c_str(),
                          valueLength);
}
...
{
    int placeId = DataModel::get(this)->getPlaceID();
    return isLegacy ? format("%sset?placeId=%i&key=%s&type=%s&scope=%s&target=&valueLength=%u&expectedValueLength=%u", serviceUrl.c_str(), placeId,
                          urlEncodeIfNeeded(key).c_str(), getDataStoreTypeString(), scopeUrlEncodedIfNeeded.c_str(), valueLength, expectedValueLength)
                    : format("%sset?placeId=%i&key=%s&type=%s&scope=%s&target=%s&valueLength=%u&expectedValueLength=%u", serviceUrl.c_str(), placeId,
                          nameUrlEncodedIfNeeded.c_str(), getDataStoreTypeString(), scopeUrlEncodedIfNeeded.c_str(), urlEncodeIfNeeded(key).c_str(),
...
{
    int placeId = DataModel::get(this)->getPlaceID();
    return isLegacy ? format("%sincrement?placeId=%i&key=%s&type=%s&scope=%s&target=&value=%i", serviceUrl.c_str(), placeId,
                          urlEncodeIfNeeded(key).c_str(), getDataStoreTypeString(), scopeUrlEncodedIfNeeded.c_str(), delta)
                    : format("%sincrement?placeId=%i&key=%s&type=%s&scope=%s&target=%s&value=%i", serviceUrl.c_str(), placeId,
                          nameUrlEncodedIfNeeded.c_str(), getDataStoreTypeString(), scopeUrlEncodedIfNeeded.c_str(), urlEncodeIfNeeded(key).c_str(),
                          delta);
}
...
DataStore::EventSlot::EventSlot(Lua::WeakFunctionRef callback)
    : callback(callback)
{
}
...
Aya::signals::connection DataStore::onUpdate(std::string key, Lua::WeakFunctionRef callback)
{
    if (!checkAccess(key, NULL))
        return Aya::signals::connection();

    FASTLOGS(FLog::DataStore, "Subscribed to key %s", key);
    shared_ptr<Aya::signal<void(Reflection::Variant)>>& signal = onUpdateKeys[key];
    if (!signal)
        signal = Aya::make_shared<Aya::signal<void(Reflection::Variant)>>();

    shared_ptr<EventSlot> slot = Aya::make_shared<EventSlot>(callback);
    Aya::signals::connection conn = signal->connect(boost::bind(&DataStore::EventSlot::fire, slot, _1));
...
void DataStore::processFetchCachedKeys(std::string* response, std::exception* exception)
{
    ...
    DataModel::processHttpRequestResponseOnLock(
        DataModel::get(this), response, exception, boost::bind(&DataStore::lockAcquiredProcessFetchCachedKeys, shared_from(this), _1, _2));
}
...
void DataStore::lockAcquiredProcessFetchCachedKeys(shared_ptr<std::string> response, shared_ptr<std::exception> exception)
{
    ...
    {
        const Reflection::Variant errorValue = itError->second;
        std::string errorMessage = errorValue.isType<std::string>() ? errorValue.cast<std::string>() : "Failed to retrieve key";
        FASTLOGS(FLog::DataStore, "Failed, error message: %s", errorMessage);
        return;
    }
    ...
    shared_ptr<const Reflection::ValueArray> keyValueArray = itData->second.cast<shared_ptr<const Aya::Reflection::ValueArray>>();
    ...
    for (Reflection::ValueArray::const_iterator it = keyValueArray->begin(); it != keyValueArray->end(); ++it)
    {
        ...
        shared_ptr<const Reflection::ValueTable> keyValueEntry = it->cast<shared_ptr<const Aya::Reflection::ValueTable>>();

        Reflection::ValueTable::const_iterator itKey = isLegacy ? keyValueEntry->find("Key") : keyValueEntry->find("Target");

        Reflection::ValueTable::const_iterator itValue = keyValueEntry->find("Value");

        if (itKey == keyValueEntry->end() || !itKey->second.isString() || itValue == keyValueEntry->end())
        {
...
void DataStore::sendBatchGet(std::stringstream& keysList)
{
    ...
    http.post(finalKeyList, Aya::Http::kContentTypeUrlEncoded, false, boost::bind(&DataStore::processFetchCachedKeys, shared_from(this), _1, _2));
}
...
{
    FASTLOG4(FLog::DataStore, "Refetching keys, budget: %i, onUpdateSize: %u, cachedKeys size: %u, state: %u", *budget, onUpdateKeys.size(),
        cachedKeys.size(), refetchState);
...
std::string OrderedDataStore::constructGetSortedUrl(bool isAscending, int pagesize, const double* minValue, const double* maxValue)
{
    ...
    url << format("%sgetSortedValues?placeId=%i&type=%s&scope=%s&key=%s&pageSize=%i&ascending=%s", serviceUrl.c_str(), placeId,
        getDataStoreTypeString(), scopeUrlEncodedIfNeeded.c_str(), nameUrlEncodedIfNeeded.c_str(), pagesize, isAscending ? "True" : "False");
...
void DataStorePages::processFetch(
    std::string* response, std::exception* exception, boost::function<void()> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    ...
    DataModel::processHttpRequestResponseOnLock(DataModel::get(ods.get()), response, exception,
        boost::bind(&DataStorePages::lockAcquiredProcessFetch, shared_from(this), _1, _2, resumeFunction, errorFunction));
}

void DataStorePages::lockAcquiredProcessFetch(shared_ptr<std::string> response, shared_ptr<std::exception> exception,
    boost::function<void()> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    shared_ptr<const Reflection::ValueTable> result;
    std::string status;
    if (!LuaWebService::parseWebJSONResponseHelper(response.get(), exception.get(), result, status))
    ...
    shared_ptr<const Reflection::ValueTable> data = itData->second.cast<shared_ptr<const Aya::Reflection::ValueTable>>();
    ...
    shared_ptr<Reflection::ValueArray> page = Aya::make_shared<Reflection::ValueArray>();
    shared_ptr<const Reflection::ValueArray> entries = itEntries->second.cast<shared_ptr<const Aya::Reflection::ValueArray>>();
    ...
    for (Reflection::ValueArray::const_iterator it = entries->begin(); it != entries->end(); ++it)
    {
        ...
        shared_ptr<Reflection::ValueTable> pageEntry = Aya::make_shared<Reflection::ValueTable>();
...
void DataStorePages::fetchNextChunk(boost::function<void()> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    ...
    httpRequest.url = exclusiveStartKey.size() == 0 ? requestUrl : format("%s&exclusiveStartKey=%s", requestUrl.c_str(), exclusiveStartKey.c_str());
