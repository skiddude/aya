...
void GuiTextButton::render2dContext(Adorn* adorn, const Instance* context)
{
    ...
    if (filterState == ContentFilter::Succeeded)
        render2dTextImpl(adorn, rect, getText(), getFont(), getFontSize(), getRenderTextColor4(), getRenderTextStrokeColor4(), getTextWrap(),
            getTextScale(), getXAlignment(), getYAlignment());
