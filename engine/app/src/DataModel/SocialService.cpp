...
static Reflection::BoundFuncDesc<SocialService, void(std::string)> func_SetFriendUrl(
    &SocialService::setFriendUrl, "SetFriendUrl", "friendUrl", Security::LocalUser);
static Reflection::BoundFuncDesc<SocialService, void(std::string)> func_SetBestFriendUrl(
    &SocialService::setBestFriendUrl, "SetBestFriendUrl", "bestFriendUrl", Security::LocalUser);
static Reflection::BoundFuncDesc<SocialService, void(std::string)> func_SetGroupUrl(
    &SocialService::setGroupUrl, "SetGroupUrl", "groupUrl", Security::LocalUser);
static Reflection::BoundFuncDesc<SocialService, void(std::string)> func_SetGroupRankUrl(
    &SocialService::setGroupRankUrl, "SetGroupRankUrl", "groupRankUrl", Security::LocalUser);
static Reflection::BoundFuncDesc<SocialService, void(std::string)> func_SetGroupRoleUrl(
    &SocialService::setGroupRoleUrl, "SetGroupRoleUrl", "groupRoleUrl", Security::LocalUser);

static Reflection::BoundFuncDesc<SocialService, void(std::string)> func_SetStuffUrl(
    &SocialService::setStuffUrl, "SetStuffUrl", "stuffUrl", Security::LocalUser);
static Reflection::BoundFuncDesc<SocialService, void(std::string)> func_SetPackageContentsUrl(
    &SocialService::setPackageContentsUrl, "SetPackageContentsUrl", "stuffUrl", Security::LocalUser);
REFLECTION_END();
...
void SocialService::dispatchRequest(
...
void SocialService::getRankInGroup(
    int playerId, int groupId, boost::function<void(int)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    if (groupRankUrl.empty())
        errorFunction("No groupRankUrl set");
    else
    {
        std::string url(Aya::format(groupRankUrl.c_str(), playerId, groupId));
        dispatchRequest(url, resumeFunction, errorFunction);
    }
}

void SocialService::getRoleInGroup(
    int playerId, int groupId, boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    if (groupRoleUrl.empty())
        errorFunction("No groupRoleUrl set");
    else
    {
        std::string url(Aya::format(groupRoleUrl.c_str(), playerId, groupId));
        dispatchRequest(url, resumeFunction, errorFunction);
    }
}

void SocialService::isFriendsWith(
    int playerId, int userId, boost::function<void(bool)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    if (friendUrl.empty())
        errorFunction("No friendUrl set");
    else
    {
        std::string url(Aya::format(friendUrl.c_str(), playerId, userId));
        dispatchRequest(url, resumeFunction, errorFunction);
    }
}
void SocialService::isBestFriendsWith(
    int playerId, int userId, boost::function<void(bool)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    ...
    {
        std::string url(Aya::format(bestFriendUrl.c_str(), playerId, userId));
        dispatchRequest(url, resumeFunction, errorFunction);
    }
}
void SocialService::isInGroup(int playerId, int groupId, boost::function<void(bool)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    ...
    {
        std::string url(Aya::format(groupUrl.c_str(), playerId, groupId));
        dispatchRequest(url, resumeFunction, errorFunction);
    }
}
void SocialService::getListOfStuff(int playerId, StuffType category, int page,
    boost::function<void(shared_ptr<const Reflection::ValueMap>)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    ...
    {
        std::string url(Aya::format(stuffUrl.c_str(), playerId, page, category));
        dispatchRequest(url, resumeFunction, errorFunction);
    }
}
void SocialService::getPackageContents(
    int assetId, boost::function<void(shared_ptr<const Reflection::ValueArray>)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    ...
    {
        std::string url(Aya::format(packageContentsUrl.c_str(), assetId));
        dispatchRequest(url, resumeFunction, errorFunction);
    }
}
