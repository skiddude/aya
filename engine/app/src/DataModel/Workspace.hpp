...
public:
    ...
    bool getNetworkStreamingEnabled() const
    {
        return networkStreamingEnabled;
    }
    ...
    bool getExperimentalSolverEnabled() const
    {
        return experimentalSolverEnabled;
    }
    void setExperimentalSolverEnabled(bool value);
    bool experimentalSolverIsEnabled(); // getter for script accessibility
    bool getExpSolverEnabled_Replicate() const
    {
        return expSolverEnabled_Replicate;
    }
    ...
    bool getNetworkFilteringEnabled() const
    {
        return networkFilteringEnabled;
    }
    void setNetworkFilteringEnabled(bool value);

    bool getAllowThirdPartySales() const
    {
        return allowThirdPartySales;
    }
    ...
    void joinToOutsiders(shared_ptr<const Instances> items, Aya::AdvArrowToolBase::JointCreationMode joinType);
    ...
    bool startDecalDrag(Decal* decal, Aya::InsertMode insertMode);
    ...
    bool getShowAxisWidget()
    {
        return showAxisWidget;
    }

    void setShow3DGrid(bool value)
    {
        show3DGrid = value;
    }
    void setShowAxisWidget(bool value)
    {
        showAxisWidget = value;
    }
...
private:
    ...
    shared_ptr<Aya::InputObject> idleMouseEvent; // copy this for the idle event
...
public:
    ...
    bool getRightMousePan() const
    {
        return inRightMousePan;
    }
    void cancelMiddleMouseTrack();
    bool getMiddleMouseTrack() const
    {
        return inMiddleMouseTrack;
    }
    Aya::signal<void(const TouchPair&)> stepTouch; // called for each touch on a step
    Aya::signal<void(shared_ptr<Camera>)> currentCameraChangedSignal;
...
public:
    ...
    shared_ptr<const Reflection::Tuple> getRayHit(
        RbxRay ray, shared_ptr<IgnoreType> ignore, bool terrainCellsAreCubes = false, bool ignoreWaterCells = false);


    boost::shared_ptr<IAdornableCollector>& getAdornableCollector()
    {
        return adornableCollector;
    }

    // IScriptFilter
    /*override*/ bool scriptShouldRun(BaseScript* script);

    /*override*/ Extents computeExtentsWorldSlow() const
    {
        return RootInstance::computeExtentsWorld();
    }
    Extents computeExtentsWorldFast();

private:
    /*override*/ Extents computeExtentsWorld()
    {
        AYAASSERT(this->computeNumParts() < 25);
        return RootInstance::computeExtentsWorld();
    } // make sure nobody is calling this directly on the workspace?
    /*override*/ const ModelInstance* getCameraOwnerModel() const
    {
        return this;
    }
    ...
    Aya::signals::scoped_connection setDefaultMouseCommandConnection;
...
public:
    ...
    double getStatsSyncHttpGetTime() const
    {
        return statsSyncHttpGetTime;
    }
    double getStatsXMLLoadTime() const
    {
        return statsXMLLoadTime;
    }
    double getStatsJoinAllTime() const
    {
        return statsJoinAllTime;
    }
    double getStatsFileTimeTotal() const
    {
        return statsSyncHttpGetTime + statsXMLLoadTime + statsJoinAllTime;
    }

    void setStatsSyncHttpGetTime(double value)
    {
        statsSyncHttpGetTime = value;
    }
    void setStatsXMLLoadTime(double value)
    {
        statsXMLLoadTime = value;
    }
    void setStatsJoinAllTime(double value)
    {
        statsJoinAllTime = value;
    }

    double getDistributedGameTime() const
    {
        return distributedGameTime;
    }
...
public:
    static bool serverIsPresent(const Instance* context); // shortcut for Aya::Network::Players::serverIsPresent
    static bool clientIsPresent(const Instance* context); // shortcut for Aya::Network::Players::clientIsPresent
...
public:
    ...
    Aya::signal<void(int)> luaPhysicsAnalyzerIssuesFound;
