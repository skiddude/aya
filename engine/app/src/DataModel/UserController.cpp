...
static Reflection::BoundFuncDesc<Controller, void(Controller::Button, std::string)> func_BindButton(
    &Controller::bindButton, "BindButton", "button", "caption", Security::None);
static Reflection::BoundFuncDesc<Controller, void(Controller::Button, std::string)> dep_bindButton(
    &Controller::bindButton, "bindButton", "button", "caption", Security::None, Reflection::Descriptor::Attributes::deprecated(func_BindButton));
static Reflection::BoundFuncDesc<Controller, void(Controller::Button)> controller_unbindButton(
    &Controller::unbindButton, "UnbindButton", "button", Security::None);
static Reflection::BoundFuncDesc<Controller, bool(Controller::Button)> controller_getButton(
    &Controller::getButton, "GetButton", "button", Security::None);
static Reflection::BoundFuncDesc<Controller, bool(Controller::Button)> dep_getButton(
    &Controller::getButton, "getButton", "button", Security::None, Reflection::Descriptor::Attributes::deprecated(controller_getButton));
...
template<>
bool Aya::StringConverter<Controller::Button>::convertToValue(const std::string& text, Controller::Button& value)
{
    if (text.find("Jump"))
    {
...
const UserInputBase* Controller::getHardwareDevice() const
{
    ...
    AYAASSERT(controllerService == ServiceProvider::find<ControllerService>(this));
...
AYA_REGISTER_CLASS(ButtonBindingWidget);
...
void ButtonBindingWidget::render2d(Adorn* adorn)
{
    ...
    if (isEnabled() && keyName != "")
    {
        ...
        adorn->drawFont2D(text, draw.center(), (float)fontSize, false, Color3::white(), Color4::clear(), Text::FONT_ARIALBOLD, Text::XALIGN_CENTER,
            Text::YALIGN_CENTER);
    }

    if (drawSelected())
    {
        adorn->outlineRect2d(draw.toRect2D(), 3, Color3::green());
    }
...
bool HumanoidController::preventMovement(Humanoid* humanoid)
{
    ...
    //  - sitting
    //  - on a platform
    return humanoid->getTyping() || humanoid->getSit() || humanoid->getPlatformStanding();
}
...
void HumanoidController::updateCamera(const Stepped& event, const NavKeys& nav)
{
    ...
    int rotation = Aya::GameBasicSettings::singleton().camLockedInCamLockMode() ? rotation = nav.leftRightASDW() : rotation = nav.leftRightArrow();
    ...
    : hardwareDevice(NULL)
{
    ...
    humanoidController->setName(sHumanoidController);
}
