...
const char* const Aya::sSelection = "Selection";
const char* const Aya::sFilteredSelection = NULL;
...
static Reflection::BoundFuncDesc<Selection, void(shared_ptr<const Instances>)> func_setSelection(
    &Selection::setSelection, "Set", "selection", Security::Plugin);
...
{
    AYAASSERT_SLOW(connections.find(instance) == connections.end());
...
{
    AYAASSERT(connections.find(instance) != connections.end());
    Connections::iterator iter = connections.find(instance);
    iter->second.disconnect();
    connections.erase(iter);
}

void Selection::propagateChangeSignalToLua(const Aya::SelectionChanged& event)
{
    luaSelectionChanged();
}
...
{
    AYAASSERT(DataModel::get(this)->write_requested);
...
void Selection::addToSelection(Instance* instance)
{
    ...
    AYAASSERT(!DataModel::get(this) || DataModel::get(this)->write_requested);
    shared_ptr<Instance> shared = shared_from(instance);
