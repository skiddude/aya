...
class MegaClusterInstance : public DescribedCreatable<MegaClusterInstance, PartInstance, sMegaCluster, Reflection::ClassDescriptor::PERSISTENT_HIDDEN>
{
public:
    ...
    /*override*/ virtual bool getDragUtilitiesSupport() const
    {
        return false;
    }
    /*override*/ virtual Faces getResizeHandleMask() const
    {
        return Faces(NORM_NONE_MASK);
    }
    ...
    void setCellScript(int x, int y, int z, Voxel::CellMaterial material, Voxel::CellBlock block = Voxel::CELL_BLOCK_Solid,
        Voxel::CellOrientation orientation = Voxel::CELL_ORIENTATION_NegX);
    void setCellsScript(Region3int16 region, Voxel::CellMaterial material, Voxel::CellBlock block = Voxel::CELL_BLOCK_Solid,
        Voxel::CellOrientation orientation = Voxel::CELL_ORIENTATION_NegX);
    ...
    Vector3 cellCornerToWorldScript(int x, int y, int z)
    {
        return cellCornerToWorld(Vector3int16(x, y, z));
    }
    ...
    int getSmoothReplicate() const
    {
        return smoothReplicate;
    }
    ...
    float getWaterWaveSize() const
    {
        return waterWaveSize;
    }
    void setWaterWaveSize(float value);

    float getWaterWaveSpeed() const
    {
        return waterWaveSpeed;
    }
    void setWaterWaveSpeed(float value);

    Voxel2::MaterialTable* getMaterialTable() const
    {
        return materialTable.get();
    }
...
#define CLUSTER_CONST_PROP_OVERRIDE(ConstType, ArgType, FieldName, Ancestor, SetMethod) \
    ...
    CLUSTER_CONST_PROP_OVERRIDE(bool, bool, Archivable, Instance, setIsArchivable);
    CLUSTER_CONST_PROP_OVERRIDE(BrickColor, BrickColor, BrickColor, PartInstance, setColor);
    CLUSTER_CONST_PROP_OVERRIDE(bool, bool, CanCollide, PartInstance, setCanCollide);
    CLUSTER_CONST_PROP_OVERRIDE(float, float, Elasticity, PartInstance, setElasticity);
    CLUSTER_CONST_PROP_OVERRIDE(float, float, Friction, PartInstance, setFriction);
    CLUSTER_CONST_PROP_OVERRIDE(bool, bool, Locked, PartInstance, setPartLocked);
    CLUSTER_CONST_PROP_OVERRIDE(PartMaterial, PartMaterial, Material, PartInstance, setRenderMaterial);
    CLUSTER_CONST_PROP_OVERRIDE(std::string, const std::string&, Name, Instance, setName);
    CLUSTER_CONST_PROP_OVERRIDE(float, float, Reflectance, PartInstance, setReflectance);
    CLUSTER_CONST_PROP_OVERRIDE(Vector3, const Vector3&, RotVelocity, PartInstance, setRotationalVelocity);
    CLUSTER_CONST_PROP_OVERRIDE(float, float, Transparency, PartInstance, setTransparency);
    CLUSTER_CONST_PROP_OVERRIDE(Vector3, const Vector3&, Velocity, PartInstance, setLinearVelocity);
    CLUSTER_CONST_PROP_OVERRIDE(PhysicalProperties, PhysicalProperties, CustomPhysicalProperties, PartInstance, setPhysicalProperties);
...
private:
    ...
    bool isCellIdealForPreference(const Vector3int16& location, const Voxel::Cell cell, bool preferSolid) const;

    bool isCellAcceptablePreferenceWaterAlternative(const Vector3int16& location, const Voxel::Cell cell, bool preferSolid) const;

    template<class Stream>
    static void encodeChunkDataIntoStream(const Voxel::Grid::Region& chunk, Stream& s);
    template<class Stream>
    static void decodeChunkDataFromStream(Voxel::Grid& grid, const Vector3int16& chunkPos, Stream& encodedData);

    template<class Stream>
    void decodeChunkDataFromStreamV1_Deprecated(const Vector3int16& chunkPos, Stream& encodedData);
