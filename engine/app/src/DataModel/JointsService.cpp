...
static Reflection::BoundFuncDesc<JointsService, void(shared_ptr<Instance>)> func_setJoinAfterMoveInstance(
    &JointsService::setJoinAfterMoveInstance, "SetJoinAfterMoveInstance", "joinInstance", Security::None);
static Reflection::BoundFuncDesc<JointsService, void(shared_ptr<Instance>)> func_setJoinAfterMoveTarget(
    &JointsService::setJoinAfterMoveTarget, "SetJoinAfterMoveTarget", "joinTarget", Security::None);
static Reflection::BoundFuncDesc<JointsService, void()> func_showPermissibleJoints(
    &JointsService::showPermissibleJoints, "ShowPermissibleJoints", Security::None);
static Reflection::BoundFuncDesc<JointsService, void()> func_createJoinAfterMoveJoints(
    &JointsService::createJoinAfterMoveJoints, "CreateJoinAfterMoveJoints", Security::None);
static Reflection::BoundFuncDesc<JointsService, void()> func_clearJoinAfterMoveJoints(
    &JointsService::clearJoinAfterMoveJoints, "ClearJoinAfterMoveJoints", Security::None);
REFLECTION_END();
...
void JointsService::onPostInsertJoint(Joint* joint, Primitive* unGroundedPrim, std::vector<Primitive*>& combiRoots)
{
    ...
    if (hasConsistentOwner)
    {
        if (!unGroundedPrim)
        {
            ...
            AYAASSERT(joint->getPrimitive(0) && joint->getPrimitive(1)
            ...
            if (Primitive* prim = joint->getPrimitive(0) ? joint->getPrimitive(0) : joint->getPrimitive(1))
            {
                ...
                if (rootPrim && !PartInstance::isPlayerCharacterPart(rootPart))
                {
                    ...
                    else
                    {
                        ...
                        if ((Aya::Network::Players::serverIsPresent(this) || DFFlag::NetworkOwnershipRuleReplicates) &&
...
void JointsService::onPostRemoveJoint(Joint* joint, std::vector<Primitive*>& prim0Roots, std::vector<Primitive*>& prim1Roots)
{
        ...
        if (rootMovingPrim0 && rootMovingPrim1 && prim0Roots.size())
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                if (!PartInstance::isPlayerCharacterPart(rootMovingPart))
                {
                    if (DFFlag::NetworkOwnershipRuleReplicates)
                    {
                        rootMovingPart->setNetworkOwnerNotifyIfServer(
                            prim0Roots[0]->getNetworkOwner(), prim0Roots[0]->getNetworkOwnershipRuleInternal() == NetworkOwnership_Manual);
                    }
                    else
                    {
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        {
            // If RootMovingPrim is non-zero, this means that this assembly is currently un-grounded, which means we should clean-up thep revious
            // roots and migrate appropriate ownership to the new root. If the RootMovingPrim is actually zero, this means that the new assembly has
            // no net change.
...
void JointsService::onAutoJoin(Joint* joint)
{
    ...
    AYAASSERT(joint);
    AYAASSERT(!joint->getJointOwner());

    boost::shared_ptr<JointInstance> ji;
    switch (joint->getJointType())
    {
    case Joint::SNAP_JOINT:
        ji = Creatable<Instance>::create<Snap>(joint);
        break;

    case Joint::WELD_JOINT:
        ji = Creatable<Instance>::create<Weld>(joint);
        break;

    case Joint::GLUE_JOINT:
        ji = Creatable<Instance>::create<Glue>(joint);
        break;

    case Joint::ROTATE_JOINT:
        ji = Creatable<Instance>::create<Rotate>(joint);
        break;

    case Joint::ROTATE_P_JOINT:
        ji = Creatable<Instance>::create<RotateP>(joint);
        break;

    case Joint::ROTATE_V_JOINT:
        ji = Creatable<Instance>::create<RotateV>(joint);
        break;
