...
#define LINE_RAND4 ((AYA_BUILDSEED & 0x3FFFF) * __LINE__)
...
#define LINE_RAND1 (((AYA_BUILDSEED & 0xFF) * (__COUNTER__ + 1)) & 0xFC)
...
static const unsigned int kGf2EncodeLut[32] = {0x455F4314, 0xB108F6D2, 0x3C297366, 0x76EFD2BB, 0xBE165929, 0xDC284CB4, 0x69A0FE16, 0xCE19BDD9,
    0xFD1E6044, 0x0B2BD610, 0xC0BFA0AE, 0xB1004FD3, 0x79D6A004, 0x78925BB3, 0x0E320F15, 0xDB56E1D6, 0x685E2DC1, 0xFB5F13E1, 0x8B1571F0, 0x1E83936A,
    0xDB2AAE2A, 0xA49F0A74, 0xD0F8ADB7, 0x53D0B56E, 0xE69C8A79, 0xD6FCEEF9, 0x80AC6B77, 0x68A47CCF, 0x92CA6C6D, 0x7170E034, 0x4CE64F7D, 0xDB8CBB83};
...
// (these values were generated from a sage script)
static const unsigned int kGf2DecodeLut[32] = {0xFCEE5D30, 0xDC401163, 0xA295C49F, 0xA73452E1, 0x848D2984, 0x18DF419E, 0x73531D65, 0x11777D3E,
    0xD203172E, 0x5D2F07BA, 0x5A96E9DE, 0x7B80D8E4, 0xCC969B58, 0x2B99050C, 0xC45979EB, 0xC124CFE9, 0xF62F63BB, 0x49B0989B, 0xEC6E91C6, 0xB9C8406B,
    0x769E8FA2, 0xF3961122, 0x42038864, 0x3A31303E, 0x21F1D62A, 0xC75CC383, 0xB9A4B34A, 0x1050D751, 0xA10126CD, 0x3D1B091B, 0xCAC3F44F, 0x7F89722D};
...
__forceinline void setHackFlagVs(unsigned int& y, const unsigned int x)
{
#ifndef _M_IX86
    uint64_t dest = ((uint64_t)(&y) + key);
    int flag = x ^ _rotl(key, 11);
    volatile int vFlag = flag;
    volatile uint64_t tmp = dest;
    vFlag ^= _rotl(key, 11);
    *(uint64_t*)(tmp - key) |= vFlag;
#else
...
// favors "bts" over "or".
template<unsigned int key>
__forceinline void setHackFlagVmp(unsigned int& y, const unsigned int x)
{
#ifndef _M_IX86
    uint64_t dest = ((uint64_t)(&y) + key);
...
template<unsigned int key>
__forceinline unsigned int getHackFlag(const unsigned int& flag)
{
#ifndef _M_IX86
    uint64_t dest = ((uint64_t)(&flag) + key);
    volatile uint64_t tmp = dest;
    return *(uint64_t*)(tmp - key);
#else
...
template<unsigned int key>
__forceinline unsigned int getIndirectly(void* addr)
{
#ifndef _M_IX86
    uint64_t dest = ((uint64_t)(addr) + key);
    volatile uint64_t tmp = dest;
    return *(uint64_t*)(tmp - key);
#else
...
template<unsigned int key1>
inline void setHackFlagVmp(unsigned int& y, const unsigned int x)
{
    y |= x;
}
template<unsigned int key1>
inline void setHackFlagVs(unsigned int& y, const unsigned int x)
{
    y |= x;
}
template<unsigned int key>
inline unsigned int getHackFlag(const unsigned int& flag)
{
    return flag;
}
#endif
...
namespace Security
{
extern unsigned int hackFlag0, hackFlag1, hackFlag2, hackFlag3, hackFlag4, hackFlag5, hackFlag6, hackFlag7, hackFlag8, hackFlag9, hackFlag10,
    hackFlag11, hackFlag12;
}
} // namespace Aya
