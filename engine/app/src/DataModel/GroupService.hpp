...
{
private:
    static void onReceivedRawGroupInfoSuccess(weak_ptr<DataModel> weakDataModel, std::string response,
    ...
    static void onReceivedRawGetGroupsSuccess(weak_ptr<DataModel> weakDataModel, std::string response,
...
public:
    ...
    void getGroupInfoAsync(
        const int groupId, boost::function<void(Reflection::Variant)> resumeFunction, boost::function<void(std::string)> errorFunction);
    void getAlliesAsync(
        const int groupId, boost::function<void(shared_ptr<Instance>)> resumeFunction, boost::function<void(std::string)> errorFunction);
    void getEnemiesAsync(
        const int groupId, boost::function<void(shared_ptr<Instance>)> resumeFunction, boost::function<void(std::string)> errorFunction);
    void getGroupsAsync(const int userId, boost::function<void(shared_ptr<const Reflection::ValueArray>)> resumeFunction,
