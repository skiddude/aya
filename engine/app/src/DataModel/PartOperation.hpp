...
protected:
    ...
    bool hasMeshData()
    {
        return meshData.value() != "" && meshData.value().size() != 0;
    }
...
public:
    ...
    const BinaryString peekChildData(Aya::Instance* context = NULL);
    const BinaryString& getChildData() const;
    const BinaryString& getChildDataBlocking(Aya::Instance* context = NULL) const;
    ...
    Vector3 getInitialSize() const
    {
        return initialSize;
    }
    void setInitialSize(const Vector3& size);

    bool getUsePartColor() const
    {
        return usePartColor;
    }
    ...
    const ContentId& getAssetId() const
    {
        return assetId;
    }
...
class UnionOperation : public DescribedCreatable<UnionOperation, PartOperation, sUnionOperation>
{
public:
...
class NegateOperation : public DescribedCreatable<NegateOperation, PartOperation, sNegateOperation>
{
public:
