...
const char* const Aya::sChangeHistoryService = "ChangeHistoryService";
...
{
    addPair(Aya::ChangeHistoryService::Aggregate, "Aggregate");
    addPair(Aya::ChangeHistoryService::Snapshot, "Snapshot");
    addPair(Aya::ChangeHistoryService::Hybrid, "Hybrid");
}
} // namespace Reflection
} // namespace Aya
...
static BoundFuncDesc<ChangeHistoryService, void(std::string)> func_SetWaypoint(
    &ChangeHistoryService::requestWaypoint2, "SetWaypoint", "name", Security::Plugin);
...
static BoundFuncDesc<ChangeHistoryService, shared_ptr<const Reflection::Tuple>()> func_CanUndo(
    &ChangeHistoryService::canUnplay2, "GetCanUndo", Security::Plugin);
static BoundFuncDesc<ChangeHistoryService, shared_ptr<const Reflection::Tuple>()> func_CanRedo(
    &ChangeHistoryService::canPlay2, "GetCanRedo", Security::Plugin);
...
Aya::ChangeHistoryService::RuntimeUndoBehavior Aya::ChangeHistoryService::runtimeUndoBehavior = Aya::ChangeHistoryService::Aggregate;

#define AYA_GET_CELL_DETAIL(cellData) (cellData & 0xff)
#define AYA_GET_CELL_MATERIAL(cellData) (Voxel::CellMaterial)((cellData >> 8) & 0xff)
#define AYA_GET_CELL_INDEX(cellData) (cellData >> 16)
#define AYA_SET_CELL_DATA(cellIndex, detail, material) ((cellIndex << 16) | (material << 8) | (detail))

#define AYA_MAX_CLUSTER_CELLS (Voxel::kXZ_CHUNK_SIZE * Voxel::kXZ_CHUNK_SIZE * Voxel::kY_CHUNK_SIZE)
...
{
    return (AYA_GET_CELL_INDEX(lhs) < AYA_GET_CELL_INDEX(rhs));
}
...
{
    return Aya::Security::Context::current().identity == Aya::Security::Replicator_;
}
...
private:
    ...
    void applyClusterData(const std::pair<SpatialRegion::Id, ChunkCells>& chunkCellData) const
    {
        ...
        for (ChunkCells::const_iterator chunkCellsIter = chunkCells.begin(); chunkCellsIter != chunkCells.end(); ++chunkCellsIter)
        {
            ...
            changeHistory->setCell(chunkCellData.first, cellIndexToVector3(AYA_GET_CELL_INDEX(*chunkCellsIter)),
                Voxel::Cell::readUnsignedCharFromDeprecatedUse(AYA_GET_CELL_DETAIL(*chunkCellsIter)), AYA_GET_CELL_MATERIAL(*chunkCellsIter));
        }
    }
    ...
    void addValue(const PropertyDescriptor& propertyDescriptor)
    {
        ...
        if (!propertyDescriptor.canReplicate() && propertyDescriptor != PartOperation::desc_ChildData &&
            propertyDescriptor != PartOperation::prop_CollisionFidelity)
            return;

        Variant value;
        propertyDescriptor.getVariant(instance.get(), value);
        // if (properties.find(&propertyDescriptor)==properties.end())
        //	Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "ChangeHistoryService::recording %s.%s\n", instance->getName().c_str(),
    ...
    void absorbClusterData(const std::pair<SpatialRegion::Id, ChunkCells>& chunkCellData)
    {
        ...
        ChunkCells chunkCellsToCopy = chunkCellData.second, &chunkCellsLocal = clusterCellsIter->second, tempVector(AYA_MAX_CLUSTER_CELLS, 0);

        // copy local data in temp vector
        for (ChunkCells::const_iterator chunkCellsLocalIter = chunkCellsLocal.begin(); chunkCellsLocalIter != chunkCellsLocal.end();
            ++chunkCellsLocalIter)
            tempVector[AYA_GET_CELL_INDEX(*chunkCellsLocalIter)] = *chunkCellsLocalIter;

        // copy data to be assigned in temp vector
        for (ChunkCells::const_iterator chunkCellsToCopyIter = chunkCellsToCopy.begin(); chunkCellsToCopyIter != chunkCellsToCopy.end();
            ++chunkCellsToCopyIter)
            tempVector[AYA_GET_CELL_INDEX(*chunkCellsToCopyIter)] = *chunkCellsToCopyIter;

        // clear local data
        ChunkCells().swap(chunkCellsLocal);
        chunkCellsLocal.reserve(AYA_MAX_CLUSTER_CELLS);
...
public:
    ...
    {
        AYAASSERT(action == Create || action == Change);
        AYAASSERT(next.action != Delete);
        if (next.parent)
    ...
    void onSetWaypoint()
    {
        ...
        for (ClusterCells::iterator iter = clusterCells.begin(); iter != clusterCells.end(); ++iter)
        {
            ...
            tempVector.reserve(AYA_MAX_CLUSTER_CELLS);
        ...
        FASTLOG1(FLog::ChangeHistoryService, "Waypoint set, memory consumed: %u KB",
            (vectorMemory + (sizeof(unsigned int) * clusterCells.size()) + sizeof(clusterCells)) / 1024);
    }
    ...
    void recordCreate()
    {
        AYAASSERT(action == Delete || action == None);
    ...
    void addClusterChunk(const Voxel::Grid* voxelStore, const SpatialRegion::Id& id)
    {
        ...
        if (!region.isGuaranteedAllEmpty())
        {
            ...
            {
                recordClusterDataSetCell(
                    cells, it.getCurrentLocation() - startLocation, it.getCellAtCurrentLocation(), it.getMaterialAtCurrentLocation());
            }
        }
    }
    ...
    ChunkCells& recordClusterDataGetChunk(const SpatialRegion::Id& chunkIndex)
    {
        ...
        if (clusterCellsIter == clusterCells.end())
        {
            ChunkCells cells(AYA_MAX_CLUSTER_CELLS, 0);
    ...
    void recordClusterDataSetCell(ChunkCells& cells, const Vector3int16& cellPos, const Voxel::Cell& cellDetail, const Voxel::CellMaterial& material)
    {
        ...
        unsigned int cellData = AYA_SET_CELL_DATA(cellIndex, Voxel::Cell::asUnsignedCharForDeprecatedUses(cellDetail), material);
    ...
    void recordClusterData(
        const SpatialRegion::Id& chunkIndex, const Vector3int16& cellPos, const Voxel::Cell& cellDetail, const Voxel::CellMaterial& material)
    {
        ...
        const SpatialRegion::Id& chunkIndex, const Vector3int16& cellPos, const Voxel::Cell& cellDetail, const Voxel::CellMaterial& material)
    {
        ...
        for (auto& cell : cells)
        {
            if (AYA_GET_CELL_INDEX(cell) == cellIndex)
            {
                cell = cellData;
                break;
            }
    ...
    void play() const
    {
        ...
        {
        case Delete:
            // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "ChangeHistoryService Delete %s 0x%X\n", instance->getName().c_str(),
            // instance.get());
            instance->setParent(NULL);
            break;
        case Change:
        {
            // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "ChangeHistoryService Change %s 0x%X\n", instance->getName().c_str(),
            // instance.get());
        ...
        case Create:
        case Replicate:
            // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "ChangeHistoryService Create %s 0x%X\n", instance->getName().c_str(),
            // instance.get());
            std::for_each(properties.begin(), properties.end(), boost::bind(&Item::apply, this, _1));
    ...
    void unplay() const
    {
        ...
        case Delete:
            ...
            break;
        case Change:
            // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "ChangeHistoryService Un-Change %s 0x%X\n", instance->getName().c_str(),
            // instance.get());
            unplayChange();
            break;
        case Create:
            // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "ChangeHistoryService Un-Create %s 0x%X\n", instance->getName().c_str(),
            // instance.get());
            instance->setParent(NULL);
            break;
        default:
    ...
    bool getCellData(const SpatialRegion::Id& chunkIndex, boost::uint32_t cellIndex, unsigned int& cellData)
    {
        ...
        unsigned int compareData = AYA_SET_CELL_DATA(cellIndex, 0, 0);
...
public:
    std::string name;
    Waypoint(ChangeHistoryService* changeHistory)
        : changeHistory(changeHistory)
        , itemsOrderMax(0)
    {
    }
    ...
    {
        AYAASSERT_SLOW(!findItem(item.instance.get()));
    ...
    void absorb(const Waypoint* next)
    {
        ...
        for (size_t i = 0; i < itemsOrdered.size(); ++i)
        {
            ...
            {
                //				AYAASSERT(src.action==Item::Create);	// TODO  - this assert is going off
                addItem(src);
            }
        }
    }
...
void ChangeHistoryService::Item::unplayDelete() const
{
    ...
    if (deleteItem)
    {
        ...
        std::for_each(deleteItem->properties.begin(), deleteItem->properties.end(), boost::bind(&Item::apply, deleteItem, _1));
    }
};
...
void ChangeHistoryService::Item::unplayProperty(const std::pair<const PropertyDescriptor*, Variant>& pair) const
{
    ...
    while (iter != changeHistory->waypoints.begin())
    {
        ...
        if (item)
        {
            ...
            if (isParentProp && (item->action == Create || item->action == Change))
            {
...
void ChangeHistoryService::Item::unplayClusterData(const std::pair<SpatialRegion::Id, ChunkCells>& chunkCellData) const
{
    ...
    for (ChunkCells::const_iterator chunkCellsIter = chunkCells.begin(); chunkCellsIter != chunkCells.end(); ++chunkCellsIter)
    {
        cellIndexInChunk = AYA_GET_CELL_INDEX(*chunkCellsIter);
        ...
        while (iter != changeHistory->waypoints.begin())
        {
            ...
            if (item)
            {
                ...
                {
                    changeHistory->setCell(chunkIndex, cellPosInChunk, Voxel::Cell::readUnsignedCharFromDeprecatedUse(AYA_GET_CELL_DETAIL(cellData)),
                        AYA_GET_CELL_MATERIAL(cellData));
                    cellFound = true;
                    break;
                }
            }
        ...
        if (!cellFound)
            changeHistory->setCell(chunkIndex, cellPosInChunk, Voxel::Constants::kUniqueEmptyCellRepresentation, Voxel::CELL_MATERIAL_Water);
    }
}
...
void ChangeHistoryService::mergeFirstTwoWaypoints()
{
    ...
    // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "ChangeHistoryService::mergeFirstTwoWaypoints %s\n", (*next)->name.c_str());
...
void ChangeHistoryService::trimWaypoints()
{
    ...
    if (playWaypoint != waypoints.end())
    {
        ...
        {
            AYAASSERT(runStartWaypoint != waypoints.end());
            runStartWaypoint = waypoints.end();
        }
...
{
    AYAASSERT(recording == NULL);
    AYAASSERT(waypoints.size() == 0);
    recording = new Waypoint(this);
    playWaypoint = waypoints.end();
    unplayWaypoint = waypoints.begin();
...
void ChangeHistoryService::onItemAdded(shared_ptr<Instance> instance)
{
    ...
    // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "ChangeHistoryService::onItemAdded %s\n", instance->getDescriptor().name.c_str());

    Item& item = isReplicatedChange() ? (*waypoints.begin())->getItem(instance) : recording->getItem(instance);
    item.recordCreate();

    shared_ptr<Aya::MegaClusterInstance> cluster = Instance::fastSharedDynamicCast<Aya::MegaClusterInstance>(instance);
    if (cluster)
    {
...
void ChangeHistoryService::terrainCellChanged(const Voxel::CellChangeInfo& info)
{
    ...
    if (isReplicatedChange())
    {
        ...
        while (iter != waypoints.begin())
        {
            ...
            if (item)
            {
                ...
                {
                    item->updateClusterData(SpatialRegion::regionContainingVoxel(cell), SpatialRegion::voxelCoordinateRelativeToEnclosingRegion(cell),
    ...
    {
        recording->getItem(megaClusterInstance)
            .recordClusterData(SpatialRegion::regionContainingVoxel(cell), SpatialRegion::voxelCoordinateRelativeToEnclosingRegion(cell),
                info.afterCell, info.afterMaterial);
    }
}
...
void ChangeHistoryService::setCell(
    const SpatialRegion::Id& chunkPos, const Vector3int16& cellInChunk, Voxel::Cell detail, Voxel::CellMaterial material)
{
    megaClusterInstance->getVoxelGrid()->setCell(
        SpatialRegion::globalVoxelCoordinateFromRegionAndRelativeCoordinate(chunkPos, cellInChunk), detail, material);
}
...
void ChangeHistoryService::onItemRemoved(shared_ptr<Instance> instance)
{
    ...
    {
        AYAASSERT(megaClusterInstance != NULL);
        AYAASSERT(megaClusterInstance == cluster);
...
void ChangeHistoryService::onItemChanged(shared_ptr<Instance> instance, const PropertyDescriptor* descriptor)
{
    ...
    if (instance->fastDynamicCast<Camera>() && (*descriptor != Aya::Instance::desc_Name) && (*descriptor != Instance::propParent))
        return;
...
void ChangeHistoryService::Waypoint::play()
{
    ...
    for (size_t i = 0; i < itemsOrdered.size(); ++i)
    {
        ...
        {
            Aya::StandardOut::singleton()->print(Aya::MESSAGE_WARNING, ex.what());
        }
    }
...
void ChangeHistoryService::Waypoint::selectModifiedParts(bool isPlay)
{
    ...
    if (pWorkspace)
    {
        Aya::Selection* pSelection = Aya::ServiceProvider::create<Aya::Selection>(pWorkspace);
        if (pSelection)
        {
            ...
            boost::unordered_set<shared_ptr<Aya::Instance>> instancesToSelect;
            boost::unordered_set<Aya::Instance*> instancesParent;
            ...
            for (boost::unordered_set<shared_ptr<Aya::Instance>>::iterator iter = instancesToSelect.begin(); iter != instancesToSelect.end(); ++iter)
...
void ChangeHistoryService::playLua()
{
    ...
    AYAASSERT(playWaypoint != waypoints.begin());
...
void Aya::ChangeHistoryService::reportMissedPhysicsChanges(shared_ptr<Aya::Instance> instance)
{
    AYAASSERT(enabled);
    AYAASSERT(!playing);
    AYAASSERT(recording);
    ...
    {
        AYAASSERT(isRecordable(instance.get()));

        Waypoints::iterator waypoint = unplayWaypoint;
        AYAASSERT(waypoint != waypoints.end());
        ...
        while (true)
        {
            if (Item* item = (*waypoint)->findItem(instance.get()))
            {
                if (!foundCFrame)
                {
                    ...
                    {
                        const Aya::CoordinateFrame& v = iter2->second.cast<Aya::CoordinateFrame>();
                ...
                if (!foundLinearVelocity)
                {
                    ...
                    if (iter2 != item->properties.end())
                    {
                        const Aya::Vector3& v = iter2->second.cast<Aya::Vector3>();
