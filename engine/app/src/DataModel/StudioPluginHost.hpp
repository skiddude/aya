...
public:
    ...
    virtual void fireDragEnterEvent(DataModel* dataModel, shared_ptr<const Aya::Instances> instances, Vector2 location) = 0;
};
...
public:
    ...
    virtual void setSetting(int assetId, const std::string& key, const Aya::Reflection::Variant& value) = 0;
    virtual void getSetting(int assetId, const std::string& key, Aya::Reflection::Variant* result) = 0;
    ...
    virtual void exportPlace(std::string filePath, Aya::ExporterSaveType exportType) = 0;
    ...
    virtual void promptForExistingAssetId(
        const std::string& assetType, boost::function<void(int)> resumeFunction, boost::function<void(std::string)> errorFunction) = 0;
};
} // namespace Aya
