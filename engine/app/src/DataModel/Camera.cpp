...
#include "Tool/ToolsArrow.hpp" // only needed for STUDIO_CAMERA_CONTROL_SHORTCUTS
...
static Reflection::EnumPropDescriptor<Camera, Camera::CameraType> desc_cameraType(
    "CameraType", category_Camera, &Camera::getCameraType, &Camera::setCameraType);

static Reflection::BoundFuncDesc<Camera, Aya::RbxRay(float, float, float)> func_viewportToWorldRay(
    &Camera::worldRayViewportLua, "ViewportPointToRay", "x", "y", "depth", 0, Security::None);
static Reflection::BoundFuncDesc<Camera, Aya::RbxRay(float, float, float)> func_screenToWorldRay(
    &Camera::worldRayLua, "ScreenPointToRay", "x", "y", "depth", 0, Security::None);

static Reflection::BoundFuncDesc<Camera, shared_ptr<const Reflection::Tuple>(Vector3)> func_worldToViewportPoint(
    &Camera::projectViewportLua, "WorldToViewportPoint", "worldPoint", Security::None);
static Reflection::BoundFuncDesc<Camera, shared_ptr<const Reflection::Tuple>(Vector3)> func_worldToScreenPoint(
    &Camera::projectLua, "WorldToScreenPoint", "worldPoint", Security::None);

static Reflection::PropDescriptor<Camera, Vector2> desc_viewport("ViewportSize", category_Data, &Camera::getViewport, NULL);

static Reflection::PropDescriptor<Camera, CoordinateFrame> desc_CFrame(
    "CFrame", category_Data, &Camera::getCameraCoordinateFrame, &Camera::setCameraCoordinateFrame);
static Reflection::PropDescriptor<Camera, CoordinateFrame> desc_CoordFrame("CoordinateFrame", category_Data, &Camera::getCameraCoordinateFrame,
    &Camera::setCameraCoordinateFrame,
    Reflection::PropertyDescriptor::Attributes::deprecated(desc_CFrame, Reflection::PropertyDescriptor::LEGACY_SCRIPTING));
...
static Reflection::PropDescriptor<Camera, CoordinateFrame> desc_focus(
    "focus", category_Data, &Camera::getCameraFocus, &Camera::setCameraFocus, Reflection::PropertyDescriptor::Attributes::deprecated(desc_Focus));
static Reflection::PropDescriptor<Camera, float> desc_FieldOfView(
    "FieldOfView", category_Data, &Camera::getFieldOfViewDegrees, &Camera::setFieldOfViewDegrees);

static Reflection::RefPropDescriptor<Camera, Instance> cameraSubjectProp(
    "CameraSubject", category_Camera, &Camera::getCameraSubjectInstanceDangerous, &Camera::setCameraSubject);
...
static Reflection::BoundFuncDesc<Camera, void(Camera::CameraPanMode)> func_setCameraPanMode(
    &Camera::setCameraPanMode, "SetCameraPanMode", "mode", Camera::CAMERAPANMODE_CLASSIC, Security::None);
...
static Reflection::BoundFuncDesc<Camera, void(CoordinateFrame, CoordinateFrame, float)> func_interpolateCamera(
    &Camera::beginCameraInterpolation, "Interpolate", "endPos", "endFocus", "duration", Security::None);
static Reflection::EventDesc<Camera, void()> event_doneInterpolating(&Camera::interpolationFinishedSignal, "InterpolationFinished");

static Reflection::EventDesc<Camera, void(bool)> event_firstPersonTransition(
    &Camera::firstPersonTransitionSignal, "FirstPersonTransition", "entering", Security::RobloxPlace);
...
static Reflection::BoundFuncDesc<Camera, CoordinateFrame()> func_GetRenderCFrame(
    &Camera::getRenderingCoordinateFrameLua, "GetRenderCFrame", Security::None);
REFLECTION_END();
...
template<>
EnumDesc<Aya::Camera::CameraPanMode>::EnumDesc()
    : EnumDescriptor("CameraPanMode")
{
    addPair(Aya::Camera::CAMERAPANMODE_CLASSIC, "Classic");
    addPair(Aya::Camera::CAMERAPANMODE_EDGEBUMP, "EdgeBump");
}

template<>
Aya::Camera::CameraPanMode& Variant::convert<Aya::Camera::CameraPanMode>(void)
{
    return genericConvert<Aya::Camera::CameraPanMode>();
}
} // namespace Reflection

template<>
bool Aya::StringConverter<Aya::Camera::CameraPanMode>::convertToValue(const std::string& text, Aya::Camera::CameraPanMode& value)
{
    if (text.find("Classic") != std::string::npos)
    {
        value = Aya::Camera::CAMERAPANMODE_CLASSIC;
        return true;
    }
    if (text.find("EdgeBump") != std::string::npos)
    {
        value = Aya::Camera::CAMERAPANMODE_EDGEBUMP;
        return true;
    }
    return false;
}
...
{
    return ((cameraSubject.get() != NULL) && ((cameraType == Camera::FOLLOW_CAMERA) || (cameraType == Camera::ATTACH_CAMERA) ||
...
bool Camera::isPartVisibleFast(const PartInstance& part, const ContactManager& contactManager, const HitTestFilter* filter) const
{
    ...
    CoordinateFrame cframe = cameraCoord;
...
void Camera::onHeartbeat(const Heartbeat& event)
{
    ...
    else if (Aya::GameBasicSettings::singleton().inStudioMode()) // only do camera interpolation if we are currently using studio
    {
...
{
    if (Aya::Time::nowFastSec() - lastHistoryPushTime < 0.5f) // don't update history too fast
        return;
    ...
    lastHistoryPushTime = Aya::Time::nowFastSec();
...
{
    AYAASSERT(camInterpolation == CAM_INTERPOLATION_CONSTANT_SPEED);
...
void Camera::beginCameraInterpolation(CoordinateFrame endPos, CoordinateFrame endFocus, float duration)
{
    ...
    {
        AYAASSERT(duration >= 0.f);
        AYAASSERT(cameraType == Camera::LOCKED_CAMERA ||
    ...
    {
        AYAASSERT(duration > 0.f);
        AYAASSERT(cameraType == Camera::LOCKED_CAMERA); // camera must be scriptable
...
void Camera::lerpToExtents(const Extents& extents)
{
    ...
    if (Aya::ServiceProvider::findServiceProvider(this) != NULL && (Aya::Network::Players::getGameMode(this) == Aya::Network::EDIT ||
                                                                       Aya::Network::Players::getGameMode(this) == Aya::Network::DPHYS_GAME_SERVER ||
                                                                       Aya::Network::Players::getGameMode(this) == Aya::Network::GAME_SERVER))
    {
...
void Camera::zoomExtents(const Extents& extents, ZoomType zoomType)
{
    ...
    AYAASSERT(G3D::isFinite(current));
...
bool Camera::canZoom(bool inwards) const
{
    ...
    // Aya::StandardOut::singleton()->printf(MESSAGE_INFO, "%d, %d, %d", cameraToFocusDistance(), Camera::distanceMin(), Camera::distanceMax());
    return inwards ? (cameraToFocusDistance() > Camera::distanceMin()) : (cameraToFocusDistance() < Camera::distanceMax());
}
...
void Camera::setCameraSubject(Instance* newSubject)
{
    if ((newSubject != cameraSubject.get()) && dynamic_cast<CameraSubject*>(newSubject))
    {
        ...
        // this is to help when people set the camera back to the character, instead of the humanoid (control schemes can't interface with camera
        // otherwise)
        shared_ptr<Aya::Instance> potentialHuman = shared_from(cameraSubject->findFirstChildByName("Humanoid"));
        if (potentialHuman && Instance::fastDynamicCast<Aya::Humanoid>(potentialHuman.get()))
...
void Camera::setFieldOfViewDegrees(float value)
{
    ...
    if (clampedValue != value)
        StandardOut::singleton()->printf(
            MESSAGE_WARNING, "FieldOfView set out of range, should be between %f and %f, setting to %f", 1.0f, 120.0f, clampedValue);
...
{
    if (isEditMode() && !hasClientPlayer())
        return nonCharacterZoom(in);

    // @mdolli
    // behavior was for some reason altered at one point so we're restoring this for old camera controls
    if (cameraType == Camera::CUSTOM_CAMERA && !hasClientPlayer())
    {
        return false;
    }

    return isCharacterCamera() ? characterZoom(in) : nonCharacterZoom(in);
}
...
bool Camera::nonCharacterZoom(float in)
{
    ...
    {
        setCameraCoordinateFrame(
            CoordinateFrame(cameraCoord.rotation, cameraCoord.translation - (lookVector * (newDistance / currentDistance - 1.0f))));
        pushCameraHistoryStack();
        return true;
    }
}
...
{
    return Aya::ServiceProvider::findServiceProvider(this) != NULL && (Aya::Network::Players::getGameMode(this) == Aya::Network::EDIT);
}

bool Camera::hasClientPlayer() const
{
    Aya::Network::Players* players = ServiceProvider::create<Network::Players>(this);
    return players && players->getLocalPlayer();
}
...
bool Camera::characterZoom(float in) // in zoom percentage
{
    ...
    // this is absolutely FUCKED for whatever reason
    // @mdolli

    Vector3 focusToGoal = cameraCoord.translation - cameraFocus.translation;
    float currentDistance = focusToGoal.magnitude();
    float newDistance = std::min(getNewZoomDistance(currentDistance, in), distanceMaxCharacter());

    // Aya::StandardOut::singleton()->printf(MESSAGE_INFO, "STUFF: %f, %f", currentDistance, newDistance);
...
bool Camera::canTilt(int up) const
{
    ...
    return (up < 0) ? (angle >= -Math::piHalf()) : (angle <= Math::piHalf());
}
...
bool Camera::tiltRadians(float tilt)
{
    if (tilt != 0.0f)
    {
        ...
        {
            if (Aya::GameBasicSettings::singleton().inHybridMode() && !isFirstPersonCamera() &&
                isCharacterCamera()) // don't allow hybrid mode to tilt so much
...
{
    AYAASSERT(angle > -100.0f);
    AYAASSERT(angle < 100.0f); // catch weird numbers here
...
void Camera::setImageServerViewNoLerp(const CoordinateFrame& modelCoord)
{
    ...
    lookCoord.rotation *=
        Matrix3::fromEulerAnglesZXY(45.0f * G3D::pif() / 180.0f, 35.0f * G3D::pi() / 180.0f, 0.0f); // changed from 40.0, 30.0 on 3/22/07
...
Vector4 Camera::projectPointToScreen(const Vector3& point) const
{
    ...
    return Vector4(
        (screenWidth / 2.0) + ((screenWidth / 2.0) * q.x), (screenHeight / 2.0) - ((screenHeight / 2.0) * q.y), zImagePlane * 2 * q.z, out4.w);
}
...
Aya::RbxRay Camera::worldRay(float x, float y, float depth) const
{
    int screenWidth = viewport.x;
    int screenHeight = viewport.y;
    ...
    return Aya::RbxRay::fromOriginAndDirection(origin + (direction * depthToNearClipPlane) + (direction * depth), direction);
}
...
Aya::Frustum Camera::frustum() const
{
    Aya::Frustum f;
    frustum(farPlaneZ(), f);
    return f;
...
void Camera::frustum(const float farPlaneZ, Aya::Frustum& fr) const
{
...
Matrix4 Camera::getProjectionPerspective() const
{
    ...
    Matrix4 projection(w, 0, 0, 0, 0, h, 0, 0, 0, 0, q, qn, 0, 0, -1, 0);
