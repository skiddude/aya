...
static const Reflection::RefPropDescriptor<VehicleSeat, Humanoid> propOccupant(
    "Occupant", "Control", &VehicleSeat::getOccupant, NULL, Reflection::PropertyDescriptor::SCRIPTING);
...
static const Reflection::PropDescriptor<VehicleSeat, float> propTurnSpeed(
    "TurnSpeed", "Control", &VehicleSeat::getTurnSpeed, &VehicleSeat::setTurnSpeed);
...
static const Reflection::PropDescriptor<VehicleSeat, bool> propEnableHud(
    "HeadsUpDisplay", "Control", &VehicleSeat::getEnableHud, &VehicleSeat::setEnableHud);
static const Reflection::PropDescriptor<VehicleSeat, int> propNumHinges(
    "AreHingesDetected", "Control", &VehicleSeat::getNumHinges, NULL, Reflection::PropertyDescriptor::UI);
static Reflection::RemoteEventDesc<VehicleSeat, void(shared_ptr<Instance>)> event_createSeatWeld(&VehicleSeat::createSeatWeldSignal,
    "RemoteCreateSeatWeld", "humanoid", Security::None, Reflection::RemoteEventCommon::REPLICATE_ONLY, Reflection::RemoteEventCommon::CLIENT_SERVER);
static Reflection::RemoteEventDesc<VehicleSeat, void()> event_destroySeatWeld(&VehicleSeat::destroySeatWeldSignal, "RemoteDestroySeatWeld",
    ...
    , enableHud(true)
{
    ...
    createSeatWeldSignal.connect(
        boost::bind(static_cast<void (VehicleSeat::*)(shared_ptr<Instance>)>(&VehicleSeat::createSeatWeldInternal), this, _1));
...
{
    if (Aya::GameBasicSettings::singleton().getVirtualVersion() >= Aya::GameBasicSettings::VERSION_2015)
    {
        return shouldRenderRetro2d;
    }
    else
    {
        return false;
...
void VehicleSeat::render2d(Adorn* adorn)
{
    ...
    adorn->drawFont2D(speedText, speedo.positionPoint(Rect::CENTER, Rect::BOTTOM), 12, false, Color3::blue(), Color4::clear(), Text::FONT_LEGACY,
        Text::XALIGN_CENTER, Text::YALIGN_TOP);
}
...
void VehicleSeat::onLocalSeated(Humanoid* humanoid)
{
    ...
    if (humanoid)
        ...
        {
            shouldRenderRetro2d = true;
            CoordinateFrame torsoCframe = torso->getCoordinateFrame();
            workspace->getCamera()->setCameraCoordinateFrame(
                CoordinateFrame((-torsoCframe.lookVector() * 15) + torsoCframe.translation + Vector3(0, 10, 0)));
        }
}
...
void VehicleSeat::doLoadHinges(Primitive* primitive)
{
    for (int i = 0; i < primitive->numChildren(); ++i)
    {
        ...
        {
            RotateJoint* rotateJoint = aya_static_cast<RotateJoint*>(joint);
...
void VehicleSeat::stepVelocityMotors()
{
        ...
        for (int i = 0; i < velocityMotors.size(); ++i) {
                ...
                float onRight = Math::polarity(seatRight.dot(	rotateVJoint->getJointWorldCoord(0).translation -
primitive->getCoordinateFrame().translation )  );
