...
static const Aya::Name& partName = Aya::Name::declare(sModel);

////////////////////////////////////////////////////////////////////////////////////


static Reflection::PropDescriptor<ModelInstance, CoordinateFrame> desc_ModelInPrimary(
    "ModelInPrimary", category_Data, &ModelInstance::getModelInPrimary, &ModelInstance::setModelInPrimary, Reflection::PropertyDescriptor::STREAMING);
static Reflection::RefPropDescriptor<ModelInstance, PartInstance> desc_primaryPartSetByUserProp(
    "PrimaryPart", category_Data, &ModelInstance::getPrimaryPartSetByUser, &ModelInstance::setPrimaryPartSetByUser);
...
static Reflection::BoundFuncDesc<ModelInstance, G3D::Vector3()> model_getExtents(
    &ModelInstance::calculateModelSize, "GetExtentsSize", Security::None);

static Reflection::BoundFuncDesc<ModelInstance, void(G3D::Vector3)> model_translateByFunc(
    &ModelInstance::translateBy, "TranslateBy", "delta", Security::None);

static Reflection::BoundFuncDesc<ModelInstance, CoordinateFrame()> model_getPrimaryCFrame(
    &ModelInstance::getPrimaryCFrame, "GetPrimaryPartCFrame", Security::None);
static Reflection::BoundFuncDesc<ModelInstance, void(CoordinateFrame)> model_setPrimaryCFrame(
    &ModelInstance::setPrimaryCFrame, "SetPrimaryPartCFrame", "cframe", Security::None);

static Reflection::BoundFuncDesc<ModelInstance, void(G3D::Vector3)> model_moveFunction(
    &ModelInstance::moveToPointAndJoin, "MoveTo", "position", Security::None);

///////////////////////////////////////////////////////////////////////////////
// Deprecated Functions
static Reflection::BoundFuncDesc<ModelInstance, G3D::Vector3()> model_getModelSize(
    &ModelInstance::calculateModelSize, "GetModelSize", Security::None, Reflection::Descriptor::Attributes::deprecated(model_getExtents));
static Reflection::BoundFuncDesc<ModelInstance, CoordinateFrame()> model_getModelCFrame(
    &ModelInstance::calculateModelCFrame, "GetModelCFrame", Security::None, Reflection::Descriptor::Attributes::deprecated(model_getPrimaryCFrame));
static Reflection::BoundFuncDesc<ModelInstance, void()> desc_makeJointsOld(
    &ModelInstance::makeJoints, "makeJoints", Security::None, Reflection::Descriptor::Attributes::deprecated(desc_makeJoints));
static Reflection::BoundFuncDesc<ModelInstance, void()> dep_breakJoints(
    &ModelInstance::breakJoints, "breakJoints", Security::None, Reflection::Descriptor::Attributes::deprecated(desc_BreakJoints));
static Reflection::BoundFuncDesc<ModelInstance, void(G3D::Vector3)> dep_moveFunction(
    &ModelInstance::moveToPointAndJoin, "moveTo", "location", Security::None, Reflection::Descriptor::Attributes::deprecated(model_moveFunction));
static Reflection::BoundFuncDesc<ModelInstance, void(G3D::Vector3)> model_moveFunctionOld(
    &ModelInstance::moveToPointAndJoin, "move", "location", Security::None, Reflection::Descriptor::Attributes::deprecated(model_moveFunction));
static Reflection::BoundFuncDesc<ModelInstance, void()> model_setIdentityOrientation(&ModelInstance::setIdentityOrientation, "SetIdentityOrientation",
    Security::None, Reflection::Descriptor::Attributes::deprecated(model_setPrimaryCFrame));
static Reflection::BoundFuncDesc<ModelInstance, void()> model_resetOrientationToIdentity(&ModelInstance::resetOrientationToIdentity,
    "ResetOrientationToIdentity", Security::None, Reflection::Descriptor::Attributes::deprecated(model_setPrimaryCFrame));
REFLECTION_END();
...
void ModelInstance::setName(const std::string& value)
{
    if (lockedName)
    {
        Aya::Security::Context::current().requirePermission(Aya::Security::WritePlayer, "set a Character's name");
    }
    Super::setName(value);
}
...
CoordinateFrame ModelInstance::getPrimaryCFrame()
{
    ...
    throw std::runtime_error("Model:GetPrimaryCFrame() failed because no PrimaryPart has been set, or the PrimaryPart no longer exists. Please set "
                             "Model.PrimaryPart before using this.");
}

static void setCFramePart(shared_ptr<Instance> instance, const CoordinateFrame& rotationCoord, const Vector3& deltaPos, const Vector3& centerPoint,
    const PartInstance* primaryPart)
{
    if (PartInstance* part = Instance::fastDynamicCast<PartInstance>(instance.get()))
    {
        if (part == primaryPart)
...
void ModelInstance::setPrimaryCFrame(CoordinateFrame newCFrame)
{
    if (PartInstance* primaryPart = getPrimaryPartSetByUser())
    {
        ...
        visitDescendants(boost::bind(
            &setCFramePart, _1, boost::ref(rotationCoord), boost::ref(deltaCFrame.translation), boost::ref(newCFrame.translation), primaryPart));
    ...
    {
        throw std::runtime_error("Model:SetPrimaryCFrame() failed because no PrimaryPart has been set, or the PrimaryPart no longer exists. Please "
                                 "set Model.PrimaryPart before using this.");
    }
}
...
void ModelInstance::onChildAdded(Instance* child)
{
    ...
    {
        AYAASSERT_IF_VALIDATING(std::find(modelModifiers.begin(), modelModifiers.end(), child) == modelModifiers.end());
...
void ModelInstance::onChildRemoving(Instance* child)
{
    if (dynamic_cast<IModelModifier*>(child))
    {
        ...
        AYAASSERT(it != modelModifiers.end());
...
PartInstance* ModelInstance::getPrimaryPart()
{
    ...
    if (primaryPartSetByUser.lock().get())
    {
        ...
        {
            AYAASSERT(0); // just want to see this happening
    ...
    {
        AYAASSERT(computedPrimaryPart->isDescendantOf(this));
...
void VisitModelDescendants(shared_ptr<Aya::Instance> instance, PartInstance** biggestPart, float* biggestSize)
{
...
{
    AYAASSERT(computedPrimaryPart == NULL);
    computedPrimaryPart = NULL;
    float biggest = -1.0;
...
void ModelInstance::setFrontAndTop(const Vector3& front)
{
    ...
    if (primaryPartSetByUser.lock().get())
    {
        AYAASSERT(primaryPartSetByUser.lock()->isDescendantOf(this));
        CoordinateFrame base;
        base.lookAt(front);
        modelInPrimary = CoordinateFrame(primaryPartSetByUser.lock()->getCoordinateFrame().rotation.transpose() * base.rotation, Vector3::zero());
        dirtyAll();
    }
}
...
bool ModelInstance::hitTestImpl(const class Aya::RbxRay& worldRay, Vector3& worldHitPoint)
{
    for (size_t i = 0; i < numChildren(); i++)
    {
        if (PVInstance* child = queryTypedChild<PVInstance>(i))
        {
            if (child->hitTest(worldRay, worldHitPoint))
                return true;
        }
    }
    return false;
...
static void testFrustumParts(shared_ptr<Instance> descendant, const Aya::Frustum& frustum, bool& isContained)
{
    if (isContained)
    { // Only test if we have a chance!
...
bool ModelInstance::containedByFrustum(const Aya::Frustum& frustum) const
{
...
void ModelInstance::render3dSelect(Adorn* adorn, SelectState selectState)
{
    ...
    if (selectState == Aya::SELECT_NORMAL && getPrimaryPartSetByUser())
        Aya::Draw::selectionBox(getPrimaryPartSetByUser()->getPart(), adorn, primaryPartSelectColor(), primaryPartLineThickness());

    Draw::selectionBox(part, adorn, selectState, 0.05f);
}
