...
public:
    ...
    NumberSequenceKeypoint(float t, float v, float e)
        : time(t)
        , value(v)
        , envelope(e)
    {
...
public:
    ...
    void resample(Vector2* result, int numPoints, float minV = -1e22f,
        float maxV = 1e22f) const; // resamples, result is {min,max} rather than {value, envelope}
