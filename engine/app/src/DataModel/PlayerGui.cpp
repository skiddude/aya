...
{
    defaultSelectionImage = Aya::Creatable<Instance>::create<ImageLabel>();
    defaultSelectionImage->setImage(TextureId("ayaasset://textures/ui/SelectionBox.png"));
    ...
    float& minIntersectDist, float& minProjection, const GuiObject* nextSelectionGuiObject)
{
    ...
    {
        const float maxAngle = Aya::Math::piHalff() / 4.0f;
...
GuiObject* BasePlayerGui::checkForTupleSelection(
    const Vector2& direction, const Rect2D& currentRect, const shared_ptr<const Reflection::Tuple>& selectionTuple)
{
        ...
        for (Reflection::ValueArray::iterator tupleIter = guiObjectValues.begin(); tupleIter != guiObjectValues.end(); ++tupleIter)
        {
            if (tupleIter->isType<shared_ptr<Instance>>())
            {
                ...
                if (GuiObject* selectableGuiObject = Instance::fastDynamicCast<GuiObject>(selectableObject.get()))
                {
                    ...
                    {
                        if (isCloserGuiObject(currentRect, direction, selectableGuiObject, minIntersectionDist, minProjection, nextSelectedGuiObject))
                        {
...
GuiObject* BasePlayerGui::checkForDefaultGuiSelection(
    const Vector2& direction, const Rect2D& currentRect, const shared_ptr<Instance> selectionAncestor)
{
    float minProjection = START_PROJECTION_DISTANCE;
    float minIntersectionDist = minProjection;
    ...
    for (std::vector<GuiObject*>::iterator iter = guiObjectsToTest.begin(); iter != guiObjectsToTest.end(); ++iter)
    {
        if (selectionAncestor)
        {
            if (((*iter) == selectionAncestor.get() || (*iter)->isDescendantOf2(selectionAncestor)) &&
                isCloserGuiObject(currentRect, direction, (*iter), minIntersectionDist, minProjection, nextSelectedGuiObject))
            {
                nextSelectedGuiObject = (*iter);
            }
        }
        else if (isCloserGuiObject(currentRect, direction, (*iter), minIntersectionDist, minProjection, nextSelectedGuiObject))
        {
            nextSelectedGuiObject = (*iter);
        }
    }
...
{
    GuiService* guiService = Aya::ServiceProvider::find<GuiService>(this);
    if (!guiService)
    {
        return NULL;
    }
...
bool BasePlayerGui::scriptShouldRun(BaseScript* script)
{
    ...
    {
        answer = Network::Players::backendProcessing(
            this, false); // modified by db 12/2/09 - this is called when BaseScript is not in the workspace at times
    }
...
GuiResponse BasePlayerGui::processGestureOnChild(GuiBase2d* guiBase, const UserInputService::Gesture gesture,
    shared_ptr<const Aya::Reflection::ValueArray> touchPositions, shared_ptr<const Reflection::Tuple> args)
{
    return guiBase->processGesture(gesture, touchPositions, args);
}

GuiResponse BasePlayerGui::processGesture(
...
static Reflection::RefPropDescriptor<PlayerGui, GuiObject> prop_PlayerGuiSelectionImageObject("SelectionImageObject", category_Appearance,
    &PlayerGui::getSelectionImageObject, &PlayerGui::setSelectionImageObject, Reflection::PropertyDescriptor::STANDARD);
static const Reflection::BoundFuncDesc<PlayerGui, void(float)> func_setTopbarTransparency(
    &PlayerGui::setTopbarTransparency, "SetTopbarTransparency", "transparency", Security::None);
static const Reflection::BoundFuncDesc<PlayerGui, float(void)> func_getTopbarTransparency(
    &PlayerGui::getTopbarTransparency, "GetTopbarTransparency", Security::None);
static Reflection::EventDesc<PlayerGui, void(float)> event_topbarTransparencyChangedSignal(
    &PlayerGui::topbarTransparencyChangedSignal, "TopbarTransparencyChangedSignal", "transparency", Security::None);
REFLECTION_END();
...
void PlayerGui::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
    if (TeleportService* ts = ServiceProvider::create<TeleportService>(newProvider))
    {
        ...
        {
            if (DataModel::get(newProvider)->getCreatorID() == TeleportService::getPreviousCreatorId() &&
                DataModel::get(newProvider)->getCreatorType() == TeleportService::getPreviousCreatorType())
            {
...
{
    return Instance::fastDynamicCast<Aya::Network::Player>(parent) != NULL;
}
...
static const Reflection::PropDescriptor<StarterGuiService, bool> prop_showGui(
    "ShowDevelopmentGui", category_Data, &StarterGuiService::getShowGui, &StarterGuiService::setShowGui);
const Reflection::PropDescriptor<StarterGuiService, bool> StarterGuiService::prop_ResetPlayerGui("ResetPlayerGuiOnSpawn", category_Data,
    &StarterGuiService::getResetPlayerGui, &StarterGuiService::setResetPlayerGui, Reflection::PropertyDescriptor::STANDARD, Security::None);

static const Reflection::BoundFuncDesc<StarterGuiService, void(StarterGuiService::CoreGuiType, bool)> func_setCoreGuiEnabled(
    &StarterGuiService::setCoreGuiEnabled, "SetCoreGuiEnabled", "coreGuiType", "enabled", Security::None);
static const Reflection::BoundFuncDesc<StarterGuiService, bool(StarterGuiService::CoreGuiType)> func_getCoreGuiEnabled(
    &StarterGuiService::getCoreGuiEnabled, "GetCoreGuiEnabled", "coreGuiType", Security::None);
static Reflection::EventDesc<StarterGuiService, void(StarterGuiService::CoreGuiType, bool)> event_coreGuiChangedSignal(
    &StarterGuiService::coreGuiChangedSignal, "CoreGuiChangedSignal", "coreGuiType", "enabled", Security::RobloxScript);
static const Reflection::BoundFuncDesc<StarterGuiService, void(std::string, Lua::WeakFunctionRef)> func_registerSetCore(
    &StarterGuiService::registerSetCore, "RegisterSetCore", "parameterName", "setFunction", Security::RobloxScript);
static const Reflection::BoundFuncDesc<StarterGuiService, void(std::string, Lua::WeakFunctionRef)> func_registerGetCore(
    &StarterGuiService::registerGetCore, "RegisterGetCore", "parameterName", "getFunction", Security::RobloxScript);
static const Reflection::BoundFuncDesc<StarterGuiService, void(std::string, Reflection::Variant)> func_setCore(
    &StarterGuiService::setCore, "SetCore", "parameterName", "value", Security::None);
static const Reflection::BoundYieldFuncDesc<StarterGuiService, Reflection::Variant(std::string)> func_getCore(
    &StarterGuiService::getCore, "GetCore", "parameterName", Security::None);
REFLECTION_END();

namespace Reflection
{
template<>
EnumDesc<StarterGuiService::CoreGuiType>::EnumDesc()
    : EnumDescriptor("CoreGuiType")
{
...
void StarterGuiService::setCoreGuiEnabled(CoreGuiType type, bool enabled)
{
    if (!Aya::Network::Players::frontendProcessing(this))
        StandardOut::singleton()->print(
            MESSAGE_WARNING, "StarterGui:SetCoreGuiEnabled called from server script.  Did you mean to use a local script?");
...
bool StarterGuiService::getCoreGuiEnabled(CoreGuiType type)
{
    if (!Aya::Network::Players::frontendProcessing(this))
        StandardOut::singleton()->print(
            MESSAGE_WARNING, "StarterGui:GetCoreGuiEnabled called from server script.  Did you mean to use a local script?");
...
void StarterGuiService::setCore(std::string parameterName, Reflection::Variant value)
{
    ...
    {
        throw Aya::runtime_error("SetCore: %s has not been registered by the CoreScripts", parameterName.c_str());
    }
}

void StarterGuiService::getCore(
    std::string parameterName, boost::function<void(Reflection::Variant)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    ...
    if (iter != getCoreFunctions.end())
    {
        ...
        if (Lua::ThreadRef threadRef = getFunction.lock())
        {
            if (ScriptContext* sc = ServiceProvider::create<ScriptContext>(this))
            {
                ...
                {
                    errorFunction(Aya::format("GetCore: Unexpected error while invoking callback: %s", e.what()));
                }
                ...
                {
                    errorFunction(Aya::format("GetCore: CoreScript function %s did not return a result", parameterName.c_str()));
                }
            }
        }
    }
    ...
    {
        errorFunction(Aya::format("GetCore: %s has not been registered by the CoreScripts", parameterName.c_str()));
    }
}
...
static Reflection::RefPropDescriptor<CoreGuiService, GuiObject> prop_CoreSelectionImageObject("SelectionImageObject", category_Appearance,
    &CoreGuiService::getSelectionImageObject, &CoreGuiService::setSelectionImageObject, Reflection::PropertyDescriptor::STANDARD,
...
static const Reflection::BoundFuncDesc<CoreGuiService, void()> func_createRobloxScreenGui(
    &CoreGuiService::createRobloxScreenGui, "CreateRobloxScreenGui", Security::RobloxScript);
REFLECTION_END();
...
{
    screenGui = Creatable<Aya::Instance>::create<ScreenGui>();
    screenGui->setName("RobloxGui");
    screenGui->setRobloxLocked(true);
    screenGui->setParent(this);
}

shared_ptr<Aya::ScreenGui> CoreGuiService::getRobloxScreenGui()
{
    if (!screenGui)
    {
    ...
    if (Aya::ScreenGui* theScreenGui = Instance::fastDynamicCast<Aya::ScreenGui>(screenGui.get()))
    {
        return shared_from(theScreenGui);
    }

    return shared_ptr<Aya::ScreenGui>();
}
...
void CoreGuiService::addChild(Aya::Instance* instance)
{
    if (instance)
        instance->setParent(screenGui.get());
}

void CoreGuiService::removeChild(Aya::Instance* instance)
{
...
void CoreGuiService::setGuiVisibility(bool visible)
{
    if (screenGui->getChildren())
    {
        Instances::const_iterator end = screenGui->getChildren()->end();
        for (Instances::const_iterator iter = screenGui->getChildren()->begin(); iter != end; ++iter)
            if (Aya::GuiObject* obj = Instance::fastDynamicCast<GuiObject>(iter->get()))
...
void CoreGuiService::displayOnScreenMessage(int slot, const std::string& message, double duration)
{
    ...
    Rect2D rect = screenGui->fastDynamicCast<Aya::GuiBase2d>()->getRect2D();

    onScreenMessages[slot] = Creatable<Aya::Instance>::create<TextLabel>();

    Aya::Frame* bottomRightControl = Instance::fastDynamicCast<Aya::Frame>(screenGui->findFirstChildByName2("BottomRightControl", true).get());
    if (bottomRightControl)
        onScreenMessages[slot]->setParent(bottomRightControl);
    else
        onScreenMessages[slot]->setParent(screenGui.get());

    if (Aya::TextLabel* textLabel = Instance::fastDynamicCast<Aya::TextLabel>(onScreenMessages[slot].get()))
    {
        ...
        {
            onScreenMessages[slot]->setParent(bottomRightControl);
            textLabel->setSize(Aya::UDim2(0, textLabel->getTextBounds().x, 1, 0));
            textLabel->setPosition(Aya::UDim2(1, -textLabel->getTextBounds().x, -1.05 + (-slot - 0.5), 0));
        }
        else
        {
            onScreenMessages[slot]->setParent(screenGui.get());
            textLabel->setSize(Aya::UDim2(0, 300, 0, 30));
            textLabel->setPosition(Aya::UDim2(1, (int)rect.x1() - ON_SCREEN_MESSAGE_DISTANCE_TO_RIGHT, 0,
                (int)rect.y1() - ON_SCREEN_MESSAGE_DISTANCE_TO_BOTTOM - ON_SCREEN_MESSAGE_DISTANCE_BETWEEN_SLOTS * slot));
        }
    ...
    if (duration != 0)
    {
        ...
        Aya::DebrisService* ds = Aya::ServiceProvider::find<Aya::DebrisService>(this);
