...
static const Reflection::PropDescriptor<FlagStand, BrickColor> prop_Color(
    "TeamColor", category_Data, &FlagStand::getTeamColor, &FlagStand::setTeamColor);
...
void FlagStand::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
    ...
    if (oldProvider == NULL)
    {
        if (Network::Players::backendProcessing(this))
        {
            ...
            FASTLOG2(
                FLog::TouchedSignal, "Connecting Flagstand to touched signal, instance: %p, part signal: %p", this, &onDemandRead()->touchedSignal);
        }
    }
...
void FlagStand::onStepped()
{
    ...
    {
        if (watchingFlag->getJoinedStand())
        { // joined somewhere else - I no longer watch
            watchingFlag.reset();
            clonedReplacementFlag.reset();
        }
        else if (!watchingFlag->getParent())
        { // destroyed or deleted while game running
            watchingFlag.reset();
...
void FlagStand::onEvent_standTouched(shared_ptr<Instance> other)
{
    if (Network::Players::backendProcessing(this) && other)
    {
        ...
        if ((p->getTeamColor() == this->getTeamColor()) && (flag->getTeamColor() != this->getTeamColor()))
        {
...
FlagStand* FlagStandService::findRandomEmptyStandForFlag(Flag* f)
{
    ...
    for (iter = flagStands.begin(); iter != flagStands.end(); iter++)
    {
        ...
        if ((flagStand->getTeamColor() == f->getTeamColor()) && !flagStand->getJoinedFlag())
        {
    ...
    if (possibleLocations.empty())
        return NULL; // no possible locations, return default
