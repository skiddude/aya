...
void FlyweightService::onChildAdded(shared_ptr<Aya::Instance> childInstance)
{
    if (shared_ptr<Aya::BinaryStringValue> strValue = Aya::Instance::fastSharedDynamicCast<Aya::BinaryStringValue>(childInstance))
    {
...
{
    AYAASSERT(str.size() > localKeyTag.size());
...
{
    bool hasHash = FFlag::StudioCSGAssets ? isHashKey(str.value())
                                          : (str.value().size() > minKeySize && (strncmp(str.value().c_str(), localKeyTag.c_str(), minKeySize) == 0));
    if (hasHash)
    {
        std::string key = getLocalKeyHash(str);

        if (instanceMap.count(key))
            if (shared_ptr<Aya::BinaryStringValue> strChild = instanceMap.at(key).ref.lock())
...
void FlyweightService::storeStringData(BinaryString& str, bool forceIncrement, const std::string& name)
{
    ...
    if (!hasHash)
    {
        ...
        if (instanceMap.count(key))
        {
            shared_ptr<Aya::BinaryStringValue> strChild = instanceMap.at(key).ref.lock();
...
void FlyweightService::removeStringData(const BinaryString& str)
{
    if (isHashKey(str.value()))
    {
        ...
        {
            if (shared_ptr<Aya::BinaryStringValue> strChild = iter->second.ref.lock())
            {
...
void FlyweightService::cleanChildren()
{
    if (numChildren())
    {
        ...
        for (Aya::Instances::const_iterator iter = getChildren()->begin(); iter != getChildren()->end(); ++iter)
        {
            if (shared_ptr<BinaryStringValue> stringValue = Aya::Instance::fastSharedDynamicCast<BinaryStringValue>(*iter))
            {
...
{
    Aya::StandardOut::singleton()->printf(Aya::MESSAGE_OUTPUT, "Instance map size:%d ,Child count:%d", (int)instanceMap.size(), (int)numChildren());
    for (FlyweightInstanceMap::const_iterator iter = instanceMap.begin(); iter != instanceMap.end(); ++iter)
    {
        if (iter->first.c_str())
            Aya::StandardOut::singleton()->printf(
                Aya::MESSAGE_OUTPUT, "Count: %d, %s Value: %s", iter->second.count, dataType(iter->first).c_str(), iter->first.c_str());
    }
}

bool FlyweightService::isChildData(shared_ptr<Aya::Instance> childData)
{
    if (shared_ptr<Aya::BinaryStringValue> bStrValue = Aya::Instance::fastSharedDynamicCast<Aya::BinaryStringValue>(childData))
...
bool FlyweightService::isHashKey(const std::string& str)
{
    ...
    if (strncmp(str.c_str(), httpStr.c_str(), httpStr.size()) == 0 || strncmp(str.c_str(), httpsStr.c_str(), httpsStr.size()) == 0)
        return true;
