...
protected:
    ...
    Aya::signal<void()>& connectionSignal;

    Aya::signals::connection listenerConnection;
    ...
    Aya::signals::connection signalConnection;
    virtual void connectSignalListener() {}

public:
    EventReplicatorBase(Aya::signal<void()>& connectionSignal, Reflection::RemoteEventDesc<Parent, Signature>& remoteEvent,
        Reflection::BoundProp<int>& connectionCount)
    ...
    void setListenerMode(bool alreadyConnected)
    {
        if (!listenerConnection.connected())
        {
            ...
            AYAASSERT(connectionSignal.empty());
...
{
public:
    EventReplicatorImpl(Aya::signal<void()>& connectionSignal, Reflection::RemoteEventDesc<Parent, Signature>& remoteEvent,
        Reflection::BoundProp<int>& connectionCount)
...
protected:
    ...
    /*implement*/ void connectSignalListener()
    {
        this->signalConnection =
            this->remoteEvent.getSignalPtr(this->instance)->connect(boost::bind(&EventReplicatorImpl::signalProducedIncremented, this));
    }
};
...
protected:
    ...
    /*implement*/ void connectSignalListener()
    {
        this->signalConnection = this->remoteEvent.getSignalPtr(this->instance)
                                     ->connect(boost::bind(&EventReplicatorImpl<1, Parent, Signature>::signalProducedIncremented, this, _1));
    }
};
...
protected:
    ...
    {
        this->signalConnection = this->remoteEvent.getSignalPtr(this->instance)
                                     ->connect(boost::bind(&EventReplicatorImpl<2, Parent, Signature>::signalProducedIncremented, this, _1, _2));
    }
};
template<class Parent, typename Signature>
...
protected:
    ...
    {
        this->signalConnection = this->remoteEvent.getSignalPtr(this->instance)
                                     ->connect(boost::bind(&EventReplicatorImpl<3, Parent, Signature>::signalProducedIncremented, this, _1, _2, _3));
    }
};
...
{
public:
    EventReplicator(Aya::signal<void()>& connectionSignal, Reflection::RemoteEventDesc<Parent, Signature>& remoteEvent,
        Reflection::BoundProp<int>& connectionCount)
...
#define DECLARE_EVENT_REPLICATOR_SIG(Parent, eventName, signature) \
    ...
    Aya::EventReplicator<Parent, signature> CHELPER2(eventReplicator, eventName);

#define category_EventReplicator "EventReplicator"
#define IMPLEMENT_EVENT_REPLICATOR(Parent, eventDesc, eventText, eventName) \
    Reflection::BoundProp<int> Parent::CHELPER3(prop_, eventName, ConnectionCount)(eventText "ConnectionCount", category_EventReplicator, \
        &Parent::CHELPER3(var, eventName, ConnectionCount), Aya::Reflection::PropertyDescriptor::REPLICATE_ONLY);

#define CONSTRUCT_EVENT_REPLICATOR(Parent, remoteSignalName, eventDesc, eventName) \
    CHELPER2(eventReplicator, eventName) \
    (remoteSignalName.connectionSignal, eventDesc, CHELPER3(prop_, eventName, ConnectionCount)), CHELPER3(var, eventName, ConnectionCount)(0)


#define CONNECT_EVENT_REPLICATOR(eventName) CHELPER2(eventReplicator, eventName).setInstance(this)
