...
const Reflection::RefPropDescriptor<SelectionLasso, Humanoid> prop_Humanoid(
    "Humanoid", category_Data, &SelectionLasso::getHumanoidDangerous, &SelectionLasso::setHumanoid);
...
void SelectionLasso::render3dAdorn(Adorn* adorn)
{
    ...
    cframe.rotation =
        cframe.rotation * rotateXtoNegZ; // cylinder comes in aligned in +x, apply _pre transform_ to get it along -z, so that lookat works right.
...
const Reflection::RefPropDescriptor<SelectionPartLasso, PartInstance> prop_Part(
    "Part", category_Data, &SelectionPartLasso::getPartDangerous, &SelectionPartLasso::setPart);
...
static const Reflection::PropDescriptor<SelectionPointLasso, Vector3> prop_Point(
    "Point", category_Data, &SelectionPointLasso::getPoint, &SelectionPointLasso::setPoint);
REFLECTION_END();
