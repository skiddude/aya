...
#include "Utility/CompactEnum.hpp"
#include "time.hpp"
#include "intrusive_set.hpp"
...
typedef Aya::Intrusive::Set<class PartInstance, class PhysicsService>::Hook PhysicsServiceHook;
...
public:
    ...
    static void convertToNewPhysicalPropRecursive(Aya::Instance* instance);
    static bool instanceOrChildrenContainsNewCustomPhysics(Aya::Instance* instance);
...
public:
    Aya::remote_signal<void(Aya::SystemAddress)>* getOrCreateNetworkOwnerChangedSignal(bool create);

    // helper function
    static void printNetAPIDisabledMessage();

    // Networking - streaming, no UI
    static const Reflection::PropDescriptor<PartInstance, Aya::SystemAddress> prop_NetworkOwner;
    static const Reflection::PropDescriptor<PartInstance, bool> prop_NetworkIsSleeping;

    static const Reflection::PropDescriptor<PartInstance, Aya::SystemAddress> prop_ConfirmedNetworkOwner;
    static const Reflection::PropDescriptor<PartInstance, Aya::SystemAddress> prop_TransitionalNetworkOwner;
    ...
    class TouchedSignal : public Aya::signal<void(shared_ptr<Instance>)>
    {
    private:
        typedef Aya::signal<void(shared_ptr<Instance>)> Super;
        ...
        public:
            TouchedSlot(const boost::function<void(shared_ptr<Instance>)>& inner, PartInstance* source)
                : inner(inner)
                , source(shared_from(source))
            {
            ...
            TouchedSlot(const TouchedSlot& other)
                : inner(other.inner)
                , source(other.source)
            {
                shared_ptr<PartInstance> source = this->source.lock();
    ...
    public:
        PartInstance* part;
        template<typename F>
        Aya::signals::connection connect(F slot)
        {
            FASTLOG1(FLog::TouchedSignal, "Signal connected (no upper message = lua) - %p", this);
            Aya::signals::connection con = Super::connect(TouchedSlot(slot, part));
    ...
    class OnDemandPartInstance
        ...
        , public Allocator<OnDemandPartInstance>
    {
    public:
        ...
        Aya::signal<void(bool)> sleepingChangedSignal;
        Aya::signal<void(shared_ptr<Instance>)> clumpChangedSignal;

        // Fired for humanoid state when CFrame changed from reflection
        Aya::signal<void()> cframeChangedFromReflectionSignal;

        TouchTransmitter* touchTransmitter;
        Aya::atomic<int> touchedSlotCount;
        TouchedSignal touchedSignal;
        TouchedSignal touchEndedSignal;

        Aya::signal<void(shared_ptr<Instance>)> deprecatedStoppedTouchingSignal;
        Aya::signal<void()> outfitChangedSignal;

        Aya::signal<void(shared_ptr<Instance>)> localSimulationTouchedSignal;
        Aya::signal<void(bool)> buoyancyChangedSignal;

        Aya::signal<void(PartInstance*, CoordinateFrame&, Aya::Velocity&, float&)> onPositionUpdatedByNetworkSignal;
    ...
    TouchedSignal* getOrCreateTouchedSignal(bool create = true)
    {
        return (onDemandRead() || create) ? &onDemandWrite()->touchedSignal : NULL;
    }
    TouchedSignal* getOrCreateTouchedEndedSignal(bool create = true)
    {
        return (onDemandRead() || create) ? &onDemandWrite()->touchEndedSignal : NULL;
    }
    Aya::signal<void(shared_ptr<Instance>)>* getOrCreateDeprecatedStoppedTouchingSignal(bool create = true)
    {
        return (onDemandRead() || create) ? &onDemandWrite()->deprecatedStoppedTouchingSignal : NULL;
    }
    Aya::signal<void(shared_ptr<Instance>)>* getOrCreateLocalSimulationTouchedSignal(bool create = true)
    {
        return (onDemandRead() || create) ? &onDemandWrite()->localSimulationTouchedSignal : NULL;
    }
    Aya::signal<void()>* getOrCreateOutfitChangedSignal(bool create = true)
    {
        return (onDemandRead() || create) ? &onDemandWrite()->outfitChangedSignal : NULL;
    }
    ...
    virtual bool hasThreeDimensionalSize()
    {
        return true;
    }
    ...
    virtual const Vector3& getMinimumUiSize() const
    {
        static Vector3 minSize(1.0f, 1.0f, 1.0f);
        return minSize;
    }
    virtual const Vector3& getMinimumUiSizeCustom() const
    {
        static Vector3 minSize(0.2f, 0.2f, 0.2f);
        return minSize;
    }
    virtual Faces getResizeHandleMask() const
    {
    ...
    const Primitive* getConstPartPrimitive() const
    {
        return primitive.get();
    }
    virtual const Primitive* getConstPartPrimitiveVirtual() const
    {
        return primitive.get();
    }
    virtual bool getDragUtilitiesSupport() const
    {
        return true;
    }

    GfxPart* getGfxPart() const
    {
        return gfxPart;
    };
    void setGfxPart(GfxPart* p)
    {
        gfxPart = p;
    };
    ...
    const std::vector<weak_ptr<SurfaceGui>>* getSurfaceGuiCookiesRead() const
    {
        return onDemandRead() ? &onDemandRead()->surfaceGuiCookies : 0;
    }
    std::vector<weak_ptr<SurfaceGui>>* getSurfaceGuiCookiesWrite()
    {
        return &onDemandWrite()->surfaceGuiCookies;
    }

    static Primitive* getPrimitive(PartInstance* p)
    {
        return p ? p->getPartPrimitive() : NULL;
    }
    static const Primitive* getConstPrimitive(const PartInstance* p)
    {
        return p ? p->getConstPartPrimitive() : NULL;
    }
    ...
    static void primitivesToParts(const G3D::Array<Primitive*>& primitives, std::vector<shared_ptr<PartInstance>>& parts);

    static void findParts(Instance* instance, std::vector<weak_ptr<PartInstance>>& parts);
...
public:
    const OnDemandPartInstance* onDemandRead() const
    {
        return static_cast<const OnDemandPartInstance*>(Super::onDemandRead());
    };
    OnDemandPartInstance* onDemandWrite()
    {
        return static_cast<OnDemandPartInstance*>(Super::onDemandWrite());
    };
    virtual void setPartSizeXml(const Vector3& rbxSize);
    const Vector3& getPartSizeXml() const;

    virtual int getResizeIncrement() const
    {
        return 1;
    }
    virtual float getMinimumResizeIncrement() const
    {
        return 0.01f;
    }
    ...
    virtual FormFactor getFormFactor() const
    {
        return SYMETRIC;
    }
    ...
    void addInterpolationSample(
        const CoordinateFrame& cf, const Velocity& vel, const RemoteTime& timeStamp, Time now, float localTimeOffset = 0.f, int numNodesAhead = 0);
    ...
    //
    const Aya::SystemAddress getNetworkOwner() const;
    void setNetworkOwner(const Aya::SystemAddress value);
    void setNetworkOwnerNotifyIfServer(const Aya::SystemAddress value, bool ownershipBecomingManual);
    void setNetworkOwnershipRuleIfServer(Aya::NetworkOwnership value);
    ...
    void setNetworkOwnerAndNotify(const Aya::SystemAddress value);
    void notifyNetworkOwnerChanged(const Aya::SystemAddress oldOwner);
#ifdef AYA_TEST_BUILD
    Aya::signal<void(float, float)> ownershipChangeSignal;
#endif
    ...
    void setNetworkOwnershipRule(Aya::NetworkOwnership value);
    const Aya::NetworkOwnership getNetworkOwnershipRule() const;
    ...
    {
        return (1.0f - ((1.0f - localTransparencyModifier) * (1.0f - getTransparencyXml())));
    }
    ...
    {
        return reflectance;
    } // if(transparency != 0) return this->reflectance; else return 0; }
    ...
    virtual void setColor3(G3D::Color3 value);
    ...
    static void checkConsistentOwnerAndRuleResetRoots(
    ...
    {
        return calcRenderingCoordinateFrame();
    }
    /*override*/ const Vector3 getRenderSize()
    {
        return getPartSizeUi();
    }
    /*override*/ void onCameraNear(float distance);
    /*override*/ virtual void getCameraIgnorePrimitives(std::vector<const Primitive*>& primitives)
    {
        primitives.push_back(getPartPrimitive());
    }
    ...
    // IHasPrimaryPart
    //
    /*implement*/ PartInstance* getPrimaryPart()
    {
        return this;
    }
    ...
    bool containedByFrustum(const Aya::Frustum& frustum) const;
    bool intersectFrustum(const Aya::Frustum& frustum) const;

private:
    void removeTouchTransmitter();
    void addTouchTransmitter();
    bool hasTouchTransmitter() const
    {
        return onDemandRead() && onDemandRead()->touchTransmitter;
    }
