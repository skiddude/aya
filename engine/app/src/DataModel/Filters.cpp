...
HitTestFilter::Result FilterInvisibleNonColliding::filterResult(const Aya::Primitive* testMe) const
{
...
HitTestFilter::Result FilterDescendents::filterResult(const Aya::Primitive* testMe) const
{
    if (inst)
    {
        const PartInstance* testPart = PartInstance::fromConstPrimitive(testMe);
...
HitTestFilter::Result FilterDescendentsList::filterResult(const Aya::Primitive* testMe) const
{
    if (instances)
    {
        const PartInstance* testPart = PartInstance::fromConstPrimitive(testMe);
...
HitTestFilter::Result FilterCharacterOcclusion::filterResult(const Aya::Primitive* testMe) const
{
    const PartInstance* testPart = PartInstance::fromConstPrimitive(testMe);
    ...
    if (((testMe->getCoordinateFrame().translation.y < headHeight) && !testMe->hasAutoJoints() && testMe->getConstAssembly() &&
            testMe->getConstAssembly()->getAssemblyIsMovingState()) ||
        (!testPart->getCanCollide()) || (testPart->getTransparencyXml() > .95f) || (Humanoid::constHumanoidFromBodyPart(testPart) != NULL) ||
...
HitTestFilter::Result FilterHumanoidParts::filterResult(const Aya::Primitive* testMe) const
{
    const PartInstance* testPart = PartInstance::fromConstPrimitive(testMe);

    return Humanoid::constHumanoidFromBodyPart(testPart) != NULL ? HitTestFilter::IGNORE_PRIM : HitTestFilter::INCLUDE_PRIM;
}
...
HitTestFilter::Result FilterHumanoidNameOcclusion::filterResult(const Aya::Primitive* testMe) const
{
    const PartInstance* testPart = PartInstance::fromConstPrimitive(testMe);
...
HitTestFilter::Result FilterSameAssembly::filterResult(const Primitive* testMe) const
{
    if (PartInstance* assemblyPrim = assemblyPart.get())
    {
        if (!testMe->getCanCollide() ||
            (assemblyPrim->getPartPrimitive()->getAssembly()->getAssemblyPrimitive() == testMe->getConstAssembly()->getConstAssemblyPrimitive()))
        {
            return HitTestFilter::IGNORE_PRIM;
        }
    }
