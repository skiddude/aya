...
private:
    ...
    // "Backend" connections
    Aya::signals::scoped_connection seatTouched;
    Aya::signals::scoped_connection humanoidDoneSitting;
    ...
    void onEvent_seatTouched(shared_ptr<Instance> other)
    {
        if (Humanoid* h = Humanoid::humanoidFromBodyPart(other.get()))
        {
            ...
            {
                if (this->sleepTimeUp() && !findSeatWeld() && !h->getSit() && !h->getDead() && !disabled &&
    ...
    void createSeatWeldInternal(Humanoid* h)
    {
        ...
        AYAASSERT(h->getTorsoSlow()); // we just checked for this
        ...
        if (DFFlag::FixAnchoredSeatingPosition)
        {
            ...
            // root->setCoordinateFrame(this->getPartPrimitive()->getCoordinateFrame() * CoordinateFrame(Vector3(0, seatOffset + torsoOffset + 0.5f,
            // 0)));
            PartInstance* part = PartInstance::fromPrimitive(root);
            part->setCoordinateFrame(
                this->getPartPrimitive()->getCoordinateFrame() * CoordinateFrame(Vector3(0, seatOffset + torsoOffset + 0.5f, 0)));
        }
...
protected:
    ...
    void onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
    {
        ...
        {
            seatTouched =
                ActionStation<Base>::onDemandWrite()->localSimulationTouchedSignal.connect(boost::bind(&SeatImpl::onEvent_seatTouched, this, _1));
        }
    }
...
public:
    Aya::remote_signal<void(shared_ptr<Instance>)> createSeatWeldSignal;
    Aya::remote_signal<void()> destroySeatWeldSignal;
