...
public:
    typedef Aya::Intrusive::Set<PartInstance, PhysicsService> Parts;
...
private:
    ...
    Aya::signals::scoped_connection touchesConnection;
    Aya::atomic<int> touchSentCounter;
    int touchResetCount;
    int touchSendListId;

    // used to determine number of physics senders
    Aya::signals::connection playersChangedConnection;

    ConcurrencyValidator concurrencyValidator;

    /*override*/ void onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider);

    Aya::signals::connection assemblyPhysicsOnConnection;
    Aya::signals::connection assemblyPhysicsOffConnection;
...
public:
    ...
    // Interface
    Aya::signal<void(shared_ptr<Instance>)> assemblyAddingSignal;  // state change scripts
    Aya::signal<void(shared_ptr<Instance>)> assemblyRemovedSignal; // state change scripts
    ...
    {
        return touchesSendList.size();
    }
    int getTouchesId()
    {
        return touchSendListId;
    }
