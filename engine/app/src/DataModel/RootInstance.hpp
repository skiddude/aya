...
#include "DataModel/ICameraOwner.hpp"
...
private:
    ...
    void insert3dView(
        const Instances& instances, PromptMode promptMode, bool suppressMove, const Vector3* positionHint = NULL, bool lerpCameraInStudio = true);
    void insertRemoteCharacterView(
        const Instances& instances, PartArray& partArray, const Vector3* positionHint, PromptMode promptMode, bool suppressMove = false);
    void insertCharacterView(const Instances& instances, PartArray& partArray);
    void insertIdeView(const Instances& instances, PartArray& partArray, PromptMode promptMode, bool suppressMove);

    void insertHopperBin(HopperBin* bin);
    void insertSpawnLocation(SpawnLocation* s);

    void insertDecal(Decal* d, Aya::InsertMode insertMode);

    void doInsertInstances(const Instances& instances, Instance* requestedParent,
        InsertMode insertMode, // RAW: don't move, TREE: only move up, 3D_View: insert point
        PromptMode promptMode, const Vector3* positionHint = NULL, Instances* remaining = NULL, bool forceSuppressMove = false,
...
public:
    void insertInstances(const Instances& instances, Instance* requestedParent,
        InsertMode insertMode, // RAW: don't move, TREE: only move up, 3D_View: insert point
        PromptMode promptMode, const Vector3* positionHint = NULL, Instances* remaining = NULL, bool lerpCameraInStudio = true);

    // Instead of putting parts in the camera's focus, we focus the camera on where parts were pasted
    void insertPasteInstances(const Instances& instances, Instance* requestedParent,
        InsertMode insertMode, // RAW: don't move, TREE: only move up, 3D_View: insert point
        PromptMode promptMode, const Vector3* positionHint = NULL, Instances* remaining = NULL, bool lerpCameraInStudio = true);
    ...
    void publicInsertRaw(const Instances& instances, Instance* requestedParent, PartArray& partArray, bool joinPartsInInstancesOnly = false,
        bool suppressPartMove = true);
