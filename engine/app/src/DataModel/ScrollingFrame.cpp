...
const Reflection::PropDescriptor<ScrollingFrame, bool> prop_scrollingEnabled(
    "ScrollingEnabled", category_Scrolling, &ScrollingFrame::getScrollingEnabled, &ScrollingFrame::setScrollingEnabled);

const Reflection::PropDescriptor<ScrollingFrame, UDim2> prop_canvasSize(
    "CanvasSize", category_Scrolling, &ScrollingFrame::getCanvasSize, &ScrollingFrame::setCanvasSize);
const Reflection::PropDescriptor<ScrollingFrame, Vector2> prop_canvasPosition(
    "CanvasPosition", category_Scrolling, &ScrollingFrame::getCanvasPosition, &ScrollingFrame::luaSetCanvasPosition);

const Reflection::PropDescriptor<ScrollingFrame, Vector2> prop_absWindowSize(
    "AbsoluteWindowSize", category_Scrolling, &ScrollingFrame::getAbsoluteWindowSize, NULL);

const Reflection::PropDescriptor<ScrollingFrame, int> prop_scrollBarThickness(
    "ScrollBarThickness", category_Scrolling, &ScrollingFrame::getScrollBarThickness, &ScrollingFrame::setScrollBarThickness);

const Reflection::PropDescriptor<ScrollingFrame, TextureId> prop_topImage(
    "TopImage", category_Scrolling, &ScrollingFrame::getTopImage, &ScrollingFrame::setTopImage);
const Reflection::PropDescriptor<ScrollingFrame, TextureId> prop_midImage(
    "MidImage", category_Scrolling, &ScrollingFrame::getMidImage, &ScrollingFrame::setMidImage);
const Reflection::PropDescriptor<ScrollingFrame, TextureId> prop_bottomImage(
    "BottomImage", category_Scrolling, &ScrollingFrame::getBottomImage, &ScrollingFrame::setBottomImage);
...
ScrollingFrame::ScrollingFrame()
    ...
    , topImage(TextureId("ayaasset://textures/ui/Scroll/scroll-top.png"))
    , midImage(TextureId("ayaasset://textures/ui/Scroll/scroll-middle.png"))
    , bottomImage(TextureId("ayaasset://textures/ui/Scroll/scroll-bottom.png"))
...
void ScrollingFrame::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
    ...
    if (newProvider)
    {
        ...
        wheelUpTimer.reset();
        if (UserInputService* inputService = Aya::ServiceProvider::find<UserInputService>(newProvider))
        {
            inputEndedConnection = inputService->coreInputEndedEvent.connect(boost::bind(&ScrollingFrame::globalInputEnded, this, _1));
        }

        if (GuiService* guiService = Aya::ServiceProvider::find<GuiService>(newProvider))
        {
            guiServicePropertyChangedConnection =
                guiService->propertyChangedSignal.connect(boost::bind(&ScrollingFrame::selectedGuiObjectChanged, this, _1));
        }
...
void ScrollingFrame::selectedGuiObjectChanged(const Aya::Reflection::PropertyDescriptor* desc)
{
    ...
    Aya::GuiObject* selectedObject = NULL;
    ...
    if (scrollDelta != Vector2::zero())
    {
        ...
        setCanvasPosition(
            Vector2(canvasPosition.x + (getCanvasRect().width() * scrollPercent.x), canvasPosition.y + (getCanvasRect().height() * scrollPercent.y)),
            false);
    }
}
...
void ScrollingFrame::onPropertyChanged(const Reflection::PropertyDescriptor& descriptor)
{
    ...
    if (descriptor == GuiBase2d::prop_AbsoluteSize || descriptor == GuiBase2d::prop_AbsolutePosition || descriptor == prop_canvasSize ||
        descriptor == prop_canvasPosition || descriptor == prop_scrollBarThickness)
    {
        resizeChildren();
        needsRecalculate = true;
    }
...
Vector2 ScrollingFrame::calculateCanvasPositionClamped(Vector2 desiredValue) const
{
    ...
    if (Aya::Math::isNanInf(desiredValue.x))
    {
        desiredValue.x = canvasPosition.x;
    }
    if (Aya::Math::isNanInf(desiredValue.y))
    {
...
void ScrollingFrame::setCanvasPosition(Vector2 value, bool printWarnings)
{
    ...
    if (newValue != value && printWarnings)
    {
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_WARNING,
            "ScrollingFrame.CanvasPosition set to invalid position (%f, %f), clamping to position (%f, %f).", value.x, value.y, newValue.x,
            newValue.y);
    }
...
void ScrollingFrame::setScrollBarThickness(int value)
{
    ...
    if (newValue != value)
    {
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_WARNING, "ScrollingFrame.ScrollBarThickness set to negative value, clamping value to 0.");
    }
...
Rect2D ScrollingFrame::getCanvasRectLocal() const
{
    ...
    return Rect2D::xywh(rectPos.x - canvasPosition.x, rectPos.y - canvasPosition.y, width, height);
}
...
Rect2D ScrollingFrame::getClippedRect()
{
    ...
    clippedRect =
        Rect2D::xywh(clippedRect.x0(), clippedRect.y0(), canScrollVertical() ? (clippedRect.width() - scrollBarThickness - 1) : clippedRect.width(),
...
Vector2 ScrollingFrame::getFrameScrollingSize() const
{
    ...
    return (getRect2D().wh() - getVerticalScrollBarSize() - getHorizontalScrollBarSize() +
            (noCorner ? Vector2(scrollBarThickness, scrollBarThickness) : Vector2::zero()));
}
...
Vector2 ScrollingFrame::getScrollWheelDelta(bool scrollingDownValue)
{
    ...
    if (lastScrollDelta <= MAX_MULTIPLY_TIME_MSEC)
    {
        ...
        {
            scrollDeltaMultiplier =
                std::min(float(scrollDeltaMultiplier), std::min((getVisibleRect2D().height() * 0.5f) / WHEEL_SCROLL_DELTA,
                                                           (getCanvasRectLocal().height() * MAX_SCROLL_WHEEL_SCROLL_PERCENT) / WHEEL_SCROLL_DELTA));
        }
        else
        {
            scrollDeltaMultiplier =
                std::min(1.0f * scrollDeltaMultiplier, (getCanvasRectLocal().height() * MAX_SCROLL_WHEEL_SCROLL_PERCENT) / WHEEL_SCROLL_DELTA);
        }
    }
    else
    {
...
{
    if (Aya::UserInputService* userInputService = Aya::ServiceProvider::find<Aya::UserInputService>(this))
...
bool ScrollingFrame::doProcessMouseEvent(const shared_ptr<InputObject>& event, const bool inputOver)
{
    ...
    if (inputOver)
    {
        if (eventLeftButtonUp && scrollDragging)
        {
            sinkEvent = true;
        }
        if (mouseWheelBackward)
        {
            sinkEvent = true;
            scrollDownWheel();
        }
...
bool ScrollingFrame::doProcessGamepadEvent(const shared_ptr<InputObject>& event, const bool inputOver)
{
    ...
    GamepadService* gamepadService = Aya::ServiceProvider::find<GamepadService>(this);
    ...
    if (event->getKeyCode() == AYA_SDLK_GAMEPAD_THUMBSTICK1 || event->isDPadEvent())
    {
        ...
        if (event->isDPadEvent() && event->getUserInputState() == InputObject::INPUT_STATE_BEGIN)
        {
            ...
            {
            case AYA_SDLK_GAMEPAD_DPADDOWN:
                scrollingDelta = Vector2(0, -1.0f);
                break;
            case AYA_SDLK_GAMEPAD_DPADUP:
                scrollingDelta = Vector2(0, 1.0f);
                break;
            case AYA_SDLK_GAMEPAD_DPADLEFT:
                scrollingDelta = Vector2(-1.0f, 0);
                break;
            case AYA_SDLK_GAMEPAD_DPADRIGHT:
                scrollingDelta = Vector2(1.0f, 0);
                break;
            default:
                break;
        ...
        else if (event->getKeyCode() == AYA_SDLK_GAMEPAD_THUMBSTICK1)
        {
...
void ScrollingFrame::renderScrollbarLines(Adorn* adorn, const bool scrollVertical, const bool scrollHorizontal)
{
    ...
    if (GuiObject* clippingObject = firstAncestorClipping())
    {
        const Aya::Rect2D clipRect = clippingObject->getClippedRect();
...
void ScrollingFrame::renderScrollbarSection(
    Adorn* adorn, GuiDrawImage& drawImage, const TextureId& image, const char* context, const Rect2D& imageRect, const Rotation2D& rotation)
{
    if (drawImage.setImage(adorn, image, GuiDrawImage::NORMAL, NULL, this, context))
    {
        ...
        {
            const Aya::Rect2D clipRect = clippingObject->getClippedRect();
            Aya::Rect2D clippedRect = clipRect.intersect(imageRect);
            ...
            if (rotation != Rotation2D())
            {
                if (DFFlag::FixRotatedHorizontalScrollBar)
                {
                    ...
                    Aya::Rect2D clipRectRotated = Rect2D::xyxy(clippedOriginRotated, clippedx1y1Rotated);
            ...
            if (clippedRect != Aya::Rect2D())
            {
                drawImage.render2d(adorn, true, clippedRect, lowerUV, upperUV, Color3::white(), rotation, Gui::NOTHING, false);
            }
        }
        else
        {
            ...
            drawImage.render2d(adorn, true, rotatedImageRect, lowerUV, upperUV, Color3::white(),
                getAbsoluteRotation().getAngle().empty() ? rotation : getAbsoluteRotation(), Gui::NOTHING, false);
        }
    }
}
...
void ScrollingFrame::renderHorizontalScrollBar(Adorn* adorn)
{
    ...
    //////////////////////////////////////////////////////////////////////////
    // Top Image
    Rect2D topImageRect = Rect2D::xywh(lastHorizontalBarSize.x0y0(), Vector2(scrollBarThickness, lastHorizontalBarSize.height()));

    renderScrollbarSection(adorn, hTopImage, topImage, ".TopImage", topImageRect, Rotation2D(RotationAngle(-90.0f), topImageRect.center()));

    //////////////////////////////////////////////////////////////////////////
    // Mid Image
    Vector2 midImageRectSize =
        Vector2(lastHorizontalBarSize.height(), std::max(MIN_SCROLLBAR_SIZE, lastHorizontalBarSize.width() - (scrollBarThickness * 2)));
    ...
    renderScrollbarSection(adorn, hMidImage, midImage, ".MidImage", midImageRect, Rotation2D(RotationAngle(-90.0f), midImageRect.center()));
    ...
    renderScrollbarSection(
        adorn, hBottomImage, bottomImage, ".BottomImage", bottomImageRect, Rotation2D(RotationAngle(-90.0f), bottomImageRect.center()));
}
...
void ScrollingFrame::renderVerticalScrollBar(Adorn* adorn)
{
    ...
    renderScrollbarSection(adorn, vTopImage, topImage, ".TopImage", topRect, Rotation2D());
    ...
    renderScrollbarSection(adorn, vMidImage, midImage, ".MidImage", midRect, Rotation2D());

    Rect2D bottomRect = Rect2D::xywh(midRect.x0y0() + Vector2(0, midRect.height()), Vector2(scrollBarThickness, scrollBarThickness));
    renderScrollbarSection(adorn, vBottomImage, bottomImage, ".BottomImage", bottomRect, Rotation2D());
}
