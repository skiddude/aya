...
Reflection::RefPropDescriptor<FloorWire, PartInstance> FloorWire::prop_From(
    "From", category_Data, &FloorWire::getFrom, &FloorWire::setFrom, Reflection::PropertyDescriptor::STANDARD);

Reflection::RefPropDescriptor<FloorWire, PartInstance> FloorWire::prop_To(
    "To", category_Data, &FloorWire::getTo, &FloorWire::setTo, Reflection::PropertyDescriptor::STANDARD);

Reflection::PropDescriptor<FloorWire, TextureId> FloorWire::prop_Texture(
    "Texture", category_Appearance, &FloorWire::getTexture, &FloorWire::setTexture, Reflection::PropertyDescriptor::STANDARD);

Reflection::PropDescriptor<FloorWire, Vector2> FloorWire::prop_TextureSize(
    "TextureSize", category_Appearance, &FloorWire::getTextureSize, &FloorWire::setTextureSize, Reflection::PropertyDescriptor::STANDARD);

Reflection::PropDescriptor<FloorWire, float> FloorWire::prop_Velocity(
    "Velocity", category_Data, &FloorWire::getVelocity, &FloorWire::setVelocity, Reflection::PropertyDescriptor::STANDARD);

Reflection::PropDescriptor<FloorWire, float> FloorWire::prop_StudsBetweenTextures("StudsBetweenTextures", category_Data,
    &FloorWire::getStudsBetweenTextures, &FloorWire::setStudsBetweenTextures, Reflection::PropertyDescriptor::STANDARD);

Reflection::PropDescriptor<FloorWire, float> FloorWire::prop_CycleOffset(
    "CycleOffset", category_Data, &FloorWire::getCycleOffset, &FloorWire::setCycleOffset, Reflection::PropertyDescriptor::STANDARD);

Reflection::PropDescriptor<FloorWire, float> FloorWire::prop_WireRadius(
    "WireRadius", category_Data, &FloorWire::getWireRadius, &FloorWire::setWireRadius, Reflection::PropertyDescriptor::STANDARD);
...
void FloorWire::setPartInstance(weak_ptr<PartInstance>& data, PartInstance* newValue, const PartProp& prop)
{
...
void FloorWire::computeSurfacePosition(const shared_ptr<PartInstance>& part, const RbxRay& ray, Vector3* out)
{
    int surfaceId;
...
bool FloorWire::incrementalBuildSegments(
    const Workspace* workspace, const ContactManager* contactManager, const Vector3& dest, bool moveInX, std::vector<Vector3>* out)
{
    ...
    if ((moveInX && fabs(deltaGoal.x) < kMinDistanceFromBlocks) || (!moveInX && fabs(deltaGoal.z) < kMinDistanceFromBlocks))
    {
        return false;
    }
    ...
    Primitive* hitPrimitive = contactManager->getHit(hitRay, NULL, &filterHumanoidParts, hitPoint);
...
void FloorWire::buildTrailSegments(
    const Workspace* workspace, const shared_ptr<PartInstance>& from, const shared_ptr<PartInstance>& to, std::vector<Vector3>* out)
{
    ...
    computeSurfacePosition(from, RbxRay(toPosition, fromPosition - toPosition), &fromPosition);
    computeSurfacePosition(to, RbxRay(fromPosition, toPosition - fromPosition), &toPosition);

    // cast a ray down from the start point to approximate "ground"
    const ContactManager* contactManager = workspace->getWorld()->getContactManager();
    RbxRay downAtFromPosition(fromPosition, Vector3(0, -1 * (toPosition - fromPosition).magnitude(), 0));
    Vector3 hitPoint;
    FilterHumanoidParts filterHumanoidParts;
    Primitive* ground = contactManager->getHit(downAtFromPosition, NULL, &filterHumanoidParts, hitPoint);
    ...
    if (ground)
    {
        ...
        if (!incrementalBuildSegments(workspace, contactManager, toPosition, true, out))
        {
            // failed, reset state and try building segmentsz in the z direction first
            out->clear();
            out->push_back(fromPosition);
            out->push_back(groundPoint);
            if (!incrementalBuildSegments(workspace, contactManager, toPosition, false, out))
            {
                out->clear();
                out->push_back(fromPosition);
            }
...
void FloorWire::drawSegments(const Workspace* workspace, const Camera* camera, const std::vector<Vector3>& segments, Adorn* adorn)
{
    ...
    for (size_t vertex = segments.size() - 1; vertex > 0; --vertex)
    {
        ...
        if (!texture.isNull())
        {
            TextureTrail::renderInternal(workspace, camera, from, to, texture, getFullName() + ".Texture", textureSize, velocity,
                studsBetweenTextures, (lastOffset / studsBetweenTextures) + cycleOffset, colorTexture, adorn);
