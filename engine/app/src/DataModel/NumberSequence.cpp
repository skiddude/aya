...
NumberSequence::NumberSequence(const NumberSequence& r, float min /* = -1e22f */, float max /* = 1e22f */)
    : m_data(r.m_data)
{
    AYAASSERT(validate(m_data, false));
...
void NumberSequence::resample(Vector2* result, int numPoints, float minV, float maxV) const
{
    ...
    AYAASSERT(m_data.front().time == 0.0f); // ==
    AYAASSERT(m_data.back().time == 1.0f);  //
    ...
    {
        t += dt;
        while (data[src + 1].time < t)
            src++; // find the next key

        float s = (t - data[src].time) / (data[src + 1].time - data[src].time);
        float v = G3D::lerp(data[src].value, data[src + 1].value, s);
        float e = G3D::lerp(data[src].envelope, data[src + 1].envelope, s);
...
bool NumberSequence::validate(const std::vector<Key>& keys, bool exc)
{
    ...
    {
        if (exc)
            throw Aya::runtime_error("NumberSequence: requires at least 2 keypoints");
        else
            return false;
    }
    ...
    for (unsigned j = 0; j < keys.size(); ++j)
    {
        ...
        {
            if (exc)
                throw Aya::runtime_error("NumberSequence: all keypoints must be ordered by time");
            else
                return false;
        }
        //        if (keys[j].value < 0)
        //            if (exc) throw Aya::runtime_error("NumberSequence: value must be non-negative"); else return false;

        if (keys[j].envelope < 0)
        {
            if (exc)
                throw Aya::runtime_error("NumberSequence: envelope must be non-negative");
            else
                return false;
        }
        ...
        {
            if (exc)
                throw Aya::runtime_error("NumberSequence: envelope must not exceed the value");
            else
                return false;
        }
    }
    ...
    {
        if (exc)
            throw Aya::runtime_error("NumberSequence must start at time=0.0");
        else
            return false;
    }
    ...
    {
        if (exc)
            throw Aya::runtime_error("NumberSequence must end at time=1.0");
        else
            return false;
    }
...
template<>
int TypedPropertyDescriptor<NumberSequence>::getDataSize(const DescribedBase* instance) const
{
...
template<>
void TypedPropertyDescriptor<NumberSequence>::readValue(DescribedBase* instance, const XmlElement* element, IReferenceBinder& binder) const
{
    std::string nodeval;
...
template<>
void TypedPropertyDescriptor<NumberSequence>::writeValue(const DescribedBase* instance, XmlElement* element) const
{
...
template<>
bool TypedPropertyDescriptor<NumberSequence>::hasStringValue() const
{
    return true;
}

template<>
std::string TypedPropertyDescriptor<NumberSequence>::getStringValue(const DescribedBase* instance) const
{
    return tostr(getValue(instance));
}

template<>
bool TypedPropertyDescriptor<NumberSequence>::setStringValue(DescribedBase* instance, const std::string& text) const
{
...
template<>
NumberSequence& Variant::convert<NumberSequence>()
{
    return genericConvert<NumberSequence>();
}

template<>
Type const& Type::getSingleton<NumberSequence>()
{
...
template<>
NumberSequenceKeypoint& Variant::convert<NumberSequenceKeypoint>()
{
    return genericConvert<NumberSequenceKeypoint>();
}

template<>
Type const& Type::getSingleton<NumberSequenceKeypoint>()
{
...
template<>
std::string StringConverter<NumberSequence>::convertToString(const NumberSequence& value)
{
    return tostr(value);
}
...
template<>
bool StringConverter<NumberSequence>::convertToValue(const std::string& text, NumberSequence& value)
{
    if (text.empty())
        return false;
    value = fromstr(text);
...
template<>
bool StringConverter<NumberSequenceKeypoint>::convertToValue(const std::string& text, NumberSequenceKeypoint& value)
{
    if (text.empty())
        return false;
...
template<>
std::string StringConverter<NumberSequenceKeypoint>::convertToString(const NumberSequenceKeypoint& value)
{
    std::ostringstream oss;
