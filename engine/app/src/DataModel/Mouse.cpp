...
static Reflection::EventDesc<Mouse, void(std::string)> desc_Mouse_KeyDown(
    &Mouse::keyDownSignal, "KeyDown", "key", Reflection::Descriptor::Attributes::deprecated(UserInputService::event_InputBegin));
static Reflection::EventDesc<Mouse, void(std::string)> dep_Mouse_KeyDown(
    &Mouse::keyDownSignal, "keyDown", "key", Reflection::Descriptor::Attributes::deprecated(UserInputService::event_InputBegin));
static Reflection::EventDesc<Mouse, void(std::string)> desc_Mouse_KeyUp(
    &Mouse::keyUpSignal, "KeyUp", "key", Reflection::Descriptor::Attributes::deprecated(UserInputService::event_InputEnd));

static Reflection::PropDescriptor<Mouse, G3D::CoordinateFrame> desc_Hit("Hit", category_Data, &Mouse::getHit, NULL);
static Reflection::PropDescriptor<Mouse, G3D::CoordinateFrame> dep_Hit("hit", category_Data, &Mouse::getHit, NULL,
    Reflection::PropertyDescriptor::Attributes::deprecated(desc_Hit, Reflection::PropertyDescriptor::HIDDEN_SCRIPTING));
static Reflection::PropDescriptor<Mouse, G3D::CoordinateFrame> desc_Origin("Origin", category_Data, &Mouse::getOrigin, NULL);
static Reflection::PropDescriptor<Mouse, Aya::RbxRay> desc_UnitRay("UnitRay", category_Data, &Mouse::getUnitRay, NULL);
static Reflection::RefPropDescriptor<Mouse, Instance> desc_TargetFilter(
    "TargetFilter", category_Data, &Mouse::getTargetFilterUnsafe, &Mouse::setTargetFilter);
static Reflection::RefPropDescriptor<Mouse, PartInstance> desc_Target("Target", category_Data, &Mouse::getTarget, NULL);
static Reflection::RefPropDescriptor<Mouse, PartInstance> dep_Target(
    "target", category_Data, &Mouse::getTarget, NULL, Reflection::PropertyDescriptor::Attributes::deprecated(desc_Target));
...
Aya::RbxRay Mouse::getUnitRay() const
{
    checkActive();
...
PartInstance* Mouse::getTarget() const
{
    ...
    FilterDescendents filter(getTargetFilter());
    return (lastEvent && lastEvent->getUserInputState() != InputObject::INPUT_STATE_NONE) ? MouseCommand::getPart(getWorkspace(), lastEvent, &filter)
                                                                                          : NULL;
}
...
NormalId Mouse::getTargetSurface() const
{
    ...
    Surface surface = (lastEvent && lastEvent->getUserInputState() != InputObject::INPUT_STATE_NONE)
                          ? MouseCommand::getSurface(getWorkspace(), lastEvent, &filter)
                          : Surface();
...
int Mouse::getViewSizeX() const
{
    ...
    if (GuiService* guiService = Aya::ServiceProvider::find<GuiService>(workspace))
    {
