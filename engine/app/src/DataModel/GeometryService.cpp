...
const char* const Aya::sGeometryService = "Geometry";
...
static void addInstanceToIgnorePrimitiveSet(shared_ptr<Aya::Instance> instance, boost::unordered_set<const Primitive*>& primitiveSet)
{
...
void GeometryService::getPartsTouchingExtentsWithIgnore(
    const Extents& extents, const Instances* ancestors, int maxCount, G3D::Array<PartInstance*>& found)
{
    if (workspace)
    {
...
void GeometryService::getPartsTouchingExtents(const Extents& extents, const Primitive* ignore, int maxCount, G3D::Array<PartInstance*>& found)
{
    if (workspace)
    {
        AYAASSERT(found.size() == 0);
...
Vector3 GeometryService::getHitLocationPartFilterDescendents(IgnoreType* ancestor, Aya::RbxRay ray, shared_ptr<PartInstance>& result,
    Vector3& surfaceNormal, PartMaterial& surfaceMaterial, bool terrainCellsAreCubes, bool ignoreWaterCells)
{
    AYAPROFILER_SCOPE("Physics", "getHitLocationPartFilterDescendents");

    Primitive* hitPrim = NULL;
    Vector3 v3result =
        getHitLocationFilterDescendents(ancestor, ray, &hitPrim, surfaceNormal, surfaceMaterial, terrainCellsAreCubes, ignoreWaterCells);
    if (hitPrim)
    {
...
template Vector3 GeometryService::getHitLocationPartFilterDescendents<Instance>(
    Instance*, Aya::RbxRay, shared_ptr<PartInstance>&, Vector3&, PartMaterial&, bool, bool);
template Vector3 GeometryService::getHitLocationPartFilterDescendents<const Instances>(
    const Instances*, Aya::RbxRay, shared_ptr<PartInstance>&, Vector3&, PartMaterial&, bool, bool);

Vector3 GeometryService::getHitLocationFilterDescendents(Instance* ancestor, Aya::RbxRay ray, Primitive** hitPrim, Vector3& surfaceNormal,
    PartMaterial& surfaceMaterial, bool terrainCellsAreCubes, bool ignoreWaterCells)
{
    ...
    *hitPrim = contactManager.getHit(ray, NULL, &filter, hitPoint, terrainCellsAreCubes, ignoreWaterCells, surfaceNormal, surfaceMaterial);
...
Vector3 GeometryService::getHitLocationFilterDescendents(const Instances* ancestors, Aya::RbxRay ray, Primitive** hitPrim, Vector3& surfaceNormal,
    PartMaterial& surfaceMaterial, bool terrainCellsAreCubes, bool ignoreWaterCells)
{
    if (!workspace)
        return Vector3::zero();
...
Vector3 GeometryService::getHitLocationFilterStairs(Instance* ancestor, Aya::RbxRay ray, Primitive** hitPrim)
{
    if (!workspace)
        return Vector3::zero();
