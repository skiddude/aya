...
static Reflection::BoundFuncDesc<Teams, void()> teams_rebalanceTeamsFunction(
    &Teams::rebalanceTeams, "RebalanceTeams", Security::None, Reflection::Descriptor::Attributes::deprecated());
...
bool Teams::isTeamGame()
{
    ...
    if (Network::Players* players = ServiceProvider::find<Network::Players>(this))
    {
        for (unsigned int n = 0; n < players->numChildren(); n++)
        {
            ...
            if (p == NULL)
                continue; // only care about player objects
...
int Teams::getNumPlayersInTeam(BrickColor color)
{
    ...
    for (unsigned int n = 0; n < players->numChildren(); n++)
    {
        ...
        if (p == NULL)
            continue; // only care about player objects
        if (p->getNeutral() == false && p->getTeamColor() == color)
            result++;
    }
    return result;
}
