...
Reflection::PropDescriptor<Sky, float> Sky::prop_SunAngularSize(
    "SunAngularSize", category_Appearance, &Sky::getSunAngularSize, &Sky::setSunAngularSize);
Reflection::PropDescriptor<Sky, float> Sky::prop_MoonAngularSize(
    "MoonAngularSize", category_Appearance, &Sky::getMoonAngularSize, &Sky::setMoonAngularSize);
    ...
    , numStars(3000)
{
    ...
    std::string prefix = Aya::GameBasicSettings::singleton().getVirtualVersion() > Aya::GameBasicSettings::VERSION_2014 ? "classic_" : "";

    skyUp = ContentId::fromAssets(std::string("textures/sky/" + prefix + "sky512_up.tex").c_str());
    skyLf = ContentId::fromAssets(std::string("textures/sky/" + prefix + "sky512_lf.tex").c_str());
    skyRt = ContentId::fromAssets(std::string("textures/sky/" + prefix + "sky512_rt.tex").c_str());
    skyBk = ContentId::fromAssets(std::string("textures/sky/" + prefix + "sky512_bk.tex").c_str());
    skyFt = ContentId::fromAssets(std::string("textures/sky/" + prefix + "sky512_ft.tex").c_str());
    skyDn = ContentId::fromAssets(std::string("textures/sky/" + prefix + "sky512_dn.tex").c_str());
