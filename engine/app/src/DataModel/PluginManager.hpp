...
public:
    ...
    void setHost(IStudioPluginHost* host)
    {
        this->host = host;
    }
    void markDeleted();
    void processIconLoaded(ContentId id, AsyncHttpQueue::RequestResult result, std::istream* stream, shared_ptr<const std::string> response);

    void setActive(bool value);

    Aya::signal<void()> clickSignal;
};
...
public:
    ...
    void setPluginManager(PluginManager* pluginManager_)
    {
    ...
    Aya::signal<void()> deactivationSignal;

    void setAssetId(int assetId);

    void openScriptDoc(shared_ptr<Aya::Instance> script, int lineNumber);
...
private:
    ...
    struct StateDataEntry
    {
        ...
        void addPlugin(shared_ptr<Plugin> p)
        {
            plugins.push_back(p);
        }
        void setActivePlugin(Plugin* p)
        {
            active = p;
            p->setActive(true);
        }
...
public:
    ...
    boost::mutex mutex;
    Aya::signal<void()> allPluginsDeactivatedSignal;
    ...
    void setStudioHost(IStudioPluginHost* host)
    {
        studioHost = host;
        host->setNotifier(this);
    }
    IStudioPluginHost* getStudioHost()
    {
        return studioHost;
    }
    void setCurrentDataModel(DataModel* dataModel)
    {
    ...
    virtual void fireDragEnterEvent(DataModel* dataModel, shared_ptr<const Aya::Instances> instances, Vector2 location);
};
}; // namespace Aya
