...
class SkateboardPlatform
    : public DescribedCreatable<SkateboardPlatform, PlatformImpl<BasicPartInstance>, sSkateboardPlatform>
    , public KernelJoint
{
public:
    enum MoveState
    ...
    static bool isGroundState(MoveState state)
    {
        return state <= STOPPING;
    }
    static bool isAirState(MoveState state)
    {
        return state >= AIR_FREE;
    }
...
public:
    ...
    Aya::signal<void(MoveState, MoveState)> moveStateChangedSignal; // void(newState, oldState)

    // this is better:
    Aya::signal<void(shared_ptr<Instance>, shared_ptr<Instance>)> equippedSignal; // void(humanoid, controller)
    Aya::signal<void(shared_ptr<Instance>)> unequippedSignal;                     // void(humanoid)
    ...
    SkateboardController* getController() const
    {
        return myController.get();
    }
    Humanoid* getControllingHumanoid() const
    {
        return humanoid;
    }
