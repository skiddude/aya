...
static Reflection::BoundFuncDesc<KeyframeSequenceProvider, ContentId(shared_ptr<Instance>)> func_setActiveKeyframeSequence(
    &KeyframeSequenceProvider::registerActiveKeyframeSequence, "RegisterActiveKeyframeSequence", "keyframeSequence", Security::None);
static Reflection::BoundFuncDesc<KeyframeSequenceProvider, ContentId(shared_ptr<Instance>)> func_setKeyframeSequence(
    &KeyframeSequenceProvider::registerKeyframeSequence, "RegisterKeyframeSequence", "keyframeSequence", Security::None);
static Reflection::BoundFuncDesc<KeyframeSequenceProvider, shared_ptr<Instance>(ContentId)> func_getKeyframeSequence(
    &KeyframeSequenceProvider::getKeyframeSequenceLua, "GetKeyframeSequence", "assetId", Security::None);
static Reflection::BoundFuncDesc<KeyframeSequenceProvider, shared_ptr<Instance>(int, bool)> func_getKeyframeSequenceById(
    &KeyframeSequenceProvider::getKeyframeSequenceByIdLua, "GetKeyframeSequenceById", "assetId", "useCache", Security::None);

static const Reflection::BoundYieldFuncDesc<KeyframeSequenceProvider, shared_ptr<const Reflection::ValueTable>(int, int)> func_getAnimations(
    &KeyframeSequenceProvider::getAnimations, "GetAnimations", "userId", "page", 1, Security::None);
REFLECTION_END();
...
{
    this->setName("KeyframeSequenceProvider");
}
ContentId KeyframeSequenceProvider::registerActiveKeyframeSequence(shared_ptr<Instance> instance)
{
    ...
    {
        throw Aya::runtime_error("Usage error: RegisterActiveKeyframeSequence can only be used in Solo mode.");
    }

    if (shared_ptr<KeyframeSequence> keyframeSequence = Instance::fastSharedDynamicCast<KeyframeSequence>(instance))
    {
        std::string id = Aya::format("active://%d", activeKeyframeId++);
...
shared_ptr<Instance> KeyframeSequenceProvider::getKeyframeSequenceByIdLua(int assetId, bool useCache)
{
    if (assetId <= 0)
    {
        throw Aya::runtime_error("GetKeyframeSequenceById assetId is not a positive number!");
    }

    std::string parameters = Aya::format("asset/?id=%d", assetId);
    std::string url = ServiceProvider::create<ContentProvider>(this)->getBaseUrl() + parameters;

    shared_ptr<KeyframeSequence> result =
        privateGetKeyframeSequence(ContentId::fromUrl(url), true, false, "KeyframeSequenceProvider:GetKeyframeSequenceById()");
    if (result)
    {
...
{
    AYAPROFILER_SCOPE("Animation", "copyKeyframeSequence");
...
static void copyLoadedData(const Instances instances, boost::weak_ptr<KeyframeSequenceProvider> keyframeSequenceProvider,
    boost::weak_ptr<KeyframeSequence> keyframeSequence, bool synchronous)
{
    if (instances.size() == 1)
    {
...
static void loadDataFromInsertService(boost::shared_ptr<Instance> ist, boost::weak_ptr<KeyframeSequenceProvider> keyframeSequenceProvider,
    boost::weak_ptr<KeyframeSequence> keyframeSequence, bool synchronous, const std::string& context)
{
    ...
    {
        Aya::StandardOut::singleton()->printf(MESSAGE_ERROR, "Animation failed to load : %s", context.c_str());
    }
...
static void KeyframeLoaderHelper(AsyncHttpQueue::RequestResult result, std::istream* stream,
    boost::weak_ptr<KeyframeSequenceProvider> keyframeSequenceProvider, boost::weak_ptr<KeyframeSequence> keyframeSequence, bool synchronous,
...
shared_ptr<KeyframeSequence> KeyframeSequenceProvider::privateGetKeyframeSequence(
    ContentId assetId, bool blocking, bool useCache, const std::string& contextString, const Instance* contextInstance)
{
    ...
    if (animationId.isActive())
    {
        ...
        throw std::runtime_error(Aya::format("Unknown 'active' animation: '%s'", animationId.c_str()));
    }
    ...
    if (!DFFlag::DontUseInsertServiceOnAnimLoad && !blocking && (assetId.isHttp() || assetId.isAsset() || assetId.isItem()))
    {
        AYAASSERT(!blocking);

        ServiceProvider::create<InsertService>(this)->loadPrivateAsset(assetId.toString(),
    ...
    else
    {
        ...
        {
            ServiceProvider::create<ContentProvider>(this)->getContent(assetId, ContentProvider::PRIORITY_ANIMATION,
                boost::bind(&AsyncKeyframeLoaderHelper, _1, _2, weak_from(this), boost::weak_ptr<KeyframeSequence>(keyframeSequence), newContext));
        }
    }
...
void KeyframeSequenceProvider::JSONHttpHelper(const std::string* response, const std::exception* exception,
    boost::function<void(shared_ptr<const Reflection::ValueTable>)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    ...
    if (response && !exception)
    {
        ...
        {
            errorFunction(Aya::format("App:JSONHttpHelper error parsing web response: %s", e.what()));
            return;
        }
        ...
        errorFunction(Aya::format("App:JSONHttpHelper had an issue because %s", exception->what()));
    else
        resumeFunction(shared_ptr<Reflection::ValueTable>());
}

void KeyframeSequenceProvider::getAnimations(int userId, int page, boost::function<void(shared_ptr<const Reflection::ValueTable>)> resumeFunction,
    boost::function<void(std::string)> errorFunction)
{
    ...
    std::string parameters = Aya::format("ownership/assets?page=%d&userID=%d&assetTypeId=24", page, userId);

    Aya::Http getAnimationsHttp((ServiceProvider::create<ContentProvider>(this)->getApiBaseUrl() + parameters).c_str());
#ifndef _WIN32
