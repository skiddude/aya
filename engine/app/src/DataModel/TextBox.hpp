...
private:
    ...
    void keyUp(RepeatKeyState::KeyType keyType, Aya::KeyCode keyCode, char key);
    void keyDown(RepeatKeyState::KeyType keyType, Aya::KeyCode keyCode, char key);
    void textInput(RepeatKeyState::KeyType keyType, std::string textInput);

    int getCursorPos(Aya::Vector2 mousePos);
    ...
    Aya::signals::scoped_connection textBoxFinishedEditingConnection;
...
public:
    ...
    bool getClearTextOnFocus() const
    {
        return clearTextOnFocus;
    }
    void setClearTextOnFocus(bool value);

    Aya::signal<void(bool, const shared_ptr<Instance>)> focusLostSignal;
    Aya::signal<void()> focusGainedSignal;
