...
bool PlayerScripts::askAddChild(const Instance* instance) const
{
    ...
    return Instance::fastDynamicCast<Aya::LocalScript>(instance) != NULL;
}
...
static void copyChildrenToPlayerScripts(Instance* scripts, Instance* playerScripts)
{
    for (size_t n = 0; n < scripts->numChildren(); n++)
    {
        ...
        c->setIsArchivable(previousArchivableValue); // Set archivable back to the previous value, default players scripts shouldn't be archivable but
                                                     // developer made scripts should
    }
}
...
void PlayerScripts::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
    ...
    if (newProvider && Aya::Network::Players::frontendProcessing(newProvider))
    {
        // Check to see if we need to load the default scripts into this object - Client side from StarterPlayerService
        Aya::Network::Player* player = findFirstAncestorOfType<Aya::Network::Player>();
        ...
        if (player && player == localPlayer)
        {
            ...
            {
                Aya::StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "PlayerScripts %s didn't find StarterPlayerService", getName().c_str());
            }
        }
    }
}
...
static Reflection::RemoteEventDesc<StarterPlayerScripts, void(shared_ptr<Instance>)> event_requestDefaultScripts(
    &StarterPlayerScripts::requestDefaultScriptsSignal, "RequestDefaultScripts", "root", Security::None,
...
static Reflection::RemoteEventDesc<StarterPlayerScripts, void(int)> event_confirmDefaultScripts(&StarterPlayerScripts::confirmDefaultScriptsSignal,
    "ConfirmDefaultScripts", "root", Security::None, Reflection::RemoteEventCommon::REPLICATE_ONLY, Reflection::RemoteEventCommon::CLIENT_SERVER);
REFLECTION_END();
...
{
    return Instance::fastDynamicCast<Aya::StarterPlayerService>(parent) != NULL;
}
...
static void PlayerScriptsLoadHelper(
    weak_ptr<StarterPlayerScripts> playerScripts, AsyncHttpQueue::RequestResult result, shared_ptr<Instances> instances)
{
...
void StarterPlayerScripts::InitializeDefaultScripts()
{
    ...
    if (!controlScript)
    {
        ServiceProvider::create<ContentProvider>(this)->loadContent(ContentId::fromAssets("avatar/scripts/characterControlScript.rbxmx"),
    ...
    if (!cameraScript)
    {
        ServiceProvider::create<ContentProvider>(this)->loadContent(ContentId::fromAssets("avatar/scripts/characterCameraScript.rbxmx"),
            ContentProvider::PRIORITY_SCRIPT, boost::bind(&PlayerScriptsLoadHelper, weak_from(this), _1, _2), AsyncHttpQueue::AsyncWrite);
    }
...
{
    Aya::Network::Player* pPlayer = Instance::fastDynamicCast<Aya::Network::Player>(player.get());
    if (pPlayer)
    {
...
void StarterPlayerScripts::defaultScriptsSend(weak_ptr<Aya::Network::Player> p)
{

    shared_ptr<Aya::Network::Player> player = p.lock();
    if (player)
    {
        Aya::Network::Player* pPlayer = player.get();
        if (pPlayer)
        {

            if (Network::Players::serverIsPresent(this)) // check for play solo
            {
                const Aya::SystemAddress& target = pPlayer->getRemoteAddressAsRbxAddress();
...
void StarterPlayerScripts::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
    ...
    if (newProvider && Aya::Network::Players::backendProcessing(newProvider))
    {
        if (RunService* runService = ServiceProvider::create<RunService>(newProvider))
        {
            ...
            {
                initializeDefaultScriptsConnection =
                    runService->runTransitionSignal.connect(boost::bind(&StarterPlayerScripts::InitializeDefaultScriptsRunService, this, _1));
            }
        }
...
{
    DescribedCreatable<StarterPlayerScripts, Instance, sStarterPlayerScripts, Reflection::ClassDescriptor::PERSISTENT>::onServiceProvider(
