...
#include "DataModel/HttpRbxApiJob.hpp"
#include "Players.hpp"
...
static Reflection::BoundYieldFuncDesc<HttpRbxApiService, std::string(std::string, bool, ThrottlingPriority)> apiGetAsyncFunction(
    &HttpRbxApiService::getAsyncLua, "GetAsync", "apiUrlPath", "useHttps", true, "priority", PRIORITY_DEFAULT, Security::RobloxScript);
static Reflection::BoundYieldFuncDesc<HttpRbxApiService,
    std::string(std::string, std::string, bool, ThrottlingPriority, HttpService::HttpContentType)>
    apiPostAsyncFunction(&HttpRbxApiService::postAsyncLua, "PostAsync", "apiUrlPath", "data", "useHttps", true, "priority", PRIORITY_DEFAULT,
        "content_type", HttpService::APPLICATION_JSON, Security::RobloxScript);
REFLECTION_END();
...
void HttpRbxApiService::setStaticApiBaseUrl(const Aya::Reflection::PropertyDescriptor* pPropertyDescriptor)
{
    if (*pPropertyDescriptor == Aya::ContentProvider::desc_baseUrl)
    {
...
void HttpRbxApiService::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
    ...
    if (newProvider)
    {
        ...
        {
            contentProviderPropertyChangedConnection =
                ServiceProvider::find<ContentProvider>(newProvider)
                    ->propertyChangedSignal.connect(boost::bind(&HttpRbxApiService::setStaticApiBaseUrl, this, _1));
        }
...
void HttpRbxApiService::playersPropertyChanged(const Aya::Reflection::PropertyDescriptor* desc)
{
...
static bool shouldRetryRequest(std::exception* exception, HttpRbxApiService::HttpApiRequest& request)
{
    ...
    {
        long statusCode = -1;
        if (Aya::http_status_error* httpError = dynamic_cast<Aya::http_status_error*>(exception))
        {
...
void httpHelperRetryLockAcquired(
    shared_ptr<HttpRbxApiService> apiService, HttpRbxApiService::HttpApiRequest request, boost::function<void(std::string)> errorFunction)
{
...
void httpHelperExecuteLockAcquired(shared_ptr<HttpRbxApiService> apiService, shared_ptr<std::string> response, shared_ptr<std::exception> exception,
    HttpRbxApiService::HttpApiRequest request, boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    if (response)
    {
...
void HttpRbxApiService::httpHelper(weak_ptr<HttpRbxApiService> weakApiService, std::string* response, std::exception* exception,
    HttpApiRequest request, ThrottlingPriority throttlePriority, boost::function<void(std::string)> resumeFunction,
    boost::function<void(std::string)> errorFunction)
{
    if (shared_ptr<HttpRbxApiService> apiService = weakApiService.lock())
    {
        ...
        {
            DataModel::processHttpRequestResponseOnLock(Aya::DataModel::get(apiService.get()), response, exception,
                boost::bind(&httpHelperRetryLockAcquired, apiService, request, errorFunction));
        }
        else
        {
            DataModel::processHttpRequestResponseOnLock(Aya::DataModel::get(apiService.get()), response, exception,
                boost::bind(&httpHelperExecuteLockAcquired, apiService, _1, /* shared_ptr<std::string> response, */
                    _2,                                                     /* shared_ptr<std::exception> exception, */
                    request, resumeFunction, errorFunction));
        }
    }
}
...
void HttpRbxApiService::postAsync(std::string urlPath, std::string data, bool useHttps, ThrottlingPriority throttlePriority,
    HttpService::HttpContentType content, boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
...
void HttpRbxApiService::postAsyncWithAdditionalHeaders(std::string urlPath, std::string data, bool useHttps, ThrottlingPriority throttlePriority,
...
void HttpRbxApiService::postAsyncLua(std::string urlPath, std::string data, bool useHttps, ThrottlingPriority throttlePriority,
    HttpService::HttpContentType content, boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    std::string fullUrl = "https" + apiBaseUrl + urlPath;
    checkAndUpdatePostUrl(fullUrl, urlPath);
    ...
    Http http(fullUrl);
    robloxScriptModifiedCheck(Aya::apiPostAsyncFunction.security);
    postAsyncInternal(http, data, content, data.size() > HTTP_POST_COMPRESSION_LIMIT, throttlePriority, resumeFunction, errorFunction);
}

void HttpRbxApiService::postAsyncInternal(Http& httpRequest, std::string& data, const HttpService::HttpContentType& contentType,
    const bool shouldCompress, const ThrottlingPriority& throttlePriority, boost::function<void(std::string)> resumeFunction,
    boost::function<void(std::string)> errorFunction)
{
...
void HttpRbxApiService::getAsync(Http& httpRequest, ThrottlingPriority throttlePriority, boost::function<void(std::string)> resumeFunction,
    boost::function<void(std::string)> errorFunction)
{
    if (!isAPIHttpRequest(httpRequest))
    {
...
void HttpRbxApiService::getAsync(std::string urlPath, bool useHttps, ThrottlingPriority throttlePriority,
    boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    std::string fullUrl = "https" + apiBaseUrl + urlPath;
...
void HttpRbxApiService::getAsyncLua(std::string urlPath, bool useHttps, ThrottlingPriority throttlePriority,
    boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    std::string fullUrl = "https" + apiBaseUrl + urlPath;
    ...
    Http http(fullUrl);
    robloxScriptModifiedCheck(Aya::apiGetAsyncFunction.security);
    getAsyncInternal(http, throttlePriority, resumeFunction, errorFunction);
}

void HttpRbxApiService::getAsyncInternal(Http& httpRequest, const ThrottlingPriority& throttlePriority,
    boost::function<void(std::string)> resumeFunction, boost::function<void(std::string)> errorFunction)
{
    HttpApiRequest apiRequest;
    apiRequest.throttlingPriority = throttlePriority;
...
void HttpRbxApiService::addThrottlingBudgets(float timeDeltaMinutes)
{
    ...
    {
        const int serverMaxBudget =
            (DFInt::HttpRbxApiRequestsPerMinutePerPlayerInServerLimit * getPlayerNum()) + DFInt::HttpRbxApiRequestsPerMinuteServerLimit;
        ...
        FASTLOG3F(FLog::HttpRbxApiBudget, "Adding budget %f, default budget is at: %f, elevated budget is at: %f", addedServerBudget,
            defaultServerThrottle.getBudget(), elevatedServerThrottle.getBudget());
    }
    ...
    if (Network::Players::clientIsPresent(this) || isPlaySolo)
    {
        ...
        FASTLOG2F(FLog::HttpRbxApiBudget, "Adding budget %f, client budget is at: %f", addedClientBudget, clientThrottle.getBudget());
    }
...
void HttpRbxApiService::HttpApiRequest::execute(HttpRbxApiService* apiService)
{
    if (isPost)
    {
        http.post(postData, httpContentType, (postData.size() > HTTP_POST_COMPRESSION_LIMIT),
            boost::bind(&HttpRbxApiService::httpHelper, weak_from(apiService), _1, _2, *this, throttlingPriority, resumeFunction, errorFunction),
            false);
    }
                ...
                if (shouldRetryRequest(&e, *this))
                {
                    if (!retrySyncRequest(http, syncResponse))
                    {
                        throw std::runtime_error(
                            format("Could not get a valid response for %s after retrying synchronous get.", http.url.c_str()).c_str());
                    }
                }
                else
                {
...
bool HttpRbxApiService::tryThrottleRequest(const HttpApiRequest& apiRequest, BudgetedThrottlingHelper& budgetThrottler,
    ...
    if (!budgetThrottler.checkAndReduceBudget())
    {
        ...
        else
        {
            std::string throttleError =
                Aya::format("number of API requests/minute exceeded limit for HTTP API throttle. Please don't issue more than %i API requests/minute "
                            "with server scripts and no more than %i API requests/minute with local scripts.",
                    DFInt::HttpRbxApiRequestsPerMinuteServerLimit + (DFInt::HttpRbxApiRequestsPerMinutePerPlayerInServerLimit * getPlayerNum()),
                    DFInt::HttpRbxApiClientPerMinuteRequestLimit);
...
bool HttpRbxApiService::executeApiRequest(
    HttpApiRequest& apiRequest, const ThrottlingPriority& throttlePriority, boost::function<void(std::string)> errorFunction)
{
    ...
    {
        if (isPlaySolo && tryThrottleRequest(apiRequest, defaultServerThrottle, throttledDefaultServerRequests, errorFunction))
        {
            return false;
        }
        ...
        else if ((Network::Players::clientIsPresent(this)) && tryThrottleRequest(apiRequest, clientThrottle, throttledClientRequests, errorFunction))
        {
            return false;
        }
    ...
    {
        if (isPlaySolo && tryThrottleRequest(apiRequest, elevatedServerThrottle, throttledElevatedServerRequests, errorFunction))
        {
            return false;
        }
        else if (Network::Players::serverIsPresent(this) &&
