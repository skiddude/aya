...
protected:
    void setPartInstance(weak_ptr<PartInstance>& data, PartInstance* newValue, const PartProp& prop);
    static void computeSurfacePosition(const shared_ptr<PartInstance>& part, const RbxRay& ray, Vector3* out);
    bool incrementalBuildSegments(
        const Workspace* workspace, const ContactManager* contactManager, const Vector3& dest, bool moveInX, std::vector<Vector3>* out);
    void buildTrailSegments(
        const Workspace* workspace, const shared_ptr<PartInstance>& from, const shared_ptr<PartInstance>& to, std::vector<Vector3>* out);
    void drawSegments(const Workspace* workspace, const Camera* camera, const std::vector<Vector3>& segments, Adorn* adorn);
