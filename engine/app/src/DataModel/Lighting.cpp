...
static Reflection::PropDescriptor<Lighting, float> prop_GeographicLatitude(
    "GeographicLatitude", category_Data, &Lighting::getGeographicLatitude, &Lighting::setGeographicLatitude);
...
static Reflection::BoundFuncDesc<Lighting, double()> func_GetMinutesAfterMidnight(
    &Lighting::getMinutesAfterMidnight, "GetMinutesAfterMidnight", Security::None);
static Reflection::BoundFuncDesc<Lighting, void(double)> func_SetMinutesAfterMidnight(
    &Lighting::setMinutesAfterMidnight, "SetMinutesAfterMidnight", "minutes", Security::None);
static Reflection::BoundFuncDesc<Lighting, double()> func_dep_GetMinutesAfterMidnight(&Lighting::getMinutesAfterMidnight, "getMinutesAfterMidnight",
    Security::None, Reflection::Descriptor::Attributes::deprecated(func_GetMinutesAfterMidnight));
static Reflection::BoundFuncDesc<Lighting, void(double)> func_dep_SetMinutesAfterMidnight(&Lighting::setMinutesAfterMidnight,
    "setMinutesAfterMidnight", "minutes", Security::None, Reflection::Descriptor::Attributes::deprecated(func_SetMinutesAfterMidnight));

static Reflection::PropDescriptor<Lighting, G3D::Color3> desc_ShadowColor(
    "ShadowColor", category_Appearance, &Lighting::getShadowColor, &Lighting::setShadowColor);
...
static Reflection::PropDescriptor<Lighting, bool> desc_GlobalShadows(
    "GlobalShadows", category_Appearance, &Lighting::getGlobalShadows, &Lighting::setGlobalShadows);

Reflection::BoundProp<float> Lighting::desc_GlobalBrightness(
    "Brightness", category_Appearance, &Lighting::globalBrightness, &Lighting::onPropChanged);
Reflection::BoundProp<G3D::Color3> Lighting::desc_TopColorShift(
    "ColorShift_Top", category_Appearance, &Lighting::topColorShift, &Lighting::onPropChanged);
Reflection::BoundProp<G3D::Color3> Lighting::desc_BottomColorShift(
    "ColorShift_Bottom", category_Appearance, &Lighting::bottomColorShift, &Lighting::onPropChanged);
...
Reflection::BoundProp<G3D::Color3> Lighting::desc_GlobalOutdoorAmbient(
    "OutdoorAmbient", category_Appearance, &Lighting::globalOutdoorAmbient, &Lighting::onPropChanged);

Reflection::BoundProp<bool> Lighting::desc_Outlines("Outlines", category_Appearance, &Lighting::outlines, &Lighting::onPropChanged);
REFLECTION_END();

const char* const Aya::sLighting = "Lighting";
...
void Lighting::onChildRemoving(Instance* child)
{
    ...
    else if (blurEffect.get() == child)
    {
        blurEffect.reset();
        fireLightingChanged(true);
    }
    else if (colorCorrectionEffect.get() == child)
    {
        colorCorrectionEffect.reset();
        fireLightingChanged(true);
    }
    else if (bloomEffect.get() == child)
    {
        bloomEffect.reset();
        fireLightingChanged(true);
    }
...
void Lighting::onChildAdded(Instance* child)
{
    ...
    else if (PostEffect* effect = Instance::fastDynamicCast<PostEffect>(child))
    {
        /*
        if (BlurEffect* blur = Instance::fastDynamicCast<BlurEffect>(effect))
        {
            this->blurEffect = shared_from(blur);
        }
        else if (ColorCorrectionEffect* colorCorrection = Instance::fastDynamicCast<ColorCorrectionEffect>(effect))
        {
            this->colorCorrectionEffect = shared_from(colorCorrection);
        }
        else if (BloomEffect* bloom = Instance::fastDynamicCast<BloomEffect>(effect))
        {
            this->bloomEffect = shared_from(bloom);
        }
 */
        fireLightingChanged(true);
    }
...
void Lighting::onChildChanged(Instance* instance, const PropertyChanged& event)
{
    ...
    if (sky.get() == instance || blurEffect.get() == instance || colorCorrectionEffect.get() == instance || bloomEffect.get() == instance)
        fireLightingChanged(true);
}
