...
#include "World/Motor6DJoint.hpp"
...
const RefPropDescriptor<JointInstance, PartInstance> JointInstance::prop_Part0(
    "Part0", category_Data, &JointInstance::getPart0Dangerous, &JointInstance::setPart0);
const RefPropDescriptor<JointInstance, PartInstance> JointInstance::prop_Part1(
    "Part1", category_Data, &JointInstance::getPart1Dangerous, &JointInstance::setPart1);
static const RefPropDescriptor<JointInstance, PartInstance> dep_Part1("part1", category_Data, &JointInstance::getPart1Dangerous,
    &JointInstance::setPart1,
    Reflection::PropertyDescriptor::Attributes::deprecated(JointInstance::prop_Part1, Reflection::PropertyDescriptor::HIDDEN_SCRIPTING));

static const PropDescriptor<ManualSurfaceJointInstance, int> prop_Surface0("Surface0", category_Data, &ManualSurfaceJointInstance::getSurface0,
    &ManualSurfaceJointInstance::setSurface0, Reflection::PropertyDescriptor::STREAMING);
static const PropDescriptor<ManualSurfaceJointInstance, int> prop_Surface1("Surface1", category_Data, &ManualSurfaceJointInstance::getSurface1,
    &ManualSurfaceJointInstance::setSurface1, Reflection::PropertyDescriptor::STREAMING);
...
static const PropDescriptor<DynamicRotate, float /**/> prop_BaseAngle(
    "BaseAngle", category_Data, &DynamicRotate::getBaseAngle, &DynamicRotate::setBaseAngle);
...
static BoundFuncDesc<Motor, void(float)> func_SetDesiredAngle(
    &Motor::setDesiredAngleUi, "SetDesiredAngle", "value", Security::None); // Non replicating local function
static const PropDescriptor<Motor, float /**/> prop_CurrentAngle(
    "CurrentAngle", category_Data, &Motor::getCurrentAngle, &Motor::setCurrentAngleUi, PropertyDescriptor::UI);
REFLECTION_END();


const char* const Aya::sJointInstance = "JointInstance";
const char* const Aya::sSnap = "Snap";
const char* const Aya::sWeld = "Weld";
const char* const Aya::sManualSurfaceJointInstance = "ManualSurfaceJointInstance";
const char* const Aya::sManualWeld = "ManualWeld";
const char* const Aya::sManualGlue = "ManualGlue";
const char* const Aya::sGlue = "Glue";
const char* const Aya::sRotate = "Rotate";
const char* const Aya::sDynamicRotate = "DynamicRotate";
const char* const Aya::sRotateP = "RotateP";
const char* const Aya::sRotateV = "RotateV";
const char* const Aya::sMotor = "Motor";
const char* const Aya::sMotor6D = "Motor6D";
...
JointInstance::~JointInstance()
{
    ...
    AYAASSERT(getParent() == NULL);
    AYAASSERT(joint->findWorld() == NULL);
...
XmlElement* JointInstance::writeXml(const boost::function<bool(Instance*)>& isInScope, Aya::CreatorRole creatorRole)
{
...
{
    return (instance->getDescriptor() == JointsService::classDescriptor() || instance->getDescriptor() == PartInstance::classDescriptor());
}
...
bool JointInstance::inEngine()
{
    ...
    AYAASSERT(answer == (computeWorld() != NULL));
    return answer;
}
...
void JointInstance::handleWorldChanged()
{
    ...
    if (oldWorld != newWorld)
    {
        ...
        if (oldWorld)
        {
            ...
            AYAASSERT_SLOW(!joint->findWorld());
        }
        if (newWorld)
        {
    ...
    : DescribedCreatable<Snap, JointInstance, sSnap>(joint)
{
    ...
    AYAASSERT(joint->getJointType() == Joint::SNAP_JOINT);
    // AYAASSERT(joint->getJointCoord(0) == CoordinateFrame());
    AYAASSERT(Math::isAxisAligned(joint->getJointCoord(0).rotation));
}
    ...
    : DescribedCreatable<Weld, JointInstance, sWeld>(joint)
{
    ...
    AYAASSERT(joint->getJointType() == Joint::WELD_JOINT);
    //	AYAASSERT(joint->getJointCoord(0) == CoordinateFrame());
    AYAASSERT(Math::isAxisAligned(joint->getJointCoord(0).rotation));
}
...
{
    return aya_static_cast<const WeldJoint*>(joint);
}
    ...
    : DescribedCreatable<ManualWeld, ManualSurfaceJointInstance, sManualWeld>(joint)
{
    ...
    AYAASSERT(joint->getJointType() == Joint::MANUAL_WELD_JOINT);
    //	AYAASSERT(joint->getJointCoord(0) == CoordinateFrame());
    AYAASSERT(Math::isAxisAligned(joint->getJointCoord(0).rotation));
}
...
{
    return aya_static_cast<const ManualWeldJoint*>(joint);
}
    ...
    : DescribedCreatable<ManualGlue, ManualSurfaceJointInstance, sManualGlue>(joint)
{
    ...
    AYAASSERT(joint->getJointType() == Joint::MANUAL_GLUE_JOINT);
}
...
{
    return aya_static_cast<const ManualGlueJoint*>(joint);
}
...
{
    AYAASSERT(joint->getJointType() == Joint::GLUE_JOINT);
}
...
{
    return aya_static_cast<GlueJoint*>(joint);
}

const GlueJoint* Glue::glueJointConst() const
{
    return aya_static_cast<const GlueJoint*>(joint);
}
...
\
    ...
    { \
        glueJoint()->setFacePoint(n, value); \
        raiseChanged(prop_F##n); \
    }
...
{
    AYAASSERT(joint->getJointType() == Joint::ROTATE_JOINT);
}
...
{
    const DynamicRotateJoint* dynamicRotateJoint = aya_static_cast<const DynamicRotateJoint*>(joint);
    return dynamicRotateJoint->getBaseAngle();
}

void DynamicRotate::setBaseAngle(float value)
{
    DynamicRotateJoint* dynamicRotateJoint = aya_static_cast<DynamicRotateJoint*>(joint);
...
{
    AYAASSERT(joint->getJointType() == Joint::ROTATE_P_JOINT);
}
...
{
    AYAASSERT(joint->getJointType() == Joint::ROTATE_V_JOINT);
}
...
{
    AYAASSERT(joint->getJointType() == Joint::MOTOR_1D_JOINT);
    AYAASSERT(0); // creating a motor by AutoJoin
    this->setName("Motor");
}
...
{
    return aya_static_cast<MotorJoint*>(joint);
}
...
{
    AYAASSERT(joint->getJointType() == Joint::MOTOR_6D_JOINT);
    AYAASSERT(0); // creating a motor by AutoJoin
...
{
    return aya_static_cast<Motor6DJoint*>(joint);
}
...
void Motor6D::setMaxVelocity(float value)
{
    ...
    {
        getMotorJoint()->maxZAngleVelocity = (value * (30.0f / 60.0f));
        raiseChanged(prop_MaxVelocity);
    }
}
