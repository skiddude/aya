...
#include "DataModel/MegaCluster.hpp" // for terrain damage
...
Reflection::PropDescriptor<Explosion, bool> Explosion::prop_Visible("Visible", category_Appearance, &Explosion::getVisible, &Explosion::setVisible);

static const Reflection::PropDescriptor<Explosion, float> propBlastRadius(
    "BlastRadius", category_Data, &Explosion::getBlastRadius, &Explosion::setBlastRadius);
static const Reflection::PropDescriptor<Explosion, float> propDestroyJoints(
    "DestroyJointRadiusPercent", category_Data, &Explosion::getDestroyJoints, &Explosion::setDestroyJoints);
...
static Reflection::EnumPropDescriptor<Explosion, Explosion::ExplosionType> prop_ExplosionType(
    "ExplosionType", category_Data, &Explosion::getExplosionType, &Explosion::setExplosionType);
...
float Explosion::visualRadius() const
{
    return blastRadius;
}

float Explosion::killRadius() const
{
    return 0.0f;
}

float Explosion::blastMaxObjectRadius() const
{
    return blastRadius * 2.0f;
}

float Explosion::killMaxObjectRadius() const
{
    return blastRadius;
}
                        ...
                        for (int i = blastRegionMin.x; i <= blastRegionMax.x; ++i)
                        {
                            for (int j = blastRegionMin.y; j <= blastRegionMax.y; ++j)
                            {
                                for (int k = blastRegionMin.z; k <= blastRegionMax.z; ++k)
                                {
                                    ...
                                    Vector3 cellCenterToExplosion = Voxel::cellToWorld_smallestCorner(cellLocation) +
                                                                    (Vector3(Voxel::kCELL_SIZE, Voxel::kCELL_SIZE, Voxel::kCELL_SIZE) / 2) - position;
                                    if (cellCenterToExplosion.dot(cellCenterToExplosion) < blastRadius * blastRadius)
                                    {
                                        terrain->getVoxelGrid()->setCell(
                                            cellLocation, Voxel::Constants::kUniqueEmptyCellRepresentation, Voxel::CELL_MATERIAL_Water);
                                    }
                                }
                            }
                        }
...
void Explosion::doKill()
{
    if (this->killRadius() > 0.0f)
    {
        ...
        {
            G3D::Array<Primitive*> primitives;
            world->getContactManager()->getPrimitivesTouchingExtents(Extents::fromCenterRadius(position, killRadius()), NULL, 0, primitives);
