...
public:
    ...
    // Should be private
    Aya::remote_signal<void(int, int, FriendEventType)> friendEventReplicatingSignal;
    Aya::remote_signal<void(int, int, FriendStatus)> friendStatusReplicatingSignal;
...
private:
    ...
    Aya::signals::scoped_connection friendStatusConnection;

    void friendEventReplicationChanged(int playerId, int otherPlayerId, FriendEventType status);
    Aya::signals::scoped_connection friendEventConnection;
    ...
    static void ProcessBulkFriendResponse(
