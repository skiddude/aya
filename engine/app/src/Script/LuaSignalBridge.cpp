...
int Bridge<EventInstance>::on_index(const EventInstance& object, const char* name, lua_State* L)
{
    ...
    {
        Aya::Security::Context::current().requirePermission(Aya::Security::LocalUser, "connectFirst");
    ...
    {
        Aya::Security::Context::current().requirePermission(Aya::Security::LocalUser, "connectLast");
    ...
    if (strcmp(name, "disconnect") == 0)
        throw Aya::runtime_error("Event:disconnect() has been deprecated. Use connection object returned by connect()");
...
class FunctionScriptSlot : public BaseScript::Slot
{
...
public:
    ...
    void operator()(const Reflection::EventArguments& arguments)
    {
        if (useSubmitTask)
        {
            ...
            {
                dm->submitTask(boost::bind(&FunctionScriptSlot::safeDoEventFire, weakSelf, arguments /*copy arguments*/), DataModelJob::Write);
            }
        }
        else
        {
    ...
    void doEventFire(const Reflection::EventArguments& arguments)
    {
        ...
        Aya::ScopedAssign<int> assign(executionDepth, executionDepth + 1);
        ...
        if (executionDepth > 6)
        {
            ...
            StandardOut::singleton()->printf(
                MESSAGE_ERROR, "Maximum event re-entrancy depth exceeded for %s.%s", descriptor->owner.name.c_str(), descriptor->name.c_str());
            ...
            if (ThreadRef functionThread = function.lock())
            {
                ...
                {
                    StandardOut::singleton()->printf(
                        MESSAGE_ERROR, "While entering function defined in script '%s', line %d", ar.short_src, ar.linedefined);
                }
            }
        ...
        if (ThreadRef functionThread = function.lock())
        {
            ...
            {
                AYAASSERT_BALLANCED_LUA_STACK(functionThread); // oldTop
                ...
                else
                {
                    ...
                    AYAASSERT(lua_isthread(functionThread, -1)); // slotThread
                ...
                if (function.empty())
                {
                    ...
                    AYAASSERT(cachedSlotThread.empty());
                    disconnect();
                    return;
                }
...
class WaitScriptSlot : public BaseScript::Slot
{
...
public:
    ...
    void operator()(const Reflection::EventArguments& arguments)
    {
        ...
        if (useSubmitTask)
        {
            if (shared_ptr<DataModel> dm = weakDm.lock())
            {
                dm->submitTask(boost::bind(&WaitScriptSlot::safeDoEventFire, weakSelf, arguments /*copy arguments*/), DataModelJob::Write);
            }
        }
        else
        {
...
int EventBridge::connect(lua_State* L)
{
    ...
    {
        if (ei.descriptor->security != Aya::Security::None)
            Aya::Security::Context::current().requirePermission(ei.descriptor->security, ei.descriptor->name.c_str());

        Reflection::Variant varResult;
        Lua::LuaArguments::get(L, 2, varResult, false);
        if (!varResult.isType<Lua::WeakFunctionRef>())
        {
            Aya::StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "Attempt to connect failed: Passed value is not a function");
        ...
        shared_ptr<Aya::Reflection::TGenericSlotWrapper<FunctionScriptSlot>> wrapper(
            Aya::make_shared<Aya::Reflection::TGenericSlotWrapper<FunctionScriptSlot>>(FunctionScriptSlot(ei.descriptor, L, 2, useSubmitTask)));
        if (useSubmitTask)
        {
...
{
    {
        ...
        if (source && ei.descriptor->isScriptable())
        {
            ...
            shared_ptr<Aya::Reflection::TGenericSlotWrapper<WaitScriptSlot>> wrapper(
                Aya::make_shared<Aya::Reflection::TGenericSlotWrapper<WaitScriptSlot>>(WaitScriptSlot(L, useSubmitTask)));
            if (useSubmitTask)
            {
                wrapper->slot.setWeakSelf(wrapper);
            }
...
template<>
const char* Bridge<Aya::signals::connection>::className("RBXScriptConnection");

template<>
int Bridge<Aya::signals::connection>::on_index(const Aya::signals::connection& object, const char* name, lua_State* L)
{
...
template<>
void Bridge<Aya::signals::connection>::on_newindex(Aya::signals::connection& object, const char* name, lua_State* L)
{
    throw Aya::runtime_error("%s cannot be assigned to", name);
}
