...
#include "Script/ExitHandlers.hpp"
...
private:
    ...
    class ScriptImpersonator : public Aya::Security::Impersonator
    {
    public:
    ...
    typedef boost::array<GlobalState, Aya::Security::COUNT_VM_Classes> GlobalStates; // separate Lua top-level states
    GlobalStates globalStates;
    Lua::WeakThreadRef commandLineSandbox;
    std::set<BaseScript*> scripts;
    Aya::Time nextPendingScripts;
    struct ScriptStart
    {
    ...
    struct SecurityAnchor
    {
        ...
        FORCEINLINE void update(const void* ptr)
        {
            ...
            localValue[0] ^= localValue[1] * AYA_BUILDSEED | 20151112;
            localValue[1] ^= localValue[0] * 20151112 | AYA_BUILDSEED;
        ...
        FORCEINLINE bool checkAnchor(const void* ptr) const
        {
            ...
            localValue[1] ^= localValue[0] * 20151112 | AYA_BUILDSEED;
            localValue[0] ^= localValue[1] * AYA_BUILDSEED | 20151112;
            return !((reinterpret_cast<size_t>(ptr) + localValue[1]) ^ (~reinterpret_cast<size_t>(ptr) + localValue[0]));
#else
            return true;
#endif
        }
    ...
    Aya::safe_queue<WaitingThread> waitingThreads;
    ...
    Aya::atomic<int> timedout; // == scripts should stop running
...
public:
    ...
    Aya::signal<void(std::string, std::string, shared_ptr<Instance>)> errorSignal;
    // A temporary signals used for diagnostic purposes
    Aya::signal<void(shared_ptr<Instance>, std::string, shared_ptr<Instance>)> camelCaseViolation;
    Aya::signal<void(lua_State*)> scriptErrorDetected;
    ...
    Reflection::Variant evaluateStudioCommandItem(const char* itemToEvaluate, shared_ptr<Aya::LuaSourceContainer> script);
    ...
    // Shutdown helpers
    bool shouldPreventNewConnections()
    {
        return preventNewConnection;
    }
    void setPreventNewConnections()
    {
        preventNewConnection = true;
    }
    void closeStates(bool resettingSimulation); // Closes down all threads
    bool haveStatesClosed()
    {
        return statesClosed;
    }
    void cleanupModules();
    ...
    bool hasScript(BaseScript* script)
    {
    ...
    void executeInNewThread(Aya::Security::Identities identity, const ProtectedString& script, const char* name);
    std::auto_ptr<Reflection::Tuple> executeInNewThread(
        Aya::Security::Identities identity, const ProtectedString& script, const char* name, const Reflection::Tuple& arguments);
    void executeInNewThreadWithExtraGlobals(Aya::Security::Identities identity, const ProtectedString& script, const char* name,
    ...
    Result resume(Aya::Lua::ThreadRef thread, int narg);

    /////////////////////////////////////////////
    // Stats
    void scriptResumedFromEvent()
    {
    ...
    double getAvgLuaGcTime()
    {
        return avgLuaGcTime.value();
    }
    double getAvgLuaGcInterval()
    {
        return avgLuaGcInterval.value();
    }
...
private:
    ...
    static void setThreadIdentityAndSandbox(lua_State* thread, Aya::Security::Identities identity, shared_ptr<BaseScript> script);
    static Aya::Security::Identities getThreadIdentity(lua_State* thread);
    ...
    void executeInNewThread(Aya::Security::Identities identity, const ProtectedString& script, const char* name,
        ...
        Scripts::Continuations continuations, lua_State* globalStateToExecuteIn = NULL,
        const std::map<std::string, shared_ptr<Instance>>* extraGlobals = NULL, unsigned int modkey = 1);
...
public:
    ...
    lua_State* getGlobalState(Aya::Security::Identities identity);
...
private:
    ...
    static void moduleContentLoaded(AsyncHttpQueue::RequestResult result, shared_ptr<Instances> instances, ScriptContext& sc,
        Security::Identities identity, lua_State* globalState, AssetModuleInfo* info);
    static void moduleContentLinkedSourcesResolved(shared_ptr<Instances> instances, shared_ptr<ModuleScript> foundModuleScript, ScriptContext& sc,
        Security::Identities identity, lua_State* globalState, AssetModuleInfo* info);
    void startRunningModuleScript(Security::Identities identity, lua_State* globalState, shared_ptr<ModuleScript> moduleScript);
    static void requireModuleScriptSuccessContinuation(shared_ptr<ModuleScript> moduleScript, lua_State* threadRunningModuleCode);
    static void requireModuleScriptErrorContinuation(shared_ptr<ModuleScript> moduleScript, lua_State* threadRunningModuleCode);

    static void reloadModuleScriptInternal(lua_State* globalState, shared_ptr<ModuleScript> moduleScript);

    static void reloadModuleScriptSuccessContinuation(shared_ptr<ModuleScript> moduleScript, lua_State* reloadThread, int oldResultRegistryIndex);

    static void reloadModuleScriptErrorContinuation(shared_ptr<ModuleScript> moduleScript, lua_State* reloadThread);
    ...
    Aya::signals::connection camelCaseViolationConnection;
...
#define AYAASSERT_BALLANCED_LUA_STACK(L) StackBalanceCheck stackBalanceCheck(L)
#define AYAASSERT_BALLANCED_LUA_STACK2(L) StackBalanceCheck stackBalanceCheck2(L)
...
#else
#define AYAASSERT_BALLANCED_LUA_STACK(L) ((void)0)
#define AYAASSERT_BALLANCED_LUA_STACK2(L) ((void)0)
