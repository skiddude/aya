...
const char* const Aya::sLuaSourceContainer = "LuaSourceContainer";

const Reflection::PropDescriptor<LuaSourceContainer, ProtectedString> prop_CachedRemoteSource("CachedRemoteSource", category_Data,
    &LuaSourceContainer::getCachedRemoteSource, &LuaSourceContainer::setCachedRemoteSource, Reflection::PropertyDescriptor::REPLICATE_CLONE);

const Reflection::PropDescriptor<LuaSourceContainer, int> prop_CachedRemoteSourceLoadState("CachedRemoteSourceLoadState", category_Data,
...
Reflection::RemoteEventDesc<LuaSourceContainer, void()> LuaSourceContainer::event_requestLock(&LuaSourceContainer::requestLock, "RequestLock",
    Security::None, Reflection::RemoteEventCommon::REPLICATE_ONLY, Reflection::RemoteEventCommon::CLIENT_SERVER);

const Reflection::RemoteEventDesc<LuaSourceContainer, void(bool)> event_lockGrantedOrNot(&LuaSourceContainer::lockGrantedOrNot, "LockGrantedOrNot",
    "granted", Security::None, Reflection::RemoteEventCommon::REPLICATE_ONLY, Reflection::RemoteEventCommon::BROADCAST);

const Reflection::RefPropDescriptor<LuaSourceContainer, Instance> prop_CurrentEditor("CurrentEditor", category_Data,
    &LuaSourceContainer::getCurrentEditor, &LuaSourceContainer::setCurrentEditor, Reflection::PropertyDescriptor::PUBLIC_REPLICATE);
...
void LuaSourceContainer::loadLinkedScripts(
    shared_ptr<ContentProvider> cp, Instance* root, AsyncHttpQueue::ResultJob jobType, boost::function<void()> callback)
{
    ...
    FASTLOG2(DFLog::PreloadLinkedScriptsTiming, "Done counting linked scripts id=%p count=%d", loadData.get(), counter);
    if (counter > 0)
    {
...
void LuaSourceContainer::loadLinkedScriptsForInstances(
    shared_ptr<ContentProvider> cp, Instances& instances, AsyncHttpQueue::ResultJob jobType, boost::function<void()> callback)
{
    shared_ptr<LinkedScriptLoadData> loadData(new LinkedScriptLoadData);
    FASTLOG1(DFLog::PreloadLinkedScriptsTiming, "Starting to preload linked scripts id=%p", loadData.get());
    int counter = 0;
...
void LuaSourceContainer::processRemoteEvent(
...
static void applyLinkedScriptResult(
    weak_ptr<LuaSourceContainer> weakScript, AsyncHttpQueue::RequestResult result, shared_ptr<const std::string> loadedSource)
{
    if (shared_ptr<LuaSourceContainer> script = weakScript.lock())
    {
        ...
        else
        {
            ...
            StandardOut::singleton()->printf(MESSAGE_ERROR,
                "Failed to load source. The LinkedSource name may have changed or been removed."); // causes segfault in some games???
        }
    }
}

void LuaSourceContainer::linkedSourceLoadedHandler(weak_ptr<LuaSourceContainer> weakScript, AsyncHttpQueue::RequestResult result,
    shared_ptr<const std::string> loadedSource, shared_ptr<LinkedScriptLoadData> metadata)
{
    int completedScripts;
    {
        ...
        metadata->scriptApplyResultClosures.push_back(boost::bind(&applyLinkedScriptResult, weakScript, result, loadedSource));
        completedScripts = metadata->scriptApplyResultClosures.size();
    }

    if (completedScripts == metadata->scriptCount)
    {
        AsyncHttpQueue::dispatchGenericCallback(
            boost::bind(&LuaSourceContainer::updateScriptInstancesUnderWriteLock, _1, metadata), metadata->context.get(), metadata->jobType);
    }
}

void LuaSourceContainer::updateScriptInstancesUnderWriteLock(DataModel* /*unused*/, shared_ptr<LinkedScriptLoadData> metadata)
{
...
void LuaSourceContainer::linkedSourceFetchingVisitor(
    shared_ptr<Instance> descendant, shared_ptr<ContentProvider> cp, AsyncHttpQueue::ResultJob jobType, shared_ptr<LinkedScriptLoadData> metadata)
{
    ContentId out;
    if (LuaSourceContainer* LuaSourceContainer = isScriptWithLinkedSource(descendant, &out))
    {
        if (DataModel* dm = DataModel::get(cp.get()))
        {
            ...
            if (out.isHttp())
            {
                ...
                if (lowerVersion.find("serverplaceid=") != std::string::npos ||
                    (DFFlag::RejectHashesInLinkedSource && lowerVersion.find("#") != std::string::npos))
                {
                    linkedSourceLoadedHandler(weak_from(LuaSourceContainer), AsyncHttpQueue::Failed, shared_ptr<const std::string>(), metadata);
                    return;
                }
                else
                {
