...
{
template<>
int Bridge<boost::intrusive_ptr<WeakThreadRef::Node>>::on_index(
    const boost::intrusive_ptr<WeakThreadRef::Node>& object, const char* name, lua_State* L)
{
    // Failure
...
{
    FASTLOG3(FLog::ThreadRefCounts, "ThreadRef=%i WeakThreadRef=%i detail::LiveThreadRef=%i", ThreadRef::getCount(), WeakThreadRef::getCount(),
...
static void onAsyncResult(ThreadRef thread, weak_ptr<ScriptContext> context, IAsyncResult* result)
{
    ...
    catch (Aya::base_exception& e)
    {
        ...
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_WARNING, "Exception caught from async call. %s", e.what());
        return;
    }

    if (shared_ptr<Aya::ScriptContext> lockedContext = context.lock())
...
Aya::Lua::detail::LiveThreadRef::LiveThreadRef(lua_State* thread)
    : L(thread)
{
    AYAASSERT(L);
...
Aya::Lua::detail::LiveThreadRef::~LiveThreadRef()
{
...
{
template<>
const Type& Type::getSingleton<Aya::Lua::WeakFunctionRef>()
{
    static TType<Aya::Lua::WeakFunctionRef> type("Function");
    return type;
}

template<>
Aya::Lua::WeakFunctionRef& Variant::convert<Aya::Lua::WeakFunctionRef>(void)
{
    // Interpret "void" (Lua nil) as a null function
    // TODO: Is that correct behavior????
    if (isType<void>())
    {
        value = Aya::Lua::WeakFunctionRef();
        _type = &Type::singleton<Aya::Lua::WeakFunctionRef>();
    }

    Aya::Lua::WeakFunctionRef* val = tryCast<Aya::Lua::WeakFunctionRef>();
    if (val == NULL)
