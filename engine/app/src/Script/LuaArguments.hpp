...
R withVariantValue(const Reflection::Variant& value, F f)
{
    ...
    if (value.isType<Aya::ProtectedString>())
        return f(value.cast<Aya::ProtectedString>());
    ...
    if (const Reflection::EnumDescriptor* desc = Reflection::EnumDescriptor::lookupDescriptor(value.type()))
    {
        ...
        if (item == NULL)
            throw Aya::runtime_error("Invalid value for enum %s", desc->name.c_str());
        return f(*item);
    }
    ...
    if (value.isType<Aya::RbxRay>())
        return f(value.cast<Aya::RbxRay>());
    ...
    if (value.isType<Aya::Region3>())
        return f(value.cast<Aya::Region3>());
    if (value.isType<Aya::Region3int16>())
        return f(value.cast<Aya::Region3int16>());
    ...
    if (value.isType<Aya::signals::connection>())
        return f(value.cast<Aya::signals::connection>());
...
public:
    ...
    static shared_ptr<Reflection::Tuple> getValues(lua_State* L)
    {
        ...
        shared_ptr<Reflection::Tuple> args(Aya::make_shared<Reflection::Tuple>(argCount));

        for (int i = 0; i < argCount; ++i)
        {
            Reflection::Variant& v = args->values.at(i);
            bool success = Aya::Lua::LuaArguments::get(L, i + 1, v, false);
            AYAASSERT(success);
        }
    ...
    static int pushArray(_InIt _First, _InIt _Last, lua_State* const L)
    {
        ...
        while (_First != _Last)
        {
            ...
            AYAASSERT(count == 1); // If not 1, then what do we do?
            lua_rawseti(L, -2, ++i);
            ++_First;
        }
        return 1;
