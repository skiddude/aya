...
#include "Script/LuaCoreFunctions.hpp"
...
#include "Script/ScriptStats.hpp"

#include "Utility/AsyncHttpQueue.hpp"
#include "Utility/RbxStringTable.hpp"
#include "Utility/StandardOut.hpp" // TODO: This cross-reference of library code sucks
...
static Reflection::EventDesc<ScriptContext, void(shared_ptr<Instance>, std::string, shared_ptr<Instance>)> event_camelCaseViolation(
    &ScriptContext::camelCaseViolation, "CamelCaseViolation", "object", "member", "script", Aya::Security::RobloxScript);
REFLECTION_END();
...
_G - SANDBOXED - by default this contains the global environment. You may, however, want to set this variable to contain the environment of the
sandbox.
...
loadfile - OVERRIDEN
dofile - OVERRIDEN - The may read arbitrary files on the file system. It can also read standard input. The code is run under the global environment,
not the sandbox, so this can be used to break out of the sandbox. Related discussion: DofileNamespaceProposal.
...
{
    AYAASSERT_BALLANCED_LUA_STACK(thread);
...
struct LuaProfiler
{
    static std::string getResumePosition(lua_State* L, int nargs)
    {
        ...
        if (L->status == 0)
        {
            ...
            return Aya::format("%s(%d)", string ? "<string>" : file, line);
        }
        else
        {
            // resume from yield
    ...
    struct StringCache
    {
        struct Function
        {
            ...
            bool operator<(const Function& o) const
            {
                return (name != o.name)           ? name < o.name
                       : (namewhat != o.namewhat) ? namewhat < o.namewhat
        ...
        const char* getCall(const char* name, const char* namewhat, const char* file, int line)
        {
            ...
            if (res.empty())
            {
                const char* type = namewhat ? ((strcmp(namewhat, "index") == 0) ? "get " : (strcmp(namewhat, "newindex") == 0) ? "set " : "") : "";

                res = Aya::format("LUACALL%s(%d): %s%s", file ? file : "?", line, type, name ? name : "?");
            }
...
static Aya::atomic<int> contextCount;


// LocalUser permissions (including Studio.ashx execution)
Reflection::BoundFuncDesc<ScriptContext, void(int)> func_AddStarterScript(
    &ScriptContext::addStarterScript, "AddStarterScript", "assetId", Security::LocalUser);

// RobloxScript permissions
Reflection::BoundFuncDesc<ScriptContext, void(int, shared_ptr<Instance>, std::string name)> func_AddCoreScript(
    &ScriptContext::addCoreScript, "AddCoreScript", "assetId", "parent", "name", Security::RobloxScript);
Reflection::BoundFuncDesc<ScriptContext, void(std::string name, shared_ptr<Instance>)> func_AddCoreScriptLocal(
    &ScriptContext::addCoreScriptLocal, "AddCoreScriptLocal", "name", "parent", Security::RobloxScript);

// General permissions
Reflection::BoundProp<bool> ScriptContext::propScriptsDisabled("ScriptsDisabled", category_State, &ScriptContext::scriptsDisabled,
    &ScriptContext::onChangedScriptEnabled, Reflection::PropertyDescriptor::UI, Security::LocalUser);
...
Reflection::BoundFuncDesc<ScriptContext, shared_ptr<const Reflection::Tuple>(bool)> func_GetHeapStats(
    &ScriptContext::getHeapStats, "GetHeapStats", "clearHighwaterMark", true, Security::RobloxScript);
Reflection::BoundFuncDesc<ScriptContext, shared_ptr<const Reflection::ValueArray>()> func_GetScriptStats(
    &ScriptContext::getScriptStatsNew, "GetScriptStats", Security::RobloxScript);
Reflection::BoundFuncDesc<ScriptContext, void(bool)> func_CollectScriptStats(
    &ScriptContext::setCollectScriptStats, "SetCollectScriptStats", "enable", false, Security::RobloxScript);

// Experimental event for error-reporting
Reflection::EventDesc<ScriptContext, void(std::string, std::string, shared_ptr<Instance>)> event_Error(
    &ScriptContext::errorSignal, "Error", "message", "stackTrace", "script", Security::None);
REFLECTION_END();
...
public:
    ...
    WaitingScriptsJob(shared_ptr<ScriptContext> scriptContext)
        : DataModelJob("LuaResumeWaitingScripts", DataModelJob::Write, false, shared_from_dynamic_cast<DataModel>(scriptContext->getParent()),
...
void ScriptContext::hook(lua_State* L, lua_Debug* ar)
{
    ...
    {
    case LUA_HOOKCALL:
        if (LuaProfiler::instance)
            LuaProfiler::instance->hookCall(L, ar);
        break;

    case LUA_HOOKRET:
    case LUA_HOOKTAILRET:
        if (LuaProfiler::instance)
            LuaProfiler::instance->hookRet(L, ar);
        break;
...
{
    if (timedout)
        if (Security::Context::current().identity == Security::GameScript_ || Security::Context::current().identity == Security::RobloxGameScript_)
            luaL_error(L, "Game script timeout");
}


void Aya::registerScriptDescriptors()
{
    Aya::Reflection::Type::singleton<int>();
    Aya::Reflection::Type::singleton<bool>();
    Aya::Reflection::Type::singleton<float>();
    Aya::Reflection::Type::singleton<double>();
    Aya::Reflection::Type::singleton<std::string>();
    Aya::Reflection::Type::singleton<Lua::WeakFunctionRef>();
    Aya::Reflection::Type::singleton<shared_ptr<GenericFunction>>();
    Aya::Reflection::Type::singleton<shared_ptr<GenericAsyncFunction>>();
    Aya::Reflection::Type::singleton<shared_ptr<Instance>>();
    Aya::Reflection::Type::singleton<G3D::Vector3>();
    Aya::Reflection::Type::singleton<G3D::Vector2>();
    Aya::Reflection::Type::singleton<G3D::Rect2D>();
    Aya::Reflection::Type::singleton<PhysicalProperties>();
    Aya::Reflection::Type::singleton<TweenInfo>();
    Aya::Reflection::Type::singleton<Aya::RbxRay>();
    Aya::Reflection::Type::singleton<G3D::CoordinateFrame>();
    Aya::Reflection::Type::singleton<G3D::Color3>();
    Aya::Reflection::Type::singleton<BrickColor>();
}


int ScriptContext::crash(lua_State* L)
{
    Aya::Security::Context::current().requirePermission(Aya::Security::LocalUser, "crash");

    AYACRASH("User-generated crash");

    return 0;
}

int getGlobalStateIndex(Aya::Security::Identities identity)
{
    // transform identity into a particular VM. this can improve security by not mixing user scripts with Aya scripts (we've
    ...
    if (Aya::Security::Context::isInRole(identity, Aya::Security::RobloxScript))
...
lua_State* ScriptContext::getGlobalState(Aya::Security::Identities identity)
{
    int vm_index = getGlobalStateIndex(identity);
    AYAASSERT(globalStates[vm_index].state);
...
bool ScriptContext::openState(size_t idx)
{
    ...
    if (!allocator)
        allocator.reset(new Aya::LuaAllocator(FLog::UseLuaMemoryPool != 0));
    ...
    lua_gc(globalState, LUA_GCSETSTEPMUL, Aya::LuaSettings::singleton().gcStepMul);
    lua_gc(globalState, LUA_GCSETPAUSE, Aya::LuaSettings::singleton().gcPause);

    RobloxExtraSpace::get(globalState)->setContext(this);

    {
        AYAASSERT_BALLANCED_LUA_STACK(globalState);
...

    {
        ...
        RandomBridge::registerClass(globalState);
...

    {
        ...
        RandomBridge::registerClassLibrary(globalState);
...
void ScriptContext::closeStates(bool resettingSimulation)
{
    ...
    {
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "Exception thrown while cleaning up Lua: %s", e.what());
        AYAASSERT(false); // we should NEVER get an exception in here
    }
...
void ScriptContext::closeState(lua_State* globalState)
{
    ...
    {
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "Exception thrown while erasing Lua refs: %s", e.what());
        AYAASSERT(false); // we should NEVER get an exception in here
    }
...
void ScriptContext::onCamelCaseViolation(shared_ptr<Instance> object, std::string memberName, shared_ptr<Instance> script)
{
    ...
    if (script)
        StandardOut::singleton()->printf(MESSAGE_WARNING, "\"%s.%s\" should be \"%s\" in %s", object->getClassName().c_str(), memberName.c_str(),
            trueName.c_str(), script->getFullName().c_str());
    else
        StandardOut::singleton()->printf(
            MESSAGE_WARNING, "\"%s.%s\" should be \"%s\"", object->getClassName().c_str(), memberName.c_str(), trueName.c_str());
}
...
ScriptContext::ScriptContext()
    ...
    , preventNewConnection(false)
    , robloxPlace(false)
    , nextPendingScripts(Aya::Time::now<Time::Fast>())
    , timedout(false)
...
shared_ptr<const Reflection::ValueArray> ScriptContext::getScriptStatsNew()
{
    ...
    const ScriptStats::ScriptActivityMeterMap& stats = scriptStats->getScriptActivityMap();
    shared_ptr<Reflection::ValueArray> result(Aya::make_shared<Reflection::ValueArray>(stats.size()));
    int pos = 0;
    for (ScriptStats::ScriptActivityMeterMap::const_iterator iter = stats.begin(); iter != stats.end(); ++iter)
    {
        std::string hash = iter->first;
...
void ScriptContext::setTimeout(double seconds)
{
    ...
    else
    {
        ...
        if (!timeoutThread)
            timeoutThread.reset(
                new boost::thread(Aya::thread_wrapper(boost::bind(&ScriptContext::onCheckTimeout, shared_from(this)), "ScriptContext Timeout")));
        else
            checkTimeout.Set();
    }
}
...
void ScriptContext::setThreadIdentityAndSandbox(lua_State* thread, Aya::Security::Identities identity, shared_ptr<BaseScript> script)
{
    ...
    AYAASSERT(getThreadIdentity(thread) == identity);
}
...
Aya::Security::Identities ScriptContext::getThreadIdentity(lua_State* thread)
{
    return thread ? RobloxExtraSpace::get(thread)->identity : Aya::Security::Anonymous;
}
...
void ScriptContext::executeInNewThread(Aya::Security::Identities identity, const ProtectedString& script, const char* name)
{
    executeInNewThread(identity, script, name, boost::bind(&pushNoArguments, _1), NULL, Scripts::Continuations());
}
...
std::auto_ptr<Reflection::Tuple> ScriptContext::executeInNewThread(
    Aya::Security::Identities identity, const ProtectedString& script, const char* name, const Reflection::Tuple& arguments)
{
    std::auto_ptr<Reflection::Tuple> result;
    executeInNewThread(identity, script, name, boost::bind(&LuaArguments::pushTuple, boost::cref(arguments), _1),
        boost::bind(&readResults, boost::ref(result), _1, _2), Scripts::Continuations());

    return result;
}

void ScriptContext::executeInNewThreadWithExtraGlobals(Aya::Security::Identities identity, const ProtectedString& script, const char* name,
    const std::map<std::string, shared_ptr<Instance>>& extraGlobals)
{
    executeInNewThread(identity, script, name, boost::bind(&pushNoArguments, _1), NULL, Scripts::Continuations(), NULL, &extraGlobals);
}
...
void ScriptContext::executeInNewThread(Aya::Security::Identities identity, const ProtectedString& script, const char* name,
    boost::function1<size_t, lua_State*> pushArguments, boost::function2<void, lua_State*, size_t> readImmediateResults,
    Scripts::Continuations continuations, lua_State* globalStateToExecuteIn, const std::map<std::string, shared_ptr<Instance>>* extraGlobals,
    unsigned int modkey)
{
    WeakThreadRef thread;
    lua_State* owner;

    const bool isCmdLine = (identity == Aya::Security::CmdLine_);
    ...
    if (isCmdLine)
    {
        if (clSandboxEmpty)
        {
            lua_State* globalState = getGlobalState(Aya::Security::CmdLine_);
            AYAASSERT_BALLANCED_LUA_STACK(globalState);
            initializeLuaStateSandbox(commandLineSandbox, globalState, Aya::Security::CmdLine_);
            lua_pop(globalState, 1);
        }
        ThreadRef safeCommandlineSandbox = commandLineSandbox.lock();
        AYAASSERT(safeCommandlineSandbox);
    ...
    if (ThreadRef safeThread = thread.lock())
    {
        if (extraGlobals)
        {
            for (std::map<std::string, shared_ptr<Instance>>::const_iterator itr = extraGlobals->begin(); itr != extraGlobals->end(); ++itr)
            {
...
void ScriptContext::resume(
    ThreadRef thread, boost::function1<size_t, lua_State*> pushArguments, boost::function2<void, lua_State*, size_t> readResults)
{
...
int ScriptContext::spawn(lua_State* thread)
{
    ...
    if (DFFlag::BadTypeOnSpawnErrorEnabled)
    {
        ...
        {
            Aya::StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "Attempt to call spawn failed: Passed value is not a function");
...
{
    if (timeout < Aya::LuaSettings::singleton().defaultWaitTime)
        timeout = Aya::LuaSettings::singleton().defaultWaitTime;
...
int ScriptContext::delay(lua_State* thread)
{
    ...
    if (DFFlag::BadTypeOnDelayErrorEnabled)
    {
        ...
        if (!varResult.isType<Lua::WeakFunctionRef>())
        {
            Aya::StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "Attempt to call delay failed: Passed value is not a function");
            ScriptContext::printCallStack(thread);
        }
    }
...
int ScriptContext::ypcall(lua_State* thread)
{
    ...
    {
        AYAASSERT(top == lua_gettop(thread));
    ...
    case ScriptContext::Yield:
    {
        ...
        AYAASSERT(RobloxExtraSpace::get(safeFunctor)->continuations.get() == NULL);
...
{
    Aya::Security::Context::current().requirePermission(Aya::Security::Plugin, "settings()");
...
{
    Aya::Security::Context::current().requirePermission(Aya::Security::None, "UserSettings()");
...
{
    Aya::Security::Context::current().requirePermission(Aya::Security::Plugin, "PluginManager()");
...
{
    Aya::Security::Context::current().requirePermission(Aya::Security::Plugin, "DebuggerManager()");

    ObjectBridge::push(thread, shared_from(&Aya::Scripting::DebuggerManager::singleton()));
    return 1;
}
...
int ScriptContext::loadLibrary(lua_State* L)
{
    ...
    unsigned int modkey =
        (getGlobalState(L) == context.globalStates[Security::VM_RobloxScriptPlus].state) ? LUAVM_MODKEY_DUMMY : context.coreScriptModKey;

    context.executeInNewThread(Security::GameScript_, source.get(), libraryName.c_str(), boost::bind(&pushNoArguments, _1),
        boost::bind(&LibraryBridge::saveLibraryResult, _1, _2, libraryName), Scripts::Continuations(), getGlobalState(L), NULL, modkey);
...
void ScriptContext::requireModuleScriptSuccessContinuation(shared_ptr<ModuleScript> moduleScript, lua_State* threadRunningModuleCode)
{
...
void ScriptContext::requireModuleScriptErrorContinuation(shared_ptr<ModuleScript> moduleScript, lua_State* threadRunningModuleCode)
{
    lua_State* globalState = ScriptContext::getGlobalState(threadRunningModuleCode);
    ModuleScript::PerVMState& vmState = moduleScript->vmState(globalState);
...
void ScriptContext::reloadModuleScriptSuccessContinuation(shared_ptr<ModuleScript> moduleScript, lua_State* reloadThread, int oldResultRegistryIndex)
{
    moduleScript->vmState(ScriptContext::getGlobalState(reloadThread)).reassignResultRegistryIndex(oldResultRegistryIndex);
}

void ScriptContext::reloadModuleScriptErrorContinuation(shared_ptr<ModuleScript> moduleScript, lua_State* reloadThread)
{
    // empty
}
...
void ScriptContext::reloadModuleScriptInternal(lua_State* globalState, shared_ptr<ModuleScript> moduleScript)
{
    ...
    // required result.
    const std::string reloadCode = "oldResult, moduleRef  = ...\n"
    ...
    if (!loaded)
    {
        ...
        throw Aya::runtime_error("syntax error: %s", err.c_str());
    }
    ...
    if (reloadResult == LUA_YIELD)
    {
        ...
        // yield capturing.
        AYAASSERT(RobloxExtraSpace::get(reloadThread)->continuations == NULL);
        Lua::Continuations continuations;
        continuations.success = boost::bind(&ScriptContext::reloadModuleScriptSuccessContinuation, moduleScript, _1, oldResultIndex);
        continuations.error = boost::bind(&ScriptContext::reloadModuleScriptErrorContinuation, moduleScript, _1);
...
void ScriptContext::moduleContentLoaded(AsyncHttpQueue::RequestResult result, shared_ptr<Instances> instances, ScriptContext& sc,
    Security::Identities identity, lua_State* globalState, AssetModuleInfo* info)
{
    AYAASSERT(result != AsyncHttpQueue::Waiting);
    AYAASSERT(info->state == AssetModuleInfo::Fetching);
    ...
    LuaSourceContainer::loadLinkedScriptsForInstances(shared_from(ServiceProvider::create<ContentProvider>(&sc)), *instances,
        AsyncHttpQueue::AsyncWrite,
        boost::bind(&ScriptContext::moduleContentLinkedSourcesResolved, instances, foundModuleScript, boost::ref(sc), identity, globalState, info));
}

void ScriptContext::moduleContentLinkedSourcesResolved(shared_ptr<Instances> instances, shared_ptr<ModuleScript> foundModuleScript, ScriptContext& sc,
    Security::Identities identity, lua_State* globalState, AssetModuleInfo* info)
{
    ...
    case ModuleScript::Running:
        ...
        std::for_each(yieldedImporters.begin(), yieldedImporters.end(), boost::bind(&ModuleScript::PerVMState::addYieldedImporter, &vmState, _1));
        break;
...
void ScriptContext::startRunningModuleScript(Security::Identities identity, lua_State* rootGlobalState, shared_ptr<ModuleScript> moduleScript)
{
    ...
    {
        AYAASSERT(RobloxExtraSpace::get(thread)->continuations == NULL);
        Lua::Continuations continuations;
        continuations.success = boost::bind(&requireModuleScriptSuccessContinuation, moduleScript, _1);
        continuations.error = boost::bind(&ScriptContext::requireModuleScriptErrorContinuation, moduleScript, _1);
...
int ScriptContext::requireModuleScript(lua_State* L)
{
    ...
    {
        if (shared_ptr<ModuleScript> moduleScript = Instance::fastSharedDynamicCast<ModuleScript>(v.cast<shared_ptr<Instance>>()))
        {
            lua_pop(L, 1);
    ...
    throw runtime_error("Attempted to call require with invalid argument(s).");
}
...
int ScriptContext::requireModuleScriptFromInstance(lua_State* L, shared_ptr<ModuleScript> moduleScript)
{
    ...
    AYAASSERT(state != ModuleScript::NotRunYet);
    ...
    else if (state == ModuleScript::Running)
    {
        ...
        AYAASSERT(RobloxExtraSpace::get(L)->yieldCaptured == false);
...
int ScriptContext::requireModuleScriptFromAssetId(lua_State* L, int assetId)
{
    ...
    if (info.state == AssetModuleInfo::NotFetchedYet)
    {
        ...
        AYAASSERT(info.yieldedImporters.empty());
        ...
        if (DFFlag::LogPrivateModuleRequires)
        {
            ...
            contentId = ContentId(Aya::format(
                "%s/asset/?id=%d&modulePlaceId=%d", ::trim_trailing_slashes(contentProvider->getBaseUrl()).c_str(), assetId, dm->getPlaceID()));
        }
        else
        {
            contentId = ContentId(Aya::format("ayaassetid://%d", assetId));
        }

        contentProvider->loadContent(contentId, ContentProvider::PRIORITY_INSERT,
            boost::bind(&ScriptContext::moduleContentLoaded, _1, _2, boost::ref(getContext(L)), Security::Context::current().identity,
                getGlobalState(L), &loadedAssetModules[assetId]),
            AsyncHttpQueue::AsyncWrite);
...
{
    Aya::Security::Context::current().requirePermission(Aya::Security::LocalUser, "loadfile");

    ContentId contentId(throwable_lua_tostring(L, -1)); /* get file name */

    if (contentId.isNull())
        throw Aya::runtime_error("Unable to load %s", contentId.c_str());

    if (!Aya::Network::isTrustedContent(contentId.c_str()))
...
{
    ObjectBridge::push(L, shared_from(Aya::ServiceProvider::create<Aya::Stats::StatsService>(&ScriptContext::getContext(L))));
    return 1;
}
...
int ScriptContext::doPrint(lua_State* L, const Aya::MessageType& messageType)
{
    std::string message;
...
void ScriptContext::onHeartbeat(const Heartbeat& heartbeat)
{
    ...
    if (Aya::Time::now<Time::Fast>() >= nextPendingScripts)
    {
        AYAASSERT(!kCLuaResumeStackSize.get() || (*kCLuaResumeStackSize) == 0);
        // Try starting pending scripts, because some of them might be postponed for the ScriptId to download...
        startPendingScripts();
        nextPendingScripts = Aya::Time::now<Time::Fast>() + Aya::Time::Interval(1);
        AYAASSERT(!kCLuaResumeStackSize.get() || (*kCLuaResumeStackSize) == 0);
    }
...
void ScriptContext::resumeWaitingScripts(const Time expirationTime)
{
...

    {
        ...
        while (count-- > 0 && waitingThreads.pop_if_present(wt))
        {
            ...
            if (Aya::Time::nowFast() > expirationTime)
            {
                throttling = true;
                break;
            }
...
void ScriptContext::stepGc()
{
    ...
    for (GlobalStates::iterator iter = globalStates.begin(); iter != globalStates.end(); ++iter)
        if (iter->state)
        {
            ...
            AYAPROFILER_LABELF("Lua", "Allocated %d Kb (total %d Kb)", gcAlloc, gcCountPre);
            AYAPROFILER_LABELF("Lua", "Freed %d Kb (requested %d Kb)", gcCountPre - gcCountPost, gcRequest);
        }
...
{
    Aya::BaseScript::adminScriptsPath = newPath;
}
...
void ScriptContext::addStarterScript(int assetId)
{
    ...
    throw Aya::runtime_error("AddStarterScript disabled (asset %d)", assetId);
}
...
void ScriptContext::addCoreScript(int assetId, shared_ptr<Instance> parent, std::string name)
{
    ...
    throw Aya::runtime_error("AddCoreScript disabled (asset %d)", assetId);
}
...
{
    if (!Aya::DebugSettings::singleton().getStackTracingEnabled())
        return;
    ...
    if (lua_getstack(thread, 0, &ar))
    {
        ...
        while (level < maxLevel && lua_getstack(thread, level++, &ar))
        {
            if (lua_getinfo(thread, "nSlu", &ar))
            {
                if (ar.currentline >= 0)
                {
                    ...
                    {
                        if (!dontPrint)
                            Aya::StandardOut::singleton()->printf(MESSAGE_INFO, "Stack Begin");

                        if (output)
                            stream << "Stack Begin";
                    }
                    if (ar.name)
                        messageLine = Aya::format("Script '%s', Line %d - %s %s", source, ar.currentline, ar.namewhat, ar.name);
                    else
                        messageLine = Aya::format("Script '%s', Line %d", source, ar.currentline);

                    if (!dontPrint)
                        Aya::StandardOut::singleton()->printf(MESSAGE_INFO, "%s", messageLine.c_str());
        ...
        {
            if (!dontPrint)
                Aya::StandardOut::singleton()->printf(MESSAGE_INFO, "Stack End");
...
void ScriptContext::reportError(lua_State* thread)
{
    ...
    {
        StandardOut::singleton()->printf(MESSAGE_ERROR, "An error occurred: %s", error);
...
ScriptContext::ScriptImpersonator::ScriptImpersonator(lua_State* thread)
    : Aya::Security::Impersonator(getThreadIdentity(thread))
{
}
...
ScriptContext::Result ScriptContext::resume(ThreadRef thread, int narg)
{
    ...
    int result;
    {
        ...
        if (timedout &&
            (Security::Context::current().identity == Security::GameScript_ || Security::Context::current().identity == Security::RobloxGameScript_))
        {
            if (script)
        ...
        AYAPROFILER_SCOPE("Lua", "$Script");
        AYAPROFILER_LABEL("Lua", script ? script->getName().c_str() : "Unknown");
...
void ScriptContext::startScript(ScriptStart scriptStart)
{
    ...
    AYAASSERT(script->getRootAncestor() == getRootAncestor());
    AYAASSERT(scripts.find(script.get()) != scripts.end());
    ...
    if (Aya::LuaSettings::singleton().areScriptStartsReported)
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Running Script \"%s\"", script->getFullName().c_str());

    try
    {
        Aya::Security::Identities identity;

        if (script->isA<CoreScript>())
            identity = Aya::Security::RobloxGameScript_;
        else if (scriptStart.options.identity != Aya::Security::GameScript_)
            identity = scriptStart.options.identity;
        else if (robloxPlace)
            identity = Aya::Security::GameScriptInRobloxPlace_;
        else
            identity = Aya::Security::GameScript_;
        ...
        if (thread == NULL)
            throw Aya::runtime_error("Unable to create a new thread for %s", script->getName().c_str());
...

        {
            ...
            if (LuaVM::canCompileScripts() && scriptStart.options.filter)
            {
                try
                {
                    macroExpandedSource = ProtectedString::fromTrustedSource(scriptStart.options.filter(protectedSource->getSource()));
                ...
                catch (const ScriptStartOptions::LuaSyntaxError& e)
                {
                    ...
                    std::string message = Aya::format("%s(%d) - %s", script->getFullName().c_str(), e.lineNumber, e.what());
...
Reflection::Variant ScriptContext::evaluateStudioCommandItem(const char* itemToEvaluate, shared_ptr<LuaSourceContainer> script)
{
    if (commandLineSandbox.empty())
        initializeLuaStateSandbox(commandLineSandbox, getGlobalState(Aya::Security::CmdLine_), Aya::Security::CmdLine_);
    ...
    Aya::Reflection::Variant result = Reflection::Variant();

    try
    {
        result = Aya::Scripting::DebuggerManager::readWatchValue(itemToEvaluate, 0, L);
    }
    catch (std::runtime_error const& e)
    {
        Aya::Log::current()->writeEntry(Aya::Log::Information, Aya::format("IntelleSenseException: %s", e.what()).c_str());
    }
...
{
    AYAASSERT(runningScripts.find(weak_from(script)) == runningScripts.end());

    ScriptContext* scriptContext = ServiceProvider::create<ScriptContext>(this);

    AYAASSERT(!scriptContext->hasScript(script));
...
void RuntimeScriptService::releaseScript(BaseScript* script)
{
    ...
    if (script->fastDynamicCast<CoreScript>())
    {
        AYAASSERT(pendingScripts.find(weakPtr) == pendingScripts.end());
        AYAASSERT(runningScripts.find(weakPtr) == runningScripts.end());
        AYAASSERT(!scriptContext->hasScript(script));
        return;
    }
...
{
    std::for_each(loadedModules.begin(), loadedModules.end(), boost::bind(&ModuleScript::cleanupAndResetState, _1));
    loadedModules.clear();
}
...
void ScriptContext::validateThreadAccess(lua_State* L)
{
    ...
    {
        AYAASSERT(DataModel::currentThreadHasWriteLock(context));
    }
}
