...
{
    typedef DescribedNonCreatable<DebuggerManager, Instance, sDebuggerManager, Reflection::ClassDescriptor::INTERNAL_LOCAL, Security::LocalUser>
        Super;
...
private:
    ...
    Aya::signals::connection errorSignalConnection;
    Aya::signals::connection descendantAddedSignalConnection;
...
public:
    ...
    BreakOnErrorMode getBreakOnErrorMode() const
    {
        return breakOnErrorMode;
    }
    ...
    void reset();
    void setScriptAutoResume(bool state)
    {
        scriptAutoResume = state;
    }

    static void hook(lua_State* L, lua_Debug* ar);

    Aya::signal<void(shared_ptr<Instance>)> debuggerAdded;
    Aya::signal<void(shared_ptr<Instance>)> debuggerRemoved;
...
protected:
    ...
    void addUnaddedDebuggerForAddedDescendant(shared_ptr<Aya::Instance> instance);
};
...
private:
    ...
    Aya::signals::scoped_connection scriptStartedConnection;
    Aya::signals::scoped_connection scriptStoppedConnection;
    Aya::signals::scoped_connection scriptParentChangedConnection;
    Aya::signals::scoped_connection scriptClonedConnection;
...
public:
    ...
    void setIgnoreDebuggerBreak(bool state)
    {
        ignoreDebuggerBreak = state;
    }
    ...
    bool isRootThreadResumed()
    {
        return rootThreadResumed;
    }
    ...
    Aya::signal<void(int)> encounteredBreak;
    Aya::signal<void()> resuming;

    Aya::signal<void(shared_ptr<Instance>)> breakpointAdded;
    Aya::signal<void(shared_ptr<Instance>)> breakpointRemoved;
    Aya::signal<void(shared_ptr<Instance>)> watchAdded;
    Aya::signal<void(shared_ptr<Instance>)> watchRemoved;
    Aya::signal<void(int, std::string, Stack)> scriptErrorDetected;
...
private:
    ...
    void onScriptParentChanged(shared_ptr<Aya::Instance> newParent);
    ...
    static Aya::Instance* getScriptForLuaState(lua_State* L);
...
class DebuggerBreakpoint
    : public DescribedCreatable<DebuggerBreakpoint, Instance, sDebuggerBreakpoint, Reflection::ClassDescriptor::PERSISTENT_HIDDEN,
          Security::LocalUser>
{
    bool enabled;
    int line;
...
class DebuggerWatch : public DescribedCreatable<DebuggerWatch, Instance, sDebuggerWatch, Reflection::ClassDescriptor::PERSISTENT, Security::LocalUser>
{
