...
const char* const Aya::sModuleScript = "ModuleScript";

const Reflection::PropDescriptor<ModuleScript, ProtectedString> ModuleScript::prop_Source(
    "Source", category_Data, &ModuleScript::getSource, &ModuleScript::setSource, Reflection::PropertyDescriptor::STANDARD, Security::Plugin);
...
{
    AYAASSERT(scriptLoadingState == NotRunYet);
...
{
    AYAASSERT(scriptLoadingState == Running);
...
void ModuleScript::PerVMState::releaseReferenceIfCompletedSuccessfully()
{
    ...
    {
        AYAASSERT(resultRegistryIndex != LUA_NOREF);
        AYAASSERT(globalStateContainingResult);
...
void ModuleScript::PerVMState::releaseScriptNodeIfPresent()
{
    ...
    // This mechanism is how Aya::signal connections get cleaned up.
    if (node)
    {
        node->eraseAllRefs();
        node.reset();
