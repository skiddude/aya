...
const luaL_Reg registry[] = {{"difftime", os_difftime}, {"time", os_time}, {NULL, NULL}};
...
static int debug_traceback(lua_State* L)
{
    ...
    Aya::ScriptContext::printCallStack(L, &callStack, true);
    lua_pushstring(L, callStack.c_str());
    return 1;
}

const luaL_Reg registry[] = {{"traceback", debug_traceback}, {NULL, NULL}};
