...
void* operator new(size_t size, Aya::ScriptParser::Allocator& alloc)
{
...
void operator delete(void*, Aya::ScriptParser::Allocator& alloc) {}
...
{
public:
    Error(const Location& location, const char* format, ...) AYA_PRINTF_ATTR(3, 4)
        : location(location)
    {
        va_list args;
...
const char* kReserved[] = {"and", "break", "do", "else", "elseif", "end", "false", "for", "function", "if", "in", "local", "nil", "not", "or",
...
struct Lexeme
{
    ...
    Lexeme(const Location& location, Type type, const std::string* data)
        ...
        , data(data)
    {
        AYAASSERT(type == String || type == Number);
    }
...
public:
    ...
    bool is() const
    {
        return getClassIndex() == AstRtti<T>::value;
    }
    template<typename T>
    T* as()
    {
        return getClassIndex() == AstRtti<T>::value ? static_cast<T*>(this) : NULL;
    }
...
{
public:
    explicit AstExpr(const Location& location)
        : AstNode(location)
    {
    }
};
...
{
public:
    explicit AstStat(const Location& location)
        : AstNode(location)
    {
    }
};
...
private:
    ...
    AstStat* parseBlockNoScope()
    {
        ...
        Location location = body.empty() ? lexer.current().location : Location(body.front()->location, body.back()->location);
    ...
    AstExprCall* parseFunctionArgs(AstExpr* func, bool self)
    {
        if (lexer.current().type == '(')
        {
            if (func->location.end.line != lexer.current().location.begin.line)
                throw Error(lexer.current().location,
    ...
    void expect(Lexeme::Type type)
    {
        if (lexer.current().type != type)
            throw Error(lexer.current().location, "Expected %s, got %s", Lexeme(Location(Position(0, 0), 0), type).toString().c_str(),
    ...
    void expectMatch(Lexeme::Type type, const Lexeme& begin)
    {
        if (lexer.current().type != type)
        {
            ...
            if (lexer.current().location.begin.line == begin.location.begin.line)
                throw Error(lexer.current().location, "Expected %s (to close %s at column %d), got %s", typeString.c_str(), begin.toString().c_str(),
                    begin.location.begin.column + 1, lexer.current().toString().c_str());
            else
                throw Error(lexer.current().location, "Expected %s (to close %s at line %d), got %s", typeString.c_str(), begin.toString().c_str(),
                    begin.location.begin.line + 1, lexer.current().toString().c_str());
        }
    }

    template<typename T>
    AstArray<T> copy(const T* data, size_t size)
    {
        AstArray<T> result;
...
AYA_PRINTF_ATTR(4, 5)
void emitWarning(ScriptAnalyzer::Result& result, ScriptAnalyzer::WarningCode code, const Location& location, const char* format, ...)
{
    ...
    Warning warning(code,
        ScriptAnalyzer::Location(
            ScriptAnalyzer::Position(location.begin.line, location.begin.column), ScriptAnalyzer::Position(location.end.line, location.end.column)),
        message);
...
private:
    ...
    void report()
    {
        for (size_t i = 0; i < globalRefs.size(); ++i)
        {
            ...
            {
                if (*g->deprecated)
                    emitWarning(*context->result, ScriptAnalyzer::Warning_DeprecatedGlobal, gv->location,
                        "Global '%s' is deprecated, use '%s' instead", gv->name.value, *g->deprecated);
                else
                    emitWarning(
                        *context->result, ScriptAnalyzer::Warning_DeprecatedGlobal, gv->location, "Global '%s' is deprecated", gv->name.value);
            }
        }
        ...
        for (DenseHashMap<AstName, Global>::const_iterator it = globals.begin(); it != globals.end(); ++it)
        {
            ...
            if (g.onlyFunctionRef && g.assigned && g.firstRef->name != context->placeholder)
                emitWarning(*context->result, ScriptAnalyzer::Warning_GlobalUsedAsLocal, g.firstRef->location,
                    "Global '%s' is only used in the enclosing function; consider changing it to local", g.firstRef->name.value);
        }
    }
    ...
    virtual bool visit(AstExprGlobal* node)
    {
        ...
        if (node->name == context->placeholder)
            emitWarning(*context->result, ScriptAnalyzer::Warning_Placeholder, node->location,
    ...
    virtual bool visit(AstStatAssign* node)
    {
        for (size_t i = 0; i < node->vars.size; ++i)
        {
            ...
            if (AstExprGlobal* gv = var->as<AstExprGlobal>())
            {
                ...
                if (g.builtin)
                    emitWarning(*context->result, ScriptAnalyzer::Warning_BuiltinGlobalWrite, gv->location,
    ...
    virtual bool visit(AstExpr* node)
    {
        ...
        if (!top.flagged)
        {
            ...
            if (location.begin.line > top.lastLine)
            {
                ...
                {
                    emitWarning(
                        *result, ScriptAnalyzer::Warning_MultiLineStatement, location, "Statement spans multiple lines; use indentation to silence");
    ...
    virtual bool visit(AstExprCall* node)
    {
        if (!node->self)
            if (AstExprIndexName* index = node->func->as<AstExprIndexName>())
                ...
                {
                    emitWarning(*result, ScriptAnalyzer::Warning_DotCall, node->func->location, "Expected ':' not '.' calling member function %s",
                        index->index.value);
                }
...
private:
    ...
    void validateType(AstExprConstantString* arg, bool creatable)
    {
        ...
        if (name.empty() || !std::binary_search(Reflection::ClassDescriptor::all_begin(), Reflection::ClassDescriptor::all_end(), name,
                                ClassDescriptorNamePredicate()))
...
private:
    ...
    void report()
    {
        for (std::set<AstLocal*>::iterator it = used.begin(); it != used.end(); ++it)
        {
            ...
            if (shadow)
                emitWarning(*result, ScriptAnalyzer::Warning_LocalShadow, local->location,
                    "Variable '%s' shadows the previous declaration at line %d", local->name.value, shadow->location.begin.line + 1);
        }
    }
        ...
        for (size_t i = 0; i < pass.tables.size(); ++i)
        {
            ...
            if (!t.bottom)
            {
                for (std::set<AstExprIndexName*>::iterator it = t.reads.begin(); it != t.reads.end(); ++it)
                {
                    ...
                    {
                        emitWarning(*context.result, ScriptAnalyzer::Warning_UnknownMember, (*it)->indexLocation, "Can not find member '%s' in table",
                            (*it)->index.value);
                    }
                }
            }
...
private:
    ...
    Value evalElement(const std::map<Key, Value>& map, const Key& key)
    {
    ...
    Value evalLookup(const Reflection::ClassDescriptor* klass, const char* name, const Location& location)
    {
        ...
        emitWarning(
            *context->result, ScriptAnalyzer::Warning_UnknownMember, location, "Can not find member '%s' in type '%s'", name, klass->name.c_str());
...
{
    Security::Identities identity = Aya::Security::GameScript_;
...
ScriptAnalyzer::Result ScriptAnalyzer::analyze(DataModel* dm, boost::shared_ptr<Instance> script, const std::string& code)
{
    ...
    {
        if (!dm)
            return ScriptAnalyzer::Result();

        AYAASSERT(dm->currentThreadHasWriteLock());
...
#define PASS(name) \
    do \
    { \
        AnalyzerPass p = {#name, name::process}; \
        passes.push_back(p); \
    } while (0)
...
#undef PASS
            ...
            for (size_t i = 0; i < (true /* StudioVariableIntellesense */ ? passes.size() : sizeof(passes) / sizeof(passes[0])); ++i)
            {
    ...
    {
        StandardOut::singleton()->printf(MESSAGE_ERROR, "ScriptAnalyzer: unexpected error %s while parsing %s", e.what(),
            script ? script->getFullName().c_str() : "unknown script");
