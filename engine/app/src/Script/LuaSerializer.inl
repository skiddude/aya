...
private:
    ...
    {
        Aya::DenseHashMap<const TString*, unsigned int> strings(NULL);
        ...
        for (Aya::DenseHashMap<const TString*, unsigned int>::const_iterator it = strings.begin(); it != strings.end(); ++it)
    ...
    static void writeString(std::ostringstream& ss, Aya::DenseHashMap<const TString*, unsigned int>& strings, const TString* value)
    {
        if (value)
        {
    ...
    static void writeProto(
        std::ostringstream& ss, Aya::DenseHashMap<const TString*, unsigned int>& strings, Proto* p, RbxOpEncoder encode, unsigned int ckey)
    {
        writeInt(ss, p->sizep);
        writeInt(ss, p->sizek);
...
public:
    static int deserialize(lua_State* L, const std::string& code, const char* chunkname, unsigned int modkey)
    {
        ...
        for (size_t i = 0; i < 4; ++i)
        {
            ...
            hashbytes[i] -= (char)(i * kBytecodeHashMultiplier);
        }
        ...
        AYAASSERT_VERY_FAST(luaG_checkcode(p, L->l_G->ckey));
    ...
    {
        Aya::DataModel::sendStats |= HATE_INVALID_BYTECODE;
...
private:
    ...
    public:
        ...
        {
            AYAASSERT_VERY_FAST(newoffset <= data.size());
    ...
    static TString* readString(VectorStream& ss, const std::vector<TString*>& strings)
    {
        ...
        AYAASSERT_VERY_FAST(index <= strings.size());
