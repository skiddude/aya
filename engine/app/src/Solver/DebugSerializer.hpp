...
{
private:
    template<typename U, void (U::*)(DebugSerializer&) const>
    struct SFINAE
    {
    };
    template<typename U>
...
public:
    ...
    {
        boost::uint8_t length = (boost::uint8_t)strlen(name);
        *this& length;
...
inline DebugSerializerScope::DebugSerializerScope(DebugSerializer& s)
    : serializer(s)
{
