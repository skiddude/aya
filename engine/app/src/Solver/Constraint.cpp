...
//
void ConstraintBallInSocket::buildEquation(ConstraintJacobianPair* _jacobian, boost::uint8_t* _useBlock, ConstraintVariables* _velocityStage,
    ConstraintVariables* _positionStage, const SolverBodyDynamicProperties& _bodyA, const SolverBodyDynamicProperties& _bodyB,
    const SolverConfig& _config, float _dt)
{
    ...
    float lengthToSolveByStabilization =
        std::min(_config.ballInSocketCorrectionDamping * errorLength, _config.ballInSocketMaxCorrectionByStabilization);
...
Constraint::Convergence ConstraintBallInSocket::testPGSConvergence(
    const float* _disp, const float* _residuals, const float* _deltaResiduals, const SolverConfig& _solverConfig)
{
    ...
    if (t < _solverConfig.inconsistentConstraintBallInSocketResidualThreshold * _solverConfig.inconsistentConstraintBallInSocketResidualThreshold)
    {
        ...
        if (s < _solverConfig.inconsistentConstraintDeltaThreshold * _solverConfig.inconsistentConstraintDeltaThreshold *
                    (_solverConfig.inconsistentConstraintBallInSocketResidualThreshold *
                        _solverConfig.inconsistentConstraintBallInSocketResidualThreshold))
        {
            return Convergence_Converges;
        }
    }
    // if( ( _deltaResiduals[0] * _residuals[0] > 0.0f ) || ( _deltaResiduals[1] * _residuals[1] > 0.0f ) || ( _deltaResiduals[2] * _residuals[2] >
    // 0.0f ) )
    else
    {
        float s = _deltaResiduals[0] * _deltaResiduals[0] + _deltaResiduals[1] * _deltaResiduals[1] + _deltaResiduals[2] * _deltaResiduals[2];
...
//
void ConstraintLegacyBreakableBallInSocket::buildEquation(ConstraintJacobianPair* _jacobian, boost::uint8_t* _useBlock,
    ConstraintVariables* _velocityStage, ConstraintVariables* _positionStage, const SolverBodyDynamicProperties& _bodyA,
    const SolverBodyDynamicProperties& _bodyB, const SolverConfig& _config, float _dt)
{
    const CoordinateFrame& cA = getBodyA()->getPvUnsafe().position;
    CoordinateFrame cB;
    if (getBodyB() != NULL)
    ...
    ConstraintVariables::setReaction(
        _velocityStage, Vector3(deltaV.dot(worldSpaceNormal), deltaV.dot(worldSpaceTangent1), deltaV.dot(worldSpaceTangent2)));
    ConstraintVariables::setReaction(
        _positionStage, Vector3(vectorToSolveByStabilization.dot(worldSpaceNormal), vectorToSolveByStabilization.dot(worldSpaceTangent1),
                            vectorToSolveByStabilization.dot(worldSpaceTangent2)));
...
#endif
ConstraintAlign2Axes::ConstraintAlign2Axes(Body* _bodyA, Body* _bodyB)
    : Constraint(Constraint::Types_Align2Axes, _bodyA, _bodyB, align2AxesDimension)
    , worldSpaceOrthogonalB1(0.0f, 0.0f, 0.0f)
    , worldSpaceOrthogonalB2(0.0f, 0.0f, 0.0f)
{
}

void ConstraintAlign2Axes::buildEquation(ConstraintJacobianPair* _jacobian, boost::uint8_t* _useBlock, ConstraintVariables* _velocityStage,
    ConstraintVariables* _positionStage, const SolverBodyDynamicProperties& _bodyA, const SolverBodyDynamicProperties& _bodyB,
    const SolverConfig& _config, float _dt)
{
    const CoordinateFrame& cA = getBodyA()->getPvUnsafe().position;
...
Constraint::Convergence ConstraintAlign2Axes::testPGSConvergence(
    const float* _disp, const float* _residuals, const float* _deltaResiduals, const SolverConfig& _solverConfig)
{
    ...
    if (t < _solverConfig.inconsistentConstraintAlign2AxesThreshold * _solverConfig.inconsistentConstraintAlign2AxesThreshold)
    {
        ...
        if (s < _solverConfig.inconsistentConstraintDeltaThreshold * _solverConfig.inconsistentConstraintDeltaThreshold *
                    (_solverConfig.inconsistentConstraintAlign2AxesThreshold * _solverConfig.inconsistentConstraintAlign2AxesThreshold))
        {
            return Convergence_Converges;
        }
    }
...
//
void ConstraintAngularVelocity::buildEquation(ConstraintJacobianPair* _jacobian, boost::uint8_t* _useBlock, ConstraintVariables* _velocityStage,
    ConstraintVariables* _positionStage, const SolverBodyDynamicProperties& _bodyA, const SolverBodyDynamicProperties& _bodyB,
    const SolverConfig& _config, float _dt)
{
    CoordinateFrame cB;
...
//
void ConstraintLinearVelocity::buildEquation(ConstraintJacobianPair* _jacobian, boost::uint8_t* _useBlock, ConstraintVariables* _velocityStage,
    ConstraintVariables* _positionStage, const SolverBodyDynamicProperties& _bodyA, const SolverBodyDynamicProperties& _bodyB,
    const SolverConfig& _config, float _dt)
{
    _useBlock[0] = 0;
...
// ConstraintLinearVelocity
//
void ConstraintLinearSpring::buildEquation(ConstraintJacobianPair* _jacobian, boost::uint8_t* _useBlock, ConstraintVariables* _velocityStage,
    ConstraintVariables* _positionStage, const SolverBodyDynamicProperties& _bodyA, const SolverBodyDynamicProperties& _bodyB,
    const SolverConfig& _config, float _dt)
{
    const CoordinateFrame& cA = getBodyA()->getPvUnsafe().position;
...
//
void ConstraintAchievePosition::buildEquation(ConstraintJacobianPair* _jacobian, boost::uint8_t* _useBlock, ConstraintVariables* _velocityStage,
    ConstraintVariables* _positionStage, const SolverBodyDynamicProperties& _bodyA, const SolverBodyDynamicProperties& _bodyB,
    const SolverConfig& _config, float _dt)
{
    const CoordinateFrame& cA = getBodyA()->getPvUnsafe().position;
    ...
    AYAASSERT(t.x >= 0.0f && t.x <= 1.0f);
    AYAASSERT(t.y >= 0.0f && t.y <= 1.0f);
    AYAASSERT(t.z >= 0.0f && t.z <= 1.0f);
...
//
void ConstraintCollision::buildEquation(ConstraintJacobianPair* _jacobian, boost::uint8_t* _useBlock, ConstraintVariables* _velocityStage,
    ConstraintVariables* _positionStage, const SolverBodyDynamicProperties& _bodyA, const SolverBodyDynamicProperties& _bodyB,
    const SolverConfig& _config, float _dt)
{
    ...
    _velocityStage[0].reaction =
        integratedNormalVelocity + restitution * (previousNormalVelocity < _config.collisionRestitutionThreshold ? 0.0f : previousNormalVelocity);
    ...
    if (previousTangentialVelocitySquared < _config.collisionFrictionStaticToDynamicThreshold * _config.collisionFrictionStaticToDynamicThreshold)
    {
        ...
        {
            float gravity = fabsf(Units::kmsAccelerationToRbx(this->getBodyA()->getGravity()));
    ...
    else
    {
        ...
        if (FFlag::PGSVariablePenetrationMarginFix)
        {
            float dynamicVelocityExcess = std::min(_config.collisionPenetrationVelocityForMinMargin,
                sqrtf(previousTangentialVelocitySquared) - _config.collisionFrictionStaticToDynamicThreshold);
...
Constraint::Convergence ConstraintCollision::testPGSConvergence(
    const float* _disp, const float* _residuals, const float* _deltaResiduals, const SolverConfig& _solverConfig)
{
    if (-_solverConfig.inconsistentConstraintCollisionBaseThreshold < _residuals[0] &&
        _residuals[0] < _solverConfig.inconsistentConstraintCollisionThreshold)
    {
...
//
void ConstraintBodyAngularVelocity::buildEquation(ConstraintJacobianPair* _jacobian, boost::uint8_t* _useBlock, ConstraintVariables* _velocityStage,
    ConstraintVariables* _positionStage, const SolverBodyDynamicProperties& _bodyA, const SolverBodyDynamicProperties& _bodyB,
    const SolverConfig& _config, float _dt)
{
    const CoordinateFrame& cA = getBodyA()->getPvUnsafe().position;
...
// ConstraintBodyAngularVelocity
//
void ConstraintLegacyAngularVelocity::buildEquation(ConstraintJacobianPair* _jacobian, boost::uint8_t* _useBlock, ConstraintVariables* _velocityStage,
    ConstraintVariables* _positionStage, const SolverBodyDynamicProperties& _bodyA, const SolverBodyDynamicProperties& _bodyB,
    const SolverConfig& _config, float _dt)
{
    _useBlock[0] = 0;
...
void ConstraintCache::cache(const ConstraintVariables& _velocityStage, const ConstraintVariables& _positionStage, float _sorVel, float _sorPos,
    bool _isCollision, const SolverConfig& config)
{
...
{
    s & velocityImpulse & velocityReaction & velocitySor & velocityCacheDamping & positionImpulse & positionReaction & positionSor &
        positionCacheDamping & velocityImpulseRegression & positionImpulseRegression;
}
