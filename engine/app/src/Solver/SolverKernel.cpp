...
#include "Solver/SolverKernel.hpp"
#include "Solver/Constraint.hpp"
#include "Solver/ConstraintJacobian.hpp"
...
static AYA_SIMD_INLINE EffectiveMassPair computeEffectiveMassPair(
    const SolverBodyMassAndInertia& _bodyA, const SolverBodyMassAndInertia& _bodyB, const ConstraintJacobianPair& _j, const SolverConfig& _config)
{
    EffectiveMass emA(
        _bodyA.getInvMass<StageSelector>() * _j.getLinA(), _bodyA.getInvInertia<StageSelector>(_config.stabilizationInertiaScale) * _j.getAngA());
    EffectiveMass emB(
        _bodyB.getInvMass<StageSelector>() * _j.getLinB(), _bodyB.getInvInertia<StageSelector>(_config.stabilizationInertiaScale) * _j.getAngB());
    return EffectiveMassPair(emA, emB);
}

// Pre-compute effective masses
void PGSComputeEffectiveMasses(EffectiveMassPair* __restrict _effectiveMassesVelStage, EffectiveMassPair* __restrict _effectiveMassesPosStage,
    size_t _constraintCount, const boost::uint8_t* _dimensions, const ConstraintJacobianPair* _jacobians, const BodyPairIndices* _pairs,
    const SolverBodyMassAndInertia* _massAndIntertia, const SolverConfig& _config)
{
    AYAPROFILER_SCOPE("Physics", "PGSComputeEffectiveMasses");
...
// Helper methods
template<int DIM>
static AYA_SIMD_INLINE void applyPreconditionerToStage(ConstraintVariables* __restrict _vars, ConstraintJacobianPair* __restrict _preJacobians,
    const boost::uint8_t* _useBlock, const ConstraintJacobianPair* __restrict _jacobian, const float* __restrict _sor,
...
// Helper methods
static AYA_SIMD_INLINE void applyPreconditionerToJacobian(
    ConstraintJacobianPair* __restrict _r, const v4f& _preconditioner, const ConstraintJacobianPair* __restrict _j, const v4f& _sor)
{
...
template<>
AYA_SIMD_INLINE void applyPreconditionerToStage<1>(ConstraintVariables* __restrict _vars, ConstraintJacobianPair* __restrict _preJacobians,
    const boost::uint8_t* _useBlock, const ConstraintJacobianPair* __restrict _jacobian, const float* __restrict _sor,
    const EffectiveMassPair* __restrict _effectiveMasses)
{
...
static AYA_SIMD_INLINE void multiplyRowByColumn(
    ConstraintJacobianPair* __restrict _r, const SymmetricMatrix2SIMD& _preconditioner, const ConstraintJacobianPair* __restrict _j, const v4f& _sor)
{
    v4f r = _preconditioner.get<row, 0>() * _j[0].get<SubMatrixSelect>() + _preconditioner.get<row, 1>() * _j[1].get<SubMatrixSelect>();
    r = splat<row>(_sor) * r;
    _r[row].set<SubMatrixSelect>(r);
}

template<class SubMatrixSelect>
static AYA_SIMD_INLINE void applyPreconditionerToSubMatrix(
    ConstraintJacobianPair* __restrict _r, const SymmetricMatrix2SIMD& _preconditioner, const ConstraintJacobianPair* __restrict _j, const v4f& _sor)
{
...
static AYA_SIMD_INLINE SymmetricMatrix2SIMD constraintMatrixDiagBlockInverse2(
    const boost::uint8_t* _useBlock, const ConstraintJacobianPair* _jacobian, const EffectiveMassPair* _massVectors)
{
...
template<>
AYA_SIMD_INLINE void applyPreconditionerToStage<2>(ConstraintVariables* __restrict _vars, ConstraintJacobianPair* __restrict _preJacobians,
    const boost::uint8_t* _useBlock, const ConstraintJacobianPair* __restrict _jacobian, const float* __restrict _sor,
    const EffectiveMassPair* __restrict _effectiveMasses)
{
    // Compute the inverse of the 2x2 diagonal block of the constraint matrix, whose entries are all the dot product of the jacobians with the
    // effective masses in all dimensions
...
template<int row, class SubMatrixSelect>
static AYA_SIMD_INLINE void multiplyRowByColumn(
    ConstraintJacobianPair* __restrict _r, const SymmetricMatrixSIMD& _preconditioner, const ConstraintJacobianPair* __restrict _j, const v4f& _sor)
{
    v4f r = _preconditioner.get<row, 0>() * _j[0].get<SubMatrixSelect>() + _preconditioner.get<row, 1>() * _j[1].get<SubMatrixSelect>() +
...
static AYA_SIMD_INLINE SymmetricMatrixSIMD constraintMatrixDiagBlockInverse3(
    const boost::uint8_t* _useBlock, const ConstraintJacobianPair* _jacobian, const EffectiveMassPair* _massVectors)
{
    v4f d01 = _jacobian[0].dot(_massVectors[1]);
...
// Dim 3 pre-conditioner
template<>
AYA_SIMD_INLINE void applyPreconditionerToStage<3>(ConstraintVariables* __restrict _vars, ConstraintJacobianPair* __restrict _preJacobians,
    const boost::uint8_t* _useBlock, const ConstraintJacobianPair* __restrict _jacobian, const float* __restrict _sor,
    const EffectiveMassPair* __restrict _effectiveMasses)
{
...
template<int DIM>
static void applyPreconditioner(ConstraintVariables* __restrict _velocityStageVariables, ConstraintVariables* __restrict _positionStageVariables,
    ConstraintJacobianPair* __restrict _preconditionedJacobiansVelStage, ConstraintJacobianPair* __restrict _preconditionedJacobiansPosStage,
    const boost::uint8_t* _useBlock, const ConstraintJacobianPair* __restrict _jacobians, const float* __restrict _sorVel,
    const float* __restrict _sorPos, const EffectiveMassPair* __restrict _effectiveMassesVelStage,
    const EffectiveMassPair* __restrict _effectiveMassesPosStage, boost::uint32_t offset)
{
    ...
    applyPreconditionerToStage<DIM>(
        currentVelStage, currentPreJacobianVelStage, currentUseBlock, currentJacobian, sorVel, currentEffectiveMassesVelStage);
    applyPreconditionerToStage<DIM>(
        currentPosStage, currentPreJacobianPosStage, currentUseBlock, currentJacobian, sorPos, currentEffectiveMassesPosStage);
}
...
//
void PGSPreconditionConstraintEquations(ConstraintJacobianPair* __restrict _preconditionedJacobiansVelStage,
    ConstraintJacobianPair* __restrict _preconditionedJacobiansPosStage, ConstraintVariables* __restrict _velocityStageVariables,
    ConstraintVariables* __restrict _positionStageVariables, size_t _constraintCount, const boost::uint8_t* __restrict _dimensions,
    const boost::uint8_t* __restrict _useBlock, const float* __restrict _sorVel, const float* __restrict _sorPos,
    const ConstraintJacobianPair* __restrict _jacobians, const EffectiveMassPair* __restrict _effectiveMassesVelStage,
    const EffectiveMassPair* __restrict _effectiveMassesPosStage)
{
    AYAPROFILER_SCOPE("Physics", "PGSPreconditionConstraintEquations");
    ...
    for (size_t c = 0; c < _constraintCount; c++)
    {
        ...
        AYAASSERT_VERY_FAST(d <= maxConstraintDimension);

        switch (d)
        {
        case 1:
            applyPreconditioner<1>(_velocityStageVariables, _positionStageVariables, _preconditionedJacobiansVelStage,
                _preconditionedJacobiansPosStage, _useBlock, _jacobians, _sorVel, _sorPos, _effectiveMassesVelStage, _effectiveMassesPosStage,
                offset);
            break;
        case 2:
            applyPreconditioner<2>(_velocityStageVariables, _positionStageVariables, _preconditionedJacobiansVelStage,
                _preconditionedJacobiansPosStage, _useBlock, _jacobians, _sorVel, _sorPos, _effectiveMassesVelStage, _effectiveMassesPosStage,
                offset);
            break;
        case 3:
            applyPreconditioner<3>(_velocityStageVariables, _positionStageVariables, _preconditionedJacobiansVelStage,
                _preconditionedJacobiansPosStage, _useBlock, _jacobians, _sorVel, _sorPos, _effectiveMassesVelStage, _effectiveMassesPosStage,
                offset);
            break;
        default:
...
//
//-----------------------------------------------------------------------------------------------------------
void PGSApplyEffectiveMassMultipliers(EffectiveMassPair* __restrict _effectiveMassesVelStage, EffectiveMassPair* __restrict _effectiveMassesPosStage,
    size_t _constraintCount, const boost::uint8_t* _dimensions, const float* _multipliers, const BodyPairIndices* _pairs, const SolverConfig& _config)
{
    AYAPROFILER_SCOPE("Physics", "PGSApplyEffectiveMassMultipliers");
...
static AYA_SIMD_INLINE VirtualDisplacement applyImpulse(const VirtualDisplacement& virD, const simd::v4f& impulse, const EffectiveMass& eff)
{
    return VirtualDisplacement(simd::mulAdd(virD.getLin(), impulse, eff.getLin()), simd::mulAdd(virD.getAng(), impulse, eff.getAng()));
}

// Main method
void PGSInitVirtualDisplacements(VirtualDisplacementArray& _virDVel, VirtualDisplacementArray& _virDPos,
    const EffectiveMassPair* __restrict _effectiveMassesVelStage, const EffectiveMassPair* __restrict _effectiveMassesPosStage,
    size_t _constraintCount, const boost::uint8_t* _dimensions, const ConstraintVariables* __restrict _velStage,
    const ConstraintVariables* __restrict _posStage, const BodyPairIndices* _pairs, const SolverConfig& _config)
{
    AYAPROFILER_SCOPE("Physics", "PGSInitVirtualDisplacements");
...
static AYA_SIMD_INLINE v4f partiallyProjectOntoJacobian(
    const ConstraintJacobianPair& _j, const VirtualDisplacement& _va, const VirtualDisplacement& _vb)
{
...
static void updateConstraintDim1(ConstraintVariables* __restrict _velStage, ConstraintVariables* __restrict _posStage,
    VirtualDisplacementPOD* __restrict _virDVel, VirtualDisplacementPOD* __restrict _virDPos,
    const ConstraintJacobianPair* __restrict _preconditionedJacobiansVel, const ConstraintJacobianPair* __restrict _preconditionedJacobiansPos,
    const EffectiveMassPair* __restrict _effectiveMassesVel, const EffectiveMassPair* __restrict _effectiveMassesPos, const BodyPairIndices& _pair)
{
    ...
    // The new impulse is the correction of the old impulse by the difference of the desired change in relative velocities with current change in
    // relative velocities due to constraint impulses, multiplied by the pre-conditioner (inverse of the diagonal element)
...
static void updateConstraintDim2(ConstraintVariables* __restrict _velStage, ConstraintVariables* __restrict _posStage,
    VirtualDisplacementPOD* __restrict _virDVel, VirtualDisplacementPOD* __restrict _virDPos,
    const ConstraintJacobianPair* __restrict _preconditionedJacobiansVel, const ConstraintJacobianPair* __restrict _preconditionedJacobiansPos,
    const EffectiveMassPair* __restrict _effectiveMassesVel, const EffectiveMassPair* __restrict _effectiveMassesPos, const BodyPairIndices& _pair)
{
    ...
    // This is equivalent to 'multiplying a row of the constraint matrix with the impulse vector and dividing by the diagonal element of the row'.
    // But because we do block PGS, this is 'multiplying a group of row (2 of them int is case) of the constraint matrix with the impulse vector and
    // multiplying by the inverse of the diagonal block'. Physically the result of this is the relative change in velocity due to currently computed
    // constraint impulses. We store the result as: projectedVirD = ( projection vel stage dim 0, projection pos stage dim 0, projection vel stage dim
    // 1, projection pos stage dim 1 )
    v4f t0 = partiallyProjectOntoJacobian(_preconditionedJacobiansVel[0], _virDVel[_pair.first], _virDVel[_pair.second]);
    ...
    // ...
    v4f oldImpulses, preconditionedReactions, minImpulses, maxImpulses;
    ConstraintVariables::gatherComponents(
        oldImpulses, preconditionedReactions, minImpulses, maxImpulses, _velStage[0], _posStage[0], _velStage[1], _posStage[1]);

    // Compute and clamp new impulses
    // The new impulse is the correction of the old impulse by the difference of the desired change in relative velocities with current change in
    // relative velocities due to constraint impulses, multiplied by the pre-conditioner (inverse of the diagonal block)
    v4f newImpulses = oldImpulses + (preconditionedReactions - projectedVirD);
    newImpulses = max(newImpulses, minImpulses);
    newImpulses = min(newImpulses, maxImpulses);
...
template<class Type>
static AYA_SIMD_INLINE void modifyImpulseBounds(v4f& minBound, v4f& maxBound, const v4f& oldImpulses)
{
}

// For collisions we need to do something different to simulate the friction cone
template<>
AYA_SIMD_INLINE void modifyImpulseBounds<CollisionType>(v4f& minBound, v4f& maxBound, const v4f& oldImpulses)
{
...
template<class Type>
static AYA_SIMD_INLINE void updateConstraintDim3OrCollisionSingleStage(ConstraintVariables* __restrict _vars,
    VirtualDisplacementPOD* __restrict _virD, const ConstraintJacobianPair* __restrict _preconditionedJacobians,
    const EffectiveMassPair* __restrict _effectiveMasses, const BodyPairIndices& _pair)
{
    // Project the virtual displacements onto the preconditioned Jacobians.
    // We first 'partially project' and then complete the projection with the 'sumAcross'.
    // This is an optimization: originally this was 3 full projects and a gather.
    // This is equivalent to 'multiplying a row of the constraint matrix with the impulse vector and dividing by the diagonal element of the row'.
    // But because we do block PGS, this is 'multiplying a group of row (3 of them int is case) of the constraint matrix with the impulse vector and
    // multiplying by the inverse of the diagonal block'. Physically the result of this is the relative change in velocity due to currently computed
    // constraint impulses. We store the result as: projectedVirD = ( projection vel stage dim 0, projection vel stage dim 1, projection vel stage dim
    // 2, * )
...
template<class Type>
static void updateConstraintDim3OrCollision(ConstraintVariables* __restrict _velStage, ConstraintVariables* __restrict _posStage,
    VirtualDisplacementPOD* __restrict _virDVel, VirtualDisplacementPOD* __restrict _virDPos,
    const ConstraintJacobianPair* __restrict _preconditionedJacobiansVel, const ConstraintJacobianPair* __restrict _preconditionedJacobiansPos,
    const EffectiveMassPair* __restrict _effectiveMassesVel, const EffectiveMassPair* __restrict _effectiveMassesPos, const BodyPairIndices& _pair)
{
...
// Kernel main loop
//
void PGSSolveKernel(ConstraintVariables* __restrict _velStage, ConstraintVariables* __restrict _posStage, VirtualDisplacementArray& _virDVel,
    VirtualDisplacementArray& _virDPos, size_t _constraintCount, size_t _collisionCount, const boost::uint8_t* _dimensions,
    const BodyPairIndices* _pairs, const ConstraintJacobianPair* _preconditionedJacobiansVelStage,
    const ConstraintJacobianPair* _preconditionedJacobiansPosStage, const EffectiveMassPair* _effectiveMassesVelStage,
    const EffectiveMassPair* _effectiveMassesPosStage, const SolverConfig& _config)
{
    AYAPROFILER_SCOPE("Physics", "PGSSolveKernel");
    ...
    for (boost::uint32_t k = 0; k < _config.pgsIterations; k++)
    {
        ...
        for (size_t c = 0; c < pureConstraintCount; c++)
        {
            ...
            // It is equivalent to:
            // for( i = 0; i < dimC; i++ )
            //   PGSComputeConstraint1( velStage+i, posStage+i, _virDVel.data.data(), _virDPos.data.data(), precondJacobianVel+i,
            //   precondJacobianPos+i, effMassesVel+i, effMassesPos+i, _pairs[c] );
            switch (dimC)
            {
            case 1:
                updateConstraintDim1(velStage, posStage, _virDVel.getData(), _virDPos.getData(), precondJacobianVel, precondJacobianPos, effMassesVel,
                    effMassesPos, _pairs[c]);
                break;
            case 2:
                updateConstraintDim2(velStage, posStage, _virDVel.getData(), _virDPos.getData(), precondJacobianVel, precondJacobianPos, effMassesVel,
                    effMassesPos, _pairs[c]);
                break;
            case 3:
                updateConstraintDim3OrCollision<ConstraintType>(velStage, posStage, _virDVel.getData(), _virDPos.getData(), precondJacobianVel,
                    precondJacobianPos, effMassesVel, effMassesPos, _pairs[c]);
                break;
            default:
                break;
            }
        ...
        for (size_t c = pureConstraintCount; c < _constraintCount; c++)
        {
            ...
            updateConstraintDim3OrCollision<CollisionType>(velStage, posStage, _virDVel.getData(), _virDPos.getData(), precondJacobianVel,
                precondJacobianPos, effMassesVel, effMassesPos, _pairs[c]);
...
static SIMD_FORCE_INLINE void updateConstraintSingleStageDim1(ConstraintVariables* __restrict _vars, float* __restrict _solutionDeltas,
    float* __restrict _solutionDeltaDeltas, VirtualDisplacementPOD* __restrict _virD,
    const ConstraintJacobianPair* __restrict _preconditionedJacobians, const EffectiveMassPair* __restrict _effectiveMasses,
    const BodyPairIndices& _pair, float _smoothingD, float _smoothingDD)
{
    // Project the virtual displacements onto the preconditioned Jacobians
    // This is equivalent to 'multiplying a row of the constraint matrix with the impulse vector and dividing by the diagonal element of the row'.
    // Physically the result of this is the relative change in velocity due to currently computed constraint impulses.
...
template<class StoreDeltasSelector>
static void singleStageNoFrictionKernelInternal(ConstraintVariables* __restrict _vars, VirtualDisplacementArray& _virD,
    float* __restrict _solutionDeltas, float* __restrict _solutionDeltaDeltas, size_t _constraintCount, size_t _collisionCount,
    const boost::uint8_t* _dimensions, const BodyPairIndices* _pairs, const ConstraintJacobianPair* _preconditionedJacobians,
    const EffectiveMassPair* _effectiveMasses, const SolverConfig& _config, float _smoothingD, float _smoothingDD)
{
    size_t pureConstraintCount = _constraintCount - _collisionCount;
    ...
    for (boost::uint32_t k = 0; k < _config.pgsIterations; k++)
    {
        ...
        for (size_t c = 0; c < pureConstraintCount; c++)
        {
            ...
            {
                updateConstraintSingleStageDim1<StoreDeltasSelector>(vars + i, solutionDeltas + i, solutionDeltaDeltas + i, _virD.getData(),
                    precondJacobian + i, effMasses + i, _pairs[c], _smoothingD, _smoothingDD);
            }
        ...
        for (size_t c = pureConstraintCount; c < _constraintCount; c++)
        {
            ...
            updateConstraintSingleStageDim1<StoreDeltasSelector>(
                vars, solutionDeltas, solutionDeltaDeltas, _virD.getData(), precondJacobian, effMasses, _pairs[c], _smoothingD, _smoothingDD);
...
// Error computation
//
void PGSSolveKernelComputeErrors(ArrayBase<float>& _impulseDeltas, ArrayBase<float>& _impulseDeltaDeltas, ArrayBase<ConstraintVariables>& _vars,
    VirtualDisplacementArray& _virD, size_t _constraintCount, size_t _collisionCount, size_t _bodyCount, const boost::uint8_t* _dimensions,
    const BodyPairIndices* _pairs, const ConstraintJacobianPair* _jacobians, const ConstraintJacobianPair* _preconditionedJacobians,
    const EffectiveMassPair* _effectiveMasses, const SolverConfig& _config)
{
    AYAPROFILER_SCOPE("Physics", "PGSSolveKernelComputeErrors");

    SolverConfig localConfig = _config;
    static float smoothingD = 0.95f;
    static float smoothingDD = 0.0f;

    singleStageNoFrictionKernelInternal<StoreDeltaImpulses>(_vars.data(), _virD, _impulseDeltas.data(), _impulseDeltaDeltas.data(), _constraintCount,
        _collisionCount, _dimensions, _pairs, _preconditionedJacobians, _effectiveMasses, localConfig, smoothingD, smoothingDD);
}
