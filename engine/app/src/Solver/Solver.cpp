...
#include "Solver/SolverSerializer.hpp"
#include "Solver/SolverProfiler.hpp"
...
static AYA_SIMD_INLINE int getBodyIndex(ArrayDynamic<SimBody*>& _anchoredBodyList, BodyIndexation& _allBodyIndexation, Body* _body)
{
...
//
static boost::uint32_t gatherConstraintPairData(ArrayBase<BodyPairIndices>& _constrainedSimBodyPairs, ArrayBase<boost::uint8_t>& _dimensions,
    ArrayBase<boost::uint32_t>& _offsets, ArrayDynamic<SimBody*>& _anchoredBodyList, BodyIndexation& _bodyIndexation,
    boost::uint32_t& _totalBlockSize, boost::uint32_t& _collisionCount, const ArrayBase<Constraint*>& _constraints)
{
    AYAPROFILER_SCOPE("Physics", "gatherConstraintPairData");
...
static AYA_SIMD_INLINE void integrateVelocities(SolverBodyDynamicProperties& _variableData, const SolverBodyMassAndInertia& _massAndInertia,
    const Vector3& _externalImpulse, const Vector3& _externalAngularImpulse, float _dt, const SolverConfig& _config)
{
    ...
    AYAASSERT_VERY_FAST(!Aya::Math::isNanInfVector3(_variableData.integratedLinearVelocity));
    ...
    _variableData.integratedAngularVelocity =
        expf(-_config.angularDamping * _dt) * _variableData.angularVelocity + invInertia * _externalAngularImpulse;
    AYAASSERT_VERY_FAST(!Aya::Math::isNanInfVector3(_variableData.integratedAngularVelocity));
}
...
static void integrateVelocitiesAndInitSimulatedObjects(ArrayDynamic<SolverBodyStaticProperties>& _bodyStaticData,
    ArrayDynamic<SolverBodyDynamicProperties>& _bodyVariableData, ArrayDynamic<SolverBodyMassAndInertia>& _massAndInertia,
    ArrayDynamic<float>& _effectiveMassMultipliers, const ArrayBase<SimBody*>& _simBodies, float _dt, const SolverConfig& _config)
{
    AYAPROFILER_SCOPE("Physics", "integrateVelocitiesAndInitSimulatedObjects");
...
//
void PGSSolver::initAnchoredObjects(ArrayDynamic<SolverBodyDynamicProperties>& _dynamicProps, ArrayDynamic<SolverBodyStaticProperties>& _staticProps,
    ArrayDynamic<SolverBodyMassAndInertia>& _massAndInertia, ArrayDynamic<float>& _effectiveMassMultipliers,
    VirtualDisplacementArray& _velStageVirDisplacements, VirtualDisplacementArray& _posStageVirDisplacements, int offsetToAnchoredObjects,
    const ArrayBase<SimBody*>& _anchoredBodyList, const SolverConfig& _config) const
{
    AYAPROFILER_SCOPE("Physics", "initAnchoredObjects");
...
static AYA_SIMD_INLINE void integratePositions(SolverBodyDynamicProperties& _dynamicProps, const SolverBodyStaticProperties& _staticProps,
    const VirtualDisplacementPOD& _velStageVirD, const VirtualDisplacementPOD& _posStageVirD, float _dt)
{
    AYAASSERT_VERY_FAST(!Aya::Math::isNanInfVector3(_velStageVirD.lin));
    AYAASSERT_VERY_FAST(!Aya::Math::isNanInfVector3(_velStageVirD.ang));
    AYAASSERT_VERY_FAST(!Aya::Math::isNanInfVector3(_posStageVirD.lin));
    AYAASSERT_VERY_FAST(!Aya::Math::isNanInfVector3(_posStageVirD.ang));
    ...
    AYAASSERT_VERY_FAST(!Aya::Math::isNanInfVector3(_dynamicProps.position));
    AYAASSERT_VERY_FAST(!Aya::Math::isNanInfVector3(_dynamicProps.orientation.row(0)));
    AYAASSERT_VERY_FAST(!Aya::Math::isNanInfVector3(_dynamicProps.orientation.row(1)));
    AYAASSERT_VERY_FAST(!Aya::Math::isNanInfVector3(_dynamicProps.orientation.row(2)));
}

void PGSSolver::integratePositionsAndUpdateSimBodies(SimBody* const* _simBodies, SolverBodyDynamicProperties* const _bodyVariableData,
    const SolverBodyStaticProperties* const _bodyStaticData, size_t _simBodyCount, const VirtualDisplacementArray& _velocityDeltas,
    const VirtualDisplacementArray& _positionDeltas, float _dt)
{
    AYAPROFILER_SCOPE("Physics", "integratePositionsAndUpdateSimBodies");
    ...
    for (int index = 0; index < (int)_simBodyCount; index++)
    {
        ...
        _simBodies[index]->updateFromSolver(_bodyVariableData[index].position, _bodyVariableData[index].orientation,
            _bodyVariableData[index].linearVelocity, _bodyVariableData[index].angularVelocity);
    }
}

static AYA_SIMD_INLINE void integratePositionsIgnoreVelocities(SolverBodyDynamicProperties& _dynamicProps,
    const SolverBodyStaticProperties& _staticProps, const VirtualDisplacementPOD& _posStageVirD, float _dt)
{
    AYAASSERT_VERY_FAST(!Aya::Math::isNanInfVector3(_posStageVirD.lin));
    AYAASSERT_VERY_FAST(!Aya::Math::isNanInfVector3(_posStageVirD.ang));
...
void PGSSolver::integratePositionsIgnoreVelocitiesAndUpdateSimBodies(SimBody* const* _simBodies, SolverBodyDynamicProperties* const _bodyVariableData,
    const SolverBodyStaticProperties* const _bodyStaticData, size_t _simBodyCount, const VirtualDisplacementArray& _positionDeltas, float _dt)
{
    AYAPROFILER_SCOPE("Physics", "integratePositionsIgnoreVelocitiesAndUpdateSimBodies");
...
void PGSSolver::detectInconsistentConstraints(VirtualDisplacementArray& _positionDeltas, ArrayBase<ConstraintVariables>& _positionStage,
    const ArrayBase<ConstraintJacobianPair>& _jacobians, const ArrayBase<ConstraintJacobianPair>& _preconditionedJacobiansPosStage,
    const ArrayBase<EffectiveMassPair>& _effectiveMassesPosStage, const ArrayBase<float>& _sorPos, const ArrayBase<boost::uint8_t>& _dimensions,
    const ArrayBase<boost::uint32_t>& _offsets, const ArrayBase<BodyPairIndices>& _simBodyPairs, const ArrayBase<Constraint*>& _constraints,
    size_t _collisionCount, const SolverConfig& _solverConfig)
{
    ...
    for (;; iteration++)
    {
        ...
        PGSSolveKernelComputeErrors(impulseDeltas, impulseDeltaDeltas, _positionStage, _positionDeltas, _constraints.size(), _collisionCount,
            _positionDeltas.getSize(), _dimensions.data(), _simBodyPairs.data(), _jacobians.data(), _preconditionedJacobiansPosStage.data(),
        ...
        for (size_t i = 0; i < _constraints.size(); i++)
        {
            ...
            switch (_constraints[i]->testPGSConvergence(
                displacements.data() + offset, residuals.data() + offset, deltaResiduals.data() + offset, _solverConfig))
            {
    ...
    out << "E = " << energy << ", dE = " << dEnergy << ", gamma = " << 100.0 * dEnergy / dEnergyAverage - 100.0 << "%%, "
        << 100.0 * d2Energy / dEnergyAverage << "%% ";
    Aya::StandardOut::singleton()->printf(Aya::MESSAGE_OUTPUT, out.str().c_str());
#endif
    ...
    {
        boost::uint32_t offset = _offsets[i];
        Constraint::Convergence convergence = _constraints[i]->testPGSConvergence(
            displacements.data() + offset, residuals.data() + offset, deltaResiduals.data() + offset, _solverConfig);
...
static void addAdjacentBodies(ArrayDynamic<boost::uint32_t>& _islandSimBodies, ArrayDynamic<boost::uint32_t>& _islandConstraints,
    ArrayBase<boost::uint8_t>& _visitedBodies, ArrayBase<boost::uint8_t>& _visitedConstraints, ArrayDynamic<boost::int32_t>& _simBodyRecursionStack,
    boost::int32_t _rootBodyIndex, const ArrayBase<std::pair<boost::int32_t, boost::int32_t>>& _bodyIndexPairs,
    const MultiIndexList<boost::uint32_t>& _adjacencyLists)
{
    ...
    for (_simBodyRecursionStack.push_back(_rootBodyIndex); _simBodyRecursionStack.size() > 0;)
    {
        ...
        for (size_t it = _adjacencyLists.getFirst(bodyIndex); _adjacencyLists.isValid(it); it = _adjacencyLists.getNext(it))
        {
...
static void breakIntoIslands(ArrayDynamic<SimBody*>& _islandSimBodies, ArrayDynamic<Constraint*>& _islandConstraints,
    ArrayDynamic<std::pair<boost::uint32_t, boost::uint32_t>>& _islandSizes, const boost::unordered_set<SimBody*>& _simBodies,
    const ArrayBase<Constraint*>& _constraints)
{
    AYAPROFILER_SCOPE("Physics", "breakIntoIslands");
    // Index the bodies
    ...
    for (boost::uint32_t bodyIndex = 0; bodyIndex < simBodyList.size(); bodyIndex++)
    {
        if (!visitedBodies[bodyIndex])
        {
            ...
            addAdjacentBodies(indexedIslandBodies, indexedIslandConstraints, visitedBodies, visitedConstraints, tempBuffer, bodyIndex, bodyIndexPairs,
                adjacencyLists);

            // Store the island size
            _islandSizes.push_back(std::pair<boost::uint32_t, boost::uint32_t>(
                static_cast<uint32_t>(indexedIslandBodies.size()), static_cast<uint32_t>(indexedIslandConstraints.size())));
...
void PGSSolver::solveInternal(const std::vector<ContactConnector*>& _contactConnectors, float _dt, boost::uint64_t debugTime, bool _throttled,
    const SolverConfig& _solverConfig)
{
    AYAPROFILER_SCOPE("Physics", "PGSSolver::solve");
    ...
    {
        AYAPROFILER_SCOPE("Physics", "fillConstraintSet");
        ...
        if (_throttled)
        {
            ...
            for (const auto& it : pureConstraintSet)
            {
                if (highPrioritySimBodies.find(it.second->getBodyA()->getRootSimBody()) == highPrioritySimBodies.end() &&
                    ((it.second->getBodyB() == NULL) ||
                        highPrioritySimBodies.find(it.second->getBodyB()->getRootSimBody()) == highPrioritySimBodies.end()))
                {
                    continue;
                }
...
void PGSSolver::solveIsland(
    const ArrayDynamic<Constraint*>& _constraints, const ArrayDynamic<SimBody*>& _simBodies, float _dt, const SolverConfig& solverConfig)
{
    ...
    {
        AYAPROFILER_SCOPE("Physics", "generateBodyIndices");
    ...
    auto totalDimension =
        gatherConstraintPairData(simBodyPairs, dimensions, offsets, anchoredBodyList, bodyIndexation, totalBlockSize, collisionCount, _constraints);
    ...
    integrateVelocitiesAndInitSimulatedObjects(
        bodyStaticData, bodyVariableData, massAndInertia, effectiveMassMultipliers, simBodyList, _dt, solverConfig);
    ...
    initAnchoredObjects(bodyVariableData, bodyStaticData, massAndInertia, effectiveMassMultipliers, velocityDeltasSIMD, positionDeltasSIMD,
        (int)simBodyList.size(), anchoredBodyList, solverConfig);
    ...
    {
        AYAPROFILER_SCOPE("Physics", "buildConstraintEquations");
        ...
        for (size_t i = 0; i < _constraints.size(); i++)
        {
            ...
            _constraints[i]->restoreCacheAndBuildEquation(jacobians.data() + currentOffset, velocityStage.data() + currentOffset,
                positionStage.data() + currentOffset, sorVel.data() + currentOffset, sorPos.data() + currentOffset, useBlock.data() + currentOffset,
    ...
    PGSComputeEffectiveMasses(effectiveMassesVel.data(), effectiveMassesPos.data(), _constraints.size(), dimensions.data(), jacobians.data(),
        simBodyPairs.data(), massAndInertia.data(), solverConfig);
    ...
    PGSPreconditionConstraintEquations(preconditionedJacobiansVelStage.data(), preconditionedJacobiansPosStage.data(), velocityStage.data(),
        positionStage.data(), _constraints.size(), dimensions.data(), useBlock.data(), sorVel.data(), sorPos.data(), jacobians.data(),
        effectiveMassesVel.data(), effectiveMassesPos.data());
    ...
    PGSApplyEffectiveMassMultipliers(effectiveMassesVel.data(), effectiveMassesPos.data(), _constraints.size(), dimensions.data(),
        effectiveMassMultipliers.data(), simBodyPairs.data(), solverConfig);
    ...
    PGSInitVirtualDisplacements(velocityDeltasSIMD, positionDeltasSIMD, effectiveMassesVel.data(), effectiveMassesPos.data(), _constraints.size(),
        dimensions.data(), velocityStage.data(), positionStage.data(), simBodyPairs.data(), solverConfig);
    ...
    if (solverConfig.inconsistentConstraintDetectorEnabled)
    {
        detectInconsistentConstraints(positionDeltasSIMD, positionStage, jacobians, preconditionedJacobiansPosStage, effectiveMassesPos, sorPos,
            dimensions, offsets, simBodyPairs, _constraints, collisionCount, solverConfig);
    }
    else
    {
        PGSSolveKernel(velocityStage.data(), positionStage.data(), velocityDeltasSIMD, positionDeltasSIMD, _constraints.size(), collisionCount,
            dimensions.data(), simBodyPairs.data(), preconditionedJacobiansVelStage.data(), preconditionedJacobiansPosStage.data(),
    ...
    {
        AYAPROFILER_SCOPE("Physics", "updateConstraintCache");
        ...
        for (size_t i = 0; i < _constraints.size(); i++)
        {
            ...
            _constraints[i]->cache(velocityStage.data() + currentOffset, positionStage.data() + currentOffset, sorVel.data() + currentOffset,
                sorPos.data() + currentOffset, solverConfig);
        }
    }
    ...
    if (solverConfig.updateSimBodies)
    {
        ...
        {
            integratePositionsAndUpdateSimBodies(
                simBodyList.data(), bodyVariableData.data(), bodyStaticData.data(), simBodyList.size(), velocityDeltasSIMD, positionDeltasSIMD, _dt);
        }
        else
        {
            integratePositionsIgnoreVelocitiesAndUpdateSimBodies(
                simBodyList.data(), bodyVariableData.data(), bodyStaticData.data(), simBodyList.size(), positionDeltasSIMD, _dt);
        }
    }
...
void PGSSolver::solveLegacy(const std::vector<ContactConnector*>& _contactConnectors, float _dt, boost::uint64_t debugTime, bool _throttled)
{
    ...
    boost::uint32_t totalBlockSize;
    auto totalDimension =
        gatherConstraintPairData(simBodyPairs, dimensions, offsets, anchoredBodyList, bodyIndexation, totalBlockSize, collisionCount, constraints);
    gatherCollisionsProfiler.end();
    ...
    ArrayDynamic<boost::uint8_t> useBlock(totalDimension, ArrayNoInit());
    {
        ...
        for (size_t i = 0; i < constraints.size(); i++)
        {
            ...
            const auto& bodyB = bodyVariableData[simBodyPairs[i].second];
            boost::uint32_t currentOffset = offsets[i];
            constraints[i]->restoreCacheAndBuildEquation(jacobians.data() + currentOffset, velocityStage.data() + currentOffset,
                positionStage.data() + currentOffset, sorVel.data() + currentOffset, sorPos.data() + currentOffset, useBlock.data() + currentOffset,
                bodyA, bodyB, solverConfig, _dt);
        }
    }
    ...
    ArrayDynamic<EffectiveMassPair> effectiveMassesVel(totalDimension, ArrayNoInit(), defaultAlignment);
    ArrayDynamic<EffectiveMassPair> effectiveMassesPos(totalDimension, ArrayNoInit(), defaultAlignment);
    PGSComputeEffectiveMasses(effectiveMassesVel.data(), effectiveMassesPos.data(), constraints.size(), dimensions.data(), jacobians.data(),
        simBodyPairs.data(), massAndInertia.data(), solverConfig);
    computeEffectiveMassesProfiler.end();
    ...
    ArrayDynamic<ConstraintJacobianPair> preconditionedJacobiansPosStage(totalDimension, ArrayNoInit(), defaultAlignment);
    PGSPreconditionConstraintEquations(preconditionedJacobiansVelStage.data(), preconditionedJacobiansPosStage.data(), velocityStage.data(),
        positionStage.data(), constraints.size(), dimensions.data(), useBlock.data(), sorVel.data(), sorPos.data(), jacobians.data(),
        effectiveMassesVel.data(), effectiveMassesPos.data());
    preconditioningProfiler.end();
    ...
    //
    multiplyEffectiveMassMultipliersProfiler.start();
    PGSApplyEffectiveMassMultipliers(effectiveMassesVel.data(), effectiveMassesPos.data(), constraints.size(), dimensions.data(),
        effectiveMassMultipliers.data(), simBodyPairs.data(), solverConfig);
    multiplyEffectiveMassMultipliersProfiler.end();
    ...
    //
    initVirDProfiler.start();
    PGSInitVirtualDisplacements(velocityDeltasSIMD, positionDeltasSIMD, effectiveMassesVel.data(), effectiveMassesPos.data(), constraints.size(),
        dimensions.data(), velocityStage.data(), positionStage.data(), simBodyPairs.data(), solverConfig);
    initVirDProfiler.end();
    ...
    //
    kernelProfiler.start();
    PGSSolveKernel(velocityStage.data(), positionStage.data(), velocityDeltasSIMD, positionDeltasSIMD, constraints.size(), collisionCount,
        dimensions.data(), simBodyPairs.data(), preconditionedJacobiansVelStage.data(), preconditionedJacobiansPosStage.data(),
        effectiveMassesVel.data(), effectiveMassesPos.data(), solverConfig);
    kernelProfiler.end();
    ...
    writeCacheProfiler.start();
    {
        ...
        for (size_t i = 0; i < constraints.size(); i++)
        {
            ...
            constraints[i]->cache(velocityStage.data() + currentOffset, positionStage.data() + currentOffset, sorVel.data() + currentOffset,
                sorPos.data() + currentOffset, solverConfig);
        }
    }
    writeCacheProfiler.end();
...
void PGSSolver::addConstraint(Constraint* _constraint)
{
    ...
    AYAASSERT(_constraint->getType() != Constraint::Types_Collision);
...
void PGSSolver::removeContactManifold(boost::uint64_t _uidA, boost::uint64_t _uidB)
{
    ...
    auto it = contactManifolds.find(pair);
    AYAASSERT(it != contactManifolds.end());
    if (it != contactManifolds.end())
    {
        it->second->referenceCount--;
...
static AYA_SIMD_INLINE void updateCollision(ConstraintCollision& _collision, const OrderedConnector& _inputConnector)
{
    ...
    AYAASSERT_VERY_FAST(!Aya::Math::isNanInf(params.normal.length()) && std::abs(1.0f - params.normal.length()) < 0.1f);
...
ContactManifold* PGSSolver::updateContactManifold(const BodyUIDPair& _pairId, const ArrayBase<OrderedConnector>& _inputManifold)
{
    ...
    AYAASSERT(cachedManifoldIt != contactManifolds.end());
    if (cachedManifoldIt != contactManifolds.end())
    {
        ...
        {
            ConstraintCollision* collision = new ConstraintCollision(cleanManifold[newContactIndex].connector->getBody(Connector::body0),
...
{
public:
    OrderedConnectorPair(const BodyUIDPair& p, const OrderedConnector& c)
        : std::pair<BodyUIDPair, OrderedConnector>(p, c)
    {
    }
...
size_t PGSSolver::addContactConnectors(ArrayDynamic<ContactManifold*>& _activeManifolds, const std::vector<ContactConnector*>& _connectors,
    const boost::unordered_set<SimBody*>& _simBodies)
{
    AYAPROFILER_SCOPE("Physics", "addContactConnectors");
