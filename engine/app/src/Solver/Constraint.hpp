...
public:
    ...
    static AYA_SIMD_INLINE void setReaction(ConstraintVariables* _vars, const Vector3& _r)
    {
    ...
    static AYA_SIMD_INLINE void setReaction(ConstraintVariables* _vars, float x, float y)
    {
        _vars[0].reaction = x;
        _vars[1].reaction = y;
    }

    static AYA_SIMD_INLINE void setImpulse(ConstraintVariables* _vars, const Vector3& _i)
    {
    ...
    static AYA_SIMD_INLINE void setImpulse(ConstraintVariables* _vars, float x, float y)
    {
        _vars[0].impulse = x;
        _vars[1].impulse = y;
    }

    static AYA_SIMD_INLINE void setMinImpulses(ConstraintVariables* _vars, const Vector3& _min)
    {
    ...
    static AYA_SIMD_INLINE void setMinImpulses(ConstraintVariables* _vars, float x, float y)
    {
        _vars[0].minImpulseValue = x;
        _vars[1].minImpulseValue = y;
    }

    static AYA_SIMD_INLINE void setMaxImpulses(ConstraintVariables* _vars, const Vector3& _max)
    {
    ...
    static AYA_SIMD_INLINE void setMaxImpulses(ConstraintVariables* _vars, float x, float y)
    {
        _vars[0].maxImpulseValue = x;
        _vars[1].maxImpulseValue = y;
    }

    static AYA_SIMD_INLINE void gatherComponents(
        simd::v4f& _impulses, simd::v4f& _reactions, simd::v4f& _min, simd::v4f& _max, const ConstraintVariables& _vars0)
    {
    ...
    static AYA_SIMD_INLINE void gatherComponents(simd::v4f& _impulses, simd::v4f& _reactions, simd::v4f& _min, simd::v4f& _max,
        const ConstraintVariables& _vars0, const ConstraintVariables& _vars1)
    {
        transpose2x4(_min, _max, _reactions, _impulses, simd::v4f(_vars0.v), simd::v4f(_vars1.v));
    }

    static AYA_SIMD_INLINE void gatherComponents(simd::v4f& _impulses, simd::v4f& _reactions, simd::v4f& _min, simd::v4f& _max,
        const ConstraintVariables& _vars0, const ConstraintVariables& _vars1, const ConstraintVariables& _vars2)
    {
        transpose3x4(_min, _max, _reactions, _impulses, (simd::v4f)_vars0.v, (simd::v4f)_vars1.v, (simd::v4f)_vars2.v);
    }

    static AYA_SIMD_INLINE void gatherComponents(simd::v4f& _impulses, simd::v4f& _reactions, simd::v4f& _min, simd::v4f& _max,
        const ConstraintVariables& _vars0, const ConstraintVariables& _vars1, const ConstraintVariables& _vars2, const ConstraintVariables& _vars3)
    {
...
public:
    ...
    void cache(const ConstraintVariables& _velocityStage, const ConstraintVariables& _positionStage, float _sorVel, float _sorPos, bool _isCollision,
...
public:
    ...
    inline void restoreCacheAndBuildEquation(ConstraintJacobianPair* _jacobian, ConstraintVariables* _velocityStage,
        ConstraintVariables* _positionStage, float* _sorVel, float* _sorPos, boost::uint8_t* _useBlock, const SolverBodyDynamicProperties& _bodyA,
        const SolverBodyDynamicProperties& _bodyB, const SolverConfig& _solverConfig, float _dt);

    // Write into the constraint cache
    // This should only be called by the solver
    inline void cache(const ConstraintVariables* _velocityStage, const ConstraintVariables* _positionStage, const float* _sorVel,
        const float* _sorPos, const SolverConfig& _config);

    // Each breakable constraint will need to implement this, and return /true/ if the constraint changes state to broken
    // This should only be called by the solver
    inline void updateBrokenState(const ConstraintVariables* _velocityStage, const ConstraintVariables* _positionStage, const SolverConfig& _config);

    void setBodyA(Body* _a)
    {
        bodyA = _a;
    }
    void setBodyB(Body* _b)
    {
        bodyB = _b;
    }
    ...
    void setUID(boost::uint64_t _index)
    {
        uid = _index;
    }
    ...
    virtual Convergence testPGSConvergence(
        const float* _disp, const float* _residuals, const float* _deltaResiduals, const SolverConfig& _solverConfig)
    {
...
private:
    ...
    virtual void buildEquation(ConstraintJacobianPair* _jacobian, boost::uint8_t* _useBlock, ConstraintVariables* _velocityStage,
        ConstraintVariables* _positionStage, const SolverBodyDynamicProperties& _bodyA, const SolverBodyDynamicProperties& _bodyB,
        const SolverConfig& _config, float _dt) = 0;

    // Each breakable constraint will need to implement this, and return /true/ if the constraint changes state to broken
    virtual bool computeBrokenState(
        const ConstraintVariables* _velocityStage, const ConstraintVariables* _positionStage, const SolverConfig& _config) const
    {
        return false;
    }
...
//
inline Constraint::Constraint(Types _type, Body* _bodyA, Body* _bodyB, uint8_t _dimensions)
    : type(_type)
...
{
    AYAASSERT(!Aya::Math::isNanInf(_vars.impulse));
    AYAASSERT(!Aya::Math::isNanInf(_vars.reaction));
    AYAASSERT(!Aya::Math::isNan(_vars.minImpulseValue));
    AYAASSERT(!Aya::Math::isNan(_vars.maxImpulseValue));
}

static inline void checkJacobian(const ConstraintJacobianPair& _j)
{
    AYAASSERT(!Aya::Math::isNanInfVector3(_j.a.lin));
    AYAASSERT(!Aya::Math::isNanInfVector3(_j.b.lin));
    AYAASSERT(!Aya::Math::isNanInfVector3(_j.a.ang));
    AYAASSERT(!Aya::Math::isNanInfVector3(_j.b.ang));
}
#endif

AYA_SIMD_INLINE void Constraint::restoreCacheAndBuildEquation(ConstraintJacobianPair* __restrict _jacobian, ConstraintVariables* __restrict _varsVel,
    ConstraintVariables* __restrict _varsPos, float* __restrict _sorVel, float* __restrict _sorPos, boost::uint8_t* _useBlock,
    const SolverBodyDynamicProperties& _bodyA, const SolverBodyDynamicProperties& _bodyB, const SolverConfig& _solverConfig, float _dt)
{
...
AYA_SIMD_INLINE void Constraint::updateBrokenState(
    const ConstraintVariables* _velocityStage, const ConstraintVariables* _positionStage, const SolverConfig& _config)
{
    if (!broken)
    {
...
inline void Constraint::cache(const ConstraintVariables* _velocityStage, const ConstraintVariables* _positionStage, const float* _sorVel,
    const float* _sorPos, const SolverConfig& _config)
{
...
{
public:
    ConstraintBallInSocket(Body* _bodyA, Body* _bodyB)
        : Constraint(Constraint::Types_BallInSocket, _bodyA, _bodyB, 3)
    {
    }
    void buildEquation(ConstraintJacobianPair* _jacobian, boost::uint8_t* _useBlock, ConstraintVariables* _velocityStage,
        ConstraintVariables* _positionStage, const SolverBodyDynamicProperties& _bodyA, const SolverBodyDynamicProperties& _bodyB,
        const SolverConfig& _config, float _dt) override;

    inline void setPivotA(const Vector3& _pivotA)
    {
        pointA = _pivotA;
    }
    inline void setPivotB(const Vector3& _pivotB)
    {
        pointB = _pivotB;
    }

    Convergence testPGSConvergence(
        const float* _disp, const float* _residuals, const float* _deltaResiduals, const SolverConfig& _solverConfig) override;
...
{
public:
    ConstraintLegacyBreakableBallInSocket(Body* _bodyA, Body* _bodyB)
        : Constraint(Constraint::Types_LegacyBreakableBallInSocket, _bodyA, _bodyB, 3)
        , pointA(0.0f)
        , pointB(0.0f)
        , broken(false)
        , maxNormalForce(std::numeric_limits<float>::infinity())
    {
    ...
    void setPivotA(const Vector3& _pivotA)
    {
        pointA = _pivotA;
    }
    void setPivotB(const Vector3& _pivotB)
    {
        pointB = _pivotB;
    }
    void setNormalOnA(const Vector3& _normal);
    void setMaxNormalForce(float _maxForce)
    {
        maxNormalForce = _maxForce;
    }
    bool computeBrokenState(
        const ConstraintVariables* _velocityStage, const ConstraintVariables* _positionStage, const SolverConfig& _config) const override;
...
{
public:
    ConstraintAngularVelocity(Body* _bodyA, Body* _bodyB)
        : Constraint(Constraint::Types_AngularVelocity, _bodyA, _bodyB, 1)
        , maxForce(0.0f)
    ...
    void setDesiredAngularVelocity(float _v)
    {
        desiredAngularVelocity = _v;
    }
    void setMaxForce(float _f)
    {
        maxForce = _f;
    }
...
{
public:
    ConstraintLinearVelocity(Body* _bodyA, Body* _bodyB)
        : Constraint(Constraint::Types_LinearVelocity, _bodyA, _bodyB, 3)
        , maxForce(0.0f)
    ...
    void setDesiredVelocity(const Vector3& _v)
    {
        desiredVelocity = _v;
    }
    void setMaxForce(const Vector3& _f)
    {
        maxForce = _f;
    }
...
{
public:
    ConstraintLinearSpring(Body* _bodyA, Body* _bodyB)
        : Constraint(Constraint::Types_LinearSpring, _bodyA, _bodyB, 3)
        , pivotA(0.0f)
        , pivotB(0.0f)
        , maxForce(0.0f)
        , p(0.0f)
...
{
public:
    ConstraintAchievePosition(Body* _bodyA, Body* _bodyB)
        : Constraint(Constraint::Types_AchievePosition, _bodyA, _bodyB, 3)
        , maxForce(0.0f)
        , targetVelocity(0.0f)
    {
    }
    ...
    void setTargetVelocity(const Vector3& _v)
    {
        targetVelocity = _v;
    }
    void setMaxForce(const Vector3& _f)
    ...
    void setMinForce(const Vector3& _f)
    {
        minForce = _f;
    }
...
{
public:
    ConstraintBodyAngularVelocity(Body* _bodyA, Body* _bodyB)
        : Constraint(Constraint::Types_BodyAngularVelocity, _bodyA, _bodyB, 3)
        , targetAngularVelocity(0.0f)
        , maxTorque(0.0f)
        , minTorque(0.0f)
        , useIntegratedVelocities(false)
    {
    }
    ...
    void setMaxTorque(const Vector3& v)
    {
        maxTorque = v;
    }
    void setMinTorque(const Vector3& v)
    {
        minTorque = v;
    }
    void setUseIntegratedVelocities(bool flag)
    {
        useIntegratedVelocities = flag;
    }
...
{
public:
    ConstraintLegacyAngularVelocity(Body* _bodyA, Body* _bodyB)
        : Constraint(Constraint::Types_LegacyAngularVelocity, _bodyA, _bodyB, 3)
        , targetAngularVelocity(0.0f)
        , maxTorque(0.0f)
        , minTorque(0.0f)
        , useIntegratedVelocities(false)
...
{
public:
    ConstraintCollision(Body* _bodyA, Body* _bodyB)
        : Constraint(Constraint::Types_Collision, _bodyA, _bodyB, 3)
    {
    ...
    void setPointA(const Vector3& _pointA)
    {
        pointA = _pointA;
    }
