...
public:
    ...
    void setInconsistentConstraintDetectorEnabled(bool value)
    {
        inconsistentConstraintDetectorEnabled = value;
    }
    void setPhysicsAnalyzerBreakOnIssue(bool value)
    {
        physicsAnalyzerBreakOnIssue = value;
    }
    bool getPhysicsAnalyzerBreakOnIssue() const
    {
        return physicsAnalyzerBreakOnIssue;
    }

    const ArrayBase<InconsistentBodyPair>& getInconsistentBodyPairs() const
    {
        return inconsistentBodyPairs;
    }
    const ArrayBase<ArrayDynamic<boost::uint64_t>>& getInconsistentBodies() const
    {
        return inconsistentBodies;
    }
...
private:
    void solveInternal(
        const std::vector<ContactConnector*>& connectors, float dt, boost::uint64_t debugTime, bool throttled, const SolverConfig& _solverConfig);
    void solveIsland(
        const ArrayDynamic<Constraint*>& constraints, const ArrayDynamic<SimBody*>& selectedSimBodies, float _dt, const SolverConfig& _solverConfig);
    ContactManifold* updateContactManifold(const BodyUIDPair& _pairId, const ArrayBase<OrderedConnector>& _manifold);
    size_t addContactConnectors(ArrayDynamic<ContactManifold*>& _activeManifolds, const std::vector<ContactConnector*>& _connectors,
        const boost::unordered_set<SimBody*>& _simBodies);
    void initAnchoredObjects(ArrayDynamic<SolverBodyDynamicProperties>& _bodyVariableData, ArrayDynamic<SolverBodyStaticProperties>& _bodyStaticData,
        ArrayDynamic<SolverBodyMassAndInertia>& _massAndInertia, ArrayDynamic<float>& _effectiveMassMultipliers,
        VirtualDisplacementArray& _velocityDeltas, VirtualDisplacementArray& _positionDeltas, int offsetToAnchoredObjects,
        const ArrayBase<SimBody*>& _anchoredBodyList, const SolverConfig& _config) const;
    void integratePositionsAndUpdateSimBodies(SimBody* const* _simBodies, SolverBodyDynamicProperties* const _bodyVariableData,
    ...
    void integratePositionsIgnoreVelocitiesAndUpdateSimBodies(SimBody* const* _simBodies, SolverBodyDynamicProperties* const _bodyVariableData,
    ...
    void detectInconsistentConstraints(VirtualDisplacementArray& positionDeltasSIMD, ArrayBase<ConstraintVariables>& positionStage,
        const ArrayBase<ConstraintJacobianPair>& jacobians, const ArrayBase<ConstraintJacobianPair>& preconditionedJacobiansPosStage,
        const ArrayBase<EffectiveMassPair>& effectiveMassesPos, const ArrayBase<float>& sorPos, const ArrayBase<boost::uint8_t>& dimensions,
        const ArrayBase<boost::uint32_t>& offsets, const ArrayBase<BodyPairIndices>& simBodyPairs, const ArrayBase<Constraint*>& constraints,
        size_t collisionCount, const SolverConfig& solverConfig);
