...
private:
    ...
    mutable Aya::spin_mutex changeTrackerMutex;

    void setTrackerSimJob(SimJobTracker& tracker, SimJob* simJob) const
    {
        Aya::spin_mutex::scoped_lock lock(changeTrackerMutex);
        tracker.setSimJob(simJob);
    }

public:
    Aya::signal<void(Primitive*)> assemblyPhysicsOnSignal;
    Aya::signal<void(Primitive*)> assemblyPhysicsOffSignal;
