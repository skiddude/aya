...
public:
    ...
    {
        return ((isFaceFace() == other->isFaceFace()) && (primitive[0] == other->primitive[0]));
    }
};
...
private:
    ...
    const Poly* facePoly() const
    {
        return poly0();
    }
    const Poly* otherPoly() const
    {
        return poly1();
    }
    ...
    // bool testVerticesInside(size_t faceId, FixedArray<VertexStatus, 8>& vertexStatus, const CoordinateFrame& vertexInFace, ConnectorArray&
    // newConnectors);
    bool testVerticesInside(size_t faceId, FixedArray<VertexStatus, CONTACT_ARRAY_SIZE>& vertexStatus, const CoordinateFrame& vertexInFace,
    ...
    void vertexInside(Primitive* pFace, Primitive* pVertex, const POLY::Vertex* inside, const POLY::Face* planeFace, ConnectorArray& newConnectors);
    ...
    void validateOneSideIntersection(
    ...
    /*override*/ bool isFaceFace() const
    {
        return true;
    }
...
public:
    ...
    void setOtherFace(const POLY::Face* aFace)
    {
        otherFace = aFace;
    }
    const POLY::Face* getNextBestOtherFace(void)
    {
        return nextBestOtherFace;
    }
};
