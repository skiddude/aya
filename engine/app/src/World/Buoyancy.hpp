...
public:
    ...
    typedef Aya::FixedArray<BuoyancyConnector*, MAX_CONNECTORS> ConnectorArray;
    ...
    ContactType getContactType() const override
    {
        return Contact_Buoyancy;
    }
...
protected:
    ...
    bool computeIsCollidingUi(
        float overlapIgnored) override; // override to always return false so can build underwater; shouldn't affect HumanoidState code
...
public:
    BuoyancyCylinderContact(Primitive* p0, Primitive* p1)
        : BuoyancyBoxContact(p0, p1)
    {
    }
    Geometry::GeometryType getType()
    {
...
public:
    BuoyancyWedgeContact(Primitive* p0, Primitive* p1)
        : BuoyancyBoxContact(p0, p1)
    {
    }
    Geometry::GeometryType getType()
...
public:
    BuoyancyCornerWedgeContact(Primitive* p0, Primitive* p1)
        : BuoyancyBoxContact(p0, p1)
    {
    }
    Geometry::GeometryType getType()
