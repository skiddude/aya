...
MovingAssemblyStage::MovingAssemblyStage(IStage* upstream, World* world)
    : IWorldStage(upstream, new StepJointsStage(this, world), world)
{
}
#pragma warning(pop)
...
void MovingAssemblyStage::addAnimatedJoint(Joint* j)
{
    if (j->canStepUi() && Joint::isMotorJoint(j))
    {
        ...
        {
            AYAASSERT(!j->MovingAssemblyStageHook::is_linked());
...
void MovingAssemblyStage::addJoint(Joint* j)
{
    if (j->canStepUi())
    {
        ...
        AYAASSERT(animatedJoints.find(j) == animatedJoints.end());
    }
}
...
{
    a->putInStage(this);
    aya_static_cast<StepJointsStage*>(getDownstreamWS())->onSimulateAssemblyAdded(a);
}


void MovingAssemblyStage::onSimulateAssemblyRemoving(Assembly* a)
{
    aya_static_cast<StepJointsStage*>(getDownstreamWS())->onSimulateAssemblyRemoving(a);
