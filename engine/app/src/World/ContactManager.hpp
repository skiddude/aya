    ...
    , public Voxel2::GridListener
{
    ...
    Primitive* getSlowHit(const G3D::Array<Primitive*>& primitives, const RbxRay& unitRay, const G3D::Array<const Primitive*>& ignorePrim,
        const HitTestFilter* filter, Vector3& hitPointWorld, Vector3& surfaceNormal, PartMaterial& surfaceMaterial, float maxDistance,
        bool& stopped) const;
    ...
    Primitive* getFastHit(const RbxRay& worldRay,       // implies distance as well
        ...
        Vector3& hitPointWorld, bool& stopped, bool terrainCellsAreCubes, bool ignoreWater, Vector3& surfaceNormal,
    ...
    bool checkMegaClusterWaterContact(Primitive* p, const Vector3int16& extentStart, const Vector3int16& extentEnd, const Vector3int16& extentSize);
    bool checkMegaClusterSmallTerrainContact(
...
public:
    ...
    // General Inquiry
    //
    ContactManagerSpatialHash* getSpatialHash()
    {
        return spatialHash;
    }
    ...
    Primitive* getHit(const RbxRay& worldRay,
        ...
        const HitTestFilter* filter,                     // set to NULL to not use
        Vector3& hitPointWorld, bool terrainCellsAreCubes = false, bool ignoreWater = false, Vector3& surfaceNormal = dummySurfaceNormal,
        PartMaterial& surfaceMaterial = dummySurfaceMaterial) const;

    void getPrimitivesTouchingExtents(const Extents& extents, const Primitive* ignore, int maxCount, G3D::Array<Primitive*>& found);
    ...
    bool intersectingMySimulation(Primitive* check, Aya::SystemAddress myLocalAddress, float overlapIgnored);
    ...
    void onNewPair(Aya::Graphics::CullableSceneNode* p0, Aya::Graphics::CullableSceneNode* p1)
    {
        AYAASSERT(0);
    }

    void checkTerrainContact(Primitive* p);
    void checkTerrainContact(Aya::Graphics::CullableSceneNode* p0) {}

    bool primitiveIsExcludedFromSpatialHash(Primitive* p);
    bool primitiveIsExcludedFromSpatialHash(Aya::Graphics::CullableSceneNode* p0)
    {
        return false;
    }

    void releasePair(Primitive* p0, Primitive* p1);
    void releasePair(Aya::Graphics::CullableSceneNode* p0, Aya::Graphics::CullableSceneNode* p1)
    {
        AYAASSERT(0);
    }
    ...
    Primitive* getMegaClusterPrimitive(void) const
    {
