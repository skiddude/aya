...
Joint::Joint(Primitive* prim0, Primitive* prim1, const CoordinateFrame& _jointCoord0, const CoordinateFrame& _jointCoord1)
    : jointOwner(NULL)
    , Edge(prim0, prim1)
    ...
    , Edge(NULL, NULL)
{
    ...
    AYAASSERT(jointCoord0 == CoordinateFrame());
    AYAASSERT(jointCoord1 == CoordinateFrame());
}
...
{
    AYAASSERT((value == NULL) != (jointOwner == NULL));
    jointOwner = value;
}
...
bool Joint::canBuildJoint(Primitive* p0, Primitive* p1, NormalId nId0, NormalId nId1, float angleMax, float planarMax)
{
    if (!p0->isGeometryOrthogonal() || !p1->isGeometryOrthogonal())
    {
        ...
        if (!Math::fuzzyAxisAligned(face0Coord.rotation, face1Coord.rotation, angleMax))
        {
            return false;
        }
...
static int biggerJointGuid2(const Joint* j0, const Joint* j1)
{
    ...
    AYAASSERT(answer == Guid::compare(j01, j00, j10, j11));
    AYAASSERT(answer == Guid::compare(j00, j01, j11, j10));
    AYAASSERT(answer == -Guid::compare(j11, j10, j00, j01));
    AYAASSERT(answer == -Guid::compare(j10, j11, j01, j00));
    return answer;
}
...
bool Joint::isHeavierThan(const SpanningEdge* other) const
{
    ...
    const Joint* j1 = aya_static_cast<const Joint*>(other);
...
{
    Primitive* p = aya_static_cast<Primitive*>(n);
...
{
    const Primitive* p = aya_static_cast<const Primitive*>(n);
...
bool Joint::FaceEdgesOverlapped(const Primitive* p0, size_t face0Id, const Primitive* p1, size_t face1Id, float adjustPartTolerance)
{
    ...
    for (int i = 0; i < numVertsInface0 - 1; ++i)
    {
        ...
        for (int j = 0; j < numVertsInface1 - 1; ++j)
        {
            ...
            bool crossing = Math::lineSegmentDistanceIfCrossing(
                face0VertInp1_A, face0VertInp1_B, face1VertInp1_A, face1VertInp1_B, distance, adjustPartTolerance);
        ...
        float distance = largeDistance;
        bool crossing =
            Math::lineSegmentDistanceIfCrossing(face0VertInp1_A, face0VertInp1_B, face1VertInp1_A, face1VertInp1_B, distance, adjustPartTolerance);
...
Aya::SurfaceType Joint::getSurfaceTypeFromNormal(const Primitive& primitive, const NormalId& normalId)
{
...
bool Joint::compatibleForStudAutoJoint(const Primitive& p0, size_t& face0Id, const Primitive& p1, size_t& face1Id)
{
    ...
    SurfaceType t0 = getSurfaceTypeFromNormal(p0, (NormalId)face0Id);
    SurfaceType t1 = getSurfaceTypeFromNormal(p1, (NormalId)face1Id);
    bool compatibleForSnap = (((t0 == STUDS) && (t1 == INLET)) || ((t0 == INLET) && (t1 == STUDS)) || ((t0 == UNIVERSAL) && (t1 == UNIVERSAL)) ||
                              ((t0 == UNIVERSAL) && (t1 == INLET || t1 == STUDS)) || ((t0 == INLET || t0 == STUDS) && (t1 == UNIVERSAL)));
...
bool Joint::inCompatibleForAnyJoint(const Primitive& p0, size_t& face0Id, const Primitive& p1, size_t& face1Id)
{
    ...
    SurfaceType t0 = getSurfaceTypeFromNormal(p0, (NormalId)face0Id);
    SurfaceType t1 = getSurfaceTypeFromNormal(p1, (NormalId)face1Id);
    bool inCompatible = (((t0 == STUDS) && (t1 != INLET)) || ((t1 == STUDS) && (t0 != INLET)) ||
                         ((t0 == UNIVERSAL) && ((t1 != STUDS) && (t1 != INLET))) || ((t1 == UNIVERSAL) && ((t0 != STUDS) && (t0 != INLET))));
