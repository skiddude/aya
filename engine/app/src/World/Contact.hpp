...
#include "Utility/FixedArray.hpp"
#include "Debug.hpp"
...
class AyaBaseClass Contact : public Edge
{
private:
    typedef Edge Super;
    ...
    // Edge Virtuals
    //
    /*override*/ virtual EdgeType getEdgeType() const
    {
        return Edge::CONTACT;
    }
...
public:
    ...
    short getNumTouchCycles()
    {
        return numTouchCycles;
    }

    int& steppingIndexFunc()
    {
        return steppingIndex;
    } // fast removal from stepping list
    ...
    static bool isContact(Edge* e)
    {
        return (e->getEdgeType() == Edge::CONTACT);
    }
    ...
    ContactParams* getContactParams(void)
    {
    ...
    bool isInContact()
    {
        return lastUiContactStep > 0;
    }

    virtual ContactType getContactType() const
    {
...
private:
    ...
    typedef Aya::FixedArray<GeoPairConnector*, 8> ConnectorArray;
...
private:
    inline static void boxProjection(const Vector3& normal0, const Matrix3& R1, const Vector3& extent1, float& projectedExtent)
    {
        ...
        projectedExtent = std::abs(extent1[0] * temp[0]) + std::abs(extent1[1] * temp[1]) + std::abs(extent1[2] * temp[2]);
    }
    ...
    bool geoFeaturesOverlap(int pointBody, int planeBody, int pointID, NormalId pointFaceID, NormalId planeFaceID);
};
...
private:
    ...
    int bPlane;
    int bOther;
    Aya::NormalId planeID;
    Aya::NormalId otherPlaneID;
...
public:
    ...
    {
        return connectors[connectorsIndex].size();
    }
