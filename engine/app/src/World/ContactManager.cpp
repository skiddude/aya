...
#include "World/BallPolyContact.hpp"
#include "World/PolyPolyContact.hpp"
#include "World/PolyCellContact.hpp"
#include "World/BallCellContact.hpp"
#include "World/BulletShapeContact.hpp"
#include "World/BulletShapeCellContact.hpp"
#include "World/BulletContact.hpp"
...
bool ContactManager::intersectingMySimulation(Primitive* check, Aya::SystemAddress myLocalAddress, float overlapIgnored)
{
    AYAASSERT_FISHING(check->getNetworkOwner() != myLocalAddress);
    ...
    if (Assembly* assembly = check->getAssembly())
    {
        ...
        while (c)
        {
            ...
            {
                if ((otherAssembly != assembly) && (otherAssembly->getAssemblyPrimitive()->getNetworkOwner() == myLocalAddress) &&
...
bool ContactManager::intersectingOthers(Primitive* check, const std::set<Primitive*>& checkSet, float overlapIgnored)
{
    ...
    while (c)
    {
        ...
        if ((checkSet.find(other) == checkSet.end()) && c->computeIsCollidingUi(overlapIgnored))
        {
            return true;
        }
        c = check->getNextContact(c);
...
shared_ptr<const Instances> ContactManager::getPartCollisions(Primitive* check)
{
    ...
    while (c)
    {
        ...
        if (check != other)
        {
            ...
            if (Aya::Instance::fastDynamicCast<MegaClusterInstance>(partInstance))
            {
...
void ContactManager::getPrimitivesTouchingExtents(const Extents& extents, const Primitive* ignore, int maxCount, G3D::Array<Primitive*>& found)
{
...
// does the same as above, just passes an Instance* instead of a Primitive* [is there a shared base class for both Primitive and Instance? didn't see
// one...
void ContactManager::getPrimitivesTouchingExtents(
    const Extents& extents, const boost::unordered_set<const Primitive*>& ignorePrimitives, int maxCount, G3D::Array<Primitive*>& found)
{
    boost::unordered_set<Primitive*> tempBuffer;
...
Primitive* ContactManager::getSlowHit(const G3D::Array<Primitive*>& primitives, const RbxRay& unitRay, const G3D::Array<const Primitive*>& ignorePrim,
    const HitTestFilter* filter, Vector3& hitPoint, Vector3& surfaceNormal, PartMaterial& surfaceMaterial, float maxDistance, bool& stopped) const
{
    AYAASSERT(unitRay.direction().isUnit());
    ...
    for (int i = 0; i < primitives.size(); ++i)
    {
        ...
        if (!p->getBody())
        {
            AYACRASH("getSlowHit with deleted primitive");
        }
...
Primitive* ContactManager::getHit(const RbxRay& worldRay, const std::vector<const Primitive*>* ignorePrim, const HitTestFilter* filter,
    Vector3& hitPoint, bool terrainCellsAreCubes, bool ignoreWater, Vector3& surfaceNormal, PartMaterial& surfaceMaterial) const
{
    ...
    bool stopped;
    Primitive* answer =
        getFastHit(worldRay, tempIgnore, filter, hitPoint, stopped, terrainCellsAreCubes, ignoreWater, surfaceNormal, surfaceMaterial);
...
Primitive* ContactManager::getFastHit(const RbxRay& worldRay, const G3D::Array<const Primitive*>& ignorePrim, const HitTestFilter* filter,
    Vector3& hitPointWorld, bool& stopped, bool terrainCellsAreCubes, bool ignoreWater, Vector3& surfaceNormal, PartMaterial& surfaceMaterial) const
{
    AYAPROFILER_SCOPE("Physics", "getFastHit");

    // don't do raycast if incoming ray does not have good (i.e. finite)
    // floating point values in it.
    for (int index = 0; index < 3; ++index)
    {
        if (!boost::math::isfinite(worldRay.origin()[index]) || !boost::math::isfinite(worldRay.direction()[index]))
        {
    ...
    AYAASSERT(worldRay.direction().magnitude() < 5000.0f);
    ...
    if (myMegaClusterPrim && unitRay.direction().isUnit())
    { // the latter check is necessary, unfortunately
        ...
        if (dontIgnore && filterResult == HitTestFilter::INCLUDE_PRIM)
        {
            ...
            if (myMegaClusterPrim->getGeometryType() == Geometry::GEOMETRY_SMOOTHCLUSTER)
            {
                ...
                collided = static_cast<SmoothClusterGeometry*>(myMegaClusterPrim->getGeometry())
                               ->castRay(unitRay, megaHitPointWorld, megaSurfaceNormal, voxelMaterial, maxDistance, ignoreWater);
                megaSurfaceMaterial = Voxel2::Conversion::getMaterialFromVoxelMaterial(voxelMaterial);
            }
            else
            {
                collided = static_cast<MegaClusterPoly*>(myMegaClusterPrim->getGeometry())
                               ->hitTest(unitRay, megaHitPointWorld, megaSurfaceNormal, maxDistance, terrainCellsAreCubes, ignoreWater);
                megaSurfaceMaterial = GRASS_MATERIAL;
            }

            distanceToMegaHit = unitRay.direction().dot(megaHitPointWorld - unitRay.origin());
            if (!collided)
                distanceToMegaHit = maxDistance;
        }
    }
    ...
    do
    {
        ...
        if (Primitive* answer =
                getSlowHit(primitives, unitRay, ignorePrim, filter, hitPointWorld, surfaceNormal, surfaceMaterial, maxDistance, stopped))
        {
            ...
            if (hashGrid.fuzzyContains(hitPointWorld, 0.001f))
            {
                if (myMegaClusterPrim && distanceToMegaHit > 0.0f)
                {
                    ...
                    if (thisDistanceToHit < distanceToMegaHit)
                        return answer;
                }
                else
                    return answer;
...
Vector3 ContactManager::findUpNearestLocationWithSpaceNeeded(
    const float maxSearchDepth, const Vector3& startCenter, const Vector3& spaceNeededToCorner)
{
    ...
    for (Vector3 searchCenter = startCenter; !foundFittingLocation && searchCenter.y <= maxSearchDepth;)
    {
        ...
        Extents searchExtents = Extents::fromCenterCorner(
            searchCenter + Vector3(0.0f, spaceNeededToCorner.y / 2.0f, 0.0f), spaceNeededToCorner * Vector3(1.0f, 2.0f, 1.0f));
        ...
        if (primsFound.size())
        {
            ...
            for (DenseHashSet<Primitive*>::const_iterator it = primsFound.begin(); primsFound.end() != it; ++it)
            {
                ...
                if (primExtents.overlapsOrTouches(searchExtents) && primExtents.max().y <= searchExtents.center().y)
                {
                    ...
                    Vector3 adjSpaceNeeded =
                        spaceNeededToCorner - Vector3(Tolerance::maxOverlapOrGap(), Tolerance::maxOverlapOrGap(), Tolerance::maxOverlapOrGap());
                    ...
                    if (intersectingOthersChecks < FInt::IntersectingOthersCallsAllowedOnSpawn)
                    {
                        ...
                        boost::shared_ptr<Aya::PartInstance> tempPart;
...
{
    AYAASSERT(!p0->getGeometry()->isTerrain() && !p1->getGeometry()->isTerrain());
    ...
    if ((p0->getPreventCollide() || p1->getPreventCollide()) && (!p0->getOwner()->reportTouches() && !p1->getOwner()->reportTouches()) &&
    ...
    if (p0->getCollideType() == Geometry::COLLIDE_BULLET || p1->getCollideType() == Geometry::COLLIDE_BULLET)
    {
    ...
    AYACRASH("ContactManager::createContact");
    return NULL;
}

void ContactManager::onNewPair(Primitive* p0, Primitive* p1)
{
    AYAASSERT_VERY_FAST(Primitive::getContact(p0, p1) == NULL);
    ...
    if ((world->getSpatialFilter()->getSimSendFilter().mode == SimSendFilter::dPhysClient) &&
        !SpatialFilter::simulatingPhase(root0->getFilterPhase()) && !SpatialFilter::simulatingPhase(root1->getFilterPhase()))
        return;
...
void ContactManager::releasePair(Primitive* p0, Primitive* p1)
{
    if (p0 && p1)
    {
        ...
        else
        {
            ...
            {
                AYAASSERT(0); // this should never happen - something amiss in adding/removing contacts from Spatial Hash stage
            }
        }
        AYAASSERT_VERY_FAST(Primitive::getContact(p0, p1) == NULL); // should be clear now
    }
}
...
void ContactManager::onPrimitiveAdded(Primitive* p)
{
    if (p->getGeometry()->isTerrain())
    {
        ...
        {
            FASTLOG2(FLog::TerrainCellListener, "ContactManager: connecting %p to vg %p", this, getVoxelGrid());
...
void ContactManager::onPrimitiveRemoved(Primitive* p)
{
    if (p->getGeometry()->isTerrain())
    {
        ...
        {
            FASTLOG2(FLog::TerrainCellListener, "ContactManager: disconnecting %p to vg %p", this, getVoxelGrid());
...
Primitive* ContactManager::getHitLegacy(const RbxRay& originDirection,
    ...
    Vector3& hitPointWorld, float& distanceToHit, const float& maxSearchDepth, bool ignoreWater) const
{
    AYAASSERT(originDirection.direction().isUnit());
    ...
    Primitive* answer = getHit(worldRay, &ignorePrims, filter, hitPointWorld, false, ignoreWater);
...
bool ContactManager::checkMegaClusterWaterContact(
    Primitive* otherPrim, const Vector3int16& extentStart, const Vector3int16& extentEnd, const Vector3int16& extentSize)
{
    ...
    if (!touchingWater)
    {
        ...
        if (((middleCell.x > extentStart.x && middleCell.x < extentEnd.x) || (middleCell.y > extentStart.y && middleCell.y < extentEnd.y) ||
...
bool ContactManager::checkMegaClusterBigTerrainContact(Primitive* otherPrim)
{
    ...
    mCPoly->findCellsTouchingGeometry(
        myMegaClusterPrim->getCoordinateFrame(), *(otherPrim->getConstGeometry()), otherPrim->getCoordinateFrame(), &cellsUnfiltered);
...
bool ContactManager::checkMegaClusterSmallTerrainContact(
    Primitive* otherPrim, const Vector3int16& extentStart, const Vector3int16& extentEnd, const Vector3int16& extentSize, bool cellChanged)
{
    bool touchingTerrain = false;

    AYAASSERT(extentSize.x * extentSize.y * extentSize.z <= LARGE_TERRAIN_CONTACT_VOLUME);
...
{
    AYAASSERT(myMegaClusterPrim && getSmoothGrid());
    ...
    static_cast<SmoothClusterGeometry*>(myMegaClusterPrim->getGeometry())
        ->getTerrainPartition()
        ->findChunksTouchingExtents(otherExtentsWorld, &chunks);
    ...
    for (int i = 0; i < otherPrim->getNumContacts();)
    {
        if (otherPrim->getContactOther(i) == myMegaClusterPrim)
        {
            ...
            AYAASSERT(contactType == Contact::Contact_Cell || contactType == Contact::Contact_Buoyancy);
    ...
    for (size_t i = 0; i < chunks.size(); ++i)
    {
        ...
        if (chunk.touchesSolid)
            if (shared_ptr<btCollisionShape> shape =
                    static_cast<SmoothClusterGeometry*>(myMegaClusterPrim->getGeometry())->getBulletChunkShape(chunk.id))
            {
                if (world->getUsingPGSSolver())
...
void ContactManager::applyDeferredMegaClusterChanges()
{
    for (UpdatedTerrainRegionsSet::const_iterator itr = updatedTerrainRegions.begin(); itr != updatedTerrainRegions.end(); ++itr)
    {
        ...
        FASTLOG3(DFLog::DeferredVoxelUpdates, "Deferred voxel region update: starting region (%d,%d,%d)", regionId.value().x, regionId.value().y,
            regionId.value().z);
...
{
    AYAPROFILER_SCOPE("Physics", "applyDeferredTerrainChanges");
