...
#include <BulletCollision/CollisionShapes/btCollisionShape.hpp>
...
public:
    btCollisionObject* getBulletCollisionObject(void)
    {
        return bulletCollisionObject.get();
    }
    ...
    const G3D::Vector3& getSize() const
    {
        return size;
    }
    ...
    {
        AYAASSERT(0); // stock geometry does not handle parameters.
    }
    ...
    virtual size_t getFaceFromLegacyNormalId(const NormalId nId) const
    {
        return nId;
    }
    virtual bool isGeometryOrthogonal(void) const
    {
        return true;
    }
    ...
    /*override*/ virtual bool findTouchingSurfacesConvex(
        const CoordinateFrame& myCf, size_t& myFaceId, const Geometry& otherGeom, const CoordinateFrame& otherCf, size_t& otherFaceId) const = 0;
    /*override*/ virtual bool FacesOverlapped(const CoordinateFrame& myCf, size_t& myFaceId, const Geometry& otherGeom,
        const CoordinateFrame& otherCf, size_t& otherFaceId, float tol) const = 0;
    ...
    // CofmOffset
    virtual Vector3 getCofmOffset() const
    {
        return Vector3::zero();
    }

    // Moment
    virtual Matrix3 getMoment(float mass) const
    {
        return Matrix3::zero();
    }
    ...
    // Hit Test
    virtual bool hitTest(const RbxRay& rayInMe, Vector3& localHitPoint, Vector3& surfaceNormal)
    {
        return false;
    }
