...
float SpatialHashStatic::hashGridSize(int level)
{
    return (float)(cellMinSize << level);
}
float SpatialHashStatic::hashGridRecip(int level)
{
    return 1.0f / SpatialHashStatic::hashGridSize(level);
}
size_t SpatialHashStatic::numBuckets(int level)
{
    return 65536;
}
...
{
    AYAASSERT_SLOW(smallLevel < bigLevel);
...
{
    Extents answer(hashGridToReal(level, hashGrid), hashGridToReal(level, hashGrid + Vector3int32::one()));

    AYAASSERT_VERY_FAST(Math::isIntegerVector3(answer.min()));
    AYAASSERT_VERY_FAST(Math::isIntegerVector3(answer.max()));
...
SHP void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::cleanup()
{
    ...
    AYAASSERT_SPATIAL_HASH(validateTallyTreeNodes());
    AYAASSERT_SPATIAL_HASH(validateNoNodesOut());
    AYAASSERT(nodesOut == 0);
}
...
SHP typename SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::SpatialNode*
SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::newNode(int level, int hash, const Vector3int32& grid)
{
    SpatialNode* answer;
    ...
    else
    {
        ...
        AYAASSERT(level == answer->treeNode->level);
    }
...
SHP typename SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::TreeNode*
SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::findTreeNode(int level, int hash, const Vector3int32& gridCoord)
{
    TreeNode* tn = hashTables[level][hash].treeNodes;
    while (tn)
    {
        AYAASSERT(tn->hashId == hash);
        AYAASSERT(tn->level == level);
...
SHP typename SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::TreeNode*
SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::createTreeNode(int level, int hash, const Vector3int32& gridCoord)
{
    TreeNode* tn;
    ...
    AYAASSERT_SPATIAL_HASH(validateTreeNodeNotHere(tn, level, hash));
...
SHP void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::insertNodeToPrimitive(
    SpatialNode* node, Primitive* p, const Vector3int32& grid, int hash)
{
    AYAASSERT(node->level == p->getSpatialNodeLevel());

    node->primitive = p;
    node->gridId = grid;
    node->hashId = hash;

    AYAASSERT_SPATIAL_HASH(validateInsertNodeToPrimitive(node, p, grid, hash));
}
...
SHP typename SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::SpatialNode*
SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::findNode(Primitive* p, const Vector3int32& grid)
{
    const int l = p->getSpatialNodeLevel();

    AYAASSERT_SPATIAL_HASH(static_cast<SpatialNode*>(p->spatialNodes)->getLevel() == l);
    AYAASSERT(SpatialHashStatic::numBuckets(l) == hashTables[l].size());
...
SHP bool SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::oldExtentsOverlap(Primitive* me, Primitive* other)
{
    ...
    else
    {
        ...
        ExtentsInt32 smallInBig =
            SpatialHashStatic::scaleExtents(smallPrim->getSpatialNodeLevel(), big->getSpatialNodeLevel(), smallPrim->getOldSpatialExtents());
...
SHP bool SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::findOtherNodesInLevel0Cell(SpatialNode* destroy)
{
    ...
    while (node_sameHash)
    {
        if (node_sameHash->gridId == g)
        {
            ...
            AYAASSERT(other != destroy->primitive);
#endif
            return true;
        }
...
{
    AYAASSERT_SPATIAL_HASH(validateTallyTreeNodes());
    AYAASSERT_SPATIAL_HASH(validateRemoveNodeFromPrimitive(destroy));
    AYAASSERT_SPATIAL_HASH(destroy->getLevel() == destroy->primitive->getSpatialNodeLevel());
    ...
    {
        AYAASSERT(destroy->treeNode);
        retireTreeNode(destroy->treeNode);
    }
    else
    {
        if (!findOtherNodesInLevel0Cell(destroy))
        { // this (conceptual) leaf cell is empty; maintain hierachy
...
{
    int childHash;
    AYAASSERT((childHash = SpatialHashStatic::getHash(childLevel, childGridCoord), 1));

    for (int l = childLevel + 1; l < MAX_LEVELS; l++)
    {
        Vector3int32 g;
        g.x = childGridCoord.x >> (l - childLevel);
        g.y = childGridCoord.y >> (l - childLevel);
        g.z = childGridCoord.z >> (l - childLevel);
        int hash = SpatialHashStatic::getHash(l, g);

        TreeNode* tn = findTreeNode(l, hash, g);
        AYAASSERT(tn);
        int offset = ((childGridCoord.x >> (l - childLevel - 1)) & 1) + (((childGridCoord.y >> (l - childLevel - 1)) & 1) << 1) +
                     (((childGridCoord.z >> (l - childLevel - 1)) & 1) << 2);
        AYAASSERT(tn->children[offset] == childHash);
        // remove child
...
{
    AYAASSERT(p->getSpatialNodeLevel() != -1);
    ...
    AYAASSERT_SPATIAL_HASH(validateTallyTreeNodes());
    AYAASSERT_VERY_FAST(!hashHasPrimitive(level, p, hash, grid));
    ...
    if (level > 0)
    {
        ...
        AYAASSERT(addedNode->getLevel() == tn_save->level);
    }
#endif
    ...
    for (int l = level; l < MAX_LEVELS; l++)
    {
        ...
        if (l > level && needHiearchyUpdate)
        {
            ...
            int offset = ((grid.x >> (l - level - 1)) & 1) + (((grid.y >> (l - level - 1)) & 1) << 1) + (((grid.z >> (l - level - 1)) & 1) << 2);
            AYAASSERT(0 <= offset && offset <= 7 && prevHash >= 0);
            // establish child "pointer"
            AYAASSERT(!tn->hasChild(offset) || tn->children[offset] == prevHash);
            ...
            if (level > 0)
            {
                AYAASSERT(addedNode->treeNode && addedNode->getLevel() == addedNode->treeNode->level);
            }
        }
    ...
    {
        AYAASSERT(addedNode && addedNode->treeNode->level == addedNode->getLevel());
...
SHP int SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::computeLevel(const Primitive* p, const Extents& extents)
{
    ...
    float maxVolumeThisLevel =
        static_cast<float>((SpatialHashStatic::cellMinSize * SpatialHashStatic::cellMinSize * SpatialHashStatic::cellMinSize) * maxCellsPerPrimitive);
...
/*
SHP void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::computeMinMax(const Primitive* p, const Extents& extents, Vector3int32& min,
Vector3int32& max)
{
        AYAASSERT(p->getSpatialNodeLevel() != -1);
...
SHP void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::primitiveAdded(Primitive* p, bool addContact)
{
    ...
    AYAASSERT_SPATIAL_HASH(validateContacts(p));
    AYAASSERT_SPATIAL_HASH(p->spatialNodes == NULL);
    AYAASSERT(p->getSpatialNodeLevel() == -1);
    ...
    AYAASSERT(p->getSpatialNodeLevel() == level);
    AYAASSERT_SPATIAL_HASH(validateContacts(p));
}


SHP void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::changeMinMax(
    Primitive* p, const ExtentsInt32* change, const ExtentsInt32* oldBox, const ExtentsInt32* newBox, bool addContact)
{
    ...
    for (int level = 0; level < MAX_LEVELS; level++)
    {
        for (int hashId = 0; hashId < (int)SpatialHashStatic::numBuckets(level); hashId++)
        {
            ...
            while (tn)
            {
                ...
                tn = nodeToFree->next;
                AYAASSERT((fastClearTreeNode(nodeToFree), true)); // only run this if asserts are on.
                delete nodeToFree;
            };
        }
    }
...
SHP void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::checkAndReleaseContacts(Primitive* p)
{
    if (Primitive::hasGetFirstContact)
    {
        ...
        while (contact)
        {
            ...
            AYAASSERT_SPATIAL_HASH(overlap == validateNodesOverlap(p, other));
            if (!overlap)
            {
...
SHP void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::primitiveExtentsChanged(Primitive* p, const Extents& extents)
{
    ...
    if ((newMin == p->getOldSpatialMin()) && (newMax == p->getOldSpatialMax()))
    {
        return;
    }
    ...
    else
    {
        ...
        AYAASSERT_SPATIAL_HASH(p->spatialNodeCount == 0);
...
SHP void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::primitiveRemoved(Primitive* p)
{
    WriteValidator writeValidator(concurrencyValidator);
    AYAASSERT_SPATIAL_HASH(validateContacts(p));
    AYAASSERT(p->getSpatialNodeLevel() != -1 || contactManager->primitiveIsExcludedFromSpatialHash(p));
...
SHP void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::onPrimitiveExtentsChanged(Primitive* p)
{
    ...
    if (!coarseMovementCallbacks.empty() && (newLevel != oldLevel || preUpdateSpatialExtents != postUpdateSpatialExtents))
    {
        typename CoarseMovementCallback::UpdateInfo info;
    ...
    for (int i = newMin.x; i <= newMax.x; ++i)
    {
        for (int j = newMin.y; j <= newMax.y; ++j)
        {
            for (int k = newMin.z; k <= newMax.z; ++k)
            {
                ...
                {
                    AYAASSERT(thisNode && thisNode->treeNode->level == thisNode->getLevel());
...
SHP void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::getPrimitivesTouchingGrids(
    const Extents& extents, const Primitive* ignore, std::size_t maxCount, boost::unordered_set<Primitive*>& answer)
{
    ReadOnlyValidator readOnlyValidator(concurrencyValidator);

    AYAASSERT(answer.size() == 0);
            ...
            for (int j = min.y; j <= max.y; ++j)
            {
                for (int k = min.z; k <= max.z; ++k)
                {
                    ...
                    for (int ff = 0; ff < foundThisGrid.size(); ++ff)
                    {
                        ...
                        if ((p != ignore) && (answer.find(p) == answer.end()) && (extents.overlapsOrTouches(calcNewExtents(p))))
                        {
                            answer.insert(p);
    ...
    AYAASSERT(foundThisGrid.size() < 200);
}

// same as above function, but we use a set of all primitives to-be-ignored for use with an ancestor check
SHP void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::getPrimitivesTouchingGrids(
    const Extents& extents, const boost::unordered_set<const Primitive*>& ignoreSet, std::size_t maxCount, boost::unordered_set<Primitive*>& answer)
{
    ReadOnlyValidator readOnlyValidator(concurrencyValidator);
        ...
        for (int i = min.x; i <= max.x; ++i)
        {
            for (int j = min.y; j <= max.y; ++j)
            {
                for (int k = min.z; k <= max.z; ++k)
                {
                    ...
                    {
                        Primitive* p = foundThisGrid[ff];
                        if ((answer.find(p) == answer.end()) && (extents.overlapsOrTouches(calcNewExtents(p))))
                        {
                            // ancestor check here
...
SHP template<typename Set>
void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::getPrimitivesOverlapping(const Extents& extents, Set& answer)
{
    ReadOnlyValidator readOnlyValidator(concurrencyValidator);
    ...
    for (int level = 0; level < MAX_LEVELS; level++)
    {
        ...
        // A small negative offset is added to extentsMax before computeMinMax to prevent querying extra layer of cells for perfectly aligned/sized
        // extents
...
SHP template<typename Set>
void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::getPrimitivesOverlappingRec(const Extents& extents, Set& answer)
{
    ReadOnlyValidator readOnlyValidator(concurrencyValidator);
...
SHP template<typename Set>
void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::getPrimitivesOverlappingRec(
    const Extents* extents, Set& answer, int level, int hash, const Vector3int32& gridCoord)
{
...
SHP void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::getPrimitivesInGrid(
    int level, const Vector3int32& grid, G3D::Array<Primitive*>& found)
{
    ReadOnlyValidator readOnlyValidator(concurrencyValidator);
    ...
    while (node)
    {
        if (node->gridId == grid)
        {
            AYAASSERT_IF_VALIDATING(!found.contains(node->primitive));
...
{
    AYAASSERT(found.size() == 0);

    Vector3int32 g = grid;

    for (int level = 0; level < MAX_LEVELS; level++, g.x >>= 1, g.y >>= 1, g.z >>= 1)
    {
...
SHP bool SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::getNextGrid(Vector3int32& grid, const RbxRay& unitRay, float maxDistance)
{
    ReadOnlyValidator readOnlyValidator(concurrencyValidator);

    AYAASSERT_VERY_FAST(unitRay.direction().isUnit());
    ...
    { // number of non zeros - 1: adjacent face (6), edge (12), corner (8)
        for (int i = low[0]; i <= high[0]; ++i)
        {
            for (int j = low[1]; j <= high[1]; ++j)
            {
                ...
                {
                    if ((std::abs(i) + std::abs(j) + std::abs(k)) == nz)
                    { // start with this number of nonzeros
                        ...
                        bool result = G3D::CollisionDetection::collisionLocationForMovingPointFixedAABox(
                            unitRay.origin(), unitRay.direction(), box, location, inside);
                        if (inside || result)
                        {
...
SHP typename SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::TreeNode*
SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::getFirstRoot()
{
    TreeNode* tn = NULL;
...
SHP typename SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::TreeNode*
SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::getNextRoot(TreeNode* prevRoot)
{
    if (prevRoot)
    {
        if (prevRoot->next)
...
SHP typename SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::TreeNode*
SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::getChild(TreeNode* parent, int octant)
{
    AYAASSERT(parent->hasChild(octant));
    if (parent->level > 0)
    {
...
SHP void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::getPrimitivesInTreeNode(TreeNode* tn, G3D::Array<Primitive*>& primitives)
{
    ...
    while (node)
    {
        ...
        {
            AYAASSERT_IF_VALIDATING(!primitives.contains(node->primitive));
...
SHP void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::visitPrimitivesInSpace(SpaceFilter* filter)
{
    ...
    while (!nodestovisit.empty() && bContinueIterating)
    {
        ...
        const Aya::Vector3int32& gridId = node->gridId;
        ...
        if (nodeinfo.action == aRecurseTreeNode)
        {
            ...
            for (int childoffset = 0; childoffset < 8; ++childoffset)
            {
                ...
                if (childLevel > 0)
                {
                    ...
                    AYAASSERT(childTreeNode);
                    AYAASSERT(childTreeNode->gridId == childGridId);
                    AYAASSERT(childTreeNode->level == childLevel);
                    AYAASSERT(tn->children[childoffset] == childTreeNode->hashId);
...
#else
                    nodestovisit.push(NodeInfo(
                        snode, aVisitSingleSpatialNode, nodeinfo.intersectResult, filter->Distance(snode->primitive->getFastFuzzyExtents())));
#endif
                }
                snode = snode->nextHashLink;
            }
...
SHP void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::visitPrimitivesInSpaceWorker(TreeNode* tn, int level, int hashId,
    const Aya::Vector3int32& gridId, int* visitOrder, IntersectResult intersectResult, SpaceFilter* filter, const Vector3& visitDir)
{
    AYAASSERT(intersectResult == irFull || intersectResult == irPartial);
...
/////////////////////////////////////////////////////////////////////////////////
//
//  Debugging - all of these should AYAASSERT(assertingSpatialHash)
//
SHP bool SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::validateInsertNodeToPrimitive(
    SpatialNode* node, Primitive* p, const Vector3int32& grid, int hash)
{
    AYAASSERT(assertingSpatialHash);
...
SHP bool SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::validateTallyTreeNodes()
{
    ...
    for (int level = 0; level < MAX_LEVELS; level++)
    {
        for (int hashId = 0; hashId < (int)SpatialHashStatic::numBuckets(level); hashId++)
        {
            ...
            {
                numTreeNodesInUse++;
                AYAASSERT(tn->childMask || tn->refByPrimitives);
                tn = tn->next;
            }
        }
    }
...
SHP bool SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::validateNoNodesOut()
{
    ...
    {
        AYAASSERT(hashTables[l].size() == SpatialHashStatic::numBuckets(l));
        for (size_t i = 0; i < SpatialHashStatic::numBuckets(l); ++i)
        {
            AYAASSERT(hashTables[l][i].nodes == NULL);
        }
    }
    return true;
}
...
SHP bool SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::validateContacts(Primitive* p)
{
    ...
    __if_exists(Primitive::getFirstContact)
    {
        ...
        while (c)
        {
            ...
            AYAASSERT_SPATIAL_HASH(validateNodesOverlap(p, other));
            AYAASSERT_SPATIAL_HASH(validateNodesOverlap(p, other));
        ...
        while (myNode)
        {
            ...
            while (hashNode)
            {
                ...
                {
                    AYAASSERT_SPATIAL_HASH(Primitive::getContact(p, hashNode->primitive));
                }
                hashNode = hashNode->nextHashLink;
            }
            myNode = myNode->nextPrimitiveLink;
...
/*
	...
	Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Random Hash distribution 1: %d   2: %d   3: %d   4: %d", u1, u2, u3, u4);
*/
	...
	Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "CURRENT HASH DISTRIBUTION");
	size_t numSlots = 100;
	for (size_t i = 0; i < numSlots; ++i)
	{
		size_t index = ( (i + 1) * numBuckets() / numSlots ) - 1;
		Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Slot: %d      Count: %d", i, counts[index]);
	}


	Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "TOP HASH DISTRIBUTION");
	for (size_t i = 0; i < numSlots; ++i)
	{
	...
	Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "TOP GRID COLLISIONS DISTRIBUTION");
	for (size_t i = 0; i < numSlots; ++i)
	{
		size_t index = numBuckets() - 1 - i;
		Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Slot: %d      Count: %d", i, differentGrids[index]);
	}
#endif
}

SHP void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::registerCoarseMovementCallback(CoarseMovementCallback* callback)
{
    coarseMovementCallbacks.push_back(callback);
}

SHP void SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>::unregisterCoarseMovementCallback(CoarseMovementCallback* callback)
{
    typename std::vector<CoarseMovementCallback*>::iterator itr = std::find(coarseMovementCallbacks.begin(), coarseMovementCallbacks.end(), callback);
