...
struct Vector3_2IntsComparer
{
    ...
    {
        if (a.vectPart.x < b.vectPart.x)
            return true;
        if (a.vectPart.x > b.vectPart.x)
            return false;
        if (a.vectPart.y < b.vectPart.y)
            return true;
        if (a.vectPart.y > b.vectPart.y)
            return false;
        if (a.vectPart.z < b.vectPart.z)
            return true;
        if (a.vectPart.z > b.vectPart.z)
            return false;
        if (a.int1 < b.int1)
...
public:
    ...
    static Token getToken(const Key& key, const Key& data)
    {
        ...
        Aya::spin_mutex::scoped_lock lock(d.mutex);
    ...
    static void returnToken(Entry* entry)
    {
        ...
        AYAASSERT(entry->count > 0);
        entry->count--;

        if (entry->count == 0)
        {
            AYAASSERT(entry == entry->iterator->second);
