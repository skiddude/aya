...
Assembly::~Assembly()
{
    AYAASSERT(history == NULL);
    AYAASSERT(state == Sim::ANCHORED);
    AYAASSERT(simJob == NULL);
    AYAASSERT(recursiveDepth == -1);
}
/*
...
{
    getAssemblyPrimitive()->setNetworkIsSleeping(
        false, Time::nowFast()); // TL (3/25/11) - Without this the assembly parent
...
{
    AYAASSERT((state == Sim::ANCHORED) || (getCurrentStage()->getStageType() >= IStage::HUMANOID_STAGE));
...
Assembly* Assembly::getPrimitiveAssemblyFast(Primitive* p)
{
    ...
    AYAASSERT_IF_VALIDATING(!answer || dynamic_cast<Assembly*>(answer));
    return answer;
}
...
const Assembly* Assembly::getConstPrimitiveAssembly(const Primitive* p)
{
    if (const IndexedMesh* clump = p->getConstComputedUpper())
    {
        ...
        AYAASSERT_IF_VALIDATING(!answer || dynamic_cast<const Assembly*>(answer));
        return answer;
    }
    return NULL;
}
...
void Assembly::getPhysics(G3D::Array<CompactCFrame>& motorAngles) const
{
    ...
    for (int i = 0; i < motorAngles.size(); ++i)
    {
        ...
        {
            const MotorJoint* m1d = aya_static_cast<const MotorJoint*>(motors[i]);
            motorAngles[i] = CompactCFrame(Vector3::zero(), Vector3::unitZ(), m1d->getCurrentAngle());
        }
        else
        {
            const Motor6DJoint* m6d = aya_static_cast<const Motor6DJoint*>(motors[i]);
...
void Assembly::setPhysics(const G3D::Array<CompactCFrame>& motorAngles, const PV& pv)
{
    ...
    if (assemblyMotors.size() == motorAngles.size())
    {
        for (int i = 0; i < assemblyMotors.size(); ++i)
        {
            ...
            {
                MotorJoint* m1d = aya_static_cast<MotorJoint*>(assemblyMotors[i]);
                // TODO: Put this assertion back in and debug http://roblox.onjira.com/browse/CLIENT-250
                // AYAASSERT(G3D::fuzzyEq(fabs(motorAngles[i].getAxis().z), 1.0f));

                AYAASSERT(motorAngles[i].translation.isZero());
            ...
            {
                Motor6DJoint* m6d = aya_static_cast<Motor6DJoint*>(assemblyMotors[i]);
        ...
        //{
        //	Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "motors: %f,%f,%f,%f", motorAngles[0].rotationangle,
        // motorAngles[1].rotationangle,  motorAngles[2].rotationangle,  motorAngles[3].rotationangle);
        // }
...
bool Assembly::isAssemblyRootPrimitive(const Primitive* p)
{
    ...
    AYAASSERT(!answer || (getConstPrimitiveAssembly(p)->getConstTypedLower<Clump>()->getConstTypedLower<Primitive>() == p));
    return answer;
}
...
{
    AYAASSERT(this->getCurrentStage()->getStageType() >= IStage::TREE_STAGE);
...
void notifyAssemblyPrimitiveMoved(Primitive* p, bool resetContacts)
{
    ...
    if (resetContacts)
    {
        ...
        {
            AYAASSERT(p->getContact(i));
            p->getContact(i)->primitiveMovedExternally();
        }
        AYAASSERT(temp == p->getNumContacts());
    }
}
