...
#include "BulletCollision/BroadphaseCollision/btDbvt.hpp"
...
public:
        ...
        : data(geometry)
    {
        ...
        AYAPROFILER_COUNTER_ADD("memory/terrain/physics", getMemorySize());
    }

    ~ChunkMeshShape()
    {
        AYAPROFILER_COUNTER_SUB("memory/terrain/physics", getMemorySize());
    }
...
struct SmoothClusterGeometry::ChunkMesh
{
    ...
    {
        AYAPROFILER_SCOPE("Physics", "generateShape");

        AYAASSERT(state == State_Dummy);
        state = State_Ready;
...
{
    grid = aya_static_cast<MegaClusterInstance*>(p->getOwner())->getSmoothGrid();
...
bool SmoothClusterGeometry::findTouchingSurfacesConvex(
...
bool SmoothClusterGeometry::FacesOverlapped(
    const CoordinateFrame& myCf, size_t& myFaceId, const Geometry& otherGeom, const CoordinateFrame& otherCf, size_t& otherFaceId, float tol) const
{
    return false;
}

bool SmoothClusterGeometry::FaceVerticesOverlapped(
    const CoordinateFrame& myCf, size_t& myFaceId, const Geometry& otherGeom, const CoordinateFrame& otherCf, size_t& otherFaceId, float tol) const
{
    return false;
}

bool SmoothClusterGeometry::FaceEdgesOverlapped(
    const CoordinateFrame& myCf, size_t& myFaceId, const Geometry& otherGeom, const CoordinateFrame& otherCf, size_t& otherFaceId, float tol) const
{
    return false;
}
...
bool SmoothClusterGeometry::hitTestTerrain(const RbxRay& rayInMe, Vector3& localHitPoint, int& surfId, CoordinateFrame& surfCf)
{
    ...
    if (castRay(rayInMe, localHitPoint, normal, material, MC_SEARCH_RAY_MAX, /* ignoreWater= */ true))
    {
        ...
        AYAASSERT(surfId != NORM_UNDEFINED);
...
bool SmoothClusterGeometry::castRay(
    const RbxRay& rayInMe, Vector3& localHitPoint, Vector3& surfaceNormal, unsigned char& surfaceMaterial, float maxDistance, bool ignoreWater)
{
    Vector3 from = rayInMe.origin();
    Vector3 to = rayInMe.origin() + maxDistance * rayInMe.direction();

    MegaClusterInstance* mci = aya_static_cast<MegaClusterInstance*>(myPrim->getOwner());
...
{
    Voxel2::Grid* grid = aya_static_cast<MegaClusterInstance*>(myPrim->getOwner())->getSmoothGrid();
...
void SmoothClusterGeometry::updateChunk(const Vector3int32& id)
{
    ...
    btDbvtVolume bounds = btDbvtVolume::FromMM(btVector3(region.begin().x, region.begin().y, region.begin().z) * Voxel::kCELL_SIZE,
        btVector3(region.end().x, region.end().y, region.end().z) * Voxel::kCELL_SIZE);
    ...
    for (size_t j = 0; j < visitCount; ++j)
    {
        ...
        if (mesh->shape && mesh->shape.unique())
        {
            ...
            if (gcUnusedMemory > size_t(FInt::SmoothTerrainPhysicsCacheSize))
            {
                ...
                AYAASSERT(mesh->state == ChunkMesh::State_Ready);
