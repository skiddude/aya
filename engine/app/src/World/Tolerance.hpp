...
public:
    ...
    static const Extents& maxExtents()
    {
        ...
        static Extents millionCube(Vector3(-fuzzyMil - (rand() % 65536), -fuzzyMil - (rand() % 65536), -fuzzyMil - (rand() % 65536)),
            Vector3(fuzzyMil + (rand() % 65536), fuzzyMil + (rand() % 65536), fuzzyMil + (rand() % 65536)));
        return millionCube;
    }
    ...
    {
        return 0.35f;
    } // plate thickness is 0.4
    static float jointOverlapMin2()
    {
        return 0.1f;
    }
    ...
    {
        return jointMaxUnaligned() * 0.5f;
    } // length of the axle is always == 2
    static float rotatePlanarMax()
    {
        return jointMaxUnaligned();
    }
    ...
    static float maxOverlapAllowedForResize()
    {
        return 3.0f * maxOverlapOrGap();
    } // 0.03;
};
