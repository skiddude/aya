...
#include "World/BlockCorners.hpp"
...
const int Block::BLOCK_FACE_VERTEX_TO_EDGE[6][4] = {4, 11, 5, 8, 8, 3, 9, 0, 0, 7, 1, 4, 9, 6, 10, 7, 1, 10, 2, 11, 5, 2, 6, 3};
...
bool Block::hitTest(const RbxRay& rayInMe, Vector3& localHitPoint, Vector3& surfaceNormal)
{
    ...
    return G3D::CollisionDetection::collisionLocationForMovingPointFixedAABox(
        rayInMe.origin(), rayInMe.direction(), AABox(-halfRealSize, halfRealSize), localHitPoint, inside, surfaceNormal);
}
...
const Vector3* Block::getEdgePoint(const Vector3int16& clip, Aya::NormalId& normalID) const
{
...
const Vector3* Block::getPlanePoint(const Vector3int16& clip, Aya::NormalId& normalID) const
{

    // normal is from the negative "zero" point positive
    if (clip.x != 0)
    {
        normalID = clip.x > 0 ? Aya::NORM_X : Aya::NORM_X_NEG;
        int x = clip.x > 0 ? 0 : 1;
        return &vertices[x * 4]; // either +x,+y,+z or -x,+y,+z
    }
    if (clip.y != 0)
    {
        normalID = clip.y > 0 ? Aya::NORM_Y : Aya::NORM_Y_NEG;
        int y = clip.y > 0 ? 0 : 1;
        return &vertices[y * 2];
    }
    AYAASSERT(clip.z);
    {
        normalID = clip.z > 0 ? Aya::NORM_Z : Aya::NORM_Z_NEG;
        int z = clip.z > 0 ? 0 : 1;
        return &vertices[z];
    }
}

GeoPairType Block::getBallBlockInfo(int onBorder, const Vector3int16 clip, const Vector3*& offset, Aya::NormalId& normalID)
{
...
GeoPairType Block::getBallInsideInfo(const Vector3& ray, const Vector3*& offset, Aya::NormalId& normalID)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        {
            min = temp;
            normalID = static_cast<Aya::NormalId>(i);
        }
        temp = ray[i] + l[i];
        if (temp < min)
        {
            min = temp;
            normalID = static_cast<Aya::NormalId>(i + 3);
        }
    }
    AYAASSERT(min != FLT_MAX);

    offset = (normalID > Aya::NORM_Z) ? &vertices[7] : &vertices[0];
    return BALL_PLANE_PAIR;
}
...
Vector2 Block::getProjectedVertex(const Vector3& vertex, Aya::NormalId normalID)
{
