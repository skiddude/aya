...
public:
    ...
    {
        AYAASSERT(Extents(e.min(), e.max()) == e);
        if (e.isNanInf())
        {
            AYAASSERT(0);
            return Extents::zero();
...
public:
    struct SpaceFilter
    {
        ...
        virtual float Distance(const Extents& extents)
        {
            return 0;
        };
    ...
    void getPrimitivesTouchingGrids(const Extents& extents, const Primitive* ingore, std::size_t maxCount, boost::unordered_set<Primitive*>& answer);

    void getPrimitivesTouchingGrids(const Extents& extents, const boost::unordered_set<const Primitive*>& ignoreSet, std::size_t maxCount,
    ...
    void getPrimitivesOverlapping(const Extents& extents, Set& answer);

    // This function recursively processes cells that overlap with extents, which is faster on large regions
    template<typename Set>
    void getPrimitivesOverlappingRec(const Extents& extents, Set& answer);
...
private:
    ...
    struct FastClearSpatialNode
    {
        ...
        FastClearSpatialNode(SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>* h)
            : hash(h) {};
    ...
    {
        SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>* hash;
        FastClearTreeNode(SpatialHash<Primitive, Contact, ContactManager, MAX_LEVELS>* h)
            : hash(h) {};
...
private:
    ...
    TreeNode* findTreeNode(int level, int hash, const Vector3int32& gridCoord);
    TreeNode* createTreeNode(int level, int hash, const Vector3int32& gridCoord);
    ...
    void changeMinMax(Primitive* p, const ExtentsInt32* change, const ExtentsInt32* oldBox, const ExtentsInt32* newBox, bool addContact = true);
    ...
    {
        return Vector3int32((grid.x << 1) + (offset & 1), // bit 0 of offset is x coord.
    ...
    void visitPrimitivesInSpaceWorker(TreeNode* tn, int level, int hashId, const Aya::Vector3int32& gridId, int* visitOrder,
        IntersectResult intersectResult, SpaceFilter* filter, const Vector3& visitDir);

    template<typename Set>
    void getPrimitivesOverlappingRec(const Extents* extents, Set& answer, int level, int hash, const Vector3int32& gridCoord);
...
#include "World/SpatialHashMultiRes.inl"
