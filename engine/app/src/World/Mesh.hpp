...
public:
    ...
    Vertex(size_t id, const Vector3& offset)
        : id(id)
        , offset(offset)
    {
    }
    ...
    {
        AYAASSERT(std::find(edges.begin(), edges.end(), value) == edges.end());
...
public:
    ...
    const Vertex* getVertex(const Face* face, size_t id) const
    {
        ...
        {
            AYAASSERT(face == backward);
            AYAASSERT(vertex[(id + 1) % 2]);
...
public:
    ...
    {
        AYAASSERT(edges.size() >= 3);
        // AYAASSERT(!(outwardPlane == Plane()));
        return outwardPlane;
    }
...
public:
    ...
    const POLY::Face* getFace(int i) const
    {
        return &faces[i];
    }
    ...
    const POLY::Vertex* getVertex(int i) const
    {
        return &vertices[i];
    }
    ...
    const POLY::Edge* getEdge(int i) const
    {
        return &edges[i];
    }
