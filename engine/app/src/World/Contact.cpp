...
float BlockBlockContact::pairHitRatio()
{
    ...
    return (denom == 0) ? -1 : (float)(pairMatches) / (float)(denom);
}

float BlockBlockContact::featureHitRatio()
{
    int denom = featureMatches + featureMisses;
    return (denom == 0) ? -1 : (float)(featureMatches) / (float)(denom);
}
...
{
    return aya_static_cast<Ball*>(getPrimitive(i)->getGeometry());
}
...
{
    return aya_static_cast<Ball*>(ballPrim()->getGeometry());
}
Block* BallBlockContact::block()
{
    return aya_static_cast<Block*>(blockPrim()->getGeometry());
}
...
bool BallBlockContact::computeIsColliding(int& onBorder, Vector3int16& clip, Vector3& projectionInBlock, float overlapIgnored)
{
    ...
    if (contactParams)
    {
        ...
        if (BallBlockContact::computeIsColliding(onBorder, clip, projectionInBlock, 0.0))
        {
            if (inKernel())
            {
                ...
                NormalId normalID;
                GeoPairType pairType = onBorder ? block()->getBallBlockInfo(onBorder, clip, offset, normalID)
...
{
    return aya_static_cast<Block*>(getPrimitive(i)->getGeometry());
}
...
GeoPairConnector* BlockBlockContact::findGeoPairConnector(Body* b0, Body* b1, GeoPairType _pairType, int param0, int param1)
{
    AYAASSERT(myData && contactParams);
...
bool BlockBlockContact::computeIsColliding(float overlapIgnored, bool& planeContact)
{
    ...
    return (Primitive::aaBoxCollide(*getPrimitive(0), *getPrimitive(1)) && getBestPlaneEdge(overlapIgnored, planeContact));
}
...
void BlockBlockContact::loadGeoPairEdgeEdge(int b0, int b1, int edge0, int edge1)
{
    ...
    GeoPairConnector* geoPair = findGeoPairConnector(getBody(b0), getBody(b1), EDGE_EDGE_PAIR, norm0, norm1);
    ...
    AYAASSERT(geoPair->getContactPoint().length < 0);
    AYAASSERT(geoPair->getContactPoint().normal.magnitude() > 0.99f);
...
void BlockBlockContact::loadGeoPairPointPlane(int pointBody, int planeBody, int pointID, NormalId pointFaceID, NormalId planeFaceID)
{
    ...
    if ((geoPair = findGeoPairConnector(getBody(pointBody), getBody(planeBody), POINT_PLANE_PAIR, pointID, planeFaceID)))
    {
        ...
        {
            geoPair->setPointPlane(
                block(pointBody)->getFaceVertex(pointFaceID, pointID), block(planeBody)->getFaceVertex(planeFaceID, 0), pointID, planeFaceID);
            geoPair->updateContactPoint();
            AYAASSERT(geoPair->getContactPoint().length < 0);
            AYAASSERT(geoPair->getContactPoint().normal.magnitude() > 0.99f);
    ...
    else
    {
        if (geoFeaturesOverlap(pointBody, planeBody, pointID, pointFaceID, planeFaceID))
        {
            ...
            geoPair->updateContactPoint();
            getKernel()->insertConnector(geoPair);
            AYAASSERT(geoPair->getContactPoint().length < -ContactConnector::overlapGoal() + 0.001);
            AYAASSERT(geoPair->getContactPoint().normal.magnitude() > 0.99f);
...
bool BlockBlockContact::geoFeaturesOverlap(int pointBody, int planeBody, int pointID, NormalId pointFaceID, NormalId planeFaceID)
{
...
GeoPairConnector* BlockBlockContactData::findGeoPairConnector(Body* b0, Body* b1, GeoPairType _pairType, int param0, int param1)
{
...
void BlockBlockContactData::loadGeoPairEdgeEdgePlane(int edgeBody, int planeBody, int edge0, int edge1)
{
    NormalId norm0 = myOwner->block(edgeBody)->getEdgeNormal(edge0);
    NormalId norm1 = myOwner->block(planeBody)->getEdgeNormal(edge1);

    GeoPairConnector* geoPair = findGeoPairConnector(myOwner->getBody(edgeBody), myOwner->getBody(planeBody), EDGE_EDGE_PLANE_PAIR, norm0, norm1);
    ...
    if (!geoPair)
    {
        ...
        geoPair->setEdgeEdgePlane(myOwner->block(edgeBody)->getEdgeVertex(edge0), myOwner->block(planeBody)->getEdgeVertex(edge1), norm0, norm1,
            planeID, myOwner->getPrimitive(edgeBody)->getSize()[norm0 % 3], myOwner->getPrimitive(planeBody)->getSize()[norm1 % 3]);
        geoPair->updateContactPoint();
        if (geoPair->getContactPoint().length >= -ContactConnector::overlapGoal())
        {
            delete geoPair;
    ...
    for (int i = witnessId; i < witnessId + 2; ++i)
    {
        ...
        Vector3 pTestInBase = cBase.pointToObjectSpace(pTest);
        {
            for (int j = separatingAxisId; j < separatingAxisId + 3; ++j)
            {
                ...
                {
                    if (checkLastFeature)
                    { // set the value of the last feature
...
int BlockBlockContactData::intersectRectQuad(Vector2& planeRect, Vector2 (&otherQuad)[4])
{
    ...
    for (q = 3; q >= 0; q--)
    {
        if (otherQuad[q].y <= planeRect.y)
        {
            rectCrossQuad[0][q] = true;
        }
        else
        {
            rectCrossQuad[0][q] = false;
            quadIn[q] = false;
        }

        if (otherQuad[q].x >= -planeRect.x)
        {
            rectCrossQuad[1][q] = true;
        }
        else
        {
            rectCrossQuad[1][q] = false;
            quadIn[q] = false;
        }

        if (otherQuad[q].y >= -planeRect.y)
        {
            rectCrossQuad[2][q] = true;
        }
        else
        {
            rectCrossQuad[2][q] = false;
            quadIn[q] = false;
        }

        if (otherQuad[q].x <= planeRect.x)
        {
            rectCrossQuad[3][q] = true;
        }
        else
        {
            rectCrossQuad[3][q] = false;
            quadIn[q] = false;
        }
    }
    ...
    for (q = 3; q >= 0; q--)
    {
        if (quadIn[q])
        {
            myOwner->loadGeoPairPointPlane(bOther, bPlane, q, otherPlaneID, planeID);
            found++;
        }
    }
    if (found == 4)
        return found; // all points of quad were in rect;
    ...
    for (q = 3; q >= 0; q--)
    {
        ...
        for (int r = 0; r < 4; r++)
        {
            ...
            if (cross >= 0.0)
            {
                quadCrossRect[q][r] = true;
            }
            else
            {
                quadCrossRect[q][r] = false;
                rectIn[r] = false;
            }
        }
    }
    ...
    for (int r = 0; r < 4; r++)
    {
        if (rectIn[r])
        {
            myOwner->loadGeoPairPointPlane(bPlane, bOther, r, planeID, otherPlaneID);
            found++;
        }
    }
    if (noQuadPts && (found == 4))
        return found; // all points of rect were in quad;
    ...
    for (int r = 0; r < 4; r++)
    {
        for (q = 3; q >= 0; q--)
        {
            if ((rectCrossQuad[r][q] != rectCrossQuad[r][(q + 3) % 4]) && (quadCrossRect[q][r] != quadCrossRect[q][(r + 1) % 4]))
            {
                loadGeoPairEdgeEdgePlane(bOther, bPlane, myOwner->block(bOther)->faceVertexToEdge(otherPlaneID, (q + 3) % 4),
