...
{
    return (p.x < aabbMin.x ? 0x01 : 0x0) | (p.x > aabbMax.x ? 0x08 : 0x0) | (p.y < aabbMin.y ? 0x02 : 0x0) | (p.y > aabbMax.y ? 0x10 : 0x0) |
           (p.z < aabbMin.z ? 0x4 : 0x0) | (p.z > aabbMax.z ? 0x20 : 0x0);
}
...
template<typename T>
static T* getBuffer(T (&stackBuffer)[32], boost::scoped_array<T>& heapBuffer, size_t bufferSize)
{
...
static void queryAABBStackless(const KDTree* tree, btTriangleCallback* callback, const Vector3& aabbMin, const Vector3& aabbMax)
{
    ...
    while (bufferOffset > 0)
    {
        ...
        else
        {
            ...
            AYAASSERT(bufferOffset + 2 <= tree->depth);
...
static void rayTriangle(KDTree::RayResult& result, const Vector3& raySource, const Vector3& rayDir, const Vector3& v0, const Vector3& v1,
    const Vector3& v2, const KDTree* tree, unsigned int tri)
{
    Vector3 edge1 = v1 - v0;
    Vector3 edge2 = v2 - v0;
...
static void queryRayStackless(
    const KDTree* tree, KDTree::RayResult& result, const Vector3& raySource, const Vector3& rayTarget, float tmin, float tmax)
{
...
void KDTree::build(
    const Vector3* vertexPositions, const unsigned char* vertexMaterials, size_t vertexCount, const unsigned int* indices, size_t triangleCount)
{
