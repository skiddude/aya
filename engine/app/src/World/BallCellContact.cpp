...
{
    AYAASSERT(aya_static_cast<Ball*>(p1->getGeometry()));
    AYAASSERT(aya_static_cast<MegaClusterPoly*>(p0->getGeometry()));
...
BallPlaneConnector* BallCellContact::newBallPlaneConnector(const POLY::Face* face)
{
    ...
    if (contactParams)
    {
        return new BallPlaneConnector(getPrimitive(1)->getBody(), getPrimitive(0)->getBody(), *contactParams, ball()->getRadius(),
            face->getVertexOffset(0), face->normal(),
...
BallEdgeConnector* BallCellContact::newBallEdgeConnector(const POLY::Edge* edge)
{
    ...
    if (contactParams)
    {
        return new BallEdgeConnector(getPrimitive(1)->getBody(), getPrimitive(0)->getBody(), *contactParams, ball()->getRadius(),
...
BallVertexConnector* BallCellContact::newBallVertexConnector(const POLY::Vertex* vertex)
{
    ...
    if (contactParams)
    {
        return new BallVertexConnector(getPrimitive(1)->getBody(), getPrimitive(0)->getBody(), *contactParams, ball()->getRadius(),
            vertex->getOffset(),
...
{
    return aya_static_cast<const Ball*>(getConstPrimitive(1)->getConstGeometry());
}

const Poly* BallCellContact::poly() const
{
    return aya_static_cast<const MegaClusterPoly*>(getConstPrimitive(0)->getConstGeometry());
}
