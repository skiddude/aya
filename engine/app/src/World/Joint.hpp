...
class AyaInterface IJointOwner{public : virtual Joint * getJoint(void){AYAASSERT(0);
return NULL;
} // namespace Aya
}
;
...
protected:
    ...
    // Edge
    //
    /*override*/ EdgeType getEdgeType() const
    {
        return Edge::JOINT;
    }
    ...
    Joint(Primitive* prim0, Primitive* prim1, const CoordinateFrame& _jointCoord0, const CoordinateFrame& _jointCoord1);
...
public:
    ...
    virtual bool joinsFace(Primitive* g, NormalId faceId) const
    {
        return false;
    }
    ...
    virtual CoordinateFrame align(Primitive* pMove, Primitive* pStay)
    {
        AYAASSERT(0);
    ...
    virtual bool canStepWorld() const
    {
        return false;
    }
    ...
    virtual bool stepUi(double distributedGameTime)
    {
        return false;
    }
    ...
    //////////////////////////////////////////////////////////
    //
    static bool isJoint(const Edge* e)
    {
    ...
    {
        return isJoint(e) ? aya_static_cast<const Joint*>(e)->getJointType() : Joint::NO_JOINT;
    }

    static bool isGroundJoint(const Edge* e)
    { // alternately, created by AutoJoin
    ...
    static bool isAutoJoint(const Joint* j)
    { // alternately, created by AutoJoin
    ...
    {
        AYAASSERT((i == 0) || (i == 1));
        return (i == 0) ? Matrix3ToNormalId(jointCoord0.rotation) : normalIdOpposite(Matrix3ToNormalId(jointCoord1.rotation));
    }

    virtual Link* resetLink()
    {
        AYAASSERT(!"Not Implemented");
        return 0;
    }
    ...
    static SurfaceType getSurfaceTypeFromNormal(
        const Primitive& primitive, const NormalId& normalId); // helper function for correct "isCompatible" function behavior
...
{
private:
    /*override*/ virtual JointType getJointType() const
    {
        return Joint::ANCHOR_JOINT;
    }

public:
    AnchorJoint(Primitive* prim)
        : Joint(prim, NULL, CoordinateFrame(), CoordinateFrame())
    {
    }
...
private:
    /*override*/ virtual JointType getJointType() const
    {
        return Joint::FREE_JOINT;
    }

public:
    FreeJoint(Primitive* prim)
        : Joint(prim, NULL, CoordinateFrame(), CoordinateFrame())
    {
    }
