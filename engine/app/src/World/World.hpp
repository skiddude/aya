...
public:
    ...
    int getThrottleIndex() const
    {
        return throttleIndex;
    }

    void setThrottleIndex(int index)
    {
        throttleIndex = index;
    }
};
...
public:
    Aya::signal<void(Joint*, Primitive*, std::vector<Primitive*>&)> postInsertJointSignal;
    Aya::signal<void(Joint*, std::vector<Primitive*>&, std::vector<Primitive*>&)> postRemoveJointSignal;
    Aya::signal<void(Joint*)> autoJoinSignal;
    Aya::signal<void(Joint*)> autoDestroySignal;
    Aya::signal<void(std::pair<Primitive*, Primitive*>)> primitiveCollideSignal;
    ...
    {
        ContactManager* ptr;
        OnPrimitiveMovingVisitor(ContactManager* p)
            : ptr(p)
        {
        }
...
public:
    ...
    btCollisionDispatcher* getBulletCollisionDispatcher(void)
    {
        return bulletDispatcher;
    }
...
private:
    ...
    void destroyAutoJoints(Primitive* p, std::set<Primitive*>* ignoreGroup, bool includeExplicit = true,
        bool includeAuto = true); // if group, then keep joints between group members
...
public:
    ...
    const G3D::Array<TouchInfo>& getTouchInfoFromLastStep()
    {
        return touchReporting;
    }
    void clearTouchInfoFromLastStep()
    {
        touchReporting.fastClear();
    }
    ...
    {
        return primitives.underlyingArray();
    }
    ...
    float getWorldStepsAccumulated()
    {
        return worldStepAccumulated;
    }
    int getUiStepId();
    int getLongUiStepId();
    void sendClumpChangedMessage(Primitive* childPrim);

    EThrottle& getEThrottle()
    {
        return eThrottle;
    }
    int getFRMThrottle()
    ...
    {
        return getPrimitives().size();
    }
    float getEnvironmentSpeed() const
    {
        return eThrottle.getEnvironmentSpeed();
    }
    float getEnvironmentSpeedPercent() const
    {
        return getEnvironmentSpeed() * 100.0f;
    }

    void setFallenPartDestroyHeight(float value)
    {
        fallenPartDestroyHeight = value;
    }
    float getFallenPartDestroyHeight() const
    {
        return fallenPartDestroyHeight;
    }
    ...
    void setGravity(float value)
    {
        ...
        {
            primitives[i]->getBody()->updateGravity(value);
        }
    }
    ...
    Aya::Profiling::CodeProfiler& getProfileWorldStep()
    {
        return *profilingWorldStep;
    }
    const Aya::Profiling::CodeProfiler& getProfileWorldStep() const
    {
        return *profilingWorldStep;
    }

    void loadProfilers(std::vector<Aya::Profiling::CodeProfiler*>& worldProfilers) const;
    ...
    void setPhysicsAnalyzerEnabled(bool value)
    {
        physicsAnalyzerEnabled = value;
    }
    ...
    {
        return physicalMaterialsMode;
    }
    void setPhysicalPropertiesMode(PhysicalPropertiesMode mode);

    // motion analytic
    void plusErrorCount(double ec)
    {
        errorCount += ec;
    }
    void plusPassCount(double pc)
    {
        passCount += pc;
    }
    double getPassCount()
