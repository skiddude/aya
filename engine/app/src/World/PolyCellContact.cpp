...
{
    if (!dynamic_cast<const Poly*>(getPrimitive(0)->getConstGeometry()))
        return;
    if (!dynamic_cast<const Poly*>(getPrimitive(1)->getConstGeometry()))
        return;
...
void PolyCellContact::findBestPair()
{
    ...
    { // only doing face face - Tim - if you set this to 3 we will get CellEdgeEdgePairs.
        PolyCellPair* testPair = testPairs[i];
        if (!bestPair->match(testPair))
        {
            float distance = testPair->test();
            if (distance > 0.0)
            { // separating plane or separating edges
                resetBestPair(testPair);
                return;
            }
            if (distance > (currentDistance + epsilonDistance()))
            { // threshold to switch
                betterPair = testPair;
...
{
    return aya_static_cast<const Poly*>(primitive[0]->getConstGeometry());
}

const Poly* PolyCellPair::poly1() const
{
    return aya_static_cast<const Poly*>(primitive[1]->getConstGeometry());
}
...
{
    AYAASSERT(verticesInObject.size() == 0);
...
float CellFaceFacePair::closestVertex(
    const POLY::Face* face, const FixedArray<Vector3, CONTACT_ARRAY_SIZE>& verticesInObject, const Vertex*& closestVertex)
{
...
bool CellFaceFacePair::pairIsValid()
{
    ...
    primitive[0]->getGeometryType() == Geometry::GEOMETRY_MEGACLUSTER ? terrain0 = aya_static_cast<MegaClusterInstance*>(primitive[0]->getOwner())
                                                                      : terrain1 = aya_static_cast<MegaClusterInstance*>(primitive[1]->getOwner());

    AYAASSERT(primitive[0]->getGeometryType() == Geometry::GEOMETRY_MEGACLUSTER || primitive[1]->getGeometryType() == Geometry::GEOMETRY_MEGACLUSTER);
    ...
    for (size_t i = 0; i < numEdges; ++i)
    {
        ...
        case ABOVE_INSIDE:
        {
            ...
            case BELOW_INSIDE:
                break;
            case BELOW_OUTSIDE:
                checkOneSideIntersection(vFrom, vTo, otherInMe, newConnectors);
                break;
            }
            break;
        }
        case ABOVE_OUTSIDE:
        {
            ...
            case BELOW_INSIDE:
                ...
                break;
            case BELOW_OUTSIDE:
                checkTwoSideIntersections(vFrom, vTo, otherInMe, newConnectors);
                break;
            }
            break;
        }
        case BELOW_INSIDE:
        {
            ...
            case BELOW_INSIDE:
                break;
            case BELOW_OUTSIDE:
                validateOneSideIntersection(vFrom, vTo, otherInMe, newConnectors);
                break;
            }
            break;
        }
        case BELOW_OUTSIDE:
        {
            ...
            case ABOVE_OUTSIDE:
                ...
                break;
            case BELOW_INSIDE:
                validateOneSideIntersection(vTo, vFrom, otherInMe, newConnectors);
                break;
            case BELOW_OUTSIDE:
                checkTwoSideIntersections(vFrom, vTo, otherInMe, newConnectors);
                break;
...
bool CellFaceFacePair::loadVertices(FixedArray<VertexStatus, CONTACT_ARRAY_SIZE>* vertexStatus, CoordinateFrame* vertexInFace,
...
bool CellFaceFacePair::testVerticesInside(
    size_t faceId, FixedArray<VertexStatus, CONTACT_ARRAY_SIZE>& vertexStatus, const CoordinateFrame& vertexInFace, ConnectorArray& newConnectors)
{
    bool allInside = true;
...
CellFaceFacePair::VertexStatus CellFaceFacePair::vertexInPoly(
    const POLY::Face* planeFace, const Mesh* planeMesh, const Vertex* vertex, const CoordinateFrame& otherInMe)
{
...
void CellFaceFacePair::validateOneSideIntersection(
    const POLY::Vertex* belowInside, const POLY::Vertex* belowOutside, const CoordinateFrame& otherInMe, ConnectorArray& newConnectors)
{
...
void CellFaceFacePair::checkOneSideIntersection(
    const POLY::Vertex* v0, const POLY::Vertex* v1, const CoordinateFrame& otherInMe, ConnectorArray& newConnectors)
{
    Vector3 p0 = otherInMe.pointToWorldSpace(v0->getOffset());
    Vector3 p1 = otherInMe.pointToWorldSpace(v1->getOffset());
...
void CellFaceFacePair::vertexInside(
    Primitive* pFace, Primitive* pVertex, const POLY::Vertex* inside, const POLY::Face* planeFace, ConnectorArray& newConnectors)
{
    FaceVertexConnector* answer = new FaceVertexConnector(
        pFace->getBody(), pVertex->getBody(), contactParams, planeFace->plane(), inside->getOffset(), planeFace->getId(), inside->getId());

    AYAASSERT(!Math::isNanInf(answer->computeOverlap()));
...
FaceEdgeConnector* CellFaceFacePair::newFaceEdgeConnector(size_t mainFaceEdgeId, const Vertex* v0, const Vertex* v1)
{
    ...
    FaceEdgeConnector* answer = new FaceEdgeConnector(primitive[0]->getBody(), primitive[1]->getBody(), contactParams, mainFace->plane(),
        mainFace->getSidePlane(mainFaceEdgeId), mainFace->getEdge(mainFaceEdgeId)->computeLine(), penetratingEdge->computeLine(), mainFaceEdgeId,
...
float CellEdgeEdgePair::test()
{
    ...
    for (size_t i = i_low; i < i_high; ++i)
    {
        ...
        if (e0->contains(closestV0))
        { // only test edges containing the extremal vertex
            ...
            for (size_t j = j_low; j < j_high; ++j)
            {
                ...
                if (e1->contains(closestV1))
                {
                    ...
                    if (crossAxis.unitize() > 1e-3f)
                    { // i.e. if not parallel threshold is 1e-3
...
{
    EdgeEdgeConnector* answer = new EdgeEdgeConnector(primitive[0]->getBody(), primitive[1]->getBody(), contactParams, bestEdge0->computeLine(),
        bestEdge1->computeLine(), bestEdge0->getId(), bestEdge1->getId());
    AYAASSERT(answer->computeOverlap());
    return answer;
}
