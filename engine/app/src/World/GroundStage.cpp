...
GroundStage::GroundStage(IStage* upstream, World* world)
    : IWorldStage(upstream, new EdgeStage(this, world), world)
{
}
#pragma warning(pop)
...
{
    return aya_static_cast<EdgeStage*>(getDownstreamWS());
}
...
{
    AYAASSERT(!Joint::findConstJoint(p, Joint::ANCHOR_JOINT));
    AYAASSERT(!Joint::findConstJoint(p, Joint::FREE_JOINT));
...
{
    Joint::JointType jointType = grounded ? Joint::ANCHOR_JOINT : Joint::FREE_JOINT;
...
void GroundStage::onEdgeAdded(Edge* e)
{
    ...
    {
        onKernelJointAdded(aya_static_cast<KernelJoint*>(e));
    }

    Super::onEdgeAdded(e); // put the joint downstream, in the primitive list

    if (Joint::isRigidJoint(e))
    { // do this after - uses the primitive list
        checkForFreeGroundJoint(aya_static_cast<RigidJoint*>(e));
    }
}
...
void GroundStage::onEdgeRemoving(Edge* e)
{
    ...
    if (Joint::isKernelJoint(e))
    {
        onKernelJointRemoving(aya_static_cast<KernelJoint*>(e));
    }
}
...
{
    AYAASSERT(k->getPrimitive(0));
    AYAASSERT(k->getPrimitive(1));
    AYAASSERT(k->getPrimitive(0)->downstreamOfStage(this));
}
...
void GroundStage::checkForFreeGroundJoint(RigidJoint* r)
{
    ...
    AYAASSERT(p0->downstreamOfStage(this));
    AYAASSERT(p1->downstreamOfStage(this));
