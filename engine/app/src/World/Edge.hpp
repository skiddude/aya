...
class AyaBaseClass Edge : public IPipelined
{
private:
...
public:
    typedef enum
    {
        JOINT,
        CONTACT
    } EdgeType; // purely to eliminate dynamic casts
    ...
    virtual ~Edge()
    {
        AYAASSERT(index0 == -1);
        AYAASSERT(index1 == -1);
        AYAASSERT(prim0 == NULL);
        AYAASSERT(prim1 == NULL);
        index0 = -1;
        index1 = -1;
    ...
    void setEdgeState(Sim::EdgeState value)
    {
        edgeState = value;
    }

    Sim::ThrottleType getThrottleType() const
    {
        return throttleType;
    }
    void setThrottleType(Sim::ThrottleType value)
    {
        throttleType = value;
    }
    ...
    Type* fastCast(EdgeType edgeType)
    {
        ...
        AYAASSERT_VERY_FAST(match == (dynamic_cast<Type*>(this) != NULL));
        return match ? static_cast<Type*>(this) : NULL;
    }

    Primitive* getPrimitive(int i)
    {
        AYAASSERT_VERY_FAST((i == 0) || (i == 1));
        return (&prim0)[i];
    }
    ...
    Primitive* otherPrimitive(const Primitive* p)
    {
        return (p == prim0) ? prim1 : prim0;
    }
    Aya::Graphics::CullableSceneNode* otherPrimitive(const Aya::Graphics::CullableSceneNode* p)
    {
        AYAASSERT(NULL);
        return NULL;
    }

    const Primitive* otherConstPrimitive(const Primitive* p) const
    {
        return (p == prim0) ? prim1 : prim0;
    }
    ...
    {
        AYAASSERT_VERY_FAST(links(p));
        return (p == prim0) ? 0 : 1;
    }

    int getIndex(const Primitive* p) const
    {
        AYAASSERT_VERY_FAST(this->links(p));
    ...
    {
        return (((p0 == prim0) && (p1 == prim1)) || ((p0 == prim1) && (p1 == prim0)));
    }
};
