...
RotateJoint* RotateJoint::surfaceTypeToJoint(
    SurfaceType surfaceType, Primitive* axlePrim, Primitive* holePrim, const CoordinateFrame& c0, const CoordinateFrame& c1)
{
    ...
    {
        Vector3 temp;
        float baseAngle = RotateConnector::computeJointAngle(axlePrim->getCoordinateFrame(), holePrim->getCoordinateFrame(), c0, c1, temp);
...
RotateJoint* RotateJoint::canBuildJoint(Primitive* p0, Primitive* p1, NormalId nId0, NormalId nId1)
{
    ...
    if (!constraint0 || (constraint1 && (p1->getSize().sum() > p0->getSize().sum())))
    {
        std::swap(t0, t1);
        std::swap(p0, p1);
        std::swap(nId0, nId1);
    ...
    CoordinateFrame holeInP1(normalIdToMatrix3(intoP1), Math::toGrid(axlePtInP1, Tolerance::mainGrid()));
    if (DFFlag::OrthonormalizeJointCoords)
...
DynamicRotateJoint::DynamicRotateJoint(
    Primitive* axlePrim, Primitive* holePrim, const CoordinateFrame& c0, const CoordinateFrame& c1, float baseAngle)
...
void DynamicRotateJoint::putInKernel(Kernel* _kernel)
{
    ...
    if (!getKernel()->getUsingPGSSolver())
    {
        ...
        rotateConnector = new RotateConnector(
            getAxlePrim()->getBody(), getHolePrim()->getBody(), jointCoord0, jointCoord1, baseAngle, getJointK(), getTorqueArmLength());
...
float DynamicRotateJoint::getChannelValue(double distributedGameTime)
{
    ...
    {
    case LegacyController::CONSTANT_INPUT:
        return paramB;
    case LegacyController::SIN_INPUT:
        return paramA * sin(static_cast<float>(distributedGameTime) * paramB);
...
void DynamicRotateJoint::setBaseAngle(float value)
{ // in joint space
    baseAngle = value;
}
...
void RotatePJoint::putInKernel(Kernel* kernel)
{
    ...
    if (getKernel()->getUsingPGSSolver())
    {
        ...
        Body* b1 = getHolePrim()->getBody();
        if (alignmentConstraint != NULL &&
            (alignmentConstraint->getBodyA()->getUID() != b0->getUID() || alignmentConstraint->getBodyB()->getUID() != b1->getUID()))
        {
            delete alignmentConstraint;
...
RotateVJoint::RotateVJoint(Primitive* axlePrim, Primitive* holePrim, const CoordinateFrame& c0, const CoordinateFrame& c1, float baseAngle)
    : DynamicRotateJoint(axlePrim, holePrim, c0, c1, baseAngle)
