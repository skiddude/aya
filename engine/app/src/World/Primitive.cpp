...
#include "World/WedgePoly.hpp"
#include "World/PrismPoly.hpp"
#include "World/PyramidPoly.hpp"
...
#include "World/Cylinder.hpp"
#include "Utility/Units.hpp"
#include "Utility/Math.hpp"
#include "NetworkOwner.hpp"

#include "btBulletCollisionCommon.hpp"
#include "BulletCollision/BroadphaseCollision/btBroadphaseProxy.hpp"
...
unsigned int Primitive::getSizeMultiplier() const
{
    ...
    {
    case Primitive::DEFAULT_SIZE:
        return 1;
    case Primitive::TORSO_SIZE:
        return 5;
    case Primitive::ROOT_SIZE:
        return 10;
    case Primitive::SEAT_SIZE:
        return 20;
...
void Primitive::setSizeMultiplier(SizeMultiplier value)
{
    if (value != sizeMultiplier)
    {
        ...
        {
            AYAASSERT(0); // show to dave - changing primitive name to/from "Torso" while in world
        }
    }
}
...
{
    AYAASSERT(!Math::isNanInfVector3(getBody()->getPos()));
    Extents answer = Extents::fromCenterCorner(getBody()->getPos(), geometry->getCenterToCorner(getBody()->getCoordinateFrame().rotation));
...
const Extents& Primitive::getFastFuzzyExtents()
{
    ...
    AYAASSERT_VERY_FAST(computeFuzzyExtents() == fuzzyExtents);
    AYAASSERT_VERY_FAST(fuzzyExtentsStateId == getBody()->getStateIndex());
...
Primitive* Primitive::downstreamPrimitive(Joint* j)
{
    ...
    else
    {
        Body* b0 = p0->getBody();
        Body* b1 = p1->getBody();
        AYAASSERT((b0->getParent() == b1) || (b1->getParent() == b0));
...
Edge* EdgeList::getNext(const Primitive* p, Edge* e) const
{
    ...
    return (nextIndex < list.size()) ? list[nextIndex].edge : NULL;
}

void EdgeList::insertEdge(Edge* e)
{
    AYAASSERT(e->getIndex(owner) == -1);
...
void EdgeList::removeEdge(Edge* e)
{
    ...
    AYAASSERT(removeIndex >= 0);
    AYAASSERT(list[removeIndex].edge == e);
...
{
    return aya_static_cast<Joint*>(joints.getEdge(id));
}
...
{
    return aya_static_cast<Contact*>(contacts.getEdge(id));
}
...
void Primitive::insertEdge(Edge* e)
{
    ...
    if (Joint::isJoint(e))
    {
        ...
        {
            AYAASSERT(AnchorJoint::isAnchorJoint(aya_static_cast<Joint*>(e)) || FreeJoint::isFreeJoint(aya_static_cast<Joint*>(e)));
        }
    }
    else
    {
        AYAASSERT_VERY_FAST(Contact::isContact(e));
...
{
    return (joints.size() > 0) ? joints.getEdge(0) : contacts.getFirst();
}

Edge* Primitive::getNextEdge(Edge* e) const
{
    AYAASSERT_VERY_FAST(e);
...
{
    return aya_static_cast<Joint*>(joints.getFirst());
}
...
{
    AYAASSERT_VERY_FAST(prev);

    Edge* e = joints.getNext(this, const_cast<Joint*>(prev));

    return aya_static_cast<const Joint*>(e);
}
...
void Primitive::setWorld(World* _world)
{
    world = _world;
    if (_world != NULL)
    {
        this->body->updateGravity(world->getGravity());
    }
}
...
{
    return aya_static_cast<Contact*>(contacts.getFirst());
}
...
Contact* Primitive::getNextContact(Contact* prev)
{
    ...
    return aya_static_cast<Contact*>(contacts.getNext(this, prev));
}
...
RigidJoint* Primitive::getFirstRigidAt(Joint* start)
{
    while (start)
    {
        ...
        {
            return aya_static_cast<RigidJoint*>(start);
        }
...
Joint* Primitive::getJoint(Primitive* p0, Primitive* p1, int index)
{
    ...
    for (int i = 0; i < leastJoints->getNumJoints(); ++i)
    {
        ...
        if (other == mostJoints)
        {
            AYAASSERT(leastJoints->getJoint(i)->links(p0, p1));
            if (foundId == index)
            {
...
Contact* Primitive::getContact(Primitive* p0, Primitive* p1)
{
    ...
    for (int i = 0; i < leastContacts->getNumContacts(); ++i)
    {
        ...
        {
            AYAASSERT(leastContacts->getContact(i)->links(p0, p1));
...
static void reportOverlap(Primitive* touchReporting, Primitive* touchOther)
{
    ...
    AYAASSERT(touchReportingAssembly);
    AYAASSERT(touchOtherAssembly);
...
Geometry* Primitive::newGeometry(Geometry::GeometryType geometryType)
{
    ...
    case Geometry::GEOMETRY_WEDGE:
        return new WedgePoly();

    case Geometry::GEOMETRY_PRISM:
        return new PrismPoly();

    case Geometry::GEOMETRY_PYRAMID:
        return new PyramidPoly();

    case Geometry::GEOMETRY_PARALLELRAMP:
        return new ParallelRampPoly();

    case Geometry::GEOMETRY_RIGHTANGLERAMP:
        return new RightAngleRampPoly();

    case Geometry::GEOMETRY_CORNERWEDGE:
        return new CornerWedgePoly();

    case Geometry::GEOMETRY_MEGACLUSTER:
        return new MegaClusterPoly(this);

    case Geometry::GEOMETRY_SMOOTHCLUSTER:
        return new SmoothClusterGeometry(this);

    case Geometry::GEOMETRY_TRI_MESH:
...
{
    // This if statement is because we have Aya Assert that we don't want to trigger
...
{
    AYAASSERT_VERY_FAST(!hasAutoJoints());
...
Vector3 Primitive::clipToSafeSize(const Vector3& newSize)
{
    ...
    if ((safeSize.x * safeSize.y * safeSize.z) > maxVolume) // up from 1 million
    {
        ...
        AYAASSERT_VERY_FAST(safeSize.x * safeSize.y * safeSize.z <= (maxVolume * 1.01f));
    }
    return safeSize;
}
...
{
    return CoordinateFrame(normalIdToMatrix3(objectFace), 0.5f * normalIdToVector3(objectFace) * geometry->getSize());
}
...
void Primitive::setPV(const PV& newPv)
{
    ...
    if (newPv != bodyPv)
    {
        ...
        AYAASSERT((world == NULL) == (assembly == NULL));
...
void Primitive::setSurfaceType(NormalId id, SurfaceType newSurfaceType)
{
    ...
    AYAASSERT_IF_VALIDATING(!hasAutoJoints());
...
void Primitive::calculateSortSize()
{
    ...
    AYAASSERT(planar * 1000.0f + 1 < (float)std::numeric_limits<unsigned int>::max());
    AYAASSERT(planar > 0.0f);
