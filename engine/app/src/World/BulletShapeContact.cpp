...
#include "BulletCollision/CollisionShapes/btConvexInternalShape.hpp"


namespace Aya
{

BulletShapeContact::BulletShapeContact(Primitive* p0, Primitive* p1, World* contactWorld)
    : Contact(p0, p1)
    , bulletManifold(NULL)
    ...
    btCollisionAlgorithm* algo, int manifoldIndex, int contactIndex, bool swapped = false)
{
    if (contactParams)
    {
        return new BulletShapeConnector(getPrimitive(0)->getBody(), getPrimitive(1)->getBody(), *contactParams, bulletColObj0, bulletColObj1, algo,
            manifoldIndex,
...
void BulletShapeContact::findClosestFeatures(BulletConnectorArray& newConnectors)
{
    ...
    if (Math::isNanInfVector3(getPrimitive(0)->getCoordinateFrame().translation) ||
        Math::isNanInfVector3(getPrimitive(1)->getCoordinateFrame().translation))
        return;
            ...
            if (valid && dis < 0.0f)
            {
                ...
                if (totalNumConnectors < BULLET_CONTACT_ARRAY_SIZE)
                {
                    BulletShapeConnector* cylConn = newBulletShapeConnector(getPrimitive(0)->getGeometry()->getBulletCollisionObject(),
                        getPrimitive(1)->getGeometry()->getBulletCollisionObject(), bulletNPAlgorithm, i, j);
                    if (cylConn)
                    {
