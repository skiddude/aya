...
public:
    ...
    static bool sendingPhase(Assembly::FilterPhase phase)
    {
        return (phase == Assembly::NoSim_Send) || (phase == Assembly::NoSim_Send_Anim);
    }
    static bool simulatingPhase(Assembly::FilterPhase phase)
    {
        return ((phase == Assembly::Sim_SendIfSim) || (phase == Assembly::Sim_BufferZone));
    }
    static bool noSimPhase(Assembly::FilterPhase phase)
    {
        return ((phase == Assembly::NoSim_Send) || (phase == Assembly::NoSim_Send_Anim) || (phase == Assembly::NoSim_SendIfSim) ||
                (phase == Assembly::NoSim_SendIfSim_Anim));
    }
    static bool animationPhase(Assembly::FilterPhase phase)
    {
        return ((phase == Assembly::NoSim_Send_Anim) || (phase == Assembly::NoSim_SendIfSim_Anim));
    }
...
private:
    ...
    public:
        ...
        MoveInstructions()
            : a(NULL)
            , from(Assembly::NOT_ASSIGNED)
            , to(Assembly::NOT_ASSIGNED)
        {
        }

        MoveInstructions(Assembly* _a, Assembly::FilterPhase _from, Assembly::FilterPhase _to)
            : a(_a)
            , from(_from)
            , to(_to)
        {
...
public:
    ...
    SimSendFilter& getSimSendFilter()
    {
        return filter;
    }
    ...
    {
        AYAASSERT(phase < Assembly::NUM_PHASES);
