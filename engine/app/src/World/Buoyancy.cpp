...
void BuoyancyContact::deleteConnectors()
{
    ...
    for (size_t i = 0; i < connectors.size(); ++i)
    {
        AYAASSERT(!connectors[i]->isInKernel());
        delete connectors[i];
    }
...
{
    if (Aya::PartInstance::fromPrimitive(primitive)->getPartType() == CYLINDER_PART)
...
void BuoyancyContact::updateBuoyancyFloatingForce()
{
    ...
    if (getPrimitive(0)->getWorld()->getUsingNewPhysicalProperties())
    {
        ...
        fullBuoyancy.y = -floaterPrim->getGeometry()->getVolume() * Units::kmsAccelerationToRbx(-9.81f) * waterDensity;
    }
    else
    {
        ...
        fullBuoyancy.y = -floaterPrim->getConstBody()->getMass() * (this->findWorld()->getGravity() / floaterPrim->getSpecificGravity());
    }
}
...
void BuoyancyContact::updateConnectors()
{
    ...
    float viscosity_K = -waterViscosity / Aya::Constants::kernelDt() / connectors.size();
...
{
    AYAASSERT(connectors.size() == 1); // Ball has only one connector
...
Aya::Vector3 BuoyancyBallContact::getWaterVelocity(int)
{
    Vector3 center = floaterPrim->getPV().position.translation;
...
Vector3 BuoyancyBoxContact::getCrossSections(int i, const Vector3& velocity)
{
    ...
    ;
    return Vector3(crossAreas[sign.x ^ index.x].x, crossAreas[sign.y ^ index.y].y, crossAreas[sign.z ^ index.z].z);
}
...
void BuoyancyBoxContact::getSurfaceAreaInDirection(const Vector3& relativeVelocityDir, float& crossArea, float& tangentArea)
{
    ...
    AYAASSERT(Math::fuzzyEq(componentWeights.x + componentWeights.y + componentWeights.z, 1.0f, 1.0e-4f));
...
BuoyancyBoxContact::BuoyancyBoxContact(Primitive* p0, Primitive* p1)
...
void Aya::BuoyancyCylinderContact::updateSubmergeRatio()
{
    ...
    float startPointRatio = (connectors[0]->getSubMergeRatio() + connectors[1]->getSubMergeRatio() + connectors[2]->getSubMergeRatio() +
                                connectors[3]->getSubMergeRatio()) *
                                0.5f -
                            1.0f;
    float endPointRatio = (connectors[4]->getSubMergeRatio() + connectors[5]->getSubMergeRatio() + connectors[6]->getSubMergeRatio() +
                              connectors[7]->getSubMergeRatio()) *
                              0.5f -
                          1.0f;
