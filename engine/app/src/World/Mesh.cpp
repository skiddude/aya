...
void Mesh::makeParallelRamp(const Vector3& size, Vector3& cofm)
{
    ...
    // float connectionThickness = Aya::PartInstance::brickHeight();
...
Edge* Mesh::findOrMakeEdge(size_t v0, size_t v1)
{
    ...
    {
        AYAASSERT(found->getVertex(NULL, 0) == vert1); // should be backwards - this is second face
        AYAASSERT(found->getVertex(NULL, 1) == vert0);
        AYAASSERT(found->getForward());
        AYAASSERT(!found->getBackward());
        return found;
    }
    else
    {
...
{
    AYAASSERT(pointInMesh(inside));
    AYAASSERT(!pointInMesh(outside));
...
{
    AYAASSERT(!pointInMesh(p0));
    AYAASSERT(!pointInMesh(p1));
    f0 = NULL;
    f1 = NULL;
...
{
    AYAASSERT(pointInInternalExtrusion(pBelowInside));
    AYAASSERT(!pointInInternalExtrusion(pBelowOutside));
...
{
    AYAASSERT(!pointInInternalExtrusion(p0));
    AYAASSERT(!pointInInternalExtrusion(p1));
...
bool Face::lineCrossesExtrusionSideBelow(const Vector3& p0, const Vector3& p1, size_t edgeId) const
{
    ...
    if (sidePlane.halfSpaceContains(p0) != sidePlane.halfSpaceContains(p1))
    { // points must be on opposite sides!
