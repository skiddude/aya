...
#include "LinearMath/btConvexHull.hpp"
...
#include "Extras/ConvexDecomposition/ConvexBuilder.hpp"
...
// Update Physics Data
void TriangleMesh::updateObjectScale(
    const std::string& decompKey, const std::string& decompStr, const G3D::Vector3& scale, const G3D::Vector3& meshScale)
{
    if (decompStr != "")
    {
...
std::string TriangleMesh::generateConvexHullData(
    int numTriangles, const unsigned int* triangleIndexBase, int numVertices, const btVector3* vertexBase)
{
    int currentVersion;
    std::stringstream decompStream;
...
BulletDecompWrapper::ShapeType* TriangleMesh::retrieveDecomposition(const std::string& str)
{
    ...
    for (unsigned int i = 0; i < meshConvexes.size(); i++)
    {
        ...
        btCollisionShape* convexShape =
            new btConvexHullShape(&(meshConvexes[i].vertices[0].getX()), meshConvexes[i].vertices.size(), sizeof(btVector3));
...
void TriangleMesh::readConvexHullData(std::vector<float>& vertices, unsigned int& numVertices, std::vector<unsigned int>& indices,
...
bool TriangleMesh::findTouchingSurfacesConvex(
...
bool TriangleMesh::FacesOverlapped(
