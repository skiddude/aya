...
static void computeManifolds(
    World* world, btCollisionAlgorithm*& algorithm, btManifoldArray& manifoldArray, btCollisionObject* colObj0, btCollisionObject* colObj1)
{
        ...
        for (int j = 0; j < manifold->getNumContacts(); ++j)
        {
            ...
            if (pt.getDistance() < 0)
            {
                if (pt.m_userPersistentData)
                {
                    ...
                    AYAASSERT(conn->bulletManifoldIndex < 0);
    ...
    for (int i = 0; i < manifoldArray.size(); ++i)
    {
        ...
        for (int j = 0; j < manifold->getNumContacts(); ++j)
        {
            ...
            if (!pt.m_userPersistentData && pt.getDistance() < 0)
            {
                ...
                {
                    AYAASSERT(!"Contact array is full, dropping contact");
                }
            }
        }
    }
...
bool BulletContact::computeIsColliding(float overlapIgnored)
{
    ...
    computeManifolds(
        world, algorithm, manifoldArray, prim0->getGeometry()->getBulletCollisionObject(), prim1->getGeometry()->getBulletCollisionObject());
...
BulletCellContact::BulletCellContact(
    World* world, Primitive* p0, Primitive* p1, const Vector3int32& feature, const shared_ptr<btCollisionShape>& cellShape)
