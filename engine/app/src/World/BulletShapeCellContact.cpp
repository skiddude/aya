...
#include "Kernel/BulletShapeConnectors.hpp"
    ...
    , world(contactWorld)
{
    ...
    btCollisionShape* bulletCellShape = aya_static_cast<MegaClusterPoly*>(p0->getGeometry())->getBulletCellShape(type);
...
void BulletShapeCellContact::updateContactParemeters(btCollisionObject* cellObj, BulletConnectorArray& connectors)
{
    ...
    for (size_t i = 0; i < connectors.size(); ++i)
    {
        ...
        {
            PartMaterial terrainContactMaterial =
                SmoothClusterGeometry::getTriangleMaterial(cellObj->getCollisionShape(), triangleIndex, connPoint.position);
...
BulletShapeCellConnector* BulletShapeCellContact::newBulletShapeCellConnector(
    btCollisionObject* bulletColObj0, btCollisionObject* bulletColObj1, btCollisionAlgorithm* algo, int manifoldIndex, int contactIndex)
{
    if (contactParams)
    {
        return new BulletShapeCellConnector(getPrimitive(0)->getBody(), getPrimitive(1)->getBody(), *contactParams, bulletColObj0, bulletColObj1,
            algo, manifoldIndex,
            contactIndex); // id of this face
    }
    else
        return NULL;
}
...
void BulletShapeCellContact::findClosestBulletCellFeatures(BulletConnectorArray& newConnectors)
{
    ...
    for (int i = 0; i < manifoldArray.size(); i++)
    {
        ...
        for (int j = 0; j < manifold->getNumContacts(); j++)
        {
            ...
            if (valid && dis < 0.0f)
            {
                AYAASSERT(totalNumConnectors < BULLET_CONTACT_ARRAY_SIZE);
                if (totalNumConnectors < BULLET_CONTACT_ARRAY_SIZE)
                {
                    totalNumConnectors++;
                    BulletShapeCellConnector* cylConn = newBulletShapeCellConnector(
                        &bulletCollisionObject, getPrimitive(1)->getGeometry()->getBulletCollisionObject(), bulletNPAlgorithm, i, j);
                    if (cylConn)
...
void BulletShapeCellContact::computeManifoldsWithBulletNarrowPhase(btManifoldArray& manifoldArray)
{
        ...
        colObj1->getCollisionShape()->getShapeType() == GIMPACT_SHAPE_PROXYTYPE)
    {
        ...
        AYAASSERT(manifoldArray.size() <= 1);
