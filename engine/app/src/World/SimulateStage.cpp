...
{
    AYAASSERT(movingDynamicAssemblies.size() == 0);
    AYAASSERT(realTimeAssemblies.size() == 0);
    AYAASSERT(movingAssemblyRoots.size() == 0);
}
...
{
    AYAASSERT(!a->getAssemblyPrimitive()->requestFixed());
...
void SimulateStage::putFirstMovingRootInSendPhysics(Assembly* a)
{
    ...
    AYAASSERT(Mechanism::isMovingAssemblyRoot(movingRoot));
