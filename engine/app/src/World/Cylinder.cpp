...
Vector3 Cylinder::getCenterToCorner(const Matrix3& rotation) const
{
    ...
    Vector3 newSize = Vector3(fabs(rotation[0][0]) * size[0] + fabs(rotation[0][1]) * size[1] + fabs(rotation[0][2]) * size[2],
...
bool Cylinder::findTouchingSurfacesConvex(
...
bool Cylinder::FaceVerticesOverlapped(
    const CoordinateFrame& myCf, size_t& myFaceId, const Geometry& otherGeom, const CoordinateFrame& otherCf, size_t& otherFaceId, float tol) const
{
    return false;
}

bool Cylinder::FaceEdgesOverlapped(
    const CoordinateFrame& myCf, size_t& myFaceId, const Geometry& otherGeom, const CoordinateFrame& otherCf, size_t& otherFaceId, float tol) const
{
    return false;
}
