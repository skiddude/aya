...
void TerrainPartitionMega::findCellsInRegion(const SpatialRegion::Id& region, const ChunkData& chunk, const Vector3int16& minOffset,
    const Vector3int16& maxOffset, std::vector<Vector3int16>* found) const
{
...
void TerrainPartitionMega::terrainCellChanged(const Voxel::CellChangeInfo& info)
{
    ...
    if (beforeNotSolid != afterNotSolid)
    {
        ...
        if (beforeNotSolid)
        {
            AYAASSERT((chunk.filled[localPos.y >> 1][localPos.z >> 2][localPos.x >> 2] & bitMask) == 0);
...
template<int Size>
static void dilate(uint32_t (&data)[Size][Size])
{
    // Dilate along X
...
void TerrainPartitionSmooth::updateChunk(const Vector3int32& id)
{
    ...
    AYAASSERT(box.getSizeX() == size && box.getSizeY() == size && box.getSizeZ() == size);
...
void TerrainPartitionSmooth::fillChunkIfTouchingExtents(const Vector3int32& chunkId, const ChunkData& chunkData, const Vector3int32& minPos,
    const Vector3int32& maxPos, std::vector<ChunkResult>* found) const
{
    ...
    AYAASSERT(regionMin.x <= regionMax.x && regionMin.y <= regionMax.y && regionMin.z <= regionMax.z);
