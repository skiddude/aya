...
#include "BulletCollision/NarrowPhaseCollision/btRaycastCallback.hpp"
...
MegaClusterPoly::MegaClusterPoly(Primitive* p)
    : myPrim(p)
{
    Grid* grid = aya_static_cast<MegaClusterInstance*>(myPrim->getOwner())->getVoxelGrid();
...
bool MegaClusterPoly::hitTest(
    const RbxRay& rayInMe, Vector3& localHitPoint, Vector3& surfaceNormal, float searchRayMax, bool treatCellsAsBlocks, bool ignoreWater)
{
    int dummySurf = -1;
...
const Grid::Region getRegionForCellLocation(const Grid* store, const Vector3int16& location, Grid::Region* previousRegion = NULL)
{
    ...
    {
        Region3int16 extents = SpatialRegion::inclusiveVoxelExtentsOfRegion(SpatialRegion::regionContainingVoxel(location));
        return store->getRegion(extents.getMinPos(), extents.getMaxPos());
    }
}

bool MegaClusterPoly::hitTestMC(const RbxRay& rayInMe, Vector3& localHitPoint, Vector3& surfaceNormal, int& surfId, CoordinateFrame& surfaceCf,
    float searchRayMax, bool treatCellsAsBlocks, bool ignoreWater)
{
    // we assume rayInMe is a unit ray, which means when time passed > searchRayMax, then the distance the ray has traveled will also be >
    // searchRayMax
    if (searchRayMax > MC_SEARCH_RAY_MAX)
        searchRayMax = MC_SEARCH_RAY_MAX;
    ...
    for (int index = 0; index < 3; ++index)
    {
        ...
        AYAASSERT(boost::math::isfinite(rayInMe.origin()[index]));
        AYAASSERT(boost::math::isfinite(rayInMe.direction()[index]));
    ...
    while (!doneSearching)
    {
        ...
        if (gridTest.solid.getBlock() != CELL_BLOCK_Empty)
        {
            ...
            case CELL_BLOCK_VerticalWedge:
                foundIntersection = doneSearching =
                    hitLocationOnVerticalWedgeCell(rayInMe, testCell, orientation, localHitPoint, surfaceNormal, surfaceCf);
                if (doneSearching)
                    hitLocationOnBlockCell(
                        rayInMe, testCell, dummyHitPoint, dummySurfaceNormal, surfId, dummyCf); // we get the surfaceId of the last cube hit
                break;
            case CELL_BLOCK_HorizontalWedge:
                foundIntersection = doneSearching =
                    hitLocationOnHorizontalWedgeCell(rayInMe, testCell, orientation, localHitPoint, surfaceNormal, surfaceCf);
                if (doneSearching)
                    hitLocationOnBlockCell(
                        rayInMe, testCell, dummyHitPoint, dummySurfaceNormal, surfId, dummyCf); // we get the surfaceId of the last cube hit
                break;
            case CELL_BLOCK_CornerWedge:
                foundIntersection = doneSearching =
                    hitLocationOnCornerWedgeCell(rayInMe, testCell, orientation, localHitPoint, surfaceNormal, surfaceCf);
                if (doneSearching)
                    hitLocationOnBlockCell(
                        rayInMe, testCell, dummyHitPoint, dummySurfaceNormal, surfId, dummyCf); // we get the surfaceId of the last cube hit
                break;
            case CELL_BLOCK_InverseCornerWedge:
                foundIntersection = doneSearching =
                    hitLocationOnInverseCornerWedgeCell(rayInMe, testCell, orientation, localHitPoint, surfaceNormal, surfaceCf);
                if (doneSearching)
                    hitLocationOnBlockCell(
                        rayInMe, testCell, dummyHitPoint, dummySurfaceNormal, surfId, dummyCf); // we get the surfaceId of the last cube hit
                break;
...
bool MegaClusterPoly::findTouchingSurfacesConvex(
    const CoordinateFrame& myCf, size_t& myFaceId, const Geometry& otherGeom, const CoordinateFrame& otherCf, size_t& otherFaceId) const
{
    AYAASSERT(false); // not implemented
    return false;
}
...
bool MegaClusterPoly::findPlanarTouchesWithGeom(
    const CoordinateFrame& myCf, const Geometry& otherGeom, const CoordinateFrame& otherCf, std::vector<Vector3int16>* cells) const
{
...
bool MegaClusterPoly::hasPlanarTouchWithGeom(
    const Vector3int16& cell, const CoordinateFrame& myCf, const Geometry& otherGeom, const CoordinateFrame& otherCf) const
{
    size_t tempFaceId;
    return findCellIntersectionWithGeom(cell, myCf, otherGeom, otherCf, tempFaceId).size() > 2;
}

std::vector<Vector3> MegaClusterPoly::findCellIntersectionWithGeom(
    const Vector3int16& cell, const CoordinateFrame& myCf, const Geometry& otherGeom, const CoordinateFrame& otherCf, size_t& otherFaceId) const
{
    std::vector<Vector3> result;

    // If the otherGeom is a ball, do not compute intersection
    // TODO: If otherGeom is CSG...
    if (otherGeom.getGeometryType() == Geometry::GEOMETRY_BALL || otherGeom.getGeometryType() == Geometry::GEOMETRY_CYLINDER ||
        otherGeom.getGeometryType() == Geometry::GEOMETRY_TRI_MESH)
        return result;

    const Poly* otherPoly = aya_static_cast<const Poly*>(&otherGeom);
...
bool MegaClusterPoly::hitLocationOnCornerWedgeCell(const RbxRay& rayInMe, const Vector3int16& testCell, const int& orientation,
    Vector3& localHitPoint, Vector3& surfaceNormal, CoordinateFrame& hitSurfaceCoord) const
{
...
bool MegaClusterPoly::hitLocationOnHorizontalWedgeCell(const RbxRay& rayInMe, const Vector3int16& testCell, const int& orientation,
    Vector3& localHitPoint, Vector3& surfaceNormal, CoordinateFrame& hitSurfaceCoord) const
{
    // Depending on orientation, specify faces and check for intersection
...
bool MegaClusterPoly::hitLocationOnVerticalWedgeCell(const RbxRay& rayInMe, const Vector3int16& testCell, const int& orientation,
    Vector3& localHitPoint, Vector3& surfaceNormal, CoordinateFrame& hitSurfaceCoord) const
{
    // Depending on orientation, specify faces and check for intersection
...
bool MegaClusterPoly::hitLocationOnInverseCornerWedgeCell(const RbxRay& rayInMe, const Vector3int16& testCell, const int& orientation,
    Vector3& localHitPoint, Vector3& surfaceNormal, CoordinateFrame& hitSurfaceCoord) const
{
    // Depending on orientation, specify faces and check for intersection
...
bool MegaClusterPoly::hitLocationOnBlockCell(const RbxRay& rayInMe, const Vector3int16& testCell, Vector3& localHitPoint, Vector3& surfaceNormal,
    int& surfId, CoordinateFrame& hitSurfaceCoord) const
{
    Vector3 t0(1, 1, 1);
    Vector3 t1(1, 1, -1);
    Vector3 t2(-1, 1, -1);
...
void MegaClusterPoly::findCellsTouchingGeometry(
    const CoordinateFrame& myCf, const Geometry& otherGeom, const CoordinateFrame& otherCf, std::vector<Vector3int16>* found) const
{
    findCellsTouchingGeometryWithBuffer(0.0, myCf, otherGeom, otherCf, found);
}

void MegaClusterPoly::findCellsTouchingGeometryWithBuffer(const float& buffer, const CoordinateFrame& myCf, const Geometry& otherGeom,
    const CoordinateFrame& otherCf, std::vector<Vector3int16>* found) const
{
    ...
    for (counter.y = minChunkId.value().y; counter.y <= maxChunkId.value().y; counter.y++)
    {
        for (counter.z = minChunkId.value().z; counter.z <= maxChunkId.value().z; counter.z++)
        {
            for (counter.x = minChunkId.value().x; counter.x <= maxChunkId.value().x; counter.x++)
            {
                ...
                Grid::Region region = grid->getRegion(minQueryCoord, maxQueryCoord);
                if (region.isGuaranteedAllEmpty())
                {
                    continue;
                }

                for (Grid::Region::iterator itr = region.begin(); itr != region.end(); ++itr)
                {
...
{
    AYAASSERT(shape >= 0 && shape < CELL_BLOCK_Empty);
