...
float SleepStage::highVelocityContact()
{
    return 1.0;
} // studs/second
...
SleepStage::SleepStage(IStage* upstream, World* world)
    : IWorldStage(upstream, new SimulateStage(this, world), world)
...
{
    AYAASSERT(numContactsInStage == 0);
    AYAASSERT(numContactsInKernel == 0);
    AYAASSERT(debugReentrant == 0);

    AYAASSERT(recursiveWakePending.empty()); // only on impact...
    AYAASSERT(wakePending.empty());
    AYAASSERT(awake.empty());
    AYAASSERT(sleepingChecking.empty()); // Edges that are awake
    AYAASSERT(sleepingDeeply.empty());   // no Edges that are awake
    AYAASSERT(removing.empty());
    AYAASSERT(steppingContacts[Sim::CAN_NOT_THROTTLE].size() == 0);
    AYAASSERT(touchingContacts[Sim::CAN_NOT_THROTTLE].size() == 0);
    AYAASSERT(steppingContacts[Sim::CAN_THROTTLE].size() == 0);
    AYAASSERT(touchingContacts[Sim::CAN_THROTTLE].size() == 0);
}
...
bool SleepStage::validate()
{
    for (AssemblySetIt aIt = awake.begin(); aIt != awake.end(); ++aIt)
    {
        ...
        AYAASSERT(assembly->getAssemblyState() == Sim::AWAKE);
...
{
    AYAPROFILER_SCOPE("Physics", "stepSleepStage");
    ...
    AYAASSERT_IF_VALIDATING(validate());

    stepAssembliesRecursiveWakePending();
    stepAssembliesWakePending();

    AYAASSERT(wakePending.empty());
    AYAASSERT(recursiveWakePending.empty());
    ...
    AYAASSERT(recursiveWakePending.empty());
    AYAASSERT_IF_VALIDATING(validate());
    AYAASSERT_IF_VALIDATING(validateJoints());
}

void SleepStage::doContacts(ContactLists& contactLists)
{
    Aya::Profiling::Mark mark(*profilingCollision, false);
...
{
    AYAPROFILER_SCOPE("Physics", "stepAssembliesRecursiveWakePending");
...
{
    AYAPROFILER_SCOPE("Physics", "stepAssembliesWakePending");
...
{
    Aya::Profiling::Mark mark(*profilingWake, false);
    AYAASSERT_IF_VALIDATING(validate());
    ...
    {
        Assembly* a = *it;
        AYAASSERT(a->getAssemblyState() == checkState);
        AYAASSERT(a->getRecursiveDepth() == -1);
    ...
    while (!aDeque.empty())
    {
        ...
        aDeque.pop_front();
        AYAASSERT(a->getRecursivePassId() == recursivePassId);
        AYAASSERT(a->getRecursiveDepth() >= 0);
        AYAASSERT(a->getRecursiveDepth() <= maxDepth);
...
{
    AYAASSERT(e->getThrottleType() != Sim::UNDEFINED_THROTTLE);
...
{
    AYAPROFILER_SCOPE("Physics", "stepContacts");

    AYAASSERT(toSleeping.empty());
    AYAASSERT(toStepping.empty());
    AYAASSERT(toContacting.empty());
    AYAASSERT(toContactingSleeping.empty());
    ...
    for (int i = 0; i < contacts.size(); ++i)
    {
        ...
        AYAASSERT(initialSize == contacts.size());
        AYAASSERT(!throttling || !canThrottle(c)); // if throttling, the contact should be one that "can't"
        AYAASSERT(c->inOrDownstreamOfStage(this));
        AYAASSERT(p0);
        AYAASSERT(p1);
        AYAASSERT(p0->getAssembly());
        AYAASSERT(p1->getAssembly());

#ifdef __AYA_CRASH_ASSERT_HP
        Joint* test = Primitive::getJoint(p0, p1);
        AYAASSERT_VERY_FAST(!test || !test->inOrDownstreamOfStage(this));
#endif
        ...
        AYAASSERT(c->getEdgeState() != Sim::UNDEFINED);
        ...
        if (newState != c->getEdgeState())
        {
            ...
            case Sim::SLEEPING:
                ...
                break;
            case Sim::CONTACTING_SLEEPING:
                toContactingSleeping.push_back(c);
                break;
            case Sim::STEPPING:
                toStepping.push_back(c);
                break;
            case Sim::CONTACTING:
                toContacting.push_back(c);
                break;
            default:
                break;
            }
    ...
    AYAPROFILER_LABELF("Physics", "%d contacts, %d connectors", initialSize, numConnectors);
}

void SleepStage::stepJoints()
{
    Aya::Profiling::Mark mark(*profilingJointSleep, false);
    AYAASSERT(toSleepingJoint.empty());
    ...
    {
        Joint* j = *it;
        AYAASSERT(j->inOrDownstreamOfStage(this));
...
{
    Aya::Profiling::Mark mark(*profilingWake, false);
    AYAASSERT(toDeep.empty());
    AYAASSERT(toSleepingChecking.empty());
...
{
    Aya::Profiling::Mark mark(*profilingSleep, false);
    AYAASSERT(toWake.empty());
    AYAASSERT(toDeep.empty());

    for (AssemblySetIt it = sleepingChecking.begin(); it != sleepingChecking.end(); ++it)
    {
        Assembly* assembly = *it;
        AYAASSERT(assembly->getAssemblyState() == Sim::SLEEPING_CHECKING);
            ...
            if (e->inOrDownstreamOfStage(this))
            {
                ...
                if (isAffecting(e))
                {
                    ...
                    if (currentDepth < maxDepth)
                    {
                        ...
                        {
                            AYAASSERT(other->getRecursiveDepth() == -1);
                    ...
                    else if (DFFlag::PGSWakeOtherAssemblyForJoints && getKernel()->getUsingPGSSolver() && e->getEdgeType() == Edge::JOINT && !visited)
                    {
                        AYAASSERT(other->getRecursiveDepth() == -1);
...
bool SleepStage::isAffecting(Edge* e)
{
    ...
    AYAASSERT(e->getEdgeState() != Sim::UNDEFINED);
...
void SleepStage::wakeEdge(Edge* e)
{
    ...
    if (Contact::isContact(e))
    {
        ...
        {
            changeContactState(aya_static_cast<Contact*>(e), Sim::CONTACTING);
        }
        else if (state == Sim::SLEEPING)
        {
            changeContactState(aya_static_cast<Contact*>(e), Sim::STEPPING);
        }
    }
    else
    {
        ...
        {
            changeJointState(aya_static_cast<Joint*>(e), Sim::STEPPING);
        }
    }
}
...
/*
                                                ...
                                                canCollide			!canCollide			canCollide
!canCollide wasTouching inContact			CONTACTING			STEPPING			CONTACTING_SLEEPING
SLEEPING
...
void SleepStage::wakeEvent(Assembly* a)
{
    ...
    if ((state != Sim::ANCHORED) && (state != Sim::AWAKE) // added this line 3/6/09 - am I missing anything? /DB
        && (state != Sim::WAKE_PENDING) && (state != Sim::RECURSIVE_WAKE_PENDING))
    {
...
void SleepStage::changeContactState(Contact* c, Sim::EdgeState newState)
{
    ...
    AYAASSERT(currentState != newState);
...
{
    AYAASSERT(!debugReentrant);
    debugReentrant = true;

    if (!a->inOrDownstreamOfStage(this))
    {
        AYAASSERT(a->inStage(IStage::HUMANOID_STAGE));
    }
    else
    {
        Sim::AssemblyState currentState = a->getAssemblyState();
        AYAASSERT(currentState != newState);
        AYAASSERT(currentState != Sim::ANCHORED);
        ...
        { // INTO KERNEL
            if (!a->downstreamOfStage(this))
            {
                ...
                aya_static_cast<SimulateStage*>(getDownstreamWS())->onAssemblyAdded(a);
            }
        }
        if (Sim::outOfKernelAssemblyState(newState))
        { // OUT OF KERNEL - note "wake pending" do not force into or out of kernel"
            if (a->downstreamOfStage(this))
            {
                if (Sim::isSleepingAssemblyState(newState))
                    a->getAssemblyPrimitive()->getBody()->getRootSimBody()->clearVelocity();
                aya_static_cast<SimulateStage*>(getDownstreamWS())->onAssemblyRemoving(a);
            }
        }
...
{
    AYAASSERT(!assembly->computeIsGrounded());
    AYAASSERT(assembly->getAssemblyState() == Sim::ANCHORED);
    ...
    else
    {
        ...
        AYAASSERT(ok);
        AYAASSERT(assembly->getAssemblyState() == Sim::WAKE_PENDING);
    }
}

void SleepStage::onAssemblyRemoving(Assembly* assembly)
{
    AYAASSERT(assembly->inOrDownstreamOfStage(this));
    AYAASSERT(assembly->getAssemblyState() != Sim::ANCHORED);
    AYAASSERT(assembly->getAssemblyState() != Sim::REMOVING);
...
void SleepStage::onEdgeAdded(Edge* e)
{
    ...
    if (Contact::isContact(e))
    {
        Contact* c = aya_static_cast<Contact*>(e);
...
void SleepStage::onEdgeRemoving(Edge* e)
{
    ...
    if (Contact::isContact(e))
    {
        if ((e->getEdgeState() == Sim::CONTACTING_SLEEPING) || (e->getEdgeState() == Sim::CONTACTING))
        {
            wakeEvent(e);
        }
        if (e->getEdgeState() != Sim::SLEEPING)
        {
            changeContactState(aya_static_cast<Contact*>(e), Sim::SLEEPING);
        }
        numContactsInStage--;
    }
    else
    {
        ...
        {
            changeJointState(aya_static_cast<Joint*>(e), Sim::SLEEPING);
        }
    }

    AYAASSERT(e->inStage(this));
    AYAASSERT(e->getEdgeState() == Sim::SLEEPING);
    AYAASSERT(e->getThrottleType() != Sim::UNDEFINED_THROTTLE);
