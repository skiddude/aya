...
#include "Extras/ConvexDecomposition/ConvexDecomposition.hpp"
...
public:
    ...
    // Getters
    const BulletDecompWrapper* getCompound() const
    {
    ...
    void updateObjectScale(
        const std::string& decompKey, const std::string& decompStr, const G3D::Vector3& scale, const G3D::Vector3& meshScale = Vector3(-1, -1, -1));
    ...
    static void readConvexHullData(std::vector<float>& vertices, unsigned int& numVertices, std::vector<unsigned int>& indices,
        unsigned int& numIndices, btTransform& trans, std::stringstream& stream);
    ...
    {
        return GEOMETRY_TRI_MESH;
    }
    /*override*/ virtual CollideType getCollideType() const
    {
    ...
    {
        return boundingBoxMesh->getMesh()->numFaces();
    }
    ...
    /*override*/ virtual bool findTouchingSurfacesConvex(
    ...
    /*override*/ virtual bool FacesOverlapped(const CoordinateFrame& myCf, size_t& myFaceId, const Geometry& otherGeom,
        const CoordinateFrame& otherCf, size_t& otherFaceId, float tol) const;
    /*override*/ virtual bool FaceVerticesOverlapped(const CoordinateFrame& myCf, size_t& myFaceId, const Geometry& otherGeom,
        const CoordinateFrame& otherCf, size_t& otherFaceId, float tol) const;
    /*override*/ virtual bool FaceEdgesOverlapped(const CoordinateFrame& myCf, size_t& myFaceId, const Geometry& otherGeom,
        const CoordinateFrame& otherCf, size_t& otherFaceId, float tol) const;
