...
BallPolyContact::BallPolyContact(Primitive* p0, Primitive* p1)
    : PolyContact(p0, p1)
{
    AYAASSERT(aya_static_cast<Ball*>(p0->getGeometry()));
    AYAASSERT(aya_static_cast<Poly*>(p1->getGeometry()));
}
...
BallPlaneConnector* BallPolyContact::newBallPlaneConnector(const POLY::Face* face)
{
    if (contactParams)
    {
        return new BallPlaneConnector(getPrimitive(0)->getBody(), getPrimitive(1)->getBody(), *contactParams, ball()->getRadius(),
...
BallEdgeConnector* BallPolyContact::newBallEdgeConnector(const POLY::Edge* edge)
{
    if (contactParams)
    {
        return new BallEdgeConnector(getPrimitive(0)->getBody(), getPrimitive(1)->getBody(), *contactParams, ball()->getRadius(),
...
BallVertexConnector* BallPolyContact::newBallVertexConnector(const POLY::Vertex* vertex)
{
    if (contactParams)
    {
        return new BallVertexConnector(getPrimitive(0)->getBody(), getPrimitive(1)->getBody(), *contactParams, ball()->getRadius(),
            vertex->getOffset(),
...
{
    return aya_static_cast<const Ball*>(getConstPrimitive(0)->getConstGeometry());
}

const Poly* BallPolyContact::poly() const
{
    return aya_static_cast<const Poly*>(getConstPrimitive(1)->getConstGeometry());
}
