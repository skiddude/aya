...
#include "Kernel/BodyPvSetter.hpp"
#include "World/Geometry.hpp"
#include "World/Edge.hpp"
#include "World/SurfaceData.hpp"
...
#include "Utility/ComputeProp.hpp"
...
class Mechanism;
class World;
template<class P, class C, class CM, int M>
class SpatialHash;
class Contact;
class Joint;
...
public:
    ...
    {
        AYAASSERT_VERY_FAST(unsigned(i) < list.size());
        AYAASSERT_VERY_FAST(list[i].edge->otherPrimitive(owner) == list[i].other);
        return list[i].edge;
    }
    ...
    Edge* getFirst() const
    {
        return (list.size() > 0) ? list[0].edge : NULL;
    }
...
public:
    ...
    typedef enum
    {
        ...
        ROOT_SIZE,
        SEAT_SIZE
    } SizeMultiplier; // overweights torsos and seats to make them roots of the spanning tree
...
private:
    ...
    static int fuzzyExtentsReset()
    {
        return -2;
    } // out of synch with body -1;
...
public:
    ...
    const NetworkOwnership getNetworkOwnershipRuleInternal() const
    {
        return networkOwnershipRule;
    }
    void setNetworkOwnershipRuleInternal(NetworkOwnership value)
    {
        networkOwnershipRule = value;
    }

    bool getNetworkIsSleeping() const
    {
        return networkIsSleeping;
    }
    ...
    bool getAnchoredProperty() const
    {
        return anchoredProperty;
    }
    ...
    {
        return (dragging || anchoredProperty);
    }

    // Collide
    void setPreventCollide(bool _preventCollide);
    bool getPreventCollide() const
    {
        return preventCollide;
    }

    bool getCanCollide() const
    {
        return !dragging && !preventCollide;
    }
    ...
    PartMaterial getPartMaterial() const
    {
        return material;
    }
    ...
    {
        return Math::planarSize(getSize());
    }
    ...
    {
        return (Extents::overlapsOrTouches(p0.getFastFuzzyExtents(), p1.getFastFuzzyExtents()));
    }
    ///////////////////////////////////////////////////////////////
...
public:
    ...
    bool hasEdge()
    {
        return ((joints.size() > 0) || (contacts.size() > 0));
    }
    ...
    Primitive* getJointOther(int id)
    {
        return joints.getOther(id);
    }
    ...
    Primitive* getContactOther(int id)
    {
        return contacts.getOther(id);
    }
