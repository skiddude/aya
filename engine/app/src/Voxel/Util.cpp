...
const BlockFaceInfo UnOrientedBlockFaceInfos[6] = {
    ...
    {{
        {
            BlockAxisFace::TopLeft,
        },                                // PlusX
        {BlockAxisFace::EmptyAllSkipped}, // PlusZ
        {
            BlockAxisFace::TopRight,
        },                                // MinusX
        {BlockAxisFace::FullNoneSkipped}, // MinusZ
...
BlockAxisFace ComputeOrientedFace(const BlockFaceInfo& wedge, FaceDirection f, CellOrientation orient)
{
    if (f < 4)
    {
    ...
    if (basis.skippedCorner == BlockAxisFace::FullNoneSkipped || basis.skippedCorner == BlockAxisFace::EmptyAllSkipped)
    {
        return basis;
    }
    ...
    // 0 1 2 3 => 0 3 2 1
    result.skippedCorner = BlockAxisFace::rotate(basis.skippedCorner, (CellOrientation)(f == MinusY ? (4 - orient) % 4 : orient));
...
void initBlockOrientationFaceMap()
{
    ...
    {
        const BlockFaceInfo& wedge = i >= kNUM_WEDGES ? UnOrientedBlockFaceInfos[0] : UnOrientedBlockFaceInfos[i];
        ...
        for (int j = 0; j < kLEGACY_MAX_MATERIALS; ++j)
        {
            for (int k = 0; k < MAX_CELL_ORIENTATIONS; ++k)
            {
                for (int l = 0; l < 6; ++l) // # of face directions
                {
                    ...
                    OrientedFaceMap[mapIndex * 6 + l] = ComputeOrientedFace(wedge, (FaceDirection)l, (CellOrientation)k);
                }
            }
        }
    }
