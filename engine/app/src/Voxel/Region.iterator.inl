...
Region<InternalStorageType>::iterator::iterator(const Region<InternalStorageType>& owningRegion)
    ...
    , reachedEnd(false)
{
    ...
    if (!owningRegion.isGuaranteedAllEmpty())
    {
        ...
        if (rangeSize.z > 1)
        {
            pointerSkipAtEndOfXLine = owningRegion.internalStorage->voxelCoordToArrayIndex(Vector3int16(minCoord.x, minCoord.y, minCoord.z + 1)) -
                                      owningRegion.internalStorage->voxelCoordToArrayIndex(Vector3int16(maxCoord.x, minCoord.y, minCoord.z));
        }
        ...
        if (rangeSize.y > 1)
        {
            pointerSkipAtEndOfZLine = owningRegion.internalStorage->voxelCoordToArrayIndex(Vector3int16(minCoord.x, minCoord.y + 1, minCoord.z)) -
                                      owningRegion.internalStorage->voxelCoordToArrayIndex(Vector3int16(maxCoord.x, minCoord.y, maxCoord.z));
        }
...
{
    return (CellMaterial)readMaterial(&owningRegion.internalStorage->getConstMaterial()[0], currentIndex, *currentCell);
}

template<class InternalStorageType>
const Cell& Region<InternalStorageType>::iterator::getNeighborCell(FaceDirection direction) const
{
    AYAASSERT_SLOW(owningRegion.contains(currentLocation + kFaceDirectionToLocationOffset[direction]));
    return currentCell[InternalStorageType::kFaceDirectionToPointerOffset[direction]];
}

template<class InternalStorageType>
const Cell& Region<InternalStorageType>::iterator::getNeighborCell(FaceDirection direction1, FaceDirection direction2) const
{
    AYAASSERT_SLOW(owningRegion.contains(currentLocation + kFaceDirectionToLocationOffset[direction1] + kFaceDirectionToLocationOffset[direction2]));
    return currentCell[InternalStorageType::kFaceDirectionToPointerOffset[direction1] +
                       InternalStorageType::kFaceDirectionToPointerOffset[direction2]];
}

template<class InternalStorageType>
CellMaterial Region<InternalStorageType>::iterator::getNeighborMaterial(FaceDirection direction) const
{
    AYAASSERT_SLOW(owningRegion.contains(currentLocation + kFaceDirectionToLocationOffset[direction]));
    const int offset(InternalStorageType::kFaceDirectionToPointerOffset[direction]);
    return (CellMaterial)readMaterial(&owningRegion.internalStorage->getConstMaterial()[0], currentIndex + offset, currentCell[offset]);
}

template<class InternalStorageType>
CellMaterial Region<InternalStorageType>::iterator::getNeighborMaterial(FaceDirection direction1, FaceDirection direction2) const
{
                AYAASSERT_SLOW(owningRegion.contains(currentLocation + 
...
template<class InternalStorageType>
const Cell& Region<InternalStorageType>::iterator::getArbitraryNeighborCell(const Vector3int16& neighborOffsets) const
{
    AYAASSERT_SLOW(owningRegion.contains(currentLocation + neighborOffsets));
    return currentCell[InternalStorageType::voxelCoordOffsetToIndexOffset(neighborOffsets)];
}

template<class InternalStorageType>
bool Region<InternalStorageType>::iterator::hasWaterAtNeighbor(const FaceDirection& direction) const
{
    AYAASSERT_SLOW(owningRegion.contains(currentLocation + kFaceDirectionToLocationOffset[direction]));
    return owningRegion.hasWaterAtSkipAllEmptyCheck(currentCell[InternalStorageType::kFaceDirectionToPointerOffset[direction]],
        currentLocation + VoxelIteratorConstants::kFaceDirectionToLocationOffset[direction]);
}

template<class InternalStorageType>
typename Region<InternalStorageType>::iterator& Region<InternalStorageType>::iterator::operator++()
{
    ++xCounter;
...
bool Region<InternalStorageType>::iterator::operator==(const iterator& other)
{
    ...
    return owningRegion == other.owningRegion && currentLocation == other.currentLocation;
}
