...
inline CellMaterial getCellMaterial_Deprecated(unsigned char cell)
{
    return (CellMaterial)(cell & 0x07);
}
inline void setCellMaterial_Deprecated(unsigned char& cell, CellMaterial material)
{
    cell = (cell & 0xf8) | ((int)material & 0x07);
}

inline CellMaterial readMaterial(const unsigned char* materials, const unsigned int cellIndex, const Cell cell)
{
    return (CellMaterial)(cell.solid.getBlock() == CELL_BLOCK_Empty ? CELL_MATERIAL_Water
...
struct BlockAxisFace
{
    ...
    {
        static SkippedCorner MIRROR[6] = {TopLeft, TopRight, BottomRight, BottomLeft, EmptyAllSkipped, FullNoneSkipped};
        return MIRROR[corner];
    }

    static inline SkippedCorner YAxisMirror(SkippedCorner corner)
    {
        static SkippedCorner MIRROR[6] = {BottomRight, BottomLeft, TopLeft, TopRight, EmptyAllSkipped, FullNoneSkipped};
        return MIRROR[corner];
    }

    static BlockAxisFace inverse(const BlockAxisFace other)
    {
        static const SkippedCorner OPPOSITE_CORNER[6] = {BottomLeft, BottomRight, TopRight, TopLeft, FullNoneSkipped, EmptyAllSkipped};
...
inline Vector3int16 worldToCell_floor(const Vector3& worldPos)
{
    ...
    return Vector3int16((int)(floorf(worldPos.x / kCELL_SIZE)) + kXZOffset, (int)(floorf(worldPos.y / kCELL_SIZE)),
        (int)(floorf(worldPos.z / kCELL_SIZE)) + kXZOffset);
}

inline Vector3 worldSpaceToCellSpace(const Vector3& worldPos)
{
    return Vector3((worldPos.x * (1.0f / kCELL_SIZE)), (worldPos.y * (1.0f / kCELL_SIZE)), (worldPos.z * (1.0f / kCELL_SIZE)));
}

inline Vector3 cellSpaceToWorldSpace(const Vector3& cellPos)
{
    return Vector3((cellPos.x * kCELL_SIZE), (cellPos.y * kCELL_SIZE), (cellPos.z * kCELL_SIZE));
}
...
{
    const int kXZOffset = 0;
    return Vector3((cellPos.x - kXZOffset) * kCELL_SIZE, cellPos.y * kCELL_SIZE, (cellPos.z - kXZOffset) * kCELL_SIZE);
}
