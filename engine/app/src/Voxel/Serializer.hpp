...
#include "Utility/FixedSizeCircularBuffer.hpp"
...
class Serializer
{
    ...
    void encodeFromPosition(const Grid* voxelStore, Vector3int16& cellpos, const SpatialRegion::Id& lastChunkPos, const Grid::Region& region,
        RecentlyEncodedBuffer& lastSeenNewCells, CellBuffer& cellBuffer, OutputStream* outputStream) const
    {
        ...
        else
        {
            ...
            while (nextChunk == lastChunkPos && cellBuffer.chk(nextPos) && nextContent == content)
            {
                copyCount++;
                cellBuffer.pop(&unread);
                AYAASSERT(nextPos == unread);
...
public:
    template<class CellBuffer, class OutputStream>
    void encodeCells(const Grid* voxelStore, CellBuffer& cellBuffer, OutputStream* outputStream, int sizeLimitInBytes) const
    {

        const Vector3int16 kCellInChunkBits(SpatialRegion::getRegionDimensionInVoxelsAsBitShifts());
        ...
        while (cellBuffer.size() > 0 && (sizeLimitInBytes == -1 || ((int)outputStream->GetNumberOfBytesUsed()) < sizeLimitInBytes))
        {
            ...
            do
            {
                encodeFromPosition(voxelStore, nextPos, lastChunkPos, region, lastSeenNewCells, cellBuffer, outputStream);
                continuing = cellBuffer.chk(nextPos) && SpatialRegion::regionContainingVoxel(nextPos) == lastChunkPos &&
                             (sizeLimitInBytes == -1 || ((int)outputStream->GetNumberOfBytesUsed()) < sizeLimitInBytes);
                if (continuing)
                {
                    Vector3int16 unused;
                    cellBuffer.pop(&unused);
                    AYAASSERT(unused == nextPos);
                }
            } while (continuing);
    ...
    void decodeCells(Grid* voxelStore, InputStream& inputStream, CellUpdateFilter& filter)
    {
        const Vector3int16 kCellInChunkBits(SpatialRegion::getRegionDimensionInVoxelsAsBitShifts());
        ...
        while (1)
        {
            ...
            cellPos.z = data;
            cellPos = SpatialRegion::globalVoxelCoordinateFromRegionAndRelativeCoordinate(chunkPos, cellPos);
