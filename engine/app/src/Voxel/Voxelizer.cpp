...
#if ((defined(_WIN32) || (!defined(AYA_PLATFORM_IOS))) && (!defined(__APPLE__))) && !defined(_M_ARM64) && !defined(__aarch64__)
#define SIMD_SSE2
#endif
...
void Voxelizer::occupancyFillBlock(
    OccupancyChunk& chunk, const Extents& chunkExtents, const Vector3& extents, const CoordinateFrame& cframe, float transparency, float meshRadius)
{
...
{
    return Vector3(fabs(cframe.rotation[0][0]) * extents[0] + fabs(cframe.rotation[0][1]) * extents[1] + fabs(cframe.rotation[0][2]) * extents[2],
...
void Voxelizer::occupancyFillBlockDF(
    OccupancyChunk& chunk, const Extents& chunkExtents, const Vector3& extents, const CoordinateFrame& cframe, float transparency)
{
    ...
    Vector3int32 extentsMax =
        fastFloorInt32((extentsWorld.high() - chunkExtents.min()) / 4.f - Vector3(0.01f, 0.01f, 0.01f)).min(kVoxelChunkSize - Vector3int32(1, 1, 1));
    ...
    Vector3 localCorner =
        cframe.pointToObjectSpace(chunkExtents.min() + Vector3(extentsMin.x + 0.5f, extentsMin.y + 0.5f, extentsMin.z + 0.5f) * 4.f) / 4.f;
...
void Voxelizer::occupancyFillMesh(
    OccupancyChunk& chunk, const Extents& chunkExtents, const Vector3& extents, const CoordinateFrame& cframe, float transparency, float meshRadius)
{
    ...
    const Vector3 invHalfLocalExtentsSquared = Vector3(1.f / (halfLocalExtents.x * halfLocalExtents.x),
        1.f / (halfLocalExtents.y * halfLocalExtents.y), 1.f / (halfLocalExtents.z * halfLocalExtents.z));
...
void Voxelizer::addMeshToPartCache(std::vector<DataModelPartCache>& partCache, PartInstance* part, OccupancyChunk& chunk, const Extents& chunkExtents,
    const Vector3& extents, const CoordinateFrame& cframe, float transparency)
{
    ...
    TriangleMesh* triangleMesh = aya_static_cast<TriangleMesh*>(geometry);
    ...
    for (auto& e : compound->getExtentArray())
    {
        ...
        partCache.push_back(
            DataModelPartCache(&Voxelizer::occupancyFillMesh, chunk, e.size * expansionFactor, ccf, transparency, extents.magnitude()));
    }
}

void Voxelizer::occupancyFillBlockDFAA(
    OccupancyChunk& chunk, const Extents& chunkExtents, const Vector3& extents_, const CoordinateFrame& cframe, float transparency)
{
    ...
    Vector3 localCorner =
        (chunkExtents.min() + Vector3(extentsMin.x + 0.5f, extentsMin.y + 0.5f, extentsMin.z + 0.5f) * 4.f - cframe.translation) / 4.f;
...
void Voxelizer::occupancyFillBlockDFSIMD(
    OccupancyChunk& chunk, const Extents& chunkExtents, const Vector3& extents_, const CoordinateFrame& cframe, float transparency)
{
#define VSPLAT(v, c) _mm_shuffle_ps(v, v, _MM_SHUFFLE(c, c, c, c))
    ...
    __m128 extentsHalfRotated = _mm_add_ps(_mm_mul_ps(_mm_and_ps(cframeRotationC0, nosign), VSPLAT(extentsHalf, 0)),
        _mm_add_ps(_mm_mul_ps(_mm_and_ps(cframeRotationC1, nosign), VSPLAT(extentsHalf, 1)),
    ...
    // A small negative offset is added to extentsMax before flooring to prevent updating extra layer of cells for perfectly aligned/sized parts
    __m128i extentsMax = fastFloorInt4Clamp0(
        _mm_min_ps(_mm_sub_ps(_mm_mul_ps(_mm_sub_ps(extentsWorldHigh, chunkOffset), _mm_set1_ps(0.25f)), _mm_set1_ps(0.001f)), chunkExtentsLimit));
    ...
    __m128 localCornerRot = _mm_add_ps(_mm_mul_ps(cframeRotation0, VSPLAT(localCornerNoRot, 0)),
        _mm_add_ps(_mm_mul_ps(cframeRotation1, VSPLAT(localCornerNoRot, 1)), _mm_mul_ps(cframeRotation2, VSPLAT(localCornerNoRot, 2))));
    ...
    for (int y = extentsMinBuf[1]; y <= extentsMaxBuf[1]; ++y)
    {
        ...
        for (int z = extentsMinBuf[2]; z <= extentsMaxBuf[2]; ++z)
        {
            ...
            for (int x = extentsMinBuf[0]; x <= extentsMaxBuf[0]; ++x)
            {
                ...
                __m128 factor = _mm_mul_ps(_mm_mul_ps(VSPLAT(factors, 0), VSPLAT(factors, 1)), _mm_mul_ps(VSPLAT(factors, 2), thickness));
...
void Voxelizer::occupancyFillSphere(
    OccupancyChunk& chunk, const Extents& chunkExtents, const Vector3& extents, const CoordinateFrame& cframe, float transparency, float meshRadius)
{
...
void Voxelizer::occupancyFillEllipsoid(
    OccupancyChunk& chunk, const Extents& chunkExtents, const Vector3& extents, const CoordinateFrame& cframe, float transparency, float meshRadius)
{
...
void Voxelizer::occupancyFillCylinderX(
    OccupancyChunk& chunk, const Extents& chunkExtents, const Vector3& extents, const CoordinateFrame& cframe, float transparency, float meshRadius)
{
...
void Voxelizer::occupancyFillCylinderY(
    OccupancyChunk& chunk, const Extents& chunkExtents, const Vector3& extents, const CoordinateFrame& cframe, float transparency, float meshRadius)
{
...
void Voxelizer::occupancyFillWedge(
    OccupancyChunk& chunk, const Extents& chunkExtents, const Vector3& extents, const CoordinateFrame& cframe, float transparency, float meshRadius)
{
...
void Voxelizer::occupancyFillCornerWedge(
    OccupancyChunk& chunk, const Extents& chunkExtents, const Vector3& extents, const CoordinateFrame& cframe, float transparency, float meshRadius)
{
...
void Voxelizer::occupancyFillTorso(
    OccupancyChunk& chunk, const Extents& chunkExtents, const Vector3& extents, const CoordinateFrame& cframe, float transparency, float meshRadius)
{
...
void Voxelizer::occupancyFillDF(OccupancyChunk& chunk, const Extents& chunkExtents, const Vector3& extents, const CoordinateFrame& cframe,
    float transparency, DistanceFunction& df)
{
    Vector3 extentsWorldSize = extentsToWorldSpace(cframe, extents);
    AABox extentsWorld = AABox(cframe.translation - extentsWorldSize * 0.5f, cframe.translation + extentsWorldSize * 0.5f);
    ...
    Vector3 localCorner =
        cframe.pointToObjectSpace(chunkExtents.min() + Vector3(extentsMin.x + 0.5f, extentsMin.y + 0.5f, extentsMin.z + 0.5f) * 4.f);
...
void Voxelizer::occupancyUpdateChunkPrepare(
    OccupancyChunk& chunk, MegaClusterInstance* terrain, ContactManager* contactManager, std::vector<DataModelPartCache>& partCache)
{
    //		TIMERFUNC();
    ...
    FASTLOG3F(FLog::Voxelizer, "LightGrid: Asking spatial hash for parts, extents min: %fx%fx%f", chunkExtents.min().x, chunkExtents.min().y,
        chunkExtents.min().z);
    FASTLOG3F(FLog::Voxelizer, "LightGrid: Asking spatial hash for parts, extents max: %fx%fx%f", chunkExtents.max().x, chunkExtents.max().y,
        chunkExtents.max().z);

    DenseHashSet<Primitive*> result(NULL);

    {
        AYAPROFILER_SCOPE("Voxel", "getPrimitivesOverlapping");
    ...
    if (terrain)
    {
        AYAPROFILER_SCOPE("Voxel", "occupancyFillTerrain");
    ...
    for (DenseHashSet<Primitive*>::const_iterator it = result.begin(); it != result.end(); ++it)
    {
        ...
        if (part->getCookie() & Aya::PartCookie::IS_HUMANOID_PART)
            continue;
...
void Voxelizer::occupancyFillTerrainMega(OccupancyChunk& chunk, Voxel::Grid& terrain, const Vector3int32& chunkOffset, const Extents& chunkExtents)
{
    ...
    for (Grid::Region::xline_iterator it = region.xLineBegin(); it != region.xLineEnd(); ++it)
    {
        ...
        AYAASSERT(it.getLineSize() == kVoxelChunkSizeXZ);
        AYAASSERT(isInsideChunkLocal(Vector3int32(0, y, z)));
...
#ifdef SIMD_SSE2
void Voxelizer::occupancyFillTerrainMegaSIMD(
    OccupancyChunk& chunk, Voxel::Grid& terrain, const Vector3int32& chunkOffset, const Extents& chunkExtents)
{
    // TIMERFUNC();
...
void Voxelizer::occupancyFillTerrainSmooth(OccupancyChunk& chunk, Voxel2::Grid& terrain, const Extents& chunkExtents)
{
    ...
    for (int y = 0; y < kVoxelChunkSizeY; ++y)
        ...
        {
            AYAASSERT(box.getSizeX() == kVoxelChunkSizeXZ);
            AYAASSERT(isInsideChunkLocal(Vector3int32(0, y, z)));
