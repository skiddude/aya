...
const Cell& Grid::getVoxelLikelyThisChunk(const SpatialRegion::Id& id, const Chunk& chunk, const Vector3int16& coord) const
{
...
void Grid::fillLocalAreaInfo(const Vector3int16& globalCoord, const Water::RelevantNeighbors& relevantNeighbors, Water::LocalAreaInfo* info) const
{
    ...
    if (relativeCoord.isBetweenInclusive(Vector3int16::one(), SpatialRegion::getMaxVoxelOffsetInsideRegion() - Vector3int16::one()))
    {
        unsigned int index = Chunk::voxelCoordToArrayIndex(globalCoord);
        info->aboveNeighbor = mainChunk.getConstData()[index + Chunk::voxelCoordOffsetToIndexOffset(relevantNeighbors.aboveNeighbor)];
        info->primaryNeighbor = mainChunk.getConstData()[index + Chunk::voxelCoordOffsetToIndexOffset(relevantNeighbors.primaryNeighbor)];
        info->secondaryNeighbor = mainChunk.getConstData()[index + Chunk::voxelCoordOffsetToIndexOffset(relevantNeighbors.secondaryNeighbor)];
        info->diagonalNeighbor = mainChunk.getConstData()[index + Chunk::voxelCoordOffsetToIndexOffset(relevantNeighbors.diagonalNeighbor)];
        info->diagonalUpNeighbor = mainChunk.getConstData()[index + Chunk::voxelCoordOffsetToIndexOffset(relevantNeighbors.diagonalUpNeighbor)];
    }
    else
    {

        info->aboveNeighbor = getVoxelLikelyThisChunk(mainChunkId, mainChunk, globalCoord + relevantNeighbors.aboveNeighbor);
        info->primaryNeighbor = getVoxelLikelyThisChunk(mainChunkId, mainChunk, globalCoord + relevantNeighbors.primaryNeighbor);
        info->secondaryNeighbor = getVoxelLikelyThisChunk(mainChunkId, mainChunk, globalCoord + relevantNeighbors.secondaryNeighbor);
        info->diagonalNeighbor = getVoxelLikelyThisChunk(mainChunkId, mainChunk, globalCoord + relevantNeighbors.diagonalNeighbor);
        info->diagonalUpNeighbor = getVoxelLikelyThisChunk(mainChunkId, mainChunk, globalCoord + relevantNeighbors.diagonalUpNeighbor);
    }
}
...
void Grid::setCell(const Vector3int16& location, Cell newCell, CellMaterial inputMaterial)
{
    ...
    if (existingChunk)
    {
        ...
        prevMaterial = readMaterial(&(existingChunk->getConstMaterial()[0]), arrayIndex, prevCell);
    }

    CellMaterial newMaterial = inputMaterial == CELL_MATERIAL_Unspecified ? prevMaterial : inputMaterial;
...
Grid::Region Grid::getRegion(const Vector3int16& minCoord, const Vector3int16& maxCoord) const
{
    SpatialRegion::Id minId = SpatialRegion::regionContainingVoxel(minCoord);
    AYAASSERT(minId == SpatialRegion::regionContainingVoxel(maxCoord));
...
CellMaterial Grid::getCellMaterial(const Vector3int16& pos) const
{
    ...
    if (const Chunk* chunk = chunkMap.find(chunkId))
    {
        ...
        return readMaterial(&chunk->getConstMaterial()[0], index, chunk->getConstData()[index]);
    }
...
{
    std::vector<Voxel::CellChangeListener*>::iterator itr = std::find(cellChangeListeners.begin(), cellChangeListeners.end(), listener);
...
std::vector<SpatialRegion::Id> Grid::getNonEmptyChunksInRegion(const Region3int16& extents) const
{
    ...
    unsigned int totalRegionCount = (maxRegion.value().x - minRegion.value().x + 1) * (maxRegion.value().y - minRegion.value().y + 1) *
