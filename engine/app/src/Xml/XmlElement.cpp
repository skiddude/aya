...
const Aya::Name& value_IDREF_null = Name::declare("null");
const Aya::Name& value_IDREF_nil = Name::declare("nil");
...
bool XmlNameValuePair::isValueEqual(const Aya::Name* value) const
{
    switch (valueType)
    {
    case STRING:
...
bool XmlNameValuePair::getValue(const Aya::Name*& value) const
{
    if (valueType == NAME)
    {
        value = nameValue;
    ...
    if (valueType == STRING)
    {
        value = &Aya::Name::declare(stringValue->c_str());
        clearValue();
        nameValue = value;
        valueType = NAME;
        return true;
...
template<>
bool XmlNameValuePair::isValueType<int64_t>() const
{
...
template<>
bool XmlNameValuePair::isValueType<const Aya::Name*>() const
{
    return valueType == NAME;
}

template<>
bool XmlNameValuePair::isValueType<Aya::InstanceHandle>() const
{
...
bool XmlNameValuePair::getValue(Aya::ContentId& value) const
{
...
bool XmlNameValuePair::getValue(int64_t& value) const
{
    if (valueType == INT64)
    {
        value = int64Value;
    ...
    if (valueType == STRING)
    {
        if (StringConverter<int64_t>::convertToValue(*stringValue, value))
        {
            clearValue();
            intValue = value;
            valueType = INT64;
...
bool XmlNameValuePair::getValue(float& value) const
{
    ...
    if (valueType == STRING)
    {
        if (Aya::StringConverter<float>::convertToValue(*stringValue, value))
        {
            clearValue();
            floatValue = value;
            valueType = FLOAT;
    ...
    AYAASSERT(valueType != DOUBLE); // No provision (yet) for converting from double back to float
...
bool XmlNameValuePair::getValue(double& value) const
{
    ...
    if (valueType == STRING)
    {
        if (Aya::StringConverter<double>::convertToValue(*stringValue, value))
        {
            clearValue();
            doubleValue = value;
            valueType = DOUBLE;
...
bool XmlNameValuePair::getValue(Aya::InstanceHandle& value) const
{

    if (valueType == NAME && *nameValue == value_IDREF_null)
    {
        clearValue();
        handleValue = new Aya::InstanceHandle(NULL);
        valueType = HANDLE;
    }
...
std::string XmlNameValuePair::toString(XmlWriter* writer) const
{
    ...
    case BOOL:
    {
        return Aya::StringConverter<bool>::convertToString(boolValue);
    };

    case INT:
    {
        return Aya::StringConverter<int>::convertToString(intValue);
    };

    case INT64:
    {
        return Aya::StringConverter<int64_t>::convertToString(int64Value);
    };

    case UINT:
    {
        return Aya::StringConverter<unsigned int>::convertToString(uintValue);
    };

    case FLOAT:
    {
        return Aya::StringConverter<float>::convertToString(floatValue);
    };

    case DOUBLE:
    {
        return Aya::StringConverter<double>::convertToString(doubleValue);
    };
