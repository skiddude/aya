...
struct MemoryInputStream
{
    ...
    void read(void* value, size_t size)
    {
        if (offset + size > datasize)
            throw Aya::runtime_error("Read offset is out of bounds while reading %d bytes", (int)size);
...
struct MemoryOutputStream
{
    ...
    MemoryOutputStream(const std::string& name)
        : name(name)
        , datasize(0)
        , capacity(0)
    {
...
static void readData(std::istream& in, void* data, int size)
{
    ...
    if (in.gcount() != size)
        throw Aya::runtime_error("Unexpected end of file while reading %d bytes", size);
}
    ...
    if (header.size)
    {
        ...
        else
        {
            ...
            if (count != result.datasize)
                throw Aya::runtime_error("Malformed data (%d != %d)", count, (int)result.datasize);
        }
    }
...
static void writeChunk(std::ostream& out, const MemoryOutputStream& stream, const char* name, unsigned int flags)
{
    ...
    else
    {
        ...
        int compressedSize =
            (flags & SerializerBinary::sfHighCompression ? LZ4_compressHC : LZ4_compress)(stream.data.get(), &compressed[0], stream.datasize);
...
template<typename T>
static void readRaw(MemoryInputStream& stream, T& value)
{
    stream.read(&value, sizeof(value));
}

template<typename T>
static void writeRaw(MemoryOutputStream& stream, const T& value)
{
...
static uint64_t encodeInt64(int64_t value)
{
    // To transform an integer: if x greater than or equal to zero, transform it with 2 * x.
    // Otherwise, use 2 * |x| - 1. In most compilers this is equivalent to (x << 1) ^ (x >> 31)
    // For 64-bit integers, the same format is used but with 63 instead of 31.
    // return (value >= 0 ? (value * 2) : (value << 1) ^ (value >> 63));
    return (uint64_t)((value << 1) ^ (value >> 63));
}

static int64_t decodeInt64(uint64_t value)
{
    // Right shift by 1 recovers the original sign bit.
    // XOR with -(value & 1) undoes the zigzag transformation.
    return (int64_t)((value >> 1) ^ -(value & 1));
}

uint64_t toBigEndian(uint64_t hostValue)
{
    auto p = reinterpret_cast<uint8_t*>(&hostValue);
    std::reverse(p, p + sizeof(hostValue));
    return hostValue;
}

uint64_t fromBigEndian(uint64_t bigEndianValue)
{
    auto p = reinterpret_cast<uint8_t*>(&bigEndianValue);
    std::reverse(p, p + sizeof(bigEndianValue));
...
static void readIntVector(MemoryInputStream& stream, std::vector<int>& values, size_t count)
{
    ...
    if (stream.offset + count * 4 > stream.datasize)
        throw Aya::runtime_error("Read offset is out of bounds while reading %d bytes", (int)count * 4);
...
void readInt64Vector(MemoryInputStream& stream, std::vector<int64_t>& values, size_t count)
{
    values.clear();
    values.reserve(count);

    // Assume readRaw properly handles reading bytes
    for (size_t i = 0; i < count; ++i)
    {
        uint64_t rawValue = 0;
        stream.read(&rawValue, sizeof(rawValue)); // This should be adjusted to correctly read in big-endian format
        rawValue = fromBigEndian(rawValue);
        int64_t decodedValue = decodeInt64(rawValue);
        values.push_back(decodedValue);
    }
}
...
void writeInt64Vector(MemoryOutputStream& stream, const std::vector<int64_t>& values)
{
    for (size_t i = 0; i < values.size(); ++i)
    {
        uint64_t encodedValue = encodeInt64(values[i]);
        encodedValue = toBigEndian(encodedValue);          // Convert to big-endian format before writing
        stream.write(&encodedValue, sizeof(encodedValue)); // Adjusted for big-endian
    }
}
...
static void readUInt8Vector(MemoryInputStream& stream, std::vector<uint8_t>& values, size_t count)
{
    values.clear();
    values.reserve(count); // space for two values, ok

    // bits needed for two instacnes = 2 * 8 = 16
    if (stream.offset + count > stream.datasize) // check byte length for both values
        throw Aya::runtime_error("Read offset is out of bounds while reading %d bytes", (int)count);
...
static void writeUInt8Vector(MemoryOutputStream& stream, const std::vector<uint8_t>& values)
{
    for (size_t i = 0; i < values.size(); ++i)
    {
        writeRaw(stream, (unsigned char)values[i]);
...
static void readPhysicalProperties(MemoryInputStream& stream, PhysicalProperties& physicalProp)
{
    ...
    if (customizeProp)
    {
        ...
        physicalProp = PhysicalProperties(density, friction, elasticity, frictionWeight, elasticityWeight);
    }
    else
    {
...
static void readTweenInfo(MemoryInputStream& stream, TweenInfo& twnInfo)
{
    float time;
    ...
    readRaw(stream, time);
    readRaw(stream, style);
    readRaw(stream, direction);
    readRaw(stream, repeatCount);
    readRaw(stream, reverses);
    readRaw(stream, delayTime);

    twnInfo = TweenInfo(style, direction, repeatCount, reverses, time, delayTime);
}

static void writeTweenInfo(MemoryOutputStream& stream, const TweenInfo& twnInfo)
{
    writeRaw(stream, twnInfo.getTime());
    writeRaw(stream, twnInfo.getStyle());
    writeRaw(stream, twnInfo.getDirection());
    writeRaw(stream, twnInfo.getRepeatCount());
    writeRaw(stream, twnInfo.getReverses());
    writeRaw(stream, twnInfo.getDelayTime());
}
...
template<typename T>
static T getPropertyValue(const Reflection::PropertyDescriptor& desc, const Instance* instance)
{
    return Reflection::ConstProperty(desc, instance).getValue<T>();
}

template<typename T>
static void setPropertyValue(const Reflection::PropertyDescriptor& desc, Instance* instance, const T& value)
{
...
void readFormatExpected(MemoryInputStream& stream, BinaryPropertyFormat expected)
{
    ...
    if (format != expected)
        throw Aya::runtime_error("Unexpected format %d (expected %d)", format, expected);
}

void readPropertyValues(MemoryInputStream& stream, const Reflection::PropertyDescriptor& desc, const std::vector<Instance*>& instances,
    const std::vector<shared_ptr<Instance>>& idMap)
{
    if (desc.type == Reflection::Type::singleton<ProtectedString>() || desc.type == Reflection::Type::singleton<std::string>() ||
        desc.type == Reflection::Type::singleton<BinaryString>())
    {
    ...
    else if (desc.type == Reflection::Type::singleton<TweenInfo>())
    {
        readFormatExpected(stream, bpfColor3uint8);

        for (size_t i = 0; i < instances.size(); ++i)
        {
            TweenInfo instanceTweenInfo;
            readTweenInfo(stream, instanceTweenInfo);
            setPropertyValue(desc, instances[i], instanceTweenInfo);
        }
    }
    else if (desc.type == Reflection::Type::singleton<G3D::Color3uint8>())
    {
        readFormatExpected(stream, bpfColor3uint8);

        std::vector<uint8_t> r, g, b;
        readUInt8Vector(stream, r, instances.size());
        readUInt8Vector(stream, g, instances.size());
        readUInt8Vector(stream, b, instances.size());

        for (size_t i = 0; i < instances.size(); ++i)
        {
            setPropertyValue(desc, instances[i], G3D::Color3uint8(r[i], g[i], b[i]));
        }
    }
    ...
    else if (desc.type == Reflection::Type::singleton<G3D::CoordinateFrame>())
    {
        ...
        if (format != bpfCFrameMatrix && format != bpfCFrameQuat)
            throw Aya::runtime_error("Unexpected cframe format %d", format);
    ...
    else if (Reflection::RefPropertyDescriptor::isRefPropertyDescriptor(desc))
    {
        ...
        for (size_t i = 0; i < instances.size(); ++i)
        {
            if (values[i] != -1 && static_cast<unsigned int>(values[i]) >= idMap.size())
                throw Aya::runtime_error("Invalid id %d", values[i]);
    ...
    else if (desc.type == Reflection::Type::singleton<int64_t>())
    {
        // Int64 values are stored as DOUBLE their actual values (if it's greater than or equal 0)
        // Otherwise, we use 2 * |x| - 1
        // This is for compression. To know more, read about Integer Transformations
        // ex: 123456 = 246912

        // check if this is a normal int value
        char format;
        readRaw(stream, format);
        // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Reading int64_t property: %s. Format: %d", desc.name.c_str(), format);

        if (format == bpfInt64)
        {
            std::vector<int64_t> values;
            // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Starting to read int64 values for property: %s", desc.name.c_str());

            // We now have the untransformed equivalents of our int64 values.
            // Roblox already has a function for untransforming 32-bit integers, so we can just recycle that for 64 bits.

            // decodeInt(int) == decodeInt64(int64_t)

            readInt64Vector(stream, values, instances.size());
            // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Completed reading int64 values for property: %s", desc.name.c_str());

            for (size_t i = 0; i < instances.size(); ++i)
            {
                // Aya::StandardOut::singleton()->printf(
                //    Aya::MESSAGE_INFO, "Setting int64 property: %s, Instance: %zu, Value: %lld", desc.name.c_str(), i, values[i]);
                setPropertyValue(desc, instances[i], values[i]);
            }
        }
        else if (format == bpfInt)
        {
            std::vector<int> values;
            // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Starting to read int values (as int64) for property: %s", desc.name.c_str());
            readIntVector(stream, values, instances.size());
            // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Completed reading int values (as int64) for property: %s",
            // desc.name.c_str());

            for (size_t i = 0; i < instances.size(); ++i)
            {
                // Aya::StandardOut::singleton()->printf(
                //    Aya::MESSAGE_INFO, "Setting int (as int64) property: %s, Instance: %zu, Value: %d", desc.name.c_str(), i, values[i]);
                setPropertyValue(desc, instances[i], static_cast<int64_t>(values[i]));
            }
        }
        else
        {
            // Aya::StandardOut::singleton()->printf(
            //    Aya::MESSAGE_WARNING, "Unexpected format for int64 property: %s. Format: %d", desc.name.c_str(), format);
            throw Aya::runtime_error("Unexpected format %d (expected %d or %d)", format, bpfInt64, bpfInt);
        }
    }
    else
    {
        throw Aya::runtime_error("Unknown property type for property %s", desc.name.c_str());
    }
}

void writePropertyValues(MemoryOutputStream& stream, const Reflection::PropertyDescriptor& desc, const std::vector<const Instance*>& instances,
    const std::map<const Instance*, int>& idMap, unsigned int flags)
{
    if (desc.type == Reflection::Type::singleton<ProtectedString>() || desc.type == Reflection::Type::singleton<std::string>() ||
        desc.type == Reflection::Type::singleton<BinaryString>())
    {
    ...
    {
        // Writing Int64 values.
        // With most of our framework done, this shouldn't be a problem at all.
        ...
        for (size_t i = 0; i < instances.size(); ++i)
            values.push_back(getPropertyValue<int64_t>(desc, instances[i]));

        writeRaw(stream, (char)bpfInt64);
        writeInt64Vector(stream, values);
    }
    else if (desc.type == Reflection::Type::singleton<float>())
    {
        std::vector<float> values;
    ...
    else if (desc.type == Reflection::Type::singleton<G3D::Color3uint8>())
    {
        ...
        for (size_t i = 0; i < instances.size(); ++i)
        {
            G3D::Color3uint8 value = getPropertyValue<G3D::Color3uint8>(desc, instances[i]);
        ...
        writeRaw(stream, (char)bpfColor3uint8);
        writeUInt8Vector(stream, r);
        writeUInt8Vector(stream, g);
        writeUInt8Vector(stream, b);
    }
    else if (desc.type == Reflection::Type::singleton<G3D::Rect2D>())
    {
        std::vector<float> x0;
    ...
    else if (desc.type == Reflection::Type::singleton<TweenInfo>())
    {
        ...
        for (size_t i = 0; i < instances.size(); ++i)
        {
            TweenInfo value = getPropertyValue<TweenInfo>(desc, instances[i]);
            writeTweenInfo(stream, value);
        }
    }
    else if (desc.type == Reflection::Type::singleton<G3D::Vector2>())
    {
...
static void serializeGatherGraph(const Instances& instances, std::vector<const Instance*>& objects, std::vector<const Instance*>& objectspost,
...
static shared_ptr<Instance> createServiceInstance(Instance* root, const Name& name)
{
    ...
    // However, some instances access DM services in onServiceProvider callback so they expect the services to already be there when parenting
    // happens. This means that we have to explicitly parent all services before the last stage. Note that Service::createChild() either returns an
    // existing service (that is already parented) or a new one (that has to be parented here).
    if (result)
...
static const Reflection::ClassDescriptor* deserializeDecodeInstances(
    MemoryInputStream& stream, Instance* root, std::vector<shared_ptr<Instance>>& idMap, std::vector<Instance*>& objects)
{
    ...
    if (format != bofPlain && format != bofServiceType)
        throw Aya::runtime_error("Unrecognized object format %d", format);
    ...
    for (size_t i = 0; i < ids.size(); ++i)
    {
        if (static_cast<unsigned int>(ids[i]) >= idMap.size())
            throw Aya::runtime_error("Invalid id %d", ids[i]);

        shared_ptr<Instance> object = (isServiceType && isRootServiceProvider && isServiceRooted[i]) ? createServiceInstance(root, typeNameName)
                                      : creator ? shared_polymorphic_downcast<Instance>(creator->create())
                                                : shared_ptr<Instance>();
        ...
        if (object)
        {
            ...
            if (idMap[ids[i]])
                throw Aya::runtime_error("Duplicate id %d", ids[i]);
...
static void deserializeDecodeProperty(MemoryInputStream& stream, const Reflection::ClassDescriptor* typeDesc,
    const std::vector<shared_ptr<Instance>>& idMap, const std::vector<Instance*>& objects)
{
...
static void deserializeDecodeParents(MemoryInputStream& stream, Instance* root, Instances* result, const std::vector<shared_ptr<Instance>>& idMap)
{
    ...
    if (format != bplfPlain)
        throw Aya::runtime_error("Unrecognized parent link format %d", format);
    ...
    for (size_t i = 0; i < oids.size(); ++i)
    {
        if (static_cast<unsigned int>(oids[i]) >= idMap.size())
            throw Aya::runtime_error("Invalid id %d", oids[i]);
        ...
        if (const shared_ptr<Instance>& object = idMap[oids[i]])
        {
            if (pids[i] != -1)
            {
                if (static_cast<unsigned int>(pids[i]) >= idMap.size())
                    throw Aya::runtime_error("Invalid id %d", pids[i]);
...
static void deserializeImpl(std::istream& in, Instance* root, Instances* result)
{
    ...
    if (memcmp(header.magic, SerializerBinary::kMagicHeader, sizeof(header.magic)) != 0)
        throw Aya::runtime_error("Unrecognized format");

    if (memcmp(header.signature, kHeaderSignature, sizeof(header.signature)) != 0)
        throw Aya::runtime_error("The file header is corrupted");

    if (header.version != 0)
        throw Aya::runtime_error("Unrecognized version %d", header.version);
    ...
    while (in.good())
    {
        ...
        if (memcmp(chunk.name, kChunkInstances, sizeof(chunk.name)) == 0)
        {
            ...
            if (typeIndex >= types.size())
                throw Aya::runtime_error("Type index out of bounds: %d", typeIndex);

            if (types[typeIndex])
                throw Aya::runtime_error("Duplicate type index: %d", typeIndex);
    ...
    throw Aya::runtime_error("Unexpected end of file");
}
...
/*
        ...
        Each chunk has the type index, a property name, followed by the storage format (see BinaryPropertyFormat), followed by a stream of property
   data. Property data is encoded depending on the type, with interleaving/bit shuffling to improve LZ compression rates.
