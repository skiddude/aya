...
// TODO: Refactor: Call this Aya::DOM or something
class SerializerV2
{
protected:
...
public:
    ...
    // reading:
    void loadInstancesFromText(const XmlElement* root, Aya::Instances& result);

    // Until DataModel becomes an Instance and it can handle "globals" like Workspace, we need to treat
    // it specially during reads:
    void load(std::istream& stream, Aya::DataModel* dataModel);
    void loadInstances(std::istream& stream, Aya::Instances& result);

private:
    void loadXML(std::istream& stream, Aya::DataModel* dataModel);
    void loadInstancesXML(const XmlElement* root, Aya::Instances& result, Aya::IReferenceBinder& binder, Aya::CreatorRole creatorRole);
    shared_ptr<Aya::Instance> loadInstanceXML(const XmlElement* itemElement, Aya::IReferenceBinder& binder, Aya::CreatorRole creatorRole);
};
