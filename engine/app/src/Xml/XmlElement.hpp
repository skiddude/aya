...
extern const Aya::Name& value_IDREF_null;
extern const Aya::Name& value_IDREF_nil;
...
private:
    ...
    union
    {
        ...
        mutable Aya::ContentId* contentIdValue;
        mutable bool boolValue;
        mutable int intValue;
        mutable int64_t int64Value;
        mutable unsigned int uintValue;
        mutable float floatValue;
        mutable double doubleValue;
        mutable const Aya::Name* nameValue;
        mutable Aya::InstanceHandle* handleValue; // TODO: with in-place constructor/destructor, could we avoid a "new"
    };
...
public:
    ...
    XmlNameValuePair(const XmlTag& tag, Aya::ContentId contentId)
        : tag(tag)
        , contentIdValue(new Aya::ContentId(contentId))
        , valueType(CONTENTID)
    {
    }
    ...
    XmlNameValuePair(const XmlTag& tag, const int64_t _number)
        : tag(tag)
        , valueType(INT64)
        , int64Value(_number)
    {
    ...
    XmlNameValuePair(const XmlTag& tag, const Aya::Name* value)
        : tag(tag)
        , valueType(NAME)
        , nameValue(value)
    {
    ...
    XmlNameValuePair(const XmlTag& tag, Aya::InstanceHandle value)
        : tag(tag)
        , valueType(HANDLE)
        , handleValue(new Aya::InstanceHandle(value))
    {
    }
    ~XmlNameValuePair()
    {
    ...
    bool isValueEqual(Aya::ContentId contentId) const;
    bool isValueEqual(int value) const;
    bool isValueEqual(int64_t value) const;
    bool isValueEqual(float value) const;
    bool isValueEqual(double value) const;
    bool isValueEqual(bool value) const;
    bool isValueEqual(const Aya::Name* value) const;
    bool isValueEqual(Aya::InstanceHandle value) const;
    ...
    bool getValue(Aya::ContentId& contentId) const;
    ...
    bool getValue(const Aya::Name*& value) const;
    bool getValue(Aya::InstanceHandle& value) const;
    // Templated version that can be implemented by clients
    template<class T>
    ...
    void setValue(Aya::ContentId contentId)
    {
        clearValue();
        contentIdValue = new Aya::ContentId(contentId);
        valueType = CONTENTID;
    ...
    void setValue(const Aya::Name* value)
    {
        clearValue();
        nameValue = value;
        valueType = NAME;
    };
    void setValue(Aya::InstanceHandle handle)
    {
        clearValue();
        handleValue = new Aya::InstanceHandle(handle);
        valueType = HANDLE;
    };
    // Templated version that can be implemented by clients
    template<class T>
...
public:
    ...
    ChildClass* firstChild()
    {
        return first;
    }
    ChildClass* nextChild(ChildClass* child)
    {
        return child->nextSibling();
    }
    const ChildClass* firstChild() const
    {
        return first;
    }
    const ChildClass* nextChild(const ChildClass* child) const
    {
        return child->nextSibling();
    }
};
...
protected:
    ...
    Sibling(SiblingClass* nextSibling)
        : next(nextSibling)
    {
    }

public:
    const SiblingClass* nextSibling() const
    {
        return next;
    }
    SiblingClass* nextSibling()
    {
        return next;
    }
...
private:
    ...
    void setNextSibling(SiblingClass* nextSibling)
    {
...
class XmlAttribute
    : public Aya::Sibling<XmlAttribute>
    , public XmlNameValuePair
    , public Aya::Allocator<XmlAttribute>
{
public:
...
class XmlElement
    : public Aya::Sibling<XmlElement>
    , public Aya::Parent<XmlElement>
    , public XmlNameValuePair
    , public Aya::Allocator<XmlElement>
{
#ifdef _DEBUG
    char leak[15];
    void recordLeak()
...
#endif
private:
    Aya::Parent<XmlAttribute> attributes;
...
public:
    ...
    inline const XmlAttribute* getNextAttribute(const XmlAttribute* attribute) const
    {
        return attributes.nextChild(attribute);
    }
    inline XmlAttribute* getFirstAttribute()
    {
        return attributes.firstChild();
    }
    inline XmlAttribute* getNextAttribute(XmlAttribute* attribute)
    {
        return attributes.nextChild(attribute);
    }
    ...
    inline bool findAttributeValue(const XmlTag& _tag, const Aya::Name*& value) const
    {
    ...
    // child elements
    //
    inline XmlElement* addChild(XmlElement* element)
    {
