...
void TextXmlWriter::xmlOrCDataEncodedWrite(std::ostream& stream, const std::string& textStr)
{
    ...
    if ((textStr.find("\n") != std::string::npos) && (textStr.find(kCDATA_CLOSE) == std::string::npos))
    {
    ...
    while (true)
    {
        ...
        if (isCloseTag(currentTag.c_str()))
        {
            ...
            if (currentElement == NULL)
                throw Aya::runtime_error("TextXmlParser::parse - Got close tag %s without open tag.", currentTag.c_str());
        ...
        else
        {
            ...
            if (newElement->getTag() == Aya::Reflection::Type::singleton<Aya::ContentId>().tag)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (tagName.compare(0, 6, "binary") == 0) // The binary tag may have attributes, so we have to compare a substring
                        {
                            ...
                            Aya::StandardOut::singleton()->printf(Aya::MESSAGE_WARNING, "Not reading binary data");
                            readText(false);
                            newElement->setValue(Aya::ContentId());
                        }
                        ...
                        {
                            newElement->setValue(Aya::ContentId(this->readText(true).c_str()));
                        }
                        else if (tag_null == tagName)
                        {
                            ...
                            throw Aya::runtime_error("TextXmlParser::parse - Unknown tag '%s'.", tagName.substr(0, 32).c_str());

                        std::string closingTag = readTag(); // closing tag
                        if (!isCloseTag(closingTag.c_str()))
                            throw Aya::runtime_error("TextXmlParser::parse - '%s' should be a closing tag", closingTag.substr(0, 32).c_str());
                    }
                }
            }
...
#else
XmlElement* parseNode(const pugi::xml_node& node)
{
    XmlElement* element = new XmlElement(XmlTag::lookup(node.name()));

    // Parse attributes
    for (pugi::xml_attribute attr = node.first_attribute(); attr; attr = attr.next_attribute())
    {
        element->addAttribute(XmlTag::lookup(attr.name()), attr.value());
    }

    // Special handling for ContentId
    if (element->getTag() == Aya::Reflection::Type::singleton<Aya::ContentId>().tag)
    {
        XmlAttribute* xsinil = element->findAttribute(name_xsinil);
        bool val;
        if (xsinil != NULL && xsinil->getValue(val) && val)
        {
            // no data
        ...
        {
            pugi::xml_node contentChild = node.first_child();
            if (contentChild)
            {
                std::string tagName = contentChild.name();
                if (tagName.compare(0, 6, "binary") == 0)
                {
                    Aya::StandardOut::singleton()->printf(Aya::MESSAGE_WARNING, "Not reading binary data");
                    element->setValue(Aya::ContentId());
                }
                else if (tag_hash == tagName)
                {
                    element->setValue(Aya::ContentId());
                ...
                {
                    element->setValue(Aya::ContentId(contentChild.child_value()));
                }
                else if (tag_null == tagName)
                {
                    element->setValue(Aya::ContentId());
            ...
            {
                // Old files might include an integer "ContentId" rather than a sub-element
                element->setValue(node.child_value());
            }
        }
    }
    else
    ...
    for (pugi::xml_node child = node.first_child(); child; child = child.next_sibling())
    {
        ...
        {
            element->addChild(parseNode(child));
        }
    }
...
std::auto_ptr<XmlElement> TextXmlParser::parse()
{
    ...
    pugi::xml_parse_result result = doc.load(buffer.str().c_str());

    if (!result)
        throw std::runtime_error("TextXmlParser::parse failed to parse XML");

    pugi::xml_node root = doc.first_child();
    if (!root)
        throw std::runtime_error("TextXmlParser::parse empty file");

    return std::auto_ptr<XmlElement>(parseNode(root));
}
#endif
...
{
    // Special handling for Aya::ContentId
    // TODO: move to Reflection::Property?
    if (xmlNode->isValueType<Aya::ContentId>())
    {
