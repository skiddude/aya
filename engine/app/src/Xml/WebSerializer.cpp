...
namespace Aya
{
XmlElement* WebSerializer::writeTable(const Aya::Reflection::ValueMap& result)
{
    XmlElement* table = new XmlElement(tag_WebTable);

    Aya::Reflection::ValueMap::const_iterator end = result.end();
    for (Aya::Reflection::ValueMap::const_iterator iter = result.begin(); iter != end; ++iter)
    {
...
XmlElement* WebSerializer::writeList(const Aya::Reflection::ValueArray& result)
{
    XmlElement* list = new XmlElement(tag_WebList);

    Aya::Reflection::ValueArray::const_iterator end = result.end();
    for (Aya::Reflection::ValueArray::const_iterator iter = result.begin(); iter != end; ++iter)
    {
...
XmlElement* WebSerializer::writeEntry(const std::string& key, const Aya::Reflection::Variant& value)
{
...
XmlElement* WebSerializer::writeValue(const Aya::Reflection::Variant& value)
{
    ...
    if (value.isType<shared_ptr<Aya::Instance>>())
    {
    ...
    if (value.isType<shared_ptr<const Aya::Reflection::ValueArray>>())
    {
        shared_ptr<const Aya::Reflection::ValueArray> valueList = value.get<shared_ptr<const Aya::Reflection::ValueArray>>();
