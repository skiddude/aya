...
class AyaBaseClass XmlWriter : public boost::noncopyable
{

protected:
    std::map<Aya::InstanceHandle, int> handles;
    typedef boost::unordered_map<std::string, Aya::InstanceHandle> IdValidationMap;
...
public:
    ...
    int getHandleIndex(Aya::InstanceHandle h)
    {
        int i;
        std::map<Aya::InstanceHandle, int>::iterator iter = handles.find(h);
        if (iter == handles.end())
        {
    ...
    bool isValidId(const std::string& id, const Aya::InstanceHandle& h) const
    {
    ...
    void recordId(const std::string& id, const Aya::InstanceHandle& h)
    {
        AYAASSERT(isValidId(id, h));
