...
/* ORDER RESERVED */
const char* const luaX_tokens[] = {"and", "break", "do", "else", "elseif", "end", "false", "for", "function", "if", "in", "local", "nil", "not", "or",
    "repeat", "return", "then", "true", "until", "while", "..", "...", "==", ">=", "<=", "~=", "<number>", "<name>", "<string>", "<eof>", NULL};
...
static void trydecpoint(LexState* ls, SemInfo* seminfo)
{
    ...
    ls->decpoint = getlocaledecpoint(); // Aya
...
static int lisxdigit(int c)
{
    return (c >= '0' && c <= '9') || (c >= 'A' && c <= 'F') || (c >= 'a' && c <= 'f');
}

// Function to convert a hexadecimal digit to its integer value
...
static unsigned long readutf8esc(LexState* ls)
{
    ...
    if (ls->current != '{')
    {
        luaX_lexerror(ls, "missing '{'", TK_STRING);
        return 0;
    }
    ...
    while (lisxdigit(ls->current))
    {
        ...
        if (r > 0x10FFFF)
        {
            luaX_lexerror(ls, "UTF-8 value too large", TK_STRING);
            return 0;
        }
        save_and_next(ls);
    }
