...
/// gcc craps out with the error while it is called from ScriptContext.cpp for registerClass fns for the Bridge e.g
/// EventBridge::registerClass(globalState);
// error: 'static int Aya::Lua::Bridge<Class, __eq>::on_index(lua_State*) [with Class = Aya::Lua::EventInstance, bool __eq = true]' is protected
#ifdef _WIN32
protected:
#endif
...
public:
    ...
    static void push(lua_State* L, boost::shared_ptr<Class> instance)
    {
        ...
        else
        {
            ...
            AYAASSERT(!lua_isnil(L, -1));           // Did you forget to call registerClassLibrary??
    ...
    static boost::shared_ptr<Class> getPtr(lua_State* L, unsigned int index)
    {
        if (lua_isnil(L, index))
            ...
            return Aya::Lua::Bridge<boost::shared_ptr<Class>, false>::getObject(L, index);
    }
