...
{
    const char* msg = (fs->f->linedefined == 0) ? luaO_pushfstring(fs->L, "main function has more than %d %s", limit, what)
...
static void check_match(LexState* ls, int what, int who, int where)
{
    if (!testnext(ls, what))
    {
        ...
        {
            luaX_syntaxerror(ls, luaO_pushfstring(ls->L, LUA_QS " expected (to close " LUA_QS " at line %d)", luaX_token2str(ls, what),
...
static int indexupvalue(FuncState* fs, TString* name, expdesc* v)
{
    ...
    luaM_growvector(fs->L, f->upvalues, f->nups, f->sizeupvalues, TString*, MAX_INT, "");
...
static void retstat(LexState* ls)
{
    ...
    else
    {
        ...
        if (hasmultret(e.k))
        {
            ...
            /* Aya: disable tail calls */
            if (e.k == VCALL && nret == 1 && false)
            { /* tail call? */
