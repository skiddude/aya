...
static FORCEINLINE void lua_aya_encoding_check(lua_State* L, Instruction i)
{
...
reentry: /* entry point */
    ...
    for (;;)
    {
        ...
        case OP_LEN:
        {
            ...
            default:
            { /* try metamethod */
                Protect(if (!call_binTM(L, rb, luaO_nilobject, ra, TM_LEN)) luaG_typeerror(L, rb, "get length of");)
            }
            }
            continue;
        }
        ...
        case OP_EQ:
        {
            ...
            TValue* rb = RKB(i);
            TValue* rc = RKC(i);
            // Aya assumption: the next opcode is a jump.
            Protect(if (equalobj(L, rb, rc) == GETARG_A(i)) dojump(L, pc, GETARG_sBx(rbxDecodeOpFast(*pc, ckey)));) pc++;
            continue;
        }
        case OP_LT:
        {
            ...
            Protect(
                // Aya assumption: the next opcode is a jump
                if (luaV_lessthan(L, RKB(i), RKC(i)) == GETARG_A(i)) dojump(L, pc, GETARG_sBx(rbxDecodeOpFast(*pc, ckey)));) pc++;
            continue;
        }
        case OP_LE:
        {
            ...
            // ra unused
            // Aya assumption: the next opcode is a jump
            Protect(if (lessequal(L, RKB(i), RKC(i)) == GETARG_A(i)) dojump(L, pc, GETARG_sBx(rbxDecodeOpFast(*pc, ckey)));) pc++;
            continue;
        }
        case OP_TEST:
        {
        ...
        case OP_SETLIST:
        {
            ...
            if (c == 0)
                c = cast_int(rbxDecodeFakeOp(*pc++, ckey));
