...
#define lua_assert(x) AYAASSERT(x)
#else
...
*/
#if LUAI_BITSINT >= 64
#define LUAI_UINT32 unsigned long long
#define LUAI_INT32 long long
#define LUAI_MAXINT32 LLONG_MAX
...
#define LUAI_TRY(L, c, a) \
    ...
    catch (Aya::base_exception const& e) \
    { \
...
class RobloxExtraSpace : public Aya::Intrusive::Set<RobloxExtraSpace>::Hook
{
    typedef Aya::Intrusive::Set<RobloxExtraSpace> AllThreads;

    struct Shared
    {
        int threadCount;
        Aya::ScriptContext* context;
    ...
    typedef Aya::Lua::WeakThreadRef::Node Node;
    boost::intrusive_ptr<Node> node;

public:
    Aya::Security::Identities identity : 5;
    bool yieldCaptured : 1;
    boost::weak_ptr<Aya::BaseScript> script; // The script associated with this thread, if any
    boost::scoped_ptr<Aya::Lua::Continuations> continuations;

    Aya::ScriptContext* context() const
    {
        return shared->context;
    }
    size_t getThreadCount() const
    {
        return (size_t)shared->threadCount;
    }

    void setContext(Aya::ScriptContext* context)
    {
...
private:
    RobloxExtraSpace()
        : shared(new Shared())
        , identity(Aya::Security::Anonymous)
        , node(0)
    {
        shared->threadCount++;
        ...
        , node(parent->node)
    {
        ...
        {
            Aya::Tokens::apiToken.addFlagSafe(Aya::kScriptContextCopy);
        }
        else
        {
            shared->allThreads.insert(*this);
    ...
    ~RobloxExtraSpace()
    {
        ...
        shared->threadCount--;
        AYAASSERT(shared->threadCount >= 0);
    }
};
#pragma pack(pop)
