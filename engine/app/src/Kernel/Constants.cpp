...
int Constants::worldStepsPerLongUiStep()
{
    return 8;
}
...
{
    return worldStepsPerLongUiStep() / 2;
}
...
int Constants::freeFallStepsPerWorldStep()
{
    return 1;
}
...
{
    return uiStepsPerSec() * worldStepsPerUiStep();
}
int Constants::kernelStepsPerSec()
{
    return worldStepsPerSec() * kernelStepsPerWorldStep();
}
int Constants::kernelStepsPerUiStep()
{
    return kernelStepsPerWorldStep() * worldStepsPerUiStep();
}
int Constants::freeFallStepsPerSec()
{
    return worldStepsPerSec() * freeFallStepsPerWorldStep();
}
int Constants::impulseSolverMaxIterations()
{
    return 60;
}
float Constants::impulseSolverAccuracy()
{
    return 0.1;
} // in unit of relative velocity
int Constants::impulseSolverAccuracyScalar()
{
    return 400;
} // how many parts to dial down accuracy for one level
float Constants::impulseSolverSymStateTorqueBound()
{
    return 0.05f;
}
float Constants::impulseSolverSymStateForceBound()
{
    return 0.001f;
}
float Constants::uiDt()
{
    return 1.0f / uiStepsPerSec();
}
float Constants::longUiStepDt()
{
    return 1.0f / longUiStepsPerSec();
}
float Constants::worldDt()
{
    return 1.0f / worldStepsPerSec();
}
float Constants::kernelDt()
{
    return 1.0f / kernelStepsPerSec();
}
float Constants::freeFallDt()
{
    return 1.0f / freeFallStepsPerSec();
}

// original joint K calcs were done and created the default jointK for 1x1x1 block of 960000 at 4560hz
float originalHz()
{
    return 30.0f * 8.0f * 19.0f;
}
// const float originalHz()						{return 30.0f * 8.0f * originalHzFactorTest;}
float Constants::unitJointK()
{
    return 960000.0f / (originalHz() * originalHz());
}
...
{
    AYAASSERT(std::abs(grid1 * 10.0f - Math::iRound(grid1 * 10.0f)) < 0.01f); // Can't enter with values too far off grid
    AYAASSERT(std::abs(grid2 * 10.0f - Math::iRound(grid2 * 10.0f)) < 0.01f); // indicates a bad snap
    ...
    { // for now, just keep increasing force proportional to ratio...
...
{
    AYAASSERT(clippedSortedSize.y >= clippedSortedSize.x);
    AYAASSERT(clippedSortedSize.z >= clippedSortedSize.y);
    AYAASSERT(clippedSortedSize.max(Vector3(1, 1, 1)) == clippedSortedSize);
