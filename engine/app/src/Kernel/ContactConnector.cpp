...
float ContactConnector::percentActive()
{
    ...
    float answer = (denom == 0) ? -1 : (float)(inContactHit) / (float)(denom);
...
bool ContactConnector::getSimBodyAndContactVelocity(SimBody*& simBody0, SimBody*& simBody1, PairParams& params, float& normalVel, Vector3& perpVel)
{
...
{
    AYAASSERT(!throttling || !canThrottle());
    ...
    if (params.length < 0.0)
    {
        ...
        if ((currentForce > maxForce) && (currentForce > 1e-8))
        ...
        threshold = (threshold != 0.0f) ? (-overlapGoal() + ((threshold + overlapGoal()) * 0.999f)) : (firstApproach == 0.0f) ? 0.0f : params.length;

        firstApproach = (firstApproach != 0.0f) ? (-overlapGoal() + ((firstApproach + overlapGoal()) * 0.999f)) : params.length;
        ...
        AYAASSERT(fabs(force.x) < Math::inf());
    }
    else
    {
        outOfContactHit++;
...
{
    AYAASSERT(geoPair.body0->getRootSimBody()->isInKernel() || geoPair.body1->getRootSimBody()->isInKernel());
    ...
    if (!impulseComputed)
    {
        ...
        if (-normalVel >= REST_THRESHOLD && !isRestingContact() && !symetricContact && velocityThisFrame >= 0.0f && velocityThisFrame <= -normalVel)
        {
