...
{
    AYAASSERT_VERY_FAST(!inStepCode);
    AYAASSERT_FISHING(Math::longestVector3Component(b->getBranchForce()) < 1e9f);
    return true;
}
...
{
    AYAASSERT(b->getRootSimBody()->getDt() == 0.0f);

    kernelData->insertBody(b); // bodies.fastAppend(b);
    AYAASSERT(validateBody(b));
...
void Kernel::removeBody(Body* b)
{
    ...
    AYAASSERT(!inStepCode);
    AYAASSERT((!b->getRootSimBody()->isContactBody() && !b->getRootSimBody()->isFreeFallBody()) ||
...
int Kernel::numFreeFallBodies() const
{
    return kernelData->freeFallBodies.size();
}
int Kernel::numRealTimeBodies() const
{
    return kernelData->realTimeBodies.size();
}
int Kernel::numJointBodies() const
{
    return kernelData->jointBodies.size();
}
int Kernel::numContactBodies() const
{
    return kernelData->contactBodies.size();
}
int Kernel::numLeafBodies() const
{
    return kernelData->leafBodies.size();
}
int Kernel::numPoints() const
{
    return kernelData->points.size();
}
int Kernel::numHumanoidConnectors() const
{
    return kernelData->humanoidConnectors.size();
}
int Kernel::numRealTimeConnectors() const
{
    return kernelData->realTimeConnectors.size();
}
int Kernel::numSecondPassConnectors() const
{
    return kernelData->secondPassConnectors.size();
}
int Kernel::numJointConnectors() const
{
    return kernelData->jointConnectors.size();
}
int Kernel::numBuoyancyConnectors() const
{
    return kernelData->buoyancyConnectors.size();
}
int Kernel::numContactConnectors() const
{
    return kernelData->contactConnectors.size();
}
int Kernel::numConnectors() const
{
    return numRealTimeConnectors() + numSecondPassConnectors() + numJointConnectors() + numContactConnectors() + numBuoyancyConnectors();
}
...
void Kernel::preStep()
{
    ...
    {
        Aya::Profiling::Mark mark(*profilingKernelBodies, false);
    ...
    {
        Aya::Profiling::Mark mark(*profilingKernelConnectors, false);
        ...
        if (!usingPGSSolver)
        {
            ...
            {
                ContactConnector* conn = aya_static_cast<ContactConnector*>(contactConnectors[i]);
...
{
    const IndexArray<SimBody, &SimBody::getRealTimeBodyIndex>& realTimeBodies(
        kernelData->realTimeBodies); // humanoid bodies that are not throttle-able
    {
        Aya::Profiling::Mark mark(*profilingKernelBodies, false);
...
{
    AYAPROFILER_SCOPE("Physics", "Kernel::stepWorld");

    IndexArray<SimBody, &SimBody::getFreeFallBodyIndex>& freeFallBodies(kernelData->freeFallBodies); // bodies with no connectors
    IndexArray<SimBody, &SimBody::getRealTimeBodyIndex>& realTimeBodies(kernelData->realTimeBodies); // humanoid bodies that are not throttle-able
    IndexArray<SimBody, &SimBody::getContactBodyIndex>& contactBodies(
        kernelData->contactBodies); // bodies with contact connectors but no joint connectors
    ...
    const IndexArray<Connector, &Connector::getRealTimeIndex>& realtimeConnectors(
        kernelData->realTimeConnectors); // connectors on humanoid body parts
    ...
    if (usingPGSSolver)
    {
        ...
        {
            AYAASSERT(validateConnector(secondPassConnectors[j]));
    ...
    else // legacy solver
    {
        ...
        if (contactConnectors.size() > 0)
        {
            ...
            int i;
            float tolerance = Constants::impulseSolverAccuracy() * (contactBodies.size() / Constants::impulseSolverAccuracyScalar() + 1);
            ...
            for (i = 0; i < Constants::impulseSolverMaxIterations(); ++i)
            {
                ...
                for (int j = 0; j < contactConnectors.size(); ++j)
                {
                    AYAASSERT(validateConnector(contactConnectors[j]));
        ...
        {
            {
                ...
                for (int j = 0; j < realtimeConnectors.size(); ++j)
                {
                    AYAASSERT(validateConnector(realtimeConnectors[j]));
                    realtimeConnectors[j]->computeForce(false);
                }

                for (int j = 0; j < jointConnectors.size(); ++j)
                {
                    AYAASSERT(validateConnector(jointConnectors[j]));
...
{
    AYAPROFILER_SCOPE("Physics", "Kernel::stepWorldThrottled");
...
void Kernel::stepFunnyPhysics(const Vector3& move)
{
    ...
    const IndexArray<SimBody, &SimBody::getRealTimeBodyIndex>& realTimeBodies(
        kernelData->realTimeBodies);                                                              // humanoid bodies that are not throttle-able
    const IndexArray<SimBody, &SimBody::getJointBodyIndex>& jointBodies(kernelData->jointBodies); // bodies with joint connectors
    const IndexArray<SimBody, &SimBody::getContactBodyIndex>& contactBodies(
        kernelData->contactBodies); // bodies with contact connectors but no joint connectors
