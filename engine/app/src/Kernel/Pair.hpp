...
public:
    ...
    struct
    {
        union
        {
            Aya::NormalId normalID0;
            float radius0;
        };
        union
        {
            Aya::NormalId normalID1;
            float radiusSum;
        };
        union
        {
            Aya::NormalId planeID; // edge/edge/plane coords - the normal from the plane
            int point0ID;
        };
    } pairData;
...
public:
    ...
    inline void computeLengthNormalPosition(PairParams& _params)
    {
        ...
        case (POINT_PLANE_PAIR):
            computePointPlane(_params);
            break;
        case (EDGE_EDGE_PLANE_PAIR):
            computeEdgeEdgePlane2(_params);
            break;
        case (EDGE_EDGE_PAIR):
            computeEdgeEdge(_params);
            break;
        default:
            AYAASSERT(0);
        }
    ...
    void setPointPlane(const Vector3* _offsetPoint, const Vector3* _offsetPlane, int _pointID, Aya::NormalId _planeNormalID)
    {
    ...
    void setEdgeEdgePlane(const Vector3* _edge0, const Vector3* _edge1, Aya::NormalId _normal0, Aya::NormalId _normal1, Aya::NormalId _planeID,
        float _edgeLength0, float _edgeLength1)
    {
        offset0 = *_edge0;
        offset1 = *_edge1;
    ...
    void setEdgeEdge(const Vector3* _edge0, const Vector3* _edge1, Aya::NormalId _normal0, Aya::NormalId _normal1)
    {
        offset0 = *_edge0;
        offset1 = *_edge1;
        pairData.normalID0 = _normal0;
    ...
    bool match(Body* _b0, Body* _b1, GeoPairType _pairType, int param0, int param1)
    {
        ...
        {
            AYAASSERT(_pairType == EDGE_EDGE_PAIR);
            return (((_b0 == body0) && (_b1 == body1) && (param0 == pairData.normalID0) && (param1 == pairData.normalID1)) ||
