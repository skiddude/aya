...
class BuoyancyConnector : public Aya::ContactConnector
{
private:
...
public:
    ...
    void getWaterBand(float& up, float& down)
    {
        up = floatDistance;
        down = sinkDistance;
    }
    void setWaterBand(const float& up, const float& down)
    {
        floatDistance = up;
        sinkDistance = down;
    }
    float getSubMergeRatio()
    {
        return submergeRatio;
    }
    void setSubMergeRatio(const float& ratio)
    {
        submergeRatio = ratio;
    }
