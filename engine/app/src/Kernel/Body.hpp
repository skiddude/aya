...
#include "Kernel/KernelIndex.hpp"
...
private:
    Aya::spin_mutex mutex; // for safe calls that require update
    ...
    int& getLeafBodyIndex()
    {
        return leafBodyIndex;
    }
    ...
    {
        return cofm;
    } // use these to insure const correctness;
    ...
    const SimBody* getConstSimBody() const
    {
        return simBody;
    }
    ...
    {
        AYAASSERT(getConstParent());
        AYAASSERT(!getConstLink()); // fails if linked (i.e. only works in same clump);
        return meInParent;
    }
    ...
    {
        return getConstParent() ? getConstParent()->calcRootConst() : this;
    }
...
public:
    ...
    SimBody* getRootSimBody()
    {
        return getRoot()->getSimBody();
    }
    const SimBody* getConstRootSimBody() const
    {
        return getRoot()->getConstSimBody();
    }
    ...
    void setGuidIndex(int _guidIndex)
    {
        guidIndex = _guidIndex;
    }
    int getGuidIndex() const
    {
        return guidIndex;
    }
    ...
    bool cofmIsClean()
    {
        return getCofm() ? !getCofm()->getIsDirty() : true;
    }
    ...
    Body* getChild(int i)
    {
        return getTypedChild<Body>(i);
    }
    const Body* getConstChild(int i) const
    {
        return getConstTypedChild<Body>(i);
    }

    Body* getParent()
    {
        return getTypedParent<Body>();
    }
    const Body* getConstParent() const
    {
        return getConstTypedParent<Body>();
    }
    ...
    {
        AYAASSERT_SLOW(root == calcRootConst());
        return root;
    }
    ...
    inline Matrix3 getIBody() const
    {
        return moment;
    }
    inline Vector3 getIBodyV3() const
    {
        return Math::toDiagonal(getIBody());
    }
    Matrix3 getIBodyAtPoint(const Vector3& point);
    inline Matrix3 getMoment() const
    {
        return getIBody();
    }
    inline Vector3 getPrincipalMoment() const
    {
        return getIBodyV3();
    }
    Matrix3 getIWorld()
    {
        return Math::momentToWorldSpace(getIBody(), getCoordinateFrame().rotation);
    }
    ...
    float getBranchMass()
    {
        return getCofm() ? getCofm()->getMass() : mass;
    }
    Matrix3 getBranchIBody()
    {
        return getCofm() ? getCofm()->getMoment() : moment;
    }
    Vector3 getBranchIBodyV3()
    {
        return Math::toDiagonal(getBranchIBody());
    }
    Matrix3 getBranchIWorld()
    {
        return Math::momentToWorldSpace(getBranchIBody(), getCoordinateFrame().rotation);
    }
    ...
    {
        AYAASSERT_FISHING(pvIsUpToDate());
        return pv;
    }
    ...
    {
        AYAASSERT(getRoot() == this); // should only be called on Root objects
        if (SimBody* s = getRootSimBody())
        {
    ...
    const Velocity& getBranchVelocity()
    {                                 // velocity at the COFM of the assembly
        AYAASSERT(getRoot() == this); // should only be called on Root objects
...
public:
    ...
    inline bool isLeafBody() const
    {
        return leafBodyIndex >= 0;
    }
