...
bool BulletShapeConnector::validObjectCFrames()
{
    ...
    return (!(Math::isNanInfVector3(geoPair.body0->getCoordinateFrame().translation) ||
              (Math::isNanInfVector3(geoPair.body1->getCoordinateFrame().translation))));
}
...
bool BulletShapeConnector::isPointInvalid(btManifoldPoint& manifoldPoint, double validThreshold)
{
    ...
    else
    {
        ...
        // Aya: force gContactThresholdOrthogonalFactor as 1.0 since that is what Bullet3 has it as -- perhaps not the best idea...?
        // if (distance2d > gContactThresholdOrthogonalFactor * gContactThresholdOrthogonalFactor * validThreshold * validThreshold)
        if (distance2d > validThreshold * validThreshold)
        {
            return true;
        }
    }
...
void BulletShapeConnector::updateConnectorPointFromManifold(bool refreshContacts)
{
    ...
    AYAASSERT(manifoldArray.size() > bulletManifoldIndex);
...
bool BulletShapeConnector::recalculateValidPoints(btManifoldArray& btManArray, Vector3& pt0InWorld, Vector3& pt1InWorld)
{
    ...
    btCollisionObjectWrapper obj0Wrap(
        0, bulletCollisionObject0->getCollisionShape(), bulletCollisionObject0, bulletCollisionObject0->getWorldTransform(), -1, -1);
    btCollisionObjectWrapper obj1Wrap(
        0, bulletCollisionObject1->getCollisionShape(), bulletCollisionObject1, bulletCollisionObject1->getWorldTransform(), -1, -1);
...
bool BulletShapeConnector::foundValidContactPointFromBulletManifold(btPersistentManifold* man, Vector3& p0World, Vector3& p1World)
{
    ...
    if ((bulletPointCacheIndex < man->getNumContacts() && man->getContactPoint(bulletPointCacheIndex).m_userPersistentData == this))
    {
        ...
        p0World =
            p1World + conPoint.getDistance() * Vector3(conPoint.m_normalWorldOnB.x(), conPoint.m_normalWorldOnB.y(), conPoint.m_normalWorldOnB.z());
...
void BulletShapeConnector::realignConnectorsToBulletContacts()
{
    ...
    {
        BulletShapeConnector* conn =
            aya_static_cast<BulletShapeConnector*>(manifoldArray[bulletManifoldIndex]->getContactPoint(i).m_userPersistentData);
        if (conn)
