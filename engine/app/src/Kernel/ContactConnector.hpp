...
public:
    ...
    static float overlapGoal()
    {
        return 0.01f;
    } // standard goal seek for overlapping objects

    /*override*/ Body* getBody(BodyIndex id)
    {
        return (id == body0) ? geoPair.body0 : geoPair.body1;
    }
    ...
    void reset()
    { // cleans up state variables for buffered version
        firstApproach = 0.0;
        threshold = 0.0;
        forceMagLast = 0.0;
        age = 0;
    ...
    inline void clearImpulseComputed()
    {
        impulseComputed = false;
    }
    ...
    bool isIntersecting()
    {
        AYAASSERT(geoPair.geoPairType == POINT_PLANE_PAIR);
    ...
    bool getSimBodyAndContactVelocity(SimBody*& simBody0, SimBody*& simBody1, PairParams& params, float& normalVel, Vector3& perpVel);
    float computeRelativeVelocity(const PairParams& params, Vector3* deltaVnormal, Vector3* perpVel);
    float computeRelativeVelocity();

    void applyContactPointForSymmetryDetection(SimBody* simBody0, SimBody* simBody1, const PairParams& params, float direction);

    const ContactParams& getContactParams() const
    {
        return contactParams;
    }
    void setContactParams(const ContactParams& params)
    {
    ...
    { // positive == bigger overlap
        updateContactPoint();
        return -contactPoint.length;
    }

    inline PairParams& getContactPoint()
    {
        return contactPoint;
    }
    inline const PairParams& getContactPoint() const
    {
        return contactPoint;
    }
...
{
public:
    GeoPairConnector(Body* b0, Body* b1, const ContactParams& contactParams)
    ...
    void setPointPlane(const Vector3* oPoint, const Vector3* oPlane, int pointId, NormalId planeId)
    {
        geoPair.setPointPlane(oPoint, oPlane, pointId, planeId);
    }

    void setEdgeEdgePlane(const Vector3* e0, const Vector3* e1, NormalId n0, NormalId n1, NormalId planeId, float edgeLength0, float edgeLength1)
    {
...
public:
    ...
    /*override*/ void updateContactPoint();
    void setBallBlock(float _radius0, const Vector3* _offset1, Aya::NormalId _normalID, GeoPairType _geoPairType)
    {
        offset1 = *_offset1;
        radius0 = _radius0;
        normalId1 = _normalID;
