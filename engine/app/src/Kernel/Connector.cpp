...
void PointToPointBreakConnector::forceToPoints(const G3D::Vector3& force)
{
    ...
    AYAASSERT_SLOW(force.magnitude() < Math::inf());
}
...
RotateConnector::RotateConnector(
    Body* _b0, Body* _b1, const CoordinateFrame& _j0, const CoordinateFrame& _j1, float _baseAngle, float kValue, float armLength)
    : b0(_b0)
    , b1(_b1)
    , j0(_j0)
    , j1(_j1)
...
{
    float angle = computeJointAngle(b0->getCoordinateFrame(), b1->getCoordinateFrame(), j0, j1, normal);

    float answer = angle - baseRotation;
    AYAASSERT_FISHING(fabs(answer) <= Math::twoPif());
    return answer;
}
...
{
    float angle = computeJointAngle(b0->getCoordinateFrameFast(), b1->getCoordinateFrameFast(), j0, j1, normal);

    float answer = angle - baseRotation;
    AYAASSERT(fabs(answer) <= Math::twoPif());
    return answer;
}

float RotateConnector::computeJointAngle(
    const CoordinateFrame& b0, const CoordinateFrame& b1, const CoordinateFrame& j0, const CoordinateFrame& j1, Vector3& normal)
{
    /*
...
void RotateConnector::computeForce(bool throttling)
{
    ...
    AYAASSERT(fabs(deltaRotation) <= Math::pif());
...
{
    Vector3 normal = Math::getWorldNormal(normalIdBody0, point0->getBody()->getCoordinateFrameFast().rotation);
