...
float SimBody::maxDebugTorque()
{
    return maxTorqueXX;
}
float SimBody::maxDebugForce()
{
    return maxForceXX;
}
float SimBody::maxDebugLinearImpulse()
{
    return maxLinearImpulseXX;
}
float SimBody::maxDebugRotationalImpulse()
{
    return maxRotationalImpulseXX;
}
...
{
    AYAASSERT(dt == 0.0f);
    AYAASSERT(freeFallBodyIndex == -1);
    AYAASSERT(realTimeBodyIndex == -1);
    AYAASSERT(jointBodyIndex == -1);
    AYAASSERT(contactBodyIndex == -1);
}
...
void SimBody::update()
{
    ...
    constantForceY = body->getBranchMass() * Units::kmsAccelerationToRbx(body->getGravity());

    AYAASSERT_VERY_FAST(Math::longestVector3Component(force) < 1e20f);
    AYAASSERT_VERY_FAST(constantForceY < 1e20f);
    AYAASSERT_VERY_FAST(body->cofmIsClean());
    AYAASSERT_FISHING(!Math::isNanInfDenormVector3(qOrientation.imag()));

    AYAASSERT(dirty); // concurrency check
...
{
    AYAASSERT(!dirty);
    AYAASSERT_SLOW(Math::longestVector3Component(force) < 1e19);
    AYAASSERT_SLOW(Math::longestVector3Component(pv.position.translation) < 1e6);
    AYAASSERT_SLOW(Math::longestVector3Component(pv.velocity.linear) < 1e6);
    AYAASSERT_SLOW(Math::longestVector3Component(force) < 1e19);
    AYAASSERT_SLOW(Math::longestVector3Component(pv.position.translation) < 1e6);
    AYAASSERT_SLOW(Math::longestVector3Component(pv.velocity.linear) < 1e6);
    ...
    AYAASSERT_VERY_FAST(!Math::isNanInfDenormVector3(pv.position.translation));
    AYAASSERT_FISHING(!Math::isNanInfDenormVector3(qOrientation.imag()));
    AYAASSERT_VERY_FAST(!Math::isNanInfDenormMatrix3(pv.position.rotation));
    AYAASSERT_VERY_FAST(!Math::isNanInfDenormVector3(pv.velocity.linear));
    AYAASSERT_VERY_FAST(!Math::isNanInfDenormVector3(pv.velocity.rotational));
...
void SimBody::stepPosition()
{
    ...
    AYAASSERT_SLOW(Math::longestVector3Component(pv.position.translation) < 1e6);
    AYAASSERT(!hasExternalForceOrImpulse());
...
void SimBody::stepFreeFall()
{
    ...
    //
    float dyHalf = Units::kmsAccelerationToRbx(body->getGravity()) * dt / 2.0f;
