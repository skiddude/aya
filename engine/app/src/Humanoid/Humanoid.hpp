...
private:
    ...
    boost::unordered_map<shared_ptr<PartInstance>, Aya::signals::scoped_connection> siblingMap;
    void updateSiblingPropertyListener(shared_ptr<PartInstance> sibling);
    void onEvent_SiblingPropertyChanged(const Aya::Reflection::PropertyDescriptor* desc);

    shared_ptr<StatusInstance> status;

    Aya::signals::scoped_connection onCFrameChangedConnection;
    Aya::signals::scoped_connection humanoidEquipConnection;
    ...
    void renderMultiplayer(Adorn* adorn, const Aya::Camera& camera);

    void renderBillboard(Adorn* adorn, const Aya::Camera& camera);
    ...
    /*override*/ virtual bool hasFocusCoord() const
    {
        return getHeadSlow() != NULL;
    }
    ...
    Vector3 getSimulatedFrictionVelocityOffset(PartInstance* floorPart, Vector3& newCharPosition, const Vector3& previousCharPosition,
...
public:
    void updateNetworkFloorPosition(PartInstance* floorPart, CoordinateFrame& previousFloorPosition, Aya::Velocity& lastFloorVelocity, float& netDt);
    ...
    bool isLocalSimulating() const
    {
        return localSimulating;
    }
    ...
    Aya::signal<void(Status)> statusAddedSignal;
    Aya::signal<void(Status)> statusRemovedSignal;
    Aya::signal<void(std::string)> customStatusAddedSignal;
    Aya::signal<void(std::string)> customStatusRemovedSignal;

    Aya::remote_signal<void(shared_ptr<Instance>)> serverEquipToolSignal;

    // Humanoid Network Update Connection
    Aya::signals::scoped_connection onPositionUpdatedByNetworkConnection;

    NameOcclusion getNameOcclusion() const
    {
        return nameOcclusion;
    }
    void setNameOcclusion(NameOcclusion value);

    HumanoidDisplayDistanceType getDisplayDistanceType() const
    {
        return displayDistanceType;
    }
    ...
    static const Vector3& defaultCharacterCorner()
    {
        static Vector3 corner(2.5f, 2.5f, 2.5f);
        return corner;
    }

    // By Definition, these are all called by HumanoidState - these are reflected
    Aya::signal<void()> diedSignal;
    Aya::signal<void(float)> swimmingSignal;
    Aya::signal<void(float)> runningSignal;  // state change scripts
    Aya::signal<void(float)> climbingSignal; // state change scripts
    Aya::signal<void(bool)> jumpingSignal;   // state change scripts
    Aya::signal<void(bool)> freeFallingSignal;
    Aya::signal<void(bool)> strafingSignal;
    Aya::signal<void(bool)> gettingUpSignal;
    Aya::signal<void(bool)> fallingDownSignal;
    Aya::signal<void(bool)> ragdollSignal;
    Aya::signal<void(bool, shared_ptr<Instance>)> seatedSignal;
    Aya::signal<void(bool)> platformStandingSignal;
    Aya::signal<void(Aya::HUMAN::StateType, Aya::HUMAN::StateType)> stateChangedSignal;
    Aya::signal<void(Aya::HUMAN::StateType, bool)> stateEnabledChangedSignal;
    Aya::HUMAN::StateType getCurrentStateType();
    Aya::HUMAN::StateType getPreviousStateType()
    {
        return previousState;
    };
    void setPreviousStateType(Aya::HUMAN::StateType newState);
    void changeState(Aya::HUMAN::StateType state);
    static bool isStateInString(const std::string& text, const Aya::HUMAN::StateType& compare, Aya::HUMAN::StateType& value);

    Aya::signal<void(float)> healthChangedSignal;

    // Internal use only - no reflection - happens both client, server
    Aya::signal<void()> doneSittingSignal;
    Aya::signal<void()> donePlatformStandingSignal;

    void equipToolInstance(shared_ptr<Instance> instance);
    void equipTool(Aya::Tool* tool);
    void unequipTools();
    ...
    void testWalkSpeed(float walkSpeed, float percentWalkSpeed) const
    {
        ...
        {
            Aya::Security::setHackFlagVs<LINE_RAND4>(Aya::Security::hackFlag11, HATE_SPEEDHACK);
        }
    ...
    float getPercentWalkSpeed() const
    {
    ...
    const Vector3& getLastFloorNormal() const
    {
        return lastFloorNormal;
    }
    void setLastFloorNormal(const Vector3& norm)
    {
        lastFloorNormal = norm;
    }
    ...
    bool getBreakJointsOnDeath() const
    {
        return shouldBreakJointsOnDeath;
    }
    void setBreakJointsOnDeath(bool value)
    {
        shouldBreakJointsOnDeath = value;
    }

    // Health / damage interface
    void setHealth(float value);
    void setHealthUi(float value);
    void zeroHealthLocal()
    {
        health = 0.0f;
    } // for a non-simulating client -zero out health without bouncing back to the server
    ...
    void setClickToWalkEnabled(bool value)
    {
        clickToWalkEnabled = value;
    }
    ...
    float getWalkAngleError() const
    {
        return walkAngleError;
    }

    void setWalkToPoint(const Vector3& value);
    const Vector3& getWalkToPoint() const
    {
        return walkToPoint;
    }
    ...
    PartInstance* getSeatPart() const
    {
        return seatPart.get();
    }
    ...
    bool getActivatePhysics()
    {
        return activatePhysics;
    }
    const Vector3 getActivatePhysicsImpulse()
    {
        return activatePhysicsImpulse;
    }
    bool getTouchedHard()
    {
        return touchedHard;
    ...
    int getRagdollCriteria() const
    {
        return ragdollCriteria;
    }

    void setPlatformStanding(bool value);
    bool getPlatformStanding() const
    {
        return platformStanding;
    }

    void setStrafe(bool value);
    bool getStrafe() const
    {
        return strafe;
    }
    ...
    bool breakJointsOnDeath() const
    {
        return shouldBreakJointsOnDeath && hadNeck && hadHealth;
    }

    void setTargetPoint(const Vector3& value);
    void setTargetPointLocal(const Vector3& value); // does not replicate
    const Vector3& getTargetPoint() const
    {
        return targetPoint;
    }
    ...
    const Vector3& getCamearaOffset() const
    {
        return cameraOffset;
    }
    ...
    const shared_ptr<PartInstance>& getLastFloor() const
    {
        return lastFloorPart;
    }
    ...
    shared_ptr<PartInstance> getRootFloorPart() const
    {
        return rootFloorMechPart;
    }

    int getCurrentFloorFilterPhase();
    int getCurrentFloorFilterPhase(Assembly* floorAssembly);

    void setLastFloorPhase(int phase)
    {
        lastFilterPhase = phase;
    }
    int getLastFloorPhase() const
    {
        return lastFilterPhase;
    }

    // Walk Utilities
    void moveTo(const Vector3& worldPosition, PartInstance* part);
    void moveTo2(Vector3 worldPosition, shared_ptr<Instance> part);
    Aya::signal<void(bool)> moveToFinishedSignal;

    bool getUseR15() const
    {
        return (rigType != HUMANOID_RIG_TYPE_R6);
    }
    Humanoid::HumanoidRigType getRigType() const
    {
        return rigType;
    }
    ...
    // Build Joints
    void buildJoints(Aya::DataModel* dm = NULL);
    ...
    const World* getConstWorld() const
    {
        return world;
    };
    ...
    bool getOwnedByLocalPlayer() const
    {
        return ownedByLocalPlayer;
    }

    bool getWalkingFromStudioTouchEmulation() const
    {
        return isWalkingFromStudioTouchEmulation;
    }
    void setWalkingFromStudioTouchEmulation(bool value)
    {
        isWalkingFromStudioTouchEmulation = value;
    }
};
