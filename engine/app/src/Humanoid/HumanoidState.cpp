...
#include "World/Buoyancy.hpp"
#include "Kernel/Body.hpp"
#include "Kernel/Constants.hpp"
#include "Players.hpp"
#include "Base/Adorn.hpp" // to do - move to cpp file when it will compile with Windows.h in front of it
#include "Draw.hpp"
...
const int STATE_TABLE[NUM_STATE_TYPES + 1][NUM_EVENT_TYPES + 1] = {xx, NO_HEALTH, NO_NECK, JUMP_CMD, STRAFE_CMD, NO_STRAFE_CMD, SIT_CMD, NO_SIT_CMD,
    PLATFORM_STAND_CMD, NO_PLATFORM_STAND_CMD, TIPPED, UPRIGHT, FACE_LDR, AWAY_LDR, OFF_FLOOR, OFF_FLOOR_GRACE, ON_FLOOR, TOUCHED, NEARLY_TOUCHED,
    TOUCHED_HARD, ACTIVATE_PHYSICS, FINISHED, TIMER_UP, NO_TOUCH_ONE_SECOND, HAS_GYRO, HAS_BUOYANCY, NO_BUOYANCY,

    FALLING_DWN, DEAD, DEAD, xx, xx, xx, SEATED, xx, PLATFORM_STANDING, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, GETTING_UP, xx, xx,
    SWIMMING, xx, RAGDOLL, DEAD, DEAD, GETTING_UP, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, GETTING_UP, xx, xx, xx, xx,
    GETTING_UP, DEAD, DEAD, xx, xx, xx, SEATED, xx, PLATFORM_STANDING, xx, xx, RUNNING, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, SWIMMING,
    xx, JUMPING, DEAD, DEAD, xx, xx, xx, SEATED, xx, PLATFORM_STANDING, xx, xx, xx, xx, xx, FREE_FALL, FREE_FALL, xx, xx, xx, xx, xx, FREE_FALL,
    FREE_FALL, xx, xx, xx, xx, SWIMMING, DEAD, DEAD, JUMPING, xx, xx, SEATED, xx, PLATFORM_STANDING, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, RAGDOLL,
    xx, xx, xx, xx, xx, xx, GETTING_UP, FREE_FALL, DEAD, DEAD, xx, xx, xx, SEATED, xx, PLATFORM_STANDING, xx, xx, xx, CLIMBING, xx, xx, xx, LANDED,
    xx, xx, xx, xx, xx, xx, xx, xx, SWIMMING, xx, FLYING, DEAD, DEAD, xx, xx, xx, SEATED, xx, PLATFORM_STANDING, xx, FALLING_DWN, xx, CLIMBING, xx,
    xx, xx, RUNNING, xx, xx, xx, xx, xx, xx, xx, xx, SWIMMING, xx, LANDED, DEAD, DEAD, xx, xx, xx, SEATED, xx, PLATFORM_STANDING, xx, FALLING_DWN, xx,
    xx, xx, xx, FREE_FALL, xx, xx, xx, xx, xx, xx, RUNNING, xx, xx, SWIMMING, xx, RUNNING, DEAD, DEAD, JUMPING, xx, xx, SEATED, xx, PLATFORM_STANDING,
    xx, FALLING_DWN, xx, CLIMBING, xx, xx, FREE_FALL, xx, xx, xx, RAGDOLL, xx, xx, xx, RUNNING_NO_PHYS, xx, SWIMMING, xx, RUNNING_SLAVE, DEAD, DEAD,
    JUMPING, xx, xx, SEATED, xx, PLATFORM_STANDING, xx, FALLING_DWN, xx, CLIMBING, xx, xx, FREE_FALL, xx, xx, xx, RAGDOLL, xx, xx, xx,
    RUNNING_NO_PHYS, xx, SWIMMING, xx, // change here for phys
    RUNNING_NO_PHYS, DEAD, DEAD, JUMPING, STRAFING_NO_PHYS, xx, SEATED, xx, PLATFORM_STANDING, xx, xx, xx, CLIMBING, xx, xx, FREE_FALL, xx, RUNNING,
    RUNNING, xx, RUNNING, xx, xx, xx, RUNNING, SWIMMING, xx, // change here for phys
    STRAFING_NO_PHYS, DEAD, DEAD, JUMPING, xx, RUNNING_NO_PHYS, SEATED, xx, PLATFORM_STANDING, xx, xx, xx, CLIMBING, xx, xx, FREE_FALL, xx, RUNNING,
    RUNNING, xx, RUNNING, xx, xx, xx, RUNNING, SWIMMING, xx, CLIMBING, DEAD, DEAD, JUMPING, xx, xx, SEATED, xx, PLATFORM_STANDING, xx, FALLING_DWN,
    xx, xx, RUNNING, CLIMBING, CLIMBING, CLIMBING, xx, xx, xx, xx, xx, xx, xx, xx, SWIMMING, xx, SEATED, DEAD, DEAD, JUMPING, xx, xx, xx, RUNNING,
    PLATFORM_STANDING, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, PLATFORM_STANDING, DEAD, DEAD, JUMPING, xx, xx, SEATED,
    xx, xx, RUNNING, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, DEAD, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx,
    xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, PHYSICS, DEAD, DEAD, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx, xx,
    xx, xx, xx, xx, xx};
...
{
    AYAASSERT(STATE_TABLE[oldState + 1][0] == oldState);
    AYAASSERT(STATE_TABLE[0][eventType + 1] == eventType);
    ...
    , floorMaterial(AIR_MATERIAL)
{
    ...
    if (torso)
        buoyancyConnections.push_back(
            torso->onDemandWrite()->buoyancyChangedSignal.connect(boost::bind(&HumanoidState::setTorsoHasBuoyancy, this, _1)));

    PartInstance* leftLeg = humanoid->getLeftLegSlow();
    if (leftLeg)
        buoyancyConnections.push_back(
            leftLeg->onDemandWrite()->buoyancyChangedSignal.connect(boost::bind(&HumanoidState::setLeftLegHasBuoyancy, this, _1)));

    PartInstance* rightLeg = humanoid->getRightLegSlow();
    if (rightLeg)
        buoyancyConnections.push_back(
            rightLeg->onDemandWrite()->buoyancyChangedSignal.connect(boost::bind(&HumanoidState::setRightLegHasBuoyancy, this, _1)));
}
...
HumanoidState::~HumanoidState()
{
    ...
    std::vector<Aya::signals::connection>::iterator buoyancyConnectionsIterator;
    for (buoyancyConnectionsIterator = buoyancyConnections.begin(); buoyancyConnectionsIterator != buoyancyConnections.end();
        ++buoyancyConnectionsIterator)
    {
...
void HumanoidState::setCanThrottleState(bool canThrottle)
{
    if (humanoid)
    {
        ...
        {
            humanoid->getHeadSlow()->getPartPrimitive()->setCanThrottle(canThrottle);
        }
        if (humanoid->getTorsoSlow())
        {
            humanoid->getTorsoSlow()->getPartPrimitive()->setCanThrottle(canThrottle);
        }
        if (humanoid->getRightArmSlow())
        {
            humanoid->getRightArmSlow()->getPartPrimitive()->setCanThrottle(canThrottle);
        }
        if (humanoid->getLeftArmSlow())
        {
            humanoid->getLeftArmSlow()->getPartPrimitive()->setCanThrottle(canThrottle);
        }
        if (humanoid->getRightLegSlow())
        {
            humanoid->getRightLegSlow()->getPartPrimitive()->setCanThrottle(canThrottle);
        }
        if (humanoid->getLeftLegSlow())
        {
            humanoid->getLeftLegSlow()->getPartPrimitive()->setCanThrottle(canThrottle);
        }
    }
}
...
void HumanoidState::render3dAdorn(Adorn* adorn)
{
    ...
    if (shouldRender || FFlag::DebugHumanoidRendering)
    {
        ...
        if (humanoid->getHeadSlow())
        {
            adorn->drawFont2D(getName().toString(), Vector2(16, yBase),
                16, // size
                false, Color3::white(), Color3::black(), Text::FONT_LEGACY, Text::XALIGN_LEFT, Text::YALIGN_BOTTOM);

            char buf[256];
            snprintf(buf, sizeof(buf), "torque(%8.1f, %8.1f, %8.1f), length:%8.1f", maxTorque.x, maxTorque.y, maxTorque.z, maxTorque.length());
            adorn->drawFont2D(std::string(buf), Vector2(16, yBase + 20),
                16, // size
                false, Color3::white(), Color3::black(), Text::FONT_LEGACY, Text::XALIGN_LEFT, Text::YALIGN_BOTTOM);

            snprintf(buf, sizeof(buf), "force(%8.1f, %8.1f, %8.1f), length:%8.1f", maxForce.x, maxForce.y, maxForce.z, maxForce.length());
            adorn->drawFont2D(std::string(buf), Vector2(16, yBase + 40),
                16, // size
                false, Color3::white(), Color3::black(), Text::FONT_LEGACY, Text::XALIGN_LEFT, Text::YALIGN_BOTTOM);

            snprintf(buf, sizeof(buf), "contact velocity : %f", maxContactVel);
            adorn->drawFont2D(std::string(buf), Vector2(16, yBase + 60),
                16, // size
                false, Color3::white(), Color3::black(), Text::FONT_LEGACY, Text::XALIGN_LEFT, Text::YALIGN_BOTTOM);
        }
    }
...
void HumanoidState::preStepSimulatorSide(float dt)
{
    ...
    if (DFFlag::CheckForHeadHit)
    {
        ...
        if (foundBody && foundHead)
        {
            ...
            hitPrim = humanoid->getWorld()->getContactManager()->getHit(caster, &dummy, mergedFilter, hitLocation);
...
void HumanoidState::simulate(shared_ptr<HumanoidState>& state, float dt)
{
    ...
    state->onStepImpl();
    AYAASSERT(!state->getAssembly() || state->getStateType() == state->stateFromAssembly());
...
void HumanoidState::updateHumanoidFloorStatus(shared_ptr<HumanoidState>& state)
{
    ...
    if (isValidSubscriptionHumanoidState(state) && lastFloorPrim)
    {
        ...
        if (floorAssembly)
        {
            ...
            PartInstance* floorRootPart = PartInstance::fromPrimitive(Aya::Mechanism::getRootMovingPrimitive(lastFloorPrim));
        ...
        if (!state->humanoid->onPositionUpdatedByNetworkConnection.connected())
        {
            state->humanoid->onPositionUpdatedByNetworkConnection =
                PartInstance::fromPrimitive(floorRootPrim)
                    ->onDemandWrite()
                    ->onPositionUpdatedByNetworkSignal.connect(boost::bind(&Humanoid::updateNetworkFloorPosition, state->humanoid, _1, _2, _3, _4));
...
bool HumanoidState::hasFloorChanged(shared_ptr<HumanoidState>& state, Primitive* lastFloorPrim)
{
    ...
    if (currentFloor && lastFloorPrim && lastFloorPrim->getAssembly())
    {
        ...
        return (
            (lastFloorPrim != currentFloor->getAssembly()->getAssemblyPrimitive()) || (lastFloorPrim->getAssembly() != currentFloor->getAssembly()));
    }
    return false;
}
...
void HumanoidState::doSimulatorStateTable(shared_ptr<HumanoidState>& state, float dt)
{
    ...
    for (int i = 0; i < NUM_EVENT_TYPES; ++i)
    {
        ...
        AYAASSERT(newType != RUNNING_SLAVE); // only should come to slave side, never out of state table
    ...
    if (isBadType)
    {
        Aya::Security::setHackFlagVmp<LINE_RAND1>(Aya::Security::hackFlag5, HATE_HSCE_EBX);
    }
    if (isBadNvReg)
    {
        Tokens::sendStatsToken.addFlagFast(HATE_HSCE_EBX);
        Tokens::simpleToken |= HATE_HSCE_EBX;
        Aya::Security::setHackFlagVmp<LINE_RAND1>(Aya::Security::hackFlag7, HATE_HSCE_EBX);
    }
}
...
bool HumanoidState::computeEvent(EventType eventType)
{
    ...
    case HAS_BUOYANCY:
        ...
        returnValue = computeHasBuoyancy();
        break;
    case NO_BUOYANCY:
    ...
    case NO_HEALTH:
        ...
        returnValue = (humanoid->getHealth() <= 0.0f);
        break;
    case NO_NECK:

        returnValue = (!humanoid->getNeck());
        break;
    case JUMP_CMD:

        returnValue = computeJumped();
        break;
    case SIT_CMD:

        returnValue = humanoid->getSit();
        break;
    case NO_SIT_CMD:

        returnValue = !humanoid->getSit();
        break;
    case PLATFORM_STAND_CMD:

        returnValue = humanoid->getPlatformStanding();
        break;
    case NO_PLATFORM_STAND_CMD:

        returnValue = !humanoid->getPlatformStanding();
        break;
    case TIPPED:

        returnValue = computeTipped();
        break;
    case UPRIGHT:

        returnValue = computeUpright();
        break;
    case FACE_LDR:

        returnValue = facingLadder;
        break;
    case AWAY_LDR:
    ...
    case NO_STRAFE_CMD:
        ...
        returnValue = !humanoid->getStrafe();
        break;
    case ON_FLOOR:

        returnValue = (floorPart != NULL) && (!DFFlag::FixJumpGracePeriod || getRelativeMovementVelocity().y <= 0.0f);
        break;
    case OFF_FLOOR:

        returnValue = (floorPart == NULL);
        break;
    case OFF_FLOOR_GRACE:

        returnValue = (noFloorTimer > fallDelay());
        break;
    case TOUCHED:

        returnValue = computeTouched();
        break;
    case NEARLY_TOUCHED:

        returnValue = computeNearlyTouched();
        break;
    case TOUCHED_HARD:

        returnValue = computeTouchedHard();
        break;
    case ACTIVATE_PHYSICS:

        returnValue = computeActivatePhysics();
        break;
    case TIMER_UP:

        returnValue = (timer <= 0.0f);
        break;
...
HumanoidState* HumanoidState::createNew(StateType newType, StateType oldType, Humanoid* humanoid)
{
    ...
    {
    case FALLING_DWN:
        return new FallingDown(humanoid, oldType);
    case GETTING_UP:
        return new GettingUp(humanoid, oldType);
    case JUMPING:
        return new Jumping(humanoid, oldType);
    case FREE_FALL:
        return new Freefall(humanoid, oldType);
    case FLYING:
        return new Flying(humanoid, oldType);
    case RAGDOLL:
        return new Ragdoll(humanoid, oldType);
    case LANDED:
        return new Landed(humanoid, oldType);
    case RUNNING:
        return new Running(humanoid, oldType);
    case RUNNING_SLAVE:
        return new RunningSlave(humanoid, oldType);
    case RUNNING_NO_PHYS:
        return new RunningNoPhysics(humanoid, oldType);
    case STRAFING_NO_PHYS:
        return new StrafingNoPhysics(humanoid, oldType);
    case CLIMBING:
        return new Climbing(humanoid, oldType);
    case SEATED:
        return new Seated(humanoid, oldType);
    case PLATFORM_STANDING:
        return new PlatformStanding(humanoid, oldType);
    case SWIMMING:
        return new Swimming(humanoid, oldType);
    case DEAD:
        return new Dead(humanoid, oldType);
    case PHYSICS:
        return new Physics(humanoid, oldType);
    default:
    {
        AYAASSERT(0);
        return new Running(humanoid, oldType);
...
void HumanoidState::fireEvent(StateType stateType, bool entering)
{
    ...
    case RUNNING_NO_PHYS:
        ...
        break;
    case CLIMBING:
        humanoid->climbingSignal(0.0f);
        break;
    case STRAFING_NO_PHYS:
        humanoid->strafingSignal(entering);
        break;
    case FALLING_DWN:
        humanoid->fallingDownSignal(entering);
        break;
    case PHYSICS:
    case RAGDOLL:
        humanoid->ragdollSignal(entering);
        break;
    case GETTING_UP:
        humanoid->gettingUpSignal(entering);
        break;
    case JUMPING:
        humanoid->jumpingSignal(entering);
        break;
    case FLYING:
    case LANDED:
    case FREE_FALL:
        humanoid->freeFallingSignal(entering);
        break;
    case SEATED:
        humanoid->seatedSignal(entering, shared_from(humanoid->getSeatPart()));
        break;
    case PLATFORM_STANDING:
        humanoid->platformStandingSignal(entering);
        break;
    case DEAD:
        humanoid->diedSignal();
        break;
...
const char* HumanoidState::getStateNameByType(StateType state)
{
    ...
    {
    case RUNNING_NO_PHYS:
        return RunningNoPhysics::name().c_str();
        break;
    case RUNNING:
        return Running::name().c_str();
        break;
    case CLIMBING:
        return Climbing::name().c_str();
        break;
    case STRAFING_NO_PHYS:
        return StrafingNoPhysics::name().c_str();
        break;
    case FALLING_DWN:
        return FallingDown::name().c_str();
        break;
    case RAGDOLL:
        return Ragdoll::name().c_str();
        break;
    case GETTING_UP:
        return GettingUp::name().c_str();
        break;
    case JUMPING:
        return Jumping::name().c_str();
        break;
    case FLYING:
        return Flying::name().c_str();
        break;
    case LANDED:
        return Landed::name().c_str();
        break;
    case FREE_FALL:
        return Freefall::name().c_str();
        break;
    case SEATED:
        return Seated::name().c_str();
        break;
    case PLATFORM_STANDING:
        return PlatformStanding::name().c_str();
        break;
    case DEAD:
        return Dead::name().c_str();
        break;
    case SWIMMING:
        return Swimming::name().c_str();
        break;
    case PHYSICS:
        return Physics::name().c_str();
        break;
...
{
    if ((computeFloorTilt() > steepSlopeAngle() || (getStateType() != RUNNING && getStateType() != RUNNING_NO_PHYS)) && humanoid->getJump() &&
        humanoid->getJumpPower() > 0.0f)
    {
        return true;
    }
    else
...
{
    if (computeTilt() > 0.90f)
    { // if (yAxis.y > (1.0 + 2.0 * Balancing::fallCutoff()) / 3.0)
        return true;
    }
    else
    {
...
{
    AYAASSERT(humanoid->getWorld());
    if (World* world = humanoid->getWorld())
    {
        if (PartInstance* foundTorso = humanoid->getTorsoSlow())
        {
            Aya::SystemAddress myLocalAddress = Aya::Network::Players::findLocalSimulatorAddress(humanoid);
        ...
        {
            AYAASSERT(0); // no torso - why am I here?
        }
    }
    return false;
}
...
static bool intersectsOutsideOfCharacter(Primitive* check, const Instance* character)
{
    ...
    while (c)
    {
        ...
        if (!PartInstance::fromPrimitive(other)->isDescendantOf(character) && c->computeIsCollidingUi(0.0f))
        {
            return true;
        }
    ...
    if (humanPrim)
    {
        for (int i = 0; i < humanPrim->getNumContacts(); ++i)
        {
            ...
            for (int j = 0; j < contact->numConnectors(); ++j)
            {
                ...
                const Primitive* objectPrim =
                    contact->getConstPrimitive(0) == humanPrim ? contact->getConstPrimitive(1) : contact->getConstPrimitive(0);
...
void HumanoidState::findFloor(shared_ptr<PartInstance>& oldFloor)
{
    if (Primitive* torsoPrim = humanoid->getTorsoPrimitiveSlow())
    {
        ...
        for (int z = 0; z <= 4; z++)
        {
            ...
            AverageFloorRayCast(floorPart, floorTouchInWorld, floorTouchNormal, floorMaterial, floorHitLocationAccumulate, floorHitCount,
                UpdateFloorPart, offset, maxDistance, humanoidAssembly, torsoC);
        }
        ...
        if (floorPart == NULL)
        {
            ...
            for (int x = -1; x <= 1; x += 2)
            {
                for (int z = -1; z <= 1; z += 2)
                {
                    ...
                    AverageFloorRayCast(floorPart, floorTouchInWorld, floorTouchNormal, floorMaterial, floorHitLocationAccumulate, floorHitCount,
                        true, offset, maxDistance, humanoidAssembly, torsoC);
                }
            }
        }
...
HitTestFilter::Result HumanoidState::filterResult(const Primitive* testMe) const
{
    ...
    if ((!testMe->getCanCollide()) || (filteringAssembly && (filteringAssembly == testMe->getConstAssembly())))
    {
...
void HumanoidState::AverageFloorRayCast(shared_ptr<PartInstance>& floorPart, Vector3& floorPartHitLocation, Vector3& floorPartHitNormal,
    PartMaterial& floorPartHitMaterial, Vector3& hitLocationAccumulator, int& hitLocationCount, const bool UpdateFloorPart, const Vector3& offset,
    const float maxDistance, Assembly* humanoidAssembly, const CoordinateFrame& torsoC)
{
...
shared_ptr<PartInstance> HumanoidState::tryFloor(
    const RbxRay& ray, Vector3& hitLocation, Vector3& hitNormal, float maxDistance, Assembly* humanoidAssembly, PartMaterial& recentFloorMaterial)
{
    AYAASSERT(humanoid->getWorld());

    AYAASSERT(humanoidAssembly);
    filteringAssembly = humanoidAssembly;
    Primitive* floorPrim = NULL;

    AYAASSERT(ray.direction().isUnit());
    RbxRay worldRay = RbxRay::fromOriginAndDirection(ray.origin(), ray.direction());
    worldRay.direction() *= maxDistance;

    floorPrim = humanoid->getWorld()->getContactManager()->getHit(worldRay, NULL, this, hitLocation, false, true, hitNormal, recentFloorMaterial);
...
const Velocity HumanoidState::getFloorPointVelocity()
{
    ...
    if (Primitive* p = getFloorPrimitive())
    {
        ...
        if (p->getWorld())
        {
            AYAASSERT(floorTouchInWorld != unitializedFloorTouch());
...
{
    Aya::Primitive* torso = humanoid->getTorsoPrimitiveSlow();
    if (torso)
    {
...
void HumanoidState::fireMovementSignal(Aya::signal<void(float)>& movementSignal, float movementVelocity)
{
    if (humanoid->getTorsoPrimitiveSlow() && !movementSignal.empty())
    {
        if (DFFlag::EnableClimbingDirection)
        {
            ...
            // This fixes an issue where changing climbing direction instantly will make your direction not ever hit 0, causing the absolute checks to
            // not see a change.
...
float ladderSearchDistance(Humanoid* pHumanoid)
{
    return searchDepth(pHumanoid) * 1.5f;
} //  1.5x search depth
...
bool HumanoidState::findPrimitiveInLadderZone(Adorn* adorn)
{
    ...
    if (adorn)
    {
        ...
        adorn->box(AABox(extents.min(), extents.max()));
    }
    return false;
}

void HumanoidState::doLadderRaycast(GeometryService* geom, const RbxRay& caster, Humanoid* humanoid, Primitive** hitPrimOut, Vector3* hitLocationOut)
{
    if (humanoid->getWorld())
    {
        ...
        *hitPrimOut = humanoid->getWorld()->getContactManager()->getHit(caster, &dummy, mergedFilter, *hitLocationOut);
...
bool HumanoidState::findLadder(Adorn* adorn)
{
    ...
    for (int i = 1; i < topRay; i++) // search for 3 studs, starting at toes		NOTE - removed bottom search
    {
        ...
        if (hitPrim)
        {
            ...
            if (p)
            {
                if (p->getPartType() == Aya::TRUSS_PART)
                    return true;
            }
        }
    ...
    if (first_space < maxClimbDistance() * heightScale && first_step > 0.0f && first_step < maxClimbDistance() * heightScale &&
...
unsigned int HumanoidState::checkComputeEvent()
{
    ...
    bool positiveTests = computeEvent(HAS_BUOYANCY) && !computeEvent(NO_BUOYANCY) && computeEvent(OFF_FLOOR) && computeEvent(SIT_CMD) &&
    ...
    bool negativeTests = !(computeEvent(HAS_BUOYANCY) || !computeEvent(NO_BUOYANCY) || computeEvent(ON_FLOOR) || computeEvent(SIT_CMD) ||
                           computeEvent(PLATFORM_STAND_CMD) || computeEvent(FACE_LDR));
