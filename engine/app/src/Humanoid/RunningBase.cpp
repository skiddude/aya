...
#include "Humanoid/RunningBase.hpp"
    ...
    , desiredAltitude(0.0f)
{
    ...
    AYAASSERT(!humanoid->getTorsoSlow() || humanoid->getTorsoSlow()->getPartPrimitive()->getEngineType() == Primitive::DYNAMICS_ENGINE);
}
...
void RunningBase::onComputeForceImpl()
{
    ...
    // Rotate with the ground
    {
        ...
        float desiredTorqueY =
            kP * root->getBranchIBodyV3().y * (floorVelocity.rotational.y + desiredVelocity.rotational.y - root->getVelocity().rotational.y);
    ...
    if ((getHumanoid()->getWorld()->getUsingPGSSolver()) || floor == NULL || floor->getAnchoredProperty() ||
    ...
    {
        AYAASSERT(floor->getAssembly()->getAssemblyPrimitive()->getBody() != root);
        float yAccelDesired =
            (kAltitudeP * (desiredAltitude - torsoBody->getPos().y)) - (kAltitudeD * (root->getVelocity().linear.y - floorVelocity.linear.y));
        if (yAccelDesired > 0.0)
        { // If yAccelDesired <= 0.0 then just free-fall
