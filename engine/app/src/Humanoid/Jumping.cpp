...
void Jumping::onComputeForceImpl()
{
    ...
    if (!getFinished())
    {
        ...
        // StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "%f", root->getVelocity().linear.y);

        if (yAccelDesired <= 0.0)
        { // We've acheived the desired velocity
            setFinished(true);
        }
        else if (findCeiling())
        {
...
shared_ptr<PartInstance> Jumping::tryCeiling(const RbxRay& ray, float maxDistance, Assembly* humanoidAssembly)
{
    ...
    Primitive* floorPrim = humanoid->getWorld()->getContactManager()->getHitLegacy(ray, NULL, this, hitPoint, distance, maxDistance, true);
