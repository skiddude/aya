    ...
    : Named<HumanoidState, sMovingNoPhysicsBase>(humanoid, priorState)
{
    ...
    AYAASSERT(!torsoPart.get() || torsoPart->getPartPrimitive()->getEngineType() == Primitive::DYNAMICS_ENGINE);
...
void MovingNoPhysicsBase::onComputeForceImpl()
{
    ...
    if (torsoPart.get())
    {
        AYAASSERT(!torsoPart->getPartPrimitive()->inKernel());
    }
#endif
}
...
void MovingNoPhysicsBase::disconnectTorso()
{
    ...
    if (torsoPart.get())
    {
        AYAASSERT(torsoPart->getPartPrimitive()->getEngineType() == Primitive::HUMANOID_ENGINE);
...
void MovingNoPhysicsBase::applyImpulseToFloor(float dt)
{
    if (Body* root = getHumanoid()->getRootBodyFast())
    {
        if (Primitive* floor = getFloorPrimitive())
        {
            ...
            float force = mass * Units::kmsAccelerationToRbx(getHumanoid()->getWorld()->getGravity()); // note kmsGravity.y is negative.
...
void MovingNoPhysicsBase::onSimulatorStepImpl(float stepDt)
{
    ...
    if (Primitive* torso = getHumanoid()->getTorsoSlow()->getPartPrimitive())
    {
        ...
        {
            AYAASSERT(!(torso->getBody()->getRootSimBody()->isInKernel())); // make sure engine not simulating with physics
            ...
            if (floor)
            {
                ...
                if (dot < steepSlopeAngle())
                {
                    ...
                    if (downhill.unitize() > Math::epsilonf())
                    {
                        ...
                        desiredVelocity.linear =
                            inputOntoSurface + (downhill * getHumanoid()->getWalkSpeed()) - (inputOntoSurface.dot(downhill) * downhill);
                    }
                }
            }
            ...
            Vector3 XZForce =
                Vector3(desiredVelocity.linear.x - currentPV.velocity.linear.x, 0.0f, desiredVelocity.linear.z - currentPV.velocity.linear.z);
            ...
            if (getHumanoid()->getWorld()->getUsingPGSSolver())
            {
                if (floor)
                {
                    ...
                    Vector3 XZDeltaV = Vector3(desiredVelocity.linear.x - currentPV.velocity.linear.x, 0.0f,
                                           desiredVelocity.linear.z - currentPV.velocity.linear.z) *
                                       kFric;
