...
float Freefall::characterVelocityInfluence()
{
    return 0.0f;
} // 0 to 1 - part of initial velocity carrying over
float Freefall::floorVelocityInfluence()
{
    return 0.95f;
} // 0 to 1 - part of initial velocity carrying over
float Freefall::velocityDecay()
{
    return 0.05f;
} // 0 to 1 - part of initial velocity lost every 1/30 sec
...
Freefall::~Freefall()
{
    ...
    if (PartInstance* head = getHumanoid()->getHeadSlow())
    {
        ...
        {
            AYAASSERT(head->getPartPrimitive()->getFriction() == 0.0f);
    ...
    if (PartInstance* torso = getHumanoid()->getTorsoSlow())
    {
        ...
        {
            AYAASSERT(torso->getPartPrimitive()->getFriction() == 0.0f);
...
void Freefall::onSimulatorStepImpl(const float stepDt)
{
    ...
    else
    {
        if (desiredWalkVelocity.linear.magnitude() > 0.1)
        {
            ...
            if (!Aya::GameBasicSettings::singleton().mouseLockedInMouseLockMode())
            {
...
void Freefall::onComputeForceImpl()
{
    ...
    if (root)
    {
        ...
        {
            desiredTorqueY =
                RunningBase::kTurnPForFreeFallPGS() * root->getBranchIBodyV3().y * (desiredVelocity.rotational.y - root->getVelocity().rotational.y);
        }
