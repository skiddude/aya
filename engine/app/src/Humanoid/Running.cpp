...
Running::Running(Humanoid* humanoid, StateType priorState)
    : Named<RunningBase, sRunning>(humanoid, priorState)
{
}
...
void Running::onComputeForceImpl()
{
    ...
    if ((lastForce.squaredLength() > forceCriteriaSquare || lastTorque.squaredLength() > forceCriteriaSquare) && computeHitByHighImpactObject())
    {
...
RunningSlave::RunningSlave(Humanoid* humanoid, StateType priorState)
    : Named<Running, sRunningSlave>(humanoid, priorState)
{
}
