...
#include "Utility/HitTestFilter.hpp"
...
typedef enum
{
    ...
    NUM_STATE_TYPES,
    xx
} StateType; // XX == NO change

typedef enum
{
    NO_HEALTH = 0, // Humanoid Commands
    NO_NECK,
    JUMP_CMD,
    STRAFE_CMD,
    NO_STRAFE_CMD,
...
private:
    ...
    bool usesLadder() const
    {
        return (usesEvent(FACE_LDR) || usesEvent(AWAY_LDR));
    }
    bool usesFloor() const
    {
        return (usesEvent(OFF_FLOOR) || usesEvent(ON_FLOOR) || usesEvent(OFF_FLOOR_GRACE));
    }
    ...
    int ladderCheck;
    virtual int ladderCheckRate()
    {
        return 2;
    }
    bool findPrimitiveInLadderZone(Adorn* adorn);
    bool findLadder(Adorn* adorn);
    void doLadderRaycast(GeometryService* geom, const RbxRay& caster, Humanoid* humanoid, Primitive** hitPrimOut, Vector3* hitLocationOut);
    void doAutoJump();
    void findFloor(shared_ptr<PartInstance>& oldFloor);
    shared_ptr<PartInstance> tryFloor(const RbxRay& ray, Vector3& hitLocation, Vector3& hitNormal, float maxDistance, Assembly* humanoidAssembly,
        PartMaterial& recentFloorMaterial);
...
protected:
    ...
    static float minMoveVelocity()
    {
        return 0.5f;
    }
    static float maxClimbDistance()
    {
        return 2.45f;
    } // studs
    static const Vector3 maxMoveForce()
    {
        static Vector3 m(1000.0f, 10000.0f, 1000.0f);
        return m;
    } // const Vector3 maxMoveAccelerationGrid(5e4f*0.02f, 5e5f*0.02f, 1e4f*0.02f);
    static const Vector3 minMoveForce()
    {
        static Vector3 m(-1000.0f, 0.0f, -1000.0f);
        return m;
    }
    static const Vector3 maxSwimmingMoveForce()
    {
        static Vector3 m(10000.0f, 1000.0f, 10000.0f);
        return m;
    }
    static const Vector3 minSwimmingMoveForce()
    {
        static Vector3 m(-10000.0f, -10000.0f, -10000.0f);
        return m;
    }
    static float fallDelay()
    {
        return 0.125f;
    }
    static float maxLinearMoveForce()
    {
        return 143.0;
    }
...
public:
    ...
    static float maxLinearGroundMoveForce()
    {
        return 500.0;
    }
...
protected:
    ...
    bool getFacingLadder() const
    {
        return facingLadder;
    }
    bool getHeadClear() const
    {
        return headClear;
    }
    ...
    {
        AYAASSERT(floorHumanoidLocationInWorld != unitializedFloorTouch());
    ...
    void fireMovementSignal(Aya::signal<void(float)>& movementSignal, float movementVelocity);
...
public:
    virtual bool armsShouldCollide() const
    {
        return true;
    }
    virtual bool legsShouldCollide() const
    {
        return true;
    }
    virtual bool headShouldCollide() const
    {
        return true;
    }
    virtual bool torsoShouldCollide() const
    {
        return true;
    }

    virtual bool enableAutoJump() const
    {
        return true;
    }

    virtual void onCFrameChangedFromReflection()
    {
        preStepFloor();
    } // recalculate floor part
    ...
    virtual float getYAxisRotationalVelocity() const
    {
        return 0.0f;
    }
    ...
    std::vector<Aya::signals::connection> buoyancyConnections;
    void setTorsoHasBuoyancy(bool value)
    {
        torsoHasBuoyancy = value;
    }
    void setLeftLegHasBuoyancy(bool value)
    {
        leftLegHasBuoyancy = value;
    }
    void setRightLegHasBuoyancy(bool value)
    {
        rightLegHasBuoyancy = value;
    }
    ...
    {
        // Aya: Small change to fix clang builds
#if defined(_WIN32) && !defined(__clang__)
        bool (Aya::HUMAN::HumanoidState::*hsce)(EventType) = &computeEvent;
