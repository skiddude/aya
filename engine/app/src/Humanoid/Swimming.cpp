...
void Swimming::onComputeForceImpl()
{
    ...
    // control torque for facing direction
    {
        ...
        AYAASSERT(!Math::isNanInfVector3(angVelRoot));

        // P control component
        Vector3 controlTorqueRoot =
            -pPitch * (root->getBranchIBody() * rootCoord.vectorToObjectSpace(pitchTilt)); // apply scalar to Vector3, not Matrix3
        ...
        // Aya::StandardOut::singleton()->printf( Aya::MESSAGE_WARNING, "Swimming facing vector (%f, %f, %f)\n", controlTorqueRoot.x,
        // controlTorqueRoot.y, controlTorqueRoot.z );
    }
}
