...
private:
    ...
    typedef std::map<boost::weak_ptr<Aya::PartInstance>, float> PartsTransparencyCollection;
    ...
    Aya::signals::scoped_connection selectionChangedConnection;
    void setToSelection();
...
protected:
    ...
    bool getExtentsAndLocation(Extents& extents, CoordinateFrame& location, bool& isLocal) const;
...
private:
    ...
    void getGridXYUsingCamera(Aya::PartInstance* part, G3D::Vector3& gridXDir, G3D::Vector3& gridYDir);

    G3D::Vector3 originalLocation;

public:
    AdvMoveTool(Workspace* workspace)
        : Named<AdvMoveToolBase, sAdvMoveTool>(workspace)
    {
    }
    ~AdvMoveTool() {}
    ...
    {
        return Creatable<MouseCommand>::create<AdvMoveTool>(workspace);
    }
};
