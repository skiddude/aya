...
public:
    AdvArrowToolBase(Workspace* workspace)
        : ArrowToolBase(workspace)
        , manualJointHelper(workspace)
    {
    }
...
{
public:
    AdvArrowTool(Workspace* workspace)
        : Named<AdvArrowToolBase, sAdvArrowTool>(workspace)
    {
    }

    /*override*/ shared_ptr<MouseCommand> isSticky() const
    {
        return Creatable<MouseCommand>::create<AdvArrowTool>(workspace);
    }
...
private:
    ...
    void getMouseInstances(std::set<shared_ptr<Instance>>& instances, const shared_ptr<InputObject>& inputObject, const Rect2D& selectBox,
        const Camera* camera, Instance* currentInstance);
