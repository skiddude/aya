...
Vector3 DragUtilities::hitObjectOrPlane(const PartArray& parts, const RbxRay& unitMouseRay, const ContactManager& contactManager, bool snapToGrid)
{
    ...
    hitObjectOrPlane(contactManager, unitMouseRay, primitives, hit, snapToGrid);
    return hit;
}
...
void DragUtilities::unJoinFromOutsiders(const PartArray& parts)
{
    ...
    AYAASSERT(notJoinedToOutsiders(parts));
}
...
void DragUtilities::move(const PartArray& parts, CoordinateFrame from, CoordinateFrame to)
{
    for (size_t i = 0; i < parts.size(); ++i)
    {
        const shared_ptr<PartInstance>& part = parts[i].lock();
...
void DragUtilities::move2(const PartArray& parts, CoordinateFrame from, CoordinateFrame to)
{
    for (size_t i = 0; i < parts.size(); ++i)
    {
        const shared_ptr<PartInstance>& part = parts[i].lock();
...
bool DragUtilities::hitObjectOrPlane(const ContactManager& contactManager, const RbxRay& unitSearchRay,
    const std::vector<const Primitive*>& ignorePrims, Vector3& hit, bool snapToGrid)
{
    AYAASSERT(unitSearchRay.direction().isUnit());
    RbxRay searchRay = MouseCommand::getSearchRay(unitSearchRay); // length determines how far to search;

    if (!contactManager.getHit(searchRay, &ignorePrims, NULL, hit))
    {
...
bool DragUtilities::hitObject(const ContactManager& contactManager, const RbxRay& unitSearchRay, const std::vector<const Primitive*>& ignorePrims,
    Vector3& hit, bool snapToGrid)
{
    AYAASSERT(unitSearchRay.direction().isUnit());
    RbxRay searchRay = MouseCommand::getSearchRay(unitSearchRay); // length determines how far to search;
...
Vector3 DragUtilities::toLocalGrid(const Vector3& deltaIn)
{
    ...
    AYAASSERT(DragUtilities::getGrid().x == DragUtilities::getGrid().y && DragUtilities::getGrid().y == DragUtilities::getGrid().z &&
