...
#endif
    AYAASSERT(!dragPart.expired());
#ifdef _DEBUG
    AYAASSERT(Workspace::getWorldIfInWorkspace(dragPart.lock().get()) == world);
#endif
    AYAASSERT(dragPart.lock()->getPartPrimitive() == drag);
    AYAASSERT(handleMultipleParts || PartInstance::nonNullInWorkspace(dragPart.lock()));

    shared_ptr<PartInstance> p(snapPart.lock());
    if (PartInstance::nonNullInWorkspace(p))
    {
#ifdef _DEBUG
        AYAASSERT(Workspace::getWorldIfInWorkspace(p.get()) == world);
#endif
...
void AdvRunDragger::snapPartFromSnapInfo()
{
    if (snapInfo.snap)
    {
        ...
        AYAASSERT(Workspace::getWorldIfInWorkspace(snapPart.lock().get()) == workspace->getWorld());
    }
    else
    {
        snapPart.reset();
...
void AdvRunDragger::initLocal(Workspace* _workspace, weak_ptr<PartInstance> _dragPart, const Vector3& _dragPointLocal, WeakParts _dragParts)
{
    AYAASSERT(!_dragPart.expired());
    AYAASSERT(PartInstance::nonNullInWorkspace(_dragPart.lock()));
    ...
    for (unsigned int i = 0; i < _dragParts.size(); i++)
    {
        ...
        AYAASSERT(!partFromGroup.expired());
#ifdef _DEBUG
        AYAASSERT(Workspace::getWorldIfInWorkspace(partFromGroup.lock().get()) == world);
#endif
        AYAASSERT(PartInstance::nonNullInWorkspace(partFromGroup.lock()));
    ...
    if (handleMultipleParts)
    {
        ...
        if (selection.size() == 1)
        {
            Aya::ModelInstance* mInstance = Aya::Instance::fastDynamicCast<Aya::ModelInstance>(selection[0]);
            if (mInstance)
    ...
    AYAASSERT(handleMultipleParts || (Workspace::getWorldIfInWorkspace(dragPart.lock().get()) == workspace->getWorld()));
...
void AdvRunDragger::init(Workspace* _workspace, weak_ptr<PartInstance> _dragPart, const Vector3& _dragPointWorld)
{
    AYAASSERT(!_dragPart.expired());
    AYAASSERT(PartInstance::nonNullInWorkspace(_dragPart.lock()));
    workspace = _workspace;
    ...
    AYAASSERT(Workspace::getWorldIfInWorkspace(dragPart.lock().get()) == workspace->getWorld());
}
...
bool AdvRunDragger::moveDragPart()
{
    ...
    AYAASSERT(Math::isOrthonormal(dragInWorld));
    AYAASSERT(Math::isOrthonormal(snapRotation));
    ...
    bool hitPlane =
#endif
        Math::intersectLinePlane(Line::fromPointAndDirection(rayInDrag.origin(), rayInDrag.direction()), plane, dragHitLocal);

    AYAASSERT_IF_VALIDATING(hitPlane);
...
AdvRunDragger::SnapInfo AdvRunDragger::rayHitsPart(const G3D::Array<Primitive*>& triedSnap, bool forceAdjacent)
{
    ...
    if (handleMultipleParts)
    {
        ...
        foundPart = MouseCommand::getMousePart(mouseRay, *workspace->getWorld()->getContactManager(), ignorePrims, &filter, answer.hitWorld);
    }
    else
    {
        foundPart = MouseCommand::getMousePart(mouseRay, *workspace->getWorld()->getContactManager(), drag, &filter, answer.hitWorld);
    }
...
AdvRunDragger::SnapInfo AdvRunDragger::bestProximatePart(const G3D::Array<Primitive*>& triedSnap, Contact::ProximityTest proximityTest)
{
    SnapInfo answer;
...
{
    AYAASSERT_VERY_FAST(!drag->hasAutoJoints());
    if (handleMultipleParts)
    {
...
void AdvRunDragger::findNoSnapPosition(const CoordinateFrame& original)
{
    ...
    PartInstance* foundPart = MouseCommand::getMousePart(mouseRay, *workspace->getWorld()->getContactManager(), ignorePrims, &filter, hitWorld);
    if (!foundPart)
    {
        ...
        if (Math::intersectRayPlane(MouseCommand::getSearchRay(mouseRay), Plane(Vector3::unitY(), Vector3::zero()), hitPoint))
        {
            ...
            if (handleMultipleParts)
            {
                AYAASSERT(primaryPart);
                Aya::CoordinateFrame cFrameBefore = primaryPart->getCoordinateFrame();
...
void AdvRunDragger::tiltPart(PartInstance* part, const CoordinateFrame& camera)
{
    ...
    AYAASSERT(0); // test: are we even hitting this?
...
void AdvRunDragger::findSafeY()
{
    if (colliding())
    {
        ...
        AYAASSERT(moved.x == 0.0);
        AYAASSERT(moved.y != 0.0);
...
bool AdvRunDragger::snap(const RbxRay& _mouseRay)
{
    ...
    if (snapInfo.snap)
    {                                            // if last snap part is now too far, zero it
    ...
    while (snapInfo.snap && (triedSnap.find(snapInfo.snap) == triedSnap.end()))
    {
        ...
        else if (rayHitsCloserPart())
        {
            failure = 3;
        }
        else if (tooCloseToCamera())
        {
            failure = 4;
        }
...
bool AdvRunDragger::snapGroup(const RbxRay& _mouseRay)
{
    ...
    PartInstance* foundPart =
        MouseCommand::getMousePart(mouseRay, *workspace->getWorld()->getContactManager(), excludeParts, &filter, answer.hitWorld);
...
{
    if (Math::intersectRayPlane(MouseCommand::getSearchRay(unitMouseRay), Plane(Vector3::unitY(), Vector3::zero()), hitPoint))
    {
        hitPoint = DragUtilities::hitObjectOrPlane(weakDragParts, unitMouseRay, *workspace->getWorld()->getContactManager(), false);
...
{
    if (dragPart.lock())
        Aya::Draw::selectionBox(dragPart.lock()->getPart(), adorn, Color4(Color3::orange()));
}
#endif
