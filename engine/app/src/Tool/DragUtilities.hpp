...
{
private:
    static bool hitObjectOrPlane(const ContactManager& contactManager, const RbxRay& unitSearchRay, const std::vector<const Primitive*>& ignorePrims,
        Vector3& hit, bool snapToGrid = true);
    static bool hitObject(const ContactManager& contactManager, const RbxRay& unitSearchRay, const std::vector<const Primitive*>& ignorePrims,
        Vector3& hit, bool snapToGrid = true);
...
public:
    ...
    static void move(const PartArray& parts, CoordinateFrame from, CoordinateFrame to);
    static void move2(const PartArray& parts, CoordinateFrame from, CoordinateFrame to);
    ...
    static Vector3 hitObjectOrPlane(const PartArray& parts, const RbxRay& unitMouseRay, const ContactManager& contactManager, bool snapToGrid = true);

    static bool hitObject(
        const PartArray& parts, const RbxRay& unitMouseRay, const ContactManager& contactManager, Vector3& hit, bool snapToGrid = true);
    ...
    static Vector3 safeMoveYDrop(
        const PartArray& parts, const Vector3& tryDrag, ContactManager& contactManager, const float customPlaneHeight = Dragger::groundPlaneDepth());
