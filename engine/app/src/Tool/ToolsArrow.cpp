...
{
    AYAASSERT(inputObject->isLeftMouseDownEvent());
    Vector3 hitWorld;
    ...
    if (frontPart)
    {
        ...
        else
        {
            ...
            return DragTool::onMouseDown(frontPart, hitWorld, instanceSelection->items(), inputObject, workspace, selectIfNoDrag);
        }
    }
    else
    {
...
shared_ptr<MouseCommand> ArrowToolBase::onPeekKeyDown(const shared_ptr<InputObject>& inputObject)
{
    ...
    AYAASSERT(inputObject->isKeyDownEvent());
    switch (inputObject->getKeyCode())
    {
    case AYA_SDLK_f: // we want to zoom in on any selected object
        if (UserInputService* userInputService = ServiceProvider::find<UserInputService>(workspace))
        ...
        break;
    case AYA_SDLK_LEFTBRACKET: // take us back one in our camera history
        if (Camera* camera = workspace->getCamera())
            camera->stepCameraHistoryBackward();
        break;
    case AYA_SDLK_RIGHTBRACKET: // take us forward one in our camera history
        if (Camera* camera = workspace->getCamera())
...
{
    // Why the fuck does this just shit itself on Studio SPECIFICALLY only on run mode
#ifndef AYA_STUDIO
...
{
template<>
EnumDesc<AdvArrowToolBase::JointCreationMode>::EnumDesc()
...
shared_ptr<MouseCommand> AdvArrowToolBase::onKeyDown(const shared_ptr<InputObject>& inputObject)
{
    switch (inputObject->getKeyCode())
    {
    case AYA_SDLK_g: // _G_rid mode cycle (1-stud -> 1/5-stud -> off -> 1-stud ->...)
    {
        switch (advGridMode)
        {
        case DRAG::ONE_STUD:
...
shared_ptr<MouseCommand> AdvArrowToolBase::onMouseDown(const shared_ptr<InputObject>& inputObject)
{
    ...
    if (frontPart)
    {
        ...
        else
        {
            ...
            ServiceClient<FilteredSelection<Instance>> instanceSelection(workspace);
            return AdvDragTool::onMouseDown(frontPart, hitWorld, instanceSelection->items(), inputObject, workspace, selectIfNoDrag);
        }
    }
    else
    {
...
void BoxSelectCommand::selectReverse(const std::set<shared_ptr<Instance>>& newItemsInBox)
{
    ...
    std::set_difference(newItemsInBox.begin(), newItemsInBox.end(), previousItemsInBox.begin(), previousItemsInBox.end(),
        std::inserter(toggleItems, toggleItems.begin()));

    std::set_difference(previousItemsInBox.begin(), previousItemsInBox.end(), newItemsInBox.begin(), newItemsInBox.end(),
        std::inserter(toggleItems, toggleItems.begin()));
...
void BoxSelectCommand::getMouseInstances(std::set<shared_ptr<Instance>>& instances, const shared_ptr<InputObject>& inputObject,
    const Rect2D& selectBox, const Camera* camera, Instance* currentInstance)
{
