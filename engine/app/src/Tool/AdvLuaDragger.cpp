...
void AdvLuaDragger::mouseDownPublic(shared_ptr<Instance> _mousePart, Vector3 _pointOnMousePart, shared_ptr<const Instances> _dragParts)
{
...
{
    FASTLOG3F(
        FLog::DragProfile, "AdvLuaDragger - received mouse ray, origin: %fx%fx%f", mouseRay.origin().x, mouseRay.origin().y, mouseRay.origin().z);

    FASTLOG3F(FLog::DragProfile, "AdvLuaDragger - received mouse ray, direction: %fx%fx%f", mouseRay.direction().x, mouseRay.direction().y,
        mouseRay.direction().z);
...
void AdvLuaDragger::startDragging()
{
    if (shared_ptr<PartInstance> safeMousePart = mousePart.lock())
    {
        ...
        advRunDragger->initLocal(Workspace::findWorkspace(safeMousePart.get()), mousePart, pointOnMousePart, dragParts);
    }
}
            ...
            else
            {
                if (AdvRunDragger::dragMultiPartsAsSinglePart)
                {
                    if (advRunDragger.get())
                    {
                        ...
                        advRunDragger->snap(unitMouseRay);
                        // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "Time to snap (multiple part new): %lf", ( std::clock() - start )
                ...
                else
                {
                    ...
                    if (getSnapHitPoint(safeMousePart.get(), unitMouseRay, hitSnapped))
                    {
                        ...
                        if (delta != Vector3::zero())
                        {
                            ...
                            for (WeakParts::const_iterator iter = dragParts.begin();
                                iter != dragParts.end() && coordinateFrameOffset < m_originalPositions.size(); ++iter)
                            {
                                if (shared_ptr<PartInstance> partInstance = iter->lock())
