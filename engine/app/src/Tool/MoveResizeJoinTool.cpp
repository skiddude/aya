...
void MoveResizeJoinTool::findTargetPV(const shared_ptr<InputObject>& inputObject)
{
    ...
    for (it = selection->begin(); it != selection->end(); ++it)
    {
        if (PartInstance* part = Instance::fastDynamicCast<PartInstance>(it->get()))
        {
            ...
            if (HandleHitTest::hitTestHandleLocal(part->getPartPrimitive()->getExtentsLocal(), part->getLocation(), HANDLE_RESIZE, gridRay,
                    hitPointGrid, localNormalId, part->getResizeHandleMask().normalIdMask))
            {
...
void MoveResizeJoinTool::render3dAdorn(Adorn* adorn)
{
    ...
    ServiceClient<Selection> selection(this->workspace);
    shared_ptr<PartInstance> hoveredPart =
        targetPV.expired() ? shared_ptr<PartInstance>() : shared_polymorphic_downcast<PartInstance>(targetPV.lock());
    ...
    for (it = selection->begin(); it != selection->end(); ++it)
    {
        if (PartInstance* part = Instance::fastDynamicCast<PartInstance>(it->get()))
        {
            ...
            Aya::DrawAdorn::handles3d(part->getPartSizeXml(), part->getCoordinateFrame(), adorn, HANDLE_RESIZE,
                workspace->getConstCamera()->coordinateFrame().translation, DrawAdorn::resizeColor(), true, part->getResizeHandleMask().normalIdMask,
                hoveredPart.get() == part ? localNormalId : NORM_UNDEFINED, Color3(0.3f, 0.7f, 1.0f));
        }
    }
}
...
void MoveResizeJoinTool::render2d(Adorn* adorn)
{
    ...
    for (it = selection->begin(); it != selection->end(); ++it)
    {
        if (PartInstance* part = Instance::fastDynamicCast<PartInstance>(it->get()))
        {
            ...
            DrawAdorn::handles2d(part->getPartSizeXml(), part->getCoordinateFrame(), *(workspace->getConstCamera()), adorn, HANDLE_RESIZE,
                DrawAdorn::resizeColor(), true, part->getResizeHandleMask().normalIdMask);

            if (FFlag::StudioUseDraggerGrid && showDraggerGrid && AdvArrowToolBase::advGridMode != DRAG::OFF && part == hoveredPart.get() &&
                dragging && part->getPartType() != BALL_PART && part->getPartType() != CYLINDER_PART)
            {
...
shared_ptr<MouseCommand> MoveResizeJoinTool::onMouseDown(const shared_ptr<InputObject>& inputObject)
{
    ...
    else
    {
        ...
        if (!Aya::AdvArrowTool::advCollisionCheckMode)
        {
...
void MoveResizeJoinTool::onMouseMove(const shared_ptr<InputObject>& inputObject)
{
    ...
    if (DFFlag::DraggerUsesNewPartOnDuplicate && scalingPart.lock() != targetPV.lock())
    {
        ...
        part->setDragging(true);
        origPartPosition =
            origPartPosition + Vector3(0.0f, part->getCoordinateFrame().translation.y - origPart->getCoordinateFrame().translation.y, 0.0f);
    }
    ...
    if (!targetPV.expired())
    {
        ...
        RbxRay gridRay = getUnitMouseRay(inputObject);
        Vector3 closePoint = Aya::Math::closestPointOnRay(axisRay, gridRay);
...
shared_ptr<MouseCommand> MoveResizeJoinTool::onMouseUp(const shared_ptr<InputObject>& inputObject)
{
    ...
    if (part)
    {
        if (!Aya::AdvArrowTool::advCollisionCheckMode && !DFFlag::RestoreTransparencyOnToolChange)
...
void MoveResizeJoinTool::capturedDrag(float axisDelta)
{
    if (targetPV.expired())
    {
        AYAASSERT(0); // should be caught higher up
    }
    else
    {
        ...
        resizeFloat(part, localNormalId, axisDelta, Aya::AdvArrowTool::advCollisionCheckMode);
    }
}
...
bool MoveResizeJoinTool::advResizeImpl(shared_ptr<PartInstance> part, NormalId localNormalId, float amount, bool checkIntersection)
{
    ...
    {
        AYAASSERT(part->getFormFactor() == PartInstance::SYMETRIC);
    ...
    if (!part->hasThreeDimensionalSize())
    {
        if (fabs(newSizeUi.x) < fabs(newSizeUi.z) && fabs(newSizeUi.x) < fabs(newSizeUi.y))
        {
            ...
            if (newSizeUi.x < minDimension)
                newSizeUi = Vector3(minDimension, minDimension, minDimension) *
                            Vector3(1.0f, originalSizeXml.y / originalSizeXml.x, originalSizeXml.z / originalSizeXml.x);
        }
        else if (fabs(newSizeUi.z) < fabs(newSizeUi.y))
        {
            ...
            if (newSizeUi.z < minDimension)
                newSizeUi = Vector3(minDimension, minDimension, minDimension) *
                            Vector3(originalSizeXml.x / originalSizeXml.z, originalSizeXml.y / originalSizeXml.z, 1.0f);
        }
        else
        {
            ...
            if (newSizeUi.y < minDimension)
                newSizeUi = Vector3(minDimension, minDimension, minDimension) *
                            Vector3(originalSizeXml.x / originalSizeXml.y, 1.0f, originalSizeXml.z / originalSizeXml.y);
        }
    }
    else
    {
        if (DFFlag::FormFactorDeprecated)
        {
            if (AdvArrowTool::advGridMode != DRAG::OFF)
            {
                ...
                /*
                        ...
                        If the new size is greater than the grid size, then don't do anything. Just set the new size. Otherwise, use a separate logic
                   to get the size: newSizeUi = newSizeUi < gridSize? (x) : newSizeUi;

                        If the fmod(originalSizeXml,gridSize) is greater than the minimum dimensions (should be 0.2) then set the part size to that
                   fmod - otherwise, set to the gridSize. This logic is there for the cases where fmod(originalSizeXml,gridSize) is very close to 0 -
                   as in, the originalSizeXml is an integer multiple of the gridSize. In this case we want the minimum size not to be 0, but the
                   gridSize.
                */
                newSizeUi.x =
                    newSizeUi.x < gridSize
                        ? (fmod(originalSizeXml.x, gridSize) >= part->getMinimumXOrZDimension() ? fmod(originalSizeXml.x, gridSize) : gridSize)
                        : newSizeUi.x;
                newSizeUi.y = newSizeUi.y < gridSize
                                  ? (fmod(originalSizeXml.y, gridSize) >= part->getMinimumYDimension() ? fmod(originalSizeXml.y, gridSize) : gridSize)
                                  : newSizeUi.y;
                newSizeUi.z =
                    newSizeUi.z < gridSize
                        ? (fmod(originalSizeXml.z, gridSize) >= part->getMinimumXOrZDimension() ? fmod(originalSizeXml.z, gridSize) : gridSize)
                        : newSizeUi.z;
            }
    ...
    if (checkIntersection)
    {
        ...
        {
            if (Dragger::intersectingWorldOrOthers(*part, *contactManager, Tolerance::maxOverlapAllowedForResize(), Dragger::maxDragDepth()))
            {
...
void MoveResizeJoinTool::setSelection(shared_ptr<Aya::Instance> oldSelection, shared_ptr<Aya::Instance> newSelection)
{
    if (shared_ptr<Aya::PVInstance> partInstance = Aya::Instance::fastSharedDynamicCast<Aya::PVInstance>(oldSelection))
    {
        if (partInstance == scalingPart.lock())
        {
            if (shared_ptr<Aya::PVInstance> pvInstance = Aya::Instance::fastSharedDynamicCast<Aya::PVInstance>((newSelection)))
            {
                scalingPart = pvInstance;
                if (DFFlag::RestoreTransparencyOnToolChange && !Aya::AdvArrowTool::advCollisionCheckMode)
                {
                    shared_ptr<PartInstance> selectedPart = Aya::Instance::fastSharedDynamicCast<Aya::PartInstance>(newSelection);
