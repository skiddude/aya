...
private:
    ...
    void tryStartDragging(const Aya::RbxRay& unitMouseRay);
    void startDragging();
    void doDrag(const Aya::RbxRay& unitMouseRay);
    bool getSnapHitPoint(PartInstance* part, const Aya::RbxRay& unitMouseRay, Vector3& hitPoint);
...
public:
    ...
    void mouseDownPublic(shared_ptr<Instance> _mousePart, Vector3 _pointOnMousePart, shared_ptr<const Instances> _dragParts);

    void mouseDown(shared_ptr<PartInstance> _mousePart, const Vector3& _pointOnMousePart, const std::vector<weak_ptr<PartInstance>> _dragParts);

    void mouseMove(Aya::RbxRay mouseRay); // inefficient, but easier to have just one version

    void mouseUp();

    const WeakParts& getParts()
    {
        return dragParts;
    }
    ...
    {
        return dragPhase == DRAGGING;
    }

    bool didDrag() const
    {
        return dragPhase == MOUSE_UP_DRAGGED;
    }
