...
private:
    ...
    SnapInfo bestProximatePart(const G3D::Array<Primitive*>& triedSnap, Contact::ProximityTest proximityTest);
    bool fallOffEdge();
...
public:
    AdvRunDragger();
    ~AdvRunDragger();
    void init(Workspace* _workspace, weak_ptr<PartInstance> _dragPart, const Vector3& _dragPointWorld);
    void initLocal(Workspace* _workspace, weak_ptr<PartInstance> _dragPart, const Vector3& _dragPointLocal, WeakParts _dragParts);
    ...
    void setGridMode(DRAG::DraggerGridMode mode)
    {
        gridMode = mode;
    }
    DRAG::DraggerGridMode getGridMode(void)
    {
        return gridMode;
    }
    void setJointCreateMode(bool mode)
    {
        jointCreateMode = mode;
    }
    bool getJointCreateMode(void)
    {
        return jointCreateMode;
    }
