...
bool Dragger::intersectingWorldOrOthers(
    PartInstance& partInstance, ContactManager& contactManager, const float tolerance, const float bottomPlaneHeight)
{
...
bool Dragger::intersectingGroundPlane(const G3D::Array<Primitive*>& primitives, float yHeight)
{
    for (int i = 0; i < primitives.size(); ++i)
    {
        Primitive* p = primitives[i];
        if ((p->getFastFuzzyExtents().min().y < yHeight) && (p->getConstGeometry()->collidesWithGroundPlane(p->getCoordinateFrame(), yHeight)))
        {
            return true;
        }
    }
...
bool Dragger::intersectingWorldOrOthers(
    const G3D::Array<Primitive*>& primitives, ContactManager& contactManager, const float tolerance, const float bottomPlaneHeight)
{
    AYAASSERT(primitives.size() > 0);

    return (intersectingGroundPlane(primitives, bottomPlaneHeight) || contactManager.intersectingOthers(primitives, tolerance));
}




bool Dragger::movePrimitivesGoal(const G3D::Array<Primitive*>& primitives, const Vector3& goal, Vector3& movedSoFar, bool snapToWorld)
{
...
void Dragger::movePrimitivesDelta(const G3D::Array<Primitive*>& primitives, const Vector3& delta, Vector3& movedSoFar)
{
    // AYAASSERT(delta == DragUtilities::toGrid(delta));
    AYAASSERT(delta != Vector3::zero());
...
void Dragger::movePrimitives(const G3D::Array<Primitive*>& primitives, const Vector3& delta, bool snapToWorld)
{
    AYAASSERT(primitives.size() > 0);
...
void Dragger::searchFine(const G3D::Array<Primitive*> primitives, Vector3& movedSoFar, ContactManager& contactManager, const float bottomPlaneHeight,
    Vector3 insidePosition, Vector3 outsidePosition)
{
    AYAASSERT(primitives.size() > 0);
...
const float maxMove()
{
    return (16000.0f / DFInt::DraggerMaxMovePercent) * 100;
}
/*
...
// Note - recursive
void Dragger::searchUpGross(
    const G3D::Array<Primitive*>& primitives, Vector3& movedSoFar, ContactManager& contactManager, const float bottomPlaneHeight)
{
    AYAASSERT(primitives.size() > 0);
...
// Recursive
void Dragger::searchDownGross(
    const G3D::Array<Primitive*>& primitives, Vector3& movedSoFar, ContactManager& contactManager, const float bottomPlaneHeight)
{
    AYAASSERT(primitives.size() > 0);
    AYAASSERT(!intersectingWorldOrOthers(primitives, contactManager, Tolerance::maxOverlapOrGap(), bottomPlaneHeight));
...
void Dragger::safePlaceAlongLine(const G3D::Array<Primitive*>& primitives, const Vector3& startMove, const Vector3& endMove, Vector3& movedSoFar,
    ContactManager& contactManager, bool snapToWorld)
{
    AYAASSERT(startMove != endMove);
    ...
    else
    {
        ...
        AYAASSERT(middleMove != startMove);
        ...
        if ((middleMove - endMove).length() < 0.001f || (FFlag::DraggerInfiniteRecursionFix && (middleMove - startMove).length() < 0.001f))
        { // done - split down to a gap of 1 division
            if (intersecting)
            {
...
Vector3 Dragger::safeMoveAlongLine(const G3D::Array<Primitive*>& primitives, const G3D::Vector3& tryMove, ContactManager& contactManager,
    const float customPlaneHeight, bool snapToWorld)
{
    if (primitives.size() == 0)
    {
        AYAASSERT(0); // just wanted to see where this happens
        return Vector3::zero();
    }
    ...
    {
        safePlaceAlongLine(primitives, Vector3::zero(), tryMove, movedSoFar, contactManager, snapToWorld);

        AYAASSERT((movedSoFar == Vector3::zero()) || !intersectingWorldOrOthers(primitives, contactManager, maxDragDepth()));
    }
...
Vector3 Dragger::safeMoveYDrop(
    const G3D::Array<Primitive*>& primitives, const G3D::Vector3& tryMove, ContactManager& contactManager, const float customPlaneHeight)
{
    if (primitives.size() == 0)
    {
        AYAASSERT(0); // just wanted to see where this happens
    ...
    if (primitives.size() > Aya::ClientAppSettings::singleton().GetValueMinPartsForOptDragging())
...
Vector3 Dragger::safeMoveNoDrop(const G3D::Array<Primitive*>& primitives, const Vector3& tryMove, ContactManager& contactManager)
{
    Vector3 movedSoFar;
...
void Dragger::safeRotate(const G3D::Array<Primitive*>& primitives, const Matrix3& rotate, ContactManager& contactManager)
{
    AYAASSERT(primitives.size());
...
void Dragger::safeRotate2(const G3D::Array<Primitive*>& primitives, const Matrix3& rotate, ContactManager& contactManager)
{
    safeRotate(primitives, rotate, contactManager);
}

Vector3 Dragger::safeMoveYDrop_EXT(
    const G3D::Array<Primitive*>& primitives, const Vector3& tryMove, ContactManager& contactManager, const float customPlaneHeight)
{
    ...
    // populate data
    Aya::Extents tmpExtent;
    std::vector<Aya::Extents> primExtents;
    ...
    {
        // extents data
        tmpExtent = Aya::Extents(Math::toGrid((*iter)->getFastFuzzyExtents().min(), Tolerance::mainGrid()),
    ...
    // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "TryMove: %f - Moved: %f - Total Time: %lf", tryMove.y, moved.y, (std::clock() -
...
void Dragger::searchUpGross_EXT(std::vector<Aya::Extents>& primExtents, const G3D::Array<Primitive*>& primitives,
    const boost::unordered_set<const Primitive*>& ignorePrimitives, ContactManager& contactManager, const float bottomPlaneHeight,
    Vector3& movedSoFar)
{
    if (fabs(movedSoFar.y) > maxMove())
    {
...
void Dragger::searchDownGross_EXT(std::vector<Aya::Extents>& primExtents, const G3D::Array<Primitive*>& primitives,
    const boost::unordered_set<const Primitive*>& ignorePrimitives, ContactManager& contactManager, const float bottomPlaneHeight,
    Vector3& movedSoFar)
{
    if (fabs(movedSoFar.y) > maxMove())
...
void Dragger::searchUpFine_EXT(std::vector<Aya::Extents>& primExtents, const G3D::Array<Primitive*>& primitives,
    const boost::unordered_set<const Primitive*>& ignorePrimitives, ContactManager& contactManager, const float bottomPlaneHeight,
    Vector3& movedSoFar)
{
    if (fabs(movedSoFar.y) > maxMove())
...
void Dragger::searchDownFine_EXT(std::vector<Aya::Extents>& primExtents, const G3D::Array<Primitive*>& primitives,
    const boost::unordered_set<const Primitive*>& ignorePrimitives, ContactManager& contactManager, const float bottomPlaneHeight,
    Vector3& movedSoFar)
{
    if (fabs(movedSoFar.y) > maxMove())
...
bool Dragger::intersectingWorldOrOthers_EXT(std::vector<Aya::Extents>& primExtents, const G3D::Array<Primitive*>& primitives,
    const boost::unordered_set<const Primitive*>& ignorePrimitives, ContactManager& contactManager, const float bottomPlaneHeight,
    const Vector3& movedSoFar)
{
    ...
    for (int a = 0; a < (int)primExtents.size(); ++a)
    {
        ...
        if (foundPrims.size())
        {
            ...
            {
                Aya::Extents tmpExtent(Math::toGrid(foundPrims[zz]->getFastFuzzyExtents().min(), Tolerance::mainGrid()),
...
bool Dragger::intersectingGroundPlane_EXT(
    const std::vector<Aya::Extents>& primExtents, const G3D::Array<Primitive*>& primitives, const float yHeight, const Vector3& movedSoFar)
{
    for (int i = 0; i < primitives.size(); ++i)
    {
...
bool Dragger::checkBallPolyIntersection(
    const Primitive* ballPrim, const CoordinateFrame& ballCFrame, const Primitive* polyPrim, const CoordinateFrame& polyCFrame)
{
...
bool Dragger::checkBallBallIntersection(
    const Primitive* ballPrim1, const CoordinateFrame& ballCFrame1, const Primitive* ballPrim2, const CoordinateFrame& ballCFrame2)
{
...
bool Dragger::checkPolyPolyIntersection(
    const Primitive* polyPrim1, const CoordinateFrame& polyCFrame1, const Primitive* polyPrim2, const CoordinateFrame& polyCFrame2)
{
...
void Dragger::moveExtents(std::vector<Aya::Extents>& primExtents, const Vector3& delta)
{
    std::vector<Aya::Extents>::iterator end_iter = primExtents.end();
    for (std::vector<Aya::Extents>::iterator iter = primExtents.begin(); iter != end_iter; ++iter)
        (*iter).shift(delta);
}

void Dragger::moveExtentsDelta(std::vector<Aya::Extents>& primExtents, const Vector3& delta, Vector3& movedSoFar)
{
