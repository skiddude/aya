...
shared_ptr<MouseCommand> AdvMoveToolBase::onMouseDown(const shared_ptr<InputObject>& inputObject)
{
    ...
    if (getOverHandle(inputObject, tempPt, normalId))
    {
        if (DFFlag::DraggerUsesNewPartOnDuplicate)
        {
            ...
            if (selection)
                selectionChangedConnection = selection->selectionChanged.connect(boost::bind(&Aya::AdvMoveToolBase::setToSelection, this));
        }
        ...
        megaDragger.reset(new MegaDragger(NULL, partArray, workspace));

        megaDragger->startDragging();
        // IMPORTANT: Transparency modification should be done only after startDragging as it will update world correctly else issues with
        // ChangeHistory if collision checks are turned off then make parts transparent after saving the original values
...
void AdvMoveToolBase::onMouseMove(const shared_ptr<InputObject>& inputObject)
{
    ...
    if (!dragging && ((inputObject->get2DPosition() - downPoint2d).length() > 4))
    {
    ...
    if (dragging && megaDragger->anyDragPartAlive())
    {
        ...
        else if (getDragType() == HANDLE_ROTATE)
        {
            ...
            Matrix3 rotation(right.x, up.x, at.x, right.y, up.y, at.y, right.z, up.z, at.z);
            ...
            if (snaped_angle > 0.001f)
            {
                ...
                snapped_rotate = megaDragger->rotateDragParts(snapped_rotate, Aya::AdvArrowTool::advCollisionCheckMode);
...
shared_ptr<MouseCommand> AdvMoveToolBase::onMouseUp(const shared_ptr<InputObject>& inputObject)
{
    ...
    if (DFFlag::RestoreTransparencyOnToolChange)
        AdvArrowToolBase::restoreSavedPartsTransparency();
    else if (!Aya::AdvArrowTool::advCollisionCheckMode)
...
void AdvMoveToolBase::render2d(Adorn* adorn)
{
    ...
    {
        Aya::DrawAdorn::handles2d(worldExtents.size(), location, *(workspace->getConstCamera()), adorn, HANDLE_MOVE, DrawAdorn::resizeColor());

        if (isLocal)
        {
            Aya::DrawAdorn::partInfoText2D(worldExtents.size(), location, *(workspace->getConstCamera()), adorn, "L", DrawAdorn::cornflowerblue);
        }
    }
}
...
void AdvMoveToolBase::render3dAdorn(Adorn* adorn)
{
    ...
    if (getExtentsAndLocation(worldExtents, location, isLocal))
    {
        Aya::DrawAdorn::handles3d(worldExtents.size(), location, adorn, HANDLE_MOVE, workspace->getConstCamera()->coordinateFrame().translation,
            DrawAdorn::resizeColor(), true, NORM_ALL_MASK, overHandleNormalId, Color3(0.3f, 0.7f, 1.0f));
    }
}

bool AdvMoveToolBase::getExtentsAndLocation(Extents& extents, CoordinateFrame& location, bool& isLocal) const
{
...
bool AdvMoveToolBase::getOverHandle(const shared_ptr<InputObject>& inputObject, Vector3& hitPointWorld, NormalId& normalId) const
{
    ...
    if (getExtentsAndLocation(extents, location, isLocal))
    {
        ...
        if (HandleHitTest::hitTestHandleLocal(extents, location, HANDLE_MOVE, this->getUnitMouseRay(inputObject), hitPointWorld, normalId))
        {
...
void AdvMoveTool::getGridXYUsingCamera(Aya::PartInstance* part, G3D::Vector3& gridXDir, G3D::Vector3& gridYDir)
{
