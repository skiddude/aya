...
void AxisToolBase::onMouseMove(const shared_ptr<InputObject>& inputObject)
{
    ...
    if (!dragging && (inputObject->get2DPosition() - downPoint2d).length() > 4)
    {
        ...
        megaDragger.reset(new MegaDragger(NULL, pvSelection->items(), workspace));
    ...
    if (dragging && megaDragger->anyDragPartAlive())
    {
        ...
        AYAASSERT(delta[(dragAxis + 1) % 3] == 0.0f);
        AYAASSERT(delta[(dragAxis + 2) % 3] == 0.0f);

        if (delta != Vector3::zero())
        {
            megaDragger->continueDragging(); // do this every time - multiplayer
            Vector3 move = (getDragType() == HANDLE_MOVE) ? megaDragger->safeMoveAlongLine(delta) : megaDragger->safeRotateAlongLine(delta);
...
void AxisToolBase::render2d(Adorn* adorn)
{
    ...
    if (getExtents(worldExtents))
    {
        ...
        Aya::DrawAdorn::handles2d(
            worldExtents.size(), CoordinateFrame(worldExtents.center()), *(workspace->getConstCamera()), adorn, getDragType(), getHandleColor());
    }
}
...
void AxisToolBase::render3dAdorn(Adorn* adorn)
{
    ...
    if (getExtents(worldExtents))
    {
        ...
        Aya::DrawAdorn::handles3d(worldExtents.size(), CoordinateFrame(worldExtents.center()), adorn, getDragType(),
            workspace->getConstCamera()->coordinateFrame().translation, Color3::orange());
    }
}
...
bool AxisToolBase::getOverHandle(const shared_ptr<InputObject>& inputObject, Vector3& hitPointWorld, NormalId& normalId) const
{
    ...
    if (getExtents(extents))
    {
        ...
        {
            result = HandleHitTest::hitTestMoveHandleWorld(extents, getUnitMouseRay(inputObject), hitPointWorld, normalId);
        }
        else
        {
            result = HandleHitTest::hitTestHandleWorld(extents, getDragType(), getUnitMouseRay(inputObject), hitPointWorld, normalId);
        }
    }
