...
void AdvRotateTool::render2d(Adorn* adorn)
{
    ...
    if (!dragging)
    {
        Aya::DrawAdorn::handles2d(extents.size(), coordFrame, *(workspace->getConstCamera()), adorn, HANDLE_ROTATE, Color3::green());

        if (isLocal)
        {
            Aya::DrawAdorn::partInfoText2D(extents.size(), coordFrame, *(workspace->getConstCamera()), adorn, "L", Aya::DrawAdorn::cornflowerblue);
        }
    }
    ...
    if (grid_normal != NORM_UNDEFINED)
    {
        ...
        DrawAdorn::circularGridAtCoord(adorn, coordFrame, extents.size(), workspace->getConstCamera()->coordinateFrame().translation,
            intToNormalId(grid_normal), Color4(DrawAdorn::axisColors[normalId % 3], 0.28f), boxesPerStud);
    }
}
...
void AdvRotateTool::render3dAdorn(Adorn* adorn)
{
    ...
    if (getExtentsAndLocation(extents, coordFrame, isLocal))
    {
        if (!isLocal)
        {
            ...
            DrawAdorn::star(adorn, extents.center(), 3.0f, Color3::yellow(), Color3::yellow(), Color3::yellow());
        ...
        DrawAdorn::handles3d(extents.size(), coordFrame, adorn, HANDLE_ROTATE, workspace->getConstCamera()->coordinateFrame().translation,
            getHandleColor(), true, getNormalMask(), mOverHandleNormalId);
    }
}
...
bool AdvRotateTool::getOverHandle(const shared_ptr<InputObject>& inputObject, Vector3& hitPointWorld, NormalId& normalId) const
{
    ...
    bool is_hit =
        HandleHitTest::hitTestHandleLocal(extents, coordFrame, HANDLE_ROTATE, getUnitMouseRay(inputObject), hitPointWorld, normalId, getNormalMask());
