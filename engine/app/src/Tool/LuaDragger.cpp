...
static Reflection::BoundFuncDesc<LuaDragger, void(shared_ptr<Instance>, Vector3, shared_ptr<const Instances>)> func_MouseDown(
    &LuaDragger::mouseDownPublic, "MouseDown", "mousePart", "pointOnMousePart", "parts", Security::None);
static Reflection::BoundFuncDesc<LuaDragger, void(Aya::RbxRay)> func_MouseMove(&LuaDragger::mouseMove, "MouseMove", "mouseRay", Security::None);
static Reflection::BoundFuncDesc<LuaDragger, void()> func_MouseUp(&LuaDragger::mouseUp, "MouseUp", Security::None);
static Reflection::BoundFuncDesc<LuaDragger, void(Aya::Vector3::Axis)> func_AxisRotate(
    &LuaDragger::axisRotate, "AxisRotate", "axis", Aya::Vector3::X_AXIS, Security::None);
REFLECTION_END();
...
void LuaDragger::mouseDownPublic(shared_ptr<Instance> _mousePart, Vector3 _pointOnMousePart, shared_ptr<const Instances> _dragParts)
{
...
void LuaDragger::mouseDown(
    shared_ptr<PartInstance> _mousePart, const Vector3& _pointOnMousePart, const std::vector<weak_ptr<PartInstance>> _dragParts)
{
    AYAASSERT(Workspace::getWorkspaceIfInWorkspace(_mousePart.get()));
    ...
    AYAASSERT(jointsIMade.size() == 0);
    AYAASSERT(runDragger.get() == NULL);
...
void LuaDragger::mouseMove(RbxRay mouseRay)
{
    ...
    {
        if (Aya::GameBasicSettings::singleton().inMousepanMode())
            Aya::GameBasicSettings::singleton().setCanMousePan(false);
        doDrag(unitRay);
    }
    else if (!Aya::GameBasicSettings::singleton().getCanMousePan())
        Aya::GameBasicSettings::singleton().setCanMousePan(true);
}
...
void LuaDragger::startDragging()
{
    if (shared_ptr<PartInstance> safeMousePart = mousePart.lock())
    {
        AYAASSERT(Workspace::getWorkspaceIfInWorkspace(safeMousePart.get()));

        DragUtilities::unJoinFromOutsiders(dragParts); // destroy joints outside the parts....

        DragUtilities::setDragging(dragParts);

        if (!safeMousePart->aligned())
        { // Align the dragging part to the grid - snap all other parts
        ...
        if (dragParts.size() == 1)
        {
            ...
            runDragger->initLocal(Workspace::findWorkspace(safeMousePart.get()), mousePart, pointOnMousePart);
        }
    }
}
...
bool LuaDragger::getSnapHitPoint(PartInstance* part, const RbxRay& unitMouseRay, Vector3& hitPoint)
{
    ...
    {
        hitPoint = DragUtilities::hitObjectOrPlane(dragParts, unitMouseRay, getContactManager(part));
