...
void RunDragger::snapInfoFromSnapPart()
{
    ...
    AYAASSERT(PartInstance::nonNullInWorkspace(dragPart.lock()));

    shared_ptr<PartInstance> p(snapPart.lock());
    if (PartInstance::nonNullInWorkspace(p))
    {
        AYAASSERT(Workspace::getWorldIfInWorkspace(p.get()) == workspace->getWorld());
...
void RunDragger::initLocal(Workspace* _workspace, weak_ptr<PartInstance> _dragPart, const Vector3& _dragPointLocal)
{
...
void RunDragger::init(Workspace* _workspace, weak_ptr<PartInstance> _dragPart, const Vector3& _dragPointWorld)
{
    AYAASSERT(!_dragPart.expired());
    AYAASSERT(PartInstance::nonNullInWorkspace(_dragPart.lock()));
    workspace = _workspace;
...
RunDragger::SnapInfo RunDragger::rayHitsPart(const G3D::Array<Primitive*>& triedSnap, bool forceAdjacent)
{
    ...
    PartInstance* foundPart = MouseCommand::getMousePart(mouseRay, *workspace->getWorld()->getContactManager(), drag, &filter, answer.hitWorld);
    if (foundPart)
    {
...
RunDragger::SnapInfo RunDragger::bestProximatePart(const G3D::Array<Primitive*>& triedSnap, Contact::ProximityTest proximityTest)
{
    SnapInfo answer;
...
void RunDragger::findNoSnapPosition(const CoordinateFrame& original)
{
    ...
    PartInstance* foundPart = MouseCommand::getMousePart(mouseRay, *workspace->getWorld()->getContactManager(), drag, &filter, hitWorld);
    if (!foundPart)
    {
