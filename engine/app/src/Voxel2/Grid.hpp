...
public:
    ...
    {
        AYAASSERT_VERY_FAST(material <= Material_Max && occupancy <= Occupancy_Max);
    ...
    unsigned char getMaterial() const
    {
        return material;
    }
    unsigned char getOccupancy() const
    {
        return occupancy;
    }
    ...
    {
        return material == other.material && occupancy == other.occupancy;
    }
    bool operator!=(const Cell& other) const
    {
        return !(*this == other);
...
public:
    Region(const Vector3int32& begin, const Vector3int32& end)
        ...
        , end_(end)
    {
        AYAASSERT_VERY_FAST(begin.x <= end.x && begin.y <= end.y && begin.z <= end.z);
    }
    ...
    const Vector3int32& begin() const
    {
        return begin_;
    }
    ...
    bool empty() const
    {
        return begin_.x == end_.x || begin_.y == end_.y || begin_.z == end_.z;
    }
...
public:
    ...
    {
        AYAASSERT_VERY_FAST(static_cast<unsigned>(x) < static_cast<unsigned>(sizeX) && static_cast<unsigned>(y) < static_cast<unsigned>(sizeY) &&
                            static_cast<unsigned>(z) < static_cast<unsigned>(sizeZ));
    ...
    Vector3int32 getSize() const
    {
        return Vector3int32(sizeX, sizeY, sizeZ);
    }
