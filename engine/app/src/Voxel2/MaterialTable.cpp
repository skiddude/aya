...
void MaterialTable::load(const std::string& file)
{
    ...
    if (!in)
        throw Aya::runtime_error("Error opening file %s", file.c_str());
    ...
    if (root.HasParseError())
        throw Aya::runtime_error("Failed to parse JSON: %s at %d", root.GetParseError(), int(root.GetErrorOffset()));
    ...
    {
        throw Aya::runtime_error("Failed to find atlas definition for platform %s", platform.c_str());
    }
