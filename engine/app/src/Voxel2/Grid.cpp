...
/**
 * Checks if the given row of cells contains any solid cells.
 *
 * @param row The row of cells to check.
 * @param size The number of cells in the row.
 *
 * @return True if any of the cells in the row are solid, false if all cells are empty.
 */
...
{
    unsigned char occupancy = (c000.getOccupancy() + c001.getOccupancy() + c010.getOccupancy() + c011.getOccupancy() + c100.getOccupancy() +
                                  c101.getOccupancy() + c110.getOccupancy() + c111.getOccupancy() + 7) /
                              8;
    ...
    for (int y = 0; y < size.y; ++y)
        for (int z = 0; z < size.z; ++z)
        {
            ...
            else
            {
                ...
                for (int x = 0; x < size.x; ++x)
                {
                    targetRow[x] = downsampleCell(sourceRow00[x * 2 + 0], sourceRow00[x * 2 + 1], sourceRow10[x * 2 + 0], sourceRow10[x * 2 + 1],
                        sourceRow01[x * 2 + 0], sourceRow01[x * 2 + 1], sourceRow11[x * 2 + 0], sourceRow11[x * 2 + 1]);
                }
            }
        }
}
...
static unsigned char readUInt8(const std::string& data, unsigned int& readOffset)
{
    if (readOffset >= data.size())
        throw Aya::runtime_error("Error while decoding data: unexpected end at offset %u", readOffset);
...
static void encodeCellRun(std::string& result, const Cell& cell, unsigned int count)
{
    ...
    // after material we have optional occupancy and optional count; count is 2+ but we'll store count-1 to have a max of 256 and reserve "0" for smth
...
static void decodeChunk(const std::string& data, unsigned int& readOffset, Box& result, std::vector<Cell>& cells)
{
    ...
    while (offset < cells.size())
    {
        ...
        if (offset + run.second > cells.size())
            throw Aya::runtime_error("Error while decoding data: chunk overflow at %u cells", offset + run.second);
...
struct DeallocateCells
{
    ...
    {
        AYAPROFILER_COUNTER_SUB("memory/terrain/voxel", size);
...
{
    AYAASSERT(size != 0 && (size & (size - 1)) == 0);
...
Region Region::expandToGrid(unsigned int size) const
{
    ...
    return Region(Vector3int32(begin_.x & ~mask, begin_.y & ~mask, begin_.z & ~mask),
...
void Box::allocate()
{
    ...
    AYAPROFILER_COUNTER_ADD("memory/terrain/voxel", size);
...
{
    AYAASSERT(std::find(listeners.begin(), listeners.end(), listener) == listeners.end());
...
void Grid::disconnectListener(GridListener* listener)
{
    ...
    AYAASSERT(it != listeners.end());
    listeners.erase(it);
}

Box Grid::read(const Region& region, int lod) const
{
    AYAPROFILER_SCOPE("Voxel", "read");

    AYAASSERT(region.aligned(1 << lod));
...
{
    AYAPROFILER_SCOPE("Voxel", "write");

    AYAASSERT(region.size() == box.getSize());
...
void Grid::deserialize(const std::string& data)
{
    ...
    if (version != 1)
        throw Aya::runtime_error("Error while decoding data: unsupported version");

    int chunkSizeLog2 = readUInt8(data, readOffset);
    int chunkSize = 1 << chunkSizeLog2;

    if (chunkSizeLog2 > 8)
        throw Aya::runtime_error("Error while decoding data: malformed chunk size");
