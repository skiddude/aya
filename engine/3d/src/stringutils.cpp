...
std::string wordWrap(const std::string& input, int numCols)
{
    ...
    while (c < inLen)
    {
        ...
        while ((len > minLength) && (input[c + len] != ' '))
        {
            len--;
        }
...
Array<std::string> stringSplit(const std::string& x, char splitChar)
{
...
std::string stringJoin(const Array<std::string>& a, char joinChar)
{
...
std::string stringJoin(const Array<std::string>& a, const std::string& joinStr)
{
