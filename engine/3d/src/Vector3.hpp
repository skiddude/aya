...
public:
    ...
    {
        return Vector3(G3D::clamp(x, low.x, high.x), G3D::clamp(y, low.y, high.y), G3D::clamp(z, low.z, high.z));
    }

    inline Vector3 clamp(float low, float high) const
    {
        return Vector3(G3D::clamp(x, low, high), G3D::clamp(y, low, high), G3D::clamp(z, low, high));
    }
    ...
    */
    ...
    static void generateStableOrthonormalBasis(
        Vector3& rkU /*tangent*/, Vector3& rkV /*binormal*/, Vector3& rkW /*normal*/, bool bUnitLengthW = true);
    ...
    {
        static const Vector3 v((float)G3D::finf(), (float)G3D::finf(), (float)G3D::finf());
        return v;
    }
    static const Vector3& nan()
    {
        static const Vector3 v((float)G3D::fnan(), (float)G3D::fnan(), (float)G3D::fnan());
        return v;
    }
    /** Smallest (most negative) representable vector */
    static const Vector3& minFinite()
    {
        static const Vector3 v(-FLT_MAX, -FLT_MAX, -FLT_MAX);
        return v;
    }
    ...
    {
        debugAssertM(G3D::fuzzyEq(length(), 1.0f), "makeAxes requires Z to have unit length");
...
//----------------------------------------------------------------------------
inline Vector3::Vector3(const double V[3])
    : x((float)V[0])
    , y((float)V[1])
    , z((float)V[2])
{
...
{
    Vector3 kCross(y * rkVector.z - z * rkVector.y, z * rkVector.x - x * rkVector.z, x * rkVector.y - y * rkVector.x);
    kCross.unitize();
    return kCross;
}
