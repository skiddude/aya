...
bool Matrix3::isOrthonormal() const
{
    ...
    return (G3D::fuzzyEq(X.dot(Y), 0.0f) && G3D::fuzzyEq(Y.dot(Z), 0.0f) && G3D::fuzzyEq(X.dot(Z), 0.0f) &&
            G3D::fuzzyEq(X.squaredMagnitude(), 1.0f) && G3D::fuzzyEq(Y.squaredMagnitude(), 1.0f) && G3D::fuzzyEq(Z.squaredMagnitude(), 1.0f));
}
...
Matrix3::Matrix3(const Quat& _q)
{
    ...
    set(1.0f - yy - zz, xy - zw, xz + yw, xy + zw, 1.0f - xx - zz, yz - xw, xz - yw, yz + xw, 1.0f - xx - yy);
}
...
//----------------------------------------------------------------------------
Matrix3::Matrix3(
    float fEntry00, float fEntry01, float fEntry02, float fEntry10, float fEntry11, float fEntry12, float fEntry20, float fEntry21, float fEntry22)
{
    set(fEntry00, fEntry01, fEntry02, fEntry10, fEntry11, fEntry12, fEntry20, fEntry21, fEntry22);
}
...
Matrix3 Matrix3::operator*(const Matrix3& rkMatrix) const
{
    ...
    for (int iRow = 0; iRow < 3; iRow++)
    {
        ...
        {
            kProd.elt[iRow][iCol] =
                elt[iRow][0] * rkMatrix.elt[0][iCol] + elt[iRow][1] * rkMatrix.elt[1][iCol] + elt[iRow][2] * rkMatrix.elt[2][iCol];
        }
    }
...
bool Matrix3::inverse(Matrix3& rkInverse, float fTolerance) const
{
    ...
    float fDet = elt[0][0] * rkInverse[0][0] + elt[0][1] * rkInverse[1][0] + elt[0][2] * rkInverse[2][0];
...
float Matrix3::determinant() const
{
    ...
    float fDet = elt[0][0] * fCofactor00 + elt[0][1] * fCofactor10 + elt[0][2] * fCofactor20;
...
void Matrix3::golubKahanStep(Matrix3& kA, Matrix3& kL, Matrix3& kR)
{
    ...
    // adjust right
    float fY = kA[0][0] - (G3D::abs(fRoot1 - fT22) <= G3D::abs(fRoot2 - fT22) ? fRoot1 : fRoot2);
    float fZ = kA[0][1];
...
void Matrix3::singularValueDecomposition(Matrix3& kL, Vector3& kS, Matrix3& kR) const
{
    ...
    for (int i = 0; i < ms_iSvdMaxIterations; i++)
    {
        ...
        bool bTest1 = (G3D::abs(kA[0][1]) <= ms_fSvdEpsilon * (G3D::abs(kA[0][0]) + G3D::abs(kA[1][1])));
        bool bTest2 = (G3D::abs(kA[1][2]) <= ms_fSvdEpsilon * (G3D::abs(kA[1][1]) + G3D::abs(kA[2][2])));
...
void Matrix3::orthonormalize()
{
    ...
    // compute q0
    float fInvLength = 1.0 / sqrt(elt[0][0] * elt[0][0] + elt[1][0] * elt[1][0] + elt[2][0] * elt[2][0]);
    ...
    // compute q1
    float fDot0 = elt[0][0] * elt[0][1] + elt[1][0] * elt[1][1] + elt[2][0] * elt[2][1];

    elt[0][1] -= fDot0 * elt[0][0];
    elt[1][1] -= fDot0 * elt[1][0];
    elt[2][1] -= fDot0 * elt[2][0];

    fInvLength = 1.0 / sqrt(elt[0][1] * elt[0][1] + elt[1][1] * elt[1][1] + elt[2][1] * elt[2][1]);
    ...
    // compute q2
    float fDot1 = elt[0][1] * elt[0][2] + elt[1][1] * elt[1][2] + elt[2][1] * elt[2][2];

    fDot0 = elt[0][0] * elt[0][2] + elt[1][0] * elt[1][2] + elt[2][0] * elt[2][2];

    elt[0][2] -= fDot0 * elt[0][0] + fDot1 * elt[0][1];
    elt[1][2] -= fDot0 * elt[1][0] + fDot1 * elt[1][1];
    elt[2][2] -= fDot0 * elt[2][0] + fDot1 * elt[2][1];

    fInvLength = 1.0 / sqrt(elt[0][2] * elt[0][2] + elt[1][2] * elt[1][2] + elt[2][2] * elt[2][2]);
...
void Matrix3::qDUDecomposition(Matrix3& kQ, Vector3& kD, Vector3& kU) const
{
    ...
    float fDot = kQ[0][0] * elt[0][1] + kQ[1][0] * elt[1][1] + kQ[2][0] * elt[2][1];
    ...
    float fDet = kQ[0][0] * kQ[1][1] * kQ[2][2] + kQ[0][1] * kQ[1][2] * kQ[2][0] + kQ[0][2] * kQ[1][0] * kQ[2][1] - kQ[0][2] * kQ[1][1] * kQ[2][0] -
    ...
    kR[0][0] = kQ[0][0] * elt[0][0] + kQ[1][0] * elt[1][0] + kQ[2][0] * elt[2][0];

    kR[0][1] = kQ[0][0] * elt[0][1] + kQ[1][0] * elt[1][1] + kQ[2][0] * elt[2][1];

    kR[1][1] = kQ[0][1] * elt[0][1] + kQ[1][1] * elt[1][1] + kQ[2][1] * elt[2][1];

    kR[0][2] = kQ[0][0] * elt[0][2] + kQ[1][0] * elt[1][2] + kQ[2][0] * elt[2][2];

    kR[1][2] = kQ[0][1] * elt[0][2] + kQ[1][1] * elt[1][2] + kQ[2][1] * elt[2][2];

    kR[2][2] = kQ[0][2] * elt[0][2] + kQ[1][2] * elt[1][2] + kQ[2][2] * elt[2][2];
    ...
    */
    ...
    while (resid > eps && iter < MAX_ITERS)
    {
        ...
        else
        {
            ...
            float gamma = sqrt(sqrt((Xit.l1Norm() * Xit.lInfNorm()) / (X.l1Norm() * X.lInfNorm())));
...
float Matrix3::diffOneNorm(const Matrix3& y) const
{
    ...
    for (int c = 0; c < 3; ++c)
    {
        ...
        float f = fabs(elt[0][c] - y[0][c]) + fabs(elt[1][c] - y[1][c]) + fabs(elt[2][c] - y[2][c]);
...
void Matrix3::toAxisAngle(Vector3& rkAxis, float& rfRadians) const
{
    ...
    if (rfRadians > epsilon)
    {
        if (rfRadians < pi() - epsilon)
        {
        ...
        else
        {
            ...
            if (elt[0][0] >= elt[1][1])
            {
                ...
                if (elt[0][0] >= elt[2][2])
                {
                    ...
                    rkAxis.x = 0.5 * sqrt(elt[0][0] - elt[1][1] - elt[2][2] + 1.0);
                ...
                else
                {
                    ...
                    rkAxis.z = 0.5 * sqrt(elt[2][2] - elt[0][0] - elt[1][1] + 1.0);
            ...
            else
            {
                ...
                if (elt[1][1] >= elt[2][2])
                {
                    ...
                    rkAxis.y = 0.5 * sqrt(elt[1][1] - elt[0][0] - elt[2][2] + 1.0);
...
bool Matrix3::qLAlgorithm(float afDiag[3], float afSubDiag[3])
{
    ...
    for (int i0 = 0; i0 < 3; i0++)
    {
        ...
        for (iIter = 0; iIter < iMaxIter; iIter++)
        {
            ...
            for (i1 = i0; i1 <= 1; i1++)
            {
                float fSum = G3D::abs(afDiag[i1]) + G3D::abs(afDiag[i1 + 1]);
...
void Matrix3::_mul(const Matrix3& A, const Matrix3& B, Matrix3& out)
{
    ...
    outRowPtr[0] = ARowPtr[0] * B.elt[0][0] + ARowPtr[1] * B.elt[1][0] + ARowPtr[2] * B.elt[2][0];
    outRowPtr[1] = ARowPtr[0] * B.elt[0][1] + ARowPtr[1] * B.elt[1][1] + ARowPtr[2] * B.elt[2][1];
    outRowPtr[2] = ARowPtr[0] * B.elt[0][2] + ARowPtr[1] * B.elt[1][2] + ARowPtr[2] * B.elt[2][2];
...
{
    return G3D::format(
        "[%g, %g, %g; %g, %g, %g; %g, %g, %g]", elt[0][0], elt[0][1], elt[0][2], elt[1][0], elt[1][1], elt[1][2], elt[2][0], elt[2][1], elt[2][2]);
}
