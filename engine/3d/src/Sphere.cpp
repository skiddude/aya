...
{
    return format("Sphere(<%g, %g, %g>, %g)", center.x, center.y, center.z, radius);
}
...
bool Sphere::culledBy(const Array<Plane>& plane, int& cullingPlaneIndex, const uint32 inMask, uint32& outMask) const
{
...
bool Sphere::culledBy(const Array<Plane>& plane, int& cullingPlaneIndex, const uint32 inMask) const
{
...
bool Sphere::culledBy(const class Plane* plane, int numPlanes, int& cullingPlane, const uint32 _inMask, uint32& childMask) const
{
...
bool Sphere::culledBy(const class Plane* plane, int numPlanes, int& cullingPlane, const uint32 _inMask) const
{
...
Vector3 Sphere::randomInteriorPoint() const
{
    ...
    do
    {
        result = Vector3(uniformRandom(-1, 1), uniformRandom(-1, 1), uniformRandom(-1, 1));
