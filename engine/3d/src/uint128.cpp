...
#if (!AYA_PLATFORM_IOS) || (AYA_PLATFORM_IOS && TARGET_IPHONE_SIMULATOR)


/** Adds two 64-bit integers, placing the result and the overflow into 64-bit integers.*/
// d9mz - error: non-constant-expression cannot be narrowed from type 'uint64' (aka 'unsigned long long') to 'uint32' (aka 'unsigned int') in
// initializer list [-Wc++11-narrowing]
static void addAndCarry(const uint64& _a, const uint64& _b, uint64& carry, uint64& result)
{

    // Use static_cast to explicitly cast the 64-bit values to 32-bit values
    ...
    uint64 tmp = static_cast<uint64>(a[0]) + b[0];

    result = tmp & 0xFFFFFFFF;
    uint32 c = static_cast<uint32>(tmp >> 32);

    tmp = static_cast<uint64>(c) + a[1] + b[1];
    result += tmp << 32;
    carry = static_cast<uint32>(tmp >> 32);
}
...
{
    // Explicitly cast the 64-bit values to 32-bit values
    // d9mz - non-constant-expression cannot be narrowed from type 'uint64' (aka 'unsigned long long') to 'uint32' (aka 'unsigned int') in initializer
...
uint128::uint128(const uint64& hi, const uint64& lo)
    : hi(hi)
    , lo(lo)
{
}

uint128::uint128(const uint64& lo)
    : hi(0)
    , lo(lo)
{
}
