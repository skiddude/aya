...
public:
    ...
    {
        return minVal.x <= x && x <= maxVal.x && minVal.y <= y && y <= maxVal.y && minVal.z <= z && z <= maxVal.z;
    }
    ...
    std::ostream&
    operator<<(std::ostream& os, const G3D::Vector3int16& v);

} // namespace G3D

template<>
struct HashTrait<G3D::Vector3int16>
{
