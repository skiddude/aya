...
public:
    ...
    */
    ...
    /**
     ...
     @cite Linux version written by Nicolai Haehnle <prefect_@gmx.net>,
...
{
    #if defined(_M_ARM64)
    return static_cast<uint64_t>(3.4e9); // 3.4 ghz (qualcomm snapdradon x elite)
    #else
    return __rdtsc();
    #endif
}
...
inline uint64 System::getCycleCount()
{
#if defined(G3D_LINUX) && !defined(__aarch64__)
    uint32 timehi, timelo;

    __asm__ __volatile__("rdtsc            " : "=a"(timelo), "=d"(timehi) :);

    return ((uint64)timehi << 32) + (uint64)timelo;
#elif defined(G3D_ANDROID) || defined(__aarch64__)
    return 0;
#endif
}
...
inline void System::endCycleCount(uint64& cycleCount)
{
    ...
    cycleCount = (uint64)((double)(instance().m_OSXCPUSpeed) * (double)UnsignedWideToUInt64(diffNS) * instance().m_secondsPerNS);
#endif
}
