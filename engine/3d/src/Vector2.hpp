...
public:
    ...
    {
        return Vector2(G3D::clamp(x, low.x, high.x), G3D::clamp(y, low.y, high.y));
    }

    inline Vector2 clamp(float low, float high) const
    {
        return Vector2((float)G3D::clamp(x, low, high), (float)G3D::clamp(y, low, high));
    }
...
inline Vector2::Vector2(const Vector2int16& v)
    : x(v.x)
    , y(v.y)
{
}
