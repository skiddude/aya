...
void GImage::LtoRGBA(const uint8* in, uint8* out, int numPixels)
{
...
void GImage::LtoRGB(const uint8* in, uint8* out, int numPixels)
{
...
void GImage::RGBtoRGBA(const uint8* in, uint8* out, int numPixels)
{
...
void GImage::RGBAtoRGB(const uint8* in, uint8* out, int numPixels)
{
...
void GImage::RGBtoBGRA(const uint8* in, uint8* out, int numPixels)
{
...
void GImage::RGBtoBGR(const uint8* in, uint8* out, int numPixels)
{
...
void GImage::RGBxRGBtoRGBA(const uint8* colorRGB, const uint8* alphaRGB, uint8* out, int numPixels)
{
...
void GImage::RGBtoARGB(const uint8* in, uint8* out, int numPixels)
{
...
void GImage::flipRGBVertical(const uint8* in, uint8* out, int width, int height)
{
...
void GImage::flipRGBAVertical(const uint8* in, uint8* out, int width, int height)
{
...
void GImage::decode(BinaryInput& input, Format format)
{
...
void GImage::decodePCX(BinaryInput& input)
{
...
GImage::Format GImage::resolveFormat(const std::string& filename, const uint8* data, int dataLen, Format maybeFormat)
{
    ...
    if (dataLen > 10)
    {
        if ((dataLen > 11) && (data[0] == 0xFF) && (memcmp(&data[6], "JFIF", 4) == 0))
        {
            return JPEG;
        }
    }
...
GImage::GImage(const uint8* data, int length, Format format, const MemoryManager::Ref& m)
    : m_memMan(m)
    , m_byte(NULL)
    , m_channels(0)
    , m_width(0)
...
GImage::GImage(int width, int height, int channels, const MemoryManager::Ref& mem)
    : m_memMan(mem)
    , m_byte(0)
    , m_channels(0)
    , m_width(0)
...
void GImage::resize(int width, int height, int channels, bool zero)
{
...
void GImage::_copy(const GImage& other)
{
...
GImage::GImage(const GImage& other, const MemoryManager::Ref& m)
    : m_memMan(m)
    , m_byte(NULL)
{
...
bool GImage::copySubImage(GImage& dest, const GImage& src, int srcX, int srcY, int srcWidth, int srcHeight)
{
    if ((src.m_width < srcX + srcWidth) || (src.m_height < srcY + srcHeight) || (srcY < 0) || (srcX < 0))
    {
...
bool GImage::pasteSubImage(GImage& dest, const GImage& src, int destX, int destY, int srcX, int srcY, int srcWidth, int srcHeight)
{

    if ((src.m_width < srcX + srcWidth) || (src.m_height < srcY + srcHeight) || (dest.m_width < destX + srcWidth) ||
        (dest.m_height < destY + srcHeight) || (srcY < 0) || (srcX < 0) || (destY < 0) || (destX < 0) || (src.channels() != dest.channels()))
    {
    ...
    {
        const uint8* srcRow = src.byte() + ((i + srcY) * src.m_width + srcX) * src.channels();
        uint8* destRow = dest.byte() + ((i + destY) * dest.width() + destX) * dest.channels();
...
bool GImage::supportedFormat(const std::string& format)
{
...
GImage::Format GImage::stringToFormat(const std::string& format)
{
...
void GImage::encode(Format format, uint8*& outData, int& outLength) const
{
...
void GImage::encode(Format format, BinaryOutput& out) const
{
...
void GImage::computeNormalMap(const GImage& bump, GImage& normal, const BumpMapPreprocess& preprocess)
{
    computeNormalMap(bump.m_width, bump.m_height, bump.m_channels, bump.byte(), normal, preprocess);
}

void GImage::computeNormalMap(int width, int height, int channels, const uint8* src, GImage& normal, const BumpMapPreprocess& preprocess)
{
...
// and divide by 255
#define ELEVATION(DX, DY) ((int)B[(((DX + x + w) % w) + ((DY + y + h) % h) * w) * stride])
            ...
            delta.y = -(ELEVATION(-1, -1) * 1 + ELEVATION(0, -1) * 2 + ELEVATION(1, -1) * 1 + -ELEVATION(-1, 1) * 1 + -ELEVATION(0, 1) * 2 +
                        -ELEVATION(1, 1) * 1);

            delta.x = -(-ELEVATION(-1, -1) * 1 + ELEVATION(1, -1) * 1 + -ELEVATION(-1, 0) * 2 + ELEVATION(1, 0) * 2 + -ELEVATION(-1, 1) * 1 +
                        ELEVATION(1, 1) * 1);
            ...
            if (lowPassBump)
            {
                H = (ELEVATION(-1, -1) + ELEVATION(0, -1) + ELEVATION(1, -1) + ELEVATION(-1, 0) + ELEVATION(0, 0) + ELEVATION(1, 0) +
                        ELEVATION(-1, 1) + ELEVATION(0, 1) + ELEVATION(1, 1)) /
                    (255.0f * 9.0f);
            }
#undef ELEVATION
...
GImage GImage::bilinearStretchBlt(int newwidth, int newheight) const
{
...
GImage GImage::stretchBlt(int newwidth, int newheight) const
{
...
bool GImage::rotateAndPasteSubImage(GImage& dest, GImage& src, int destX, int destY, int srcX, int srcY, int srcWidth, int srcHeight, int rotation)
{

    // TODO: add sanity checks in case user is an idiot.
    if (src.channels() != 4 || dest.channels() != 4)
        return false;
    ...
    for (int i = 0; i < s; i++)
    {
        for (int d = 1; d < s - 1 - i; d++)
        {
            G3D::Color4uint8 color =
                GImage::interpolateColor(getPixel(temp, 0, i), getPixel(temp, s - 1 - i, s - 1), 1.0f - ((float)d / (float)(s - i - 1)));
