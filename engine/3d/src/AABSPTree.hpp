...
private:
    ...
    */
    ...
    public:
        ...
        PivotLT(Vector3::Axis a, float l)
            : sortAxis(a)
            , sortLocation(l)
        {
        }
        ...
        */
        ...
        void getIntersectingMembers(const AABox& box, const Sphere& sphere, Array<T>& members, bool useSphere) const
        {
            ...
            for (int v = 0; v < valueArray.size(); ++v)
            {
                ...
                if (bounds.intersects(box) && (!useSphere || bounds.intersects(sphere)))
                {
     ...
     */
    Node* makeNode(Array<Handle>& point, int beginIndex, int endIndex, int valuesPerNode, int numMeanSplits)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                std::sort(point.getCArray() + beginIndex, point.getCArray() + endIndex + 1, CenterLT(splitAxis));

                // Intentional integer divide
                int midIndex = (beginIndex + endIndex) / 2;

                // Choose the split location between the two middle elements
                const Vector3 median = (point[midIndex].bounds.high() + point[iMin(midIndex + 1, point.size() - 1)].bounds.low()) * 0.5;
            ...
            // test for overlap
            std::sort(point.getCArray() + beginIndex, point.getCArray() + endIndex + 1, PivotLT(splitAxis, splitLocation));
            ...
            for (overlapBeginIndex = beginIndex;
                 (overlapBeginIndex <= endIndex) && (point[overlapBeginIndex].bounds.high()[splitAxis] < splitLocation); ++overlapBeginIndex)
            {
            }

            for (overlapEndIndex = endIndex; (overlapEndIndex >= beginIndex) && (point[overlapEndIndex].bounds.low()[splitAxis] > splitLocation);
                 --overlapEndIndex)
            {
            }
            ...
            {
                node->child[0] = makeNode(point, beginIndex, overlapBeginIndex - 1, valuesPerNode, numMeanSplits - 1);
            }

            if (overlapEndIndex < endIndex)
            {
                node->child[1] = makeNode(point, overlapEndIndex + 1, endIndex, valuesPerNode, numMeanSplits - 1);
            }
        }
...
public:
    ...
    */
    ...
    {
        debugAssertM(contains(value), "Tried to remove an element from a "
...
private:
    ...
    static void getIntersectingMembers(const Array<Plane>& plane, Array<T>& members, Node* node, uint32 parentMask)
    {
        ...
        else
        {
            ...
            for (int c = 0; c < 2; ++c)
            {
                if (node->child[c] && !node->child[c]->splitBounds.culledBy(plane, dummy, parentMask, childMask))
                {
...
public:
    ...
    public:
        ...
        bool operator==(const BoxIntersectionIterator& other) const
        {
            ...
            else
            {
                ...
                if ((box != other.box) || (node != other.node) || (nextValueArrayIndex != other.nextValueArrayIndex) ||
        ...
        BoxIntersectionIterator& operator++()
        {
            ...
            while (!isEnd && !foundIntersection)
            {
                ...
                while ((!isEnd) && (nextValueArrayIndex >= node->valueArray.length()))
                {
                    ...
                    // processing.
                    if ((node->child[1] != NULL) && (box.high()[node->splitAxis] > node->splitLocation))
                    {
                    ...
                    // processing.
                    if ((node->child[0] != NULL) && (box.low()[node->splitAxis] < node->splitLocation))
                    {
    ...
    private:
        ...
        struct StackFrame
        {
            ...
            void init(const Node* inNode, const Ray& ray, float inMinTime, float inMaxTime)
            {
                ...
                double splitTime = (splitLocation - ray.origin[splitAxis]) / ray.direction[splitAxis];
                ...
                double rayLocation = ray.origin[splitAxis] + ray.direction[splitAxis] * minTime;

                if (rayLocation == splitLocation)
                {
                    // We're right on the split. Look ahead.
                    rayLocation = ray.origin[splitAxis] + ray.direction[splitAxis] * maxTime;
                }
        ...
        RayIntersectionIterator& operator++()
        {
            ...
            while (true)
            {
                ...
                if (s->valIndex >= s->node->valueArray.length())
                {
                    ...
                    Node* child = (s->nextChild >= 0) ? s->node->child[s->nextChild] : NULL;
                    ...
                    {
                        ++stackIndex;
                        stack[stackIndex].init(child, ray, childStartTime, childEndTime);
                    }
                ...
                else
                {
                    ...
                    if (s->startTime == s->minTime)
                    {
                        ...
                        if (CollisionDetection::collisionLocationForMovingPointFixedAABox(
                                ray.origin, ray.direction, s->node->valueArray[s->valIndex].bounds, location))
                        {
    ...
    */
    ...
    private:
        ...
        Iterator(const typename Table<T, Node*>::Iterator& it)
            : it(it)
        {
        }
