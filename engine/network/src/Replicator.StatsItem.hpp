...
public:
    StatsItem(Replicator* replicator, int version)
        : version(version)
        , Item(*replicator)
    {
    }
...
class Replicator::Stats : public Aya::Stats::Item
{
    ...
    void addStat(const ReplicatorStats& stats, const char* name, const ReplicatorStats::PacketType type)
    {
...
public:
        ...
        , instanceBits(0)
    {
        ...
        addStat(
            stats, ReplicatorStats::kPacketTypeNames[ReplicatorStats::PACKET_TYPE_CategoryBehavior], ReplicatorStats::PACKET_TYPE_CategoryBehavior);
        ...
        addStat(stats, ReplicatorStats::kPacketTypeNames[ReplicatorStats::PACKET_TYPE_CategoryAppearance],
            ReplicatorStats::PACKET_TYPE_CategoryAppearance);
        ...
        physicsPacketsSent->createBoundChildItem("Size", stats.physicsSenderStats.physicsPacketsSentSize);
        ...
        touchPacketsSent->createBoundChildItem("Size", stats.physicsSenderStats.touchPacketsSentSize);
        touchPacketsSent->createBoundChildItem("WaitingTouches", stats.physicsSenderStats.waitingTouches);
    ...
    virtual void update()
    {
        ...
        if (shared_ptr<const Replicator> locked = replicator.lock())
        {
            ...
            if (locked->rakPeer)
            {
                ...
                ping->formatValue(
                    stats.peerStats.lastPing, "%d avg:%d best:%d", stats.peerStats.lastPing, stats.peerStats.averagePing, stats.peerStats.lowestPing);
