...
struct DeserializedPacket
{
    ...
    DeserializedPacket() {}
    DeserializedPacket(RakNet::Packet* pkt)
        : rawPacket(pkt)
    {
    }
};
...
class Replicator
    : public Aya::DescribedNonCreatable<Replicator, IdSerializer, sReplicator, Reflection::ClassDescriptor::INTERNAL_LOCAL>
...
public:
    ...
    struct ReplicationData
    {
        ...
        Aya::signals::connection connection;
    ...
    struct ClusterReplicationData
    {
        ...
        Aya::signals::connection parentConnection;
        ...
        bool hasDataToSend()
        {
            return updateBuffer.size() > 0 || initialSendIterator.size() > 0 || initialChunkIterator.size() > 0 || initialSendSmooth.size() > 0 ||
                   updateBufferSmooth.size() > 0;
        }
    };
    ...
    double incomingPacketsCountHeadWaitTimeSec(const Aya::Time& timeNow);
    ...
    {
        return approximateSizeOfPendingClusterDeltas;
    }
    ...
    Replicator(RakNet::SystemAddress remotePlayerId, boost::shared_ptr<ConcurrentRakPeer> rakPeer, NetworkSettings* networkSettings,
        bool clusterDebounceEnabled);

    ~Replicator();

    double getRakOffsetTime()
    {
        return rakTimeOffset;
    }
    ...
    Aya::signal<void(std::string, bool)> disconnectionSignal;
    ...
    {
        return true;
    } // this need to be set to true for OnInternalPacket to be called
    virtual void OnInternalPacket(
        RakNet::InternalPacket* internalPacket, unsigned frameNumber, RakNet::SystemAddress remoteSystemAddress, RakNet::TimeMS time, int isSend);

    virtual const Aya::Name& getClassName() const
    {
    ...
    bool isPropertyChangedPending(const Aya::Reflection::ConstProperty& property) const;

    virtual void requestCharacter()
    {
        throw std::runtime_error("");
    }
    virtual void requestInstances()
    {
        throw std::runtime_error("");
    } // overwrite in client replicator
    ...
    const ReplicatorStats& stats() const
    {
    ...
    inline const NetworkSettings& settings() const
    {
    ...
    virtual bool isProtocolCompatible() const
    {
        return true;
    }
    ...
    virtual bool isServerReplicator()
    {
        return false;
    }
    ...
    Time getSendQueueHeadTime() const
    {
        return pendingItems.head_time();
    }

    Aya::Time remoteRaknetTimeToLocalRbxTime(const RemoteTime& time);
    Aya::Time raknetTimeToRbxTime(const RakNet::Time& time);
    RakNet::Time rbxTimeToRakNetTime(const Aya::Time& time);
...
protected:
    ...
    // Item pools
    boost::scoped_ptr<Aya::AutoMemPool> newInstancePool;
    boost::scoped_ptr<Aya::AutoMemPool> deleteInstancePool;
    boost::scoped_ptr<Aya::AutoMemPool> changePropertyPool;
    boost::scoped_ptr<Aya::AutoMemPool> eventInvocationPool;
    boost::scoped_ptr<Aya::AutoMemPool> pingPool;
    boost::scoped_ptr<Aya::AutoMemPool> pingBackPool;
    boost::scoped_ptr<Aya::AutoMemPool> referencePropertyChangedPool;
    ...
    typedef boost::unordered_set<Aya::Reflection::ConstProperty, boost::hash<Aya::Reflection::ConstProperty>,
        std::equal_to<Aya::Reflection::ConstProperty>, boost::fast_pool_allocator<Aya::Reflection::ConstProperty>>
    ...
    mutable Aya::mutex pendingInstancesMutex;
    ...
    // Data-in/out
    typedef std::map<const Aya::Name*, shared_ptr<Instance>> DefaultObjects;
    ...
    Aya::timestamped_safe_queue<RakNet::Packet*> incomingPackets;
    ...
    SharedDictionary<Aya::SystemAddress> systemAddressDictionary;
    SharedDictionary<Aya::ContentId> contentIdDictionary;
    ...
    virtual boost::optional<ProtectedString> decodeProtectedString(
        const std::string& value, const Instance* instance, const Reflection::PropertyDescriptor& desc) = 0;

    virtual bool isLegalReceiveInstance(Instance* instance, Instance* parent)
    {
        return true;
    }
    virtual bool isLegalDeleteInstance(Instance* instance)
    {
        return true;
    }
    virtual bool isLegalReceiveEvent(Instance* instance, const Reflection::EventDescriptor& desc)
    {
        return true;
    }
    virtual bool isLegalReceiveProperty(Instance* instance, const Reflection::PropertyDescriptor& desc)
    {
        return true;
    }

    virtual bool isLegalSendProperty(Instance* instance, const Reflection::PropertyDescriptor& desc) = 0;
    virtual bool isLegalSendInstance(const Instance* instance)
    {
        return true;
    }
    virtual bool isLegalSendEvent(Instance* instance, const Reflection::EventDescriptor& desc)
    {
        return true;
    }

    virtual bool prepareRemotePlayer(shared_ptr<Instance> instance)
    {
        return false;
    }
    virtual bool checkRemotePlayer()
    {
        return true;
    }
    ...
    virtual void writeChangedRefProperty(
    ...
    virtual FilterResult filterReceivedChangedProperty(Instance* instance, const Reflection::PropertyDescriptor& desc)
    {
        return Accept;
    }
    virtual FilterResult filterReceivedParent(Instance* instance, Instance* parent)
    {
        return Accept;
    }
    ...
    const Instance* getDefault(const Aya::Name& className);
    ...
    void readProperties(RakNet::BitStream& inBitstream, Instance* instance, PropertyCacheType cacheType, bool useDictionary,
        bool preventBounceBack = true, std::vector<PropValuePair>* valueArray = NULL);
    void readPropertiesInternal(
        Reflection::Property& property, RakNet::BitStream& inBitstream, bool useDictionary, bool preventBounceBack, Reflection::Variant* value);
    ...
    void writePropertiesInternal(const Instance* instance, const Reflection::ConstProperty& property, RakNet::BitStream& outBitstream,
    ...
    virtual void addTopReplicationContainer(
    ...
    static void RemoteCheatHelper2(weak_ptr<Aya::DataModel> weakDataModel);

    void serializePropertyValue(const Reflection::ConstProperty& property, RakNet::BitStream& outBitStream, bool useDictionary);
    void deserializePropertyValue(
        RakNet::BitStream& inBitStream, Reflection::Property property, bool useDictionary, bool preventBounceBack, Reflection::Variant* value);
    ...
    virtual bool isClassRemoved(const Instance* instance)
    {
        return false;
    }
    virtual bool isPropertyRemoved(const Instance* instance, const Name& propertyName)
    {
        return false;
    }
    virtual bool isEventRemoved(const Instance* instance, const Name& eventName)
    {
        return false;
    }
    ...
    virtual bool ProcessOutdatedChangedProperty(RakNet::BitStream& inBitstream, const Aya::Guid::Data& id, const Instance* instance,
        const Reflection::PropertyDescriptor* propertyDescriptor, unsigned int propId)
    {
        return false;
    }
    virtual bool ProcessOutdatedProperties(RakNet::BitStream& inBitstream, Instance* instance, PropertyCacheType cacheType, bool useDictionary,
        bool preventBounceBack, std::vector<PropValuePair>* valueArray)
    {
        return false;
    }
    virtual bool ProcessOutdatedInstance(RakNet::BitStream& inBitstream, bool isJoinData, const Aya::Guid::Data& id,
        const Reflection::ClassDescriptor* classDescriptor, unsigned int classId)
    {
        return false;
    }
    virtual bool ProcessOutdatedEventInvocation(RakNet::BitStream& inBitstream, const Aya::Guid::Data& id, const Instance* instance,
        const Reflection::EventDescriptor* eventDescriptor, unsigned int eventId)
    {
        return false;
    }
    virtual bool ProcessOutdatedEnumSerialization(const Reflection::Type& type, const Reflection::Variant& value, RakNet::BitStream& outBitStream)
    {
        return false;
    }
    virtual bool ProcessOutdatedEnumDeserialization(RakNet::BitStream& inBitStream, const Reflection::Type& type, Reflection::Variant& value)
    {
        return false;
    }
    virtual bool ProcessOutdatedPropertyEnumSerialization(const Reflection::ConstProperty& property, RakNet::BitStream& outBitStream)
    {
        return false;
    }
    virtual bool ProcessOutdatedPropertyEnumDeserialization(Reflection::Property& property, RakNet::BitStream& inBitStream)
    {
        return false;
    }
...
private:
    typedef Aya::DescribedNonCreatable<Replicator, IdSerializer, sReplicator, Reflection::ClassDescriptor::INTERNAL_LOCAL> Super;
    ...
    void createStatsItems(Aya::Stats::StatsService* stats);
    void updateStatsItem(Aya::Stats::StatsService* stats);
    ...
    Aya::signals::scoped_connection sendFilteredChatMessageConnection;
    void sendFilteredChatMessage(const RakNet::SystemAddress& systemAddress, const shared_ptr<RakNet::BitStream>& data,
        const shared_ptr<Instance> sourceInstance, const std::string& blacklist, const std::string& whitelist);
    ...
    void assignRef(Reflection::Property& property, Aya::Guid::Data id);
    ...
    Aya::timestamped_safe_queue<DeserializedPacket> deserializedPackets;
