...
#include "Replicator.PingJob.hpp"
#include "Replicator.ProcessPacketsJob.hpp"
#include "Replicator.ReferencePropertyChangedItem.hpp"
#include "Replicator.SendDataJob.hpp"
...
#include "Voxel2/BitSerializer.hpp"
...
static Reflection::EventDesc<Replicator, void(std::string, bool)> event_Disconnection(
    &Replicator::disconnectionSignal, "Disconnection", "peer", "lostConnection", Security::LocalUser);
...
static Reflection::BoundFuncDesc<Replicator, std::string(int)> func_getRakStatsString(
    &Replicator::getRakStatsString, "GetRakStatsString", "verbosityLevel", 0, Security::Plugin);
static Reflection::BoundFuncDesc<Replicator, void()> func_DisableProcessPackets(
    &Replicator::disableProcessPackets, "DisableProcessPackets", Security::LocalUser);
static Reflection::BoundFuncDesc<Replicator, void()> func_EnableProcessPackets(
    &Replicator::enableProcessPackets, "EnableProcessPackets", Security::LocalUser);
static Reflection::BoundFuncDesc<Replicator, void(double)> func_SetPropSyncExpiration(
    &Replicator::setPropSyncExpiration, "SetPropSyncExpiration", "seconds", Security::LocalUser);

static Reflection::PropDescriptor<Replicator, int> prop_port(
    "Port", category_Data, &Replicator::getPort, NULL, Reflection::PropertyDescriptor::UI, Security::LocalUser);
static Reflection::PropDescriptor<Replicator, std::string> prop_ip(
    "MachineAddress", category_Data, &Replicator::getIpAddress, NULL, Reflection::PropertyDescriptor::UI, Security::Roblox);
REFLECTION_END();

Aya::Time Replicator::remoteRaknetTimeToLocalRbxTime(const RemoteTime& time)
{
    return time - Time::Interval(rakTimeOffset);
}

Aya::Time Replicator::raknetTimeToRbxTime(const RakNet::Time& time)
{
    Aya::RemoteTime t((double)time / 1000.0f);
    return t - Time::Interval(rakTimeOffset);
}

RakNet::Time Replicator::rbxTimeToRakNetTime(const Aya::Time& time)
{
...
bool Replicator::isPropertyCacheable(const Reflection::Type& type)
{
    ...
    if (type.isType<std::string>() || type.isType<Aya::ProtectedString>() || type.isType<Aya::BinaryString>() || type.isType<Aya::SystemAddress>() ||
        type.isType<Aya::ContentId>() || isRef)
    {
        return false;
    }
...
void Replicator::writeProperties(const Instance* instance, RakNet::BitStream& outBitstream, PropertyCacheType cacheType, bool useDictionary)
{
    ...
    Aya::Reflection::ConstPropertyIterator iter = instance->properties_begin();
    Aya::Reflection::ConstPropertyIterator end = instance->properties_end();
    while (iter != end)
    {
...
void Replicator::writePropertiesInternal(const Instance* instance, const Reflection::ConstProperty& property, RakNet::BitStream& outBitstream,
    PropertyCacheType cacheType, bool useDictionary)
{
    ...
    if ((descriptor.canReplicate()) && (cacheType == PropertyCacheType_All || isPropertyCacheable(descriptor.type) == cacheable) &&
        (descriptor != Instance::propParent)) // don't write the parent now
    {
        ...
        if (!isPropertyRemoved(instance, descriptor.name))
        {
            ...
            if (descriptor.type.isType<bool>())
            {
                ...
                if (settings().printBits)
                {
                    Aya::StandardOut::singleton()->printf(
                        Aya::MESSAGE_SENSITIVE, "   write %s %s, 1 bit", descriptor.type.name.c_str(), descriptor.name.c_str());
                }
            }
            ...
            else
            {
                ...
                if (settings().printBits)
                {
                    Aya::StandardOut::singleton()->printf(Aya::MESSAGE_SENSITIVE, "   write %s %s, %d bit", descriptor.type.name.c_str(),
                        descriptor.name.c_str(), outBitstream.GetWriteOffset() - start);
                }
            }
        }
    }
...
bool ReplicatorJob::canSendPacket(shared_ptr<Replicator>& safeReplicator, PacketPriority packetPriority)
{
    if (safeReplicator)
    {
        return safeReplicator->getBufferCountAvailable(safeReplicator->settings().canSendPacketBufferLimit, packetPriority) > 0;
    }
    else
    {
        return false;
...
int Replicator::getBufferCountAvailable(int bufferLimit, PacketPriority priority)
{
    ...
    if (statistics->isLimitedByOutgoingBandwidthLimit && settings().isThrottledByOutgoingBandwidthLimit)
    {
        return 0;
    }

    if (statistics->isLimitedByCongestionControl && settings().isThrottledByCongestionControl)
    {
        return 0;
    }
...
Replicator::Replicator(
    RakNet::SystemAddress remotePlayerId, boost::shared_ptr<ConcurrentRakPeer> rakPeer, NetworkSettings* networkSettings, bool clusterDebounceEnabled)
    : rakPeer(rakPeer)
    ...
    , deserializePacketsThreadEnabled(false)
{
    if (rakPeer)
    {
        rakPeer->addStats(remotePlayerId, boost::bind(&Replicator::onStatisticsChanged, this, _1));
...
void Replicator::createPhysicsReceiver(NetworkSettings::PhysicsReceiveMethod method, bool isServer)
{
    ...
    else
    {
        ...
        AYAASSERT(!physicsReceiver);
        switch (method)
        {
        case NetworkSettings::Interpolation:
        {
            //			AYAASSERT(!settings().distributedPhysicsEnabled);
...
Replicator::~Replicator()
{
    AYAASSERT(replicationContainers.size() == 0);
...
void Replicator::addTopReplicationContainer(
    Instance* instance, bool replicateProperties, bool replicateChildren, boost::function<void(shared_ptr<Instance>)> replicationMethodFunc)
{
    ...
    topReplicationContainersMap.insert(std::make_pair<const Reflection::ClassDescriptor*, TopReplConts::iterator>(
        &instance->getDescriptor(), topReplicationContainers.insert(topReplicationContainers.end(), instance)));
    ...
    if (replicator)
    {
        ...
        if (replicator->settings().isQueueErrorComputed)
        {
            if (int maxBytesSend = replicator->getAdjustedMtuSize())
            {
                ...
                AYAASSERT(multiplier <= std::max(1, (int)deltas));
...
void Replicator::addTopReplicationContainers(ServiceProvider* newProvider)
{
    ...
    addTopReplicationContainer(ServiceProvider::create<ReplicatedFirst>(newProvider), false, false, replicationMethodFunc);

    addTopReplicationContainer(ServiceProvider::create<Aya::Lighting>(newProvider), true, true, replicationMethodFunc);
    addTopReplicationContainer(ServiceProvider::create<Aya::Soundscape::SoundService>(newProvider), true, false, replicationMethodFunc);
    addTopReplicationContainer(ServiceProvider::create<Aya::StarterPackService>(newProvider), false, true, replicationMethodFunc);
    addTopReplicationContainer(ServiceProvider::create<Aya::StarterGuiService>(newProvider), false, true, replicationMethodFunc);
    addTopReplicationContainer(ServiceProvider::create<Aya::StarterPlayerService>(newProvider), false, true, replicationMethodFunc);

    addTopReplicationContainer(ServiceProvider::create<Aya::CSGDictionaryService>(newProvider), false, true, replicationMethodFunc);

    FASTLOG1F(DFLog::NetworkJoin, "addTopReplicationContainer for Workspace called @ %f s", Time::nowFastSec());
    addTopReplicationContainer(ServiceProvider::find<Aya::Workspace>(newProvider), true, true, replicationMethodFunc);
    ...
    addTopReplicationContainer(ServiceProvider::create<Aya::JointsService>(newProvider), false, true, replicationMethodFunc);
    addTopReplicationContainer(players = ServiceProvider::create<Aya::Network::Players>(newProvider), true, true, replicationMethodFunc);
    addTopReplicationContainer(ServiceProvider::create<Aya::Teams>(newProvider), false, true, replicationMethodFunc);
    addTopReplicationContainer(ServiceProvider::create<Aya::InsertService>(newProvider), true, true, replicationMethodFunc);
    addTopReplicationContainer(ServiceProvider::create<Aya::ChatService>(newProvider), true, true, replicationMethodFunc);
    addTopReplicationContainer(ServiceProvider::create<Aya::FriendService>(newProvider), true, true, replicationMethodFunc);
    addTopReplicationContainer(ServiceProvider::create<Aya::MarketplaceService>(newProvider), true, true, replicationMethodFunc);
    addTopReplicationContainer(ServiceProvider::create<Aya::BadgeService>(newProvider), true, false, replicationMethodFunc);
    addTopReplicationContainer(ServiceProvider::create<Aya::ReplicatedStorage>(newProvider), true, true, replicationMethodFunc);
    addTopReplicationContainer(ServiceProvider::create<Aya::RobloxReplicatedStorage>(newProvider), true, true, replicationMethodFunc);
    addTopReplicationContainer(ServiceProvider::create<Aya::TestService>(newProvider), true, true, replicationMethodFunc);
...
{
    FASTLOG2(FLog::Network, "Replicator:canReplicateInstance - start, instance is %s, replicationProtocolVersion = %i", instance->getName().c_str(),
        replicationProtocolVersion);

    if (Instance::fastDynamicCast<Aya::ReplicatedFirst>(instance))
        return (replicationProtocolVersion >= 25);
    if (Instance::fastDynamicCast<Aya::AdService>(instance))
...
void Replicator::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
    if (deserializePacketsThread)
    {
        AYAASSERT(deserializePacketsThreadEnabled);
        deserializePacketsThreadEnabled = false;
        packetReceivedEvent.Set();
    ...
    updateStatsItem(ServiceProvider::find<Aya::Stats::StatsService>(newProvider));

    if (oldProvider)
    {
        Aya::DataModel* oldDM = static_cast<Aya::DataModel*>(oldProvider);
    ...
    if (newProvider)
    {
        ...
        Aya::DataModel* newDM = static_cast<Aya::DataModel*>(newProvider);
...
void Replicator::sendFilteredChatMessage(const RakNet::SystemAddress& systemAddress, const shared_ptr<RakNet::BitStream>& data,
    const shared_ptr<Instance> sourceInstance, const std::string& whitelist, const std::string& blacklist)
{
    if (remotePlayerId == systemAddress)
        return;
    Player* player = findTargetPlayer();
    if (!player || Player::CHAT_FILTER_WHITELIST == player->getChatFilterType())
    { // assume player's whitelisted
...
double Replicator::incomingPacketsCountHeadWaitTimeSec(const Aya::Time& timeNow)
{
    if (deserializePacketsThread)
...
void Replicator::updateStatsItem(Aya::Stats::StatsService* stats)
{
    if (statsItem != NULL)
    {
...
template<>
bool SenderDictionary<Aya::SystemAddress>::isDefaultValue(const Aya::SystemAddress& value)
{
    return value.empty();
}

template<>
void ReceiverDictionary<Aya::SystemAddress>::setDefault(Aya::SystemAddress& value)
{
    value.clear();
}

template<>
bool SenderDictionary<Aya::ContentId>::isDefaultValue(const Aya::ContentId& value)
{
    return value.isNull();
}

template<>
void ReceiverDictionary<Aya::ContentId>::setDefault(Aya::ContentId& value)
{
    value.clear();
}

template<>
bool SenderDictionary<Aya::Guid::Scope>::isDefaultValue(const Aya::Guid::Scope& value)
{
    return value.isNull();
}

template<>
void ReceiverDictionary<Aya::Guid::Scope>::setDefault(Aya::Guid::Scope& value)
{
    value.setNull();
}
...
void Replicator::serializeEventInvocation(const Reflection::EventInvocation& eventInvocation, RakNet::BitStream& outBitStream)
{
    ...
    for (std::list<Reflection::SignatureDescriptor::Item>::const_iterator typeIter = signatureDescriptor.arguments.begin();
        typeIter != signatureDescriptor.arguments.end(); ++typeIter, ++valueIter)
    {
        AYAASSERT(valueIter != eventInvocation.args.end());
...
bool Replicator::serializeValue(const Reflection::Type& type, const Reflection::Variant& value, RakNet::BitStream& outBitStream)
{
    ...
    else if (type == Reflection::Type::singleton<int64_t>())
    {
        serializeGeneric<int64_t>(value, outBitStream);
    }
    ...
    else if (type == Reflection::Type::singleton<Aya::RbxRay>())
    {
        serializeGeneric<Aya::RbxRay>(value, outBitStream);
    }
    ...
    else if (type == Reflection::Type::singleton<Aya::Region3>())
    {
    ...
    else if (type == Reflection::Type::singleton<Aya::Region3int16>())
    {
    ...
    else if (type == Reflection::Type::singleton<Aya::SystemAddress>())
    {
        systemAddressDictionary.send(outBitStream, value.cast<Aya::SystemAddress>());
    }
    else if (type == Reflection::Type::singleton<Aya::ContentId>())
    {
        contentIdDictionary.send(outBitStream, value.cast<Aya::ContentId>());
    }
    else if (type == Reflection::Type::singleton<shared_ptr<Aya::Instance>>())
    {
        serializeInstanceRef(value.cast<shared_ptr<Aya::Instance>>().get(), outBitStream);
    }
    ...
    else if (type == Reflection::Type::singleton<shared_ptr<const Aya::Reflection::Tuple>>())
    {
        const Aya::Reflection::Tuple* data = value.cast<shared_ptr<const Aya::Reflection::Tuple>>().get();
    ...
    else if (type == Reflection::Type::singleton<shared_ptr<const Aya::Reflection::ValueArray>>())
    {
        const Aya::Reflection::ValueArray* data = value.cast<shared_ptr<const Aya::Reflection::ValueArray>>().get();
    ...
    else if (type == Reflection::Type::singleton<shared_ptr<const Aya::Reflection::ValueTable>>())
    {
        const Aya::Reflection::ValueTable* data = value.cast<shared_ptr<const Aya::Reflection::ValueTable>>().get();

        int size = data->size();
        outBitStream << size;

        for (Aya::Reflection::ValueTable::const_iterator it = data->begin(); it != data->end(); ++it)
        {
            outBitStream << it->first;
            serializeVariant(it->second, outBitStream);
        }
    }
    else if (type == Reflection::Type::singleton<shared_ptr<const Aya::Reflection::ValueMap>>())
    {
        const Aya::Reflection::ValueMap* data = value.cast<shared_ptr<const Aya::Reflection::ValueMap>>().get();

        int size = data->size();
        outBitStream << size;

        for (Aya::Reflection::ValueMap::const_iterator it = data->begin(); it != data->end(); ++it)
        {
            outBitStream << it->first;
            serializeVariant(it->second, outBitStream);
        }
    ...
    else if (type == Reflection::Type::singleton<TweenInfo>())
    {
        serializeGeneric<TweenInfo>(value, outBitStream);
    }
    else if (type == Reflection::Type::singleton<G3D::Color3uint8>())
    {
        serializeGeneric<G3D::Color3uint8>(value, outBitStream);
    }
    else
    {
        return false;
...
{
    if (property.getDescriptor().type == Reflection::Type::singleton<Aya::ProtectedString>())
    {
    ...
    else if (property.getDescriptor().type == Reflection::Type::singleton<int64_t>())
    {
        serialize<int64_t>(property, outBitStream);
    }
    ...
    else if (property.getDescriptor().type == Reflection::Type::singleton<Aya::RbxRay>())
    {
        serialize<Aya::RbxRay>(property, outBitStream);
    }
    ...
    else if (Reflection::RefPropertyDescriptor::isRefPropertyDescriptor(property.getDescriptor()))
    {
        const Reflection::RefPropertyDescriptor* desc =
            boost::polymorphic_downcast<const Reflection::RefPropertyDescriptor*>(&property.getDescriptor());
        Aya::Instance* instance = boost::polymorphic_downcast<Instance*>(desc->getRefValue(property.getInstance()));
    ...
    else if (property.getDescriptor().type == Reflection::Type::singleton<Aya::ContentId>())
    {
        if (useDictionary)
            contentIdDictionary.send(outBitStream, property.getValue<Aya::ContentId>());
        else
            serialize<Aya::ContentId>(property, outBitStream);
    }
    else if (property.getDescriptor().type == Reflection::Type::singleton<Aya::SystemAddress>())
    {
        if (useDictionary)
            systemAddressDictionary.send(outBitStream, property.getValue<Aya::SystemAddress>());
        else
            outBitStream << property.getValue<Aya::SystemAddress>();
    }
    ...
    else if (property.getDescriptor().type == Reflection::Type::singleton<TweenInfo>())
    {
        serialize<TweenInfo>(property, outBitStream);
    }
    else if (property.getDescriptor().type == Reflection::Type::singleton<G3D::Color3uint8>())
    {
        serialize<G3D::Color3uint8>(property, outBitStream);
    }
    else
    {
...
void Replicator::deserializeEventInvocation(RakNet::BitStream& inBitStream, Reflection::EventInvocation& eventInvocation, bool resolveRefTypes)
{
    ...
    AYAASSERT(signature.arguments.size() == numEntries);
    ...
    for (Reflection::SignatureDescriptor::Arguments::const_iterator iter = signature.arguments.begin(); iter != signature.arguments.end(); ++iter)
    {
        ...
        else if (!resolveRefTypes && type == Reflection::Type::singleton<shared_ptr<Aya::Instance>>())
        {
...
bool Replicator::deserializeValue(RakNet::BitStream& inBitStream, const Reflection::Type& type, Reflection::Variant& value)
{
    if (type == Reflection::Type::singleton<bool>())
        ...
        deserializeGeneric<int64_t>(value, inBitStream);
    else if (type == Reflection::Type::singleton<long>())
    {
        int v;
        inBitStream >> v;
    ...
    else if (type == Reflection::Type::singleton<BinaryString>())
    {
        ...
        deserializeGeneric<Aya::RbxRay>(value, inBitStream);
    else if (type == Reflection::Type::singleton<Faces>())
    ...
    {
        Aya::SystemAddress valueAddress;
        systemAddressDictionary.receive(inBitStream, valueAddress);
        value = valueAddress;
    }
    else if (type == Reflection::Type::singleton<shared_ptr<const Aya::Reflection::Tuple>>())
    {
        shared_ptr<Aya::Reflection::Tuple> data(new Aya::Reflection::Tuple());
    ...
    {
        shared_ptr<Aya::Reflection::ValueArray> data(new Aya::Reflection::ValueArray());
    ...
    {
        shared_ptr<Aya::Reflection::ValueTable> data(new Aya::Reflection::ValueTable());
    ...
    {
        shared_ptr<Aya::Reflection::ValueMap> data(new Aya::Reflection::ValueMap());
    ...
    {
        deserializeGeneric<TweenInfo>(value, inBitStream);
    }
    else if (type == Reflection::Type::singleton<G3D::Color3uint8>())
    {
        deserializeGeneric<G3D::Color3uint8>(value, inBitStream);
    }
    else
    {
        return false;
...
void Replicator::RemoteCheatHelper2(weak_ptr<Aya::DataModel> weakDataModel)
{
    if (shared_ptr<Aya::DataModel> dataModel = weakDataModel.lock())
    {
        if (Aya::Network::Player* player = Network::Players::findLocalPlayer(dataModel.get()))
        {
...
void Replicator::deserializePropertyValue(
    RakNet::BitStream& inBitStream, Reflection::Property property, bool useDictionary, bool preventBounceBack, Reflection::Variant* outValue)
{
    // NOTE:
    ...
    {
        AYAASSERT(deserializingProperty == NULL);
    ...
    if (property.getDescriptor().type == Reflection::Type::singleton<Aya::ProtectedString>())
    {
        ...
        boost::optional<ProtectedString> value =
            decodeProtectedString(valueString.value(), static_cast<const Instance*>(property.getInstance()), property.getDescriptor());
    ...
    else if (property.getDescriptor().type == Reflection::Type::singleton<BinaryString>())
    {
        ...
        outValue ? deserializeGeneric<int64_t>(*outValue, inBitStream) : deserialize<int64_t>(property, inBitStream);
    else if (property.getDescriptor().type == Reflection::Type::singleton<float>())
        ...
        outValue ? deserializeGeneric<Aya::RbxRay>(*outValue, inBitStream) : deserialize<Aya::RbxRay>(property, inBitStream);
    else if (property.getDescriptor().type == Reflection::Type::singleton<Faces>())
    ...
    else if (property.getDescriptor().type == Reflection::Type::singleton<Aya::ContentId>())
    {
        if (useDictionary)
        {
            ...
            outValue ? deserializeGeneric<Aya::ContentId>(*outValue, inBitStream) : deserialize<Aya::ContentId>(property, inBitStream);
    }
    else if (property.getDescriptor().type == Reflection::Type::singleton<Aya::SystemAddress>())
    {
        Aya::SystemAddress value;
        if (useDictionary)
    ...
    {
        outValue ? deserializeGeneric<TweenInfo>(*outValue, inBitStream) : deserialize<TweenInfo>(property, inBitStream);
    }
    else if (property.getDescriptor().type == Reflection::Type::singleton<G3D::Color3uint8>())
    {
        outValue ? deserializeGeneric<G3D::Color3uint8>(*outValue, inBitStream) : deserialize<G3D::Color3uint8>(property, inBitStream);
    }
    else
        AYAASSERT(false);
}
...
void Replicator::writeChangedRefProperty(
...
bool Replicator::wantReplicate(const Instance* source) const
{
    ...
    case Reflection::PLAYER_REPLICATE:
        if (const Aya::Network::Player* player = Instance::fastDynamicCast<Aya::Network::Player>(source->getParent()))
        {
            if (const Aya::Network::Player* targetPlayer = findTargetPlayer())
            {
...
void Replicator::onChildAdded(shared_ptr<Instance> child, boost::function<void(shared_ptr<Instance>)> replicationMethodFunc)
{
    AYAASSERT(child.get() != removingInstance);
    ...
    if (replicationContainers.find(child.get()) == replicationContainers.end() && megaClusterInstance != child.get())
    {
    ...
    for (size_t i = 0; i < ids.size(); ++i)
    {
        ...
        if (streamingEnabled && streamJob)
        {
            StreamRegion::Id regionId =
                StreamRegion::regionContainingVoxel((chunkId << ClusterReplicationData::kUpdateChunkSizeLog2).toVector3int16());
...
Replicator::ReplicationData& Replicator::addReplicationData(shared_ptr<Instance> instance, bool listenToChanges, bool replicateChildren)
{
    ...
    if (cluster)
    {
        AYAASSERT(megaClusterInstance == NULL);
        ...
        else
        {
            if (cluster->isSmooth())
            {
                AYAASSERT(oneQuarterClusterPacketCache);
        ...
        else
        {
            ...
            clusterReplicationData.parentConnection =
                instance->ancestryChangedSignal.connect(boost::bind(&Replicator::onTerrainParentChanged, this, _1, _2));
        }
    }
    ...
    else
    {
        ...
        AYAASSERT(oldData.instance == instance);
...
{
    Aya::mutex::scoped_lock lock(pendingInstancesMutex);

    if (megaClusterInstance && instance == megaClusterInstance && !clusterReplicationData.readyToSendChunks)
    {
...
bool Replicator::isPropertyChangedPending(const Aya::Reflection::ConstProperty& property) const
{
...
void Replicator::onCombinedSignal(ReplicationData* replicationData, Instance::CombinedSignalType type, const ICombinedSignalData* genericData)
{
    ...
    case Instance::CHILD_ADDED:
    {
        if (replicationData->replicateChildren)
        {
            ...
            if (wantReplicate(data->child.get()) && !isReplicationContainer(data->child.get()) && megaClusterInstance != data->child.get())
            {
...
void Replicator::onEventInvocation(
    Instance* instance, const Reflection::EventDescriptor* descriptor, const Reflection::EventArguments* args, const SystemAddress* target)
{
    ...
    if (deserializingEventInvocation != NULL &&
        Reflection::EventInvocation(Reflection::Event(*descriptor, shared_from(instance)), *args) == *deserializingEventInvocation)
        return;
...
void Replicator::onPropertyChanged(Instance* instance, const Reflection::PropertyDescriptor* descriptor)
{
    if (Reflection::RefPropertyDescriptor::isRefPropertyDescriptor(*descriptor))
    {
        pendingItems.push_back(new (referencePropertyChangedPool.get())
                ReferencePropertyChangedItem(this, shared_from(instance), static_cast<const Reflection::RefPropertyDescriptor&>(*descriptor)));
    }
    else
    {
        Player* player = findTargetPlayer();
        ...
        if ((m && instance->isDescendantOf(m)) || (player == instance))
        {
            ...
            pendingChangedPropertyItems.insert(Aya::Reflection::ConstProperty(*descriptor, instance));
        }
        else
        {
...
void Replicator::logPacketError(RakNet::Packet* packet, const std::string& type, const std::string& message)
{
    std::string errorMessage =
        Aya::format("Error while processing packet: %s (packet id: %d, packet length: %d)", message.c_str(), packet->data[0], packet->length);

    Aya::StandardOut::singleton()->print(Aya::MESSAGE_ERROR, "Error while processing packet.");
    Aya::StandardOut::singleton()->print(Aya::MESSAGE_SENSITIVE, errorMessage.c_str());
}
...
void Replicator::processDeserializedPacket(const DeserializedPacket& deserializedPacket)
{
    ...
    {
        AYAPROFILER_SCOPE("Network", "processDeserializedPacket");
        AYAPROFILER_LABELF("Network", "ID %d (%d bytes)", deserializedPacket.rawPacket->data[0], deserializedPacket.rawPacket->length);
        AYAPROFILER_LABELF("Network", "%d items", int(deserializedPacket.deserializedItems.size()));
...
bool Replicator::processNextIncomingPacket()
{
...

    {
        ...
        catch (Aya::physics_receiver_exception& e)
        {
        ...
        catch (Aya::network_stream_exception& e)
        {
            logPacketError(packet, "Stream", e.what());
            rakPeer->DeallocatePacket(packet);
...
int Replicator::sendItems(ItemSender& sender, ItemQueue& itemQueue)
{
    ...
    while (itemQueue.pop_if_present(item))
    {
        ...
        replicatorStats.dataTimeInQueue.sample((Aya::Time::now<Aya::Time::Fast>() - item->timestamp).msec());
        numSent++;
    }
...
void sendClusterContentHelper(
    MegaClusterInstance* megaClusterInstance, shared_ptr<RakNet::BitStream>& bitStream, Container& container, int maxBytesSend)
{
...
void Replicator::sendClusterChunk(const StreamRegion::Id& regionId)
{
    ...
    {
        if (megaClusterInstance->getVoxelGrid()
                ->getRegion(StreamRegion::getMinVoxelCoordinateInsideRegion(regionId), StreamRegion::getMaxVoxelCoordinateInsideRegion(regionId))
                .isGuaranteedAllEmpty())
            return;
...
bool Replicator::sendClusterPacket()
{
    ...
    *bitStream << false; // using regular iterator (non-streaming)
    ...
    rakPeer->Send(bitStream, settings().getDataSendPriority(), DATAMODEL_RELIABILITY, DATA_CHANNEL, remotePlayerId, false);
...
const Instance* Replicator::getDefault(const Aya::Name& className)
{
    ...
    if (iter == defaultObjects.end())
    {
        ...
        shared_ptr<Instance> instance = Creatable<Instance>::createByName(className, Aya::ReplicationCreator);
        if (!instance && strcmp(className.c_str(), "Workspace"))
            Aya::StandardOut::singleton()->printf(Aya::MESSAGE_WARNING, "Replication: Can't create default object of type %s", className.c_str());
...
void Replicator::compressBitStream(const RakNet::BitStream& inUncompressedBitStream, RakNet::BitStream& outCompressedBitStream, uint8_t compressRatio)
{
    ...
    boost::iostreams::stream<boost::iostreams::array_source> source(
        (char*)inUncompressedBitStream.GetData(), inUncompressedBitStream.GetNumberOfBytesUsed());
...
#ifdef NETWORK_DEBUG
    StandardOut::singleton()->printf(MESSAGE_INFO, "Compressed bitStream from %d bytes to %d bytes with level %d compressRatio in %f seconds",
        inUncompressedBitStream.GetNumberOfBytesUsed(), outCompressedBitStream.GetNumberOfBytesUsed(), compressRatio, timer.delta().seconds());
#endif
}

void Replicator::decompressBitStream(RakNet::BitStream& inCompressedBitStream, RakNet::BitStream& outUncompressedBitStream)
{
    AYAPROFILER_SCOPE("Network", "decompressBitStream");
    ...
    inCompressedBitStream.Read(buffer, len);

    // read and decompress join data
    std::string decompressedData;

    // decompress the data
    boost::iostreams::stream<boost::iostreams::array_source> source(buffer, len);
    ...
    AYAPROFILER_LABELF(
        "Network", "%d bytes -> %d bytes", int(inCompressedBitStream.GetNumberOfBytesUsed()), int(outUncompressedBitStream.GetNumberOfBytesUsed()));
}
...
void Replicator::readPropertiesFromValueArray(const std::vector<PropValuePair>& propValueArray, Instance* instance)
{
    for (std::vector<PropValuePair>::const_iterator iter = propValueArray.begin(); iter != propValueArray.end(); iter++)
    {
        ...
        else if (descriptor.type == Reflection::Type::singleton<Aya::ContentId>())
        {
            Aya::ContentId value = iter->value.get<Aya::ContentId>();
...
void Replicator::readProperties(RakNet::BitStream& inBitstream, Instance* instance, PropertyCacheType cacheType, bool useDictionary,
    bool preventBounceBack, std::vector<PropValuePair>* valueArray)
{
    ...
    Aya::Reflection::PropertyIterator iter = instance->properties_begin();
    Aya::Reflection::PropertyIterator end = instance->properties_end();
    while (iter != end)
    {
        Reflection::Property property = *iter;
        const Reflection::PropertyDescriptor& descriptor = property.getDescriptor();
        if ((descriptor.canReplicate()) && (cacheType == PropertyCacheType_All || isPropertyCacheable(descriptor.type) == cacheable) &&
            (!cacheable || descriptor != Instance::propParent)) // don't read parent if it's cacheable (PropertyCacheType_Cacheable) or potentially
                                                                // cacheable (PropertyCacheType_All)
        {
#ifdef NETWORK_DEBUG
...
void Replicator::readPropertiesInternal(
    Reflection::Property& property, RakNet::BitStream& inBitstream, bool useDictionary, bool preventBounceBack, Reflection::Variant* outValue)
{
    ...
    if (descriptor.type.isType<bool>())
    {
        ...
        {
            Aya::StandardOut::singleton()->printf(
                Aya::MESSAGE_SENSITIVE, "   read %s %s, 1 bit", descriptor.type.name.c_str(), descriptor.name.c_str());
        }
    }
    else
    {
        ...
        if (isDefault)
        {
            ...
            {
                Aya::StandardOut::singleton()->printf(
                    Aya::MESSAGE_SENSITIVE, "   read %s %s, 1 bit (default)", descriptor.type.name.c_str(), descriptor.name.c_str());
            }
        }
        else
        {
            ...
            if (settings().printBits)
            {
                Aya::StandardOut::singleton()->printf(Aya::MESSAGE_SENSITIVE, "   read %s %s, %d bits", descriptor.type.name.c_str(),
                    descriptor.name.c_str(), inBitstream.GetReadOffset() - start + 1);
            }
        }
    }
}
...
void Replicator::readInstanceNew(RakNet::BitStream& inBitstream, bool isJoinData)
{
    ...
    unsigned int classId = classDictionary.receive(inBitstream, classDescriptor,
    ...
    if (settings().printInstances)
    {
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_SENSITIVE,
            ...
            classDescriptor->name.c_str(), id.readableString().c_str(), RakNetAddressToString(remotePlayerId).c_str());
    }
    ...
    if (guidRegistry->lookupByGuid(id, instance))
    {
        ...
        if (!instance)
            throw Aya::runtime_error("readInstanceNew got a null object (guid %s)", id.readableString(32).c_str());

        if (instance->getDescriptor() != *classDescriptor)
        {
            std::string message =
                Aya::format("Replication: Bad re-binding %s-%s << %s, %s-%s", classDescriptor->name.c_str(), id.readableString().c_str(),
                    RakNetAddressToString(remotePlayerId).c_str(), instance->getClassName().c_str(), id.readableString().c_str());
    ...
    else
    {
        ...
        instance = Creatable<Instance>::createByName(classDescriptor->name, Aya::ReplicationCreator);
        if (!instance)
        {
    ...
    {
        AYAASSERT(data);
        AYAASSERT(!data->listenToChanges);
        data->listenToChanges = true;
        AYAASSERT(Name::lookup("Terrain") == classDescriptor->name || data->connection.connected());

        // no ReplicatedFirst members are allowed to replicate anything once received!
        {
            ReplicatedFirst* replicatedFirst = Aya::ServiceProvider::find<ReplicatedFirst>(parent.get());
...
void Replicator::readInstanceNewItem(DeserializedNewInstanceItem* item, bool isJoinData)
{
    ...
    else if (!reject)
    {
        ...
        AYAASSERT(!data->listenToChanges);
        data->listenToChanges = true;
        AYAASSERT(data->connection.connected());
...
void Replicator::receiveCluster(RakNet::BitStream& inBitstream, Instance* instance, bool usingOneQuarterIterator)
{
    if (instance == NULL)
        return;
    AYAASSERT(instance == megaClusterInstance);
    ...
    if (megaClusterInstance->isSmooth())
    {
        ...
        while (chunkSizeLog2)
        {
            ...
            bool skipChunk = clusterDebounceEnabled && chunkSizeLog2 == ClusterReplicationData::kUpdateChunkSizeLog2 &&
...
void Replicator::readItem(RakNet::BitStream& inBitstream, Item::ItemType itemType)
{
    ...
    if (DFFlag::ReadDeSerializeProcessFlow)
    {
        ...
        if (item)
            item->process(*this); // make sure it is not NULL
    }
    else
    {
        switch (itemType)
...
void Replicator::processMarker(int id)
{
    ...
    {
        Aya::StandardOut::singleton()->printf(
            Aya::MESSAGE_SENSITIVE, "Received marker %d from %s", id, RakNetAddressToString(remotePlayerId).c_str());
    }
    ...
    {
        AYAASSERT(id == incomingMarkers.front()->id);
...
void Replicator::readChangedProperty(RakNet::BitStream& inBitstream)
{
    ...
    if (instance)
    {
        ...
        {
            throw Aya::runtime_error("Replication: Bad re-binding prop %s-%s << %s", instance->getClassName().c_str(),
                propertyDescriptor->name.c_str(), RakNetAddressToString(remotePlayerId).c_str());
        }
    }
#ifdef NETWORK_DEBUG
    // if (propertyDescriptor->name == "CFrame")
    //{
    //     StandardOut::singleton()->printf(MESSAGE_WARNING, "[ChangedProperty] Received property (%s) for %s", propertyDescriptor->name.c_str(),
    ...
    if (settings().printProperties)
    {
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_SENSITIVE,
            ...
            instance ? instance->getClassName().c_str() : "?", id.readableString().c_str(), propertyDescriptor->name.c_str(),
            RakNetAddressToString(remotePlayerId).c_str(), (inBitstream.GetReadOffset() - start) / 8);
    }
    if (settings().trackDataTypes)
    {
...
{
    AYAASSERT(prop.getDescriptor() == Instance::propParent);
...
void Replicator::readChangedPropertyItem(DeserializedChangePropertyItem* item, Reflection::Property prop)
{
    ...
    else
    {
        ...
        {
            Aya::ContentId value = item->value.get<Aya::ContentId>();
...
void Replicator::readEventInvocation(RakNet::BitStream& inBitstream)
{
    ...
    if (settings().printEvents)
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_SENSITIVE,
            ...
            instance ? instance->getClassName().c_str() : "?", id.readableString().c_str(), eventDescriptor->name.c_str(),
            RakNetAddressToString(remotePlayerId).c_str());
    ...
    {
        throw Aya::runtime_error("Replication: Bad re-binding event %s-%s << %s", instance->getClassName().c_str(), eventDescriptor->name.c_str(),
            RakNetAddressToString(remotePlayerId).c_str());
    }
...
void Replicator::deleteInstanceById(Guid::Data id)
{
    ...
    if (guidRegistry->lookupByGuid(id, instance))
    {
        ...
        if (settings().printInstances)
        {
            Aya::StandardOut::singleton()->printf(Aya::MESSAGE_SENSITIVE, "Replication: ~%s:%s << %s",
                instance ? instance->getClassName().c_str() : "NULL", id.readableString().c_str(), RakNetAddressToString(remotePlayerId).c_str());
        }
        ...
        if (!rejected)
            if (disconnectReplicationData(instance))
            {
                ...
                AYAASSERT(removingInstance == NULL);
                Aya::ScopedAssign<Instance*> assign(removingInstance, instance.get());
    ...
    else
    {
        if (settings().printInstances)
        {
            Aya::StandardOut::singleton()->printf(Aya::MESSAGE_SENSITIVE, "Replication ~??? << %s", RakNetAddressToString(remotePlayerId).c_str());
        }
    }
...
void Replicator::processPacket(Packet* packet)
{
    ...
    case ID_TIMESTAMP:
    {
        if (physicsReceiver.get())
        {
            ...
            catch (Aya::base_exception& e)
            {
                ...
                throw Aya::physics_receiver_exception(e.what());
            }
        ...
        {
#ifdef NETWORK_DEBUG
            StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "Received physics packet before PhysicsReceiver is created.");
#endif
            // AYACRASH();			// we crash here because if not in distributed physics mode, there should be no physicsReceiver on the
            // server side
        }
    }
    break;
    ...
    {
#ifdef NETWORK_DEBUG
        StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Syncing dictionaries..");
#endif
...
#ifdef NETWORK_DEBUG
        StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Dictionaries and SFFlags synced.");
#endif
    }
    break;
    }
...
{
#ifdef NETWORK_DEBUG
    StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "sendDictionaries");
#endif
    RakNet::BitStream bitStream;
...
void Replicator::enableDeserializePacketThread()
{
    ...
    deserializePacketsThread.reset(new boost::thread(
        Aya::thread_wrapper(boost::bind(&Replicator::deserializePacketsThreadImpl, shared_from(this)), "Deserialize Packets Thread")));
...
void Replicator::deserializePacketsThreadImpl()
{
    ...
    while (deserializePacketsThreadEnabled)
    {
        ...
        if (!packetsToDeserailze.empty())
        {
            ...
            AYAPROFILER_SCOPE("Network", "deserializePacket");
            AYAPROFILER_LABELF("Network", "ID %d (%d bytes)", receivedPacket->data[0], receivedPacket->length);
    ...
    if (packet->systemAddress == remotePlayerId)
    {
        try
        {
            ...
            case ID_CONNECTION_LOST:
                ...
                // StandardOut::singleton()->printf(MESSAGE_SENSITIVE, "Lost connection to %s", RakNetAddressToString(packet->systemAddress).c_str());
            ...
            case ID_OPUS_DATA:
                switch (players->OnReceiveOpusData(getRemotePlayer(), this->rakPeer->rawPeer(), packet, packet->data[0]))
                {
                case Players::PLAYERS_STOP_PROCESSING:
                    return RR_STOP_PROCESSING;
                default:
        ...
        catch (Aya::base_exception& e)
        {
            ...
            Aya::StandardOut::singleton()->printf(
                Aya::MESSAGE_ERROR, "Replicator::OnReceive packet %d: %s", (unsigned char)packet->data[0], what ? what : "empty error string");
...
void Replicator::OnInternalPacket(
    InternalPacket* internalPacket, unsigned frameNumber, RakNet::SystemAddress remoteSystemAddress, RakNet::TimeMS time, int isSend)
{
    ...
    if (isSend)
    {
        ...
        {
            if (settings().printSplitMessages && (internalPacket->splitPacketIndex == 0))
            { // only print first split
                StandardOut::singleton()->printf(MESSAGE_INFO, "split message, id %u, size %d, split count %d", internalPacket->data[0],
                    internalPacket->dataBitLength / 8, internalPacket->splitPacketCount);
            }
            replicatorStats.numberOfSplitMessages++;

            if (internalPacket->splitPacketCount > (uint32_t)DFInt::RakNetMaxSplitPacketCount)
            {
                AYAASSERT(internalPacket->splitPacketCount < (uint32_t)DFInt::RakNetMaxSplitPacketCount);
...
{
    {
        ...
        switch (reason)
        {
        case DisconnectReason_BadHash:
            r = "BadHash";
            break;
        case DisconnectReason_SecurityKeyMismatch:
            r = "SecurityKeyMismatch";
            break;
        case DisconnectReason_ProtocolMismatch:
            r = "ProtocolMismatch";
            break;
        case DisconnectReason_ReceivePacketError:
            r = "ReceivePacketError";
            break;
        case DisconnectReason_ReceivePacketStreamError:
            r = "ReceivePacketStreamError";
            break;
        case DisconnectReason_SendPacketError:
            r = "SendPacketError";
            break;
        case DisconnectReason_IllegalTeleport:
            r = "IllegalTeleport";
            break;
        case DisconnectReason_DuplicatePlayer:
            r = "DuplicatePlayer";
            break;
        case DisconnectReason_DuplicateTicket:
            r = "DuplicateTicket";
            break;
        case DisconnectReason_TimeOut:
            r = "TimeOut";
            break;
        case DisconnectReason_LuaKick:
            r = "LuaKick";
            break;
        case DisconnectReason_OnRemoteSysStats:
            r = "OnRemoteSysStats";
            break;
        default:
            break;
...
shared_ptr<Instance> Replicator::sendMarker()
{
    ...
    *bitStream << id;
    ...
    {
        Aya::StandardOut::singleton()->printf(
            Aya::MESSAGE_SENSITIVE, "Replicator: Requesting Marker %d of %s", id, RakNetAddressToString(remotePlayerId).c_str());
    }
...
{
    AYAASSERT(settings().getReplicationMtuAdjust() <= 0);
    return std::max(0, replicatorStats.peerStats.mtuSize + settings().getReplicationMtuAdjust());
}

size_t Replicator::getPhysicsMtuSize() const
{
    AYAASSERT(settings().getPhysicsMtuAdjust() <= 0);
    return std::max(0, replicatorStats.peerStats.mtuSize + settings().getPhysicsMtuAdjust());
}
...
std::string Replicator::getMetric(const std::string& metric) const
{
    if (metric == "Network Send")
    {
        ...
        fmt % sendDataJob->averageStepsPerSecond() % (1000.0 * sendDataJob->averageStepTime()) % (int)(100.0 * sendDataJob->averageDutyCycle());
        return fmt.str();
    }
...
void Replicator::assignRef(Reflection::Property& property, Aya::Guid::Data id)
{
    Instance* baldReferencer = aya_static_cast<Instance*>(property.getInstance());
    ...
    const Reflection::RefPropertyDescriptor* desc = static_cast<const Reflection::RefPropertyDescriptor*>(&property.getDescriptor());

    AYAASSERT(*desc != Instance::propParent);
...
void Replicator::assignParent(Instance* instance, Instance* parent)
{
    ...
    {
        AYAASSERT(instance->getParent() == NULL);
        return;
    }
...
void Replicator::assignDefaultPropertyValue(Reflection::Property& property, bool preventBounceBack, Reflection::Variant* var)
{
    ...
    Instance* instance = aya_static_cast<Instance*>(property.getInstance());
    ...
    if (instance && defaultInstance)
    {
        try
        {
            if (Reflection::RefPropertyDescriptor::isRefPropertyDescriptor(descriptor))
            {
                ...
                Aya::Instance* refInstance = boost::polymorphic_downcast<Instance*>(desc->getRefValue(defaultInstance));
