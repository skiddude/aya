...
static Reflection::BoundFuncDesc<Server, void(int, int)> server_startFunction(
    &Server::start, "Start", "port", 0, "threadSleepTime", 20, Security::Plugin);
...
static Reflection::BoundFuncDesc<Server, void(bool)> func_SetIsPlayerAuthenticationRequired(
    &Server::setIsPlayerAuthenticationRequired, "SetIsPlayerAuthenticationRequired", "value", Security::Roblox);
static Reflection::EventDesc<Server, void(shared_ptr<Instance>, FilterResult, shared_ptr<Instance>, std::string)> desc_dataBasicFiltered(
    &Server::dataBasicFilteredSignal, "DataBasicFiltered", "peer", "result", "instance", "member", Security::LocalUser);
static Reflection::EventDesc<Server, void(shared_ptr<Instance>, FilterResult, shared_ptr<Instance>, std::string)> desc_dataCustomFiltered(
    &Server::dataCustomFilteredSignal, "DataCustomFiltered", "peer", "result", "instance", "member", Security::LocalUser);

Reflection::EventDesc<Server, void(std::string, shared_ptr<Instance>)> event_IncommingConnection(
    &Server::incommingConnectionSignal, "IncommingConnection", "peer", "replicator", Security::RobloxScript);
REFLECTION_END();
            ...
            if (ServerReplicator* rep = Instance::fastDynamicCast<ServerReplicator>(iter->get()))
            {
                if (Player* player = rep->getRemotePlayer())
                {
                    ...
                    if (elapsedTime > 5 * 60)
                    {
                        ...
                        totalPhysicsBytesSendPerSec +=
                            stats.physicsSenderStats.physicsPacketsSent.rate() * stats.physicsSenderStats.physicsPacketsSentSize.value();
                        numPlayers++;
...
void Server::start(int port, int threadSleepTime)
{
    if (Players::clientIsPresent(this))
        throw Aya::runtime_error("Can not call server, client is present.");

    FASTLOG(FLog::Network, "NetworkServer:Start");

    AyaService* ayaService = ServiceProvider::create<AyaService>(this);
    ayaService->setHostPort(port);
    ...
    if (res != RakNet::RAKNET_STARTED)
        throw std::runtime_error(Aya::format("Failed to start network server, id %d", res));
...
void Server::onCreateRakPeer()
{
    ...
    rakPeer->rawPeer()->SetIncomingPassword(Network::password.c_str(), Network::password.size());
}
...
void Server::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
    ...
    if (newProvider && Players::clientIsPresent(newProvider))
        throw Aya::runtime_error("Can not create server, client is present.");
...
RakNet::PluginReceiveResult Server::OnReceive(RakNet::Packet* packet)
{
    ...
    case ID_NEW_INCOMING_CONNECTION:
    {
        ...
        try
        {
            ...
            StandardOut::singleton()->printf(MESSAGE_SENSITIVE, "New connection from %s", RakNetAddressToString(packet->systemAddress).c_str());
...
{
    boost::unordered_map<std::string, long>::const_iterator it =
        isCurrentFormat ? scriptsByCurrentBytecode.find(bytecode) : scriptsByLegacyBytecode.find(bytecode);
    boost::unordered_map<std::string, long>::const_iterator end = isCurrentFormat ? scriptsByCurrentBytecode.end() : scriptsByLegacyBytecode.end();
