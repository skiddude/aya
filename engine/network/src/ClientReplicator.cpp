...
class ClientReplicator::SendOpusDataJob : public DataModelJob
{
    ClientReplicator* clientReplicator;
    constexpr static const double kStepFrequency = 1.0 / ((OPUS_VOICE_BUFSIZE / (OPUS_VOICE_SAMPLERATE / 1000.0)) / 1000.0);

public:
    SendOpusDataJob(ClientReplicator* clientReplicator)
        : DataModelJob("SendOpusDataJob", DataModelJob::None, false, shared_from(DataModel::get(clientReplicator)), Time::Interval(0))
        , clientReplicator(clientReplicator)
    {
    }
    ...
    {
        ServiceProvider::create<Players>(clientReplicator)->SendOpusData();
...
const char* const Aya::Network::sClientReplicator = "ClientReplicator";


static Reflection::BoundFuncDesc<ClientReplicator, void(bool)> func_requestServerStats(
    &ClientReplicator::requestServerStats, "RequestServerStats", "request", Security::RobloxScript);
static Reflection::EventDesc<ClientReplicator, void(shared_ptr<const Reflection::ValueTable>)> event_StatsReceived(
    &ClientReplicator::statsReceivedSignal, "StatsReceived", "stats", Security::RobloxScript);
REFLECTION_END();
...
public:
    CFrameAcknowledgementItem(ClientReplicator* client, shared_ptr<const PartInstance> instance)
        : PooledItem(*client)
        , instance(instance)
        , client(client)
    {
...
ClientReplicator::ClientReplicator(
    RakNet::SystemAddress systemAddress, Client* client, RakNet::SystemAddress clientAddress, NetworkSettings* networkSettings)
    ...
    , memoryLevel(Aya::MemoryStats::MEMORYLEVEL_OK)
{
    ...
    if (strictFilter && (strictFilter->filterNew(instance, instance->getParent()) == Reject))
    {
        if (settings().printDataFilters)
            Aya::StandardOut::singleton()->printf(
                Aya::MESSAGE_WARNING, "Filtering is enabled. New Instance %s will not be replicated.", instance->getFullName().c_str());
...
bool ClientReplicator::isLegalSendEvent(Instance* instance, const Reflection::EventDescriptor& desc)
{
    ...
    {
        if (settings().printDataFilters)
            Aya::StandardOut::singleton()->printf(Aya::MESSAGE_WARNING, "Filtering is enabled. Event %s for instance %s will not be replicated.",
                desc.name.c_str(), instance->getFullName().c_str());
...
{
public:
    RequestCharacterItem(Replicator* replicator)
        : Item(*replicator)
    {
    }
    ...
    /*implement*/ bool write(RakNet::BitStream& bitStream)
    {
        ...
        if (replicator.settings().printInstances)
        {
            ...
            StandardOut::singleton()->printf(MESSAGE_SENSITIVE, "%s: Requesting character for %s",
                RakNetAddressToString(replicator.remotePlayerId).c_str(), id.readableString().c_str());
        }
...
RakNet::PluginReceiveResult ClientReplicator::OnReceive(RakNet::Packet* packet)
{
    ...
    {
        std::string mismatchString = Aya::format("Protocol mismatch from %s", RakNetAddressToString(packet->systemAddress).c_str());
        ...
        AYAASSERT(client);
        client->connectionFailedSignal(
            RakNetAddressToString(packet->systemAddress), (int)packet->data[0], "Network protocol mismatch. Please upgrade.");
...
{
    AYAPROFILER_SCOPE("Network", "processPacket");
    AYAPROFILER_LABELF("Network", "ID %d (%d bytes)", packet->data[0], packet->length);
    ...
    case ID_SET_GLOBALS:
    {
...
#ifdef ENABLE_VOICE_CHAT
            opusJob.reset(new SendOpusDataJob(this));
            TaskScheduler::singleton().add(opusJob);
#endif
        ...
        for (unsigned int i = 0; i < numTopRepContainers; i++)
        {
            ...
            classDictionary.receive(inBitstream, classDescriptor,
...
shared_ptr<DeserializedItem> ClientReplicator::deserializeItem(RakNet::BitStream& inBitstream, Aya::Network::Item::ItemType itemType)
{
    AYAPROFILER_SCOPE("Network", "deserializeItem");
...
void ClientReplicator::readItem(RakNet::BitStream& inBitstream, Aya::Network::Item::ItemType itemType)
{
    switch (itemType)
    {
    default:
...
void ClientReplicator::processTag(int tag)
{
    ...
    {
        if (ReplicatedFirst* repFirst = Aya::ServiceProvider::find<ReplicatedFirst>(this))
        {
...
void ClientReplicator::readTag(RakNet::BitStream& inBitstream)
{
    ...
    {
        Aya::StandardOut::singleton()->printf(Aya::MESSAGE_SENSITIVE, "Received tag %d from %s", id, RakNetAddressToString(remotePlayerId).c_str());
    }
...
void ClientReplicator::readStreamData(RakNet::BitStream& bitStream)
{
    ...
    bitStream >> lowBit;
    bitStream >> highBit;
    Replicator::StreamJob::RegionIteratorSuccessor successorBitMask =
        (Replicator::StreamJob::RegionIteratorSuccessor)((int)lowBit + (((int)highBit) << 1));
    ...
    FASTLOG2(
        DFLog::PartStreamingRequests, "Received %d instances, %d pending requests remaining", numRead, pendingInstanceRequests - numInstancesRead);
}
...
public:
    ClientCapacityUpdateItem(Replicator* replicator, int _numInstanceDiff, short _maxRegionRadius)
        : Item(*replicator)
        , numInstanceDiff(_numInstanceDiff)
        , maxRegionRadius(_maxRegionRadius)
    {
    }
...
{
    return (!gcJob->pendingGC()) && memoryLevel >= Aya::MemoryStats::MEMORYLEVEL_OK;
}

bool ClientReplicator::needGC()
{
    return gcJob->pendingGC() || memoryLevel <= Aya::MemoryStats::MEMORYLEVEL_ALL_LOW;
}
...
{
    memoryLevel =
        Aya::MemoryStats::slowCheckMemoryLevel(((Aya::MemoryStats::memsize_t)NetworkSettings::singleton().getExtraMemoryUsedInMB()) * 1024 * 1024);
    if (memoryLevel == Aya::MemoryStats::MEMORYLEVEL_ONLY_PHYSICAL_CRITICAL_LOW)
    {
    ...
    if (canUpdateClientCapacity())
    {
        ...
        else
        {
            ...
            else
            {
                ...
                clientInstanceQuota = (Aya::Network::kMaxIncomePacketWaitTime - predictedTotalInstanceProcessTime) / avgInstanceProcessTime;
        ...
        if (instanceQuotaDiff != 0 || maxRegionDistanceChanged)
        {
            ...
            {
                StandardOut::singleton()->printf(Aya::MESSAGE_INFO,
                    "clientInstanceQuota %d, packet in queue %d, predictedTotalInstanceProcessTime %f, avgStreamDataReadTime %f, "
                    "avgInstancesPerStreamData %f",
                    clientInstanceQuota, (int)incomingPacketsCount(), predictedTotalInstanceProcessTime, avgStreamDataReadTime.value(),
                    avgInstancesPerStreamData.value());
            }
...
{
    AYAASSERT(streamingEnabled && prop.getDescriptor() == Instance::propParent);
...
void ClientReplicator::writePropAcknowledgementIfNeeded(
    const Instance* instance, const Reflection::PropertyDescriptor& desc, RakNet::BitStream& outBitStream)
{
...
void ClientReplicator::writeChangedRefProperty(
...
void ClientReplicator::onServiceProvider(ServiceProvider* oldProvider, ServiceProvider* newProvider)
{
    ...
    if (opusJob)
    {
        TaskScheduler::singleton().remove(opusJob);
        opusJob.reset();
    }
#endif
...
void ClientReplicator::streamOutPartHelper(const Guid::Data& data, PartInstance* part, shared_ptr<Instance> descendant)
{

    if (shared_ptr<JointInstance> jointInstance = Instance::fastSharedDynamicCast<JointInstance>(descendant))
    {
        ...
        {
            Aya::ScopedAssign<Instance*> assign(removingInstance, jointInstance.get());
...
{
    if (shared_ptr<JointInstance> jointInstance = Instance::fastSharedDynamicCast<JointInstance>(instance))
    {
        AYAASSERT(Joint::isManualJoint(jointInstance->getJoint()) == false);
...
void ClientReplicator::streamOutInstance(Instance* instance, bool deleteImmediately)
{
    ...
    CPUPROFILER_START(Aya::Network::NetworkProfiler::PROFILER_streamOutPart);

    disconnectReplicationData(shared_from(instance));

    CPUPROFILER_STEP(Aya::Network::NetworkProfiler::PROFILER_streamOutPart);
    ...
    if (PartInstance* part = Instance::fastDynamicCast<PartInstance>(instance))
    {
        ...
        for (int i = 0; i < prim->getNumJoints(); ++i)
        {
            ...
            {
                JointInstance* jointInstance = static_cast<JointInstance*>(joint->getJointOwner());
        ...
        if (deleteImmediately)
        {
            ...
            {
                jointsService->visitDescendants(boost::bind(&ClientReplicator::streamOutPartHelper, this, data, part, _1));
            }
        }
    }
    else
    ...
    if (deleteImmediately)
    {
        Aya::ScopedAssign<Instance*> assign(removingInstance, instance);
...
{
#ifdef NETWORK_DEBUG
    StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Syncing schema from server..");
#endif
    RakNet::BitStream bitStream;
    ...
    for (size_t i = 0; i < numClass; i++)
    {
        ...
        shared_ptr<ReflectionClassContainer> classContainer(
            new ReflectionClassContainer(classId, classNameName, (Reflection::ReplicationLevel)replicationLevel));
        ...
        for (size_t j = 0; j < numProp; j++)
        {
            ...
            shared_ptr<ReflectionPropertyContainer> propertyContainer(new ReflectionPropertyContainer(
                propId, Name::declare(propName.C_String()), typeId, std::string(propType.C_String()), type, canReplicate, enumMSB));
            if (!type)
            {
                if (!isEnum)
                {
                    StandardOut::singleton()->printf(MESSAGE_WARNING,
                        "A new type (%s) is used for property '%s', please make sure this property will not be replicated to or from the legacy "
                        "clients.",
                        propType.C_String(), propName.C_String());
                }
            ...
            if (propDesc)
            {
                ...
                if (propDesc->canReplicate() != canReplicate || strcmp(propType.C_String(), propDesc->type.name.c_str()) != 0)
                {
                ...
                if (FFlag::DebugProtocolSynchronization)
                {
                    ...
                    {
                        StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Simulating property adding / changing on WedgePart.BrickColor");
                        propertyContainer->needSync = true;
            ...
            {
                // StandardOut::singleton()->printf(Aya::MESSAGE_WARNING, "Unknown property %s (%s)", propName, propType);
                propertyContainer->needSync = true;
            }
        ...
        for (size_t j = 0; j < numEvent; j++)
        {
            ...
            // StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Event %s, %d", eventContainer->name.c_str(), eventContainer->argTypes.size());
            ...
            if (eventDesc)
            {
                ...
                else
                {
                        ...
                        argIter != eventDesc->getSignature().arguments.end(); ++argIter)
                    {
                        ...
                        // In this case, changing or removing enum values should be prohibited or customized code should be added to handle legacy
                        // clients
                ...
                if (FFlag::DebugProtocolSynchronization)
                {
                    ...
                    {
                        StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Simulating event adding / changing on Player.OnTeleport");
                        eventContainer->needSync = true;
                        ClassDescriptor::ClassDescriptors::const_iterator iter = ClassDescriptor::all_begin();
                        while (iter != ClassDescriptor::all_end())
                        {
    ...
    if (*(instance->getDescriptor().isOutdated))
    {
        ...
        if (classIter != serverClasses.end())
        {
            ...
            if (reflectionClass->needSync == true)
            {
                ...
                for (; propIter != reflectionClass->properties.end(); propIter++)
                {
                    ...
                    if (reflectionProperty->needSync == true)
                    {
                        if (reflectionProperty->canReplicate)
                        {
                            ...
                            if (cacheType == PropertyCacheType_All ||
                                isPropertyCacheable(reflectionProperty->type, reflectionProperty->enumMSB > 0) == cacheable)
                            {
                                // optimization for bool properties
                                if (settings().printBits)
                                {
                                    std::string str = Aya::format("   write %s %s, 1 bit (using server schema)",
                                        reflectionProperty->type ? reflectionProperty->type->name.c_str() : "Unknown",
                                        reflectionProperty->name.c_str());

                                    Aya::StandardOut::singleton()->printf(Aya::MESSAGE_SENSITIVE, "%s", str.c_str());
                                }
                                outBitstream << true;
                            }
                        }
                ...
                AYAASSERT(iter == instance->properties_end());
            }
            else
            {
...
bool ClientReplicator::ProcessOutdatedChangedProperty(RakNet::BitStream& inBitstream, const Aya::Guid::Data& id, const Instance* instance,
...
bool ClientReplicator::ProcessOutdatedProperties(RakNet::BitStream& inBitstream, Instance* instance, PropertyCacheType cacheType, bool useDictionary,
                    ...
                    while (iter != end)
                    {
                        ...
                        if (descriptor.name == reflectionProperty->name)
                        {
                            if (descriptor.canReplicate() &&
                                ...
                                (!cacheable ||
                                    descriptor != Instance::propParent)) // don't read parent if it's cacheable (PropertyCacheType_Cacheable) or
                                                                         // potentially cacheable (PropertyCacheType_All)
                            {
                                if (valueArray)
                                {
...
bool ClientReplicator::ProcessOutdatedInstance(RakNet::BitStream& inBitstream, bool isJoinData, const Aya::Guid::Data& id,
    const Reflection::ClassDescriptor* classDescriptor, unsigned int classId)
{
    ...
    if (FFlag::DebugProtocolSynchronization)
    {
        ...
        {
            StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Simulating class adding on %s", classDescriptor->name.c_str());
    ...
    if (classDescriptor == NULL)
    {
        ...
        for (; classIter != serverClasses.end(); classIter++)
        {
            ...
            if (reflectionClass->id == classId)
            {
                ...
                serverInstanceClassMap.insert(std::pair<Aya::Guid::Data, shared_ptr<ReflectionClassContainer>>(id, reflectionClass));

                if (settings().printInstances)
                {
                    Aya::StandardOut::singleton()->printf(Aya::MESSAGE_SENSITIVE,
                        "Skipping: %s:%s << %s", // note: remove player always on the left
                        reflectionClass->name.c_str(), id.readableString().c_str(), RakNetAddressToString(remotePlayerId).c_str());
                }
...
bool ClientReplicator::ProcessOutdatedEventInvocation(RakNet::BitStream& inBitstream, const Aya::Guid::Data& id, const Instance* instance,
...
bool ClientReplicator::ProcessOutdatedEnumSerialization(
...
void ClientReplicator::skipPropertyValue(
    RakNet::BitStream& inBitStream, const shared_ptr<ClientReplicator::ReflectionPropertyContainer>& prop, bool useDictionary)
{
    ...
    else if (type == Reflection::Type::singleton<Aya::ProtectedString>())
    {
    ...
    {
        G3D::Color3uint8 dummy;
        inBitStream >> dummy;
    }
    else
    {
        StandardOut::singleton()->printf(MESSAGE_ERROR,
            "A new property type is not supported on this legacy client. Please flag off the replication before client and server are synchronized");
        AYAASSERT(false);
    }
}

void ClientReplicator::skipPropertiesInternal(
    const shared_ptr<ClientReplicator::ReflectionPropertyContainer>& prop, RakNet::BitStream& inBitstream, bool useDictionary)
{
    ...
    if (prop->type && prop->type->isType<bool>())
    {
        ...
        if (settings().printBits)
        {
            Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "   skip %s %s, 1 bit", prop->typeName.c_str(), prop->name.c_str());
        }
    }
    else
    {
        ...
        if (isDefault)
        {
            ...
            {
                Aya::StandardOut::singleton()->printf(
                    Aya::MESSAGE_INFO, "   skip %s %s, 1 bit (default)", prop->typeName.c_str(), prop->name.c_str());
            }
        }
        else
        {
            ...
            {
                Aya::StandardOut::singleton()->printf(
                    Aya::MESSAGE_INFO, "   skip %s %s, %d bits", prop->typeName.c_str(), prop->name.c_str(), inBitstream.GetReadOffset() - start + 1);
            }
        }
    }
}
...
bool ClientReplicator::getServerBasedProperty(
    const Name& className, int propertyId, shared_ptr<ClientReplicator::ReflectionPropertyContainer>& serverBasedProperty)
{
...
bool ClientReplicator::getServerBasedEvent(
    const Name& className, int eventId, shared_ptr<ClientReplicator::ReflectionEventContainer>& serverBasedEvent)
{
    ReflectionClassMap::const_iterator classIter = serverClasses.find(&className);
...
bool ClientReplicator::isClassRemoved(const Instance* instance)
{
    ...
    if (FFlag::DebugProtocolSynchronization)
    {
        ...
        {
            StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Simulating class removal TrussPart");
            return true;
        }
    }
#endif
...
bool ClientReplicator::isPropertyRemoved(const Instance* instance, const Name& propertyName)
{
    ...
    if (FFlag::DebugProtocolSynchronization)
    {
        ...
        {
            StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Simulating property removal on WedgePart.Material");
            return true;
        }
    }
#endif
...
bool ClientReplicator::isEventRemoved(const Instance* instance, const Name& eventName)
{
    ...
    if (FFlag::DebugProtocolSynchronization)
    {
        ...
        {
            StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Simulating event removal on Tool.Activated");
            return true;
        }
    }
#endif
...
bool ClientReplicator::hasEnumChanged(const Reflection::EnumDescriptor& enumDesc, size_t& newMSB)
{
    ...
    if (FFlag::DebugProtocolSynchronization)
    {
        ...
        {
            StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "Simulating enum modification on %s", enumDesc.name.c_str());
...
std::string ClientReplicator::encodeProtectedString(
...
boost::optional<ProtectedString> ClientReplicator::decodeProtectedString(
