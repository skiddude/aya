...
Replicator::StreamJob::StreamRegionIterator::StreamRegionIterator(Replicator& replicator)
    ...
    , centerPosition(0, -10000,
          0) // this should be a safe number to trigger a reset on first use, because anything below -500 is automatically removed by physics engine
{
...
bool Replicator::StreamJob::StreamRegionIterator::resetCenter(const Vector3& worldPos, bool force)
{
    ...
    if (force || (fabsf(distToNewCenterPos - distToCurCenterPos) > kStreamCenterResetThreshold))
    {
        ...
        FASTLOG3(
            DFLog::PartStreamingRequests, "worldMinRegion: (%d,%d,%d)", worldMinRegion.value().x, worldMinRegion.value().y, worldMinRegion.value().z);
        FASTLOG3(
            DFLog::PartStreamingRequests, "worldMaxRegion: (%d,%d,%d)", worldMaxRegion.value().x, worldMaxRegion.value().y, worldMaxRegion.value().z);
...
void Replicator::StreamJob::StreamRegionIterator::updateWorldExtents()
{
    if (workspace)
    {
        ...
        AYAASSERT(worldMinRegion != worldMaxRegion);
    }
    else
        AYAASSERT(false);
}
...
bool Replicator::StreamJob::StreamRegionIterator::getNextRegion(StreamRegion::Id& regionId)
{
    ...
    if (centerRegion.value().x > worldMaxRegion.value().x || centerRegion.value().y > worldMaxRegion.value().y ||
        centerRegion.value().z > worldMaxRegion.value().z || centerRegion.value().x < worldMinRegion.value().x ||
        centerRegion.value().y < worldMinRegion.value().y || centerRegion.value().z < worldMinRegion.value().z)
        return false;
    ...
    while (regionRadius <= getMaxRegionRadius())
    {
        ...
        {
            if (current.x > minRegion.x && current.x < maxRegion.x && current.y > minRegion.y && current.y < maxRegion.y && current.z > minRegion.z &&
                current.z < maxRegion.z)
            {
                ++current.y;
                currentRegion = StreamRegion::Id(current);
                continue;
...
void Replicator::StreamJob::updateClientQuota(int diff, short maxRegionRadius)
{
    ...
    AYAASSERT(numClientInstanceQuota >= 0);
    ...
    if (replicator->settings().printStreamInstanceQuota)
    {
        StandardOut::singleton()->printf(
            Aya::MESSAGE_INFO, "Received new client instance quota: %d, max region radius: %d", numClientInstanceQuota, maxRegionRadius);
    }
}
...
{
    for (std::deque<Replicator::StreamJob::StreamDataItem*>::const_iterator iter = pendingStreamItems.begin(); iter != pendingStreamItems.end();
        iter++)
    {
...
PartInstance* findRootPartInstanceAncestor(Instance* instance, PartInstance* bestSoFar)
{
    ...
    return findRootPartInstanceAncestor(instance->getParent(), instanceAsPart ? instanceAsPart : bestSoFar);
}
    ...
    if (iter != replicator->replicationContainers.end())
    {
        ...
        if (rootAncestor != NULL)
        {
            ...
            StreamRegion::IdExtents idExtents = StreamRegion::regionExtentsFromContactManagerLevelAndExtents(
                rootAncestorPrim->getSpatialNodeLevel(), rootAncestorPrim->getOldSpatialExtents());
...
bool Replicator::StreamJob::isAreaInStreamedRadius(const Vector3& center, float radius) const
{
    ...
    Extents streamedExt =
        Extents::fromCenterRadius(regionIterator.getCenterPosition(), regionIterator.getRadius() * StreamRegion::Id::streamGridCellSizeInStuds());
...
{
    AYAASSERT(player == replicator->findTargetPlayer());
    ...
    playerSpawningConnection =
        player->nextSpawnLocationChangedSignal.connect(boost::bind(&StreamJob::setStreamCenterAndSentMinRegions, this, _1, true));
...
bool Replicator::StreamJob::streamTerrain(StreamDataItem& item)
{
    ...
    {
        FASTLOG3(DFLog::PartStreamingRequests, "Streaming terrain region (%d,%d,%d)", regionId.value().x, regionId.value().y, regionId.value().z);
...
TaskScheduler::StepResult Replicator::StreamJob::stepDataModelJob(const Stats& stats)
{
    ...
    if (replicator)
    {
        ...
        {
            streamJobConsecutiveRuns = updateStepsRequiredForCyclicExecutive(stats.timespanSinceLastStep.seconds(), (float)dataSendRate,
                (float)DFInt::MaxConsecutiveStreamJobWorkLoad, (float)DFInt::MaxConsecutiveStreamJobWorkLoad);
        }
        ...
        try
        {
            if (replicator->replicatorStats.dataPacketsReceived.value() <
                Time::Interval(1.f)) // if the client becomes idle / busy for more than a second, we stop collecting new instances
            {
                int numberToCollect = numClientInstanceQuota /
                                      (1.f / stats.timespanSinceLastStep.seconds()); // distribute the collection of one second content to each steps
                AYAASSERT(numberToCollect >= 0);
                if (numCollectedInstances < numberToCollect)
                {
                    AYAASSERT(Workspace::findWorkspace(replicator.get()));
                    regionIterator.updateWorldExtents();
...
#ifdef NETWORK_DEBUG
                    // StandardOut::singleton()->printf(Aya::MESSAGE_INFO, "collect %d instances (%d), interrupted because %s", numCollectedInstances,
                    // numberToCollect, Time::now<Time::Fast>() > t ? "timeout":"all collected");
#endif
                }
            }
        ...
        catch (Aya::base_exception& e)
        {
            ...
            Aya::StandardOut::singleton()->printf(Aya::MESSAGE_ERROR, "Network Data stream: %s", what ? what : "empty error string");
...
int Replicator::StreamJob::sendItems(Container& itemList, int maxInstances, Time syncTime)
{
    ...
    while (!bitStreamFull && itemList.size())
    {
        ...
        FASTLOG4(DFLog::PartStreamingRequests, "Sent %d items, %d pending requests remain, %d collected remain, bitstream full: %d",
            item->getInstancesSentLastWrite(), instancesToWrite, numCollectedInstances, bitStreamFull);
...
void Replicator::StreamJob::clearPendingItems()
{
    ...
    AYAASSERT(pendingStreamItems.size() == 0);
    numCollectedInstances = 0;
}
...
bool Replicator::StreamJob::collectPartsFromNextRegion(bool highPriority)
{
    ...
    if (highPriority)
    {
        pendingHighPriorityStreamItems.insert(
            std::make_pair(static_cast<float>(StreamRegion::Id::getRegionLongestAxisDistance(regionIterator.getCenterRegion(), regionId)), item));
    }
    else
    {
...
void Replicator::StreamJob::onPlayerTorsoChanged(const Reflection::PropertyDescriptor* desc)
{
    ...
    if (*desc == PartInstance::prop_CFrame)
    {
        ...
        {
            FASTLOG3(FLog::NetworkStreaming, "torso changed to (%d,%d,%d)", (int)torso->getCoordinateFrame().translation.x,
                (int)torso->getCoordinateFrame().translation.y, (int)torso->getCoordinateFrame().translation.z);
...
void Replicator::StreamJob::coarsePrimitiveMovement(Primitive* p, const UpdateInfo& info)
{
    ...
    {
        FASTLOGS(FLog::NetworkStreaming, "Ignoring region transition for part %s because it is a child of character", instance->getName().c_str());
    ...
    if (newRegionExtents.intersectsContainer(collectedRegions, &foundRegionId))
    {
        ...
        {
            FASTLOGS(FLog::NetworkStreaming, "Sending %s in special packet because it entered a sent region", instance->getName().c_str());
            ...
            pendingHighPriorityStreamItems.insert(std::make_pair(
                static_cast<float>(StreamRegion::Id::getRegionLongestAxisDistance(regionIterator.getCenterRegion(), foundRegionId)), item));
        }
