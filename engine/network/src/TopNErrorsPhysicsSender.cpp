...
void TopNErrorsPhysicsSender::step()
{
    ...
    */
    ...
    while (iter != nuggetList.end())
    {
        ...
        if (!DFFlag::PhysicsSenderSleepingUpdate)
        {
            if (!nugget->part->PhysicsServiceHook::is_linked())
            {
                ...
                NuggetMap::iterator mapIter =
                    nuggetMap.find(nugget->part); // need to do find() here because boost unordered_map invalidates iterators during insert
                AYAASSERT(mapIter != nuggetMap.end());
    ...
    std::nth_element(nuggetList.begin(), nuggetList.begin() + static_cast<int>(replicator.physicsSenderStats().physicsItemsPerPacket.value()),
        nuggetList.end(), Nugget::compError);

    FASTLOG2(FLog::PhysicsSenderSleepingLog, "Asleep: %d Total: %d", numAsleep, (int)nuggetList.size());
}

int TopNErrorsPhysicsSender::sendPacket(int maxPackets, PacketPriority packetPriority, ReplicatorStats::PhysicsSenderStats* stats)
{
    ...
    while (iter != nuggetList.end())
    {
        ...
        if (!bitStream)
        {
            ...
            *bitStream << (unsigned char)ID_TIMESTAMP;
            ...
            if (!SFFlag::getPhysicsPacketSendWorldStepTimestamp() && DFFlag::PhysicsSenderUseOwnerTimestamp &&
                nugget->part->computeNetworkOwnerIsSomeoneElse())
            {
                // retrieve the original packet send time of the part
                AYAASSERT(timestamp >= nugget->part->raknetTime);
            ...
            *bitStream << (unsigned char)ID_PHYSICS;
            ...
            if (SFFlag::getPhysicsPacketSendWorldStepTimestamp())
            {
                ...
                else
                {
                    ...
                    interpolationTimeStamp = (RakNet::Time)((TaskScheduler::singleton().lastCyclcTimestamp +
                                                                Time::Interval(replicator.getRakOffsetTime() - timeOffsetDueToPhysics))
                                                                .timestampSeconds() *
                                                            1000.0);
                }
...
#else
                *bitStream << static_cast<unsigned long long>(interpolationTimeStamp);
#endif
            }
        }
        ...
        {
            AYAASSERT(nugget->part->getConstPartPrimitive()->getConstAssembly());

            if (!DFFlag::PhysicsSenderSleepingUpdate)
            {
                nugget->onSent(currentStepTimestamp, sentCFrame, accumulatedError);
                AYAASSERT((*iter)->lastSent == sentCFrame);
...
bool TopNErrorsPhysicsSender::sendPhysicsData(
    RakNet::BitStream& bitStream, const Nugget& nugget, CoordinateFrame& outLastSendCFrame, float& accumulatedError, const PartInstance* playerHead)
{
    sendDetailed = nugget.sendDetailed;
    return PhysicsSender::sendPhysicsData(
        bitStream, nugget.part.get(), sendDetailed, nugget.lastSendTime, outLastSendCFrame, accumulatedError, playerHead);
}

void TopNErrorsPhysicsSender::writeAssembly(
...
float TopNErrorsPhysicsSender::Nugget::minDistance()
{
    return 9.0f;
}
float TopNErrorsPhysicsSender::Nugget::maxDistance()
{
    return 1000000.0f;
}
float TopNErrorsPhysicsSender::Nugget::minSize()
{
    return 2.0f;
}
float TopNErrorsPhysicsSender::Nugget::maxSize()
{
    return 50.0f;
}
...
void TopNErrorsPhysicsSender::Nugget::computeError(
    const CoordinateFrame& focus, const ModelInstance* focusModel, int stepId, const TopNErrorsPhysicsSender* sender)
{
...
void TopNErrorsPhysicsSender::writePV(
    RakNet::BitStream& bitStream, const Assembly* a, Compressor::CompressionType compressionType, bool crossPacketCompression)
{
...
bool TopNErrorsPhysicsSender::writeMovementHistory(RakNet::BitStream& bitStream, const MovementHistory& history, const Assembly* assembly,
    ...
    float& accumulatedError, const PartInstance* playerHead)
{
    ...
    if (history.hasHistory(accumulatedError) ||
        Math::sumDeltaAxis(currentCFrame.rotation, lastSendCFrame.rotation) > DFInt::PhysicsSenderRotationThresholdThousandth / 1000.0f)
    {
        hasMovement = true;
        bitStream << true; // has movement

        bool crossPacketCompression = (!replicator.isStreamingEnabled()) && history.getNumNodes() > 2 &&
                                      compressionType != Compressor::UNCOMPRESSED && accumulatedError <= MH_TOLERABLE_COMPRESSION_ERROR &&
                                      (!lastSendTime.isZero()) && (!lastSendCFrame.translation.isZero());
        if (crossPacketCompression)
        {
        ...
        history.getMovementNodeList(
            lastSendTime, currentStepTimestamp, movementNodeList, crossPacketCompression, lastSendCFrame, calculatedCFrame, calculatedLinearVelocity);
        ...
        if (crossPacketCompression)
        {
            ...
            // if the part is within the client's simulation radius, check the velocity to make sure it's accurate enough for the client to take over
            // the ownership
            ...
            if (positionalError > MH_TOLERABLE_COMPRESSION_ERROR || isEstimatedVelocaityInaccurate || isPvOverThreshold)
            {
                ...
                if (positionalError > MH_TOLERABLE_COMPRESSION_ERROR)
                {
                    StandardOut::singleton()->printf(
                        MESSAGE_INFO, "Calculated positional error (%f) too large, send full CFrame instead ", positionalError);
                }
...
bool TopNErrorsPhysicsSender::isSleepingRootPrimitive(const PartInstance* part)
{
    ...
    // Search for:  AYAASSERT(getConstMechanismRootMovingPart(part) == part)
