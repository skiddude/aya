...
void serializeEnumIndex(
    const Reflection::EnumDescriptor* enumDesc, const size_t& index, RakNet::BitStream& bitStream, size_t enumSizeMSB /*default to 0*/)
{
    AYAASSERT(index < enumDesc->getEnumCount());
    if (enumSizeMSB == 0)
    {
...
void deserializeEnumIndex(
    const Reflection::EnumDescriptor* enumDesc, size_t& index, RakNet::BitStream& bitStream, size_t enumSizeMSB /*default to 0*/)
{
    if (enumSizeMSB == 0)
    {
        enumSizeMSB = enumDesc->getEnumCountMSB();
    ...
    if (index >= enumDesc->getEnumCount())
    {
        ...
        StandardOut::singleton()->printf(Aya::MESSAGE_ERROR,
            "Enum value overflow on %s, size %d, index %d. Set to 0. (Are you using an outdated client?)", enumDesc->name.c_str(),
            (int)enumDesc->getEnumCount(), (int)index);
        index = 0;
    }
}

void serializeEnum(
    const Reflection::EnumDescriptor* enumDesc, const Reflection::Variant& value, RakNet::BitStream& bitStream, size_t enumSizeMSB /*default to 0*/)
{
...
void deserializeEnum(
    const Reflection::EnumDescriptor* enumDesc, Reflection::Variant& result, RakNet::BitStream& bitStream, size_t enumSizeMSB /*default to 0*/)
{
    size_t index = 0;
    deserializeEnumIndex(enumDesc, index, bitStream, enumSizeMSB);
    if (!enumDesc->convertToValue(index, result))
        throw Aya::network_stream_exception("deserializeEnum conversion failed");
}
...
void serializeStringCompressed(const std::string& value, RakNet::BitStream& stream)
{
    ...
    if (size > MAX_STRING_SIZE)
        throw Aya::network_stream_exception(Aya::format("BitStream string write: String too long: %u", size));
...
void deserializeStringCompressed(std::string& value, RakNet::BitStream& stream)
{
    ...
    if (size > MAX_STRING_SIZE)
        throw Aya::network_stream_exception(
            Aya::format("BitStream >> std::string: Bad string length: %d, bit pos: %d", (int)size, stream.GetReadOffset()));
...
template<>
RakNet::BitStream& operator>>(RakNet::BitStream& stream, Aya::Guid::Scope& value)
{
...
RakNet::BitStream& operator<<(RakNet::BitStream& stream, const BinaryString& value)
{
    ...
    if (size > MAX_BINARY_STRING_SIZE)
        throw Aya::network_stream_exception(Aya::format("BitStream string write: BinaryString too long: %u", size));
...
RakNet::BitStream& operator>>(RakNet::BitStream& stream, BinaryString& value)
{
    ...
    uint32_t size;
    if (!stream.Read(size))
        throw Aya::network_stream_exception("BitStream >> BinaryString: failed to read length");

    if (size > MAX_BINARY_STRING_SIZE)
        throw Aya::network_stream_exception("BitStream >> BinaryString: Bad string length");
...
RakNet::BitStream& operator<<(RakNet::BitStream& stream, const Aya::ContentId& value)
{
...
template<>
RakNet::BitStream& operator>>(RakNet::BitStream& stream, Aya::ContentId& value)
{
    std::string text;
    stream >> text;
...
{
    AYAASSERT_VERY_FAST(G3D::isFinite(value.r));
    AYAASSERT_VERY_FAST(G3D::isFinite(value.g));
    AYAASSERT_VERY_FAST(G3D::isFinite(value.b));
...
{
    AYAASSERT_VERY_FAST(G3D::isFinite(value.x));
    AYAASSERT_VERY_FAST(G3D::isFinite(value.y));
    AYAASSERT_VERY_FAST(G3D::isFinite(value.z));
...
{
    AYAASSERT_FISHING(G3D::isFinite(value.x));
    AYAASSERT_FISHING(G3D::isFinite(value.y));
...
RakNet::BitStream& operator<<(RakNet::BitStream& stream, const StreamRegion::Id& value)
{
    ...
    if (vv.x <= 127 && vv.y <= 127 && vv.z <= 127 && vv.x >= -128 && vv.y >= -128 && vv.z >= -128)
    {
...
RakNet::BitStream& operator<<(RakNet::BitStream& stream, const G3D::Vector3& value)
{
    AYAASSERT_FISHING(G3D::isFinite(value.x));
    AYAASSERT_FISHING(G3D::isFinite(value.y));
    AYAASSERT_FISHING(G3D::isFinite(value.z));
...
template<>
RakNet::BitStream& operator>>(RakNet::BitStream& stream, Aya::Velocity& value)
{
    stream >> value.linear;
    stream >> value.rotational;
    return stream;
...
RakNet::BitStream& operator<<(RakNet::BitStream& stream, const G3D::CoordinateFrame& cf)
{
    ...
    else
    {
        ...
        AYAASSERT_VERY_FAST(G3D::isFinite(q.w));
        AYAASSERT_VERY_FAST(G3D::isFinite(q.x));
        AYAASSERT_VERY_FAST(G3D::isFinite(q.y));
        AYAASSERT_VERY_FAST(G3D::isFinite(q.z));
...
RakNet::BitStream& operator>>(RakNet::BitStream& stream, G3D::CoordinateFrame& value)
{
    ...
    else
    {
        ...
        if (!stream.ReadNormQuat(q.w, q.x, q.y, q.z))
            throw Aya::network_stream_exception("BitStream >> CoordinateFrame ReadNormQuat failed");
#else
        // Orientation quaternions are unit quaternions, so max and min are 1 and -1.
        // WriteNormQuat (if using LOSSY_QUAT) uses 6 bytes + 4 bits
        // Straight streaming of 4 floats uses 16 bytes
...
RakNet::BitStream& operator>>(RakNet::BitStream& stream, TweenInfo& twnInfo)
{
    ...
    // Convert string to Aya::Name, then to enum
    const Aya::Name& styleName = Aya::Name::lookup(styleStr.c_str());
    const Aya::Name& directionName = Aya::Name::lookup(directionStr.c_str());
...
template<>
void serialize<Aya::ContentId>(const ConstProperty& property, RakNet::BitStream& bitStream)
{
...
template<>
void serialize<Aya::RbxRay>(const ConstProperty& property, RakNet::BitStream& bitStream)
{
    bitStream << property.getValue<Aya::RbxRay>();
}

template<>
void deserialize<Aya::RbxRay>(Property& property, RakNet::BitStream& bitStream)
{
    RbxRay c;
    bitStream >> c;
    if (property.getInstance())
...
template<>
void deserialize<Aya::ContentId>(Property& property, RakNet::BitStream& bitStream)
{
    std::string value;
    bitStream >> value;
    if (property.getInstance())
...
void serializeGuidScope(RakNet::BitStream& stream, const Aya::Guid::Scope& value, bool canDisableCompression)
{
...
void deserializeGuidScope(RakNet::BitStream& stream, Aya::Guid::Scope& value, bool canDisableCompression)
{
    std::string str;
    if (canDisableCompression)
    {
...
void IdSerializer::serializeId(RakNet::BitStream& stream, const Aya::Guid::Data& id)
{
...
void IdSerializer::serializeIdWithoutDictionary(RakNet::BitStream& stream, const Aya::Guid::Data& id)
{
    if (id.scope.isNull())
    {
...
void IdSerializer::deserializeId(RakNet::BitStream& stream, Aya::Guid::Data& id)
{
...
void IdSerializer::deserializeIdWithoutDictionary(RakNet::BitStream& stream, Aya::Guid::Data& id)
{
    unsigned char code = 0;
...
void IdSerializer::resolvePendingReferences(Instance* instance, Aya::Guid::Data id)
{
...
bool IdSerializer::deserializeInstanceRef(RakNet::BitStream& stream, shared_ptr<Instance>& instance, Aya::Guid::Data& id)
{
    deserializeId(stream, id);
    bool answer = guidRegistry->lookupByGuid(id, instance);
    AYAASSERT(!instance || !ServiceProvider::findServiceProvider(instance.get()) ||
              (ServiceProvider::findServiceProvider(instance.get()) == ServiceProvider::findServiceProvider(this)));

    return answer;
}

void IdSerializer::addPendingRef(const Reflection::RefPropertyDescriptor* desc, boost::shared_ptr<Instance> instance, Aya::Guid::Data id)
{
...
{
    const Aya::Name& n = Aya::Name::lookup(s);
...
void DescriptorReceiver<EventDescriptor>::learnName(std::string s, int i, uint32_t checksum)
{
    ...
    const Aya::Name& n = Aya::Name::lookup(words[0]);
