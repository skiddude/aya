...
public:
    ...
    bool insert(const boost::uint64_t& id, const G3D::Vector2int16& size, unsigned dataPitch, unsigned char* data, const TextureRegion*& outRegion);

    const G3D::Vector2int16& getLastFailedSize()
    {
        return lastFailedSize;
    }
...
private:
    ...
    G3D::Vector2int16 lastFailedSize;
    ...
    struct Interval1D
    {
        Interval1D(unsigned min, unsigned max)
            : min(min)
            , max(max)
        {
        }
