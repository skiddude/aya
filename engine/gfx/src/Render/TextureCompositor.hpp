...
struct TextureCompositorLayer
{
    ...
    TextureCompositorLayer(const MeshId& mesh, const TextureId& texture, Vector2 offset, CompositMode mode = Composit_BlendTexture)
        : mesh(mesh)
        , texture(texture)
    ...
    TextureCompositorLayer(const MeshId& mesh, const Color3& color, Vector2 offset)
        : mesh(mesh)
        , color(color)
        , offset(offset)
...
public:
    ...
    JobHandle getJob(const std::string& textureid, const std::string& context, unsigned int width, unsigned int height, const Vector2& canvasSize,
        const std::vector<TextureCompositorLayer>& layers);
    ...
    const TextureCompositorConfiguration& getConfiguration() const
    {
        return config;
    }
