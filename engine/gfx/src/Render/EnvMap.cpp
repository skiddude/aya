...
EnvMap::EnvMap(VisualEngine* ve)
    : Resource(ve->getDevice())
{
    ...
    texture = dev->createTexture(Texture::Type_Cube, Texture::Format_RGBA8, kCubemapSize, kCubemapSize, 1,
        Texture::getMaxMipCount(kCubemapSize, kCubemapSize, 1), Texture::Usage_Renderbuffer);
    for (int j = 0; j < 6; ++j)
    {
...
void EnvMap::update(DeviceContext* context, double gameTime)
{
    ...
    {
        double realTime = Aya::Time::nowFastSec();
        if (visualEngine->getSettings()->getEagerBulkExecution())
        {
            markDirty(true);
        }
        else if (fabs(envmapLastTimeOfDay - gameTime) > kEnvmapGameTimePeriod ||
                 fabs(envmapLastRealTime - realTime) > kEnvmapRealtimePeriod) // extra refresh at midnight - who cares
        {
