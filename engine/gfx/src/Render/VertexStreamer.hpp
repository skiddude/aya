...
public:
    ...
    void rectBlt(const shared_ptr<Texture>& texptr, const Color4& color4, const Vector2& x0y0, const Vector2& x1y0, const Vector2& x0y1,
        const Vector2& x1y1, const Vector2& tex0, const Vector2& tex1, BatchTextureType batchTexType, bool ignoreTexture = false);
    ...
    void spriteBlt3D(const shared_ptr<Texture>& texptr, const Color4& color4, BatchTextureType batchTexType, const Vector3& x0y0, const Vector3& x1y0,
        const Vector3& x0y1, const Vector3& x1y1, const Vector2& tex0, const Vector2& tex1, int zIndex, bool alwaysOnTop);
...
private:
    ...
    struct Vertex
    {
        Vertex(const Aya::Vector3& pos, unsigned int color, const Aya::Vector2& uv)
            : pos(pos)
            , color(color)
            , uv(uv)
        {
    ...
    struct VertexChunk
    {
        VertexChunk(const shared_ptr<Texture>& texture, Geometry::Primitive primitive, unsigned int vertexStart, unsigned int vertexCount,
            BatchTextureType batchTexType, int zIndex, bool forceTop)
            : texture(texture)
            , primitive(primitive)
    ...
    VertexChunk* prepareChunk(const shared_ptr<Texture>& texPtr, Geometry::Primitive primitive, unsigned int vertexCount,
        CoordinateSpace coordinateSpace, BatchTextureType batchTextureType, bool ignoreTexture = false, int zIndex = -1, bool forceTop = false);
    void renderInternal(
        DeviceContext* context, CoordinateSpace coordinateSpace, const RenderCamera& camera, RenderPassStats& stats, int renderIndex = -1);
};
} // namespace Graphics
} // namespace Aya