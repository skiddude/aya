...
public:
    ...
    void add(const MeshId& mesh, const ContentId& texture, Vector2 offset,
        TextureCompositorLayer::CompositMode mode = TextureCompositorLayer::Composit_BlendTexture)
    {
        layers.push_back(TextureCompositorLayer(mesh, texture, offset, mode));
        ...
        nameAppend(mode);
        nameAppend(":");
        nameAppend(offset.toString());
        nameAppend("]");
    }

    void add(const MeshId& mesh, const BrickColor& color, Vector2 offset)
    {
        layers.push_back(TextureCompositorLayer(mesh, color.color3(), offset));
...
private:
    ...
    void nameAppend(const Vector2& value)
    {
        char buf[32];
...
static float getAccoutrementQuality(Accoutrement* acc, PartInstance* part)
{
    ...
    if (attachmentTop < 0.f || attachmentBottom < -1.75f)
        return 0.f;
...
static std::pair<bool, G3D::Vector4> getPartCompositConfiguration(
    PartInstance* part, const HumanoidIdentifier& hi, const AccoutrementMeshes& accoutrements)
{
    // DO NOT mess around with these values.
    // For now they are hardcoded until I figure out the calculations required to achieve these results.
    // @mdolli

    if (hi.upperTorso == part || hi.lowerTorso == part)
        return std::make_pair(true, G3D::Vector4(0.0f, 0.0f, 0.3789f, 0.5313f));

    if (hi.leftLowerArm == part || hi.leftUpperArm == part || hi.leftHand == part)
        return std::make_pair(true, G3D::Vector4(0.4844f, 0.0f, 0.2578f, 0.556640625f));

    if (hi.rightLowerArm == part || hi.rightUpperArm == part || hi.rightHand == part)
        return std::make_pair(true, G3D::Vector4(0.7422f, 0.0f, 0.2578f, 0.556640625f));

    if (hi.leftLowerLeg == part || hi.leftUpperLeg == part || hi.leftFoot == part)
        return std::make_pair(true, G3D::Vector4(0.4844f, 0.44921875f, 0.2578f, 0.5469f));

    if (hi.rightLowerLeg == part || hi.rightUpperLeg == part || hi.rightFoot == part)
        return std::make_pair(true, G3D::Vector4(0.7422f, 0.44921875f, 0.2578f, 0.5469f));
    ...
    if (!hi.humanoid->getUseR15() && slot >= 0)
    {
        const G3D::Vector4& cfg = kTextureCompositSlotConfiguration[slot];
        G3D::Vector4 borderAdjustment(kTextureCompositExtraBorderWidth, kTextureCompositExtraBorderHeight, -2.f * kTextureCompositExtraBorderWidth,
            -2.f * kTextureCompositExtraBorderHeight);
...
static MeshId getExtraSlotMeshId(PartInstance* part, const HumanoidIdentifier& hi, const AccoutrementMeshes& accoutrements)
{
    ...
    return MeshId(format("ayaasset://avatar/compositing/CompositExtraSlot%d.mesh", slotId));
}

static void prepareHumanoidTextureCompositing(
    TextureCompositingDescription& desc, const HumanoidIdentifier& hi, const AccoutrementMeshes& accoutrements, CharacterMesh* mesh)
{
    if (!hi.humanoid->getUseR15())
    {
        if (hi.torso)
            desc.add(MeshId("ayaasset://avatar/compositing/CompositTorsoBase.mesh"), hi.torso->getColor(), Vector2());

        if (hi.leftArm)
            desc.add(MeshId("ayaasset://avatar/compositing/CompositLeftArmBase.mesh"), hi.leftArm->getColor(), Vector2());

        if (hi.rightArm)
            desc.add(MeshId("ayaasset://avatar/compositing/CompositRightArmBase.mesh"), hi.rightArm->getColor(), Vector2());

        if (hi.leftLeg)
            desc.add(MeshId("ayaasset://avatar/compositing/CompositLeftLegBase.mesh"), hi.leftLeg->getColor(), Vector2());

        if (hi.rightLeg)
            desc.add(MeshId("ayaasset://avatar/compositing/CompositRightLegBase.mesh"), hi.rightLeg->getColor(), Vector2());
    }
    ...
    if (hi.head)
    {
        ...
        desc.add(slotMeshId, hi.head->getColor(), Vector2());

        if (headMesh && !headMesh->getTextureId().isNull())
            desc.add(slotMeshId, headMesh->getTextureId(), Vector2());
            ...
            for (size_t i = children.size(); i > 0; --i)
                if (Decal* decal = Instance::fastDynamicCast<Decal>(children[i - 1].get()))
                    if (decal->getFace() == NORM_Z_NEG && !decal->getTexture().isNull())
                        desc.add(slotMeshId, decal->getTexture(), Vector2());
        }
    }

    if (mesh && !mesh->getBaseTextureId().isNull())
        desc.add(MeshId("ayaasset://avatar/compositing/CompositFullAtlasBaseTexture.mesh"), mesh->getBaseTextureId(), Vector2());

    if (hi.humanoid->getUseR15())
    {
        if (hi.upperTorso && hi.lowerTorso)
            desc.add(MeshId("ayaasset://avatar/compositing/R15CompositTorsoBase.mesh"), hi.upperTorso->getColor(), Vector2(0.0f, 240.0f));

        if (!hi.shirt.isNull())
            desc.add(MeshId("ayaasset://avatar/compositing/R15CompositTorsoBase.mesh"), hi.shirt, Vector2(0.0f, 240.0f));

        if (hi.leftLowerLeg && hi.leftUpperLeg && hi.leftFoot)
            desc.add(MeshId("ayaasset://avatar/compositing/R15CompositLeftArmBase.mesh"), hi.leftUpperLeg->getColor(), Vector2(496, 0.0f));

        if (!hi.pants.isNull())
            desc.add(MeshId("ayaasset://avatar/compositing/R15CompositLeftArmBase.mesh"), hi.pants, Vector2(496, 0.0f));

        if (hi.rightLowerLeg && hi.rightUpperLeg && hi.rightFoot)
            desc.add(MeshId("ayaasset://avatar/compositing/R15CompositRightArmBase.mesh"), hi.rightUpperLeg->getColor(), Vector2(760.0f, 0.0f));

        if (!hi.pants.isNull())
            desc.add(MeshId("ayaasset://avatar/compositing/R15CompositRightArmBase.mesh"), hi.pants, Vector2(760.0f, 0.0f));

        if (hi.leftLowerArm && hi.leftUpperArm && hi.leftHand)
            desc.add(MeshId("ayaasset://avatar/compositing/R15CompositLeftArmBase.mesh"), hi.leftLowerArm->getColor(), Vector2(496.0f, 226.0f));

        if (!hi.shirt.isNull())
            desc.add(MeshId("ayaasset://avatar/compositing/R15CompositLeftArmBase.mesh"), hi.shirt, Vector2(496.0f, 226.0f));

        if (hi.rightLowerArm && hi.rightUpperArm && hi.rightHand)
            desc.add(MeshId("ayaasset://avatar/compositing/R15CompositRightArmBase.mesh"), hi.rightLowerArm->getColor(), Vector2(760.0f, 226.0f));

        if (!hi.shirt.isNull())
            desc.add(MeshId("ayaasset://avatar/compositing/R15CompositRightArmBase.mesh"), hi.shirt, Vector2(760.0f, 226.0f));
    }
    else
    {
        bool applyStuds = (Aya::GameBasicSettings::singleton().getVirtualVersion() == GameBasicSettings::VERSION_2012 && hi.shirtGraphic.isNull());

        if (!hi.pants.isNull())
            desc.add(MeshId("ayaasset://avatar/compositing/CompositPantsTemplate.mesh"), hi.pants, Vector2());
        else if (applyStuds && !hi.getRelevantMesh(hi.leftLeg) && !hi.getRelevantMesh(hi.rightLeg))
            desc.add(MeshId("ayaasset://avatar/compositing/CompositPantsTemplate.mesh"),
                *new ContentId("ayaasset://textures/classic/STUDS_Bottom.png"), Vector2());

        if (!hi.shirt.isNull())
            desc.add(MeshId("ayaasset://avatar/compositing/CompositShirtTemplate.mesh"), hi.shirt, Vector2());
        else if (applyStuds && !hi.getRelevantMesh(hi.torso) && !hi.getRelevantMesh(hi.leftArm) && !hi.getRelevantMesh(hi.rightArm))
            desc.add(MeshId("ayaasset://avatar/compositing/CompositShirtTemplate.mesh"), *new ContentId("ayaasset://textures/classic/STUDS_Top.png"),
                Vector2());

        if (!hi.shirtGraphic.isNull())
            desc.add(MeshId("ayaasset://avatar/compositing/CompositTShirt.mesh"), hi.shirtGraphic, Vector2());
    }

    if (mesh && !mesh->getOverlayTextureId().isNull())
        desc.add(MeshId("ayaasset://avatar/compositing/CompositFullAtlasOverlayTexture.mesh"), mesh->getOverlayTextureId(), Vector2());
    ...
    if (!hi.humanoid->getUseR15())
    {
        for (size_t i = 0; i < kTextureCompositAccoutrementCount; ++i)
        {
            if (accoutrements[i].mesh)
            {
                ...
                desc.add(slotMeshId, accoutrements[i].part->getColor(), Vector2());
                desc.add(slotMeshId, accoutrements[i].mesh->getTextureId(), Vector2(), TextureCompositorLayer::Composit_BlitTextureAlphaMagnify4x);
            }
        }
    }
}

static std::pair<TextureRef, TextureCompositor::JobHandle> createHumanoidTextureComposit(
    VisualEngine* visualEngine, const HumanoidIdentifier& hi, const AccoutrementMeshes& accoutrements, CharacterMesh* mesh)
{
    // God damn it
    Vector2 canvasSize((hi.humanoid->getUseR15() ? 1024.0f : kTextureCompositCanvasWidth), kTextureCompositCanvasHeight);

    TextureCompositingDescription desc;
    prepareHumanoidTextureCompositing(desc, hi, accoutrements, mesh);

    TextureCompositor::JobHandle job = visualEngine->getTextureCompositor()->getJob(
        desc.getName(), hi.humanoid->getFullName() + " Clothes", kTextureCompositWidth, kTextureCompositHeight, canvasSize, desc.getLayers());
    TextureRef texture = visualEngine->getTextureCompositor()->getTexture(job);

    return std::make_pair(texture, job);
}

static std::pair<TextureRef, TextureCompositor::JobHandle> createHumanoidTextureCached(VisualEngine* visualEngine, const HumanoidIdentifier& hi,
    const AccoutrementMeshes& accoutrements, CharacterMesh* mesh, std::pair<Humanoid*, TextureCompositor::JobHandle>& compositCache)
{
...
static std::pair<std::pair<TextureRef, TextureCompositor::JobHandle>, G3D::Vector4> createHumanoidTexture(VisualEngine* visualEngine,
    PartInstance* part, const HumanoidIdentifier& hi, unsigned int flags, std::pair<Humanoid*, TextureCompositor::JobHandle>& compositCache)
{
    ...
    CharacterMesh* mesh =
        (part == hi.upperTorso || part == hi.lowerTorso || part == hi.rightUpperArm || part == hi.rightLowerArm || part == hi.rightHand ||
            part == hi.leftUpperArm || part == hi.leftLowerArm || part == hi.leftHand || part == hi.rightUpperLeg || part == hi.rightLowerLeg ||
            part == hi.rightFoot || part == hi.leftUpperLeg || part == hi.leftLowerLeg || part == hi.leftFoot || part == hi.torso ||
            part == hi.leftArm || part == hi.rightArm || part == hi.leftLeg || part == hi.rightLeg)
    ...
    // making it impossible to use just one composit texture per character. we therefore cache composit texture by humanoid pointer, but only if the
    // mesh has the same configuration as torso
    bool useCache = (mesh == hi.torsoMesh) || (mesh && hi.torsoMesh && mesh->getBaseTextureId() == hi.torsoMesh->getBaseTextureId() &&
                                                  mesh->getOverlayTextureId() == hi.torsoMesh->getOverlayTextureId());

    std::pair<TextureRef, TextureCompositor::JobHandle> texture =
        useCache ? createHumanoidTextureCached(visualEngine, hi, accoutrements, mesh, compositCache)
...
static int getMaterialId(PartMaterial material, bool reflectance)
{
    ...
    {
    case PLASTIC_MATERIAL:
        return 0 + reflectance;
    case SMOOTH_PLASTIC_MATERIAL:
        return 2 + reflectance;
    case WOOD_MATERIAL:
        return 4;
    case WOODPLANKS_MATERIAL:
        return 5;
...
static const std::string kTextureExtension = ".jxl";

// @mdolli
static void setupStencilShadowTechnique(Technique& technique, Technique& technique2)
{
    return;
}
...
static void setupTechnique(Technique& technique, unsigned int flags, bool hasGlow = false)
{
    /* BLENDING and Glow
       ...
       Non Glow-Transparent
        - alpha = src.alpha * (1 - dst.alpha) + dst // imagine this as dst + lerp(src.alpha, 0, dst.alpha). Higher values of src.alpha decreases glow
       intensity more then smaller ones.
     */
    ...
    if (flags & MaterialGenerator::Flag_Transparent)
    {
        ...
        if (hasGlow) // this is regular alpha blend. Src.a is just inverted for reasons explained in comment couple lines above.
            technique.setBlendState(
                BlendState(BlendState::Factor_InvSrcAlpha, BlendState::Factor_SrcAlpha, BlendState::Factor_Zero, BlendState::Factor_SrcAlpha));
        else*/
        technique.setBlendState(
            BlendState(BlendState::Factor_SrcAlpha, BlendState::Factor_InvSrcAlpha, BlendState::Factor_InvDstAlpha, BlendState::Factor_One));
...
static void setupPlasticTextures(VisualEngine* visualEngine, Technique& technique)
{
    ...
    std::string prefix = visualEngine->isClassicRendering() ? "classic_" : "";

    technique.setTexture(5, tm->load(ContentId("ayaasset://textures/plastic/" + prefix + "diffuse.jxl"), TextureManager::Fallback_Gray),
        SamplerState::Filter_Anisotropic);
    technique.setTexture(6, tm->load(ContentId("ayaasset://textures/plastic/" + prefix + "normal.jxl"), TextureManager::Fallback_NormalMap),
        SamplerState::Filter_Anisotropic);
    technique.setTexture(8, tm->load(ContentId("ayaasset://textures/plastic/normaldetail" + kTextureExtension), TextureManager::Fallback_NormalMap),
        SamplerState::Filter_Linear);
}
...
static void setupComplexMaterialTextures(
    VisualEngine* visualEngine, Technique& technique, const std::string& materialName, const TextureRef* wangTileTex)
{
    TextureManager* tm = visualEngine->getTextureManager();
    std::string texturePath = "ayaasset://textures/" + materialName + "/";

    safeToLower(texturePath);
    bool classic = visualEngine->isClassicRendering();
    std::string prefix = classic ? "classic_" : "";

    technique.setTexture(5, tm->load(ContentId(texturePath + prefix + "diffuse" + kTextureExtension), TextureManager::Fallback_White),
        SamplerState::Filter_Anisotropic);

    // on classic rely on default maps for normal and specular, ONLY FOR FABRIC AND SAND
    if (materialName == "fabric" || materialName == "sand")
    {
        prefix = "";
        classic = false;
    }

    technique.setTexture(6, tm->load(ContentId(texturePath + prefix + "normal" + kTextureExtension), TextureManager::Fallback_NormalMap),
        SamplerState::Filter_Anisotropic);
    technique.setTexture(7,
        tm->load(ContentId(texturePath + prefix + "specular" + (classic ? ".dds" : kTextureExtension)), TextureManager::Fallback_Black),
        SamplerState::Filter_Anisotropic); // DDS for classic specular maps since it has mipmaps

    if (wangTileTex)
        technique.setTexture(8, *wangTileTex, SamplerState::Filter_Point);
    else
        technique.setTexture(8, tm->load(ContentId(texturePath + prefix + "normaldetail" + kTextureExtension), TextureManager::Fallback_NormalMap),
            SamplerState::Filter_Anisotropic);
}
...
static void setupLQMaterialTextures(VisualEngine* visualEngine, Technique& technique, const std::string& materialName, const TextureRef* wangTileTex)
{
    ...
    technique.setTexture(
        5, tm->load(ContentId(texturePath + prefix + "diffuse" + kTextureExtension), TextureManager::Fallback_White), SamplerState::Filter_Linear);
...
static void setupMaterialTextures(
    VisualEngine* ve, Technique& technique, PartMaterial renderMaterial, const std::string& materialName, const TextureRef* wangTileTex)
{
...
{
    wangTilesTex = visualEngine->getTextureManager()->load(ContentId("ayaasset://textures/wangIndex.dds"), TextureManager::Fallback_Black);
}

shared_ptr<Material> MaterialGenerator::createStencilShadowMaterial()
{
...
shared_ptr<Material> MaterialGenerator::createBaseMaterial(unsigned int flags)
{
    ...
    if ((flags & (Flag_Transparent | Flag_ForceDecal | Flag_ForceDecalTexture)) == 0)
        if (shared_ptr<ShaderProgram> program =
                visualEngine->getShaderManager()->getProgram("Default" + vertexSkinning + "HQVS", "DefaultHQGBufferFS"))
        {
...
shared_ptr<Material> MaterialGenerator::createRenderMaterial(unsigned int flags, PartMaterial renderMaterial, bool reflectance)
{
    ...
    AYAASSERT(materialId >= 0 && materialId < ARRAYSIZE(renderMaterialCache));
    ...
    ContentId studs(std::string("ayaasset://textures/" + std::string(visualEngine->isClassicRendering() ? "classic/" : "") + "studs.dds"));
    ...
    std::string vertexSkinning = (flags & Flag_Skinned) ? "Skinned" : "Static";
    std::string vertexShader = (renderMaterial == SMOOTH_PLASTIC_MATERIAL || renderMaterial == NEON_MATERIAL)
                                   ? "Default" + vertexSkinning + "HQVS"
                                   : "Default" + vertexSkinning + "SurfaceHQVS";

    if ((flags & Flag_Transparent) == 0)
        if (shared_ptr<ShaderProgram> program =
                visualEngine->getShaderManager()->getProgram(vertexShader, "Default" + materialNameReflectance + "HQGBufferFS"))
        {
            Technique technique(program, 0);
    ...
    {
    case Aya::PLASTIC_MATERIAL:
    case Aya::SMOOTH_PLASTIC_MATERIAL:
    case Aya::NEON_MATERIAL:
    {
    ...
    if (!visualEngine->isClassicRendering())
...
shared_ptr<Material> MaterialGenerator::createTexturedMaterial(const TextureRef& texture, const std::string& textureName, unsigned int flags)
{
    ...
    SamplerState::Filter filter =
        (flags & (Flag_ForceDecal | Flag_ForceDecalTexture)) ? SamplerState::Filter_Anisotropic : SamplerState::Filter_Linear;
...
MaterialGenerator::Result MaterialGenerator::createMaterialForPart(PartInstance* part, const HumanoidIdentifier* hi, unsigned int flags)
{
    ...
    {
        std::pair<std::pair<TextureRef, TextureCompositor::JobHandle>, G3D::Vector4> htp =
            createHumanoidTexture(visualEngine, part, *hi, flags, compositCache);

        if (htp.first.first.getTexture())
        {
            shared_ptr<Material> material =
                createTexturedMaterial(htp.first.first, visualEngine->getTextureCompositor()->getTextureId(htp.first.second), flags);
    ...
    if (FileMesh* fileMesh = getFileMesh(specialShape))
    {
        ...
        TextureRef texture = textureId.isNull() ? TextureRef()
                                                : visualEngine->getTextureManager()->load(
                                                      textureId, TextureManager::Fallback_Gray, fileMesh->getFullName() + ".TextureId");
    ...
    // meshpartinstance
    // don't do this for R15 parts

    if (MeshPartInstance* meshPart = part->fastDynamicCast<MeshPartInstance>())
    {
        if (hi && !hi->humanoid->getUseR15())
        {
            TextureId textureId = TextureId("ayaasset://textures/meshPartFallback.png");
...
MaterialGenerator::Result MaterialGenerator::createMaterialForDecal(Decal* decal, unsigned int flags)
{
    ...
    TextureRef texture = textureId.isNull() ? TextureRef()
                                            : visualEngine->getTextureManager()->load(
                                                  textureId, TextureManager::Fallback_BlackTransparent, decal->getFullName() + ".Texture");

    return texture.getTexture() ? Result(createTexturedMaterial(texture, textureId.toString(), flags), Result_UsesTexture) : Result();
}
...
{
    // erase all entries (so not even shared ptr, its just empty)
    for (unsigned int i = 0; i < ARRAYSIZE(baseMaterialCache); ++i)
        baseMaterialCache[i].reset();

    for (unsigned int i = 0; i < ARRAYSIZE(renderMaterialCache); ++i)
        for (unsigned int j = 0; j < ARRAYSIZE(renderMaterialCache[i]); ++j)
            renderMaterialCache[i][j].reset();
...
void MaterialGenerator::garbageCollectIncremental()
{
    for (unsigned int i = 0; i < ARRAYSIZE(texturedMaterialCache); ++i)
    {
        ...
        size_t visitCount = std::min(cache.map.size(), std::max(cache.map.size(), cache.gcSizeLast) - cache.gcSizeLast + 2048);
...
Vector2int16 MaterialGenerator::getSpecular(PartMaterial material)
{
    ...
    {
    case PLASTIC_MATERIAL:
        return Vector2int16(102, 9);
    case SMOOTH_PLASTIC_MATERIAL:
        return Vector2int16(191, 81);
    case NEON_MATERIAL:
        return Vector2int16(191, 81);
    case WOOD_MATERIAL:
        return Vector2int16(64, 32);
    case WOODPLANKS_MATERIAL:
        return Vector2int16(71, 53);
    case MARBLE_MATERIAL:
        return Vector2int16(179, 54);
    case SLATE_MATERIAL:
        return Vector2int16(36, 20);
    case CONCRETE_MATERIAL:
        return Vector2int16(38, 22);
    case GRANITE_MATERIAL:
        return Vector2int16(48, 24);
    case BRICK_MATERIAL:
        return Vector2int16(33, 44);
    case PEBBLE_MATERIAL:
        return Vector2int16(18, 22);
    case COBBLESTONE_MATERIAL:
        return Vector2int16(54, 22);
    case RUST_MATERIAL:
        return Vector2int16(89, 103);
    case DIAMONDPLATE_MATERIAL:
        return Vector2int16(230, 160);
    case ALUMINUM_MATERIAL:
        return Vector2int16(238, 240);
    case METAL_MATERIAL:
        return Vector2int16(204, 120);
    case GRASS_MATERIAL:
        return Vector2int16(43, 18);
    case SAND_MATERIAL:
        return Vector2int16(18, 6);
    case FABRIC_MATERIAL:
        return Vector2int16(8, 16);
    case ICE_MATERIAL:
        return Vector2int16(255, 190);

    default:
        AYAASSERT(0); // You missed new material
        return Vector2int16(0, 50);
    }
}

unsigned int MaterialGenerator::createFlags(bool skinned, Aya::PartInstance* part, const HumanoidIdentifier* hi, bool& ignoreDecalsOut)
{
