...
#if (defined(AYA_PLATFORM_IOS) && !TARGET_IPHONE_SIMULATOR) || defined(__ANDROID__) || defined(_M_ARM64) || defined(__aarch64__)
#include <arm_neon.h>
#endif

LOGVARIABLE(RenderLightGrid, 1)

LOGVARIABLE(RenderLightGridAgeProportion, 1)
LOGVARIABLE(RenderLightGridBorderGlobalCutoff, 256)
LOGVARIABLE(RenderLightGridBorderSkylightCutoff, 256)

#if (defined(_WIN32) || (!defined(AYA_PLATFORM_IOS))) && !defined(_M_ARM64) && !defined(__aarch64__)
#define SIMD_SSE2
#endif

#if (defined(AYA_PLATFORM_IOS) && !TARGET_IPHONE_SIMULATOR) || defined(__APPLE__) || defined(__ANDROID__) || defined(_M_ARM64)
#define SIMD_NEON
#endif
...
const int kLightGridDirectionalDirtyThreshold = 32;
const int kLightGridSkylightDirtyThreshold = 32;
const int kLightGridShadowDirtyThreshold = 32;
...
void fillLightingBorders(
    LightingData& lighting, const Vector3int32& index, const Vector3int32& corner, const Vector3int32& size, const Color3uint8& skyAmbient)
{
    if (index.x == corner.x)
...
LightGridChunk* LightGrid::getChunkByLocalIndex(const Vector3int32& index)
{
    ...
    AYAASSERT(offset < chunks.size());
    return chunks[offset];
}

bool LightGrid::isChunkInsideGridLocal(const Vector3int32& index)
{
    return static_cast<unsigned int>(index.x) < static_cast<unsigned int>(chunkCount.x) &&
...
{
    AYAASSERT(chunkCount.x > 0 && chunkCount.y > 0 && chunkCount.z > 0);
    ...
    if (textureMode != Texture_None)
    {
        ...
        {
            texture = visualEngine->getDevice()->createTexture(Texture::Type_3D, Texture::Format_RGBA8, chunkCount.x * kLightGridChunkSizeXZ,
                chunkCount.z * kLightGridChunkSizeXZ, chunkCount.y * kLightGridChunkSizeY, 1, Texture::Usage_Dynamic);
        }
        else
        {
            ...
            AYAASSERT(gridDepthSqrt * gridDepthSqrt == gridDepth);

            texture = visualEngine->getDevice()->createTexture(Texture::Type_2D, Texture::Format_RGBA8,
                chunkCount.x * kLightGridChunkSizeXZ * gridDepthSqrt, chunkCount.z * kLightGridChunkSizeXZ * gridDepthSqrt, 1, 1,
                Texture::Usage_Dynamic);
            ...
            if (visualEngine->getDevice()->getCaps().supportsTextureHalfFloat)
            {
                ...
                lookupTexture =
                    visualEngine->getDevice()->createTexture(Texture::Type_2D, Texture::Format_RGBA16F, gridDepth, 1, 1, 1, Texture::Usage_Static);
            ...
            else
            {
                ...
                lookupTexture =
                    visualEngine->getDevice()->createTexture(Texture::Type_2D, Texture::Format_RGBA8, gridDepth, 1, 1, 1, Texture::Usage_Static);
...
LightGrid::LightGrid(
    VisualEngine* visualEngine, const shared_ptr<Texture>& texture, const shared_ptr<Texture>& lookupTexture, const Vector3int32& chunkCount)
...
inline bool isPartFixed(Aya::PartInstance* part)
{
...
void LightGrid::occupancyUpdateChunkPrepare(
    Voxel::OccupancyChunk& chunk, MegaClusterInstance* terrain, ContactManager* contactManager, std::vector<DataModelPartCache>& partCache)
{
    AYAPROFILER_SCOPE("Render", "occupancyUpdateChunkPrepare");

    size_t partCacheOffset = partCache.size();

    voxelizer.occupancyUpdateChunkPrepare(chunk, terrain, contactManager, partCache);

    AYAPROFILER_LABELF("Render", "Primitives: %d", int(partCache.size() - partCacheOffset));
}

void LightGrid::occupancyUpdateChunkPerform(const std::vector<DataModelPartCache>& partCache)
{
    AYAPROFILER_SCOPE("Render", "occupancyUpdateChunkPerform");

    voxelizer.occupancyUpdateChunkPerform(partCache);

    AYAPROFILER_LABELF("Render", "Primitives: %d", int(partCache.size()));
}

void LightGrid::lightingUpdateChunkLocal(LightGridChunk& chunk, SpatialHashedScene* spatialHashedScene)
{
    AYAPROFILER_SCOPE("Render", "lightingUpdateChunkLocal");
...
{
    AYAPROFILER_SCOPE("Render", "lightingUpdateChunkGlobal");
...
{
    AYAPROFILER_SCOPE("Render", "lightingUpdateChunkSkylight");
...
{
    AYAPROFILER_SCOPE("Render", "lightingUpdateChunkAverage");
...
void LightGrid::invalidateExtents(const Aya::Extents& extents, unsigned int status)
{
...
bool LightGrid::updateGridCenter(const Vector3& cameraTarget, bool skipChunkUpdate)
{
    ...
    if ((cameraChunkCenterOffset.x == -1 || cameraChunkCenterOffset.x == 0) && (cameraChunkCenterOffset.y == -1 || cameraChunkCenterOffset.y == 0) &&
    ...
    AYAASSERT(newCorner != cornerChunkIndex);
...
void LightGrid::updateAgePriorityForChunks(const Vector3& cameraTarget)
{
    ...
    for (size_t i = 0; i < chunks.size(); ++i)
    {
        ...
        Vector3int32 chunkCenter =
            chunk.index * kLightGridChunkSize + Vector3int32(kLightGridChunkSizeXZ / 2, kLightGridChunkSizeY / 2, kLightGridChunkSizeXZ / 2);

        Vector3int32 diff = Vector3int32(abs(cellPos.x - chunkCenter.x) / kLightGridChunkSizeXZ,
            abs(cellPos.y - chunkCenter.y) / (kLightGridChunkSizeY / 2), abs(cellPos.z - chunkCenter.z) / kLightGridChunkSizeXZ);
...
void LightGrid::updateBorderColor(const Vector3& cameraTarget, const Frustum& cameraFrustum)
{
    ...
    for (size_t i = 0; i < chunks.size(); ++i)
    {
        ...
        bool isBorderXZ = !isChunkInsideGridLocal(chunk.index - cornerChunkIndex - Vector3int32(1, 0, 1)) ||
...
{
    AYAPROFILER_SCOPE("Render", "computeAverageColor");
...
void LightGrid::stepCursor(Vector3int32& cursor)
{
    ...
    AYAASSERT(cursor.max(chunkCount) == chunkCount);
    AYAASSERT(cursor.min(Vector3int32(0, 0, 0)) == Vector3int32(0, 0, 0));
}
...
{
    return Vector3(cornerChunkIndex.x, cornerChunkIndex.y, cornerChunkIndex.z) *
           Vector3(kLightGridChunkSizeXZ, kLightGridChunkSizeY, kLightGridChunkSizeXZ) * 4.f;
}
...
Color4uint8 LightGrid::getBorderColor() const
{
    ...
    return Color4uint8((skyAmbient.r * skylight + 255) >> 8, (skyAmbient.g * skylight + 255) >> 8, (skyAmbient.b * skylight + 255) >> 8, global);
}
...
template<bool NegX, bool NegZ>
void LightGrid::lightingUpdateDirectionalImpl(LightGridChunk& chunk, const Vector3int32& lightContrib)
{
#define IRRADIANCE(x, y, z) irradianceScratch[(y) + 1][(z) + 1][(x) + 1]

    AYAASSERT(lightContrib.x + lightContrib.y + lightContrib.z == 256);
    ...
    for (int i = 0; i < 256; ++i)
        tableXSat[i] = 0;
    for (int i = 0; i < 256; ++i)
        tableXSat[i + 256] = i * lightContrib.x;
#endif
        ...
        for (int zi = 0; zi < kLightGridChunkSizeXZ; ++zi)
        {
            ...
            if (useSIMD)
            {
                ...
                for (int xi = 0; xi < kLightGridChunkSizeXZ; xi += 16)
                {
                    ...
                    __m128i result0 =
                        _mm_add_epi16(round, _mm_add_epi16(_mm_mullo_epi16(incomingY0, contribY), _mm_mullo_epi16(incomingZ0, contribZ)));
                    __m128i result1 =
                        _mm_add_epi16(round, _mm_add_epi16(_mm_mullo_epi16(incomingY1, contribY), _mm_mullo_epi16(incomingZ1, contribZ)));
...
#define IRRADIANCE(x, y, z) irradianceScratch[(y) + 1][(z) + 1][(x) + 1]
    ...
    for (int x = 0; x < kLightGridChunkSizeXZ; ++x)
    {
        ...
        IRRADIANCE(x, kLightGridChunkSizeY, kLightGridChunkSizeXZ) =
            (IRRADIANCE(x, kLightGridChunkSizeY - 1, kLightGridChunkSizeXZ) + IRRADIANCE(x, kLightGridChunkSizeY, kLightGridChunkSizeXZ - 1) + 1) / 2;
    }

    for (int z = 0; z < kLightGridChunkSizeXZ; ++z)
    {
        IRRADIANCE(-1, kLightGridChunkSizeY, z) = (IRRADIANCE(-1, kLightGridChunkSizeY - 1, z) + IRRADIANCE(0, kLightGridChunkSizeY, z) + 1) / 2;
        IRRADIANCE(kLightGridChunkSizeXZ, kLightGridChunkSizeY, z) =
            (IRRADIANCE(kLightGridChunkSizeXZ, kLightGridChunkSizeY - 1, z) + IRRADIANCE(kLightGridChunkSizeXZ - 1, kLightGridChunkSizeY, z) + 1) / 2;
    }
    ...
    for (int y = 0; y < kLightGridChunkSizeY + 1; ++y)
    {
        ...
        IRRADIANCE(kLightGridChunkSizeXZ, y, kLightGridChunkSizeXZ) =
            (IRRADIANCE(kLightGridChunkSizeXZ, y, kLightGridChunkSizeXZ - 1) + IRRADIANCE(kLightGridChunkSizeXZ - 1, y, kLightGridChunkSizeXZ) + 1) /
            2;
    }
...
{
    AYAPROFILER_SCOPE("Render", "lightingGetLights");
...
{
    AYAPROFILER_SCOPE("Render", "lightingUpdateLightScratch");
    ...
    Vector3int32 extentsMax =
        fastFloorInt32((extents.max() - chunkExtents.min()) / 4.f + Vector3(0.99f, 0.99f, 0.99f)).min(kLightGridChunkSize + Vector3int32(1, 1, 1));
    ...
    else if (lobj->getType() == LightObject::Type_Spot)
    {
        if (lobj->getShadowMap())
        {
            ...
            SIMDCALL(lightingUpdateSpotLightScratch,
                <true>(extentsMin, extentsMax, position, range, lobj->getDirection(), lobj->getAngle(), color, intensity));
        }
        else
        {
            SIMDCALL(lightingUpdateSpotLightScratch,
                <false>(extentsMin, extentsMax, position, range, lobj->getDirection(), lobj->getAngle(), color, intensity));
        }
    }
    else if (lobj->getType() == LightObject::Type_Surface)
    {
        ...
        SIMDCALL(lightingUpdateSurfaceLightScratch,
            <false>(extentsMin, extentsMax, position, range, lobj->getDirection(), lobj->getAngle(), axisU, axisV, color, intensity));
    }
    else
    {
        AYAASSERT(false);
...
void LightGrid::lightingComputeShadowMask(const LightGridChunk& chunk, const Vector3int32& extentsMin, const Vector3int32& extentsMax,
    const Vector3& lightPosition, LightShadowMap& shadowMap)
{
    AYAPROFILER_SCOPE("Render", "lightingComputeShadowMask");
    ...
    lightingTransferShadowMaskToShadowSlice<0>(
        chunk, extentsMinChunk, extentsMaxChunk, lightCell, shadowMap.size, shadowMap.sliceX, shadowMap.sliceX);
    lightingTransferShadowMaskToShadowSlice<1>(
        chunk, extentsMinChunk, extentsMaxChunk, lightCell, shadowMap.size, shadowMap.sliceY0, shadowMap.sliceY1);
    lightingTransferShadowMaskToShadowSlice<2>(
        chunk, extentsMinChunk, extentsMaxChunk, lightCell, shadowMap.size, shadowMap.sliceZ, shadowMap.sliceZ);

    // The code above fills all voxels inside the chunk, but for edge cases (light intersects the outer border) it does not fill three faces out of 6
    // of the border, and does not fill any edges or corners in the outer border either. Since blurring needs those let's copy/interpolate them from
...
void LightGrid::lightingFixupShadowMaskBorder(const Vector3int32& extentsMin, const Vector3int32& extentsMax, const Vector3int32& lightCell)
{
    ...
    if (extentsMin.x == 0 || extentsMin.y == 0 || extentsMin.z == 0 || extentsMax.x == kLightGridChunkSizeXZ + 1 ||
        extentsMax.y == kLightGridChunkSizeY + 1 || extentsMax.z == kLightGridChunkSizeXZ + 1)
    {
#define SHADOW(x, y, z) shadowMask[(y) + 1][(z) + 1][(x) + 1]
        ...
        for (int x = 0; x < kLightGridChunkSizeXZ; ++x)
        {
            ...
            SHADOW(x, kLightGridChunkSizeY, kLightGridChunkSizeXZ) =
                (SHADOW(x, kLightGridChunkSizeY - 1, kLightGridChunkSizeXZ) + SHADOW(x, kLightGridChunkSizeY, kLightGridChunkSizeXZ - 1) + 1) / 2;
        ...
        for (int z = 0; z < kLightGridChunkSizeXZ; ++z)
        {
            ...
            SHADOW(kLightGridChunkSizeXZ, kLightGridChunkSizeY, z) =
                (SHADOW(kLightGridChunkSizeXZ, kLightGridChunkSizeY - 1, z) + SHADOW(kLightGridChunkSizeXZ - 1, kLightGridChunkSizeY, z) + 1) / 2;
        ...
        for (int y = -1; y < kLightGridChunkSizeY + 1; ++y)
        {
            ...
            SHADOW(kLightGridChunkSizeXZ, y, kLightGridChunkSizeXZ) =
                (SHADOW(kLightGridChunkSizeXZ, y, kLightGridChunkSizeXZ - 1) + SHADOW(kLightGridChunkSizeXZ - 1, y, kLightGridChunkSizeXZ) + 1) / 2;
        }
...
template<int Axis>
void LightGrid::lightingTransferShadowSliceToShadowMask(const Vector3int32& extentsMin, const Vector3int32& extentsMax, const Vector3int32& lightCell,
    int shadowMapSize, const LightShadowSlice& shadowSlice0, const LightShadowSlice& shadowSlice1)
{
...
template<int Axis>
void LightGrid::lightingTransferShadowMaskToShadowSlice(const LightGridChunk& chunk, const Vector3int32& extentsMin, const Vector3int32& extentsMax,
    const Vector3int32& lightCell, int shadowMapSize, LightShadowSlice& shadowSlice0, LightShadowSlice& shadowSlice1)
{
    if (shadowMapSize == 0 || !shadowSlice0.data || !shadowSlice1.data)
        return;
    ...
    AYAASSERT(!(&shadowSlice0 == &shadowSlice1 && intersectsMin && intersectsMax));
...
void LightGrid::lightingComputeShadowMaskXYZ(const LightGridChunk& chunk, const Vector3int32& extentsMin, const Vector3int32& extentsMax,
    const Vector3int32& lightCell, const Vector3& lightFraction)
{
    if (lightCell.x >= extentsMin.x)
    {
        lightingComputeShadowMaskYZ<true>(
            chunk, extentsMin, Vector3int32(std::min(extentsMax.x, lightCell.x), extentsMax.y, extentsMax.z), lightCell, lightFraction);
    }

    if (lightCell.x < extentsMax.x)
    {
        lightingComputeShadowMaskYZ<false>(
            chunk, Vector3int32(std::max(extentsMin.x, lightCell.x + 1), extentsMin.y, extentsMin.z), extentsMax, lightCell, lightFraction);
    }
}

template<bool NegX>
void LightGrid::lightingComputeShadowMaskYZ(const LightGridChunk& chunk, const Vector3int32& extentsMin, const Vector3int32& extentsMax,
    const Vector3int32& lightCell, const Vector3& lightFraction)
{
    if (lightCell.y >= extentsMin.y)
    {
        lightingComputeShadowMaskZ<NegX, true>(
            chunk, extentsMin, Vector3int32(extentsMax.x, std::min(extentsMax.y, lightCell.y), extentsMax.z), lightCell, lightFraction);
    }

    if (lightCell.y < extentsMax.y)
    {
        lightingComputeShadowMaskZ<NegX, false>(
            chunk, Vector3int32(extentsMin.x, std::max(extentsMin.y, lightCell.y + 1), extentsMin.z), extentsMax, lightCell, lightFraction);
    }
}

template<bool NegX, bool NegY>
void LightGrid::lightingComputeShadowMaskZ(const LightGridChunk& chunk, const Vector3int32& extentsMin, const Vector3int32& extentsMax,
    const Vector3int32& lightCell, const Vector3& lightFraction)
{
    if (lightCell.z >= extentsMin.z)
    {
        lightingComputeShadowMask<NegX, NegY, true>(
            chunk, extentsMin, Vector3int32(extentsMax.x, extentsMax.y, std::min(extentsMax.z, lightCell.z)), lightCell, lightFraction);
    }

    if (lightCell.z < extentsMax.z)
    {
        lightingComputeShadowMask<NegX, NegY, false>(
            chunk, Vector3int32(extentsMin.x, extentsMin.y, std::max(extentsMin.z, lightCell.z + 1)), extentsMax, lightCell, lightFraction);
    }
}
...
template<bool NegX, bool NegY, bool NegZ>
void LightGrid::lightingComputeShadowMaskImplLUT(const LightGridChunk& chunk, const Vector3int32& extentsMin, const Vector3int32& extentsMax,
    const Vector3int32& lightCell, const Vector3& lightFraction)
{
#define SHADOW(x, y, z) shadowMask[(y) + 1][(z) + 1][(x) + 1]
    ...
    AYAASSERT(extentsMin.x >= 0 && extentsMin.y >= 0 && extentsMin.z >= 0);
    AYAASSERT(extentsMax.x < kLightGridChunkSizeXZ && extentsMax.y < kLightGridChunkSizeY && extentsMax.z < kLightGridChunkSizeXZ);
...
template<bool NegX, bool NegY, bool NegZ>
void LightGrid::lightingComputeShadowMaskImplSmooth(const LightGridChunk& chunk, const Vector3int32& extentsMin, const Vector3int32& extentsMax,
    const Vector3int32& lightCell, const Vector3& lightFraction)
{
#define SHADOW(x, y, z) shadowMask[(y) + 1][(z) + 1][(x) + 1]
    ...
    for (int yi = 0; yi <= size.y; ++yi)
    {
        ...
        for (int zi = 0; zi <= size.z; ++zi)
        {
            ...
            for (int xi = 0; xi <= size.x; ++xi)
            {
                ...
                int result = (lutX == 0 && lutY == 0 && lutZ == 0)
                                 ? 255
...
#ifdef SIMD_SSE2
template<bool NegX, bool NegY, bool NegZ>
void LightGrid::lightingComputeShadowMaskImplSmoothSIMD(const LightGridChunk& chunk, const Vector3int32& extentsMin, const Vector3int32& extentsMax,
    const Vector3int32& lightCell, const Vector3& lightFraction)
{
#define SHADOW(x, y, z) shadowMask[(y) + 1][(z) + 1][(x) + 1]
    ...
    __m128 contribXStart1 = _mm_add_ps(NegX ? _mm_setr_ps(4, 3, 2, 1) : _mm_setr_ps(1, 2, 3, 4), _mm_set1_ps(lutOffsetX + fractionX));
...
void LightGrid::lightingUpdatePointLightScratch(const Vector3int32& extentsMin, const Vector3int32& extentsMax, const Vector3& lightPosition,
    float lightRadius, const Color3uint8& lightColor, float lightIntensity)
{
    ...
    for (int y = extentsMin.y; y <= extentsMax.y; ++y)
    {
        for (int z = extentsMin.z; z <= extentsMax.z; ++z)
        {
            for (int x = extentsMin.x; x <= extentsMax.x; ++x)
            {
                ...
                int color =
                    (((lightColor.r * factor) >> 16) << 20) + (((lightColor.g * factor) >> 16) << 10) + (((lightColor.b * factor) >> 16) << 0);
...
#ifdef SIMD_SSE2
template<bool Shadows>
void LightGrid::lightingUpdatePointLightScratchSIMD(const Vector3int32& extentsMin, const Vector3int32& extentsMax, const Vector3& lightPosition,
    float lightRadius, const Color3uint8& lightColor, float lightIntensity_)
{
    ...
    for (int y = extentsMin.y; y <= extentsMax.y; ++y)
    {
        ...
        for (int z = extentsMin.z; z <= extentsMax.z; ++z)
        {
            ...
            for (int x = extentsMin.x; x <= extentsMax.x;
                x += 4) // note: this loop processes a few extra values - it's ok since we pad X in lighting scratch by 4 pixels
            {
...
template<bool Shadows>
void LightGrid::lightingUpdateSpotLightScratch(const Vector3int32& extentsMin, const Vector3int32& extentsMax, const Vector3& lightPosition,
    float lightRadius, const Vector3& lightDirection, float lightConeAngle, const Color3uint8& lightColor, float lightIntensity)
{
    float lightRadiusInvSq = 1 / (lightRadius * lightRadius);
        ...
        for (int z = extentsMin.z; z <= extentsMax.z; ++z)
        {
            for (int x = extentsMin.x; x <= extentsMax.x; ++x)
            {
                ...
                float coneAttenuation =
                    (dot(lightDirection, normalize(cellPosition - lightPosition)) - lightConeHalfAngleCos) * lightConeAttenuationScale;
...
#ifdef SIMD_SSE2
template<bool Shadows>
void LightGrid::lightingUpdateSpotLightScratchSIMD(const Vector3int32& extentsMin, const Vector3int32& extentsMax, const Vector3& lightPosition,
    float lightRadius, const Vector3& lightDirection, float lightConeAngle, const Color3uint8& lightColor, float lightIntensity_)
{
    __m128 zero = _mm_setzero_ps();
    __m128 one = _mm_set1_ps(1.f);
    __m128 half = _mm_set1_ps(0.5f);
...
template<bool Shadows>
void LightGrid::lightingUpdateSurfaceLightScratch(const Vector3int32& extentsMin, const Vector3int32& extentsMax, const Vector3& lightPosition,
    float lightRadius, const Vector3& lightDirection, float lightConeAngle, const Vector4& axisU, const Vector4& axisV, const Color3uint8& lightColor,
    ...
    for (int y = extentsMin.y; y <= extentsMax.y; ++y)
    {
        for (int z = extentsMin.z; z <= extentsMax.z; ++z)
        {
            for (int x = extentsMin.x; x <= extentsMax.x; ++x)
            {
                ...
                float coneAttenuation =
                    (dot(lightDirection, normalize(cellPosition - lightProjectedPosition)) - lightConeHalfAngleCos) * lightConeAttenuationScale;
                float attenuation = saturate(distAttenuation * std::max(0.f, coneAttenuation) * lightIntensity);
...
#ifdef SIMD_SSE2
template<bool Shadows>
void LightGrid::lightingUpdateSurfaceLightScratchSIMD(const Vector3int32& extentsMin, const Vector3int32& extentsMax, const Vector3& lightPosition,
    float lightRadius, const Vector3& lightDirection, float lightConeAngle, const Vector4& axisU, const Vector4& axisV, const Color3uint8& lightColor,
    ...
    for (int y = extentsMin.y; y <= extentsMax.y; ++y)
    {
        ...
        for (int z = extentsMin.z; z <= extentsMax.z; ++z)
        {
                ...
                x += 4) // note: this loop processes a few extra values - it's ok since we pad X in lighting scratch by 4 pixels
            {
                ...
                __m128 cellVectorXProjected =
                    _mm_sub_ps(cellVectorX, _mm_add_ps(_mm_mul_ps(projUDistClamped, axisUX), _mm_mul_ps(projVDistClamped, axisVX)));
                __m128 cellVectorYProjected =
                    _mm_sub_ps(cellVectorY, _mm_add_ps(_mm_mul_ps(projUDistClamped, axisUY), _mm_mul_ps(projVDistClamped, axisVY)));
                __m128 cellVectorZProjected =
                    _mm_sub_ps(cellVectorZ, _mm_add_ps(_mm_mul_ps(projUDistClamped, axisUZ), _mm_mul_ps(projVDistClamped, axisVZ)));

                __m128 cellVectorYZDSqProjected =
                    _mm_add_ps(_mm_mul_ps(cellVectorYProjected, cellVectorYProjected), _mm_mul_ps(cellVectorZProjected, cellVectorZProjected));
                __m128 distSquared = _mm_add_ps(_mm_mul_ps(cellVectorXProjected, cellVectorXProjected), cellVectorYZDSqProjected);
                __m128 distInv = _mm_rsqrt_ps(distSquared);

                __m128 cellVectorDotYZProjected =
                    _mm_add_ps(_mm_mul_ps(cellVectorYProjected, lightDirectionY), _mm_mul_ps(cellVectorZProjected, lightDirectionZ));
    ...
    for (int y = extentsMin.y; y <= extentsMax.y; ++y)
    {
        ...
        for (int z = extentsMin.z; z <= extentsMax.z; ++z)
        {
                ...
                x += 4) // note: this loop processes a few extra values - it's ok since we pad X in lighting scratch by 4 pixels
            {
                ...
                float32x4_t cellVectorYZDSqProjected =
                    vmlaq_f32(vmulq_f32(cellVectorYProjected, cellVectorYProjected), cellVectorZProjected, cellVectorZProjected);
                float32x4_t distSquared = vmlaq_f32(cellVectorYZDSqProjected, cellVectorXProjected, cellVectorXProjected);
                float32x4_t distInv = vrsqrteq_f32(distSquared);

                float32x4_t cellVectorDotYZProjected =
                    vmlaq_f32(vmulq_f32(cellVectorYProjected, lightDirectionY), cellVectorZProjected, lightDirectionZ);
...
template<bool AxisY>
void LightGrid::lightingBlurAxisYZScratch()
{
...
#ifdef SIMD_SSE2
template<bool AxisY>
void LightGrid::lightingBlurAxisYZScratchSIMD()
{
#define LIGHTING(k) (AxisY ? lightingScratch[k][i][j] : lightingScratch[i][k][j])
    ...
    for (int i = 0; i < (AxisY ? kLightGridChunkSizeXZ : kLightGridChunkSizeY) + 2; ++i)
        for (int j = 0; j < kLightGridChunkSizeXZ + 2;
            j += 4) // note: this loop processes a few extra values - it's ok since we pad X in lighting scratch by 4 pixels
        {
...
#define LIGHTING(k) lightingScratch[i][j][k]
    ...
    for (int i = 1; i < kLightGridChunkSizeY + 1; ++i)
        for (int j = 1; j < kLightGridChunkSizeXZ + 1; ++j)
        {
            for (int k = 1; k < kLightGridChunkSizeXZ + 1;
                k += 4) // note: this loop processes a few extra values - it's ok since we pad X in lighting scratch by 4 pixels
            {
...
{
    AYAPROFILER_SCOPE("Render", "lightingComposit");
...
void LightGrid::lightingCompositImpl(
    const LightGridChunk& chunk, unsigned char* data, unsigned int rowPitch, unsigned int slicePitch, const Color3uint8& skyAmbient)
{
...
#ifdef SIMD_SSE2
void LightGrid::lightingCompositImplSIMD(
    const LightGridChunk& chunk, unsigned char* data, unsigned int rowPitch, unsigned int slicePitch, const Color3uint8& skyAmbient)
{
    __m128i zero = _mm_setzero_si128();
    __m128i round = _mm_set1_epi16(255);
...
{
    AYAPROFILER_SCOPE("Render", "lightingUploadChunk");
...
{
    AYAPROFILER_SCOPE("Render", "lightingUploadCommit");
...
void LightGrid::lightingUploadAll()
{
    ...
    if (texture->getType() == Texture::Type_3D && texture->supportsLocking())
    {
        ...
        for (size_t i = 0; i < chunks.size(); ++i)
        {
            ...
            AYAASSERT(locked.rowPitch >= kLightGridChunkSizeXZ * 4);

            unsigned char* dataSlice =
                static_cast<unsigned char*>(locked.data) + locked.slicePitch * gridPos.y + locked.rowPitch * gridPos.z + gridPos.x * 4;
...
{
    AYAPROFILER_SCOPE("Render", "relocateGrid");
        ...
        for (int cz = 0; cz < chunkCount.z; ++cz)
            for (int cx = 0; cx < chunkCount.x; ++cx)
            {
                ...
                if (!chunks[offset])
                {
                    ...
                    chunk->dirty = LightGridChunk::Dirty_Occupancy | LightGridChunk::Dirty_LightingGlobal | LightGridChunk::Dirty_LightingLocal |
                                   LightGridChunk::Dirty_LightingSkylight;
                    chunk->neverDirty = 0;
