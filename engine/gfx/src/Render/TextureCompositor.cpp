...
static const unsigned int kTextureCompositorDefaultBudget = 16 * 8192 * 8192;
static const unsigned int kTextureCompositorOrphanedBudgetLimit = 64 * 8192 * 8192;
...
static TextureCompositorConfiguration calculateConfiguration(const RenderCaps* caps)
{
    ...
    unsigned int budget = FLog::RenderTextureCompositorBudget
                              ? FLog::RenderTextureCompositorBudget * 8192 * 8192
...
template<typename T>
struct WeakPtrEqualPredicate
{
    const boost::weak_ptr<T>* lhs;

    WeakPtrEqualPredicate(const boost::weak_ptr<T>* lhs)
        : lhs(lhs)
    {
    }
...
template<typename T>
struct ExistsInSetPredicate
{
    const std::set<T>* set;

    ExistsInSetPredicate(const std::set<T>* set)
        : set(set)
    {
    }
...
struct DistanceUpdatePredicate
{
    ...
    DistanceUpdatePredicate(void* job, const Vector3& focus, float* distance)
        : job(job)
        , focus(focus)
        , distance(distance)
    {
...
public:
    ...
    const shared_ptr<GeometryBatch>* requestMesh(const MeshId& meshId)
    {
        ...
        shared_ptr<FileMeshData> cachedMesh =
            boost::static_pointer_cast<FileMeshData>(mcp->requestContent(meshId, ContentProvider::PRIORITY_MESH, false, result));
        ...
        // Aya::StandardOut::singleton()->printf(Aya::MESSAGE_INFO, meshId.c_str());
        return &cache;
    }
...
{
public:
    TextureCompositorJob(VisualEngine* visualEngine, const Vector2& canvasSize, const std::vector<TextureCompositorLayer>& layers,
        float contentPriority, const std::string& context)
        : visualEngine(visualEngine)
    ...
    void render(DeviceContext* context, const shared_ptr<Framebuffer>& framebuffer, shared_ptr<Texture>& texture)
    {
        AYAASSERT(isReady());
        ...
        Matrix4 projection = Matrix4(2.f / canvasSize.x, 0.f, 0.f, -1.f - offset * 2.f / static_cast<float>(framebuffer->getWidth()), 0.f,
            2.f / canvasSize.y, 0.f, -1.f + offset * 2.f / static_cast<float>(framebuffer->getHeight()), 0.f, 0.f, 0.001f,
            0.5f, // shift depth to 0.5 to avoid clipping; don't multiply Z by zero to avoid d3d debug warnings.
            0.f, 0.f, 0.f, 1.f);
        ...
        for (size_t i = 0; i < layers.size(); ++i)
        {
            // We clear the framebuffer before loading the next layer.
            // HACK: Body colors are composited before clothing, so we can check if we're reusing meshes
            // Basically, we only clear the framebuffer if meshes are different
            // @mdolli

            LayerData& ld = layers[i];
            if (i > 0 && !ld.desc.offset.isZero())
            {
                LayerData& lastLd = layers[i - 1];
                if (lastLd.mesh->get() != ld.mesh->get())
                    context->clearFramebuffer(DeviceContext::Buffer_Color, clearColor, 1.f, 0);
            }

            shared_ptr<ShaderProgram> program = visualEngine->getShaderManager()->getProgramOrFFP(
                "TexCompVS", ld.desc.mode == TextureCompositorLayer::Composit_BlendTexture ? "TexCompPMAFS" : "TexCompFS");

            if (program && *ld.mesh && ld.texture.getTexture())
            {
                context->setBlendState(
                    ld.desc.mode == TextureCompositorLayer::Composit_BlendTexture ? BlendState::Mode_PremultipliedAlphaBlend : BlendState::Mode_None);
                ...
                float colorData[] = {ld.desc.color.r, ld.desc.color.g, ld.desc.color.b,
                    (ld.desc.mode == TextureCompositorLayer::Composit_BlitTextureAlphaMagnify4x) ? 4.f : 1.f};
                context->setConstant(program->getConstantHandle("Color"), colorData, 1);

                context->draw(**ld.mesh);

                // Theoretically, our current framebuffer should have exactly the only texture we need.
                // Hence why we copy this framebuffer over to the job texture at our specified offset :)
                // @mdolli
                ...
                if (texture->getUsage() != Texture::Usage_Renderbuffer)
                {
                    try
                    {
                        int x = static_cast<int>(ld.desc.offset.x);
                        int y = static_cast<int>(ld.desc.offset.y);

                        context->copyFramebuffer(
                            framebuffer.get(), texture.get(), static_cast<int>(ld.desc.offset.x), static_cast<int>(ld.desc.offset.y));
                    }
                    catch (const Aya::base_exception& e)
                    {
                        Aya::StandardOut::singleton()->printf(MESSAGE_OUTPUT, "TextureCompositor copyFramebuffer failed: %s", e.what());
                    }
                }
            }
        }
...
TextureCompositor::JobHandle TextureCompositor::getJob(const std::string& textureid, const std::string& context, unsigned int width,
    unsigned int height, const Vector2& canvasSize, const std::vector<TextureCompositorLayer>& layers)
{
...
void TextureCompositor::resetMeshCache()
{
    // meshCache.reset(new TextureCompositorMeshCache(visualEngine));
}
...
void TextureCompositor::updatePrioritiesAndOrphanJobs(const Vector3& pointOfInterest)
{
    ...
    for (JobMap::iterator it = jobs.begin(); it != jobs.end();)
    {
        ...
        job.instances.erase(std::remove_if(job.instances.begin(), job.instances.end(), DistanceUpdatePredicate(&job, pointOfInterest, &distance)),
            job.instances.end());
...
void TextureCompositor::garbageCollectOrphanedJobs()
{
    ...
    unsigned int maxOrphanedSize =
        (totalSize > config.budget) ? 0 : std::min(config.budget - totalSize, std::min(config.budget / 4, kTextureCompositorOrphanedBudgetLimit));
    ...
    if (orphanedSize > maxOrphanedSize)
    {
        ...
        for (size_t i = 0; i < sweep.size(); ++i)
        {
            ...
            if (sweep[i] && texture)
            {
                ...
                {
                    FASTLOG1(FLog::RenderTextureCompositor, "TC Job[%p]: cancelling blit in progress since the texture is destroyed",
                        orphanedJobs[i].get());
    ...
    orphanedJobs.erase(
        std::remove_if(orphanedJobs.begin(), orphanedJobs.end(), TextureIsNullPredicate<boost::shared_ptr<Job>>()), orphanedJobs.end());
}
...
void TextureCompositor::garbageCollectFull()
{
    ...
    for (size_t i = 0; i < orphanedJobs.size(); ++i)
    {
        ...
        if (texture)
        {
            ...
            {
                FASTLOG1(
                    FLog::RenderTextureCompositor, "TC Job[%p]: cancelling blit in progress since the texture is destroyed", orphanedJobs[i].get());
...
{
    AYAPROFILER_SCOPE("Render", "TextureCompositor::render");
    AYAPROFILER_SCOPE("GPU", "TextureCompositor::render");
    if (renderedJob.job)
    {
...
void TextureCompositor::updateJob(Job& job)
{
    if (!job.job)
    {
        ...
        job.job.reset(new TextureCompositorJob(
            visualEngine, job.desc.canvasSize, job.desc.layers, ContentProvider::PRIORITY_CHARACTER + priority, job.context));
    }
...
void TextureCompositor::renderJobIfNecessary(Job& job, size_t activePosition, DeviceContext* context)
{
    ...
    unsigned int pendingSize =
        ...
        getProjectedActiveTextureSize(0, activePosition); // all active jobs before us are assumed to get high-res textures (even if our assets load
                                                          // first we don't get high-res texture out of order)
    ...
    if (!job.texture || job.texture->getWidth() != width || job.texture->getHeight() != height)
    {
        if (job.texture)
        {
            // We're resampling an existing job; rather than lose the texture, let's add it to orphaned queue as an empty job - garbage collection
            // will take care of it
            orphanTextureFromJob(job);
        }

        FASTLOG5(FLog::RenderTextureCompositor, "TC Job[%p]: render (totalSize %d, pendingSize %d, budget %d -> width %d)", &job, totalSize,
            pendingSize, config.budget, width);
...
shared_ptr<Framebuffer> TextureCompositor::getOrCreateFramebufer(const shared_ptr<Texture>& texture)
{
    ...
    shared_ptr<Texture> rt = visualEngine->getDevice()->createTexture(
        Texture::Type_2D, Texture::Format_RGBA8, texture->getWidth(), texture->getHeight(), 1, 1, Texture::Usage_Renderbuffer);
