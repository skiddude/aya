    ...
    , lastFailedSize(0x7FFF, 0x7FFF)
{
    ...
    AYAASSERT((height & (height - 1)) == 0); // glyph texture height has to be power of two!
    moduloParam = height - 1;

    updateWindow = new unsigned char[textureSize.x * 2 * DFInt::TexAtlasUpdateLineHeight];
    texture =
        visualEngine->getDevice()->createTexture(Texture::Type_2D, Texture::Format_L8, textureSize.x, textureSize.y, 1, 1, Texture::Usage_Dynamic);
}
...
void TextureAtlas::upload()
{
    ...
    if (uploadRect.min < uploadRect.max)
    {
        ...
        texture.getTexture()->upload(0, 0, Aya::Graphics::TextureRegion(0, texureMin, textureSize.x, height), dataPtr, textureSize.x * (height));

        if (textureMax < texureMin)
        {
            dataPtr = &updateWindow[uploadWindowStartIdx + height * textureSize.x];
            texture.getTexture()->upload(0, 0, Aya::Graphics::TextureRegion(0, 0, textureSize.x, textureMax), dataPtr, textureSize.x * textureMax);
        }
...
bool TextureAtlas::insert(
    const boost::uint64_t& id, const Vector2int16& size, unsigned dataPitch, unsigned char* data, const TextureRegion*& outRegion)
{
    ...
    for (int i = 0; i < size.y; ++i)
    {
        ...
        AYAASSERT((int)yPos < DFInt::TexAtlasUpdateLineHeight * 2);
