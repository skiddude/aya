...
public:
    ...
    void bind(const shared_ptr<Aya::PartInstance>& part, const shared_ptr<Aya::Light>& light);

    LightShadowMap* getShadowMap() const
    {
    ...
    const Vector4& getAxisU() const
    {
        return axisU;
    }
    const Vector4& getAxisV() const
    {
        return axisV;
    }
...
private:
    ...
    void onParentSizeChangedEx(const Aya::Reflection::PropertyDescriptor* pd);

    void invalidateLighting(const Extents& bbox);

    shared_ptr<Aya::PartInstance> part;
    shared_ptr<Aya::Light> light;
