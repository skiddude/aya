...
public:
    ...
    struct Result
    {
        explicit Result(const shared_ptr<Material>& material = shared_ptr<Material>(), unsigned int flags = 0, unsigned int features = 0,
            const G3D::Vector4& uvOffsetScale = G3D::Vector4(0, 0, 1, 1))
            : material(material)
            , flags(flags)
            , features(features)
    ...
    static unsigned int createFlags(bool skinned, Aya::PartInstance* part, const HumanoidIdentifier* humanoidIdentifier, bool& ignoreDecalsOut);

    static unsigned int getDiffuseMapStage();

    // make this public im pissed of
    // @mdolli
    shared_ptr<Material> createBaseMaterial(unsigned int flags);
    shared_ptr<Material> createStencilShadowMaterial();
...
private:
    ...
    shared_ptr<Material> renderMaterialCache[kMatCacheSize]
                                            [Flag_CacheMask + 1];    // an entry for each render material and each supported flag combination
