...
static const char* kTextureExt = "pvr";
#else
static const char* kTextureExt = "jxl";
#endif
...
static void setupTechnique(Technique& technique, VisualEngine* visualEngine, const TextureRef& nm0, const TextureRef& nm1)
{
    ...
    technique.setTexture(
        2, visualEngine->getSceneManager()->getEnvMap()->getTexture(), SamplerState(SamplerState::Filter_Linear, SamplerState::Address_Clamp));
...
const shared_ptr<Material>& Water::getLegacyMaterial()
{
    ...
    if (shared_ptr<ShaderProgram> program = visualEngine->getShaderManager()->getProgramFFP())
    {
        ...
        technique.setTexture(
            0, tm->load(ContentId("ayaasset://textures/water_Subsurface.dds"), TextureManager::Fallback_White), SamplerState::Filter_Linear);
...
const shared_ptr<Material>& Water::getSmoothMaterial()
{
    ...
    if (shared_ptr<ShaderProgram> program = visualEngine->getShaderManager()->getProgram("SmoothWaterHQVS", "SmoothWaterHQGBufferFS"))
    {
        ...
        technique.setTexture(
            5, visualEngine->getSceneManager()->getGBufferColor(), SamplerState(SamplerState::Filter_Linear, SamplerState::Address_Clamp));
        technique.setTexture(
            6, visualEngine->getSceneManager()->getGBufferDepth(), SamplerState(SamplerState::Filter_Point, SamplerState::Address_Clamp));
...
{
    AYAASSERT(frame < ARRAYSIZE(normalMaps));

    if (!normalMaps[frame].getTexture())
    {
        char id[256];
        sprintf(id, "ayaasset://textures/water/normal_%02d.%s", frame + 1, kTextureExt);
