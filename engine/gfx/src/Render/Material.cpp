...
void Technique::setConstant(int handle, const G3D::Vector4& value)
{
    if (handle < 0)
        return;
...
void Technique::setConstant(const char* name, const G3D::Vector4& value)
{
...
void Technique::apply(DeviceContext* context) const
{
    ...
    AYAASSERT((samplerMask & mask) == samplerMask);
}
...
{
    AYAASSERT(techniques.empty() ||
              (techniques.back().getPass() == technique.getPass() && techniques.back().getLodIndex() < technique.getLodIndex()) ||
              techniques.back().getPass() < technique.getPass());
