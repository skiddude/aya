...
static const OFFSETINFOV2 kAxisSideLookup[6] = {
    {
        // PlusX
        Vector3int16(HSIZE, HSIZE, -HSIZE), Vector3int16(HSIZE, HSIZE, HSIZE), Vector3int16(HSIZE, -HSIZE, HSIZE),
        Vector3int16(HSIZE, -HSIZE, -HSIZE), packNormal(1, 0, 0), // normal
        packNormal(0, 0, -1),                                     // tangent
    },
    {
        // PlusZ
        Vector3int16(HSIZE, HSIZE, HSIZE), Vector3int16(-HSIZE, HSIZE, HSIZE), Vector3int16(-HSIZE, -HSIZE, HSIZE),
        Vector3int16(HSIZE, -HSIZE, HSIZE), packNormal(0, 0, 1), // normal
    ...
    },
    {
        // PlusY
        Vector3int16(HSIZE, HSIZE, -HSIZE), Vector3int16(-HSIZE, HSIZE, -HSIZE), Vector3int16(-HSIZE, HSIZE, HSIZE),
        Vector3int16(HSIZE, HSIZE, HSIZE), packNormal(0, 1, 0), // normal
        packNormal(1, 0, 0),                                    // tangent
    },
    {
        // MinusY
...
struct MaterialTextureCoordinates
{
        ...
        , oneCellPixels((endPixel - startPixel) / logicalCellSize)
    {
        ...
        AYAASSERT((logicalCellSize.x & (logicalCellSize.x - 1)) == 0);
        AYAASSERT((logicalCellSize.y & (logicalCellSize.y - 1)) == 0);
    }
...
static const Vector3int16 kFaceDirectionLocationOffset[6] = {
    Vector3int16(1, 0, 0), Vector3int16(0, 0, 1), Vector3int16(-1, 0, 0), Vector3int16(0, 0, -1), Vector3int16(0, 1, 0), Vector3int16(0, -1, 0)};
...
struct WaterRenderPredicate
{
    ...
    inline RenderPredStatus internal(const typename VoxelStore::Region::iterator& iterator, FaceDirection direction) const
    {
        ...
        // possible.
        bool centerQualifiesToRender = !center.isEmpty() && center.solid.getBlock() != CELL_BLOCK_Solid &&
                                       (center.solid.getBlock() == CELL_BLOCK_Empty || iterator.hasWaterAtCurrentLocation());

        bool neighborQualifiesToRender = !neighbor.isEmpty() && neighbor.solid.getBlock() != CELL_BLOCK_Solid &&
                                         (neighbor.solid.getBlock() == CELL_BLOCK_Empty || iterator.hasWaterAtNeighbor(direction));

        bool willRender = centerQualifiesToRender != neighborQualifiesToRender;
        return (RenderPredStatus)(willRender + (willRender && neighborQualifiesToRender));
    }
...
{
    return Vector3int16(kCELL_SIZE * (logical.x & (kXZ_CHUNK_SIZE - 1)) + centerOrig.x, kCELL_SIZE * (logical.y & (kY_CHUNK_SIZE - 1)) + centerOrig.y,
...
static void initLookupTables()
{
    for (unsigned outlineMask = 0; outlineMask < 16; outlineMask++)
    {
        ...
        {
            Vector4 edgeMin = Vector4(outlineMask & 0x1 ? 0.0f : 10.0f, outlineMask & 0x2 ? 0.0f : 10.0f, outlineMask & 0x4 ? 0.0f : 10.0f,
                outlineMask & 0x8 ? 0.0f : 10.0f);
            Vector4 edgeMax = Vector4(outlineMask & 0x1 ? 4.0f : 10.0f, outlineMask & 0x2 ? 4.0f : 10.0f, outlineMask & 0x4 ? 4.0f : 10.0f,
                outlineMask & 0x8 ? 4.0f : 10.0f);
            ...
            for (unsigned int i = 0; i < 4; ++i)
            {
                if (i == skippedIndex)
                {
                    ...
                    {
                    case 0:
                        lookup.values[i] = Color4uint8(edgeMin.x, edgeMin.y, edgeMax.z, edgeMin.w);
                        break;
                    case 1:
                        lookup.values[i] = Color4uint8(edgeMin.x, edgeMax.y, edgeMin.z, edgeMin.w);
                        break;
                    case 2:
                        lookup.values[i] = Color4uint8(edgeMin.x, edgeMin.y, edgeMin.z, edgeMax.w);
                        break;
                    case 3:
                        lookup.values[i] = Color4uint8(edgeMax.x, edgeMin.y, edgeMin.z, edgeMin.w);
                        break;
                    }
                }
                else
...
inline void outputFace(Vertex* output, const Vector3int16& center, const OFFSETINFOV2& offsetInfo, const BlockAxisFace& face,
    const Vector2int16 highUv[4], const Vector2int16 lowUv[4], const unsigned char outlineMask)
{
...
template<typename Vertex>
inline void outputFace(
    Vertex* output, const Vector3int16& center, const OFFSETINFOV2& offsetInfo, const BlockAxisFace& face, const Vector2int16 highUv[4])
{
    const Color4uint8& normal = offsetInfo.normal;
    const FaceVertexLookup& fvl = gFaceVertexLookup[face.skippedCorner];
...
struct RenderLookup
{
    ...
    {
        return Vector2int16(cellLocation.x * uDot.x + cellLocation.y * uDot.y + cellLocation.z * uDot.z,
...
struct SolidTerrainRenderer
{
    ...
    void renderHelper(const Voxel::Cell cell, const CellMaterial material, const Vector3int16& location, bool isTopSide, const Vector3int16& center,
        FaceDirection faceDirection, unsigned char outlineMask)
    {
    ...
    void apply(const typename VoxelStore::Region::iterator& iterator, FaceDirection faceDirection, RenderPredStatus status)
    {
        ...
        if (status & RENDER_FORWARD)
        {
            ...
            bool isTopSide = !impossibleTopSide && iterator.getNeighborCell(PlusY).isEmpty();
            renderHelper(iterator.getCellAtCurrentLocation(), iterator.getMaterialAtCurrentLocation(), iterator.getCurrentLocation(), isTopSide,
        ...
        if (status & RENDER_BACKWARD)
        {
            ...
            bool isTopSide =
                !impossibleTopSide && iterator.getArbitraryNeighborCell(faceDirectionOffset + kFaceDirectionLocationOffset[PlusY]).isEmpty();
            center += faceDirectionOffset * kCELL_SIZE;
            renderHelper(iterator.getNeighborCell(faceDirection), iterator.getNeighborMaterial(faceDirection),
                iterator.getCurrentLocation() + faceDirectionOffset, isTopSide, center, kOppositeSideOffset[faceDirection], outlineMask);
        }
    }
    ...
    void wedgeFace(const typename VoxelStore::Region::iterator& iterator)
    {
        ...
        OFFSETINFOV2 offsetInfo = {CornerLookup[index0].posoffset, CornerLookup[index1].posoffset, CornerLookup[index2].posoffset,
            CornerLookup[index3].posoffset, normal, tangent};
...
struct WaterFaceRenderer
{
    ...
    void apply(const typename VoxelStore::Region::iterator& iterator, FaceDirection faceDirection, RenderPredStatus status)
    {
        ...
        static const Vector2int16 standardUvs[4] = {Vector2int16(0, 0), Vector2int16(kAtlasPixelDimension, 0),
            Vector2int16(kAtlasPixelDimension, kAtlasPixelDimension), Vector2int16(0, kAtlasPixelDimension)};
...
const std::string MegaCluster::kTerrainTexClose = "ayaasset://textures/terrain/diffuse";
const std::string MegaCluster::kTerrainTexFar = "ayaasset://textures/terrain/diffusefar";
const std::string MegaCluster::kTerrainTexNormals = "ayaasset://textures/terrain/normal";
const std::string MegaCluster::kTerrainTexSpecular = "ayaasset://textures/terrain/specular";
...
MegaCluster::MegaCluster(VisualEngine* visualEngine, const boost::shared_ptr<PartInstance>& part)
    ...
    , storage(NULL)
{
    AYAASSERT(part->getPartType() == MEGACLUSTER_PART);
    partInstance = part;

    AYAASSERT(partInstance->getGfxPart() == NULL);
...
void MegaCluster::terrainCellChanged(const CellChangeInfo& info)
{
    ...
    if (SpatialRegion::regionContainingVoxel(pos - Vector3int16(1, 1, 1)) != SpatialRegion::regionContainingVoxel(pos + Vector3int16(1, 1, 1)))
    {
        ...
        AYAASSERT(usedNeighborTerrainChunks <= 3);
        ...
        for (unsigned int firstDim = 0; firstDim < usedNeighborTerrainChunks; ++firstDim)
        {
            ...
            for (unsigned int secondDim = firstDim + 1; secondDim < usedNeighborTerrainChunks; ++secondDim)
            {
                ...
                {
                    markDirty(chunk + neighborTerrainChunks[firstDim] + neighborTerrainChunks[secondDim] + neighborTerrainChunks[thirdDim],
                        terrainChanged, waterChanged);
                }
            }
        }
    }
...
void MegaCluster::markDirty(const SpatialRegion::Id& pos, bool solidDirty, bool waterDirty)
{
    ...
    FASTLOG5(FLog::MegaClusterDirty, "MegaCluster: Marking chunk %dx%dx%d dirty (solid %d, water %d)", pos.value().x, pos.value().y, pos.value().z,
        solidDirty, waterDirty);
...
RenderEntity* MegaCluster::createGeometry(RenderNode* node, const shared_ptr<VertexBuffer>& vbuf, const shared_ptr<IndexBuffer>& ibuf,
    const shared_ptr<Material>& material, RenderQueue::Id renderQueueId, bool isWater)
{
...
const shared_ptr<VertexLayout>& MegaCluster::getVertexLayout(bool isWater)
{
    ...
    if (!vertexLayout)
    {
        ...
        if (useShaders)
        {
            ...
            {
                elements.push_back(
                    VertexLayout::Element(0, offsetof(WaterVertex, pos), VertexLayout::Format_UByte4, VertexLayout::Semantic_Position));
                elements.push_back(
                    VertexLayout::Element(0, offsetof(WaterVertex, normal), VertexLayout::Format_UByte4, VertexLayout::Semantic_Normal));
            }
            else
            {
                elements.push_back(
                    VertexLayout::Element(0, offsetof(TerrainVertex, pos), VertexLayout::Format_UByte4, VertexLayout::Semantic_Position));
                elements.push_back(
                    VertexLayout::Element(0, offsetof(TerrainVertex, normal), VertexLayout::Format_UByte4, VertexLayout::Semantic_Normal));
                elements.push_back(
                    VertexLayout::Element(0, offsetof(TerrainVertex, uv), VertexLayout::Format_Short4, VertexLayout::Semantic_Texture, 0));
                elements.push_back(
                    VertexLayout::Element(0, offsetof(TerrainVertex, edgeDistances), VertexLayout::Format_UByte4, VertexLayout::Semantic_Texture, 1));
                elements.push_back(
                    VertexLayout::Element(0, offsetof(TerrainVertex, tangent), VertexLayout::Format_UByte4, VertexLayout::Semantic_Texture, 2));
            }
        }
        else
        {
            elements.push_back(
                VertexLayout::Element(0, offsetof(TerrainVertexFFP, pos), VertexLayout::Format_Float3, VertexLayout::Semantic_Position));
            elements.push_back(
                VertexLayout::Element(0, offsetof(TerrainVertexFFP, normal), VertexLayout::Format_Float3, VertexLayout::Semantic_Normal));
            elements.push_back(
                VertexLayout::Element(0, offsetof(TerrainVertexFFP, uv), VertexLayout::Format_Float2, VertexLayout::Semantic_Texture, 0));
        }

        vertexLayout = visualEngine->getDevice()->createVertexLayout(elements);
        AYAASSERT(vertexLayout);
    }
...
template<typename T>
static shared_ptr<IndexBuffer> generateQuadIB(Device* device, unsigned int quads)
{
...
static void setupTextures(VisualEngine* visualEngine, Technique& technique)
{
    ...
    technique.setTexture(
        0, tm->load(ContentId(MegaCluster::kTerrainTexClose + kTextureExtension), TextureManager::Fallback_White), SamplerState::Filter_Linear);
    technique.setTexture(
        1, tm->load(ContentId(MegaCluster::kTerrainTexFar + kTextureExtension), TextureManager::Fallback_White), SamplerState::Filter_Linear);
    technique.setTexture(
        2, tm->load(ContentId(MegaCluster::kTerrainTexNormals + kTextureExtension), TextureManager::Fallback_NormalMap), SamplerState::Filter_Linear);
    technique.setTexture(
        3, tm->load(ContentId(MegaCluster::kTerrainTexSpecular + kTextureExtension), TextureManager::Fallback_Black), SamplerState::Filter_Linear);
