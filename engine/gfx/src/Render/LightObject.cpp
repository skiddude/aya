...
static Extents computeConeExtents(const Vector3& position, float radius, const Vector3& direction, float angle)
{
    ...
    for (int axis = 0; axis < 6; ++axis)
    {
        ...
        {
            Matrix3 AAT = Matrix3(A.x * A.x, A.x * A.y, A.x * A.z, A.y * A.x, A.y * A.y, A.y * A.z, A.z * A.x, A.z * A.y, A.z * A.z);
...
{
    AYAASSERT(newSize > 0);
    AYAASSERT(newSize % 2 == 1); // all resize requests are odd; this simplifies the copy logic below
...
void LightObject::onParentSizeChangedEx(const Aya::Reflection::PropertyDescriptor* pd)
{
...
void LightObject::onPropertyChangedEx(const Aya::Reflection::PropertyDescriptor* descriptor)
{
    invalidateEntity();
}
...
void LightObject::onAncestorChangedEx()
{
    ...
    else
    {
        ...
        Aya::PartInstance* parent = Aya::Instance::fastDynamicCast<Aya::PartInstance>(lightCopy->getParent());
...
void LightObject::bind(const shared_ptr<Aya::PartInstance>& part, const shared_ptr<Aya::Light>& light)
{
    AYAASSERT(!this->part && !this->light);
    AYAASSERT(light);
