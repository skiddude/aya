...
void VertexStreamer::renderPrepare()
{
    ...
    AYAASSERT(vertexData.size() <= kVertexBufferMaxCount);
...
{
    AYAPROFILER_SCOPE("Render", "render3D");
...
{
    AYAPROFILER_SCOPE("Render", "render3DNoDepth");
...
{
    AYAPROFILER_SCOPE("Render", "render2D");
...
void VertexStreamer::renderInternal(
    DeviceContext* context, CoordinateSpace coordinateSpace, const RenderCamera& camera, RenderPassStats& stats, int renderIndex)
{
...
VertexStreamer::VertexChunk* VertexStreamer::prepareChunk(const shared_ptr<Texture>& texture, Geometry::Primitive primitive, unsigned int vertexCount,
    CoordinateSpace coordinateSpace, BatchTextureType batchTexType, bool ignoreTexture, int zIndex, bool forceTop)
{
    ...
    const shared_ptr<Texture>& actualTexture =
        ignoreTexture ? visualEngine->getTextureManager()->getFallbackTexture(TextureManager::Fallback_White) : texture;
    ...
    if (orderedChunks.size() > 0)
    {
        ...
        newsection = (last.texture != actualTexture) || last.primitive != primitive || last.vertexStart + last.vertexCount != vertexStart ||
                     last.index != zIndex || last.alwaysOnTop != forceTop;
    }
    ...
    {
        AYAASSERT(orderedChunks.back().batchTextureType == batchTexType);
...
void VertexStreamer::rectBlt(const shared_ptr<Texture>& texptr, const Color4& color4, const Vector2& x0y0, const Vector2& x1y0, const Vector2& x0y1,
    const Vector2& x1y1, const Vector2& tex0, const Vector2& tex1, BatchTextureType batchTexType, bool ignoreTexture)
{
...
void VertexStreamer::spriteBlt3D(const shared_ptr<Texture>& texptr, const Color4& color4, BatchTextureType batchTexType, const Vector3& x0y0,
    const Vector3& x1y0, const Vector3& x0y1, const Vector3& x1y1, const Vector2& tex0, const Vector2& tex1, int zIndex, bool alwaysOnTop)
{
    if (prepareChunk(texptr, Geometry::Primitive_Triangles, 6, alwaysOnTop || zIndex >= 0 ? CS_WorldSpaceNoDepth : CS_WorldSpace, batchTexType, false,
            zIndex, alwaysOnTop))
    {
        unsigned int color = packColor(color4, colorOrderBGR);
