...
void Sky::StarData::resize(Sky* sky, unsigned int count, bool dynamic)
{
    ...
    else
    {
        ...
        buffer = sky->visualEngine->getDevice()->createVertexBuffer(
            sizeof(SkyVertex), count, dynamic ? GeometryBuffer::Usage_Dynamic : GeometryBuffer::Usage_Static);

        batch.reset(new GeometryBatch(sky->visualEngine->getDevice()->createGeometry(sky->layout, buffer, shared_ptr<IndexBuffer>()),
            Geometry::Primitive_Points, count, count));
    }
}
    ...
    , readyState(false)
{
    ...
    // preload sun/mon
    sun = visualEngine->getTextureManager()->load(ContentId("ayaasset://sky/sun.jpg"), TextureManager::Fallback_BlackTransparent);
    moon = visualEngine->getTextureManager()->load(ContentId("ayaasset://sky/moon.jpg"), TextureManager::Fallback_BlackTransparent);
}
...
{
    AYAPROFILER_SCOPE("Render", "Sky");
    AYAPROFILER_SCOPE("GPU", "Sky");
    ...
    if (program)
    {
        ...
        if (1)
        {
            ...
            for (int i = 0; i < 6; ++i)
            {
                ...
                Matrix3 rotation = Matrix3::fromAxisAngle(Vector3(0, 1, 0), adjustAngle) * Matrix3::fromDiagonal(Vector3(-1, 1, 1)) *
                                   normalIdToMatrix3(static_cast<NormalId>(i));
                ...
                context->bindTexture(0,
                    skyBox[i].getTexture().get() ? skyBox[i].getTexture().get()
                                                 : visualEngine->getTextureManager()->getFallbackTexture(TextureManager::Fallback_White).get(),
        ...
        if (drawStars && starLight != Brightness_None && (starsNormal.batch || starsTwinkle.batch))
        {
            ...
            context->bindTexture(
                0, visualEngine->getTextureManager()->getFallbackTexture(TextureManager::Fallback_White).get(), SamplerState::Filter_Linear);
...
void Sky::loadSkyBox(TextureRef* textures, const ContentId& rt, const ContentId& lf, const ContentId& bk, const ContentId& ft, const ContentId& up,
    const ContentId& dn)
{
