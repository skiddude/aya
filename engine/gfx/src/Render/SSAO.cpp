    ...
    , currentLevel(ssaoNone)
{
    ...
    static const int rotationIndex[4][4] = {{4, 1, 3, 7}, {8, 11, 15, 6},
        {
            10,
            2,
            14,
...
void SSAO::update(SSAOLevel level, unsigned int width, unsigned int height)
{
    ...
    else
    {
        if (!data.get() || data->width != width || data->height != height)
        {
            ...
            {
                Aya::StandardOut::singleton()->printf(MESSAGE_ERROR, "SSAO: Error creating targets: %s", e.what());
...
{
    AYAASSERT(currentLevel != ssaoNone);

    SceneManager::GBuffer* gb = visualEngine->getSceneManager()->getGBuffer();
    AYAASSERT(gb);

    PIX_SCOPE(context, "SSAO/renderCompute");
    AYAPROFILER_SCOPE("Render", "SSAO::renderCompute");
    AYAPROFILER_SCOPE("GPU", "SSAO::renderCompute");

    // Downsample depth
    context->bindFramebuffer(data->depthHalfFB.get());

    if (ScreenSpaceEffect::renderFullscreenBegin(context, visualEngine, "SSAODepthDownVS", "SSAODepthDownFS", BlendState::Mode_None,
            data->depthHalfFB->getWidth(), data->depthHalfFB->getHeight()))
    {
    ...
    if (ScreenSpaceEffect::renderFullscreenBegin(
            context, visualEngine, "PassThroughVS", "SSAOFS", BlendState::Mode_None, data->ssaoFB[0]->getWidth(), data->ssaoFB[0]->getHeight()))
    {
    ...
    if (ScreenSpaceEffect::renderFullscreenBegin(
            context, visualEngine, "SSAOBlurXVS", "SSAOBlurXFS", BlendState::Mode_None, data->ssaoFB[1]->getWidth(), data->ssaoFB[1]->getHeight()))
    {
        context->bindTexture(0, data->depthHalf.get(), SamplerState(SamplerState::Filter_Point, SamplerState::Address_Clamp));
    ...
    if (ScreenSpaceEffect::renderFullscreenBegin(
            context, visualEngine, "SSAOBlurYVS", "SSAOBlurYFS", BlendState::Mode_None, data->ssaoFB[0]->getWidth(), data->ssaoFB[0]->getHeight()))
    {
        context->bindTexture(0, data->depthHalf.get(), SamplerState(SamplerState::Filter_Point, SamplerState::Address_Clamp));
...
void SSAO::renderApply(DeviceContext* context)
{
    ...
    AYAPROFILER_SCOPE("Render", "SSAO::renderApply");
    AYAPROFILER_SCOPE("GPU", "SSAO::renderApply");

    // Composite SSAO
    if (currentLevel == ssaoFullBlank)
    {
        if (ScreenSpaceEffect::renderFullscreenBegin(
                context, visualEngine, "PassThroughVS", "PassThroughFS", BlendState::Mode_None, data->width, data->height))
        {
    ...
    {
        if (ScreenSpaceEffect::renderFullscreenBegin(
                context, visualEngine, "SSAOCompositVS", "SSAOCompositFS", BlendState::Mode_None, data->width, data->height))
        {
            context->bindTexture(0, data->depthHalf.get(), SamplerState(SamplerState::Filter_Point, SamplerState::Address_Clamp));
...
void SSAO::renderComposit(DeviceContext* context)
{
    ...
    AYAPROFILER_SCOPE("Render", "SSAO::renderComposit");
    AYAPROFILER_SCOPE("GPU", "SSAO::renderComposit");
