...
#include "Render/MegaCluster.hpp"
...
#include "Render/SpatialGrid.hpp"
...
FASTFLAGVARIABLE(CustomEmitterRenderEnabled, true)
...
SceneUpdater::SceneUpdater(shared_ptr<Aya::DataModel> dataModel, VisualEngine* ve)
    : dataModel(dataModel)
    ...
    , computeLightingEnabled(true)
{
    ...
    workspaceDescendantAddedConnection =
        dataModel->getWorkspace()->onDemandWrite()->descendantAddedSignal.connect(boost::bind(&SceneUpdater::onWorkspaceDescendantAdded, this, _1));

    FASTLOG1(FLog::GfxClusters, "After initial bind, added parts: %u", mAddedParts.size());

    Aya::Vector3 cellExtents = Aya::Vector3(32 * 4, 16 * 4, 32 * 4);
    ...
    {
        Aya::Lighting* lighting = ServiceProvider::find<Lighting>(dataModel.get());
        if (lighting)
...
void SceneUpdater::onWorkspaceDescendantAdded(shared_ptr<Aya::Instance> descendant)
{
    // See if the new instance is a PartInstance

    Aya::PartInstance* pi = Aya::Instance::fastDynamicCast<Aya::PartInstance>(descendant.get());
    if (pi != NULL)
    {
...
bool SceneUpdater::isPartStatic(Aya::PartInstance* part)
{
    return part->getSleeping();
}

void SceneUpdater::queuePartToCreate(const boost::shared_ptr<Aya::PartInstance>& part)
{
    Aya::mutex::scoped_lock scoped_lock(queue_mutex);
...
void SceneUpdater::queueAttachementToCreate(const boost::shared_ptr<Aya::Instance>& instance)
{
    Aya::mutex::scoped_lock scoped_lock(queue_mutex);
...
{
    AYAPROFILER_SCOPE("Render", "processPendingAttachments");
...
void SceneUpdater::queueChunkInvalidateMegaCluster(Aya::GfxPart* part, const SpatialRegion::Id& pos, bool isWaterChunk)
{
    AYAASSERT(part->getPartInstance()->getPartType() == Aya::MEGACLUSTER_PART);
    Aya::mutex::scoped_lock scoped_lock(queue_mutex);
    FASTLOG3(FLog::MegaClusterDirty, "Chunk queued for update: %u x %u x %u", pos.value().x, pos.value().y, pos.value().z);

    const Camera* camera = dataModel->getWorkspace()->getConstCamera();
    Aya::Vector3 testLocation(SpatialRegion::centerOfRegionInGlobalCoordStuds(pos).toVector3());
...
void SceneUpdater::queueFullInvalidateMegaCluster(Aya::GfxPart* part)
{
    AYAASSERT(part->getPartInstance()->getPartType() == Aya::MEGACLUSTER_PART);
...
void SceneUpdater::queueInvalidatePart(Aya::GfxPart* part)
{
...
void SceneUpdater::queueInvalidateFastCluster(Aya::GfxPart* cluster)
{
...
void SceneUpdater::queuePriorityInvalidateFastCluster(Aya::GfxPart* cluster)
{
...
void SceneUpdater::notifyWaitingForAssets(Aya::GfxPart* part, const std::vector<Aya::ContentId>& ids)
{
    std::vector<Aya::ContentId> uids = ids;
...
{
    AYAPROFILER_SCOPE("Render", "updateInvalidParts");
...
{
    AYAPROFILER_SCOPE("Render", "updateWaitingParts");
    ...
    for (auto& part : waitingParts)
    {
        ...
        AYAASSERT(part.assets->first == part.part);
...
{
    AYAPROFILER_SCOPE("Render", "updateClusters");
...
{
    AYAPROFILER_SCOPE("Render", "updateInvalidatedFastClusters");
    ...
    if (FLog::GfxClusters)
    {
        ...
        {
            FASTLOG2(FLog::GfxClusters, "Invalidating Fast Clusters, PriorityInvalidatedClusters: %u, InvalidatedClusters: %u",
                mPriorityInvalidateFastClusters.size(), mInvalidatedFastClusters.size());
        }
    }
    ...
    do
    {
        ...
        if (mPriorityInvalidateFastClusters.empty())
        {
            ...
            FASTLOG3(FLog::GfxClusters, "Updating invalidated cluster %p (remaining: %u, bulkExecution: %u)", cluster,
                mPriorityInvalidateFastClusters.size(), bulkExecution);
        }
        else
        {
...
void SceneUpdater::notifyAwake(Aya::GfxPart* part)
{
    Aya::mutex::scoped_lock scoped_lock(queue_mutex);
...
void SceneUpdater::notifySleeping(Aya::GfxPart* part)
{
    Aya::mutex::scoped_lock scoped_lock(queue_mutex);
...
struct IsChunkFromCluster
{
    ...
    IsChunkFromCluster(GfxPart* part)
        : part(part)
    {
    }
...
void SceneUpdater::notifyDestroyed(Aya::GfxPart* part)
{
    ...
    if (mMegaClusters.count(part))
    {
        ...
        mCloseChunkInvalidates.erase(
            std::remove_if(mCloseChunkInvalidates.begin(), mCloseChunkInvalidates.end(), IsChunkFromCluster(part)), mCloseChunkInvalidates.end());
        mMiddleChunkInvalidates.erase(
            std::remove_if(mMiddleChunkInvalidates.begin(), mMiddleChunkInvalidates.end(), IsChunkFromCluster(part)), mMiddleChunkInvalidates.end());
        mFarChunkInvalidates.erase(
            std::remove_if(mFarChunkInvalidates.begin(), mFarChunkInvalidates.end(), IsChunkFromCluster(part)), mFarChunkInvalidates.end());
...
void SceneUpdater::queueFastClusterCheck(Aya::GfxPart* cluster, bool isFW)
{
...
{
    AYAPROFILER_SCOPE("Render", "updateDynamicParts");

    Aya::Profiling::Mark mark(*mRenderStats->updateDynamicParts, true, true);
...
{
    AYAPROFILER_SCOPE("Render", "processPendingClusters");
    ...
    {
        shared_ptr<Aya::PartInstance> part = it->second.lock();
        if (part)
            addMegaCluster(part);
    }
}
...
{
    AYAPROFILER_SCOPE("Render", "processPendingParts");
    ...
    for (PartInstanceSet::iterator it = localCopy.begin(); it != localCopy.end(); ++it)
    {
        ...
        {
            FASTLOG2(
                FLog::GfxClustersFull, "Part %p died or removed from workspace before we could process it (died: %u)", it->first, p.get() == NULL);
        }
    }
}

void SceneUpdater::updatePrepare(unsigned long currentFrameNum, const Aya::Frustum& updateFrustum)
{
    AYAPROFILER_SCOPE("Render", "UpdatePrepare");
    ...
    Aya::Profiling::Mark mark(*mRenderStats->updateSceneGraph, true, true);
...
void SceneUpdater::lightingInvalidateOccupancy(const Aya::Extents& extents, const Aya::Vector3& highPriorityPoint, bool isFixed)
{
    if (LightGrid* lgrid = mVisualEngine->getLightGrid())
    {
...
void SceneUpdater::lightingInvalidateLocal(const Aya::Extents& extents)
{
    if (LightGrid* lgrid = mVisualEngine->getLightGrid())
    {
...
void SceneUpdater::onPropertyChanged(const Aya::Reflection::PropertyDescriptor* descriptor)
{
...
{
    AYAPROFILER_SCOPE("Render", "computeLightingPrepare");
    ...
    if (LightGrid* lgrid = mVisualEngine->getLightGrid())
    {
        ...
        {
            Aya::Vector3 hit = Aya::Vector3::zero();

            if (FFlag::FixCameraTargetStudio)
            {
                Aya::Vector3 origin = mVisualEngine->getSceneManager()->getMinumumSqDistanceCenter();
                Aya::Vector3 direction = camera->getCameraCoordinateFrame().vectorToWorldSpace(Aya::Vector3(0.0f, 0.0f, -1.0f));
...
{
    AYAPROFILER_SCOPE("Render", "computeLightingPerform");
    ...
    if (LightGrid* lgrid = mVisualEngine->getLightGrid())
    {
        Aya::Timer<Aya::Time::Benchmark> timer;
        ...
        if ((!mLightgridMoved || bulkExecution) && mLightingActive)
        {
            ...
            if ((!mLightgridMoved || bulkExecution) && mLightingActive)
            {
                ...
                {
                    FASTLOG3(FLog::RenderLightGrid, "LightGrid: Updated %d chunks in %d usec (occupancy: %d chunks)", mLastLightingUpdates,
                        (int)(timer.delta().msec() * 1000), mLastOccupancyUpdates);
                }
            }
        }

        // Pass grid offset to shader via global data
        Aya::Vector3 gridOffset = lgrid->getGridCornerOffset();
        Aya::Vector3 gridSize = lgrid->getGridSize();
        Aya::Color4uint8 borderColor = lgrid->getBorderColor();

        float frmRadius = mVisualEngine->getFrameRateManager()->getLightGridRadius();

        Aya::Vector3 gridCenter = gridOffset + gridSize / 2.f;
        Aya::Vector3 gridRadius = Aya::Vector3(frmRadius, gridSize.y / 2.f, frmRadius).min(gridSize / 2.f);

        // world space -> texture space: v * scale + offset
        Aya::Vector3 gridTextureScale = Aya::Vector3(1.f / gridSize.x, 1.f / gridSize.y, 1.f / gridSize.z);
        ...
        Aya::Vector3 gridTextureOffset = lgrid->getWrapSafeOffset() * gridTextureScale;
        ...
        Aya::Vector3 gridRadiusEffective = gridRadius * 0.95f;

        Aya::Vector3 gridTextureSpaceCenter = gridCenter * gridTextureScale + gridTextureOffset;
        Aya::Vector3 gridTextureSpaceRadius = gridRadiusEffective * gridTextureScale;
...
{
    AYAPROFILER_SCOPE("Render", "checkFastClusters");
    ...
    while (!mFastClustersToCheck.empty() || !mFastClustersToCheckFW.empty())
    {
        ...
        if (!mFastClustersToCheckFW.empty())
        {
            ...
            AYAASSERT(gfxcluster->isFW());
        }
        else
        {
            AYAASSERT(!mFastClustersToCheck.empty());
        ...
        if (gfxcluster->isFW())
        {
            AYAASSERT(!mFastClustersToCheckFW.empty());
    ...
    AYAASSERT(mSeenFastClusters.size() <= FAST_CLUSTER_PRIORITY_INVALIDATE_BUDGET);
}
...
void SceneUpdater::addMegaCluster(const shared_ptr<Aya::PartInstance>& part)
{
...
Aya::Humanoid* SceneUpdater::getHumanoid(Aya::PartInstance* part)
{
    ...
    {
        Aya::Instance* parent = part->getParent();

        // Regular humanoid part
        if (part->getCookie() & PartCookie::IS_HUMANOID_PART)
            return Aya::Humanoid::modelIsCharacter(parent);

        // For the purposes of flex clustering, we treat accoutrements as humanoid parts since we can composit their textures
        if (Aya::Instance::isA<Aya::Accoutrement>(parent) || Aya::Instance::isA<Aya::Tool>(parent))
            return Aya::Humanoid::modelIsCharacter(parent->getParent());

        return NULL;
    }
}

Aya::WindowAverage<double, double>::Stats SceneUpdater::getLightingTimeStats()
{
...
void SceneUpdater::addFastPart(const shared_ptr<Aya::PartInstance>& part, bool isFW, bool priorityPart)
{
    if (Aya::Humanoid* humanoid = getHumanoid(part.get()))
    {
...
void SceneUpdater::eraseAllHumanoidClusters()
{
    //
}
...
void SceneUpdater::destroyFastCluster(FastCluster* cluster)
{
    ...
    {
        AYAASSERT(!"Owned FastClusters must be deleted via their owners (see SuperCluster.h)");
    }
}
...
void SceneUpdater::addAttachment(const shared_ptr<Aya::Instance>& instance)
{
    PartInstance* part = 0;
    if (Aya::Instance::fastDynamicCast<Aya::ForceField>(instance.get())) // Special case for forcefield - only put on character's Torso
        if (DFFlag::UseR15Character)
        {
            Humanoid* humanoid = Aya::Instance::fastDynamicCast<Humanoid>(instance->getParent()->findFirstChildByName2("Humanoid", false).get());
            ;
            if (humanoid)
                part = humanoid->getVisibleTorsoSlow();
            else
                part = Aya::Instance::fastDynamicCast<PartInstance>(instance->getParent()->findFirstChildByName2("Torso", false).get());
        }
        else
        {
            part = Aya::Instance::fastDynamicCast<PartInstance>(instance->getParent()->findFirstChildByName2("Torso", false).get());
        ...
        part = Aya::Instance::fastDynamicCast<PartInstance>(instance->getParent());

    if (dynamic_cast<Effect*>(instance.get()))
    {
        if (shared_ptr<Aya::Light> light = Instance::fastSharedDynamicCast<Aya::Light>(instance))
        {
        ...
        else if (FFlag::RenderNewExplosionEnable && instance->isA<Aya::Explosion>())
        {
        ...
        else if (FFlag::CustomEmitterRenderEnabled && instance->isA<Aya::CustomParticleEmitter>())
        {
...
unsigned SceneUpdater::getChunkBudget()
{
    ...
    {
        Aya::Vector3int32 chunkCount = mVisualEngine->getLightGrid()->getChunkCount();
