...
public:
    ...
    const shared_ptr<Texture>& getFallbackTexture(Fallback fallback)
    {
        return fallbackTextures[fallback];
    }
...
private:
    ...
    shared_ptr<Aya::safe_queue<LoadedImage>> pendingImages;
    ...
    static void loadImageHttpCallback(const weak_ptr<ThreadPool>& loadingPool, const weak_ptr<Aya::safe_queue<LoadedImage>>& pendingImages,
        const shared_ptr<const std::string>& content, const ContentId& id, unsigned int maxTextureSize, unsigned int flags,
        const std::string& context);

    static void loadImageFile(const shared_ptr<Aya::safe_queue<LoadedImage>>& pendingImages, const ContentId& id, const ContentId& loadId,
        unsigned int maxTextureSize, unsigned int flags, bool useRetina, const std::string& context = "");
    static void loadImageHttp(const shared_ptr<Aya::safe_queue<LoadedImage>>& pendingImages, const ContentId& id,
        const shared_ptr<const std::string>& content, unsigned int maxTextureSize, unsigned int flags, const std::string& context = "");
    static void loadImage(const shared_ptr<Aya::safe_queue<LoadedImage>>& pendingImages, const ContentId& id, std::istream& stream,
        unsigned int maxTextureSize, unsigned int flags, int scale, const std::string& context = "");
    static void loadImageError(
