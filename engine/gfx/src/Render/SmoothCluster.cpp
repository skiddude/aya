...
{
public:
    SmoothClusterRenderEntity(RenderNode* node, const GeometryBatch& geometry, const shared_ptr<Material>& material, RenderQueue::Id renderQueueId,
        std::vector<float>* constantTable, const Vector4& unpackInfo)
    ...
    unsigned int getWorldTransforms4x3(float* buffer, unsigned int maxTransforms, const void** cacheKey) const override
    {
        ...
        AYAASSERT(constantTable->size() <= maxTransforms * 3 * 4);
...
std::pair<RenderEntity*, RenderEntity*> SmoothClusterBase::uploadGeometry(RenderNode* node, const Vector4& unpackInfo, size_t vertexSize,
    const void* vertices, size_t vertexCount, const std::vector<unsigned int>& solidIndices, const std::vector<unsigned int>& waterIndices)
{
    AYAPROFILER_SCOPE("Render", "uploadGeometry");
    ...
    if (!solidIndices.empty())
    {
        ...
        solidEntity = new SmoothClusterRenderEntity(node,
            GeometryBatch(geometry, Geometry::Primitive_Triangles, ibuf->getElementCount(), vbuf->getElementCount()), material,
            RenderQueue::Id_Opaque, &materialConstants, unpackInfo);
    }
    ...
    if (!waterIndices.empty())
    {
        ...
        waterEntity = new SmoothClusterRenderEntity(node,
            GeometryBatch(geometry, Geometry::Primitive_Triangles, ibuf->getElementCount(), vbuf->getElementCount()), material,
            RenderQueue::Id_TransparentUnsorted, &materialConstants, unpackInfo);
    }
...
const shared_ptr<VertexLayout>& SmoothClusterBase::getVertexLayout()
{
    if (!vertexLayout)
    {
        ...
        elements.push_back(
            VertexLayout::Element(0, offsetof(GraphicsVertexPacked, position), VertexLayout::Format_Short4, VertexLayout::Semantic_Position));
        elements.push_back(
            VertexLayout::Element(0, offsetof(GraphicsVertexPacked, normal), VertexLayout::Format_UByte4, VertexLayout::Semantic_Normal));
        elements.push_back(
            VertexLayout::Element(0, offsetof(GraphicsVertexPacked, material0), VertexLayout::Format_UByte4, VertexLayout::Semantic_Texture, 0));
        elements.push_back(
            VertexLayout::Element(0, offsetof(GraphicsVertexPacked, material1), VertexLayout::Format_UByte4, VertexLayout::Semantic_Texture, 1));
...
static void setupTextures(VisualEngine* visualEngine, Technique& technique)
{
    ...
    technique.setTexture(0, tm->load(ContentId("ayaasset://terrain/diffuse" + kTextureExtension), TextureManager::Fallback_White),
        lodIndex < 2 ? SamplerState::Filter_Anisotropic : SamplerState::Filter_Linear);

    if (lodIndex < 1)
        technique.setTexture(
            1, tm->load(ContentId("ayaasset://terrain/normal" + kTextureExtension), TextureManager::Fallback_NormalMap), SamplerState::Filter_Linear);

    if (lodIndex < 2)
        technique.setTexture(
            2, tm->load(ContentId("ayaasset://terrain/specular" + kTextureExtension), TextureManager::Fallback_Black), SamplerState::Filter_Linear);
...
{
    AYAPROFILER_SCOPE("Render", "updateChunk");
...
std::pair<RenderEntity*, RenderEntity*> SmoothClusterChunked::createChunkGeometry(const SpatialRegion::Id& pos, unsigned int* outQuads)
{
    ...
    Voxel2::Region region =
        Voxel2::Region(Vector3int32(extents.getMinPos()), Vector3int32(extents.getMaxPos()) + Vector3int32::one()).expand(kChunkBorder);
    ...
    *outQuads = (graphicsGeometry.solidIndices.size() + graphicsGeometry.waterIndices.size()) / 6;
    ...
    return uploadGeometry(updateChunkNode(pos), packInfo.second, sizeof(GraphicsVertexPacked), &graphicsGeometry.vertices[0],
        graphicsGeometry.vertices.size(), graphicsGeometry.solidIndices, graphicsGeometry.waterIndices);
}
...
{
    AYAASSERT(id.second + 1 < kChunkLevels);
...
std::pair<RenderEntity*, RenderEntity*> SmoothClusterLOD::createChunkGeometry(Chunk& chunk, unsigned int* outQuads)
{
    ...
    *outQuads = (graphicsGeometry.solidIndices.size() + graphicsGeometry.waterIndices.size()) / 6;
    ...
    return uploadGeometry(updateChunkNode(chunk), packInfo.second, sizeof(GraphicsVertexPacked), &graphicsGeometry.vertices[0],
        graphicsGeometry.vertices.size(), graphicsGeometry.solidIndices, graphicsGeometry.waterIndices);
}
