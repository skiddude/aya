...
{
public:
    RenderEntity(RenderNode* node, const GeometryBatch& geometry, const shared_ptr<Material>& material, RenderQueue::Id renderQueueId,
        unsigned char lodMask = 0xff);
    ...
    void computeStencilGeometry(RenderOperation& ope);
...
protected:
    ...
    shared_ptr<VertexBuffer> shadow_vertex;
    shared_ptr<IndexBuffer> shadow_index;
    GeometryBatch* shadow_geometry;
...
public:
    ...
    const std::vector<RenderEntity*>& getEntities() const
    {
        return entities;
    }
