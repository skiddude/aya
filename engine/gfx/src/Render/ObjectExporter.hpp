...
{
public:
    ObjectExporter(VisualEngine* visEngine, DataModel* dataModel)
    ...
    static bool exportToJSON(ExporterSaveType saveType, ExporterFormat fileFormat, DataModel* dataModel, VisualEngine* visualEngine,
        bool encodeBase64, std::string* strOut);
...
protected:
    ...
    struct ExporterMaterial
    {
        ...
        ExporterMaterial(
            const std::string& nameIn, const Color4uint8& colorIn, float specularIntensityIn, float specularPowerIn, shared_ptr<Texture> textureIn)
            : name(nameIn)
            , color(colorIn)
            , texture(textureIn)
        ...
        {
            return color == other.color && texture == other.texture && specularIntensity == other.specularIntensity &&
    ...
    void createSmoothTerrainModel(const std::vector<Voxel2::Mesher::GraphicsVertex>& vertices, const std::vector<unsigned int>& indices,
...
{
public:
    ObjectExporterObj(VisualEngine* visEngine, DataModel* dataModel)
        : ObjectExporter(visEngine, dataModel)
    {
    }
