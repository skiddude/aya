...
struct SpatialGridIndex
{
    ...
    SpatialGridIndex(const Vector3int16& position, unsigned short flags)
        : position(position)
        , flags(flags)
    {
    }
...
public:
    ...
    SpatialGridIndex getIndexUnsafe(Aya::PartInstance* part, unsigned short flags) const
    {
...
private:
    typedef boost::unordered_map<SpatialGridIndex, Cell, boost::hash<SpatialGridIndex>, std::equal_to<SpatialGridIndex>,
        boost::fast_pool_allocator<SpatialGridIndex>>
        SpatialMap;
    SpatialMap mMap;
