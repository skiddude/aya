...
struct UVAtlasInfo
{
    ...
    UVAtlasInfo(bool tileX, float scaleY, float offsetY)
        : tileX(tileX)
        , scaleY(scaleY)
        , offsetY(offsetY)
    {
...
{
    return (options.flags & GeometryGenerator::Flag_TransformUV)
...
inline
#endif
    void fillVertex(GeometryGenerator::Vertex& r, const Vector3& pos, const Vector3& normal, const Vector2& uv, const Vector2& uvStuds,
        const Color4uint8& color, const Color4uint8& extra, const Vector3& tangent,
...
{
    r[0] = (unsigned short)(offset + i00);
    r[1] = (unsigned short)(offset + i10);
    r[2] = (unsigned short)(offset + i01);

    r[3] = (unsigned short)(offset + i01);
    r[4] = (unsigned short)(offset + i10);
    r[5] = (unsigned short)(offset + i11);
}
...
inline void extendBoundsBlock(Vector3& boundsMin, Vector3& boundsMax, const Vector3& center, const Vector3& size, const Vector3& axisX,
    const Vector3& axisY, const Vector3& axisZ)
{
    Vector3 extent = size * 0.5f;

    Vector3 boundsExtent = Vector3(fabsf(axisX.x) * extent.x + fabsf(axisY.x) * extent.y + fabsf(axisZ.x) * extent.z,
...
#define LOOKUP(ifBevel, ifRoundness, ifBulge, head) \
    if (G3D::fuzzyEq(bevel, ifBevel) && G3D::fuzzyEq(roundness, ifRoundness) && G3D::fuzzyEq(bulge, ifBulge)) \
    return MeshId("ayaasset://avatar/heads/head" head ".mesh")
...
static float getPartTransparency(PartInstance* part)
{
    ...
    return (part->getRenderMaterial() == PLASTIC_MATERIAL) ? part->getTransparencyUi() * (1 - getPartReflectance(part)) : part->getTransparencyUi();
}
...
static Color4uint8 getBaseColor(PartInstance* part, Decal* decal, DataModelMesh* specialShape, const GeometryGenerator::Options& options)
{
    ...
    // Decal uses its color property for tinting
    if (decal)
        return Color4uint8(Color3uint8(decal->getColor3()), alpha);
    ...
    if (options.flags & GeometryGenerator::Flag_VertexColorPart)
        return Color4uint8(Color3uint8(part->getColor3()), alpha);

    return Color4uint8(255, 255, 255, alpha);
}

static Color4uint8 getColor(
    PartInstance* part, Decal* decal, DataModelMesh* specialShape, const GeometryGenerator::Options& options, unsigned int randomSeed, bool isBlock)
{
    Color4uint8 diffuse = getBaseColor(part, decal, specialShape, options);

    Color4uint8 color = (isBlock && (options.flags & GeometryGenerator::Flag_RandomizeBlockAppearance))
                            ? Color4uint8((diffuse.r & 0xf8) | (randomSeed & 7), (diffuse.g & 0xf8) | (randomSeed & 7),
                                  (diffuse.b & 0xf8) | (randomSeed & 7), diffuse.a)
                            : diffuse;
...
static Color4uint8 getExtra(PartInstance* part, const GeometryGenerator::Options& options)
{
    ...
    return (options.flags & GeometryGenerator::Flag_ExtraIsZero) ? Color4uint8(0, 0, 0, 0)
...
Vector3 verticalRotate(const Vector3& pos)
{
    return vertical ? Vector3(pos.y, -pos.x, pos.z) : pos;
}

template<bool vertical>
Vector2 verticalRotateDecal(const Vector2& pos)
{
    return vertical ? Vector2(-pos.y, pos.x) : pos;
}

template<bool vertical>
NormalId verticalRotate(NormalId id)
{
    if (!vertical)
        return id;
...
template<bool vertical>
Vector2 getDecalUVVertical(Decal* decal, const Vector3& size, bool ignoreSurfaceType)
{
    if (!ignoreSurfaceType)
    {
        if (DecalTexture* texture = decal->fastDynamicCast<DecalTexture>())
        {
            ...
            {
            case NORM_X:
                return Vector2(size.z, size.y) / texture->getStudsPerTile();
            case NORM_Y:
                return Vector2(-size.x, size.z) / texture->getStudsPerTile();
            case NORM_Z:
                return Vector2(size.x, size.y) / texture->getStudsPerTile();
...
template<bool vertical>
void GeometryGenerator::addCylinder(const Vector3& size, const Vector3& center, PartInstance* part, Decal* decal, const Options& options,
    unsigned int randomSeed, bool ignoreMaterialsStuds)
{
    size_t vertexOffset = mVertexCount;
    size_t indexOffset = mIndexCount;
    ...
    Vector3 facesDirU[] = {-axisY, -axisZ, -axisY, -axisY, axisZ, -axisY};

    Vector3 facesDirV[] = {axisZ, axisX, -axisX, -axisZ, axisX, axisX};
    ...
    for (int face = 0; face < 6; ++face)
    {
        ...
        if (face % 3 == 0)
        {
            ...
            Vector2 mainMultiplier =
                decal ? getDecalUVVertical<vertical>(decal, genSize, ignoreMaterialsStuds) : Vector2(primaryDimension, primaryDimension);
            Vector2 normal2d;
            ...
            fillVertex(vertices[vertexOffset + vertexCounter], outPos, normal, (mainUV + surfaceOffset) * surfaceTiling, Vector2::zero(), color,
                extra, tangents[face], Vector4(NO_OUTLINES, NO_OUTLINES, NO_OUTLINES, radius + outlineOffset));
            vertexCounter++;
            ...
            for (unsigned adjFace = 0; adjFace < 4; adjFace++)
            {
                ...
                for (unsigned tess = 0; tess < tesselationFactor + 1; tess++)
                {
                    ...
                    fillVertex(vertices[vertexOffset + vertexCounter], outPos, normal, (mainUV + surfaceOffset) * surfaceTiling, Vector2::zero(),
                        color, extra, tangents[face], Vector4(NO_OUTLINES, NO_OUTLINES, NO_OUTLINES, outlineOffset));
        ...
        else
        {
            ...
            UVAtlasInfo studsAtlasInfo = getStudsAtlasInfo((ignoreMaterialsStuds || (options.flags & Flag_IgnoreStuds))
                                                               ? NO_SURFACE
                                                               : part->getConstPartPrimitive()->getSurfaceType((NormalId)face));
            ...
            Vector2 mainOffsetOrig =
                decal ? getDecalUVVertical<vertical>(decal, genSize, ignoreMaterialsStuds) : Vector2(genSize.x, primaryDimension);
            ...
            {
                unsigned prevVertexCounter = vertexCounter;
                Vector2 studStart = Vector2(studOffsetU,
                    studsAtlasInfo.offsetY + frac(quad * primaryDimensionStuds / quadNumberF) / TESSELATION_PIECE * studsAtlasInfo.scaleY);
                ...
                for (unsigned iU = 0; iU < tesselationPerQuad + 1; iU++)
                {
                    ...
                    for (unsigned iV = 0; iV < 2; iV++)
                    {
                        ...
                        Vector4 edgeDistances = Vector4(NO_OUTLINES, NO_OUTLINES, capLeftOutlineOffset + (iV == 0 ? 0 : genSize.x),
                            capRightOutlineOffset + (iV == 0 ? genSize.x : 0));

                        fillVertex(vertices[vertexOffset + vertexCounter], outPos, worldNormal, mainUV * surfaceTiling, studsV, color, extra,
                ...
                {
                    fillQuadIndices(&indices[indexOffset + faceCounter * 3], vertexOffset + prevVertexCounter, iU * 2, iU * 2 + 1, (iU + 1) * 2,
                        (iU + 1) * 2 + 1);
                    faceCounter += 2;
                }
            }
    ...
    AYAASSERT(faceCounter == 2 * quads + tris);
    AYAASSERT(vertexCounter == totalVerts);
...
void GeometryGenerator::addSphere(const Vector3& size, const Vector3& center, PartInstance* part, Decal* decal, const Options& options,
    unsigned int randomSeed, bool ignoreMaterialsStuds)
{
    size_t vertexOffset = mVertexCount;
    size_t indexOffset = mIndexCount;
    ...
    static const Vector3 facesDirU[] = {-axisY, -axisZ, -axisY, -axisY, axisZ, -axisY};

    static const Vector3 facesDirV[] = {axisZ, axisX, -axisX, -axisZ, axisX, axisX};
    ...
    Vector3 tangents[] = {worldAxisZ, worldAxisX, -worldAxisX, -worldAxisZ, worldAxisX, worldAxisX};
    ...
    for (int face = 0; face < 6; ++face)
    {
        ...
        Vector2 mainOffset =
            decal ? getDecalUV(decal, size, ignoreMaterialsStuds) : abs(Vector2(dot(facesDirV[face], size), dot(facesDirU[face], size)));
        ...
        {
            unsigned prevVertexCounter = vertexCounter;
            Vector2 studStart = Vector2(studOffsetU * tesselationPerQuad,
                studsAtlasInfo.offsetY + frac(quad * primaryDimensionStuds / quadNumberF) / TESSELATION_PIECE * studsAtlasInfo.scaleY);
            ...
            for (unsigned iU = 0; iU < tesselationPerQuad + 1; iU++)
            {
                ...
                for (unsigned iV = 0; iV < tesselationFactor + 1; iV++)
                {
                    ...
                    fillVertex(vertices[vertexOffset + vertexCounter], options.cframe.translation + worldOffset + center, worldNormal,
                        mainCoordsV * surfaceTiling, studsV, color, extra, tangent);
                    vertexCounter++;
                    worlsPosV += worldDirV;
                    studsV.x -= studOffsetU;
                    mainCoordsV.x -= mainOffset.x;
            ...
            for (unsigned iU = 0; iU < tesselationPerQuad; iU++)
            {
                ...
                {
                    fillQuadIndices(&indices[indexOffset + faceCounter * 6], vertexOffset + prevVertexCounter, iU * (tesselationFactor + 1) + iV,
                        iU * (tesselationFactor + 1) + iV + 1, (iU + 1) * (tesselationFactor + 1) + iV, (iU + 1) * (tesselationFactor + 1) + iV + 1);
                    faceCounter++;
                }
            }
        }
...
void GeometryGenerator::addWedge(const Vector3& size, const Vector3& center, PartInstance* part, Decal* decal, const Options& options,
    unsigned int randomSeed, bool ignoreMaterialsStuds)
{
    size_t vertexOffset = mVertexCount;
    size_t indexOffset = mIndexCount;
    ...
    if (quads > 32768 / 4)
    {
        ...
        // This is both to ensure that we never exceed 16-bit index limit even if we had some vertices before, and to avoid potential issues with
    ...
    Vector3 normalOffset = (options.flags & Flag_RandomizeBlockAppearance) ? kNormalOffsetTable[(randomSeed >> 3) & 31] : Vector3(0, 0, 0);
    ...
    else
    {
        struct WEDGESTEPDESC
        {
            ...
            inline WEDGESTEPDESC(const Vector3& faceDir0, const Vector3& faceDir1, float studOffset0, float studOffset1)
                : faceDir0(faceDir0)
                , faceDir1(faceDir1)
                , studOffset0(studOffset0)
                , studOffset1(studOffset1)
            {
            }
        ...
        for (int face = 0; face < ARRAYSIZE(wedgeFacesDesc); ++face)
        {
            ...
            for (int quad = 0; quad < quadNumber - 1; quad++)
            {
                ...
                fillVertex(vertices[vertexOffset + faceCounter * 4 + 0], start0 + offset0, normals[face], uvs[0] * surfaceTiling, uvStuds[0], color,
                    extra, tangents[face], edge0);
                fillVertex(vertices[vertexOffset + faceCounter * 4 + 1], start1 + offset1, normals[face], uvs[1] * surfaceTiling, uvStuds[1], color,
                    extra, tangents[face], edge1);
                fillVertex(vertices[vertexOffset + faceCounter * 4 + 2], start0 + nextOffset0, normals[face], uvs[2] * surfaceTiling, uvStuds[2],
                    color, extra, tangents[face], nextEdge0);
                fillVertex(vertices[vertexOffset + faceCounter * 4 + 3], start1 + nextOffset1, normals[face], uvs[3] * surfaceTiling, uvStuds[3],
                    color, extra, tangents[face], nextEdge1);
            ...
            if (quadNumber > 0)
            {
                ...
                fillVertex(vertices[vertexOffset + faceCounter * 4 + 1], start1 + offset1, normals[face], uvs[1] * surfaceTiling, uvStuds[1], color,
                    extra, tangents[face], edge1);
                fillVertex(vertices[vertexOffset + faceCounter * 4 + 2], corners[desc.end0], normals[face], uv2 * surfaceTiling, uvStuds2, color,
                    extra, tangents[face], nextEdge0);
                fillVertex(vertices[vertexOffset + faceCounter * 4 + 3], corners[desc.end1], normals[face], uv3 * surfaceTiling, uvStuds3, color,
                    extra, tangents[face], nextEdge1);
...
static const float kBevelSize = 0.05f; // Bevel size in studs

void GeometryGenerator::addBlock(const Vector3& size, const Vector3& center, PartInstance* part, Decal* decal, const Options& options,
    unsigned int randomSeed, bool ignoreMaterialsStuds)
{
    size_t vertexOffset = mVertexCount;
    size_t indexOffset = mIndexCount;
...
#ifdef BEVELTRON
    bool pleaseBevel = true;
#else
    bool pleaseBevel = false;
#endif

    float bevelAmount = pleaseBevel ? std::min(kBevelSize, std::min(size.x, std::min(size.y, size.z)) * 0.1f) : 0.0f;
    ...
    if (!decal)
    {
        ...
        // Add quads for bevel edges (12 edges total) and triangles for corners (8 corners)
        if (pleaseBevel && bevelAmount > 0.001f)
        {
            quads += 12; // 12 edge bevels (quads)
            // Note: 8 corner bevels are triangles, which equals 8 * 3 / 6 = 4 quads worth of indices
            // But we'll handle them separately, so we need 8 * 1 triangle = 8 triangles
            // In terms of index count: 8 triangles * 3 indices = 24 indices = 4 quads worth
            // But vertex count: 8 triangles * 3 vertices = 24 vertices (not 32 for quads)
            // We'll just allocate vertices individually
        }
    }
    ...
    // Add vertices and indices for corner triangles if beveling
    if (!decal && pleaseBevel && bevelAmount > 0.001f)
    {
        mVertexCount += 8 * 3; // 8 corners * 3 vertices per triangle
        mIndexCount += 8 * 3;  // 8 corners * 3 indices per triangle
    }
    ...
    // For beveled blocks, we need to inset the corners
    Vector3 bevelInsetX = axisX * bevelAmount;
    Vector3 bevelInsetY = axisY * bevelAmount;
    Vector3 bevelInsetZ = axisZ * bevelAmount;
    ...
    // Base corners (used for non-beveled rendering or as reference)
    Vector3 corners[8] = {
        cornerO,
        cornerO + cornerZ,
        cornerO + cornerY,
    ...
    // If beveling, create beveled corners (inset from edges)
    // We'll use these for the main face geometry
    if (pleaseBevel && bevelAmount > 0.001f)
    {
        // Inset each corner by the bevel amount along all three axes
        corners[0] += bevelInsetX + bevelInsetY + bevelInsetZ;  // ---
        corners[1] += bevelInsetX + bevelInsetY - bevelInsetZ;  // --+
        corners[2] += bevelInsetX - bevelInsetY + bevelInsetZ;  // -+-
        corners[3] += bevelInsetX - bevelInsetY - bevelInsetZ;  // -++
        corners[4] += -bevelInsetX + bevelInsetY + bevelInsetZ; // +--
        corners[5] += -bevelInsetX + bevelInsetY - bevelInsetZ; // +-+
        corners[6] += -bevelInsetX - bevelInsetY + bevelInsetZ; // ++-
        corners[7] += -bevelInsetX - bevelInsetY - bevelInsetZ; // +++
    }
    ...
    Vector3 normals[6] = {
        normalize(normalOffset + axisX),
        normalize(normalOffset + axisY),
        normalize(normalOffset + axisZ),
        normalize(normalOffset - axisX),
        normalize(normalOffset - axisY),
        normalize(normalOffset - axisZ),
    };
    ...
    {
        Vector3 facesDir[] = {-axisY, -axisZ, -axisY, -axisY, axisZ, -axisY};
        ...
        for (int face = 0; face < ARRAYSIZE(facesDesc); ++face)
        {
            ...
            for (int quad = 0; quad < quadNumber - 1; quad++)
            {
                ...
                fillVertex(vertices[vertexOffset + faceCounter * 4 + 0], start0 + offset, normals[face], uvs[0] * surfaceTiling, uvStuds[0], color,
                    extra, tangents[face], edge0 + edgeOffset);
                fillVertex(vertices[vertexOffset + faceCounter * 4 + 1], start1 + offset, normals[face], uvs[1] * surfaceTiling, uvStuds[1], color,
                    extra, tangents[face], edge1 + edgeOffset);
                fillVertex(vertices[vertexOffset + faceCounter * 4 + 2], start0 + nextOffset, normals[face], uvs[2] * surfaceTiling, uvStuds[2],
                    color, extra, tangents[face], edge0 + nextEdgeOffset);
                fillVertex(vertices[vertexOffset + faceCounter * 4 + 3], start1 + nextOffset, normals[face], uvs[3] * surfaceTiling, uvStuds[3],
                    color, extra, tangents[face], edge1 + nextEdgeOffset);
            ...
            if (quadNumber > 0)
            {
                ...
                fillVertex(vertices[vertexOffset + faceCounter * 4 + 1], start1 + offset, normals[face], uvs[1] * surfaceTiling, uvStuds[1], color,
                    extra, tangents[face], edge1 + edgeOffset);
                fillVertex(vertices[vertexOffset + faceCounter * 4 + 2], corners[desc.end0], normals[face], uv2 * surfaceTiling, uvStuds2, color,
                    extra, tangents[face], edge0 + finalEdgeOffset);
                fillVertex(vertices[vertexOffset + faceCounter * 4 + 3], corners[desc.end1], normals[face], uv3 * surfaceTiling, uvStuds3, color,
                    extra, tangents[face], edge1 + finalEdgeOffset);
        ...
        // Generate bevel edge geometry (12 edges: 4 along X, 4 along Y, 4 along Z)
        if (pleaseBevel && bevelAmount > 0.001f)
        {
            // We need the original (non-inset) corner positions for the outer edge of bevels
            Vector3 outerCorners[8] = {
                cornerO,
                cornerO + cornerZ,
                cornerO + cornerY,
                cornerO + cornerY + cornerZ,
            ...
            // Edge bevel structure: each edge connects two corners
            // We'll create a quad strip for each edge from outer corner to inner corner
            struct EdgeBevel
            {
                int corner0, corner1; // The two corners this edge connects
                Vector3 normalDir;    // Direction perpendicular to edge for bevel normal
            };

            // 12 edges defined by corner pairs
            // Edges along X axis (4 edges)
            EdgeBevel edges[12] = {
                {0, 4, normalize(normals[4] + normals[5])}, // Bottom-back edge (Y-, Z-)
                {1, 5, normalize(normals[4] + normals[2])}, // Bottom-front edge (Y-, Z+)
                {2, 6, normalize(normals[1] + normals[5])}, // Top-back edge (Y+, Z-)
                {3, 7, normalize(normals[1] + normals[2])}, // Top-front edge (Y+, Z+)
                // Edges along Y axis (4 edges)
                {0, 2, normalize(normals[3] + normals[5])}, // Left-back edge (X-, Z-)
                {1, 3, normalize(normals[3] + normals[2])}, // Left-front edge (X-, Z+)
                {4, 6, normalize(normals[0] + normals[5])}, // Right-back edge (X+, Z-)
                {5, 7, normalize(normals[0] + normals[2])}, // Right-front edge (X+, Z+)
                // Edges along Z axis (4 edges)
                {0, 1, normalize(normals[3] + normals[4])}, // Left-bottom edge (X-, Y-)
                {2, 3, normalize(normals[3] + normals[1])}, // Left-top edge (X-, Y+)
                {4, 5, normalize(normals[0] + normals[4])}, // Right-bottom edge (X+, Y-)
                {6, 7, normalize(normals[0] + normals[1])}, // Right-top edge (X+, Y+)
            };

            for (int e = 0; e < 12; e++)
            {
                const EdgeBevel& edge = edges[e];

                // Create a quad from outer corners to inner (beveled) corners
                Vector3 outer0 = outerCorners[edge.corner0];
                Vector3 outer1 = outerCorners[edge.corner1];
                Vector3 inner0 = corners[edge.corner0];
                Vector3 inner1 = corners[edge.corner1];

                // Simple UVs for bevel edges
                Vector2 bevelUV0(0, 0);
                Vector2 bevelUV1(1, 0);
                Vector2 bevelUV2(0, 1);
                Vector2 bevelUV3(1, 1);

                fillVertex(vertices[vertexOffset + faceCounter * 4 + 0], outer0, edge.normalDir, bevelUV0, Vector2(), color, extra, Vector3());
                fillVertex(vertices[vertexOffset + faceCounter * 4 + 1], outer1, edge.normalDir, bevelUV1, Vector2(), color, extra, Vector3());
                fillVertex(vertices[vertexOffset + faceCounter * 4 + 2], inner0, edge.normalDir, bevelUV2, Vector2(), color, extra, Vector3());
                fillVertex(vertices[vertexOffset + faceCounter * 4 + 3], inner1, edge.normalDir, bevelUV3, Vector2(), color, extra, Vector3());

                fillQuadIndices(&indices[indexOffset + faceCounter * 6], vertexOffset + faceCounter * 4, 0, 1, 2, 3);
                faceCounter++;
            }

            // Generate corner bevel triangles (8 corners, each with 3 beveled edges meeting)
            // At each corner, we need a triangle connecting the 3 outer edge points to form the corner bevel
            // The 8 corners and their adjacent edge directions:
            struct CornerBevel
            {
                int cornerIdx;    // Which corner (0-7)
                Vector3 edgeDir1; // First edge direction from this corner
                Vector3 edgeDir2; // Second edge direction
                Vector3 edgeDir3; // Third edge direction
            };

            CornerBevel cornerBevels[8] = {
                {0, cornerX, cornerY, cornerZ},    // Corner 0: extends in +X, +Y, +Z
                {1, cornerX, cornerY, -cornerZ},   // Corner 1: extends in +X, +Y, -Z
                {2, cornerX, -cornerY, cornerZ},   // Corner 2: extends in +X, -Y, +Z
                {3, cornerX, -cornerY, -cornerZ},  // Corner 3: extends in +X, -Y, -Z
                {4, -cornerX, cornerY, cornerZ},   // Corner 4: extends in -X, +Y, +Z
                {5, -cornerX, cornerY, -cornerZ},  // Corner 5: extends in -X, +Y, -Z
                {6, -cornerX, -cornerY, cornerZ},  // Corner 6: extends in -X, -Y, +Z
                {7, -cornerX, -cornerY, -cornerZ}, // Corner 7: extends in -X, -Y, -Z
            };

            // We need to track triangle vertex/index offset separately
            unsigned int triangleVertexBase = vertexOffset + faceCounter * 4;
            unsigned int triangleIndexBase = indexOffset + faceCounter * 6;

            for (int c = 0; c < 8; c++)
            {
                const CornerBevel& cb = cornerBevels[c];

                // The three outer points along the edges from this corner
                // Each outer point is offset from the outer corner by bevelAmount in one direction
                Vector3 outer1 = outerCorners[cb.cornerIdx] + normalize(cb.edgeDir1) * bevelAmount;
                Vector3 outer2 = outerCorners[cb.cornerIdx] + normalize(cb.edgeDir2) * bevelAmount;
                Vector3 outer3 = outerCorners[cb.cornerIdx] + normalize(cb.edgeDir3) * bevelAmount;

                // Calculate corner bevel normal (pointing outward from corner)
                Vector3 cornerNormal = normalize(outerCorners[cb.cornerIdx] - corners[cb.cornerIdx]);

                // Create triangle vertices
                fillVertex(vertices[triangleVertexBase + c * 3 + 0], outer1, cornerNormal, Vector2(0, 0), Vector2(), color, extra, Vector3());
                fillVertex(vertices[triangleVertexBase + c * 3 + 1], outer2, cornerNormal, Vector2(1, 0), Vector2(), color, extra, Vector3());
                fillVertex(vertices[triangleVertexBase + c * 3 + 2], outer3, cornerNormal, Vector2(0.5f, 1), Vector2(), color, extra, Vector3());

                // Create triangle indices
                indices[triangleIndexBase + c * 3 + 0] = (unsigned short)(triangleVertexBase + c * 3 + 0);
                indices[triangleIndexBase + c * 3 + 1] = (unsigned short)(triangleVertexBase + c * 3 + 1);
                indices[triangleIndexBase + c * 3 + 2] = (unsigned short)(triangleVertexBase + c * 3 + 2);
            }
        }
...
void GeometryGenerator::addTorso(
    const Vector3& size, const Vector3& center, PartInstance* part, Decal* decal, const Options& options, unsigned int randomSeed)
{
    int quads = decal ? 1 : 6;
    ...
    Vector3 normals[6] = {
        normalize(Vector3(size.y, shoulderInset, 0)),
        Vector3(0, 1, 0),
        Vector3(0, 0, 1),
        normalize(Vector3(-size.y, shoulderInset, 0)),
        Vector3(0, -1, 0),
    ...
    if (decal)
    {
        ...
        fillVertex(vertices[vertexOffset + 0], cframe.pointToWorldSpace(corners[faces[face][0]]), normal, Vector2(uv.x, 0), Vector2(), color, extra,
            Vector3());
        fillVertex(
            vertices[vertexOffset + 1], cframe.pointToWorldSpace(corners[faces[face][1]]), normal, Vector2(0, 0), Vector2(), color, extra, Vector3());
        fillVertex(vertices[vertexOffset + 2], cframe.pointToWorldSpace(corners[faces[face][2]]), normal, Vector2(uv.x, uv.y), Vector2(), color,
            extra, Vector3());
        fillVertex(vertices[vertexOffset + 3], cframe.pointToWorldSpace(corners[faces[face][3]]), normal, Vector2(0, uv.y), Vector2(), color, extra,
            Vector3());
    ...
    else
    {
        ...
        for (int face = 0; face < ARRAYSIZE(facesDesc); ++face)
        {
            ...
            fillVertex(vertices[vertexOffset + faceCounter * 4 + 0], cframe.pointToWorldSpace(start0), normal,
                (surfaceOffset + Vector2(uTiling, 0)) * surfaceTiling, Vector2(), color, extra, tangent);
            fillVertex(vertices[vertexOffset + faceCounter * 4 + 1], cframe.pointToWorldSpace(start1), normal, surfaceOffset * surfaceTiling,
                Vector2(), color, extra, tangent);
            fillVertex(vertices[vertexOffset + faceCounter * 4 + 2], cframe.pointToWorldSpace(end0), normal,
                (surfaceOffset + Vector2(uTiling, vTiling)) * surfaceTiling, Vector2(), color, extra, tangent);
            fillVertex(vertices[vertexOffset + faceCounter * 4 + 3], cframe.pointToWorldSpace(end1), normal,
                (surfaceOffset + Vector2(0, vTiling)) * surfaceTiling, Vector2(), color, extra, tangent);
...
struct TrussQuadBuilder
{
    ...
    TrussQuadBuilder(GeometryGenerator::Vertex* vertices, unsigned short* indices, unsigned int vertexOffset, unsigned int indexOffset,
        Vector3* bboxMin, Vector3* bboxMax, const Vector3& size, PartInstance* part, const GeometryGenerator::Options& options,
    ...
    void emit(const Vector3& pos, const Vector3& udir, float usize, const Vector3& vdir, float vsize)
    {
        if (vertices)
        {
            ...
            fillVertex(vertices[vertexOffset + 0], cframe.pointToWorldSpace(corner0), normal,
                (surfaceOffset + Vector2(dot(corner0, udir), dot(corner0, vdir))) * surfaceTiling, Vector2(), color, extra, tangent);
            fillVertex(vertices[vertexOffset + 1], cframe.pointToWorldSpace(corner1), normal,
                (surfaceOffset + Vector2(dot(corner1, udir), dot(corner1, vdir))) * surfaceTiling, Vector2(), color, extra, tangent);
            fillVertex(vertices[vertexOffset + 2], cframe.pointToWorldSpace(corner2), normal,
                (surfaceOffset + Vector2(dot(corner2, udir), dot(corner2, vdir))) * surfaceTiling, Vector2(), color, extra, tangent);
            fillVertex(vertices[vertexOffset + 3], cframe.pointToWorldSpace(corner3), normal,
                (surfaceOffset + Vector2(dot(corner3, udir), dot(corner3, vdir))) * surfaceTiling, Vector2(), color, extra, tangent);
...
void GeometryGenerator::addTruss(int style, const Vector3& size, PartInstance* part, Decal* decal, const Options& options, unsigned int randomSeed)
{
    ...
    {
    case Vector3::X_AXIS:
        return addTrussY(Vector3(-size.x / 2, 0, 0), Vector3(0, 0, -1), Vector3(1, 0, 0), Vector3(0, -1, 0), static_cast<int>(size.x / 2 + 0.005f),
            style, size, part, options, randomSeed);

    case Vector3::Y_AXIS:
        return addTrussY(Vector3(0, -size.y / 2, 0), Vector3(1, 0, 0), Vector3(0, 1, 0), Vector3(0, 0, 1), static_cast<int>(size.y / 2 + 0.005f),
            style, size, part, options, randomSeed);

    case Vector3::Z_AXIS:
        return addTrussY(Vector3(0, 0, -size.z / 2), Vector3(1, 0, 0), Vector3(0, 0, 1), Vector3(0, -1, 0), static_cast<int>(size.z / 2 + 0.005f),
            style, size, part, options, randomSeed);
    default:
        break;
    }
...
void GeometryGenerator::addTrussY(const Vector3& origin, const Vector3& axisX, const Vector3& axisY, const Vector3& axisZ, int segments, int style,
    const Vector3& size, PartInstance* part, const Options& options, unsigned int randomSeed)
{
    if (segments == 0)
        return;
...
void GeometryGenerator::addTrussSideX(const Vector3& origin, const Vector3& axisX, const Vector3& axisY, const Vector3& axisZ, bool reverseBridge,
    int segments, int style, const Vector3& size, PartInstance* part, const Options& options, unsigned int randomSeed)
{
    ...
    if (style != ExtrudedPartInstance::NO_CROSS_BEAM && segments > 1)
    {
        ...
        {
            bool reverse =
                style == ExtrudedPartInstance::FULL_ALTERNATING_CROSS_BEAM ? (i % 2 == 1) : (reverseBridge ? i >= segments / 2 : i < segments / 2);
...
void GeometryGenerator::addPartImpl(
    PartInstance* part, Decal* decal, const Options& options, const Resources& resources, const HumanoidIdentifier* hi, bool ignoreMaterialsStuds)
{
    ...
    if (specialShape)
    {
        if (SpecialShape* shape = specialShape->fastDynamicCast<SpecialShape>())
        {
            ...
            case SpecialShape::CYLINDER_MESH:
                return addCylinder<false>(
                    part->getPartSizeXml() * abs(shape->getScale()), offset, part, decal, options, randomSeed, ignoreMaterialsStuds);
        ...
        {
            return addBlock(part->getPartSizeXml() * abs(shape->getScale()), shape->getOffset(), part, decal, options, randomSeed,
                /* ignoreMaterialsStuds= */ true);
        }
        else if (CylinderMesh* shape = specialShape->fastDynamicCast<CylinderMesh>())
        {
            return addCylinder<true>(part->getPartSizeXml() * abs(shape->getScale()), shape->getOffset(), part, decal, options, randomSeed,
                /* ignoreMaterialsStuds= */ true);
        }
...
static boost::shared_ptr<AssetType> fetchMesh(const AssetIdType& meshFile, Instance* instance, AsyncResult* asyncResult)
{
    ...
    boost::shared_ptr<AssetType> meshData =
        boost::static_pointer_cast<AssetType>(mcp->requestContent(meshFile, ContentProvider::PRIORITY_MESH, true, reqResult));
...
GeometryGenerator::Resources GeometryGenerator::fetchResources(
    PartInstance* part, const HumanoidIdentifier* hi, unsigned int flags, AsyncResult* asyncResult)
{
    DataModelMesh* specialShape = getSpecialShape(part);
    ...
    if (MeshPartInstance* meshPart = part->fastDynamicCast<MeshPartInstance>())
    {
        ...
        {
            AYAASSERT(hi && hi->humanoid == humanoid);
    ...
    if (part->getCookie() & PartCookie::HAS_HEADMESH)
    {
        ...
        boost::shared_ptr<FileMeshData> fileMeshData =
            fetchMesh<MeshContentProvider, FileMeshData, MeshId>(MeshId("ayaasset://fonts/head.mesh"), part, asyncResult);
        return Resources(fileMeshData ? fileMeshData : kDummyMeshData);
    }
    ...
    if (FFlag::StudioCSGAssets && !FFlag::CSGLoadBlocking)
    {
        if (PartOperation* operation = part->fastDynamicCast<PartOperation>())
        {
            ...
            {
                shared_ptr<PartOperationAsset> partOperationAsset =
                    fetchMesh<SolidModelContentProvider, PartOperationAsset, ContentId>(operation->getAssetId(), part, asyncResult);
...
void GeometryGenerator::addInstance(
    PartInstance* part, Decal* decal, const Options& options, const Resources& resources, const HumanoidIdentifier* humanoidIdentifier /*=NULL*/)
{
    ...
    AYAASSERT(!mVertices || mVertexCount < (1 << 16));
}
...
GeometryGenerator::Options::Options(VisualEngine* visualEngine, const PartInstance& part, Decal* decal, const CoordinateFrame& localTransform,
    unsigned int materialFlags, const Vector4& uvOffsetScale, unsigned int extra)
{
    unsigned int randomizeFlag = 0;

    if (FFlag::NoRandomColorsWithoutOutlines)
    {
        randomizeFlag = shouldRandomizeColor(visualEngine, part, decal) ? GeometryGenerator::Flag_RandomizeBlockAppearance : 0;
    }
    else
    {
        randomizeFlag = decal && decal->isA<DecalTexture>() ? 0 : GeometryGenerator::Flag_RandomizeBlockAppearance;
    }
    ...
    unsigned int colorOrderFlag = visualEngine->getDevice()->getCaps().colorOrderBGR ? GeometryGenerator::Flag_DiffuseColorBGRA : 0;

    unsigned int studFlag = (visualEngine->getSettings()->getDrawConnectors() || part.getRenderMaterial() == Aya::PLASTIC_MATERIAL)
                                ? 0
                                : GeometryGenerator::Flag_IgnoreStuds;

    unsigned int extraFlag = visualEngine->getRenderCaps()->getSkinningBoneCount() > 0 ? 0 : GeometryGenerator::Flag_ExtraIsZero;
...
void GeometryGenerator::addOperation(PartInstance* part, Decal* decal, const Options& options, const Resources& resources, unsigned int randomSeed)
{
    ...
    Vector3 size = part->getPartSizeXml();
    Aya::Color4uint8 partColor = getColor(part, decal, NULL, options, randomSeed, true);
    const Aya::Color4uint8& extra = getExtra(part, options);
    ...
    if (isNegate)
    {
        partColor = Aya::Color4uint8(255, 154, 161, 150);
    ...
    if (!FFlag::FixGlowingCSG)
    {
        for (size_t vi = 0; vi < meshVertices.size(); vi++)
        {
            ...
            {
                uv = Vector2(uv.x * surfaceTiling + surfaceOffset.x, uv.y * surfaceTiling + surfaceOffset.y);
            }
            ...
            fillVertex(vertices[vertexOffset + vi], pos, normal, uv, uvStuds,
                vertColor /*Color4uint8(rand() % 255, rand() % 255, rand() % 255, 255)*/, extra, tangent);
        }

        for (size_t ii = 0; ii < meshIndices.size(); ii++)
        {
            indices[indexOffset + ii] = (unsigned short)(vertexOffset + meshIndices[ii]);
        }
    }
    else
    {
        ...
        else
        {
            ...
            for (size_t ii = 0; ii < indexFaceRamep.size(); ii++)
                indices[indexOffset + ii] = (unsigned short)(vertexOffset + indexFaceRamep[ii]);
        }
    }

    Vector3 boundsCenter = cframe.pointToWorldSpace(Aya::Vector3::zero());
    Vector3 axisX = Vector3(cframe.rotation[0][0], cframe.rotation[1][0], cframe.rotation[2][0]);
    Vector3 axisY = Vector3(cframe.rotation[0][1], cframe.rotation[1][1], cframe.rotation[2][1]);
    Vector3 axisZ = Vector3(cframe.rotation[0][2], cframe.rotation[1][2], cframe.rotation[2][2]);
...
void GeometryGenerator::addOperationDecompositionDebug(
    PartOperation* operation, PartInstance* part, Decal* decal, const Options& options, unsigned int randomSeed)
{
...
void GeometryGenerator::fillBlockShapeDebug(PartInstance* part, const Options& options)
{
    ...
    Color4uint8 color = Aya::Color4uint8(randGen.value() % 255, randGen.value() % 255, randGen.value() % 255, 100);
    Color4uint8 extra = getExtra(part, options);
...
void GeometryGenerator::fillDecompShapeDebug(PartOperation* operation, PartInstance* part, const Options& options)
{
    ...
    if (operation->getPrimitive(operation)->getGeometry()->getGeometryType() != Aya::Geometry::GEOMETRY_TRI_MESH)
        return;
    ...
    for (unsigned int i = 0; i < meshConvexes.size(); i++)
    {
        const Aya::Color4uint8 color = Aya::Color4uint8(randGen.value() % 255, randGen.value() % 255, randGen.value() % 255, 100);
        ...
        {
            indices[indexOffset + childIndexOffset + j] = (unsigned short)(vertexOffset + childVertexOffset + meshConvexes[i].indices[j]);
        }
