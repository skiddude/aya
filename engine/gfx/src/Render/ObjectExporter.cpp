...
bool ObjectExporter::exportToFile(
    const std::string& fileName, ExporterSaveType saveType, ExporterFormat fileFormat, DataModel* dataModel, VisualEngine* visualEngine)
{
...
bool ObjectExporter::exportToJSON(
    ExporterSaveType saveType, ExporterFormat fileFormat, DataModel* dataModel, VisualEngine* visualEngine, bool encodeBase64, std::string* strOut)
{
    scoped_ptr<ObjectExporter> exporter;
    switch (fileFormat)
    {
...
void ObjectExporter::createTerrainModel(
    const TerrainVerts& vertArray, const std::string& name, const ExporterMaterial* material, const Vector3& toWorldSpace)
{
...
void ObjectExporter::createSmoothTerrainModel(const std::vector<Voxel2::Mesher::GraphicsVertex>& vertices, const std::vector<unsigned int>& indices,
    const std::string& name, const ExporterMaterial* material)
{
...
void ObjectExporter::prepareTerrain(MegaClusterInstance* megaCluster)
{
    ...
    TextureRef texRef = visualEngine->getTextureManager()->load(ContentId(MegaCluster::kTerrainTexClose + ".jxl"), TextureManager::Fallback_White);
...
void ObjectExporter::processPart(PartInstance* part, Decal* decal)
{
    ...
    if (count.getVertexCount() > 0 && count.getIndexCount() >= 3)
    {
        ...
        options = GeometryGenerator::Options(
            visualEngine, *part, decal, part->getPart().coordinateFrame, materialResult.flags, materialResult.uvOffsetScale, 0);
        ...
        if (!techniques.empty())
        {
            ...
            if (texRef.getTexture() && !visualEngine->getTextureManager()->isFallbackTexture(texRef.getTexture()) &&
...
bool ObjectExporterObj::saveFiles(const std::string& fileName)
{
    if (!models.empty())
    {
        ...
        exportFile << "# Generated by Aya Studio\n\n";
...
void ObjectExporterObj::exportIndices(
    std::ostream& exportFile, const char* keyWord, const unsigned short* indices, unsigned startIndex, unsigned offset)
{
    exportFile << keyWord << " ";
    for (char i = 0; i < 3; ++i)
    {
