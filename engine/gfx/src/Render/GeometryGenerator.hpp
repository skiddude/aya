...
public:
    ...
    struct Options
    {
        ...
        Options(VisualEngine* visualEngine, const PartInstance& part, Decal* decal, const CoordinateFrame& localTransform, unsigned int materialFlags,
            const Vector4& uvOffsetScale, unsigned int extra);
    ...
    public:
        Resources() {}
        Resources(boost::shared_ptr<FileMeshData> fileMesh)
        {
            fileMeshData = fileMesh;
        }
        Resources(boost::shared_ptr<CSGMesh> csgMesh)
        {
            csgMeshData = csgMesh;
        }
    ...
    {
        AYAASSERT(mVertices); // geometry is not generated if mVertices is NULL
...
private:
    ...
    void addPartImpl(PartInstance* part, Decal* decal, const Options& options, const Resources& resources, const HumanoidIdentifier* hi,
        bool ignoreMaterialsStuds);

    void addBlock(const Vector3& size, const Vector3& center, PartInstance* part, Decal* decal, const Options& options, unsigned int randomSeed,
        bool ignoreMaterialsStuds);
    template<bool corner>
    void addWedge(const Vector3& size, const Vector3& center, PartInstance* part, Decal* decal, const Options& options, unsigned int randomSeed,
        bool ignoreMaterialsStuds);
    void addSphere(const Vector3& size, const Vector3& center, PartInstance* part, Decal* decal, const Options& options, unsigned int randomSeed,
        bool ignoreMaterialsStuds);
    template<bool vertical>
    void addCylinder(const Vector3& size, const Vector3& center, PartInstance* part, Decal* decal, const Options& options, unsigned int randomSeed,
        bool ignoreMaterialsStuds);
    ...
    void addTrussY(const Vector3& origin, const Vector3& axisX, const Vector3& axisY, const Vector3& axisZ, int segments, int style,
        const Vector3& size, PartInstance* part, const Options& options, unsigned int randomSeed);
    void addTrussSideX(const Vector3& origin, const Vector3& axisX, const Vector3& axisY, const Vector3& axisZ, bool reverseBridge, int segments,
        int style, const Vector3& size, PartInstance* part, const Options& options, unsigned int randomSeed);
    void addFileMesh(FileMeshData* data, DataModelMesh* specialShape, PartInstance* part, Decal* decal, const HumanoidIdentifier* humanoidIdentifier,
