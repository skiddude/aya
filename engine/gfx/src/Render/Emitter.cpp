...
void EmitterShared::init(VisualEngine* ve)
{
    ...
    {
        vlayout =
            dev->createVertexLayout(std::vector<VertexLayout::Element>(kVertexDecl, kVertexDecl + sizeof(kVertexDecl) / sizeof(kVertexDecl[0])));
    }
    ...
    : enableCurves(enableCurves_)
{
    ...
    def.mainTexture = "ayaasset://textures/particles/sparkles_main.jxl";
    def.colorStripTexture = "ayaasset://textures/particles/sparkles_color.jxl";
    def.alphaStripTexture = "ayaasset://textures/particles/common_alpha.jxl";
...
void Emitter::draw(RenderQueue& rq)
{
    ...
    AYAASSERT((char*)sharedState->vblock <= (char*)sharedState->vbptr && (char*)sharedState->vbptr <= (char*)sharedState->vbend);
...
static BlendState createBlendState(Emitter::BlendMode blendMode)
{
    ...
    case Emitter::Blend_AlphaOne:
        ...
        break;
    default:
        AYAASSERT(false); // did you add new mode?
...
void Emitter::setAppearance(const Appearance& a, const std::string& context)
{
    ...
    if (!sharedState->shaders[a.shader])
        return;
