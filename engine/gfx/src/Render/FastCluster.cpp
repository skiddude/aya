...
#include "Render/SuperCluster.hpp"
...
// Shadow Volume Mesh Generation
// Implementation loosely based on 2013/2007 symbols, whatever
// @mdolli

class FastClusterShadowGenerator
{
public:
    void generateShadowVolume(GeometryGenerator::Vertex* vertices, unsigned short* indices, unsigned int vertexCount, unsigned int indexCount,
        std::vector<Vector3>* shadowVertices, std::vector<unsigned short>* shadowIndices, GlobalShaderData shaderData)
    {
        lightDir = shaderData.Lamp0Dir.xyz();
        computeSilhouetteEdges(vertices, indices, vertexCount, indexCount);

        for (const auto& edge : silhouetteEdges)
        {
            if (edge.triIdx2 == -1)
            { // Only process silhouette edges
                Vector3 v0 = vertices[edge.idx0].pos;
                Vector3 v1 = vertices[edge.idx1].pos;
                Vector3 v0Extruded = v0 + (lightDir * 1000000000.0f);
                Vector3 v1Extruded = v1 + (lightDir * 1000000000.0f);

                // Add original and extruded vertices
                shadowVertices->push_back(v0);
                shadowVertices->push_back(v1);
                shadowVertices->push_back(v0Extruded);
                shadowVertices->push_back(v1Extruded);

                // Create quad faces (two triangles) for the edge
                int baseIndex = shadowVertices->size() - 4;
                shadowIndices->push_back(baseIndex);
        ...
        /*// Add near cap (original vertices and indices)
        ...
        {
            shadowIndices->push_back(indices[i]);
            shadowIndices->push_back(indices[i + 1]);
            shadowIndices->push_back(indices[i + 2]);
        }

        // Add far cap (extruded vertices)
        for(int i = 0; i < vertexCount; i++)
        {
            Vector3 extruded = vertices[i].pos + (lightDir * 1000.0f);
            shadowVertices->push_back(extruded);
        }

        // Add far cap indices, reverse winding order
        for(size_t i = 0; i < indexCount; i += 3)
        {
            shadowIndices->push_back(vertexCount + indices[i]);
            shadowIndices->push_back(vertexCount + indices[i + 1]);
            shadowIndices->push_back(vertexCount + indices[i + 2]);
        } */
    }

    void computeSilhouetteEdges(GeometryGenerator::Vertex* vertices, unsigned short* indices, unsigned int vertexCount, unsigned int indexCount)
    {
        silhouetteEdges.clear();

        std::vector<m_Triangle> triangles;
        ...
        for (size_t i = 0; i < indexCount; i += 3)
        {
            ...
            m_Triangle triangle;
            triangle.a = vertices[idx0];
            triangle.b = vertices[idx1];
            triangle.c = vertices[idx2];
            triangle.indices[0] = idx0;
            triangle.indices[1] = idx1;
            triangle.indices[2] = idx2;

            Vector3 normal = (vertices[idx1].pos - vertices[idx0].pos).cross(vertices[idx2].pos - vertices[idx0].pos);
            bool isFrontFacing = normal.dot(lightDir) > 0;

            triangle.lightFacing = isFrontFacing;
            triangles.push_back(triangle);

            if (isFrontFacing)
            {
                m_Edge edge1(idx0, idx1, triangles.size() - 1);
                m_Edge edge2(idx1, idx2, triangles.size() - 1);
                m_Edge edge3(idx2, idx0, triangles.size() - 1);

                auto add_or_update_edge = [&](m_Edge& edge)
                {
                    auto it = std::find_if(silhouetteEdges.begin(), silhouetteEdges.end(),
                        [&](const m_Edge& e)
                        {
                            return (e.idx0 == edge.idx0 && e.idx1 == edge.idx1) || (e.idx0 == edge.idx1 && e.idx1 == edge.idx0);
                        });
                    if (it != silhouetteEdges.end())
                    {
                        if (it->triIdx1 == -1)
                        {
                            it->triIdx1 = edge.triIdx1;
                        }
                        else
                        {
                            it->triIdx2 = edge.triIdx1;
                        }
                    }
                    else
                    {
                ...
                add_or_update_edge(edge1);
                add_or_update_edge(edge2);
                add_or_update_edge(edge3);
            }
        }
    }
...
private:
    ...
    struct m_Edge
    {
        ...
        m_Edge(unsigned int _idx0, unsigned int _idx1, int _triangleIdx1)
            : idx0(_idx0)
            , idx1(_idx1)
            , triIdx1(_triangleIdx1)
            , triIdx2(-1)
        {
        }
    };

    std::vector<m_Edge> silhouetteEdges;
};
class FastClusterMeshGenerator
{
public:
    FastClusterMeshGenerator(VisualEngine* visualEngine, Aya::Humanoid* humanoid, unsigned int maxBones, bool isFW)
    ...
    void addBone(Aya::PartInstance* root)
    {
        Bone bone;
        bone.root = root;
    ...
    void addInstance(size_t boneIndex, Aya::PartInstance* part, Aya::Decal* decal, unsigned int materialFlags, RenderQueue::Id renderQueue,
        Aya::AsyncResult* asyncResult)
    {
        AYAASSERT(boneIndex < bones.size());
        ...
        {
            AYAASSERT(mg.materialResultFlags == material.flags);
        }
        ...
        {
            AYAASSERT(batch.bones.size() < maxBonesPerBatch);
    ...
    unsigned int finalize(FastCluster* cluster, FastClusterSharedGeometry& sharedGeometry)
    {
        ...
        for (size_t groupBegin = 0; groupBegin < batches.size();)
        {
            ...
            {
                geometry = visualEngine->getDevice()->createGeometry(
                    getVertexLayout(), sharedGeometry.vertexBuffer, sharedGeometry.indexBuffer, groupVertexOffset);
            ...
            for (size_t bi = groupBegin; bi < groupEnd; ++bi)
            {
                ...
                Extents bounds = generateBatchGeometry(mg, batch, sharedVertexData + geometryBaseVertex, sharedIndexData + sharedIndexOffset,
                    vertexOffset, instanceVertexCount, cluster->isFW());
                ...
                GeometryBatch geometryBatch(geometry, Geometry::Primitive_Triangles, sharedIndexOffset, batch.counter.getIndexCount(), vertexOffset,
                    vertexOffset + batch.counter.getVertexCount());
                ...
                cluster->addEntity(new FastClusterEntity(
                    cluster, geometryBatch, mg.material, mg.decalMaterialOpaque, queueId, lodMask, batch.bones, bounds, mg.materialFeatures));
            ...
            if (groupEnd - groupBegin > 1)
            {
                ...
                // create geometry batch
                GeometryBatch geometryBatch(geometry, Geometry::Primitive_Triangles, groupIndexOffset, sharedIndexOffset - groupIndexOffset,
                    groupVertexOffset - geometryBaseVertex, sharedVertexOffset - geometryBaseVertex);

                // setup entity
                cluster->addEntity(new FastClusterEntity(cluster, geometryBatch, mg.material, mg.decalMaterialOpaque, mg.renderQueue,
                    /* lodMask= */ 1 << 2, batch.bones, groupBounds, mg.materialFeatures));
            }
        ...
        {
            AYAASSERT(sharedVertexOffset == sharedVertexCount && sharedIndexOffset == sharedIndexCount);

            AYAPROFILER_SCOPE("Render", "upload");
        ...
        /*
        {
            // Theoretically, we can generate shadow volumes here
            FastClusterShadowGenerator shadowGenerator;

            // ...race condition?
            // rj: generally speaking w/o a scene (and therefore a scenemanager) there won't be any fast clusters, so this should be safe to comment
        out?
            // d9mz: only make stencils when 2014 or 2013 or 2012!

            if(visualEngine->getSceneManager() != nullptr && Aya::GameBasicSettings::singleton().getVirtualVersion() >
        Aya::GameBasicSettings::VERSION_2014)
            {
                std::vector<Vector3> shadowVertices;
                std::vector<unsigned short> shadowIndices;

                GlobalShaderData shaderData = visualEngine->getSceneManager()->readGlobalShaderData();
                shadowGenerator.generateShadowVolume(sharedVertexData, sharedIndexData, sharedVertexCount, sharedIndexCount, &shadowVertices,
        &shadowIndices, shaderData);

                shared_ptr<VertexBuffer> vertexBuffer = visualEngine->getDevice()->createVertexBuffer(sizeof(Vector3), shadowVertices.size(),
        GeometryBuffer::Usage_Static); shared_ptr<IndexBuffer> indexBuffer = visualEngine->getDevice()->createIndexBuffer(sizeof(unsigned short),
        shadowIndices.size(), GeometryBuffer::Usage_Static);

                vertexBuffer->upload(0, shadowVertices.data(), (sizeof(Vector3) * shadowVertices.size()));
                indexBuffer->upload(0, shadowIndices.data(), (sizeof(unsigned short) * shadowIndices.size()));
                ...
                shared_ptr<Geometry> shadowGeometry = visualEngine->getDevice()->createGeometry(layout, vertexBuffer, indexBuffer);
                GeometryBatch geometryBatch(shadowGeometry, Geometry::Primitive_Triangles, shadowIndices.size(), shadowVertices.size());

                // setup entity
                cluster->addEntity(new RenderEntity(cluster, geometryBatch, visualEngine->getMaterialGenerator()->createStencilShadowMaterial(),
        RenderQueue::Id_Shadows));
            }
        }
        */
    ...
    Aya::PartInstance* getBoneRoot(unsigned int i)
    {
        return bones[i].root;
    }
...
private:
    ...
    struct Bone
    {
        Aya::PartInstance* root;
        Aya::Vector3 boundsMin;
        Aya::Vector3 boundsMax;
    };
    ...
    Aya::PartInstance* getLastPart(const MaterialGroup& mg)
    {
        if (mg.batches.empty())
            return NULL;
    ...
    const shared_ptr<VertexLayout>& getVertexLayout()
    {
        ...
        if (!p)
        {
            ...
            elements.push_back(
                VertexLayout::Element(0, offsetof(GeometryGenerator::Vertex, pos), VertexLayout::Format_Float3, VertexLayout::Semantic_Position));
            elements.push_back(
                VertexLayout::Element(0, offsetof(GeometryGenerator::Vertex, normal), VertexLayout::Format_Float3, VertexLayout::Semantic_Normal));

            elements.push_back(
                VertexLayout::Element(0, offsetof(GeometryGenerator::Vertex, color), VertexLayout::Format_Color, VertexLayout::Semantic_Color, 0));
            ...
            if (useShaders)
            {
                elements.push_back(VertexLayout::Element(
                    0, offsetof(GeometryGenerator::Vertex, extra), VertexLayout::Format_UByte4, VertexLayout::Semantic_Color, 1));
            }
            else
            {
                elements.push_back(VertexLayout::Element(
                    0, offsetof(GeometryGenerator::Vertex, extra), VertexLayout::Format_Color, VertexLayout::Semantic_Color, 1));
            }

            elements.push_back(
                VertexLayout::Element(0, offsetof(GeometryGenerator::Vertex, uv), VertexLayout::Format_Float2, VertexLayout::Semantic_Texture, 0));
            elements.push_back(VertexLayout::Element(
                0, offsetof(GeometryGenerator::Vertex, uvStuds), VertexLayout::Format_Float2, VertexLayout::Semantic_Texture, 1));

            elements.push_back(VertexLayout::Element(
                0, offsetof(GeometryGenerator::Vertex, tangent), VertexLayout::Format_Float3, VertexLayout::Semantic_Texture, 2));
            elements.push_back(VertexLayout::Element(
                0, offsetof(GeometryGenerator::Vertex, edgeDistances), VertexLayout::Format_Float4, VertexLayout::Semantic_Texture, 3));
    ...
    bool canUseBuffer(unsigned int currentCount, unsigned int desiredCount)
    {
        ...
        return desiredCount <= currentCount && currentCount - desiredCount < currentCount / 10;
    }
    ...
    void setupSharedGeometry(FastClusterSharedGeometry& sharedGeometry, unsigned int vertexCount, unsigned int indexCount, bool isFW)
    {
        ...
        {
            sharedGeometry.vertexBuffer =
                visualEngine->getDevice()->createVertexBuffer(sizeof(GeometryGenerator::Vertex), vertexCount, GeometryBuffer::Usage_Static);
        }

        // Create index buffer
        if (!sharedGeometry.indexBuffer || !canUseBuffer(sharedGeometry.indexBuffer->getElementCount(), indexCount))
        {
            sharedGeometry.indexBuffer =
                visualEngine->getDevice()->createIndexBuffer(sizeof(unsigned short), indexCount, GeometryBuffer::Usage_Static);
        }
    }
    ...
    Extents generateBatchGeometry(const MaterialGroup& mg, const Batch& batch, GeometryGenerator::Vertex* vbptr, unsigned short* ibptr,
        unsigned int vertexOffset, std::vector<unsigned int>& instanceVertexCount, bool isFW)
    {
        AYAPROFILER_SCOPE("Render", "generateBatchGeometry");
        ...
        for (size_t i = 0; i < batch.instances.size(); ++i)
        {
            ...
            GeometryGenerator::Options options(visualEngine, *bi.part, bi.decal, getRelativeTransform(bi.part, bone.root), mg.materialResultFlags,
                bi.uvOffsetScale, bi.localBoneIndex);
            ...
            {
                Aya::AABox partBounds = generator.getBounds();
        ...
        AYAASSERT(generator.getVertexCount() == batch.counter.getVertexCount() + vertexOffset &&
                  generator.getIndexCount() == batch.counter.getIndexCount());
    ...
    struct BatchMaterialPlasticLODComparator
    {
        bool operator()(const std::pair<MaterialGroup*, Batch*>& lhs, const std::pair<MaterialGroup*, Batch*>& rhs) const
        {
            ...
            return (ml->materialResultFlags & MaterialGenerator::Result_PlasticLOD) >
                   (mr->materialResultFlags & MaterialGenerator::Result_PlasticLOD);
        }
    };
};
...
{
    template<typename T>
    bool operator()(const T& part) const
    {
...
FastClusterEntity::FastClusterEntity(FastCluster* cluster, const GeometryBatch& geometry, const shared_ptr<Material>& material,
    const shared_ptr<Material>& decalMaterialOpaque, RenderQueue::Id renderQueueId, unsigned char lodMask, const std::vector<unsigned int>& bones,
    const Extents& localBounds, unsigned int extraFeatures)
...
{
    if (useCache(cacheKey, this))
        return 0;

    AYAASSERT(bones.size() <= maxTransforms);
    ...
    , cluster(cluster)
{
    ...
    AYAASSERT(part->getGfxPart() == NULL);
...
void FastClusterBinding::onCoordinateFrameChanged()
{
    ...
    {
        FASTLOG3(
            FLog::RenderFastCluster, "FastCluster[%p]: part %p with binding %p requests coordinate frame change", cluster, partInstance.get(), this);
...
{
    FASTLOG4(FLog::RenderFastCluster, "FastCluster[%p]: unbind part %p from binding %p (%d connections)", cluster, partInstance.get(), this,
        connections.size());

    AYAASSERT(partInstance->getGfxPart() == cluster || partInstance->getGfxPart() == NULL);
    GfxBinding::unbind();
}
    ...
    , dirty(false)
{
    ...
    else
    {
        ...
        FASTLOG5(FLog::RenderFastCluster, "FastCluster[%p]: create (grid %dx%dx%d, flags %d)", this, spatialIndex.position.x, spatialIndex.position.y,
            spatialIndex.position.z, spatialIndex.flags);
    }

    SceneUpdater* su = visualEngine->getSceneUpdater();

    // Each FastCluster should listen to Lighting changes for stencil shadows
    // VirtualVersion changes aswell
    // @mdolli

    // rj: disabled, seems like engine automatically culls anyway
    // Lighting* lighting = visualEngine->getLighting();
    // lightingChangedConnection = lighting->lightingChangedSignal.connect(boost::bind(&FastCluster::invalidateEntity, this));
...
void FastCluster::checkCluster()
{
    ...
    FASTLOG5(FLog::RenderFastCluster, "FastCluster[%p]: checking cluster (spatial index %dx%dx%d-%u)", this, spIndex.position.x, spIndex.position.y,
        spIndex.position.z, spIndex.flags);
    ...
    for (size_t i = 0; i < parts.size(); ++i)
    {
        ...
        else
        {
            ...
            if (spIndex.flags != newspIndex.flags || !(spIndex.position - newspIndex.position).isBetweenInclusive(m, M))
            {
                ...
                {
                    AYAASSERT(selfInvalidate == true); // Should never fail on first added cluster
                    queueClusterCheck();
                    break;
                }

                if (spIndex.flags != newspIndex.flags)
                    FASTLOG4(FLog::RenderFastCluster, "FastCluster[%p]: part %p changed FW status from %d to %d", this, part.instance, spIndex.flags,
                        newspIndex.flags);
                else
                    FASTLOG5(FLog::RenderFastCluster, "FastCluster[%p]: part %p changed spatial index to %dx%dx%d", this, part.instance,
                        spIndex.position.x, spIndex.position.y, spIndex.position.z);
...
void FastCluster::checkBindings()
{
    for (size_t i = 0; i < parts.size(); ++i)
    {
        ...
        else
        {
            ...
            {
                FASTLOG4(
                    FLog::RenderFastCluster, "FastCluster[%p]: part %p changed humanoid from %p to %p", this, part.instance, newHumanoid, humanoid);
...
void FastCluster::updateEntity(bool assetsUpdated)
{
    ...
    FASTLOG4(
        FLog::RenderFastCluster, "FastCluster[%p]: updateEntity for %d parts (dirty: %d, light dirty: %d)", this, parts.size(), dirty, lightDirty);
    ...
    FASTLOG5(FLog::RenderFastCluster, "FastCluster[%p]: updated geometry for %d parts in %d usec (%d entities, %d vertices)", this, parts.size(),
        (int)(timer.delta().msec() * 1000), entities.size(), totalVertexCount);
}
...
void FastCluster::updateClumpGrouping()
{
    ...
    FASTLOG4(FLog::RenderFastCluster, "FastCluster[%p]: %d parts, %d single-clump parts, %d multi-clump parts", this, parts.size(),
        middle - parts.begin(), parts.end() - middle);
}

unsigned int FastCluster::updateGeometry(AsyncResult* asyncResult)
{
    AYAPROFILER_SCOPE("Render", "updateGeometry");
    ...
    for (size_t parti = 0; parti < parts.size(); ++parti)
    {
        ...
        if (part.instance->getTransparencyUi() < 1)
            generator.addInstance(
                boneIndex, part.instance, NULL, materialFlags, partTransparent ? RenderQueue::Id_Transparent : RenderQueue::Id_Opaque, asyncResult);
        ...
        if ((part.instance->getCookie() & PartCookie::HAS_DECALS) && part.instance->getChildren() && !ignoreDecals)
        {
            ...
            for (size_t i = 0; i < children.size(); ++i)
            {
                if (Decal* decal = children[i]->fastDynamicCast<Decal>())
                {
                    ...
                    {
                        RenderQueue::Id renderQueue = partTransparent         ? RenderQueue::Id_Transparent
                                                      : decalTransparency > 0 ? RenderQueue::Id_TransparentDecals
                                                                              : RenderQueue::Id_Decals;
